...
public class StorageMovementComparator implements Comparator<StorageMovement> {
	...
	public StorageMovementComparator(boolean _b) {
		this.sortAdrl = _b;
	}
	
	@Override
	public int compare(StorageMovement s1, StorageMovement s2) {
		if (sortAdrl) {
	 ...
	 */
	private Integer sortLocation(String l1, String l2) {
		...
		if (l1.matches("^[0-9]{1,2}$")) { // boite transfert
			...
			String[] splitted1 = l1.split("\\.");
			String[] splitted2 = l2.split("\\.");
			// IRELEC check
