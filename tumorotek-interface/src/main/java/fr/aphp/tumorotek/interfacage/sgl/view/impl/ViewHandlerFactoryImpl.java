...
public class ViewHandlerFactoryImpl implements ViewHandlerFactory
{
   ...
   public void setViewHandler(final ViewHandler dv){
      this.viewHandler = dv;
   }

   public void setDemLigResultProcessor(final DemLigResultProcessor _d){
      this.demLigResultProcessor = _d;
   }

   @Override
   public DossierExterne sendQuery(final Emetteur _e, final String sglNumDos, final Banque bank){

      if(_e.getLogiciel().getNom().equals("DAVINCI")){
         return viewHandler.sendQuery(_e, sglNumDos, "davinci.properties", new DaVinciResutProcessor(), bank);
      }else if(_e.getLogiciel().getNom().equals("DEMLIG")){
         return viewHandler.sendQuery(_e, sglNumDos, "demlig.properties", demLigResultProcessor, bank);
      }
      return null;
