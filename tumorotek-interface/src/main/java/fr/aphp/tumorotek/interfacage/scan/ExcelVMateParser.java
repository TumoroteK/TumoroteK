...
public class ExcelVMateParser {
    ...
    public void setScanTerminaleManager(ScanTerminaleManager _s) {
		this.scanTerminaleManager = _s;
	}

	public void handleVMateScan(@Body InputStream body) {
    	scanTerminaleManager.createObjectManager(processExcelVMate(body), null);
    }

    public ScanTerminale processExcelVMate(InputStream body) {

    	ScanTerminale sT = new ScanTerminale();
    	Map<String, Integer> colsPosition = new HashMap<String, Integer>();
        ...
        try {
            ...
            for(Iterator<Row> rit = sheet.rowIterator(); rit.hasNext();) {
                ...
                if(!headersFound) {  // // headers
                    headersFound = true;
                    colNum = 0;
                    for(Iterator<Cell> cit = row.cellIterator(); cit.hasNext(); ++colNum) {
                	...
                	if (dateScan == null) {
                		...
                		cell.setCellType(Cell.CELL_TYPE_STRING);
                		dateScan = df.parse(parsedDate 
                			+ " " + cell.getStringCellValue());
                	}
                	
                	// scanTube
                	ScanTube tube = new ScanTube();
