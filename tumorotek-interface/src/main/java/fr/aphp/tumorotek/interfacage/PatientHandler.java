...
public class PatientHandler {
	...
	private PipeParser pipe = new PipeParser();
	
	public PatientHandler() {
	}

	public void setPatientSipManager(PatientSipManager pM) {
	...
	public void setUtilisateurManager(UtilisateurManager uManager) {
	...
	public void setMaxSipTableSize(Integer m) {
	 ...
	 */
	public void handleCreation(SipMessage body) {
		patientSipManager
			.createOrUpdatePatientInTempTableManager(body.toPatientSip(), 
	 ...
	 * @throws HL7Exception 
	 */
	public void handleCreation(AbstractMessage body) 
										throws IOException, HL7Exception {		
		patientSipManager
			.createOrUpdatePatientInTempTableManager(patientSipManager
					.parseHl7MessagePID(pipe.encode(body)), getMaxSipTableSize(), false);
	}
	 ...
	 * @param body SipMessage
	 */
	public void handleModification(SipMessage body) {
		PatientSip sip = body.toPatientSip();
		doModification(sip);
	}
	 ...
	 * @throws HL7Exception 
	 */
	public void handleModification(AbstractMessage body) 
										throws IOException, HL7Exception {
				
		PatientSip sip = patientSipManager
					.parseHl7MessagePID(pipe.encode(body));
		doModification(sip);
	}
	 ...
	 * Si non -> creation.
	 */
	private void doModification(PatientSip pSip) {
		Patient inBase = patientSipManager.doSynchronizePatientManager(pSip);
		if (inBase != null) { // synchro
			patientSipManager.updatePatientSystem(pSip, inBase);
		} else {
			patientSipManager.createOrUpdatePatientInTempTableManager(pSip, 
														getMaxSipTableSize(), false);
		}
	}
	 ...
	 * @param body SipMessage
	 */
	public void handleFusion(SipMessage body) {
		PatientSip sipA = body.toPatientActif();
		PatientSip sipP = body.toPatientPassif();
	 ...
	 * @throws HL7Exception 
	 */
	public void handleFusion(AbstractMessage body) 
								throws IOException, HL7Exception {
		PatientSip sipA = patientSipManager
						.parseHl7MessagePID(pipe.encode(body));
		PatientSip sipP = patientSipManager
							.parseHl7MessageMRG(pipe.encode(body));
	 ...
	 */	
	public void doFusion(PatientSip sipA, PatientSip sipP) {
		if (sipA != null && sipP != null) {
			Patient inBaseA = patientSipManager
										.doSynchronizePatientManager(sipA);
			Patient inBaseP = patientSipManager
										.doSynchronizePatientManager(sipP);
			...
			if (inBaseP == null) { //passif dans temporaire ou inexistant
				...
				List<PatientSip> passif = patientSipManager
								.findByNipLikeManager(sipP.getNip(), true);
				if (!passif.isEmpty()) {
					for (PatientSipSejour sej : passif.get(0).getSejours()) {
						PatientSipSejour newSej = new PatientSipSejour();
				...
				if (inBaseA == null) { //actif dans temporaire ou inexistant
					...
					patientSipManager
						.createOrUpdatePatientInTempTableManager(sipA, 
								getMaxSipTableSize(), true);
				}
			} else { //passif dans TK
				if (inBaseA != null) { //actif dans TK
					// -> fusion TK car les deux patients sont dans TK
					patientSipManager
						.fusionPatientSystemManager(inBaseA, inBaseP);
					...
					if (!patientSipManager
						.findByNipLikeManager(sipA.getNip(), true).isEmpty()) {
						patientSipManager.removeObjectManager(patientSipManager
							.findByNipLikeManager(sipA.getNip(), true).get(0));
					}
				}
			}
		}
	 ...
	 */
	public void handleInclusionMelbase(StudySubjectType subject) 
