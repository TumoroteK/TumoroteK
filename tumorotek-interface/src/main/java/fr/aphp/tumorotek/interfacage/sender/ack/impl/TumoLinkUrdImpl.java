...
public class TumoLinkUrdImpl implements TumoLinkUrd {
	...
	private Log log = LogFactory.getLog(TumoLinkUrdImpl.class);
	...
	public void sendMessage(TKAnnotableObject prel, String dosExtId, String url) {
		...
		if (prel != null && dosExtId != null && url != null) {
			...
			try {
				camelTemplate.sendBodyAndHeader("direct:ack-sgl", 
						createUDMMessage((Prelevement) prel, dosExtId, url).encode(), 
							"CamelFileName", setFileName(prel, false, null, datef));
				
				// ok file
				camelTemplate.sendBodyAndHeader("direct:ack-sgl", 
						"", "CamelFileName", setFileName(prel, true, null, datef));
	...
	public UDM_Q05 createUDMMessage(Prelevement prel, String dosExtId, String url) 
										throws HL7Exception, IOException {
		
		UDM_Q05 udm = new UDM_Q05();
		udm.initQuickstart("UDM", "Q05", "P");
		
		//MSH
		MSH mshSegment = udm.getMSH();
		...
		mshSegment.getMsh10_MessageControlID()
			.setValue("TKUDIA000" + mshSegment.getMsh10_MessageControlID().getValue());
		...
		XCN msh3 = urdSegment.getRUWhoSubjectDefinition(0);
	...
	public String setFileName(TKAnnotableObject prel, boolean isOkFile, Integer part, 
			String currtime) {
		return ((Prelevement) prel).getPrelevementId() + "_" + currtime 
			+ (!isOkFile ? ".hl7" : ".ok");		
	}
	...
	public boolean useRecepteur(Recepteur r) {
		if (r != null && r.getLogiciel() != null
				&& r.getLogiciel().getNom().equals("DIAMIC")
