	...
	public void handleMessageSGL_SSU(final String body) throws HL7Exception, IOException {
		...
		if (sendingApp.equalsIgnoreCase("Genno")) {
			...
			final Hashtable<String, List<String>> contenu = 
					interfacageParsingUtils.parseFileToInjectInTk(config, body).get(0);

			prelNature = interfacageParsingUtils
					.getValueFromBlocAndEmplacement(contenu, config, "SPM", "5.2.1");
			
			log.debug("GENNO message SSU_U03 SPM-2 nature prélèvement: " + prelNature);
			
			spm2 = interfacageParsingUtils
					.getValueFromBlocAndEmplacement(contenu, config, "SPM", "2");

			log.debug("GENNO message SSU_U03 SPM-2 code prélèvement: " + spm2);
			
			spmObx10 = interfacageParsingUtils
					.getValueFromBlocAndEmplacement(contenu, config, "SPM_OBX10", "5");
