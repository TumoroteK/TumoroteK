...
public class SglHandler {
	...
	private Log log = LogFactory.getLog(SglHandler.class);
	...
	public void setResourceBundleTumo(ResourceBundleTumo rTumo) {
		this.resourceBundleTumo = rTumo;
	}
	
	public void setMyHl7(HL7DataFormat my) {
		this.myHl7 = my;
	}
	 ...
	 * @throws HL7Exception 
	 */
	public void handleMessageSGL(String body) throws HL7Exception {
		
		String sendingApp = null;
		
		Message msg = myHl7.getParser().parse(body);
		
		
		// les messages venant de APIX et DIAMIC sont ORU_R01 v2.5
		// sendingApp = ((ORU_R01) body.getMessage())
		sendingApp = ((ORU_R01) msg)
				.getMSH().getSendingApplication()
	 ...
	 */
	public void handleMessage(String body, String inbox) {
		...
		if (resourceBundleTumo.doesResourceBundleExists("tumorotek.properties")) {
			...
			if (res != null) {
				inboxesProp = resourceBundleTumo
					.getResourceBundle("tumorotek.properties")
					.getString("INTERFACAGES_INBOXES");
			}
		}
		...
		if (inboxesProp != null) {		
			...
			Emetteur emet;
			try {
				emet = interfacageParsingUtils
					.extractEmetteurFromFileToInjectInTk(
							inboxesProp, 
					body, inbox);
				log.debug(emet);
				String sglMap = interfacageParsingUtils
				.extractXMLFIleFromFileToInjectInTk(inboxesProp, 
						body, inbox);
// , emet);
				log.debug(sglMap);
				interfacageParsingUtils
					.parseInterfacageXmlFile(sglMap, body, emet, true, 
							getMaxDossierTableSize());
			...
			throw new RuntimeException("Interfacage SGL: "
						+ "tumorotek.properties not found!");
		}
	...
	public int getMaxDossierTableSize() {
		...
		try {
			...
			Integer maxEnvV = (Integer) ctx.lookup("java:comp/env/interfacage/maxSglTableSize");
			if (maxEnvV != null) {
