...
public class HprimMarshallerTest2 {
	...
	public void testUnmarshalVenue() throws JAXBException, ParseException {
		...
		File file = new File("/home/mathieu/Documents/tumorotek/interfacages/"
				+ "HPRIM_GEMA/venuePatient.xml");
		JAXBContext jaxbContext = JAXBContext.newInstance(HprimMessage.class);
 
		Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
		HprimMessage message = (HprimMessage) jaxbUnmarshaller.unmarshal(file);
		
		assertTrue(message.getEntete().getNumEvt().equals(23783L));
		assertTrue(message.getEntete().getDateMes()
				.equals(("2011-09-09T13:31:58")));
		...
		assertTrue(message.toPatientSip().getDateNaissance()
				.equals(new SimpleDateFormat("dd/MM/yyyy")
				.parse(("02/05/1975"))));
		assertTrue(message.toPatientSip().getSejours().size() == 1);
		assertTrue(message.toPatientSip().getSejours().iterator().next().getNumero()
				.equals("531604772"));
	...
	public void testUnmarshalFusion() throws JAXBException, ParseException {
		...
		assertTrue(message.toPatientPassif().getDateNaissance()
				.equals(new SimpleDateFormat("dd/MM/yyyy")
				.parse(("11/03/1960"))));
