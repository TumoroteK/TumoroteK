...
public class InclusionMarshallerTest2 {
	...
	public void testInclusionHandler() throws JAXBException, ParseException {
		...
		File file = new File("/home/mathieu/Documents/tumorotek/interfacages/"
				+ "inclusion/subjects.xml");
		JAXBContext jaxbContext = JAXBContext
						.newInstance(CreateRequest.class);
		...
		if (patientHandler != null) {
			...
			patientHandler
				.handleInclusionMelbase(message.getStudySubject().get(0));
			
			assertFalse(patientManager
							.findByNomLikeManager("10-10", true).isEmpty());
			Patient p = patientManager
							.findByNomLikeManager("10-10", true).get(0);
			
			assertNull(p.getNip());
			assertTrue(p.getPrenom().equals("M;B R"));
			assertTrue(p.getSexe().equals("M;B R"));
			assertTrue(p.getDateNaissance()
	...
	@Test
	@Rollback(true)
	public void testInclusionMarshaller() throws JAXBException, 
		...
		File file = new File("/home/mathieu/Bureau/file.xml");
		
		JAXBContext jaxbContext = JAXBContext
								.newInstance(CreateRequest.class);
		
		Marshaller jaxbMarshallerT = jaxbContext.createMarshaller();
		
		jaxbMarshallerT.setEventHandler(
				new ValidationEventHandler() {
					public boolean handleEvent(ValidationEvent event) {
						throw new RuntimeException(event.getMessage(),
                                   event.getLinkedException());
	...
	@Test
	@Rollback(true)
	public void testAddSubject() throws ParseException, 
	...
	@Test
	@Rollback(true)
	public void testAddSubjectTK() throws ParseException, 
								DatatypeConfigurationException {
		assertTrue(inclusionServiceClient.addSubjectTK(setSubject()));
		
	}
	
	private StudySubjectType setSubject() throws ParseException, 
										DatatypeConfigurationException {
		StudySubjectType st = new StudySubjectType();
		...
		st.setEnrollmentDate(DatatypeFactory
				.newInstance().newXMLGregorianCalendar(g));
		
		StudyRefType srt = new StudyRefType();
		srt.setIdentifier("MELBASE");
		SiteRefType sr = new SiteRefType();
		sr.setIdentifier("26");
		srt.setSiteRef(sr);
		st.setStudyRef(srt);
