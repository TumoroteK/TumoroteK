...
public class ExcelVMateParserTest
{
   ...
   @Test
   public void testParse() throws JAXBException, ParseException, IOException{

      // HSSFWorkbook xls test
      final File file = new File("src/test/java/fr/aphp/tumorotek/interfacage/scan/test5.xls");

      FileInputStream fis = new FileInputStream(file);
      final ScanTerminale sT = excelVMateParser.processExcelVMate(fis);

      assertTrue(sT.getName().equals("test5"));
      assertTrue(sT.getDateScan().getTime().equals(new SimpleDateFormat("YYYYMMDD HH:mm:ss").parse("20160401 11:59:28")));
      ...
      assertTrue(new ArrayList<>(sT.getScanTubes()).get(0).getCode() == null);
      
      fis.close();
   }

   @Test
   public void testPersist() throws ParseException, IOException{
      final File file = new File("src/test/java/fr/aphp/tumorotek/interfacage/scan/test5.xls");

      final int scans = scanTerminaleManager.findAllManager().size();

      FileInputStream fis = new FileInputStream(file);
      
      excelVMateParser.handleVMateScan(fis);

      assertTrue(scanTerminaleManager.findAllManager().size() == scans + 1);

      final ScanTerminale sT = scanTerminaleManager.findAllManager().get(scans); // last scan
      assertTrue(sT.getName().equals("test5"));
      assertTrue(sT.getDateScan().getTime().equals(new SimpleDateFormat("YYYYMMDD HH:mm:ss").parse("20160401 11:59:28")));
      assertTrue(sT.getWidth() == 12);
      assertTrue(sT.getHeight() == 8);
   ...
   public void testRoute() throws Exception{
      ...
      final int tots = scanTerminaleManager.findAllManager().size();

      final MockEndpoint mockBeans = (MockEndpoint) camelContext.getEndpoint("mock:bean:excelVMateParser");
      final MockEndpoint mockDeads = (MockEndpoint) camelContext.getEndpoint("mock:deads");
      ...
      final ScanDevice sD = scanDeviceDao.findById(1);

      final List<ScanTerminale> scans = scanTerminaleManager.findByDeviceManager(sD);
      ...
      // clean up
      for(final ScanTerminale scanTerminale : scans){
