#### Mise à jour de TumoroteK version 2.3.0.11-SNAPSHOT
...
Renommer la copie en :
        ...
        tumorotek##2.3.0.11-SNAPSHOT.xml
        
3. Modifier dans ce même fichier `tumorotek##2.3.0.11-SNAPSHOT.xml`, les variables suivantes :
...
4. Télécharger le [Dossier d'installation](https://github.com/TumoroteK/TumoroteK/releases/download/v2.3.0.11-SNAPSHOT/tumorotek-install-2.3.0.11-SNAPSHOT.zip)

5. Dézipper le dossier

6. Copier/déplacer l'archive `tumorotek##2.3.0.11-SNAPSHOT.war` sur le serveur Tomcat dans le dossier `<PATH_TOMCAT>/webapps`
    ...
    A cette étape de l'installation, le contenu du fichier `tumorotek##2.3.0.11-SNAPSHOT.xml` est automatiquement reporté dans `tumorotek.properties` et le fichier `tumorotek##2.3.0.11-SNAPSHOT.xml` est supprimé.
