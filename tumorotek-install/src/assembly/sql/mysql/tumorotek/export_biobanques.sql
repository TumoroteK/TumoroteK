...
CREATE PROCEDURE `select_biobanques_data`(IN entite INT, IN eds BOOLEAN)
  BEGIN
    ...
    SELECT trim(ad.CONTACT)                                                                        as 'id_depositor',
           ''                                                                                      as 'id_familiy',
...
CREATE PROCEDURE `fill_tmp_biobanques_adds`()
  BEGIN
    INSERT INTO TMP_BIOBANQUES_ADDS (ECHANTILLON_ID, CONTACT, CONTENEUR_TEMP, QUANTITE, CODE_ORGANE, CODE_LES, PF, BANQUE_IDENT)
    SELECT e.ECHANTILLON_ID,
           ...
           (SELECT IF(code like '%:%', LEFT(code, 2), code)
            FROM CODE_ASSIGNE c
            WHERE c.echantillon_id = e.ECHANTILLON_ID
              AND c.is_organe = 1
              AND c.export = 1),
           (SELECT code FROM CODE_ASSIGNE c WHERE c.echantillon_id = e.ECHANTILLON_ID
                                              AND c.is_morpho = 1
                                              AND c.export = 1),
           p.NOM,
           b.IDENTIFICATION
