...
CREATE FUNCTION countStockableObjects(entiteId INT(1), plateforme VARCHAR(250), banque VARCHAR(250), date_debut DATE, date_fin DATE, stocke BOOLEAN,
                                      etype    BOOLEAN, cim BOOLEAN, adicap BOOLEAN)
  ...
  BEGIN
    ...
    IF stocke IS NOT NULL AND stocke = 1
    THEN
    ...
    IF entiteId = 3 OR entiteId IS NULL
    THEN
      SET entiteId = 3;
    ...
    ELSEIF etype IS NOT NULL and etype = 1
      THEN
        IF entiteId = 3
        THEN
        ...
        SET whereTable2 = CONCAT(
            ' join PLATEFORME p on p.PLATEFORME_ID = et.PLATEFORME_ID join BANQUE b on p.PLATEFORME_ID = b.PLATEFORME_ID where p.NOM = ''',
            plateforme, ''' and b.NOM = ''', banque, '''');
    ELSEIF cim IS NOT NULL and cim = 1
      THEN
        SET colAlias = 'm.CODE';
        SET joinCol = 'm.CODE';
        ...
        SET whereTable = CONCAT(' and p.NOM = ''', plateforme, ''' and b.NOM = ''', banque, '''');
        SET whereTable2 = CONCAT(
            ' join PRELEVEMENT r on r.MALADIE_ID = m.MALADIE_ID join BANQUE b on r.BANQUE_ID = b.BANQUE_ID join PLATEFORME p on p.PLATEFORME_ID = b.PLATEFORME_ID where p.NOM = ''',
            plateforme, ''' and b.NOM = ''', banque, '''');
    ELSEIF adicap IS NOT NULL and adicap = 1
      THEN
        SET colAlias = 'c.CODE';
        SET joinCol = 'c.CODE';
        ...
        SET whereTable2 = CONCAT(
            ' join ECHANTILLON e on c.ECHANTILLON_ID = e.ECHANTILLON_ID join BANQUE b on e.BANQUE_ID = b.BANQUE_ID join PLATEFORME p on p.PLATEFORME_ID = b.PLATEFORME_ID where p.NOM = ''',
            plateforme, ''' and b.NOM = ''', banque, ''' and c.IS_ORGANE = 1');
    END IF;
    ...
    -- JOINS
    SET req = CONCAT(req, ' from ', targetEntity,
                     ' e join BANQUE b on b.banque_id = e.banque_id join PLATEFORME p on p.plateforme_id = b.plateforme_id ');
