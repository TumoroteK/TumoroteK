  ...
  BEGIN
    ...
    SELECT id,
           ...
           (SELECT count(*) FROM PRELEVEMENT pr
                                   INNER JOIN MALADIE m WHERE pr.maladie_id = m.maladie_id
                                                          AND m.patient_id = id),
           (SELECT ut.login
            FROM UTILISATEUR ut
    ...
    SELECT p.prelevement_id,
           ...
           (select GROUP_CONCAT(nc.nom)
            ...
            WHERE ct.conformite_type_id = 1
              AND p.prelevement_id = onc.objet_id)                                                       as 'Raison_de_non_conformité',
...
CREATE PROCEDURE `fill_tmp_table_echan`(IN id INTEGER)
  BEGIN
    ...
    SELECT e.echantillon_id                                                                                   as 'objet_id',
           ...
           tumoral,
           lateralite,
           (SELECT GROUP_CONCAT(ca.code ORDER BY ca.ordre) FROM CODE_ASSIGNE ca WHERE ca.IS_ORGANE = 1
                                                                                  AND ca.echantillon_id = id) as 'code_organe',
           (SELECT GROUP_CONCAT(ca.code ORDER BY ca.ordre) FROM CODE_ASSIGNE ca WHERE ca.IS_MORPHO = 1
                                                                                  AND ca.echantillon_id = id) as 'codes_lésionnels',
           (SELECT COUNT(tr.objet_id)
            FROM TRANSFORMATION tr
                   INNER JOIN PROD_DERIVE pd ON tr.TRANSFORMATION_ID = pd.TRANSFORMATION_ID
            WHERE tr.OBJET_ID = id
    ...
    SELECT prod_derive_id                                             as 'id_derive',
           ...
           (select GROUP_CONCAT(nc.nom)
            ...
            WHERE ct.conformite_type_id = 4
              AND pd.prod_derive_id = onc.objet_id)                   as 'Raison_de_non_conformité_après_traitement',
           ...
           (select GROUP_CONCAT(nc.nom)
            ...
            WHERE ct.conformite_type_id = 5
              AND pd.prod_derive_id = onc.objet_id)                   as 'Raison_de_non_conformité_pour_la_cession',
           (SELECT COUNT(tr.objet_id)
            FROM TRANSFORMATION tr
                   INNER JOIN PROD_DERIVE pd ON tr.TRANSFORMATION_ID = pd.TRANSFORMATION_ID
            WHERE tr.OBJET_ID = id
    ...
    SELECT c.cession_id,
           ...
           IF((select count(objet_id) < 20 FROM CEDER_OBJET WHERE CESSION_ID = id
              ...
              (SELECT GROUP_CONCAT(e.code)
               ...
               WHERE c.CESSION_ID = id
                 AND co.ENTITE_ID = 3),
              CONCAT((SELECT GROUP_CONCAT(code)
                      FROM ECHANTILLON
                             JOIN (SELECT OBJET_ID FROM CEDER_OBJET WHERE CESSION_ID = id
                                                                      AND ENTITE_ID = 3 LIMIT 0, 20) z on z.OBJET_ID = ECHANTILLON_ID), '...')
               ),
           (SELECT count(e.code)
            FROM ECHANTILLON e
                   LEFT JOIN CEDER_OBJET co ON e.echantillon_id = co.objet_id
           ...
           IF((select count(objet_id) < 20 FROM CEDER_OBJET WHERE CESSION_ID = id
              ...
              CONCAT((SELECT GROUP_CONCAT(code)
                      FROM PROD_DERIVE
                             JOIN (select OBJET_ID FROM CEDER_OBJET WHERE CESSION_ID = id
                                                                      AND ENTITE_ID = 8 LIMIT 0, 20) z on z.OBJET_ID = PROD_DERIVE_ID), '...')
               ),
