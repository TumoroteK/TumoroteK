...
CREATE PROCEDURE `select_inca_data`()
  BEGIN
    SELECT trim(ad.FINESS)                                                                                       as '01 : Identifiant site',
           ...
           date_format(
             if(left(tpae.PATIENT_ETAT, 1) = 'D', tpae.DATE_DECES, tpae.DATE_ETAT), '%Y%m%d')                    as '06 : Date de l''etat',
           ...
           if(tpe.TUMORAL,
              left(get_annotation_inca_item(tpe.ECHANTILLON_ID, '032/044%Controle%tissu%', 3, 1), 1), '')        as '32 : Controle sur tissu tumoral',
           ...
           if(tpe.TUMORAL is null OR tpe.TUMORAL = 0, get_prod_type_assoc(tpe.ECHANTILLON_ID, 'PROTEINE'),
              '')                                                                                                as '49 : PROTEINE derive non tumoral',
...
CREATE PROCEDURE `fill_tmp_table_inca_adds`(IN _id INTEGER)
  BEGIN
    INSERT INTO TMP_INCA_ADDS (ECHANTILLON_ID, FINESS, CENTRE_STOCKAGE, CLASSIF, CONTACT, CONTENEUR_TEMP, MODE_PREPARATION, QUANTITE, QUANTITE_UNITE)
    SELECT _id,
           (SELECT et.nom
           ...
           get_classif(_id),
           (SELECT CONCAT(c.nom, ' ', IFNULL(c.prenom, ''))
...
READS SQL DATA
  ...
  BEGIN
    ...
    SET cd = (SELECT c.code
              ...
              WHERE e.echantillon_id = id_obj
                AND c.is_organe = isorg
                AND c.table_codage_id = tableId
              limit 0,1);

    -- recupere le code exporte
    IF cd IS NULL
    THEN
      SET cd = (SELECT c.code FROM CODE_ASSIGNE c WHERE c.echantillon_id = id_obj
                                                    AND c.is_organe = isorg
                                                    AND c.export = 1);
      ...
      THEN
        ...
        ELSE
          ...
          IF isorg = 1
          THEN
            SET transcode = (SELECT a.code
                             FROM tumorotek_codes.CIM_MASTER m
                                    JOIN tumorotek_codes.ADICAPCIM_TOPO t ON t.sid = m.sid
                                    JOIN tumorotek_codes.ADICAP a ON a.adicap_id = t.adicap_id
                             WHERE m.code = cd
                             limit 0,1);
          ELSE
            SET transcode = (SELECT a.code
                             FROM tumorotek_codes.CIMO_MORPHO m
