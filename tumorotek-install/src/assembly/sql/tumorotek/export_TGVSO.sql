...
CREATE PROCEDURE `select_tvgso_data`(IN hemato BOOLEAN)
  BEGIN
    ...
    SELECT trim(ad.FINESS)                                                                                                                       as '01 : Identifiant site',
           ...
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O', ad.CONTENEUR_TEMP,
              '')                                                                                                                                as '19a : Mode de conservation tumoral',
-- if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O', trim(left(tpe.ECHANTILLON_TYPE, locate(':', tpe.ECHANTILLON_TYPE) - 1)), '') as '19b : Type echantillon tumoral',
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O', trim(tpe.ECHANTILLON_TYPE),
              '')                                                                                                                                as '19b : Type echantillon tumoral',
           ...
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O',
              ...
              '')                                                                                                                                as '20 : Delai congelation tumoral',
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O',
              left(get_annotation_item(tpe.ECHANTILLON_ID, '032%Controle%tissu%', 3), 1),
              '')                                                                                                                                as '21 : Controle sur tissu tumoral',
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O', ad.quantite,
              '')                                                                                                                                as '22a : Quantite echantillon tumoral',
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O', ad.quantite_unite,
              '')                                                                                                                                as '22b : Unite quantite echantillon tumoral',
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'O',
           ...
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'N', ad.CONTENEUR_TEMP,
              '')                                                                                                                                as '28a : Mode de conservation non tumoral',
-- if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'N', trim(left(tpe.ECHANTILLON_TYPE, locate(':', tpe.ECHANTILLON_TYPE) - 1)), '') as '28b : Type echantillon non tumoral',
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'N', trim(tpe.ECHANTILLON_TYPE),
              '')                                                                                                                                as '28b : Type echantillon non tumoral',
           ...
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'N', ad.quantite,
              '')                                                                                                                                as '31a : Quantite echantillon non tumoral',
           if(is_tumoral(hemato, tpe.ECHANTILLON_TYPE, tpe.TUMORAL, ad.CODE_LES) = 'N', ad.quantite_unite,
              '')                                                                                                                                as '31b : Unite quantite echantillon non tumoral',
...
CREATE PROCEDURE `fill_tmp_table_adds`(IN _id INTEGER, IN hemato BOOLEAN)
  BEGIN
    INSERT INTO TMP_TVGSO_ADDS (ECHANTILLON_ID,
                                FINESS,
                                CENTRE_STOCKAGE,
                                CONTACT,
                                CODE_ORGANE,
    ...
    SELECT _id,
           (SELECT et.nom
            ...
            WHERE tee.echantillon_id = _id),
           get_centre_stockage(_id),
           (SELECT TRIM(CONCAT(c.nom, ' ', IFNULL(c.prenom, '')))
           ...
           (SELECT IF(code like '%:%', LEFT(code, 2), code) FROM CODE_ASSIGNE c WHERE c.echantillon_id = _id
                                                                                  AND c.is_organe = 1
                                                                                  AND c.export = 1),
           (SELECT code FROM CODE_ASSIGNE c WHERE c.echantillon_id = _id
                                              AND c.is_morpho = 1
                                              AND c.export = 1),
           get_conteneur_temp(_id, hemato),
           ifnull((select p.nom from MODE_PREPA p
                                       JOIN ECHANTILLON e on e.mode_prepa_id = p.mode_prepa_id WHERE e.echantillon_id = _id),
           ...
           ifnull((SELECT unite from UNITE u
                                       join ECHANTILLON e on e.quantite_unite_id = u.unite_id WHERE e.echantillon_id = _id), '');
  END&&
...
CREATE FUNCTION `get_annotation_item`(id_obj INT(10), champ_nom VARCHAR(250), id_entite INT(10))
  ...
  BEGIN
    ...
    SET annoVal = (SELECT GROUP_CONCAT(i.label SEPARATOR ',')
                   FROM ITEM i,
                        ANNOTATION_VALEUR a,
                        CHAMP_ANNOTATION c,
                        TABLE_ANNOTATION t
    ...
    IF annoVal IS NOT NULL
    THEN
      IF annoVal like 'X%INCONNU'
      THEN
        RETURN 'X';
      END IF;
      RETURN annoVal;
    ELSEIF champ_nom like '022%TYPE%'
      THEN
        RETURN '9 : INCONNU';
    END IF;
...
CREATE FUNCTION `get_annotation_date`(id_obj INT(10), champ_nom VARCHAR(250), id_entite INT(10))
  ...
  BEGIN
    ...
    SET annoVal = (SELECT a.anno_date
                   FROM ANNOTATION_VALEUR a,
                        CHAMP_ANNOTATION c,
                        TABLE_ANNOTATION t
                   WHERE a.objet_id = id_obj
                     AND c.nom like champ_nom
...
CREATE FUNCTION `get_centre_stockage`(id_echan INT(10))
  ...
  BEGIN
    ...
    SET centre = (SELECT distinct et.nom
                  FROM ETABLISSEMENT et,
                       SERVICE s,
                       ECHANTILLON e,
                       SERVICE_COLLABORATEUR sc
    ...
    THEN
      IF centre like '%BORDEAUX%'
      THEN
        IF centre like '%CHU%'
        THEN
          RETURN '1TGSOCHUBDX';
        ELSEIF centre like '%CLC%'
          THEN
            RETURN '2TGSOCLCBDX';
        END IF;
      ELSEIF centre like '%LIMOGES%'
        THEN
          RETURN '3TGSOCHULIM';
      ELSEIF centre like '%MONTPELLIER%'
        THEN
          IF centre like '%CHU%'
          THEN
            RETURN '4TGSOCHUMPL';
          ELSEIF centre like '%CLC%'
            THEN
              RETURN '5TGSOCLCMPL';
          END IF;
      ELSEIF centre like '%NIMES%'
        THEN
          RETURN '6TGSOCHUNIM';
      ELSEIF centre like '%TOULOUSE%'
        THEN
          IF centre like '%CHU%'
          THEN
            RETURN '7TGSOCHUTLS';
...
CREATE FUNCTION `get_conteneur_temp`(id_obj INT(10), hemato BOOLEAN)
  ...
  BEGIN
    ...
    ELSE
      SET emp_id = (SELECT p.emplacement_id
                    from PROD_DERIVE p
                           join TRANSFORMATION t on t.transformation_id = p.transformation_id
                    WHERE t.entite_id = 3
                      and t.objet_id = id_obj
                      and p.emplacement_id is not null
                    order by p.prod_derive_id
                    LIMIT 1);
    END IF;

    IF emp_id IS NOT NULL
    THEN
      SET temperature = (SELECT temp FROM CONTENEUR WHERE conteneur_id = get_conteneur(emp_id));
    ELSE
      RETURN '';
    END IF;


    IF temperature = -196.0
    THEN
      RETURN '4';
    ELSEIF temperature = -140.0
      THEN
        RETURN '3';
    ELSEIF temperature = -80.0
      THEN
        RETURN '2';
    ELSEIF temperature = -20.0
      THEN
        RETURN '1';
    END IF;
...
CREATE FUNCTION `is_tumoral`(hemato BOOLEAN, etype VARCHAR(250), tumoral BOOLEAN, code_les VARCHAR(200))
  ...
  BEGIN
    ...
    THEN
      ...
      END IF;
    ELSE
      IF code_les IS NULL OR code_les != '0N00'
      THEN
        RETURN 'O';
      END IF;
    END IF;
