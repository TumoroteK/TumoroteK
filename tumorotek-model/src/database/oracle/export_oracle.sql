...
BEGIN
    INSERT INTO TMP_MALADIE_EXPORT ( 
       -- MALADIE_ID,
		...
		--  MEDECIN_MALADIE,
        PATIENT_ID)
select stragg(libelle), stragg(code), 
	stragg(date_diagnostic), stragg(date_debut), patient_id 
...
BEGIN
	INSERT
		  ...
		  SELECT p.prelevement_id,
		    ...
		    pt.type,
		    p.sterile,
		    (select stragg(r.nom) from RISQUE r JOIN PRELEVEMENT_RISQUE pr ON r.risque_id = pr.risque_id WHERE pr.prelevement_id = p.prelevement_id) AS risque_infectieux,
		    p.conforme_arrivee,
		    (select stragg(nc.nom) FROM OBJET_NON_CONFORME onc LEFT JOIN NON_CONFORMITE nc ON onc.non_conformite_id = nc.non_conformite_id WHERE p.prelevement_id = onc.objet_id AND onc.entite_id = 2) AS Raison_de_non_conformite,
		    ...
		    p.patient_nda AS Num_Dossier_Patient,
		   	(SELECT stragg(code) from (select distinct ca.code, ca.ordre FROM CODE_ASSIGNE ca INNER JOIN ECHANTILLON e 
		    	ON e.echantillon_id = ca.echantillon_id WHERE ca.IS_ORGANE=1 AND e.prelevement_id = prel_id ORDER BY ca.ordre)),
		   	(SELECT stragg(code) from (select distinct ca.code, ca.ordre FROM CODE_ASSIGNE ca INNER JOIN ECHANTILLON e 
		   		ON e.echantillon_id = ca.echantillon_id WHERE ca.IS_MORPHO=1 AND e.prelevement_id = prel_id ORDER BY ca.ordre)),
        ...
        (select stragg(c.nom) FROM MALADIE_MEDECIN mm JOIN COLLABORATEUR c on mm.collaborateur_id = c.collaborateur_id WHERE mm.maladie_id = p.maladie_id),
		    pat.patient_id
...
BEGIN
  ...
  SELECT e.echantillon_id,
    ...
    (SELECT stragg(nc.nom) FROM OBJET_NON_CONFORME onc
    ...
    )  AS Raison_non_conform_apr_traitem,
    conforme_cession AS Conforme_Cession,
    (SELECT stragg(nc.nom) FROM OBJET_NON_CONFORME onc
    	LEFT JOIN NON_CONFORMITE nc ON onc.non_conformite_id = nc.non_conformite_id 
    	LEFT JOIN CONFORMITE_TYPE ct ON nc.conformite_type_id = ct.conformite_type_id 
   		WHERE ct.conformite_type_id = 3 AND onc.objet_id = id AND onc.entite_id = 3
    ) AS Raison_non_conformite_cession,
    tumoral,
    lateralite,
 	(select stragg(code) from (select code from CODE_ASSIGNE where echantillon_id = id AND IS_ORGANE = 1 order by code)) AS code_organe,
    (select stragg(code) from (select code from CODE_ASSIGNE where echantillon_id = id AND IS_MORPHO = 1 order by code)) AS codes_lesionnels,
    (SELECT COUNT(tr.objet_id)
    FROM TRANSFORMATION tr
    INNER JOIN PROD_DERIVE pd
    ON tr.TRANSFORMATION_ID = pd.TRANSFORMATION_ID
  ...
  SELECT prod_derive_id AS id_derive,
   ...
   (SELECT u2.unite FROM TRANSFORMATION t
    ...
    (SELECT stragg(nc.nom) FROM OBJET_NON_CONFORME onc
    	LEFT JOIN NON_CONFORMITE nc ON onc.non_conformite_id = nc.non_conformite_id
    	LEFT JOIN CONFORMITE_TYPE ct ON nc.conformite_type_id  = ct.conformite_type_id
    		WHERE ct.conformite_type_id = 2 AND pd.prod_derive_id  = onc.objet_id) AS Raison_non_conform_apr_trait,
    pd.conforme_cession AS Conforme_Cession,
    (SELECT stragg(nc.nom) FROM OBJET_NON_CONFORME onc
   		LEFT JOIN NON_CONFORMITE nc ON onc.non_conformite_id = nc.non_conformite_id
    	LEFT JOIN CONFORMITE_TYPE ct ON nc.conformite_type_id = ct.conformite_type_id
    		WHERE ct.conformite_type_id = 3 AND pd.prod_derive_id = onc.objet_id) AS Raison_non_conformite_cession,
