...
BEGIN
	...
	IF id_terminale is not null THEN
		SET id_enceinte = (select t.enceinte_id from TERMINALE t 
			where t.terminale_id=id_terminale);
		
		SET adrl = (select t.terminale_id from TERMINALE t 
			where t.terminale_id=id_terminale);
...
BEGIN
	IF id_enceinte is not null THEN 
		SELECT t.terminale_id FROM TERMINALE t 
	...
	ELSE 
		SELECT t.terminale_id FROM TERMINALE t 
			WHERE (t.nom like tnom OR t.alias like tnom) AND get_terminale_adrl_as_ids(t.terminale_id, id_conteneur) is not null;		
	END IF;
END$$
