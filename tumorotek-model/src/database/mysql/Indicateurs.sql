...
BEGIN    
    CASE
		...
		WHEN name_Proc="count_prel_conforme_arrive" then CALL stats_count_prel_conforme_arrive(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_prel_conforme_apres_traitement" then CALL stats_count_prel_conforme_apres_traitement(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_echan_conforme_apres_traitement" then CALL stats_count_echan_conforme_apres_traitement(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_echan_conforme_cession" then CALL stats_count_echan_conforme_cession(date_debut, date_fin, sModeleId);
		...
		WHEN name_Proc="count_echan_echantype" then CALL stats_count_echan_echantype(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_prod_prodtype" then CALL stats_count_prod_prodtype(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_prel_prelnature" then CALL stats_count_prel_prelnature(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_requalif" then CALL stats_count_requalif(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_prepa_simple" then CALL stats_count_prepa_simple(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_prepa_complexe" then CALL stats_count_prepa_complex(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_echan_ambiante" then CALL stats_count_echan_ambiante(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_echan_moins4" then CALL stats_count_echan_moins4(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_madispo" then CALL stats_count_madispo(date_debut, date_fin, sModeleId);
		WHEN name_Proc="count_cederobj" then CALL stats_count_cederobj(date_debut, date_fin, sModeleId);	
		ELSE BEGIN END;
END CASE;
...
BEGIN
    TRUNCATE TABLE counts;
    SET @sql = COMPOSE_POURCENTAGESQL(null);
    SET @sql = CONCAT(@sql, ' (SELECT p.banque_id, count(distinct p.prelevement_id) as cc FROM PRELEVEMENT p JOIN ECHANTILLON e on e.prelevement_id=p.prelevement_id');
    SET @sql = CONCAT(@sql, ' WHERE p.DATE_PRELEVEMENT BETWEEN ''', date_debut, ''' AND ''', date_fin, '''');
