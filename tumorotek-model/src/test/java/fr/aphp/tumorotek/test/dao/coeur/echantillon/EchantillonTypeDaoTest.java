...
/**
 *
 * Classe de test pour le DAO EchantillonTypeDao et le bean du domaine
 * EchantillonType.
 *
...
public class EchantillonTypeDaoTest extends AbstractDaoTest {
	...
	public void testFindByOrder() {
		...
		list = echantillonTypeDao.findByPfOrder(pf);
		assertTrue(list.size() == 1);
		list = echantillonTypeDao.findByPfOrder(null);
	...
	/**
	 * Test l'insertion, la mise à jour et la suppression d'un type d'échantillon.
	 * 
	...
	public void testCrudEchanType() throws Exception {
		...
		assertEquals(new Integer(5), e.getId());

		// Test de la mise à jour
		final EchantillonType e2 = echantillonTypeDao.findById(new Integer(5)).get();
		assertNotNull(e2);
		assertTrue(e2.getNom().equals("TYPE"));
		assertTrue(e2.getIncaCat().equals("CAT10"));
		e2.setNom(updatedType);
		echantillonTypeDao.save(e2);
		assertTrue(echantillonTypeDao.findById(new Integer(5)).get().getNom().equals(updatedType));
	...
	public void testToString() {
		final EchantillonType type1 = echantillonTypeDao.findById(1).get();
		assertTrue(type1.toString().equals("{" + type1.getNom() + "}"));
