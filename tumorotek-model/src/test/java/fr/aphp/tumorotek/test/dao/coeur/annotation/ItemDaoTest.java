...
/**
 *
 * Classe de test pour le DAO ItemDao et le bean du domaine Item. Classe de test
 * créée le 01/02/10.
 *
...
public class ItemDaoTest extends AbstractDaoTest {
	...
	public void testToString() throws ParseException {
		...
		i2.setChampAnnotation(champAnnotationDao.findById(11).get());
		assertTrue(i2.toString().equals("{Empty Item}"));
	...
	public void testFindByChamp() {
		List<Item> items = itemDao.findByChamp(champAnnotationDao.findById(12).get());
		assertTrue(items.size() == 3);
		items = itemDao.findByChamp(champAnnotationDao.findById(1).get());
	...
	public void testCrudItem() {
		...
		thes.setTableAnnotation(tableAnnotationDao.findById(1).get());
		...
		// Test de l'insertion
		final ChampAnnotation thes2 = champAnnotationDao.findById(maxId).get();
		assertNotNull(thes2);
		...
		assertTrue(itemDao.findById(88).get().getLabel().equals("new"));
		assertTrue(itemDao.findById(88).get().getValeur().equals("newVal"));
		...
		itemDao.save(i3);
		assertTrue(itemDao.findByChamp(thes2).size() == 1);
		assertTrue(itemDao.findById(88).get().getLabel().equals("staple"));
		assertFalse(itemDao.findById(88).get().getValeur() != null);
	...
	public void testEqualsAndHashCode() {
		...
		final ChampAnnotation t1 = champAnnotationDao.findById(1).get();
		final ChampAnnotation t2 = champAnnotationDao.findById(2).get();
	...
	public void testClone() {
		final Item i = itemDao.findById(1).get();
	...
	public void testFindByChampAndPlateforme() {
		...
		ChampAnnotation c = champAnnotationDao.findById(38).get();
		assertTrue(itemDao.findByChampAndPlateforme(c, pf).isEmpty());

		c = champAnnotationDao.findById(11).get();
