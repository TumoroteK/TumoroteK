...
/**
 *
 * Classe de test pour le DAO CombinaisonDao et le bean du domaine Combinaison.
...
public class CombinaisonDaoTest extends AbstractDaoTest {
	...
	/**
	 * Test l'insertion, la mise à jour et la suppression d'une combinaison.
	 * 
	...
	public void testCrudCombinaison() throws Exception {
		DataType dataType = dataTypeDao.findById(2).get();
		ChampEntite chEntite = new ChampEntite(this.entiteDao.findById(2).get(), "champEntite1", dataType, false, true,
				"000-0", false, null);
		...
		dataType = dataTypeDao.findById(1).get();
		chEntite = new ChampEntite(this.entiteDao.findById(1).get(), "champEntite2", dataType, false, false, null,
				false, null);
		this.champEntiteDao.save(chEntite);
		...
		final Champ champ1 = this.champDao.findById(id1).get();
		final Champ champ2 = this.champDao.findById(id2).get();
		...
		final List<Combinaison> combinaisons = IterableUtils.toList(combinaisonDao.findAll());
		...
		final Combinaison c2 = this.combinaisonDao.findById(idObject).get();
		assertNotNull(c2);
		...
		final Champ updatedChamp1 = this.champDao.findById(id2).get();
		final Champ updatedChamp2 = this.champDao.findById(id1).get();
		...
		assertNotNull(this.combinaisonDao.findById(idObject).get().getChamp1());
		assertTrue(this.combinaisonDao.findById(idObject).get().getChamp1().equals(updatedChamp1));
		assertNotNull(this.combinaisonDao.findById(idObject).get().getChamp2());
		assertTrue(this.combinaisonDao.findById(idObject).get().getChamp2().equals(updatedChamp2));
		assertNotNull(this.combinaisonDao.findById(idObject).get().getOperateur());
		assertTrue(this.combinaisonDao.findById(idObject).get().getOperateur().equals(updatedOperateur));

		// Test de la délétion
		this.combinaisonDao.deleteById(idObject);
		assertFalse(this.combinaisonDao.findById(idObject).isPresent());

		// On supprimé les éléments créés
		if(this.champDao.existsById(id1)) {
			this.champDao.deleteById(id1);
		}
		if(this.champDao.existsById(id2)) {
	...
	public void testEquals() {
		...
		for (int i = 0; i < Math.pow(2, 3); i++) {
			...
			if (toTest >= 2) {
				champ1 = champDao.findById(2).get();
			}
			...
			Champ champ2 = null;
			if (toTest > 0) {
				champ2 = champDao.findById(3).get();
			}
	...
	public void testToString() {
		final Combinaison c1 = combinaisonDao.findById(1).get();
		if (c1.getChamp1() != null && c1.getChamp2() != null && c1.getOperateur() != null) {
			assertTrue(
					c1.toString().equals(c1.getChamp1().toString() + " " + c1.getOperateur() + " " + c1.getChamp2()));
		} else {
