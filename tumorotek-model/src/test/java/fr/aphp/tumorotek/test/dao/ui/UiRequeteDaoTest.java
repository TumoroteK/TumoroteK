 ...
 * accepté les termes.
 **/
package fr.aphp.tumorotek.test.dao.ui;
...
public class UiRequeteDaoTest extends AbstractDaoTest {
	...
	public void testFindById() {
		UiRequete r = uiRequeteDao.findById(1).get();
		assertNotNull(r);
		assertFalse(uiRequeteDao.findById(100).isPresent());
	}
	...
	public void testFindByUtilisateurAndEntite() {
		List<UiRequete> reqs = uiRequeteDao.findByUtilisateurAndEntite(utilisateurDao.findById(1).get(),
				entiteDao.findById(2).get());
		assertTrue(reqs.size() == 3);
		assertTrue(reqs.get(0).getOrdre() == 1);
		assertTrue(reqs.get(2).getOrdre() == 3);

		reqs = uiRequeteDao.findByUtilisateurAndEntite(utilisateurDao.findById(1).get(), entiteDao.findById(3).get());
		assertTrue(reqs.isEmpty());

		reqs = uiRequeteDao.findByUtilisateurAndEntite(utilisateurDao.findById(5).get(), entiteDao.findById(3).get());
		assertTrue(reqs.size() == 1);

		reqs = uiRequeteDao.findByUtilisateurAndEntite(utilisateurDao.findById(5).get(), null);
		assertTrue(reqs.isEmpty());

		reqs = uiRequeteDao.findByUtilisateurAndEntite(null, entiteDao.findById(2).get());
		assertTrue(reqs.isEmpty());
	}

	//@Test
	public void testFindByNomUtilisateurAndEntite() {
		List<UiRequete> reqs = uiRequeteDao.findByNomUtilisateurAndEntite(utilisateurDao.findById(1).get(),
				entiteDao.findById(2).get(), "REQ1");
		assertTrue(reqs.size() == 1);

		reqs = uiRequeteDao.findByNomUtilisateurAndEntite(utilisateurDao.findById(1).get(), entiteDao.findById(2).get(),
				"REQ2");
		assertTrue(reqs.size() == 1);
		...
		reqs = uiRequeteDao.findByNomUtilisateurAndEntite(null, entiteDao.findById(2).get(), "REQ2");
		assertTrue(reqs.isEmpty());

		reqs = uiRequeteDao.findByNomUtilisateurAndEntite(utilisateurDao.findById(1).get(), null, "REQ2");
		assertTrue(reqs.isEmpty());
	}
	...
	public void testCrud() throws Exception {
		...
		final UiRequete r1 = new UiRequete(null, utilisateurDao.findById(2).get(), entiteDao.findById(8).get(),
				"TESTREQ", 2);
		...
		final UiRequete r2 = uiRequeteDao.findById(idR1).get();
		...
		final UiRequete r3 = uiRequeteDao.findById(idR1).get();
	...
	public void testToString() {
		// assertTrue(uiRequeteDao.findById(1).toString().equals("{REQ1,
		// Prelevement}")).get();
