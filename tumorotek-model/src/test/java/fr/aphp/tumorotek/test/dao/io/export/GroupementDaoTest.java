...
/**
 *
 * Classe de test pour le DAO GroupementDao et le bean du domaine Groupement.
...
public class GroupementDaoTest extends AbstractDaoTest {
	...
	public void testFindEnfants() throws Exception {
		final List<Groupement> parents = IterableUtils.toList(groupementDao.findAll());
	...
	/**
	 * Test l'insertion, la mise à jour et la suppression d'un groupement.
	 * 
	...
	public void testCrudGroupement() throws Exception {
		/** Create. */
		Critere c = new Critere(this.champDao.findById(3).get(), "!=", "toto");
		this.critereDao.save(c);
		final int id1 = c.getCritereId();
		c = new Critere(this.champDao.findById(4).get(), "=", "tti");
		this.critereDao.save(c);
		final int id2 = c.getCritereId();

		final String condition = "and";
		final Critere critere1 = this.critereDao.findById(id1).get();
		final Critere critere2 = this.critereDao.findById(id2).get();
		final Groupement parent = this.groupementDao.findById(1).get();
		...
		final List<Groupement> groupements = IterableUtils.toList(groupementDao.findAll());
		...
		/** Update. */
		final Groupement g2 = this.groupementDao.findById(idObject).get();
		assertNotNull(g2);
		...
		final Critere updatedCritere1 = this.critereDao.findById(id2).get();
		final Critere updatedCritere2 = this.critereDao.findById(id1).get();
		final Groupement updatedParent = this.groupementDao.findById(2).get();
		...
		assertNotNull(this.groupementDao.findById(idObject).get().getOperateur());
		assertTrue(this.groupementDao.findById(idObject).get().getOperateur().equals(updatedCondition));
		if (this.groupementDao.findById(idObject).get().getCritere1() != null) {
			assertTrue(this.groupementDao.findById(idObject).get().getCritere1().equals(updatedCritere1));
		} else {
			assertNull(updatedCritere1);
		}
		if (this.groupementDao.findById(idObject).get().getCritere2() != null) {
			assertTrue(this.groupementDao.findById(idObject).get().getCritere2().equals(updatedCritere2));
		} else {
			assertNull(updatedCritere2);
		}
		if (this.groupementDao.findById(idObject).get().getParent() != null) {
			assertTrue(this.groupementDao.findById(idObject).get().getParent().equals(updatedParent));
		} else {
		...
		assertFalse(this.groupementDao.findById(idObject).isPresent());
	...
	public void testToString() {
		final Groupement g1 = groupementDao.findById(1).get();
	...
	public void testEquals() {
		...
		for (int i = 0; i < Math.pow(2, 4); i++) {
			...
			if (i / 4 >= 0) {
				critere1 = critereDao.findById(4).get();
			}
			...
			if (toTest >= 2) {
				critere2 = critereDao.findById(2).get();
			}
			...
			if (toTest > 0) {
				parent = groupementDao.findById(1).get();
			}
	...
	public void testHashCode() {
		...
		for (int i = 0; i < Math.pow(2, 4); i++) {
			...
			int hashCritere1 = 0;
			if (toTest >= 4) {
				critere1 = critereDao.findById(3).get();
			...
			int hashCritere2 = 0;
			if (toTest >= 2) {
				critere2 = critereDao.findById(1).get();
			...
			int hashParent = 0;
			if (toTest > 0) {
				parent = groupementDao.findById(3).get();
