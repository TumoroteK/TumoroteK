...
/**
 *
 * Classe de test pour le DAO PrelevementDao et le bean du domaine Prelevement.
...
public class PrelevementDaoTest extends AbstractDaoTest {
	...
	public void testToString() {
		Prelevement p1 = prelevementDao.findById(1).get();
	...
	/**
	 * Test l'appel de la méthode findByDatePrelevementAfterDate(). throws Exception
	...
	/**
	 * Test l'appel de la méthode findByDatePrelevementAfterDateWithBanque(). throws
	...
	/**
	 * Test l'appel de la méthode findByConsentDateAfterDate(). throws Exception
	 * Lance une exception en cas d'erreur.
	 */
	@Test
	...
	public void testFindByPrelevementType() {
		PrelevementType type = prelevementTypeDao.findById(1).get();
		List<Prelevement> prels = prelevementDao.findByPrelevementType(type);
		assertTrue(prels.size() == 3);
		type = prelevementTypeDao.findById(2).get();
	...
	public void testFindByNature() {
		Nature nature = natureDao.findById(1).get();
		List<Prelevement> prels = prelevementDao.findByNature(nature);
		assertTrue(prels.size() == 2);
		nature = natureDao.findById(4).get();
	...
	public void testFindByConsentType() {
		ConsentType type = consentTypeDao.findById(3).get();
		List<Prelevement> prels = prelevementDao.findByConsentType(type);
		assertTrue(prels.size() == 3);
		type = consentTypeDao.findById(2).get();
	...
	public void testFindByPatientNomOrNipInList() {
		...
		bks.add(banqueDao.findById(3).get());
	...
	public void testCrudPrelevement() throws Exception {
		...
		final Collaborateur preleveur = collaborateurDao.findById(1).get();
		...
		final PrelevementType ptype = prelevementTypeDao.findById(1).get();
		p.setPrelevementType(ptype);
		final ConditType cType = conditTypeDao.findById(1).get();
		p.setConditType(cType);
		final ConditMilieu milieu = conditMilieuDao.findById(1).get();
		...
		final Collaborateur op = collaborateurDao.findById(2).get();
		p.setOperateur(op);
		p.setQuantite(null);
		...
		final Risque r2 = risqueDao.findById(2).get();
		risks.add(r2);
		final Risque r3 = risqueDao.findById(3).get();
		risks.add(r3);
		p.setRisques(risks);
		...
		final Prelevement p2 = prelevementDao.findById(prelId).get();
		assertNotNull(p2);
		assertTrue(p2.getBanque().equals(b));
		final Banque b2 = banqueDao.findById(3).get();
		p2.setBanque(b2);
		assertTrue(p2.getCode() == "pre1234");
		p2.setCode("pre55");
		assertTrue(p2.getNature().equals(n));
		final Nature n2 = natureDao.findById(2).get();
		p2.setNature(n2);
		assertTrue(p2.getConsentType().equals(ct));
		final ConsentType ct2 = consentTypeDao.findById(1).get();
		p2.setConsentType(ct2);
		assertTrue(p2.getMaladie().equals(m));
		final Maladie m2 = maladieDao.findById(2).get();
		p2.setMaladie(m2);
		...
		final Collaborateur preleveur2 = collaborateurDao.findById(2).get();
		p2.setPreleveur(preleveur2);
		assertTrue(p2.getServicePreleveur().equals(s));
		final Service s2 = serviceDao.findById(3).get();
		...
		final PrelevementType ptype2 = prelevementTypeDao.findById(2).get();
		p2.setPrelevementType(ptype2);
		assertTrue(p2.getConditType().equals(cType));
		final ConditType cType2 = conditTypeDao.findById(2).get();
		p2.setConditType(cType2);
		assertTrue(p2.getConditMilieu().equals(milieu));
		final ConditMilieu milieu2 = conditMilieuDao.findById(2).get();
		...
		final Collaborateur op2 = collaborateurDao.findById(3).get();
		p2.setOperateur(op2);
		...
		p2.getRisques().clear();
		final Risque r1 = risqueDao.findById(1).get();
		p2.getRisques().add(r1);
		p2.setQuantite((float) 12.55544);

		// test l'update
		prelevementDao.save(p2);
		assertTrue(prelevementDao.findById(prelId).get().getBanque().equals(b2));
		assertTrue(prelevementDao.findById(prelId).get().getCode() == "pre55");
		assertTrue(prelevementDao.findById(prelId).get().getNature().equals(n2));
		assertTrue(prelevementDao.findById(prelId).get().getConsentType().equals(ct2));
		assertTrue(prelevementDao.findById(prelId).get().getMaladie().equals(m2));
		assertTrue(prelevementDao.findById(prelId).get().getConsentDate().equals(ctDate2));
		assertTrue(prelevementDao.findById(prelId).get().getPreleveur().equals(preleveur2));
		assertTrue(prelevementDao.findById(prelId).get().getServicePreleveur().equals(s2));
		assertTrue(prelevementDao.findById(prelId).get().getDatePrelevement() == null);
		assertTrue(prelevementDao.findById(prelId).get().getPrelevementType().equals(ptype2));
		assertTrue(prelevementDao.findById(prelId).get().getConditType().equals(cType2));
		assertTrue(prelevementDao.findById(prelId).get().getConditMilieu().equals(milieu2));
		assertTrue(prelevementDao.findById(prelId).get().getConditNbr() == 11);
		assertTrue(prelevementDao.findById(prelId).get().getDateDepart().equals(depDate2));
		assertTrue(prelevementDao.findById(prelId).get().getTransporteur().equals(t2));
		assertTrue(prelevementDao.findById(prelId).get().getTransportTemp().equals(new Float(-10.5)));
		assertTrue(prelevementDao.findById(prelId).get().getDateArrivee().equals(arrDate2));
		assertTrue(prelevementDao.findById(prelId).get().getOperateur().equals(op2));
		assertTrue(prelevementDao.findById(prelId).get().getQuantiteUnite().equals(qU2));
		assertTrue(prelevementDao.findById(prelId).get().getPatientNda() == "NDA2");
		assertTrue(prelevementDao.findById(prelId).get().getNumeroLabo() == "12345");
		assertFalse(prelevementDao.findById(prelId).get().getSterile());
		assertTrue(prelevementDao.findById(prelId).get().getCongArrivee());
		assertTrue(prelevementDao.findById(prelId).get().getConformeArrivee());
		assertFalse(prelevementDao.findById(prelId).get().getCongDepart());
		assertTrue(prelevementDao.findById(prelId).get().getEtatIncomplet());
		assertTrue(prelevementDao.findById(prelId).get().getArchive());
	...
	public void testClone() {
		...
		/*
		 * if (p1.getVolume() != null) { assertTrue(p1.getVolume()
		 * .equals(p2.getVolume())); } else { assertNull(p2.getVolume()); } if
		 * (p1.getVolumeUnite() != null) { assertTrue(p1.getVolumeUnite()
		 * .equals(p2.getVolumeUnite())); } else { assertNull(p2.getVolumeUnite()); }
		 */
	...
	public void testFindCountEclConsentByDates() throws ParseException {
		...
		cTypes.add(consentTypeDao.findById(3).get());
		res = prelevementDao.findCountEclConsentByDates(cTypes, d1, d2, banks);
	...
	public void testFindAssociatePrelsOfType() {
		...
		nats.add(natureDao.findById(2).get());

		final Maladie m4 = maladieDao.findById(4).get();

		List<Prelevement> res = prelevementDao.findAssociatePrelsOfType(m4, nats, banks);
		assertTrue(res.size() == 1);

		nats.add(natureDao.findById(1).get());
		...
		final Maladie m3 = maladieDao.findById(3).get();
	...
	public void testFindByEchantillonId() {
		...
		assertTrue(prels.size() == 1);
		assertTrue(prels.get(0).equals(prelevementDao.findById(1).get()));
		prels = prelevementDao.findByEchantillonId(4);
		assertTrue(prels.size() == 1);
		assertTrue(prels.get(0).equals(prelevementDao.findById(3).get()));
	...
	public void testFindByEtablissementVide() {
		...
		assertTrue(prels.size() == 1);
		assertTrue(prels.contains(prelevementDao.findById(4).get()));
	}
	...
	public void testFindByPatientAndBanques() {
		...
		// MAYER = 1 prel
		Patient pat = patientDao.findById(1).get();
		...
		prels = prelevementDao.findByPatientAndBanques(pat, banks);
		assertTrue(prels.size() == 1);
		assertTrue(prels.contains(prelevementDao.findById(3).get()));

		// SOLIS = aucun prelevement
		pat = patientDao.findById(2).get();
		prels = prelevementDao.findByPatientAndBanques(pat, banks);
		assertTrue(prels.isEmpty());

		// DELPHINO = 3 prels
		pat = patientDao.findById(3).get();

		prels = prelevementDao.findByPatientAndBanques(pat, banks);
		assertTrue(prels.size() == 2);
		assertTrue(prels.contains(prelevementDao.findById(1).get()));
		assertTrue(prels.contains(prelevementDao.findById(2).get()));

		banks.add(banqueDao.findById(3).get());
		prels = prelevementDao.findByPatientAndBanques(pat, banks);
		assertTrue(prels.size() == 3);
		assertTrue(prels.contains(prelevementDao.findById(1).get()));
		assertTrue(prels.contains(prelevementDao.findById(2).get()));
		assertTrue(prels.contains(prelevementDao.findById(5).get()));

		banks.remove(banqueDao.findById(1).get());
		prels = prelevementDao.findByPatientAndBanques(pat, banks);
		assertTrue(prels.size() == 1);
		assertTrue(prels.contains(prelevementDao.findById(5).get()));
	...
	public void testFindByEtablissementLaboInter() {
		...
		Etablissement etab1 = etablissementDao.findById(1).get();
		List<Prelevement> prels = prelevementDao.findByEtablissementLaboInter(etab1, banks);
		assertTrue(prels.size() == 1);
		assertTrue(prels.get(0).getPrelevementId() == 1);

		prels = prelevementDao.findByEtablissementLaboInter(etablissementDao.findById(2).get(), banks);
		assertTrue(prels.isEmpty());
	...
	public void testFindByServiceLaboInter() {
		...
		Service srv1 = serviceDao.findById(1).get();
		List<Prelevement> prels = prelevementDao.findByServiceLaboInter(srv1, banks);
		assertTrue(prels.size() == 1);
		assertTrue(prels.get(0).getPrelevementId() == 1);

		prels = prelevementDao.findByServiceLaboInter(serviceDao.findById(4).get(), banks);
		assertTrue(prels.isEmpty());
	...
	public void testFindByOperateurLaboInter() {
		...
		Collaborateur ope2 = collaborateurDao.findById(2).get();
		List<Prelevement> prels = prelevementDao.findByCollaborateurLaboInter(ope2, banks);
		assertTrue(prels.size() == 1);
		assertTrue(prels.get(0).getPrelevementId() == 1);

		prels = prelevementDao.findByCollaborateurLaboInter(collaborateurDao.findById(1).get(), banks);
		assertTrue(prels.isEmpty());
