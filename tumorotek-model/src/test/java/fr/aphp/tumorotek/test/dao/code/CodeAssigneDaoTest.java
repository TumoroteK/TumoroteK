...
/**
 *
 * Classe de test pour le DAO CodeAssigneDao et le bean du domaine CodeAssigne.
...
public class CodeAssigneDaoTest extends AbstractDaoTest {
	...
	public void testFindCodesMorphoByEchantillon() {
		...
		e = echantillonDao.findById(3).get();
	...
	public void findByExcludedId() {
		final CodeAssigne c = codeAssigneDao.findById(1).get();
		List<CodeAssigne> codes = codeAssigneDao.findByExcludedId(c.getCodeAssigneId(), c.getCode(),
				c.getEchantillon());
		assertTrue(codes.size() == 0);
		codes = codeAssigneDao.findByExcludedId(8, "BL", echantillonDao.findById(1).get());
		assertTrue(codes.size() == 1);
	}
	...
	public void testCrud() throws Exception {
		...
		final Echantillon e = echantillonDao.findById(1).get();
		final TableCodage tc = tableCodageDao.findById(4).get();
		c.setEchantillon(e);
		c.setTableCodage(tc);
		c.setCodeRefId(1);
		c.setCode("TEST");
		...
		final CodeAssigne c2 = codeAssigneDao.findById(cId).get();
		assertNotNull(c2);
		...
		assertTrue(codeAssigneDao.findById(cId).get().getCode().equals(codeUpdated));
	...
	public void testClone() {
		final CodeAssigne c = codeAssigneDao.findById(2).get();
		// c.setEchanExpLes(echantillonDao.findById(3)).get();
		// c.setEchanExpOrg(echantillonDao.findById(3)).get();
