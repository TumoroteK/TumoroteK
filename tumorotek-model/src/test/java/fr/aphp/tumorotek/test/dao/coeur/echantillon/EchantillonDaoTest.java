 ...
 * accepté les termes.
 **/
package fr.aphp.tumorotek.test.dao.coeur.echantillon;
...
/**
 *
 * Classe de test pour le DAO EchantillonDao et le bean du domaine Echantillon.
...
public class EchantillonDaoTest extends AbstractDaoTest {
	...
	/**
	 * Test l'appel de la méthode findByDateStockAfterDateWithBanque(). throws
	...
	public void testFindByEchanQualite() {
		EchanQualite qualite = echanQualiteDao.findById(1).get();
		List<Echantillon> echans = echantillonDao.findByEchanQualite(qualite);
		assertTrue(echans.size() == 3);
		qualite = echanQualiteDao.findById(2).get();
	...
	public void testFindByEchantillonType() {
		EchantillonType type = echantillonTypeDao.findById(1).get();
		List<Echantillon> echans = echantillonDao.findByEchantillonType(type);
		assertTrue(echans.size() == 3);
		type = echantillonTypeDao.findById(3).get();
	...
	// Fichier anapath = fichierDao.findById(1).get();
	// List<Echantillon> echans = echantillonDao.findByCrAnapath(anapath);
	// assertTrue(echans.size() == 1);
	// Fichier anapath2 = fichierDao.findById(3).get();
	...
	public void testFindByModePrepa() {
		final ModePrepa mode = modePrepaDao.findById(1).get();
		List<Echantillon> echans = echantillonDao.findByModePrepa(mode);
		assertTrue(echans.size() == 2);
		final ModePrepa mode2 = modePrepaDao.findById(4).get();
	...
	public void testFindByPrelevement() {
		Prelevement prlvt = prelevementDao.findById(1).get();
		List<Echantillon> echans = echantillonDao.findByPrelevement(prlvt);
		assertTrue(echans.size() == 2);
		prlvt = prelevementDao.findById(2).get();
	...
	public void testFindRestantsByPrelevement() {
		...
		assertTrue(echans.size() == 0);
		prlvt = prelevementDao.findById(3).get();
		echans = echantillonDao.findRestantsByPrelevement(prlvt);
		assertTrue(echans.size() == 0);
	}
	...
	public void testCrud() throws Exception {
		...
		final Prelevement p = prelevementDao.findById(1).get();
		final Banque b = banqueDao.findById(1).get();
		final EchantillonType et = echantillonTypeDao.findById(2).get();
		e.setCode("code");
		...
		final Echantillon e2 = echantillonDao.findById(echanId).get();
		...
		assertTrue(echantillonDao.findById(echanId).get().getCode().equals(codeUpdated));
		assertFalse(echantillonDao.findById(echanId).get().getSterile());
		assertFalse(echantillonDao.findById(echanId).get().getConformeTraitement());
		assertFalse(echantillonDao.findById(echanId).get().getConformeCession());
		assertNull(echantillonDao.findById(echanId).get().getQuantite());
		assertNull(echantillonDao.findById(echanId).get().getQuantiteInit());
	...
	public void testToString() {
		final Echantillon echan1 = echantillonDao.findById(1).get();
	...
	public void testClone() {
		 ...
		 * assertTrue(e1.getVolume().equals(e2.getVolume())); } else {
		 * assertNull(e2.getVolume()); } if (e1.getVolumeInit() != null) {
		 * assertTrue(e1.getVolumeInit().equals(e2.getVolumeInit())); } else {
		 * assertNull(e2.getVolumeInit()); } if (e1.getVolumeUnite() != null) {
		 * assertTrue(e1.getVolumeUnite().equals(e2.getVolumeUnite())); } else {
	...
	public void testCountCongelesByDatesExt() throws ParseException {
		...
		final Etablissement e2 = etablissementDao.findById(2).get();
		etabs.add(e2);
		...
		final Etablissement e3 = etablissementDao.findById(3).get();
		etabs.add(e3);
		etabs.remove(e1);
		res = echantillonDao.findCountSamplesByDatesExt(d1, d2, banks, etabs);
		assertTrue(res.get(0) == 3);
	...
	public void testFindCountEchansByCessTypes() throws ParseException {
		...
		final Banque b2 = banqueDao.findById(2).get();
		banks.add(b2);
		final CessionType cSan = cessionTypeDao.findById(1).get();
		...
		final CessionType cRech = cessionTypeDao.findById(2).get();
		...
		final CessionType cDest = cessionTypeDao.findById(3).get();
	...
	public void testFindAssociateEchansOfType() {
		...
		types.add(echantillonTypeDao.findById(3).get());
		...
		types.add(echantillonTypeDao.findById(1).get());
		...
		final Maladie m1 = maladieDao.findById(1).get();
	...
	public void testFindCountByPrelevement() {
		...
		final Prelevement p3 = prelevementDao.findById(3).get();
		res = echantillonDao.findCountByPrelevement(p3);
		assertTrue(res.get(0) == 1);

		final Prelevement p4 = prelevementDao.findById(4).get();
	...
	public void testFindByEmplacementDao() {
		List<Echantillon> echans = echantillonDao.findByEmplacement(terminaleDao.findById(1).get(), 3);
		assertTrue(echans.size() == 1);
		assertTrue(echans.contains(echantillonDao.findById(2).get()));
		echans = echantillonDao.findByEmplacement(terminaleDao.findById(1).get(), 1);
		assertTrue(echans.isEmpty());
		echans = echantillonDao.findByEmplacement(null, 1);
		assertTrue(echans.isEmpty());
		echans = echantillonDao.findByEmplacement(terminaleDao.findById(1).get(), null);
		assertTrue(echans.isEmpty());
	}
}
