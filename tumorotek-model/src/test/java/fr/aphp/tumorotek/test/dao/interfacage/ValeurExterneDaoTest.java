...
/**
 *
 * Classe de test pour le DAO ValeurExterneDao et le bean du domaine
 * ValeurExterne.
 *
...
public class ValeurExterneDaoTest extends AbstractDaoTest {
	...
	public void testFindByBlocExterne() {
		...
		final BlocExterne b3 = blocExterneDao.findById(3).get();
	...
	public void testCrud() throws Exception {
		...
		final ValeurExterne v2 = valeurExterneDao.findById(new Integer(id)).get();
		...
		assertTrue(valeurExterneDao.findById(new Integer(id)).get().getValeur().equals("VALEUR"));
		assertTrue(valeurExterneDao.findById(new Integer(id)).get().getChampAnnotationId() == 1);
		assertTrue(valeurExterneDao.findById(new Integer(id)).get().getChampEntiteId() == 2);
		assertEquals(new String(valeurExterneDao.findById(new Integer(id)).get().getContenu()), "CONTENT...");
	...
	public void testEquals() {
		...
		final Integer a2 = 2;
		final BlocExterne b1 = blocExterneDao.findById(1).get();
		final BlocExterne b2 = blocExterneDao.findById(2).get();
		final ValeurExterne v1 = new ValeurExterne();
	...
	public void testToString() {
		final ValeurExterne v1 = valeurExterneDao.findById(1).get();
	...
	public void testFindByDossierChampEntiteIdAndBlocEntiteId() {
		DossierExterne dos = dossierExterneDao.findById(4).get();
		List<ValeurExterne> liste = valeurExterneDao.findByDossierChampEntiteIdAndBlocEntiteId(dos, 44, 2);
		assertTrue(liste.size() == 1);
		assertTrue(liste.get(0).getValeur().equals("NDA127896 BI"));

		liste = valeurExterneDao.findByDossierChampEntiteIdAndBlocEntiteId(dos, 230, 3);
		assertTrue(liste.size() == 1);
		assertTrue(liste.get(0).getValeur().equals("GHLOJ7F4;GHOTJMF4"));

		liste = valeurExterneDao.findByDossierChampEntiteIdAndBlocEntiteId(dos, 89, 3);
		assertTrue(liste.isEmpty());

		liste = valeurExterneDao.findByDossierChampEntiteIdAndBlocEntiteId(null, 89, 3);
		assertTrue(liste.isEmpty());

		liste = valeurExterneDao.findByDossierChampEntiteIdAndBlocEntiteId(dos, null, 3);
		assertTrue(liste.isEmpty());

		liste = valeurExterneDao.findByDossierChampEntiteIdAndBlocEntiteId(dos, 89, null);
		assertTrue(liste.isEmpty());
	}
}