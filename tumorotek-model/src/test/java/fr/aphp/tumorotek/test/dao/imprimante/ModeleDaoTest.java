...
public class ModeleDaoTest extends AbstractDaoTest {
	...
	public void testCrud() throws Exception {
		...
		final ModeleType mt = modeleTypeDao.findById(1).get();
		...
		final Modele m2 = modeleDao.findById(new Integer(4)).get();
		...
		m2.setIsQRCode(true);
		modeleDao.save(m2);
		assertTrue(modeleDao.findById(new Integer(4)).get().getNom().equals("UP"));
		assertTrue(m2.getIsQRCode());

		// Test de la délétion
		modeleDao.deleteById(new Integer(4));
		assertFalse(modeleDao.findById(new Integer(4)).isPresent());
