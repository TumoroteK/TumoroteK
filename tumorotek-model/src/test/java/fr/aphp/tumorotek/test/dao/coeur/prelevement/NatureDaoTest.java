...
/**
 *
 * Classe de test pour le DAO NatureDao et le bean du domaine Nature (de
 * prelevement). Classe de test créée le 01/10/09.
 *
...
public class NatureDaoTest extends AbstractDaoTest {
	...
	public void testToString() {
		Nature n1 = natureDao.findById(1).get();
	...
	/**
	 * Test l'insertion, la mise à jour et la suppression d'une nature de
	 * prelevement.
	 * 
	...
	public void testCrudNature() throws Exception {
		...
		n.setNom("URINES");
		n.setPlateforme(plateformeDao.findById(1).get());
		// Test de l'insertion
		natureDao.save(n);
		assertEquals(new Integer(5), n.getId());

		// Test de la mise à jour
		final Nature n2 = natureDao.findById(new Integer(5)).get();
		assertNotNull(n2);
		assertTrue(n2.getNom().equals("URINES"));
		n2.setNom(updatedNature);
		natureDao.save(n2);
		assertTrue(natureDao.findById(new Integer(5)).get().getNom().equals(updatedNature));
	...
	public void testClone() {
		final Nature n1 = natureDao.findById(1).get();
