...
/**
 *
 * Classe de test pour le DAO CessionDao et le bean du domaine Cession.
 *
...
public class CessionDaoTest extends AbstractDaoTest {
	...
	public void testFindByContrat() {
		...
		final Contrat contrat2 = contratDao.findById(2).get();
		final Contrat contrat3 = contratDao.findById(3).get();
	...
	public void testCrudCession() throws Exception {
		...
		final CessionType cessType = cessionTypeDao.findById(1).get();
		final CessionExamen cessExam = cessionExamenDao.findById(1).get();
		final Contrat contrat = contratDao.findById(1).get();
		final Collaborateur collab = collaborateurDao.findById(1).get();
		final Service serv = serviceDao.findById(1).get();
		final CessionStatut statut = cessionStatutDao.findById(1).get();
		final Transporteur transp = transporteurDao.findById(1).get();
		final DestructionMotif motif = destructionMotifDao.findById(1).get();
		...
		final Cession c2 = cessionDao.findById(new Integer(5)).get();
		assertNotNull(c2);
		...
		cessionDao.save(c2);
		assertTrue(cessionDao.findById(new Integer(5)).get().getNumero() == updatedNumero);
		assertTrue(cessionDao.findById(new Integer(5)).get().getDemandeDate().equals(upDate));

		// Test de la délétion
		cessionDao.deleteById(new Integer(5));
		assertFalse(cessionDao.findById(new Integer(5)).isPresent());
	}
