...
public class UtilisateurDaoTest extends AbstractDaoTest {
	...
	public void testFindByLoginAndArchive() {
		...
		pfs.clear();
		pfs.add(plateformeDao.findById(2).get());
	...
	public void testFindByCollaborateur() {
		...
		assertTrue(utilisateurs.size() == 1);
		final Collaborateur c2 = collaborateurDao.findById(4).get();
...
//		}
		...
		final Utilisateur u2 = utilisateurDao.findById(id).get();
		assertNotNull(u2);
		...
		assertTrue(utilisateurDao.findById(id).get().getLogin().equals(updatedLogin));
