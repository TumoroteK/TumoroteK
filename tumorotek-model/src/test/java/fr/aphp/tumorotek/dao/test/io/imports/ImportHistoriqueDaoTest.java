...
public class ImportHistoriqueDaoTest extends AbstractDaoTest {
	...
	public void testReadAll() {
		List<ImportHistorique> liste = importHistoriqueDao.findAll();
	...
	public void testFindByTemplateWithOrder() {
		...
		List<ImportHistorique> liste = importHistoriqueDao
			.findByTemplateWithOrder(it1);
	...
	public void testCrud() throws Exception {
		...
		Set<Importation> imports = new HashSet<Importation>();
		imports.add(i1);
		imports.add(i2);
		...
		ImportHistorique ih2 = importHistoriqueDao.findById(new Integer(4));
		assertNotNull(ih2);
		...
		assertTrue(importHistoriqueDao.findById(new Integer(4)).
				getDate().equals(cal2));
		assertTrue(importHistoriqueDao.findById(new Integer(4)).
				getImportations().size() == 2);
	...
	public void testEquals() {
		...
		ImportTemplate t1 = importTemplateDao.findById(1);
		ImportTemplate t2 = importTemplateDao.findById(2);
		ImportHistorique ih1 = new ImportHistorique();
	...
	public void testToString() {
		ImportHistorique ih1 = importHistoriqueDao.findById(1);
		assertTrue(ih1.toString().
				equals("{" + ih1.getDate() + ", " 
				+ ih1.getImportTemplate().getNom() 
				+ "(ImportTemplate)}"));
	...
	public void testClone() {
		...
		if (ih1.getImportHistoriqueId() != null) {
			assertTrue(ih1.getImportHistoriqueId() 
					== ih2.getImportHistoriqueId());
		} else {
			assertNull(ih2.getImportHistoriqueId());
		}
		
		if (ih1.getImportTemplate() != null) {
			assertTrue(ih1.getImportTemplate()
					.equals(ih2.getImportTemplate()));
		} else {
	...
	public void testFindPrelevementByImportHistorique() {
		...
		prels.addAll(importHistoriqueDao
			.findPrelevementByImportHistorique(importHistoriqueDao.findById(2)));
