...
public class UiRequeteDaoTest extends AbstractDaoTest {
	...
	public void setUiRequeteDao(UiRequeteDao u) {
		this.uiRequeteDao = u;
	}
	...
	public void setUtilisateurDao(UtilisateurDao t) {
	...
	public void testFindAll() {
		List<UiRequete> liste = uiRequeteDao.findAll();
		assertTrue(liste.size() == 4);
	}

	public void testFindByUtilisateurAndEntite() {
		List<UiRequete> reqs = uiRequeteDao.findByUtilisateurAndEntite(utilisateurDao.findById(1), 
																		entiteDao.findById(2));
		assertTrue(reqs.size() == 3);
		assertTrue(reqs.get(0).getOrdre() == 1);
		assertTrue(reqs.get(2).getOrdre() == 3);
		
		reqs = uiRequeteDao.findByUtilisateurAndEntite(utilisateurDao.findById(1), 
				entiteDao.findById(3));
		assertTrue(reqs.isEmpty());
		
		reqs = uiRequeteDao.findByUtilisateurAndEntite(utilisateurDao.findById(5), 
				entiteDao.findById(3));
	...
	public void testFindByNomUtilisateurAndEntite() {
		List<UiRequete> reqs = uiRequeteDao.findByNomUtilisateurAndEntite(utilisateurDao.findById(1), 
																		entiteDao.findById(2),"REQ1");
		assertTrue(reqs.size() == 1);

		reqs = uiRequeteDao.findByNomUtilisateurAndEntite(utilisateurDao.findById(1), 
				entiteDao.findById(2),"REQ2");
		assertTrue(reqs.size() == 1);
		
		reqs = uiRequeteDao.findByNomUtilisateurAndEntite(utilisateurDao.findById(1), 
				entiteDao.findById(2),"DAVID");
		assertTrue(reqs.isEmpty());
	...
	public void testCrud() throws Exception {
		...
		UiRequete r1 = new UiRequete(null, utilisateurDao.findById(2), entiteDao.findById(8), "TESTREQ", 2);
		
		UiCompValue u1 = new UiCompValue(null, r1, "codeDeriveBox", "textbox", null, "PTRA.12", null, null);
		r1.getUiCompValues().add(u1);
		UiCompValue u2 = new UiCompValue(null, r1, "annoDeriveBox123", "listbox", 3, null, null, null);
		r1.getUiCompValues().add(u2);

		// Test de l'insertion
		uiRequeteDao.createObject(r1);
		Integer idR1 = r1.getUiRequeteId();
		assertNotNull(idR1);

		UiRequete r2 = uiRequeteDao.findById(idR1);
		...
		r2.setOrdre(1);
		r2.setNom("TESTREQ22");
		UiCompValue u3 = new UiCompValue(null, r2, "sexeMFBox", "checkbox", null, null, true, null);
		...
		UiRequete r3 = uiRequeteDao.findById(idR1);
	...
	public void testEquals() {
		...
		UiRequete r1 = new UiRequete();
		UiRequete r2 = new UiRequete();
