...
public class ChampImprimeDaoTest extends AbstractDaoTest {
	...
	public void testReadAll() {
		List<ChampImprime> liste = champImprimeDao.findAll();
	...
	public void testFindById() {
		ChampEntite c1 = champEntiteDao.findById(54);
		Template t1 = templateDao.findById(1);
		BlocImpression b1 = blocImpressionDao.findById(5);
	...
	public void testFindByExcludedPK() {
		...
		List<ChampImprime> liste = 
			champImprimeDao.findByExcludedPK(pk);
	...
	public void testFindByTemplate() {
		Template t1 = templateDao.findById(1);
		List<ChampImprime> liste = 
			champImprimeDao.findByTemplate(t1);
		assertTrue(liste.size() == 4);
	...
	public void testCrud() {
		...
		ChampEntite c1 = champEntiteDao.findById(53);
		Template t1 = templateDao.findById(1);
		BlocImpression b1 = blocImpressionDao.findById(5);
		Integer ordre = 5;
		Integer ordreUp = 6;
		...
		ChampImprimePK pk = new ChampImprimePK();
		pk.setChampEntite(c1);
		pk.setTemplate(t1);
		pk.setBlocImpression(b1);
		ChampImprime ci2 = champImprimeDao.findById(pk);
		assertNotNull(ci2);
		...
		assertTrue(champImprimeDao.findById(pk)
				.getOrdre() == ordreUp);
		assertTrue(champImprimeDao.findAll().size() == 5);
	...
	private void populateClefsToTestEqualsAndHashCode(
			ChampImprime ci1, 
			ChampImprime ci2) throws ParseException {
		
		ChampEntite c1 = champEntiteDao.findById(1);
		ChampEntite c2 = champEntiteDao.findById(2);
		Template t1 = templateDao.findById(1);
		BlocImpression b1 = blocImpressionDao.findById(5);
		ChampImprimePK pk1 = new ChampImprimePK(t1, c1, b1);
		ChampImprimePK pk2 = new ChampImprimePK(t1, c2, b1);
		ChampImprimePK pk3 = new ChampImprimePK(t1, c1, b1);
		ChampImprimePK[] pks = new ChampImprimePK[]{null, pk1, pk2, pk3};
	...
	public void testToString() {
		...
		BlocImpression b1 = blocImpressionDao.findById(5);
		ChampImprimePK pk1 = new ChampImprimePK(t1, c1, b1);
		ChampImprime ci1 = champImprimeDao.findById(pk1);
		
		assertTrue(ci1.toString().
				equals("{" + ci1.getPk().toString() + "}"));
