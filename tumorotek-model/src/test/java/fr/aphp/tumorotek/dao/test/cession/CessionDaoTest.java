...
public class CessionDaoTest extends AbstractDaoTest {
	...
	private String updatedNumero = "999";
	...
	public void setCessionTypeDao(CessionTypeDao dao) {
		this.cessionTypeDao = dao;
	}

	public void setCessionExamenDao(CessionExamenDao dao) {
		this.cessionExamenDao = dao;
	}

	public void setContratDao(ContratDao dao) {
		this.contratDao = dao;
	}
	...
	public void setCessionStatutDao(CessionStatutDao dao) {
		this.cessionStatutDao = dao;
	}

	public void setTransporteurDao(TransporteurDao dao) {
		this.transporteurDao = dao;
	}

	public void setDestructionMotifDao(DestructionMotifDao dao) {
	...
	public void testReadAllCessions() {
		List<Cession> liste = cessionDao.findAll();
	...
	public void testFindByNumeroWithBanqueReturnIds() {
		...
		List<Integer> liste = cessionDao.findByNumeroWithBanqueReturnIds(
				"55", banks);
		assertTrue(liste.size() == 1);
	...
	public void testFindByBanques() {
		...
		List<Cession> res = cessionDao.findByBanques(banks);
	...
	public void testFindByBanquesAllIds() {
		...
		banks.add(banqueDao.findById(1));
		banks.add(banqueDao.findById(2));
		List<Integer> res = cessionDao.findByBanquesAllIds(banks);
		assertTrue(res.size() == 4);
	}
	...
	public void testFindByContrat() {
		...
		Contrat contrat2 = contratDao.findById(2);
		Contrat contrat3 = contratDao.findById(3);
	...
	public void testFindByCessionStatutAndBanqueReturnIds() {
		...
		List<Integer> liste = cessionDao
						.findByCessionStatutAndBanqueReturnIds(
								"VALIDEE", banks);
		assertTrue(liste.size() == 2);
		
		liste = cessionDao.findByCessionStatutAndBanqueReturnIds(
				"EN ATTENTE", banks);
		assertTrue(liste.size() == 0);
	...
	public void testFindByEtatIncompletAndBanquesReturnIds() {
		...
		banks.add(b1);
		List<Integer> liste = cessionDao
			.findByEtatIncompletAndBanquesReturnIds(false, banks);
		assertTrue(liste.size() == 3);
		
		liste = cessionDao.findByEtatIncompletAndBanquesReturnIds(
				true, banks);
		assertTrue(liste.size() == 0);
	...
	public void testCrudCession() throws Exception {
		...
		CessionType cessType = cessionTypeDao.findById(1);
		CessionExamen cessExam = cessionExamenDao.findById(1);
		Contrat contrat = contratDao.findById(1);
		...
		Date date = new SimpleDateFormat("dd/MM/yyyy").parse("01/09/2009");
		Date upDate = new SimpleDateFormat("dd/MM/yyyy").parse("25/09/2009");
		Calendar cal = Calendar.getInstance();
		cal.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("25/09/2009 12:12:23"));
		...
		Cession c2 = cessionDao.findById(new Integer(5));
		assertNotNull(c2);
		...
		assertTrue(c2.getTemperature() == (float) -20);
		...
		assertTrue(cessionDao.findById(new Integer(5)).
				getNumero() == updatedNumero);
		assertTrue(cessionDao.findById(new Integer(5)).
				getDemandeDate().equals(upDate));
	...
	public void testToString() {
		...
		assertTrue(c1.entiteNom().equals(entiteDao
							.findByNom("Cession").get(0).getNom()));
	...
	public void testClone() {
		...
		if (c1.getDestructionMotif() != null) {
			assertTrue(c1.getDestructionMotif().equals(
					c2.getDestructionMotif()));
		} else {
