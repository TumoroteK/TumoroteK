...
/**
 * 
 * Classe de test pour le DAO TableAnnotationDao et le 
 * bean du domaine TableAnnotation.
...
public class TableAnnotationDaoTest extends AbstractDaoTest {
	...
	public void setCatalogueDao(CatalogueDao cataDao) {
	...
	public void testToString() throws ParseException {
		...
		assertTrue(t1.toString().
				equals("{Patient.TABLE_PAT1}"));
		
		TableAnnotation t2 = new TableAnnotation();
		assertTrue(t2.toString().
				equals("{Empty TableAnnotation}"));
		t2.setEntite(entiteDao.findById(1));
		assertTrue(t2.toString().
				equals("{Empty TableAnnotation}"));
		t2.setEntite(null);
		t2.setNom("testTable");
	...
	public void testReadAllTables() {
		List<TableAnnotation> tabs = tableAnnotationDao.findAll();
		assertTrue(tabs.size() == 11);
	}
	
	public void testFindByNom() {
		List<TableAnnotation> tabs = tableAnnotationDao
										.findByNom("TABLE_ECHAN%");
	...
	public void testFindByEntiteAndBanque() {
		...
		List<TableAnnotation> tabs = tableAnnotationDao
											.findByEntiteAndBanque(e, b);
		assertTrue(tabs.size() == 4);
		// verfie l'ordre
		assertTrue(tabs.get(0).equals(tableAnnotationDao.findById(9)));
		assertTrue(tabs.get(1).equals(tableAnnotationDao.findById(3)));
		assertTrue(tabs.get(2).equals(tableAnnotationDao.findById(4)));
		assertTrue(tabs.get(3).equals(tableAnnotationDao.findById(5)));
	...
	public void testFindByEntiteBanqueAndCatalogue() {
		...
		Banque b = banqueDao.findById(1);
		List<TableAnnotation> tabs = tableAnnotationDao
			.findByEntiteBanqueAndCatalogue(
				e, b, "INCa");
		assertTrue(tabs.size() == 2);
		// verfie l'ordre
		assertTrue(tabs.get(0).equals(tableAnnotationDao.findById(9)));
		assertTrue(tabs.get(1).equals(tableAnnotationDao.findById(3)));
		tabs = tableAnnotationDao.findByEntiteBanqueAndCatalogue(
				e, b, "tmp");
		assertTrue(tabs.size() == 0);
	...
	public void testFindByEntite() {
		...
		List<TableAnnotation> tabs = tableAnnotationDao
									.findByEntiteAndPlateforme(e, pf1);
		assertTrue(tabs.size() == 2);
		// verfie l'ordre
		// assertTrue(tabs.get(0).equals(tableAnnotationDao.findById(3)));
		assertTrue(tabs.get(0).equals(tableAnnotationDao.findById(4)));
		assertTrue(tabs.get(1).equals(tableAnnotationDao.findById(5)));
		e = entiteDao.findByNom("Patient").get(0);
	...
	public void testFindMaxOrdreForBanqueAndEntite() {
		Entite e = entiteDao.findByNom("Echantillon").get(0);
		Banque b = banqueDao.findById(1);
		List<Integer> maxOrdre = tableAnnotationDao
					.findMaxOrdreForBanqueAndEntite(e, b);
		assertNotNull(maxOrdre.get(0) == 4);
		e = entiteDao.findByNom("Patient").get(0);
		maxOrdre = tableAnnotationDao
						.findMaxOrdreForBanqueAndEntite(e, b);
		...
		maxOrdre = tableAnnotationDao
						.findMaxOrdreForBanqueAndEntite(null, b);
		assertNull(maxOrdre.get(0));
		maxOrdre = tableAnnotationDao
						.findMaxOrdreForBanqueAndEntite(e, null);
		assertNull(maxOrdre.get(0));
		maxOrdre = tableAnnotationDao
						.findMaxOrdreForBanqueAndEntite(null, null);
		assertNull(maxOrdre.get(0));
	}
	...
	public void testFindByPlateforme() {
		...
		List<TableAnnotation> tabs = 
							tableAnnotationDao.findByPlateforme(pf1);
	...
	public void testCrudTableAnnotation() {
		...
		Catalogue cINca = catalogueDao.findById(1);
		table.setCatalogue(cINca);
		TableAnnotationBanquePK pk1 = 
				new TableAnnotationBanquePK(banqueDao.findById(1), table);
		TableAnnotationBanque tab1 = new TableAnnotationBanque();
		tab1.setPk(pk1); tab1.setOrdre(1);
		TableAnnotationBanquePK pk2 = 
			new TableAnnotationBanquePK(banqueDao.findById(2), table);
		TableAnnotationBanque tab2 = new TableAnnotationBanque();
		tab2.setPk(pk2); tab2.setOrdre(2);
		...
		TableAnnotation table2 = tableAnnotationDao.findById(12);
		assertNotNull(table2);
		assertTrue(table2.getNom().equals("testTable"));
		assertTrue(table2.getDescription().equals("testTab descr"));
		assertTrue(table2.getEntite()
						.equals(entiteDao.findByNom("Conteneur").get(0)));
		...
		TableAnnotationBanquePK pk3 = 
			new TableAnnotationBanquePK(banqueDao.findById(3), table2);
		TableAnnotationBanque tab3 = new TableAnnotationBanque();
		tab3.setPk(pk3); tab3.setOrdre(1);
		table2.getTableAnnotationBanques()
					.add(tableAnnotationBanqueDao.mergeObject(tab1));
		table2.getTableAnnotationBanques()
					.add(tableAnnotationBanqueDao.mergeObject(tab2));
		table2.getTableAnnotationBanques()
					.add(tableAnnotationBanqueDao.mergeObject(tab3));
		...
		assertTrue(tableAnnotationDao.findById(12).getEntite()
							.equals(entiteDao.findByNom("Conteneur").get(0)));
	...
	public void testEqualsAndHashCode() {
		TableAnnotation t1 = new TableAnnotation();
		TableAnnotation t2 = new TableAnnotation();
	...
	public void testClone() {
		...
		TableAnnotation t2 = t.clone();
		assertTrue(t.equals(t2));
		if (t.getTableAnnotationId() != null) {
			assertTrue(t.getTableAnnotationId()
						.equals(t2.getTableAnnotationId()));
		} else {
		...
		if (t.getChampAnnotations() != null) {
			assertTrue(t.getChampAnnotations()
									.equals(t2.getChampAnnotations()));
		} else {
	...
	public void testFindByCatalogues() {
		...
		Catalogue c2 = catalogueDao.findById(2);
		catas.add(c2);
