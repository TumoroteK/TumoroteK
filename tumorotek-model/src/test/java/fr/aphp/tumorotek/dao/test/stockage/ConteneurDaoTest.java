...
/**
 * 
 * Classe de test pour le DAO ConteneurDao et le bean 
 * du domaine Conteneur.
 * 
...
public class ConteneurDaoTest extends AbstractDaoTest {
	...
	public void testReadAll() {
		List<Conteneur> liste = conteneurDao.findAll();
	...
	public void testFindByPlateformeOrigWithOrder() {
		...
		List<Conteneur> liste = conteneurDao
				.findByPlateformeOrigWithOrder(plateformeDao.findById(1));
		assertTrue(liste.size() == 3);
		assertTrue(liste.get(0).getCode().equals("CC1"));
		
		liste = conteneurDao
			.findByPlateformeOrigWithOrder(plateformeDao.findById(2));
		assertTrue(liste.size() == 1);
		
		liste = conteneurDao
				.findByPlateformeOrigWithOrder(plateformeDao.findById(3));
		assertTrue(liste.size() == 0);
	...
	public void testFindByBanqueIdWithExcludedId() {
		List<Conteneur> liste = conteneurDao
			.findByBanqueIdWithExcludedId(1, 1);
	...
	public void testCrud() throws Exception {
		...
		ConteneurType ct = conteneurTypeDao.findById(1);
		...
		Set<ConteneurPlateforme> plateformes = new HashSet<ConteneurPlateforme>();
		ConteneurPlateforme cp1 = new ConteneurPlateforme(c, p1);
		ConteneurPlateforme cp2 = new ConteneurPlateforme(c, p2);
		cp2.setPartage(true);
		plateformes.add(cp1); plateformes.add(cp2);
		String codeUp = "UP";
		Float temp = (float) -50.0;
		...
		// Test de l'insertion
		conteneurDao.createObject(c);
		assertEquals(new Integer(5), c.getConteneurId());
		
		Conteneur c2 = conteneurDao.findById(new Integer(5));
		...
		banques.add(b2);
		cp1.setPartage(true);
		conteneurDao.updateObject(c2);
		assertTrue(conteneurDao.findById(
				new Integer(5)).getCode().equals(codeUp));
		assertTrue(conteneurDao.findById(
				new Integer(5)).getBanques().size() == 2);
		assertTrue(conteneurDao.findById(
				new Integer(5)).getConteneurPlateformes().size() == 2);
		...
		// Test de la délétion
		conteneurDao.removeObject(new Integer(5));
		assertNull(conteneurDao.findById(new Integer(5)));
	...
	public void testClone() {
		...
		if (c1.getDescription() != null) {
			assertTrue(c1.getDescription()
					.equals(c2.getDescription()));
		} else {
			assertNull(c2.getDescription());
		}
		
		if (c1.getService() != null) {
			assertTrue(c1.getService()
					.equals(c2.getService()));
		} else {
			assertNull(c2.getService());
		}
				
		if (c1.getBanques() != null) {
			assertTrue(c1.getBanques()
					.equals(c2.getBanques()));
		} else {
			assertNull(c2.getBanques());
		}
		
		if (c1.getConteneurPlateformes() != null) {
			assertTrue(c1.getConteneurPlateformes()
					.equals(c2.getConteneurPlateformes()));
		} else {
			assertNull(c2.getConteneurPlateformes());
		}
		
		if (c1.getIncidents() != null) {
			assertTrue(c1.getIncidents()
					.equals(c2.getIncidents()));
		} else {
			assertNull(c2.getIncidents());
		}
		
		if (c1.getRetours() != null) {
			assertTrue(c1.getRetours()
					.equals(c2.getRetours()));
		} else {
	...
	public void testFindByPartage() {
		List<Conteneur> conts = conteneurDao.findByPartage(plateformeDao.findById(1), true);
		assertTrue(conts.size() == 1);
		assertTrue(conts.get(0).getCode().equals("CC1"));
		conts = conteneurDao.findByPartage(plateformeDao.findById(1), false);
		assertTrue(conts.size() == 2);
		conts = conteneurDao.findByPartage(plateformeDao.findById(2), true);
		assertTrue(conts.size() == 1);
		conts = conteneurDao.findByPartage(plateformeDao.findById(2), false);
		assertTrue(conts.size() == 1);
		conts = conteneurDao.findByPartage(plateformeDao.findById(3), false);
		assertTrue(conts.isEmpty());
		conts = conteneurDao.findByPartage(null, true);
		assertTrue(conts.isEmpty());
		conts = conteneurDao.findByPartage(plateformeDao.findById(1), null);
		assertTrue(conts.isEmpty());
	}
