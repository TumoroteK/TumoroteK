...
public class ChampEntiteDaoTest extends AbstractDaoTest  {
	...
	public void setImportTemplateDao(ImportTemplateDao itDao) {
		this.importTemplateDao = itDao;
	}

	public void testFindByEntiteAndNom() {
		Entite e1 = entiteDao.findById(1);
		List<ChampEntite> liste = champEntiteDao
			.findByEntiteAndNom(e1, "Nom");
	...
	public void testFindByEntiteAndImport() {
		Entite e1 = entiteDao.findById(1);
		List<ChampEntite> liste = champEntiteDao
			.findByEntiteAndImport(e1, false);
	...
	public void testFindByEntiteImportObligatoire() {
		Entite e1 = entiteDao.findById(1);
		List<ChampEntite> liste = champEntiteDao
			.findByEntiteImportObligatoire(e1, true, false);
		assertTrue(liste.size() == 5);
		
		liste = champEntiteDao.findByEntiteImportObligatoire(
				e1, true, true);
	...
	public void testCrudChamp() throws Exception {
		...
		Entite entite = this.entiteDao.findById(entiteId);
		DataType dataType = this.dataTypeDao.findById(dataTypeId);
	
		ChampEntite c = new ChampEntite();
		c.setEntite(entite);
		...
		List<ChampEntite> champEntites = this.champEntiteDao.findAll();
		Iterator<ChampEntite> itChampEntites = champEntites.iterator();
		boolean found = false;
		while (itChampEntites.hasNext()) {
			ChampEntite temp = itChampEntites.next();
			if (temp.equals(c)) {
				found = true;
		...
		ChampEntite c2 = this.champEntiteDao.findById(idObject);
		assertNotNull(c2);
		...
		Entite updatedEntite
			= this.entiteDao.findById(updatedEntiteId);
		DataType updatedDataType
			= this.dataTypeDao.findById(updatedDataTypeId);
		...
		assertTrue(this.champEntiteDao.findById(idObject).getEntite()
				.equals(updatedEntite));
		assertNotNull(this.champEntiteDao.findById(idObject).isNullable());
		assertTrue(this.champEntiteDao.findById(idObject).isNullable()
				.equals(updatedNullable));
		assertNotNull(this.champEntiteDao.findById(idObject).getDataType());
		assertTrue(this.champEntiteDao.findById(idObject).getDataType()
				.equals(updatedDataType));
		assertNotNull(this.champEntiteDao.findById(idObject).isUnique());
		assertTrue(this.champEntiteDao.findById(idObject).isUnique()
				.equals(updatedUnique));
		if (this.champEntiteDao.findById(idObject).getValeurDefaut() != null) {
			assertTrue(this.champEntiteDao.findById(idObject).getValeurDefaut()
					.equals(updatedValeurDefaut));
		} else {
			assertNull(updatedValeurDefaut);
		}
		assertNotNull(this.champEntiteDao.findById(idObject).getNom());
		assertTrue(this.champEntiteDao.findById(idObject).getNom()
				.equals(updatedNom));
		// Test de la délétion
	...
	public void testToString() {
		...
		ChampEntite c2 = new ChampEntite();
	...
	public void testEquals() {
		...
		for (int i = 0; i < Math.pow(2, 2); i++) {
			ChampEntite champ1 = new ChampEntite();
			ChampEntite champ2 = new ChampEntite();			
			Entite entite = null;
			if (i >= 2) {
	...
	public void testHashCode() {
		//On boucle sur les 4 possibilités
		for (int i = 0; i < Math.pow(2, 2); i++) {
			ChampEntite champ = new ChampEntite();			
			int hash = 7;
			Entite entite = null;
			int hashEntite = 0;
			if (i >= 2) {				
