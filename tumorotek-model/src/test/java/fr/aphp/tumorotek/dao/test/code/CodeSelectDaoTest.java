...
public class CodeSelectDaoTest extends AbstractDaoTest {
	...
	public void testReadAllCodes() {
		List<CodeSelect> codeSelects = codeSelectDao.findAll();
	...
	public void testFindByUtilisateurAndBanque() {
		...
		List<CodeSelect> codeSelects = 
			codeSelectDao.findByUtilisateurAndBanque(u, b);
	...
	public void testFindByCodeDossier() {
		...
		List<CodeSelect> codes 
			= codeSelectDao.findByCodeDossier(dos);
	...
	public void testFindByRootDossier() {
		...
		Utilisateur u = utilisateurDao.findById(1);
		List<CodeSelect> codes 
			= codeSelectDao.findByRootDossier(u, b);
		assertTrue(codes.size() == 1);
	}
	
	public void findByExcludedId() {
		CodeSelect c = codeSelectDao.findById(1);
		List<CodeSelect> codes 
			= codeSelectDao.findByExcludedId(c.getCodeSelectId());
	...
	public void testCrud() throws Exception {
		CodeSelect c = new CodeSelect();
		
		Utilisateur u = utilisateurDao.findById(2);
		Banque b = banqueDao.findById(1);
		TableCodage tc = tableCodesDao.findById(3);
		CodeDossier dos = codeDossierDao.findById(3);
		c.setUtilisateur(u);
		c.setBanque(b);
		c.setTableCodage(tc);
		...
		CodeSelect c2 = codeSelectDao.findById(new Integer(6));
		assertNotNull(c2);
	...
	public void testEquals() {
		...
		TableCodage t1 = tableCodesDao.findById(1);
		TableCodage t2 = tableCodesDao.findById(2);
		CodeSelect c1 = new CodeSelect();
	...
	public void testToString() {
		CodeSelect a = new CodeSelect();
		TableCodage t = tableCodesDao.findById(1);
		a.setTableCodage(t);
		a.setCodeId(33);
	...
	public void testFindByRootDossierAndBanque() {
		Banque b = banqueDao.findById(1);
		List<CodeSelect> codes 
			= codeSelectDao.findByRootDossierAndBanque(b);
		assertTrue(codes.size() == 1);
		b = banqueDao.findById(2);
