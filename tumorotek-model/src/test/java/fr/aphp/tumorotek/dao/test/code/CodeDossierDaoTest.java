...
public class CodeDossierDaoTest extends AbstractDaoTest {
	...
	public void testReadAllCodeDossiers() {
		List<CodeDossier> codes = codeDossierDao.findAll();
		assertTrue(codes.size() == 4);
	}
	
	public void testFindByNomLike() {
		Banque b = banqueDao.findById(1);
		List<CodeDossier> doss = codeDossierDao
											.findByNomLike("DossierU%", b);
	...
	public void findByCodeDossierParent() {
		...
		List<CodeDossier> doss 
			= codeDossierDao.findByCodeDossierParent(dos);
	...
	public void findByRootCodeDossierUtilisateur() {
		Banque b = banqueDao.findById(1);
		List<CodeDossier> doss 
			= codeDossierDao.findByRootCodeDossierUtilisateur(b);
		assertTrue(doss.size() == 2);
	}
	...
	public void findByRootCodeDossierSelect() {
		...
		List<CodeDossier> doss 
			= codeDossierDao.findByRootCodeDossierSelect(u, b);
		assertTrue(doss.size() == 1);
	...
	public void testFindBySelectUtilisateurAndBanque() {
		...
		Banque b = banqueDao.findById(1);
		List<CodeDossier> doss 
			= codeDossierDao.findBySelectUtilisateurAndBanque(u, b);
		assertTrue(doss.size() == 1);
		u = utilisateurDao.findById(2);
	...
	public void testFindByUtilisateurAndBanque() {
		...
		Banque b = banqueDao.findById(1);
		List<CodeDossier> doss 
			= codeDossierDao.findByUtilisateurAndBanque(u, b);
		assertTrue(doss.size() == 2);
		u = utilisateurDao.findById(2);
	...
	public void findByExcludedId() {
		CodeDossier c = codeDossierDao.findById(1);
		List<CodeDossier> doss 
			= codeDossierDao.findByExcludedId(c.getCodeDossierId());
	...
	public void testCrud() throws Exception {
		...
		CodeDossier c2 = codeDossierDao.findById(new Integer(5));
		assertNotNull(c2);
		...
		assertTrue(codeDossierDao.
				findById(new Integer(5)).getNom() == "update");
	...
	public void testEqualsAndHashCode() {
		CodeDossier c1 = new CodeDossier();
		CodeDossier c2 = new CodeDossier();
	...
	public void testFindByRootDossierBanque() {
		Banque b = banqueDao.findById(1);
		List<CodeDossier> doss 
			= codeDossierDao.findByRootDossierBanque(b, true);
		assertTrue(doss.size() == 1);
		assertTrue(doss.get(0).getCodeDossierId() == 3);
