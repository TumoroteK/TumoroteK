...
public class PrelevementDaoTest extends AbstractDaoTest {
	...
	public void testFindAllPrelevements() {
		List<Prelevement> prels = prelevementDao.findAll();
	...
	public void testFindByCodeOrNumLaboWithBanque() {
		...
		List<Prelevement> prels = 
			prelevementDao.findByCodeOrNumLaboWithBanque("PRLVT1", b1);
		assertTrue(prels.size() == 1);
		prels = 
			prelevementDao.findByCodeOrNumLaboWithBanque("PRLVT1", null);
		assertTrue(prels.size() == 0);
		...
		prels = 
			prelevementDao.findByCodeOrNumLaboWithBanque("12234", b1);
		assertTrue(prels.size() == 1);
	...
	public void testFindByCodeInPlateforme() {
		...
		List<Prelevement> prels = 
			prelevementDao.findByCodeInPlateforme("PRLVT1", p1);
		assertTrue(prels.size() == 1);
		prels = 
			prelevementDao.findByCodeInPlateforme("PRLVT1", null);
		assertTrue(prels.size() == 0);
		...
		prels = 
			prelevementDao.findByCodeInPlateforme("12234", p1);
		assertTrue(prels.size() == 0);
		prels = prelevementDao.findByCodeInPlateforme("PTRA", p1);
		assertTrue(prels.size() == 0);
	...
	public void testFindByCodeOrNumLaboWithBanqueReturnIds() {
		Banque b1 = banqueDao.findById(1);
		List<Integer> prels = 
			prelevementDao.findByCodeOrNumLaboWithBanqueReturnIds("PRLVT1", b1);
		assertTrue(prels.size() == 1);
		prels = 
			prelevementDao.findByCodeOrNumLaboWithBanqueReturnIds(
					"PRLVT1", null);
		assertTrue(prels.size() == 0);
		prels = prelevementDao.findByCodeOrNumLaboWithBanqueReturnIds(
				"PTRA", b1);
		assertTrue(prels.size() == 0);
		prels = prelevementDao.findByCodeOrNumLaboWithBanqueReturnIds(
				"PTRA", null);
		...
		prels = 
			prelevementDao.findByCodeOrNumLaboWithBanqueReturnIds("12234", b1);
		assertTrue(prels.size() == 1);
		prels = prelevementDao.findByCodeOrNumLaboWithBanqueReturnIds(
				"PTRA", b1);
		assertTrue(prels.size() == 0);
	...
	public void testToString() {
		...
		assertTrue(p1.entiteNom()
				.equals(entiteDao.findByNom("Prelevement").get(0).getNom()));
		p1 = new Prelevement();
	...
	public void testFindByDatePrelevementAfterDate() throws Exception {
		...
		sDate.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("07/09/1983 12:12:23"));
		List<Prelevement> prels = prelevementDao.
								findByDatePrelevementAfterDate(sDate);
		assertTrue(prels.size() == 2);
		sDate.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("15/09/2009 12:12:23"));
	 ...
	 * throws Exception Lance une exception en cas d'erreur.
	 */
	public void testFindByDatePrelevementAfterDateWithBanque() 
	throws Exception {
		Banque b1 = banqueDao.findById(1);
		Calendar sDate = Calendar.getInstance();
		sDate.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("07/09/1983 00:00:01"));
		List<Prelevement> prels = prelevementDao.
			findByDatePrelevementAfterDateWithBanque(sDate, b1);
		assertTrue(prels.size() == 1);
		sDate.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("15/09/2009 00:00:01"));
		prels = prelevementDao.findByDatePrelevementAfterDateWithBanque(
																sDate, b1);
		assertTrue(prels.size() == 0);
		prels = prelevementDao.findByDatePrelevementAfterDateWithBanque(
				null, b1);
		...
		sDate.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("07/09/1983 00:00:01"));
		prels = prelevementDao.
			findByDatePrelevementAfterDateWithBanque(sDate, null);
		assertTrue(prels.size() == 0);
		prels = prelevementDao.findByDatePrelevementAfterDateWithBanque(
				null, null);
		assertTrue(prels.size() == 0);
	...
	public void testFindByConsentDateAfterDate() throws Exception {
		...
		List<Prelevement> prels = prelevementDao.
		findByConsentDateAfterDate(search);
		assertTrue(prels.size() == 1);
...
//	
	...
	public void testFindByMaladieLibelleLike() {
		List<Prelevement> prels = prelevementDao
			.findByMaladieLibelleLike("Addiction coco");
		assertTrue(prels.size() == 1);
	...
	public void testFindByBanqueSelectNda() {
		Banque b1 = banqueDao.findById(1);
		List<String> ndas = prelevementDao.findByBanqueSelectNda(b1);
	...
	public void testFindByMaladieAndOtherBanques() {
		...
		Maladie m1 = maladieDao.findById(4);
		List<Prelevement> prels = prelevementDao
										.findByMaladieAndOtherBanques(m1, b1);
		assertTrue(prels.size() == 1);
	...
	public void testFindByPatientNomNip() {
		Banque b1 = banqueDao.findById(1);
		List<Integer> prels = prelevementDao
			.findByPatientNomReturnIds("DELPHINO", b1);
		assertTrue(prels.size() == 2);
	...
	public void testFindByCodesAndBanquesInList() {
		...
		List<Prelevement> liste = prelevementDao
			.findByCodesAndBanquesInList(codes, bks);
		assertTrue(liste.size() == 3);
	...
	public void testFindByPatientNomOrNipInList() {
		...
		List<Integer> liste = prelevementDao.findByPatientNomOrNipInList(
				criteres, bks);
		assertTrue(liste.size() == 3);
		
		bks.add(banqueDao.findById(3));
		liste = prelevementDao.findByPatientNomOrNipInList(
				criteres, bks);
	...
	public void testFindByCodeOrNumLaboInListWithBanque() {
		...
		List<Integer> liste = prelevementDao
		.findByCodeOrNumLaboInListWithBanque(
				criteres, bks);
		assertTrue(liste.size() == 2);
		
		bks.add(banqueDao.findById(3));
		liste = prelevementDao.findByCodeOrNumLaboInListWithBanque(
				criteres, bks);
		assertTrue(liste.size() == 3);
	...
	public void testCrudPrelevement() throws Exception {
		...
		Risque r2 = risqueDao.findById(2); risks.add(r2);
		Risque r3 = risqueDao.findById(3); risks.add(r3);
		p.setRisques(risks);		
		
		// Test de l'insertion
		prelevementDao.createObject(p);
		Integer prelId = p.getPrelevementId();
		assertNotNull(prelId);
		
		// Test de la mise à jour
		Prelevement p2 = prelevementDao.findById(prelId);
		assertNotNull(p2);
		assertTrue(p2.getBanque().equals(b));
		Banque b2 = banqueDao.findById(3);
		p2.setBanque(b2);
		assertTrue(p2.getCode() == "pre1234");
		p2.setCode("pre55");
		assertTrue(p2.getNature().equals(n));
		Nature n2 = natureDao.findById(2);
		p2.setNature(n2);
		assertTrue(p2.getConsentType().equals(ct));
		ConsentType ct2 = consentTypeDao.findById(1);
		p2.setConsentType(ct2);
		assertTrue(p2.getMaladie().equals(m));
		Maladie m2 = maladieDao.findById(2);
		p2.setMaladie(m2);
		assertTrue(p2.getConsentDate() == null);
		Date ctDate2 = new SimpleDateFormat("dd/MM/yyyy").parse("12/10/2005");
		p2.setConsentDate(ctDate2);
		assertTrue(p2.getPreleveur().equals(preleveur));
		Collaborateur preleveur2 = collaborateurDao.findById(2);
		p2.setPreleveur(preleveur2);
		assertTrue(p2.getServicePreleveur().equals(s));
		Service s2 = serviceDao.findById(3);
		...
		PrelevementType ptype2 = prelevementTypeDao.findById(2);
		p2.setPrelevementType(ptype2);
		assertTrue(p2.getConditType().equals(cType));
		ConditType cType2 = conditTypeDao.findById(2);
		p2.setConditType(cType2);
		assertTrue(p2.getConditMilieu().equals(milieu));
		ConditMilieu milieu2 = conditMilieuDao.findById(2);
		p2.setConditMilieu(milieu2);
		assertTrue(p2.getConditNbr() == 10);
		p2.setConditNbr(11);
		assertTrue(p2.getDateDepart().equals(depDate));
		Calendar depDate2 = Calendar.getInstance();
		depDate2.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("14/10/2005 12:12:23"));
		p2.setDateDepart(depDate2);
		assertTrue(p2.getTransporteur().equals(t));
		Transporteur t2 = transporteurDao.findById(2);
		...
		Calendar arrDate2 = Calendar.getInstance();
		arrDate2.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("15/10/2005 12:12:23"));
		p2.setDateArrivee(arrDate2);
		assertTrue(p2.getOperateur().equals(op));
		Collaborateur op2 = collaborateurDao.findById(3);
		p2.setOperateur(op2);
		assertNull(p2.getQuantite());
		assertTrue(p2.getQuantiteUnite().equals(qU));
		List<Unite> unites  = uniteDao.findByUnite("µg");
		Unite qU2 = unites.get(0);
		...
		p2.getRisques().clear();
		Risque r1 = risqueDao.findById(1); p2.getRisques().add(r1);
		p2.setQuantite((float) 12.55544);
		
		//test l'update
		prelevementDao.updateObject(p2);
		assertTrue(prelevementDao.findById(
				prelId).getBanque().equals(b2));
		assertTrue(prelevementDao.findById(
				prelId).getCode() == "pre55");
		assertTrue(prelevementDao.findById(
				prelId).getNature().equals(n2));
		assertTrue(prelevementDao.findById(
				prelId).getConsentType().equals(ct2));
		assertTrue(prelevementDao.findById(
				prelId).getMaladie().equals(m2));
		assertTrue(prelevementDao.findById(
				prelId).getConsentDate().equals(ctDate2));
		assertTrue(prelevementDao.findById(
				prelId).getPreleveur().equals(preleveur2));
		assertTrue(prelevementDao.findById(
				prelId).getServicePreleveur().equals(s2));
		assertTrue(prelevementDao.findById(
				prelId).getDatePrelevement() == null);
		assertTrue(prelevementDao.findById(
				prelId).getPrelevementType().equals(ptype2));
		assertTrue(prelevementDao.findById(
				prelId).getConditType().equals(cType2));
		assertTrue(prelevementDao.findById(
				prelId).getConditMilieu().equals(milieu2));
		assertTrue(prelevementDao.findById(
				prelId).getConditNbr() == 11);
		assertTrue(prelevementDao.findById(
				prelId).getDateDepart().equals(depDate2));
		assertTrue(prelevementDao.findById(
				prelId).getTransporteur().equals(t2));
		assertTrue(prelevementDao.findById(
				prelId).getTransportTemp().equals(new Float(-10.5)));
		assertTrue(prelevementDao.findById(
				prelId).getDateArrivee().equals(arrDate2));
		assertTrue(prelevementDao.findById(
				prelId).getOperateur().equals(op2));
		assertTrue(prelevementDao.findById(
				prelId).getQuantiteUnite().equals(qU2));
		assertTrue(prelevementDao.findById(
				prelId).getPatientNda() == "NDA2");
		assertTrue(prelevementDao.findById(
				prelId).getNumeroLabo() == "12345");
		assertFalse(prelevementDao.findById(
				prelId).getSterile());
		assertTrue(prelevementDao.findById(
				prelId).getCongArrivee());
		assertTrue(prelevementDao.findById(
				prelId).getConformeArrivee());
		assertFalse(prelevementDao.findById(
				prelId).getCongDepart());
		assertTrue(prelevementDao.findById(
				prelId).getEtatIncomplet());
		assertTrue(prelevementDao.findById(
				prelId).getArchive());
	...
	public void testEquals() {
		...
		Banque b1 = banqueDao.findById(1);
		Banque b2 = banqueDao.findById(2);
		Banque b3 = new Banque();
		b3.setNom(b1.getNom());
		b3.setPlateforme(b1.getPlateforme());
		assertTrue(b1.equals(b3));
		assertFalse(b1 == b3);
		Banque[] banques = new Banque[]{null, b1, b2, b3};
		String[] codes = new String[]{null, "code1", "code2", ""};
	...
	public void testHashCode() {
		...
		Banque b1 = banqueDao.findById(1);
		Banque b2 = banqueDao.findById(2);
		Banque[] banques = new Banque[]{null, b1, b2};
		String[] codes = new String[]{null, "code1", "code2", ""};
	...
	public void testClone() {
		Prelevement p1 = prelevementDao.findById(1);
		Prelevement p2 = p1.clone();
		assertTrue(p1.equals(p2));
		
		if (p1.getPrelevementId() != null) {
			assertTrue(p1.getPrelevementId()
					.equals(p2.getPrelevementId()));
		} else {
			assertNull(p2.getPrelevementId());
		}
		if (p1.getBanque() != null) {
			assertTrue(p1.getBanque()
					.equals(p2.getBanque()));
		} else {
			assertNull(p2.getBanque());
		}
		if (p1.getCode() != null) {
			assertTrue(p1.getCode()
					.equals(p2.getCode()));
		} else {
			assertNull(p2.getCode());
		}
		if (p1.getNature() != null) {
			assertTrue(p1.getNature()
					.equals(p2.getNature()));
		} else {
			assertNull(p2.getNature());
		}
		if (p1.getMaladie() != null) {
			Maladie m1 = p1.getMaladie();
			Maladie m2 = p2.getMaladie();
			assertTrue(m1.getLibelle().equals(m2.getLibelle())
					&& m1.getPatient().equals(m2.getPatient()));
		} else {
			assertNull(p2.getMaladie());
		}
		if (p1.getConsentType() != null) {
			assertTrue(p1.getConsentType()
					.equals(p2.getConsentType()));
		} else {
			assertNull(p2.getConsentType());
		}
		if (p1.getConsentDate() != null) {
			assertTrue(p1.getConsentDate()
					.equals(p2.getConsentDate()));
		} else {
			assertNull(p2.getConsentDate());
		}
		if (p1.getPreleveur() != null) {
			assertTrue(p1.getPreleveur()
					.equals(p2.getPreleveur()));
		} else {
			assertNull(p2.getPreleveur());
		}
		if (p1.getServicePreleveur() != null) {
			assertTrue(p1.getServicePreleveur()
					.equals(p2.getServicePreleveur()));
		} else {
			assertNull(p2.getServicePreleveur());
		}
		if (p1.getDatePrelevement() != null) {
			assertTrue(p1.getDatePrelevement()
					.equals(p2.getDatePrelevement()));
		} else {
			assertNull(p2.getDatePrelevement());
		}
		if (p1.getPrelevementType() != null) {
			assertTrue(p1.getPrelevementType()
					.equals(p2.getPrelevementType()));
		} else {
			assertNull(p2.getPrelevementType());
		}
		if (p1.getConditType() != null) {
			assertTrue(p1.getConditType()
					.equals(p2.getConditType()));
		} else {
			assertNull(p2.getConditType());
		}
		if (p1.getConditMilieu() != null) {
			assertTrue(p1.getConditMilieu()
					.equals(p2.getConditMilieu()));
		} else {
			assertNull(p2.getConditMilieu());
		}
		if (p1.getConditNbr() != null) {
			assertTrue(p1.getConditNbr()
					.equals(p2.getConditNbr()));
		} else {
			assertNull(p2.getConditNbr());
		}
		if (p1.getDateDepart() != null) {
			assertTrue(p1.getDateDepart()
					.equals(p2.getDateDepart()));
		} else {
			assertNull(p2.getDateDepart());
		}
		if (p1.getTransporteur() != null) {
			assertTrue(p1.getTransporteur()
					.equals(p2.getTransporteur()));
		} else {
			assertNull(p2.getTransporteur());
		}
		if (p1.getTransportTemp() != null) {
			assertTrue(p1.getTransportTemp()
					.equals(p2.getTransportTemp()));
		} else {
			assertNull(p2.getTransportTemp());
		}
		if (p1.getDateArrivee() != null) {
			assertTrue(p1.getDateArrivee()
					.equals(p2.getDateArrivee()));
		} else {
			assertNull(p2.getDateArrivee());
		}
		if (p1.getOperateur() != null) {
			assertTrue(p1.getOperateur()
					.equals(p2.getOperateur()));
		} else {
			assertNull(p2.getOperateur());
		}
		if (p1.getQuantite() != null) {
			assertTrue(p1.getQuantite()
					.equals(p2.getQuantite()));
		} else {
			assertNull(p2.getQuantite());
		}
		if (p1.getQuantiteUnite() != null) {
			assertTrue(p1.getQuantiteUnite()
					.equals(p2.getQuantiteUnite()));
		} else {
		...
		if (p1.getPatientNda() != null) {
			assertTrue(p1.getPatientNda()
					.equals(p2.getPatientNda()));
		} else {
			assertNull(p2.getPatientNda());
		}
		if (p1.getNumeroLabo() != null) {
			assertTrue(p1.getNumeroLabo()
					.equals(p2.getNumeroLabo()));
		} else {
			assertNull(p2.getNumeroLabo());
		}
		if (p1.getSterile() != null) {
			assertTrue(p1.getSterile()
					.equals(p2.getSterile()));
		} else {
		...
		if (p1.getConformeArrivee() != null) {
			assertTrue(p1.getConformeArrivee()
					.equals(p2.getConformeArrivee()));
		} else {
			assertNull(p2.getConformeArrivee());
		}
		if (p1.getEtatIncomplet() != null) {
			assertTrue(p1.getEtatIncomplet()
					.equals(p2.getEtatIncomplet()));
		} else {
			assertNull(p2.getEtatIncomplet());
		}
		if (p1.getArchive() != null) {
			assertTrue(p1.getArchive()
					.equals(p2.getArchive()));
		} else {
	...
	public void testFindByBanques() {
		...
		List<Prelevement> res = prelevementDao.findByBanques(banks);
	...
	public void testFindByBanquesAllIds() {
		...
		banks.add(banqueDao.findById(1));
		banks.add(banqueDao.findById(2));
		List<Integer> res = prelevementDao.findByBanquesAllIds(banks);
		assertTrue(res.size() == 4);
	}
	...
	public void testFindCountEclConsentByDates() throws ParseException {
		...
		List<Long> res = prelevementDao
					.findCountEclConsentByDates(cTypes, d1, d2, banks);
	...
	public void testFindCountByOrganeByDates() throws ParseException {
		...
		List<Prelevement> res = prelevementDao
							.findByOrganeByDates(codes, d1, d2, banks);
	...
	public void testFindCountByOrganeByDatesConsent() throws ParseException {
		...
		List<Prelevement> res = prelevementDao
			.findByOrganeByDatesConsent(codes, d1, d2, banks, cTypes);
		assertTrue(res.size() == 0);
	...
	public void testFindAssociatePrelsOfType() {
		...
		List<Nature> nats = new ArrayList<Nature>();
		nats.add(natureDao.findById(2));
		
		Maladie m4 = maladieDao.findById(4);
		
		List<Prelevement> res = 
			prelevementDao.findAssociatePrelsOfType(m4, nats, banks);
		assertTrue(res.size() == 1);
		...
		Maladie m3 = maladieDao.findById(3);
	...
	public void testFindByMaladieAndNature() throws ParseException {
		...
		cal.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
									.parse("06/09/1983 10:00:00"));
		List<Prelevement> prels = prelevementDao
							.findByMaladieAndNature(m4, "SANG", cal);
		assertTrue(prels.size() == 2);
	...
	public void testFindByEtablissementNom() {
		List<Banque> banks = banqueDao.findByNom("BANQUE1");
		List<Prelevement> prels = prelevementDao
			.findByEtablissementNom("%SAINT%", banks);
