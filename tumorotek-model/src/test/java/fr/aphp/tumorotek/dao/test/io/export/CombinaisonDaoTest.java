...
public class CombinaisonDaoTest extends AbstractDaoTest {
	 ...
	 */
	public void setCombinaisonDao(CombinaisonDao oDao) {
	...
	public void testCrudCombinaison() throws Exception {
		...
		ChampEntite chEntite = new ChampEntite(this.entiteDao.findById(2),
		...
		chEntite = new ChampEntite(this.entiteDao.findById(1), "champEntite2",
		...
		Champ champ1 = this.champDao.findById(id1);		
		Champ champ2 = this.champDao.findById(id2);
		...
		List<Combinaison> combinaisons = this.combinaisonDao.findAll();
		Iterator<Combinaison> itCombinaison = combinaisons.iterator();
		boolean found = false;
		while (itCombinaison.hasNext()) {
			Combinaison temp = itCombinaison.next();
			if (temp.equals(c)) {
				found = true;
		...
		Combinaison c2 = this.combinaisonDao.findById(idObject);
		assertNotNull(c2);
		...
		Champ updatedChamp1 = this.champDao.findById(id2);
		Champ updatedChamp2 = this.champDao.findById(id1);
		String updatedOperateur = "-";
		...
		assertTrue(this.combinaisonDao.findById(idObject).getChamp1()
				.equals(updatedChamp1));
		assertNotNull(this.combinaisonDao.findById(idObject).getChamp2());
		assertTrue(this.combinaisonDao.findById(idObject).getChamp2()
				.equals(updatedChamp2));
		assertNotNull(this.combinaisonDao.findById(idObject).getOperateur());
		assertTrue(this.combinaisonDao.findById(idObject).getOperateur()
				.equals(updatedOperateur));
	...
	public void testToString() {
		Combinaison c1 = combinaisonDao.findById(1);
		if (c1.getChamp1() != null && c1.getChamp2() != null
				&& c1.getOperateur() != null) {
			assertTrue(c1.toString().equals(
					c1.getChamp1().toString() + " " + c1.getOperateur() + " "
							+ c1.getChamp2()));
		} else {
