...
public class AffichageDaoTest extends AbstractDaoTest  {
    ...
    public void testFindAffichagesByUtilisateur() throws Exception {
		...
		while (itUtil.hasNext()) {
			...
			List<Affichage> affichages = this.affichageDao
    ...
    public void testFindByIntituleUtilisateur() throws Exception {
		...
		List<Affichage> liste = affichageDao
			.findByIntituleUtilisateur("Essen%", u1);
		assertTrue(liste.size() == 1);
	...
	public void testFindByExcludedId() {
		List<Affichage> liste = affichageDao.findAll();
		Iterator<Affichage> it = liste.iterator();
		while (it.hasNext()) {
			Affichage temp = it.next();
			List<Affichage> affichages = 
				affichageDao.findByExcludedId(temp.getAffichageId());
	...
	public void testCrudAffichage() throws Exception {
		...
		Iterator<Affichage> itAffichage = affichages.iterator();
		boolean found = false;
		while (itAffichage.hasNext()) {
			Affichage temp = itAffichage.next();
			if (temp.equals(a)) {
				found = true;
		...
		Affichage a2 = this.affichageDao.findByIntitule("intitule").get(0);
		assertNotNull(a2);
		...
		assertNotNull(this.affichageDao.findById(idObject).getIntitule());
		assertTrue(this.affichageDao.findById(idObject).getIntitule()
				.equals(updatedIntitule));
		assertNotNull(this.affichageDao.findById(idObject).getCreateur());
		assertTrue(this.affichageDao.findById(idObject).getCreateur()
				.equals(updatedCreateur));
		assertNotNull(this.affichageDao.findById(idObject).getNbLignes());
		assertTrue(this.affichageDao.findById(idObject).getNbLignes()
				.equals(updatedNbLignes));
		assertNotNull(this.affichageDao.findById(idObject).getBanque());
		assertTrue(this.affichageDao.findById(idObject).getBanque()
	...
	public void testToString() {
		Affichage a1 = affichageDao.findById(1);
	...
	public void testEquals() {
		...
		for (int i = 0; i < Math.pow(2, 2); i++) {
			Affichage affichage1 = new Affichage();
			Affichage affichage2 = new Affichage();			
