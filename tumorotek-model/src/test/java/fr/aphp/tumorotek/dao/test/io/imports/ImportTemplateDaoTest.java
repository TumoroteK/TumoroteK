...
/**
 *
 * Classe de test pour le DAO ImportTemplateDao et le bean du domaine
 * ImportTemplate. Classe créée le 24/01/2011.
 *
...
public class ImportTemplateDaoTest extends AbstractDaoTest {
	...
	public void testCrud() throws Exception {
		...
		final ImportTemplate it2 = importTemplateDao.findById(tpId).get();
		assertNotNull(it2);
		...
		assertTrue(importTemplateDao.findById(tpId).get().getNom().equals(nomUp));
		assertTrue(importTemplateDao.findById(tpId).get().getEntites().size() == 3);
		assertTrue(it2.getDeriveParentEntite().equals(e3));

		// Test de la délétion
		importTemplateDao.deleteById(new Integer(5));
		assertFalse(importTemplateDao.findById(new Integer(5)).isPresent());
	}
