...
public class LaboInterDaoTest extends AbstractDaoTest {
	...
	private Integer ordreUpdated = 2;
	private Boolean sterileUpdated = false;
	private Float conserTempUpdated = new Float(25.0);
	private Float transportTempUpdated = new Float(25.0);
	 ...
	 */
	public void setLaboInterDao(LaboInterDao lDao) {
	...
	public void testReadAllLabos() {
		List<LaboInter> labos = laboInterDao.findAll();
	...
	public void testToString() {
		...
		assertTrue(l1.toString().equals("{" + l1.getPrelevement().getCode() 
				+ ", " + l1.getOrdre().toString() + "}"));
		l1 = new LaboInter();
	...
	public void testFindByPrelevementWithOnlyOrder() {
		Prelevement p = prelevementDao.findById(1);
		List<Integer> labos = laboInterDao
			.findByPrelevementWithOnlyOrder(p);
		assertTrue(labos.size() == 3);
	...
	public void testFindByPrelevementWithOnlyOrderAndExcludedId() {
		Prelevement p = prelevementDao.findById(1);
		List<Integer> labos = laboInterDao
			.findByPrelevementWithOnlyOrderAndExcludedId(p, 1);
		assertTrue(labos.size() == 2);
	...
	public void testCrudLaboInter() throws Exception {
		LaboInter l = new LaboInter();
		Prelevement p = prelevementDao.findById(2);
		l.setPrelevement(p);
		l.setOrdre(1);
		Service s = serviceDao.findById(3);
		l.setService(s);
		...
		LaboInter l2 = laboInterDao.findById(new Integer(4));
		assertNotNull(l2);
		...
		Calendar dateDepartUpdated = Calendar.getInstance();
		dateDepartUpdated.setTime(new Date(5000));
		Calendar dateArriveeUpdated = Calendar.getInstance();
		...
		Service s2 = serviceDao.findById(4);
		l2.setService(s2);
		...
		assertTrue(laboInterDao.findById(new Integer(4)).getPrelevement()
				.equals(p2));
		assertTrue(laboInterDao.findById(new Integer(4))
				.getOrdre() == ordreUpdated);
		assertTrue(laboInterDao.findById(new Integer(4)).getService()
				.equals(s2));
		assertTrue(laboInterDao.findById(new Integer(4))
				.getDateArrivee().equals(dateArriveeUpdated));
		assertTrue(laboInterDao.findById(new Integer(4))
				.getConservTemp().equals(conserTempUpdated));
		assertFalse(laboInterDao.findById(new Integer(4)).getSterile());
		assertTrue(laboInterDao.findById(new Integer(4)).getCongelation());
		assertTrue(laboInterDao.findById(new Integer(4))
				.getTransportTemp().equals(transportTempUpdated));
		assertTrue(laboInterDao.findById(new Integer(4))
				.getDateDepart().equals(dateDepartUpdated));
		assertTrue(laboInterDao.findById(new Integer(4)).getCollaborateur()
				.equals(c2));
		assertTrue(laboInterDao.findById(new Integer(4)).getTransporteur()
				.equals(t2));
	...
	public void testEquals() {
		...
		Prelevement p3 = prelevementDao.findById(1);
		assertTrue(p1.equals(p3));
		Prelevement[] prelevements = new Prelevement[]{null, p1, p2, p3};
		Integer[] ordres = new Integer[]{null, 1, 2};
	...
	public void testHashCode() {
		...
		LaboInter l2 = new LaboInter();
		l2.setLaboInterId(2);
		LaboInter t3 = new LaboInter();
		l1.setLaboInterId(3);
		assertTrue(t3.hashCode() > 0);
		
		Prelevement p1 = prelevementDao.findById(1);
		Prelevement p2 = prelevementDao.findById(3);
		Prelevement[] prelevements = new Prelevement[]{null, p1, p2};
		Integer[] ordres = new Integer[]{null, 1, 2};
