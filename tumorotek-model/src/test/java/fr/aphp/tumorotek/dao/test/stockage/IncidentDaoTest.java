...
public class IncidentDaoTest extends AbstractDaoTest {
	...
	public void testReadAlls() {
		List<Incident> liste = incidentDao.findAll();
	...
	public void testCrud() throws Exception {
		...
		i.setDate(d1);
		// Test de l'insertion
		incidentDao.createObject(i);
		
		Integer iId = i.getIncidentId();
		
		// Test de la mise à jour
		Incident ct2 = incidentDao.findById(iId);
		assertNotNull(ct2);
		assertTrue(ct2.getDate().equals(d1));
		ct2.setNom(updatedNom);
		incidentDao.updateObject(ct2);
		assertTrue(incidentDao.findById(iId).
				getNom().equals(updatedNom));
		
		// Test de la délétion
		incidentDao.removeObject(iId);
		assertNull(incidentDao.findById(iId));
	...
	public void testClone() {
		Incident incident1 = incidentDao.findById(1);
		...
		if (incident1.getDate() != null) {
			assertTrue(incident1.getDate()
					.equals(incident2.getDate()));
		} else {
			assertNull(incident2.getDate());
		}
		
		if (incident1.getDescription() != null) {
			assertTrue(incident1.getDescription()
					.equals(incident2.getDescription()));
		} else {
			assertNull(incident2.getDescription());
		}
		
		if (incident1.getConteneur() != null) {
			assertTrue(incident1.getConteneur()
					.equals(incident2.getConteneur()));
		} else {
