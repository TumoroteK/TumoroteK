...
public class AdicapDaoTest extends AbstractDaoTest {
	...
	public void testReadAllAdicaps() {
		List<Adicap> adicaps = adicapDao.findAll();
	...
	public void testFindByTopoParent() {
		Adicap parent = adicapDao.findById(42);
		List<Adicap> adicaps = adicapDao
						.findByAdicapParentAndCodeOrLibelle(parent, "%");
		assertTrue(adicaps.size() == 8);
		assertTrue(adicapDao
				.findByAdicapParentAndCodeOrLibelle(adicaps.get(7), "%")
				.size() == 6);
		assertTrue(adicapDao
				.findByAdicapParentAndCodeOrLibelle(adicaps.get(7), "")
				.isEmpty());
		adicaps = adicapDao
				.findByAdicapParentAndCodeOrLibelle(parent, "PHARYNX");
		assertTrue(adicaps.size() == 1);
		adicaps = adicapDao
				.findByAdicapParentAndCodeOrLibelle(parent, "%PHARYNX%");
		assertTrue(adicaps.size() == 4);
		Adicap parent2 = adicapDao.findById(2);
		adicaps = adicapDao.findByAdicapParentAndCodeOrLibelle(parent2,"%");
		assertTrue(adicaps.size() == 0);
		assertTrue(adicapDao
				.findByAdicapParentAndCodeOrLibelle(null, "%")
				.isEmpty());
		assertTrue(adicapDao
				.findByAdicapParentAndCodeOrLibelle(parent, null)
				.isEmpty());
	}
	
	public void testFindByAdicapGroupeAndCodeOrLibelle() {
		List<Adicap> adicaps = adicapDao
			.findByAdicapGroupeAndCodeOrLibelle(adicapGroupeDao.findById(2), "%");
		assertTrue(adicaps.size() == 22);
		adicaps = adicapDao
				.findByAdicapGroupeAndCodeOrLibelle(adicapGroupeDao.findById(2), "%CYTO%");
		assertTrue(adicaps.size() == 4);
		adicaps = adicapDao
				.findByAdicapGroupeAndCodeOrLibelle(adicapGroupeDao.findById(2), "K");
		assertTrue(adicaps.size() == 1);
		assertTrue(adicapDao
				.findByAdicapGroupeAndCodeOrLibelle(adicapGroupeDao.findById(2), "")
				.isEmpty());
		assertTrue(adicapDao
				.findByAdicapGroupeAndCodeOrLibelle(null, "")
				.isEmpty());
		assertTrue(adicapDao
				.findByAdicapGroupeAndCodeOrLibelle(adicapGroupeDao.findById(2), null)
				.isEmpty());
