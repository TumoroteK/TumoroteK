...
public class EchantillonDaoTest extends AbstractDaoTest {
	...
	public void setPlateformeDao(PlateformeDao p) {
		this.plateformeDao = p;
	}

	public void testReadAllCategories() {
		List<Echantillon> echans = echantillonDao.findAll();
	...
	public void testFindByBanques() {
		...
		List<Echantillon> res = echantillonDao.findByBanques(banks);
	...
	public void testFindByBanquesAllIds() {
		...
		banks.add(banqueDao.findById(1));
		banks.add(banqueDao.findById(2));
		List<Integer> res = echantillonDao.findByBanquesAllIds(banks);
		assertTrue(res.size() == 4);
	}
	...
	public void testFindByCodeWithBanque() {
		...
		List<Echantillon> echans = echantillonDao
								.findByCodeWithBanque("EHT.1", b1);
		assertTrue(echans.size() == 1);
	...
	public void testFindByCodeInPlateforme() {
		...
		List<Echantillon> echans = 
			echantillonDao.findByCodeInPlateforme("PTRA.1", p1);
		assertTrue(echans.size() == 1);
	...
	public void testFindByCodeWithBanqueReturnIds() {
		Banque b1 = banqueDao.findById(1);
		Banque b2 = banqueDao.findById(2);
		List<Integer> echans = echantillonDao
								.findByCodeWithBanqueReturnIds("EHT.1", b1);
		assertTrue(echans.size() == 1);
	...
	public void testFindByDateStockAfterDate() throws Exception {
		...
		search.setTime(new SimpleDateFormat("dd/MM/yyyy")
													.parse("15/01/2009"));
		List<Echantillon> echans = echantillonDao.
	...
	public void testFindByDateStockAfterDateWithBanque() throws Exception {
		...
		List<Echantillon> echans = echantillonDao.
			findByDateStockAfterDateWithBanque(search, b1);
		assertTrue(echans.size() == 2);
		...
		echans = echantillonDao.
			findByDateStockAfterDateWithBanque(null, b1);
		assertTrue(echans.size() == 0);
...
//	}
	...
	public void testFindByModePrepa() {
		ModePrepa mode = modePrepaDao.findById(1);
		List<Echantillon> echans = echantillonDao.findByModePrepa(mode);
		assertTrue(echans.size() == 2);
		ModePrepa mode2 = modePrepaDao.findById(4);
	...
	public void testFindRestantsByPrelevement() {
		Prelevement prlvt = prelevementDao.findById(1);
		List<Echantillon> echans = echantillonDao
											.findRestantsByPrelevement(prlvt);
		assertTrue(echans.size() == 1);
		prlvt = prelevementDao.findById(2);
	...
	public void testFindByPrelevementAndStatut() {
		...
		List<Echantillon> echans = echantillonDao
						.findByPrelevementAndStatut(prlvt, statut);
		assertTrue(echans.size() == 2);
		prlvt = prelevementDao.findById(2);
	...
	public void testFindByBanqueAndQuantiteSelectCode() {
		Banque b1 = banqueDao.findById(1);
		List<String> codes = echantillonDao
	...
	public void findByBanqueInListStatutSelectCode() {
		...
		ObjetStatut o2 = objetStatutDao.findById(2);
		codes = echantillonDao.findByBanqueInListStatutSelectCode(
				banques, o2);
		assertTrue(codes.size() == 1);
	...
	public void testFindByPatientNomOrNipInList() {
		...
		List<Integer> liste = echantillonDao.findByPatientNomOrNipInList(
				criteres, bks);
		...
		criteres.add("DELPHINO");
		criteres.add("12");
		liste = echantillonDao.findByPatientNomOrNipInList(
				criteres, bks);
		assertTrue(liste.size() == 4);
	...
	public void testFindByCodeOrNumLaboInListWithBanque() {
		...
		List<Object[]> liste = 
			echantillonDao.findByCodeInListWithBanque(criteres, bks);
		assertTrue(liste.size() == 3);
		...
		criteres = new ArrayList<String>();
		criteres.add("PTRA.1");
		liste = echantillonDao.findByCodeInListWithBanque(
				criteres, bks);
	...
	public void testCrud() throws Exception {
		...
		Banque b = banqueDao.findById(1);
		EchantillonType et = echantillonTypeDao.findById(2);
		e.setCode("code");
		...
		Integer echanId = e.getEchantillonId();
		
		// Test de la mise à jour
		Echantillon e2 = echantillonDao.findById(echanId);
	...
	public void testToString() {
		...
		assertTrue(echan1.entiteNom()
			.equals(entiteDao.findByNom("Echantillon").get(0).getNom()));
	...
	public void testClone() {
		...
		if (e1.getPrelevement() != null) {
			Prelevement p1 = e1.getPrelevement();
			Prelevement p2 = e2.getPrelevement();
			assertTrue(p1.getCode().equals(p2.getCode())
					&& p1.getBanque().equals(p2.getBanque()));
		} else {
		...
		if (e1.getConformeTraitement() != null) {
			assertTrue(e1.getConformeTraitement().equals(
					e2.getConformeTraitement()));
		} else {
			assertNull(e2.getConformeTraitement());
		}
		if (e1.getConformeCession() != null) {
			assertTrue(e1.getConformeCession().equals(
					e2.getConformeCession()));
		} else {
	...
	public void testNewEchantillon() {
		...
		Echantillon newEchan = new Echantillon();
	...
	public void testCountCongelesByDates() throws ParseException {
		...
		List<Long> res = echantillonDao
							.findCountSamplesByDates(d1, d2, banks);
		...
		d1.setTime(new SimpleDateFormat("dd/MM/yyyy hh:MM:ss")
					.parse("31/10/2010 00:10:00"));
		d2.setTime(new SimpleDateFormat("dd/MM/yyyy hh:MM:ss")
					.parse("31/10/2011 00:10:00"));		
		res = echantillonDao.findCountSamplesByDates(d1, d2, banks);
		assertTrue(res.get(0) == 0);
	...
	public void testFindByMaladieAndType() throws ParseException {
		...
		List<Echantillon> liste = echantillonDao
							.findByMaladieAndType(m4, "CELLULES", cal);
	...
	public void testCountCongelesByDatesExt() throws ParseException {
		...
		Etablissement e2 = etablissementDao.findById(2);
		etabs.add(e2);
		List<Long> res = echantillonDao
							.findCountSamplesByDatesExt(d1, d2, banks, etabs);
		assertTrue(res.get(0) == 3);
		...
		Etablissement e3 = etablissementDao.findById(3);
		etabs.add(e3);
		etabs.remove(e1);
		res = echantillonDao.findCountSamplesByDatesExt(d1, d2, banks, etabs);
		assertTrue(res.get(0) == 3);
	...
	public void testFindCountEchansByCessTypes() throws ParseException {
		...
		Date d2 = new SimpleDateFormat("dd/MM/yyyy").parse("14/11/2009"); 
		List<Banque> banks = new ArrayList<Banque>();
		Banque b1 = banqueDao.findById(1);
		Banque b2 = banqueDao.findById(2);
		banks.add(b2);
		...
		List<Long> res = echantillonDao
						.findCountEchansByCessTypes(cSan, d1, d2, banks);
		assertTrue(res.get(0) == 0);
	...
	public void testFindAssociateEchansOfType() {
		...
		List<EchantillonType> types = new ArrayList<EchantillonType>();
		types.add(echantillonTypeDao.findById(3));
		
		Maladie m4 = maladieDao.findById(4);
		Prelevement p1 = prelevementDao.findById(1);
		
		List<Echantillon> res = 
			echantillonDao.findAssociateEchansOfType(m4, types, banks, null);
		assertTrue(res.size() == 0);
	...
	public void testFindCountByPrelevementAndStockeReserve() {
		Prelevement p1 = prelevementDao.findById(1);
		List<Long> res = echantillonDao
			.findCountByPrelevementAndStockeReserve(p1);
		assertTrue(res.get(0) == 2);
	...
	public void testSortByCode() {
		...
		e4.setCode("TATAT");
		Echantillon e5 = new Echantillon();
		e5.setCode("ZAAA.ZZ");
		Echantillon e6 = new Echantillon();
		e6.setCode("ZAAA.23A");
		Echantillon e7 = new Echantillon();
		e7.setCode("ZAAA.2");
		Echantillon e8 = new Echantillon();
		e8.setCode("AAA.2");
		Echantillon e9 = new Echantillon();
		e9.setCode("ZAAA.");
		Echantillon e10 = new Echantillon();
		e10.setCode("ZAAA..");
		List<Echantillon> echans = new ArrayList<Echantillon>();
		echans.add(e5); echans.add(e2); echans.add(e3);
		echans.add(e9);
		echans.add(e10);
		echans.add(e8); echans.add(e6); echans.add(e7);
		echans.add(e1); echans.add(e4); 
		Collections
			.sort(echans, new Echantillon.CodeComparator(true));
	...
	public void testFindByEmplacementDao() {
		List<Echantillon> echans = echantillonDao
					.findByEmplacement(terminaleDao.findById(1), 3);
		assertTrue(echans.size() == 1);
