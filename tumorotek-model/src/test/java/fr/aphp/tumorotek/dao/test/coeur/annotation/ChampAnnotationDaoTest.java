...
public class ChampAnnotationDaoTest extends AbstractDaoTest {
	...
	public void setImportTemplateDao(ImportTemplateDao i) {
	...
	public void testToString() throws ParseException {
		...
		assertTrue(c1.toString().
				equals("{ChampAnnotation: TABLE_PAT1.Alphanum1}"));
		
		ChampAnnotation c2 = new ChampAnnotation();
		assertTrue(c2.toString().
				equals("{Empty ChampAnnotation}"));
	...
	public void testReadAllChamps() {
		List<ChampAnnotation> champs = champAnnotationDao.findAll();
		assertTrue(champs.size() == 48);
	}
	
	public void testFindByNom() {
		List<ChampAnnotation> champs = champAnnotationDao
										.findByNom("Bool%");
	...
	public void testCrudChampAnnotation() {
		...
		ChampAnnotation champ = new ChampAnnotation();		
		...
		Integer maxId = champ.getChampAnnotationId();
		
		// Test de la mise à jour
		ChampAnnotation champ2 = champAnnotationDao.findById(maxId);
		assertNotNull(champ2);
		assertTrue(champ2.getNom().equals("testAnno"));
		assertTrue(champ2.getDataType().equals(dataTypeDao.findByType("fichier")
																	.get(0)));
		assertTrue(champ2.getTableAnnotation()
									.equals(tableAnnotationDao.findById(6)));
		...
		assertTrue(champAnnotationDao.findById(maxId).getNom()
												.equals("testAnno2"));
	...
	public void testEqualsAndHashCode() {
		...
		TableAnnotation t3 = new TableAnnotation();
		t3.setNom(t2.getNom());
	...
	public void testClone() {
		...
		ChampAnnotation c2 = c.clone();
		assertTrue(c.equals(c2));
		if (c.getChampAnnotationId() != null) {
			assertTrue(c.getChampAnnotationId()
						.equals(c2.getChampAnnotationId()));
		} else {
		...
		if (c.getAnnotationDefauts() != null) {
			assertTrue(c.getAnnotationDefauts()
									.equals(c2.getAnnotationDefauts()));
		} else {
			assertNull(c2.getAnnotationDefauts());
		}
		if (c.getAnnotationValeurs() != null) {
			assertTrue(c.getAnnotationValeurs()
									.equals(c2.getAnnotationValeurs()));
		} else {
	...
	public void testFindByEditByCatalogue() {
		...
		List<ChampAnnotation> chps = champAnnotationDao
										.findByEditByCatalogue(t);
	...
	public void testFindChampsFichiersByTable() {
		TableAnnotation t = tableAnnotationDao.findById(7);
		DataType dt = dataTypeDao.findById(7);
		List<ChampAnnotation> chps = champAnnotationDao
							.findByTableAndType(t, dt);
	...
	public void testFindCriteresByChampAnnotation() {
    	ChampAnnotation c1 = champAnnotationDao.findById(1);
    	List<Critere> res = champAnnotationDao
    					.findCriteresByChampAnnotation(c1);
   ...
   public void testFindResultatsByChampAnnotation() {
    	ChampAnnotation c1 = champAnnotationDao.findById(1);
    	List<Resultat> res = champAnnotationDao
    						.findResultatsByChampAnnotation(c1);
    	assertTrue(res.isEmpty());
	...
	public void testFindImportColonnesByChampAnnotation() {
    	ChampAnnotation c = champAnnotationDao.findById(1);
    	List<ImportColonne> res = champAnnotationDao
    						.findImportColonnesByChampAnnotation(c);
    	assertTrue(res.isEmpty());
	...
	public void testFindByChpLEtiquetteChampAnnotation() {
    	ChampAnnotation c1 = champAnnotationDao.findById(1);
    	List<ChampLigneEtiquette> res = 
    		champAnnotationDao.findChpLEtiquetteByChampAnnotation(c1);
    	assertTrue(res.isEmpty());
    	res = champAnnotationDao
    				.findChpLEtiquetteByChampAnnotation(null);
    	assertTrue(res.isEmpty());
    }
	
	public void testfindByImportTemplateAndEntite() {
		List<ChampAnnotation> chpA = champAnnotationDao
				.findByImportTemplateAndEntite(importTemplateDao.findById(1), entiteDao.findById(2));
