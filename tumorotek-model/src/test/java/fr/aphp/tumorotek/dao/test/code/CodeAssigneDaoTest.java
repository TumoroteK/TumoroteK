...
public class CodeAssigneDaoTest extends AbstractDaoTest {
	 ...
	 */
	public void setCodeAssigneDao(CodeAssigneDao cDao) {
	...
	public void testReadAllCategories() {
		List<CodeAssigne> codediags = codeAssigneDao.findAll();
	...
	public void testFindByCodeAndEchantillon() {
		...
		List<CodeAssigne> codes = 
				codeAssigneDao.findByCodeAndEchantillon("BL%", e1);
		assertTrue(codes.size() == 2);
	...
	public void testFindCodesMorphoByEchantillon() {
		...
		List<CodeAssigne> codeOrganes = 
			codeAssigneDao.findCodesMorphoByEchantillon(e);
	...
	public void testFindCodesOrganeByEchantillon() {
		Echantillon e = echantillonDao.findById(1);
		List<CodeAssigne> codeOrganes = 
			codeAssigneDao.findCodesOrganeByEchantillon(e);
	...
	public void findByExcludedId() {
		CodeAssigne c = codeAssigneDao.findById(1);
		List<CodeAssigne> codes 
			= codeAssigneDao.findByExcludedId(c.getCodeAssigneId(), 
														c.getCode(), 
												c.getEchantillon());
		assertTrue(codes.size() == 0);
		codes = codeAssigneDao.findByExcludedId(8, "BL", 
	...
	public void testCrud() throws Exception {
		...
		String codeUpdated = "UPDATED";
		
		Echantillon e = echantillonDao.findById(1);
		TableCodage tc = tableCodageDao.findById(4);
		c.setEchantillon(e);
		c.setTableCodage(tc);
		c.setCodeRefId(1);
		c.setCode("TEST");
		...
		Integer cId = c.getCodeAssigneId();
		
		// Test de la mise à jour
		CodeAssigne c2 = codeAssigneDao.findById(cId);
		assertNotNull(c2);
	...
	public void testFindCodeLesExportedByPrelevement() {
		...
		List<CodeAssigne> codes = codeAssigneDao
									.findCodesLesExportedByPrelevement(p1);
		assertTrue(codes.size() == 1);
	...
	public void testFindCodeOrgExportedByPrelevement() {
		Prelevement p1 = prelevementDao.findById(1);
		List<CodeAssigne> codes = codeAssigneDao
									.findCodesOrgExportedByPrelevement(p1);
		assertTrue(codes.size() == 1);
	...
	public void testFindCodeOrgExportedByPatient() {
		...
		List<CodeAssigne> codes = codeAssigneDao
									.findCodesOrgExportedByPatient(p3);
		assertTrue(codes.size() == 1);
		assertTrue(codes.get(0).getCode().equals("BL"));
