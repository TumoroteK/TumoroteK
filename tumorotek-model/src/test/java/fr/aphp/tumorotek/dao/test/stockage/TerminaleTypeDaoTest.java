...
public class TerminaleTypeDaoTest extends AbstractDaoTest {
	...
	public void testReadAlls() {
		List<TerminaleType> liste = terminaleTypeDao.findAll();
	...
	public void testFindByOrder() {
		List<TerminaleType> list = terminaleTypeDao.findByOrder();
	...
	public void testFindByExcludedId() {
		...
		TerminaleType type = liste.get(0);
		assertNotNull(type);
	...
	public void testFindByType() {
		List<TerminaleType> liste = terminaleTypeDao
			.findByType("VISOTUBE_16_TRI");
	...
	public void testCrud() throws Exception {
		...
		TerminaleType tt = new TerminaleType();
		...
		// Test de l'insertion
		terminaleTypeDao.createObject(tt);
		assertEquals(new Integer(7), tt.getTerminaleTypeId());
		
		// Test de la mise à jour
		TerminaleType tt2 = terminaleTypeDao.findById(new Integer(7));
		assertNotNull(tt2);
		assertTrue(tt2.getType().equals("Type"));
		assertFalse(tt2.getDepartNumHaut());
		tt2.setType(updatedType);
		tt2.setDepartNumHaut(true);
		terminaleTypeDao.updateObject(tt2);
		assertTrue(terminaleTypeDao.findById(new Integer(7)).
				getType().equals(updatedType));
		assertTrue(terminaleTypeDao.findById(new Integer(7)).
				getDepartNumHaut());
		
		// Test de la délétion
		terminaleTypeDao.removeObject(new Integer(7));
		assertNull(terminaleTypeDao.findById(new Integer(7)));
	...
	public void testEquals() {
		String type = "TYPE";
		String type2 = "TYPE2";
		TerminaleType tt1 = new TerminaleType();
		tt1.setType(type);
		TerminaleType tt2 = new TerminaleType();
		tt2.setType(type);
	...
	public void testHashCode() {
		...
		TerminaleType tt2 = new TerminaleType();
		tt2.setType(type);
		TerminaleType ct3 = new TerminaleType();
		ct3.setType(type);
		assertTrue(ct3.hashCode() > 0);
		
		int hash = tt1.hashCode();
	...
	public void testToString() {
		TerminaleType tt1 = terminaleTypeDao.findById(1);
		assertTrue(tt1.toString().
				equals("{" + tt1.getType() + "}"));
	...
	public void testClone() {
		TerminaleType t1 = terminaleTypeDao.findById(1);
		...
		if (t1.getTerminaleTypeId() != null) {
			assertTrue(t1.getTerminaleTypeId() 
					== t2.getTerminaleTypeId());
		} else {
