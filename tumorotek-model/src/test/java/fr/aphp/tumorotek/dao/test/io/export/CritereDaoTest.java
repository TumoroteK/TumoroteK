...
public class CritereDaoTest extends AbstractDaoTest {
	...
	public void setCombinaisonDao(CombinaisonDao coDao) {
	...
	public void testCrudCritere() throws Exception {
		...
		List<Critere> criteres = this.critereDao.findAll();
		Iterator<Critere> itCritere = criteres.iterator();
		boolean found = false;
		while (itCritere.hasNext()) {
			Critere temp = itCritere.next();
			if (temp.equals(c)) {
				found = true;
		...
		Critere c2 = this.critereDao.findById(idObject);
		assertNotNull(c2);
		...
		final Combinaison updatedCombinaison
			= this.combinaisonDao.findById(idCo2);
		...
		assertTrue(this.critereDao.findById(idObject).getOperateur()
				.equals(updatedOperateur));
		if (this.critereDao.findById(idObject).getChamp() != null) {
			assertTrue(this.critereDao.findById(idObject).getChamp()
					.equals(updatedChamp));
		} else {
			assertNull(updatedChamp);
		}		
		if (this.critereDao.findById(idObject).getCombinaison() != null) {
			assertTrue(this.critereDao.findById(idObject).getCombinaison()
					.equals(updatedCombinaison));
		} else {
			assertNull(updatedCombinaison);
		}
		assertNotNull(this.critereDao.findById(idObject).getValeur());
		assertTrue(this.critereDao.findById(idObject).getValeur()
					.equals(updatedValeur));
		// Test de la délétion
	...
	public void testToString() {
		Critere c1 = critereDao.findById(1);
		assertTrue(c1.toString().equals(
				c1.getChamp().toString() + " " + c1.getOperateur() + " "
						+ c1.getValeur()));
