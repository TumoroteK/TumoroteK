...
//	private TransporteurDao transporteur;
	...
	private String updatedAdr = "Adr mise a jour";
...
//	}
	...
	public void testReadAllCoordonnees() {
		List<Coordonnee> coordonnees = coordonneeDao.findAll();
	...
	public void testFindByAdresse() {
		List<Coordonnee> coordonnees = coordonneeDao.
			findByAdresse("40 rue Worth");
		assertTrue(coordonnees.size() == 1);
		//pour verifier que la relation one-to-one fonctionne
		assertTrue(coordonnees.iterator().next().
				getEtablissement().getEtablissementId() == 2);
...
//	}
	...
	public void testFindByCollaborateurIdAndExcludedId() {
		List<Coordonnee> coordonnees = coordonneeDao
			.findByCollaborateurIdAndExcludedId(4, 3);
		assertTrue(coordonnees.size() == 0);
	...
	public void testCrudCoordonnee() throws Exception {
		...
		int id = c.getCoordonneeId();
		assertTrue(coordonneeDao.findAll().size() == 6);
		
		// Test de l'update
		Coordonnee c2 = coordonneeDao.findById(id);
	...
	public void testClone() {
		Coordonnee c1 = coordonneeDao.findById(1);
		Coordonnee c2 = c1.clone();
		assertTrue(c1.equals(c2));
