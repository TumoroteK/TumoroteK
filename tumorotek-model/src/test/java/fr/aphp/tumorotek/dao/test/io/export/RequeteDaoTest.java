...
public class RequeteDaoTest extends AbstractDaoTest {
	 ...
	 */
	public void setGroupementDao(GroupementDao gDao) {
	 ...
	 * @param cDao est le bean Dao.
	 */
	public void setCombinaisonDao(CombinaisonDao cDao) {
    ...
    public void testFindRequetesByUtilisateurId() throws Exception {
    	...
    	Iterator<Utilisateur> itUtilisateur = utilisateurs.iterator();
    	while (itUtilisateur.hasNext()) {
    		Utilisateur utilisateur = itUtilisateur.next();
    		List<Requete> requetes = requeteDao.findByUtilisateur(utilisateur);
    ...
    public void testFindByIntituleUtilisateur() throws Exception {
		...
		List<Requete> liste = requeteDao
			.findByIntituleUtilisateur("Echantillon%", u1);
		assertTrue(liste.size() == 1);
	...
	public void testCrudRequete() throws Exception {
		...
		List<Requete> requetes = this.requeteDao.findAll();
		Iterator<Requete> itRequete = requetes.iterator();
		boolean found = false;
		while (itRequete.hasNext()) {
			Requete temp = itRequete.next();
			if (temp.equals(r)) {
				found = true;
		...
		Requete r2 = this.requeteDao.findById(idObject);
		assertNotNull(r2);
		...
		final Groupement updatedGroupementRacine
			= this.groupementDao.findById(idG2);
		...
		assertTrue(this.requeteDao.findById(idObject).getCreateur()
				.equals(updatedCreateur));
		assertNotNull(updatedGroupementRacine);
		assertTrue(this.requeteDao.findById(idObject).getGroupementRacine()
				.equals(updatedGroupementRacine));
		assertNotNull(updatedIntitule);
		assertTrue(this.requeteDao.findById(idObject).getIntitule()
				.equals(updatedIntitule));
		assertNotNull(this.requeteDao.findById(idObject).getBanque());
		assertTrue(this.requeteDao.findById(idObject).getBanque()
					.equals(updatedBanque));
		// Test de la délétion
