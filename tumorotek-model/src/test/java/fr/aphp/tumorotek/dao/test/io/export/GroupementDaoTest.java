...
public class GroupementDaoTest extends AbstractDaoTest {
    ...
    public void testFindEnfants() throws Exception {
    	List<Groupement> parents = this.groupementDao.findAll();
    	Iterator<Groupement> itParents = parents.iterator();
    	while (itParents.hasNext()) {
    		Groupement parent = itParents.next();
    		ArrayList<Groupement> enfants
    			= this.groupementDao.findEnfants(parent);
	...
	public void testCrudGroupement() throws Exception {
		...
		int id1 = c.getCritereId();
		c = new Critere(this.champDao.findById(4), "=", "tti");
		this.critereDao.createObject(c);
		int id2 = c.getCritereId();
		...
		List<Groupement> groupements = this.groupementDao.findAll();
		Iterator<Groupement> itGroupement = groupements.iterator();
		boolean found = false;
		while (itGroupement.hasNext()) {
			Groupement temp = itGroupement.next();
			if (temp.equals(g)) {
				found = true;
		...
		/** Update. */
		Groupement g2 = this.groupementDao.findById(idObject);
		assertNotNull(g2);
		...
		assertTrue(this.groupementDao.findById(idObject).getOperateur()
				.equals(updatedCondition));
		if (this.groupementDao.findById(idObject).getCritere1() != null) {
			assertTrue(this.groupementDao.findById(idObject).getCritere1()
					.equals(updatedCritere1));
		} else {
			assertNull(updatedCritere1);
		}
		if (this.groupementDao.findById(idObject).getCritere2() != null) {
			assertTrue(this.groupementDao.findById(idObject).getCritere2()
					.equals(updatedCritere2));
		} else {
			assertNull(updatedCritere2);
		}
		if (this.groupementDao.findById(idObject).getParent() != null) {
			assertTrue(this.groupementDao.findById(idObject).getParent()
					.equals(updatedParent));
		} else {
	...
	public void testToString() {
		Groupement g1 = groupementDao.findById(1);
		assertTrue(g1.toString().equals("{" + g1.getGroupementId() + "}"));
		
		Groupement g2 = new Groupement();
