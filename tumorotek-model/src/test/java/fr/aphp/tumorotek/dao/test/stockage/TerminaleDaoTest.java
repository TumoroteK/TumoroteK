...
/**
 *
 * Classe de test pour le DAO TerminaleDao et le bean du domaine Terminale.
 *
...
public class TerminaleDaoTest extends AbstractDaoTest {
	...
	public void testReadAll() {
		final List<Terminale> liste = IterableUtils.toList(IterableUtils.toList(terminaleDao.findAll()));
	...
	public void testFindByExcludedIdEnceinte() {
		...
		final Enceinte e3 = enceinteDao.findById(3).get();
	...
	public void testCrud() throws Exception {
		...
		final TerminaleType type = terminaleTypeDao.findById(1).get();
		final Entite ent = entiteDao.findById(3).get();
		final Banque b1 = banqueDao.findById(1).get();
		final TerminaleNumerotation num = terminaleNumerotationDao.findById(1).get();
		...
		final Terminale t2 = terminaleDao.findById(new Integer(7)).get();
		...
		t2.setNom(nomUp);
		terminaleDao.save(t2);
		assertTrue(terminaleDao.findById(new Integer(7)).get().getNom().equals(nomUp));

		// Test de la délétion
		terminaleDao.deleteById(new Integer(7));
		assertFalse(terminaleDao.findById(new Integer(7)).isPresent());
