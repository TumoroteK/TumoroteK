...
public class RetourDaoTest extends AbstractDaoTest {
	...
	public void testReadAllRetours() {
		List<Retour> retours = retourDao.findAll();
		assertTrue(retours.size() == 8);
	}
	
	public void testFindByMaxId() {
		List<Integer> res = retourDao.findByMaxId();
	...
	public void testCrudRetour() throws Exception {
		...
		String adrl1 = "CC1.R1.T1.BT1.77";
		String adrl2 = "CC1.R1.T1.BT2.5";
		...
		Integer rId = retour.getRetourId();
		
		// Test de la mise à jour
		Retour retour2 = retourDao.findById(rId);
		assertNotNull(retour2);
		assertTrue(retour2.getObjetId() == 4);
		assertTrue(retour2.getEntite()
							.equals(entiteDao.findByNom("Echantillon").get(0)));
		...
		assertTrue(retour2.getCollaborateur()
						.equals(collaborateurDao.findById(2)));
		assertTrue(retour2.getObservations()
						.equals("sortie pour transformation"));
		assertTrue(retour2.getTransformation()
						.equals(transformationDao.findById(4)));
		...
		assertTrue(retourDao.findById(rId).getTempMoyenne()
											.equals(new Float(22.69)));
		assertTrue(retourDao.findById(rId).getSterile());
		assertNull(retourDao.findById(rId).getCollaborateur());
		assertTrue(retourDao.findById(rId).getObservations()
							.equals("sortie pour traitement"));
	...
	public void testToString() throws ParseException {
		Retour r1 = retourDao.findById(1);
		assertTrue(r1.toString().
				equals("{Echantillon:1 01/01/2010 00:00:00}"));
		
		Retour r2 = new Retour();
		assertTrue(r2.toString().
				equals("{Empty Retour}"));
	...
	public void testClone() {
		Retour r2 = retourDao.findById(2);
		Retour clone = r2.clone();
		...
		assertTrue(clone.getOldEmplacementAdrl().equals(
				r2.getOldEmplacementAdrl()));
		assertNull(clone.getTransformation());
		assertNull(clone.getConteneur());
		assertNull(clone.getIncident());
		assertTrue(clone.getObjetStatut().equals(
				r2.getObjetStatut()));
		assertTrue(clone.getImpact().equals(
				r2.getImpact()));
	...
	public void testFindByObjectDateRetourEmpty() {
		List<Integer> objetsIds = new ArrayList<Integer>();
		
		objetsIds.add(1); 	objetsIds.add(2); 
