...
/**
 *
 * Classe de test pour le DAO RetourDao et le bean du domaine Retour.
 *
...
public class RetourDaoTest extends AbstractDaoTest {
	...
	public void testFindByExcludedId() {
		List<Retour> retours = retourDao.findByExcludedId(1, 1, entiteDao.findById(3).get());
		assertTrue(retours.size() == 4);
		retours = retourDao.findByExcludedId(9, 1, entiteDao.findById(3).get());
		assertTrue(retours.size() == 5);
	...
	public void testCrudRetour() throws Exception {
		...
		retour.setCollaborateur(collaborateurDao.findById(2).get());
		retour.setObservations("sortie pour transformation");
		retour.setTransformation(transformationDao.findById(4).get());
		retour.setOldEmplacementAdrl(adrl1);
		retour.setObjetStatut(objetStatutDao.findById(4).get());
		...
		final Retour retour2 = retourDao.findById(rId).get();
		...
		assertTrue(retour2.getDateSortie().getTime()
				.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss").parse("01/01/2010 12:10:25")));
		...
		assertTrue(retour2.getCollaborateur().equals(collaborateurDao.findById(2).get()));
		assertTrue(retour2.getObservations().equals("sortie pour transformation"));
		assertTrue(retour2.getTransformation().equals(transformationDao.findById(4).get()));
		...
		retour2.setObjetStatut(objetStatutDao.findById(2).get());

		retourDao.save(retour2);

		assertNotNull(retourDao.findById(rId).get().getDateSortie());
		assertNotNull(retourDao.findById(rId).get().getDateRetour());
		assertTrue(retourDao.findById(rId).get().getTempMoyenne().equals(new Float(22.69)));
		assertTrue(retourDao.findById(rId).get().getSterile());
		assertFalse(retourDao.findById(rId).get().getCollaborateur() != null);
		assertTrue(retourDao.findById(rId).get().getObservations().equals("sortie pour traitement"));
		assertFalse(retourDao.findById(rId).get().getTransformation() != null);
		assertTrue(retourDao.findById(rId).get().getOldEmplacementAdrl().equals(adrl2));
	...
	public void testClone() {
		final Retour r2 = retourDao.findById(2).get();
	...
	public void testFindByObjDates() throws ParseException {
		...
		List<Retour> rs = retourDao.findByObjDates(cal, 1, entiteDao.findById(3).get(), -1);
		assertTrue(rs.size() == 1);
		assertTrue(rs.get(0).getRetourId().equals(4));

		rs = retourDao.findByObjDates(cal, 4, entiteDao.findById(3).get(), 0);
		...
		rs = retourDao.findByObjDates(cal, 3, entiteDao.findById(8).get(), 0);
		...
		rs = retourDao.findByObjDates(cal, null, entiteDao.findById(8).get(), 2);
		assertTrue(rs.isEmpty());

		rs = retourDao.findByObjDates(null, 3, entiteDao.findById(8).get(), 1);
		assertTrue(rs.isEmpty());
	}
	...
	public void testFindObjIdsByDatesAndEntite() throws ParseException {
		final Calendar cal = Calendar.getInstance();
		cal.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("06/01/2010 12:12"));
		List<Integer> rs = retourDao.findObjIdsByDatesAndEntite(cal, entiteDao.findById(3).get());
		assertTrue(rs.size() == 1);
		assertTrue(rs.get(0).equals(1));

		cal.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("06/01/2012 12:12"));
		rs = retourDao.findObjIdsByDatesAndEntite(cal, entiteDao.findById(3).get());
		assertTrue(rs.isEmpty());
		...
		rs = retourDao.findObjIdsByDatesAndEntite(cal, entiteDao.findById(8).get());
		assertTrue(rs.size() == 1);
		...
		rs = retourDao.findObjIdsByDatesAndEntite(null, entiteDao.findById(8).get());
		assertTrue(rs.isEmpty());
	}
	...
	public void testFindByObjInsideDates() throws ParseException {
		...
		List<Retour> rs = retourDao.findByObjInsideDates(cal1, cal2, 1, entiteDao.findById(3).get(), 0);
		assertTrue(rs.size() == 1);
		assertTrue(rs.get(0).getRetourId().equals(5));

		cal1.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("31/12/2009 12:12"));
		rs = retourDao.findByObjInsideDates(cal1, cal2, 1, entiteDao.findById(3).get(), -1);
		assertTrue(rs.size() == 5);

		rs = retourDao.findByObjInsideDates(cal1, cal2, 4, entiteDao.findById(3).get(), 0);
		assertTrue(rs.size() == 1);
		assertTrue(rs.get(0).getRetourId().equals(6));

		rs = retourDao.findByObjInsideDates(cal1, cal2, 2, entiteDao.findById(3).get(), 0);
		assertTrue(rs.isEmpty());

		cal1.setTime(new SimpleDateFormat("dd/MM/yyyy").parse("01/01/2010"));

		rs = retourDao.findByObjInsideDates(cal1, cal1, 4, entiteDao.findById(3).get(), 0);
		assertTrue(rs.size() == 1);
		assertTrue(rs.get(0).getRetourId().equals(6));

		rs = retourDao.findByObjInsideDates(cal1, cal1, 3, entiteDao.findById(8).get(), 0);
		assertTrue(rs.size() == 2);
		assertTrue(rs.get(0).getRetourId() > 6);
		...
		rs = retourDao.findByObjInsideDates(cal1, cal1, null, entiteDao.findById(8).get(), 2);
		assertTrue(rs.isEmpty());

		rs = retourDao.findByObjInsideDates(cal1, null, 3, entiteDao.findById(8).get(), 3);
		assertTrue(rs.isEmpty());

		rs = retourDao.findByObjInsideDates(null, cal1, 3, entiteDao.findById(8).get(), 4);
		assertTrue(rs.isEmpty());
	}
	...
	public void testFindObjIdsInsideDatesEntite() throws ParseException {
		...
		List<Integer> rs = retourDao.findObjIdsInsideDatesEntite(cal1, cal2, entiteDao.findById(3).get());
		assertTrue(rs.size() == 2);
		assertTrue(rs.contains(1));
		assertTrue(rs.contains(4));

		cal1.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("31/12/2009 12:12"));
		rs = retourDao.findObjIdsInsideDatesEntite(cal1, cal2, entiteDao.findById(3).get());
		assertTrue(rs.size() == 2);

		rs = retourDao.findObjIdsInsideDatesEntite(cal1, cal2, entiteDao.findById(8).get());
		assertTrue(rs.size() == 1);
		...
		rs = retourDao.findObjIdsInsideDatesEntite(cal1, null, entiteDao.findById(8).get());
		assertTrue(rs.isEmpty());

		rs = retourDao.findObjIdsInsideDatesEntite(null, cal2, entiteDao.findById(8).get());
		assertTrue(rs.isEmpty());
	}
	...
	public void testFindByObjectDateRetourEmpty() {
		...
		objetsIds.add(1);
		objetsIds.add(2);
		List<Retour> rets = retourDao.findByObjectsDateRetourEmpty(objetsIds, entiteDao.findById(3).get());
		assertTrue(rets.isEmpty());

		objetsIds.add(3);
		rets = retourDao.findByObjectsDateRetourEmpty(objetsIds, entiteDao.findById(8).get());
		assertTrue(rets.isEmpty());
	}

	@Test
	public void testFindByObjectAndImpact() {
		List<Retour> impacts = retourDao.findByObjectAndImpact(1, entiteDao.findById(3).get(), true);
		assertTrue(impacts.size() == 3);
		impacts = retourDao.findByObjectAndImpact(1, entiteDao.findById(3).get(), false);
		assertTrue(impacts.size() == 1);
		impacts = retourDao.findByObjectAndImpact(1, entiteDao.findById(3).get(), null);
		assertTrue(impacts.size() == 0);
		impacts = retourDao.findByObjectAndImpact(4, entiteDao.findById(3).get(), true);
		assertTrue(impacts.size() == 0);
		impacts = retourDao.findByObjectAndImpact(2, entiteDao.findById(3).get(), true);
		assertTrue(impacts.size() == 0);
		impacts = retourDao.findByObjectAndImpact(3, entiteDao.findById(8).get(), true);
		assertTrue(impacts.size() == 0);
		impacts = retourDao.findByObjectAndImpact(3, entiteDao.findById(8).get(), false);
