...
/**
 * 
 * Classe de test pour le DAO CodeUtilisateurDao et le 
 * bean du domaine CodeUtilisateur.
 * Classe de test créée le 24/09/09.
 * 
...
public class CodeUtilisateurDaoTest extends AbstractDaoTest {
	...
	public void setCodeDossierDao(CodeDossierDao cDosDao) {
		this.codeDossierDao = cDosDao;
	}

	public void setTableCodageDao(TableCodageDao tCDao) {
	...
	public void testReadAllCodes() {
		List<CodeUtilisateur> codes = codeUtilisateurDao.findAll();
	...
	public void testFindByCodeLike() {
		...
		banks.add(b1);
		List<CodeUtilisateur> codes = codeUtilisateurDao
											.findByCodeLike("code%", banks);
	...
	public void testFindByLibelleLike() {
		...
		banks.add(b1);
		List<CodeUtilisateur> codes = codeUtilisateurDao
							.findByLibelleLike("libelle1%", banks);
	...
	public void testFindByUtilisateurAndBanque() {
		...
		List<CodeUtilisateur> codes 
			= codeUtilisateurDao.findByUtilisateurAndBanque(u, b);
		assertTrue(codes.size() == 5);
	...
	public void testFindByCodeDossier() {
		...
		List<CodeUtilisateur> codes 
			= codeUtilisateurDao.findByCodeDossier(dos);
	...
	public void testFindByRootDossier() {
		Banque b = banqueDao.findById(1);
		List<CodeUtilisateur> codes 
			= codeUtilisateurDao.findByRootDossier(b);
		assertTrue(codes.size() == 1);
	}
	...
	public void testFindByCodeParent() {
		...
		List<CodeUtilisateur> codes 
			= codeUtilisateurDao.findByCodeParent(c);
		assertTrue(codes.size() == 2);
		c = codeUtilisateurDao.findById(1);
		codes = codeUtilisateurDao.findByCodeParent(c);
		assertTrue(codes.size() == 0);
	}
	
	public void findByExcludedId() {
		CodeUtilisateur c = codeUtilisateurDao.findById(1);
		List<CodeUtilisateur> codes 
			= codeUtilisateurDao.findByExcludedId(c.getCodeUtilisateurId());
		assertTrue(codes.size() == 5);
	...
	public void testFindByTranscodage() {
		...
		TableCodage adicap = tableCodageDao.findByNom("ADICAP").get(0);
		List<CodeUtilisateur> codes = 
				codeUtilisateurDao.findByTranscodage(adicap, 1, banks);
	...
	public void testCrud() throws Exception {
		...
		//Transcodes
		Set<TranscodeUtilisateur> codes = new HashSet<TranscodeUtilisateur>();
		TranscodeUtilisateur tr1 = new TranscodeUtilisateur();
		TableCodage t1 = tableCodageDao.findById(4);
		tr1.setTableCodage(t1);
		tr1.setCodeId(1);
		tr1.setCodeUtilisateur(c);
		TranscodeUtilisateur tr2 = new TranscodeUtilisateur();
		tr2.setTableCodage(t1);
		tr2.setCodeId(2);
		tr2.setCodeUtilisateur(c);
		codes.add(tr1); codes.add(tr2);
		...
		assertTrue(codeUtilisateurDao.
				findById(new Integer(7)).getTranscodes().size() == 2);
		
		// Test de la mise à jour
		CodeUtilisateur c2 = codeUtilisateurDao.findById(new Integer(7));
		assertNotNull(c2);
...
//		c2.setTranscodes(codes);
		...
		assertTrue(codeUtilisateurDao.
				findById(new Integer(7)).getCode() == "update");
//		assertTrue(codeUtilisateurDao.
//				findById(new Integer(7)).getTranscodes().size() == 1);
	...
	public void testEquals() {
		...
		Banque b1 = banqueDao.findById(1);
		Banque b2 = banqueDao.findById(2);
		CodeUtilisateur c1 = new CodeUtilisateur();
		CodeUtilisateur tr2 = new CodeUtilisateur();
	...
	public void testHashCode() {
		...
		tr2.setBanque(b);
		tr2.setCode("hash");
		CodeUtilisateur c3 = new CodeUtilisateur();
	...
	public void testToString() {
		...
		assertTrue(c1.toString()
				.equals("{CodeUtilisateur: " + c1.getCode() + "}"));
	...
	public void testClone() {
		...
		CodeUtilisateur clone = c1.clone();
	...
	public void testEqualsAndHashCodeTranscodeUtilisateur() {
		...
		CodeUtilisateur c1 = codeUtilisateurDao.findById(1);
		CodeUtilisateur c2 = codeUtilisateurDao.findById(2);
		CodeUtilisateur c3 = new CodeUtilisateur();
