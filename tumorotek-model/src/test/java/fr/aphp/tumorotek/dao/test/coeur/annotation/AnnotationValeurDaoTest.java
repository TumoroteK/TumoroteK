...
public class AnnotationValeurDaoTest extends AbstractDaoTest {
	...
	public void setAnnotationDefautDao(AnnotationValeurDao avDao) {
	...
	public void setTableAnnotationDao(TableAnnotationDao tabDao) {
	...
	public void testToString() throws ParseException {
		AnnotationValeur av1 = annotationValeurDao.findById(1);
		assertTrue(av1.toString().
				equals("{Valeur: Alphanum1.AlphanumValue1}"));	
		AnnotationValeur av2 = new AnnotationValeur();
		assertTrue(av2.toString().
				equals("{Empty AnnotationValeur}"));
		...
		assertTrue(av2.toString().
				equals("{Valeur: Alphanum1.item1-1}"));
		av2.setTexte("You and me always");
		assertTrue(av2.toString().
				equals("{Valeur: Alphanum1.You ...}"));
		av2.setTexte("You");
		assertTrue(av2.toString().
				equals("{Valeur: Alphanum1.You}"));
		Calendar sDate = Calendar.getInstance();
		sDate.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
										.parse("12/02/2006 12:02:45"));
		av2.setDate(sDate);
		assertTrue(av2.toString().
				equals("{Valeur: Alphanum1.12/02/2006 12:02:45}"));
		av2.setBool(true);
	...
	public void testReadAllValeurs() {
		List<AnnotationValeur> valeurs = annotationValeurDao.findAll();
		assertTrue(valeurs.size() == 12);
	}
	
	public void testFindByChampAndObjetId() {
		List<AnnotationValeur> valeurs = annotationValeurDao
			.findByChampAndObjetId(champAnnotationDao.findById(12), 1);
		assertTrue(valeurs.size() == 2);
		valeurs = annotationValeurDao
			.findByChampAndObjetId(champAnnotationDao.findById(8), 1);
		assertTrue(valeurs.size() == 0);
		valeurs = annotationValeurDao
			.findByChampAndObjetId(champAnnotationDao.findById(4), 3);
		assertTrue(valeurs.size() == 0);
	...
	public void testFindByTableAndBanque() {
		List<AnnotationValeur> valeurs = annotationValeurDao
			.findByTableAndBanque(tableAnnotationDao.findById(3), 
												banqueDao.findById(1));
		assertTrue(valeurs.size() == 3);
		valeurs = annotationValeurDao
			.findByTableAndBanque(tableAnnotationDao.findById(3), 
												banqueDao.findById(3));
		assertTrue(valeurs.size() == 0);
		valeurs = annotationValeurDao
				.findByTableAndBanque(tableAnnotationDao.findById(4), 
											banqueDao.findById(1));
		assertTrue(valeurs.size() == 0);
	...
	public void testFindByExcludedId() {
		...
		List<AnnotationValeur> valeurs = 
			annotationValeurDao.findByExcludedId(av1.getChampAnnotation(), 
														av1.getObjetId(), 1);
		assertTrue(valeurs.size() == 0);
		assertFalse(valeurs.contains(av1));
		AnnotationValeur av12 = annotationValeurDao.findById(12);
		valeurs = annotationValeurDao
			.findByExcludedId(av12.getChampAnnotation(), av12.getObjetId(), 12);
	...
	public void testCrudValeur() {
		...
		Integer aId = av2.getAnnotationValeurId();
		
		assertTrue(annotationValeurDao
				.findByChampAndObjetId(champAnnotationDao.findById(2), 2)
															.size() == 1);
		assertTrue(annotationValeurDao.findById(aId)
		...
		assertTrue(annotationValeurDao.findById(aId).getAlphanum()
													.equals("val2"));
	...
	public void testEqualsAndHashCode() {
		...
		ChampAnnotation c1 = champAnnotationDao.findById(1);
		ChampAnnotation c2 = champAnnotationDao.findById(2);
		...
		Item it1 = itemDao.findById(1);
		Item it2 = itemDao.findById(2);
		Item it3 = new Item();
	...
	public void testClone() throws IOException {
		AnnotationValeur av1 = annotationValeurDao.findById(5);
		...
		assertTrue(clone.getAnnotationValeurId()
									.equals(av1.getAnnotationValeurId()));
	...
	public void testFindByObjectIdAndEntite() {
		List<AnnotationValeur> vals = 
			annotationValeurDao.findByObjectIdAndEntite(new Integer(1), 
									entiteDao.findByNom("Echantillon").get(0));
		assertTrue(vals.size() == 6);
		
		vals = 
			annotationValeurDao.findByObjectIdAndEntite(new Integer(1), 
									entiteDao.findByNom("Patient").get(0));
		assertTrue(vals.size() == 1);
		
		vals = 
			annotationValeurDao.findByObjectIdAndEntite(new Integer(3), 
									entiteDao.findByNom("Echantillon").get(0));
	...
	public void testFormateAnnotationValeur() {
		...
		AnnotationValeur av7 = annotationValeurDao.findById(7);
		assertTrue(av7.formateAnnotationValeur().equals("textVal1"));
		
		AnnotationValeur av11 = annotationValeurDao.findById(11);
		assertTrue(av11.formateAnnotationValeur().equals("http://google.com"));
		
		AnnotationValeur av5 = annotationValeurDao.findById(5);
		assertTrue(av5.formateAnnotationValeur().equals("12/12/2002 10:00"));
		
		AnnotationValeur av4 = annotationValeurDao.findById(4);
		assertTrue(av4.formateAnnotationValeur().equals("Oui"));
		
		AnnotationValeur av8 = annotationValeurDao.findById(8);
		assertTrue(av8.formateAnnotationValeur().equals("item1-1"));
		
		// @since 2.1 teste nom fichier
		AnnotationValeur av10 = annotationValeurDao.findById(10);
	...
	public void testFindCountByTableAnnotationBanque() {
		...
		Banque b1 = banqueDao.findById(1);
		List<Long> count = annotationValeurDao
				.findCountByTableAnnotationBanque(t3, b1);
		assertTrue(count.get(0) == 3);
		Banque b2 = banqueDao.findById(2);
		count = annotationValeurDao
				.findCountByTableAnnotationBanque(t3, b2);
		assertTrue(count.get(0) == 2);
		TableAnnotation t5 = tableAnnotationDao.findById(5);
		count = annotationValeurDao
				.findCountByTableAnnotationBanque(t5, b2);
		assertTrue(count.get(0) == 0);
		count = annotationValeurDao
				.findCountByTableAnnotationBanque(t5, b1);
		assertTrue(count.get(0) == 3);
		count = annotationValeurDao
				.findCountByTableAnnotationBanque(t5, null);
		assertTrue(count.get(0) == 0);
		count = annotationValeurDao
				.findCountByTableAnnotationBanque(null, b1);
		assertTrue(count.get(0) == 0);
	...
	public void testGetValeur() throws ParseException {
		...
		file.setNom("file1");
		av.setFichier(file);
		Item i = new Item(); i.setLabel("ite1");
		av.setItem(i);
