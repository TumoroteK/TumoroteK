...
public class ResultatDaoTest extends AbstractDaoTest {
    ...
    public void testFindByAffichage() throws Exception {
		List<Affichage> affichages = this.affichageDao.findAll();
		Iterator<Affichage> itAff = affichages.iterator();
		while (itAff.hasNext()) {
			Affichage affichage = itAff.next();
			List<Resultat> resultats = this.resultatDao
					.findByAffichage(affichage);
			assertTrue(affichage.getResultats().size() == resultats.size());
			Iterator<Resultat> it = resultats.iterator();			
			while (it.hasNext()) {
				Resultat resultat = it.next();
				boolean found = false;
				Iterator<Resultat> it2 = affichage.getResultats().iterator();
				while (it2.hasNext()) {
	...
	public void testCrudResultat() throws Exception {
		...
		ChampEntite chEntite = new ChampEntite(entiteDao.findById(2),
				"champEntite1", dataType, false, true, "000-0", false, null);
		...
		chEntite = new ChampEntite(entiteDao.findById(1), "champEntite2",
				dataType, false, false, null, false, null);
		champEntiteDao.createObject(chEntite);
		...
		Champ champ = champDao.findById(idCh1);
		...
		Utilisateur utilisateur = utilisateurDao.findById(idU);
		...
		Iterator<Resultat> itResultat = resultats.iterator();
		boolean found = false;
		while (itResultat.hasNext()) {
			Resultat temp = itResultat.next();
			if (temp.equals(r)) {
				found = true;
		...
		Resultat r2 = resultatDao.findById(idObject);
		assertNotNull(r2);
		...
		assertTrue(resultatDao.findById(idObject).getChamp()
				.equals(updatedChamp));
		if (resultatDao.findById(idObject).getFormat() != null) {
			assertTrue(resultatDao.findById(idObject).getFormat()
					.equals(updatedFormat));
		} else {
			assertNull(updatedFormat);
		}
		assertNotNull(resultatDao.findById(idObject).getNomColonne());
		assertTrue(resultatDao.findById(idObject).getNomColonne()
				.equals(updatedNomColonne));
		assertNotNull(resultatDao.findById(idObject).getPosition());
		assertTrue(resultatDao.findById(idObject).getPosition()
				.equals(updatedPosition));
		assertNotNull(resultatDao.findById(idObject).getTri());
		assertTrue(resultatDao.findById(idObject).getTri()
				.equals(updatedTri));
		assertNotNull(resultatDao.findById(idObject).getOrdreTri());
		assertTrue(resultatDao.findById(idObject).getOrdreTri()
				.equals(updatedOrdreTri));
		assertNotNull(resultatDao.findById(idObject).getAffichage());
		assertTrue(resultatDao.findById(idObject).getAffichage()
	...
	public void testToString() {
		Resultat r1 = resultatDao.findById(1);
