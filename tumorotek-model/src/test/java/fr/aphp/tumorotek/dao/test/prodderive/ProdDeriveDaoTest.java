...
/**
 * 
 * Classe de test pour le DAO ProdDeriveDao et le bean du 
 * domaine ProdDerive.
...
public class ProdDeriveDaoTest extends AbstractDaoTest {
	...
	public void testReadAllCategories() {
		List<ProdDerive> derives = prodDeriveDao.findAll();
	...
	public void testFindByBanques() {
		...
		List<ProdDerive> res = prodDeriveDao.findByBanques(banks);
	...
	public void testFindByBanquesAllIds() {
		...
		banks.add(banqueDao.findById(1));
		banks.add(banqueDao.findById(2));
		List<Integer> res = prodDeriveDao.findByBanquesAllIds(banks);
		assertTrue(res.size() == 4);
	}
	...
	public void testFindByCodeOrLaboWithBanque() {
		...
		List<ProdDerive> derives = prodDeriveDao
			.findByCodeOrLaboWithBanque("EHT.1.1", b1);
		assertTrue(derives.size() == 1);
	...
	public void testFindByCodeInPlateforme() {
		...
		List<ProdDerive> ders = 
			prodDeriveDao.findByCodeInPlateforme("PTRA.1.1", p1);
	...
	public void testFindByCodeOrLaboWithBanqueReturnIds() {
		...
		List<Integer> derives = prodDeriveDao
			.findByCodeOrLaboWithBanqueReturnIds("EHT.1.1", b1);
		assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao.findByCodeOrLaboWithBanqueReturnIds("PTRA", b1);
		assertTrue(derives.size() == 0);
		
		derives = prodDeriveDao.findByCodeOrLaboWithBanqueReturnIds(
				"PTRA%", b1);
		assertTrue(derives.size() == 2);
	...
	public void testFindByDateStockAfterDate() throws Exception {
		...
		List<ProdDerive> derives = prodDeriveDao.
			findByDateStockAfterDate(search);
		assertTrue(derives.size() == 4);
	...
	public void testFindByDateTransformationAfterDate() throws Exception {
		...
		List<ProdDerive> derives = 
			prodDeriveDao.findByDateTransformationAfterDate(search);
		assertTrue(derives.size() == 4);
	 ...
	 * throws Exception Lance une exception en cas d'erreur.
	 */
	public void testFindByDateTransformationAfterDateWithBanque() 
	throws Exception {
		Banque b1 = banqueDao.findById(1);
		...
		search.setTime(new SimpleDateFormat("dd/MM/yyyy").parse("15/01/2009"));
		List<ProdDerive> derives = 
			prodDeriveDao.findByDateTransformationAfterDateWithBanque(
					search, b1);
		assertTrue(derives.size() == 3);
		
		search.setTime(new SimpleDateFormat("dd/MM/yyyy").parse("01/07/2009"));
		derives = prodDeriveDao
			.findByDateTransformationAfterDateWithBanque(search, b1);
		assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao
			.findByDateTransformationAfterDateWithBanque(search, null);
		assertTrue(derives.size() == 0);
		
		derives = prodDeriveDao
			.findByDateTransformationAfterDateWithBanque(null, b1);
		assertTrue(derives.size() == 0);
	}
	...
	public void testFindByModePrepaDerive() {
		...
		m1 = modePrepaDeriveDao.findById(4);
	...
	public void testFindByProdQualite() {
		...
		qualite = prodQualiteDao.findById(3);
	...
	public void testFindByTransformation() {
		...
		transfo = transformationDao.findById(5);
	...
	public void testFindByBanqueAndQuantiteSelectCode() {
		Banque b1 = banqueDao.findById(1);
		List<String> codes = prodDeriveDao
			.findByBanqueAndQuantiteSelectCode(b1);
		assertTrue(codes.size() == 2);
	...
	public void testFindByBanqueStatutSelectCode() {
		Banque b1 = banqueDao.findById(1);
		ObjetStatut o1 = objetStatutDao.findById(1);
		List<String> codes = prodDeriveDao
			.findByBanqueStatutSelectCode(b1, o1);
		assertTrue(codes.size() == 2);
		assertTrue(codes.get(0).equals("PTRA.1.1"));
		
		ObjetStatut o2 = objetStatutDao.findById(2);
		codes = prodDeriveDao.findByBanqueStatutSelectCode(b1, o2);
		assertTrue(codes.size() == 1);
		
		ObjetStatut o3 = objetStatutDao.findById(3);
	...
	public void testFindByBanqueInListStatutSelectCode() {
		...
		ObjetStatut o1 = objetStatutDao.findById(1);
		List<String> codes = prodDeriveDao
			.findByBanqueInListStatutSelectCode(banques, o1);
		assertTrue(codes.size() == 3);
	...
	public void testFindByTerminale() {
		...
		Terminale term1 = terminaleDao.findById(1);
		List<ProdDerive> liste = prodDeriveDao
			.findByTerminale(entite, term1);
		...
		Terminale term2 = terminaleDao.findById(2);
	...
	public void testFindByParentAndType() {
		...
		List<ProdDerive> derives = prodDeriveDao.findByParentAndType(
				1, entite, "ADN");
		assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao.findByParentAndType(
				2, entite, "ADN");
		assertTrue(derives.size() == 0);
	...
	public void testFindByParent() {
		Entite entite = entiteDao.findByNom("Echantillon").get(0);
		List<ProdDerive> derives = 
			prodDeriveDao.findByParent(1, entite);
		assertTrue(derives.size() == 1);
	...
	public void testFindByEchantillonPatientNomReturnIds() {
		Banque b1 = banqueDao.findById(1);
		List<Integer> derives = prodDeriveDao
			.findByEchantillonPatientNomReturnIds("DELPHINO", b1);
		assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao
				.findByEchantillonPatientNomReturnIds("876", b1);
			assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao
			.findByEchantillonPatientNomReturnIds("MAYER", b1);
		assertTrue(derives.size() == 0);
		
		derives = prodDeriveDao
				.findByEchantillonPatientNomReturnIds("12%", b1);
			assertTrue(derives.size() == 0);
		
		Banque b2 = banqueDao.findById(2);
		derives = prodDeriveDao
			.findByEchantillonPatientNomReturnIds("MAYER", b2);
		assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao
				.findByEchantillonPatientNomReturnIds("12%", b2);
			assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao
			.findByEchantillonPatientNomReturnIds("JACKSON", b1);
		assertTrue(derives.size() == 0);
		
		derives = prodDeriveDao
			.findByEchantillonPatientNomReturnIds(null, b1);
		assertTrue(derives.size() == 0);
	}
	...
	public void testFindByPrelevementPatientNomreturnIds() {
		Banque b1 = banqueDao.findById(1);
		List<Integer> derives = prodDeriveDao
			.findByPrelevementPatientNomreturnIds("DELPHINO", b1);
		assertTrue(derives.size() == 1);
		
		derives = prodDeriveDao
			.findByPrelevementPatientNomreturnIds("MAYER", b1);
		assertTrue(derives.size() == 0);
		
		derives = prodDeriveDao
			.findByPrelevementPatientNomreturnIds(null, b1);
		assertTrue(derives.size() == 0);
	}
	...
	public void testFindByCodeInListWithBanque() {
		...
		List<Object[]> liste = prodDeriveDao
		.findByCodeInListWithBanque(
				criteres, bks);
		assertTrue(liste.size() == 3);
		...
		criteres.add("PTRA.1.1");
		liste = prodDeriveDao.findByCodeInListWithBanque(
				criteres, bks);
	...
	public void testFindByEchantillonPatientNomInListReturnIds() {
		...
		List<Integer> liste = prodDeriveDao
			.findByEchantillonPatientNomInListReturnIds(
				criteres, bks);
		assertTrue(liste.size() == 2);
		...
		criteres.add("DELPHINO");
		criteres.add("12");
		liste = prodDeriveDao.findByEchantillonPatientNomInListReturnIds(
				criteres, bks);
		assertTrue(liste.size() == 2);
	...
	public void testFindByPrelevementPatientNomInListreturnIds() {
		...
		List<Integer> liste = prodDeriveDao
			.findByPrelevementPatientNomInListreturnIds(
				criteres, bks);
		assertTrue(liste.size() == 1);
		...
		criteres.add("DELPHINO");
		criteres.add("12");
		liste = prodDeriveDao.findByPrelevementPatientNomInListreturnIds(
				criteres, bks);
		assertTrue(liste.size() == 1);
	...
	public void testCrud() throws Exception {
		...
		String codeUpdated = "CodeUp";
		ObjetStatut statut = objetStatutDao.findById(1);
		
		//Emplacement e = emplacementDao.findById(1);
		Banque b = banqueDao.findById(1);
		//assertNotNull(e);
		ProdType pt = prodTypeDao.findById(1);
		assertNotNull(pt);
		p.setCode("code");
		//p.setEmplacement(e);
		p.setProdType(pt);
		...
		assertTrue(prodDeriveDao.findById(
				new Integer(5)).getCode().equals(codeUpdated));
		assertNull(prodDeriveDao.findById(
				new Integer(5)).getVolume());
		assertNull(prodDeriveDao.findById(
				new Integer(5)).getVolumeInit());
		assertNull(prodDeriveDao.findById(
				new Integer(5)).getConc());
		assertNull(prodDeriveDao.findById(
				new Integer(5)).getQuantite());
		assertNull(prodDeriveDao.findById(
				new Integer(5)).getQuantiteInit());
	...
	public void testToString() {
		...
		assertTrue(p1.entiteNom()
			.equals(entiteDao.findByNom("ProdDerive").get(0).getNom()));
		ProdDerive p2 = new ProdDerive();
	...
	public void testClone() {
		...
		if (p1.getEmplacement() != null) {
			assertTrue(p1.getEmplacement().getAdrl().equals(
					p2.getEmplacement().getAdrl()));
		} else {
		...
		if (p1.getDateTransformation() != null) {
			assertTrue(p1.getDateTransformation()
					.equals(p2.getDateTransformation()));
		} else {
		...
		if (p1.getConformeTraitement() != null) {
			assertTrue(p1.getConformeTraitement().equals(
					p2.getConformeTraitement()));
		} else {
			assertNull(p2.getConformeTraitement());
		}
		if (p1.getConformeCession() != null) {
			assertTrue(p1.getConformeCession().equals(
					p2.getConformeCession()));
		} else {
	...
	public void testSortByCode() {
		...
		ProdDerive p3 = new ProdDerive();
		p3.setCode("TEST.1.9");
		ProdDerive p4 = new ProdDerive();
		p4.setCode("TEST.12");
		ProdDerive p5 = new ProdDerive();
		p5.setCode("TEST.3");
		ProdDerive p6 = new ProdDerive();
		p6.setCode("TEST.1.1");
		ProdDerive p7 = new ProdDerive();
		p7.setCode("TEST.1.1.1");
		ProdDerive p8 = new ProdDerive();
		p8.setCode("ZAAA.1.1");
		ProdDerive p9 = new ProdDerive();
		p9.setCode("ZAAA..");
		ProdDerive p10 = new ProdDerive();
		p10.setCode("AAA.1.1");
		List<ProdDerive> prods = new ArrayList<ProdDerive>();
		prods.add(p5); prods.add(p2); prods.add(p3);
		prods.add(p9);
		prods.add(p10);
		prods.add(p8); prods.add(p6); prods.add(p7);
		prods.add(p1); prods.add(p4); 
		Collections
			.sort(prods, new ProdDerive.CodeComparator(false));
	...
	public void testFindByEmplacementDao() {
		List<ProdDerive> derives = prodDeriveDao
				.findByEmplacement(terminaleDao.findById(1), 1);
		assertTrue(derives.size() == 1);
		assertTrue(derives.contains(prodDeriveDao.findById(1)));
		derives = prodDeriveDao.findByEmplacement(terminaleDao.findById(1), 3);
		assertTrue(derives.isEmpty());
		derives = prodDeriveDao.findByEmplacement(null, 1);
		assertTrue(derives.isEmpty());
		derives = prodDeriveDao.findByEmplacement(terminaleDao.findById(1), null);
		assertTrue(derives.isEmpty());
	}
	...
	public void testFindCountByParent() {
		...
		cc = prodDeriveDao.findCountByParent(1, entiteDao.findById(3)).get(0);
		assertTrue(cc.longValue() == 1);
		cc = prodDeriveDao.findCountByParent(2, entiteDao.findById(3)).get(0);
		assertTrue(cc.longValue() == 0);
		cc = prodDeriveDao.findCountByParent(4, entiteDao.findById(3)).get(0);
		assertTrue(cc.longValue() == 1);
		cc = prodDeriveDao.findCountByParent(1, entiteDao.findById(8)).get(0);
		assertTrue(cc.longValue() == 1);
