...
/**
 *
 * Interface pour le DAO du bean de domaine Echantillon. Interface créée le
 * 21/09/09.
 *
 ...
 */
@Repository
public interface EchantillonDao extends CrudRepository<Echantillon, Integer> {
	 ...
	 * @return une liste d'échantillons.
	 */
	@Query("SELECT e FROM Echantillon e WHERE e.echantillonId in (?1)")
	...
	/**
	 * Recherche les échantillons dont le code est 'like' le paramètre pour la
	...
	/**
	 * Recherche les échantillons dont la date de stockage est plus récente que
	...
	/**
	 * Recherche tous les codes Echantillons, sauf celui dont l'id est passé en
	 * paramètre.
	 * 
	 * @param echantillonId Identifiant de l'Echantillon que l'on souhaite exclure
	 ...
	 * @return une liste d'échantillons.
	 */
	@Query("SELECT e FROM Echantillon e WHERE e.objetStatut = ?1")
	 ...
	 * @return une liste d'échantillons.
	 */
	@Query("SELECT e FROM Echantillon e WHERE e.echanQualite = ?1")
	...
	/**
	 * Recherche les échantillons dont le mode de préparation est passé en
	 * paramètre.
	 * 
	 * @param modePrepa ModePrepa pour lequel on recherche des échantillons.
	 * @return une liste d'échantillons.
	 */
	@Query("SELECT e FROM Echantillon e WHERE e.modePrepa = ?1")
	List<Echantillon> findByModePrepa(ModePrepa modePrepa);

	/**
	 * Recherche les échantillons dont le prélèvement est passé en paramètre.
	 * 
	...
	/**
	 * Recherche les échantillons dont le prélèvement et le statut sont passés en
	 * paramètres.
	 * 
	 * @param prelevement Prelevement des échantillons que l'on recherche.
	...
	/**
	 * Recherche les codes d'échantillons dont la banque est passée en paramètre.
	 * 
	 * @param banque Banque des échantillons que l'on recherche.
	 * @return une liste de codes d'échantillons.
	 */
	@Query("SELECT e.code FROM Echantillon e WHERE e.banque = ?1")
	List<String> findByBanqueSelectCode(Banque banque);

	/**
	 * Recherche les codes d'échantillons dont la banque est passée en paramètre et
	 * la quantité est non égale à 0.
	 * 
	 * @param banque Banque des échantillons que l'on recherche.
	 * @return une liste de codes d'échantillons.
	 */
	...
	/**
	 * Recherche les codes d'échantillons dont la banque est passée en paramètre et
	 * la quantité est non égale à 0 OU l'échantillon faisant partie d'une cession
	 * de type traitement
	 * 
	 * @param banque Banque des échantillons que l'on recherche.
	 * @return une liste de codes d'échantillons.
	...
	/**
	 * Recherche les codes d'échantillons dont la banque et le statut sont passés en
	 * paramètres.
	 * 
	 * @param banque      Banque des échantillons que l'on recherche.
	 ...
	 * paramètres.
	 * 
	 * @param banques     Liste des Banques des échantillons que l'on recherche.
	 * @param objetStatut ObjetStatut des échantillons.
	 * @return une liste de codes d'échantillons.
	 */
	...
	/**
	 * INCa reporting: compte les echantillons dont la date de congélation est
	 * incluse dans les dates passées en paramètres et qui appartiennent à la
	 * banque. Une liste d'établissement peut être passée en paramètre pour exclure
	 * tous les échantillons techniqués dans ces derniers (pour compter les
	 * échantillons extérieurs aux établissement locaux par exemple).
	 * 
	 ...
	 * INCa reporting: compte les echantillons dont la date de congélation est
	 * incluse dans les dates passées en paramètres et qui appartiennent à la
	 * banque. Une liste d'établissement est passée en paramètre pour exclure tous
	...
	/**
	 * Compte les échantillons cédés dans le cadre de cession dont le type est passé
	 * en paramètre et dont la date de validation/destruction et incluses entre les
	 * dates passées en paramètres. Les échantillons comptés doivent appartenir à la
	 * liste de banque passée en paramètres.
	 * 
	 * @param cType
	 * @param d1
	 * @param d2
	...
	/**
	 * Compte les échantillons auxquels est affecté un collaborateur passée en
	 * paramètres.
	 * 
	 * @param collaborateur
	 * @return long
	...
	@Query("SELECT count(e) FROM Echantillon e, Operation o WHERE e.echantillonId = o.objetId "
			+ "and e.collaborateur = (?1) AND o.operationType.nom = 'Creation' AND o.entite.nom = 'Echantillon'")
	List<Long> findCountCreatedByCollaborateur(Collaborateur colla);

	/**
	 * Compte les échantillons dont l'opérateur est le collaborateur passée en
	 * paramètres.
	 * 
	 * @param collaborateur
	 * @return long
	...
	/**
	 * 
	 * Recherche parmis les échantillons d'une maladie ceux dont le type est un de
	 * ceux passé en paramètre et appartenant à une des banques de la liste passée
	 * en paramètre. En cas de maladie nulle, utilise le critère prélèvement.
	 * 
	 * @param maladie
	 * @param types
	 * @param banks
	...
	/**
	 * Recherche les Echantillons pour une maladie et un type donnés, issus d'un
	 * prélèvement à la date passée en paramètre.
	 * 
	...
	/**
	 * Compte le nombre d'échantillons d'un prélèvement qui sont stockés ou
	 * reservés.
	 */
	@Query("SELECT count(distinct e) FROM Echantillon e WHERE e.prelevement = ?1 "
	...
	/**
	 * Recherche les échantillons Ids dont les patients ont un nom ou un nip dans la
	 * liste.
	 * 
	...
	/**
	 * Recherche les échantillons Ids dont le code ou le num de labo est dans la
	 * liste.
	 * 
	 * @param criteres Criteres pour lesquels on recherche des échantillons.
	 * @param banques  Banques auxquelles appartiennent les échantillons.
	 ...
	 * dans une terminale (car l'emplacement peut ne pas être créé). Cette recherche
	 * a pour objectif un contrôle de validation à la création des nouveaux objets.
	 * 
	 * @param Terminale
	...
	/**
	 * Recherche des échnatillons correspondant à une liste de code dans une
	 * plateforme donnée
	 * 
