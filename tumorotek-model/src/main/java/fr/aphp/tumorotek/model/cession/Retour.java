...
/**
 * 
 * Objet persistant mappant la table RETOUR.
...
@Entity
@Table(name = "RETOUR")
@NamedQueries(value = {@NamedQuery(name = "Retour.findByObjetId", 
			query = "SELECT r FROM Retour r WHERE r.objetId = ?1 "
		...
		@NamedQuery(name = "Retour.findByObjectAndImpact", 
			query = "SELECT r FROM Retour r WHERE r.objetId = ?1" 
					+ " AND r.entite = ?2 AND r.impact = ?3 "
					+ "ORDER BY r.dateSortie"),
		@NamedQuery(name = "Retour.findByObjectsDateRetourEmpty", 
			query = "SELECT r FROM Retour r WHERE "
					+ " r.entite = ?2 AND r.dateRetour is null"
					+ " AND r.objetId in (?1)"),
		@NamedQuery(name = "Retour.findByExcludedId", 
			query = "SELECT r FROM Retour r WHERE r.retourId != ?1 "
				+ " AND r.objetId = ?2" 
				+ " AND r.entite = ?3"),
		@NamedQuery(name = "Retour.findByMaxId", 
		...
		@NamedQuery(name = "Retour.findByObjDates", 
			query = "SELECT r FROM Retour r where r.dateSortie <= ?1 "
				+ "AND r.dateRetour >= ?1 AND r.objetId = ?2 and r.entite = ?3 "
				+ "AND r.retourId <> ?4"),
		...
		@NamedQuery(name = "Retour.findByObjInsideDates", 
			query = "SELECT r FROM Retour r where r.dateSortie >= ?1 "
					+ "AND r.dateRetour <= ?2 AND r.objetId = ?3 and r.entite = ?4 "
					+ "AND r.retourId <> ?5"),
		@NamedQuery(name = "Retour.findObjIdsInsideDatesEntite", 
			query = "SELECT distinct r.objetId FROM Retour r where r.dateSortie >= ?1 "
				+ "AND r.dateRetour <= ?2 AND r.entite = ?3"),
		@NamedQuery(name = "Retour.findByCollaborateur", 
...
public class Retour implements TKdataObject, Serializable {
	...
	@GenericGenerator(name = "autoincrement", strategy = "native", 
		parameters = {@Parameter(name="sequence", value="retourSeq")})
	public Integer getRetourId() {
		return this.retourId;
	}

	public void setRetourId(Integer id) {
		this.retourId = id;
	}
	...
	public void setObjetId(Integer id) {
		this.objetId = id;
	}
	...
	public void setDateSortie(Calendar cal) {
		if (cal != null) {
	...
	public void setDateRetour(Calendar cal) {
		if (cal != null) {
	...
	public void setTempMoyenne(Float temp) {
	...
	public void setOldEmplacementAdrl(String oAdrl) {
	...
	public void setCession(Cession cess) {
		this.cession = cess;
	}
	...
	public void setConteneur(Conteneur cu) {
		this.conteneur = cu;
	}
	...
	public void setIncident(Incident inc) {
		this.incident = inc;
	}
	...
	public void setObjetStatut(ObjetStatut o) {
		this.objetStatut = o;
	}
	...
	public void setTkObject(TKStockableObject t) {
		this.tkObject = t;
	}

	/**
	 * 2 objets sont considérés comme égaux s'ils ont les même dates
	 * et le même couple objet-entite id.
