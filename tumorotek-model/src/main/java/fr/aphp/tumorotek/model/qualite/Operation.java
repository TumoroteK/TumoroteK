...
@NamedQueries(value = {@NamedQuery(name = "Operation.findByObjetIdAndEntite", 
			query = "SELECT o FROM Operation o WHERE o.objetId = ?1 AND"
				+ " o.entite = ?2 ORDER BY o.date, o.operationId"), 
		@NamedQuery(name = "Operation.findByObjetIdAndEntiteForHistorique", 
			query = "SELECT o FROM Operation o WHERE o.objetId = ?1 AND"
				+ " o.entite = ?2 AND o.operationType.nom != 'Login' " 
				+ "AND o.operationType.nom != 'Logout' " 
				+ "ORDER BY o.date, o.operationId"), 
		@NamedQuery(name = "Operation.findByUtilisateur", 
			query = "SELECT o FROM Operation o WHERE o.utilisateur = ?1"),
		@NamedQuery(name = "Operation.findByExcludedId", 
			query = "SELECT o FROM Operation o " 
				+ "WHERE o.operationId != ?1"),
		@NamedQuery(name = "Operation.findByDate", 
			query = "SELECT o FROM Operation o " 
				+ "WHERE o.date = ?1"),
		@NamedQuery(name = "Operation.findByAfterDate", 
			query = "SELECT o FROM Operation o " 
				+ "WHERE o.date >= ?1 order by o.date"),
		@NamedQuery(name = "Operation.findByBeforeDate", 
			query = "SELECT o FROM Operation o " 
				+ "WHERE o.date <= ?1 order by o.date"),
		@NamedQuery(name = "Operation.findByBetweenDates", 
			query = "SELECT o FROM Operation o " 
				+ "WHERE o.date >= ?1 AND o.date <= ?2 order by o.date"),
		@NamedQuery(name = "Operation.findByObjetIdEntiteAndOperationType", 
			query = "SELECT o FROM Operation o WHERE o.objetId = ?1 AND"
				+ " o.entite = ?2 AND o.operationType = ?3") })
public class Operation implements Serializable {
	...
	public String toString() {
		if (this.operationType != null && this.entite != null
				&& this.objetId != null) {
			return "{" + this.operationType.getNom() + ", " 
						+ this.entite.getNom() + ", "
						+ this.objetId + "}";
		} else {
	...
	@GenericGenerator(name = "autoincrement", strategy = "native", 
		parameters = {@Parameter(name="sequence", value="opSeq")})
	...
	public void setOperationType(OperationType opeType) {
	...
	public void setIdentificationDossier(String _i) {
	...
	public boolean equals(Object obj) {
		...
		return ((this.objetId == test.objetId || (this.objetId != null 
			...
			&& (this.operationType == test.operationType 
					...
					&& this.operationType.equals(test.operationType))) 
			&& (this.entite == test.entite || (this.entite != null 
							&& this.entite.equals(test.entite)))
			&& (this.date == test.date || (this.date != null 
