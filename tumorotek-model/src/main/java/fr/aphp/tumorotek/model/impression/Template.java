...
@Entity
@Table(name = "TEMPLATE")
@NamedQueries(value = {@NamedQuery(name = "Template.findByBanque", 
			query = "SELECT t FROM Template t " 
				+ "WHERE t.banque = ?1 " 
				+ "ORDER BY t.entite, t.nom"),
		@NamedQuery(name = "Template.findByBanqueEntite", 
			query = "SELECT t FROM Template t " 
				+ "WHERE t.banque = ?1 AND t.entite = ?2 " 
				+ "ORDER BY t.nom"),
		@NamedQuery(name = "Template.findByExcludedId", 
			query = "SELECT t FROM Template t " 
				+ "WHERE t.banque = ?1 AND t.templateId != ?2") })
public class Template extends Object implements TKdataObject, 
														Serializable {
	...
	private Set<BlocImpressionTemplate> blocImpressionTemplates 
		= new HashSet<BlocImpressionTemplate>();
	...
	public void setEnTete(String tete) {
		this.enTete = tete;
	}
	...
	public void setPiedPage(String pied) {
		this.piedPage = pied;
	}
	...
	public void setBlocImpressionTemplates(
			Set<BlocImpressionTemplate> blocImpressions) {
	...
	public boolean equals(Object obj) {
		...
		} else { return (this.nom.equals(test.nom) 
