...
/**
 *
 * Objet persistant mappant la table CODE_ASSIGNE. Classe créée le 17/09/09.
 *
...
//@NamedQueries(
//   value = {@NamedQuery(name = "CodeAssigne.findByCodeLike", query = "SELECT c FROM CodeAssigne c WHERE c.code like ?1"),
...
//      @NamedQuery(name = "CodeAssigne.findByLibelleLike", query = "SELECT c FROM CodeAssigne c WHERE c.libelle like ?1"),
...
//            + "AND c.export = 1 AND c.isOrgane = 1" + "ORDER BY c.echantillon.echantillonId")})
public class CodeAssigne implements CodeCommon, Serializable {
	...
	@GenericGenerator(name = "autoincrement", strategy = "native", parameters = {
			@Parameter(name = "sequence", value = "codeAssigneSeq") })
	public Integer getCodeAssigneId() {
		return this.codeAssigneId;
	}

	public void setCodeAssigneId(final Integer id) {
	...
	public void setIsMorpho(final Boolean morpho) {
	...
	public void setIsOrgane(final Boolean isO) {
		this.isOrgane = isO;
	}
	...
	public void setCodeRefId(final Integer codeRef) {
	...
	/**
	 * 2 codes sont considérés comme égaux s'ils ont les mêmes code venant de la
	 * meme table de codage et la meme reference vers l'échantillon.
	 * 
	...
	public CodeAssigne clone() {
		final CodeAssigne clone = new CodeAssigne();
