...
/**
 * 
 * Objet persistant mappant la table CIM_MASTER.
...
@NamedQueries(value = {@NamedQuery(name = "CimMaster.findByCodeLike", 
			query = "SELECT c FROM CimMaster c WHERE c.code like ?1"),
		@NamedQuery(name = "CimMaster.findByLibelleLike", 
			query = "SELECT c FROM CimMaster c WHERE c.libelle like ?1"),
		@NamedQuery(name = "CimMaster.findByLevel", 
			query = "SELECT c FROM CimMaster c WHERE c.level = ?1"),
		@NamedQuery(name = "CimMaster.findByCimo3", 
			query = "SELECT c FROM CimMaster c WHERE c.cimo3 = ?1") })
public class CimMaster implements CodeCommon, Serializable {
	...
	private Set<CimLibelle> cimLibelles = new HashSet<CimLibelle>(); 
	...
	public void setCimLibelles(Set<CimLibelle> cimLibs) {
    ...
    @JoinTable(
            ...
            inverseJoinColumns = @JoinColumn(name = "ADICAP_ID")
    )
