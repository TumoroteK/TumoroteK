...
@NamedQueries(value = {@NamedQuery(name = "ConditMilieu.findByMilieu", 
			query = "SELECT c FROM ConditMilieu c WHERE c.milieu like ?1"),
		@NamedQuery(name = "ConditMilieu.findByExcludedId", 
			query = "SELECT c FROM ConditMilieu c " 
				+ "WHERE c.conditMilieuId != ?1"),
		@NamedQuery(name = "ConditMilieu.findByOrder", 
			query = "SELECT c FROM ConditMilieu c "
				+ "WHERE c.plateforme = ?1 ORDER BY c.milieu") })
public class ConditMilieu implements Serializable, TKThesaurusObject {
	...
	public void setConditMilieuId(Integer id) {
	...
	public void setMilieu(String mil) {
		this.milieu = mil;
	}
	...
	public boolean equals(Object obj) {
		...
		ConditMilieu test = (ConditMilieu) obj;
		return ((this.milieu == test.milieu || (this.milieu != null 
				&& this.milieu.equals(test.milieu)))
