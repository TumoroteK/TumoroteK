...
@Entity
@Table(name = "RISQUE")
@NamedQueries(value = {@NamedQuery(name = "Risque.findByNom",
			query = "SELECT r FROM Risque r WHERE r.nom like ?1"),
		@NamedQuery(name = "Risque.findByInfectieux", 
			query = "SELECT r FROM Risque r WHERE r.infectieux = ?1"),
		@NamedQuery(name = "Risque.findByPrelevementId", 
			query = "SELECT r FROM Risque r "
				+ "left join r.prelevements p "
				+ "WHERE p.prelevementId = ?1"),
		@NamedQuery(name = "Risque.findByExcludedId", 
			query = "SELECT r FROM Risque r " 
				+ "WHERE r.risqueId != ?1"),
		@NamedQuery(name = "Risque.findByOrder", 
			query = "SELECT r FROM Risque r "
					+ "WHERE r.plateforme = ?1 ORDER BY r.nom") })
public class Risque implements Serializable, TKThesaurusObject {
	...
	public void setRisqueId(Integer id) {
		this.risqueId = id;
	}
	...
	public void setInfectieux(Boolean inf) {
		this.infectieux = inf;
	}
	...
	@ManyToMany(
			targetEntity = Prelevement.class
	)
    @JoinTable(
    		name = "PRELEVEMENT_RISQUE",
            joinColumns = @JoinColumn(name = "RISQUE_ID"),
