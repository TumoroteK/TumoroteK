...
@NamedQueries(value = {@NamedQuery(name = "CodeDossier.findByNomLike",
			query = "SELECT c FROM CodeDossier c WHERE c.nom like ?1 "
				+ "AND c.banque = ?2"),
		@NamedQuery(name = "CodeDossier.findByCodeDossierParent",
			query = "SELECT c FROM CodeDossier c " 
				+ "WHERE c.dossierParent = ?1"),
		@NamedQuery(name = "CodeDossier.findByRootCodeDossierUtilisateur",
			query = "SELECT c FROM CodeDossier c " 
				+ "WHERE c.dossierParent is null "
				+ "AND c.banque = ?1 AND c.codeSelect = 0"),
		@NamedQuery(name = "CodeDossier.findByRootCodeDossierSelect",
			query = "SELECT c FROM CodeDossier c " 
				+ "WHERE c.dossierParent is null "
				+ "AND c.banque = ?2 AND c.utilisateur = ?1 "
				+ "AND c.codeSelect = 1"),
		...
		@NamedQuery(name = "CodeDossier.findByRootDossierBanque",
			query = "SELECT c FROM CodeDossier c " 
				...
				+ "AND c.banque = ?1 "
				+ "AND c.codeSelect = ?2"), 									
		@NamedQuery(name = "CodeDossier.findByExcludedId", 
			query = "SELECT c FROM CodeDossier c "
				+ "WHERE c.codeDossierId != ?1") })
...
private static final long serialVersionUID = 1L;
	...
	public void setCodeDossierId(Integer cDosId) {
	...
	public void setCodeSelect(Boolean codeSel) {
	...
	public void setDossierParent(CodeDossier dosParent) {
	...
	public boolean equals(Object obj) {
		...
		CodeDossier test = (CodeDossier) obj;
	...
	public CodeDossier clone() {
		CodeDossier clone = new CodeDossier();
