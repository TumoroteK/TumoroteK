...
@Entity
@Table(name = "CONTEXTE")
@NamedQueries(value = {@NamedQuery(name = "Contexte.findByNom",
		query = "SELECT c FROM Contexte c WHERE c.nom = ?1"),
		@NamedQuery(name = "Contexte.findByBanqueId", 
		query = "SELECT c FROM Contexte c "
			+ "left join c.banques b "
			+ "WHERE b.banqueId = ?1"),
		@NamedQuery(name = "Contexte.findByOrder", 
...
//	}
	...
	public void setContexteId(Integer cId) {
		this.contexteId = cId;
	}
	...
	@ManyToMany(
		mappedBy = "contextes",
	    targetEntity = Catalogue.class
	)
	public Set<Catalogue> getCatalogues() {
		return catalogues;
	}

	public void setCatalogues(Set<Catalogue> catals) {
