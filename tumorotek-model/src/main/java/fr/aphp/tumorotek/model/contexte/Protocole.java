...
@NamedQueries(value = {@NamedQuery(name = "Protocole.findByNom", 
			query = "SELECT p FROM Protocole p WHERE p.nom like ?1 "
				+ "order by p.nom"),
		@NamedQuery(name = "Protocole.findByExcludedId", 
			query = "SELECT p FROM Protocole p " 
				 + "WHERE p.protocoleId != ?1"), 
		@NamedQuery(name = "Protocole.findByOrder", 
					query = "SELECT p FROM Protocole p "
						+ "WHERE p.plateforme = ?1 ORDER BY p.nom")
		})
public class Protocole implements java.io.Serializable, TKThesaurusObject {
	...
	public void setProtocoleId(Integer pId) {
	...
	@ManyToMany(
	        mappedBy = "protocoles",
	        targetEntity = PrelevementSero.class
	)
