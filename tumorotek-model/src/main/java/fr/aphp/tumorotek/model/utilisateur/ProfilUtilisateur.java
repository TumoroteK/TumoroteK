...
@AssociationOverrides({
	@AssociationOverride(name = "pk.profil", 
...
@NamedQueries(value = {@NamedQuery(name = "ProfilUtilisateur.findDoublon", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk = ?1"),
		@NamedQuery(name = "ProfilUtilisateur.findByExcludedPK", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk != ?1"),
		@NamedQuery(name = "ProfilUtilisateur.findByProfil", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk.profil = ?1 AND p.pk.utilisateur.archive = ?2"
				+ " ORDER BY p.pk.utilisateur.login"),
		@NamedQuery(name = "ProfilUtilisateur.findByUtilisateur", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk.utilisateur = ?1 AND p.pk.banque.archive = ?2 "
				+ "order by p.pk.banque.nom"), 
		@NamedQuery(name = "ProfilUtilisateur.findByBanque", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk.banque = ?1 AND p.pk.utilisateur.archive = ?2 "
				+ "ORDER BY p.pk.utilisateur.login"),
		@NamedQuery(name = "ProfilUtilisateur.findByUtilisateurProfil", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk.utilisateur = ?1 AND p.pk.profil = ?2"),
		@NamedQuery(name = "ProfilUtilisateur.findByUtilisateurBanque", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk.utilisateur = ?1 AND p.pk.banque = ?2"),
		@NamedQuery(name = "ProfilUtilisateur.findByBanqueProfil", 
			query = "SELECT p FROM ProfilUtilisateur p " 
				+ "WHERE p.pk.banque = ?1 AND p.pk.profil = ?2") })
public class ProfilUtilisateur implements Serializable, Comparable<ProfilUtilisateur> {
	...
	@EmbeddedId
	@AttributeOverrides({
		@AttributeOverride(name = "profil", 
				column = @Column(name = "PROFIL_ID")),
		@AttributeOverride(name = "utilisateur", 
				column = @Column(name = "UTILISATEUR_ID")),
		@AttributeOverride(name = "banque", 
				column = @Column(name = "BANQUE_ID")) })
    public ProfilUtilisateurPK getPk() {
        return pk;
    }
 
	public void setPk(ProfilUtilisateurPK ppk) {
        this.pk = ppk;
    }
	...
	public boolean equals(Object obj) {
		...
		ProfilUtilisateur test = (ProfilUtilisateur) obj;
		return ((this.pk == test.pk 
				|| (this.pk != null 
	...
	public ProfilUtilisateur clone() {
		ProfilUtilisateur clone = new ProfilUtilisateur();
	...
	@Override
	public int compareTo(ProfilUtilisateur o) {
