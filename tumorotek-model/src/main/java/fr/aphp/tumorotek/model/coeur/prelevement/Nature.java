...
@Entity
@Table(name = "NATURE")
@NamedQueries(value = {@NamedQuery(name = "Nature.findByNature", 
			query = "SELECT n FROM Nature n WHERE n.nature like ?1"),
		@NamedQuery(name = "Nature.findByExcludedId", 
			query = "SELECT n FROM Nature n " 
				+ "WHERE n.natureId != ?1"),
		@NamedQuery(name = "Nature.findByOrder", 
				query = "SELECT n FROM Nature n "
						+ "WHERE n.plateforme = ?1 ORDER BY n.nature") })
public class Nature implements Serializable, TKThesaurusObject {
	...
	public void setNatureId(Integer id) {
		this.natureId = id;
	}
	...
	public boolean equals(Object obj) {
		...
		Nature test = (Nature) obj;
		return ((this.nature == test.nature || (this.nature != null 
				&& this.nature.equals(test.nature))) 
