...
@NamedQueries(value = {
		@NamedQuery(name = "AnnotationValeur.findByChampAndObjetId", 
			query = "SELECT a FROM AnnotationValeur a WHERE"
							+ " a.champAnnotation = ?1 AND a.objetId = ?2"),
		@NamedQuery(name = "AnnotationValeur.findByTableAndBanque", 
			query = "SELECT a FROM AnnotationValeur a JOIN a.champAnnotation c"
					+ " WHERE c.tableAnnotation = ?1 AND a.banque = ?2"),
		@NamedQuery(name = "AnnotationValeur.findByExcludedId", 
			query = "SELECT a FROM AnnotationValeur a " 
					+ "WHERE a.champAnnotation = ?1 AND a.objetId = ?2 "
					+ "AND a.annotationValeurId != ?3"),
		@NamedQuery(name = "AnnotationValeur.findByObjectIdAndEntite", 
				query = "SELECT a FROM AnnotationValeur a "
					+ "join a.champAnnotation as chp "
					+ "join chp.tableAnnotation as tbl "
					+ "WHERE a.objetId = ?1 AND tbl.entite = ?2 "
					+ "ORDER BY a.champAnnotation.champAnnotationId"),
		@NamedQuery(name = "AnnotationValeur.findCountByItem", 
				query = "SELECT count(a) FROM AnnotationValeur a WHERE "
					+ "a.item = ?1"),
		@NamedQuery(name = "AnnotationValeur.findCountByTableAnnotationBanque", 
				query = "SELECT count(a) FROM AnnotationValeur a "
					+ "JOIN a.champAnnotation c "
					+ "WHERE c.tableAnnotation = ?1 AND a.banque = ?2")
				})
public class AnnotationValeur extends AnnotationCommon implements Serializable, 
														TKFileSettableObject {
	
	private Log log = LogFactory.getLog(AnnotationValeur.class);
	...
	public void setAnnotationValeurId(Integer id) {
	...
	public void setObjetId(Integer objId) {
		this.objetId = objId;
	}
	...
	@Override
	public void setAlphanum(String alpha) {
		this.alphanum = alpha;
	}
	...
	@Override
	public void setTexte(String tex) {
		this.texte = tex;
	}
	...
	@Override
	public void setChampAnnotation(ChampAnnotation chpA) {
	...
	public boolean equals(Object obj) {
		...
		AnnotationValeur test = (AnnotationValeur) obj;

		return (((this.objetId != null 
				&& this.objetId.equals(test.objetId))
					|| this.objetId == test.objetId)
				&& (this.champAnnotation == test.champAnnotation 
					|| (this.champAnnotation != null 
					&& this.champAnnotation.equals(test.champAnnotation))) 
				&& (this.item == test.item || (this.item != null 
						&& this.item.equals(test.item)))
	...
	public String toString() {
		...
		if (this.champAnnotation != null) {
			...
			if (this.alphanum != null) {
				...
				output = output + new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
	...
	public AnnotationValeur clone() {
		AnnotationValeur clone = new AnnotationValeur();
	...
	public Object getValeur() {
		if (champAnnotation.getDataType().getType().equals("alphanum")) {
			...
			return getTexte();
		} else if (champAnnotation.getDataType()
