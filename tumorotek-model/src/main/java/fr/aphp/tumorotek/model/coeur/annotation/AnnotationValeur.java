...
public class AnnotationValeur extends AnnotationCommon implements Serializable, TKFileSettableObject {
	...
	@GenericGenerator(name = "autoincrement", strategy = "native", parameters = {
			@Parameter(name = "sequence", value = "annoValSeq") })
	...
	/**
	 * 2 valeurs sont considerees comme egales si ils ont les mêmes references vers
	 * le champ annotation, vers l'objet et vers la banque auxquelles elles sont
	 * attribuees. La propriete item intervient pour differencier les valeurs lors
	 * d'une selection multiple.
	 * 
	 ...
	 * l'item auxquels la valeur est attribuee.
	 * 
	...
	private Object getValeur(DataType dataType) {
		...
		case "thesaurus":
		case "thesaurusM":
			valeur = getItem().getLabel(); // TODO Pourquoi on retourne un string et non l'Item ? le string est retourné
											// avec formateAnnotationValeur()
			break;
		case "fichier":
			valeur = getFichier().getNom(); // TODO Pourquoi on retourne un string et non le fichier ? le string est
											// retourné avec formateAnnotationValeur().
			break;
		case "duree":
	...
	/**
	 * Assigne la valeur selon le type d'objet passé en paramètre et selon le
	 * datatype si renseigné
	 * 
	 * @param valeur valeur
	 */
	@Transient
