...
public interface TKStockableObject extends TKAnnotableObject {
	...
	public static class CodeComparator implements Comparator<TKStockableObject> {		
		...
		public CodeComparator (boolean c) {
			compareId = true;
		}
		
		public int compare(TKStockableObject p1, TKStockableObject p2) {
...
//			}
		...
		private int compareAfterDot(String s1, String s2) {
			if (s1.contains(".") && s2.contains(".") 
					&& s1.substring(0, s1.indexOf("."))
					.equals(s2.substring(0, s2.indexOf(".")))) {
				try {
					return new Integer(s1.substring(s1.indexOf(".") + 1))
						.compareTo(new Integer(s2.substring(s2.indexOf(".") + 1)));
				} catch (NumberFormatException ne) {
					return compareAfterDot(s1.substring(s1.indexOf(".") + 1),
