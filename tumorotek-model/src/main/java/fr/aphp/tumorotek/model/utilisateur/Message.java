...
@Entity
@Table(name = "MESSAGE")
@NamedQueries(value = {@NamedQuery(name = "Message.findByExpediteur", 
				query = "SELECT m FROM Message m WHERE " 
						+ "m.expediteur = ?1"),
				@NamedQuery(name = "Message.findByDestinataire", 
						query = "SELECT m FROM Message m WHERE "
						+ "m.destinataire = ?1"),
				@NamedQuery(name = "Message.findDoublon", 
					query = "SELECT m FROM Message m WHERE " 
						+ "m.objet = ?1 AND m.expediteur = ?2"
						+ " AND m.destinataire = ?3"
						+ " AND m.texte = ?4") })
public class Message implements Serializable {
	...
	public void setImportance(Integer imp) {
		this.importance = imp;
	}
	...
	public void setExpediteur(Utilisateur exp) {
		this.expediteur = exp;
	}
	...
	public boolean equals(Object obj) {
		...
		if (this.messageId != null) { //l'objet a id assigne
			...
			return ((this.objet == test.objet 
					|| (this.objet != null 
							&& this.objet.equals(test.objet))) 
					&& (this.expediteur == test.expediteur 
							|| (this.expediteur != null 
							&& this.expediteur.equals(test.expediteur))) 
					&& (this.destinataire == test.destinataire 
							|| (this.destinataire != null 
							&& this.destinataire.equals(test.destinataire)))
					&& (this.texte == test.texte || (this.texte != null 
									&& this.texte.equals(test.texte))));
		}
	}
