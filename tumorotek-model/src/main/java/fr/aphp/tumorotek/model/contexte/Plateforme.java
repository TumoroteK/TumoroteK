...
@NamedQueries(value = {@NamedQuery(name = "Plateforme.findByNom", 
			query = "SELECT p FROM Plateforme p WHERE p.nom = ?1"), 
		@NamedQuery(name = "Plateforme.findByAlias", 
				query = "SELECT p FROM Plateforme p WHERE p.alias = ?1"),
		@NamedQuery(name = "Plateforme.findByCollaborateur", 
				query = "SELECT p FROM Plateforme p " 
					+ "WHERE p.collaborateur = ?1"),
		@NamedQuery(name = "Plateforme.findByBanqueId", 
				query = "SELECT p FROM Plateforme p "
					+ "left join p.banques b "
					+ "WHERE b.banqueId = ?1"),
		@NamedQuery(name = "Plateforme.findByIdWithFetch", 
				query = "SELECT p FROM Plateforme p LEFT JOIN FETCH " 
					+ "p.collaborateur WHERE p.plateformeId = ?1"),
		@NamedQuery(name = "Plateforme.findByOrder", 
				query = "SELECT p FROM Plateforme p ORDER BY p.nom"),
		@NamedQuery(name = "Plateforme.findByExcludedId", 
				query = "SELECT p FROM Plateforme p " 
					+ "WHERE p.plateformeId != ?1") })
public class Plateforme implements java.io.Serializable,
	TKFantomableObject, TKdataObject, Comparable<Plateforme> {
	...
	private Set<NonConformite> nonConformites = new HashSet<NonConformite>();
	 ...
	 * @param a est l'alias.
	 */
	public Plateforme(Integer id, String n, String a) {
	...
	public void setPlateformeId(Integer pId) {
	...
	public void setConteneurPlateformes(Set<ConteneurPlateforme> conts) {
		this.conteneurPlateformes = conts;
	}

	@ManyToMany(
        mappedBy = "plateformes",
	...
	public void setUtilisateurs(Set<Utilisateur> u) {
		this.utilisateurs = u;
	}
	...
	public void setSModeles(Set<SModele> ss) {
		this.sModeles = ss;
	}
	...
	public void setContrats(Set<Contrat> c) {
		this.contrats = c;
	}
	...
	public boolean equals(Object obj) {
		...
		Plateforme test = (Plateforme) obj;
	...
	public Plateforme clone() {
		Plateforme clone = new Plateforme();
	...
	@Override
	public int compareTo(Plateforme arg0) {
