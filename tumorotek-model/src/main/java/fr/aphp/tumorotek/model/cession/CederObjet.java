...
@AssociationOverrides({
	@AssociationOverride(name = "pk.cession", 
			joinColumns = @JoinColumn(name = "CESSION_ID", 
					referencedColumnName = "CESSION_ID")),
	@AssociationOverride(name = "pk.entite", 
			joinColumns = @JoinColumn(name = "ENTITE_ID",
					referencedColumnName = "ENTITE_ID"))
        })
@AttributeOverride(column = @Column(name = "OBJET_ID"), name = "objetId")
@NamedQueries(value = {@NamedQuery(name = "CederObjet.findByExcludedPK", 
			query = "SELECT c FROM CederObjet c " 
				+ "WHERE c.pk != ?1"),
		@NamedQuery(name = "CederObjet.findByEntite", 
			query = "SELECT c FROM CederObjet c " 
				+ "WHERE c.pk.entite = ?1"),
		@NamedQuery(name = "CederObjet.findByEntiteObjet", 
			query = "SELECT c FROM CederObjet c " 
				+ "WHERE c.pk.entite = ?1 AND c.pk.objetId = ?2"),
		@NamedQuery(name = "CederObjet.findByObjetId", 
				query = "SELECT c FROM CederObjet c " 
					+ "WHERE c.pk.objetId = ?1"),
		@NamedQuery(name = "CederObjet.findByCessionEntite", 
			query = "SELECT c FROM CederObjet c " 
				+ "WHERE c.pk.cession = ?1 AND c.pk.entite = ?2"),
		@NamedQuery(name = "CederObjet.findObjectsCessedCount",
			query = "SELECT count(c.pk.objetId) FROM CederObjet c " 
				+ "WHERE c.pk.cession = ?1 and c.pk.entite = ?2"), 
		@NamedQuery(name = "CederObjet.findCodesEchantillonByCession",
			query = "SELECT e.code FROM CederObjet c, Echantillon e "
				+ "WHERE c.pk.objetId = e.echantillonId "
				+ "AND c.pk.cession = ?1 AND c.pk.entite.entiteId = 3 "
				+ "ORDER BY e.code"), 
		@NamedQuery(name = "CederObjet.findCodesDeriveByCession",
			query = "SELECT e.code FROM CederObjet c, ProdDerive e "
				+ "WHERE c.pk.objetId = e.prodDeriveId "
				+ "AND c.pk.cession = ?1 AND c.pk.entite.entiteId = 8 "
				+ "ORDER BY e.code"), 
		@NamedQuery(name = "CederObjet.findCountObjCession",
			query = "SELECT count(c.pk.cession) FROM CederObjet c " 
				+ "WHERE c.pk.objetId = ?1 AND c.pk.entite = ?2"),
	})
public class CederObjet implements Serializable {
	...
	@EmbeddedId
	@AttributeOverrides({
		@AttributeOverride(name = "cession", 
				column = @Column(name = "CESSION_ID")),
		@AttributeOverride(name = "entite", 
				column = @Column(name = "ENTITE_ID")),
		@AttributeOverride(name = "objetId", 
				column = @Column(name = "OBJET_ID")) })
	public CederObjetPK getPk() {
		return pk;
	}

	public void setPk(CederObjetPK cpk) {
		this.pk = cpk;
	}
	...
	public void setQuantiteUnite(Unite qu) {
	...
	public boolean equals(Object obj) {
		...
		CederObjet test = (CederObjet) obj;
		return ((this.pk == test.pk || (this.pk != null 
				&& this.pk.equals(test.pk)))	
		);
	}
	...
	public CederObjet clone() {
		CederObjet clone = new CederObjet();
