...
@AssociationOverrides({
	@AssociationOverride(name = "pk.champEntite", 
			joinColumns = @JoinColumn(name = "CHAMP_ENTITE_ID", 
					referencedColumnName = "CHAMP_ENTITE_ID")),
	@AssociationOverride(name = "pk.blocImpression", 
			joinColumns = @JoinColumn(name = "BLOC_IMPRESSION_ID",
					referencedColumnName = "BLOC_IMPRESSION_ID"))
        })
@NamedQueries(value = {@NamedQuery(name = "ChampEntiteBloc." 
		+ "findByBlocImpression", 
			query = "SELECT c FROM ChampEntiteBloc c " 
				+ "WHERE c.pk.blocImpression = ?1 " 
				+ "ORDER BY c.ordre"),
		@NamedQuery(name = "ChampEntiteBloc." 
				+ "findByExcludedPK", 
			query = "SELECT c FROM ChampEntiteBloc c " 
				+ "WHERE c.pk != ?1") })
public class ChampEntiteBloc implements Serializable {
	...
	@EmbeddedId
	@AttributeOverrides({
		@AttributeOverride(name = "champEntite", 
				column = @Column(name = "CHAMP_ENTITE_ID")),
		@AttributeOverride(name = "blocImpression", 
				column = @Column(name = "BLOC_IMPRESSION_ID")) })
	public ChampEntiteBlocPK getPk() {
		return pk;
	}

	public void setPk(ChampEntiteBlocPK p) {
		this.pk = p;
	}
	...
	public void setChampEntite(ChampEntite champ) {
	...
	public boolean equals(Object obj) {
		...
		ChampEntiteBloc test = (ChampEntiteBloc) obj;
	 ...
	 */
	public ChampEntiteBloc clone() {
		ChampEntiteBloc clone = new ChampEntiteBloc();
