...
@Entity
@Table(name = "PROFIL")
@NamedQueries(value = {@NamedQuery(name = "Profil.findByOrder",
			query = "SELECT p FROM Profil p ORDER BY p.nom"),
		@NamedQuery(name = "Profil.findByNom",
			query = "SELECT p FROM Profil p WHERE p.nom like ?1"),
		@NamedQuery(name = "Profil.findByAnonyme", 
			query = "SELECT p FROM Profil p " 
				+ "WHERE p.anonyme = ?1"),
		@NamedQuery(name = "Profil.findByDoublon", 
			query = "SELECT p FROM Profil p " 
				+ "WHERE p.nom = ?1 AND p.anonyme = ?2"),
		@NamedQuery(name = "Profil.findByExcludedId", 
			query = "SELECT p FROM Profil p " 
				+ "WHERE p.profilId != ?1"),
		@NamedQuery(name = "Profil.findByPlateformeAndArchive", 
			query = "SELECT p FROM Profil p " 
					+ "WHERE p.plateforme = ?1 AND p.archive = ?2 "
					+ "ORDER BY p.nom")})
public class Profil implements TKdataObject, Serializable, Comparable<Profil> {
	...
	public void setProfilId(Integer id) {
		this.profilId = id;
	}
	...
	public void setAccesAdministration(Boolean aAdministration) {
	...
	public void setDroitObjets(Set<DroitObjet> droits) {
	...
	public void setProfilExport(Integer pExport) {
