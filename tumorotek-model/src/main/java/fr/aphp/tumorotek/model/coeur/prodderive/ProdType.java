...
@NamedQueries(value = {@NamedQuery(name = "ProdType.findByType", 
			query = "SELECT p FROM ProdType p WHERE p.type like ?1"),
		@NamedQuery(name = "ProdType.findByProdDeriveId", 
			query = "SELECT p FROM ProdType p "
				+ "left join p.prodDerives d " 
				+ "WHERE d.prodDeriveId = ?1"),
		@NamedQuery(name = "ProdType.findByExcludedId", 
			query = "SELECT p FROM ProdType p " 
				+ "WHERE p.prodTypeId != ?1"),
		@NamedQuery(name = "ProdType.findByOrder", 
			query = "SELECT p FROM ProdType p "
...
public class ProdType implements Serializable, TKThesaurusObject {
	...
	public void setProdTypeId(Integer id) {
		this.prodTypeId = id;
	}
