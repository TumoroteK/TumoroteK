...
@AssociationOverrides({
	...
	@AssociationOverride(name = "pk.operationType", 
			joinColumns = @JoinColumn(name = "OPERATION_TYPE_ID"))
        })
@NamedQueries(value = {
		@NamedQuery(name = "DroitObjet.findByDoublon", 
			query = "SELECT d FROM DroitObjet d WHERE d.pk = ?1"),
		@NamedQuery(name = "DroitObjet.findByExcludedPK", 
			query = "SELECT d FROM DroitObjet d " 
				+ "WHERE d.pk != ?1"),
		@NamedQuery(name = "DroitObjet.findByProfil", 
			query = "SELECT d FROM DroitObjet d " 
				+ "WHERE d.pk.profil = ?1"),
		@NamedQuery(name = "DroitObjet.findByProfilEntite", 
			query = "SELECT d FROM DroitObjet d " 
				+ "WHERE d.pk.profil = ?1 AND d.pk.entite = ?2"),
		@NamedQuery(name = "DroitObjet.findByProfilOperation", 
			query = "SELECT d FROM DroitObjet d " 
				+ "WHERE d.pk.profil = ?1 " 
				+ "AND d.pk.operationType = ?2") })
public class DroitObjet implements Serializable {
	...
	@AttributeOverrides({
		...
		@AttributeOverride(name = "operationType", 
				column = @Column(name = "OPERATION_TYPE_ID")) })
	public DroitObjetPK getPk() {
		return pk;
	}

	public void setPk(DroitObjetPK dpk) {
		this.pk = dpk;
	}
	...
	public boolean equals(Object obj) {
		...
		DroitObjet test = (DroitObjet) obj;
	...
	public DroitObjet clone() {
		DroitObjet clone = new DroitObjet();
