...
public class CederObjetDecorator {
	...
	public CederObjetDecorator(CederObjet objet, boolean newCeder,
			boolean _a, boolean _ck) {
		this.cederObjet = objet;
		isAnonyme = _a;
		checked = _ck;
		
		if (this.cederObjet.getEntite() != null) {
			if (this.cederObjet.getEntite().getNom()
					.equals("Echantillon")) {
				echantillon = (Echantillon) ManagerLocator.getEntiteManager()
					.findObjectByEntiteAndIdManager(cederObjet.getEntite()
			...
			} else if (this.cederObjet.getEntite()
					.getNom().equals("ProdDerive")) {
				prodDerive = (ProdDerive) ManagerLocator.getEntiteManager()
					.findObjectByEntiteAndIdManager(cederObjet.getEntite()
						, cederObjet.getObjetId());
	...
	public void setCederObjet(CederObjet ceder) {
	...
	public void setQuantiteMax(Float quantite) {
	...
	public void setQuantiteRestante(Float quantite) {
	...
	public String getDateDemandeCessionFormatted() {
		if (this.cederObjet != null 
				&& this.cederObjet.getCession() != null) {
			return ObjectTypesFormatters.dateRenderer2(
					this.cederObjet.getCession().getDemandeDate());
		} else {
			return "-";
		}
	}
	...
	public String getDateValidationCessionFormatted() {
		if (this.cederObjet != null 
				&& this.cederObjet.getCession() != null) {
			String value = "-";
			if (this.cederObjet.getCession().getCessionType()
					.getType().toUpperCase().equals("DESTRUCTION")) {
				value = ObjectTypesFormatters.dateRenderer2(
						this.cederObjet.getCession().getDestructionDate());
			} else {
				value = ObjectTypesFormatters.dateRenderer2(
					this.cederObjet.getCession().getValidationDate());
			}
			return value;
		} else {
			return "-";
	...
	public String getCessionDemandeur() {
		if (this.cederObjet != null 
				&& this.cederObjet.getCession() != null
				&& this.cederObjet.getCession().getDemandeur() != null) {
	...
	public String getCessionDestinataire() {
		if (this.cederObjet != null 
			...
			if (this.cederObjet.getCession().getCessionType()
					.getType().toUpperCase().equals("DESTRUCTION")) {
				if (this.cederObjet.getCession()
						.getDemandeur() != null) {
					value = this.cederObjet.getCession()
						.getDemandeur().getNom();
				}
			} else {
				if (this.cederObjet.getCession()
						.getDestinataire() != null) {
					value = this.cederObjet.getCession()
	...
	public String getCessionType() {
		if (this.cederObjet != null 
			...
			return Labels.getLabel("cession.type." 
					+ this.cederObjet.getCession().getCessionType()
					.getType().toLowerCase());
		} else {
			return "-";
		}
	...
	public String getCessionEtude() {
		if (this.cederObjet != null 
			...
			if (this.cederObjet.getCession().getCessionType()
					.getType().toUpperCase().equals("DESTRUCTION")) {
				if (this.cederObjet.getCession()
						.getDestructionMotif() != null) {
					value = this.cederObjet.getCession()
						.getDestructionMotif().getMotif();
				}
			} else if (this.cederObjet.getCession().getCessionType()
					.getType().toUpperCase().equals("SANITAIRE")) {
				if (this.cederObjet.getCession().getCessionExamen() != null) {
					value = this.cederObjet.getCession()
						.getCessionExamen().getExamen();
				}
			} else {
	...
	public String getCessionStatut() {
		if (this.cederObjet != null 
			...
			return ObjectTypesFormatters
					.ILNObjectStatut(this.cederObjet.getCession()
							.getCessionStatut());
		} else {
			return "-";
		}
	...
	public String getQuantiteCedeeFormatted() {
		if (this.cederObjet != null 
			...
			if (this.cederObjet.getCession().getCessionStatut() != null) {
				...
				if (this.cederObjet.getCession().getCessionStatut().getStatut()
						.equals("VALIDEE")) {
					return getQuantiteDemandeeFormatted();
				} else {
					if (this.cederObjet.getQuantiteUnite() != null) {
						return "0.0 " 
							+ this.cederObjet.getQuantiteUnite().getUnite();
					} else {
						return "0.0";
					}
				}
	...
	public String getEchantillonType() {
		if (this.echantillon != null
				&& this.echantillon.getEchantillonType() != null) {
	...
	public String getProdDeriveType() {
		if (this.prodDerive != null
				&& this.prodDerive.getProdType() != null) {
	...
	public String getCederQuantiteUnite() {
		if (this.cederObjet != null
				&& this.cederObjet.getQuantiteUnite() != null) {
	 ...
	 */
	public static List<CederObjet> extractListe(
			List<CederObjetDecorator> cedes) {
		List<CederObjet> liste = new ArrayList<CederObjet>();
		Iterator<CederObjetDecorator> it = cedes.iterator();
	...
	public String getStatutJuridique() {
		...
		if (echantillon != null) {
			prlvt = ManagerLocator.getEchantillonManager()
		...
		if (!isAnonyme) {
			...
			if (adrl.equals("")) {
				id.add(cederObjet.getObjetId());
					 rets = ManagerLocator.getRetourManager()
							.findByObjectDateRetourEmptyManager(id,
									cederObjet.getEntite());
				if (!rets.isEmpty()) {
	...
	public Integer getNbSorties() {
		if (echantillon != null) {
			return new Integer(ManagerLocator.getRetourManager()
					.getRetoursForObjectManager(echantillon).size());
		} else if (prodDerive != null) {
			return new Integer(ManagerLocator.getRetourManager()
					.getRetoursForObjectManager(prodDerive).size());
		} 
		return null;
	}
	...
	public void setNewStatut(ObjetStatut n) {
		this.newStatut = n;
	}
	...
	public void setOldEmplacement(Emplacement o) {
	...
	public void setRetour(Retour e) {
		this.retour = e;
	}
	...
	public boolean equals(Object obj) {
		...
		CederObjetDecorator deco = (CederObjetDecorator) obj;
	...
	public void switchQuantiteAndVolume(Unite unite) {
		
		if (unite.getType().equals("volume")) {	
			// restaure volume
			Float vol = AbstractController
					.calculerVolumeRestant(prodDerive, getQuantiteMax());		
			quantiteMax = vol;
			this.cederObjet.setQuantite(vol);
		} else {
			Float qte = AbstractController
					.calculerQuantiteRestante(prodDerive, getQuantiteMax());	
			quantiteMax = qte;
	...
	public Boolean getImpactVisible() {
		return !ManagerLocator.getRetourManager()
			.findByObjectAndImpactManager(getTKobj(), true).isEmpty();
	}
	...
	public String getPrelNcfs() {
		...
		if (getPrelNC()) {
			...
			String noconfs = ": ";
			Iterator<NonConformite> ncIt = ManagerLocator.getNonConformiteManager()
					.getFromObjetNonConformes(ManagerLocator
					.getObjetNonConformeManager()
					.findByObjetAndTypeManager(parent, "Arrivee")).iterator();
			while (ncIt.hasNext()) {
			...
			ncfs = ObjectTypesFormatters
					.getLabel("tooltip.nonconforme.arrivee", new String[]{noconfs});
		}
		return ncfs;
	}
	...
	public String getTKObjTraitementNcfs() {
		...
		if (getTKObjTraitementNC()) {
			...
			Iterator<NonConformite> ncIt = ManagerLocator.getNonConformiteManager()
					.getFromObjetNonConformes(ManagerLocator
					.getObjetNonConformeManager()
					.findByObjetAndTypeManager(getTKobj(), "Traitement")).iterator();
			while (ncIt.hasNext()) {
				 noconfs = noconfs + ncIt.next().getNom();
				 if (ncIt.hasNext()) {
					 noconfs = noconfs + ", ";
			...
			ncfs = ObjectTypesFormatters
					.getLabel("tooltip.nonconforme.traitement", new String[]{noconfs});
		}
		return ncfs;
	}
	...
	public String getTKObjCessionNcfs() {
		...
		if (getTKObjCessionNC()) {
			...
			Iterator<NonConformite> ncIt = ManagerLocator.getNonConformiteManager()
					.getFromObjetNonConformes(ManagerLocator
					.getObjetNonConformeManager()
					.findByObjetAndTypeManager(getTKobj(), "Cession")).iterator();
			while (ncIt.hasNext()) {
				 noconfs = noconfs + ncIt.next().getNom();
				 if (ncIt.hasNext()) {
					 noconfs = noconfs + ", ";
			...
			ncfs = ObjectTypesFormatters
					.getLabel("tooltip.nonconforme.cession", new String[]{noconfs});
		}
		return ncfs;
	}
	...
	public void setChecked(boolean _c) {
		this.checked = _c;
	}
