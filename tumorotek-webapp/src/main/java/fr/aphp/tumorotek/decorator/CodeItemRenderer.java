 ...
 * @version 2.0
 */
public class CodeItemRenderer implements ListitemRenderer<Object>
{
   ...
   public void render(final Listitem li, final Object data, final int index){
      ...
      new Listcell(code.getCode()).setParent(li);

      // si CIM, fecth le libelle
      //        String libelles = new String();
      //		Iterator<CimLibelle> libsIt = getCimLibelles().iterator();
      //		while (libsIt.hasNext()) {
      //			libelles = libelles + libsIt.next().getLibelle();
      //			if (libsIt.hasNext()) {
      //				libelles = libelles + "\n";
      //			}
      //		}

      final Listcell libCell = new Listcell(code.getLibelle());
      libCell.setParent(li);
      li.setValue(code);
   }
}
