...
public class TKSelectObjectRenderer implements RowRenderer<Object> {
	 ...
	 */
	protected String getDateCreation(Object obj) {
		Calendar date = ManagerLocator.getOperationManager()
			.findDateCreationManager(obj);
		if (date != null) {
	 ...
	 * @param row Parent
	 */
	protected void drawListLabel(
			List<String> liste, Row row, Component parent) {
		
		if (!liste.isEmpty()) {
			Label label1 = new Label(liste.get(0));
	...
	public void setTtesCollections(boolean c) {
		this.ttesCollections = c;
	}

	public void setSelectedObjects(List<? extends Object> s) {
