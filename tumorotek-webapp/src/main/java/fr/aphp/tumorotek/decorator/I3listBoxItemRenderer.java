...
public class I3listBoxItemRenderer implements ListitemRenderer<Object> {
	...
	private Log log = LogFactory.getLog(I3listBoxItemRenderer.class);
	 ...
	 * Constructeur.
	 */
	public I3listBoxItemRenderer(String fname) {
	...
	public void render(Listitem li, Object data, int index) {
		...
		if (data != null) {
			try {
				Field f = data.getClass().getDeclaredField(this.fieldName);
				// private -> acessible for refection only
				f.setAccessible(true);
						
				String label = Labels.getLabel(data.getClass()
							.getSimpleName() + "." + (String) f.get(data));
				...
				if (data.getClass().equals("TableCodage")) {
					Field fVersion = data.getClass().getDeclaredField("version");
					if (fVersion != null) {
						fVersion.setAccessible(true);
						if (fVersion.get(data) != null) {
							label = label + " (" 
										+ (String) fVersion.get(data) + ")";
						}
					}
				}
