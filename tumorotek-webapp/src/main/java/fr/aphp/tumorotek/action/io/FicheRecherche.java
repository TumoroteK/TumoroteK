...
public class FicheRecherche extends AbstractFicheCombineController {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[]{intituleLabel, requeteLabel, 
				affichageLabel});
		setObjBoxsComponents(new Component[] { intituleBox, requetesBox,
				affichagesBox });		
		setRequiredMarks(new Component[]{intituleRequired, requeteRequired, 
				affichageRequired});	
	...
	public void setObject(TKdataObject obj) {
		...
		if (this.recherche.getRechercheId() == null) {
			banquesDeRecherche = new ArrayList<Banque>();
		} else {
			banquesDeRecherche = ManagerLocator.getRechercheManager()
	...
	public void switchToCreateMode() {
		...
		this.affichages = ManagerLocator.getAffichageManager()
				.findByBanqueManager(
				SessionUtils.getSelectedBanques(sessionScope).get(0));
		Collections.sort(affichages);
		affichages.add(0, null);
		this.affichagesBox.setModel(new SimpleListModel<Affichage>(affichages));
		
		this.requetes = ManagerLocator.getRequeteManager()
				.findByBanqueManager(
				SessionUtils.getSelectedBanques(sessionScope).get(0));
		Collections.sort(requetes);
		requetes.add(0, null);
		this.requetesBox.setModel(new SimpleListModel<Requete>(requetes));
		
		Utilisateur user = (Utilisateur) sessionScope.get("User");
		Entite entiteRequete = ManagerLocator
			.getEntiteManager().findByNomManager("Requete").get(0);
		this.banques = ManagerLocator.getBanqueManager()
				.findByEntiteConsultByUtilisateurManager(user, entiteRequete, 
	...
	public void switchToStaticMode() {
		boolean isEmptyObj = this.recherche.equals(new Recherche());
	...
	public void switchToEditMode() {		
		...
		Entite entiteRequete = ManagerLocator
			.getEntiteManager().findByNomManager("Requete").get(0);
		this.banques = ManagerLocator.getBanqueManager()
				.findByEntiteConsultByUtilisateurManager(
						user, entiteRequete, 
						SessionUtils.getPlateforme(sessionScope));
		Collections.sort(banques);
		// this.banquesBox.setModel(new SimpleListModel(banques));
...
//		}
		...
		if (this.recherche != null) {
			...
			if (Messagebox.show(ObjectTypesFormatters.getLabel(
					"message.deletion.message", new String[] { Labels
							.getLabel("message.deletion.recherche") }),
							Labels.getLabel("message.deletion.title"),
							Messagebox.YES | Messagebox.NO, 
							Messagebox.QUESTION) == Messagebox.YES) {

				// suppression de la recherche
				ManagerLocator.getRechercheManager()
					.removeObjectManager(this.recherche);

				if (getListeRecherches() != null) {
					// on enlève la recherche de la liste
					getListeRecherches()
						.removeObjectAndUpdateList(this.recherche);
				}
				clearData();
			}
	...
	public void onSelect$affichagesBox() {
		this.recherche.setAffichage(affichages.get(affichagesBox
				.getSelectedIndex()));
	...
	public void onSelect$requetesBox() {
		this.recherche
				.setRequete(requetes.get(requetesBox.getSelectedIndex()));
		Clients.clearWrongValue(requetesBox);
	}
	
	protected Recherche saveRecherche(Utilisateur createur) {
		...
		ManagerLocator.getRechercheManager().createObjectManager(recherche,
				recherche.getAffichage(), recherche.getRequete(),
				banks, createur,
				SessionUtils.getSelectedBanques(sessionScope).get(0));
		return recherche;
	}

	protected void updateRecherche(Utilisateur createur) {
		//On récupère les banques sélectionées
		List<Banque> banks = new ArrayList<Banque>();
		banks.add(SessionUtils.getSelectedBanques(sessionScope).get(0));
		/*Set<Listitem> items = banquesBox.getSelectedItems();
		...
		ManagerLocator.getRechercheManager().updateObjectManager(recherche,
				recherche.getAffichage(), recherche.getRequete(),
				banks, createur,
				SessionUtils.getSelectedBanques(sessionScope).get(0));
	...
	public void setRequetes(List<Requete> reqs) {
		this.requetes = reqs;
	}
	...
	public void setAffichages(List<Affichage> affs) {
	...
	public void setBanques(List<Banque> bs) {
		this.banques = bs;
	}
	...
	public void setBanquesDeRecherche(List<Banque> banquesDeRech) {
	 ...
	 */
	private void setOpenListeRecherche(boolean open) {
		//On ferme la liste des recherches
