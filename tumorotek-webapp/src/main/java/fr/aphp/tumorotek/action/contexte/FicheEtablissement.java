...
public class FicheEtablissement extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(FicheEtablissement.class);
	...
	private String servicesGroupHeader = 
							Labels.getLabel("etablissement.services");
	private String mode;
	...
	public void setObject(TKdataObject obj) {
		...
		if (etablissement.getEtablissementId() != null) {
			services = ManagerLocator.getEtablissementManager()
				.getServicesWithOrderManager(etablissement);
		}
	...
	public void switchToStaticMode() {
		...
		super.switchToStaticMode(
				this.etablissement.equals(new Etablissement()));
	...
	public void createNewObject() {
		...
		ManagerLocator.getEtablissementManager().createObjectManager(
				etablissement, coordonnee, selectedCategorie,
				SessionUtils.getLoggedUser(sessionScope));
		
		if (getListeCollaborations() != null) {
			getListeCollaborations().updateTree();
			getListeCollaborations().openEtablissement(
					etablissement, false, true);
		}
	}
	...
	public void updateObject() {
		...
		ManagerLocator.getEtablissementManager().updateObjectManager(
				etablissement, coordonnee, selectedCategorie,
				SessionUtils.getLoggedUser(sessionScope),
				cascadeArchive);
	...
	public boolean prepareDeleteObject() {
		...
		if (isUsed) {
			setDeleteMessage(Labels
				.getLabel("etablissement.deletion.isUsedCascade"));
		} 
		if (isReferenced) {
			setDeleteMessage(Labels
				.getLabel("etablissement.deletion.isReferencedCascade"));
			if (getObject().getArchive()) { // deja archivé
				setDeletable(false);
				setDeleteMessage(Labels
					.getLabel("etablissement.deletion.isAlreadyArchived"));
			}
		}
	...
	public void onClick$addService() {
		getObjectTabController()
			.switchToFicheServiceCreationMode(this.etablissement);
	}
	...
	public void onOpen$groupServicesEtab() {
		String id = groupServicesEtab.getUuid();
	...
	public void setFieldsToUpperCase() {
		if (this.etablissement.getNom() != null) {
			this.etablissement.setNom(
				this.etablissement.getNom().toUpperCase().trim());
		}
	...
	public void onCheck$archiveBox() {
		...
		if (archiveBox.isChecked()) {
			if (Messagebox.show(Labels
					.getLabel("message.archive.message.etablissement"),
			...
			if (Messagebox.show(Labels
					.getLabel("message.not.archive.message.etablissement"),
	...
	public void initEditableMode() {
		categories = ManagerLocator.getCategorieManager()
...
//	}
	...
	public void drawActionsForEtablissement() {
		...
		addService
			.setDisabled(!drawActionOnOneButton("Collaborateur", 
										"Creation"));
	}
	...
	public void onClick$print() {
		if (this.etablissement != null) {
			...
			ManagerLocator.getXmlUtils().addHautDePage(root, 
					Labels.getLabel("ficheEtablissement.panel.title"),
					false, null);
	...
	public void addInfosGeneralesToPrint(Element page) {
		...
		CoupleValeur cp1 = new CoupleValeur(
				Labels.getLabel("etablissement.nom"), 
				tmp);
		...
		CoupleValeur cp2 = new CoupleValeur(
				Labels.getLabel("etablissement.finess"), 
				tmp);
		...
		CoupleValeur cp3 = new CoupleValeur(
				Labels.getLabel("etablissement.categorie"), 
				tmp);
		...
		if (etablissement.getArchive() != null) {
			...
			.booleanLitteralFormatter(this.etablissement.getArchive());
		} else {
			tmp = "";
		}
		CoupleValeur cp4 = new CoupleValeur(
				Labels.getLabel("etablissement.archive"), 
				tmp);
	...
	public void setSelectedCategorie(Categorie selected) {
	...
	public String getLocalFormated() {
		...
		if (etablissement != null) {
			return ObjectTypesFormatters
				.booleanLitteralFormatter(this.etablissement.isLocal());
		} else {
			return "";
		}
	}
