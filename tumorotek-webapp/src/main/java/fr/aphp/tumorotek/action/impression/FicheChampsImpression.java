...
public class FicheChampsImpression extends AbstractFicheController {
	...
	private List<ChampImpressionDecorator> champs = 
					new ArrayList<ChampImpressionDecorator>();
	private String path;
	
	private ChampImpressionRowRenderer champImpressionRenderer 
						= new ChampImpressionRowRenderer();
	 ...
	 * @param event
	 */
	public void onCheckChamp(ForwardEvent event) {
		...
		ChampImpressionDecorator deco = 
			(ChampImpressionDecorator) event.getData();
	 ...
	 * @param event
	 */
	public void onClickUp(ForwardEvent event) {
	 ...
	 * @param event
	 */
	public void onClickDown(ForwardEvent event) {
		// on récupère le champ associé à l'event
		ChampImpressionDecorator deco = 
			(ChampImpressionDecorator) event.getData();
		downBloc(deco);
	 ...
	 * @param deco
	 */
	private void upBloc(ChampImpressionDecorator deco) {
		int index = champs.indexOf(deco);
		ChampImpressionDecorator supObjectInList = null;
		if (index - 1 > -1) {
			supObjectInList = (ChampImpressionDecorator) 
				champs.get(index - 1);
	 ...
	 * @param bloc
	 */
	private void downBloc(ChampImpressionDecorator deco) {
		int index = champs.indexOf(deco);
	...
	public void onClick$validate() {
		List<ChampEntite> champsFinaux = new ArrayList<ChampEntite>();
		...
		if (Path.getComponent(path) != null) {
			...
			Events.postEvent(
				new Event("onGetChangeChampImpression", 
					Path.getComponent(path), 
					blocImpressionDecorator));
		}
	...
	public void setBlocImpressionDecorator(
			BlocImpressionDecorator bDecorator) {
		this.blocImpressionDecorator = bDecorator;
		
		champs = new ArrayList<ChampImpressionDecorator>();
		// on récupère les champs du bloc
		for (int i = 0; i < blocImpressionDecorator
			.getChampEntites().size(); i++) {
			ChampImpressionDecorator deco = new ChampImpressionDecorator(
					blocImpressionDecorator
					.getChampEntites().get(i));
			champs.add(deco);
		}
		
		// on extrait tous les champs disponibles
		List<ChampEntiteBloc> cebs = ManagerLocator
			.getChampEntiteBlocManager().findByBlocManager(
				blocImpressionDecorator.getBlocImpression());
		for (int i = 0; i < cebs.size(); i++) {
			if (!blocImpressionDecorator.getChampEntites().contains(
					cebs.get(i).getChampEntite())) {
				ChampImpressionDecorator deco = new ChampImpressionDecorator(
						cebs.get(i).getChampEntite());
				deco.setImprimer(false);
				champs.add(deco);
	...
	public void setChamps(List<ChampImpressionDecorator> c) {
		this.champs = c;
	}
	...
	public void setChampImpressionRenderer(
			ChampImpressionRowRenderer cRenderer) {
