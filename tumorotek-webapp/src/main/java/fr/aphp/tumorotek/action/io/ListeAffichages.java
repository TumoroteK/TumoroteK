...
public class ListeAffichages extends ListeOngletRecherche
{
   ...
   public void setListObjects(final List<? extends TKdataObject> objs){
      ...
      this.listObjects.addAll((List<Affichage>) objs);
   }

   @Override
   public void addToListObjects(final TKdataObject obj, final Integer pos){
      if(pos != null){
         getListObjects().add(pos.intValue(), (Affichage) obj);
      }else{
         getListObjects().add((Affichage) obj);
      }
   }
   ...
   public void initObjectsBox(){
      ...
      final List<Affichage> affs =
         ManagerLocator.getAffichageManager().findByBanqueInLIstManager(SessionUtils.getSelectedBanques(sessionScope));
   ...
   public AbstractFicheCombineController getFiche(){
      return ((FicheAffichage) self.getParent().getParent().getParent().getFellow("ficheRegion").getFellow("ficheAffichage")
         .getFellow("winFicheAffichage").getAttributeOrFellow("winFicheAffichage$composer", true));
   }
   ...
   public void onClickObject(final Event event){
      ...
      final Affichage edit = ((Affichage) getCurrentObject()).clone();
   ...
   @Override
   public List<Affichage> getListObjects(){
