...
public class ListeCollaborations extends AbstractController {
	...
	private String collabsIsolesGroupHeader = 
		Labels.getLabel("collaborateurs.isoles");
	 ...
	 */
	private List<Collaborateur> collabsWithoutService = 
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		collabsWithoutService = ManagerLocator.getCollaborateurManager()
	 ...
	 */
	public void swithToSelectionMode(String type, String from,
		...
		if (oldSelection != null) {
			...
			getBinder().loadAttribute(
					self.getFellow("mainTreeContext"), "treeitemRenderer");
		}
		...
		if (filtres != null) {
			for (int i = 0; i < filtres.size(); i++) {
				if (filtres.get(i).getClass()
						.getSimpleName().equals("Etablissement")) {
					
					openEtablissement((Etablissement) 
							filtres.get(i), true, false);
					
				} else if (filtres.get(i).getClass()
						.getSimpleName().equals("Service")) {
					
					openService((Service) filtres.get(i), true, false);
					
				} else if (filtres.get(i).getClass()
						.getSimpleName().equals("Collaborateur")) {
					
					openCollaborateurInTree((Collaborateur) 
							filtres.get(i), true, false);
				}
			}
		}
	...
	public void updateTree() {
		...
		// Maj de l'arbre
		getBinder().loadAttribute(
				self.getFellow("mainTreeContext"), "model");
	}
	...
	public void updateTreeForServicesAfterResearch(List<Service> services) {
		...
		for (int i = 0; i < services.size(); i++) {
			...
			EtablissementNode node = new EtablissementNode(
					serv.getEtablissement());
	 ...
	 */
	public void updateTreeForCollaborateursAfterResearch(
		...
		for (int i = 0; i < collaborateurs.size(); i++) {
			...
			Set<Service> services = ManagerLocator.getCollaborateurManager()
	...
	public void updateCollabsWithoutService() {
		...
		// Maj de la liste
		getBinder().loadAttribute(
				self.getFellow("collabsIsolesBox"), "model");
	...
	public CollaborationsController getCollaborationsController() {
		return ((CollaborationsController) self.getParent().getParent()
				.getParent()
	...
	public void onSelect$mainTreeContext() {
		...
		if (obj instanceof EtablissementNode) {
			...
			getCollaborationsController()
				.switchToFicheEtablissementMode(node.getEtablissement());
			
		} else if (obj instanceof ServiceNode) {
			
			ServiceNode node = (ServiceNode) obj;
			getCollaborationsController()
				.switchToFicheServiceMode(node.getService());
		...
		if (selectItem.isVisible()) {
			...
			if (obj != null && obj.getClass().getSimpleName()
					.equals(typeObjectToSelect + "Node")) {
		...
		if (mainTreeContext.getSelectedItem() != null) {
			...
			if (typeObjectToSelect.equals("Etablissement")) {
				...
				if (obj instanceof EtablissementNode) {
					...
					if (node.getEtablissement().getArchive()) {
						throw new WrongValueException(selectItem, Labels
						.getLabel("message.wrong.selection.inactif.message"));
					} else {
						...
						if (Path.getComponent(pageToSendObject) != null) {
							...
							Events.postEvent(
								new Event("onGetObjectFromSelection", 
									Path.getComponent(pageToSendObject), 
									node.getEtablissement()));
						}
					...
					sb.append(Labels.getLabel(
							"message.wrong.selection.message"));
					sb.append(" ");
			...
			if (typeObjectToSelect.equals("Etablissement")) {
				...
				Collaborateur collab = (Collaborateur) 
					collabsIsolesBox.getSelectedItem().getValue();
				// si l'objet est archivé : sélection impossible
			...
			Events.postEvent(new Event("onGetCollaborateurFromSelection", 
					Path.getComponent(pageToSendObject), null));
	...
	public void onClick$newEtabItem() {
		getCollaborationsController()
			.switchToFicheEtablissementCreationMode();
	}
	...
	public void onClick$newCollabItem() {
		getCollaborationsController()
			.switchToFicheCollaborateurCreationMode(null);
	}
	...
	public void onClick$find() {
		...
		if (selectedIndex == 0) {
			...
			if (nomCollab.isChecked()) {
				...
				List<Collaborateur> collabs = ManagerLocator
					.getCollaborateurManager()
					.findByNomLikeBothSideManager(searchNomCollab);
			...
			if (nomEtab.isChecked()) {
				...
				List<Etablissement> etabs = ManagerLocator
					.getEtablissementManager()
					.findByNomLikeBothSideManager(searchNomEtab);
				...
				List<Etablissement> etabs = ManagerLocator
				.getEtablissementManager()
				.findByVilleLikeManager(searchVilleEtab);
			...
			if (nomService.isChecked()) {
				...
				List<Service> services = ManagerLocator.getServiceManager()
					.findByNomLikeBothSideManager(searchNomService);
	 ...
	 */
	public void openEtablissement(Etablissement etablissement,
			boolean open, boolean select) {
		...
		if (select) {
			...
			getCollaborationsController()
				.switchToFicheEtablissementMode(((EtablissementNode) 
				ttm.getChild(ttm.getPath(etabNode))).getEtablissement());
		}
	}
...
//		}
		...
		if (select) {
			...
			getCollaborationsController()
				.switchToFicheServiceMode(((ServiceNode) 
			ttm.getChild(ttm.getPath(servNode))).getService());
	 ...
	 */
	public void openCollaborateurInTreeOrList(Collaborateur collab, 
			boolean open, boolean select) {
	 ...
	 */
	public void openCollaborateurInTree(Collaborateur collab, 
			boolean open, boolean select) {
		...
		while (itServices.hasNext()) {
			...
			EtablissementNode node = 
					new EtablissementNode(tmp.getEtablissement());
...
//		}
		...
		if (select) {
			...
			getCollaborationsController()
				.switchToFicheCollaborateurMode(((CollaborateurNode) 
			ttm.getChild(ttm.getPath(collabNode))).getCollaborateur());
	 ...
	 * @param select Si true, on affiche le collaborateur.
	 */
	public void openCollaborateurInList(Collaborateur collab, 
			boolean open, boolean select) {
		...
		if (collabsWithoutService.contains(collab)) {
			if (select) {
				...
				getCollaborationsController()
					.switchToFicheCollaborateurMode(collab);
			}
		}
		
	}
	
	public List<Etablissement> getEtablissementForServices(
	...
	public List<Etablissement> getEtablissementForCollaborateurs(
	...
	public List<Etablissement> getEtablissementsForObjects(
			List<Object> objects) {
		List<Etablissement> etabs = new ArrayList<Etablissement>();
		...
		for (int i = 0; i < objects.size(); i++) {
			...
			if (currentObject != null) {
				...
				if (currentObject.getClass()
				...
				} else if (currentObject.getClass()
						.getSimpleName().equals("Service")) {
					...
					// on récupère les services du collaborateur
					Set<Service> services = ManagerLocator.getCollaborateurManager()
						.getServicesManager(currentCollab);
	...
	public void onSelect$collabsIsolesBox() {
		...
		getCollaborationsController()
			.switchToFicheCollaborateurMode(selectedCollaborateur);
