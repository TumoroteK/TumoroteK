 ...
 * @version 2.0
 */
public class ModificationMultipleDatebox 
	 ...
	 * @param df
	 */
	public void setDateboxFormat(String df) {
		multiDatebox.setFormat(df);
		eraseMultiDatebox.setFormat(df);
	}
	
	public void setCalendar(boolean cal) {
		this.isCalendar = cal;
		if (!isCalendar) {
			setDateboxFormat(Labels
	...
	public Object getNewValue() {
		if (super.getNewValue() == null 
					|| super.getNewValue() instanceof Date) {
	...
	public AnnotateDataBinder getBinder() {
		return ((AnnotateDataBinder) self
				.getParent()
				.getAttributeOrFellow("modificationDatebox", true));
	}
	...
	public void passValueToEraserBox() {
		if (getValues()
			...
			if (!isCalendar) {
				eraseMultiDatebox
					.setValue((Date) getValues()
						.get(multiListBox.getSelectedIndex()));
			} else {
				eraseMultiDatebox
					.setValue(((Calendar) getValues()
						.get(multiListBox.getSelectedIndex())).getTime());
			}
		} else {
