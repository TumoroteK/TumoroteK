...
public class FicheModifMultiPatient extends AbstractFicheModifMultiController
{
   ...
   public AbstractObjectTabController getObjectTabController(){
      return ((PatientController) self.getParent().getParent().getParent().getFellow("winPatient")
   ...
   public void updateMultiObjects(){
      ...
      final List<Patient> clones = new ArrayList<>();

      boolean hasAnyChange = false;
      for(int i = 0; i < getObjsToEdit().size(); i++){
         final Patient current = ((Patient) getObjsToEdit().get(i)).clone();
      ...
      if(hasAnyChange){
         ManagerLocator.getPatientManager().updateMultipleObjectsManager(clones, (List<Patient>) getObjsToEdit(),
   ...
   public void onGetChangeOnChamp(final Event e){
      ...
      if(tmp.getValue() instanceof LabelCodeItem){
         ...
         if("patientEtat".equals(tmp.getChamp())){
            final SimpleChampValue nullifier = new SimpleChampValue();
