...
public class ChampLigneEtiquetteRowRenderer implements RowRenderer<ChampLigneEtiquette> {
	...
	@Override
	public void render(Row row, ChampLigneEtiquette data, int index) throws Exception {
		ChampLigneEtiquette cle = data;
		...
		if (cle.getChamp().getChampAnnotation() != null) {
			...
			champ = ObjectTypesFormatters.getLabelForChampEntite(
					cle.getChamp().getChampEntite());
		}
		...
		if (cle.getExpReg() != null) {
			if (cle.getExpReg().contains(">")) {
				...
				sb.append(" ");
				sb.append(cle.getExpReg().substring(1, 
						cle.getExpReg().indexOf(",")));
				sb.append(" à ");
				sb.append(cle.getExpReg().substring(
						cle.getExpReg().indexOf(",") + 1, 
						cle.getExpReg().indexOf("]")));
			}
		} else {
			sb.append(Labels.getLabel(
		...
		Label expLabel = new Label(sb.toString());
		expLabel.setParent(row);
		
		// Entite
		Label entiteLabel = new Label(cle.getEntite().getNom());
