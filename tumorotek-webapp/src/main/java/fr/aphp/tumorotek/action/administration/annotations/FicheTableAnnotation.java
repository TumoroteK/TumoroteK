...
public class FicheTableAnnotation extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(FicheTableAnnotation.class);
	...
	private List<ChampAnnotationDecorator> champs = 
								new ArrayList<ChampAnnotationDecorator>();
	private List<ChampAnnotationDecorator> copyChamps = 
								new ArrayList<ChampAnnotationDecorator>();
	
	private List<ChampAnnotationDecorator> champsToCreateOrEdit = 
								new ArrayList<ChampAnnotationDecorator>();
	private List<ChampAnnotation> champsToDelete = 
	...
	private List<Banque> copyBanques = new ArrayList<Banque>();
	...
	private static I3listBoxItemRenderer typeRenderer = 
								new I3listBoxItemRenderer("type");
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		Executions
			.createComponents("/zuls/administration/BanquesAssociees.zul", 
													banquesAssociees, null);
	...
	public void createNewObject() {		
		...
		List<ChampAnnotation> chps = prepareChamps();
		
		if (banques.isEmpty()) {
			banques.add(SessionUtils
								.getSelectedBanques(sessionScope).get(0));
		}
		
		ManagerLocator.getTableAnnotationManager()
			.createOrUpdateObjectManager(table, 
								selectedEntite, null, 
								chps, banques, null,
		...
		ManagerLocator
			.getTableAnnotationManager()
				.updateChampOrdersManager(ChampAnnotationDecorator
													.unDecore(champs));
		...
		if (banques.contains(SessionUtils
						.getSelectedBanques(sessionScope).get(0))) {
			getMainWindow().updateSelectedBanque(SessionUtils
							.getSelectedBanques(sessionScope).get(0));
		}
	}
	...
	public void onClick$createC() {
		Clients.showBusy(Labels
				.getLabel("ficheAnno.modification.encours"));
	...
	public void removeObject(String comments) {		
		// suppression
		ManagerLocator.getTableAnnotationManager()
			.removeObjectManager(table, comments, 
					SessionUtils.getLoggedUser(sessionScope), 
								SessionUtils.getSystemBaseDir());
	}
...
//		}	
	...
	public void updateObject() {
		...
		for (int i = 0; i < champsToDelete.size(); i++) {
			ManagerLocator.getChampAnnotationManager()
					.removeObjectManager(champsToDelete.get(i), null, 
							SessionUtils.getLoggedUser(sessionScope), 
									SessionUtils.getSystemBaseDir());
		}
		...
		ManagerLocator.getTableAnnotationManager()
			.createOrUpdateObjectManager(table, null, null, 
					...
					SessionUtils.getLoggedUser(sessionScope), 
				"modification", SessionUtils.getSystemBaseDir(), null);
	...
	public void initEditableMode() {	
		...
		if (entites.isEmpty()) {
			entites.addAll(ManagerLocator.getEntiteManager()
												.findAnnotablesManager());
		}
...
//		}
		...
		if (types.isEmpty()) {
			types.addAll(ManagerLocator.getDataTypeManager()
									.findAllObjectsManager());
		}
		...
		if (table.getTableAnnotationId() != null) {
			...
			if (table.getCatalogue() == null) {
				 list = ChampAnnotationDecorator.decorateListe(
						new ArrayList<ChampAnnotation>(
							ManagerLocator.getTableAnnotationManager()
									.getChampAnnotationsManager(table)), 
							SessionUtils
								.getSelectedBanques(sessionScope).get(0), 
																	false); 
			} else {
				list = ChampAnnotationDecorator.decorateListe(
						ManagerLocator.getChampAnnotationManager()
										.findByEditByCatalogueManager(table), 
								SessionUtils
									.getSelectedBanques(sessionScope).get(0), 
																		true);
			}
...
//			}
			...
			// banques
			banques.addAll(ManagerLocator
					.getTableAnnotationManager().getBanquesManager(table));
		} else {
			if (!SessionUtils
	...
	public void setChamps(List<ChampAnnotationDecorator> chps) {
		this.champs.clear();
	...
	public void setSelectedDataType(DataType sType) {
...
//	} 
	...
	public String getEntiteTableNom() {
		if (this.table != null && this.table.getEntite() != null) {
			return (Labels.getLabel("Entite." 
					+ this.table.getEntite().getNom()));
		} 
		return null;
	}
	...
	public void onClick$addChamp() {
		...
		if (this.beforeEditClone != null) { 
			...
			if (this.beforeEditClone.getChampAnnotationId() != null) {
				this.currentChampEdited
					.setChampAnnotation(this.beforeEditClone);
		...
		ListModel<ChampAnnotationDecorator> list = 
				new ListModelList<ChampAnnotationDecorator> (champs);
		...
		// colore Row
		((Row) rows.getChildren().get(champs.indexOf(currentChampEdited)))
			.setStyle("background-color: #fddfa9");
	}
	 ...
	 * @throws IOException 
	 */
	public void onClick$editChamp(Event event) throws IOException {
		revertCurrentEdition();
		this.currentChampEdited = (ChampAnnotationDecorator) 
	...
	public void onClick$deleteChamp(Event event) {
		...
		List<? extends Object> refChps = new ArrayList<Object>();
		if (this.currentChampEdited
				.getChamp().getChampAnnotationId() != null) {
			refChps = ManagerLocator.getChampAnnotationManager()
				.isUsedObjectManager(currentChampEdited.getChamp());
		...
		if (refChps.isEmpty()) {
			...
			Messagebox.QUESTION) == Messagebox.YES) {
				...
				if (champsToCreateOrEdit
							.contains(this.currentChampEdited)) {
					champsToCreateOrEdit
							.remove(this.currentChampEdited);
				}
			...
			Iterator<? extends Object> refsIt = refChps.iterator();
			...
			if (hasLEtiquettes) {
				sb.append("<li>");
				sb.append(Labels
						.getLabel("ficheAnno.champ.isUsed.ligneEtiquette"));
				sb.append("</li>");
			}
			sb.append("</ul>");
			sb.append(Labels.getLabel("ficheAnno.champ.isUsed.end"));
			openDeleteWindow(page, sb.toString(), false, 
	 ...
	 * @param event
	 */
	public void onClick$validateChamp(Event event) {
		// lance constraint nom champ
		((Textbox) ((Row) 
			AbstractListeController2
				.getBindingData((ForwardEvent) event, true))
	 ...
	 * @param event
	 */
	public void onClick$revertChamp(Event event) {
		revertCurrentEdition();
		this.currentChampEdited = null;
		this.beforeEditClone = null;
		getBinder().loadAttribute(champsGrid, "model");
	...
	public void onClick$upChamp(Event event) {
		revertCurrentEdition();
		upObject((ChampAnnotationDecorator) 
				AbstractListeController2
					.getBindingData((ForwardEvent) event, false));
		getBinder().loadAttribute(champsGrid, "model");
	}
	...
	public void onClick$downChamp(Event event) {
		revertCurrentEdition();
		ChampAnnotationDecorator chpDeco = (ChampAnnotationDecorator) 
			AbstractListeController2
					.getBindingData((ForwardEvent) event, false); 
		int tabIndex = champs.indexOf(chpDeco);
	 ...
	 */
	private void upObject(ChampAnnotationDecorator obj) {
	 ...
	 * @param event
	 */
	public void onClick$addDefaut(Event event) {	
	 ...
	 */
	public void openAnnotationDefautModal(Event event, 
						ChampAnnotationDecorator chpDeco, Component backComp, 
															boolean isStatic) {
		...
		if (!isBlockModal()) {
			...
			final DefautComponent ua = (DefautComponent) 
				page.getComponentDefinition("defautComp", false)
									.newInstance(page, 
								"fr.aphp.tumorotek.action.administration."
											+ "annotations.DefautComponent");
	 ...
	 * @param event
	 */
	public void onClick$editDefaut(Event event) {
		openAnnotationDefautModal(event, this.currentChampEdited, self, false);
	}
	 ...
	 * @param event
	 */
	public void onClick$detailsLabel(Event event) {	
		
		ChampAnnotationDecorator deco = (ChampAnnotationDecorator) 
				AbstractListeController2
				.getBindingData((ForwardEvent) event, false);
		if (deco != null && deco.getChamp().getDataType() != null 
			&& deco.getChamp().getDataType().getType().matches("thesaurus.*")) {
	...
	private void revertCurrentEdition() {
		if (this.currentChampEdited != null) {
			...
			if (this.currentChampEdited.getChamp()
								.getChampAnnotationId() != null
					|| this.currentChampEdited.isValidated()) {
				this.currentChampEdited
						.setChampAnnotation(this.beforeEditClone);
	 ...
	 * @param event
	 */
	public void onSelect$typeBoxEachChamp(Event event) {
		// AnnotationDefaut
		this.currentChampEdited.setDataType(selectedDataType);
		this.currentChampEdited.reset();
				
		// Obligatoire
		Checkbox obligBox = ((Checkbox) 
		((Component) ((Row) AbstractListeController2
				.getBindingData((ForwardEvent) event, true))
					.getChildren().get(3))
						.getFirstChild());
		// Combine
		Checkbox combiBox = ((Checkbox) 
			((Component) ((Row) AbstractListeController2
				.getBindingData((ForwardEvent) event, true))
					.getChildren().get(4))
						.getFirstChild());
		...
		// defautDiv
		Div defautDiv = ((Div) ((Row) AbstractListeController2
						.getBindingData((ForwardEvent) event, true))
												.getChildren().get(2));
	...
	private List<ChampAnnotation> prepareChamps() {
		List<ChampAnnotation> chps = new ArrayList<ChampAnnotation>();
		ChampAnnotation chp;
		for (int i = 0; i < champsToCreateOrEdit.size(); i++) {
			chp = champsToCreateOrEdit.get(i).getChamp();
			chps.add(chp);
			
			Iterator<AnnotationDefaut> itor = 
				 			champsToCreateOrEdit.get(i).getDefauts().iterator();
	 ...
	 * @param chps
	 */
	private void updateDecorators(List<ChampAnnotation> chps) {
		for (int i = 0; i < champsToCreateOrEdit.size(); i++) {
			// empty renvoye quand modification du champ
			if (!chps.get(i).getAnnotationDefauts().isEmpty()) {
				Set<AnnotationDefaut> defs = 
							new LinkedHashSet<AnnotationDefaut>();
	...
	public void setBanques(List<Banque> banks) {
		this.banques.clear();
	...
	public BanquesAssociees getBanquesAssociees() {
		return (BanquesAssociees) self
				.getFellow("banquesAssociees")
					.getFellow("winBanquesAssociees")
