...
public class ProdDeriveRowRenderer extends TKSelectObjectRenderer {
	...
	public void setAccessStockage(boolean ac) {
		this.accessStockage = ac;
	}
	
	public ProdDeriveRowRenderer(boolean select, boolean cols) {
	...
	public void render(Row row, Object data, int index) {
		...
		ProdDerive derive = (ProdDerive) data;
				
		Hlayout icones = TKStockableObjectUtils.drawListIcones(derive, null, null);
		icones.setParent(row);
		
		// code
		Label codeLabel = new Label(derive.getCode());
		if (accessible) {
			codeLabel.addForward(null, codeLabel.getParent(), 
					"onClickObject", derive);
		...
		new Label(ObjectTypesFormatters
				.dateRenderer2(derive.getDateStock())).setParent(row);
		...
		if (derive.getObjetStatut() != null) {
			Label statut = new Label(ObjectTypesFormatters
		...
		if (!anonyme && accessStockage) {
			Label emplLabel = new Label(getEmplacementAdrl(derive));
			if (accessStockage) {
				emplLabel.setSclass("formLink");
				emplLabel.addForward(null, emplLabel.getParent(), 
						"onClickObjectEmplacement", 
	 ...
	 */
	public String getTypeParent(ProdDerive prodDerive) {
	 ...
	 */
	public Label getCodeParent(ProdDerive prodDerive) {
		...
		if (transformation != null) {
			...
			if (typeParent.equals("Prelevement")) {
				...
				Echantillon echan = ((Echantillon) 
					ManagerLocator.getEntiteManager()
						.findObjectByEntiteAndIdManager(transformation
								.getEntite(), transformation.getObjetId()));
				code = echan.getCode();
				...
				ProdDerive pd = ((ProdDerive) ManagerLocator
					.getEntiteManager()
						.findObjectByEntiteAndIdManager(transformation
								.getEntite(), transformation.getObjetId()));
				code = pd.getCode();
	 ...
	 * @return String.
	 */
	public String getQuantite(ProdDerive prodDerive) {
	 ...
	 * @return String.
	 */
	public String getVolume(ProdDerive prodDerive) {
		StringBuffer sb = new StringBuffer();
	 ...
	 * @return String.
	 */
	public String getConcentration(ProdDerive prodDerive) {
		StringBuffer sb = new StringBuffer();
	...
	public String getEmplacementAdrl(ProdDerive prodDerive) {
		if (prodDerive != null 
	...
	public static int getNbDerives(ProdDerive prodDerive) {
		return ManagerLocator.getProdDeriveManager()
			.getProdDerivesManager(prodDerive).size();
	}
	
	public Integer getNbCessions(ProdDerive prodDerive) {
	 ...
	 */
	public String getDateCreation(ProdDerive prodDerive) {
		Calendar date = ManagerLocator.getOperationManager()
			.findDateCreationManager(prodDerive);
		if (date != null) {
	...
	public String getPatient(ProdDerive derive) {
		Prelevement prlvt = ManagerLocator.getProdDeriveManager()
	...
	public void setAccessible(boolean acc) {
		this.accessible = acc;
	}

	public void setAccessEchantillon(boolean ae) {
		this.accessEchantillon = ae;
	}

	public void setAccessPrelevement(boolean ap) {
