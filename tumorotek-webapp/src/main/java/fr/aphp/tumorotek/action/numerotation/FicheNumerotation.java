...
public class FicheNumerotation extends AbstractFicheCombineController {
	...
	private List<Entite> allEntites = new ArrayList<Entite>();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjBoxsComponents(new Component[]{
				...
				this.collectionBox,
				this.entitesBox,
				this.codePrefixeNumerotation,
				this.codeSuffixeNumerotation,
				this.xxxLabel,
				this.codeHelpEditLabel
		});
		
		setRequiredMarks(new Component[]{
				this.collectionRequired,
				this.entiteRequired,
				this.startRequired,
	...
	public void onClick$createC() {
		...
		if (selectedBanque == null) {
			throw new WrongValueException(
				collectionBox, 
				Labels.getLabel("numerotation.error.banque"));
		}
		if (selectedEntite == null) {
			throw new WrongValueException(
				entitesBox, 
				Labels.getLabel("numerotation.error.entite"));
		}
		
		Clients.showBusy(Labels.getLabel(
				"numerotation.creation.encours"));
	...
	public void createNewObject() {
		...
		this.numerotation.setCurrentIncrement(
				this.numerotation.getStartIncrement() - 1);
		
		// update de l'objet
		ManagerLocator.getNumerotationManager().createObjectManager(
				numerotation, 
				selectedBanque, 
				selectedEntite);
	...
	public void removeObject(String comments) {
		ManagerLocator.getNumerotationManager()
			.removeObjectManager(numerotation);
	}
	...
	private ListeNumerotation getListeNumerotation() {
		return (ListeNumerotation) getObjectTabController().getListe();
	}

	@Override
	public void onClick$validateC() {
		// Validations
		if (this.numerotation.getCurrentIncrement() 
				< this.numerotation.getStartIncrement()) {
			throw new WrongValueException(
					currentBox, 
					Labels.getLabel("numerotation.error.currentIncrement"));
		}
	 ...
	 * @throws Exception
	 */
	public void onSelect$collectionBox(Event event) throws Exception {
		entites = new ArrayList<Entite>();
		if (selectedBanque != null) {
			Clients.clearWrongValue(collectionBox);
			List<Entite> tmp = ManagerLocator.getNumerotationManager()
				.findByBanqueSelectEntiteManager(selectedBanque);
		...
		entites.add(0, null);
		ListModel<Entite> listTmp = new ListModelList<Entite>(entites);
	...
	public void onBlur$codePrefixeNumerotation() {
		codePrefixeNumerotation.setValue(
				codePrefixeNumerotation.getValue().toUpperCase());
	}
	...
	public void onBlur$codeSuffixeNumerotation() {
		codeSuffixeNumerotation.setValue(
				codeSuffixeNumerotation.getValue().toUpperCase());
	}
	
	public void initEditableMode() {
		allEntites = new ArrayList<Entite>();
		allEntites.add(ManagerLocator.getEntiteManager()
		...
		banques = ManagerLocator.getBanqueManager()
			.findByUtilisateurIsAdminManager(user, 
		...
		if (numerotation.getCodeFormula() != null) {
			if (numerotation.getCodeFormula().contains("[]")) {
				codePrefixe = numerotation.getCodeFormula().substring(
						0, numerotation.getCodeFormula().indexOf("["));
				codeSuffixe = numerotation.getCodeFormula().substring(
						numerotation.getCodeFormula().indexOf("]") + 1,
	...
	public String getZeroFillFormated() {
		if (this.numerotation != null) {
			return ObjectTypesFormatters
				.booleanLitteralFormatter(this.numerotation.getZeroFill());
		} else {
			return "";
		}
	}
	...
	public void setNumerotation(Numerotation n) {
		this.numerotation = n;
	}
	...
	public void setAllEntites(List<Entite> allE) {
	...
	public void setCodePrefixe(String codeP) {
	...
	public void setCodeSuffixe(String codeS) {
