...
public class ListeUtilisateur extends AbstractListeController2 {
	...
	private List<Utilisateur> listObjects = 
									new ArrayList<Utilisateur>();
	private List<Plateforme> pfs = new ArrayList<Plateforme>();
	...
	private UtilisateurRowRenderer utilisateurRenderer 
		= new UtilisateurRowRenderer();
	
	public List<Plateforme> getPfs() {
		return pfs;
	}

	public void setPfs(List<Plateforme> pfs) {
		this.pfs = pfs;
	}
	...
	public void doAfterCompose(Component comp) throws Exception {	
		...
		int height = getMainWindow().getListPanelHeight() + 142;
		listPanel.setHeight(height + "px");
		
		utilisateurRenderer.setUser(SessionUtils
	...
	public void removeObjectFromList(TKdataObject obj) {
		getListObjects().remove((Utilisateur) obj);
	}
	...
	public void initObjectsBox() {
		...
		List<Utilisateur> utilisateurs = 
			ManagerLocator.getUtilisateurManager()
				.findByArchiveManager(findArchive, pfs);
	...
	public void onCheck$availPfsBox() {
		...
		if (availPfsBox.isChecked()) {
			if (SessionUtils
					.getLoggedUser(sessionScope).isSuperAdmin()) {
				pfs.addAll(ManagerLocator.getPlateformeManager()
					.findAllObjectsManager());
			} else {
				pfs.addAll(ManagerLocator.getUtilisateurManager()
					.getPlateformesManager(SessionUtils
							.getLoggedUser(sessionScope)));
			}
	...
	public void onClickObject(Event event) {	
		...
		Utilisateur edit = ((Utilisateur) getCurrentObject()).clone();
	...
	public void setUtilisateurRenderer(UtilisateurRowRenderer uRenderer) {
	...
	public void onClick$find() {
		...
		if (findLoginBox.getValue() != null) {
			...
			List<Utilisateur> utilisateurs = 
				ManagerLocator.getUtilisateurManager()
					.findByLoginAndArchiveManager(
							findLoginBox.getValue(), 
				findArchive, pfs);
