...
public abstract class AbstractFicheModifMultiController 
										extends AbstractFicheController {
	
	private static final long serialVersionUID = 1L;

	private TKdataObject baseObject;
	private String modifEncoursLabel = "general.modifMulti.encours";
	
	private List< ? extends Object> objsToEdit = new ArrayList<Object>();
	
	public void setObjsToEdit(List<? extends Object> objs) {
	...
	public void setBaseObject(TKdataObject obj) {
		this.baseObject = obj;
	}
	...
	public void onGetChangeOnChamp(Event e) {
		...
		try {		
			if (!(tmp.getValue() instanceof List)) {
				PropertyUtils.setSimpleProperty(getObject(), 
										tmp.getChamp(), tmp.getValue());
			} else {
				PropertyUtils.setSimpleProperty(getObject(), 
					tmp.getChamp(), ((List<Object>) tmp.getValue()).get(0));
				
				if (tmp.getChamp().contains("Init")) {
					PropertyUtils.setSimpleProperty(getObject(), 
							tmp.getChamp().replace("Init", "Unite"),
	 ...
	 */
	public abstract void updateLabelChanged(String champ, String printValue, 
																boolean reset);
		...
		try {
			...
			if (getObjectTabController().getListe() != null) {
				...
				getObjectTabController().getListe()
					.updateMultiObjectsGridListFromOtherPage((List<TKdataObject>)
															getObjsToEdit());
			}
	...
	public boolean updateMultiAnnotationValeurs() {
		getObjectTabController()
			.getFicheAnnotation().populateValeursActionLists(false, true);
		return (!getObjectTabController().getFicheAnnotation()
								.getValeursToCreateOrUpdate().isEmpty()
					|| !getObjectTabController()
						.getFicheAnnotation()
							.getValeursToDelete().isEmpty());
	}
