 ...
 * @version 2.0
 */
public class CodesSelectedRowRenderer implements RowRenderer {
	...
	public void render(Row row, Object data, int index) {
		...
		CodeCommon code = (CodeCommon) data;
		
		// checkbox
		Checkbox checkCode = new Checkbox();
		checkCode.addForward("onCheck", row.getParent(), "onCheckObject", code);
		checkCode.setChecked(((List<CodeCommon>) 
					row.getParent().getAttribute("checkedCodes"))
															.contains(data));
		...
		if (upAndDown) {
			...
			if (row.getPreviousSibling() != null) {
				...
				upImg.setHeight("8px");
				upImg.addForward("onClick", row.getParent(), 
														"onClickUpCode", code);
				upImg.setParent(row);
			} else { // cell vide
			...
			if (row.getNextSibling() != null) {
				...
				downImg.addForward("onClick", row.getParent(), 
													"onClickDownCode", code);
