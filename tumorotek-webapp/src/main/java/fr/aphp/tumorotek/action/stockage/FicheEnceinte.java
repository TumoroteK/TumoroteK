...
public class FicheEnceinte extends AbstractFicheCombineStockageController
{
   ...
   private List<EnceinteType> types = new ArrayList<>();

   private EnceinteType selectedEnceinteType;

   private List<EnceinteType> childEnceintesTypes = new ArrayList<>();
   ...
   // TK-291
   private List<OldEmplTrace> emplForRetours = new ArrayList<>();
   ...
   public void doAfterCompose(final Component comp) throws Exception{
      ...
      setObjLabelsComponents(new Component[] {this.nomLabel, this.aliasLabel, this.typeLabel, this.entiteReserveeLabel,
         this.banquesReserveesLabel, this.menuBar, this.couleurEnceinteImg, this.incidentsList});

      setObjBoxsComponents(new Component[] {this.nomBox, this.aliasBox, this.enceinteTypeBox, this.entiteBox,
         this.couleurEnceinteBox, this.incidentsListEdit});

      setRequiredMarks(new Component[] {this.nomRequired, this.typeRequired});
      ...
      this.objCreateComponents = new Component[] {this.createEnceintes, this.createEnceintesButton};

      this.objArborescenceComponents = new Component[] {this.rowCreateTitle1, this.rowCreateTitle2, this.rowEnceintesEdit,
         this.rowTerminaleType, this.rowNbPlacesTerminale, this.rowTerminaleNumerotation, this.rowEnceintesEdit,
         this.typesTerminaleBox, this.numerotationsBox, this.enceintesListEdit, this.nbPlacesEachEnceinteBox};
   ...
   public void setObject(final TKdataObject e){
      ...
      if(enceinte.getEnceinteId() != null){
         ...
         initIncidents(ManagerLocator.getIncidentManager().findByEnceinteManager(enceinte));
      ...
      if(enceinte.getEnceinteId() != null){
         disableActionsForForeignPlateformeConteneur(
            ManagerLocator.getEnceinteManager().getConteneurManager(enceinte).getPlateformeOrig());
      }
   ...
   public void switchToStaticMode(){
      ...
      super.switchToStaticMode(this.enceinte.getEnceinteId() == null);
      ...
      if(this.enceinte.getEnceinteId() == null){
         ...
         if(banques.size() > 0 && !banques.contains(SessionUtils.getSelectedBanques(sessionScope).get(0))){
            deplacer.setVisible(false);
   ...
   public void switchToEditMode(){
      ...
      if(ManagerLocator.getEnceinteManager().getEnceintesManager(enceinte).size() == 0
         && ManagerLocator.getEnceinteManager().getTerminalesManager(enceinte).size() == 0){
         for(int i = 0; i < objCreateComponents.length; i++){
            objCreateComponents[i].setVisible(true);
         }
      }else{
   ...
   public void switchToDeplacerMode(){
      ...
      // adresse de départ
      adresseActuelle = ManagerLocator.getEnceinteManager().getAdrlManager(enceinte);
   ...
   public void createNewObject(){
      try{
         ...
         if(!arborescenceCreee){
            // creation de l'objet
            ManagerLocator.getEnceinteManager().createObjectManager(enceinte, selectedEnceinteType, conteneur, enceintePere,
               selectedEntite, banques, selectedCouleur, SessionUtils.getLoggedUser(sessionScope));
         }else{
            ...
            // création de l'objet
            ManagerLocator.getEnceinteManager().createAllArborescenceManager(enceinte, enceintes, terminale, firstPositions,
               banques, SessionUtils.getLoggedUser(sessionScope));
         }
   ...
   public void onClick$deplacer(){
      if(this.enceinte.getEnceinteId() != null){
         ...
         getObjectTabController().getListeStockages()
            .switchToDeplacementMode(ManagerLocator.getEnceinteManager().getDistinctBanquesFromTkObjectsManager(enceinte));
      }
   }
   ...
   public void setFieldsToUpperCase(){
      if(this.enceinte.getNom() != null){
         this.enceinte.setNom(this.enceinte.getNom().toUpperCase().trim());
      }
   }
   ...
   public void onClick$createEnceintesButton(){
      ...
      if(this.conteneur != null){
         ...
         cont = ManagerLocator.getEnceinteManager().getConteneurManager(this.enceintePere);
         start = ManagerLocator.getEnceinteManager().getLevelEnceinte(this.enceintePere);
         ++start;
      }
   ...
   public void setEmptyToNulls(){
      ...
      if(couleurEnceinteBox.getSelectedItem() != null){
         if(((Couleur) couleurEnceinteBox.getSelectedItem().getValue()).getCouleurId() != null){
            selectedCouleur = (Couleur) couleurEnceinteBox.getSelectedItem().getValue();
         }
      }
   ...
   public void updateObject(){
      try{
         ...
         if(!arborescenceCreee){
            // update de l'objet
            ManagerLocator.getEnceinteManager().updateObjectManager(enceinte, selectedEnceinteType, this.enceinte.getConteneur(),
               this.enceinte.getEnceintePere(), selectedEntite, banques, selectedCouleur, getIncidents(),
            ...
            // création de l'objet
            ManagerLocator.getEnceinteManager().updatewithCreateAllArborescenceManager(enceinte, enceintes, terminale,
               firstPositions, banques, SessionUtils.getLoggedUser(sessionScope));
         }
      }catch(final Exception e){
         throw new RuntimeException(e.getMessage());
      }
   ...
   protected List<String> deplacerObject(){
      ...
      final Conteneur c = enceinte.getConteneur();
      final Enceinte ep = enceinte.getEnceintePere();
      final Integer p = enceinte.getPosition();
      ...
      try{
         ...
         if(enceinteDestination.isVide()){
            ...
            ManagerLocator.getEnceinteManager().updateObjectManager(enceinte, enceinte.getEnceinteType(),
               this.enceinteDestination.getEnceinte().getConteneur(), this.enceinteDestination.getEnceinte().getEnceintePere(),
               this.enceinte.getEntite(), banques, this.enceinte.getCouleur(), null, SessionUtils.getLoggedUser(sessionScope),
               ops);
         }else{
            ...
            ManagerLocator.getEnceinteManager().echangerDeuxEnceintesManager(enceinte, enceinteAEchanger,
         ...
         errorMsg.add("- Doublon détecté lors du " + "déplacement de l'enceinte.");
      }catch(final RequiredObjectIsNullException re){
         errorMsg.add("Objet obligatoire manquant lors du déplacement " + "de l'enceinte.");
      }catch(final InvalidParentException ipe){
         errorMsg.add("- Erreur sur le " + "parent de l'enceinte.");
      }catch(final InvalidPositionException ipose){
         errorMsg.add("- Erreur sur la " + "position de l'enceinte.");
   ...
   public boolean prepareDeleteObject(){
      final boolean isUsed = ManagerLocator.getEnceinteManager().isUsedObjectManager(getObject());

      setDeleteMessage(
         ObjectTypesFormatters.getLabel("message.deletion.message", new String[] {Labels.getLabel(getDeletionMessage())}));
      setCascadable(false);
      if(isUsed){
         setDeleteMessage(Labels.getLabel("enceinte.deletion.isUsed"));
      }
      setDeletable(!isUsed);
   ...
   public void removeObject(final String comments){
      ManagerLocator.getEnceinteManager().removeObjectManager(getObject(), comments, SessionUtils.getLoggedUser(sessionScope));
   }
   ...
   public void onLaterCreate(){
      ...
      try{
         ...
         if(getObjectTabController().getListeStockages() != null){
            ...
            getObjectTabController().getListeStockages().updateEnceinte(enceinte, true);
         }
   ...
   public void onLaterUpdate(){
      ...
      try{
         ...
         // des stockages
         if(getObjectTabController().getListeStockages() != null){
            getObjectTabController().getListeStockages().updateEnceinte(enceinte, false);
         }
   ...
   public void onLaterDeplacement(){
      ...
      if(!getObjStatutIncompatibleForRetour(prepareObjectsAndEmplacements(), null) && deplacerObject().size() == 0){

         // on vérifie que l'on retrouve bien la page contenant la liste
         // des stockages
         if(getObjectTabController().getListeStockages() != null){
            // echange des enceintes
            getObjectTabController().getListeStockages().deplacerDeuxEnceintes(enceinte, enceinteAEchanger);
         }
      ...
      if(!getEmplForRetours().isEmpty()){
         ...
         if(askForRetoursCreation(getEmplForRetours().size(), null, null, new ArrayList<OldEmplTrace>(), null,
            ...
            openRetourFormModale(null, true, null, null,
               emplForRetours.stream().map(o -> o.getTkObj()).collect(Collectors.toList()), emplForRetours, null, null, null,
               null, Labels.getLabel("ficheRetour.deplacement"), null);
         }
      }
   }
   ...
   public void onLaterDelete(final Event event){
      try{
         ...
         if(getObjectTabController().getListeStockages() != null){
            ...
            getObjectTabController().getListeStockages().deleteEnceinte(conteneur, enceintePere, position);
         }
    ...
    * @param event
    */
   public void onGetUpdateTaille(final Event event){
   ...
   public void initEditableMode(){
      ...
      types = ManagerLocator.getEnceinteTypeManager().findByOrderManager(SessionUtils.getPlateforme(sessionScope));
      ...
      couleurEnceinteBox.setModel(new SimpleListModel<>(couleurs));
      couleurEnceinteBox.setSelectedIndex(couleurs.indexOf(selectedCouleur));
    ...
    * @param conteneur ne permettant pas le déplacement (destination ou départ)
    * @param missBanks TK-251 le conteneur destination n'est pas accessible pas toutes ces banques
    * @version 2.2.1
    */
   public void definirEnceinteDestination(final EnceinteNode node, final Conteneur uCont, final List<Banque> missBanks){
      ...
      if(node != null){
         ...
         positionDestination = enceinteDestination.getEnceinte().getPosition();
         ...
         if(!enceinteDestination.isVide()){
            ...
            adresseDestination = ManagerLocator.getEnceinteManager().getAdrlManager(enceinteDestination.getEnceinte());
            ...
            if(enceinteDestination.getEnceinte().getConteneur() != null){
               adresseDestination = enceinteDestination.getEnceinte().getConteneur().getCode();
            }else{
               final StringBuffer sb = new StringBuffer();
               sb.append(ManagerLocator.getEnceinteManager().getAdrlManager(enceinteDestination.getEnceinte().getEnceintePere()));
               sb.append(".");
               sb.append(enceinteDestination.getEnceinte().getEnceintePere().getNom());
         ...
         // TK-251
         ((Label) rowErreurDestination.getFellow("errMsgLabel")).setValue(uCont != null && missBanks != null
            ? ObjectTypesFormatters.getLabel("stockage.deplacement.conteneur.destination.banks",
               new String[] {uCont.getCode(), missBanks.stream().map(b -> b.getNom()).collect(Collectors.joining(", "))})
            : Labels.getLabel("terminale.deplacement.emplacement.destination.incorrect"));
      }
   }
   ...
   public String getCouleurSrc(){
      ...
      if(this.enceinte != null && this.enceinte.getEnceinteId() != null && this.enceinte.getCouleur() != null){
         sb.append(colorSrc);
   ...
   public void setBanquesReservees(final String reservees){
   ...
   public void setTypes(final List<EnceinteType> t){
      this.types = t;
   }
   ...
   public void setEnceintePere(final Enceinte ep){
   ...
   public void setContenuDestination(final String contenu){
   ...
   public void setEnceinteDestination(final EnceinteNode enceinteDest){
   ...
   public void setEnceinteAEchanger(final Enceinte enceinteAEch){
   ...
   public void setChildEnceintesTypes(final List<EnceinteType> childEnceintes){
   ...
   public void setSelectedChildEnceinteType(final EnceinteType selectedChild){
   ...
   public void setSelectedTerminaleType(final TerminaleType selectedTerminaleT){
   ...
   public void setSelectedTerminaleNumerotation(final TerminaleNumerotation selectedTerminaleNum){
   ...
   public void setCouleurs(final List<Couleur> c){
      this.couleurs = c;
   }
   ...
   public List<OldEmplTrace> getEmplForRetours(){
      return emplForRetours;
   }

   public void setEmplForRetours(final List<OldEmplTrace> e){
   ...
   private List<TKStockableObject> prepareObjectsAndEmplacements(){
      ...
      List<Terminale> terms = ManagerLocator.getEnceinteManager().getAllTerminalesInArborescenceManager(enceinte);
      for(int i = 0; i < terms.size(); i++){
         // emplForRetours.putAll(ManagerLocator.getTerminaleManager().getTkObjectsAndEmplacementsManager(terms.get(i)));
         emplForRetours.addAll(ManagerLocator.getTerminaleManager().getTkObjectsEmplacementTracesManager(terms.get(i)));
      }
      ...
      if(enceinteDestination != null && !enceinteDestination.isVide()){
         ...
         // ces valeurs seront utilisées pour la création des
         // RETOURs dans le cadre de ce déplacement
         terms = ManagerLocator.getEnceinteManager().getAllTerminalesInArborescenceManager(enceinteDestination.getEnceinte());
         for(int i = 0; i < terms.size(); i++){
            // emplForRetours.putAll(ManagerLocator.getTerminaleManager().getTkObjectsAndEmplacementsManager(terms.get(i)));
            emplForRetours.addAll(ManagerLocator.getTerminaleManager().getTkObjectsEmplacementTracesManager(terms.get(i)));
         }
      }
      return emplForRetours.stream().map(o -> o.getTkObj()).collect(Collectors.toList());
   }
    ...
    */
   @Override
   public boolean disableActionsForForeignPlateformeConteneur(final Plateforme pOrig){
      final boolean enabled = super.disableActionsForForeignPlateformeConteneur(pOrig);
