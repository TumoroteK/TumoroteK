...
public class FicheEnceinte extends AbstractFicheCombineStockageController
{
	...
	// TK-291
	private List<OldEmplTrace> emplForRetours = new ArrayList<OldEmplTrace>();
...
//	}
	...
	public void onClick$deplacer(){
		if(this.enceinte.getEnceinteId() != null){
			...
			getObjectTabController().getListeStockages().switchToDeplacementMode( 
					ManagerLocator.getEnceinteManager().getDistinctBanquesFromTkObjectsManager(enceinte));
		}
	}
		...
		if(!getEmplForRetours().isEmpty()){
			...
			if(askForRetoursCreation(getEmplForRetours().size(), null, null, new ArrayList<OldEmplTrace>(), null,
				...
				openRetourFormModale(null, true, null, null, 
						emplForRetours.stream().map(o -> o.getTkObj()).collect(Collectors.toList()),
	 ...
	 * @version 2.2.1 
	 */
	public void definirEnceinteDestination(final EnceinteNode node, Conteneur uCont, List<Banque> missBanks){
		...
		if(node != null){
			...
			// TK-251
			((Label) rowErreurDestination.getFellow("errMsgLabel"))
			.setValue(uCont != null && missBanks != null ? 
					ObjectTypesFormatters.getLabel("stockage.deplacement.conteneur.destination.banks", 
							new String[] { uCont.getCode(), 
									missBanks.stream().map(b -> b.getNom()).collect(Collectors.joining(", "))})
