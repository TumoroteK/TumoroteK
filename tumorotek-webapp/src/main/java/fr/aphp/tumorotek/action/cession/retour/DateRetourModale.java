...
public class DateRetourModale {
	...
	private List<Retour> retoursToUpdate = new ArrayList<Retour>();
	private List<SimpleCodeDateSortie> simpleCDs = 
							new ArrayList<SimpleCodeDateSortie>();
	...
	@Init
	public void init(
		@ExecutionArgParam("objs") List<TKStockableObject> objs,
		@ExecutionArgParam("controller") AbstractObjectTabController ctr,
	 ...
	 */
	@Command
	public void validate(@BindingParam("dateRetour") java.util.Calendar dateRetour) {	
		try {
			for (Retour ret : getRetoursToUpdate()) {
				ret.setDateRetour(dateRetour);
			}
			ManagerLocator.getRetourManager()
				.updateMultipleObjectManager(getRetoursToUpdate(), 
										null, null, null, null, user);
			
			// mise à jour liste		
			if (controller != null && controller.getListe() != null) {
				if (controller instanceof EchantillonController 
						&& controller.getMainWindow().isFullfilledComponent(
					...
					controller.getListe()
						.updateMultiObjectsGridListFromOtherPage(echans);
					...
					controller.getListe()
					.updateMultiObjectsGridListFromOtherPage(derives);
				}
			}
		} catch (Exception e) {
			Messagebox.show(AbstractController.handleExceptionMessage(e), 
	...
	public void setObjects(List<TKStockableObject> obs) {
		this.objects = obs;
...
//		}
	...
	public void setRetoursToUpdate(List<Retour> rs) {
	...
	public void setDateRetour(Calendar d) {
		this.dateRetour = d;
	}
	...
	public void setSimpleCDs(List<SimpleCodeDateSortie> ss) {
		this.simpleCDs = ss;
	}
	...
	public void setController(AbstractObjectTabController c) {
		this.controller = c;
	}
	...
	public class SimpleCodeDateSortie {
		...
		public SimpleCodeDateSortie(String c, Calendar dateS) {
			code = c;
			dateSortie = dateS;
		}
		...
		public void setDateSortie(Calendar dS) {
			this.dateSortie = dS;
		}
