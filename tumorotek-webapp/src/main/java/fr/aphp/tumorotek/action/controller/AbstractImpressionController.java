...
public abstract class AbstractImpressionController 
	...
	protected List<BlocImpressionDecorator> blocImpressionsDecorated = 
	 ...
	 */
	public void openChampsWindow(Page page, 
		 ...
		 if (!blockModal) {
			...
			final HtmlMacroComponent ua = populateChampsModal(
					win, page, deco, path);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateChampsModal(
		...
		page.getComponentDefinition("champImpression", false)
		...
		((FicheChampsImpression) ua.getFellow("winChampImpression")
				.getAttributeOrFellow("winChampImpression$composer", true))
	 ...
	 * @param bloc
	 */
	public void upBloc(BlocImpressionDecorator deco) {
		int blocIndex = blocImpressionsDecorated.indexOf(deco);
		BlocImpressionDecorator supObjectInList = null;
		if (blocIndex - 1 > -1) {
			supObjectInList = (BlocImpressionDecorator) 
				blocImpressionsDecorated.get(blocIndex - 1);
			blocImpressionsDecorated.set(blocIndex, supObjectInList);
			blocImpressionsDecorated.set(blocIndex - 1, 
					(BlocImpressionDecorator) deco);
		}
	}
	 ...
	 * @param bloc
	 */
	public void downBloc(BlocImpressionDecorator deco) {
		int blocIndex = blocImpressionsDecorated.indexOf(deco);
	 ...
	 * @param event
	 */
	public void onCheckBloc(ForwardEvent event) {
		...
		BlocImpressionDecorator deco = 
			(BlocImpressionDecorator) event.getData();
	 ...
	 * @param event
	 */
	public void onClickEditChamps(ForwardEvent event) {
		// on récupère le bloc associé à l'event
		BlocImpressionDecorator deco = 
			(BlocImpressionDecorator) event.getData();
	 ...
	 * @param e
	 */
	public void onGetChangeChampImpression(Event e) {
		BlocImpressionDecorator deco = (BlocImpressionDecorator) e.getData();
	...
	public void setBlockModal(boolean block) {
	...
	public void setBlocImpressionsDecorated(
			List<BlocImpressionDecorator> bDecorated) {
