...
public class DefautComponent extends HtmlMacroComponent {
	...
	private Log log = LogFactory.getLog(DefautComponent.class);
	...
	private List<Item> itemsCopy = new ArrayList<Item>();
	...
	public void setChpDeco(ChampAnnotationDecorator cpD) {
		this.chpDeco = cpD;
	}
	
	public void setBackComp(Component bC) {
		this.backComp = bC;
	}
	...
	public void afterCompose() {
		...
		this.addEventListener("onUpdateItem", 
		...
		this.addEventListener("onAddDefautForItem", 
				new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
		...
		this.addEventListener("onDropDefautForItem", 
				new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
		...
		if (chpDeco.isThesaurus()) {
			getFirstChild().getFellow("addItemButton")
				.addEventListener("onClick", new EventListener<Event>() {
					public void onEvent(Event event) throws Exception {
	...
	private void drawDefautEditablesComponents() {
		if (this.chpDeco.isAlphanum()) {
			...
			if (chpDeco.getDefauts().iterator().next().getDate() != null) {
				((Datebox) box).setValue(chpDeco.getDefauts().iterator()
			...
			// ((Datebox) box).setWidth("200px");
			if (chpDeco.getDefauts().iterator().next().getDate() != null) {
				((CalendarBox) box).setValue(chpDeco.getDefauts().iterator()
											.next().getDate());
			}
			...
			if (chpDeco.getDefauts().iterator().next().getAlphanum() != null) {
				((Decimalbox) box).setValue(new BigDecimal(
						chpDeco.getDefauts().iterator().next().getAlphanum()));
			}
			...
			box = new Textbox();
			((Textbox) box).setHflex("1");
			ConstHyperlien constr = new ConstHyperlien();
			constr.setNullable(true);
			((Textbox) box).setConstraint(constr);
			if (chpDeco.getDefauts().iterator().next().getAlphanum() != null) {
				((Textbox) box).setValue(chpDeco.getDefauts().iterator()
	...
	private Component drawItemGrid() {
		...
		Rows rows = new Rows();
		rows.addEventListener("onClickUpdateItem", 
				new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
				openItemModale((Item) event.getData(), false);
			}
		});
		rows.addEventListener("onClickDeleteItem", 
				new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
		...
		//copy la liste items
		itemsCopy.clear();
		List<Item> its = new ArrayList<Item>(chpDeco.getItems());
...
//		}
		...
		ListModel<Item> model = new ListModelList<Item>(itemsCopy);
		itemGrid.setModel(model);
		//Renderer
		AnnoItemRowRenderer renderer = new AnnoItemRowRenderer();
	...
	private void extractValuesFromDefautBox() {
		if (this.chpDeco.isAlphanum() || this.chpDeco.isHyperlien()) {
			if (!((Textbox) box).getValue().equals("")) {
				defauts.iterator().next()
							.setAlphanum(((Textbox) box).getValue());
			} else {
				defauts.iterator().next().setAlphanum(null);
			}
		} else if (this.chpDeco.isBoolean()) {
			if (((Listbox) box).getSelectedItem() != null) {
				defauts.iterator().next().setBool((Boolean) 
					((Listbox) box).getSelectedItem().getValue());
			} else {	
			...
			if (((CalendarBox) box).getValue() != null) {
				Calendar sDate = ((CalendarBox) box).getValue();
				defauts.iterator().next().setDate(sDate);
			} else {
				defauts.iterator().next().setDate(null);
			}
			...
			if (((Decimalbox) box).getValue() != null) {
				defauts.iterator().next()
						.setAlphanum(((Decimalbox) box).getValue().toString());
			} else {
				defauts.iterator().next().setAlphanum(null);
			}
		} else if (this.chpDeco.isTexte()) {
	 ...
	 */
	public void openItemModale(Item item, boolean isCreate) {
		...
		if (!blockModal) {
			...
			final ItemComponent ua = (ItemComponent) 
				getPage().getComponentDefinition("itemComp", false)
									.newInstance(getPage(), 
								"fr.aphp.tumorotek.action.administration."
											+ "annotations.ItemComponent");
			ua.setItem(item);
			ua.setBackComp(this);
			...
			if (!chpDeco.isThesaurusM()) {
				Iterator<AnnotationDefaut> itor = defauts.iterator();
				Item it;
				while (itor.hasNext()) {
			...
			ua.setIsDefaut(AnnoItemRowRenderer
									.isItemDefaut(item, defauts));
	 ...
	 */
	private void addOrUpdateItemIntoList(Item item, boolean isCreate) {		
		// ajout
		if (item != null && isCreate) {
			item.setChampAnnotation(chpDeco.getChamp());
			
			if (chpDeco.getCatalogueChp()) {
				item.setPlateforme(chpDeco.getCurrentBanque()
												.getPlateforme());
			}
	 ...
	 * @param it
	 */
	private void deleteItemFromList(Item it) {
		if (it != null) {
			// warning
			if (it.getItemId() != null 
				&& ManagerLocator
					.getChampAnnotationManager().isUsedItemManager(it)) {
				// confirmation demandée
				if (Messagebox.show(Labels
							.getLabel("item.deletion.isUsed"),
	 ...
	 */
	private void addDefautsToList(Item it) {
		AnnotationDefaut def = new AnnotationDefaut();
	 ...
	 * @param item
	 */
	private void removeDefautsFromList(Item item) {
		Iterator<AnnotationDefaut> itor = defauts.iterator();
		AnnotationDefaut def;
		while (itor.hasNext()) {
			def = itor.next();
	...
	private void createBoolbox() {		
		...
		boolsItem = new ArrayList<Listitem>();
