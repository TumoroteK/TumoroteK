...
public class CataloguesRowRenderer implements RowRenderer<Catalogue> {
	...
	@Override
	public void render(Row row, Catalogue data, int index) {
		...
		if (row.getParent().getAttribute("checkedCatalogues") != null) {
			...
			checkCat.addForward("onCheck", row.getParent(), 
											"onCheckCatalogue", catalogue);
			checkCat.setChecked(((List<Catalogue>) 
					row.getParent().getAttribute("checkedCatalogues"))
															.contains(data));
