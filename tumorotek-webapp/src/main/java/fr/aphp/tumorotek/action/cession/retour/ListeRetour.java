...
public class ListeRetour extends AbstractListeController2
{
   ...
   public void setLwinRetour(final Window lw){
      this.lwinRetour = lw;
   }
   ...
   public void addToListObjects(final TKdataObject obj, final Integer pos){
      if(pos != null){
         getListObjects().add(pos.intValue(), obj);
      }else{
         getListObjects().add(obj);
      }
   }
   ...
   public void setObject(final TKStockableObject o){
      this.object = o;
      initObjectsBox();
   }
   ...
   public void initObjectsBox(){
      ...
      final List<Retour> rets = ManagerLocator.getRetourManager().getRetoursForObjectManager(object);

      listObjects.addAll(RetourDecorator.decorateListe(rets, object != null ? object.getObjetStatut() : null));
    ...
    * @param event
    */
   public void onClick$viewObj(final Event event) throws IOException{
      openRetourFormModale(((RetourDecorator) AbstractListeController2.getBindingData((ForwardEvent) event, false)).getRetour(),
         false);
   }
   ...
   public void onClick$deleteObj(final Event event){
      // confirmation
      if(Messagebox.show(
         ObjectTypesFormatters.getLabel("message.deletion.message", new String[] {Labels.getLabel("message.deletion.retour")}),
         Labels.getLabel("message.deletion.title"), Messagebox.YES | Messagebox.NO, Messagebox.QUESTION) == Messagebox.YES){

         final RetourDecorator deco = (RetourDecorator) AbstractListeController2.getBindingData((ForwardEvent) event, false);

         ManagerLocator.getRetourManager().removeObjectManager(deco.getRetour());

         setObject(ManagerLocator.getRetourManager().getObjetFromRetourManager(deco.getRetour()));
   ...
   public boolean updateObjectGridListFromOtherPage(final Object obj, final boolean select){
      ...
      // l'objet passé en paramètre est cloné
      final RetourDecorator edit = new RetourDecorator((Retour) obj, getObject().getObjetStatut());
   ...
   public void addToObjectList(final Object newObj){
      ...
      // du formulaire
      final RetourDecorator newRetour = new RetourDecorator(((Retour) newObj).clone(), getObject().getObjetStatut());
      ...
      final Rows rows = objectsListGrid.getRows();
      final List<Component> comps = rows.getChildren();
      selectRow((Row) comps.get(this.listObjects.indexOf(newRetour)), newRetour);
   ...
   public boolean getDisableAddNew(){
      if(canEditObject){
         return getObject() == null || getObject().getObjetStatut() == null
    ...
    */
   private void openRetourFormModale(final Retour retour, final boolean b){
      final Hashtable<TKStockableObject, String> oldEmps = new Hashtable<>();
      oldEmps.put(getObject(), ObjectTypesFormatters.getEmplacementAdrl(getObject()));

      openRetourFormModale(retour, b, this, getObject(), null, null, null, null, null, null, null, null);
   }
