...
public class FicheCessionEdit extends AbstractFicheEditController {
	...
	private Log log = LogFactory.getLog(FicheCessionEdit.class);
	...
	private Label cederMultiEchantillonsLabel;
	private Label cederMultiProdDerivesLabel;
	...
	private List<CederObjet> echantillonsCedes = new ArrayList<CederObjet>();
	private List<CederObjet> derivesCedes = new ArrayList<CederObjet>();
	private List<CederObjetDecorator> echantillonsCedesDecores 
		= new ArrayList<CederObjetDecorator>();
	private List<CederObjetDecorator> derivesCedesDecores 
		= new ArrayList<CederObjetDecorator>();
	...
	private int _pageSizeP = 10;
	private int _startPageNumberP = 0;
	private int _totalSizeP = 0;
	private boolean _needsTotalSizeUpdateP = true;
	
	// Associations.
	private List<CessionType> types = new ArrayList<CessionType>();
	private CessionType selectedCessionType;
	private List<CessionExamen> examens = new ArrayList<CessionExamen>();
	private CessionExamen selectedCessionExamen;
	private List<Contrat> contrats = new ArrayList<Contrat>();
	private List<String> contratsNoms = new ArrayList<String>();
	private Contrat selectedContrat;
	private List<CessionStatut> statuts = new ArrayList<CessionStatut>();
	private CessionStatut selectedCessionStatut;
	private List<Transporteur> transporteurs = new ArrayList<Transporteur>();
	private Transporteur selectedTransporteur;
	private List<DestructionMotif> motifs = new ArrayList<DestructionMotif>();
	...
	private List<String> codesEchans = new ArrayList<String>();
	private List<String> codesDerives = new ArrayList<String>();
	...
	private List<Retour> retoursToComplete = new ArrayList<Retour>();
	
	// nouveaux objets cedes
	private List<TKStockableObject> newlyCeds = 
	...
	private List<Collaborateur> destinataires = new ArrayList<Collaborateur>();
	private Collaborateur selectedDestinataire;
	private List<Service> servicesDest = new ArrayList<Service>();
	...
	private String echantillonsGroupHeader = 
		Labels.getLabel("cession.echantillons");
	private String derivesGroupHeader = 
		Labels.getLabel("cession.prodDerive");
	...
	private List<CederObjetDecorator> revertedObjs = new ArrayList<CederObjetDecorator>();
	...
	public void setObject(TKdataObject c) {
		...
		if (cession.getCessionId() != null) {
			...
			echantillonsCedes = ManagerLocator.getCederObjetManager()
				.getEchantillonsCedesByCessionManager(cession);
			 echantillonsCedesDecores = cedeObjFactory
				.decorateListe(echantillonsCedes);
					
			// récupération des dérivés cédés
			derivesCedes = ManagerLocator.getCederObjetManager()
				.getProdDerivesCedesByCessionManager(cession);
			derivesCedesDecores = cedeObjFactory
				.decorateListe(derivesCedes);
			
		
			selectedContrat = this.cession.getContrat();
		}
		
		if (this.cession.getDestinataire() != null
				&& destinataires.contains(
					this.cession.getDestinataire())) {
		...
		if (this.cession.getServiceDest() != null
				&& services.contains(this.cession.getServiceDest())) {
	...
	private void initEchansModel(int pageN) {
	...
	private void initDerivesModel(int pageN) {
	...
	public void switchToEditMode() {
		...
		if (this.cession.getCessionType().getType()
				.toUpperCase().equals("SANITAIRE")) {
		...
		} else if (this.cession.getCessionType().getType()
				.toUpperCase().equals("RECHERCHE")) {
			switchToRechercheMode();
		} else if (this.cession.getCessionType().getType()
				.toUpperCase().equals("DESTRUCTION")) {
		...
		// objets cédés
		if (this.cession.getCessionStatut()
			...
			if (!cederEchantillonsButton.isDisabled()) {
				this.cederMultiEchantillonsLabel.setVisible(true);
			} else {
				this.cederMultiEchantillonsLabel.setVisible(false);
			}
			if (!cederDerivesButton.isDisabled()) {
				this.cederMultiProdDerivesLabel.setVisible(true);
			} else {
				this.cederMultiProdDerivesLabel.setVisible(false);
			}
	...
	public void switchToCreateMode(List<Echantillon> echansACeder,
		...
		if (this.selectedCessionType.getType()
				.toUpperCase().equals("SANITAIRE")) {
			switchToSanitaireMode();
			this.demandeurHelpLabel.setVisible(false);
			this.descriptionHelpLabel.setVisible(false);
			this.dateValidationHelpLabel.setVisible(false);
			this.titreEtudeHelpLabel.setVisible(false);
		} else if (this.selectedCessionType.getType()
				.toUpperCase().equals("RECHERCHE")) {
			switchToRechercheMode();
		} else if (this.selectedCessionType.getType()
				.toUpperCase().equals("DESTRUCTION")) {
		...
		if (echansACeder != null) {
			...
			for (int i = echansACeder.size() - 1; i >= 0; i--) {
				Echantillon echan = echansACeder.get(i);
		...
		if (derivesACeder != null) {
			...
			for (int i = derivesACeder.size() - 1; i >= 0; i--) {
				ProdDerive derive = derivesACeder.get(i);
	...
	public void initEditableMode() {
		...
		CessionType tmp = ManagerLocator.getCessionTypeManager()
			.findByTypeLikeManager("Recherche", false).get(0);
		typesBox.setModel(new SimpleListModel<CessionType>(types));
		if (this.cession.getCessionType() != null) {
			selectedCessionType = this.cession.getCessionType();
			typesBox.setSelectedIndex(types.indexOf(
					this.cession.getCessionType()));
		} else {
			...
			typesBox.setSelectedIndex(types.indexOf(
					this.selectedCessionType));
		}
		// init des examens
		examens = (List<CessionExamen>) 
			ManagerLocator.getCessionExamenManager()
				.findByOrderManager(SessionUtils.getPlateforme(sessionScope));
		examens.add(0, null);
		selectedCessionExamen = this.cession.getCessionExamen();
		
		// init des Mtas
		contrats = ManagerLocator.getContratManager()
			.findAllObjectsByPlateformeManager(
				SessionUtils.getPlateforme(sessionScope));
		selectedContrat = this.cession.getContrat();
		
		contratsNoms = new ArrayList<String>();
		for (int i = 0; i < contrats.size(); i++) {
			StringBuffer sb = new StringBuffer();
			sb.append(contrats.get(i).getNumero());
			if (contrats.get(i).getTitreProjet() != null
					&& !contrats.get(i).getTitreProjet().equals("")) {
				sb.append(" (");
		...
		if (selectedContrat != null) {
			...
			if (selectedContrat.getTitreProjet() != null
					&& !selectedContrat.getTitreProjet().equals("")) {
				sb.append(" (");
		...
		transporteurs = ManagerLocator.getTransporteurManager()
			.findAllActiveManager();
		transporteurs.add(0, null);
		selectedTransporteur = this.cession.getTransporteur();
		
		// init des motifs
		motifs = (List<DestructionMotif>) 
			ManagerLocator.getDestructionMotifManager()
				.findByOrderManager(SessionUtils.getPlateforme(sessionScope));
		motifs.add(0, null);
		selectedDestructionMotif = this.cession.getDestructionMotif();
		
		temperatures = ManagerLocator.getTemperatureManager()
		...
		if (this.cession.getDemandeur() != null
			&& !collaborateurs.contains(this.cession.getDemandeur())) {
			collaborateurs.add(this.cession.getDemandeur());
			nomsAndPrenoms.add(this.cession.getDemandeur().getNomAndPrenom());
		}
		if (this.cession.getDestinataire() != null
			&& !collaborateurs.contains(this.cession.getDestinataire())) {
			collaborateurs.add(this.cession.getDestinataire());
			nomsAndPrenoms.add(this.cession
									.getDestinataire().getNomAndPrenom());
		}
		if (this.cession.getExecutant() != null
			&& !collaborateurs.contains(this.cession.getExecutant())) {
		...
		services.add(0, null);
		ListModel<Service> listTmp = new ListModelList<Service>(services);
		servicesBoxCession.setModel(listTmp);
		servicesBoxCession.setSelectedIndex(services.indexOf(selectedService));
		
		// init des collaborateurs
		if (selectedService != null) {
			destinataires = ManagerLocator.getServiceManager()
				.getActiveCollaborateursWithOrderManager(
						selectedService);
		} else {
			destinataires = ManagerLocator.getCollaborateurManager()
				.findAllActiveObjectsWithOrderManager();
		}
		destinataires.add(0, null);
		
		ListModel<Collaborateur> listCollabs = 
					new ListModelList<Collaborateur> (destinataires);
		destinataireBoxCession.setModel(listCollabs);
		destinataireBoxCession.setSelectedIndex(
				destinataires.indexOf(selectedDestinataire));
		
		// Gestion des statuts
		// si la cession n'a pas de statut, c'est le statut "EN ATTENTE"
		// qui sera sélectionné
		statuts = ManagerLocator.getCessionStatutManager()
			.findAllObjectsManager();
		statutsBox.setModel(new SimpleListModel<CessionStatut>(statuts));
		statutsBox2.setModel(new SimpleListModel<CessionStatut>(statuts));
		CessionStatut validee = ManagerLocator.getCessionStatutManager()
			.findByStatutLikeManager("VALIDEE", true).get(0);
		CessionStatut refusee = ManagerLocator.getCessionStatutManager()
			.findByStatutLikeManager("REFUSEE", true).get(0);
		CessionStatut attente = ManagerLocator.getCessionStatutManager()
			.findByStatutLikeManager("EN ATTENTE", true).get(0);
		...
		if (selectedCessionStatut.equals(validee)
				|| selectedCessionStatut.equals(refusee)) {
		...
		if (selectedCessionStatut.getStatut().equals("EN ATTENTE")) {
			...
			if (sessionScope.containsKey("AdminPF")) {
				availableBanques = ManagerLocator.getBanqueManager()
					.findByPlateformeAndArchiveManager(
							SessionUtils.getPlateforme(sessionScope), false);
			} else {
				// init des banques dispos
				availableBanques = ManagerLocator
				.getBanqueManager()
				.findByEntiteModifByUtilisateurManager(
						SessionUtils.getLoggedUser(sessionScope), 
						entite, SessionUtils.getPlateforme(sessionScope));
			}
			
			// init des listbox des banques
			SimpleListModel<Banque> list = 
				new SimpleListModel<Banque>(availableBanques);
			banquesEchantillonsBox.setModel(list);
			banquesEchantillonsBox.setSelectedIndex(
					availableBanques.indexOf(SessionUtils
				.getSelectedBanques(sessionScope).get(0)));
			banquesProdDerivesBox.setModel(list);
			banquesProdDerivesBox.setSelectedIndex(
					availableBanques.indexOf(SessionUtils
				.getSelectedBanques(sessionScope).get(0)));
			
			codesEchans = ManagerLocator.getEchantillonManager()
				.findAllCodesForBanqueAndStockesManager(
	...
	public void createNewObject() {
		...
		try {
			...
			List<CederObjet> objetsCedes = new ArrayList<CederObjet>();
			echantillonsCedes.clear();
			for (int i = 0; i < echantillonsCedesDecores.size(); i++) {
				objetsCedes.add(echantillonsCedesDecores.get(i)
						.getCederObjet());
				echantillonsCedes.add(echantillonsCedesDecores.get(i)
						.getCederObjet());
			}
			derivesCedes.clear();
			...
			// create de l'objet
			ManagerLocator.getCessionManager().createObjectManager(cession, 
					SessionUtils.getSelectedBanques(sessionScope).get(0), 
					selectedCessionType, selectedCessionExamen, 
					selectedContrat, selectedDestinataire, selectedService, 
					selectedDemandeur, selectedCessionStatut, 
					selectedExecutant, 
					selectedTransporteur, selectedDestructionMotif, 
					getObjectTabController().getFicheAnnotation()
											.getValeursToCreateOrUpdate(),
					filesCreated,
					SessionUtils.getLoggedUser(sessionScope), 
					objetsCedes, SessionUtils.getSystemBaseDir());

			Document doc = null;
			if (cession.getCessionStatut().getStatut().equals("VALIDEE")) {
				doc = CessionUtils.createFileHtmlToPrint(cession, 
										echantillonsCedes, derivesCedes);
			}
...
//			}
	...
	public void updateObject() {
		...
		try {
			...
			// update de l'objet
			ManagerLocator.getCessionManager().updateObjectManager(cession, 
					cession.getBanque(), 
					selectedCessionType, selectedCessionExamen, 
					selectedContrat, selectedDestinataire, selectedService, 
					selectedDemandeur, selectedCessionStatut, 
			...
			if (getCopy().getCessionStatut()
				...
				getObjectTabController()
					.updateContrat(getObject().getContrat());
	...
	public boolean onLaterCreate() {
		try {
			...
			if (!getNewlyCeds().isEmpty()) {
				...
				proposeRetoursCreation(getNewlyCeds(), 
						null,
						date, 
						cession, null, null, 
						ManagerLocator.getOperationTypeManager()
									.findByNomLikeManager("Creation", true).get(0),
									Labels.getLabel("ficheRetour.observations.cession"), 
									getSelectedExecutant());
				//}	
			}
			if (terminaleDestockage != null) {
				getStockageController()
					.switchToFicheTerminaleMode(terminaleDestockage, null);
			}
	...
	private List<TKdataObject> revertCessedObjects() {
		List<TKdataObject> children = new ArrayList<TKdataObject>();


		for (CederObjetDecorator deco : revertedObjs) {
	...
	protected void setEmptyToNulls() {
		if (this.cession.getDescription() != null 
				&& this.cession.getDescription().equals("")) {
			this.cession.setDescription(null);
		}
		
		if (this.cession.getObservations() != null
				&& this.cession.getObservations().equals("")) {
			this.cession.setObservations(null);
		}
		
		if (this.cession.getEtudeTitre() != null 
				&& this.cession.getEtudeTitre().equals("")) {
			this.cession.setEtudeTitre(null);
		}
		
		// on place tous les objets cédés dans une seule liste
		List<CederObjet> objetsCedes = new ArrayList<CederObjet>();
		echantillonsCedes = new ArrayList<CederObjet>();
		for (int i = 0; i < echantillonsCedesDecores.size(); i++) {
			objetsCedes.add(echantillonsCedesDecores.get(i)
					.getCederObjet());
			echantillonsCedes.add(echantillonsCedesDecores.get(i)
		...
		derivesCedes = new ArrayList<CederObjet>();
		for (int i = 0; i < derivesCedesDecores.size(); i++) {
			objetsCedes.add(derivesCedesDecores.get(i).getCederObjet());
			derivesCedes.add(derivesCedesDecores.get(i).getCederObjet());
		}
		...
		String selectedNomAndPremonDemandeur = this.demandeurBox.getValue()
			.toUpperCase();
		...
		String selectedNomAndPremonExecutant = this.executantBox.getValue()
			.toUpperCase();
	...
	public void setDepartDateProxy(Date date) {
	...
	public void setArriveeDateProxy(Date date) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
	...
	public void setDestructionDateProxy(Date date) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
...
//	}
	...
	private StockageController getStockageController() {
		if (getMainWindow()
				.isFullfilledComponent("stockagePanel", "winStockages")) {
			return ((StockageController) getMainWindow()
				.getMainTabbox().getTabpanels()
				.getFellow("stockagePanel")
				.getFellow("winStockages")
	 ...
	 * @param event Event sur la grille contenant la liste des Services.
	 * @return Un Service.
	 */
	 ...
	 * @throws Exception
	 */
	public void onClick$codeEchantillonCedeEdit(Event event) {
	 ...
	 * @throws Exception
	 */
	public void onClick$codeProdDeriveCedeEdit(Event event) {
	 ...
	 * @throws Exception
	 */
	public void onClick$codeEchantillonCede(Event event) {
		onClickEchantillonCode(event); 
	}
	 ...
	 * @throws Exception
	 */
	public void onClick$codeProdDeriveCede(Event event) {
		onClickProdDeriveCode(event);
	}
	...
	public void onClickEchantillonCode(Event event) {
		CederObjetDecorator deco = (CederObjetDecorator) AbstractListeController2
				.getBindingData((ForwardEvent) event, false);
		
		displayObjectData(deco.getEchantillon());
	}

	public void onSelectAllEchantillons(Event e) {
		List<CederObjetDecorator> decos = 
				cedeObjFactory.decorateListe(echantillonsCedes);
		displayObjectsListData(new 
			ArrayList<TKAnnotableObject>(cedeObjFactory.undecorateListe(decos)));
	}
	...
	public void onSelectAllDerives(Event e) {
		List<CederObjetDecorator> decos = 
	...
	public void onSelect$typesBox() {
		int ind = typesBox.getSelectedIndex();
		
		if (ind > -1) {
			selectedCessionType = types.get(ind);
		}
		
		if (selectedCessionType.getType()
				.toUpperCase().equals("SANITAIRE")) {
			switchToSanitaireMode();
			this.demandeurHelpLabel.setVisible(false);
			this.descriptionHelpLabel.setVisible(false);
			this.dateValidationHelpLabel.setVisible(false);
			this.titreEtudeHelpLabel.setVisible(false);
			statutsBox.setSelectedIndex(
					statuts.indexOf(selectedCessionStatut));
		...
		} else if (selectedCessionType.getType()
				.toUpperCase().equals("RECHERCHE")) {
			switchToRechercheMode();
	...
	public void onSelect$statutsBox() {
		int ind = statutsBox.getSelectedIndex();
	...
	public void onSelect$statutsBox2() {
		int ind = statutsBox2.getSelectedIndex();
	...
	public void onSelect$banquesEchantillonsBox() {
		Banque selected = availableBanques.get(
				banquesEchantillonsBox.getSelectedIndex());
		
		codesEchans = ManagerLocator.getEchantillonManager()
			.findAllCodesForBanqueAndStockesManager(selected);
		echantillonsBox.setModel(new CustomSimpleListModel(codesEchans));
	...
	public void onSelect$banquesProdDerivesBox() {
		Banque selected = availableBanques.get(
				banquesProdDerivesBox.getSelectedIndex());
	 ...
	 */
	public void onSelect$temperatureListBox(Event event) {
		
		if (selectedTemperature != null) {
			cession.setTemperature(
					selectedTemperature.getTemperature());
		} else {
			cession.setTemperature(null);
		}
	}
	 ...
	 * @throws Exception
	 */
	public void onSelect$etabsBoxCession(Event event) throws Exception {
		if (selectedEtablissement != null) {
			services = ManagerLocator.getEtablissementManager()
	 ...
	 * @throws Exception
	 */
	public void onSelect$servicesBoxCession(Event event) throws Exception {
		int ind = servicesBoxCession.getSelectedIndex();
	 ...
	 * @throws Exception
	 */
	public void onSelect$destinataireBoxCession(Event event) throws Exception {
		int ind = destinataireBoxCession.getSelectedIndex();
	...
	public void onClick$remplissageAuto() {
		if (selectedContrat != null) {
			...
			Collaborateur coll = selectedContrat.getCollaborateur();
			if (coll != null) {
				if (nomsAndPrenoms.contains(coll.getNomAndPrenom())) {
					int indColl = nomsAndPrenoms.indexOf(
							coll.getNomAndPrenom());
			...
			if (selectedContrat.getDateValidation() != null) {
				this.cession.setValidationDate(
						selectedContrat.getDateValidation());
			}
			...
			if (selectedContrat.getDescription() != null) {
				this.cession.setDescription(
						selectedContrat.getDescription());
			}
		}
	}
	 ...
	 * restants.
	 */
	public void showLastEchantillons(int start, int end, int pageIdx) {
		Rows rows = echantillonsList.getRows();
		...
		Hashtable<Integer, Integer> prels = 
				new Hashtable<Integer, Integer>();
		
		for (int i = start; i < end; i++) {
			Prelevement prel = echantillonsCedesDecores.get(i).getEchantillon()
					.getPrelevement();
			if (prel != null) {
				if (prels.containsKey(prel.getPrelevementId())) {
					prels.put(prel.getPrelevementId(),
							prels.get(prel.getPrelevementId()) + 1);
				} else {
					prels.put(prel.getPrelevementId(), 1);
				}
				if (PrelevementUtils.getNbEchanRestants(prel)
						- prels.get(prel.getPrelevementId()) <= 0) {
					++nb;
					Row row = (Row) comps.get(i - pageIdx);
	...
	public void onClick$cederEchantillonsButton() {
		String code = echantillonsBox.getValue(); //.toUpperCase();
		...
		if (code != null && !code.equals("")) {
			...
			if (codesEchans.contains(code)) {
				...
				Echantillon added = ManagerLocator.getEchantillonManager()
					.findByCodeLikeWithBanqueManager(
					code, selected, true).get(0);
				...
				throw new WrongValueException(echantillonsBox, Labels.getLabel(
				"cession.error.code.invalid.echantillon"));
			}
		} else {
			throw new WrongValueException(echantillonsBox, Labels.getLabel(
			"cession.error.empty.code.echantillon"));
		}
	}
	...
	public void onClick$cederMultiEchantillonsLabel() {
		EchantillonController.backToMe(
				getMainWindow(), page);
		...
		getEchantillonController().switchToSelectMode(
				Path.getPath(self), availableBanques);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void onGetEchantillonsFromSelection(Event e) {
		List<Echantillon> echansACeder = (List<Echantillon>) e.getData();
		// si une liste d'objets à céder est passée en paramètre
		if (echansACeder != null) {
			boolean nonStocke = false;
			for (int i = echansACeder.size() - 1; i >= 0 ; i--) {
				// on vérifie que l'élément renvoyé est bien un échantillon
				if (echansACeder.get(i).getClass()
						.getSimpleName().equals("Echantillon")) {
					Echantillon echan = (Echantillon) echansACeder.get(i);
	 ...
	 */
	public void cederEchantillon(Echantillon echantillonACeder) {
		
		if (echantillonACeder != null) {
			// création d'un nouveau CederObjet
			CederObjet obj = new CederObjet();
			obj.setCession(this.cession);
			obj.setEntite(ManagerLocator.getEntiteManager()
			...
			if (!echantillonsCedes.contains(obj)) {
				...
				CederObjetDecorator decorated =
						new CederObjetDecorator(obj, true, isAnonyme(), false);
...
//			}
			...
			throw new WrongValueException(echantillonsBox, 
			Labels.getLabel("cession.error.code.invalid.echantillon"));
		}
	}
	...
	public void onClick$cederDerivesButton() {
		String code = derivesBox.getValue().toUpperCase();
		...
		if (code != null && !code.equals("")) {
			...
			if (codesDerives.contains(code)) {
				...
				ProdDerive addedDerive = ManagerLocator.getProdDeriveManager()
				...
				throw new WrongValueException(derivesBox, Labels.getLabel(
				"cession.error.code.invalid.prodDerive"));
			}
		} else {
			throw new WrongValueException(derivesBox, Labels.getLabel(
			"cession.error.empty.code.prodDerive"));
		}
	}
	...
	public void onClick$cederMultiProdDerivesLabel() {
		ProdDeriveController.backToMe(
					getMainWindow(), page);
		
		// on bloque tous les panels sauf celui des échantillons
		getMainWindow().blockAllPanelsExceptOne("deriveTab");
		// on passe en mode de sélection
		getProdDeriveController().switchToSelectMode(
				Path.getPath(self), availableBanques);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void onGetDerivesFromSelection(Event e) {
		List<ProdDerive> derivesACeder = (List<ProdDerive>) e.getData();
		// si une liste d'objets à céder est passée en paramètre
		if (derivesACeder != null) {
			boolean nonStocke = false;
			for (int i = derivesACeder.size() - 1; i >= 0 ; i--) {
				// on vérifie que l'élément renvoyé est bien un dérivé
				if (derivesACeder.get(i).getClass()
	 ...
	 */
	public void cederProdDerive(ProdDerive deriveACeder) {
...
//			}
			...
			throw new WrongValueException(derivesBox, 
			Labels.getLabel("cession.error.code.invalid.prodDerive"));
		}
	}
	 ...
	 */
	public void onBlur$echantillonQuantiteBox(Event event) {
		// on récupère le decorator modifié
		CederObjetDecorator deco = (CederObjetDecorator) 
		...
		if (deco.getCederObjet().getQuantite() != null
				&& deco.getCederObjet().getQuantite() 
				<= deco.getQuantiteMax()) {
			Float res = deco.getQuantiteMax() 
				- deco.getCederObjet().getQuantite();
			// arrondi pour éviter les valeurs 20.00012
			deco.setQuantiteRestante(ObjectTypesFormatters
					.floor(res, 3));
		}
	}
	 ...
	 */
	public void onBlur$deriveQuantiteBox(Event event) {
		// on récupère le decorator modifié
		CederObjetDecorator deco = (CederObjetDecorator) 
			AbstractListeController2
				.getBindingData((ForwardEvent) event, false);
	...
	public void onSelect$deriveUnitesBox(Event event) {
		CederObjetDecorator deco = (CederObjetDecorator) 
				AbstractListeController2
					.getBindingData((ForwardEvent) event, false);
		
		deco.setSelectedUnite(deco.getUnites()
				.get(((Listbox) ((ForwardEvent) event).getOrigin().getTarget())
						.getSelectedIndex()));
	 ...
	 */
	public void onClick$deleteEchan(Event event) {
		
		// on demande confirmation à l'utilisateur
		// de supprimer l'échantillon
		if (Messagebox.show(ObjectTypesFormatters.getLabel(
				"message.cession.retrait", 
				new String[]{Labels
					.getLabel("message.deletion.echantillon")}),
...
//						codesEchans));
			...
			if (echantillonsCedesDecores.size() % 10 == 0
					&& _startPageNumberE > 0) {
				_startPageNumberE --;
			}
	 ...
	 */
	public void onClick$deleteDerive(Event event) {
		...
		if (Messagebox.show(ObjectTypesFormatters.getLabel(
				"message.cession.retrait", 
				new String[]{Labels
					.getLabel("message.deletion.derive")}),
		Labels.getLabel("message.deletion.title"), 
		Messagebox.YES | Messagebox.NO, 
		Messagebox.QUESTION) == Messagebox.YES) {
...
//				derivesListEdit.setModel(list);
			...
			if (derivesCedesDecores.size() % 10 == 0
					&& _startPageNumberP > 0) {
				_startPageNumberP --;
			}
	 ...
	 * seront recalculées.
	 */
	public void prepareCedesTKobjs(boolean calculerValeurs) {
		
		List<Retour> incompletes = new ArrayList<Retour>();
		List<Integer> ids = new ArrayList<Integer>();
		Entite e;

		List<CederObjetDecorator> cedeDecos = new ArrayList<CederObjetDecorator>();
		cedeDecos.addAll(echantillonsCedesDecores);
		cedeDecos.addAll(derivesCedesDecores);
		
		// pour chaque échantillon cédé
		for (CederObjetDecorator deco : cedeDecos) {
			
			TKStockableObject tkObj = deco.getTKobj();
...
//					}
				...
				if (tkObj instanceof ProdDerive) { 
					if (deco.getSelectedUnite() != null 
							&& deco.getSelectedUnite().getType().equals("volume")) {
						((ProdDerive) tkObj).setQuantite(
								calculerQuantiteRestante(
						((ProdDerive) tkObj), ((ProdDerive) tkObj).getVolume()));
					} else {
						((ProdDerive) tkObj).setVolume(
						calculerVolumeRestant(
						((ProdDerive) tkObj), ((ProdDerive) tkObj).getQuantite()));	
					} 
				}
			}
			...
			e = ManagerLocator.getEntiteManager()
					.findByNomManager(tkObj.entiteNom()).get(0);
			incompletes.clear();
			ids.clear(); ids.add(tkObj.listableObjectId());
			incompletes.addAll(ManagerLocator.getRetourManager()
					.findByObjectDateRetourEmptyManager(ids, e));
	...
	public void getRetourIncompleteFromModale(List<CederObjetDecorator> decos) {
		
		if (!cession.getCessionStatut().getStatut().equals("EN ATTENTE")) {
			
			Map<String,String> echansRetourIncompletes = 
					new HashMap<String,String>();
			Map<String,String> derivesRetourIncompletes = 
					new HashMap<String,String>();
			// seuls sont concernés les tkObjs en cession partielle 
			// avec un retour précédemment créé par une cession 
			// mise EN ATTENTE
			for (CederObjetDecorator deco : decos) {
				if (deco.getRetour() != null && deco.getRetour().getRetourId() != null
					&& deco.getTKobj().getObjetStatut() != null
					&& !deco.getTKobj().getObjetStatut().getStatut().equals("RESERVE")) {
					if (deco.getEchantillon() != null) { 
						echansRetourIncompletes.put(deco.getEchantillonCode(), 
							ObjectTypesFormatters.dateRenderer2(deco.getRetour()
													.getDateSortie()));
					} else {
			...
			if (!echansRetourIncompletes.isEmpty() || 
					!derivesRetourIncompletes.isEmpty()) {
				
				Calendar dateInit = cession.getDepartDate() != null ?
						cession.getDepartDate() : Calendar.getInstance();
				
				Clients.clearBusy();
				openDateRetourCompleteModale(echansRetourIncompletes,
						derivesRetourIncompletes,
						dateInit, this, self);
		...
		for (CederObjetDecorator deco : cedeDecos) {
			...
			if (deco.getNewStatut().getStatut().equals("EPUISE")) {
				...
				if (tkObj instanceof Echantillon) {
					empl = ManagerLocator.getEchantillonManager()
							.getEmplacementManager((Echantillon) tkObj);
				} else {
					empl = ManagerLocator.getProdDeriveManager()
							.getEmplacementManager((ProdDerive) tkObj);
				}
			...
			} else if (!cession.getCessionStatut().getStatut().equals("EN ATTENTE")
			...
			if (deco.getNewStatut() != null 
				&& (deco.getNewStatut().getStatut().equals("EPUISE") 
					|| deco.getNewStatut().getStatut().equals("DETRUIT"))) {
			...
			if (tkObj instanceof Echantillon) {
				Prelevement prlvt = ManagerLocator.getEchantillonManager()
					.getPrelevementManager((Echantillon) tkObj);
				ManagerLocator.getEchantillonManager().updateObjectManager(
						(Echantillon) tkObj, 
						((Echantillon) tkObj).getBanque(), 
						prlvt, 
						((Echantillon) tkObj).getCollaborateur(), 
						deco.getNewStatut(), 
						tkObj.getEmplacement(), 
						((Echantillon) tkObj).getEchantillonType(), 
						null, null,
						tkObj.getQuantiteUnite(), 
						((Echantillon) tkObj).getEchanQualite(), 
						((Echantillon) tkObj).getModePrepa(), 
						// ((Echantillon) tkObj).getCrAnapath(), null,
						((Echantillon) tkObj).getReservation(),
						null, null, null, null,
						SessionUtils.getLoggedUser(sessionScope),
						false, ops, null);
				...
				ManagerLocator.getProdDeriveManager().updateObjectManager(
						((ProdDerive) tkObj), 
						tkObj.getBanque(), 
						((ProdDerive) tkObj).getProdType(), 
						deco.getNewStatut(), 
						((ProdDerive) tkObj).getCollaborateur(), 
						tkObj.getEmplacement(), 
						((ProdDerive) tkObj).getVolumeUnite(), 
						((ProdDerive) tkObj).getConcUnite(), 
						((ProdDerive) tkObj).getQuantiteUnite(), 
						((ProdDerive) tkObj).getModePrepaDerive(),
						((ProdDerive) tkObj).getProdQualite(), 
						((ProdDerive) tkObj).getTransformation(), 
						null, null, null, null, 
						((ProdDerive) tkObj).getReservation(), 
						SessionUtils.getLoggedUser(sessionScope),
						false, ops, null);
			...
			if (!newlyCeds.contains(tkObj) 
					&& !tkObj
						.getObjetStatut().getStatut().equals("EPUISE")
					&& !tkObj
						.getObjetStatut().getStatut().equals("RESERVE")
					&& (deco.getRetour() == null || deco.getRetour().getRetourId() == null)) {
				newlyCeds.add(tkObj);
			...
			if (getMainWindow().isFullfilledComponent(oPanel, oWin)) {
				if (controller != null && controller.getListe() != null) {	
					// update de l'échantillon dans la liste
					controller.getListe()
						.updateObjectGridListFromOtherPage(tkObj, false);
				}
			}
		}		
	}
...
//	}
	...
	public void onBlur$dateDemandeBox() {
		...
		if (dateDemandeBox.getErrorMessage() != null
				&& dateDemandeBox.getErrorMessage().contains(
						dateDemandeBox.getFormat())) {
			badDateFormat = true;
		}
		if (!badDateFormat) {
			dateDemandeBox.clearErrorMessage(true);
			validateCoherenceDate(dateDemandeBox, 
					dateDemandeBox.getValue());
		}
	}
	...
	public void onBlur$dateValidationBox() {
		...
		if (dateValidationBox.getErrorMessage() != null
				&& dateValidationBox.getErrorMessage().contains(
						dateValidationBox.getFormat())) {
			badDateFormat = true;
		}
		if (!badDateFormat) {
			dateValidationBox.clearErrorMessage(true);
			validateCoherenceDate(dateValidationBox, 
					dateValidationBox.getValue());
		}
	}
	
	public void onBlur$dateDepartCalBox() {
		Datebox box = (Datebox) dateDepartCalBox
		...
		if (!badDateFormat) {
			dateDepartCalBox.clearErrorMessage(
	...
	public void onBlur$dateArriveeCalBox() {
		Datebox box = (Datebox) dateArriveeCalBox
			.getFirstChild().getFirstChild();
		boolean badDateFormat = false;
		if (box.getErrorMessage() != null
				&& box.getErrorMessage().contains(
		...
		if (!badDateFormat) {
			dateArriveeCalBox.clearErrorMessage(
	...
	public void onBlur$dateDestructionCalBox() {
		Datebox box = (Datebox) dateDestructionCalBox
			.getFirstChild().getFirstChild();
		boolean badDateFormat = false;
		if (box.getErrorMessage() != null
				&& box.getErrorMessage().contains(
		...
		if (!badDateFormat) {
			dateDestructionCalBox.clearErrorMessage(
					dateDestructionCalBox.getValue());
			validateCoherenceDate(dateDestructionCalBox, 
					dateDestructionCalBox.getValue());
		} else {
			throw new WrongValueException(
					dateDestructionCalBox, 
					Labels.getLabel("validation.invalid.date"));
		}
	}
...
//		
	 ...
	 * @throws Exception
	 */
	public void onSelect$contratsComboBox(Event event) throws Exception {
		if (contratsComboBox.getSelectedItem() != null) {
			String value = contratsComboBox.getSelectedItem().getLabel();
			
			if (contratsNoms.contains(value)) {
				selectedContrat = contrats.get(contratsNoms.indexOf(
					value));
			} else {
				selectedContrat = null;
			}
	 ...
	 * @throws Exception
	 */
	public void onBlur$contratsComboBox(Event event) throws Exception {
		if (contratsComboBox.getSelectedItem() != null) {
			String value = contratsComboBox.getSelectedItem().getLabel();
			...
			if (contratsNoms.contains(contratsComboBox.getValue()
						.toUpperCase())) {
				selectedContrat = contrats.get(contratsNoms.indexOf(
						contratsComboBox.getValue().toUpperCase()));
			} else {
				selectedContrat = null;
			}
		} else {
	...
	public boolean getEchantillonsListSizeSupOne() {
		if (getDroitsConsultation().containsKey("Echantillon")
				...
				&& this.cession.getCessionStatut() != null
				&& !this.cession.getCessionStatut()
				.getStatut().equals("EN ATTENTE")) {
	...
	public String getDateDemandeFormated() {
		if (this.cession != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.cession.getDemandeDate());
		} else {
			return null;
		}
	}
	...
	public void setSelectedCessionType(CessionType selected) {
	...
	public void setExamens(List<CessionExamen> exams) {
		this.examens = exams;
	}
	...
	public void setSelectedCessionExamen(CessionExamen selected) {
	...
	public void setSelectedContrat(Contrat selected) {
	...
	public void setSelectedCessionStatut(CessionStatut selected) {
	...
	public void setSelectedTransporteur(Transporteur selected) {
	...
	public void setSelectedDestructionMotif(
			DestructionMotif selected) {
	...
	public void setSelectedDemandeur(Collaborateur selected) {
	...
	public void setSelectedDestinataire(Collaborateur selected) {
	...
	public void setSelectedExecutant(Collaborateur selected) {
	...
	public void setNomsService(List<String> nomsS) {
	...
	public String getDateValidationFormated() {
		if (this.cession != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.cession.getValidationDate());
		} else {
			return null;
		}
	}
	...
	public String getDateDemandeContratFormated() {		
		if (this.selectedContrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.selectedContrat
	...
	public String getDateSignatureContratFormated() {
		if (this.selectedContrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.selectedContrat.getDateSignature());
		} else {
			return null;
		}
	}
	...
	public String getDateValidationContratFormated() {
		if (this.selectedContrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.selectedContrat.getDateValidation());
		} else {
			return null;
		}
	}
	...
	public class ConstQuantiteDemandee implements Constraint {
		...
		public void validate(Component comp, Object value) {
			...
			CederObjetDecorator deco = 
					(CederObjetDecorator) getBindingData(comp);
			
			if (quantiteValue != null) {
				Float qteDemandee = quantiteValue.floatValue();
				...
				if (qteDemandee >= zero) {
					...
					throw new WrongValueException(
							...
							"cession.error.ceder.quantite.negative"));
				}
			} else {
				if (deco.getTKobj().getQuantite() != null 
						|| (deco.getTKobj() instanceof ProdDerive 
								&& deco.getProdDerive().getVolume() != null)) {
	...
	public void setCttQuantiteDemandee(Constraint ctt) {
	...
	public String getSClassDemandeur() {
		if (this.cession != null) {
			return ObjectTypesFormatters
				.sClassCollaborateur(this.cession.getDemandeur());
		} else {
			return "";
		}
	}
	...
	public String getSClassDestinataire() {
		if (this.cession != null) {
			return ObjectTypesFormatters
				.sClassCollaborateur(this.cession.getDestinataire());
		} else {
			return "";
		}
	}
	...
	public String getSClassExecutant() {
		if (this.cession != null) {
			return ObjectTypesFormatters
				.sClassCollaborateur(this.cession.getExecutant());
		} else {
			return "";
		}
	}
	...
	public String getSClassServiceDest() {
		if (this.cession != null) {
			return ObjectTypesFormatters
				.sClassService(this.cession.getServiceDest());
		} else {
			return "";
		}
	}
	...
	public String getSClassCollaborateurHelp() {
		if (this.selectedContrat != null) {
			return ObjectTypesFormatters
				.sClassCollaborateur(this
					.selectedContrat.getCollaborateur());
		} else {
			return "";
		}
	}
	
	public void setVisibleRow(Row row, boolean visible) {
		for (int i = 0; i < row.getChildren().size(); i++) {
			Component comp = (Component) row.getChildren().get(i);
	...
	public void setTerminaleDestockage(Terminale terminale) {
		this.terminaleDestockage = terminale;
	}
	...
	public void setAvailableBanques(List<Banque> aBanques) {
	...
	public void setTemperatures(List<Temperature> t) {
		this.temperatures = t;
	}
	...
	public void setSelectedTemperature(Temperature selected) {
	...
	public void setDestinataires(List<Collaborateur> d) {
	...
	public void setServicesDest(List<Service> s) {
		this.servicesDest = s;
	}
	...
	public void setEtablissements(List<Etablissement> e) {
	...
	public void setContratsNoms(List<String> contratsNs) {
	...
	private void refreshModelE(int activePage){
		echansPaging.setPageSize(_pageSizeE);
        echansModel = new ObjectPagingModel(activePage, _pageSizeE, 
        				echantillonsCedesDecores, null, null);
        ...
        int start = _startPageNumberE * _pageSizeE;
	...
	public void onPaging$echansPaging(ForwardEvent event){
        final PagingEvent pe = (PagingEvent) event.getOrigin();
        _startPageNumberE = pe.getActivePage();
        refreshModelE(_startPageNumberE);
    }
	
	private void refreshModelP(int activePage){
		derivesPaging.setPageSize(_pageSizeP);
		derivesModel = new ObjectPagingModel(activePage, _pageSizeP, 
				derivesCedesDecores, null, null);
	...
	public void onPaging$derivesPaging(ForwardEvent event){
        final PagingEvent pe = (PagingEvent) event.getOrigin();
	...
	public void setRetoursToComplete(List<Retour> rc) {
		this.retoursToComplete = rc;
	}

	public void setDateRetourEchansComplete(Calendar dec) {
		this.dateRetourEchansComplete = dec;
	}

	public void setDateRetourDerivesComplete(Calendar drc) {
	 ...
	 * @param ce Exception
	 */
	public void setCancelException(CancelFromModalException ce) {
		cancelException = ce;
	}
