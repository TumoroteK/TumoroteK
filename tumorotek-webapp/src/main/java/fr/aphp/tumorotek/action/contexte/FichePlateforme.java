...
public class FichePlateforme extends AbstractFicheCombineController
{
   ...
   // TK-252
   private final List<Utilisateur> superadmins = new ArrayList<>();
   ...
   public void doAfterCompose(final Component comp) throws Exception{
      ...
      superadmins.addAll(ManagerLocator.getUtilisateurManager().findBySuperAndArchiveManager(false, true));
   ...
   public void switchToCreateMode(){
      ...
      if(!SessionUtils.getLoggedUser(sessionScope).isSuperAdmin()){
         administrateurs.add((SessionUtils.getLoggedUser(sessionScope)));
   ...
   public void removeObject(final String comments){
      ManagerLocator.getPlateformeManager().removeObjectManager(getObject(), comments, SessionUtils.getLoggedUser(sessionScope),
      ...
      if(administrateurs.contains(SessionUtils.getLoggedUser(sessionScope))){
         sessionScope.put("User",
            ManagerLocator.getUtilisateurManager().findByIdManager(SessionUtils.getLoggedUser(sessionScope).getUtilisateurId()));
      }
   }
   ...
   public void createNewObject(){
      ...
      plateforme = ManagerLocator.getPlateformeManager().createObjectManager(plateforme, selectedCollaborateur, administrateurs,
   ...
   public void initEditableMode(){
      ...
      final List<Plateforme> pfs =
         ManagerLocator.getUtilisateurManager().getAvailablePlateformesManager(SessionUtils.getLoggedUser(sessionScope));
      if(!pfs.isEmpty()){
   ...
   public List<Utilisateur> getAdministrateurs(){
      final List<Utilisateur> admins = new ArrayList<>();
