...
public class ProdDeriveController extends AbstractObjectTabController
{
   ...
   public void doAfterCompose(final Component comp) throws Exception{
      ...
      setEntiteTab(ManagerLocator.getEntiteManager().findByNomManager("ProdDerive").get(0));
   ...
   public FicheMultiProdDerive getMultiFicheEdit(){
      return ((FicheMultiProdDerive) self.getFellow("divProdDeriveEdit").getFellow("fwinMultiProdDerive")
   ...
   public ListeProdDerive getListe(){
      return ((ListeProdDerive) this.self.getFellow("listeProdDerive").getFellow("lwinProdDerive")
         .getAttributeOrFellow("lwinProdDerive$composer", true));
   }

   @Override
   public AbstractFicheModifMultiController getFicheModifMulti(){
      return ((FicheModifMultiProdDerive) self.getFellow("modifMultiDiv").getFellow("fwinModifMultiProdDerive")
   ...
   public void switchToCreateMode(final TKdataObject parent){
      ...
      Executions.createComponents(createZulPath, divProdDeriveEdit, null);
   ...
   public FicheMultiProdDerive getFicheMultiProdDerive(){
      return (FicheMultiProdDerive) this.self.getFellow("divProdDeriveEdit").getFellow("fwinMultiProdDerive")
         .getAttributeOrFellow("fwinMultiProdDerive$composer", true);
   }
   ...
   public static AbstractObjectTabController backToMe(final MainWindow window, final Page page){
      ...
      if(panels != null){
         ...
         final Tabpanel panel = (Tabpanel) panels.getFellow("derivePanel");

         window.createMacroComponent("/zuls/prodderive/ProdDerive.zul", "winProdDerive", panel);

         tabController =
            ((ProdDeriveController) panel.getFellow("winProdDerive").getAttributeOrFellow("winProdDerive$composer", true));
   ...
   public List<? extends Object> getChildrenObjects(final TKdataObject obj){
      ...
      childrens.addAll(ManagerLocator.getProdDeriveManager().getProdDerivesManager((ProdDerive) obj));
   ...
   public Map<Entite, List<Integer>> getParentsObjectsIds(final List<Integer> ids){
      ...
      parents.put(echanEntite, ManagerLocator.getCorrespondanceIdManager().findTargetIdsFromIdsManager(ids, getEntiteTab(),
         echanEntite, SessionUtils.getSelectedBanques(sessionScope), true));

      // derives
      final Entite deriveEntite = ManagerLocator.getEntiteManager().findByNomManager("ProdDerive").get(0);
      parents.put(deriveEntite, ManagerLocator.getCorrespondanceIdManager().findTargetIdsFromIdsManager(ids, getEntiteTab(),
         deriveEntite, SessionUtils.getSelectedBanques(sessionScope), false));
