...
public class ListePrelevement extends AbstractListeController2 {
	...
	public List<Integer> doFindObjects() {
		...
		if (dateCreation.isChecked()) {
			prelevements = ManagerLocator.getPrelevementManager().findAfterDateCreationReturnIdsManager(
					getSearchDateCreation(), SessionUtils.getSelectedBanques(sessionScope));
			...
			if (!searchCode.equals("")) {
				if (searchCode.contains(",")) {
					...
					prelevements = ManagerLocator.getPrelevementManager()
							.findByCodeOrNumLaboLikeBothSideWithBanqueReturnIdsManager(searchCode,
									SessionUtils.getSelectedBanques(sessionScope), true);
				}
			} else {
				if (Messagebox.show(
						ObjectTypesFormatters.getLabel("message.research.message", new String[] { "Prelevement" }),
						Labels.getLabel("message.research.title"), Messagebox.YES | Messagebox.NO,
	...
	public void onClick$findByListCodes() {
		...
		final List<Integer> prelevements = ManagerLocator.getPrelevementManager()
				.findByCodeOrNumLaboInListManager(codes, SessionUtils.getSelectedBanques(sessionScope));
	...
	public void onClick$findByListPatients() {
		...
		final List<Integer> prelevements = ManagerLocator.getPrelevementManager()
				.findByPatientNomOrNipInListManager(pats, SessionUtils.getSelectedBanques(sessionScope));
		// affichage de ces résultats
		showResultsAfterSearchByList(prelevements);
	}
	...
	public void onClickMaladie(final Event event) {
		...
		if (tabController != null) {
			...
			((FichePatientStatic) tabController.getFicheStatic())
					.openMaladiePanel(((Prelevement) getCurrentObject()).getMaladie());
		}
	}
	...
	private static HtmlMacroComponent populateChangeCollectionModal(final Prelevement[] prlvts, final Window win,
			final Page page, final MainWindow main) {
		...
		((ChangeBanqueModale) ua.getFellow("fwinChangeBanqueModale")
				.getAttributeOrFellow("fwinChangeBanqueModale$composer", true)).init(prlvts, main);
