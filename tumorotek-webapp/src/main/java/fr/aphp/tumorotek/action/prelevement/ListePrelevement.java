...
public class ListePrelevement extends AbstractListeController2 {
	...
	private Log log = LogFactory.getLog(ListePrelevement.class);

	private static final long serialVersionUID = -4444512949273177133L;

	private List<Prelevement> listObjects = new ArrayList<Prelevement>();
	private List<Prelevement> selectedObjects = new ArrayList<Prelevement>();
	...
	private PrelevementRowRenderer listObjectsRenderer = 
			new PrelevementRowRenderer(true, false);
	private PrelevementsNbEchantillonsComparator comparatorAsc = 
			new PrelevementsNbEchantillonsComparator(true);
	private PrelevementsNbEchantillonsComparator comparatorDesc = 
			new PrelevementsNbEchantillonsComparator(false);
	...
	public void setSearchNomPatient(String searchN) {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		listObjectsRenderer.setEmetteurs(SessionUtils
				.getEmetteursInterfacages(sessionScope));
	}

	@Override
	public void setListObjectsRenderer(
			TKSelectObjectRenderer listObjectsRenderer) {
	...
	public void removeObjectFromList(TKdataObject obj) {
		getListObjects().remove((Prelevement) obj);
	}
	...
	public void removeFromSelectedObjects(TKdataObject obj) {
		if (getSelectedObjects().contains(obj)) {
			getSelectedObjects().remove((Prelevement) obj);
		}
	}
	...
	public void initObjectsBox() {
		...
		List<Prelevement> prlvts = ManagerLocator.getPrelevementManager()
	...
	public List<? extends TKdataObject> extractLastObjectsCreated() {
		return ManagerLocator.getPrelevementManager().findLastCreationManager(
	...
	public List<Integer> doFindObjects() {
		...
		List<Integer> prelevements = new ArrayList<Integer>();
		...
		if (dateCreation.isChecked()) {
			...
			if (!searchCode.equals("")) {
				if (searchCode.contains(",")) {
					...
					prelevements = ManagerLocator.getPrelevementManager()
							.findByCodeOrNumLaboInListManager(
									codes,
									SessionUtils
											.getSelectedBanques(sessionScope));
				} else {
					prelevements = ManagerLocator
			...
			if (searchNomPatient.contains(",")) {
				List<String> pats = ObjectTypesFormatters
						.formateStringToList(searchNomPatient);
				prelevements = ManagerLocator.getPrelevementManager()
						.findByPatientNomOrNipInListManager(pats,
								SessionUtils.getSelectedBanques(sessionScope));
			} else {
				prelevements = ManagerLocator.getPrelevementManager()
						.findByPatientNomReturnIdsManager(searchNomPatient,
	...
	public void onClick$findByListCodes() {
		...
		List<Integer> prelevements = ManagerLocator.getPrelevementManager()
				.findByCodeOrNumLaboInListManager(codes,
						SessionUtils.getSelectedBanques(sessionScope));
	 ...
	 *            l'objet Prelevement).
	 */
	public void onClickPatient(Event event) {
		...
		PatientController tabController = (PatientController) PatientController
				.backToMe(getMainWindow(), page);

		if (tabController != null) {
			tabController
					.switchToFicheStaticMode(((Prelevement) getCurrentObject())
							.getMaladie().getPatient());
		}
	}
	 ...
	 *            l'objet Prelevement).
	 */
	public void onClickMaladie(Event event) {
		...
		if (tabController != null) {
			tabController
					.switchToFicheStaticMode(((Prelevement) getCurrentObject())
							.getMaladie().getPatient());
			((FichePatientStatic) tabController.getFicheStatic())
					.openMaladiePanel(((Prelevement) getCurrentObject())
							.getMaladie());
		}
	}
	...
	public void colorateGrid() {
		...
		for (int i = 0; i < listObjects.size(); i++) {
			Prelevement prel = (Prelevement) listObjects.get(i);
			if (PrelevementUtils.getNbEchanRestants(prel) == 0) {
				Row row = (Row) comps.get(i);
	...
	public void deselectRow() {
		...
		if (getCurrentObject() != null && getCurrentRow() != null) {
			...
			if (ind > -1) {
				...
				if (PrelevementUtils
						.getNbEchanRestants((Prelevement) getCurrentObject()) == 0) {
					getCurrentRow().setStyle("background-color : #FEBAB3");
				} else if (PrelevementUtils
						.getNbEchanRestants((Prelevement) getCurrentObject()) == 1) {
	...
	public void onClick$changeCollectionItem() {
		Prelevement[] prlvts = getSelectedObjects().toArray(
				new Prelevement[getSelectedObjects().size()]);
	 ...
	 */
	public void openChangeModaleWindow(Prelevement[] prlvts) {
		if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua = populateChangeCollectionModal(prlvts,
					win, page, getMainWindow());
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateChangeCollectionModal(
			Prelevement[] prlvts, Window win, Page page, MainWindow main) {
		HtmlMacroComponent ua;
		ua = (HtmlMacroComponent) page.getComponentDefinition(
				"changeBanqueModale", false).newInstance(page, null);
		ua.setParent(win);
	...
	public void onLaterFindDossiers() {
		listObjects = new ArrayList<Prelevement>();
		clearSelection();
		setListObjects(ManagerLocator.getPrelevementManager()
				.findByDossierExternesManager(
						SessionUtils.getSelectedBanques(sessionScope),
	 ...
	 * 
	 */	
	public void onClickDossierExt(Event event) {
		if (getObjectTabController() != null) {
			getObjectTabController()
				.switchToFicheStaticMode((Prelevement) event.getData());
	...
	public void setCanAccessPatient(boolean canAccess) {
		this.canAccessPatient = canAccess;
	}

	public boolean getBanqueDefMaladies() {
		if (sessionScope.containsKey("Banque")) {
			return SessionUtils.getSelectedBanques(sessionScope).get(0)
					.getDefMaladies();
		}
		return true;
	}
	...
	public void setComparatorAsc(PrelevementsNbEchantillonsComparator c) {
	...
	public void setComparatorDesc(PrelevementsNbEchantillonsComparator c) {
		this.comparatorDesc = c;
	}
	
	@Override
	public void batchDelete(List<Integer> ids, String comment) {
		ManagerLocator.getPrelevementManager().removeListFromIdsManager(ids, comment, 
