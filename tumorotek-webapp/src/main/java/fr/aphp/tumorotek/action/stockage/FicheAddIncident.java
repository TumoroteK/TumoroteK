...
public class FicheAddIncident extends AbstractFicheCombineController {
	...
	public void init(Conteneur cont, Component par, Enceinte enc, Terminale term) {
		this.conteneur = cont;
		this.enceinte = enc;
		this.terminale = term;
		this.parent = par;
		...
		try {
			...
			List<Terminale> terminales = null;
			 if (conteneur != null) {
				...
				terminales = new ArrayList<Terminale>();
				terminales.add(terminale);
			}
			for (int i = 0; i < terminales.size(); i++) {
				objs.addAll(ManagerLocator.getTerminaleManager()
						.getEchantillonsManager(terminales.get(i)));
				objs.addAll(ManagerLocator.getTerminaleManager()
						.getProdDerivesManager(terminales.get(i)));
			}
						
			// s'il y a des objets dans le conteneur
			if (!getObjStatutIncompatibleForRetour(objs, "INCIDENT") ) {
				// sauvegarde
				ManagerLocator.getIncidentManager()
								.createObjectManager(incident, conteneur, enceinte, terminale);
				// envoie de l'utilisateur modifié
				Events.postEvent("onGetAddedIncident", 
						getParent(), incident);
		 
				// on demande à l'utilisateur s'il souhaite
				// créer des retours
				if (objs.size() > 0  && askForRetoursCreation(objs.size(), null, null, null, 
						incident, ManagerLocator.getOperationTypeManager()
...
//					}
					...
					openRetourFormModale(null, true, null, null, 
						objs, null,
						null, null, incident, incident.getDate(), 
						incident.getEnceinte() != null ?
							Labels.getLabel("ficheRetour.observations.incident.enceinte") : 
							incident.getTerminale() != null ?
								Labels.getLabel("ficheRetour.observations.incident.terminale") : 
