...
public class FicheTransporteur extends AbstractFicheCombineController
{
   ...
   public void doAfterCompose(final Component comp) throws Exception{
      ...
      setObjLabelsComponents(new Component[] {this.nomLabel, this.adresseLabel, this.cpLabel, this.villeLabel, this.paysLabel,
         this.telLabel, this.faxLabel, this.mailLabel, this.contactNomLabel, this.contactPrenomLabel, this.contactTelLabel,
         this.contactFaxLabel, this.contactMailLabel, this.menuBar, this.archiveLabel});

      setObjBoxsComponents(new Component[] {this.nomRequired, this.contactNomRequired});

      setRequiredMarks(new Component[] {this.nomBox, this.adresseBox, this.cpBox, this.villeBox, this.paysBox, this.telBox,
         this.faxBox, this.mailBox, this.contactNomBox, this.contactPrenomBox, this.contactTelBox, this.contactFaxBox,
         this.contactMailBox, this.archiveBox});
   ...
   public void setObject(final TKdataObject obj){
      ...
      if(transporteur != null && transporteur.getTransporteurId() != null){
   ...
   public void switchToStaticMode(){
      super.switchToStaticMode(this.transporteur.equals(new Transporteur()));
   ...
   public void onClick$createC(){
      Clients.showBusy(Labels.getLabel("transporteur.creation.encours"));
   ...
   public void createNewObject(){
      ...
      // update de l'objet
      ManagerLocator.getTransporteurManager().createObjectManager(transporteur, coordonnee,
   ...
   public void updateObject(){
      ...
      // update de l'objet
      ManagerLocator.getTransporteurManager().updateObjectManager(transporteur, coordonnee,
         SessionUtils.getLoggedUser(sessionScope));
   ...
   public boolean prepareDeleteObject(){
      final boolean isReferenced = ManagerLocator.getTransporteurManager().isReferencedObjectManager(getObject());
      setDeleteMessage(
         ObjectTypesFormatters.getLabel("message.deletion.message", new String[] {Labels.getLabel(getDeletionMessage())}));
      setDeletable(true);
      if(isReferenced){
         setDeleteMessage(Labels.getLabel("transporteur.deletion.isReferenced"));
         if(getObject().getArchive()){ // deja archivé
            setDeletable(false);
            setDeleteMessage(Labels.getLabel("transporteur.deletion.isAlreadyArchived"));
         }
      }
   ...
   public void removeObject(final String comments){
      if(isFantomable()){ // suppression possible
         ManagerLocator.getTransporteurManager().removeObjectManager(getObject(), comments,
            SessionUtils.getLoggedUser(sessionScope));
      }else{ // archivage
         getObject().setArchive(true);
         ManagerLocator.getTransporteurManager().updateObjectManager(getObject(), getObject().getCoordonnee(),
            SessionUtils.getLoggedUser(sessionScope));
      }
   }
   ...
   public void onLaterDelete(final Event event){
      try{
         ...
         if(isFantomable()){
            if(getListeTransporteur() != null){
               getListeTransporteur().removeObjectAndUpdateList(getObject());
            }
            // clear form
            clearData();
         }else{
            if(getListeTransporteur() != null){
               // update du transporteur dans la liste
               getListeTransporteur().updateObjectGridList(getObject());
            }
         }
   ...
   public void setFieldsToUpperCase(){
      if(this.transporteur.getNom() != null){
         this.transporteur.setNom(this.transporteur.getNom().toUpperCase().trim());
      }
   ...
   public void setTransporteur(final Transporteur t){
      this.transporteur = t;
   }
