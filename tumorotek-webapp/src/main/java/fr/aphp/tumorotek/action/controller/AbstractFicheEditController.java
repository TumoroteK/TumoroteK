 ...
 *
 */
public abstract class AbstractFicheEditController 
	...
	public void setWaitLabel(String w) {
		this.waitLabel = w;
	}
...
//			}
	...
	public void updateObjectWithAnnots() {
		...
		getObjectTabController()
			.getFicheAnnotation().clearValeursLists(false);
		
		// prepare les listes de valeurs à manipuler
		getObjectTabController()
			.getFicheAnnotation().populateValeursActionLists(false, false);
	...
	public void setNumerotationButtonVisible() {
		if (getObjectTabController() != null
			...
			if (ManagerLocator.getNumerotationManager()
					.findByBanqueAndEntiteManager(
							SessionUtils.getSelectedBanques(
									sessionScope).get(0), 
							getObjectTabController().getEntiteTab())
								.size() > 0) {
				currentNumerotation = ManagerLocator.getNumerotationManager()
						.findByBanqueAndEntiteManager(
						SessionUtils.getSelectedBanques(
								sessionScope).get(0), 
						getObjectTabController().getEntiteTab()).get(0);
	...
	public String generateCodeAndUpdateNumerotation() {
		if (currentNumerotation != null) {
			currentNumerotation.setCurrentIncrement(
					currentNumerotation.getCurrentIncrement() + 1);
			// ManagerLocator.getNumerotationManager().updateObjectManager(
			// 		currentNumerotation, 
			//		currentNumerotation.getBanque(), 
			//		currentNumerotation.getEntite());
			return ManagerLocator.getNumerotationManager()
				.getGeneratedCodeManager(currentNumerotation);		
		} else {
			return null;
		}
	}
	...
	public void setCurrentNumerotation(Numerotation cNumerotation) {
	 ...
	 * concernés.
	 */
	public void postStorageData(Map<TKStockableObject, Emplacement> tkEmpls) {
		if (tkEmpls.size() > 0) {
			try {
				// Recepteurs
