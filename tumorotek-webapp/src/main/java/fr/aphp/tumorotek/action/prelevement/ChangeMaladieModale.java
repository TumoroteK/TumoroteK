...
public class ChangeMaladieModale extends GenericForwardComposer<Component>
{
   ...
   public void init(final Prelevement prel, final MainWindow mw){
      prelevement = prel;
      main = mw;
      ...
      if(currentMaladie != null && currentMaladie.getPatient() != null){
         ...
         patientDdnLabel.setValue(ObjectTypesFormatters.dateRenderer2(patient.getDateNaissance()));
   ...
   public void onClick$validate(){
      Clients.showBusy(Labels.getLabel("fichePrelevement.switchMaladie.encours"));
   ...
   public void onLaterUpdate(){
      ...
      if(maladiesBox.getSelectedItem() != null){
         ...
         // ferme wait message
         Clients.clearBusy();
         throw new WrongValueException(maladiesBox, Labels.getLabel("fichePrelevement.switchMaladie" + ".maladiesCibles.error"));
      }

      ManagerLocator.getPrelevementManager().switchMaladieManager(prelevement, dest, SessionUtils.getLoggedUser(sessionScope));

      // fermeture de la fenêtre
      Events.postEvent(new Event("onClose", self.getRoot()));

      // MAJ des fiches et des listes
      getPrelevementController().getListe().updateObjectGridListFromOtherPage(prelevement, true);
      if(!getPrelevementController().getReferencingObjectControllers().isEmpty()){
         getPrelevementController().getReferencingObjectControllers().get(0).getListe()
            .updateObjectGridListFromOtherPage(prelevement.getMaladie().getPatient(), true);
      }
   ...
   public PrelevementController getPrelevementController(){
      return (PrelevementController) main.getMainTabbox().getTabpanels().getFellow("prelevementPanel").getFellow("winPrelevement")
