...
public class FicheModeleDynModale extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(FicheModeleDynModale.class);
	...
	private List<ModeleType> modeleTypes = new ArrayList<ModeleType>();
	private ModeleType selectedModeleType;
	private List<LigneEtiquette> ligneEtiquettes = new ArrayList<LigneEtiquette>();
	private List<LigneEtiquette> ligneEtiquettesToRemove = new ArrayList<LigneEtiquette>();
	private ListModelList<LigneEtiquetteDecorator> ligneEtiquettesDeco = 
					new ListModelList<LigneEtiquetteDecorator>();
...
//	private static final String QR_ACTIVATE = "oui";
	 ...
	 * @param comp
	 */
	public void initImprimanteModale(Modele mod, Component comp) {
		setParent(comp);
		setObject(mod);
	...
	public void onClick$createC() {
		...
		if (selectedModeleType == null) {
			throw new WrongValueException(modeleTypesBox,
					Labels.getLabel("fiche.modele.error.modeleType"));
		}
	...
	public void createNewObject() {
		...
		ligneEtiquettes = new ArrayList<LigneEtiquette>();
		// hashtable contenant les champs à créer pour chaque ligne
		Hashtable<LigneEtiquette, List<ChampLigneEtiquette>> champsForLignes = new Hashtable<LigneEtiquette, List<ChampLigneEtiquette>>();
		...
		for (int i = 0; i < ligneEtiquettesDeco.size(); i++) {
			...
			champsForLignes.put(ligneEtiquettesDeco.get(i).getLigneEtiquette(),
					ligneEtiquettesDeco.get(i).getChamps());
		}

		// update de l'objet
		ManagerLocator.getModeleManager().createObjectManager(modele,
				SessionUtils.getPlateforme(sessionScope), selectedModeleType,
	...
	public void updateObject() {
		...
		Hashtable<LigneEtiquette, List<ChampLigneEtiquette>> champsToRemoveForLignes = new Hashtable<LigneEtiquette, List<ChampLigneEtiquette>>();
		// pour chaque decorator de ligne, on va ajouter la ligne à la
		// liste et ses champs à la hashtable
		for (int i = 0; i < ligneEtiquettesDeco.size(); i++) {
			ligneEtiquettes.add(ligneEtiquettesDeco.get(i).getLigneEtiquette());
			...
			if (ligneEtiquettesDeco.get(i).getChampsToRemove() != null
					&& ligneEtiquettesDeco.get(i).getChampsToRemove().size() > 0) {
				champsToRemoveForLignes.put(ligneEtiquettesDeco.get(i)
						.getLigneEtiquette(), ligneEtiquettesDeco.get(i)
						.getChampsToRemove());
			}
		}

		// update de l'objet
		ManagerLocator.getModeleManager().updateObjectManager(modele,
				SessionUtils.getPlateforme(sessionScope), selectedModeleType,
				ligneEtiquettes, ligneEtiquettesToRemove, champsForLignes,
				champsToRemoveForLignes);
	}
	...
	public void initEditableMode() {
		...
		modeleTypes = ManagerLocator.getModeleTypeManager()
		...
		if (this.modele.getModeleId() != null) {
			...
			ligneEtiquettes = ManagerLocator.getLigneEtiquetteManager()
					.findByModeleManager(modele);
			ligneEtiquettesDeco.addAll(LigneEtiquetteDecorator
					.decorateListe(ligneEtiquettes));
		} else {
	 ...
	 * @param event
	 */
	public void onClickEditLigne(ForwardEvent event) {
		if (event.getData() != null) {
			LigneEtiquetteDecorator deco = (LigneEtiquetteDecorator) event
					.getData();
		...
		if (event.getData() != null) {
			...
			if (Messagebox.show(ObjectTypesFormatters.getLabel(
					"message.deletion.message", new String[] { Labels
							.getLabel("message.deletion.ligneEtiquette") }),
					Labels.getLabel("message.deletion.title"), Messagebox.YES
				...
				int idx = ligneEtiquettesDeco.indexOf(deco);
				for (int i = idx + 1; i < ligneEtiquettesDeco.size(); i++) {
					ligneEtiquettesDeco
							.get(i)
							.getLigneEtiquette()
							.setOrdre(
									ligneEtiquettesDeco.get(i)
											.getLigneEtiquette().getOrdre() - 1);
				}
				ligneEtiquettesDeco.remove(deco);
				ligneEtiquettes.remove(le);
				ListModel<LigneEtiquetteDecorator> list = 
						new ListModelList<LigneEtiquetteDecorator>(ligneEtiquettesDeco);
	...
	public void onClickDownLigne(ForwardEvent event) {
		if (event.getData() != null) {
			LigneEtiquetteDecorator deco = (LigneEtiquetteDecorator) event
					.getData();
			int tabIndex = ligneEtiquettesDeco.indexOf(deco);
	 ...
	 *            a monter d'un cran
	 */
	private void upObject(LigneEtiquetteDecorator obj) {
		int tabIndex = ligneEtiquettesDeco.indexOf(obj);
		LigneEtiquetteDecorator supObjectInList = null;
		if (tabIndex - 1 > -1) {
			supObjectInList = ligneEtiquettesDeco.get(tabIndex - 1);
			supObjectInList.getLigneEtiquette().setOrdre(
					supObjectInList.getLigneEtiquette().getOrdre() + 1);
			obj.getLigneEtiquette().setOrdre(
					obj.getLigneEtiquette().getOrdre() - 1);
	...
	public void onClick$addLigne() {
		openLigneEtiquetteWindow(page, self, new LigneEtiquetteDecorator(
				new LigneEtiquette(), null), true, qrcode.isChecked());
	}
	 ...
	 * @param e
	 */
	public void onGetLigneEtiquette(Event e) {
		if (e.getData() != null) {
			LigneEtiquetteDecorator deco = (LigneEtiquetteDecorator) e
					.getData();
			...
			if (ligneEtiquettesDeco.contains(deco)) {
				...
				deco.getLigneEtiquette().setOrdre(
						ligneEtiquettesDeco.size() + 1);
	 ...
	 *            Ligne etiquette.
	 */
	public void openLigneEtiquetteWindow(Page page, Component comp,
			LigneEtiquetteDecorator deco, boolean creationMode, 
			boolean isQRcode) {
		if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua = populateFicheLigneEtiquetteModal(win,
					page, comp, deco, creationMode, isQRcode);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateFicheLigneEtiquetteModal(
			Window win, Page page, Component comp,
			LigneEtiquetteDecorator deco, boolean creationMode, boolean isQRCode) {
		...
		ua = (HtmlMacroComponent) page.getComponentDefinition(
				"ficheLigneEtiquetteModale", false).newInstance(page, null);
		ua.setParent(win);
		ua.setId("openLigneEtiquetteModale");
		ua.applyProperties();
		ua.afterCompose();

		((FicheLigneEtiquetteModale) ua
				.getFellow("fwinLigneEtiquetteModale")
				.getAttributeOrFellow("fwinLigneEtiquetteModale$composer", true))
				.initModale(deco, comp, creationMode, isQRCode);
...
//		openEtiquetteWindow(page, true, modele, tmp, null);
		...
		AffectationImprimante aff = new AffectationImprimante();
...
//	}
	...
	public void setModeleTypesBox(Listbox m) {
	...
	public void setModeleTypes(List<ModeleType> m) {
		this.modeleTypes = m;
	}
	...
	public void setSelectedModeleType(ModeleType s) {
	...
	public void setLigneEtiquettes(List<LigneEtiquette> l) {
	...
	public void setLigneEtiquetteRowRenderer(LigneEtiquetteRowRenderer l) {
	...
	public void setLigneEtiquettesDeco(ListModelList<LigneEtiquetteDecorator> l) {
	...
	public void setLigneEtiquettesToRemove(List<LigneEtiquette> l) {
