...
public class FicheRequete extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(FicheRequete.class);
	...
	private ListModelList<CritereDecorator> criteresModel = 
						new ListModelList<CritereDecorator>();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjBoxsComponents(new Component[]{this.intituleBox,
				returnCritereCol, rowTemporaire, addCritereButton});
	...
	public void setObject(TKdataObject obj) {
		...
		if (this.requete.getRequeteId() != null) {
			node = GroupementNode
				.convertFromGroupement(requete.getGroupementRacine(), null);
			this.exportNodesGrid.setModel(new SimpleListModel<ExportNode>(node
				.getExportNodeList()));			
		} else {
			node = null;
			this.exportNodesGrid.setModel(new SimpleListModel<ExportNode>(
				new ArrayList<ExportNode>()));	
		}
	...
	public void switchToStaticMode() {
		...
		super.switchToStaticMode(requete == null 
				|| this.requete.equals(new Requete()));
	...
	public void switchToEditMode() {
		...
		node = GroupementNode.convertFromGroupement(requete
	...
	public void createNewObject() {
		try {
			setEmptyToNulls();
			setObject(saveRequete(SessionUtils
		...
		if (requete != null) {
			if (Messagebox.show(ObjectTypesFormatters.getLabel(
					"message.deletion.message", new String[] { Labels
							.getLabel("message.deletion.requete") }),
				...
				try {
					 ...
					 */
					ManagerLocator.getRequeteManager()
						.removeObjectManager(requete);
	...
	public void revertRequete() {
		...
		requete.setGroupementRacine(((Requete) getClone())
											.getGroupementRacine());
	...
	public void onClick$deleteCritere(Event event) {
		// on demande confirmation à l'utilisateur
		// de supprimer le critere
		if (Messagebox.show(ObjectTypesFormatters.getLabel(
				"message.deletion.message", 
				new String[]{Labels
					.getLabel("message.deletion.critere")}), 
		Labels.getLabel("message.deletion.title"), 
		Messagebox.YES | Messagebox.NO, 
		Messagebox.QUESTION) == Messagebox.YES) {
			// on récupère le critère que l'utilisateur veut
			// suppimer
			CritereDecorator cr = (CritereDecorator) 
	 ...
	 * @param event
	 */
	public void onClick$putCritere(Event event) {

		// on récupère le critère que l'utilisateur veut
		// supprimer
		CritereDecorator critereDecorator = (CritereDecorator) 
			AbstractListeController2
						.getBindingData((ForwardEvent) event, false);
		...
		if (node != null) {
			...
			win.setTitle(Labels
					.getLabel("ficheSaisieOperateurCritere.panel.title"));
			...
			ua = (HtmlMacroComponent)
				page.getComponentDefinition("saisieOperateurCritere", false)
			...
			((FicheSaisieOperateurCritere) 
				ua.getFellow("fwinSaisieOperateurCritere")
					.getAttributeOrFellow("fwinSaisieOperateurCritere$composer", true))
					.init(Path.getPath(self));			
			...
			exportNodesGrid.setModel(new SimpleListModel<ExportNode>(node
					.getExportNodeList()));
		}
	 ...
	 */
	public void openAddCritereWindow(Page page, Banque b) {
		 if (!isBlockModal()) {
			...
			ua = (HtmlMacroComponent)
					page.getComponentDefinition("addCritereModale", false)
												.newInstance(page, null);
			ua.setParent(win);
			...
			((FicheAddCritere) 
					ua.getFellow("fwinAddCritereModale")
					.getAttributeOrFellow("fwinAddCritereModale$composer", true))
					.init(self, b);
			ua.setVisible(false);
	...
	public void onGetCritere(Event e) {
		if (e.getData() != null) {
			CritereDecorator cd = (CritereDecorator) e.getData();
...
//			getBinder().loadComponent(criteresGrid);
	...
	public void onCloseSaisieOperateurCritere(Event e) {
		if (e.getData() != null) {
			// On ajoute un nouveau CritereNode au GroupementNode
			this.getNode().addCritere(this.getCritere(),
					(CritereNode) ((Object[]) e.getData())[0],
...
//			criteresGrid.setModel(list);
	...
	public void onClick$returnCritere(Event event) {

		// on récupère les critères que l'utilisateur veut
		// remettre dans la liste temporaires
		ExportNode en = (ExportNode) 
			AbstractListeController2
				.getBindingData((ForwardEvent) event, false);
		// on enlève le critère de la liste et on la met à jour
		List<Critere> crit = new ArrayList<Critere>();
		...
		if (node != null) {
			ListModel<ExportNode> list = 
					new ListModelList<ExportNode>(node.getExportNodeList());
			exportNodesGrid.setModel(list);
		} else {
			exportNodesGrid.setModel(new SimpleListModel<ExportNode>(
					new ArrayList<ExportNode>()));
		}

	}
	
	public void onClick$removeCritere(Event event) {
		// on demande confirmation à l'utilisateur
		...
		if (Messagebox.show(ObjectTypesFormatters
			...
			if (node != null) {
				ListModel<ExportNode> list = new ListModelList<ExportNode>(node.
											getExportNodeList());
				exportNodesGrid.setModel(list);
			} else {
				exportNodesGrid.setModel(new SimpleListModel<ExportNode>(
	...
	protected Requete saveRequete(Utilisateur createur) {
		requete = new Requete();
		requete.setIntitule(intituleBox.getValue());
		requete.setGroupementRacine(node.convertToGroupement());
		ManagerLocator.getRequeteManager().createObjectManager(requete,
				requete.getGroupementRacine(), createur,
				SessionUtils.getSelectedBanques(sessionScope).get(0));
		return requete;
	}

	protected Requete updateRequete(Utilisateur createur) {
		...
		ManagerLocator.getRequeteManager()
			.updateObjectManager(((Requete) getClone()), groupement, createur);
...
//	}
	...
	public void setNode(GroupementNode n) {
		this.node = n;
	}
...
//	
	 ...
	 */
	private void setOpenListeRequete(boolean open) {
		//On ferme la liste des requetes
	...
	private ConstWord constr = new ConstWord();
	{
	...
	public void setCriteresModel(ListModelList<CritereDecorator> cs) {
