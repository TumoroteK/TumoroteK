...
public class AbstractUiRequeteVM {
	...
	private ListModelList<UiRequete> uiRequetes = new ListModelList<UiRequete>();
	private ListModelList<SearchHistory> searchHistories = new ListModelList<SearchHistory>();
	 ...
	 * @param et Entite
	 */
	public AbstractUiRequeteVM(Utilisateur ut, Entite et, List<SearchHistory> sessionHistList) {
		...
		if (sessionHistList != null && et != null) {
			for (SearchHistory searchHistory : sessionHistList) {
	 ...
	 */
	protected void createSearchHistory(List<RechercheCompValues> usedComponents, List<RechercheCompValues> objOps,
									List<RechercheCompValues> usedAnnoComponents, List<RechercheCompValues> annoOps,
									int resultsSize) {
	 ...
	 */
	public void putSearchHistoryValues(Div rechercheRows) {
		
		// clean up
		for (Component target : rechercheRows.getFellows()) {
		...
		SearchHistory sH = getSearchHistories().getSelection().iterator().next();
		List<RechercheCompValues> rcvs = sH.getListSearchHistoryComponent();
		Group currGrp;
		Component target;
		for (RechercheCompValues rechValue : rcvs) {
			currGrp = (Group) rechercheRows.getFellow(rechValue.getGroupId());
			if (!currGrp.isOpen()) {
				currGrp.setOpen(true);
			}
			target = (InputElement) currGrp.getFellow(rechValue.getCompId());
