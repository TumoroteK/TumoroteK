...
public class MainWindow extends GenericForwardComposer<Component> {
	...
	private List<String> availableTabsNames = new ArrayList<String>();
	...
	private Hashtable<String, String> echantillonTypesCouleur = new Hashtable<String, String>();
	private Hashtable<String, String> prodDeriveTypesCouleur = new Hashtable<String, String>();
	...
	public void setMainBinder(AnnotateDataBinder mBinder) {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		Window win = (Window) mainCenterVbox.getFellow("main");
		// Récupère le borderlayout principal contenant toute la
		// fenêtre TK
		Borderlayout mainBorderLayout = (Borderlayout) win
				.getFellow("mainBorderLayout");
		...
		// background
		North north = (North) mainBorderLayout.getFellow("northTopBanniere");
		north.setStyle("background: url(${c:encodeURL('/images/topTumoBanniere_left_repeat.png')}); border:0;");

		// label du bouton
		Button logout = (Button) mainBorderLayout.getFellow("northTopBanniere")
		...
		int bottomHeight = screenHeight - getWindowAvailableHeight();
		...
		mainTabbox = (MainTabbox) mainBorderLayout.getFellow(
				"mainCenterBorderLayout").getFellow("mainTabbox");
		initAvailableTabsNames();

		mainBanquesListBox = (Listbox) mainBorderLayout.getFellow(
				"northTopBanniere").getFellow("mainBanquesListBox");

		Button edit = (Button) mainBorderLayout.getFellow("northTopBanniere")
		// .getFellow("hboxTopBanniere")
				.getFellow("editUser");
...
//		}
		...
		searchHistoryList = new LinkedList<SearchHistory>();
		sessionScope.put(SEARCH_HISTORY_SESSION, searchHistoryList);	
		
 		if (sessionScope.containsKey("tkobj")) {
			displayTkObjDetails((TKAnnotableObject) 
										sessionScope.get("tkobj"));
		}
 		
		scanButtonDiv = (Div) mainBorderLayout.getFellow("northTopBanniere")
				.getFellow("scanButtonDiv");
	...
	public void prepareListBanques() {
		...
		banques = ManagerLocator.getUtilisateurManager()
				.getAvailableBanquesByPlateformeManager(user, pf);
		if (canAccessToutesCollections(user, pf)) {
			toutesColl = new Banque();
			toutesColl
					.setNom(Labels.getLabel("select.banque.toutesCollection"));
	 ...
	 */
	public boolean canAccessToutesCollections(Utilisateur user, Plateforme pf) {
		boolean can = true;
		...
		if (banques.size() > 1) {
			...
			if (!pfs.contains(pf) && !user.isSuperAdmin()) {
				...
				for (int i = 0; i < banques.size(); i++) {
					List<ProfilUtilisateur> liste = ManagerLocator
							.getProfilUtilisateurManager()
							.findByUtilisateurBanqueManager(user,
									banques.get(i));
	...
	public Tabbox getMainTabbox() {
		return (Tabbox) self.getFellow("mainHbox").getFellow("mainCenterVbox")
				.getFellow("main").getFellow("mainBorderLayout")
	 ...
	 * @param disable
	 */
	public void disableBanqueListbox(boolean disable) {
		Listbox banqueListbox = (Listbox) self.getFellow("main").getFellow(
				"mainBanquesListBox");
	...
	private void resetControllers() {
		...
		Tabpanel selectedPanel = getMainTabbox().getSelectedPanel();

		Iterator<Component> it = getMainTabbox().getTabpanels().getChildren()
				.iterator();
		Tabpanel curr;
		while (it.hasNext()) {
			...
			if (curr.getId().equals("stockagePanel")) {
				...
				if (curr.getId().equals("patientPanel")) {
					Components.removeAllChildren(curr);
					mainTabbox.setSelectedTab((Tab) mainTabbox
							.getFellow("patientTab"));
					...
					Components.removeAllChildren(curr);
					mainTabbox.setSelectedTab((Tab) mainTabbox
							.getFellow("cessionTab"));
					Events.postEvent("onSelect", mainTabbox, null);
				} else if (curr.getId().equals("recherchePanel")) {
					((RequeteController) getMainTabbox().getTabpanels()
							.getFellow("recherchePanel")
							.getFellow("winRechercheComplexe")
							.getFellow("exportTabbox")
							.getFellow("panelModelisation")
							.getFellow("modelisationTabbox")
							.getFellow("panelRequete")
							.getFellow("requeteMacro").getFellow("winRequete")
							.getAttributeOrFellow("winRequete$composer", true))
							.reset();

					((AffichageController) getMainTabbox()
							.getTabpanels()
							.getFellow("recherchePanel")
							.getFellow("winRechercheComplexe")
							.getFellow("exportTabbox")
							.getFellow("panelModelisation")
							.getFellow("modelisationTabbox")
							.getFellow("panelAffichage")
							.getFellow("affichageMacro")
							.getFellow("winAffichage")
							.getAttributeOrFellow("winAffichage$composer", true))
							.reset();

					((RechercheController) getMainTabbox()
							.getTabpanels()
							.getFellow("recherchePanel")
							.getFellow("winRechercheComplexe")
							.getFellow("exportTabbox")
							...
							.getAttributeOrFellow("winRecherche$composer", true))
							.reset();

					((RechercheComplexeController) getMainTabbox()
							.getTabpanels()
							.getFellow("recherchePanel")
							.getFellow("winRechercheComplexe")
							.getAttributeOrFellow(
									"winRechercheComplexe$composer", true))
							.resetExecutionController();
				} else if (curr.getId().equals("administrationPanel")) {
					Components.removeAllChildren(curr);
					if (sessionScope.containsKey("AccesAdmin") 
							& (Boolean) sessionScope.get("AccesAdmin")) {
						mainTabbox.setSelectedTab((Tab) mainTabbox
								.getFellow("administrationTab"));
					} else {
						mainTabbox.setSelectedTab((Tab) mainTabbox
								.getFellow("prelevementTab"));
					}					
	...
	public StockageController getStockageController() {
		...
		Tabpanels panels = getMainTabbox().getTabpanels();

		return (StockageController) panels.getFellow("stockagePanel")
	...
	public Integer getRightWindowWidth() {
		...
		Integer width = ((screenWidth - getWindowAvailableWidth()) / 2) - 5;
	...
	public Integer getListPanelHeight() {
		...
		int listPanelHeight = getPanelHeight() - 175;
	...
	public Integer getAnnoPanelHeight() {
		int listPanelHeight = getPanelHeight() - 26;
		return listPanelHeight;
	}
	...
	public Integer getDivSeparatorWidth() {
		Integer width = getWindowAvailableWidth() - 560;
	...
	public void initAvailableTabsNames() {
		...
		if (!getDroitOnAction("Patient", "Consultation")) {
			Tab tab = (Tab) mainTabbox.getFellow("patientTab");
		...
		if (!getDroitOnAction("Prelevement", "Consultation")) {
			Tab tab = (Tab) mainTabbox.getFellow("prelevementTab");
			tab.setDisabled(true);
		} else {
		...
		if (!getDroitOnAction("Echantillon", "Consultation")) {
			Tab tab = (Tab) mainTabbox.getFellow("echantillonTab");
			tab.setDisabled(true);
		} else {
		...
		if (!getDroitOnAction("ProdDerive", "Consultation")) {
			Tab tab = (Tab) mainTabbox.getFellow("deriveTab");
			tab.setDisabled(true);
		} else {
		...
		if (!getDroitOnAction("Cession", "Consultation")) {
			Tab tab = (Tab) mainTabbox.getFellow("cessionTab");
			tab.setDisabled(true);
		} else {
		...
		if (!getDroitOnAction("Stockage", "Consultation")) {
			Tab tab = (Tab) mainTabbox.getFellow("stockageTab");
			tab.setDisabled(true);
		} else {
		...
		if (!getDroitOnAction("Requete", "Consultation")) {
			Tab tab = (Tab) mainTabbox.getFellow("rechercheTab");
			tab.setDisabled(true);
		} else {
		...
		if (sessionScope.containsKey("ToutesCollections")) {
			...
			Tab tab = (Tab) mainTabbox.getFellow("administrationTab");
			tab.setDisabled(true);
		} else {
			// Tab administration
			if (sessionScope.containsKey("AccesAdmin")
	...
	private void setStartingPanel() {
		if (mainTabbox.getSelectedTab() == null
				|| !availableTabsNames.contains(mainTabbox.getSelectedTab()
						.getId())) {
			...
			//	mainTabbox.setSelectedTab(accueil);
				...
				if (availableTabsNames.size() > 1 
						&& !availableTabsNames.get(1).equals("statTab")) { 
	...
	public void unblockAllPanels() {
		for (int i = 0; i < availableTabsNames.size(); i++) {
			Tab tab = (Tab) getMainTabbox()
					.getFellow(availableTabsNames.get(i));
	...
	public void extractTypesColors() {
		echantillonTypesCouleur = new Hashtable<String, String>();
		List<CouleurEntiteType> couleurs = ManagerLocator
				.getCouleurEntiteTypeManager()
				.findAllCouleursForEchanTypeByBanqueManager(selectedBanque);

		// pour chaque association, on l'ajoute dans l'hashtable
		for (int i = 0; i < couleurs.size(); i++) {
			CouleurEntiteType couleur = couleurs.get(i);
			echantillonTypesCouleur.put(couleur.getEchantillonType().getType(),
					couleur.getCouleur().getCouleur());
		}

		prodDeriveTypesCouleur = new Hashtable<String, String>();
		...
		for (int i = 0; i < couleurs.size(); i++) {
			CouleurEntiteType couleur = couleurs.get(i);
			prodDeriveTypesCouleur.put(couleur.getProdType().getType(), couleur
	...
	public void setScreenHeight(Integer screenH) {
	...
	public void setScreenWidth(Integer screenW) {
	...
	public void setTabsNames(List<String> tabsNs) {
	...
	public void setEchantillonTypesCouleur(
			Hashtable<String, String> eTypesCouleur) {
	...
	public void setProdDeriveTypesCouleur(
			Hashtable<String, String> pTypesCouleur) {
	...
	public void setScreenUpdated(boolean screenUp) {
	 ...
	 *            TabPanel parent.
	 */
	public void createMacroComponent(String zul, String macroId,
			Tabpanel parentItem) {
		if (!parentItem.hasFellow(macroId)) {
			Component comp = Executions.createComponents(zul, parentItem, null);
			comp.setId(macroId);
		}
	}
	 ...
	 *            TabPanel parent.
	 */
	public void destroyContentPanel(Tabpanel parentItem) {
		if (parentItem != null) {
	 ...
	 */
	public void updateDroitsForSelectedBanque(Banque bk) {
		
		ConnexionUtils.generateDroitsForSelectedBanque(bk, (Utilisateur) 
								sessionScope.get("User"), sessionScope);
	}
	 ...
	 */
	public Hashtable<String, List<OperationType>> getOperationsForProfil(
			Profil profil) {
		Hashtable<String, List<OperationType>> operations = new Hashtable<String, List<OperationType>>();

		// droits sur les patients
		operations.put("Patient", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "Patient"));

		// droits sur les Prelevements
		operations.put("Prelevement", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "Prelevement"));

		// droits sur les Echantillons
		operations.put("Echantillon", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "Echantillon"));

		// droits sur les ProdDerives
		operations.put("ProdDerive", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "ProdDerive"));

		// droits sur les Cessions
		operations.put("Cession", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "Cession"));

		// droits sur les Stockages
		operations.put("Stockage", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "Stockage"));

		// droits sur les Collaborateurs
		operations.put("Collaborateur", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "Collaborateur"));

		// droits sur les Requetes
		operations.put("Requete", ManagerLocator.getDroitObjetManager()
				.getOperationsByProfilEntiteManager(profil, "Requete"));
	 ...
	 */
	public boolean isFullfilledComponent(String panelName, String macroName) {
		Tabpanels panels = mainTabbox.getTabpanels();
		Tabpanel panel = (Tabpanel) panels.getFellow(panelName);
	...
	public void setAvailableTabsNames(List<String> availableTabs) {
	...
	public boolean getDroitOnAction(String nomEntite, String nomOperation) {
		Boolean admin = false;
		if ((Boolean) sessionScope.containsKey("Admin")) {
	...
	public void onGetUserUpdated(Event e) {
		if (e.getData() != null) {

			Utilisateur user = (Utilisateur) e.getData();
			...
			Borderlayout mainBorderLayout = (Borderlayout) win
					.getFellow("mainBorderLayout");
			Button logout = (Button) mainBorderLayout.getFellow(
					"northTopBanniere").getFellow("buttonLogout");
			logout.setLabel(sb.toString());
	...
	public void openFicheUtilisateurWindow() {
		if (!isBlockModal()) {
			...
			Utilisateur user = SessionUtils.getLoggedUser(sessionScope).clone();

			final HtmlMacroComponent ua = populateFicheUtilisateurModal(win,
					page, user, Path.getPath(self));
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateFicheUtilisateurModal(Window win,
			Page page, Utilisateur user, String path) {
		...
		ua = (HtmlMacroComponent) page.getComponentDefinition(
				"ficheUtilisateurModale", false).newInstance(page, null);
		ua.setParent(win);
		...
		((FicheUtilisateurModale) ua.getFellow("fwinUtilisateurModale")
				.getAttributeOrFellow("fwinUtilisateurModale$composer", true))
				.init(user, path);
	...
	public void onBookmarkChange$mainWin(BookmarkEvent event) {
		try {
			Tabpanel item = (Tabpanel) ((MainTabbox) mainTabbox)
	...
	public void launchImportPatientFromCrfProdedure(Patient patient) {
		...
		maladie.setLibelle(SessionUtils.getSelectedBanques(sessionScope).get(0)
				.getDefautMaladie());
		maladie.setCode(SessionUtils.getSelectedBanques(sessionScope).get(0)
		...
		if (ManagerLocator.getPatientManager().findDoublonManager(patient)) {
			List<Patient> liste = ManagerLocator.getPatientManager()
					.findByNomLikeManager(patient.getNom(), true);
			...
			if (ManagerLocator.getMaladieManager().findDoublonManager(maladie)) {
				List<Maladie> mals = ManagerLocator.getMaladieManager()
						.findByLibelleLikeManager(maladie.getLibelle(), true);
		...
		PrelevementController tabController = (PrelevementController) PrelevementController
				.backToMe(this, page);

		// si on arrive à récupérer le panel prelevement et son controller
		if (tabController != null) {
			tabController.switchToCreateMode(null);
			tabController.getFicheEdit().injectPatientAndMaladieInFiche(
					patient, maladie);
	 ...
	 * @since 2.0.10
	 */
	public void displayTkObjDetails(TKAnnotableObject tkObj) {
		...
		if (tkObj instanceof Prelevement) {
			tabController = (PrelevementController) PrelevementController
					.backToMe(this, page);
		}
	...
	private Hashtable<String, Boolean> droitsConsultation = new Hashtable<String, Boolean>();
	...
	public void initDroitsConsultation() {
		...
		List<String> eNoms = new ArrayList<String>();
		eNoms.add("Patient");
	...
	@SuppressWarnings("unchecked")
	public void drawConsultationLinks(List<String> entites) {
		if (sessionScope.containsKey("Admin") 
			...
			Hashtable<String, List<OperationType>> droitsSession = new Hashtable<String, List<OperationType>>();
	
			if (sessionScope.containsKey("Droits")) { // logiquement car SelectBanque
				// on extrait les droits de l'utilisateur
				droitsSession = (Hashtable<String, List<OperationType>>) sessionScope
						.get("Droits");
			}
			
			for (int i = 0; i < entites.size(); i++) {
				List<OperationType> ops = droitsSession.get(entites.get(i));
	...
	public void onSwitchBanqueFromMaladiePrelClick(Event event) {
	...
	public void onScanTimer() {
		if (scanTimer.isRunning()) {
			...
			AbstractObjectTabController controller = getSelectedTabController();
			if (controller != null) {
				ListModelList<ScanTerminale> scansModel = new ListModelList<ScanTerminale>(ManagerLocator
					.getScanTerminaleManager().findByDeviceManager(null));
				if (!scansModel.isEmpty()) {
					...
					if (scansModel.size() > 1) {
						...
						ScanTerminale lastScan = scansModel.get(0);
	...
	public void onSelectScan(Event ev) {
	...
	public void onSelectScanAndDelete(Event ev) {
		if (ev != null) {
			onSelectScan(ev);
		}
