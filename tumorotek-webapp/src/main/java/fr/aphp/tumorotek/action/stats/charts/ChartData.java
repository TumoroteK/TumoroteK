...
public class ChartData {
	...
	private ChartDataTriggers triggers;
	 ...
	 */
	public ChartData(Date date_debut, Date date_fin, Set<Plateforme> pfs, 
			ChartDataTriggers trgs, String pfNom, String bNom) {
		...
		if (triggers.getPatientPfModelTrg()) {
			patientPfModel = ManagerLocator.getGraphesModeleManager()
				.platformeViewByPatientManager(date_debut, date_fin, isOracle);
			restrictPfs(patientPfModel, pfs);
		}
		if (triggers.getPatientCollModelTrg()) {
			patientCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByPatientManager(date_debut, date_fin, pfNom, isOracle);
		}
		//--- 
		if (triggers.getPrelevementPfModelTrg()) {
			prelevementPfModel = ManagerLocator.getGraphesModeleManager()
				.platformeViewByPrelevementManager(date_debut, date_fin, isOracle);
			restrictPfs(prelevementPfModel, pfs);
		}
		if (triggers.getPrelevementCollModelTrg()) {
			prelevementCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByPrelevementManager(date_debut, date_fin, pfNom, isOracle);
		}
		if (triggers.getPrelEtabCollModelTrg()) {
			prelEtabCollModel = ManagerLocator.getGraphesModeleManager()
				.prelevementByEtablissementByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
		if (triggers.getPrelTypeCollModelTrg()) {
			prelTypeCollModel = ManagerLocator.getGraphesModeleManager()
				.prelevementTypeByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
		if (triggers.getPrelConsentCollModelTrg()) {
			prelConsentCollModel = ManagerLocator.getGraphesModeleManager()
				.prelevementByConsentementByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
		//---
		if (triggers.getEchanPfModelTrg()) {
			echanPfModel = ManagerLocator.getGraphesModeleManager()
				.platformeViewByEchantillonManager(date_debut, date_fin, isOracle);
			restrictPfs(echanPfModel, pfs);
		}
		if (triggers.getEchanCollModelTrg()) {
			echanCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByEchantillonManager(date_debut, date_fin, pfNom, isOracle);
		}
		if (triggers.getEchanTypeCollModelTrg()) {
			echanTypeCollModel = ManagerLocator.getGraphesModeleManager()
				.echantillonTypeByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
		if (triggers.getEchanCimCollModelTrg()) {
			echanCimCollModel = ManagerLocator.getGraphesModeleManager()
				.echantillonsCIM10ByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
		if (triggers.getEchanOrgCollModelTrg()) {
			echanOrgCollModel = ManagerLocator.getGraphesModeleManager()
				.echantillonsADICAPByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
		
		//---
		if (triggers.getDerivePfModelTrg()) {
			derivePfModel = ManagerLocator.getGraphesModeleManager()
				.platformeViewByDeriveManager(date_debut, date_fin, isOracle);
			restrictPfs(derivePfModel, pfs);
		}
		if (triggers.getDeriveCollModelTrg()) {
			deriveCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByDeriveManager(date_debut, date_fin, pfNom, isOracle);
		}
		if (triggers.getDeriveTypeCollModelTrg()) {
			deriveTypeCollModel = ManagerLocator.getGraphesModeleManager()
				.deriveTypeByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
		
		//---
		
		if (triggers.getCessionPfModelTrg()) {
			cessionPfModel = ManagerLocator.getGraphesModeleManager()
				.platformeViewByCessionManager(date_debut, date_fin, isOracle);
			restrictPfs(cessionPfModel, pfs);
		}
		if (triggers.getCessionCollModelTrg()) {
			cessionCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByCessionManager(date_debut, date_fin, pfNom, isOracle);
		}
		if (triggers.getEchansCedesCollModelTrg()) {
			echansCedesCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByEchansCedesManager(date_debut, date_fin, pfNom, isOracle);
		}
		if (triggers.getDerivesCedesCollModelTrg()) {
			derivesCedesCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByDerivesCedesManager(date_debut, date_fin, pfNom, isOracle);
		}
		if (triggers.getCessionTypeCollModelTrg()) {
			cessionTypeCollModel = ManagerLocator.getGraphesModeleManager()
				.cessionTypeByCollectionManager(date_debut, date_fin, bNom, pfNom, isOracle);
		}
	}
	 ...
	 * @param pfNom
	 */
	public void updateCountsAfterPlateformeChange(Date date_debut, Date date_fin, String pfNom) {
		patientCollModel = ManagerLocator.getGraphesModeleManager()
				.collectionViewByPatientManager(date_debut, date_fin, pfNom, isOracle);
	 ...
	 */
	public void updateCountsAfterBanqueChange(Date date_debut, Date date_fin, String bNom, 
									String entiteNom, String pfNom) {
	 ...
	 */
	private void restrictPfs(GraphesModele grM, Set<Plateforme> pfs) {
		
		List<String> pfNamesRestrict = new ArrayList<String>();
	 ...
	 */
	public static void buildCategoryModel(GraphesModele grM, 
										CategoryModel model) {
		if (model != null && grM != null) {
			model.clear();
			for (Map.Entry<String, List<Integer>> entry : 
								grM.getCountsMap().entrySet()) {
				for (int i = 0; i < entry.getValue().size(); i++) {
					model.setValue("main" + (i == 0 ? "" : i), 
	 ...
	 */
	public static void buildPieModel(GraphesModele grM, 
			PieModel model) {		
		if (model != null && grM != null) {
			model.clear();
			for (Map.Entry<String, List<Integer>> entry : 
