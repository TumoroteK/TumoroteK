...
public class ImprimanteModeleModale {
	...
	@Init
	public void init(@ExecutionArgParam("pf") Plateforme pf, 
			@ExecutionArgParam("objs") List<? extends TKStockableObject> objs, 
			@ExecutionArgParam("affectation") AffectationImprimante aff, 
		...
		imprimantes.addAll(ManagerLocator
				.getImprimanteManager().findByPlateformeManager(pf));
		
		if (isTest) { // supprime les imprimantes mbio de la liste
			List<Imprimante> impsMbio = new ArrayList<Imprimante>();
		...
		modeles.addAll(ManagerLocator
				.getModeleManager().findByPlateformeManager(pf));
		modeles.add(0, null);
	...
	public void onSelect$imprimantesBox() {		
		if (getSelectedImprimante().getImprimanteApi()
										.getNom().equals("mbio")) {
			...
			if (getObjects() != null) {
				...
				if (selectedImprimante.getImprimanteApi().getNom().equals("tumo")) {
					...
					try {
						...
						Messagebox.show(AbstractController
								.handleExceptionMessage(re), "Error",
								Messagebox.OK, Messagebox.ERROR);
					} catch (Exception e) {
						e.printStackTrace();
						Messagebox.show(AbstractController
								.handleExceptionMessage(e), "Error",
								Messagebox.OK, Messagebox.ERROR);
					}
				} else if (selectedImprimante.getImprimanteApi()
											.getNom().equals("mbio")) {
					// si on utilise l'API mbio
					MBioFileProperties mBioFile = new MBioFileProperties(
							selectedImprimante.getMbioPrinter());
		
					MBioBarcodePrinter printer = 
						new MBioBarcodePrinter(mBioFile,
							mBioFile.getConfDir());
					...
					if (completed != 1) {
						...
						Messagebox
						.show(Labels.getLabel("general.impression.ok"),
								"OK", Messagebox.OK, Messagebox.INFORMATION);
					}
				}
			} else { //impression libre
				openEtiquetteWindow(getSelectedModele(), 
						getSelectedImprimante());
			}
		}
	...
	public void setObjects(List<? extends TKStockableObject> obs) {
		this.objects = obs;
	}
	...
	public void setSelectedImprimante(Imprimante i) {
		this.selectedImprimante = i;
		
		// efface le modele si mbio
		if (this.selectedImprimante.getImprimanteApi()
										.getNom().equals("mbio")) {
			setSelectedModele(null);
		}
	}
	...
	public void setSelectedModele(Modele m) {
		this.selectedModele = m;
	}	
	
	private void setAffectation(AffectationImprimante aff) {
	...
	public void openEtiquetteWindow(Modele modele, Imprimante imp) {
...
//			}
	...
	public boolean getIsMbio() {
		return (getSelectedImprimante() != null && getSelectedImprimante()
				.getImprimanteApi().getNom().equals("mbio")
			|| isTest);
	}
	...
	public void setSelectedRawLang(String s) {
	...
	public void setBarcodeByZPL(BarcodeFieldDefault b) {
		this.barcodeByZPL = b;
	}
	...
	public void setBarcodeByJS(BarcodeFieldDefault b) {
		this.barcodeByJS = b;
	}
}