...
public class TKChart extends HtmlMacroComponent
{
   ...
   public void setModel(final GraphesModele model){
      this.model = model;
   ...
   private void initCategories(){
      ...
      int i = 0;
      if(getModel() != null){
         for(final String key : getModel().getCountsMap().keySet()){
            final CategoryDetails cat = new CategoryDetails(key, getModel().getCountsMap().get(key).get(0),
               type.equals("stacked_bar") ? getModel().getCountsMap().get(key).get(1) : null, true,
   ...
   public void doPostTooltipText(final Event event){
      final Area a = ((Area) ((MouseEvent) event).getAreaComponent());
      if(this.getParent() != null && a != null){
         Events.postEvent("onChartAreaClick", this.getParent().getParent(), a.getTooltiptext());
      }
   }
   ...
   public void updateCategoryModel(){
      // colors
      final List<Paint> newColors = new ArrayList<>();
      final List<CategoryDetails> sels = new ArrayList<>(categories.getSelection());
      Collections.sort(sels);
      AbstractChartModel newModel = null;
      if(type.equals("bar")){
         newModel = new SimpleCategoryModel();
         for(final CategoryDetails detail : sels){
            ((CategoryModel) newModel).setValue("main", detail.getCategory(), detail.getValue());
         ...
         for(final CategoryDetails detail : sels){
            ((CategoryModel) newModel).setValue("main", detail.getCategory(), detail.getValue());
            ((CategoryModel) newModel).setValue("main1", detail.getCategory(), detail.getNonStockValue());
            newColors.add(detail.getColor());
         }
      }

      ((TKChartEngine) chart.getEngine()).setColorSet(newColors.toArray(new Color[newColors.size()]));
   ...
   public void getDataAsExcel(){
      ...
      for(final CategoryDetails detail : categories){
         row = sheet.createRow(currentRow);
         currentRow++;
         cell = row.createCell(0);
         cell.setCellType(Cell.CELL_TYPE_STRING);
   ...
   public void closePopup(final Event e){
   ...
   public void onMouseOverPopup(final Event e){
      if(onMouseOutEvent != null){
   ...
   public void onMouseOutPopup(final Event e){
      buttonsPop.close();
   }
   ...
   private void updateWidth(){
      if(type.equals("pie")){
         ...
         chart.setWidth(
            String.valueOf(categories.size() > 10 ? (!zoomed ? 35 : 50) * categories.size() : (!zoomed ? 400 : 600)) + "px");
      }
   ...
   public void setCategories(final ListModelList<CategoryDetails> categories){
   ...
   public void setZoomed(final Boolean z){
      this.zoomed = z;
