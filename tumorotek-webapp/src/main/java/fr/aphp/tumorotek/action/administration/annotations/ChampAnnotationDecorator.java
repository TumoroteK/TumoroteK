...
public class ChampAnnotationDecorator extends SmallObjDecorator {
	...
	private LinkedHashSet<AnnotationDefaut> defauts = 
									new LinkedHashSet<AnnotationDefaut>();
	private LinkedHashSet<AnnotationDefaut> defautsCopy = 
									new LinkedHashSet<AnnotationDefaut>();
	...
	public ChampAnnotationDecorator clone() {
		ChampAnnotationDecorator clone = 
			new ChampAnnotationDecorator(getChamp(), currentBanque, 
													getCatalogueChp());
		return clone;
	}
	 ...
	 * @param chp
	 */
	public ChampAnnotationDecorator(ChampAnnotation chp, Banque bank, 
											boolean isCatalogueChp) {
	...
	public void setChampAnnotation(ChampAnnotation chp) {
		this.champ = chp;
		if (isThesaurus()) {
			defautsContainsEmptyFirst(ManagerLocator
				.getChampAnnotationManager().getAnnotationDefautsManager(chp), 
					this.defauts);			
			
			this.items = new ArrayList<Item>(ManagerLocator
				.getChampAnnotationManager()
					.getItemsManager(chp, currentBanque));
			
		} else {
			//type alphanum par defaut
			this.defauts.addAll(ManagerLocator.getChampAnnotationManager()
										.getAnnotationDefautsManager(chp));
	...
	public void setCreation(boolean c) {
		this.creation = c;
	}
	...
	public void setCatalogueChp(boolean c) {
		this.catalogueChp = c;
	}
	...
	public String getDataType() {
		if (this.champ.getDataType() != null) {
			return Labels.getLabel("DataType." 
					+ this.champ.getDataType().getType());
		}
		return null;
	}
	...
	public void setObligatoire(boolean obl) {
	...
	public String getObligatoireFormatted() {
		if (this.champ != null) {
			return ObjectTypesFormatters
						.booleanLitteralFormatter(getObligatoire());
		}
		return null;
	}
	...
	public String getCombineFormatted() {
		if (this.champ != null) {
			return ObjectTypesFormatters
						.booleanLitteralFormatter(this.champ.getCombine());
		}
		return null;
	}
	...
	public boolean getIsCombinable() {
		if (this.champ != null && this.champ.getDataType() != null) {
			return (!"fichier".equals(this.champ.getDataType().getType())
					&& !"boolean".equals(this.champ.getDataType().getType()));
		}
		return !catalogueChp;
	}
	...
	public Boolean getDefautEraserDisplay() {
		return (!isThesaurus() && !isThesaurusM()
				&& !this.defaut.isEmpty() || !this.items.isEmpty()) 
														&& !getStatique();
	}
	...
	public String getDetailsDisplay() {
		if (!this.items.isEmpty() && getStatique() 
				&& this.items.size() > 5) {
			return "formLink";
		} else {
			return "";
	...
	public Boolean getAddDefautDisplay() {
		return (this.defaut.isEmpty() && this.items.isEmpty())  
														&& !getStatique();
	}
	...
	public String getDefautsFormatted() {
		if (isAlphanum() || isHyperlien()) {
			...
			return ObjectTypesFormatters
				.booleanLitteralFormatter(this.defaut.getBool());
		} else if (isDate() || isDatetime()) {
			return ObjectTypesFormatters
				.dateRenderer2(this.defaut.getDate());
		} else if (isNum() && this.defaut.getAlphanum() != null) {
			return ObjectTypesFormatters
					.doubleLitteralFormatter(Double
								.parseDouble(this.defaut.getAlphanum()));
		} else if (isTexte()) {
			return this.defaut.getTexte();
		} else if (isThesaurus()) {
			if (items.size() <= 5) {
				return ObjectTypesFormatters
								.renderItems(this.items, this.defauts);
			} else {
	...
	public void setDefauts(Set<AnnotationDefaut> defs) {
	 ...
	 */
	public static List<ChampAnnotationDecorator> 
					decorateListe(List<ChampAnnotation> chps, Banque bank, 
													boolean isCatalogueChp) {
		List<ChampAnnotationDecorator> liste = 
									new ArrayList<ChampAnnotationDecorator>();
		Iterator<ChampAnnotation> it = chps.iterator();
		while (it.hasNext()) {
			liste.add(new ChampAnnotationDecorator(it.next(), bank, 
														isCatalogueChp));
		}
		return liste;
	}
	 ...
	 */
	public static List<ChampAnnotation> 
							unDecore(List<ChampAnnotationDecorator> decos) {
		List<ChampAnnotation> liste = new ArrayList<ChampAnnotation>();
		Iterator<ChampAnnotationDecorator> it = decos.iterator();
		while (it.hasNext()) {
	...
	public boolean equals(Object obj) {
		...
		ChampAnnotationDecorator deco = (ChampAnnotationDecorator) obj;
	 ...
	 */
	public static class ChampAnnotationDecoratorComparator 
												implements Comparator<Object> {

		@Override
		public int compare(Object arg0, Object arg1) {
			ChampAnnotationDecorator chp1 = (ChampAnnotationDecorator) arg0;
			ChampAnnotationDecorator chp2 = (ChampAnnotationDecorator) arg1;
	...
	public void checkEmptyDefaut() {
		if (!this.champ.getAnnotationDefauts().isEmpty() 
				&& this.champ.getAnnotationDefauts().iterator()
													.next().isEmpty()) {
	 ...
	 */
	private void defautsContainsEmptyFirst(Set<AnnotationDefaut> defs, 
			LinkedHashSet<AnnotationDefaut> newSet) {
		Iterator<AnnotationDefaut> itor = defs.iterator();
		...
		if (!hasEmpty) {
			AnnotationDefaut empty = new AnnotationDefaut();
	...
	public void cloneDefauts() {
		defautsCopy.clear();
		Iterator<AnnotationDefaut> defautsIt = defauts.iterator();
