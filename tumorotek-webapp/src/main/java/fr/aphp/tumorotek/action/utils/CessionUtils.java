...
public class CessionUtils {
	 ...
	 */
	public static Document createFileHtmlToPrint(Cession cession, 
			List<CederObjet> echantillonsCedes, 
			List<CederObjet> derivesCedes) {
...
//				cedeObjFactory.decorateListe(derivesCedes);
		...
		if (echantillonsCedes.size() > 0
			...
			doc = ManagerLocator.getXmlUtils()
			...
			if (echantillonsCedes.size() > 0) {
				...
				for (int i = 0; i < echantillonsCedes.size(); i++) {
					...
					Echantillon echan = ManagerLocator.getEchantillonManager()
							.findByIdManager(echantillonsCedes.get(i).getObjetId());
					...
					if (emp != null) {
						...
						// titre intermédiaire
						if (i == 0) {
							newBi.setTitreIntermediaire(Labels
									.getLabel("impression.boite.echanillons"));
						}
			...
			if (derivesCedes.size() > 0) {
				...
				for (int i = 0; i < derivesCedes.size(); i++) {
					...
					ProdDerive derive = ManagerLocator.getProdDeriveManager()
							.findByIdManager(derivesCedes.get(i).getObjetId());
					...
					if (i == 0) {
						newBi.setTitreIntermediaire(Labels
								.getLabel("impression.boite.prodDerive"));
					}
