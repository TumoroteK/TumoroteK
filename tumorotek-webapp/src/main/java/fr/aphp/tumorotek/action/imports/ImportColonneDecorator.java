...
public class ImportColonneDecorator {
	...
	public ImportColonneDecorator(ImportColonne ic) {
		colonne = ic;
	}
	...
	public void setColonne(ImportColonne c) {
		this.colonne = c;
	}
	...
	public String getChamp() {
		...
		if (colonne.getChamp() != null) { 
			if (colonne.getChamp().getChampEntite() != null) {
				champ = getLabelForChampEntite(colonne.getChamp()
						.getChampEntite());
			} else {
	...
	public String getFormat() {
		...
		if (colonne.getChamp() != null) { 
			if (colonne.getChamp().getChampEntite() != null) {
				if (colonne.getChamp().getChampEntite().getNom()
						.equals("EmplacementId")) {
					format = Labels.getLabel(
							"importColonne.Type.Emplacement");
				} else if (colonne.getChamp().getChampEntite()
						.getQueryChamp() != null) {
					format = "thesaurus";
				} else {
					format = colonne.getChamp().getChampEntite()
						.getDataType().getType();
				}
			} else {
				format = colonne.getChamp().getChampAnnotation()
					.getDataType().getType();
			}
		} else { // subderive header
			if (colonne.getNom().equals("code.parent")) {
	...
	public boolean getCanDelete() {
		if (colonne.getChamp() != null 
				&& colonne.getChamp().getChampEntite() != null) {
			return colonne.getChamp().getChampEntite()
				.isNullable();
		}
		return canDelete;
	}
	...
	public void setDisableEditLabel(Boolean d) {
		this.disableEditLabel = d;
	}

	public String getLabelForChampEntite(ChampEntite champ) {
	 ...
	 * @param cederobjets
	 * @param isSubderive rend artificiellement les 3 colonnes de l'entete non supprimables
	 * @return CederObjets décorées.
	 */
	public static List<ImportColonneDecorator> 
			decorateListe(List<ImportColonne> objets, boolean isSubderive) {
		List<ImportColonneDecorator> liste = 
									new ArrayList<ImportColonneDecorator>();
		Iterator<ImportColonne> it = objets.iterator();
		int i = 0;
		while (it.hasNext()) {
	 ...
	 */
	public static List<ImportColonne> extractListe(
			List<ImportColonneDecorator> cedes) {
		List<ImportColonne> liste = new ArrayList<ImportColonne>();
		Iterator<ImportColonneDecorator> it = cedes.iterator();
	...
	public boolean equals(Object obj) {
		...
		ImportColonneDecorator deco = (ImportColonneDecorator) obj;
