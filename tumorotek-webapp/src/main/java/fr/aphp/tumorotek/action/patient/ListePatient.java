...
public class ListePatient extends AbstractListeController2 {
	...
	private List<Patient> listObjects = new ArrayList<Patient>();
	private List<Patient> selectedObjects = new ArrayList<Patient>();
	...
	private PatientRowRenderer listObjectsRenderer = 
											new PatientRowRenderer(true);
	private PatientsNbPrelevementsComparator comparatorAsc = 
			new PatientsNbPrelevementsComparator(true);
	private PatientsNbPrelevementsComparator comparatorDesc = 
		new PatientsNbPrelevementsComparator(false);
	...
	public void setSearchNomPatient(String search) {
	...
	public void setSearchNipPatient(String search) {
	...
	public void doAfterCompose(Component comp) throws Exception {	
		...
		this.listObjectsRenderer
			.setBanques(PatientUtils
		...
		comparatorAsc.setBanques(PatientUtils
				.getBanquesConsultForPrelevement(sessionScope));
		comparatorDesc.setBanques(PatientUtils
				.getBanquesConsultForPrelevement(sessionScope));
	...
	public void removeObjectFromList(TKdataObject obj) {
		getListObjects().remove((Patient) obj);
	}
	...
	public void removeFromSelectedObjects(TKdataObject obj) {
		if (getSelectedObjects().contains(obj)) {
			getSelectedObjects().remove((Patient) obj);
		}
	}
	...
	public void initObjectsBox() {
		...
		List<Patient> patients = ManagerLocator.getPatientManager()
		...
		if (dateCreation.isChecked()) {			
			...
			if (!searchNomPatient.equals("")) {
				if (searchNomPatient.contains(",")) {
					...
					patients = ManagerLocator.getPatientManager()
						.findByNomInListManager(pats, 
								SessionUtils.getSelectedBanques(sessionScope));
				} else {
					patients = ManagerLocator.getPatientManager()
					.findByNomLikeBothSideReturnIdsManager(
							searchNomPatient, 
							SessionUtils
								.getSelectedBanques(sessionScope),
							true);
			...
			if (!searchNipPatient.equals("")) {
				if (searchNipPatient.contains(",")) {
					List<String> pats = ObjectTypesFormatters
						.formateStringToList(searchNipPatient);
					patients = ManagerLocator.getPatientManager()
						.findByNipInListManager(pats, 
								SessionUtils.getSelectedBanques(sessionScope));
				} else {
					patients = ManagerLocator.getPatientManager()
					.findByNipLikeBothSideReturnIdsManager(
							searchNipPatient, SessionUtils
							.getSelectedBanques(sessionScope),
							true);	
				}
			} else {
	...
	public void onClick$findByListPatientNoms() {
		...
		List<String> noms = getListStringToSearch();
		List<Integer> patList = ManagerLocator.getPatientManager()
			.findByNomInListManager(noms, 
				SessionUtils.getSelectedBanques(sessionScope));
	...
	public void onClick$findByListPatientNips() {
		...
		// fichier excel que l'utilisateur va uploader
		List<String> nips = getListStringToSearch();
		List<Integer> patients = ManagerLocator.getPatientManager()
			.findByNipInListManager(nips, 
					SessionUtils.getSelectedBanques(sessionScope));
		// affichage de ces résultats
	...
	public void setComparatorAsc(PatientsNbPrelevementsComparator c) {
	...
	public void setComparatorDesc(PatientsNbPrelevementsComparator c) {
		this.comparatorDesc = c;
	}
	
	@Override
	public void batchDelete(List<Integer> ids, String comment) {
		ManagerLocator.getPatientManager().removeListFromIdsManager(ids, comment, 
