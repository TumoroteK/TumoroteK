...
public class ImportColonneRowRenderer implements RowRenderer<ImportColonne> {
	...
	@Override
	public void render(Row row, ImportColonne data, int index) throws Exception {
		ImportColonne colonne = data;
		
		// nom colonne
		Label colonneLabel = new Label(colonne.getNom());
		...
		Label champLabel = new Label(champ);
...
//			}
		...
		Html entiteLabel = new Html(entite);
		...
		if (colonne.getChamp() != null) {
			if (colonne.getChamp().getChampEntite() != null) {
				ob = !colonne.getChamp().getChampEntite()
					.isNullable();
			}
		} else { // subderive header
			ob = true;
		}
		Label obligatoireLabel = new Label(ObjectTypesFormatters
					.booleanLitteralFormatter(ob));
		if (ob) {
