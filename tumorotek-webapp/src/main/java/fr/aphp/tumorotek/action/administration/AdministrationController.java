...
public class AdministrationController extends AbstractObjectTabController {
	...
	public void reset() {
		...
		// reset des comptes
		getUtilisateurController().getFicheCombine()
	...
	public void initAvailableTabsNames() {
		...
		if (!getDroitOnAction("Collaborateur", "Consultation")) {
			Tab tab = (Tab) adminTabbox.getFellow("collaborationsTab");
		...
		if (!getDroitOnAction("Cession", "Consultation")) {			
			Tab tab = (Tab) adminTabbox.getFellow("contratsTab");
			tab.setDisabled(true);
		} else {
		...
		// Tab Import
		if (!canCurrentUserImport()
			||	SessionUtils.getSelectedBanques(sessionScope).size() != 1) {
			Tab tab = (Tab) adminTabbox.getFellow("importsTab");
			tab.setDisabled(true);
		} else {
		...
		if (!(Boolean) sessionScope.get("Admin") 
				|| SessionUtils.getSelectedBanques(sessionScope).isEmpty()) {
			Tab tab = (Tab) adminTabbox.getFellow("impressionTab");
			tab.setDisabled(true);
		} else {
		...
		if (!sessionScope.containsKey("AdminPF")) {
			Tab tabImp = (Tab) adminTabbox.getFellow("imprimantesTab");
		...
		// Tab Outils
		if (!sessionScope.containsKey("AdminPF")) {
			Tab tabImp = (Tab) adminTabbox.getFellow("outilsTab");
			tabImp.setDisabled(true);
		} else {
		...
		// Tab historique
		if (!(Boolean) sessionScope.get("Admin")) {
			Tab tabImp = (Tab) adminTabbox.getFellow("historiqueTab");
			tabImp.setDisabled(true);
		} else {
		...
		if (!SessionUtils.getLoggedUser(sessionScope).isSuperAdmin()
				&& !sessionScope.containsKey("AdminPF")) {
			Tab tabPf = (Tab) adminTabbox.getFellow("plateformesTab");
	...
	public BanqueController getBanqueController() {
		return (BanqueController) self.getFellow("adminTabbox")
			.getFellow("banquePanel")
				.getFellow("banqueMacro")
					.getFellow("winBanque")
						.getAttributeOrFellow("winBanque$composer", true);
	}
	...
	public ContratController getContratController() {
		...
		return (ContratController) self.getFellow("adminTabbox")
	...
	public CollaborationsController getCollaborationsController() {
		...
		return (CollaborationsController) self
	...
	public UtilisateurController getUtilisateurController() {
		if (self.getFellow("adminTabbox")
				.getFellow("utilisateursPanel")
				.getFellowIfAny("utilisateursMacro") != null) {
			return (UtilisateurController) self
				.getFellow("adminTabbox")
				.getFellow("utilisateursPanel")
	 ...
	 */
	public void selectUtilisateurInController(Utilisateur user) {
		UtilisateurController controller = (UtilisateurController) 
			UtilisateurController
			.backToMe(getMainWindow(), adminTabbox);
	 ...
	 * @since 2.1
	 */
	public void selectProfilInController(Profil profil) {
	 ...
	 */
	public void selectBanqueInController(Banque banque) {
		BanqueController controller = (BanqueController) 
			BanqueController.backToMe(getMainWindow(), adminTabbox);
	...
	public ImpressionController getImpressionController() {
		...
		return (ImpressionController) self
			.getFellow("adminTabbox")
				.getFellow("impressionsPanel")
				.getFellow("impressionsMacro")
				.getFellow("winTemplate")
	...
	public ThesaurusController getThesaurusController() {
		...
		return (ThesaurusController) self
				.getFellow("adminTabbox")
				.getFellow("thesaurusPanel")
				.getFellow("thesaurusMacro")
				.getFellow("winThesaurus")
				.getAttributeOrFellow("winThesaurus$composer", true);
	...
	public TerminaleTypeController getTerminaleTypeController() {
		...
		return (TerminaleTypeController) self
				.getFellow("adminTabbox")
				.getFellow("terminaleTypePanel")
				.getFellow("terminaleTypeMacro")
				.getFellow("winTerminaleType")
				.getAttributeOrFellow("winTerminaleType$composer", true);
	...
	public CodesController getCodesController() {
		...
		return (CodesController) self
				.getFellow("adminTabbox")
				.getFellow("codesPanel")
				.getFellow("codesMacro")
				.getFellow("winCodes")
	...
	public ImportController getImportController() {
		...
		return (ImportController) self
				.getFellow("adminTabbox")
				.getFellow("importsPanel")
				.getFellow("importsMacro")
				.getFellow("winImportTemplate")
				.getAttributeOrFellow("winImportTemplate$composer", true);
	 ...
	 */
	public boolean isFullfilledComponent(
		String panelName, String macroName) {
		Tabpanels panels = adminTabbox.getTabpanels();
	...
	public void onSelect$adminTabbox() {
        ...
        if (item != null && item.getId().equals("utilisateursPanel")) {
        	getMainWindow().createMacroComponent(
					"/zuls/utilisateur/Utilisateur.zul", 
					"winUtilisateur", item);
        }
