...
public final class PrelevementUtils {
	 ...
	 * @return String
	 */
	public static String getPatientNomAndPrenom(Prelevement prel) {
		StringBuffer sb = new StringBuffer();

		Maladie maladie = prel.getMaladie();
		
		if (maladie != null && maladie.getPatient() != null) {
			Patient pat = maladie.getPatient();
	 ...
	 * @return x restants
	 */
	public static int getNbEchanRestants(Prelevement prel) {
		return ManagerLocator.getEchantillonManager()
					.findCountRestantsByPrelevementManager(prel)
					.intValue();
	}
	 ...
	 * @return y (x restants)
	 */
	public static String getNbEchanRestantsSurTotalEtStockes(
			Prelevement prel) {
		StringBuffer sb = new StringBuffer();
	 ...
	 * @return x stockés
	 */
	public static int getNbEchanStockesEtReserves(Prelevement prel) {
		return ManagerLocator.getEchantillonManager()
			.findCountByPrelevementAndStockeReserveManager(prel)
			.intValue();
	}
	 ...
	 * @return y (x stockes)
	 */
	public static String getNbEchanStockesSurTotal(Prelevement prel) {
		StringBuffer sb = new StringBuffer();
		sb.append(getNbEchanStockesEtReserves(prel));
		sb.append(" / ");
		sb.append(ManagerLocator.getPrelevementManager()
				.getEchantillonsManager(prel).size());
	 ...
	 * @since 2.0.9
	 */
	public static Hlayout drawListIcones(Prelevement prel) {
		Hlayout icones = new Hlayout();
		//infectieux
		...
		if (prel != null && prel.getConformeArrivee() != null) {
			...
			if (prel.getConformeArrivee()) {
				...
				nonConf.setTooltiptext(Labels
							.getLabel("tooltip.conforme.arrivee"));
			} else {
				String noconfs = ": ";
				Iterator<NonConformite> ncIt = ManagerLocator.getNonConformiteManager()
						...
						.findByObjetAndTypeManager(prel, "Arrivee")).iterator();
				while (ncIt.hasNext()) {
				...
				nonConf.setTooltiptext(ObjectTypesFormatters
