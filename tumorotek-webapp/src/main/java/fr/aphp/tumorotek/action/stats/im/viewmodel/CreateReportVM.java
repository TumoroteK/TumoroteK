 ...
 *
 */
public class CreateReportVM extends AbstractListGridVM {

	protected static Log log = LogFactory.getLog(CreateReportVM.class);
	...
	private Map<Indicateur, ArrayList<ValueToExport>> dataMap = 
			new HashMap<Indicateur, ArrayList<ValueToExport>>();

	
	@Command
	@NotifyChange("selectedModel")
	public void generateReport(@BindingParam("date1") java.util.Date date1,
			@BindingParam("date2") java.util.Date date2) {
	...
	public void setPercentDisplay(Boolean _p) {
		this.percentDisplay = _p;
	}
	
	
	@Command
	@NotifyChange({"percentDisplay",
		"switchPercentClass"
	})
	...
	public Validator getDateValidator() {
		return new AbstractValidator() {
			public void validate(ValidationContext ctx) {
				Date dateDebut = (Date) ctx.getProperty().getValue();// the main
																	// property
				Date dateFin = (Date) ctx.getProperties("dateFin")[0]
						.getValue();
				
				if (!isValidDate(dateDebut, dateFin)) {
					addInvalidMessage(ctx, "dates",
							Labels.getLabel("stats.report.dates.period.illegal"));
				}
			}
	...
	public void setSwitchPercentClass(String _s) {
	...
	public void downloadDataAsExcel() {
		...
		HSSFCellStyle intTotStyle = wb.createCellStyle();
		HSSFDataFormat format = wb.createDataFormat();
		intTotStyle.setDataFormat(format.getFormat("00"));
		
		HSSFCellStyle nbStyle = wb.createCellStyle();
		nbStyle.setDataFormat(format.getFormat("00.00"));
		
		List<CellRangeAddress> spans = new ArrayList<CellRangeAddress>();
		...
		for (Indicateur indic : getGridIndicateurs()) {
			cell = row.createCell(currColl);
			cell.setCellType(Cell.CELL_TYPE_STRING);
		...
		// data
		int i = 0;
		for (StatResultsRow data : getRows()) {
			row = sheet.createRow(currentRow);
			currentRow++;
			currColl = 0;
			// banque
			...
			if (!percentDisplay) {
				...
				for (Number n : data.getValuesPourcentage()) {
					cell = row.createCell(currColl);
					cell.setCellType(Cell.CELL_TYPE_NUMERIC);
					cell.setCellValue(n.doubleValue());
			...
			if (data.getFirstSubdivForBanque() && data.getRowspan() > 1) {
				CellRangeAddress spanAddr = new CellRangeAddress(i+1, i+data.getRowspan(), 0, 0);
				spans.add(spanAddr);
			}
		...
		try {
			...
			String date = 
				new SimpleDateFormat("yyysyMMddHHmm").format(cal.getTime());
			AMedia media = new AMedia(getSelectedModel().getNom() 
