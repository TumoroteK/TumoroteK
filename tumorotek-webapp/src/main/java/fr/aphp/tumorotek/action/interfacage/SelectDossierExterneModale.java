...
public class SelectDossierExterneModale extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(SelectDossierExterneModale.class);
	...
	private List<DossierExterne> dossierExternes = 
		new ArrayList<DossierExterne>();
	private DossierExterne selectedDossierExterne;
	private DossierExterneRenderer dossierExterneRenderer = 
		new DossierExterneRenderer();
	...
	public void searchForDossierExternes() {
		...
		List<Emetteur> emetteurs = SessionUtils
			.getEmetteursInterfacages(sessionScope);
		
		// Set intermediaire pour éviter doublons
		Set<DossierExterne> dosSet = new HashSet<DossierExterne>();
		
		dosSet.addAll(ManagerLocator.getDossierExterneManager()
			.findByEmetteurInListAndIdentificationManager(
					emetteurs, numDossier));
		if (prelevement != null) {
			dosSet.addAll(ManagerLocator.getDossierExterneManager()
					.findByEmetteurInListAndIdentificationManager(
							emetteurs, prelevement.getNumeroLabo()));
		}
		
		dossierExternes.addAll(dosSet);
		Collections.sort(dossierExternes, new Comparator<DossierExterne>() {
	        public int compare(DossierExterne o1, DossierExterne o2) {	     
		...
		// view
		DossierExterne dExt;
		for (Emetteur emet : emetteurs) {
			dExt = ManagerLocator.getViewHandlerFactory()
					.sendQuery(emet, numDossier);
			if (dExt != null) {
				dossierExternes.add(dExt);
			}
			if (prelevement != null) {
				dExt = ManagerLocator.getViewHandlerFactory()
						.sendQuery(emet, prelevement.getNumeroLabo());
				if (dExt != null) {
					dossierExternes.add(dExt);
				}
			}
	...
	public void onSelect$dossiersBox(Event event) {
		deselectRow();
	...
	public void deselectRow() {
		...
		if (getCurrentDossierExterne() != null && getCurrentIten() != null) {
			int ind = dossierExternes.indexOf(getCurrentDossierExterne());
	 ...
	 */
	public void selectRow(Listitem item, DossierExterne doss) {
		setCurrentIten(item);
	...
	public void onLaterSelect() {
		...
		if (!edit) {
			try {
				ResultatInjection resultat = ManagerLocator
					.getInjectionManager()
					.injectDossierManager(
						selectedDossierExterne, 
				...
				if (Path.getComponent(path) != null) {
					if (resultat != null) {
						...
						Events.postEvent(
							new Event("onGetInjectionDossierExterneDone", 
				...
				// ferme wait message
				Clients.clearBusy();
				Messagebox.show(Labels.getLabel("erreur.interface"), 
			...
			openImportDossierExterneWindow(
					page, path, prelevement, selectedDossierExterne);
		}
	}
	 ...
	 */
	public void openImportDossierExterneWindow(Page page, 
			String p,
			Prelevement prlvt,
		 ...
		 if (!isBlockModal()) {
			...
			win.setSizable(true);
			win.setTitle(ObjectTypesFormatters.getLabel(
					"import.dossierExternes.title", 
					new String[]{prelevement.getCode()}));
			...
			final HtmlMacroComponent ua = populateImportDossierExterneModal(
					win, page, p, prelevement, selectedDossierExterne);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateImportDossierExterneModal(
		...
		page.getComponentDefinition("importDossierExterneModale", false)
		...
		((ImportDossierExterneModale) 
				ua.getFellow("fwinImportDossierModale")
				.getAttributeOrFellow(
						"fwinImportDossierModale$composer", true))
				.init(p, prlvt, dossier);
	...
	public void setNumDossier(String n) {
		this.numDossier = n;
	}
	...
	public void setDossierExternes(List<DossierExterne> d) {
	...
	public void setSelectedDossierExterne(DossierExterne s) {
	...
	public void setDossierExterneRenderer(
			DossierExterneRenderer renderer) {
	...
	public void setCurrentIten(Listitem c) {
		this.currentIten = c;
	}
	...
	public void setCurrentDossierExterne(DossierExterne c) {
