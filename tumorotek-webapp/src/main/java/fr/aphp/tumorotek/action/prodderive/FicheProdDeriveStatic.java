...
public class FicheProdDeriveStatic extends AbstractFicheStaticController {
	...
	public void setObject(TKdataObject e) {
		...
		if (isAnonyme()) {
			AbstractController.makeLabelAnonyme(
					patientLabelDerive, false);
		} else {
	...
	public void showParentInformation() {
		...
		if (typeParent.equals("Prelevement")) {
			groupParent
				.setLabel(Labels.getLabel("ficheProdDerive.prelevement.titre"));
			...
			groupParent
				.setLabel(Labels.getLabel("ficheProdDerive.echantillon.titre"));
			for (int i = 0; i < objLabelsPrlvtParent.length; i++) {
			...
			groupParent
				.setLabel(Labels.getLabel("ficheProdDerive.prodderive.titre"));
			for (int i = 0; i < objLabelsPrlvtParent.length; i++) {
				objLabelsPrlvtParent[i].setVisible(false);
				objLabelsEchanParent[i].setVisible(false);
	...
	public void prepareDeleteObject() {
		boolean isUsed = ManagerLocator
			.getProdDeriveManager().isUsedObjectManager(getObject());
		boolean isCessed = ManagerLocator
			.getProdDeriveManager().isCessedObjectManager(getObject());
		...
		if (!isCessed) {
			if (isUsed) {
				setDeleteMessage(Labels
						.getLabel("derive.deletion.isUsedCascade"));
			}
		} else { //objet non supprimable car cédé
			setDeleteMessage(Labels
					.getLabel("derive.deletion.isUsedNonCascade"));
		}
	...
	public String getDatePrelevementFormated() {
		if (getParentPrlvt() != null) {
			return ObjectTypesFormatters
				.dateRenderer2(getParentPrlvt().getDatePrelevement());
		} else {
			return null;
		}
	}
		...
		if (getParentEchantillon() != null) {
			if (isAnonyme()) {
				...
				return ManagerLocator.getEchantillonManager()
					.getEmplacementAdrlManager(getParentEchantillon());
			}
		} else {
			return null;
		}
		...
		if (getParentProdDerive() != null) {
			if (isAnonyme()) {
				...
				return ManagerLocator.getProdDeriveManager()
					.getEmplacementAdrlManager(getParentProdDerive());
			}
		} else {
			return null;
		}
	...
	public String getDateCongelationFormated() {
		if (this.prodDerive != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.prodDerive.getDateStock());
		} else {
			return null;
		}
	}
	...
	public String getDateTransformationFormated() {
		if (this.prodDerive != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.prodDerive.getDateTransformation());
		} else {
			return null;
		}
	}
	...
	public void onOpen$groupDerivesDerive() {
		String id = groupDerivesDerive.getUuid();
	...
	public void onOpen$groupCessionsDerive() {
		String id = groupCessionsDerive.getUuid();
		String idTop = panelChildrenWithScroll.getUuid();
		Clients.evalJavaScript("document.getElementById('" + idTop + "')" 
				+ ".scrollTop = document.getElementById('" + id + "')" 
				+ ".offsetTop;");	
	...
	public void onClick$prlvtLinkDerive(Event event) throws Exception {
		if (getDroitsConsultation().get("Prelevement")) {
			...
			tabController
				.switchToFicheStaticMode((Prelevement) getParentObject());	
		}
	}
	...
	public void onClick$echanLinkDerive(Event event) throws Exception {
		if (getDroitsConsultation().get("Echantillon")) {
			...
			tabController.switchToFicheStaticMode((Echantillon) 
													getParentObject());
		}
	}
	 ...
	 * @throws Exception
	 */
	public void onClick$deriveLinkDerive(Event event) throws Exception {
		getObjectTabController().switchToFicheStaticMode((ProdDerive) 
													getParentObject());	
	}
	...
	public void onClickProdDeriveCode(Event event) {
		if (event != null) {
			...
			getObjectTabController().getListe()
				.getBinder().loadAttribute(getObjectTabController().getListe()
		...
		if (this.prodDerive != null) {
			...
			if (!getObjStatutIncompatibleForRetour(objs, "TRANSFORMATION")) {
				getObjectTabController()
					.switchToCreateMode(this.prodDerive);		
			}
		}
	}
		...
		if (getDroitsConsultation().get("Patient")) {
			...
			if (prodDerive.getProdDeriveId() != null) {
				prlvt = ManagerLocator.getProdDeriveManager()
					.getPrelevementParent(prodDerive);
			}
			Maladie maladie = prlvt.getMaladie();
	...
	public void applyDroitsOnFiche() {
		...
		if (canStockage) {
			emplacementLabelDerive.addForward("onClick", self, 
									"onClickObjectEmplacementFromFiche");
		} else {
			emplacementLabelDerive.removeForward("onClick", self, 
									"onClickObjectEmplacementFromFiche");
		}
...
//		setDroitsConsultation(drawConsultationLinks(entites));
			...
			if (this.prodDerive.getConformeTraitement() != null) {
				sb.append(ObjectTypesFormatters
					.booleanLitteralFormatter(
							this.prodDerive.getConformeTraitement()));
	...
	public String getConformeCessionFormated() {
		...
		if (this.prodDerive != null) {
			if (this.prodDerive.getConformeCession() != null) {
				sb.append(ObjectTypesFormatters
					.booleanLitteralFormatter(
							this.prodDerive.getConformeCession()));
	...
	public void setSortiesGroupHeader(String s) {
		...
		try {
			...
			if (getObjectTabController() != null) {
				if (getObjectTabController().getListe() != null) {
					...
					if (getObjectTabController().getListe()
					...
					getObjectTabController().getListe().
						removeObjectFromList(getObject());
				}
			}
