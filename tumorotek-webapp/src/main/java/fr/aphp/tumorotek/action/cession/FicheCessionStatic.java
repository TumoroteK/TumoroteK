...
public class FicheCessionStatic extends AbstractFicheStaticController {
	...
	private String echantillonsGroupHeader = Labels
			.getLabel("cession.echantillons");
	private String derivesGroupHeader = Labels.getLabel("cession.prodDerive");
	
	// @since 2.1 checked Objects
	private List<Integer> checkedEchantillonIds = new ArrayList<Integer>();
	private List<Integer> checkedDeriveIds = new ArrayList<Integer>();
	...
	public void setObject(TKdataObject c) {
		...
		if (this.cession.getCessionType() != null) {
			if (this.cession.getCessionType().getType().toUpperCase()
					.equals("SANITAIRE")) {
				switchToSanitaireMode();
			} else if (this.cession.getCessionType().getType().toUpperCase()
					.equals("RECHERCHE")) {
		...
		printCessionPlan.setDisabled(cession.getCessionStatut() == null 
			 || cession.getCessionStatut().getStatut().equals("VALIDEE") 
	...
	public void removeObject(String comments) {
		...
		try {
			ManagerLocator.getCessionManager().removeObjectManager(getObject(),
					comments, SessionUtils.getLoggedUser(sessionScope), filesToDelete);
			
			// update de la liste des enfants et l'enfant en fiche
			getObjectTabController().updateReferencedObjects(
											revertAllCessedObjects());
	...
	public void onLaterDelete(Event event) {
		try {
			Contrat cont = getObject().getContrat();
			...
			if (getObjectTabController() != null) {
				if (getObjectTabController().getListe() != null) {
					...
					getObjectTabController().getListe().removeObjectAndUpdateList(
							getObject());
				}
	...
	public String getDateValidationFormated() {
		if (this.cession != null) {
			return ObjectTypesFormatters.dateRenderer2(this.cession
					.getValidationDate());
		} else {
			return null;
		}
	...
	public String getSClassEtablissement() {
		if (this.cession != null && this.cession.getServiceDest() != null
				&& this.cession.getServiceDest().getEtablissement() != null) {
			return ObjectTypesFormatters.sClassEtablissement(this.cession
					.getServiceDest().getEtablissement());
		} else {
			return "";
		}
	}
	...
	public String getSClassDemandeur() {
		if (this.cession != null) {
			return ObjectTypesFormatters.sClassCollaborateur(this.cession
					.getDemandeur());
		} else {
			return "";
		}
	...
	public String getSClassServiceDest() {
		if (this.cession != null) {
			return ObjectTypesFormatters.sClassService(this.cession
					.getServiceDest());
		} else {
			return "";
		}
	...
	public void onSelectAllEchantillons(Event e) {
		List<CederObjetDecorator> decos = 
				cdEchansFactory.decorateListe(echantillonsCedes);
		displayObjectsListData(new 
			ArrayList<TKAnnotableObject>(cdEchansFactory.undecorateListe(decos)));
	}
	...
	public void onSelectAllDerives(Event e) {
		List<CederObjetDecorator> decos = 
				cdDerivesFactory.decorateListe(derivesCedes);
		displayObjectsListData(new 
			ArrayList<TKAnnotableObject>(cdDerivesFactory.undecorateListe(decos)));
	}
	...
	public void onClick$printCessionPlan() {
		Document doc = CessionUtils.createFileHtmlToPrint(cession, 
	...
	public void onClick$exportItem() throws ClassNotFoundException, NoSuchMethodException, SecurityException, InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
	...
	public void onLaterExportEchantillon() throws ClassNotFoundException,
		...
		Object o = constr.newInstance(desktop, ConfigManager.ENTITE_ID_ECHANTILLON, 
				objsIds, SessionUtils.getSelectedBanques(sessionScope),
				isExportAnonyme(), ConfigManager.DEFAULT_EXPORT, 
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void onLaterExportDerives() throws ClassNotFoundException,
			NoSuchMethodException, SecurityException, InstantiationException,
			IllegalAccessException, IllegalArgumentException,
			InvocationTargetException {
		...
		for (CederObjet cObj : derivesCedes) {
			objsIds.add(cObj.getObjetId());
		}
		...
		Constructor<?> constr = exportThread.getConstructor(Desktop.class,
				int.class, List.class, List.class, boolean.class, short.class,
				Utilisateur.class, List.class, HtmlMacroComponent.class, Map.class);
		Object o = constr.newInstance(desktop, ConfigManager.ENTITE_ID_DERIVE, 
				objsIds, SessionUtils.getSelectedBanques(sessionScope),
				isExportAnonyme(), ConfigManager.DEFAULT_EXPORT, 
				SessionUtils.getLoggedUser(sessionScope), 
				getObjectTabController().getListe().getRestrictedTableIds(), callProgressBar(), null);
...
//		}
		...
		if (admin) {
			...
			if (sessionScope.containsKey("Droits")) {
				...
				if ((export.equals("Export") || export.equals("ExportAnonyme"))
						&& getDroitsConsultation().get("Echantillon")) {
					setCanExportEchantillons(true);
				} else {
...
//	}
	...
	private List<TKdataObject> revertAllCessedObjects() {
		...
		if (!getCession().getCessionStatut().getStatut().equals("REFUSEE")) {
			...
			for (Object deco : cdEchansFactory.decorateListe(echantillonsCedes)) {
			...
			for (Object deco : cdDerivesFactory.decorateListe(derivesCedes)) {
	...
	public void setCanExportEchantillons(boolean c) {
	...
	public void setCanExportDerives(boolean c) {
	...
	public void setExportAnonyme(boolean a) {
	...
	public String getCessionType() {
		if (cession != null && cession.getCessionType() != null) {
			return Labels.getLabel("cession.type."
					+ cession.getCessionType().getType().toLowerCase());
		} else {
			return null;
		}
	}
	...
	private void refreshModelE(int activePage){
        ...
        echansModel = new ObjectPagingModel(activePage, _pageSizeE, 
        						echantillonsCedes, cdEchansFactory, null);
	...
	private void refreshModelP(int activePage){
		...
		derivesModel = new ObjectPagingModel(activePage, _pageSizeP, 
				derivesCedes, cdDerivesFactory, null);
	 ...
	 * @param sT
	 */
	public void applyTKObjectsCheckFromScan(ScanTerminale sT) {
		List<String> codes = ManagerLocator.getScanTerminaleManager()
				.findTKObjectCodesManager(sT);
	
		// check echantillon
		List<String> falseCkeckEchantillonCodes = new ArrayList<String>();
		CederObjet mockCObj = new CederObjet();
		CederObjetPK mockPk = new CederObjetPK();
		mockCObj.setPk(mockPk);
		Entite echanEntite = ManagerLocator.getEntiteManager().findByIdManager(3);
		boolean upCedesEchans = false;
		for (Integer eId : ManagerLocator.getEchantillonManager()
		...
		// check derives
		List<String> falseCkeckDeriveCodes = new ArrayList<String>();
		Entite deriveEntite = ManagerLocator.getEntiteManager().findByIdManager(8);
		boolean upCedesDerives = false;
		for (Integer pId : ManagerLocator.getProdDeriveManager()
				.findByCodeInListManager(codes,
						SessionUtils.getSelectedBanques(sessionScope), new ArrayList<String>())) {
			...
			if (!getDerivesCedes().contains(mockCObj)) {
				falseCkeckDeriveCodes.add(ManagerLocator
							.getProdDeriveManager().findByIdManager(pId).getCode());
		...
		// notifications!
		// complete check!!
		if (getCession().getLastScanCheckDate() == null 
				&& getCheckedEchantillonIds().size() == getEchantillonsCedes().size()
				&& getCheckedDeriveIds().size() == getDerivesCedes().size()) {
			Clients.showNotification(Labels.getLabel("scan.cession.objects.complete"), 
					"info", null, null, 2000, true);
			ManagerLocator.getCessionManager()
				.applyScanCheckDateManager(getCession(), sT.getDateScan());
		...
		if (!falseCkeckEchantillonCodes.isEmpty()
				|| !falseCkeckDeriveCodes.isEmpty()) {
			
			StringBuilder codesDetails = new StringBuilder();
			// echans
			if (!falseCkeckEchantillonCodes.isEmpty()) {
				codesDetails.append(Labels.getLabel("Entite.Echantillon.pluriel"));
				codesDetails.append(": ");
				Iterator<String> cIt = falseCkeckEchantillonCodes.iterator();
				while (cIt.hasNext()) {
			...
			if (!falseCkeckDeriveCodes.isEmpty()) {
				...
				codesDetails.append(": ");
				Iterator<String> cIt = falseCkeckDeriveCodes.iterator();
				while (cIt.hasNext()) {
					codesDetails.append(cIt.next());
					if (cIt.hasNext()) {
						codesDetails.append(", ");
