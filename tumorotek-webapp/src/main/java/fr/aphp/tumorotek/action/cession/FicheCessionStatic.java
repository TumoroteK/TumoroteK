...
public class FicheCessionStatic extends AbstractFicheStaticController
{
   ...
   public void setObject(final TKdataObject c){
      ...
      storageRobotItem.setVisible(storageRobotItemVisible() && getObject() != null && getObject().getCessionStatut() != null
   ...
   public void onClickProduitRetourList(final Event event){
      final CederObjetDecorator cederObjetDeco =
   ...
   public void onMouseOverProduitRetourList(final Event event){
      final CederObjetDecorator cederObjetDeco =
         (CederObjetDecorator) AbstractListeController2.getBindingData((ForwardEvent) event, false);
      final Integer prodRetourCount = cederObjetDeco.getCederObjet().getProduitRetourList().size();
      if(prodRetourCount > 0){
         if(prodRetourCount < 50){
            ...
            for(final ProdDerive prodDeriveRetour : cederObjetDeco.getCederObjet().getProduitRetourList()){
    ...
    * @since 2.2.0
    */
   public void onClick$retourBtnEch(final Event event){
      onClickRetour(event);
   }
    ...
    * @since 2.2.0
    */
   public void onClick$retourBtnProdDerive(final Event event){
      onClickRetour(event);
   }

   private void onClickRetour(final Event event){
      final CederObjetDecorator objetCede =
         (CederObjetDecorator) AbstractListeController2.getBindingData((ForwardEvent) event, false);
      final ProdDeriveController prodDeriveController =
         (ProdDeriveController) ProdDeriveController.backToMe(getMainWindow(), page);
    ...
    * @since 2.2.0
    */
   public void onClick$cancelRetourBtnEch(final Event event){
    ...
    * @since 2.2.0
    */
   public void onClick$cancelRetourBtnProdDerive(final Event event){
      onClickCancelRetour(event);
   }

   private void onClickCancelRetour(final Event event){
      final CederObjetDecorator objetCede =
         (CederObjetDecorator) AbstractListeController2.getBindingData((ForwardEvent) event, false);
      objetCede.getCederObjet().setStatut(ECederObjetStatut.ANNULE);
      ManagerLocator.getCederObjetManager().updateObjectManager(objetCede.getCederObjet(), cession,
         objetCede.getCederObjet().getEntite(), objetCede.getCederObjet().getQuantiteUnite());
   ...
   public void onLaterExportEchantillon() throws ClassNotFoundException, NoSuchMethodException, SecurityException,
      ...
      final EContexte contexte = SessionUtils.getCurrentContexte();

      final ProfilExport pExport = getObjectTabController().getListe().askForNonAnonymizationStatut();
   ...
   public void onLaterExportDerives() throws ClassNotFoundException, NoSuchMethodException, SecurityException,
      ...
      getObjectTabController().enregistrerValeursChampsCalculesPourExport(
         ManagerLocator.getEntiteManager().findByNomManager("ProdDerive").get(0), objsIds,
         getObjectTabController().getListe().getRestrictedTableIds());
   ...
   public Boolean getIsCessionTraitementAndValide(){
      if(null != this.cession && null != this.cession.getCessionType()){
         return "Traitement".equals(this.cession.getCessionType().getType())
            && this.cession.getCessionStatut().getStatut().equals("VALIDEE");
      }
      return false;
   }
