...
public class ChangeNumerotationModale extends GenericForwardComposer<Component> {
	...
	private List<Terminale> terminales = new ArrayList<Terminale>();
	...
	public void init(Object parent, MainWindow mw) {
		main = mw;
		if (parent != null) {
			// récupération des terminales enfants
			if (parent instanceof Conteneur) {
				conteneur = (Conteneur) parent;
				terminales = ManagerLocator.getConteneurManager()
					.getAllTerminalesInArborescenceManager(conteneur);
			} else if (parent instanceof Enceinte) {
				enceinte = (Enceinte) parent;
				terminales = ManagerLocator.getEnceinteManager()
		...
		if (terminales.size() > 0) {
			...
			if (conteneur != null) {
				warnLabel.setContent(ObjectTypesFormatters
					.getLabel("terminale.numerotation.warn.conteneur",
						new String[]{conteneur.getCode(),
							String.valueOf(terminales.size())}));
			} else {
				warnLabel.setContent(ObjectTypesFormatters
						.getLabel("terminale.numerotation.warn.enceinte",
							new String[]{enceinte.getNom(),
								String.valueOf(terminales.size())}));
			}
		} else {
			stopImg.setVisible(true);
			warnLabel.setContent(Labels.getLabel(
					"terminale.numerotation.stop"));
		...
		numerotationsBox.setModel(new SimpleListModel<TerminaleNumerotation>(numerotations));
		if (!numerotations.isEmpty()) {
			numerotationsBox.setSelectedItem(
					numerotationsBox.getItemAtIndex(0));
		}
	}
	...
	public void onClick$validate() {
		if (numerotationsBox.getSelectedItem() != null) {
			selectedNumerotation = (TerminaleNumerotation) 
				numerotationsBox.getSelectedItem().getValue();
		} else {
	...
	public void onLaterUpdate() {
		try {
			...
			ManagerLocator.getTerminaleManager()
				.updateNumerotationForMultiTerminales(
						terminales, selectedNumerotation, 
			...
			getMain().updateSelectedBanque(SessionUtils
	...
	public void setTerminales(List<Terminale> t) {
		this.terminales = t;
	}
	...
	public void setNumerotations(List<TerminaleNumerotation> n) {
	...
	public void setSelectedNumerotation(TerminaleNumerotation s) {
