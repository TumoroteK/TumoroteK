...
public class ChangeBanqueModale extends GenericForwardComposer<Component> {
	...
	public void init(Prelevement[] prlvts, MainWindow mw) {
		prelevements = prlvts;
		main = mw;
		prlvtsGrid.setModel(new ListModelList<Prelevement>(prelevements));
	...
	private void setObjectsFromPrelevement(Prelevement p) {
		// nettoie
		echans.clear();
		derives.clear();
		
		Iterator<TKAnnotableObject> childIt = ManagerLocator
				.getPrelevementManager().getPrelevementChildrenManager(p)
				.iterator();
		...
		echantillonsGrid.setModel(new ListModelList<Echantillon>(echans));
		...
		derivesGrid.setModel(new ListModelList<ProdDerive>(derives));

		// affiche le warning sur les emplacements ou non
		emplWarnLabel.setVisible(displayEmplWarning());

		// banques
		banques = ManagerLocator.getBanqueManager().findBanqueForSwitchManager(
				p, SessionUtils.getLoggedUser(sessionScope));
		banquesBox.setModel(new SimpleListModel<Banque>(banques));
		if (!banques.isEmpty()) {
	...
	public void onLaterUpdate() {
		...
		if (banquesBox.getSelectedItem() != null) {
			...
			// ferme wait message
			Clients.clearBusy();
			throw new WrongValueException(banquesBox,
					Labels.getLabel("fichePrelevement.switchBanque"
							+ ".banquesCibles.error"));
		}
		...
		ManagerLocator.getPrelevementManager()
				.switchBanqueMultiplePrelevementManager(prelevements, dest,
						true, SessionUtils.getLoggedUser(sessionScope));
	...
	public void setPrelevements(Prelevement[] prelevements) {
	 ...
	 */
	public void onClickPrel(Event e) {
...
class MyGridRenderer implements RowRenderer<Prelevement> {
	...
	@Override
	public void render(Row row, Prelevement p, int index) throws Exception {

		Label codeLbl = new Label(p.getCode());
