 ...
 *
 */
public class ModificationMultipleFile 
	...
	public Object extractValueFromEraserBox() {
		if (file != null && fileEraseBox.getValue() != null
				&& !fileEraseBox.getValue().equals("")) { 
	...
	public Object extractValueFromMultiBox() {
		if (file != null && fileUniqueBox.getValue() != null
				&& !fileUniqueBox.getValue().equals("")) { 
		...
		for (int i = 0; i < getListObjets().size(); i++) {
			try {
				...
				if (tmp != null && !tmp.equals("")) {
					if (!paths.contains(((Fichier) tmp).getPath())
						|| !getStringValues().contains(((Fichier) tmp).getNom())) {	
						paths.add(((Fichier) tmp).getPath());
						getValues().add(tmp);
	...
	public AnnotateDataBinder getBinder() {
		return ((AnnotateDataBinder) self
				.getParent()
				.getAttributeOrFellow("modificationFile", true));
	}	
	...
	public void onClick$addUniqueFile() {
		...
		if (medias != null && medias.length > 0) {
			...
			getCrAnapathConstraint()
				.validate(fileUniqueBox, medias[0].getName());
		}		
	}
	 ...
	 */
	public void showDeleteAndFileUniqueBox(boolean visible) {
		fileUniqueBox.setVisible(visible);
	}
	
	public void onBlur$fileUniqueBox() {
		getCrAnapathConstraint()
		.validate(fileUniqueBox, fileUniqueBox.getValue());
	}
	...
	public void onClick$addEraseFile() {
		...
		if (medias != null && medias.length > 0) {
			...
			getCrAnapathConstraint()
				.validate(fileEraseBox, medias[0].getName());
		}		
	}
	 ...
	 * @param boolean visible
	 */
	public void showDeleteAndFileEraseBox(boolean visible) {
		fileEraseBox.setVisible(visible);
	}
	
	public void onBlur$fileEraseBox() {
		getCrAnapathConstraint()
		.validate(fileEraseBox, fileUniqueBox.getValue());
	}
	...
	public void onClick$validate() {
		...
		if ((getNewValue() != null 
			&& ((getOldUniqueValue() != null
				&& !((Fichier) getNewValue()).getNom()
				.equals(((Fichier) getOldUniqueValue()).getNom())) 
					|| getHasNulls())) 
