...
public class EchantillonRowRenderer extends TKSelectObjectRenderer {
	...
	private Log log = LogFactory.getLog(EchantillonRowRenderer.class);
	...
	public EchantillonRowRenderer(boolean select, boolean cols) {
	...
	public void render(Row row, Object data, int index)  {
		...
		Echantillon echan = (Echantillon) data;
		
		Hlayout icones = TKStockableObjectUtils.drawListIcones(echan, null, null);
		
		//infectieux
		Prelevement prel = ManagerLocator.getEchantillonManager()
		...
		if (prel != null) {
			...
			if (risky) {
				...
				bioHzd.setTooltiptext(ObjectTypesFormatters
					.getLabel("tooltip.risque", new String[]{risks}));				
			}
		...
		// code
		Label codeLabel = new Label(echan.getCode());
		if (accessible) {
		...
		new Label(ObjectTypesFormatters
				.dateRenderer2(echan.getDateStock())).setParent(row);
		log.debug("date");
		...
		ObjectTypesFormatters.drawCodesExpLabel(ManagerLocator
			.getCodeAssigneManager()
				.findCodesMorphoByEchantillonManager(echan), row, null, false);
		...
		if (!anonyme && accessStockage) {
			Label emplLabel = new Label(getEmplacementAdrl(echan));
			if (accessStockage) {
	 ...
	 * @return String.
	 */
	public String getQuantite(Echantillon echantillon) {
	...
	public String getEmplacementAdrl(Echantillon echantillon) {
	...
	public String getDelaiCgl(Echantillon echantillon) {
		StringBuffer sb = new StringBuffer();
		String heureLabel = "";
		String minLabel = "";
		if (echantillon.getDelaiCgl() != null 
				&& echantillon.getDelaiCgl() > -1) {
			Float heure = echantillon.getDelaiCgl() / 60;
	...
	public static int getNbDerives(Echantillon echantillon) {
		return ManagerLocator.getEchantillonManager()
			.getProdDerivesManager(echantillon).size();
	}
	
	public static int getNbCessions(Echantillon echantillon) {
		return ManagerLocator.getCederObjetManager()
			.findByObjetManager(echantillon).size();
	}
	 ...
	 */
	public String getDateCreation(Echantillon echantillon) {
...
//	}
	...
	public String getCodeAssigneInString(Echantillon echantillon,
			boolean isOrg) {
		List<CodeAssigne> codes;
		if (isOrg) {
			codes = ManagerLocator.getCodeAssigneManager()
	...
	public String getPatient(Echantillon echan) {
