...
public class ImprimanteController extends AbstractObjectTabController {
	...
	private Log log = LogFactory.getLog(ImprimanteController.class);
	...
	private static ImprimanteRowRenderer imprimanteRenderer = 
			new ImprimanteRowRenderer();
	private static ModeleRowRenderer modeleRenderer = 
		new ModeleRowRenderer();
	...
	public void initAssociations() {
		imprimantes = ManagerLocator.getImprimanteManager()
			.findByPlateformeManager(
					SessionUtils.getPlateforme(sessionScope));
		imprimantesGroup.setLabel(ObjectTypesFormatters
				.getLabel(
				"imprimante.controller.imprimantes.group", 
						new String[]{String.valueOf(imprimantes.size())}));
		
		modeles = ManagerLocator.getModeleManager()
			.findByPlateformeManager(
				SessionUtils.getPlateforme(sessionScope));
		modelesGroup.setLabel(ObjectTypesFormatters
				.getLabel(
				"imprimante.controller.modeles.group", 
	...
	public void onClickImprimante(ForwardEvent event) {
	...
	public void onClickModele(ForwardEvent event) {
		if (event.getData() != null) {
			Modele mod = (Modele) event.getData();
	...
	public void onGetImprimanteDone(Event e) {
		imprimantes = ManagerLocator.getImprimanteManager()
			.findByPlateformeManager(
				SessionUtils.getPlateforme(sessionScope));
		imprimantesGroup.setLabel(ObjectTypesFormatters
	 ...
	 * @param event
	 */
	public void onClickDeleteImprimante(ForwardEvent event) {
		Imprimante imp = (Imprimante) event.getData();
		
		if (imp != null) {
			// confirmation
			if (Messagebox.show(Labels.getLabel(
					"imprimante.controller.delete.imprimante"),
			Labels.getLabel("message.deletion.title"), 
			Messagebox.YES | Messagebox.NO, 
			Messagebox.QUESTION) == Messagebox.YES) {
				// suppression
				ManagerLocator.getImprimanteManager()
					.removeObjectManager(imp);
				// maj
				imprimantes = ManagerLocator.getImprimanteManager()
					.findByPlateformeManager(
						SessionUtils.getPlateforme(sessionScope));
				imprimantesGroup.setLabel(ObjectTypesFormatters
					...
					"imprimante.controller.imprimantes.group", 
							new String[]{
						String.valueOf(imprimantes.size())}));
	...
	public void onGetModeleDone(Event e) {
		modeles = ManagerLocator.getModeleManager()
			.findByPlateformeManager(
				SessionUtils.getPlateforme(sessionScope));
		modelesGroup.setLabel(ObjectTypesFormatters
	 ...
	 * @param event
	 */
	public void onClickDeleteModele(ForwardEvent event) {
		Modele mod = (Modele) event.getData();
		
		if (mod != null) {
			// confirmation
			if (Messagebox.show(Labels.getLabel(
					"imprimante.controller.delete.modele"),
			Labels.getLabel("message.deletion.title"), 
			Messagebox.YES | Messagebox.NO, 
			Messagebox.QUESTION) == Messagebox.YES) {
				// suppression
				ManagerLocator.getModeleManager()
					.removeObjectManager(mod);
				// maj
				modeles = ManagerLocator.getModeleManager()
					.findByPlateformeManager(
					SessionUtils.getPlateforme(sessionScope));
	...
	public void onOpen$affectationsGroup() {
		if (affectationsGroup.isOpen()) {
			...
			// utilisateurs actifs
			List<Utilisateur> users = ManagerLocator
			...
			for (int i = 0; i < users.size(); i++) {
				...
				List<Banque> bks = ManagerLocator.getBanqueManager()
					.findByUtilisateurAndPFManager(users.get(i), 
							SessionUtils.getPlateforme(sessionScope));
		...
		try {
			...
			List<AffectationImprimante> liste = ManagerLocator
				...
				.findByBanqueUtilisateurManager(
						aff.getBanque(), aff.getUtilisateur());
			if (liste.size() > 0) {
				ai = liste.get(0);
			} else {
	...
	public void openImprimanteWindow(Page page,
			Component comp, Imprimante imp) {
		 if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua = populateFicheImprimanteModal(
					win, page, comp, imp);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateFicheImprimanteModal(
		...
		page.getComponentDefinition("ficheImprimanteModale", false)
		...
		((FicheImprimanteModale) ua.getFellow("fwinImprimanteModale")
				.getAttributeOrFellow("fwinImprimanteModale$composer", true))
	 ...
	 * @param mod Modèle.
	 */
	public void openModeleWindow(Page page,
			Component comp, Modele mod) {
		 if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua = populateFicheModeleModal(
					win, page, comp, mod);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateFicheModeleModal(
			Window win, Page page,
			Component comp, Modele mod) {
		// HtmlMacroComponent contenu dans la fenêtre : il correspond
		// au composant des collaborations.
		HtmlMacroComponent ua;
		ua = (HtmlMacroComponent)
		page.getComponentDefinition("ficheModeleModale", false)
		.newInstance(page, null);
		ua.setParent(win);
		...
		((FicheModeleModale) ua.getFellow("fwinModeleModale")
				.getAttributeOrFellow("fwinModeleModale$composer", true))
	 ...
	 * @param mod Modèle.
	 */
	public void openModeleDynWindow(Page page,
			Component comp, Modele mod) {
		 if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua = populateFicheModeleDynModal(
					win, page, comp, mod);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateFicheModeleDynModal(
			Window win, Page page,
			Component comp, Modele mod) {
		// HtmlMacroComponent contenu dans la fenêtre : il correspond
		...
		HtmlMacroComponent ua;
		ua = (HtmlMacroComponent)
		page.getComponentDefinition("ficheModeleDynModale", false)
			.newInstance(page, null);
		ua.setParent(win);
		...
		((FicheModeleDynModale) ua.getFellow("fwinModeleDynModale")
				.getAttributeOrFellow("fwinModeleDynModale$composer", true))
				.initImprimanteModale(mod, comp);
	...
	public void setImprimantes(List<Imprimante> i) {
		this.imprimantes = i;
	}
	...
	public void setModeles(List<Modele> m) {
		this.modeles = m;
	}
	...
	public static void setImprimanteRenderer(
			ImprimanteRowRenderer iRenderer) {
	...
	public static void setModeleRenderer(ModeleRowRenderer mRenderer) {
