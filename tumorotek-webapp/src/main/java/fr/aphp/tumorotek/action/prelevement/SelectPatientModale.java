...
public class SelectPatientModale {
	...
	private Log log = LogFactory.getLog(SelectPatientModale.class);
	...
	private List<Patient> patientsSip = new ArrayList<Patient>();
	private Patient selectedPatient;
	private Patient currentPatient;
	private Listitem currentIten;
	private PatientItemRenderer patientRenderer = 
									new PatientItemRenderer(false);
	@Wire
	...
	@Init
	public void init(@ExecutionArgParam("path") String pathToPage, 
					@ExecutionArgParam("methode") String methode,
						@ExecutionArgParam("isFusion") Boolean isFusion,
						@ExecutionArgParam("critere") String critere,
						@ExecutionArgParam("patAExclure") Patient patAExclure) {
	...
	public void searchForPatients() {
		...
		if (!isFusionPatients) {
			...
			if (SipFactory.isDirectSip()) {
				log.info("->Debut traitement recherche de patients " 
						+ "dans serveur d'identités");
				...
				while (found) {
			    	...
			    	found = ManagerLocator.getResourceBundleSip()
			    		.doesResourceBundleExists(nomFichier.toString());
					...
					if (found) {
						...
						if (ManagerLocator.getResourceBundleSip()
								.doesResourceBundleExists(
								"serveur_Identites.properties")) {
							searchPatientsInSip(
								"serveur_Identites.properties");
						}
					}
				...
				Set<PatientSip> listPatientSip = new HashSet<PatientSip>();
				listPatientSip.addAll(ManagerLocator.getPatientSipManager()
						.findByNipLikeManager(critereValue, true));
				listPatientSip.addAll(ManagerLocator.getPatientSipManager()
						.findByNomLikeManager(critereValue, true));
				listPatientSip.addAll(ManagerLocator.getPatientSipManager()
						.findByNumeroSejourManager(critereValue, true));
				
				List<Patient> listPatientTmp = new ArrayList<Patient>();
				Iterator<PatientSip> it = listPatientSip.iterator();
				while (it.hasNext()) {
			...
			if (patientAExclure != null) {
				...
				for (int i = 0; i < patients.size(); i++) {
					if (patients.get(i).getPatientId() != null
							&& patientAExclure.getPatientId() != null
							&& patients.get(i).getPatientId().equals(
								patientAExclure.getPatientId())) {
						idx = i;
					}
				}
	...
	public void searchPatientsInSip(String filePropertiesName) {
		log.info("->Debut traitement recherche de patients " 
				+ "dans serveur d'identités");
		Sip sipActif = SipFactory.getSip(filePropertiesName);
		// recherche par nip
		List<Patient> listPatientTmp = new ArrayList<Patient>();
		...
		// recherche par nom
		listPatientTmp = new ArrayList<Patient>();
		...
		// recherche par nda
		listPatientTmp = new ArrayList<Patient>();
		listPatientTmp = sipActif.getPatientsServeurNumDossier(
				critereValue);
		// ajouts a la liste des patients, en supprimant les
		// doublons de la liste de resultats
		// provenant du serveur d'identite patient.
		fusionerNouvelleListeAvecPrincipale(listPatientTmp);
		log.info("->Fin recherche de patients " 
				+ "dans serveur d'identités");
	}
	
	public void fusionerNouvelleListeAvecPrincipale(List<Patient> newListe) {
	...
	@Command
	public void onSelect$patientsBox(Event event) {
		deselectRow();
...
//	}
	...
	public void deselectRow() {
		...
		if (getCurrentPatient() != null && getCurrentIten() != null) {
			int ind = patients.indexOf(getCurrentPatient());
	 ...
	 */
	public void selectRow(Listitem item, Patient pat) {
		setCurrentIten(item);
	 ...
	 * @return Une Row.
	 */
	public static Listitem getItem(ForwardEvent event) {
...
//				.getAttribute("zkplus.databind.TEMPLATEMAP");
	...
	public boolean isDoublonPatient() {
		if (getCurrentPatient().getPatientId() == null) {
			return ManagerLocator.getPatientManager()
				.findDoublonManager(getCurrentPatient());
		} else {
			return false;
		}
	}
...
//	}
	...
	public void setCritereValue(String c) {
		this.critereValue = c;
	}
	...
	public void setSelectedPatient(Patient sPatient) {
	...
	public void setCurrentIten(Listitem i) {
		this.currentIten = i;
	}
...
//	}
	...
	public void setFusionPatients(Boolean i) {
	...
	public void setReturnMethode(String r) {
	...
	public void setPatientAExclure(Patient p) {
	...
	public void setPatientsSip(List<Patient> p) {
		this.patientsSip = p;
	}
}
