...
public class PrelevementItemRenderer implements ListitemRenderer<Prelevement> {
	...
	public void setFromOtherConsultBanks(List<Banque> oBks) {
		...
		if (getOtherConsultBanks() == null) { // mode otherBank prelevement
			...
			ObjectTypesFormatters.drawCodesExpLabel(ManagerLocator
					.getCodeAssigneManager()
				   	.findFirstCodesOrgByPrelevementManager(prel), null, 
				   												li, true);
			
			// diagnostic
			ObjectTypesFormatters.drawCodesExpLabel(ManagerLocator
					.getCodeAssigneManager()
				   	.findFirstCodesLesByPrelevementManager(prel), null, 
				   												li, true);
		} else {
		//	new Listcell(Labels.getLabel("Contexte." 
		//			+ prel.getBanque().getContexte().getNom())).setParent(li);
			new Listcell(prel.getServicePreleveur() != null ? 
				prel.getServicePreleveur().getEtablissement().getNom() : 
					"").setParent(li);
		}
	...
	public void setOtherConsultBanks(List<Banque> ob) {
