 ...
 *
 */
public class ModificationMultipleTextbox 
	 ...
	 * @param up
	 */
	public void setUpperCaseOnBlur(Boolean up) {
		if (up != null && up) {
			multiTextbox.addEventListener("onBlur", new EventListener<Event>() {
				public void onEvent(Event event) throws Exception {
					multiTextbox
						.setValue(multiTextbox.getValue()
								.toUpperCase().trim());
				}
			});
			eraseMultiTextbox.addEventListener("onBlur", new EventListener<Event>() {
				public void onEvent(Event event) throws Exception {
					eraseMultiTextbox
						.setValue(eraseMultiTextbox.getValue()
								.toUpperCase().trim());
				}
			});
		}	
	...
	public String formatLocalObject(Object obj) {
		...
		if (getIsCombined() && obj != null
				&& ((String) obj).equals("system.tk.unknownExistingValue")) {
	...
	public AnnotateDataBinder getBinder() {
		return ((AnnotateDataBinder) self
				.getParent()
				.getAttributeOrFellow("modificationTextbox", true));
	}
	
	@Override
	public void passValueToEraserBox() {
		eraseMultiTextbox
			.setValue((String) multiListBox.getSelectedItem().getValue());
	}
