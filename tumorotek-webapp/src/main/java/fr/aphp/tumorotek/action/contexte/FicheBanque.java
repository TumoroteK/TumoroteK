...
public class FicheBanque extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(FicheBanque.class);
	...
	private List<String> nomsServices = new ArrayList<String>();
	...
	private List<Catalogue> selectedCatalogues = new ArrayList<Catalogue>();

	private List<ProfilUtilisateur> profilUtilisateurs = 
											new ArrayList<ProfilUtilisateur>();
	private ProfilUtilisateurRowRenderer profilUtilisateurRowRenderer = 
		new ProfilUtilisateurRowRenderer(false);
	private ServiceWithEtablissementRenderer serviceRenderer = 
		new ServiceWithEtablissementRenderer();
	...
	private List<BanqueTableCodage> codifications = 
									new ArrayList<BanqueTableCodage>();
	private List<TableAnnotation> tablesAnnoPat = 
									new ArrayList<TableAnnotation>();
	private List<TableAnnotation> tablesAnnoPrel = 
									new ArrayList<TableAnnotation>();
	private List<TableAnnotation> tablesAnnoEchan = 
										new ArrayList<TableAnnotation>();
	private List<TableAnnotation> tablesAnnoDerive = 
										new ArrayList<TableAnnotation>();	
	private List<TableAnnotation> tablesAnnoCess = 
										new ArrayList<TableAnnotation>();
	private List<CouleurEntiteType> coulTypesEchan = 
											new ArrayList<CouleurEntiteType>();
	private List<CouleurEntiteType> copyCoulTypesEchan = 
											new ArrayList<CouleurEntiteType>();
	private List<CouleurEntiteType> coulTypesDerives = 
											new ArrayList<CouleurEntiteType>();
	private List<CouleurEntiteType> copyCoulTypesDerives = 
											new ArrayList<CouleurEntiteType>();
	
	private I3listBoxItemRenderer contexteRenderer = 
											new I3listBoxItemRenderer("nom");
	private CouleurItemRenderer couleurRenderer = new CouleurItemRenderer();
	private CataloguesRowRenderer catalogueRenderer = 
											new CataloguesRowRenderer();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[]{
				...
				this.respLabel,
				this.contactLabel,
				this.defMaladieLabel,
				this.defautLibLabel,
				this.defautCodeLabel,
				this.autoCrossLabel,
				this.contexteLabel,
				this.couleurEchanLabel,
				this.couleurDeriveLabel,
				this.menuBar,
				this.gridCatalogues, 
				this.archiveLabel
		});
		...
		setObjBoxsComponents(new Component[]{
				...
				this.autoCrossBox,
				this.contexteBox,
				this.couleurEchanBox,
				this.couleurDeriveBox,
				//this.checkCataCol,
				this.cataloguesBox, 
				this.archiveBox
		});
		...
		Executions
			.createComponents("/zuls/contexte/CodificationsAssociees.zul", 
												codificationsAssociees, null);
		getCodificationsAssociees().setGroupHeader(groupCodifications);
		
		Executions
			.createComponents("/zuls/contexte/CoulEntiteTypesAssociees.zul", 
													coulTypesEchanWin, null);
		Executions
			.createComponents("/zuls/contexte/CoulEntiteTypesAssociees.zul", 
												coulTypesDeriveWin, null);
	...
	public void createNewObject() {
		...
		List<TableAnnotation> tabs = ManagerLocator
			.getTableAnnotationManager()
				.findByCataloguesManager(selectedCatalogues);
...
//		} 
		...
		List<CouleurEntiteType> coulTypes = new ArrayList<CouleurEntiteType>();
		coulTypes.addAll(coulTypesEchan);
		coulTypes.addAll(coulTypesDerives);
			
		ManagerLocator.getBanqueManager().createOrUpdateObjectManager(
				banque, 
				SessionUtils.getPlateforme(sessionScope),
				selectedContexte,
				selectedService, 
				selectedCollaborateur, selectedContact, 
				ConteneurDecorator.extractConteneursFromDecos(conteneurs), 
				codifications,
				tabsPat, tabsPrel, tabsEchan, tabsDerive, null,
				coulTypes, selectedEchanCouleur, selectedDeriveCouleur, 
		...
		banque = ManagerLocator.getBanqueManager()
								.findByIdManager(banque.getBanqueId());
...
//	}
	...
	public void onClick$createC() {
		...
		if (selectedContexte != null 
				&& selectedContexte.getContexteId() != null) {
		...
		Clients.showBusy(Labels
				.getLabel("ficheBanque.modification.encours"));
	...
	public boolean prepareDeleteObject() {
		boolean isReferenced = ManagerLocator
			.getBanqueManager().isReferencedObjectManager(getObject());
		...
		if (isReferenced) {
			setDeleteMessage(Labels
					.getLabel("banque.deletion.isReferenced"));
		} 
	...
	public void removeObject(String comments) {		
		...
		ManagerLocator.getPlateformeManager()
			.getBanquesManager(SessionUtils.getPlateforme(sessionScope))
			.remove(getObject());
	}
	...
	public void onLaterDelete(Event event) {
		try {
			...
			if (getObjectTabController().getListe() != null
				&& !getObjectTabController().getListe()
								.getListObjects().isEmpty()) {
				getObjectTabController().getListe().selectFirstObjet();
				
				getMainWindow().prepareListBanques();
				getMainWindow().updateSelectedBanque(getObjectTabController()
						.getListe()
						.getListObjects().get(0));
			}
...
//	}
	...
	public void onLaterUpdate() {
		try {
			...
			if (banque.getBanqueId().equals(SessionUtils
					.getSelectedBanques(sessionScope).get(0).getBanqueId())) {
	 ...
	 * @throws Exception
	 */
	public void onSelect$proprioBox(Event event) throws Exception {
		int ind = proprioBox.getSelectedIndex();
		if (ind > -1) {
	 ...
	 * @throws Exception
	 */
	public void onSelect$collabBox(Event event) throws Exception {
		int ind = collabBox.getSelectedIndex();
		if (ind > -1) {
	 ...
	 * @throws Exception
	 */
	public void onSelect$contactBox(Event event) throws Exception {
		int ind = contactBox.getSelectedIndex();
		if (ind > -1) {
	...
	public void setEmptyToNulls() {
		...
		if (this.banque.getNom() != null 
				&& this.banque.getNom().equals("")) {
			this.banque.setNom(null);
		}
		
		if (this.banque.getIdentification() != null 
				&& this.banque.getIdentification().equals("")) {
			this.banque.setIdentification(null);
		}
		
		if (this.banque.getDescription() != null && 
				this.banque.getDescription().equals("")) {
			this.banque.setDescription(null);
		}
		
		if (this.banque.getDefautMaladie() != null 
				&& this.banque.getDefautMaladie().equals("")) {
			this.banque.setDefautMaladie(null);
		}
		
		if (this.banque.getDefautMaladieCode() != null 
				&& this.banque.getDefautMaladieCode().equals("")) {
			this.banque.setDefautMaladieCode(null);
		}
		
		selectedEchanCouleur = null;
		if (couleurEchanBox.getSelectedItem() != null) {
			if (((Couleur) couleurEchanBox.getSelectedItem().getValue())
									.getCouleurId() != null) {
				selectedEchanCouleur = (Couleur) 
					couleurEchanBox.getSelectedItem().getValue();
			}
		}
		
		selectedDeriveCouleur = null;
		if (couleurDeriveBox.getSelectedItem() != null) {
			if (((Couleur) couleurDeriveBox.getSelectedItem().getValue())
									.getCouleurId() != null) {
				selectedDeriveCouleur = (Couleur) 
					couleurDeriveBox.getSelectedItem().getValue();
			}
		}
...
//		}
		...
		// Gestion du service
		String selectedNomService = proprioBox.getValue().toUpperCase();
	...
	public void updateObject() {
		...
		List<TableAnnotation> tabsPat = TableAnnotationDecorator
				.undecorateListe(getTablesPatSorterController().getObjs());
		List<TableAnnotation> tabsPrel = TableAnnotationDecorator
				.undecorateListe(getTablesPrelSorterController().getObjs());
		List<TableAnnotation> tabsEchan = TableAnnotationDecorator
				.undecorateListe(getTablesEchanSorterController().getObjs());
		List<TableAnnotation> tabsDerive = TableAnnotationDecorator
				.undecorateListe(getTablesDeriveSorterController().getObjs());
		List<TableAnnotation> tabsCess = TableAnnotationDecorator
				.undecorateListe(getTablesCessSorterController().getObjs());
		
		// suppr catalogue
		@SuppressWarnings("unchecked")
		List<Catalogue> supprCatalogues = ListUtils
			.removeAll(ManagerLocator.getCatalogueManager()
				.findByAssignedBanqueManager(banque), selectedCatalogues);
		
		tabs.addAll(ManagerLocator.getTableAnnotationManager()
								.findByCataloguesManager(supprCatalogues));
	...
	//		}
		...
		tabs.clear();
		@SuppressWarnings("unchecked")
		List<Catalogue> newCatalogues = ListUtils
			.removeAll(selectedCatalogues, ManagerLocator.getCatalogueManager()
				.findByAssignedBanqueManager(banque));
		tabs.addAll(ManagerLocator.getTableAnnotationManager()
								.findByCataloguesManager(newCatalogues));
...
//				.undecorateListe(getTablesCessSorterController().getObjs()));
		...
		ManagerLocator.getBanqueManager().createOrUpdateObjectManager(
				banque, null, selectedContexte,
				selectedService, 
				selectedCollaborateur, selectedContact, 
				ConteneurDecorator.extractConteneursFromDecos(conteneurs), 
				codifications,
				tabsPat, tabsPrel, tabsEchan, tabsDerive, tabsCess,
				coulTypes, selectedEchanCouleur, selectedDeriveCouleur, 
	...
	public void initEditableMode() {	
		...
		if (contextes.isEmpty()) {
			...
			contextes.addAll(ManagerLocator.getContexteManager()
													.findByOrderManager());
		}
		...
		collaborateurs.addAll(ManagerLocator.getCollaborateurManager()
		...
		services.clear();
		nomsServices.clear();
		services.addAll(ManagerLocator.getServiceManager()
				.findAllActiveObjectsWithOrderManager());
		for (int i = 0; i < services.size(); i++) {
			StringBuffer sb = new StringBuffer();
			if (services.get(i) != null 
					&& services.get(i).getNom() != null) {
				sb.append(services.get(i).getNom());
				if (services.get(i).getEtablissement() != null
						&& services.get(i).getEtablissement()
						.getNom() != null) {
					sb.append(" (");
		...
		couleurEchanBox.setModel(new SimpleListModel<Couleur>(couleurs));
		couleurEchanBox
			.setSelectedIndex(couleurs.indexOf(selectedEchanCouleur));
		couleurDeriveBox.setModel(new SimpleListModel<Couleur>(couleurs));
		couleurDeriveBox
			.setSelectedIndex(couleurs.indexOf(selectedDeriveCouleur));
	...
	private void initAssociations() {
		...
		if (banque.getBanqueId() != null) {
			...
			if (selectedContexte != null) {
				catalogues.addAll(ManagerLocator
						.getContexteManager()
						.getCataloguesManager(selectedContexte));
			}
			selectedCatalogues.addAll(ManagerLocator.getCatalogueManager()
					.findByAssignedBanqueManager(banque));
			...
			if (this.banque.getCollaborateur() != null) {
				...
				if (!collaborateurs.contains(
						this.banque.getCollaborateur())) {
					collaborateurs.add(this.banque.getCollaborateur());
					selectedCollaborateur = this.banque.getCollaborateur();
					nomsAndPrenoms.add(this.banque.getCollaborateur()
														.getNomAndPrenom());
				}
			}
			...
			if (this.banque.getContact() != null) {
				...
				if (!collaborateurs.contains(
						this.banque.getContact())) {
					collaborateurs.add(this.banque.getContact());
					selectedContact = this.banque.getContact();
					nomsAndPrenoms.add(this.banque.getContact()
														.getNomAndPrenom());
				}
			}
			...
			if (this.banque.getProprietaire() != null) {
				...
				if (selectedService != null 
						&& selectedService.getNom() != null) {
					sb.append(selectedService.getNom());
					if (selectedService.getEtablissement() != null
							&& selectedService.getEtablissement()
							.getNom() != null) {
						sb.append(" (");
			...
			profilUtilisateurs.addAll(ManagerLocator
				.getProfilUtilisateurManager().findByBanqueManager(banque, false));
			
			selectedEchanCouleur = banque.getEchantillonCouleur();
			selectedDeriveCouleur = banque.getProdDeriveCouleur();
			
			conteneurs.addAll(ConteneurDecorator.decorateListe(ManagerLocator
				.getConteneurManager().findByBanqueWithOrderManager(banque), null));
			
			codifications.addAll(ManagerLocator
				.getBanqueManager()
					.getBanqueTableCodageByBanqueManager(banque));
			
			coulTypesEchan.addAll(ManagerLocator.getCouleurEntiteTypeManager()
						.findAllCouleursForEchanTypeByBanqueManager(banque));
			
			coulTypesDerives.addAll(ManagerLocator.getCouleurEntiteTypeManager()
					.findAllCouleursForProdTypeByBanqueManager(banque));
			
			tablesAnnoPat.addAll(ManagerLocator.getTableAnnotationManager()
				.findByEntiteAndBanqueManager(ManagerLocator.getEntiteManager()
							.findByNomManager("Patient").get(0), banque));
			
			tablesAnnoPrel.addAll(ManagerLocator.getTableAnnotationManager()
				.findByEntiteAndBanqueManager(ManagerLocator.getEntiteManager()
							.findByNomManager("Prelevement").get(0), banque));
			
			tablesAnnoEchan.addAll(ManagerLocator.getTableAnnotationManager()
				.findByEntiteAndBanqueManager(ManagerLocator.getEntiteManager()
							.findByNomManager("Echantillon").get(0), banque));
			
			tablesAnnoDerive.addAll(ManagerLocator.getTableAnnotationManager()
				.findByEntiteAndBanqueManager(ManagerLocator.getEntiteManager()
							.findByNomManager("ProdDerive").get(0), banque));
			
			tablesAnnoCess.addAll(ManagerLocator.getTableAnnotationManager()
					.findByEntiteAndBanqueManager(ManagerLocator.getEntiteManager()
								.findByNomManager("Cession").get(0), banque));
		}
		...
		getTablesPatSorterController()
			.setObjs(TableAnnotationDecorator.decorateListe(tablesAnnoPat));
		getTablesPrelSorterController()
			.setObjs(TableAnnotationDecorator.decorateListe(tablesAnnoPrel));
		getTablesEchanSorterController()
			.setObjs(TableAnnotationDecorator.decorateListe(tablesAnnoEchan));
		getTablesDeriveSorterController()
			.setObjs(TableAnnotationDecorator.decorateListe(tablesAnnoDerive));
		getTablesCessSorterController()
		.setObjs(TableAnnotationDecorator.decorateListe(tablesAnnoCess));
	}
	 ...
	 * @param event
	 */
	public void onClickProfilUtilisateur(Event event) {
		ProfilUtilisateur pf = (ProfilUtilisateur) event.getData();
		getAdministrationController()
			.selectUtilisateurInController(pf.getUtilisateur());
	}
	 ...
	 * @since 2.1
	 */
	public void onClickProfilUtilisateurProfil(Event event) {
		ProfilUtilisateur pf = (ProfilUtilisateur) event.getData();
		getAdministrationController()
	...
	public Document createDocumentXML() {
		...
		Element page1 = ManagerLocator.getXmlUtils().addPage(
				...
				.getLabel("impression.banque.title", 
	 ...
	 * @param page
	 */
	public void addInfosBanqueToPrint(Element page) {
		// Nom
		String tmp = "";
		...
		CoupleValeur cp1 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.Nom"), 
				tmp);
		// Identification
		...
		CoupleValeur cp2 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.Identification"), 
				tmp);
		...
		CoupleValeur cp3 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.Description"), 
				tmp);
		...
		if (banque.getProprietaire() != null) {
			...
			if (banque.getProprietaire().getEtablissement() != null) {
				sb.append(" (");
				sb.append(banque.getProprietaire()
							.getEtablissement().getNom());
				sb.append(")");
			}
			tmp = sb.toString();
		...
		CoupleValeur cp4 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.Proprietaire"), 
				tmp);
		LigneDeuxColonnesParagraphe li3 = new LigneDeuxColonnesParagraphe(cp4);
		...
		CoupleValeur cp5 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.Collaborateur"), 
				tmp);
		...
		CoupleValeur cp6 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.Contact"), 
				tmp);
		LigneDeuxColonnesParagraphe li5 = new LigneDeuxColonnesParagraphe(cp6);
		
		// Maladie
		if (banque.getDefMaladies() != null) {
			tmp = ObjectTypesFormatters
				.booleanLitteralFormatter(this.banque.getDefMaladies());
		} else {
			tmp = "-";
		}
		CoupleValeur cp7 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.DefMaladies"), 
				tmp);
		// Libellé
		...
		CoupleValeur cp8 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.MaladieDefaut"), 
				tmp);
		...
		CoupleValeur cp8b = new CoupleValeur(
				Labels.getLabel("Champ.Banque.MaladieCodeDefaut"), 
				tmp);
		
		LigneParagraphe li6b = new LigneParagraphe("",
				new CoupleValeur[]{cp8, cp8b});
		
		// Prlvts accessibles aux autres collections
		if (banque.getAutoriseCrossPatient() != null) {
			tmp = ObjectTypesFormatters
				.booleanLitteralFormatter(this.banque
					.getAutoriseCrossPatient());
		} else {
			tmp = "-";
		}
		CoupleSimpleValeur cp9 = new CoupleSimpleValeur(
				Labels.getLabel("Champ.Banque.AutoriseCrossPatient"),
				tmp);
		LigneSimpleParagraphe li7 = new LigneSimpleParagraphe(cp9);
		...
		CoupleValeur cp10 = new CoupleValeur(
				Labels.getLabel("Champ.Banque.Contexte"), 
				tmp);
		// Catalogues
		...
		CoupleValeur cp11 = new CoupleValeur(
				Labels.getLabel("ficheBanque.contexte.catalogues"), 
				tmp);
		...
		Paragraphe par1 = new Paragraphe(
				null,  
				new Object[]{li1, li2, li3, li4, li5, li6, li6b, li7, li8}, 
				null,
				null, null);
	 ...
	 * @param page
	 */
	public void addInfosAnnotationsToPrint(Element page) {
		String tmp = "";
		// Tables patients
		...
		CoupleValeur cp1 = new CoupleValeur(
				Labels.getLabel("Entite.Patient"), 
				tmp);
		...
		CoupleValeur cp2 = new CoupleValeur(
				Labels.getLabel("Entite.Prelevement"), 
				tmp);
		...
		CoupleValeur cp3 = new CoupleValeur(
				Labels.getLabel("Entite.Echantillon"), 
				tmp);
		LigneDeuxColonnesParagraphe li3 = new LigneDeuxColonnesParagraphe(cp3);
		...
		CoupleValeur cp4 = new CoupleValeur(
				Labels.getLabel("Entite.ProdDerive"), 
				tmp);
		...
		CoupleValeur cp5 = new CoupleValeur(
				Labels.getLabel("Entite.Cession"), 
				tmp);
		LigneDeuxColonnesParagraphe li5 = new LigneDeuxColonnesParagraphe(cp5);
		
		Paragraphe par1 = new Paragraphe(
				Labels.getLabel("ficheBanque.contexte.tablesAnnos"),  
	 ...
	 * @param page
	 */
	public void addInfosCodificationsToPrint(Element page) {
		String tmp = "";
		...
		CoupleSimpleValeur cp1 = new CoupleSimpleValeur(
				Labels.getLabel("Champ.Banque.Codifications"),
				tmp);
	...
	public void addInfosListeUtilisateurs(Element page) {
		// Entete
		String[] listeEntete = new String[2];
		...
		// liste des cédés
		LigneListe[] liste = new LigneListe[tmp.size()];
		for (int i = 0; i < tmp.size(); i++) {
			String[] valeurs = new String[2];
			// Login
			valeurs[0] = tmp.get(i)
				.getUtilisateur().getLogin();
			// Profil
	...
	public void setSelectedContact(Collaborateur selected) {
	...
	public void setSelectedContexte(Contexte sContexte) {
	...
	public void onGetObjectFromSelection(Event e) {
		...
		if (this.banque.getCollaborateur() != null 
			&& !collaborateurs.contains(this.banque.getCollaborateur())) {
			collaborateurs.add(this.banque.getCollaborateur());
			nomsAndPrenoms.add(this.banque.getCollaborateur()
					.getNomAndPrenom());
		}
	...
	public String getSClassCollaborateur() {
		if (this.banque != null) {
			return ObjectTypesFormatters
				.sClassCollaborateur(this.banque.getCollaborateur());
		} else {
			return "";
		}
	}
	...
	public String getSClassContact() {
		if (this.banque != null) {
			return ObjectTypesFormatters
				.sClassCollaborateur(this.banque.getContact());
		} else {
			return "";
		}
	}
	...
	public String getSClassService() {
		if (this.banque != null) {
			return ObjectTypesFormatters
							.sClassService(this.banque.getProprietaire());
		} else {
			return "";
		}
	}
	...
	public String getArchiveFormatted() {
		if (this.banque != null) {
			return ObjectTypesFormatters
				.booleanLitteralFormatter(this.banque.getArchive());
		} else {
			return "";
		}
	}
	...
	public String getStyleCouleurEchan() {
		if (this.banque != null 
						&& this.banque.getEchantillonCouleur() != null) {
	...
	public String getEchantillonCouleur() {
		if (banque != null 
				&& banque.getEchantillonCouleur() != null) {
			return Labels.getLabel("Couleur."  
					+ banque.getEchantillonCouleur().getCouleur());
		} else {
			return null;
		}
	}
	...
	public String getProdDeriveCouleur() {
		if (banque != null 
				&& banque.getProdDeriveCouleur() != null) {
			return Labels.getLabel("Couleur."  
					+ banque.getProdDeriveCouleur().getCouleur());
		} else {
			return null;
		}
	}
...
//	}
	...
	public void updateConteneurs() {
		if (this.banque != null && this.banque.getBanqueId() != null) {
			getConteneursAssocies().setObjects(ManagerLocator
				.getConteneurManager()
					.findByBanqueWithOrderManager(banque));
		}
	}
	...
	public void setCodifications(List<BanqueTableCodage> cdf) {
...
//	}
	...
	public CodificationsAssociees getCodificationsAssociees() {
		return (CodificationsAssociees) self.getFellow("codificationsAssociees")
					.getFellow("winCodificationsAssociees")
	...
	public void setCoulTypesEchan(List<CouleurEntiteType> ctE) {
	...
	public CoulTypesAssociees getCoulTypesEchanAssociees() {
		return (CoulTypesAssociees) self.getFellow("coulTypesEchanWin")
					.getFellow("winCoulTypesAssociees")
	...
	public void setCoulTypesDerives(List<CouleurEntiteType> ctD) {
	...
	public CoulTypesAssociees getCoulTypesProdDeriveAssociees() {
		return (CoulTypesAssociees) self.getFellow("coulTypesDeriveWin")
					.getFellow("winCoulTypesAssociees")
					.getAttributeOrFellow("winCoulTypesAssociees$composer", true);
	}
	...
	public void setProfilUtilisateurRowRenderer(
			ProfilUtilisateurRowRenderer pRowRenderer) {
	...
	public void setTablesAnnoPat(List<TableAnnotation> t) {
	...
	public void setTablesAnnoPrel(List<TableAnnotation> t) {
	...
	public void setTablesAnnoEchan(List<TableAnnotation> t) {
	...
	public void setTablesAnnoDerive(List<TableAnnotation> t) {
	...
	public void setHautPage(String h) {
		this.hautPage = h;
	}
	 ...
	 * selSelectedCodes)
	 */
	public void onCheckCatalogue$catRows(ForwardEvent event) {
		// on récupère la checkbox associé à l'event
		Checkbox box = (Checkbox) 
			((ForwardEvent) event.getOrigin())
							.getOrigin().getTarget();
		Catalogue cat = (Catalogue) 
			((ForwardEvent) event.getOrigin()).getData();
	...
	public void setServiceRenderer(ServiceWithEtablissementRenderer sRenderer) {
	...
	public void setNomsServices(List<String> n) {
		this.nomsServices = n;
	}
	 ...
	 */
	public void onBlur$defautCodeBox(Event e) {
		
		if (!defautCodeBox.getValue().equals("")) {
			Clients.showBusy(maladieRow, 
					Labels.getLabel("libelle.recherche.encours"));
			Events.echoEvent("onLaterFindLibelle", self, null);
		}
		defautCodeBox.setValue(defautCodeBox.getValue().toUpperCase().trim());
	}
	
	public void onLaterFindLibelle(Event e) {
	 ...
	 * @since 2.1
	 */
	public void onCheck$utilisateursArchiveBox(Event e) {
		profilUtilisateurs.clear();
		...
		if (banque != null && banque.getBanqueId() != null) {
			if (utilisateursArchiveBox.isChecked()) {
				...
				profilUtilisateurs.addAll(ManagerLocator
					.getProfilUtilisateurManager().findByBanqueManager(banque, null));
			}
		}
	}
}
