 ...
 *
 */
public abstract class AbstractFicheStaticController extends AbstractFicheController
{
   ...
   public void reloadObject(){
      if(getObject() != null){
         final TKdataObject obj = getObjectTabController().loadById(getObject().listableObjectId());
         if(obj != null){
            setObject(obj);
         }else{
            setNewObject();
   ...
   public void setCanSeeHistorique(final boolean cHistorique){
   ...
   public static void openFicheContratWindow(final Page page, final Contrat contrat){
   ...
   private static HtmlMacroComponent contratComponent(final Window win, final Page page, final Contrat contrat){
      // HtmlMacroComponent contenu dans la fenêtre : il correspond
      // au composant des collaborations.
      HtmlMacroComponent ua;
      ua = (HtmlMacroComponent) page.getComponentDefinition("contrat", false).newInstance(page, null);
      ua.setParent(win);
      ua.setId("openFicheContrat");
      ua.applyProperties();
      ua.afterCompose();

      ((ContratController) ua.getFellow("winContrat").getAttributeOrFellow("winContrat$composer", true))
