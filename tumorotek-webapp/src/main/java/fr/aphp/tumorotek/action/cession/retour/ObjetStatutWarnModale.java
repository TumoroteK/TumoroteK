...
public class ObjetStatutWarnModale {
	...
	private List<TKStockableObject> objs = new ArrayList<TKStockableObject>();
	private List<Echantillon> echans = new ArrayList<Echantillon>();
	private List<Echantillon> echansOtherBanks = new ArrayList<Echantillon>(); 
	private List<ProdDerive> derives = new ArrayList<ProdDerive>(); 
	private List<Echantillon> derivesOtherBanks = new ArrayList<Echantillon>(); 
	...
	@Init
	public void init(@ExecutionArgParam("objs") List<TKStockableObject> objs, 
			@ExecutionArgParam("banque") Banque bank, 
			@ExecutionArgParam("main") MainWindow main,
			@ExecutionArgParam("operation") String op) {	
	...
	public String getGeneralWarning() {
		if (getSingleObj()) {
			String out =  ObjectTypesFormatters
					.getLabel("message.retour.single.warning", 
					new String[]{getObjs().get(0).getCode()});
	...
	private void setObjs(List<TKStockableObject> o) {

		objs = o;
		
		Iterator<TKStockableObject> it = objs.iterator();
	...
	public String getEchantillonsWarn() {
		return String.valueOf(getEchans().size()) + " " 
				+ Labels.getLabel("cession.echantillons") + " "
				+ getCurrBanque().getNom();
	}
	...
	public void selectEchans() {
		...
		EchantillonController tabController = (EchantillonController) EchantillonController
				.backToMe(getMain(), fwinObjetStatutWarnModale.getPage());
	...
	public String getEchantillonsOtherBanksWarn() {
		return String.valueOf(getEchansOtherBanks().size()) + " " 
				+ Labels.getLabel("cession.echantillons") + " " 
	...
	public String getDerivesWarn() {
		return String.valueOf(getDerives().size()) + " "
	...
	public String getDerivesOtherBanksWarn() {
		return String.valueOf(getDerivesOtherBanks().size()) + " " 
				+ Labels.getLabel("cession.prodDerive") + " "
				+ Labels.getLabel("message.retour.warning.otherbanks");
	}
	...
	public void setEchans(List<Echantillon> e) {
		this.echans = e;
	}
	...
	public void setEchansOtherBanks(List<Echantillon> e) {
	...
	public void setDerivesOtherBanks(List<Echantillon> d) {
	...
	public void setCurrBanque(Banque cB) {
		this.currBanque = cB;
	}
	...
	public void setSingleObj(Boolean s) {
		this.singleObj = s;
	}
