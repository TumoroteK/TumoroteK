...
public class FicheService extends AbstractFicheCombineController {
	...
	private List<Collaborateur> collaborateursActifs = 
		new ArrayList<Collaborateur>();
	private List<Collaborateur> collaborateursInactifs = 
		new ArrayList<Collaborateur>();
	private List<Collaborateur> collaborateursEdit = 
		new ArrayList<Collaborateur>();
	...
	private String collabsActifsGroupHeader = 
		Labels.getLabel("service.collaborateurs.actifs");
	private String collabsInactifsGroupHeader = 
		Labels.getLabel("service.collaborateurs.inactifs");
	private String mode;
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[]{this.nomLabel,
				...
				this.faxLabel,
				this.mailLabel,
				this.collabsActifsList,
				this.collabsInactifsList,
				this.archiveLabel,
				this.menuBar,
				this.groupCollabsActifsService,
				this.groupCollabsInactifsService
			});
		...
		setObjBoxsComponents(new Component[]{this.nomBox,
				...
				this.mailBox,
				this.etabsBox,
				this.collabsListEdit,
				this.rowCollabsTitle,
				this.rowAddCollab,
				this.addCollabBox,
				this.archiveBox,
				this.groupCollabsEditService
			});
		
		setRequiredMarks(new Component[]{this.etabRequired, 
				this.nomRequired});	
	...
	public void setObject(TKdataObject obj) {
		...
		if (service.getServiceId() != null) {
			collaborateursEdit.addAll(ManagerLocator.getServiceManager()
				.getCollaborateursWithOrderManager(service));
			collaborateursActifs.addAll(ManagerLocator.getCollaborateurManager()
					.findByServicesAndArchiveManager(service, false));
			collaborateursInactifs.addAll(
					ManagerLocator.getCollaborateurManager()
					.findByServicesAndArchiveManager(service, true));
		}
	...
	public void switchToCreateMode() {
		...
		collabsActifsGroupHeader = Labels.getLabel(
				"service.collaborateurs.actifs");
		collabsInactifsGroupHeader = Labels.getLabel(
			"service.collaborateurs.inactifs");
		...
		if (selectedEtablissement.getCoordonnee() != null) {
			this.coordonnee.setAdresse(
					selectedEtablissement.getCoordonnee().getAdresse());
			this.coordonnee.setCp(
	...
	public void switchToStaticMode() {
		...
		super.switchToStaticMode(
				this.service.equals(new Service()));
	...
	public void createNewObject() {
		...
		ManagerLocator.getServiceManager().createObjectManager(
				service, coordonnee, selectedEtablissement, 
				collaborateursEdit, 
	...
	public void updateObject() {
		...
		ManagerLocator.getServiceManager().updateObjectManager(
				service, coordonnee, selectedEtablissement, 
				collaborateursEdit, 
				SessionUtils.getLoggedUser(sessionScope),
				cascadeArchive, true);
		...
		if (getListeCollaborations() != null) {
			getListeCollaborations().updateTree();
			getListeCollaborations().openService(
					this.service, false, true);
			getListeCollaborations().updateCollabsWithoutService();
		}
	}
	...
	public boolean prepareDeleteObject() {
		boolean isUsed = ManagerLocator
			.getServiceManager().isUsedObjectManager(getObject());
		boolean isReferenced = ManagerLocator
			.getServiceManager().isReferencedObjectManager(getObject());
		...
		if (isUsed) {
			setDeleteMessage(Labels
					.getLabel("service.deletion.isUsedCascade"));
		} 
		if (isReferenced) {
			setDeleteMessage(Labels
				.getLabel("service.deletion.isReferencedCascade"));
			if (getObject().getArchive()) { // deja archivé
				setDeletable(false);
				setDeleteMessage(Labels
					.getLabel("service.deletion.isAlreadyArchived"));
			}
		}
			...
			if (isFantomable()) { // suppression possible
				...
				ManagerLocator.getServiceManager()
					.updateObjectManager(getObject(), 
							...
							getObject().getEtablissement(), null, 
							SessionUtils.getLoggedUser(sessionScope), 
							true, false);
			}
		} else {
			...
			if (isFantomable()) {
				...
				if (getListeCollaborations() != null) {
					getListeCollaborations()
								.openService(getObject(), false, true);
					getListeCollaborations().updateCollabsWithoutService();
				}
			}
	...
	public void onClick$deleteCollab(Event event) {
		...
		ListModel<Collaborateur> list = new 
					ListModelList<Collaborateur>(collaborateursEdit);
	...
	public void onClick$addCollaborateur() {
		String selectedNomAndPremon = this.addCollabBox
		...
		if (ind > -1) {
			...
			if (!collaborateursEdit.contains(collab)) {
				...
				ListModel<Collaborateur> list = 
						new ListModelList<Collaborateur>(collaborateursEdit);
				collabsListEdit.setModel(list);
			}
			
			addCollabBox.setValue(null);
		} else {
			throw new WrongValueException(addCollabBox, 
			Labels.getLabel("service.error.ajout.collaborateur"));
		}
	...
	public void onClick$newCollaborateur() {
		getObjectTabController()
			.switchToFicheCollaborateurCreationMode(this.service);
	}
	...
	public void setFieldsToUpperCase() {
		if (this.service.getNom() != null) {
			this.service.setNom(
				this.service.getNom().toUpperCase().trim());
		}
	...
	public void onSelect$etabsBox() {
		if (createMode && selectedEtablissement != null
	...
	public void onCheck$archiveBox() {
		...
		if (archiveBox.isChecked()) {
			if (Messagebox.show(Labels
					.getLabel("message.archive.message.service"),
			Labels.getLabel("message.archive.title"), 
			...
			if (Messagebox.show(Labels
					.getLabel("message.not.archive.message.service"),
			Labels.getLabel("messagenot.not.archive.title"), 
			Messagebox.YES | Messagebox.NO, 
			Messagebox.QUESTION) == Messagebox.YES) {
				cascadeArchive = true;
			} else {
...
//	}
	...
	public void drawActionsForService() {
		...
		newCollaborateur.setDisabled(!drawActionOnOneButton("Collaborateur", 
												"Creation"));
	}
	...
	public void onClick$print() {
		if (this.service != null) {
			...
			ManagerLocator.getXmlUtils().addHautDePage(root, 
					Labels.getLabel("ficheService.panel.title"),
					false, null);
	...
	public void addInfosGeneralesToPrint(Element page) {
		...
		if (service.getArchive() != null) {
			...
			.booleanLitteralFormatter(this.service.getArchive());
		} else {
			tmp = "";
		}
		CoupleValeur cp3 = new CoupleValeur(
				Labels.getLabel("service.archive"), 
				tmp);
	...
	public void addCollaborateursActifsToPrint(Element page) {
		...
		EnteteListe entetes = new EnteteListe(new String[]{
				Labels.getLabel("collaborateur.nom"),
				Labels.getLabel("collaborateur.prenom"),
				Labels.getLabel("collaborateur.specialite")});
		...
		for (int i = 0; i < collaborateursActifs.size(); i++) {
			...
			if (collaborateursActifs.get(i).getSpecialite() != null) {
				valeurs[2] = collaborateursActifs.get(i)
				.getSpecialite().getNom();
			} else {
				valeurs[2] = "-";
			}
	...
	public void addCollaborateursInactifsToPrint(Element page) {
		...
		for (int i = 0; i < collaborateursInactifs.size(); i++) {
			...
			if (collaborateursInactifs.get(i).getSpecialite() != null) {
				valeurs[2] = collaborateursInactifs.get(i)
					.getSpecialite().getNom();
			} else {
				valeurs[2] = "-";
			}
	...
	public void onClick$collabActifNom(Event event) {
		...
		if (getListeCollaborations() != null) {
			getListeCollaborations()
				.openCollaborateurInTree(collab, true, true);
		}
	}
