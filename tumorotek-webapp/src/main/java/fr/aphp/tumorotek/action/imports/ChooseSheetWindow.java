...
public class ChooseSheetWindow {
	...
	private List<TabFileSheet> sheets = new ArrayList<TabFileSheet>(); 		
	...
	@Init
	public void init(@ExecutionArgParam("selSheets") List<TabFileSheet> selTbs, 
			@ExecutionArgParam("sheets") List<Sheet> shs,
			@ExecutionArgParam("parent") Component p, 
			@ExecutionArgParam("embedded") Boolean emb) {
		...
		if (shs != null) {
			TabFileSheet tfs;
			for (Sheet sheet : shs) {
				tfs = new TabFileSheet(sheet.getSheetName(), 
						sheet.getPhysicalNumberOfRows() - 1, false);
				sheets.add(tfs);
			}
	...
	public void update(@BindingParam("selSheets") List<TabFileSheet> selTbs) {
		updateSelSheets(selTbs);
	}
	
	private void updateSelSheets(List<TabFileSheet> selTbs) {
		// disable 
		if (selTbs != null) {
			for (TabFileSheet tf : selTbs) {
	...
	public void importSheet() {	
		...
		if (selectedSheet != null) {
			...
			Events.echoEvent("onLaterImport", parent, 
								getSelectedSheet().getName());
		}	
	}
	...
	public void setSelectedSheet(TabFileSheet ts) {
		selectedSheet = ts;
	}
