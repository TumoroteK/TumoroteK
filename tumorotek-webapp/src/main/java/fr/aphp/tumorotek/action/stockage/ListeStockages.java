...
public class ListeStockages extends AbstractController
{
   ...
   public void doAfterCompose(final Component comp) throws Exception{
      ...
      final StockageRootNode root = new StockageRootNode(SessionUtils.getSelectedBanques(sessionScope));
      root.readChildren();
      ...
      if(currentObject instanceof EnceinteNode){
         ...
         if(obj instanceof EnceinteNode){
            ...
            if(!nodeDep.equals(nodeDest)){
               ...
               if(nodeDep.getConteneur().getNbrNiv().equals(nodeDest.getConteneur().getNbrNiv())
                  ...
                  getStockageController().getFicheEnceinte().definirEnceinteDestination(nodeDest);
               }else{
                  // on met la destination en rouge
                  final org.zkoss.zul.Treerow row = selectedDestinationItem.getTreerow();
                  colorateRowInRed(row);
                  getStockageController().getFicheEnceinte().definirEnceinteDestination(null);
               }
            }else{
               selectedDestinationItem = null;
            }
         ...
         if(obj instanceof TerminaleNode){
            ...
            if(!nodeDep.equals(nodeDest)){
               ...
               colorateRowInGreen(row);
               // on l'envoie à la fiche enceinte
               getStockageController().getFicheTerminale().definirTerminaleDestination(nodeDest);
               //					} else {
               //						// on met la destination en rouge
            ...
            final org.zkoss.zul.Treerow row = selectedDestinationItem.getTreerow();
            colorateRowInRed(row);
            getStockageController().getFicheTerminale().definirTerminaleDestination(null);
   ...
   public void updateConteneur(final Conteneur conteneur){
      ...
      final ConteneurNode node = new ConteneurNode(conteneur, null);
   ...
   public void openTerminaleFromEmplacement(final Emplacement empl){
      ...
      final int cPath = rootNodes.indexOf(new ConteneurNode(cont, null));
