...
public class ListeStockages extends AbstractController
{
   ...
   // @since 2.2.1
   // TK-251
   private final List<Banque> restrictedBanqueDeplacement = new ArrayList<>();
    ...
    * @version 2.2.1
    */
   public void switchToDeplacementMode(final List<Banque> banks){
    ...
    * @since 2.2.1-IRELEC
    */
   private void removePlateformeRestrictedConteneurs(final boolean checkRestrictStock){

      final List<Conteneur> toRemove = new ArrayList<>();
      for(final Conteneur cont : getRootConteneurs()){
         if(!isCurrentPFOrUserAtLeastAdminfPF(cont.getPlateformeOrig())){
            if(checkRestrictStock){
               final ConteneurPlateforme cp = ManagerLocator.getConteneurManager().getOneConteneurPlateformeManager(cont,
      ...
      // update conteneurs
      ((StockageRootNode) ttm.getRoot()).getConteneurs().removeAll(toRemove);
   ...
   private boolean checkConteneurDeplacementCoherence(final TumoTreeNode destObj){
      final Conteneur departCont = getRootConteneurs().get(ttm.getPath((TumoTreeNode) currentObject)[0]);
      final Conteneur destCont = getRootConteneurs().get(ttm.getSelectionPath()[0]);

      final Set<Banque> departContBanks = ManagerLocator.getConteneurManager().getBanquesManager(departCont);
      final Set<Banque> destContBanks = ManagerLocator.getConteneurManager().getBanquesManager(destCont);

      // current objects banks
      final List<Banque> movedObjectBanks = destObj instanceof EnceinteNode
