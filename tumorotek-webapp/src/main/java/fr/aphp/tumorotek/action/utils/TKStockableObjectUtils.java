...
public final class TKStockableObjectUtils {
	 ...
	 * @since 2.0.9
	 */
	public static Hlayout drawListIcones(TKStockableObject obj, List<NonConformite> ncfsTrait, List<NonConformite> ncfsCess) {
		
		Hlayout icones = new Hlayout();
		
		// impact qualité
		if (obj != null && !ManagerLocator.getRetourManager()
				.findByObjectAndImpactManager(obj, true).isEmpty()) {
			Div imp = new Div();
			imp.setWidth("18px");
			imp.setHeight("18px");
		...
		if (obj != null && obj.getConformeTraitement() != null) {
			...
			if (obj.getConformeTraitement()) {
				...
				nonConf.setTooltiptext(Labels
						.getLabel("tooltip.conforme.traitement"));
			} else {
				String noconfsTrait = ": ";
				...
				if (((TKdataObject) obj).listableObjectId() != null) {
					ncIt = ManagerLocator.getNonConformiteManager()
						...
						.findByObjetAndTypeManager(obj, "Traitement")).iterator();
				} else {
				...
				nonConf.setTooltiptext(ObjectTypesFormatters
						.getLabel("tooltip.nonconforme.traitement", new String[]{noconfsTrait}));
			}
		...
		if (obj != null && obj.getConformeCession() != null) {
			...
			if (obj.getConformeCession()) {
				...
				nonConf.setTooltiptext(Labels
						.getLabel("tooltip.conforme.cession"));
			} else {
				String noconfsCess = ": ";
				Iterator<NonConformite> ncIt;
				if (((TKdataObject) obj).listableObjectId() != null) {
					ncIt = ManagerLocator.getNonConformiteManager()
						.getFromObjetNonConformes(ManagerLocator
						.getObjetNonConformeManager()
						.findByObjetAndTypeManager(obj, "Cession")).iterator();
				} else {
				...
				nonConf.setTooltiptext(ObjectTypesFormatters
						.getLabel("tooltip.nonconforme.cession", 
								new String[]{noconfsCess}));
			}
			nonConf.setParent(icones);
		}
		return icones;
