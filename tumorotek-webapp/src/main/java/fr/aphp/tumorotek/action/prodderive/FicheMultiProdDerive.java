...
public class FicheMultiProdDerive extends FicheProdDeriveEdit {
	...
	private Log log = LogFactory.getLog(FicheModifMultiProdDerive.class);
	...
	//Objets Principaux.
	// ajout de dérivés
	private List<ProdDerive> prodDerives = new ArrayList<ProdDerive>();
	private List<ProdDeriveDecorator2> prodDerivesDecorated = 
									new ArrayList<ProdDeriveDecorator2>();
	private List<List<ProdDerive>> addedProdDerives = new ArrayList<List<ProdDerive>>();
	private List<String> usedCodesProdDerives = new ArrayList<String>();
	private Hashtable<ProdDerive, Emplacement> derivesEmpl = 
		new Hashtable<ProdDerive, Emplacement>();
	...
	private ProdDeriveDecoratorRowRenderer deriveDecoRenderer = 
			new ProdDeriveDecoratorRowRenderer();
	
	private Retour transfoRetour = null;
	
	private List<List<AnnotationValeur>> annoBatches = 
						new ArrayList<List<AnnotationValeur>> ();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		stockageDerives
			.setVisible(getDroitOnAction("Stockage", "Consultation"));
	...
	public boolean onLaterCreate() {
		...
		try {
			...
			for (int i = 0; i < addedProdDerives.size(); i++) {
				for (ProdDerive prod : this.addedProdDerives.get(i)) {
			...
			if (re instanceof DeriveBatchSaveException 
		        ...
		        map.put("message", 
		        	handleExceptionMessage(((DeriveBatchSaveException) re).getTargetExeption()));
	...
	public void createNewObject() {	
		...
		try {
			...
			if (!getTypeParent().equals("Aucun")) {
				...
				getTransformation()
						.setQuantiteUnite(getSelectedTransfoQuantiteUnite());
			...
			if (getTypeParent().equals("Echantillon")
				...
				List<Retour> rets = ManagerLocator.getRetourManager()
						.findByObjectDateRetourEmptyManager(id, 
				...
				if (statut.getStatut().equals("EPUISE")) {	
					...
					transfoRetour
						.setObservations(Labels.getLabel("ficheRetour.observations.derives")
				...
				if (transfoRetour != null) {
					BeanValidator.validateObject(transfoRetour, 
						new Validator[]{ManagerLocator.getRetourValidator()});
				}
			}
			...
			for (int i = 0; i < addedProdDerives.size(); i++) {
				...
				ManagerLocator.getProdDeriveManager()
					.createDeriveListWithAnnotsManager(addedProdDerives.get(i), 
										getBanque(), getTransformation(), 
			...
			for (int i = 0; i < prodDerivesDecorated.size(); i++) {
				if (prodDerivesDecorated.get(i).getProdDerive()
						.getConformeTraitement() != null
						&& !prodDerivesDecorated.get(i).getProdDerive()
			...
			if (transfoRetour != null) {
				...
				((TKStockableObject) getParentObject())
					.setObjetStatut(ManagerLocator.getObjetStatutManager()
							.findByStatutLikeManager("NON STOCKE", true).get(0));
				
				ManagerLocator.getRetourManager()
				.createOrUpdateObjectManager(transfoRetour, 
						(TKStockableObject) getParentObject(), 
						getOldEmplacement(), null, null, getTransformation(),
						 null,
						SessionUtils.getLoggedUser(sessionScope), 
						"creation");
			...
			Iterator<ProdDeriveDecorator2> itP = 
					prodDerivesDecorated.iterator();
	...
	public void saveEmplacements() {
		...
		Set<ProdDerive> prods = (Set<ProdDerive>) derivesEmpl.keySet();
		Iterator<ProdDerive> it = prods.iterator();
		while (it.hasNext()) {
			ProdDerive tmp = it.next();
			Emplacement empl = derivesEmpl.get(tmp);
...
//		}
		...
		// emplacement
		prods = (Set<ProdDerive>) derivesEmpl.keySet();
		it = prods.iterator();
		...
		while (it.hasNext()) {
			...
			ManagerLocator.getProdDeriveManager().updateObjectManager(
					...
					derivesEmpl.get(deriveToUpdate), 
			...
			ProdDeriveDecorator2 deco = new 
			ProdDeriveDecorator2(deriveToUpdate);
			// si le dérivé existait déjà, on MAJ sa fiche
			// dans la liste des dérivés
			for (List<ProdDerive> derives : addedProdDerives) {
	...
	public void createFileHtmlToPrint() {
		...
		if (derivesEmpl.size() > 0) {
			...
			for (int k = 0; k < prodDerivesDecorated.size(); k++) {
				...
				if (prods.contains(prodDerivesDecorated.get(k)
						.getProdDerive())) {
					ProdDerive derive = prodDerivesDecorated.get(k)
						.getProdDerive();
	...
	public void onGetObjectFromSelection(Event e) {
		...
		collabBoxDerive
				.setModel(new CustomSimpleListModel(getNomsAndPrenoms()));	
		...
		if (e.getData() != null) {
			...
			if (getNomsAndPrenoms().contains(coll.getNomAndPrenom())) {
				...
				collabBoxDerive
					.setValue(getSelectedCollaborateur().getNomAndPrenom());
			}
		}
	}
	 ...
	 */
	public void onClick$addProdDerives(Event event) {
		try {
			...
			if (getSelectedType() == null) {
				...
				throw new WrongValueException(
					typesBoxDerive, 
					Labels.getLabel("ficheProdDerive.error.type"));
			}
			...
			if (getProdDerive().getDateTransformation() != null) {
				dateSortie = getProdDerive().getDateTransformation()
					.getTime();
			} else {
				dateSortie = null;
			}
			...
			List<ProdDerive> newDerivesBatch = new ArrayList<ProdDerive>();
			...
			for (int i = first; i <= last; i++) {
				...
				ProdDerive newProdDerive = new ProdDerive();
				...
				newProdDerive.setQuantiteInit(getProdDerive()
														.getQuantiteInit());
				...
				newProdDerive.setDateTransformation(getProdDerive()
												.getDateTransformation());
				newProdDerive.setReservation(null);
				newProdDerive.setEtatIncomplet(getProdDerive()
													.getEtatIncomplet());
				newProdDerive.setArchive(getProdDerive().getArchive());
				newProdDerive.setModePrepaDerive(getSelectedModePrepaDerive());
				newProdDerive.setConformeTraitement(getProdDerive()
						.getConformeTraitement());
				newProdDerive.setConformeCession(getProdDerive()
						.getConformeCession());
			
				ProdDeriveDecorator2 deco = new 
					ProdDeriveDecorator2(newProdDerive);
			...
			ListModel<ProdDeriveDecorator2> list = 
				new ListModelList<ProdDeriveDecorator2>(prodDerivesDecorated);
...
//				.getBindingData((ForwardEvent) event, false);
		...
		ProdDeriveDecorator2 deco = (ProdDeriveDecorator2) 
	...
	public void onClick$stockageDerives() {
		if (!stockageDerives.isDisabled()) {
			...
			List<ProdDerive> derivesToStock = new ArrayList<ProdDerive>();
			for (int i = 0; i < prodDerives.size(); i++) {
				if (prodDerives.get(i).getObjetStatut()
			...
			// ils sont réservés
			Enumeration<Emplacement> empls = derivesEmpl.elements();
			...
			tabController.switchToStockerMode(
					null, derivesToStock, Path.getPath(self),
	...
	public void onGetResultsFromStockage(Event e) {
		...
		Hashtable<ProdDerive, Emplacement> results = 
			(Hashtable<ProdDerive, Emplacement>) e.getData();
	 ...
	 */
	private void updateDecoList(Hashtable<ProdDerive, Emplacement> results) {
		ObjetStatut stocke = ManagerLocator.getObjetStatutManager()
				.findByStatutLikeManager("STOCKE", true).get(0);
			
		Set<ProdDerive> prods = (Set<ProdDerive>) results.keySet();
		Iterator<ProdDerive> it = prods.iterator();
		// pour chaque dérivé qui vient d'être stocké
		while (it.hasNext()) {
			ProdDerive tmp = it.next();
			ProdDeriveDecorator2 deco = new ProdDeriveDecorator2(tmp);
			// on récupère son decorator correspondant dans la liste
			if (prodDerivesDecorated.contains(deco)) {
				ProdDeriveDecorator2 decoUp = prodDerivesDecorated
					.get(prodDerivesDecorated.indexOf(deco));
		...
		getBinder().loadAttribute(
				self.getFellow("derivesList"), "model");		
	}
	...
	public void clearForm(boolean setParentStatic) {
		...
		((ListModelList<Object>) typesBoxDerive.getModel())
			.clearSelection();
		
		setSelectedQuantiteUnite(null);
		quantiteUnitesBoxDerive.clearSelection();
		((ListModelList<Object>) quantiteUnitesBoxDerive.getModel())
			.clearSelection();
		
		setSelectedVolumeUnite(null);
		volumeUnitesBoxDerive.clearSelection();
		((ListModelList<Object>) volumeUnitesBoxDerive.getModel())
			.clearSelection();
		
		setSelectedConcUnite(null);
		concentrationUnitesBoxDerive.clearSelection();
		((ListModelList<Object>) concentrationUnitesBoxDerive.getModel())
			.clearSelection();
		...
		((ListModelList<Object>) qualitesBoxDerive.getModel())
			.clearSelection();
		
		setSelectedModePrepaDerive(null);
		modePrepaBoxDerive.clearSelection();
		((ListModelList<Object>) modePrepaBoxDerive.getModel())
			.clearSelection();
	...
	public void initEditableMode() {
		...
		setTypes((List<ProdType>) ManagerLocator.getProdTypeManager()
				.findByOrderManager(SessionUtils.getPlateforme(sessionScope)));
		getTypes().add(0, null);
		setSelectedType(getTypes().get(0));

		// on récupèé les unités de quantité
		setQuantiteUnites(ManagerLocator.getUniteManager()
			.findByTypeLikeManager("masse", true));
		getQuantiteUnites().addAll(ManagerLocator.getUniteManager()
				.findByTypeLikeManager("discret", true));
		getQuantiteUnites().add(0, null);
		
		// on récupèé les unités de volume
		setVolumeUnites(ManagerLocator.getUniteManager()
			.findByTypeLikeManager("volume", true));
		getVolumeUnites().add(0, null);
		
		// on récupèé les unités de conc
		setConcUnites(ManagerLocator.getUniteManager()
			.findByTypeLikeManager("concentration", true));
		getConcUnites().add(0, null);
		
		setQualites((List<ProdQualite>) ManagerLocator.getProdQualiteManager()
			.findByOrderManager(SessionUtils.getPlateforme(sessionScope)));
		getQualites().add(0, null);
		
		setModePrepaDerives((List<ModePrepaDerive>) ManagerLocator
				.getModePrepaDeriveManager()
				.findByOrderManager(SessionUtils.getPlateforme(sessionScope)));
		...
		if (getSelectedCollaborateur() != null 
			&& getCollaborateurs().contains(getSelectedCollaborateur())) {
		...
		if (getParentObject() != null 
				&& ((TKAnnotableObject) getParentObject())
													.getBanque() != null) {
		...
		usedCodesProdDerives = ManagerLocator.getProdDeriveManager()
	...
	private void initDeriveInfosFromParent() {
		...
		if (getTypeParent().equals("Prelevement")) {
			...
			prodDerives = ManagerLocator.getPrelevementManager()
				.getProdDerivesManager((Prelevement) getParentObject());
			
			if (((Prelevement) 
					getParentObject()).getOperateur() != null) {
				setSelectedCollaborateur(((Prelevement) 
			...
			// on récupère la quantité et le volume disponible
			setQuantiteMax(((Echantillon) 
							getParentObject()).getQuantite());
			
			prodDerives = ManagerLocator.getEchantillonManager()
				.getProdDerivesManager(((Echantillon) getParentObject()));
			
			if (((Echantillon) getParentObject())
									.getCollaborateur() != null) {
				setSelectedCollaborateur(((Echantillon) 
						getParentObject()).getCollaborateur());
			}
			...
			// on récupère la quantité et le volume disponible
			setQuantiteMax(((ProdDerive) 
					getParentObject()).getQuantite());
			
			prodDerives = ManagerLocator.getProdDeriveManager()
				.getProdDerivesManager(((ProdDerive) getParentObject()));
			
			if (((ProdDerive) getParentObject())
									.getCollaborateur() != null) {
				setSelectedCollaborateur(((ProdDerive) 
						getParentObject()).getCollaborateur());
			}
		} else {
			prodDerives.clear();
	...
	public List<String> findDoublons(Integer premier, Integer dernier) {
		...
		String codePrefixeTmp = this.codePrefixeLabelDerive.getValue();
		for (int i = premier; i <= dernier; i++) {
			StringBuffer sb = new StringBuffer();
			if (codePrefixeTmp != null 
					&& !codePrefixeTmp.trim().equals("")) {
	...
	public void setProdDerives(List<ProdDerive> derives) {
	...
	public void setProdDerivesDecorated(
			List<ProdDeriveDecorator2> derivesDecorated) {
	...
	public void setAddedProdDerives(List<List<ProdDerive>> addedDerives) {
	...
	public void setUsedCodesProdDerives(List<String> usedCodesDerives) {
	...
	public void setSelectParent(boolean select) {
	...
	public void setDerivesEmpl(Hashtable<ProdDerive, Emplacement> derivesE) {
				...
				if (premierCode != null) {
					...
					if (dernierCode != null) {
						...
						if (dernierCode < premierCode) {
							...
							List<String> doublons = findDoublons(premierCode,
									dernierCode);
							...
							if (doublons.size() > 0) {
								...
								if (doublons.size() == 1) {
									sb.append(Labels
											.getLabel("ficheMultiProdDerive.error.un.prodDerive"
													+ ".existant"));
									sb.append(doublons.get(0));
								} else {
									sb.append(Labels
											.getLabel("ficheMultiProdDerive.error.plus.prodDerive"
													+ ".existant"));
								...
								sb.append(Labels
										.getLabel("ficheMultiProdDerive.error.prodDerive"
												+ ".existant.saisie"));
								...
								dernierCodeBoxDerive.setConstraint("");
								dernierCodeBoxDerive.setValue(tmp);
								dernierCodeBoxDerive
										.setConstraint(cttDernierCode);
							}
						}
					} else {
						// sinon on enlève toutes les erreurs affichées
				...
				if (dernierCode != null) {
					...
					if (premierCode != null) {
						...
						if (dernierCode < premierCode) {
							throw new WrongValueException(
									comp,
									Labels.getLabel("ficheMultiProdDerive.error.premier.code"
											+ ".superieur"));
						} else {
							...
							if (doublons.size() > 0) {
								...
								if (doublons.size() == 1) {
									sb.append(Labels
											.getLabel("ficheMultiProdDerive.error.un"
													+ ".prodDerive.existant"));
									sb.append(doublons.get(0));
								} else {
									sb.append(Labels
											.getLabel("ficheMultiProdDerive.error.plus."
													+ "prodDerive.existant"));
									for (int i = 0; i < doublons.size(); i++) {
										sb.append(doublons.get(i));
										if (i + 1 < doublons.size()) {
								...
								premierCodeBoxDerive.setConstraint("");
								premierCodeBoxDerive.setValue(tmp);
								premierCodeBoxDerive
										.setConstraint(cttPremierCode);
							}
						}
					} else {
						// sinon on enlève toutes les erreurs affichées
	...
	/************************** PARENT    ************************************/
	/*************************************************************************/
	private List<String> listParentTypes = new ArrayList<String>();
	...
	public void setSelectedParent(String selected) {
	...
	public void setSelectedCodeParent(String selected) {
...
//		codesParentBoxDerive.setConstraint(cttCodeParent);
	...
	public void addSelectParentMode() {
		...
		listParentTypes = new ArrayList<String>();
		...
		codesParent = ManagerLocator.getEchantillonManager()
			.findAllCodesForBanqueAndQuantiteManager(SessionUtils
	 ...
	 * @throws Exception
	 */
	public void onBlur$codesParentBoxDerive(Event event) throws Exception {
	 ...
	 * @throws Exception
	 */
	public void onSelect$typesParentBoxDerive(Event event) throws Exception {
		
		// on récupère le type sélectionné
		int index = typesParentBoxDerive.getSelectedIndex();
		String tmp = "";
		if (index == 0) {
			tmp = "Echantillon";
		} else if (index == 1) {
		...
		if (!tmp.equals(getTypeParent())) {
			...
			prodDerives.clear();
			prodDerivesDecorated = ProdDeriveDecorator2
				.decorateListe(prodDerives);
			ListModel<ProdDeriveDecorator2> list = 
				new ListModelList<ProdDeriveDecorator2>(prodDerivesDecorated);
			derivesList.setModel(list);
			getBinder().loadAttribute(derivesList, "model");
	 ...
	 */
	public void noParentMode(boolean noParent) {
		setParentObject(null);
		if (noParent) {
			codeParentLabelDerive.setVisible(false);
			requiredCodeParentDerive.setVisible(false);
	...
	public void setCttCodeParent(Constraint ctt) {
	...
	public class ConstCodeParent implements Constraint {
		...
		public void validate(Component comp, Object value) {
			...
			if (code.length() == 0) {
				throw new WrongValueException(
				comp, "Champ vide non autorisé. Vous devez spécifier un " 
				+ "code présent dans la liste.");
			} else if (!codesParent.contains(code)) {
				throw new WrongValueException(
				comp, "Valeur non autorisée. Vous devez spécifier une " 
...
//	
	...
	public void setCttQuantiteInit(Constraint ctt) {
			...
			if (quantiteInitValue != null) {
				...
				if (getQuantiteInit() < 0) {
					...
					if (getProdDerive().getVolumeInit() != null
							&& getProdDerive().getConc() != null) {
						
						Float res = getProdDerive().getVolumeInit() 
							* getProdDerive().getConc();
...
//	}
	...
	public void setDateSortie(Date d) {
		this.dateSortie = d;
	}
	...
	public void setLettres(List<String> lettres) {
	...
	public void applyTKObjectsCodesFromScan(ScanTerminale sT) {
		if (!getAddedProdDerives().isEmpty() 
							&& !sT.getScanTubes().isEmpty()) {
			int i = 0;
			
			Textbox tb;
			for (ProdDeriveDecorator2 deco : getProdDerivesDecorated()) {
				if (deco.isNew() && deco.getAdrlTmp() == null) {
					if (i < sT.getScanTubes().size()) {
						tb = (Textbox) derivesList.getRows().getChildren()
							.get(getProdDerivesDecorated().indexOf(deco))
