...
public class GridResultsModel extends ObjectPagingModel
{
   ...
   public GridResultsModel(final int startPageNumber, final int pageSize, final List<Object> objs, final Affichage aff){
   ...
   protected List<? extends Object> getPageData(final int itemStartNumber, final int pageSize){
      ...
      for(int i = itemStartNumber; i < end; i++){
         matriceObjets.add(RechercheUtilsManager.getListeObjetsCorrespondants(getObjets().get(i), getAffichage(), null));
      }

      final List<List<Object>> matriceAffichable = new ArrayList<>();

      /** On charge la matrice affichable. */
      RechercheUtils.loadMatriceAffichable(matriceObjets, matriceAffichable, getAffichage());
