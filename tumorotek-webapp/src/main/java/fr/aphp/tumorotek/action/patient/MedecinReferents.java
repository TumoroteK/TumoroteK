...
public class MedecinReferents extends AbstractController
{
   ...
   public void setMedecins(final List<Collaborateur> meds){
      this.medecins = meds;
   }
    ...
    * @param cur
    */
   public void setSelectedMedecin(final Collaborateur cur){
      this.selectedMedecin = cur;
   }

   public void setFicheParent(final AbstractController parent){
    ...
    */
   public void onClick$medecinLink(final Event event){
      if(getDroitsConsultation().get("Collaborateur")){
         // recupere le medecin
         final Object med = AbstractListeController2.getBindingData((ForwardEvent) event, false);

         // ouvre la modale
         openCollaborationsWindow(page, "context.modal.referent", "details", med, null, null, null, null);
      }
   }
    ...
    */
   public void onClick$deleteCell(final Event event){
   ...
   public void onClick$addMed(){
      ...
      // ouvre la modale
      openCollaborationsWindow(page, "patient.button.newMedecin", "select", null, "Collaborateur", null, Path.getPath(self), old);
   ...
   private void refreshReferentsHeaderCount(){
      Events.postEvent("onRefreshHeader", self.getParent().getParent().getFellow("referentsGroup"), null);
   }
   ...
   public void drawActionsForMedecins(){
      addMed.setDisabled(!drawActionOnOneButton("Collaborateur", "Consultation"));
