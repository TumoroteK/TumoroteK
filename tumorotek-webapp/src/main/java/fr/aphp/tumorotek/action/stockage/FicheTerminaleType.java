...
public class FicheTerminaleType extends AbstractFicheCombineController
{
   ...
   private void switchTypeBoiteDisplay(final boolean isRect){
      // on affiche les bons composants
      rowEditTaille.setVisible(isRect);
      rowEditNbLignes.setVisible(!isRect);
      rowDefinitionLignes.setVisible(!isRect);
   }
   ...
   public void onClick$definirNbLignes(){
      ...
      // TK-249: check passe du client au serveur
      if(nbLignesValue == null){
         throw new WrongValueException(nbLignesBox, MZul.EMPTY_NOT_ALLOWED);
      }
   ...
   public void defineTerminaleType(){
      ...
      if(radioBoiteRectangulaire.isChecked()){
         ...
         // TK-249: check passe du client au serveur
         if(hauteurValue == null){
            throw new WrongValueException(hauteurBox, MZul.EMPTY_NOT_ALLOWED);
         }
         if(longueurValue == null){
            throw new WrongValueException(longueurBox, MZul.EMPTY_NOT_ALLOWED);
         }
      ...
      }else if(radioBoiteComplexe.isChecked()){ // sinon, boite complexe
