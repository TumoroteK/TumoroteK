...
public class FicheContrat extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(FicheContrat.class);
	...
	// Associations.
	private List<ProtocoleType> types = new ArrayList<ProtocoleType>();
	...
	private List<String> nomsEtablissements = new ArrayList<String>();
	private List<Etablissement> etablissements = new ArrayList<Etablissement>();
	private Etablissement selectedEtablissement;
	private List<Cession> cessions = new ArrayList<Cession>();
	
	private boolean detailMode;
	
	// Variables formulaire.
	private String cessionsGroupHeader = Labels
											.getLabel("contrat.cessions.title");
	private CessionRowRenderer cessionRenderer = new CessionRowRenderer(false, false);
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[]{
				...
				this.dateDemandeCessionLabel,
				this.dateValidationLabel,
				this.dateDemandeRedactionLabel,
				this.dateEnvoiLabel,
				this.dateSignatureLabel,
				this.titreLabel,
				this.etabLabelContrat,
				this.serviceLabelContrat,
				this.operateurLabelContrat,
				this.groupCessionsContrat,
				this.cessionsGrid,
				this.rowGridCessions,
				this.menuBar,
				this.montantLabel,
				this.rowNbEchantillons,
				this.rowNbProdDerives,
				this.groupDelaisContrat,
				this.rowDelaiEnvoi,
				this.rowDelaiGlobal,
				this.rowDelaiSignature,
				this.rowDelaiValidation
		});
		...
		setObjBoxsComponents(new Component[]{
				...
				this.typesBox,
				this.descriptionBox,
				this.dateDemandeCessionBox,
				this.dateValidationBox,
				this.dateDemandeRedactionBox,
				this.dateEnvoiBox,
				this.dateSignatureBox,
				this.titreBox,
				this.etabsBoxContrat,
				this.operateursBoxDiv,
				this.servicesBoxDiv,
				this.montantBox
		});
	...
	public void setObject(TKdataObject obj) {
		...
		if (contrat.getContratId() != null) {
			cessions.addAll(ManagerLocator.getContratManager()
									.getCessionsManager(contrat));
			...
			if (this.contrat.getService() != null
					&& services.contains(this.contrat.getService())) {
			...
			cessions = new ArrayList<Cession>();
			selectedCollaborateur = null;
		...
		for (int i = 0; i < cessions.size(); i++) {
			if (cessions.get(i).getCessionStatut() != null
					&& !cessions.get(i).getCessionStatut()
					.getStatut().equals("REFUSEE")) {
				nbEchantillons += ManagerLocator.getCederObjetManager()
					.getEchantillonsCedesByCessionManager(
							cessions.get(i)).size();
				nbProdDerives += ManagerLocator.getCederObjetManager()
				.getProdDerivesCedesByCessionManager(
						cessions.get(i)).size();
			}
		}
		nbEchantillonsLabel.setValue(ObjectTypesFormatters
				.getLabel("contrat.nb.echantillons", 
				new String[]{String.valueOf(nbEchantillons)}));
		nbProdDerivesLabel.setValue(ObjectTypesFormatters
				.getLabel("contrat.nb.prodDerives", 
				new String[]{String.valueOf(nbProdDerives)}));
		
		// calcul du délai de validation
		if (this.contrat.getDateDemandeCession() != null
				&& this.contrat.getDateValidation() != null) {
			Calendar calDem = Calendar.getInstance();
			calDem.setTime(contrat.getDateDemandeCession());
			Calendar calVal = Calendar.getInstance();
			calVal.setTime(contrat.getDateValidation());
			Long diffMillis = calVal.getTimeInMillis()
				- calDem.getTimeInMillis();
			// ie 86400000 = 24*60*60*1000
			// soit le nb de lillisecondes dans une année
			Long div = new Long(86400000L);
			int jours = (int) (diffMillis / div);
			delaiValidationLabel.setValue(ObjectTypesFormatters
				.getLabel("contrat.delai.validation", 
					new String[]{String.valueOf(jours)}));
		} else {
			delaiValidationLabel.setValue(ObjectTypesFormatters
					.getLabel("contrat.delai.validation", 
						new String[]{"-"}));
		...
		if (this.contrat.getDateDemandeRedaction() != null
				&& this.contrat.getDateEnvoiContrat() != null) {
			Calendar calRedac = Calendar.getInstance();
			calRedac.setTime(contrat.getDateDemandeRedaction());
			Calendar calEnvoi = Calendar.getInstance();
			calEnvoi.setTime(contrat.getDateEnvoiContrat());
			Long diffMillis = calEnvoi.getTimeInMillis()
				- calRedac.getTimeInMillis();
			// ie 86400000 = 24*60*60*1000
			// soit le nb de lillisecondes dans une année
			Long div = new Long(86400000L);
			int jours = (int) (diffMillis / div);
			delaiEnvoiLabel.setValue(ObjectTypesFormatters
				.getLabel("contrat.delai.envoi", 
					new String[]{String.valueOf(jours)}));
		} else {
			delaiEnvoiLabel.setValue(ObjectTypesFormatters
					.getLabel("contrat.delai.envoi", 
		...
		if (this.contrat.getDateEnvoiContrat() != null
				&& this.contrat.getDateSignature() != null) {
			Calendar calEnvoi = Calendar.getInstance();
			calEnvoi.setTime(contrat.getDateEnvoiContrat());
			Calendar calSign = Calendar.getInstance();
			calSign.setTime(contrat.getDateSignature());
			Long diffMillis = calSign.getTimeInMillis()
				- calEnvoi.getTimeInMillis();
			// ie 86400000 = 24*60*60*1000
			// soit le nb de lillisecondes dans une année
			Long div = new Long(86400000L);
			int jours = (int) (diffMillis / div);
			delaiSignatureLabel.setValue(ObjectTypesFormatters
				.getLabel("contrat.delai.signature", 
					new String[]{String.valueOf(jours)}));
		} else {
			delaiSignatureLabel.setValue(ObjectTypesFormatters
					.getLabel("contrat.delai.signature", 
		...
		if (this.contrat.getDateDemandeCession() != null
			...
			Long div = new Long(86400000L);
			int jours = (int) (diffMillis / div);
			delaiGlobalLabel.setValue(ObjectTypesFormatters
				.getLabel("contrat.delai.global", 
					new String[]{String.valueOf(jours)}));
		} else {
			delaiGlobalLabel.setValue(ObjectTypesFormatters
					.getLabel("contrat.delai.global", 
	 ...
	 */
	public void switchToStaticMode(boolean dM) {
		this.detailMode = dM;
		switchToStaticMode();
	...
	public void createNewObject() {
		...
		ManagerLocator.getContratManager().createObjectManager(
				contrat, selectedCollaborateur, 
				selectedService,
	...
	private ListeContrat getListeContrat() {
		return (ListeContrat) getObjectTabController().getListe();
	}
...
//	}
	...
	public boolean prepareDeleteObject() {
		boolean isUsed = ManagerLocator
			.getContratManager().isUsedObjectManager(getObject());
		setCascadable(false);
		...
		if (isUsed) {
			setDeleteMessage(Labels
					.getLabel("contrat.deletion.isReferenced"));
		} 
		setDeletable(!isUsed);		
	...
	public void removeObject(String comments) {		
		ManagerLocator.getContratManager().
...
//	}
	...
	public void setFieldsToUpperCase() {
		if (this.contrat.getNumero() != null) {
			this.contrat.setNumero(this.contrat
					.getNumero().toUpperCase().trim());
		}
		
		if (this.contrat.getTitreProjet() != null) {
			this.contrat.setTitreProjet(this.contrat
					.getTitreProjet().toUpperCase().trim());
		}
	}
	 ...
	 * @throws Exception
	 */
	public void onSelect$etabsBoxContrat(Event event) throws Exception {
		int ind = etabsBoxContrat.getSelectedIndex();
		if (ind > -1) {
	 ...
	 * @throws Exception
	 */
	public void onSelect$servicesBoxContrat(Event event) throws Exception {
		int ind = servicesBoxContrat.getSelectedIndex();
		if (ind > -1) {
	 ...
	 * @throws Exception
	 */
	public void onSelect$operateursBoxContrat(Event event) throws Exception {
		int ind = operateursBoxContrat.getSelectedIndex();
		if (ind > -1) {
...
//		}
	...
	public void onSelectAllCessions(Event e) {
		...
		displayObjectsListData(new 
				ArrayList<TKAnnotableObject>(getCessions()));
	}
	...
	public void setEmptyToNulls() {
		...
		String selectedNomAndPremon = operateursBoxContrat
		...
		// Gestion du service
		String selectedNomService = servicesBoxContrat.getValue().toUpperCase();
		...
		String selectedNom = etabsBoxContrat.getValue().toUpperCase();
	...
	public void updateObject() {
		...
		ManagerLocator.getContratManager().updateObjectManager(
				contrat, selectedCollaborateur, 
				selectedService, 
				selectedEtablissement,
				selectedType, 
	...
	public void initEditableMode() {
		if (types.isEmpty()) {
			types.addAll((List<ProtocoleType>) 
					ManagerLocator.getProtocoleTypeManager()
	...
	public void initCollaborations() {
		...
		etabsBoxContrat.setModel(
				new CustomSimpleListModel(nomsEtablissements));
		etabsBoxContrat.setValue("");
		if (this.contrat.getEtablissement() != null) {
			selectedEtablissement = this.contrat.getEtablissement();
			etabsBoxContrat.setValue(
					selectedEtablissement.getNom());	
			if (!etablissements.contains(
					this.contrat.getEtablissement())) {
				etablissements.add(this.contrat.getEtablissement());
				selectedEtablissement = this.contrat.getEtablissement();
				nomsEtablissements.add(this.contrat.getEtablissement()
													.getNom());
			}
		}
		...
		operateursBoxContrat.setModel(
				new CustomSimpleListModel(nomsAndPrenoms));
		operateursBoxContrat.setValue("");
		if (this.contrat.getCollaborateur() != null) {
			selectedCollaborateur = this.contrat.getCollaborateur();
			operateursBoxContrat.setValue(
					selectedCollaborateur.getNomAndPrenom());	
			if (!collaborateurs.contains(
					this.contrat.getCollaborateur())) {
				collaborateurs.add(this.contrat.getCollaborateur());
				selectedCollaborateur = this.contrat.getCollaborateur();
				nomsAndPrenoms.add(this.contrat.getCollaborateur()
													.getNomAndPrenom());
			}
		}
	}
	...
	public Document createDocumentXML() {
		...
		Element page1 = ManagerLocator.getXmlUtils().addPage(
				...
				.getLabel("impression.contrat.title", 
				new String[]{contrat.getNumero()}));
	...
	public void addInfosProjetToPrint(Element page) {
		...
		CoupleValeur cp3 = new CoupleValeur(
				Labels.getLabel("contrat.titreProjet"), 
				tmp);
		...
		CoupleValeur cp4 = new CoupleValeur(
				Labels.getLabel("contrat.protocoleType"), 
				tmp);
		...
		CoupleValeur cp6 = new CoupleValeur(
				Labels.getLabel("contrat.etablissement"), 
				tmp);
		LigneDeuxColonnesParagraphe li5 = new LigneDeuxColonnesParagraphe(cp6);
		
		// Service
		if (contrat.getService() != null
				&& contrat.getService().getNom() != null) {
			tmp = contrat.getService().getNom();
		} else {
			tmp = "-";
		}
		CoupleValeur cp7 = new CoupleValeur(
				Labels.getLabel("contrat.service"), 
				tmp);
		LigneDeuxColonnesParagraphe li6 = new LigneDeuxColonnesParagraphe(cp7);
		...
		CoupleValeur cp8 = new CoupleValeur(
				Labels.getLabel("contrat.collaborateur"), 
				tmp);
		LigneDeuxColonnesParagraphe li7 = new LigneDeuxColonnesParagraphe(cp8);
		
		Paragraphe par1 = new Paragraphe(
				Labels.getLabel("contrat.group.projet"),  
				new Object[]{li1, li2, li3, li4, li5, li6, li7}, 
				null,
				null, null);
	 ...
	 * @param page
	 */
	public void addInfosContratToPrint(Element page) {
		// Date de demande de rédaction
		String tmp = "";
		if (contrat.getDateDemandeRedaction() != null) {
			tmp = ObjectTypesFormatters
				.dateRenderer2(contrat.getDateDemandeRedaction());
		} else {
			tmp = "-";
		}
		CoupleValeur cp1 = new CoupleValeur(
				Labels.getLabel("contrat.dateDemande.redaction"), 
				tmp);
		// Date d'envoie du contrat
		if (contrat.getDateEnvoiContrat() != null) {
			tmp = ObjectTypesFormatters.dateRenderer2(
				contrat.getDateEnvoiContrat());
		} else {
			tmp = "-";
		}
		CoupleValeur cp2 = new CoupleValeur(
				Labels.getLabel("contrat.dateEnvoi"), 
				tmp);
		LigneParagraphe li1 = new LigneParagraphe("",
				new CoupleValeur[]{cp1, cp2});
		...
		CoupleValeur cp3 = new CoupleValeur(
				Labels.getLabel("contrat.dateSignature"), 
				tmp);
		LigneDeuxColonnesParagraphe li2 = new LigneDeuxColonnesParagraphe(cp3);
		...
		CoupleValeur cp4 = new CoupleValeur(
				Labels.getLabel("contrat.montant"), 
				tmp);
		LigneDeuxColonnesParagraphe li3 = new LigneDeuxColonnesParagraphe(cp4);
		
		Paragraphe par1 = new Paragraphe(
				Labels.getLabel("contrat.group.contrat"),  
				new Object[]{li1, li2, li3}, 
				null,
				null, null);
		ManagerLocator.getXmlUtils().addParagraphe(page, par1);
	}
	 ...
	 * @param page
	 */
	public void addInfosDelaisToPrint(Element page) {
		// delai de validation
		String tmp = "";
		if (this.contrat.getDateDemandeCession() != null
				&& this.contrat.getDateValidation() != null) {
		...
		CoupleSimpleValeur cp2 = new CoupleSimpleValeur(
				ObjectTypesFormatters
					.getLabel("contrat.delai.envoi", 
						new String[]{String.valueOf(tmp)}),
				"");
		...
		CoupleSimpleValeur cp4 = new CoupleSimpleValeur(
				ObjectTypesFormatters
					.getLabel("contrat.delai.global", 
						new String[]{String.valueOf(tmp)}),
				"");
		...
		Paragraphe par1 = new Paragraphe(
				Labels.getLabel("contrat.group.delais"),  
	 ...
	 */
	public void addInfosListeCessions(Element page) {
		// Entete
		...
		// liste des cédés
		LigneListe[] liste = new LigneListe[cessions.size()];
		for (int i = 0; i < cessions.size(); i++) {
			String[] valeurs = new String[6];
			// numero
			valeurs[0] = cessions.get(i).getNumero();
			// nb echantillons
			valeurs[1] = String.valueOf(ManagerLocator
					.getCederObjetManager()
					.getEchantillonsCedesByCessionManager(
						cessions.get(i)).size());
			// nb derives
			valeurs[2] = String.valueOf(ManagerLocator
					.getCederObjetManager()
					.getProdDerivesCedesByCessionManager(
						cessions.get(i)).size());
			// date de demande
			if (cessions.get(i).getDemandeDate() != null) {
				valeurs[3] = ObjectTypesFormatters
					.dateRenderer2(cessions.get(i).getDemandeDate());
			} else {
				valeurs[3] = "-";
			}
			// date de cession
			if (cessions.get(i).getDepartDate() != null) {
				valeurs[4] = ObjectTypesFormatters
					.dateRenderer2(cessions.get(i).getDepartDate());
			} else {
				valeurs[4] = "-";
			}
			// cession statut
			if (cessions.get(i).getCessionStatut() != null) {
				valeurs[5] = ObjectTypesFormatters
						.ILNObjectStatut(cessions.get(i).getCessionStatut());
			} else {
				valeurs[5] = "-";
			}
	...
	public void onBlur$dateDemandeCessionBox() {
		...
		if (!badDateFormat) {
			...
			validateCoherenceDate(dateDemandeCessionBox, 
					dateDemandeCessionBox.getValue());
		}
	}
	...
	public void onBlur$dateDemandeRedactionBox() {
		...
		if (!badDateFormat) {
			...
			validateCoherenceDate(dateDemandeRedactionBox, 
					dateDemandeRedactionBox.getValue());
		}
	}
	...
	public void onBlur$dateEnvoiBox() {
		...
		if (!badDateFormat) {
			...
			validateCoherenceDate(dateEnvoiBox, 
					dateEnvoiBox.getValue());
		}
	}
	...
	public void onBlur$dateSignatureBox() {
		...
		if (!badDateFormat) {
			...
			validateCoherenceDate(dateSignatureBox, 
					dateSignatureBox.getValue());
		}
	}
	 ...
	 * @param collab
	 */
	public void selectCollaborateurFromModale(Collaborateur collab) {
		if (nomsAndPrenoms.contains(collab.getNomAndPrenom())) {
			int ind = nomsAndPrenoms.indexOf(collab.getNomAndPrenom());
			selectedCollaborateur = collaborateurs.get(ind);
			operateursBoxContrat.setValue(
					selectedCollaborateur.getNomAndPrenom());
			
			List<Service> tmpServices = new ArrayList<Service>();
			tmpServices.addAll(ManagerLocator.getCollaborateurManager()
					.getServicesManager(selectedCollaborateur));
			if (tmpServices.size() == 1) {
				...
				while (i < tmpServices.size() && !found) {
					if (tmpServices.get(i).getEtablissement().equals(
							selectedCollaborateur.getEtablissement())) {
			...
			if (selectedService != null) {
				servicesBoxContrat.setValue(nomsServices.get(
	 ...
	 * @param collab
	 */
	public void selectServiceFromModale(Service serv) {
	...
	public void setSelectedType(ProtocoleType selected) {
	...
	public String getSClassCollaborateur() {
		if (this.contrat != null) {
			return ObjectTypesFormatters
				.sClassCollaborateur(this.contrat.getCollaborateur());
		} else {
			return "";
		}
	}
	...
	public String getSClassService() {
		if (this.contrat != null) {
			return ObjectTypesFormatters
							.sClassService(this.contrat.getService());
		} else {
			return "";
		}
	}
	...
	public String getDateDemandeCessionFormated() {		
		if (this.contrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.contrat.getDateDemandeCession());
		} else {
			return null;
		}
	}
	...
	public String getDateValidationFormated() {		
		if (this.contrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.contrat.getDateValidation());
		} else {
			return null;
		}
	}
	...
	public String getDateDemandeRedactionFormated() {		
		if (this.contrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.contrat.getDateDemandeRedaction());
		} else {
			return null;
		}
	}
	...
	public String getDateEnvoiFormated() {		
		if (this.contrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.contrat.getDateEnvoiContrat());
		} else {
			return null;
		}
	}
	...
	public String getDateSignatureFormated() {		
		if (this.contrat != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.contrat.getDateSignature());
		} else {
			return null;
		}
	}
	...
	public void setSelectedEtablissement(Etablissement selectedE) {
	...
	public void setNbEchantillons(Integer nbE) {
	...
	public void setNbProdDerives(Integer nbP) {
	...
	public void setNomsEtablissements(List<String> n) {
