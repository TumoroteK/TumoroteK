 ...
 * @since 2.2.1 action enceinte disabled si accès depuis autre plateforme
 * @since 2.2.3-genno fix https://tumorotek.myjetbrains.com/youtrack/issue/TK-291
 *
...
public class FicheTerminale extends AbstractFicheCombineStockageController
{
   ...
   private void updateButtonActionsWithPlateformeOrigine(){
      ...
      final Conteneur cont =
         terminale != null ? ManagerLocator.getEnceinteManager().getConteneurManager(terminale.getEnceinte()) : null;
      final boolean orig = cont != null ? cont.getPlateformeOrig().equals(SessionUtils.getCurrentPlateforme()) : false;
   ...
   public void onClick$deplacer(){
      if(this.terminale.getTerminaleId() != null){
         ...
         getObjectTabController().getListeStockages()
            .switchToDeplacementMode(ManagerLocator.getTerminaleManager().getDistinctBanquesFromTkObjectsManager(terminale));
      }
   }
   ...
   public void createRetoursApresDeplacementTerminale(){
      ...
      if(!emplForRetours.isEmpty()){
         ...
         // créer des retours
         if(askForRetoursCreation(emplForRetours.size(), null, null, new ArrayList<OldEmplTrace>(), null,
   ...
   public void onLaterContenu(){
      ...
      // s'il n'y a pas d'erreurs lors de l'update
      if(!getObjStatutIncompatibleForRetour(emplForRetours.stream().map(o -> o.getTkObj()).collect(Collectors.toList()), null)
         && changeContenu()){
   ...
   public void initEditableMode(){
      ...
      banques = ManagerLocator.getUtilisateurManager().getAvailableBanquesByPlateformeManager(
         SessionUtils.getLoggedUser(sessionScope), SessionUtils.getCurrentPlateforme(), false);
      banques.add(0, null);
    ...
    * @param node TerminaleNode représentant la destination de l'enceinte.
    ...
    * @version 2.2.1
    */
   public void definirTerminaleDestination(final TerminaleNode node, final Conteneur uCont, final List<Banque> missBanks){
   ...
   public List<TKStockableObject> prepareObjectsAndEmplacements(){
      ...
      // TK-291
      emplForRetours.addAll(ManagerLocator.getTerminaleManager().getTkObjectsEmplacementTracesManager(terminale));
      ...
      if(terminaleDestination != null && !terminaleDestination.isVide()){
         ...
         // TK-291
         emplForRetours.addAll(
            ManagerLocator.getTerminaleManager().getTkObjectsEmplacementTracesManager(terminaleDestination.getTerminale()));
