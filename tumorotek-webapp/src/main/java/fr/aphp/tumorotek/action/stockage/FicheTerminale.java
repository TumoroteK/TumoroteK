...
public class FicheTerminale extends AbstractFicheCombineStockageController {
	...
	private Log log = LogFactory.getLog(FicheTerminale.class);
	...
	private List<TerminaleType> types = new ArrayList<TerminaleType>();
	private TerminaleType selectedTerminaleType;
	private List<TerminaleNumerotation> numerotations = new ArrayList<TerminaleNumerotation>();
	...
	private List<EmplacementDecorator> emplacementDecos = new ArrayList<EmplacementDecorator>();
	private List<Emplacement> movedEmplacements = new ArrayList<Emplacement>();
	private CouleurItemRenderer couleurRenderer = new CouleurItemRenderer();
	private ListModelList<Couleur> couleurs = new ListModelList<Couleur>();
	...
	private List<Div> imagesEmplacements = new ArrayList<Div>();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[] { this.nomLabel,
				this.aliasLabel, this.entiteReserveeLabel,
				this.banqueReserveeLabel, this.modeleBoite, this.rowViewBoite,
				this.menuBar, this.couleurTerminaleImg,
				this.incidentsList});

		setObjBoxsComponents(new Component[] { this.nomBox, this.aliasBox,
				this.entiteBox, this.banqueBox, this.couleurTerminaleBox,
				this.incidentsListEdit});

		setRequiredMarks(new Component[] { this.nomRequired });

		this.objRowsComponents = new Component[] { this.rowNomAlias,
				this.rowType, this.rowNbPlaces };

		this.objDeplacementComponents = new Component[] {
				this.rowEmplacementTitle1, this.rowAdresseActuelle,
				this.rowPositionActuelle, this.rowEmplacementTitle2,
				this.rowAdresseDestination, this.rowPositionDestination,
				this.rowContenuDestination, this.validateDeplacement,
				this.revertDeplacement, this.rowNomDepart,
				this.rowNomDestination };

		this.objContenuComponents = new Component[] { this.rowContenuInfo1,
				this.validateContenu, this.revertContenu, this.rowHistorique,
				this.rowHistoriqueTitle };
	...
	@Override
	public void setObject(TKdataObject t) {
		...
		if (terminale.getTerminaleId() != null) {
			...
			initIncidents(ManagerLocator.getIncidentManager()
					.findByTerminaleManager(terminale));
			...
			selectedTerminaleNumerotation = this.terminale
					.getTerminaleNumerotation();

			if (this.terminale != null
					&& this.terminale.getTerminaleId() != null) {
				adrlTerminale = ManagerLocator.getEmplacementManager()
						.getTerminaleAdrlManager(terminale);
			}
	...
	public void switchToEditMode() {
		...
		if (!ManagerLocator.getTerminaleManager()
				.isUsedObjectManager(terminale)) {
			typeLabel.setVisible(false);
			typesBox.setVisible(true);
		} else {
			typeLabel.setVisible(true);
	...
	public void switchToDeplacerMode() {
		...
		// adresse de départ
		adresseActuelle = ManagerLocator.getEmplacementManager()
				.getTerminaleAdrlManager(terminale);
	...
	public void switchToContenuMode() {
		...
		for (int i = 0; i < getImagesEmplacements().size(); i++) {
			Div img = getImagesEmplacements().get(i);
			EmplacementDecorator deco = (EmplacementDecorator) img
		...
		movedEmplacements = new ArrayList<Emplacement>();
	...
	public void createNewObject() {
		...
		try {
			...
			// update de l'objet
			ManagerLocator.getTerminaleManager().createObjectManager(terminale,
					this.enceinte, selectedTerminaleType, selectedBanque,
					selectedEntite, selectedTerminaleNumerotation,
					selectedCouleur, SessionUtils.getLoggedUser(sessionScope));
		} catch (Exception e) {
	...
	public boolean prepareDeleteObject() {
		boolean isUsed = ManagerLocator.getTerminaleManager()
				.isUsedObjectManager(getObject());

		setDeleteMessage(ObjectTypesFormatters.getLabel(
				"message.deletion.message",
				new String[] { Labels.getLabel(getDeletionMessage()) }));
		setCascadable(false);
		if (isUsed) {
	...
	public void removeObject(String comments) {
		ManagerLocator.getTerminaleManager().removeObjectManager(getObject(),
				comments, SessionUtils.getLoggedUser(sessionScope));
	}
	...
	public void setEmptyToNulls() {
		...
		if (!couleurs.getSelection().isEmpty()) {
			Couleur c = couleurs.getSelection().iterator().next();
...
//		}
	...
	public void updateObject() {
		try {
			...
			// update de l'objet
			ManagerLocator.getTerminaleManager().updateObjectManager(terminale,
					this.terminale.getEnceinte(), selectedTerminaleType,
					selectedBanque, selectedEntite,
					selectedTerminaleNumerotation, selectedCouleur,
					getIncidents(),
	...
	protected List<String> deplacerTerminale() {
		...
		Enceinte ep = terminale.getEnceinte();
		Integer p = terminale.getPosition();
		...
		try {
			...
			if (terminaleDestination != null && terminaleDestination.isVide()) {
				...
				List<OperationType> ops = ManagerLocator
						.getOperationTypeManager().findByNomLikeManager(
								"Deplacement", true);
				ManagerLocator.getTerminaleManager().updateObjectManager(
						terminale,
						terminaleDestination.getTerminale().getEnceinte(),
						terminale.getTerminaleType(), terminale.getBanque(),
						terminale.getEntite(),
						terminale.getTerminaleNumerotation(),
						terminale.getCouleur(), null,
						SessionUtils.getLoggedUser(sessionScope), ops);
			} else {
				// on échange des emplacements des 2 terminales
				terminaleAEchanger = terminaleDestination.getTerminale()
						.clone();
				...
				ManagerLocator.getTerminaleManager()
						.echangerDeuxTerminalesManager(terminale,
								terminaleAEchanger,
			...
			errorMsg.add("- Erreur sur la "
					+ "position de l'enceinte terminale.");
	...
	protected boolean changeContenu() {
		...
		try {
			...
			ManagerLocator.getEmplacementManager()
					.deplacerMultiEmplacementsManager(movedEmplacements,
							SessionUtils.getLoggedUser(sessionScope));
			
			for (int i = 0; i < movedEmplacements.size(); i++) {
				Emplacement empl = movedEmplacements.get(i);
				if (empl.getEntite() != null && empl.getObjetId() != null
						&& empl.getEntite().getNom().equals("Echantillon")) {
					...
					if (getMainWindow().isFullfilledComponent(
						...
						if (getObjectTabController().getEchantillonController() != null) {
							...
							getObjectTabController().getEchantillonController()
									.getListe()
	...
	public void onLaterCreate() {
		...
		try {
			...
			if (getObjectTabController().getListeStockages() != null) {
				...
				getObjectTabController().getListeStockages().updateTerminale(
						terminale, true);
			}
			...
			Clients.clearBusy();
			Messagebox.show(handleExceptionMessage(re), "Error", Messagebox.OK,
					Messagebox.ERROR);
		} finally {
			Clients.clearBusy();
		}
	...
	public void onLaterDelete(Event event) {
		try {
			...
			// des stockages
			if (getObjectTabController().getListeStockages() != null) {
				getObjectTabController().getListeStockages().deleteTerminale(
						enceinte, position);
			}
	...
	public void onLaterDeplacement() {
		...
		if (!getObjStatutIncompatibleForRetour(prepareObjectsAndEmplacements(), null) 
			...
			if (getObjectTabController().getListeStockages() != null) {
				...
				getObjectTabController().getListeStockages()
						.deplacerDeuxTerminales(terminale, terminaleAEchanger);
			}
...
//		}
		if (!emplForRetours.isEmpty()) {
			...
			// créer des retours
			if (askForRetoursCreation(emplForRetours.size(), null, null, 
					new HashMap<TKStockableObject, Emplacement>(), null,
...
//				}
				...
				openRetourFormModale(null, true, null, null, new ArrayList<TKStockableObject>(emplForRetours.keySet()), 
						emplForRetours,
						null, null, null, null,
	...
	public void onLaterContenu() {
		...
		// s'il n'y a pas d'erreurs lors de l'update
		if (!getObjStatutIncompatibleForRetour(new 
				ArrayList<TKStockableObject>(emplForRetours.keySet()), null) 
				&& changeContenu()) {
	...
	public void initEditableMode() {
		...
		types = ManagerLocator.getTerminaleTypeManager()
	...
	public void definirTerminaleDestination(TerminaleNode node) {
		...
		if (node != null) {
			...
			positionDestination = terminaleDestination.getTerminale()
					.getPosition();
			...
			if (!terminaleDestination.isVide()) {
				contenuDestination = terminaleDestination.getTerminale()
						.getNom();
				adresseDestination = ManagerLocator.getEmplacementManager()
						.getTerminaleAdrlManager(
								terminaleDestination.getTerminale());
				rowNomDestination.setVisible(true);
				getBinder().loadAttribute(self.getFellow("nomDestinationBox"),
						"value");
				...
				StringBuffer sb = new StringBuffer();
				sb.append(ManagerLocator.getEnceinteManager().getAdrlManager(
						terminaleDestination.getTerminale().getEnceinte()));
				sb.append(".");
				sb.append(terminaleDestination.getTerminale().getEnceinte()
						.getNom());
		...
		if (e.getData() != null) {
			...
			if (!deco.getVide()) {
				if (deco.getEmplacement().getEntite().getNom()
						.equals("Echantillon")) {
					Echantillon echan = (Echantillon) ManagerLocator
							.getEntiteManager().findObjectByEntiteAndIdManager(
									deco.getEmplacement().getEntite(),
									deco.getEmplacement().getObjetId());
					...
					if (echan != null) {
						if (!echan.getBanque().getArchive()) {
							if (getDroitEchantillonConsultation(echan)) {
								...
								Messagebox.show(Labels
										.getLabel("cederObjet.acces.interdit"),
										Labels.getLabel("cederObjet.acces"
												+ ".interdit.title"),
										Messagebox.OK, Messagebox.EXCLAMATION);
							}
						} else { // banque archivée
							Messagebox.show(Labels
									.getLabel("banque.archived.objet.warning"),
									Labels.getLabel("cederObjet.acces.interdit"),
									Messagebox.OK, Messagebox.EXCLAMATION);
						}
					}
						...
						.equals("ProdDerive")) {
					ProdDerive derive = (ProdDerive) ManagerLocator
							.getEntiteManager().findObjectByEntiteAndIdManager(
									deco.getEmplacement().getEntite(),
									deco.getEmplacement().getObjetId());
	 ...
	 * @param obj
	 */
	public void switchToFicheObject(Object obj) {
		if (obj instanceof Echantillon) {
			

			if (!SessionUtils.getSelectedBanques(sessionScope).contains(
					((Echantillon) obj).getBanque())) {
				getMainWindow().updateSelectedBanque(
						((Echantillon) obj).getBanque());
			}
			
			EchantillonController tabController = (EchantillonController) EchantillonController
					.backToMe(getMainWindow(), page);

			tabController.switchToFicheStaticMode((Echantillon) obj);
		} else if (obj instanceof ProdDerive) {
			ProdDeriveController tabController = (ProdDeriveController) ProdDeriveController
					.backToMe(getMainWindow(), page);

			// change la banque au besoin
			if (!SessionUtils.getSelectedBanques(sessionScope).contains(
					((ProdDerive) obj).getBanque())) {
				getMainWindow().updateSelectedBanque(
						((ProdDerive) obj).getBanque());
			}
	 ...
	 * @return
	 */
	public Boolean getDroitEchantillonConsultation(Echantillon echan) {
		boolean acces = false;

		Banque bk = echan.getBanque();
	 ...
	 * @version 2.1
	 */
	public Boolean getDroitProdDeriveConsultation(ProdDerive derive) {
		boolean acces = false;
	...
	public void onDropImage(DropEvent e) {
		...
		if (e.getTarget() != null) {
			...
			EmplacementDiv imgDep = (EmplacementDiv) e.getDragged();
			EmplacementDiv imgDest = (EmplacementDiv) e.getTarget();
			...
			if ((Messagebox.show(sb.toString(),
					Labels.getLabel("validation.drop.title"), Messagebox.YES
							| Messagebox.NO, Messagebox.QUESTION) == Messagebox.YES)) {

				Emplacement empDep = decoDep.getEmplacement();
				Emplacement empDest = decoDest.getEmplacement();
				
				// retours
				if (!empDep.getVide()) {
					if (empDep.getEntite().getNom().equals("Echantillon")) {
						emplForRetours.put(ManagerLocator.getEchantillonManager()
							.findByIdManager(empDep.getObjetId()), empDep.clone());
					} else if (empDep.getEntite().getNom().equals("ProdDerive")) {
						emplForRetours.put(ManagerLocator.getProdDeriveManager()
							.findByIdManager(empDep.getObjetId()), empDep.clone());
					}
				}
				if (!empDest.getVide()) {
					if (empDest.getEntite().getNom().equals("Echantillon")) {
						emplForRetours.put(ManagerLocator.getEchantillonManager()
							.findByIdManager(empDest.getObjetId()), empDest.clone());
				...
				Integer posDep = decoDep.getPosition();
				Integer posDest = decoDest.getPosition();
				...
				String styleImgDep = imgDep.getImageStyle();
				String styleImgDest = imgDest.getImageStyle();
				imgDep.setImageStyle(styleImgDest);
				imgDest.setImageStyle(styleImgDep);
				String overlayClassDep = imgDep.getOverlay();
				String overlayClassDest = imgDest.getOverlay();
	...
	public void onClick$deplacerEmplacements() {
		...
		getObjectTabController().switchToDeplacerEmplacementsMode(
						this.terminale, mismatches);
	}
	...
	public void onClick$destockerEmplacements() {
		...
		if (currScanTerminaleDTO != null) {
			decos = new ArrayList<EmplacementDecorator>();
			Emplacement empl = new Emplacement();
			for (ScanTube tube : currScanTerminaleDTO.getEmplacementsToFree().keySet()) {
				empl.setTerminale(currScanTerminaleDTO.getTerminale());
				empl.setPosition(tube.getPosition());
				decos.add(getEmplacementDecos()
						.get(getEmplacementDecos()
								.indexOf(new EmplacementDecorator(empl))));
			}
			currScanTerminaleDTO = null;
		}
		getObjectTabController()
			.switchToDestockageEmplacementsMode(this.terminale, decos);
	}
	...
	public void onLaterAffichageEchantillons() {
		...
		EchantillonController tabController = (EchantillonController) EchantillonController
				.backToMe(getMainWindow(), page);
		tabController.getListe().updateListContent(
				ManagerLocator.getTerminaleManager().getEchantillonsManager(
						terminale));
	...
	public void onLaterAffichageDerives() {
		...
		ProdDeriveController tabController = (ProdDeriveController) ProdDeriveController
				.backToMe(getMainWindow(), page);
		tabController.getListe().updateListContent(
				ManagerLocator.getTerminaleManager().getProdDerivesManager(
						terminale));
		tabController.switchToOnlyListeMode();
	...
	public void initEmplacements() {
		...
		emplacements = ManagerLocator.getEmplacementManager()
		...
		for (int i = 0; i < emplacements.size(); i++) {
			...
			while (emp.getPosition() > cpt) {
				Emplacement newE = new Emplacement();
				newE.setPosition(cpt);
				newE.setTerminale(terminale);
				EmplacementDecorator deco = new EmplacementDecorator(newE);
			...
			EmplacementDecorator deco = new EmplacementDecorator(emp);
		...
		// si aucun emplacement n'est défini à la position "cpt"
		// on crée un decorator vide
		while (terminale.getTerminaleType().getNbPlaces() > getEmplacementDecos()
				.size()) {
			Emplacement newE = new Emplacement();
			newE.setPosition(cpt);
			newE.setTerminale(terminale);
	...
	public void initModelisation() {
		...
		if (terminale.getTerminaleType().getScheme() == null) {
			...
			if (terminale.getTerminaleType().getHauteur() != null
				...
				Vlayout mainVbox = new Vlayout();
				mainVbox.setSpacing("0");
				mainVbox.setStyle("overflow:visible");
				
				int width = 20 + (30 * terminale.getTerminaleType().getLongueur());
				...
				int cpt = 0;
				List<Hlayout> lignes = new ArrayList<Hlayout>();
				for (int i = 0; i < terminale.getTerminaleType().getHauteur(); i++) {
					...
					for (int j = 0; j < terminale.getTerminaleType()
							.getLongueur(); j++) {
						EmplacementDecorator deco = getEmplacementDecos().get(cpt);

						Div img = createImage(deco);
						img.setParent(hBox);
			...
			String[] values = terminale.getTerminaleType().getScheme()
					.split(";");
	...
	public String getValueAbscisse(Integer num) {
		if (terminale.getTerminaleNumerotation().getColonne().equals("CAR")) {
			return Utils.createListChars(num, null, new ArrayList<String>())
					.get(num - 1);
		} else {
	...
	public Div createImage(EmplacementDecorator deco) {
		...
		if (deco.getVide()) {
			...
			if (getCurrentSelection() != null 
					&& getCurrentSelection().getPosition()
					.equals(deco.getEmplacement().getPosition())) {
...
//	}
	 ...
	 * @since 2.0.13
	 */
	public String getImageStyle(EmplacementDecorator deco) {
		StringBuffer sb = new StringBuffer();
		...
		return "background: transparent url('" 
			+ Executions.getCurrent().encodeURL(sb.toString()) 
			+ "');";
	}
...
//	}
	...
	public void onClick$print() {
		if (this.terminale.getTerminaleId() != null) {
			Document doc = ManagerLocator.getXmlUtils()
					.createJDomDocumentContenuBoite();
			Element root = doc.getRootElement();
			Element page = ManagerLocator.getXmlUtils().addPageContenuBoite(
					root, Labels.getLabel("impression.contenu.boite.title"));

			// création des parents pour récupérer la boite
			// Récup des parents de la boite
			List<Object> parents = ManagerLocator.getTerminaleManager()
					.getListOfParentsManager(this.terminale);
			...
			for (int j = 0; j < parents.size(); j++) {
				if (parents.get(j).getClass().getSimpleName()
					...
					instructions.add(ObjectTypesFormatters.getLabel(
							"impression.contenu" + ".boite.parent.conteneur",
							new String[] { c.getCode() }));
				} else if (parents.get(j).getClass().getSimpleName()
						.equals("Enceinte")) {
					Enceinte e = (Enceinte) parents.get(j);
					instructions.add(ObjectTypesFormatters.getLabel(
							"impression.contenu" + ".boite.parent.enceinte",
							new String[] { e.getNom() }));
				}
			}
			instructions.add(ObjectTypesFormatters.getLabel(
					"impression.contenu" + ".boite.parent.terminale",
					new String[] { terminale.getNom() }));

			List<String> statistiques = new ArrayList<String>();
			// nombre d'échantillons dans la boite
			statistiques.add(ObjectTypesFormatters.getLabel(
					"impression.contenu" + ".boite.nb.echantillons",
					new String[] { String.valueOf(ManagerLocator
							.getTerminaleManager()
							.getEchantillonsManager(terminale).size()) }));

			// nombre de dérivés dans la boite
			statistiques.add(ObjectTypesFormatters.getLabel(
					"impression.contenu" + ".boite.nb.prodDerives",
					new String[] { String.valueOf(ManagerLocator
							.getTerminaleManager()
							.getProdDerivesManager(terminale).size()) }));

			BoiteContenu boite = new BoiteContenu(terminale, statistiques,
					instructions,
					Labels.getLabel("impression.contenu.boite.vide"));

			Hashtable<String, String> echantillonsTypesCouleurs = new Hashtable<String, String>();
			String echantillonCouleur = null;
			if (getMainWindow().getSelectedBanque().getEchantillonCouleur() != null) {
				echantillonCouleur = getMainWindow().getSelectedBanque()
						.getEchantillonCouleur().getCouleur();
			}
			Hashtable<String, String> prodDerivesTypesCouleurs = new Hashtable<String, String>();
			String prodDeriveCouleur = null;
			if (getMainWindow().getSelectedBanque().getProdDeriveCouleur() != null) {
				prodDeriveCouleur = getMainWindow().getSelectedBanque()
						.getProdDeriveCouleur().getCouleur();
			}
			if (getMainWindow().getEchantillonTypesCouleur() != null) {
				echantillonsTypesCouleurs = getMainWindow()
						.getEchantillonTypesCouleur();
			}
			// si l'emplacement contient un dérivé
			if (getMainWindow().getProdDeriveTypesCouleur() != null) {
				prodDerivesTypesCouleurs = getMainWindow()
						.getProdDeriveTypesCouleur();
			}
			...
			ManagerLocator.getXmlUtils().addBoiteContenu(page, boite,
					adrImages.toString(), echantillonsTypesCouleurs,
					echantillonCouleur, prodDerivesTypesCouleurs,
					prodDeriveCouleur);
	...
	public String getCouleurSrc() {
		...
		if (this.terminale != null && this.terminale.getTerminaleId() != null
				&& this.terminale.getCouleur() != null) {
			sb.append(colorSrc);
	...
	public void setBanqueReservee(String reservee) {
	...
	public void setSelectedBanque(Banque selected) {
	...
	public void setEnceinte(Enceinte e) {
		this.enceinte = e;
	}
	...
	public void setTypes(List<TerminaleType> t) {
		this.types = t;
	}
	...
	public void setSelectedTerminaleNumerotation(TerminaleNumerotation selected) {
	...
	public void setEmplacements(List<Emplacement> emp) {
	...
	public void setContenuDestination(String contenuDest) {
	...
	public void setTerminaleDestination(TerminaleNode terminaleDest) {
	...
	public void setTerminaleAEchanger(Terminale terminaleAEch) {
	...
	public void setAdrlTerminale(String adrlTerm) {
	...
	public void setImagesEmplacements(List<Div> images) {
	...
	public void setMovedEmplacements(List<Emplacement> movedEmps) {
	...
	public void setCouleurs(ListModelList<Couleur> c) {
		this.couleurs = c;
	}
	...
	public void setCurrentSelection(Emplacement c) {
	 ...
	 * @param empl
	 */
	public void switchToStaticMode(Emplacement empl) {
	...
	public List<TKStockableObject> prepareObjectsAndEmplacements() {
		...
		if (terminaleDestination != null && !terminaleDestination.isVide()) {
			...
			// RETOURs dans le cadre de ce déplacement
			emplForRetours.putAll(ManagerLocator.getTerminaleManager()
					.getTkObjectsAndEmplacementsManager(terminaleDestination.getTerminale()));
		}
	...
	public void setEmplacementDecos(List<EmplacementDecorator> e) {
	...
	public void applyChecksOnScan(TKScanTerminaleDTO dto) {
		...
		// mismatches
		for (ScanTube tube : dto.getEmplacementsMismatch().keySet()) {
			...
			String info = "mismatch:" + tube.getCode() + " " + dto.getEmplacementsMismatch().get(tube).getCode();
			Clients.evalJavaScript("notifyEmplacement('" + info + "',jq('#"+ img.getUuid() + "'), 'error')");
		}
		
		// missings
		for (ScanTube tube : dto.getEmplacementsToFill().keySet()) {
			empl.setTerminale(dto.getTerminale());
			empl.setPosition(tube.getPosition());
			img = (EmplacementDiv) getImagesEmplacements().get(getEmplacementDecos()
					.indexOf(new EmplacementDecorator(empl)));
			img.setOverlay("missing");
			String info = "missing here :" + tube.getCode();
			Clients.evalJavaScript("notifyEmplacement('" + info + "',jq('#"+ img.getUuid() + "'), 'warn')");
		}
		
		// filled
		for (ScanTube tube : dto.getEmplacementsToFree().keySet()) {
			empl.setTerminale(dto.getTerminale());
			empl.setPosition(tube.getPosition());
			img = (EmplacementDiv) getImagesEmplacements().get(getEmplacementDecos()
					.indexOf(new EmplacementDecorator(empl)));
			img.setOverlay("filled");
			String info = "filled here :" + dto.getEmplacementsToFree().get(tube).getCode();
