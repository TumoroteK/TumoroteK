...
public class FicheTerminale extends AbstractFicheCombineStockageController
{
	...
	public void onClick$deplacer(){
		if(this.terminale.getTerminaleId() != null){
			...
			getObjectTabController().getListeStockages().switchToDeplacementMode( 
					ManagerLocator.getTerminaleManager().getDistinctBanquesFromTkObjectsManager(terminale));
		}
	}
		...
		if(!emplForRetours.isEmpty()){
			...
			if(askForRetoursCreation(emplForRetours.size(), null, null, new ArrayList<OldEmplTrace>(), null,
				...
				openRetourFormModale(null, true, null, null, 
					emplForRetours.stream().map(o -> o.getTkObj())
						.collect(Collectors.toList()), 
					emplForRetours, null, null, null, null, 
	...
	public void onLaterContenu(){
		...
		// s'il n'y a pas d'erreurs lors de l'update
		if(!getObjStatutIncompatibleForRetour(emplForRetours.stream().map(o -> o.getTkObj())
				.collect(Collectors.toList()), null) && changeContenu()){
	...
	public void initEditableMode(){
		...
		banques = ManagerLocator.getUtilisateurManager()
				.getAvailableBanquesByPlateformeManager(SessionUtils.getLoggedUser(sessionScope), 
						SessionUtils.getCurrentPlateforme(), false);
		banques.add(0, null);
	 ...
	 * @version 2.2.1 
	 */
	public void definirTerminaleDestination(final TerminaleNode node, Conteneur uCont, List<Banque> missBanks){
	...
	public List<TKStockableObject> prepareObjectsAndEmplacements(){
		...
		// TK-291
		emplForRetours.addAll(ManagerLocator.getTerminaleManager()
				.getTkObjectsEmplacementTracesManager(terminale));
		...
		if(terminaleDestination != null && !terminaleDestination.isVide()){
			...
			// TK-291
			emplForRetours.addAll(ManagerLocator.getTerminaleManager()
					.getTkObjectsEmplacementTracesManager(terminaleDestination.getTerminale()));
