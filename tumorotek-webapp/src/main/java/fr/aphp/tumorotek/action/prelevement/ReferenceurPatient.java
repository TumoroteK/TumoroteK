...
public class ReferenceurPatient extends GenericForwardComposer<Component> {
	...
	// maladiesBox items
	private List<MaladieDecorator> maladies = new ArrayList<MaladieDecorator>();
	private MaladieDecorator selectedMaladie;
	private MaladieDecorator selectedMaladieByNda = null;
	
	private static ListitemRenderer<Patient> patientRenderer = 
		new PatientItemRenderer(true);
	...
	public void onCheck$radioGroup() {
		...
		if (radioGroup.getSelectedItem().getValue().equals("find")) {
			displayExistingPatient(true);
		} else if (radioGroup.getSelectedItem().getValue()
													.equals("new")) {
...
//	}
	...
	public void onClick$goForIt() {
		...
		((FichePrelevementEdit) self.getParent().getParent()
		...
		((FichePrelevementEdit) self.getParent().getParent()
				...
				.openSelectPatientWindow(Path.getPath(self), 
						"onGetPatientFromSelection", false, critereValue, null);
	}
		...
		if (e.getData() != null) {
			...
			if (patSel.getPatientId() != null) {
				...
				if (SipFactory.isMessagesSip()) {
					PatientSip pSip = ManagerLocator
								.getPatientSipManager()
								.findByNipLikeManager(patSel.getNip(), true)
								.get(0);
					((FichePrelevementEdit) self.getParent().getParent()
						.getAttributeOrFellow("fwinPrelevementEdit$composer", true))
						.getObjectTabController()
	 ...
	 * @param patientNda
	 */
	public void setPatientAndMaladieFromOutSideReferenceur(
			Patient pat, Maladie maladie, String nda) {
		...
		if (pat.getPatientId() != null) {
			...
			((FicheMaladie) ficheMaladieWithPatientDiv
	...
	private void selectPatientAuto(Patient selected) {
		...
		if (this.banqueDefMaladies) {	
			List<Maladie> res = new ArrayList<Maladie>(ManagerLocator
					.getMaladieManager()
						.findByPatientNoSystemManager(selected));
			...
			if (maladies.size() > 0) { 
				if (selectedMaladieByNda == null) {
					...
					this.selectedMaladie = this.maladies
						.get(maladies.indexOf(selectedMaladieByNda));
					((FichePrelevementEdit) self.getParent().getParent()
							.getAttributeOrFellow("fwinPrelevementEdit$composer", true))
			...
			if (maladie == null) {
				...
				maladie.setLibelle(SessionUtils
		...
		((FichePrelevementEdit) self.getParent().getParent()
			.getAttributeOrFellow("fwinPrelevementEdit$composer", true))
				.selectRisques(ManagerLocator.getRisqueManager()
					.findByPatientAndPlateformeManager(pat, 
	...
	public void onSelect$maladiesBox() {
		Maladie selected = 
			((MaladieDecorator) this.maladiesBox
							.getSelectedItem().getValue()).getMaladie();
		((FichePrelevementEdit) self.getParent().getParent()
					.getAttributeOrFellow("fwinPrelevementEdit$composer", true))
													.setMaladie(selected);
	}
...
//												embeddedFicheMaladieDiv, null);
		...
		((FicheMaladie) embeddedFicheMaladieDiv
				.getFellow("fwinMaladie")
				.getAttributeOrFellow("fwinMaladie$composer", true))
				.setEmbedded(true);
		// informe la fiche prelevement de la presence du formulaire
		((FichePrelevementEdit) self.getParent()
								.getParent()
								.getAttributeOrFellow(
										"fwinPrelevementEdit$composer", true))
	...
	private void displayEmbeddedPatient(boolean show, Patient pat) {
		...
		if (show) {		
			...
			Executions
				.createComponents("/zuls/patient/FichePatientEdit.zul", 
												fichePatientDiv, null);
			((FichePatientEdit) fichePatientDiv
					.getFellow("fwinPatientEdit")
					.getAttributeOrFellow("fwinPatientEdit$composer", true))
					.setEmbedded(pat);
...
//									ficheMaladieWithPatientDiv, null);
			...
			if (SessionUtils.getSelectedBanques(sessionScope)
				...
				// affiche le bloc div
				self.getFellow("newPatientDiv")
					.getFellow("newMaladieBox").setVisible(true);
			} else {
				// efface le bloc div
				self.getFellow("newPatientDiv")
					.getFellow("newMaladieBox").setVisible(false);
				// cree la maladie sous-jacente
				((Textbox) ficheMaladieWithPatientDiv
					.getFellow("fwinMaladie")
					.getFellow("libelleBox"))
						.setValue((SessionUtils
							.getSelectedBanques(sessionScope)
												.get(0).getNom() 
										+ "-defaut"));
				((Maladie) ((FicheMaladie) ficheMaladieWithPatientDiv
						.getFellow("fwinMaladie")
						.getAttributeOrFellow("fwinMaladie$composer", true))
							.getObject()).setSystemeDefaut(true);
			...
			((Maladie) ((FicheMaladie) ficheMaladieWithPatientDiv
					.getFellow("fwinMaladie")
					.getAttributeOrFellow("fwinMaladie$composer", true))
					.getObject()).setPatient(((FichePatientEdit) 
							fichePatientDiv.getFellow("fwinPatientEdit")
							.getAttributeOrFellow("fwinPatientEdit$composer", true))
									.getObject());
	...
	public boolean getDroitOnAction(
			String nomEntite, String nomOperation) {
		Boolean admin = false;
