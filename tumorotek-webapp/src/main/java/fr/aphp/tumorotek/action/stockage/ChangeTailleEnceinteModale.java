...
public class ChangeTailleEnceinteModale 
	...
	public void init(Component comp, Enceinte obj) {
		this.enceinte = obj;
		this.parent = comp;
		
		nbPlaces = this.enceinte.getNbPlaces();
		nbPlacesOcc = ManagerLocator.getEnceinteManager()
				.getEnceintesManager(enceinte).size() 
			+ ManagerLocator.getEnceinteManager()
			.getTerminalesManager(enceinte).size();
		
		nbPlacesLabel = ObjectTypesFormatters.getLabel("enceinte.taille.edit.status", 
				new String[] {
				String.valueOf(nbPlacesOcc), String.valueOf(nbPlaces)});
	...
	public void onClick$validate() {
		...
		if (nbPlaces < nbPlacesOcc) {
			throw new WrongValueException(nbPlacesBox, 
				ObjectTypesFormatters.getLabel("enceinte.taille.exception", 
					new String[] {enceinte.getNom(), 
						String.valueOf(nbPlacesOcc)}));
		} else {
	...
	public void onLaterUpdate() {
		enceinte = ManagerLocator.getEnceinteManager()
			.updateTailleEnceinteManager(enceinte, 
					nbPlaces - enceinte.getNbPlaces(), 
	...
	public void setNbPlaces(Integer n) {
		this.nbPlaces = n;
	}
