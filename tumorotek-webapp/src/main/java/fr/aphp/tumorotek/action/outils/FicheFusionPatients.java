...
public class FicheFusionPatients extends AbstractFicheCombineController {
	...
	private Log log = LogFactory.getLog(FicheFusionPatients.class);
	...
	public void onClick$searchFirstPatient() {
		String critereValue = nomFirstBox.getValue();
		openSelectPatientWindow(Path.getPath(self), 
				"onGetFirstPatientFromSelection", true, 
				critereValue, patientASupprimer);
	}
	 ...
	 */
	public void onGetFirstPatientFromSelection(Event e) {
		if (e.getData() != null) {
			...
			List<Maladie> maladies = ManagerLocator
				.getMaladieManager()
				.findByPatientNoSystemManager(patientAConserver);
			...
			List<Collaborateur> medecins = new ArrayList<Collaborateur>(
					ManagerLocator
					.getPatientManager()
					.getMedecinsManager(patientAConserver));
			values = new ArrayList<String>();
	...
	public void onClick$searchSecondPatient() {
		String critereValue = nomSecondBox.getValue();
		openSelectPatientWindow(Path.getPath(self), 
				"onGetSecondPatientFromSelection", true, 
				critereValue, patientAConserver);
	}
	 ...
	 * @param e Event contenant le patient sélectionné.
	 */
	public void onGetSecondPatientFromSelection(Event e) {
		if (e.getData() != null) {
			...
			List<Maladie> maladies = ManagerLocator
				.getMaladieManager()
				.findByPatientNoSystemManager(patientASupprimer);
			List<String> values = new ArrayList<String>();
			for (int i = 0; i < maladies.size(); i++) {
				StringBuffer sb = new StringBuffer();
				sb.append(maladies.get(i).getLibelle());
			...
			List<Collaborateur> medecins = new ArrayList<Collaborateur>(
					ManagerLocator
					.getPatientManager()
					.getMedecinsManager(patientASupprimer));
			values = new ArrayList<String>();
			for (int i = 0; i < medecins.size(); i++) {
				values.add(medecins.get(i).getNomAndPrenom());
			}
	 ...
	 * @param event
	 */
	public void onFusionTriggered(Event event) {
	...
	public void onLaterFusion() {
		try {
			...
			ManagerLocator.getPatientManager().fusionPatientManager(
					patientAConserver, patientASupprimer, 
			...
			if (getMainWindow()
				...
				PatientController patientController = 
					((PatientController) getMainWindow()
									.getMainTabbox()
									.getTabpanels()
				...
				patientController.getListe().removeObjectFromList(
						patientASupprimer);
				
				// on update le patient conservé
				patientController.getListe()
					.updateObjectGridListFromOtherPage(patientAConserver, true);
				...
				if (getMainWindow()
					...
					PrelevementController prelevementController = 
						((PrelevementController) getMainWindow()
										.getMainTabbox()
										.getTabpanels()
	...
	public void setEmptyToNulls() {
		if (this.patientAConserver.getNom() != null 
			&& this.patientAConserver.getNom().equals("")) {
			this.patientAConserver.setNom(null);
		}
		if (this.patientAConserver.getNip() != null 
			&& this.patientAConserver.getNip().equals("")) {
			this.patientAConserver.setNip(null);
		}
		if (this.patientAConserver.getNomNaissance() != null
			&& this.patientAConserver.getNomNaissance().equals("")) {
			this.patientAConserver.setNomNaissance(null);
		}
		if (this.patientAConserver.getPrenom() != null
			&& this.patientAConserver.getPrenom().equals("")) {
			this.patientAConserver.setPrenom(null);
		}
		if (this.patientAConserver.getPaysNaissance() != null
			&& this.patientAConserver.getPaysNaissance().equals("")) {
			this.patientAConserver.setPaysNaissance(null);
		}
		if (this.patientAConserver.getVilleNaissance() != null
			&& this.patientAConserver.getVilleNaissance().equals("")) {
...
//	}
	 ...
	 * @param message
	 */
	public void openFusionWindow(Page page, String message) {
		 if (!isBlockModal()) {
...
//			win.setHeight(String.valueOf(height) + "px");
			...
			ua = (HtmlMacroComponent)
					page.getComponentDefinition("fusionModale", false)
												.newInstance(page, null);
			ua.setParent(win);
			...
			((FicheFusionModale) ua.getFellow("fwinFusionModale")
					.getAttributeOrFellow("fwinFusionModale$composer", true))
					.init(message, self);
			ua.setVisible(false);
	 ...
	 * @param row Parent
	 */
	private void drawLabelWithPopup(List<String> values, Div div) {
	...
	public String getPatientAConserverDateNaissanceFormated() {
		if (patientAConserver != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.patientAConserver
						.getDateNaissance());
		} else {
			return null;
		}
	...
	public String getPatientASupprimerDateNaissanceFormated() {
		if (patientASupprimer != null) {
			return ObjectTypesFormatters
				.dateRenderer2(this.patientASupprimer
						.getDateNaissance());
		} else {
			return null;
		}
	...
	public void setPatientAConserver(Patient p) {
	...
	public void setPatientASupprimer(Patient p) {
