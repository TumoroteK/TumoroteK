...
public abstract class AbstractController extends GenericForwardComposer<Component>
{
	...
	public static String handleExceptionMessage(final Exception ex) throws WrongValueException{
	   ...
	   //new version 2.3.1.0-rc5 (TK-817):
		else if(ex instanceof TKBasicRuntimeException) {
         if(((TKBasicRuntimeException)ex).hasParam()) {
            message = new StringBuilder(Labels.getLabel(ex.getMessage(), ((TKBasicRuntimeException)ex).getMessageParams()));
         }
         else {
