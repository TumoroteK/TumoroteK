...
public abstract class AbstractController extends GenericForwardComposer<Component> {
	...
	public void setBlockModal(boolean b) {
		this.blockModal = b;
	}
	...
	public void updateApplicationScreenSize() {
		...
		if (sessionScope.containsKey("screenHeight")) {
			Integer screenHeight = (Integer) sessionScope.get("screenHeight");
			getMainWindow().setScreenHeight(screenHeight);
		}
		if (sessionScope.containsKey("screenWidth")) {
			Integer screenWidth = (Integer) sessionScope.get("screenWidth");
	...
	public MainWindow getMainWindow() {
		return (MainWindow) page.getFellow("mainWin").getAttributeOrFellow(
				"mainWin$composer", true);
	}
	...
	public boolean drawActionOnOneButton(String nomEntite, String nomOperation) {
		Boolean admin = false;
		...
		if (admin) {
			...
			OperationType operation = ManagerLocator.getOperationTypeManager()
	...
	public boolean getDroitOnAction(String nomEntite, String nomOperation) {
		...
		if (admin) {
			...
			// on extrait l'OperationType de la base
			OperationType opeation = ManagerLocator.getOperationTypeManager()
					.findByNomLikeManager(nomOperation, true).get(0);
	 ...
	 */
	public void openSelectPatientWindow(
		...
		Window window = (Window) Executions.createComponents("/zuls/prelevement/SelectPatientModale.zul", 
															null, map);
		window.doModal();
	}
	 ...
	 */
	public void openCollaborationsWindow(Page page, String title, String mode,
			Object obj, String entite, List<Object> list, String path,
		...
		if (!isBlockModal()) {
			...
			win.setWidth("800px");
			int height = getMainWindow().getPanelHeight() + 35;
			win.setHeight(height + "px");
			win.setClosable(true);

			final HtmlMacroComponent ua = populateModal(win, page, mode, obj,
					entite, list, path, oldSelection);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateModal(Window win, Page page,
			String mode, Object obj, String entite, List<Object> list,
			String path, List<Object> oldSelection) {
		// HtmlMacroComponent contenu dans la fenêtre : il correspond
		// au composant des collaborations.
		HtmlMacroComponent ua;
		ua = (HtmlMacroComponent) page.getComponentDefinition("collaborations",
		...
		if ("select".equals(mode)) {
			((CollaborationsController) ua.getFellow("winCollaborations")
			...
			((CollaborationsController) ua.getFellow("winCollaborations")
					.getAttributeOrFellow("winCollaborations$composer", true))
					.getListeCollaborations().openCollaborateurInTree(
							(Collaborateur) obj, true, true);
			((CollaborationsController) ua.getFellow("winCollaborations")
					.getAttributeOrFellow("winCollaborations$composer", true))
	 ...
	 */
	public void openCodesModal(Page page, String path, boolean isMorpho,
			boolean isOrgane, boolean isDiagnostic, boolean toCims) {
		...
		if (!isBlockModal()) {
			...
			if (isOrgane) {
				titleParam = new String[] { Labels
						.getLabel("codes.modal.title.organe") };
			} else if (isMorpho) {
				titleParam = new String[] { Labels
						.getLabel("codes.modal.title.morpho") };
			} else if (isDiagnostic) {
				titleParam = new String[] { Labels
						.getLabel("codes.modal.title.diagno") };
			} else {
				titleParam = new String[] { Labels
						.getLabel("codes.modal.title.general") };
			}
			win.setTitle(ObjectTypesFormatters.getLabel("code.component.title",
					titleParam));
			win.setBorder("normal");
			win.setClosable(true);

			final HtmlMacroComponent ua = (HtmlMacroComponent) page
					.getComponentDefinition("codesModale", false).newInstance(
							page, null);
			...
			((CodesController) ua.getFellow("winCodes").getAttributeOrFellow(
					"winCodes$composer", true)).init(path, isOrgane, isMorpho,
					isDiagnostic, toCims);
	 ...
	 */
	public static String handleExceptionMessage(Exception ex) throws WrongValueException {
		String message = Labels.getLabel("validation.exception.inconnu");
		if (ex instanceof ValidationException) {
			message = Labels.getLabel("validation.error");
			message = message + "\n";
			Iterator<Errors> errs = (((ValidationException) ex).getErrors())
					.iterator();
			String errCode;
			while (errs.hasNext()) {
		...
		if (ex instanceof TKException) {
			...
			new String[] {((TKException) ex).getIdentificationObjetException()});
				...
				message = sb.toString() 
					+ (Labels.getLabel(ex.getMessage()) != null ? 
							Labels.getLabel(ex.getMessage()) : message);
			} 
		}
	 ...
	 */
	public static void makeLabelAnonyme(Label label, boolean details) {
		if (details) {
	 ...
	 *            Objet à imprimer.
	 */
	public void openEtiquetteWindow(Page page, Modele modele,
			Imprimante imp) {
		if (!isBlockModal()) {
			setBlockModal(true);
			...
			final HtmlMacroComponent ua = populateEtiquetteModal(win, page,
																modele, imp);
			ua.setVisible(false);
	...
	private static HtmlMacroComponent populateEtiquetteModal(Window win,
			Page page, Modele modele, Imprimante imp) {
		...
		if (isDefault) {
			ua = (HtmlMacroComponent) page.getComponentDefinition(
					"ficheEtiquetteModale", false).newInstance(page, null);
			ua.setParent(win);
			ua.setId("openEtiquetteModale");
			ua.applyProperties();
			ua.afterCompose();
			((FicheEtiquetteModale) ua.getFellow("fwinEtiquetteModale")
					.getAttributeOrFellow("fwinEtiquetteModale$composer", true))
					.init(modele, imp, null, null, null);
		} else {
			ua = (HtmlMacroComponent) page.getComponentDefinition(
					"ficheEtiquetteDynModale", false).newInstance(page, null);
			ua.setParent(win);
			ua.setId("openEtiquetteDyModale");
			ua.applyProperties();
			ua.afterCompose();
			((FicheEtiquetteDynModale) ua.getFellow("fwinEtiquetteDynModale")
					.getAttributeOrFellow("fwinEtiquetteDynModale$composer",
	 ...
	 */
	public ObjetStatut findStatutForTKStockableObject(TKStockableObject tkobj) {
...
//		} else {
		...
		List<Cession> cessionsEnAttente = ManagerLocator
				...
				.getAllCessionsByStatutAndObjetManager("EN ATTENTE",
						tkobj);
		...
		if (cessionsEnAttente.size() == 0) {
			...
			if (tkobj.getObjetStatut() != null 
					&& tkobj.getObjetStatut().getStatut().equals("DETRUIT")) {
				epuise = ManagerLocator.getObjetStatutManager()
		...
		// if (stocke != null && epuise != null) {
			...
			if (tkobj.getQuantite() != null) {
				...
				if (tkobj.getQuantite().equals(zero)) {
					...
					if (tkobj.getObjetStatut() != null 
							&& !tkobj.getObjetStatut().getStatut().equals("EPUISE")
...
//					}
			...
			} else if (tkobj instanceof ProdDerive 
				&& ((ProdDerive) tkobj).getVolume() != null) {
				// si sa valeur est égale a 0 => epuisé
...
//	}
	...
	public static Float calculerVolumeRestant(ProdDerive derive, Float qte) {
		Float vol = (float) 0.0;

		// maj du volume
		if (derive.getQuantiteInit() != null
				&& !derive.getQuantiteInit().equals(new Float(0.0))) {
			if (derive.getVolumeInit() != null) {
				Float rapport = qte / derive.getQuantiteInit();
	...
	public static Float calculerQuantiteRestante(ProdDerive derive, Float vol) {
		Float quantite = (float) 0.0;

		if (derive.getVolumeInit() != null
				&& !derive.getVolumeInit().equals(new Float(0.0))) {
			if (derive.getQuantiteInit() != null) {
				Float rapport = vol / derive.getVolumeInit();
	 ...
	 */
	public void openHeadersWindow(Page page, Component comp) {
		if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua;
			ua = (HtmlMacroComponent) page.getComponentDefinition(
					"ficheHeadersModale", false).newInstance(page, null);
			ua.setParent(win);
			ua.setId("ficheHeadersModaleComponent");
			ua.applyProperties();
			ua.afterCompose();

			((FicheHeadersModale) ua.getFellow("fwinHeadersModale")
					.getAttributeOrFellow("fwinHeadersModale$composer", true))
					.setParent(comp);
			ua.setVisible(false);
	...
	public void onClickObjectEmplacement(Event event) {
		...
		if (event.getData() != null) {
			...
			StockageController tabController = (StockageController) StockageController
					.backToMe(getMainWindow(), page);

			if (tabController != null
					&& tabController.getListeStockages() != null) {
				tabController.getListeStockages().openTerminaleFromEmplacement(
						(Emplacement) event.getData());
			}
		}
	}
	 ...
	 * @return
	 */
	public boolean askForRetoursCreation(Integer nbObjs, Cession cession,
			Transformation transformation, Map<TKStockableObject, Emplacement> emplacements,
			Incident incident, OperationType opType) {
		String texte = null;
		...
		if (nbObjs > 1) {
			countObjs = ObjectTypesFormatters.getLabel(
					"message.retour.create.nobjets",
					new String[] { String.valueOf(nbObjs) });
		} else { // objs.size() == 1
			countObjs = Labels.getLabel("message.retour.create.unobjet");
		}

		if (cession != null) {
			objetDescr = Labels.getLabel("Entite.Cession") + " "
					+ cession.getNumero();
			texte = ObjectTypesFormatters.getLabel(
					"message.retour.create.list", new String[] { countObjs,
							opType.getNom().toLowerCase(), objetDescr });
			...
			objetDescr = Labels.getLabel("Entite.Incident") + " "
					+ incident.getNom();
			texte = ObjectTypesFormatters.getLabel(
					"message.retour.create.list", new String[] { countObjs,
							opType.getNom(), objetDescr });
		} else if (emplacements != null) {
			texte = ObjectTypesFormatters.getLabel(
					"message.retour.create.deplacement",
					new String[] { countObjs });
		}

		// confirmation
		if (Messagebox.show(texte,
				Labels.getLabel("message.retour.create.title"), Messagebox.YES
	 ...
	 * @param collaborateur
	 */
	public void proposeRetoursCreation(List<TKStockableObject> objs,
			HashMap<TKStockableObject, Emplacement> oldEmplacements,
			Date date, Cession cession, Transformation transformation, 
			Incident incident, OperationType opType,
			String observation, Collaborateur operateur) {

		if (objs != null && !objs.isEmpty() && opType != null) {
			if (askForRetoursCreation(objs.size(), cession, transformation,
					oldEmplacements, incident, opType)) {
				openRetourFormModale(null, true, null, null, objs,
						oldEmplacements, cession, transformation, 
						incident, date, observation, operateur);
			}
		}
	}
	 ...
	 */
	public void openRetourFormModale(Retour retour, boolean edit,
			ListeRetour listRetour, TKStockableObject tkObject,
			List<TKStockableObject> objs,
			HashMap<TKStockableObject, Emplacement> oldEmplacements,
			Cession cession, Transformation transformation, 
			Incident incident, Date dateSortie, String observation, 
			Collaborateur operateur) {
		if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua = (HtmlMacroComponent) page
					.getComponentDefinition("retourModale", false).newInstance(
							page, null);
			...
			// la fiche
			if (listRetour != null) {
				((FicheRetour) ua.getFellow("fwinRetour").getAttributeOrFellow(
						"fwinRetour$composer", true))
						.setListeRetour(listRetour);
			}
			if (retour != null) { // modification
				((FicheRetour) ua.getFellow("fwinRetour").getAttributeOrFellow(
						"fwinRetour$composer", true)).setObject(retour);
			}
			
			// tkObject
			((FicheRetour) ua.getFellow("fwinRetour")
						.getAttributeOrFellow("fwinRetour$composer",
								true)).setTkObject(tkObject);

			if (!edit) {
				((FicheRetour) ua.getFellow("fwinRetour").getAttributeOrFellow(
						"fwinRetour$composer", true)).switchToStaticMode();
			} else {
				if (retour == null) {
					if (objs != null)  { // creation multiple
						((FicheRetour) ua.getFellow("fwinRetour")
								.getAttributeOrFellow("fwinRetour$composer",
										true)).setObjects(objs);
						if (cession != null) {
							((FicheRetour) ua.getFellow("fwinRetour")
...
//						}
					...
					((FicheRetour) ua.getFellow("fwinRetour")
							.getAttributeOrFellow("fwinRetour$composer", true))
	 ...
	 *            parent ayant demandé la fenêtre.
	 */
	public void openRechercheBiocapWindow(Page page, Component parent, 
		...
		if (!isBlockModal()) {
	        ...
	        if (!parent.hasFellow("fwinBiocapModale")) {
	        	Executions
					.createComponents("/zuls/recherche/FicheRechercheBiocapModale.zul", 
																parent, map);
	        }
...
//		}
	 ...
	 */
	public void openPasswordWindow(Page page, Component comp, Utilisateur user) {
		if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua;
			ua = (HtmlMacroComponent) page.getComponentDefinition(
					"fichePasswordModale", false).newInstance(page, null);
			ua.setParent(win);
			ua.setId("fichePasswordModaleComponent");
			ua.applyProperties();
			ua.afterCompose();

			((FichePasswordModale) ua.getFellow("fwinPasswordModale")
					.getAttributeOrFellow("fwinPasswordModale$composer", true))
	...
	public EchantillonController getEchantillonController() {
		if (getMainWindow().isFullfilledComponent("echantillonPanel",
				"winEchantillon")) {
			return ((EchantillonController) getMainWindow().getMainTabbox()
	...
	public ProdDeriveController getProdDeriveController() {
		if (getMainWindow().isFullfilledComponent("derivePanel",
				"winProdDerive")) {
			return ((ProdDeriveController) getMainWindow().getMainTabbox()
					.getTabpanels().getFellow("derivePanel")
	 ...
	 */
	public void openImprimanteModeleModale(Plateforme pf, 
			List<? extends TKStockableObject> objs, 
			AffectationImprimante affectation, 
			List<LigneEtiquette> lignes) {
		if (!isBlockModal()) {
	 ...
	 * @version 2.0.10
	 */
	public void openAddIncidentWindow(Page page, Component comp,
			Conteneur conteneur, Enceinte enceinte, Terminale terminale) {
		if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua;
			ua = (HtmlMacroComponent) page.getComponentDefinition(
					"ficheAddIncidentModale", false).newInstance(page, null);
			ua.setParent(win);
			ua.setId("ficheAddIncidentModaleComponent");
			ua.applyProperties();
			ua.afterCompose();

			((FicheAddIncident) ua.getFellow("fwinAddIncident")
					.getAttributeOrFellow("fwinAddIncident$composer", true))
	...
	public HtmlMacroComponent callProgressBar() {
		...
		progressBar = (HtmlMacroComponent) page.getComponentDefinition(
				"progressBar", false).newInstance(page, null);
	...
	public ProgressBarComponent getBar() {
		return ((ProgressBarComponent) progressBar
				.getFellow("progressPanel").getAttributeOrFellow(
						"progressPanel$composer", true));
	}
			...
			if (isAnonyme()) {
				...
				if (sessionScope.containsKey("Export")) {
					if (sessionScope.get("Export").equals("Export")) {
						isExportAnonyme = false;
					} else if (sessionScope.get("Export").equals(
							"ExportAnonyme")) {
						isExportAnonyme = true;
					} else {
						isExportAnonyme = false;
	 ...
	 */
	public void openObjetStatutWarnModale(List<TKStockableObject> objs, 
														String operation) {
		if (!isBlockModal()) {
	 ...
	 */
	public boolean 
			getObjStatutIncompatibleForRetour(List<? extends Object> objs, String operation) {
		List<TKStockableObject> encours = new ArrayList<TKStockableObject>();
		Iterator<? extends Object> itS = objs.iterator();
		List<Integer> objIds = new ArrayList<Integer>();
		List<Retour> rets = new ArrayList<Retour>();
		TKStockableObject obj;
		while (itS.hasNext()) {
			obj = (TKStockableObject) itS.next();
			if (obj.getObjetStatut()
					.getStatut().equals("EPUISE")
				|| (obj.getObjetStatut()
					.getStatut().equals("RESERVE") 
					&& operation != null && (operation.equals("CESSION")
						|| operation.equals("TRANSFORMATION")))) {
				encours.add(obj);
				...
				if (operation != null && !operation.equals("TRANSFORMATION")) {
					...
					if (obj.entiteNom().equals("Echantillon")) {
						rets.addAll(ManagerLocator.getRetourManager()
							.findByObjectDateRetourEmptyManager(objIds, 
				...
				retLoop : for (Retour retour : rets) {
	 ...
	 * @since 2.0.10
	 */
	public void openDateRetourModale(AbstractListeController2 liste, 
			List<TKStockableObject> objs) {
		...
		if (!isBlockModal()) {
			...
			Executions.createComponents("/zuls/cession/retour/DateRetourModale.zul", 
																null, map);
...
//	}
	 ...
	 * PopUp window appelée pour confirmation de la recherche.
	 */
	public void openResultatsWindow(Page page, List<Integer> results, Component comp,
			String entite, AbstractObjectTabController controller) {
		if (!isBlockModal()) {
	 ...
	 * @since 2.0.10
	 */
	public void openRestrictTablesModale(AbstractListeController2 liste, 
												Component parent, 
												Entite e) {
		...
		if (!isBlockModal()) {
			...
			Executions.createComponents("/zuls/export/RestrictTablesModale.zul", 
																null, map);
	 ...
	 * @return 
	 */
	public Boolean getDroitListableObjectConsultation(TKAnnotableObject obj, 
			boolean displayWarning) {
		boolean acces = getDroitEntiteObjectConsultation(obj.getBanque(), 
														obj.entiteNom());
		if (acces) {
			return true;
		} else if (displayWarning) {
				Messagebox.show(Labels.getLabel("error.resource.notallowed"),
						Labels.getLabel("error.resource.notallowed.title"),
	 ...
	 * @return boolean
	 */
	public Boolean getDroitEntiteObjectConsultation(Banque banque, String entiteNom) {
		
		// si on reste dans banque courante
		// utilise getDroitsConsultation
		if (SessionUtils.getSelectedBanques(sessionScope).contains(banque)
				&& getDroitsConsultation().get(entiteNom) != null
		...
		Utilisateur user = SessionUtils.getLoggedUser(sessionScope);
		...
		if (pfs.contains(banque.getPlateforme())) {
			...
			List<ProfilUtilisateur> profils = ManagerLocator
					...
					.findByUtilisateurBanqueManager(user, banque);
	 ...
	 * @param event
	 */
	public void displayObjectData(TKAnnotableObject tkObj) {
		...
		if (tkObj != null) { // un seul element a afficher
			...
			if (getDroitListableObjectConsultation(tkObj, true)) {
				...
				AbstractObjectTabController controller = 
						backToControllerFromEntiteNom(tkObj.entiteNom());
	 ...
	 * @param event
	 */
	public void displayObjectsListData(List<TKAnnotableObject> tkObjList) {
	
		if (tkObjList != null && !tkObjList.isEmpty()) {
			String entiteNom = tkObjList.get(0).entiteNom();
			List<Banque> availBanques = new ArrayList<Banque>();
			for (Banque banque : getMainWindow().getBanques()) {
				if (banque.getBanqueId() != null 
					&& getDroitEntiteObjectConsultation(banque, entiteNom)) {
			...
			boolean warn = false;
			List<TKAnnotableObject> objs = new ArrayList<TKAnnotableObject>();
			Set<Banque> objBanks = new HashSet<Banque>();
			...
			try {
				if (!objs.isEmpty()) {		
					Iterator<Banque> itBk = objBanks.iterator();
					Banque bq;
					...
					AbstractObjectTabController tabController = 
							backToControllerFromEntiteNom(entiteNom);
		
					tabController.getListe().updateMultiObjectsGridListFromOtherPage(
							new ArrayList<TKdataObject>(objs));
				...
				if (warn) {
					Messagebox.show(Labels.getLabel("warn.resource.notallowed"),
							Labels.getLabel("error.resource.notallowed.title"),
							Messagebox.OK, Messagebox.EXCLAMATION);
				}
			} catch (Exception e) {
				Messagebox.show(Labels.getLabel("error.toutesColl.notallowed"),
						Labels.getLabel("error.resource.notallowed.title"),
						Messagebox.OK, Messagebox.EXCLAMATION);
	 ...
	 * @version 2.0.10
	 */
	public AbstractObjectTabController backToControllerFromEntiteNom(String nom) {
	...
	public Boolean hasAllCreationRights(Profil profil) {
		
		OperationType creation = ManagerLocator.getOperationTypeManager()
				.findByNomLikeManager("creation", true).get(0);
		OperationType annotation = ManagerLocator.getOperationTypeManager()
		...
		if (profil != null) {
			return ManagerLocator.getDroitObjetManager()
				.hasProfilOperationOnEntitesManager(profil, creation, entites)
				&& ManagerLocator.getDroitObjetManager()
				.hasProfilOperationOnEntitesManager(profil, annotation, entites);
		}
...
//		    Executions.createComponents("/zuls/cession/DateRetourComplete.zul", parent, map);
			...
			final HtmlMacroComponent ua = (HtmlMacroComponent) page
					.getComponentDefinition("dateRetourCompleteModale", false).newInstance(
							page, null);
			...
			((DateRetourCompleteModale) ua.getFellow("fwinDateRetourComplete")
					.getAttributeOrFellow(
					"fwinDateRetourComplete$composer", true))
					.init(echansRetourIncompletes,
	 ...
	 */
	public void openDeleteWindow(Page page, String message, 
			boolean casc, boolean fantomable, boolean del, Component comp,
			boolean archive) {
		 if (!isBlockModal()) {
			...
			final HtmlMacroComponent ua;
			ua = (HtmlMacroComponent)
					page.getComponentDefinition("deleteModale", false)
												.newInstance(page, null);
			ua.setParent(win);
			...
			((DeleteModale) ua.getFellow("fwinDeleteModale")
					.getAttributeOrFellow("fwinDeleteModale$composer", true))
					.init(message, casc, fantomable, del, comp, archive);
			ua.setVisible(false);
	 ...
	 * @since 2.0.13
	 */
	public void openConsentTypeUsed(ConsentTypeUsedTable table) {
		if (!isBlockModal()) {
