...
public abstract class AbstractController extends GenericForwardComposer<Component>
{
   ...
   public static String handleExceptionMessage(final Exception ex) throws WrongValueException{
      ...
      if(ex instanceof ValidationException){
         ...
         message = new StringBuilder(ObjectTypesFormatters.getLabel("validation.requiredObject",
    ...
    * @version 2.2.3-genno fix TK-291
    */
   public void proposeRetoursCreation(final List<TKStockableObject> objs, final List<OldEmplTrace> oldEmplacements,
      final Date date, final Cession cession, final Transformation transformation, final Incident incident,
      final OperationType opType, final String observation, final Collaborateur operateur){
   ...
   public void openRetourFormModale(final Retour retour, final boolean edit, final ListeRetour listRetour,
      final TKStockableObject tkObject, final List<TKStockableObject> objs, final List<OldEmplTrace> oldEmplacements,
      final Cession cession, final Transformation transformation, final Incident incident, final Date dateSortie,
      final String observation, final Collaborateur operateur){
      if(!isBlockModal()){
   ...
   public ProfilExport openExportAnonymeModale(){
      ...
      final Window win = (Window) Executions.createComponents("/zuls/export/ExportAnonymeModale.zul", null, null);

      win.doModal();

      // EXPORT NOMINATIF uniquement si export modale renvoie anonymize = false
      if(win != null && win.hasAttribute("anonymize") && !((Boolean) win.getAttribute("anonymize")).booleanValue()){
    ...
    * @version 2.2.1-IRELEC
    */
   public void postStorageData(final List<? extends TKStockableObject> tkObjs, final boolean destock){
      ...
      if(!SessionUtils.getRecepteursInterfacages(sessionScope).isEmpty() && tkObjs != null){
         ...
         // only in stock obj
         final List<TKStockableObject> inStock = new ArrayList<>();
         for(final TKStockableObject tkO : tkObjs){
         ...
         if(!inStock.isEmpty()){
            ...
            final Window window = (Window) Executions.createComponents("/zuls/stockage/BoiteTransfertModale.zul",
   ...
   public boolean storageRobotItemVisible(){
      if(sessionScope.containsKey("AdminPF")){
         for(final fr.aphp.tumorotek.model.interfacage.Recepteur recept : SessionUtils.getRecepteursInterfacages(sessionScope)){
    ...
    * @return boolean
    */
   public boolean isCurrentPFOrUserAtLeastAdminfPF(final Plateforme pOrig){
      final boolean enable = pOrig.equals(SessionUtils.getCurrentPlateforme())
         || SessionUtils.getLoggedUser(sessionScope).isSuperAdmin() || ManagerLocator.getUtilisateurManager()
