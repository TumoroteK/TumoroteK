...
public class FicheRechercheAvanceeCession extends AbstractFicheRechercheAvancee
{
	...
	public void onLaterFind(){
		...
		if(!oneValueEntered){
			...
			if(criteresStandards.size() > 0){
				// @since 2.2.1 TK-272 
				// execute éventuelle double requête sur date
				// avant intersection
				final List<Integer> objects = ManagerLocator.getTraitementQueryManager()
						.findObjetByCriteresWithBanquesManager(criteresStandards,
				...
				if(isDateCession){
					...
					// @since 2.2.1 TK-271 vérifie critère n'est pas une annotation
					for(int i = 0; i < criteresStandards.size(); i++){
						if(criteresStandards.get(i).getChamp().getChampEntite() != null 
								&& criteresStandards.get(i).getChamp().getChampEntite().getNom().equals("DestructionDate")){
					...
					// relance la query
					// les deux requêtes ne peuvent pas remonter des ids
					// identiques car cession destruction OU recherche/sanitaire
					objects.addAll(ManagerLocator.getTraitementQueryManager()
	...
	@Override
	public void setGroupLaboIntersOpened(Boolean g) {		
	}
}
