...
public class FicheRechercheAvanceeCession extends AbstractFicheRechercheAvancee {
	 ...
	 * Associations.
	 */
	private List<CessionStatut> cessionStatuts = new ArrayList<CessionStatut>();
	private List<CessionType> cessionTypes = new ArrayList<CessionType>();
	private List<Collaborateur> demandeurs = new ArrayList<Collaborateur>();
	private List<Contrat> contrats = new ArrayList<Contrat>();
	private List<CessionExamen> cessionExamens = new ArrayList<CessionExamen>();
	private List<DestructionMotif> destructions = new ArrayList<DestructionMotif>();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		objCessionComponents = new Component[] { this.numeroCessionBox,
				this.statutCessionBox, this.typeCessionBox,
				this.dateCession1Box, this.dateCession2Box,
				this.dateValidation1Box, this.dateValidation2Box,
				this.demandeurCessionBox, this.contratCessionBox,
				this.examenCessionBox, this.destructionCessionBox,
				this.titreCessionBox, this.descriptionCessionBox,
				this.observationsCessionBox };

		objOperateurs = new Component[] { this.operateursDateCessionBox,  
				this.operateursDateValidationBox };
	...
	public void initRechercheAvancee(Entite entite, String path, 
		...
		// init des listes
		cessionStatuts = ManagerLocator.getCessionStatutManager()
				.findAllObjectsManager();
		cessionStatuts.add(0, null);
		cessionTypes = ManagerLocator.getCessionTypeManager()
				.findAllObjectsManager();
		cessionTypes.add(0, null);
		demandeurs = ManagerLocator.getCollaborateurManager()
		...
		contrats.add(0, null);
		cessionExamens = (List<CessionExamen>) ManagerLocator
				.getCessionExamenManager().findByOrderManager(
						SessionUtils.getPlateforme(sessionScope));
		cessionExamens.add(0, null);
		destructions = (List<DestructionMotif>) ManagerLocator
				.getDestructionMotifManager().findByOrderManager(
						SessionUtils.getPlateforme(sessionScope));
		...
		groupAnnotations.setLabel(Labels
				.getLabel("recherche.avancee.cessions.annotations"));
	...
	public void onLaterFind() {
		...
		if (!oneValueEntered) {
			resultatsIds.addAll(ManagerLocator.getCessionManager()
			...
			if (criteresStandards.size() > 0) {
				...
				if (isDateCession) {
					...
					for (int i = 0; i < criteresStandards.size(); i++) {
						if (criteresStandards.get(i).getChamp().getChampEntite().getNom()
								.equals("DestructionDate")) {
							criteresStandards.get(i).getChamp().setChampEntite(ManagerLocator
								.getChampEntiteManager()
								.findByEntiteAndNomManager(criteresStandards.get(i).getChamp()
										.getChampEntite().getEntite(), "DepartDate")
										.get(0));
		...
		if (resultatsIds.size() > 500) {
			openResultatsWindow(page, resultatsIds, self, "Cession", 
	...
	public void executeQueriesForCessions() {
		...
		for (int i = 0; i < objCessionComponents.length; i++) {
			// si c'est un textbox
			if (objCessionComponents[i].getClass().getSimpleName()
					.equals("Textbox")) {
				Textbox current = (Textbox) objCessionComponents[i];
			...
			} else if (objCessionComponents[i].getClass().getSimpleName()
					.equals("Intbox")) {
				Intbox current = (Intbox) objCessionComponents[i];
				// si une valeur a été saisie
				if (current.getValue() != null) {
					// exécution de la requête
					...
					.equals("Listbox")) {
				Listbox current = (Listbox) objCessionComponents[i];
				// si une valeur a été saisie
				if (current.getSelectedIndex() > 0) {
					...
					.equals("Datebox")) {
				Datebox current = (Datebox) objCessionComponents[i];
				// si une valeur a été saisie
				if (current.getValue() != null) {
					...
					if (current.getId().equals("dateValidation1Box")) {
						...
						operateur = (String) operateursDateValidationBox
					...
					.equals("CalendarBox")) {
				CalendarBox current = (CalendarBox) objCessionComponents[i];
				// si une valeur a été saisie
				if (current.getValue() != null) {
					...
					if (current.getId().equals("dateCession1Box")) {
						...
						operateur = (String) operateursDateCessionBox
								.getListModel().getElementAt(idx);
						if (operateur.equals("[..]")) {
							operateur = ">=";
						}
	...
	public void setCessionStatuts(List<CessionStatut> c) {
	...
	public void setCessionTypes(List<CessionType> c) {
		this.cessionTypes = c;
	}
	...
	public void setDemandeurs(List<Collaborateur> d) {
		this.demandeurs = d;
	}
	...
	public void setContrats(List<Contrat> c) {
		this.contrats = c;
	}
	...
	public void setCessionExamens(List<CessionExamen> c) {
	...
	public void setDestructions(List<DestructionMotif> d) {
		this.destructions = d;
	}
	...
	public String getCessionDestructionLabel() {
		return Labels.getLabel("Champ.Cession.DepartDate") + "/" 
				+ Labels.getLabel("cession.type.destruction");
	}
