...
public class ListeContrat extends AbstractListeController2 {
	...
	private List<ContratDecorator> listObjects = 
									new ArrayList<ContratDecorator>();
	...
	public void removeObjectFromList(TKdataObject obj) {
		getListObjects().remove((ContratDecorator) obj);
	}
	...
	public void initObjectsBox() {		
		List<Contrat> contrats = ManagerLocator.getContratManager()
	...
	public void onClickObject(Event event) {
		...
		Contrat edit = ((ContratDecorator) getCurrentObject())
							.getContrat().clone();
	...
	public boolean updateObjectGridListFromOtherPage(Object obj, boolean select) {
		...
		// l'objet passé en paramètre est cloné
		ContratDecorator edit = new ContratDecorator((Contrat) obj);
		...
		if (getListObjects().contains(edit)) {
			...
			if (select) {
				...
				getObjectTabController().getFicheCombine()
										.setObject(edit.getContrat());
			}
			updated = true;
		}	
		return updated;
	...
	public void addToObjectList(Object newObj) {
		...
		// du formulaire
		ContratDecorator newContrat = 
			new ContratDecorator(((Contrat) newObj).clone());
		...
		selectRow((Row) comps.get(
				this.listObjects.indexOf(newContrat)), newContrat);
	...
	public void removeObjectAndUpdateList(TKdataObject obj) {
		ContratDecorator deco = new ContratDecorator((Contrat) obj);		
