...
public class FicheRechercheAvancee extends AbstractFicheRechercheAvancee {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		objPatientComponents = new Component[] { this.nipPatientBox,
				this.nomPatientBox, this.nomNaissancePatientBox,
				this.prenomPatientBox, this.dateNaissance1Box,
				this.dateNaissance2Box, this.sexeFBox, this.sexeHBox,
				this.sexeIndBox, this.etatVBox, this.etatDBox,
				this.etatIncBox, this.medecinsBox };

		objMaladieComponents = new Component[] { this.libelleMaladieBox,
				this.codeMaladieBox, this.dateDebutMaladie1Box,
				this.dateDebutMaladie2Box, this.dateDiagnosticMaladie1Box,
				this.dateDiagnosticMaladie2Box,
				this.medecinsMaPatBox 
		};

		objPrelevementComponents = new Component[] { this.ndaPatientBox,
				this.codePrelevementBox, this.natutePrelevementBox,
				this.codeLaboPrelevementBox, this.datePrelevement1Box,
				this.datePrelevement2Box, this.statutJuridiqueBox,
				this.etablissementPreleveurBox, this.servicePreleveurBox,
				this.nbEchantillonBox, this.agePrlvtBox, this.risquesBox,
				this.conformeBoolBox, this.nonConformitesArriveeBox, 
				};

		objEchantillonComponents = new Component[] { this.codeEchantillonBox,
				this.typeEchantillonBox, this.qualiteEchantillonBox,
				this.statutEchantillonBox, this.quantiteEchantillonBox,
				this.delaiCglEchantillonBox, this.tempStockEchantillonBox,
				this.modePreparationEchantillonBox, this.codeLesionnelBox,
				this.codeOrganeBox, 
				this.conformeEchanTraitementBoolBox, this.nonConformitesEchanTraitementBox,
				this.conformeEchanCessionBoolBox, this.nonConformitesEchanCessionBox,
				this.crAnapathFilebox};

		objProdDeriveComponents = new Component[] { this.codeProdDeriveBox,
				this.typeProdDeriveBox, this.codeLaboProdDeriveBox, 
				this.qualiteProdDeriveBox,
				this.statutProdDeriveBox, this.volumeDeriveBox,
				this.quantiteDeriveBox, this.tempStockDeriveBox,
		...
		objAnonymeComponents = new Component[] { this.nipPatientBox,
				this.nomPatientBox, this.ndaPatientBox,
				this.nomNaissancePatientBox, this.dateNaissance1Box,
				this.dateNaissance2Box };

		objOperateurs = new Component[] { this.operateursAgePrlvtBox,
				this.operateursDateDebutBox, this.operateursDateDiagBox,
				this.operateursDateNaissanceBox, this.operateursDatePrlvtBox,
				this.operateursDelaiCglEchantillonBox, 
				this.operateursTempStockEchantillonBox,
				this.operateursNbEchantillonBox,
				this.operateursQuantiteDeriveBox,
				this.operateursQuantiteEchantillonBox,
				this.operateursVolumeDerivesBox,
				this.operateursTempStockDeriveBox};
	...
	private void prepareContextComponents() {
		if (SessionUtils.isSeroContexte(sessionScope)) {
			objPrelevementContextComponents = new Component[] {
					this.protocolesBox, this.diagCompBox };
		}
	}
	...
	public void initRechercheAvancee(Entite entite, String path, boolean ano, 
		...
		if (anonyme) {
			for (int i = 0; i < objAnonymeComponents.length; i++) {
				if (objAnonymeComponents[i].getClass().getSimpleName()
						.equals("Textbox")) {
					Textbox box = (Textbox) objAnonymeComponents[i];
					box.setDisabled(true);
				} else if (objAnonymeComponents[i].getClass().getSimpleName()
						.equals("Datebox")) {
					Datebox box = (Datebox) objAnonymeComponents[i];
					box.setDisabled(true);
				}
			}
		}
		...
		natures.add(0, null);
		consentTypes = (List<ConsentType>) ManagerLocator
				.getConsentTypeManager().findByOrderManager(
		...
		echanQualites = (List<EchanQualite>) ManagerLocator
				.getEchanQualiteManager().findByOrderManager(
						SessionUtils.getPlateforme(sessionScope));
		echanQualites.add(0, null);
		objetStatuts = ManagerLocator.getObjetStatutManager()
				.findAllObjectsManager();
		objetStatuts.add(0, null);
		prodDeriveTypes = (List<ProdType>) ManagerLocator.getProdTypeManager()
				.findByOrderManager(SessionUtils.getPlateforme(sessionScope));
		prodDeriveTypes.add(0, null);
		deriveQualites = (List<ProdQualite>) ManagerLocator
				.getProdQualiteManager().findByOrderManager(
						SessionUtils.getPlateforme(sessionScope));
		deriveQualites.add(0, null);
		modePrepas = (List<ModePrepa>) ManagerLocator.getModePrepaManager()
				.findByOrderManager(SessionUtils.getPlateforme(sessionScope));
		modePrepas.add(0, null);
		
		nCarrivee = ManagerLocator.getNonConformiteManager()
				.findByPlateformeEntiteAndTypeStringManager(
				SessionUtils.getPlateforme(sessionScope), 
				"Arrivee", ManagerLocator.getEntiteManager()
				.findByNomManager("Prelevement").get(0));
		nCarrivee.add(0, null);
		
		nCechanTraitement = ManagerLocator.getNonConformiteManager()
				.findByPlateformeEntiteAndTypeStringManager(
				SessionUtils.getPlateforme(sessionScope), 
				"Traitement", ManagerLocator.getEntiteManager()
				.findByNomManager("Echantillon").get(0));
		nCechanTraitement.add(0, null);
		
		nCechanCession = ManagerLocator.getNonConformiteManager()
				.findByPlateformeEntiteAndTypeStringManager(
				SessionUtils.getPlateforme(sessionScope), 
				"Cession", ManagerLocator.getEntiteManager()
				.findByNomManager("Echantillon").get(0));
		nCechanCession.add(0, null);
		
		nCderiveTraitement = ManagerLocator.getNonConformiteManager()
				.findByPlateformeEntiteAndTypeStringManager(
				SessionUtils.getPlateforme(sessionScope), 
				"Traitement", ManagerLocator.getEntiteManager()
		...
		nCderiveCession = ManagerLocator.getNonConformiteManager()
				.findByPlateformeEntiteAndTypeStringManager(
				SessionUtils.getPlateforme(sessionScope), 
				"Cession", ManagerLocator.getEntiteManager()
				.findByNomManager("ProdDerive").get(0));
		...
		// since 2.0.13
		risquesModel = new ListModelList<Risque>();
		risquesModel.addAll((List<Risque>) ManagerLocator.getRisqueManager()
				.findByOrderManager(SessionUtils.getPlateforme(sessionScope)));


		if (SessionUtils.isSeroContexte(sessionScope)) {
			protocoles.addAll((List<Protocole>) ManagerLocator
					.getProtocoleManager().findByOrderManager(
							SessionUtils.getPlateforme(sessionScope)));
		...
		if (entiteToSearch.getNom().equals("Patient")) {
			groupAnnotations.setLabel(Labels
					.getLabel("recherche.avancee.patient.annotations"));
		} else if (entiteToSearch.getNom().equals("Prelevement")) {
			groupAnnotations.setLabel(Labels
					.getLabel("recherche.avancee.prelevements.annotations"));
		} else if (entiteToSearch.getNom().equals("Echantillon")) {
			critereOnEchantillon = true;
			groupAnnotations.setLabel(Labels
					.getLabel("recherche.avancee.echantillons.annotations"));
		} else if (entiteToSearch.getNom().equals("ProdDerive")) {
			groupAnnotations.setLabel(Labels
					.getLabel("recherche.avancee.prodDerives.annotations"));
	...
	public void onOpen$groupAnnotations() {
		...
		String id = groupAnnotations.getUuid();
	...
	public void onOpen$groupProdDerives() {
		String id = groupProdDerives.getUuid();
		String idTop = panelChildrenWithScroll.getUuid();
		Clients.evalJavaScript("document.getElementById('" + idTop + "')"
				+ ".scrollTop = document.getElementById('" + id + "')"
				+ ".offsetTop;");
	 ...
	 * @throws Exception
	 */
	public void onSelect$etablissementPreleveurBox(Event event)
			throws Exception {

		Etablissement etab = null;
		if (etablissementPreleveurBox.getListModel() != null) {
			etab = (Etablissement) etablissementPreleveurBox.getListModel()
					.getElementAt(etablissementPreleveurBox.getSelectedIndex());
		} else {
			etab = (Etablissement) etablissementPreleveurBox.getSelectedItem()
					.getValue();
		}
	...
	public void onClick$find() {
		...
		resultatsIds = new ArrayList<Integer>();
		...
		if (!oneValueEntered) {
			if (Messagebox.show(ObjectTypesFormatters.getLabel(
					...
					new String[] { entiteToSearch.getNom() }), Labels
	...
	public void onLaterFind() {
		...
		if (!oneValueEntered) {
			...
			if (entiteToSearch.getNom().equals("Patient")) {
				if (searchPatientsBox != null && searchPatientsBox.isChecked()) {
					resultatsIds.addAll(ManagerLocator.getPatientManager()
							.findAllObjectsIdsWithBanquesManager(
									SessionUtils
			...
			if (criteresStandards.size() > 0) {
				if (!otherQuery) {
					...
					List<Integer> objectsIds = ManagerLocator
							.getTraitementQueryManager()
							.findObjetByCriteresWithBanquesManager(
									criteresStandards, banques,
									valeursStandards);

					resultatsIds = ListUtils.intersection(resultatsIds,
							objectsIds);
				}
				otherQuery = true;
			}
			...
			if (criteresDerives1.size() > 0) {
				...
				List<Integer> objs1 = ManagerLocator
						.getTraitementQueryManager()
						.findObjetByCriteresWithBanquesDeriveVersionManager(
								criteresDerives1, banques, valeursDerives1,
				...
				if (!criteresDerives2.isEmpty()) {
					List<Integer> objs2 = ManagerLocator
							.getTraitementQueryManager()
							.findObjetByCriteresWithBanquesDeriveVersionManager(
									criteresDerives2, banques, valeursDerives2,
									searchForProdDerives);
		...
		if (resultatsIds.size() > 500) {
			openResultatsWindow(page, resultatsIds, self,
					entiteToSearch.getNom(), getObjectTabController());
			...
			Messagebox.show(Labels.getLabel("recherche.avancee.no.results"),
					...
					new Messagebox.Button[] { Messagebox.Button.CANCEL,
							...
							new String [] {Labels.getLabel("general.cancel"), 
				Labels.getLabel("general.retry")}, Messagebox.QUESTION, null,
	...
	public void addCritereToLimitPatients() {
		if (entiteToSearch.getNom().equals("Patient")
				&& searchPatientsBox != null && searchPatientsBox.isChecked()) {
			boolean present = false;
			Entite ePrlvt = ManagerLocator.getEntiteManager()
					.findByNomManager("Prelevement").get(0);
			ChampEntite ce = ManagerLocator.getChampEntiteManager()
					.findByEntiteAndNomManager(ePrlvt, "Code").get(0);
			for (int i = 0; i < criteresStandards.size(); i++) {
				Critere crit = criteresStandards.get(i);
				if (crit.getChamp() != null
						&& crit.getChamp().getChampEntite() != null
			...
			if (!present) {
				...
				Champ champ = new Champ(ce);
				champ.setChampParent(parent1ToQueryPrlvt);

				Critere crit = new Critere(champ, "like", null);
	...
	public void createChampsParentsToQueryPatients() {
		...
		// accès aux maladies
		Entite patientEntite = ManagerLocator.getEntiteManager()
				.findByNomManager("Patient").get(0);
		ChampEntite champEntiteMaladies = ManagerLocator
				.getChampEntiteManager()
				.findByEntiteAndNomManager(patientEntite, "Maladies").get(0);
		...
		ChampEntite champEntitePrelevements = ManagerLocator
				.getChampEntiteManager()
				.findByEntiteAndNomManager(maladieEntite, "Prelevements")
				.get(0);
		...
		Entite prlvtEntite = ManagerLocator.getEntiteManager()
				.findByNomManager("Prelevement").get(0);
		ChampEntite champEntiteEchantillons = ManagerLocator
				.getChampEntiteManager()
				.findByEntiteAndNomManager(prlvtEntite, "Echantillons").get(0);
		...
		ChampEntite champEntitePrlvtProdDerives = ManagerLocator
				.getChampEntiteManager()
				.findByEntiteAndNomManager(prlvtEntite, "ProdDerives").get(0);
	...
	public void createChampsParentsToQueryProdDerives() {
		...
		Entite echanEntite = ManagerLocator.getEntiteManager()
				.findByNomManager("Echantillon").get(0);
		ChampEntite champEntiteEchanDerives = ManagerLocator
				.getChampEntiteManager()
				.findByEntiteAndNomManager(echanEntite, "ProdDerives").get(0);
		...
		ChampEntite champEntitePrlvtDerives = ManagerLocator
				.getChampEntiteManager()
				.findByEntiteAndNomManager(prlvtEntite, "ProdDerives").get(0);
	...
	public void executeQueriesForPatients() {
		...
		List<Object> sexes = new ArrayList<Object>();
		List<Object> etats = new ArrayList<Object>();
		for (int i = 0; i < objPatientComponents.length; i++) {
			if (objPatientComponents[i] != null) {
				...
				if (objPatientComponents[i].getClass().getSimpleName()
						.equals("Textbox")) {
					Textbox current = (Textbox) objPatientComponents[i];
					...
					if (current.getValue() != null
						...
						executeSimpleQueryForTextbox(current,
								parent1ToQueryPatient, parent2ToQueryPatient,
								oneValueEntered, false);
	
						oneValueEntered = true;
					}
				} else if (objPatientComponents[i].getClass().getSimpleName()
						.equals("Listbox")) {
					...
					if (current.getSelectedIndex() > 0) {
						...
						if (current.getId().equals("medecinsBox")) {
							Entite patientEntite = ManagerLocator
									.getEntiteManager().findByNomManager("Patient")
									.get(0);
							ChampEntite champMedecins = ManagerLocator
									.getChampEntiteManager()
									.findByEntiteAndNomManager(patientEntite,
											"PatientMedecins").get(0);
						...
						// exécution de la requête
						executeSimpleQueryForListbox(current, parent1, parent2,
						...
						.equals("Combobox")) {
					Combobox current = (Combobox) objPatientComponents[i];
					// si une valeur a été saisie
					if (current.getSelectedIndex() > 0) {
						Champ parent1 = null;
						Champ parent2 = null;
						...
						// exécution de la requête
						executeSimpleQueryForCombobox(current, parent1, parent2,
								oneValueEntered, null);
					...
					if (current.getValue() != null) {
						...
						if (operateur != null) {
							executeSimpleQueryForDatebox(current,
									parent1ToQueryPatient, parent2ToQueryPatient,
									oneValueEntered, operateur, false);
						}
						...
						.equals("Checkbox")) {
					Checkbox current = (Checkbox) objPatientComponents[i];
					// si une valeur a été saisie
					if (current.isChecked()) {
						if (current.hasAttribute("sexe")) {
							sexes.add((String) current.getAttribute("sexe"));
						} else {
							etats.add((String) current.getAttribute("etat"));
						}
		...
		if (sexes.size() > 0) {
			executeListQuery("Sexe", "Patient", ManagerLocator.getEntiteManager().findByIdManager(1), null,
					oneValueEntered, sexes, false);
			oneValueEntered = true;
		}
		if (etats.size() > 0) {
			executeListQuery("PatientEtat", "Patient", ManagerLocator.getEntiteManager().findByIdManager(1), 
				null, oneValueEntered, etats, false);
			oneValueEntered = true;
		}
	}
			...
			if (objMaladieComponents[i] != null) {
				...
				if (objMaladieComponents[i].getClass().getSimpleName()
					...
					if (current.getValue() != null
						...
						// exécution de la requête
						executeSimpleQueryForTextbox(current,
								parent1ToQueryMaladie, parent2ToQueryMaladie,
								oneValueEntered, false);
	
						oneValueEntered = true;
					}
				} else if (objMaladieComponents[i].getClass().getSimpleName()
						.equals("Datebox")) {
					...
					if (current.getValue() != null) {
						...
						if (current.getId().equals("dateDebutMaladie1Box")) {
							...
							operateur = (String) operateursDateDebutBox
									.getListModel().getElementAt(idx);
							if (operateur.equals("[..]")) {
								operateur = ">=";
							}
						...
						.equals("Combobox")) {
					Combobox current = (Combobox) objMaladieComponents[i];
					// si une valeur a été saisie
					if (current.getSelectedIndex() > 0) {
						Champ parent1 = null;
						Champ parent2 = null;
						...
						if (current.getId().equals("medecinsMaPatBox")) {
							Entite maladieEntite = ManagerLocator
									.getEntiteManager().findByNomManager("Maladie")
									.get(0);
							ChampEntite champMedecins = ManagerLocator
									.getChampEntiteManager()
									.findByEntiteAndNomManager(maladieEntite,
											"Collaborateurs").get(0);
							parent1 = new Champ(champMedecins);
						...
						// exécution de la requête
						executeSimpleQueryForCombobox(current, parent1, 
								parent2,
								oneValueEntered, null);
		...
		for (int i = 0; i < objPrelevementComponents.length; i++) {
			...
			// si c'est un textbox
				if (objPrelevementComponents[i].getClass().getSimpleName()
					.equals("Textbox")) {
					Textbox current = (Textbox) objPrelevementComponents[i];
					// si une valeur a été saisie
					if (current.getValue() != null
							&& !current.getValue().equals("")) {
...
//						} else 
						if (current.getId().equals("nonConformitesArriveeBox")) {
							ChampEntite idPrel =  ManagerLocator
									.getChampEntiteManager().findByIdManager(21);
							...
							if (!entiteToSearch.getNom().equals("Prelevement")) {	
								...
								executeObjetByNonConformite(current, oneValueEntered, 
										ManagerLocator.getConformiteTypeManager()
											.findByIdManager(1), 
										current.getValue(), parent1, parent2, 
															searchForProdDerives); 
							} else {
								executeObjetByNonConformite(current, oneValueEntered, 
										ManagerLocator.getConformiteTypeManager()
											.findByIdManager(1), 
										current.getValue(), null, null, searchForProdDerives); 
							}
				...
				} else if (objPrelevementComponents[i].getClass().getSimpleName()
						.equals("Listbox")) {
					...
					if (!current.isMultiple()) { 
						if (current.getSelectedIndex() > 0) {
							if (current.getId().equals("medecinsMaPatBox")) {
								...
								Collaborateur collab = getCollaborateurs().get(
										current.getSelectedIndex());
								// execution de la requête
								executePrelevementsByMedecinQuery(oneValueEntered,
										collab);
								RechercheCompValues rcv = new RechercheCompValues();
								...
								if (current.getId().equals("etablissementPreleveurBox")) {
									Entite prlvtEntite = ManagerLocator
											.getEntiteManager()
											.findByNomManager("Prelevement").get(0);
									ChampEntite champService = ManagerLocator
											.getChampEntiteManager()
											.findByEntiteAndNomManager(prlvtEntite,
													"ServicePreleveurId").get(0);
						...
						if (current.getId().equals("risquesBox") && 
								!getRisquesModel().getSelection().isEmpty()) {
							Entite prlvtEntite = ManagerLocator.getEntiteManager()
									.findByNomManager("Prelevement").get(0);
							ChampEntite champRisques = ManagerLocator
									.getChampEntiteManager()
							...
							// exécution de la requête
							List<Object> rNoms = new ArrayList<Object>();
							for (Risque r : getRisquesModel().getSelection()) {
								rNoms.add(r.getNom());
							}
							
							boolean cumulative = ((Checkbox) current.getNextSibling()).isChecked();
							executeListQuery("nom", "Risque", prlvtEntite, parent1,
									oneValueEntered, rNoms, cumulative);
						...
						.equals("Datebox")) {
					Datebox current = (Datebox) objPrelevementComponents[i];
					// si une valeur a été saisie
					if (current.getValue() != null) {
						...
						if (operateur != null) {
							executeSimpleQueryForDatebox(current,
									parent1ToQueryPrlvt, parent2ToQueryPrlvt,
									oneValueEntered, operateur, false);
						}
						...
						.equals("Intbox")) {
					Intbox current = (Intbox) objPrelevementComponents[i];
					// si une valeur a été saisie
					if (current.getValue() != null) {
						...
						if (current.getId().equals("nbEchantillonBox")) {
							...
							operateur = (String) operateursNbEchantillonBox
									.getListModel().getElementAt(idx);
							Integer nb = current.getValue();
							// execution de la requête
							executeNbEchantillonsQuery(oneValueEntered, operateur,
									nb);
							...
							operateur = (String) operateursAgePrlvtBox
									.getListModel().getElementAt(idx);
							Integer age = current.getValue();
							// execution de la requête
							executeAgeAuPrelevementQuery(oneValueEntered,
									operateur, age);
						}
					...
					if (current.getValue() != null
						...
						if (current.getId().equals("codeOrganeBox")) {
							...
							if (!entiteToSearch.getNom().equals("Echantillon")) {
								ChampEntite idEchan =  ManagerLocator
										.getChampEntiteManager().findByIdManager(50);
								Champ parent1 = new Champ(idEchan);
								parent1.setChampParent(parentToQueryEchantillon);
								
								executeObjetByNonConformite(current, oneValueEntered, 
										ManagerLocator.getConformiteTypeManager()
											.findByIdManager(2), 
										current.getValue(), parent1, null, searchForProdDerives);
							} else {
								executeObjetByNonConformite(current, oneValueEntered, 
										ManagerLocator.getConformiteTypeManager()
											.findByIdManager(2), 
				...
				if (objEchantillonComponents[i].getClass().getSimpleName()
					...
					if (current.getSelectedIndex() > 0) {
						if (!current.getId().equals("crAnapathFilebox")) {
							...
							executeFileQuery(current, getEntiteToSearch(), 
									new Boolean((String) current.getSelectedItem().getValue()));
						}
						oneValueEntered = true;
					}
				}
					...
					if (current.getValue() != null) {
						...
						if (current.getId().equals("tempStockEchantillonBox")) {
							...
							if (operateursTempStockEchantillonBox.getSelectedIndex() > 0) {
								idx = operateursTempStockEchantillonBox
										.getSelectedIndex();
							}
							operateur = (String) operateursTempStockEchantillonBox
									.getListModel().getElementAt(idx);
							...
							if (current.getId().equals("quantiteEchantillonBox")) {
								...
								if (operateursDelaiCglEchantillonBox.getSelectedIndex() > 0) {
									idx = operateursDelaiCglEchantillonBox
											.getSelectedIndex();
								}
								operateur = (String) operateursDelaiCglEchantillonBox
										.getListModel().getElementAt(idx);
							}
	...
	public void executeQueriesForProdDerives() {
		...
		for (int i = 0; i < objProdDeriveComponents.length; i++) {
			if (objProdDeriveComponents[i] != null) {
				// si c'est un textbox
				if (objProdDeriveComponents[i].getClass().getSimpleName()
						.equals("Textbox")) {
					Textbox current = (Textbox) objProdDeriveComponents[i];
					// si une valeur a été saisie
					if (current.getValue() != null
							&& !current.getValue().equals("")) {		
						 if (current.getId().equals("nonConformitesDeriveTraitementBox")) {
							 if (!entiteToSearch.getNom().equals("ProdDerive")) {
								 ChampEntite idDerive =  ManagerLocator
											.getChampEntiteManager().findByIdManager(76);
									Champ parent1 = new Champ(idDerive);
									parent1.setChampParent(parent1ToQueryProdDerive);
									Champ parent2 = new Champ(idDerive);
									parent2.setChampParent(parent2ToQueryProdDerive);
								 
								 executeObjetByNonConformite(current, oneValueEntered, 
										ManagerLocator.getConformiteTypeManager()
											.findByIdManager(4), 
										current.getValue(), parent1, parent2, searchForProdDerives); 
								} else {
									executeObjetByNonConformite(current, oneValueEntered, 
											ManagerLocator.getConformiteTypeManager()
												.findByIdManager(4), 
				...
				if (objProdDeriveComponents[i].getClass().getSimpleName()
						.equals("Listbox")) {
					Listbox current = (Listbox) objProdDeriveComponents[i];
					// si une valeur a été saisie
					if (current.getSelectedIndex() > 0) {
						// exécution de la requête
				...
				if (objProdDeriveComponents[i].getClass().getSimpleName()
						.equals("Decimalbox")) {
					Decimalbox current = (Decimalbox) objProdDeriveComponents[i];
					// si une valeur a été saisie
					if (current.getValue() != null) {
						...
						if (current.getId().equals("tempStockDeriveBox")) {
							...
							if (operateursTempStockDeriveBox.getSelectedIndex() > 0) {
								idx = operateursTempStockDeriveBox
										.getSelectedIndex();
							}
							operateur = (String) operateursTempStockDeriveBox
									.getListModel().getElementAt(idx);
							...
							if (current.getId().equals("quantiteDeriveBox")) {
								...
								if (operateursQuantiteDeriveBox.getSelectedIndex() > 0) {
									idx = operateursQuantiteDeriveBox
											.getSelectedIndex();
								}
								operateur = (String) operateursQuantiteDeriveBox
										.getListModel().getElementAt(idx);
								...
								operateur = (String) operateursVolumeDerivesBox
										.getListModel().getElementAt(idx);
							}
		
							// exécution de la requête
							executeDeriveQueryForDecimalbox(current, oneValueEntered,
									operateur);
						}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void executeNbEchantillonsQuery(boolean firstQuery,
			String operateur, Integer nbEchantillons) {
		...
		if (!otherQuery) {
			resultatsIds = ManagerLocator.getTraitementQueryManager()
					.findPrelevementsByNbEchantillonsWithBanquesManager(
							operateur, nbEchantillons, banques);
		} else {
			// sinon on fait une intersection des résultats de la
			// requête avec ceux déjà trouvés
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void executeAgeAuPrelevementQuery(boolean firstQuery,
			String operateur, Integer age) {
		...
		try {
			...
			if (!otherQuery) {
				resultatsIds = ManagerLocator.getTraitementQueryManager()
						.findPrelevementsByAgePatientWithBanquesManager(operateur,
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void executePrelevementsByMedecinQuery(boolean firstQuery,
		...
		if (!otherQuery) {
			resultatsIds = ManagerLocator.getTraitementQueryManager()
					.findPrelevementsByMedecinsManager(collab, banques);
		} else {
			// sinon on fait une intersection des résultats de la
			// requête avec ceux déjà trouvés
			List<Integer> objects = ManagerLocator.getTraitementQueryManager()
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void executeObjetByNonConformite(Textbox current, boolean firstQuery,
			ConformiteType cType, String cNom, Champ p1, Champ p2, 
			boolean searchForDerive) {

		// si c'est la 1ère requête les résultats vont directement
		// dans la liste
		List<Integer> objects = ManagerLocator.getTraitementQueryManager()
				.findObjetIdsFromNonConformiteNomManager(cNom, cType, 
						SessionUtils.getPlateforme(sessionScope), 
						SessionUtils.getSelectedBanques(sessionScope));
		
		List<? extends Object> o1 = null;
		List<? extends Object> o2 = null;
		if (p1 != null && p1.getChampParent() != null 
				&& !p1.getChampParent().getChampEntite().equals(cType.getEntite())) {
			Critere c = new Critere(p1, "in", "");
			
			if (!searchForDerive && !p1.getChampEntite().getNom().matches(".*ProdDerive.*")) {
				o1 = ManagerLocator.getTraitementQueryManager()
					.findObjetByCritereWithBanquesManager(c, 
							SessionUtils.getSelectedBanques(sessionScope), 
							objects, true);
			} else {
				o1 = ManagerLocator.getTraitementQueryManager()
						.findObjetByCritereWithBanquesDeriveVersionManager(c,  
								SessionUtils.getSelectedBanques(sessionScope), 
								objects, searchForDerive, true);
			}
		}
		if (p2 != null && p2.getChampParent() != null 
				&& !p2.getChampParent().getChampEntite().equals(cType.getEntite())) {
			Critere c = new Critere(p2, "in", "");
			
			if (!searchForDerive && !p2.getChampEntite().getNom().matches(".*ProdDerive.*")) {
				o2 = ManagerLocator.getTraitementQueryManager()
					.findObjetByCritereWithBanquesManager(c, 
							SessionUtils.getSelectedBanques(sessionScope), 
							objects, true);
			} else {
		...
		if (!otherQuery) {
			resultatsIds = (List<Integer>) objects;
		} else {
			resultatsIds = ListUtils.intersection(resultatsIds, objects);
		}
		otherQuery = true;
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void executeTempStockQuery(boolean firstQuery, 
			Decimalbox current, String op, Entite ent) {
		...
		if (!otherQuery) {
			resultatsIds.addAll((List<Integer>) ManagerLocator.getTraitementQueryManager()
					.findTKStockableObjectsByTempStockWithBanquesManager(ent, 
					current.getValue().floatValue(), op, banques, true));
		} else {
			// sinon on fait une intersection des résultats de la
			// requête avec ceux déjà trouvés
			List<Integer> objects = (List<Integer>) ManagerLocator.getTraitementQueryManager()
					.findTKStockableObjectsByTempStockWithBanquesManager(ent, 
							current.getValue().floatValue(), op, banques, true);
	...
	public void setEntiteToSearch(Entite entite) {
		this.entiteToSearch = entite;
	}
	...
	public void setPathToRespond(String path) {
		this.pathToRespond = path;
	}
	...
	public void setConsentTypes(List<ConsentType> consentTs) {
	...
	public void setServices(List<Service> servs) {
		this.services = servs;
	}
	...
	public void setEchanQualites(List<EchanQualite> es) {
	...
	public void setObjetStatuts(List<ObjetStatut> objetSs) {
	...
	public void setEchantillonTypes(List<EchantillonType> eTypes) {
	...
	public void setCollaborateurs(List<Collaborateur> cs) {
	...
	public void setNCarrivee(List<NonConformite> n) {
		this.nCarrivee = n;
	}
	...
	public void setNCechanTraitement(List<NonConformite> t) {
	...
	public void setNCechanCession(List<NonConformite> t) {
	...
	public void setNCderiveTraitement(List<NonConformite> t) {
	...
	public void setNCderiveCession(List<NonConformite> t) {
	...
	public void setOperateursDecimaux(List<String> opsDecimaux) {
	...
	public void setOperateursDates(List<String> oDates) {
	...
	public void setParent1ToQueryPatient(Champ parentToQuery) {
	...
	public void setParent2ToQueryPatient(Champ parentToQuery) {
	...
	public void setParent1ToQueryMaladie(Champ parentToQuery) {
	...
	public void setParent2ToQueryMaladie(Champ parentToQuery) {
	...
	public void setParent1ToQueryPrlvt(Champ parent1ToQuery) {
	...
	public void setParentToQueryEchantillon(Champ parentToQuery) {
	...
	public void setOneValueEntered(boolean oneValue) {
	...
	public void setResultats(List<Object> res) {
		this.resultats = res;
	}
	...
	public void setAnnotationAlreadyOpen(boolean aOpen) {
	...
	public void setParent1ToQueryProdDerive(Champ parent1ProdDerive) {
	...
	public void setParent2ToQueryProdDerive(Champ parent2ProdDerive) {
	...
	public void setSearchForProdDerives(boolean searchDerives) {
	...
	public void setProdDeriveTypes(List<ProdType> prodTypes) {
	...
	public void setDeriveQualites(List<ProdQualite> dQualites) {
	...
	public void setParent2ToQueryPrlvt(Champ parent2ToQuery) {
	...
	public void setModePrepas(List<ModePrepa> mPrepas) {
	...
	public void setCriteresStandards(List<Critere> criteres) {
	...
	public void setValeursStandards(List<Object> valeurs) {
	...
	public void setOtherQuery(boolean other) {
	...
	public void setCritereOnEchantillon(boolean critereOnE) {
	...
	public void setCriteresDerives1(List<Critere> criteres) {
	...
	public void setValeursDerives1(List<Object> valeurs) {
	...
	public void setCriteresDerives2(List<Critere> criteres) {
	...
	public void setValeursDerives2(List<Object> valeurs) {
	...
	private void executeQueriesForPrelevementsContext() {
		...
		Entite prlvtEntite = ManagerLocator.getEntiteManager()
				.findByNomManager("Prelevement").get(0);
		ChampEntite delegate = ManagerLocator.getChampEntiteManager()
				.findByEntiteAndNomManager(prlvtEntite, "Delegate").get(0);
		...
		for (int i = 0; i < objPrelevementContextComponents.length; i++) {
			...
			if (objPrelevementContextComponents[i].getClass().getSimpleName()
					.equals("Textbox")) {
				Textbox current = (Textbox) objPrelevementContextComponents[i];
				// si une valeur a été saisie
				if (current.getValue() != null
						&& !current.getValue().equals("")) {
			...
			if (objPrelevementContextComponents[i].getClass().getSimpleName()
					.equals("Listbox")) {
				Listbox current = (Listbox) objPrelevementContextComponents[i];
				// si une valeur a été saisie
				if (current.getSelectedIndex() > 0) {
					if (current.getId().equals("protocolesBox")) {
						delegate = ManagerLocator
								.getChampEntiteManager()
								.findByEntiteAndNomManager(prlvtEntite,
										"Delegate.protocoles").get(0);
					...
					executeSimpleQueryForListbox(current, parent1, parent2,
							oneValueEntered,
							((String) current.getAttribute("attribut"))
									.toLowerCase());
				}
				oneValueEntered = true;
			}
	...
	public void onSelect$nonConformitesArriveeBoxHelper() {
		if (nonConformitesArriveeBoxHelper.getSelectedIndex() > 0) {
			nonConformitesArriveeBox.setValue(getNCarrivee()
				.get(nonConformitesArriveeBoxHelper.getSelectedIndex()).getNom());
		} else {
	...
	public void onSelect$nonConformitesEchanTraitementBoxHelper() {
		if (nonConformitesEchanTraitementBoxHelper.getSelectedIndex() > 0) {
			nonConformitesEchanTraitementBox.setValue(getNCechanTraitement()
				.get(nonConformitesEchanTraitementBoxHelper.getSelectedIndex()).getNom());
		} else {
	...
	public void onSelect$nonConformitesEchanCessionBoxHelper() {
		if (nonConformitesEchanCessionBoxHelper.getSelectedIndex() > 0) {
			nonConformitesEchanCessionBox.setValue(getNCechanCession()
				.get(nonConformitesEchanCessionBoxHelper.getSelectedIndex()).getNom());
		} else {
	...
	public void onSelect$nonConformitesDeriveTraitementBoxHelper() {
		if (nonConformitesDeriveTraitementBoxHelper.getSelectedIndex() > 0) {
			nonConformitesDeriveTraitementBox.setValue(getNCderiveTraitement()
				.get(nonConformitesDeriveTraitementBoxHelper.getSelectedIndex()).getNom());
		} else {
	...
	public void onSelect$nonConformitesDeriveCessionBoxHelper() {
		if (nonConformitesDeriveCessionBoxHelper.getSelectedIndex() > 0) {
			nonConformitesDeriveCessionBox.setValue(getNCderiveCession()
				.get(nonConformitesDeriveCessionBoxHelper.getSelectedIndex()).getNom());
		} else {
	...
	public List<String> getOpenedGroups() {
		List<String> openedGroups = new ArrayList<String>();
