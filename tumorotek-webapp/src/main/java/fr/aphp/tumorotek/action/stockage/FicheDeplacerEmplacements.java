...
public class FicheDeplacerEmplacements extends FicheTerminale
{
	...
	public void onLaterDeplacement(){
		...
		if(!getObjStatutIncompatibleForRetour(prepareObjectsAndEmplacements(), null) && deplacerEmplacements()){
			...
			if(getEmplForRetours().size() > 0){
				...
				// créer des retours
				if(askForRetoursCreation(getEmplForRetours().size(), null, null, new ArrayList<OldEmplTrace>(),
						null, ManagerLocator.getOperationTypeManager().findByNomLikeManager("creation", true).get(0))){

					// ouverture de la modale
					openRetourFormModale(null, true, null, null, getEmplForRetours().stream().map(o -> o.getTkObj())
			          		  .collect(Collectors.toList()), getEmplForRetours(),
	...
	public void updateEchantillons(final List<Emplacement> emplFinaux, final List<TKStockableObject> echans){
		...
		for(int i = 0; i < echans.size(); i++){
			...
			ManagerLocator.getEchantillonManager().updateObjectManager(echanToUpdate,
					...
					echanToUpdate.getEchanQualite(), echanToUpdate.getModePrepa(),
	...
	public void updateProdDerives(final List<Emplacement> emplFinaux, final List<ProdDerive> prods){
		...
		for(int i = 0; i < prods.size(); i++){
			...
			ManagerLocator.getProdDeriveManager().updateObjectManager(deriveToUpdate,
					...
					deriveToUpdate.getTransformation(), null, null, null, null,
	 ...
	 */
	@Override
	public void definirTerminaleDestination(final TerminaleNode node, Conteneur cont, List<Banque> anks){
	 ...
	 * @since 2.2.1
	 */
	private void checkEmplacementDeplacementConteneurCoherence(EmplacementDecorator source, 
			EmplacementDecorator dest) {

		if (source != null && dest != null 
				&& source.getEmplacement() != null && dest.getEmplacement() != null) {
			
			Conteneur destCont = ManagerLocator
					.getEmplacementManager().getConteneurManager(dest.getEmplacement());
			//Set<Banque> banks = ManagerLocator.getConteneurManager().getBanquesManager(destCont);
			List<Banque> banks = ManagerLocator.getBanqueManager().findByConteneurManager(destCont);
			...
			if (!banks.contains(source.getTkStockObj().getBanque())) {
				throw new TKException(ObjectTypesFormatters.getLabel("stockage.deplacement.conteneur.destination.banks", 
						new String[] { destCont.getCode(), 
								source.getTkStockObj().getBanque().getNom() }));
			}
		...
		for(int i = 0; i < deplacements.size(); i++){
			...
			if(deco.getEmplDestination() != null){
				...
				if(!listeBoitesDepart.contains(newBiDep)){
					...
					// si l'élément a deplacer est un echantillon
					bi.getElements().add(
							ObjectTypesFormatters.getLabel("impression.boite.numero.objet", new String[] {pos.toString(), deco.getCode()}));
				}
			}
		}
