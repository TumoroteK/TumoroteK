...
public class FicheEtiquetteDynModale extends FicheEtiquetteModale
{
   ...
   private final List<Row> rowsToAdd = new ArrayList<>();
   ...
   public void init(@ExecutionArgParam("modele") final Modele m, @ExecutionArgParam("imprimante") final Imprimante imp,
      ...
      @ExecutionArgParam("parent") final Window win, @ExecutionArgParam("lignes") final List<LigneEtiquette> _ligs){
      setModele(m);
      setRawLang(rL);
      ...
      if(getModele() != null){
         if(_ligs == null){
            lignes = ManagerLocator.getLigneEtiquetteManager().findByModeleManager(getModele());
         }else{
            lignes = _ligs;
         }
      }
   ...
   private void drawLignes(){
      ...
      for(int i = 0; i < lignes.size(); i++){
         ...
         if(lignes.get(i).getEntete() != null){
            final Label label = new Label(lignes.get(i).getEntete());
            ...
            if(lignes.get(i).getIsBarcode()){
               ...
               if(lignes.get(i).getLigneEtiquetteId() != null){
                  champs = ManagerLocator.getChampLigneEtiquetteManager().findByLigneEtiquetteManager(lignes.get(i));
               }
   ...
   public List<LigneEtiquette> createListeEtiquette(final String inc){
      final List<LigneEtiquette> ls = new ArrayList<>();
      ls.addAll(lignes);
      ...
      for(int i = 0; i < textboxes.size(); i++){
         ...
         final String contenu = textboxes.get(i).getValue();
   ...
   public void createListLignes(final int i){
      final List<LigneEtiquette> tmpLigne = new ArrayList<>();
   ...
   public void setTextboxes(final List<Textbox> t){
      this.textboxes = t;
   }
   ...
   public void setCheckboxes(final List<Checkbox> c){
      this.checkboxes = c;
   }
   ...
   public void setLabels(final List<Label> l){
      this.labels = l;
   }
   ...
   @Command
   public void cancel(){
      Events.postEvent("onClose", fwinEtiquetteDynModale, null);
