 ...
 *
 */
public class FicheModifMultiEchantillon extends 
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		List<LabelCodeItem> labs = new ArrayList<LabelCodeItem>();
		labs.add(new LabelCodeItem(Labels
				.getLabel("echantillon.lateralite.G"), "G"));
		labs.add(new LabelCodeItem(Labels
				.getLabel("echantillon.lateralite.D"), "D"));
		labs.add(new LabelCodeItem(Labels
				.getLabel("echantillon.lateralite.I"), "I"));
		labs.add(new LabelCodeItem(Labels
				.getLabel("echantillon.lateralite.B"), "B"));
		lats = labs;
	}
	...
	public AbstractObjectTabController getObjectTabController() {
		return ((EchantillonController) self.getParent().getParent()
				.getParent()
...
//			quantiteLabelChanged.setVisible(!reset);
	...
	public void updateMultiObjects() {
		...
		List<Echantillon> clones = new ArrayList<Echantillon>();
		...
		for (int i = 0; i < getObjsToEdit().size(); i++) {
			Echantillon current = ((Echantillon) 
...
//			}
			...
			if (!organeLabelChanged.getValue().equals("")
					|| !codeLesLabelChanged.getValue().equals("")) {
				if (!organeLabelChanged.getValue().equals("")) {
					codesToDelete.addAll(ManagerLocator.getCodeAssigneManager()
							.findCodesOrganeByEchantillonManager(current));
				}
				if (!codeLesLabelChanged.getValue().equals("")) {
					codesToDelete.addAll(ManagerLocator.getCodeAssigneManager()
							.findCodesMorphoByEchantillonManager(current));
				} 				
		...
		if (hasAnyChange) {
			ManagerLocator.getEchantillonManager()
				.updateMultipleObjectsManager(clones, 
						(List<Echantillon>) getObjsToEdit(), 
						codes,
						codesToDelete,
						crAnapath, anapathStream, deleteCrAnapath, 
	...
	public void onClick$echantillonTypeMultiLabel() {					
		...
		openModificationMultipleWindow(page, 
				...
				"Listbox", 
				getObjsToEdit(), 
				"Champ.Echantillon.EchantillonType.Type", 
				"echantillonType",
				(List<Object>) types,
				"type",
				null, 
...
//	}
	...
	public void onClick$quantiteInitMultiLabel() {	
		...
		openModificationMultipleWindow(page, 
				...
				"Quantification", 
				getObjsToEdit(), 
				"Champ.Echantillon.QuantiteInit", 
				"quantiteInit",
				(List<Object>) units,
				"unite",
				null, 
	...
	public void onClick$modePrepaMultiLabel() {		
		List< ? extends Object> modes = ManagerLocator
		.getModePrepaManager()
			.findByOrderManager(SessionUtils.getPlateforme(sessionScope));
	...
	public void onClick$delaiCglMultiLabel() {		
		openModificationMultipleWindow(page, 
				...
				"Floatbox", 
				getObjsToEdit(), 
				"Champ.Echantillon.DelaiCgl", 
				"delaiCgl",
				null,
				null,
				null, 
				new SimpleConstraint(SimpleConstraint.NO_NEGATIVE),
				false,
				null, 
				false);
	}
	...
	public void onClick$operateurMultiLabel() {		
		...
		openModificationMultipleWindow(page, 
				...
				"Combobox", 
				getObjsToEdit(), 
				"Champ.Echantillon.Collaborateur", 
				"collaborateur",
				(List<Object>) ops,
				"nomAndPrenom",
				null, 
	...
	public void onClick$qualiteMultiLabel() {		
		...
		openModificationMultipleWindow(page, 
				...
				"Listbox", 
				getObjsToEdit(), 
				"Champ.Echantillon.EchanQualite", 
				"echanQualite",
				(List<Object>) quals,
				"echanQualite",
				null, 
	...
	public void onClick$crAnapathMultiLabel() {	
		...
		openModificationMultipleWindow(page, 
				...
				"Filebox", 
				getObjsToEdit(), 
				"ficheEchantillon.crAnapathLabel", 
				"crAnapath",
				null,
				null,
				null, 
	...
	public void onClick$codeLesMultiLabel() {	
		openModificationMultipleWindow(page, 
				...
				"CodesBox", 
				getObjsToEdit(), 
				"ficheEchantillon.codeLesionelLabel", 
				"codeMorphos",
				null,
				null,
				null, 
	...
	public void onClick$nonConformeTraitementMultiLabel() {	
		...
		openModificationMultipleWindow(page, 
				...
				"Conformitebox", 
				getObjsToEdit(), 
				"Champ.Echantillon.ConformeTraitement", 
				"conformeTraitement",
	...
	public void onClick$nonConformeCessionMultiLabel() {	
		...
		openModificationMultipleWindow(page, 
				...
				"Conformitebox", 
				getObjsToEdit(), 
				"Champ.Echantillon.ConformeCession", 
				"conformeCession",
				(List<Object>) nonConfs,
				"Cession",
				null, 
	...
	public void onGetChangeOnChamp(Event e) {
		...
		if (tmp.getValue() instanceof LabelCodeItem) {
			...
			CodesPack pck = (CodesPack) tmp.getValue();
			if ("codeOrganes".equals(tmp.getChamp())) {
				getObject().getCodesAssignes().addAll(CodeAssigneDecorator
								.undecorateListe(pck.getCodesToCreateOrEdit()));
			...
			FilePack pck = (FilePack) tmp.getValue();
	...
	public void onResetMulti(Event event) {
		...
		if ("codeOrganes".equals(tmp.getChamp()) 
				|| "codeMorphos".equals(tmp.getChamp())
				|| "crAnapath".equals(tmp.getChamp())) {
			List<CodeAssigne> codesToRemove = new ArrayList<CodeAssigne>();
			if ("codeOrganes".equals(tmp.getChamp())) {
				Iterator<CodeAssigne> codesIt = getObject().getCodesAssignes().iterator();
				CodeAssigne next;			
