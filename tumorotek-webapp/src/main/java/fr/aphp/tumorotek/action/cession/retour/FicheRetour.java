...
public class FicheRetour extends AbstractFicheCombineController {
	...
	private ListModelList<TKStockableObject> objects = 
							new ListModelList<TKStockableObject>();
	// private Set<Listitem> selectedObjectsItem = new HashSet<Listitem>();
	private HashMap<TKStockableObject, Emplacement> oldEmplacements = 
	...
	public void setObjects(List<TKStockableObject> objs) {
		if (objs != null) {
			this.objects.addAll(objs);
		}
		Collections
			.sort(getObjects(), new TKStockableObject.CodeComparator(false));
	}

	public void setCession(Cession ces) {
		this.cession = ces;
		// les évènements de stockage ne peuvent être incomplets 
		// que si la cession est attente. 
		boolean validee = ces != null && !ces.getCessionStatut()
				.getStatut().equals("EN ATTENTE");
		dateRetourOblig = dateRetourOblig || validee;
		dateRetourCalBox.setDisabled(!validee);
	}

	public void setTransformation(Transformation tr) {
		this.transformation = tr;
		dateRetourOblig = (dateRetourOblig || tr != null);
	}

	public void setIncident(Incident ic) {
		this.incident = ic;
...
//	}
	 ...
	 * @param Date in
	 */
	public void setInitDateSortie(Date in) {
		if (in != null) {
			...
			if (cession != null && !cession.getCessionStatut()
				.getStatut().equals("EN ATTENTE")) {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[]{
				this.dateSortieLabel,
				this.dateRetourLabel,
				this.tempMoyenneLabel,
				this.sterileLabel,
				this.collaborateurLabel,
				this.observationsLabel,
				this.emplacementLabel,
		});
		
		setObjBoxsComponents(new Component[]{
				this.dateSortieCalBox,
				this.dateRetourCalBox,
				this.tempMoyenneBox,
				this.collaborateurBox,
				this.sterileBox,
				this.observationsBox, 
				this.temperatureListBox,
				this.emplacementBox,
				this.impactBox
		});
		
		setRequiredMarks(new Component[]{dateSortieRequired, 
										tempMoyenneRequired});	
	...
	public void setObject(TKdataObject obj) {
		...
		if (tkObject == null) {
			tkObject = (TKStockableObject) ManagerLocator.getRetourManager()
					.getObjetFromRetourManager(retour);
		}
	...
	public void setTkObject(TKStockableObject tk) {
		this.tkObject = tk;
		if (retour.getRetourId() == null) { // creation
			if (tk instanceof Echantillon) {
				setOldEmplacementAdrl(ManagerLocator.getEchantillonManager()
						.getEmplacementAdrlManager((Echantillon) tk));
			} else if (tk instanceof ProdDerive) {
				setOldEmplacementAdrl(ManagerLocator.getProdDeriveManager()
						.getEmplacementAdrlManager((ProdDerive) tk));
			}
		}
	...
	public void switchToCreateMode(String obs) {
...
//				}
				 ...
				 ((Selectable<TKStockableObject>) objectsBox.getModel())
				 	.setMultiple(true);
				 ((Selectable<TKStockableObject>) objectsBox.getModel())
...
//			}
	...
	public void createNewObject() {
		...
		if (tkObject != null) {
			ManagerLocator.getRetourManager()
				.createOrUpdateObjectManager(retour, tkObject, null,
						selectedCollaborateur, cession, transformation, 
						 incident, 
						SessionUtils.getLoggedUser(sessionScope), 
						"creation");
		} else { // creation liste
			List<TKStockableObject> objToUpdate = new ArrayList<TKStockableObject>();
			if (getObjects().size() <= 100 &&  getObjects().size() > 1) {
				objToUpdate.addAll(findSelectedObjList());	
			} else {
				objToUpdate.addAll(getObjects());
			}
			insertOk = ManagerLocator.getRetourManager()
					.createRetourHugeListManager(
						objToUpdate, 
						oldEmplacements, 
						retour, 
						selectedCollaborateur, cession, transformation, 
			...
			if (insertOk && (retour.getDateRetour() == null || 
					(retour.getImpact() != null && retour.getImpact())
				|| (retour.getSterile() != null && !retour.getSterile())) 
					&& getObjects().size() < 100) {
				
				List<TKdataObject> derives = new ArrayList<TKdataObject>();
				List<TKdataObject> echans = new ArrayList<TKdataObject>();
				...
				for (int i = 0; i < getObjects().size(); i++) {
					if (getObjects().get(i) instanceof Echantillon) {
						...
						// des échantillons
						// refresh object
						echans.add(ManagerLocator.getEchantillonManager()
								.findByIdManager(getObjects().get(i)
								.listableObjectId()));
						...
						// refresh object
						derives.add(ManagerLocator.getProdDeriveManager()
							.findByIdManager(getObjects().get(i)
									.listableObjectId()));
					}
				}
				...
				if (!echans.isEmpty() 
					&& getMainWindow().isFullfilledComponent(
						"echantillonPanel", "winEchantillon") 
					&& getEchantillonController() != null
					&& getEchantillonController()
					.getListe() != null) {
					getEchantillonController().getListe()
					.updateMultiObjectsGridListInPlace(echans);
				}
				// update de l'échantillon dans la liste
				if (!derives.isEmpty()
						&& getMainWindow().isFullfilledComponent(
					...
					.updateMultiObjectsGridListInPlace(derives);
				}
			}
		}
	}
	...
	public void setEmptyToNulls() {
		...
		String selectedNomAndPremon = collaborateurBox.getValue()
														.toUpperCase();
	...
	public void updateObject() {
		...
		.createOrUpdateObjectManager(retour, tkObject, null,
			selectedCollaborateur, cession, transformation, 
			incident, SessionUtils.getLoggedUser(sessionScope), 
			"modification");
	}
	...
	public void initEditableMode() {		
		...
		if (selectedCollaborateur != null
				&& collaborateurs.contains(selectedCollaborateur)) {
	...
	public void onClick$validateC() {
		try {
			...
			if (getListeRetour() != null) {
				getListeRetour()
					.updateObjectGridListFromOtherPage(getObject(), true);
			}
	...
	public void onLaterCreate() {
		try {
			validateCoherenceDate(dateSortieCalBox, 
							dateSortieCalBox.getValue());
			validateCoherenceDate(dateRetourCalBox, 
							dateRetourCalBox.getValue());
			createNewObject();
			...
			if (insertOk) {
				...
				if (getListeRetour() != null) {
					...
					Messagebox.show(Labels
								.getLabel("message.retour.create.done"),
							Labels.getLabel("message.retour.create.title"), 
							Messagebox.OK, 
				...
				Messagebox.show(
						Labels.getLabel("message.retour.error"), 
						"Error", Messagebox.OK, Messagebox.ERROR);
			}
	...
	private List<TKStockableObject> findSelectedObjList() {
		if (getObjects().size() <= 100) {
			List<TKStockableObject> list = new ArrayList<TKStockableObject>();
...
//			}
//			return list;
			list.addAll(((Selectable<TKStockableObject>) objectsBox.getModel())
					.getSelection());
			return list;
		} else {
			return getObjects();
...
//			} 
	...
	public String getSClassCollaborateur() {
		return ObjectTypesFormatters
			.sClassCollaborateur(this.retour.getCollaborateur());
	}
	
	public String getDateSortieFormatted() {
		return ObjectTypesFormatters
			.dateRenderer2(this.retour.getDateSortie());

	}
	
	public String getDateRetourFormatted() {		
		return ObjectTypesFormatters
			.dateRenderer2(this.retour.getDateRetour());
	}
	
	public String getSterileFormatted() {		
		return ObjectTypesFormatters
			.booleanLitteralFormatter(this.retour.getSterile());
	}
	...
	public void onBlur$dateRetourCalBox() {
		if (dateRetourCalBox.getValue() == null 
				&& !dateRetourCalBox.isHasChanged()) {
		...
		if (comp.getId().equals("dateSortieCalBox")) {
			if (value == null || value.equals("")) { 
				throw new WrongValueException(
						comp, Labels.getLabel("validation.syntax.empty"));
			}
			retour.setDateSortie(((CalendarBox) comp).getValue());			
			if (retour.getDateRetour() != null) { 
				if (!ValidationUtilities
				.checkWithDate(retour.getDateSortie(), "dateSortie", 
					retour.getDateRetour(), "date", "validation",
								"DateRetour", null, true)) {
					throw new WrongValueException(
						comp, Labels.getLabel("date"
								+ ".validation.supDateRetour"));
				}
			} 
		} else if (comp.getId().equals("dateRetourCalBox")) {
			retour.setDateRetour(((CalendarBox) comp).getValue());
			if (retour.getDateRetour() != null 
					&& retour.getDateSortie() != null
				&& !ValidationUtilities
				.checkWithDate(retour.getDateRetour(), "dateRetour", 
					retour.getDateSortie(), "date", "validation",
								"DateSortie", null, false)) {
				throw new WrongValueException(
						comp, Labels.getLabel("date"
								+ ".validation.infDateSortie"));
			} 
		}		
	}
	...
	public void onSelect$temperatureListBox(Event event) {
		...
		if (selectedTemperature != null) {
			tempMoyenneBox.setValue(new BigDecimal(selectedTemperature
													.getTemperature()));
		} 
	}
	...
	public void setOldEmplacements(
			HashMap<TKStockableObject, Emplacement> o) {
...
//	}
	...
	public void setInsertOk(boolean ok) {
		this.insertOk = ok;
	}
	...
	public void setOldEmplacementAdrl(String a) {
	...
	public Boolean getDotVisible() {
		return retour != null && retour.getImpact() != null 
				&& retour.getImpact() && !impactBox.isVisible();
	}
}
