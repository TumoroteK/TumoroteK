...
private static final long serialVersionUID = 1L;
	...
	public void setIsCreate(boolean isC) {
		this.isCreate = isC;
	}
	
	public void setDisableDefaut(boolean dD) {
		this.disableDefaut = dD;
	}
	
	public void setIsDefaut(boolean isDef) {
		this.isDefaut = isDef;
	}
	...
	public void afterCompose() {
		...
		// textboxes values
		((Textbox) getFirstChild().getFellow("labelBox"))
											.setValue(item.getLabel());
		
		((Textbox) getFirstChild().getFellow("valBox"))
											.setValue(item.getValeur());
		
		ConstWord constr = new ConstWord();
		constr.setNullable(false);
		((Textbox) getFirstChild().getFellow("labelBox"))
												.setConstraint(constr);
		ConstWord constr2 = new ConstWord();
		constr2.setNullable(true);
		((Textbox) getFirstChild().getFellow("valBox"))
												.setConstraint(constr2);
		
		// checkbox defaut
		((Checkbox) getFirstChild().getFellow("defautBox"))
												.setDisabled(disableDefaut);
		if (!disableDefaut) {
			((Checkbox) getFirstChild().getFellow("defautBox"))
												.setChecked(isDefaut);
		}
		...
		// Events listeners
		getFirstChild().getFellow("valImg")
			.addEventListener("onClick", new EventListener() {
			public void onEvent(Event event) throws Exception {
				recordItem();
			}
		});
		getFirstChild().getFellow("cancelImg")
			.addEventListener("onClick", new EventListener() {
				public void onEvent(Event event) throws Exception {
					cancelItem();
				}
		});
		getFirstChild().getFellow("defautBox")
		.addEventListener("onCheck", new EventListener() {
		public void onEvent(Event event) throws Exception {
			defautChged = true;
		}
	});
	...
	private void recordItem() {
		...
		item.setLabel(((Textbox) getFirstChild()
									.getFellow("labelBox")).getValue());
		if (!("".equals(((Textbox) getFirstChild()
								.getFellow("valBox")).getValue()))) {
			item.setValeur(((Textbox) getFirstChild()
									.getFellow("valBox")).getValue());
		} else {
			item.setValeur(null);
		}
		...
		// defaut
		if (defautChged) {
			if (((Checkbox) getFirstChild()
								.getFellow("defautBox")).isChecked()) {
				Events.postEvent(new Event("onAddDefautForItem", 
														backComp, item));
			} else {
				Events.postEvent(new Event("onDropDefautForItem", 
														backComp, item));
			}
		}
