...
public class FusionVM {
	...
	private ListModelList<DuoEntites> duoModel = new ListModelList<DuoEntites>();
    ...
    public void afterCompose(@ContextParam(ContextType.VIEW) Component view){
        ...
        if (winContexteFusion != null) {
        	winContexteFusion.setHeight(((MainWindow) 
        			view.getPage().getFellow("mainWin").getAttributeOrFellow(
    				"mainWin$composer", true)).getPanelHeight() - 5 + "px");
		}
	...
	@Command
	public void onDeleted(@BindingParam("toRemove") DuoEntites duo) {		
		duoModel.remove(duo);
	}
	...
	public void rechercheDoublon() {
		duoModel.clear();
		List<TKFantomableObject> lsA = new ArrayList<TKFantomableObject>();
		List<TKFantomableObject> lsB = new ArrayList<TKFantomableObject>();
		...
		switch (entiteRecherche) {				
			case "Collaborateur":
				...
				if (critereRecherche != null && critereRecherche.equals("Ville")) {
					lsA.addAll(ManagerLocator.getCollaborateurManager()
							.findByVilleLikeManager(rechValue));
				} else {
					lsA.addAll(ManagerLocator.getCollaborateurManager()
							.findByNomLikeManager(rechValue, false));
				}
				break;
			case "Service":
				lsB.addAll(ManagerLocator.getServiceManager().findAllObjectsManager());
				if (critereRecherche != null && critereRecherche.equals("Ville")) {
					lsA.addAll(ManagerLocator.getServiceManager()
												.findByVilleLikeManager(rechValue));
				} else {
					lsA.addAll(ManagerLocator.getServiceManager()
							.findByNomLikeManager(rechValue, false));
			...
			case "Etablissement":
				...
				if (critereRecherche != null && critereRecherche.equals("Ville")) {
					lsA.addAll(ManagerLocator.getEtablissementManager()
											.findByVilleLikeManager(rechValue));
				} else {
					lsA.addAll(ManagerLocator.getEtablissementManager()
							.findByNomLikeManager(rechValue, false));
		...
		for (TKFantomableObject a : lsA) {
			for (TKFantomableObject b : lsB) {
				addDuoIfDegreSimil(a, b);
			}
		}
	}

	private void addDuoIfDegreSimil(TKFantomableObject a, TKFantomableObject b) {

		double degreSimil;
		float tauxSimilitudeF = (float) tauxSimilitude/100;
		if(a != null && b != null && a != b) {
			switch (entiteRecherche) {
			case "Collaborateur":
				Collaborateur colla = (Collaborateur) a;
				Collaborateur collb = (Collaborateur) b;
				
				if (!colla.getCollaborateurId().equals(collb.getCollaborateurId())) {
	
					degreSimil = (LevenshteinDistance.similarityTwoParams(colla.getNom(), collb.getNom(),
							colla.getPrenom(), collb.getPrenom()));
					
					if (degreSimil >= tauxSimilitudeF) {
						DuoEntites duo = new DuoEntites(colla.getCollaborateurId(),
								collb.getCollaborateurId(),
								...
								collb.getEtablissement() != null ? collb.getEtablissement().getNom() : null,
								degreSimil);
				...
				break;
			case "Service":
				Service serva = (Service) a;
				Service servb = (Service) b;
	
				if (!serva.getServiceId().equals(servb.getServiceId())) {
					degreSimil = LevenshteinDistance.similarityTwoParams(serva
							.getNom(), servb.getNom(), serva.getEtablissement()
							.getNom(), servb.getEtablissement().getNom());
					if (degreSimil >= tauxSimilitudeF) {
						DuoEntites duo = new DuoEntites(serva.getServiceId(),
								servb.getServiceId(), serva.getNom(),
								servb.getNom(), serva.getEtablissement()
										.getNom(), servb.getEtablissement()
										.getNom(), degreSimil);
						if (!duoModel.contains(duo)) {
							duoModel.add(duo);
						}
				...
				break;
			case "Etablissement":
				Etablissement etala = (Etablissement) a;
				Etablissement etalb = (Etablissement) b;
	
				if (!etala.getEtablissementId().equals(etalb.getEtablissementId())) {
					degreSimil = LevenshteinDistance.similarityTwoParams(etala
							.getNom(), etalb.getNom(),
							etala.getCoordonnee() != null ? etala
									.getCoordonnee().getVille() : null, etalb
									.getCoordonnee() != null ? etalb
									.getCoordonnee().getVille() : null);
					if (degreSimil >= tauxSimilitudeF) {
						DuoEntites duo = new DuoEntites(
								etala.getEtablissementId(),
								etalb.getEtablissementId(), etala.getNom(),
	...
	public void onClicked() {
		...
		if (!winContexteFusion.hasFellow("affichageFusion")) {
			...
			Executions.createComponents(
	                "/zuls/outils/contexte/fusion/FormulaireFusion.zul", winContexteFusion, map);
	...
	public void setDuoSelectedEntites(DuoEntites duoSelectedEntites) {
	...
	public void setTauxSimilitude(int tauxSimilitude) {
	...
	public void setRechValue(String r) {
		this.rechValue = r;
	}
