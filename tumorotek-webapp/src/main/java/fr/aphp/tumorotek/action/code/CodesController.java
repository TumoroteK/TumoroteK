...
public class CodesController extends AbstractController {
	...
	private I3listBoxItemRenderer codageRenderer = 
											new I3listBoxItemRenderer("nom");
	private static CodesSelectedRowRenderer selCodesRenderer =
											new CodesSelectedRowRenderer();
	
	private static CodesSelectedRowRenderer transCodesRenderer =
												new CodesSelectedRowRenderer();
	static {
		transCodesRenderer.setUpAndDown(false);
	}
	
	private List<TableCodage> tableCodages = new ArrayList<TableCodage>();
	...
	private List<CodeCommon> selectedCodes = new ArrayList<CodeCommon>();
	private List<CodeCommon> transCodes = new ArrayList<CodeCommon>();
	
	private List<CodeCommon> selTransCodes = new ArrayList<CodeCommon>();
	private List<CodeCommon> selSelectedCodes = new ArrayList<CodeCommon>();
	private List<CodeCommon> res = new ArrayList<CodeCommon>();
	...
	public void setSelectedTableCodage(TableCodage stCodage) {
	...
	public void setCodeOrLibelle(String c) {
	 ...
	 * @param cCode
	 */
	public void setCurrCode(CodeCommon cCode) {
		if (!isDiagnostic) {
			this.currCode = cCode;
			if (cCode != null) {
			...
			if (currCode != null) { 
				if (!selectedCodes.contains(currCode)) {
					...
					selectedCodes
							.set(selectedCodes.indexOf(currCode), currCode);
				}
			}
		} else if (cCode != null) { // renvoie directement le code cliqué.
			if (Path.getComponent(fromComponentPath) != null) {
				String eventName = "onGetCodeFromAssist";

				Events.postEvent(
					new Event(eventName, 
						Path.getComponent(fromComponentPath), 
						cCode));
			}
	...
	public void setCurrDossier(CodeDossier cDos) {
		if (cDos != null) {
			this.currCode = null;
		}
	...
	public void setCurrTable(TableCodage cTable) {
...
//	}
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		tableCodages = ManagerLocator.getTableCodageManager()
		...
		if (self.getParent() != null && self.getParent().getParent() != null) { 
			if (self.getParent().getParent() instanceof Window) { // is modale
				selectedTableCodage = ManagerLocator.getTableCodageManager()
											.findByNomManager("ADICAP").get(0);
				isModal = true;
				mainBorder.setHeight(
					(getMainWindow().getWindowAvailableHeight() - 37) + "px");
				mainBorder.setWidth(
					(getMainWindow().getWindowAvailableWidth() - 20) + "px");
			
				//selectedBanque = (Banque) getMainWindow().getSelectedBanque();
			} else {
				selectedTableCodage = ManagerLocator.getTableCodageManager()
										.findByNomManager("UTILISATEUR").get(0);
				isModal = false;
				mainBorder.setHeight(
					(getMainWindow().getWindowAvailableHeight() - 150) + "px");
				mainBorder.setWidth(
					(getMainWindow().getWindowAvailableWidth() - 150) + "px");
		...
		selCodesRows.addEventListener("onCheckObject", new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
				...
				// active ou non le bouton d'ajout selection
				addSelectedToTranscodeButton
					.setDisabled(!isInEdition || currDossier != null
										|| selSelectedCodes.size() == 0);
				
				addSelectedToBookmarksButton
					.setDisabled(isInEdition 
							|| selSelectedCodes.size() == 0
							|| (!sessionScope.containsKey("Admin")
								|| !((Boolean) sessionScope.get("Admin"))));
				
				returnSelCodesButton
					.setDisabled(!isModal || isInEdition 
										|| selSelectedCodes.size() == 0);
			}
		});
		...
		transCodesRows.addEventListener("onClickDeleteCode", 
		...
		transCodesRows.addEventListener("onCheckObject", 
											new EventListener<Event>() {
			public void onEvent(Event event) throws Exception {
				onCheckCode((ForwardEvent) event, selTransCodes);	
				// active ou non le bouton d'ajout selection
				addTranscodeToSelectedButton
								.setDisabled(selTransCodes.size() == 0);
			}
		});
	 ...
	 */
	public void init(String path, boolean isOrg, boolean isMor, 
										boolean isDiag, boolean toC) {
		...
		if (isOrgane) {
			setSelectedTableCodage(ManagerLocator.getTableCodageManager()
										.findByNomManager("ADICAP").get(0));
			
			browseClassification();
		} else if (toCims) { 
			setSelectedTableCodage(ManagerLocator.getTableCodageManager()
					.findByNomManager("CIM_MASTER").get(0));
			browseClassification();
		} else { // affiche favoris
			setSelectedTableCodage(ManagerLocator.getTableCodageManager()
										.findByNomManager("FAVORIS").get(0));
			browseClassification();
		}
		disableTables();
	}
	...
	private void disableTables() {
		codifBox.getItemAtIndex(tableCodages
			.indexOf(ManagerLocator.getTableCodageManager()
				.findByNomManager("CIMO_MORPHO").get(0)))
	...
	public String getSelectedCodesLabel() {
		return Labels.getLabel("code.selections") + "("
			+ selectedCodes.size() + ")";
	}
	...
	public String getTransCodesLabel() {
		return ObjectTypesFormatters.getLabel("code.recherche.transcodage",
				new String[]{String.valueOf(transCodes.size())});
	}
	 ...
	 */
	private void drawCodeResultsPanel(List<CodeCommon> codes, 
				Component parentDiv, String label, boolean transcode) {
		// arguments
		Map<Object, Object> args = new HashMap<Object, Object>();
		args.put("title", ObjectTypesFormatters
			.getLabel(label, 
						new String[]{String.valueOf(codes.size())}));
		if (!codes.isEmpty()) {
			SimpleListModel<CodeCommon> model = 
								new SimpleListModel<CodeCommon>(codes); 
			args.put("codes", model);
			CodeItemRenderer renderer = new CodeItemRenderer();
		...
		Executions
			.createComponents("/zuls/code/CodeResultsPanel.zul",
											parentDiv, args);
	}
	...
	private List<CodeCommon> transcode() {
		...
		if (isModal) { // ne transcode que pour les codifs associées à la banque
			return ManagerLocator.getTableCodageManager()
							.transcodeManager(currCode, getTablesForBanques(), 
				SessionUtils.getSelectedBanques(applicationScope));
		} else { // administration -> transcode sur toutes les tables 
			return ManagerLocator.getTableCodageManager()
				.transcodeManager(currCode, ManagerLocator
							.getTableCodageManager().findAllObjectsManager(),
	...
	public void onSelect$codifBox() {
		...
		selectedTableCodage = (TableCodage) 
							codifBox.getSelectedItem().getValue();
		...
		if (!"CIMO_MORPHO".equals(selectedTableCodage.getNom())) {
			...
			codeOrLibelle = "%";
			CodeUtils.findCodes(codeOrLibelle, isOrgane, isMorpho, res, 
					selectedTableCodage, exactMatch, 
					SessionUtils.getSelectedBanques(sessionScope));
			drawCodeResultsPanel(res, resHBox, 
											"code.recherche.resultats", false);
		}
	}
	...
	private void browseClassification() {
		...
		Executions
			.createComponents("/zuls/code/CodeBrowserComponent.zul",
											resHBox, null);
//		
...
//						.setIsInModale(isModal);
	 ...
	 * @param event
	 */
	public void onSelectCode(Event event) {
		Listbox target = (Listbox) 
						((ForwardEvent) event).getOrigin().getTarget();
	...
	private void disableButtons() {
		...
		addTranscodeToSelectedButton
						.setDisabled(isInEdition || currCode == null
										|| selTransCodes.size() == 0);
		returnSelCodesButton.setDisabled(!isModal || isInEdition 
									|| selSelectedCodes.size() == 0);
		
		if (("UTILISATEUR".equals(selectedTableCodage.getNom())
				|| "FAVORIS".equals(selectedTableCodage.getNom()))
				&& sessionScope.containsKey("Admin") 
				&& (Boolean) sessionScope.get("Admin")) {
			addSelectedToTranscodeButton
				.setDisabled(!isInEdition || currDossier != null
								|| selSelectedCodes.size() == 0);
			deleteButton.setDisabled(isInEdition 
					|| (currCode == null && currDossier == null));			
			if ("UTILISATEUR".equals(selectedTableCodage.getNom())) {
				newDossierButton.setDisabled(isInEdition || !isBrowsing 
														|| currCode != null);
				newCodeButton.setDisabled(isInEdition || !isBrowsing);
				editButton.setDisabled(isInEdition 
						|| (currCode == null && currDossier == null));	
			} else {
				newDossierButton.setDisabled(isInEdition || !isBrowsing
														|| currCode != null);
				...
				addSelectedToBookmarksButton
					.setDisabled(isInEdition || selSelectedCodes.size() == 0);
		...
		addSelectedToTranscodeButton
			.setVisible("UTILISATEUR".equals(selectedTableCodage.getNom()));
		addSelectedToBookmarksButton
			.setVisible("FAVORIS".equals(selectedTableCodage.getNom()));
	}
	...
	public void onClick$transcodeButton() {
		if (currCode != null && currTable != null) {
			...
			drawCodeResultsPanel(transcode(), resHBox, 
								"code.recherche.transcodage", true);
		}
	}
	...
	public void onClick$newDossierButton() {
		...
		currDossier
			.setCodeSelect("FAVORIS".equals(selectedTableCodage.getNom()));
	...
	public void onClick$validate() {
		...
		try {
			if (isCreation) {
				...
				ManagerLocator.getCodeUtilisateurManager()
					.createOrUpdateManager((CodeUtilisateur) currCode, null, 
						SessionUtils.getSelectedBanques(sessionScope).get(0), 
																utilisateur,
																null, 
									new LinkedHashSet<CodeCommon>(transCodes), 
														"creation");
			...
			if (isBrowsing) {
				...
				SimpleListModel<CodeCommon> model = 
									new SimpleListModel<CodeCommon>(res);
				((Listbox) 
					resHBox.getFirstChild()
									.getFellow("resBox")).setModel(model);
				((Listbox) 
					resHBox.getFirstChild().getFellow("resBox")).invalidate();
	...
	public void onClick$validateDos() {	
		...
		if (currDossier != null 
							&& currDossier.getDescription().equals("")) {
		...
		try {
			if (isCreation) {
				...
				ManagerLocator.getCodeDossierManager()
					.createOrUpdateManager(currDossier, null, 
						SessionUtils.getSelectedBanques(sessionScope).get(0), 
				...
				ManagerLocator.getCodeDossierManager()
						.createOrUpdateManager(currDossier, null, null, null, 
																"modification");
				if (isBrowsing) {
	...
	public void onClick$cancel() {
		...
		if (currCode != null
				&& !currCode.equals(new CodeUtilisateur())) {
			this.transCodes = transcode();
		}
			...
			if ("UTILISATEUR".equals(selectedTableCodage.getNom())) {
				...
				if (codeToDelete != null) {
					...
					if (codeToDelete.getCodeSelect() != null) {
						ManagerLocator.getCodeSelectManager()
							.removeObjectManager(codeToDelete.getCodeSelect());
					} 						
					selectedCodes.remove(codeToDelete);
				} else if (dossierToDelete != null) {
					ManagerLocator.getCodeDossierManager()
	...
	public void onClick$addSelectedToBookmarksButton() {
		...
		List<CodeCommon> bookmarked = 
			ManagerLocator.getCodeSelectManager()
				.findCodesFromSelectByUtilisateurAndBanqueManager(utilisateur, 
						SessionUtils.getSelectedBanques(sessionScope).get(0));
		
		Iterator<CodeCommon> it = selSelectedCodes.iterator();
		CodeCommon next;
		CodeSelect sel;
		List<CodeCommon> alreadyBookmarked = new ArrayList<CodeCommon>();
		while (it.hasNext()) {
			next = it.next();
			if (!bookmarked.contains(next)) {
				sel = new CodeSelect();
				sel.setCodeId(next.getCodeId());
				sel.setTableCodage(ManagerLocator.getCommonUtilsManager()
								.getTableCodageFromCodeCommonManager(next));
				
				ManagerLocator.getCodeSelectManager()
					.createOrUpdateManager(sel, dosParent, 
						SessionUtils.getSelectedBanques(sessionScope).get(0), 
													utilisateur, "creation");
			} else {
		...
		if (!alreadyBookmarked.isEmpty()) {
			StringBuilder listAlreadyBld = new StringBuilder();
			...
			Messagebox.show(ObjectTypesFormatters
					.getLabel("message.bookmark.alreadyRecorded", 
						new String[]{listAlreadyBld.toString()}),
				Labels.getLabel("message.bookmark.alreadyWarning"), 
...
//		}
		...
		if (Path.getComponent(fromComponentPath) != null) {
			...
			String eventName = "onGetCodesFromAssist";
	 ...
	 */
	public void switchLabelsToCodeOrDossier(boolean toCode) {
		if (toCode) {
	 ...
	 * @param code
	 */
	private void upCode(Object code) {
		int codeIndex = selectedCodes.indexOf(code);
		CodeCommon supObjectInList = null;
		if (codeIndex - 1 > -1) {
			supObjectInList = (CodeCommon) selectedCodes.get(codeIndex - 1);
	 ...
	 * @param code
	 */
	private void downCode(Object code) {
		int codeIndex = selectedCodes.indexOf(code);
	 ...
	 * selSelectedCodes)
	 */
	private void onCheckCode(ForwardEvent event, List<CodeCommon> selected) {
		// on récupère la checkbox associé à l'event
		Checkbox box = (Checkbox) event.getOrigin().getTarget();
		CodeCommon code = (CodeCommon) event.getData();
