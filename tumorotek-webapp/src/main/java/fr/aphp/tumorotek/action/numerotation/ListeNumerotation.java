...
public class ListeNumerotation extends AbstractListeController2 {

	private static final long serialVersionUID = 6575020319511994454L;
	
	private List<Numerotation> listObjects = new ArrayList<Numerotation>();
	private NumerotationRowRenderer numerotationRenderer = 
		new NumerotationRowRenderer();
	...
	@Override
	public List<Numerotation> getListObjects() {
	...
	public void addToListObjects(TKdataObject obj, Integer pos) {
		if (pos != null) { 
			getListObjects().add(pos.intValue(), (Numerotation) obj);
		} else {
			getListObjects().add((Numerotation) obj);
		}
	}
	
	@Override
	public void removeObjectFromList(TKdataObject obj) {
		getListObjects().remove((Numerotation) obj);
	}
	
	@Override
	public void initObjectsBox() {
		Utilisateur user = SessionUtils.getLoggedUser(sessionScope);
		List<Banque> banques = ManagerLocator.getBanqueManager()
			.findByUtilisateurIsAdminManager(user, 
							SessionUtils.getPlateforme(sessionScope));
		List<Numerotation> nums = ManagerLocator.getNumerotationManager()
	...
	public void onClick$viewObject(Event event) {
		...
		Numerotation edit = ((Numerotation) getCurrentObject()).clone();
	...
	public NumerotationRowRenderer getNumerotationRenderer() {
		return numerotationRenderer;
	}

	public void setNumerotationRenderer(NumerotationRowRenderer nRenderer) {
		this.numerotationRenderer = nRenderer;
	}
