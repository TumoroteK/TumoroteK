...
public class PatientRowRenderer extends TKSelectObjectRenderer {
	...
	public PatientRowRenderer(boolean select) {
	...
	public void render(Row row, Object data, int index) {
		...
		Patient pat = (Patient) data;
		patient = pat;
		
		
		if (anonyme) {
			Label nomLabel = createAnonymeLink();
			...
			Label nomLabel = new Label(pat.getNom());
			nomLabel.addForward(null, nomLabel.getParent(), 
													"onClickObject", pat);
	    ...
	    if (anonyme) {
	    	...
	    	new Label(ObjectTypesFormatters
			 	.dateRenderer2(pat.getDateNaissance())).setParent(row);
//	    	String dateN = null;
...
//			new Label(dateN).setParent(row);
	   ...
	   ObjectTypesFormatters.drawCodesExpLabel(ManagerLocator
			...
			.findFirstCodesOrgByPatientManager(pat), row, null, true);
	 ...
	 * @param row Parent
	 */
	private void drawMaladieLabel(Patient pat, Row row) {
		
		List<Maladie> mals = ManagerLocator
					.getMaladieManager().findByPatientNoSystemManager(pat);
		...
		if (!maladies.isEmpty()) {
			Label mal1Label = new Label(maladies.get(0).getLibelle());
			...
			if (maladies.size() > 1) {
				...
				Iterator<Maladie> it = maladies.iterator();
				Maladie next;
				String label = null;
	...
	public String getNbPrelevements() {
		return String.valueOf(PatientUtils
				.getNbPrelsForPatientAndUser(patient, banques));
	}
}
