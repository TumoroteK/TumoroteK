...
public class FicheAddCritere extends GenericForwardComposer<Component> {
	...
	public void init(Component p, Banque b) {
		parent = p;
		banque = b;
		...
		this.entitesBox.setModel(new 
						SimpleListModel<EntiteDecorator>(entites));		
		this.champs = new ArrayList<ChampDecorator>();
		this.champsBox.setModel(new SimpleListModel<ChampDecorator>(champs));
		this.sousChamps = new ArrayList<ChampDecorator>();
		this.sousChampsBox.setModel(new 
						SimpleListModel<ChampDecorator>(sousChamps));		
		this.operateurs = new ArrayList<String>();
		this.operateursBox.setModel(new SimpleListModel<String>(operateurs));
	...
	public void onSelect$entitesBox() {
		...
		champs = new ArrayList<ChampDecorator>();
		if (this.entitesBox.getSelectedIndex() > 0) {
			entite = entites.get(this.entitesBox
										.getSelectedIndex()).getEntite();
			// On cherche les ChampEntite de l'Entite.
			List<ChampEntite> champsEntite = ManagerLocator
				.getChampEntiteManager()
				.findByEntiteAndImportManager(entite, true);
			Iterator<ChampEntite> itCE = champsEntite.iterator();
			while (itCE.hasNext()) {
				ChampEntite ce = itCE.next();
			...
			List<ChampAnnotation> champsAnnotations = 
				new ArrayList<ChampAnnotation>();
			for (int i = 0; i < tas.size(); i++) {
				champsAnnotations.addAll(ManagerLocator
						.getChampAnnotationManager()
						.findByTableManager(tas.get(i)));
			}
			for (int i = 0; i < champsAnnotations.size(); i++) {
				champs.add(new ChampDecorator(new Champ(
						champsAnnotations.get(i))));
			}
		} else {
	...
	public void onSelect$champsBox() {
		...
		if (champsBox.getSelectedIndex() > 0) {
			...
			if (champ.getChampAnnotation() != null) {
				if (champ.getChampAnnotation().getNom()
												.matches("^[a-zA-Z]+Id$")) {
					id = true;
				}
			...
			if (id) {
				...
				sousChamps = new ArrayList<ChampDecorator>();
				sousChamps.add(null);
				sousChamp = null;
				
				/** On cherche l'entité correspondant au Champ */
				String nomChampEntite = champ.getChampEntite().getNom();
				...
				if (champ.getChampEntite().getEntite()
					...
					if (champ.getChampEntite().getNom().equals("OperateurId")) {
						nomEntite = "Collaborateur";
										...
										.getNom().equals("PreleveurId")) {
						nomEntite = "Collaborateur";
					} else if (champ.getChampEntite()
									.getNom().equals("ServicePreleveurId")) {
						nomEntite = "Service";
					} else if (champ.getChampEntite()
										.getNom().equals("QuantiteUniteId")) {
						nomEntite = "Unite";
					} 
					...
					if (champ.getChampEntite()
										.getNom().equals("VolumeUniteId")) {
						nomEntite = "Unite";
					} else if (champ.getChampEntite()
										.getNom().equals("QuantiteUniteId")) {
						nomEntite = "Unite";
					} else if (champ.getChampEntite()
											.getNom().equals("ConcUniteId")) {
						nomEntite = "Unite";
					}
				} else if (champ.getChampEntite().getEntite()
												.getNom().equals("Cession")) {
					if (champ.getChampEntite().getNom().equals("DemandeurId")) {
						nomEntite = "Collaborateur";
					} else if (champ.getChampEntite()
										.getNom().equals("DestinataireId")) {
						nomEntite = "Collaborateur";
					} else if (champ.getChampEntite()
										.getNom().equals("ServiceDestId")) {
						nomEntite = "Service";
					} else if (champ.getChampEntite()
											.getNom().equals("ExecutantId")) {
						nomEntite = "Collaborateur";
					}
				} else if (champ.getChampEntite().getEntite()
						.getNom().equals("Service")) {
					// hack Etablissement preleveur
					nomEntite = "Etablissement";
				}
				if (nomEntite == null) {
					nomEntite = nomChampEntite.substring(0, nomChampEntite
							.length() - 2);
				}
				List<Entite> ents = ManagerLocator.getEntiteManager()
						.findByNomManager(nomEntite);
				Entite entite2 = null;
				if (ents.size() > 0) {
					entite2 = ents.get(0);
				}
				...
				List<ChampEntite> champsEntite = ManagerLocator
						.getChampEntiteManager().findByEntiteManager(entite2);
				Iterator<ChampEntite> itCE = champsEntite.iterator();
				while (itCE.hasNext()) {
					ChampEntite ce = itCE.next();
					if (ce != null) {
						if (!ce.getNom().matches("^[a-zA-Z]+Id$")) {
							Champ sschp = new Champ(ce);
				...
				if (dataType.getType() != null) {
					if (dataType.getType().equals("alphanum")
							|| dataType.getType().equals("thesaurus")
							|| dataType.getType().equals("boolean")
							|| dataType.getType().equals("texte")
					...
					} else if (dataType.getType().matches("date.*")
							|| dataType.getType().equals("num")) {
						operateurs.add("=");
						operateurs.add("!=");
						operateurs.add("<");
						operateurs.add("<=");
					...
					if (champ.getChampEntite() != null && 
							!champ.getChampEntite().getNom().equals("AgeAuPrelevement")) {
	...
	public void onSelect$sousChampsBox() {
		...
		if (sousChampsBox.getSelectedIndex() > 0) {
			sousChamp = sousChamps.get(sousChampsBox
										.getSelectedIndex()).getChamp();
			...
			if (dataType.getType() != null) {
				if (dataType.getType().equals("alphanum")
						...
						|| dataType.getType().equals("texte")
						|| dataType.getType().equals("thesaurusM")
						|| dataType.getType().equals("hyperlien")) {
					operateurs.add("=");
					operateurs.add("!=");
					if (dataType.getType().equals("alphanum")
						|| dataType.getType().equals("thesaurus")
	...
	public void onClick$addCritere() {
		...
		if (sousChamp != null) {
			cd = new CritereDecorator(new Critere(sousChamp, 
											operateur, value));
		} else {
			cd = new CritereDecorator(new Critere(champ, 
											operateur, value));
		}
	...
	public void setEntites(List<EntiteDecorator> e) {
		this.entites = e;
	}
	...
	public void setSousChamps(List<ChampDecorator> s) {
		this.sousChamps = s;
	}
	...
	public void setChamps(List<ChampDecorator> c) {
		this.champs = c;
	}
	...
	public void setOperateurs(List<String> o) {
		this.operateurs = o;
	}
	...
	public void setSousChamp(Champ s) {
		this.sousChamp = s;
	}
	...
	public void setOperateur(String o) {
		this.operateur = o;
	}
	 ...
	 * @since 2.0.10
	 */
	private void addCustomChampToPrelevement(List<ChampDecorator> champs) {
		 if (entite.getNom().equals("Prelevement")) {
			// Etablissement preleveur
			champs.add(6, new ChampDecorator(new Champ(ManagerLocator
					.getChampEntiteManager().findByIdManager(193))));
			
			// Age au prélèvement
			champs.add(8, new ChampDecorator(new Champ(ManagerLocator
					.getChampEntiteManager().findByIdManager(254))));
		 } else if (entite.getNom().equals("Echantillon")) {
			champs.add(5, new ChampDecorator(new Champ(ManagerLocator
					.getChampEntiteManager()
					.findByIdManager(265))));
		 } else if (entite.getNom().equals("ProdDerive")) {
			champs.add(10, new ChampDecorator(new Champ(ManagerLocator
					.getChampEntiteManager()
					.findByIdManager(266))));
		}
	}
