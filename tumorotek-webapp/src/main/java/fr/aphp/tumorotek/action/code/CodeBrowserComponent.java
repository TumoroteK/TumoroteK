...
public class CodeBrowserComponent extends GenericForwardComposer<Component> {
	...
	private Log log = LogFactory.getLog(CodeBrowserComponent.class);
	...
	private CodeTreeitemRenderer renderer = new CodeTreeitemRenderer();

	public void setCodification(TableCodage codif) {
		this.codification = codif;
	}

	public void setIsOrgane(Boolean isT) {
		this.isOrgane = isT;
	}
	
	public void setIsMorpho(Boolean isT) {
		this.isMorpho = isT;
	}
	...
	public CodeNode getTreeSelectedValue() {
		if (mainTreeContext.getSelectedItem() != null) {
			return (CodeNode) mainTreeContext
	...
	public CodeNode getTreeSelectedValueAsParent() {
		if (mainTreeContext.getSelectedItem() != null) {
			...
			CodeNode c = (CodeNode) mainTreeContext
								.getSelectedItem().getValue();
			if (!c.isRoot()) {
				return c;
			}
	...
	public void populateCodeTreeModel() {
		...
		List<TumoTreeNode> children = new ArrayList<TumoTreeNode>();
		
		List< ? extends CodeCommon> codes = new ArrayList<CodeCommon>();
		...
		if ("ADICAP".equals(codification.getNom())) { 
			List<AdicapGroupe> dicos = 
				ManagerLocator.getAdicapManager()
											.findDictionnairesManager();
			if (isMorpho) { // D4-D5-D6-D7 UNIQUEMENT		
				for (int i = 3; i < 7; i++) {
					CodeNode node = 
							new CodeNode(dicos.get(i), codification);
					children.add(node);
				}
			} else if (isOrgane) { // D3 uniquement
				codes = ManagerLocator.getAdicapManager()
								.findByAdicapGroupeManager(dicos.get(2));
				for (int i = 0; i < codes.size(); i++) {
					CodeNode node = 
							new CodeNode(codes.get(i), codification, true);
					children.add(node);
				}
			} else {
			...
			codes = ManagerLocator
				.getCimMasterManager().findByCimParentManager(null);
			for (int i = 0; i < codes.size(); i++) {
				CodeNode node = 
						new CodeNode(codes.get(i), codification, null);
				children.add(node);
			}
			root.setChildren((ArrayList<TumoTreeNode>) children);
		} else if ("UTILISATEUR".equals(codification.getNom())) {
			CodeNode root2 = new CodeNode();
			CodeDossier rootDos = new CodeDossier();
			rootDos.setNom("/");
			root2.setDossier(rootDos);
			root2.setRoot(true);
			ArrayList<TumoTreeNode> rootChildren = 
												new ArrayList<TumoTreeNode>();
			rootChildren.add(root2);
			root.setChildren(rootChildren);
			
			List<CodeDossier> dos = new ArrayList<CodeDossier>();
			
			if (!SessionUtils
						.getSelectedBanques(sessionScope).isEmpty()) {
				dos = ManagerLocator.getCodeDossierManager()
					.findByRootDossierBanqueManager(SessionUtils
							.getSelectedBanques(sessionScope).get(0), false);
		
				codes = ManagerLocator.getCodeUtilisateurManager()
				.findByRootDossierManager(SessionUtils
						.getSelectedBanques(sessionScope).get(0));
			}
			
			for (int i = 0; i < dos.size(); i++) {
				CodeNode node = new CodeNode(dos.get(i), codification);
				children.add(node);
			}
			for (int i = 0; i < codes.size(); i++) {
				CodeNode node = 
			...
			dos = ManagerLocator.getCodeDossierManager()
					.findByRootDossierBanqueManager(SessionUtils
					.getSelectedBanques(sessionScope).get(0), true);
			...
			codes = ManagerLocator.getCodeSelectManager()
				.findByRootDossierAndBanqueManager(
		...
		if ("UTILISATEUR".equals(codification.getNom()) 
				|| "FAVORIS".equals(codification.getNom())) {
			int[] apath = {0};
	 ...
	 * @param event
	 */
	public void onSelect$mainTreeContext(Event event) {
		CodeNode node = getTreeSelectedValue();
	...
	private CodesController getCodeController() {
		return ((CodesController) self.getParent().getParent()
				.getParent().getParent()
				.getAttributeOrFellow("winCodes$composer", true));
	}
	
	private AnnotateDataBinder getBinder() {		
		return (AnnotateDataBinder) getCodeController().getBinder();
	}	
	
	public void reloadTree(CodeNode selectedNode, boolean isParent) {
