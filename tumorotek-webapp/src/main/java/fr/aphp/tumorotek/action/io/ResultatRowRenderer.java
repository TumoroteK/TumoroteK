...
public class ResultatRowRenderer implements RowRenderer<ResultatRow> {
	...
	@Override
	public void render(Row row, ResultatRow data, int index) throws Exception {
		
		ResultatRow resultat = data;
		
		for (int i = 0; i < resultat.getValuesToShow().size(); i++) {
			Label label = null;
			if (resultat.getValuesToShow().get(i) != null) {
				if (!(resultat.getValuesToShow()
						.get(i) instanceof Date)
						&& !(resultat.getValuesToShow()
								.get(i) instanceof Calendar)) {
					label = new Label(resultat.getValuesToShow()
						.get(i).toString());
					decorateLabel(label, resultats.get(i), 
						resultat.getObjects().get(i));
				} else {
					label = new Label(ObjectTypesFormatters
							.dateRenderer2(resultat.getValuesToShow()
							.get(i)));
				}
			} else {
				label = new Label("-");
	...
	public void decorateLabel(Label label, Resultat res, Object obj) {
		...
		if (res != null && res.getChamp() != null 
				...
				if (entite.getNom().equals("Patient")
					...
					if (obj instanceof Prelevement) {
						...
						label.addForward(null, label.getParent(),
	...
	public void setResultats(List<Resultat> r) {
		this.resultats = r;
	}
