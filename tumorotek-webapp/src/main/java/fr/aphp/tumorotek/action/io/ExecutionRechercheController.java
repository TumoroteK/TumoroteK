...
public class ExecutionRechercheController extends AbstractObjectTabController {
	...
	private Log log = LogFactory.getLog(ExecutionRechercheController.class);
	...
	public void initAssociations() {
		// on va afficher toutes les recherches pour les banques
		// accessibles par l'utilisateur
		List<Banque> tmp = getMainWindow().getBanques();
		...
		recherches = ManagerLocator.getRechercheManager()
				.findByBanqueInLIstManager(banques);
		...
		if (selectedRecherche != null) {
			...
			try {
				if (isAnonyme()) {
					...
					// affichages
					Iterator<Resultat> itRes = 
							ManagerLocator.getResultatManager()
								.findByAffichageManager(selectedRecherche
							.getAffichage()).iterator();
					while (itRes.hasNext()) {
						Resultat res = itRes.next();
						if (res.getChamp() != null 
							&& res.getChamp().getChampEntite() != null
							&& res.getChamp().getChampEntite()
								.getEntite().getNom().equals("Patient")) {
							if (res.getChamp().getChampEntite().getNom()
												.equals("Nip") 
					...
					if (!reqNominative) {
						// criteres
						Iterator<Critere> itCrits = 
							ManagerLocator.getGroupementManager()
							.findCriteresManager(selectedRecherche
							.getRequete().getGroupementRacine()).iterator();
						while (itCrits.hasNext()) {
							Critere crit = itCrits.next();
							if (crit.getChamp() != null 
								&& crit.getChamp().getChampEntite() != null
								&& crit.getChamp().getChampEntite()
									.getEntite().getNom().equals("Patient")) {
								if (crit.getChamp().getChampEntite().getNom()
										.equals("Nip") 
								|| crit.getChamp().getChampEntite().getNom()
										.equals("Nom") 
								|| crit.getChamp().getChampEntite().getNom()
										.equals("Prenom") 
								|| crit.getChamp().getChampEntite().getNom()
					...
					if (reqNominative) {
						...
						.getLabel("execution.recherche.erreur.nominative"));
					}
				}
	...
	public void onClick$exportItem() {
		if (selectedRecherche != null) {
			...
			// REFLEXIVITE
			ExportRechercheComplexe exp = 
				new ExportRechercheComplexe();
			exp.init(desktop, 
					objets, 
					isAnonyme(), 
					callProgressBar(), affichage, 
					selectedRecherche.getIntitule(), 
	...
	public void onClick$exportItemINCa() {
		onLaterExportCatalogue((List<Integer>) getObjectsFromList("Echantillon", true), 
				ConfigManager.INCA_EXPORT, 3, false, null);
	}

	@SuppressWarnings("unchecked")
	public void onClick$exportItemTVGSO() {
		onLaterExportCatalogue((List<Integer>) getObjectsFromList("Echantillon", true), 
				ConfigManager.TVGSO_EXPORT, 3, false, null);
	}
	
	@SuppressWarnings("unchecked")
	public void onClick$exportItemTVGSOcsv() {
		onLaterExportCatalogue((List<Integer>) getObjectsFromList("Echantillon", true), 
				ConfigManager.TVGSO_EXPORT, 3, true, null);
	}

	@SuppressWarnings("unchecked")
	public void onClick$exportItemBIOCAP() {
		onLaterExportCatalogue((List<Integer>) getObjectsFromList("Echantillon", true), 
				ConfigManager.BIOCAP_EXPORT, 3, false, null);
	}
	...
	public void openSetCriteresValuesWindow() {
		if (!isBlockModal()) {
			...
			ua = (HtmlMacroComponent) page.getComponentDefinition(
					"setCriteresValuesModale", false).newInstance(page, null);
			ua.setParent(win);
			ua.setId("setCriteresValuesModaleComponent");
			ua.applyProperties();
			ua.afterCompose();

			List<Banque> bks = SessionUtils.getSelectedBanques(sessionScope);
			((FicheSetCriteresValues) ua.getFellow(
					"fwinSetCriteresvaluesModale").getAttributeOrFellow(
					"fwinSetCriteresvaluesModale$composer", true)).init(bks,
					self, selectedRecherche);
			ua.setVisible(false);
...
//	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void onGetSearchResults(Event e) {
		// cleanUp();

		affichage = selectedRecherche.getAffichage().clone();
		affichage.setResultats(ManagerLocator.getResultatManager()
		...
		List<Resultat> resultats = affichage.getResultats();
		// On trie les résultats par ordre de position
		if (resultats.size() > 1) {
			for (int i = 1; i < resultats.size(); i++) {
				for (int j = i - 1; j >= 0; j--) {
					if (resultats.get(j + 1).getPosition() < resultats.get(
							j).getPosition()) {
		...
		Iterator<Resultat> itCol = resultats.iterator();
		while (itCol.hasNext()) {
			Resultat res = itCol.next();
			Column colObjet = new Column(res.getNomColonne());
	...
	public List< ? extends Object> getObjectsFromList(String entiteObjects, boolean getId) {
		...
		for (int i = 0; i < objets.size(); i++) {
			matriceObjets
			.add(RechercheUtils
			.getListeObjetsCorrespondants(objets.get(i), affichage, entiteObjects));
		}
...
//		return resultats;
	...
	public void onLaterShowResultsInList() {
		List<? extends Object> resultats = getObjectsFromList(entite, true);
	 ...
	 */
	public void onClickPatientNip(Event event) {
	 ...
	 */
	public void onClickPrelevementCode(Event event) {
	...
	public void onClickEchantillonCode(Event event) {
		...
		if (tabController != null) {
			tabController
					.switchToFicheStaticMode((Echantillon) event.getData());
		}
	}
	 ...
	 */
	public void onClickCessionNumero(Event event) {
...
//		setDroitsConsultation(drawConsultationLinks(entites));
		...
		showPrelevements.setDisabled(!getDroitsConsultation()
				.get("Prelevement"));
		showEchantillons.setDisabled(!getDroitsConsultation()
				.get("Echantillon"));
	...
	public void setRecherches(List<Recherche> r) {
		this.recherches = r;
	}
	...
	public void setSelectedRecherche(Recherche selectedR) {
	...
	private void refreshModel(int activePage){
        ...
        model = new GridResultsModel(activePage, _pageSize, 
        								objets, affichage);
	...
	public void onPaging$resPaging(ForwardEvent event){
