 ...
 * @version 2.0
 */
public class AffectationRowRenderer implements RowRenderer {
	
	public AffectationRowRenderer(Plateforme pf) {
		plateforme = pf;
	}
	...
	public void render(Row row, Object data, int index) throws Exception {
		AffectationDecorator aff = (AffectationDecorator) data;
		
		Listbox liImprimantes = new Listbox();
		Listbox liModeles = new Listbox();
		...
		Label userLabel = new Label(tmp);
		userLabel.setClass("formLabel");
		userLabel.setParent(row);
		
		// banque
		Label banqueLabel = new Label(aff.getBanque().getNom());
		...
		if (!aff.isEdit()) {
			...
			Label imprimanteLabel = new Label(tmp);
			imprimanteLabel.setParent(row);
		} else {
			// sinon, on récupère les imprimantes de la pf
			List<Imprimante> imps = ManagerLocator.getImprimanteManager()
				.findByPlateformeManager(plateforme);
			imps.add(0, null);
			// on extrait leurs noms
			List<String> impsString = new ArrayList<String>();
		...
		if (!aff.isEdit()) {
			...
			Label modeleLabel = new Label(tmp);
			modeleLabel.setParent(row);
		} else {
			// sinon, on récupère les modèles de la pf
			List<Modele> mods = ManagerLocator.getModeleManager()
				.findByPlateformeManager(plateforme);
			mods.add(0, null);
			List<String> modsString = new ArrayList<String>();
			...
			// on affiche une listbox
			ListModel l = new ListModelList(modsString);
		...
		if (!aff.isEdit()) {
			...
			if (canEdit) {
				...
				divImage.addForward("onClick", divEdit.getParent(), 
						"onClickEditAffectation", aff);
				divImage.setParent(divEdit);
			}
			divEdit.setParent(row);
		} else {
			// sinon on affiche le bouton de validation
			Div divValidate = new Div();
			...
			divImage.addForward("onClick", divValidate.getParent(), 
					"onClickValidateAffectation", datas);
		...
		if (!aff.isEdit()) {
			...
			divImage.addForward("onClick", divCancel.getParent(), 
					"onClickCancelAffectation", aff);
		...
		StringBuffer rowStyle = new StringBuffer();
		if (!aff.isFirst()) {
	...
	public void setCanEdit(boolean ce) {
		this.canEdit = ce;
	}
