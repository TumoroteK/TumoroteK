 ...
 *
 */
public class EditModelVM extends AbstractListGridVM {
	...
	// list existing model
	private List<SModele> existingSModels;
	private SModele selectedExistingModel;
	
	private ListModelList<Indicateur> indicateursModel;
	private List<Indicateur> selectedIndicateurs = new ArrayList<Indicateur>();
	...
	@Init
	public void initData(@ContextParam(ContextType.COMPONENT) Window win) {
		this.win = win;
		
		existingSModels = ManagerLocator.getSModeleManager()
				.findByPlateformeManager(SessionUtils.getCurrentPlateforme());
		existingSModels.add(0, null);
		
		indicateursModel = new ListModelList<Indicateur>(ManagerLocator
				.getIndicateurManager().findNullSubdivisionIndicateursManager());
		
		banquesModel = new ListModelList<Banque>(ManagerLocator
				.getBanqueManager().findByPlateformeAndArchiveManager(
						SessionUtils.getCurrentPlateforme(), null));
		
		banquesModel.setMultiple(true);

		subdivisions = ManagerLocator.getSubdivisionManager()
	...
	/********************* Commands *****************************************/
	@Command
	@NotifyChange({"selectedExistingModel",
			"indicateursModel",
			"inEdition"})
	public void onChangeSelectedExistingModel() {
		...
		if (selectedExistingModel != null) {
			selectedBanques.addAll(ManagerLocator.getSModeleManager()
					.getBanquesManager(selectedExistingModel));
			...
			selectedIndicateurs.addAll(ManagerLocator.getIndicateurManager()
					.findBySModeleManager(selectedExistingModel));
	...
	public void onDeleteModel() {
		setInEdition(false);
		ManagerLocator.getSModeleManager().removeObjectManager(
				selectedExistingModel);
		existingSModels.remove(selectedExistingModel);
	...
	public void onSaveModel() {
		...
		if (selectedIndicateurs.isEmpty()) {
			Clients.showNotification(
					Labels.getLabel("stats.smodel.warn.noindicateurs"), "warning", win,
					"middle_center", 2000);
		} else if (selectedBanques.isEmpty()) {
			Clients.showNotification(
					Labels.getLabel("stats.smodel.warn.nobanques"), "warning", win,
					"middle_center", 2000);
		} else  {
			...
			if (selectedExistingModel.getSmodeleId() != null) {
				...
				selectedExistingModel = 
					ManagerLocator.getSModeleManager().updateObjectManager(
						selectedExistingModel,
						selectedExistingModel.getPlateforme(),
						getGridIndicateurs(), selectedBanques);
				
				int pos = existingSModels.indexOf(selectedExistingModel);
				existingSModels.remove(selectedExistingModel);
				existingSModels.add(pos, selectedExistingModel);

//				Clients.showNotification(selectedExistingModel.getNom()
//						+ "Modèle modifié avec succés !!", "info", win,
//						"center", 2000);
			} else { // create
				ManagerLocator.getSModeleManager().createObjectManager(
						selectedExistingModel,
				...
				existingSModels.add(selectedExistingModel);
				Collections.sort(existingSModels);
...
//						"middle_center", 2000);
	...
	public void changeGridIndicateurOrder(@BindingParam("dropEvt") DropEvent e, 
			@BindingParam("indic") Indicateur indic) {
		Column dragged = (Column) e.getDragged();
		Column target = (Column) e.getTarget();
		if (!gridView.getColumns().insertBefore(dragged, target)) {
			gridView.getColumns().insertBefore(target, dragged);
		}
		List<Component> child = gridView.getColumns().getChildren();
		// rectificatif pos col Banque 
		int pos = child.indexOf(dragged) - 2;
	...
	private void updatePreviewGrid(Boolean orderedIndics) {
	...
	public void onSelectIndicateur(@BindingParam("item") Indicateur idc) {
		// getGridIndicateurs().clear();
		// getGridIndicateurs().addAll(indicateursModel.getSelection());
	...
	public void onSelectAllIndicateurs() {
		...
		if (!getGridIndicateurs().containsAll(indicateursModel)) {
			for (Indicateur indic : indicateursModel) {
	...
	public void onChangeSubdivision(@BindingParam("item") Subdivision subd) {
		selectedIndicateurs.clear();
	...
	private void resetIndicateurs() {
		...
		if (selectedSubdivision != null) {
			indicateursModel.addAll(ManagerLocator.getIndicateurManager()
	...
	@Command
	public void OnMouseOverIndicateurList(
			@BindingParam("target") Component target,
			@BindingParam("item") Indicateur idc) {
...
//	}
	...
	public void setSelectedBanques(List<Banque> s) {
	...
	public void setSelectedExistingModel(SModele selectedExistingModel) {
