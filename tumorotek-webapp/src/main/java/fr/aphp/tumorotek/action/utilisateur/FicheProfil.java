...
public class FicheProfil extends AbstractFicheCombineController {
	...
	private Hashtable<Checkbox, List<Checkbox>> rowsCheckBoxs = 
		new Hashtable<Checkbox, List<Checkbox>>();
	private Hashtable<Checkbox, List<Checkbox>> annonymesRowsCheckBoxs = 
		new Hashtable<Checkbox, List<Checkbox>>();
	
	// Associations.
	private java.util.Hashtable<String, Entite> entites = 
							new java.util.Hashtable<String, Entite>();
	private java.util.Hashtable<String, OperationType> operationTypes = 
						new java.util.Hashtable<String, OperationType>();
	private List<DroitObjet> droitsObjets = new ArrayList<DroitObjet>();
	...
	private ProfilUtilisateurRowRenderer profilUtilisateurRowRenderer = 
			new ProfilUtilisateurRowRenderer(true);
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjBoxsComponents(new Component[]{
				this.nomBox,
				this.anonymeBox,
				this.accesAdministrationBox,
				this.rowCocher,
				this.exportListBox,
				this.exportAttentionLabel,
				this.archiveBox
		});
		...
		this.droitLabelsComponents = new Component[]{
				this.consultationPatientLabel,
				this.creationPatientLabel,
				this.modificationPatientLabel,
				this.suppressionPatientLabel,
				this.consultationPrelevementLabel,
				this.creationPrelevementLabel,
				this.modificationPrelevementLabel,
				this.suppressionPrelevementLabel,
				this.consultationEchantillonLabel,
				this.creationEchantillonLabel,
				this.modificationEchantillonLabel,
				this.suppressionEchantillonLabel,
				this.consultationProdDeriveLabel,
				this.creationProdDeriveLabel,
				this.modificationProdDeriveLabel,
				this.suppressionProdDeriveLabel,
				this.consultationCessionLabel,
				this.creationCessionLabel,
				this.modificationCessionLabel,
				this.suppressionCessionLabel,
				this.consultationStockageLabel,
				this.creationStockageLabel,
				this.modificationStockageLabel,
				this.suppressionStockageLabel,
				this.consultationAnnotationLabel,
				this.creationAnnotationLabel,
				this.modificationAnnotationLabel,
				this.suppressionAnnotationLabel,
				this.consultationCollaborateurLabel,
				this.creationCollaborateurLabel,
				this.modificationCollaborateurLabel,
				this.suppressionCollaborateurLabel,
				this.consultationRequeteLabel,
				this.creationRequeteLabel,
				this.modificationRequeteLabel,
				this.suppressionRequeteLabel
		};
		
		this.droitBoxComponents = new Component[]{
				this.consultationPatientBox,
				this.creationPatientBox,
				this.modificationPatientBox,
				this.suppressionPatientBox,
				this.modifMultipliePatientBox,
				this.annotationPatientBox,
				this.consultationPrelevementBox,
				this.creationPrelevementBox,
				this.modificationPrelevementBox,
				this.suppressionPrelevementBox,
				this.modifMultipliePrelevementBox,
				this.annotationPrelevementBox,
				this.consultationEchantillonBox,
				this.creationEchantillonBox,
				this.modificationEchantillonBox,
				this.suppressionEchantillonBox,
				this.modifMultiplieEchantillonBox,
				this.annotationEchantillonBox,
				this.consultationProdDeriveBox,
				this.creationProdDeriveBox,
				this.modificationProdDeriveBox,
				this.suppressionProdDeriveBox,
				this.modifMultiplieProdDeriveBox,
				this.annotationProdDeriveBox,
				this.consultationCessionBox,
				this.creationCessionBox,
				this.modificationCessionBox,
				this.suppressionCessionBox,
				this.modifMultiplieCessionBox,
				this.annotationCessionBox,
				this.consultationStockageBox,
				this.creationStockageBox,
				this.modificationStockageBox,
				this.suppressionStockageBox,
				this.consultationCollaborateurBox,
				this.creationCollaborateurBox,
				this.modificationCollaborateurBox,
				this.suppressionCollaborateurBox,
				this.consultationRequeteBox,
				this.creationRequeteBox,
				this.modificationRequeteBox,
				this.suppressionRequeteBox
		};
	...
	public void setObject(TKdataObject obj) {
		...
		if (profil != null && profil.getProfilId() != null) {
			droitsObjets = ManagerLocator.getDroitObjetManager()
				.findByProfilManager(profil);
			
			// gestion de l'export
			if (this.profil.getAdmin() != null
					&& this.profil.getAdmin()) {
				selectedExport = Labels.getLabel(
						"profil.droit.export.nominatif");
			} else {
				if (this.profil.getProfilExport() != null) {
					if (this.profil.getProfilExport() == 0) {
						selectedExport = Labels.getLabel(
							"profil.droit.export.non");
					} else if (this.profil.getProfilExport() == 1) {
						selectedExport = Labels.getLabel(
							"profil.droit.export.anonyme");
					} else if (this.profil.getProfilExport() == 2) {
						selectedExport = Labels.getLabel(
							"profil.droit.export.nominatif");
					}
				}
			...
			selectedExport = Labels.getLabel("profil.droit.export.non");
		}
	...
	private void initAssociations() {
		...
		if (profil.getProfilId() != null) {
			...
			profilUtilisateurs.addAll(ManagerLocator
					.getProfilUtilisateurManager().findByProfilManager(profil, false));	
		}
	}
	...
	public void switchToStaticMode() {
		...
		if (this.profil != null && this.profil.getAdmin() != null
				&& this.profil.getAdmin()) {
	...
	public void switchToEditMode() {
		...
		if (!this.profil.getAnonyme()) {
			...
			initRowsCheckBoxs();
			List<Checkbox> checkBoxToEnable = new ArrayList<Checkbox>();
			for (int i = 0; i < droitBoxComponents.length; i++) {
				// on extrait l'operation gérée par le label : son nom
				// contenu dans le custom-attribute "operation"
				OperationType type = operationTypes.get(
						droitBoxComponents[i].getAttribute("operation"));
				Entite entite = entites.get(droitBoxComponents[i]
				                            .getAttribute("entite"));
				
				if (!entite.getNom().equals("Stockage")
						&& !entite.getNom().equals("Requete")
						&& !entite.getNom().equals("Collaborateur")) {
					
					if (type.getNom().equals("Consultation")) {
						Checkbox box = (Checkbox) (Checkbox) 
							droitBoxComponents[i];
						box.setDisabled(false);
						
						if (annonymesRowsCheckBoxs.containsKey(box)) {
							List<Checkbox> boxs = annonymesRowsCheckBoxs
								.get(box);
	...
	public void createNewObject() {
		...
		this.profil.setAccesAdministration(
				accesAdministrationBox.isChecked());
			
		// on extrait les droits cochés
		List<DroitObjet> newDroits = extractDroitObjets();
			
		// update de l'objet
		ManagerLocator.getProfilManager().createObjectManager(
				profil, 
				newDroits,
	...
	private ListeProfil getListeProfil() {
		return (ListeProfil) getObjectTabController().getListe();
	}
	...
	public boolean prepareDeleteObject() {
		boolean isUsed = ManagerLocator
			.getProfilManager().isUsedObjectManager(getObject());
		...
		if (isUsed) {
			setDeleteMessage(Labels
					.getLabel("profil.deletion.isUsed"));
		} 
		setDeletable(!isUsed);
		return false;
	}
	...
	public void removeObject(String comments) {		
		ManagerLocator.getProfilManager()
...
//	}
	...
	public void updateObject() {
		...
		// update de l'objet
		ManagerLocator.getProfilManager().updateObjectManager(
				profil, 
				newDroits,
				SessionUtils.getLoggedUser(sessionScope));
	 ...
	 * @throws Exception
	 */
	public void onSelect$exportListBox(Event event) throws Exception {
		selectedExport = exportListBox.getSelectedItem().getLabel();
	}
	...
	public void initEditableMode() {
		...
		if (this.profil.getAccesAdministration() != null) {
			accesAdministrationBox.setChecked(
					this.profil.getAccesAdministration());
		} else {
	...
	public void initHashTables() {
		entites = new java.util.Hashtable<String, Entite>();
		// entité Patient
		entites.put("Patient", ManagerLocator.getEntiteManager()
				.findByNomManager("Patient").get(0));
		// entité Prelevement
		entites.put("Prelevement", ManagerLocator.getEntiteManager()
				.findByNomManager("Prelevement").get(0));
		// entité Echantillon
		entites.put("Echantillon", ManagerLocator.getEntiteManager()
				.findByNomManager("Echantillon").get(0));
		// entité ProdDerive
		entites.put("ProdDerive", ManagerLocator.getEntiteManager()
				.findByNomManager("ProdDerive").get(0));
		// entité Cession
		entites.put("Cession", ManagerLocator.getEntiteManager()
				.findByNomManager("Cession").get(0));
		// entité Stockage
		entites.put("Stockage", ManagerLocator.getEntiteManager()
				.findByNomManager("Stockage").get(0));
		// entité Collaborateur
		entites.put("Collaborateur", ManagerLocator.getEntiteManager()
				.findByNomManager("Collaborateur").get(0));
		// entité Requete
		entites.put("Requete", ManagerLocator.getEntiteManager()
				.findByNomManager("Requete").get(0));
		
		operationTypes = new java.util.Hashtable<String, OperationType>();
		// operation Consultation
		operationTypes.put("Consultation", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("Consultation", true).get(0));
		// operation Creation
		operationTypes.put("Creation", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("Creation", true).get(0));
		// operation Modification
		operationTypes.put("Modification", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("Modification", true).get(0));
		// operation Archivage
		operationTypes.put("Archivage", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("Archivage", true).get(0));
		// operation ModifMultiple
		operationTypes.put("ModifMultiple", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("ModifMultiple", true).get(0));
		// operation Annotation
		operationTypes.put("Annotation", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("Annotation", true).get(0));
		// operation Import
		operationTypes.put("Import", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("Import", true).get(0));
		// operation Export
		operationTypes.put("Export", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("Export", true).get(0));
		// operation ExportAnonyme
		operationTypes.put("ExportAnonyme", ManagerLocator
				.getOperationTypeManager()
				.findByNomLikeManager("ExportAnonyme", true).get(0));
	}
	...
	public void initDroitsLabels() {
		...
		for (int i = 0; i < droitLabelsComponents.length; i++) {
			...
			if (!this.profil.equals(new Profil())) {
				...
				Entite entite = entites.get(droitLabelsComponents[i]
				               .getAttribute("entite"));
				// on extrait l'operation gérée par le label : son nom
				// contenu dans le custom-attribute "operation"
				OperationType type = operationTypes.get(
					droitLabelsComponents[i].getAttribute("operation"));
				
				// on construit un DroitObjet à partir de l'entité et
				// de l'operation que l'on vient d'extraire
				DroitObjet droit = new DroitObjet();
				...
				((Label) droitLabelsComponents[i])
					.setValue(ObjectTypesFormatters
					.booleanLitteralFormatter(droitsObjets.contains(droit)));
			} else {
				((Label) droitLabelsComponents[i])
						.setValue("");
			}
		...
		for (int i = 0; i < droitBoxComponents.length; i++) {
			...
			if (!this.profil.equals(new Profil())) {
				...
				if (this.profil.getAdmin() != null && this.profil.getAdmin()) {
					...
					// checkbox, sinonn non
					((Checkbox) droitBoxComponents[i])
						.setChecked(droitsObjets.contains(droit));
				}
			} else {
				((Checkbox) droitBoxComponents[i])
					.setChecked(false);
	...
	public List<DroitObjet> extractDroitObjets() {
		List<DroitObjet> newDroits = new ArrayList<DroitObjet>();
		selectedExport = exportListBox.getSelectedItem().getLabel();
		
		// pour toutes les checkboxs pour les droits
		for (int i = 0; i < droitBoxComponents.length; i++) {
			Checkbox check = (Checkbox) droitBoxComponents[i];
			
			if (check.isChecked()) {
				// on extrait l'entité gérée par le label : son nom
				// contenu dans le custom-attribute "entite"
				Entite entite = entites.get(check
				               .getAttribute("entite"));
				// on extrait l'operation gérée par le label : son nom
				// contenu dans le custom-attribute "operation"
				OperationType type = operationTypes.get(
						check.getAttribute("operation"));
		...
		// gestion de l'export
		if (selectedExport.equals(
				Labels.getLabel("profil.droit.export.nominatif"))
				|| selectedExport.equals(
					Labels.getLabel("profil.droit.export.anonyme"))) {
			if (selectedExport.equals(
					Labels.getLabel("profil.droit.export.nominatif"))) {
				this.profil.setProfilExport(2);
			} else {
				this.profil.setProfilExport(1);
			}
		} else {
			this.profil.setProfilExport(0);
		}
	...
	public void initRowsCheckBoxs() {
		...
		list = new ArrayList<Checkbox>();
		annonymesRowsCheckBoxs.put(consultationPatientBox, 
				new ArrayList<Checkbox>(list));
		
		list = new ArrayList<Checkbox>();
		list.add(modificationPrelevementBox);
		list.add(annotationPrelevementBox);
		annonymesRowsCheckBoxs.put(consultationPrelevementBox, 
				new ArrayList<Checkbox>(list));
		
		list = new ArrayList<Checkbox>();
		list.add(modificationEchantillonBox);
		list.add(annotationEchantillonBox);
		annonymesRowsCheckBoxs.put(consultationEchantillonBox, 
				new ArrayList<Checkbox>(list));
		
		list = new ArrayList<Checkbox>();
		list.add(modificationProdDeriveBox);
		list.add(annotationProdDeriveBox);
		annonymesRowsCheckBoxs.put(consultationProdDeriveBox, 
				new ArrayList<Checkbox>(list));
		
		list = new ArrayList<Checkbox>();
		list.add(modificationCessionBox);
		list.add(annotationCessionBox);
		annonymesRowsCheckBoxs.put(consultationCessionBox, 
				new ArrayList<Checkbox>(list));
	}
	
	public void onCheckItem(ForwardEvent e) {
		Event first = e.getOrigin();
		Checkbox box = (Checkbox) first.getTarget();
	...
	public void onCheck$anonymeBox() {
		...
		for (int i = 0; i < droitBoxComponents.length; i++) {
			...
			if (anonymeBox.isChecked()) {
				...
				if (!entite.getNom().equals("Stockage")
					...
					if (type.getNom().equals("Consultation")) {
						...
						if (annonymesRowsCheckBoxs.containsKey(box)) {
							...
							List<Checkbox> boxsBis = rowsCheckBoxs
								.get(box);
							for (int j = 0; j < boxsBis.size(); j++) {
								if (!checkBoxToEnable.contains(
										boxsBis.get(j))) {
				...
				if (type.getNom().equals("Consultation")) {
					Checkbox box = (Checkbox) (Checkbox) droitBoxComponents[i];
					box.setDisabled(false);
					
					if (rowsCheckBoxs.containsKey(box)) {
						List<Checkbox> boxs = rowsCheckBoxs.get(box);
	...
	public String getAnonymeFormated() {
		...
		if (this.profil != null) {
			return ObjectTypesFormatters
				.booleanLitteralFormatter(this.profil.getAnonyme());
		} else {
			return "";
		}
	}
...
//		}
	...
	public String getExportFormated() {
		...
		if (this.profil != null && this.profil.getProfilId() != null) {
			if (this.profil.getAdmin() != null
				...
				OperationType exportOp = operationTypes.get("Export");
				List<DroitObjet> list = ManagerLocator.getDroitObjetManager()
					.findByProfilOperationManager(profil, exportOp);
				
				return ObjectTypesFormatters
					.booleanLitteralFormatter(list.size() == 6);
			}
		} else {
			return "";
		}
	...
	public void setEntites(java.util.Hashtable<String, Entite> e) {
		this.entites = e;
	}
	...
	public void setOperationTypes(
			java.util.Hashtable<String, OperationType> oTypes) {
	...
	public void setDroitsObjets(List<DroitObjet> dObjets) {
	...
	public void setRowsCheckBoxs(Hashtable<Checkbox, List<Checkbox>> r) {
	...
	public void setSelectedExport(String selectedE) {
		this.selectedExport = selectedE;
	}
	...
	public void setAnnonymesRowsCheckBoxs(
			Hashtable<Checkbox, List<Checkbox>> a) {
	...
	public String getArchiveFormated() {
		...
		if (this.profil != null) {
			return ObjectTypesFormatters
				.booleanLitteralFormatter(this.profil.isArchive());
		} else {
			return "";
		}
	}
	...
	public void onCheck$utilisateursArchiveBox(Event e) {
		...
		if (profil != null && profil.getProfilId() != null) {
			if (utilisateursArchiveBox.isChecked()) {
				...
				profilUtilisateurs.addAll(ManagerLocator
					.getProfilUtilisateurManager().findByProfilManager(profil, null));
			}
		}
	}
	 ...
	 * @since 2.1
	 */
	public void onClickProfilUtilisateurBanque(Event event) {
		ProfilUtilisateur pf = (ProfilUtilisateur) event.getData();
		getAdministrationController()
