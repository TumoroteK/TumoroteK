 ...
 * @version 2.0
 */
public class ImprimanteRowRenderer implements RowRenderer<Object>
{
   ...
   public void render(final Row row, final Object data, final int index) throws Exception{
      final Imprimante imp = (Imprimante) data;

      // nom
      final Label nomLabel = new Label(imp.getNom());
      nomLabel.addForward(null, nomLabel.getParent(), "onClickImprimante", imp);
      ...
      if(imp.getImprimanteApi() != null){
         new Label(String.valueOf(imp.getImprimanteApi().getNom())).setParent(row);
      }else{
         new Label(null).setParent(row);
      }
      ...
      divImage.addForward("onClick", divDelete.getParent(), "onClickDeleteImprimante", imp);
