...
private static final long serialVersionUID = 3551763682958457361L;
	...
	private List<SmallObjDecorator> codesToCreateOrEdit = 
							new ArrayList<SmallObjDecorator>();
	private CodeAssigne codeToExport = null;
	private List<CodeAssigne> codesToDelete = 
	...
	public void setCodesToCreateOrEdit(List<SmallObjDecorator> c) {
	...
	public void setCodesToDelete(List<CodeAssigne> c) {
		this.codesToDelete = c;
	}
	
	public boolean isEmpty() {
		return codeToExport == null 
			&& getCodesToCreateOrEdit().isEmpty();
//			&& getCodesToDelete().isEmpty();
	...
	public boolean equals(Object obj) {
		...
		CodesPack test = (CodesPack) obj;
		return ((this.codeToExport == test.codeToExport
				|| (this.codeToExport != null 
						&& this.codeToExport.equals(test.codeToExport)))
				&& (this.codesToDelete == test.codesToDelete
						|| (this.codesToDelete != null 
						&& this.codesToDelete.equals(test.codesToDelete)))
				&& (this.codesToCreateOrEdit == test.codesToCreateOrEdit
						|| (this.codesToCreateOrEdit != null 
					&& this.codesToCreateOrEdit
								.equals(test.codesToCreateOrEdit)))
				);
	}
	...
	public CodesPack clone() {
		...
		for (int i = 0; i < getCodesToCreateOrEdit().size(); i++) {
			clone.getCodesToCreateOrEdit()
				.add(((CodeAssigneDecorator) 
						getCodesToCreateOrEdit().get(i)).clone());
		}
		for (int i = 0; i < getCodesToDelete().size(); i++) {
			clone.getCodesToDelete()
				.add(getCodesToDelete().get(i).clone());
		}
