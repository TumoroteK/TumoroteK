...
public class SelectDossierExternesToSaveModale implements Serializable {
	...
	private ListModelList<DossierDecorator> dossiersDecorators = new ListModelList<DossierDecorator>();
	private Map<DossierExterne, String> dosErr = new HashMap<DossierExterne, String>();
	...
	@Init
	public void init(@ExecutionArgParam("emetteur") final Emetteur emet, 
					@ExecutionArgParam("mainWindow") final MainWindow _mW) {
		this.emetteur = emet;
		this.mainWindow = _mW;
	...
	public void searchForDossierExternes() {
		...
		Collection<DossierExterne> dossiersPrimaires = CollectionUtils
			.subtract(ManagerLocator.getDossierExterneManager()
				.findByEmetteurAndEntiteNullManager(emetteur), 
					ManagerLocator.getInjectionManager()
						.findExistingPrelevementByEmetteurManager(emetteur, SessionUtils.getCurrentPlateforme()));
		
		// decoration pour affichage
		dossiersDecorators.addAll(dossiersPrimaires.stream()
				.map(d -> new DossierDecorator(d, dosErr.get(d)))
				.collect(Collectors.toList()));
			
		// synchronisation silencieuse des dossiers dérivés enfants dont 
		// le prélèvement parent existe déja en base
		if (firstTimeSync) {
			dossiersSaved = ManagerLocator.getInjectionManager().synchronizeDeriveChildrenManager(emetteur, 
				SessionUtils.getCurrentPlateforme(), SessionUtils.getLoggedUser(_sess.getAttributes()),
	...
	public void save(@ContextParam(ContextType.VIEW) Window win) {

		dosErr.clear();
		
		for (DossierDecorator dos : dossiersDecorators.getSelection()) {
			try {
				ManagerLocator.getInjectionManager().saveDossierAndChildrenManager(
					dos.getDossierExterne(), 
					SessionUtils.getCurrentBanque(_sess.getAttributes()), 
					SessionUtils.getLoggedUser(_sess.getAttributes()),
					SessionUtils.getSystemBaseDir());
				dossiersSaved = true;
			} catch (Exception e) {
				dosErr.put(dos.getDossierExterne(), 
		...
		for (DossierDecorator dos : dossiersDecorators.getSelection()) {
			try {
				ManagerLocator.getDossierExterneManager()
						.removeObjectManager(dos.getDossierExterne());
			} catch (Exception e) {
				AbstractController.handleExceptionMessage(e);
			}
		}
	...
	public class DossierDecorator {
		...
		public DossierDecorator(DossierExterne _d, String _e) {
			this.dossierExterne = _d;
			this.errMsg = _e;
			if (_d != null) {
				List<BlocExterne> bPat = ManagerLocator.getBlocExterneManager()
					.findByDossierExterneAndEntiteManager(_d, 
						ManagerLocator.getEntiteManager().findByIdManager(1));
				
				ValeurExterne nomVal = ManagerLocator.getValeurExterneManager()
						.findByBlocExterneManager(bPat.get(0)).stream()
						.filter(v -> v.getChampEntiteId().equals(3)).findFirst().orElse(null);
				if (nomVal != null) {
					setNom(nomVal.getValeur());
				}
				
				List<BlocExterne> bPrel = ManagerLocator.getBlocExterneManager()
						.findByDossierExterneAndEntiteManager(_d, 
							ManagerLocator.getEntiteManager().findByIdManager(2));
					
				if (bPrel.size() > 0) {
					ValeurExterne natureVal = ManagerLocator.getValeurExterneManager()
						.findByBlocExterneManager(bPrel.get(0)).stream()
						.filter(v -> v.getChampEntiteId().equals(24)).findFirst().orElse(null);
					if (natureVal != null) {
				...
				setNbChildren(ManagerLocator.getDossierExterneManager()
					.findChildrenByEmetteurValeurManager(emetteur, 23, 
							_d.getIdentificationDossier()).size());
			}
		}
		...
		public void setDossierExterne(DossierExterne _d) {
			this.dossierExterne = _d;
		}
		...
		public void setNature(String _n) {
			this.nature = _n;
		}
		...
		public void setNbChildren(Integer _c) {
			this.nbChildren = _c;
		}
		...
		public void setErrMsg(String _e) {
			this.errMsg = _e;
		}
		...
		public String getDateOperationFormatted() {
			if (dossierExterne != null) {
				return ObjectTypesFormatters
					.dateRenderer2(getDossierExterne().getDateOperation());
			}
			return null;
		}
		...
		public Boolean getInError() {
		 ...
		 @Override
		   ...
		   public boolean equals(Object obj){
				...
				DossierDecorator del = (DossierDecorator) obj;
