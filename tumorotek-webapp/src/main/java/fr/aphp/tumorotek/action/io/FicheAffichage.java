...
public class FicheAffichage extends AbstractFicheCombineController {
	...
	private List<ResultatDecorator> resultats = 
								new ArrayList<ResultatDecorator>();
	private List<Resultat> resultatsToRemove = new ArrayList<Resultat>();
	
	private List<EntiteDecorator> entites = new ArrayList<EntiteDecorator>();	
	...
	public EventListener<Event> dropListener = new EventListener<Event>() {
		public void onEvent(Event e) throws Exception {
			...
			for (int i = 0; i < ((Listbox) target.getParent())
											.getItemCount(); i++) {
				Listitem temp = ((Listbox) target.getParent())
											.getItemAtIndex(i);
				ResultatDecorator rDeco = ((ResultatDecorator) temp.getValue());
				Resultat r = rDeco.getResultat();
				r.setPosition(i + 1);
				r.setOrdreTri(i + 1);
				res.add(r);
			}
			...
			resultatsBox
				.setModel(new SimpleListModel<ResultatDecorator>(resultats));
			//addDropListeners();
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[]{intituleLabel, nbLignesLabel,
				resultatsBoxStatic});
		setObjBoxsComponents(new Component[] { intituleBox, nbLignesBox,
				removeImg, addChamp, resultatsBox});
	...
	public void setObject(TKdataObject obj) {
		...
		if (affichage.getAffichageId() != null) {
			resultats.addAll(ResultatDecorator
					.decorateListe(ManagerLocator.getResultatManager()
								.findByAffichageManager(affichage)));
		}
		Collections.sort(resultats);
		resultatsBox.setModel(new 
				SimpleListModel<ResultatDecorator>(resultats));
	...
	public void switchToCreateMode() {
		...
		if (this.entites.isEmpty()) {
			...
			this.entites.add(new EntiteDecorator(ManagerLocator
					.getEntiteManager().findByNomManager("Patient").get(0)));
			this.entites.add(new EntiteDecorator(ManagerLocator
					.getEntiteManager().findByNomManager("Maladie").get(0)));
			this.entites.add(new EntiteDecorator(ManagerLocator
				.getEntiteManager().findByNomManager("Prelevement").get(0)));
			this.entites.add(new EntiteDecorator(ManagerLocator
				.getEntiteManager().findByNomManager("Echantillon").get(0)));
			this.entites.add(new EntiteDecorator(ManagerLocator
					.getEntiteManager().findByNomManager("ProdDerive").get(0)));
			this.entites.add(new EntiteDecorator(ManagerLocator
					.getEntiteManager().findByNomManager("Cession").get(0)));
		}		
		this.resultats.clear();		
		resultatsBox.setModel(new SimpleListModel<ResultatDecorator>(resultats));
		
		//On ferme la liste des affichages
		setOpenListeAffichage(false);
		
		resultatsToRemove = new ArrayList<Resultat>();
	...
	public void switchToStaticMode() {
		...
		boolean isEmptyObj = this.affichage.equals(new Affichage());
	...
	public void switchToEditMode() {
		...
		//On initialise la liste d'entités
		this.entites = new ArrayList<EntiteDecorator>();
		this.entites.add(null);
		this.entites.add(new EntiteDecorator(ManagerLocator
				.getEntiteManager().findByNomManager("Patient").get(0)));
		this.entites.add(new EntiteDecorator(ManagerLocator
		...
		Collections.sort(resultats);
		this.resultatsBox.setModel(new 
				SimpleListModel<ResultatDecorator>(resultats));
	...
	public void checkNomsResultats() {
		...
		for (int i = 0; i < resultats.size(); i++) {
			for (int j = i + 1; j < resultats.size(); j++) {
				if (resultats.get(i).getResultat().getNomColonne().equals(
						resultats.get(j).getResultat().getNomColonne())) {
					alreadyIn = true;
				}
			}
		}
		...
		if (alreadyIn) {
			throw new RuntimeException(Labels
					.getLabel("affichage.doublon.nomColonne"));
		}
	}
	...
	public void onClick$createC() {
		if (resultats.size() <= 0) {
			Messagebox.show(Labels.getLabel(
					"affichage.validation.containsResultat"), 
			Labels.getLabel("general.error"), 
			Messagebox.OK, 
			Messagebox.ERROR);
		} else {
			Clients.showBusy(Labels.getLabel(
					"affichage.creation.encours"));
	...
	public void onLaterCreate() {
		try {
			...
			getObjectTabController().getTopController()
	...
	public void onClick$deleteC() {
		...
		if (affichage != null) {
			if (Messagebox.show(ObjectTypesFormatters.getLabel(
					"message.deletion.message", new String[] { Labels
							.getLabel("message.deletion.affichage") }),
				...
				try {
					 ...
					 */
					ManagerLocator.getAffichageManager()
						.removeObjectManager(
							affichage);

					// On retire l'affichage de la liste
					getListeAffichages()
						.removeObjectAndUpdateList(affichage);
	...
	public void onClick$validateC() {
		if (resultats.size() <= 0) {
			...
			Clients.showBusy(
					Labels.getLabel("affichage.creation.encours"));
	 ...
	 */
	public void onClick$removeImg(Event event) {
		// on récupère le critère que l'utilisateur veut
		// suppimer
		ResultatDecorator rDeco = (ResultatDecorator) 
		...
		Resultat r = rDeco.getResultat();
		int ordreTri = r.getOrdreTri();
		// on met à jour la position de chaque colonne
		for (int i = 0; i < resultats.size(); i++) {
			if (resultats.get(i).getResultat().getOrdreTri() > ordreTri) {
				resultats.get(i).getResultat().setOrdreTri(
					resultats.get(i).getResultat().getOrdreTri() - 1);
				resultats.get(i).getResultat().setPosition(resultats.get(i)
						.getResultat().getOrdreTri());
			}
		}
	 ...
	 * @param event
	 */
	public void onClick$upArrowDiv(Event event) {
	 ...
	 * @param event
	 */
	public void onClick$downArrowDiv(Event event) {
		// on récupère le critère
		ResultatDecorator rDeco = (ResultatDecorator) 
			AbstractListeController2
				.getBindingData((ForwardEvent) event, false);
		
		int tabIndex = resultats.indexOf(rDeco);
	 ...
	 */
	private void upObject(ResultatDecorator obj) {
		int tabIndex = resultats.indexOf(obj);
		ResultatDecorator supObjectInList = null;
		if (tabIndex - 1 > -1) {
			supObjectInList = resultats.get(tabIndex - 1);
			supObjectInList.getResultat().setPosition(
					supObjectInList.getResultat().getPosition() + 1);
			supObjectInList.getResultat().setOrdreTri(
					supObjectInList.getResultat().getOrdreTri() + 1);
			obj.getResultat().setPosition(
					obj.getResultat().getPosition() - 1);
			obj.getResultat().setOrdreTri(
					obj.getResultat().getOrdreTri() - 1);
	 ...
	 * @return
	 */
	protected Affichage saveAffichage(Utilisateur createur) {
		ManagerLocator.getAffichageManager()
			.createObjectManager(affichage, ResultatDecorator
				.extractListe(resultats), createur,
	 ...
	 * @param createur
	 */
	protected void updateAffichage(Utilisateur createur) {
		ManagerLocator.getAffichageManager().updateObjectManager(affichage,
				ResultatDecorator.extractListe(resultats),
				resultatsToRemove);				
	}
	...
	public void onClick$addChamp() {
		...
		for (int i = 0; i < resultats.size(); i++) {
			if (resultats.get(i).getResultat().getChamp()
					.getChampParent() == null) {
				chps.add(resultats.get(i).getResultat().getChamp());
			} else {
				chps.add(resultats.get(i).getResultat()
						.getChamp().getChampParent());
			}
		}
		openChampsAffichageWindow(page, chps,
				SessionUtils.getSelectedBanques(sessionScope).get(0));
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void onGetChamps(Event e) {
		if (e.getData() != null) {
			
			// pour chaque champ
			for (Champ currChamp : (List<Champ>) e.getData()) {
				Champ sousChampTmp = null;
				
				if (currChamp.getChampEntite() != null
						&& currChamp.getChampEntite().getQueryChamp() != null) {
					sousChampTmp = new Champ(currChamp
							.getChampEntite().getQueryChamp());
				...
				String nom = "";
				if (currChamp.getChampEntite() != null) {
					nom = ObjectTypesFormatters.getLabelForChampEntite(
							currChamp.getChampEntite());
				} else {
				...
				if (sousChampTmp != null) {
					this.resultats
						.add(new ResultatDecorator(new Resultat(
								nom, 
								sousChampTmp, true,
								resultats.size() + 1, 
								resultats.size() + 1, null,
								affichage)));
				} else {
					this.resultats
						.add(new ResultatDecorator(new Resultat(
	...
	/**
	 * PopUp window appelée permettre la sélection des champs à afficher.
	 * @param page dans laquelle inclure la modale
	 * @param oldSelected Liste des champs déjà sélectionnés.
	 */
	public void openChampsAffichageWindow(Page page, 
			List<Champ> oldSelected, Banque bank) {
		 if (!isBlockModal()) {
			...
			ua = (HtmlMacroComponent)
					page.getComponentDefinition("champsAffichageModale", false)
			...
			((FicheChampsAffichageModale) 
					ua.getFellow("fwinChampsAffichageModale")
					.getAttributeOrFellow("fwinChampsAffichageModale$composer", true))
					.init(oldSelected, self, bank);
			ua.setVisible(false);
	...
	public ListeAffichages getListeAffichages() {
		return ((ListeAffichages) self.getParent().getParent().getParent()
				.getParent()
				.getFellow("listeRegion")
				.getFellow("listeAffichages")
				.getFellow("lwinAffichages").
				getAttributeOrFellow("lwinAffichages$composer", true));
	}
...
//	}
	 ...
	 */
	private void setOpenListeAffichage(boolean open) {
		//On ferme la liste des affichages
		((org.zkoss.zul.West) self.getParent().getParent().getParent()
				.getParent()
				.getFellow("listeRegion")).setOpen(open);
	}
	...
	public void setResultats(List<ResultatDecorator> r) {
		this.resultats = r;
	}
	...
	public void setResultatsToRemove(List<Resultat> res) {
