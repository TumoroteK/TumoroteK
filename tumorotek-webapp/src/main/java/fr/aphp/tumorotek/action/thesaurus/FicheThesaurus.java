...
public class FicheThesaurus extends AbstractFicheCombineController {
	...
	private ThesaurusRowRenderer listValeursRenderer = 
		new ThesaurusRowRenderer();
	private CrudManager thesManager;
	private List<? extends Object> listValeurs = new ArrayList<Object>();
	...
	public void setObject(TKdataObject obj) {
		...
		winPanel.setTitle(ObjectTypesFormatters
						.getLabel("thesaurus.fiche.title", 
					new String[]{typeThesaurus.getLabel()}));
	 ...
	 */
	public void initListeValeurs(boolean reset) {
		listValeurs = new ArrayList<Object>();
		if (reset) {
			thesManager = null;
		...
		if (thesManager == null) {
			...
			listValeurs = ((TKThesaurusManager) thesManager)
				.findByOrderManager(SessionUtils
								.getPlateforme(sessionScope));
		} else {
			if (typeThesaurus.getNom().equals("Specialite")) {
				listValeurs = ((SpecialiteManager) thesManager)
								.findAllObjectsManager();
			} else if (typeThesaurus.getNom().equals("Categorie")) {
				listValeurs = ((CategorieManager) thesManager)
										.findAllObjectsManager();
			}
		}
	 ...
	 * bouton delete.
	 */
	public String getValeur(Object value) {		
	...
	public void onClick$addNewValeur() {
		...
		if (typeThesaurus.getNom().equals("Nature")) {
			...
			((NonConformite) value).setPlateforme(
				SessionUtils.getPlateforme(sessionScope));
			if (typeThesaurus.getNom().equals("NonConformiteArrivee")) {
				((NonConformite) value).setConformiteType(
						ManagerLocator.getConformiteTypeManager()
						.findByEntiteAndTypeManager("Arrivee", 
				ManagerLocator.getEntiteManager().findByIdManager(2)).get(0));
			} else if (typeThesaurus.getNom().equals("NonConformiteTraitementEchan")) {
				((NonConformite) value).setConformiteType(
						ManagerLocator.getConformiteTypeManager()
						.findByEntiteAndTypeManager("Traitement", 
				ManagerLocator.getEntiteManager().findByIdManager(3)).get(0));
			} else if (typeThesaurus.getNom().equals("NonConformiteCessionEchan")) {
				((NonConformite) value).setConformiteType(
						ManagerLocator.getConformiteTypeManager()
						.findByEntiteAndTypeManager("Cession", 
				ManagerLocator.getEntiteManager().findByIdManager(3)).get(0));
			} else if (typeThesaurus.getNom().equals("NonConformiteTraitementDerive")) {
				((NonConformite) value).setConformiteType(
						ManagerLocator.getConformiteTypeManager()
						.findByEntiteAndTypeManager("Traitement", 
				ManagerLocator.getEntiteManager().findByIdManager(8)).get(0));
			} else if (typeThesaurus.getNom().equals("NonConformiteCessionDerive")) {
				((NonConformite) value).setConformiteType(
						ManagerLocator.getConformiteTypeManager()
						.findByEntiteAndTypeManager("Cession", 
		...
		if (value instanceof TKThesaurusObject) {
			((TKThesaurusObject) value)
				.setPlateforme(SessionUtils.getPlateforme(sessionScope));
		}
		
		// création du titre
		String title = ObjectTypesFormatters
			.getLabel("thesaurus.creation.valeur", 
			new String[]{typeThesaurus.getLabel()});
		
		// appel de la fenêtre modale
		openChampThesaurusWindow(page, Path.getPath(self), 
				value, getConstraint(), 
				getSecondConstraint(), title, true);
	}
	 ...
	 * @param event
	 */
	public void onClickUpdateItem(ForwardEvent event) {
		// on récupère le thesaurus associé à l'event
		Object value = event.getData();
		
		// creation du titre
		String title = ObjectTypesFormatters
			.getLabel("thesaurus.modification.valeur", 
			new String[]{typeThesaurus.getLabel()});
	...
	private boolean isUsedValeur(Object value) {
		if (value.getClass().getSimpleName().equals("Specialite")) {
			return ManagerLocator
				.getSpecialiteManager().isUsedObjectManager(value);
		} else if (value.getClass().getSimpleName().equals("Categorie")) {
			return ManagerLocator
				.getCategorieManager().isUsedObjectManager(value);
		} else if (value.getClass().getSimpleName().equals("NonConformite")) {
			return ManagerLocator.getNonConformiteManager()
				.isUsedObjectManager((NonConformite) value);
		} else {
	 ...
	 * @param event
	 */
	public void onClickDeleteItem(ForwardEvent event) {
		// on récupère le thesaurus associé à l'event
		Object value = event.getData();
		
		String nom = getValeur(value);
		
		boolean isUsed = isUsedValeur(value);
		boolean isDeletable = true;
		
		String message = ObjectTypesFormatters.getLabel(
				"message.deletion.message", 
				new String[]{ObjectTypesFormatters.getLabel(
				"message.deletion.value.thesaurus", 
				new String[]{nom})});
		if (isUsed) {
			if (typeThesaurus.getNom().equals("Nature") 
					|| typeThesaurus.getNom().equals("EchantillonType")
					|| typeThesaurus.getNom().equals("ProdType")
					|| typeThesaurus.getNom().equals("ConsentType")
	...
	public ConstWord getSecondConstraint() {
		...
		if (typeThesaurus.getNom().equals("EnceinteType")) {
			constraint = ThesaurusConstraints
				.getEnceinteTypePrefixeConstraint();
		}
	 ...
	 */
	public void onGetSaveDoneOnValue(Event e) {
	...
	public void setTypeThesaurus(Thesaurus typeTh) {
	...
	public void setListValeursRenderer(ThesaurusRowRenderer listRenderer) {
	...
	public void setListValeurs(List<Object> listVals) {
