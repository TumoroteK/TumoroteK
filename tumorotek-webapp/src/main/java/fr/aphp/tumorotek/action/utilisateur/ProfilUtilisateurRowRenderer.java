...
public class ProfilUtilisateurRowRenderer implements RowRenderer<ProfilUtilisateur> {
	...
	public ProfilUtilisateurRowRenderer(boolean _b) {
		banqueRender = _b;
	}
	
	@Override
	public void render(Row row, ProfilUtilisateur profil, int index) throws Exception {
		
		// login
		Label loginLabel = new Label(profil.getUtilisateur()
					.getLogin());
		...
		loginLabel.addForward(null, loginLabel.getParent(), 
				"onClickProfilUtilisateur", profil);
		loginLabel.setParent(row);
		
		if (!banqueRender) {
			// profil
			Label profilLabel = new Label(profil.getProfil()
						.getNom());
			// @since 2.1
			...
			profilLabel.addForward(null, profilLabel.getParent(), 
					"onClickProfilUtilisateurProfil", profil);
			profilLabel.setParent(row);
		} else {
			// banque
			Label banqueLabel = new Label(profil.getBanque()
						.getNom());
			// @since 2.1
			...
			banqueLabel.addForward(null, banqueLabel.getParent(), 
					"onClickProfilUtilisateurBanque", profil);
