...
public class FicheSaisieOperateurCritere 
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		setObjLabelsComponents(new Component[]{this.operateurLabel,
				this.critereLabel, this.errorLabel
		});
		
		setObjBoxsComponents(new Component[]{this.operateursBox,
				this.criteresBox
		});
		
		setRequiredMarks(new Component[]{this.operateurRequired,
				this.critereRequired
		});
	...
	public void init(String p) {
		...
		this.operateursBox.setModel(new SimpleListModel<String>(this.operateurs));
		
		this.criteres = new ArrayList<CritereNode>();
		
		FicheRequete fiche = getFicheRequete();
		GroupementNode n = fiche.getNode();
		...
		if (this.criteres.size() == 1) {
			this.criteresBox
				.setModel(new SimpleListModel<CritereNode>(this.criteres));
	...
	private FicheRequete getFicheRequete() {
		return (FicheRequete) Path.getComponent(path).getAttributeOrFellow(
				"winFicheRequete$composer", true);
	}
	...
	public void onClick$cancelC() {
		...
		if (Path.getComponent(path) != null) {
			...
			Events.postEvent(
				new Event("onCloseSaisieOperateurCritere", 
										Path.getComponent(path), null));
		}
	...
	public void onClick$validateC() {
		...
		if (Path.getComponent(path) != null) {
			// on envoie un event à cette page avec
			// l'établissement sélectionné
			Events.postEvent(new Event("onCloseSaisieOperateurCritere",
					Path.getComponent(path), retour));
		}
		// fermeture de la fenêtre
		Events.postEvent(new Event("onClose", self.getParent()
														.getParent()));
	}
	...
	private void validateForm() {
		if (operateursBox.getModel().getSize() > 1
				&& operateursBox.getSelectedIndex() <= 0) {
			 throw new WrongValueException(operateursBox, Labels
					.getLabel("operateur.invalid.selection"));
		}
		if (critereNode == null) {
			throw new WrongValueException(criteresBox, Labels
					.getLabel("critere.invalid.selection"));
		}
	}
	...
	public void setOperateurs(List<String> ops) {
		this.operateurs = ops;
	}
	...
	public void setCriteres(List<CritereNode> crits) {
		this.criteres = crits;
	}
	...
	public void setCritereNode(CritereNode cNode) {
