...
public class ConteneursAssocies extends OneToManyComponent {
	...
	private List<ConteneurDecorator> objects = new ArrayList<ConteneurDecorator>();
	...
	public void removeFromListObjects(Object obj) {
		getObjects().remove((ConteneurDecorator) obj);
	}
	...
	public void onClick$objLinkLabel(Event event) {
		if (objLinkLabel.getSclass().equals("formLink")) {
			...
			Object cont = AbstractListeController2
			...
			controller
				.switchToFicheConteneurMode(((ConteneurDecorator) cont).getConteneur());
		}
	}
	...
	public List< ? extends Object> findObjectsAddable() {
		...
		List<ConteneurDecorator> conts = new ArrayList<ConteneurDecorator>();
			
		if (conteneursDeBanque) {
			conts.addAll(ConteneurDecorator
				.decorateListe(ManagerLocator.getConteneurManager()
				.findByPlateformeOrigWithOrderManager(getPlateforme()), null));
		}
		
		conts.addAll(ConteneurDecorator.decorateListe(ManagerLocator
				.getConteneurManager().findByPartageManager(getPlateforme(), conteneursDeBanque), 
	...
	public void drawActionForComponent() {
		addObj
			.setDisabled(!SessionUtils.isAdminPF(sessionScope));
//
...
//		setDroitsConsultation(drawConsultationLinks(entites));
	...
	public void setConteneursDeBanque(boolean cDeBanque) {
	...
	public void onClick$deleteImage(Event event) {
		...
		ConteneurDecorator cur = (ConteneurDecorator) AbstractListeController2
				.getBindingData((ForwardEvent) event, false); 
		
		if (!isConteneursDeBanque()) { // referencement depuis plateforme
			if (ManagerLocator.getConteneurManager()
					.findByBanquesWithOrderManager(new 
						ArrayList<Banque>(ManagerLocator.getPlateformeManager()
							.getBanquesManager(cur.getCurrent())))
							.contains(cur.getConteneur())) {
				Messagebox.show(Labels
						.getLabel("plateforme.conteneur.remove.error"), 
				...
				Messagebox.show(Labels
						.getLabel("banque.conteneur.remove.warning"), 
							Labels.getLabel("general.warning"), 
