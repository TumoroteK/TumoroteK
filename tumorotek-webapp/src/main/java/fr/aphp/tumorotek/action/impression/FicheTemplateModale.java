...
public class FicheTemplateModale extends AbstractImpressionController
{
   ...
   private final Logger log = LoggerFactory.getLogger(FicheTemplateModale.class);
   ...
   public void switchToStaticMode(){
      ...
      if(ETemplateType.BLOC == selectedTemplate.getType()){
         ...
         final String fileExtension = TemplateUtils.getFileExtension(template);
         ...
         for(final CleImpression cle : cles){
            final CleImpressionDecorator cleImprDeco = new CleImpressionDecorator(cle);
   ...
   public void onClick$print(){
      if(ETemplateType.BLOC == template.getType()){
         ...
         for(final CleImpressionDecorator cleImprDeco : cleImpressionDecoratorList){
            if(null == cleImprDeco.getCleImpression().getChamp()){
               throw new WrongValueException(print,
                  "La clé " + cleImprDeco.getCleImpression().getNom() + " n'a pas de champ renseigné");
               ...
               throw new WrongValueException(print, "Le champ " + cleImprDeco.getCleImpression().getChamp()
                  + " correspondant à la clé " + cleImprDeco.getCleImpression().getNom() + " n'est pas renseigné");
   ...
   public void onLaterPrint(){
      ...
      if(ETemplateType.DOC == this.template.getType()){
         TemplateUtils.replaceKeysInDocumentTemplateAndDownload(template, objectToPrint,
            nomFile + TemplateUtils.getFileExtension(template));
   ...
   public void createBlocInfosForEchantillon(final Echantillon echantillon){
      ...
      String tmp = "";
      final CoupleValeur cpVide = new CoupleValeur("", "");
      tmp = EchantillonRowRenderer.formatQuantite(echantillon);
      ...
      // Délai Cgl
      final CoupleValeur cp3 =
         new CoupleValeur(Labels.getLabel("Champ.Echantillon.DelaiCgl"), EchantillonRowRenderer.formatDelaiCgl(echantillon));
      final LigneParagraphe li2 = new LigneParagraphe("", new CoupleValeur[] {cp2, cp3});
      ...
      // Emplacement
      final CoupleValeur cp5 = new CoupleValeur(Labels.getLabel("Champ.Echantillon.Emplacement"),
         EchantillonRowRenderer.getEmplacementAdrl(echantillon));
      cp5.setAnonyme(anonyme);
      // Statut
      ...
      while(it.hasNext()){
         ...
         for(int j = 0; j < champs.size(); j++){
            ...
            if(champs.get(j).getNom().equals("Code")){
               ...
               val.append(EchantillonRowRenderer.formatQuantite(echan));
               ...
               if(!anonyme){
                  val.append(EchantillonRowRenderer.getEmplacementAdrl(echan));
               }else{
                  val.append("-");
               }
            }else if(champs.get(j).getNom().equals("DelaiCgl")){
               val.append(EchantillonRowRenderer.formatDelaiCgl(echan));
   ...
   public void setCleImpressionDecoratorList(final List<CleImpressionDecorator> cleImpressionDecoratorList){
