 ...
 * @version 2.0
 */
public class AnnoItemRowRenderer implements RowRenderer {
	...
	public void render(final Row row, final Object data, int index) {
		...
		// label row
		Label labLabel = new Label(item.getLabel());
		labLabel.setParent(row);		
		// valeur row
		Label valLabel = new Label(item.getValeur());
		valLabel.setParent(row);
		
		// defaut row
		Label defLabel = 
			new Label(ObjectTypesFormatters
					.booleanLitteralFormatter(isItemDefaut(item, defauts)));
		...
		editImg.addForward("onClick", row.getParent(), 
	 ...
	 */
	public static boolean isItemDefaut(Item it, Set<AnnotationDefaut> defs) {
		if (it != null) {
			Iterator<AnnotationDefaut> itor = 
					(Iterator<AnnotationDefaut>) defs.iterator();
			while (itor.hasNext()) {
