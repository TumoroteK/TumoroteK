...
public class FicheRechercheBiocapModale {
	...
	private List<ObjetStatut> statuts = new ArrayList<ObjetStatut>();
	...
	@Init
	public void init(@ExecutionArgParam("parent") Component p, 
						@ExecutionArgParam("controller") AbstractObjectTabController aotc) {
		parent = p;
		tabController = aotc;
	}
	...
	public void selectGroup(@BindingParam("data") Object data) {
	    if(data instanceof ServiceGroupModel.ServiceInfo) {
	    	ServiceGroupModel.ServiceInfo groupInfo = (ServiceGroupModel.ServiceInfo) data;
	        ...
	        for(int childIndex = 0; childIndex < childCount; childIndex++) {
	            Service serv = groupModel.getChild(groupIndex, childIndex);
	            if(added) {
	...
	public class ServiceComparator implements Comparator<Service>, 
									GroupComparator<Service>, Serializable {
		...
		public int compareGroup(Service s1, Service s2) {
	...
	public void onLaterFind() throws NamingException {
		Calendar calInf = Calendar.getInstance();
		calInf.setTime(dateInf);
		Calendar calSup = Calendar.getInstance();
		calSup.setTime(dateSup);
		// on lance la recherche BIOCAP
		List<Integer> ids = ManagerLocator.getTraitementQueryManager()
			.findEchantillonsByRequeteBiocapManager(SessionUtils.getDbms(),
					SessionUtils.getSelectedBanques(_sess.getAttributes()), 
					groupModel.getSelectedServices(), calInf, calSup, age, objetStatut);
		
		if (ids.size() > 500) {
			tabController.openResultatsWindow(parent.getPage(), ids, parent,
					"Echantillon", tabController);
			// search history settings
			// createSearchHistory();
		} else {
			List<Echantillon> values = ManagerLocator
	...
	public void setStatuts(List<ObjetStatut> s) {
		this.statuts = s;
	}
}
