 ...
 *
 */
public class ModificationMultipleCodeAssigne 
	...
	public void init(String pathToPage, String methodToCall,
			...
			String entiteNom, Constraint constr, Boolean isComb) {
		super.init(pathToPage, methodToCall, objs, label, 
				champToEdit, entiteNom,
				constr, isComb);
	...
	private CodesPack getPackFromCodeEditor(CodeAssigneEditableGrid editor) {		
		// encore en edition
		if (editor.getCurrentObjEdited() != null 
					&& editor.getCurrentObjEdited().getEdition()) {
			throw new WrongValueException(
					editor.getObjGrid(), 
					Labels.getLabel("grid.currentObjet.edited"));
		}
		
		CodesPack pack = new CodesPack();

		
		for (int i = 0; i < editor.getObjs().size(); i++) {
			// trouve l'export
			if (((CodeAssigneDecorator) editor.getObjs().get(i)).getExport()) {
				pack.setCodeToExport((CodeAssigne) 
						editor.getObjs().get(i).getObj());
				break;
			}
		}
		...
		Iterator<SmallObjDecorator> it = editor.getObjToDelete().iterator();
		while (it.hasNext()) {
			pack.getCodesToDelete().add((CodeAssigne) 
	...
	public void passValueToEraserBox() {
		if (getSelectedValue() != null) {
			CodesPack pck = (CodesPack) getSelectedValue();
			getCodesEraseController().setObjs((List<SmallObjDecorator>) 
										pck.getCodesToCreateOrEdit());
		} else {
	...
	public void extractValuesFromObjects() {
		...
		for (int i = 0; i < getListObjets().size(); i++) {
			...
			// CodeAssigne exp;
			if (isOrg()) {
				codes.addAll(ManagerLocator.getCodeAssigneManager()
			...
			if (!codes.isEmpty()) {
				CodesPack pck = new CodesPack();
				pck.setCodesToCreateOrEdit(CodeAssigneDecorator
										.cleanDecorateListe(codes));
	...
	public void setUniqueValueToMultiBox() {
		...
		getCodesUniqueController()
			.setObjs((List<SmallObjDecorator>) 
					((CodesPack) getNewValue())
						.getCodesToCreateOrEdit());
	}
	...
	public String formatLocalObject(Object obj) {
		...
		if (obj != null) {
			if (((CodesPack) obj).getCodesToCreateOrEdit() != null) {
				...
				for (int i = 0; 
					i < ((CodesPack) obj).getCodesToCreateOrEdit().size();
					i++) {
						bld.append(((CodeAssigneDecorator) 
							((CodesPack) obj)
								.getCodesToCreateOrEdit().get(i)).getCode());
						// export
						if (((CodeAssigneDecorator) ((CodesPack) obj)
							.getCodesToCreateOrEdit().get(i)).getObj()
							.equals(((CodesPack) obj).getCodeToExport())) {
							bld.append("*");
						}
						// connecteur ' - '
						if (i < ((CodesPack) obj)
								.getCodesToCreateOrEdit().size() - 1) {
							bld.append(" - ");
						}
					}
				out = bld.toString();
	...
	public AnnotateDataBinder getBinder() {
		return ((AnnotateDataBinder) self
				.getParent()
				.getAttributeOrFellow("modificationCodeAssigne", true));
	}	
}
