...
public class DossierExterneRenderer implements ListitemRenderer<DossierExterne> {
	...
	@Override
	public void render(Listitem li, DossierExterne data, int index) throws Exception {
		DossierExterne dossier =  data;
		
		// numero dossier
		new Listcell(dossier.getIdentificationDossier()).setParent(li);
		
		// Date dossier
		new Listcell(ObjectTypesFormatters.dateRenderer2(
				dossier.getDateOperation())).setParent(li);
