...
public class FichePatientEdit extends AbstractFicheEditController {
	...
	private Textbox nomNaisBox;
	private Textbox prenomBox;
	private Datebox dateNaisBox;
	private Textbox villeNaisBox;
	private Textbox paysNaisBox;
	private Datebox dateEtatDecesBox;
	private Listbox sexeBox;
	
	private Toolbar toolbar;
	
	// included ndaBox (embedded in Prelevement mode only)
	private Label ndaFieldLabel;
	...
	private Date dateEtatDeces;
	
	// traduction pour affichage
	private LabelCodeItem selectedSexe;
	private LabelCodeItem selectedEtat;
	...
	public void createNewObject() {
		...
		try {
			...
			ManagerLocator.getPatientManager()
				.createOrUpdateObjectManager(patient, null, medecins, liens, 
					getObjectTabController().getFicheAnnotation()
												.getValeursToCreateOrUpdate(),
							null, filesCreated, null, 
		...
		if (this.selectedSexe != null) {
			if (selectedSexe.getCode() == null) {
				throw new WrongValueException(
						sexeBox, 
					Labels.getLabel("patient.error.sexe"));
			}
	...
	public void prepareDataBeforeSave(boolean setMedecins) {
		...
		if (setMedecins && medecins.size() > 0) {
			...
			Set<PatientMedecin> pmeds = new HashSet<PatientMedecin>();
			PatientMedecin pm;
	 ...
	 */
	public static LabelCodeItem setSexeItemFromDBValue(Patient pat) {
		if (pat.getSexe() != null) {
	...
	public void setSelectedSexe(LabelCodeItem s) {
		this.selectedSexe = s;
	}
	...
	public void setSelectedEtat(LabelCodeItem et) {
	...
	public List<LabelCodeItem> getEtats() {
		if (this.selectedSexe == null 
				|| !("F".equals(this.selectedSexe.getCode()))) {
	...
	public void setDateEtatDeces(Date dEd) {
	 ...
	 */
	private void accordEtatToGender() {
		if (!("F".equals(this.selectedSexe.getCode()))) {
			if (this.selectedEtat != null 
					&& this.selectedEtat.getLabel()
						.equals(Labels.getLabel("patient.etat.vivant.f"))) {
				this.selectedEtat = PatientUtils.ETAT_V;
			} else if (this.selectedEtat != null 
					&& this.selectedEtat.getLabel()
						.equals(Labels.getLabel("patient.etat.decede.f"))) {
				this.selectedEtat = PatientUtils.ETAT_D;
			}
		} else {
			if (this.selectedEtat != null 
					&& this.selectedEtat.getLabel()
						.equals(Labels.getLabel("patient.etat.vivant"))) {
				this.selectedEtat = PatientUtils.ETAT_VF;
			} else if (this.selectedEtat != null 
					&& this.selectedEtat.getLabel()
						.equals(Labels.getLabel("patient.etat.decede"))) {
				this.selectedEtat = PatientUtils.ETAT_DF;
			}
		}
	}
	...
	private void accordDateToEtat() {
		if (this.selectedEtat != null 
				&& ("D".equals(this.selectedEtat.getCode()))) {
	 ...
	 */
	private void recordDateEtatDeces() {
		boolean isDead = (this.selectedEtat != null 
				&& ("D".equals(this.selectedEtat.getCode())));
		if (isDead) {
...
//			this.patient.setDateEtat(null);
	...
	public void setFieldsToUpperCase() {
		if (this.patient.getNom() != null) {
			this.patient.setNom(
				this.patient.getNom().toUpperCase().trim());
		}
		
		if (this.patient.getPrenom() != null) {
			this.patient.setPrenom(
				this.patient.getPrenom().toUpperCase().trim());
		}
		
		if (this.patient.getNomNaissance() != null) {
			this.patient.setNomNaissance(
				this.patient.getNomNaissance().toUpperCase().trim());
		}
		
		if (this.patient.getVilleNaissance() != null) {
			this.patient.setVilleNaissance(
				this.patient.getVilleNaissance().toUpperCase().trim());
		}
		
		if (this.patient.getPaysNaissance() != null) {
			this.patient.setPaysNaissance(
				this.patient.getPaysNaissance().toUpperCase().trim());
		}
	}
...
//		if (!badDateFormat) {
			...
			validateCoherenceDate(dateEtatDecesBox, 
					dateEtatDecesBox.getValue());
//		}
	}
...
//		if (!badDateFormat) {
			Clients.clearWrongValue(dateEtatDecesBox);
			validateCoherenceDate(
					dateEtatDecesBox, dateEtatDecesBox.getValue());
			Clients.clearWrongValue(dateNaisBox);
			validateCoherenceDate(dateNaisBox, dateNaisBox.getValue());
//		}
	...
	protected void validateCoherenceDate(Component comp, Object value) {
		...
		if (dateValue == null || dateValue.equals("")) { 
			...
			if (comp.getId().equals("dateEtatDecesBox")) {
				if (this.selectedEtat != null 
						&& ("D".equals(this.selectedEtat.getCode()))) { // mort
			...
			if (comp.getId().equals("dateNaisBox")) {
				field = "dateNaissance";
				this.patient.setDateNaissance(dateValue);
				errs = ManagerLocator.getPatientValidator()
						.checkDateNaissanceCoherence(this.patient);
			}
			...
			if (comp.getId().equals("dateEtatDecesBox")) {
				if (this.selectedEtat != null 
					...
					errs = PatientValidatorImpl
						.checkDateDecesCoherence(this.patient);
				} else { // vivant ou inconnu
					field = "dateEtat";
					this.patient.setDateEtat(dateValue);
					errs = PatientValidatorImpl
						.checkDateEtatCoherence(this.patient);
				}
			}
			...
			public void onEvent(Event event) throws Exception {
				((CalendarBox) self.getParent()
					...
					.getFellow("datePrelCalBox"))
						.clearErrorMessage(((CalendarBox) self.getParent()
							.getParent()
							.getParent()
							.getParent()
							.getParent()
