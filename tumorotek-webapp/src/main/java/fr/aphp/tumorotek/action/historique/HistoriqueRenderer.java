...
public class HistoriqueRenderer implements RowRenderer<Operation> {
	...
	@Override
	public void render(Row row, Operation op, int index) throws Exception {
		...
		String label = Labels.getLabel("OperationType." 
				+ op.getOperationType().getNom());
		if (label != null) {
			type.append(label);
		} else {
			...
			if (type.toString().equals("Creation")) {
				if (ManagerLocator.getImportHistoriqueManager()
						.findImportationsByEntiteAndObjectIdManager(
								op.getEntite(), 
								op.getObjetId()).size() > 0) {
					type.append(" (Import)");
				}
			}
		}
		...
		new Label(ObjectTypesFormatters
				.dateRenderer2(op.getDate())).setParent(row);
		
		// applique style si v1
		if (op.getV1()) {
			Label lab = new Label("v1");
	...
	public static String getEntite(Operation op) {
		String nom = "-";
		
		if (op.getEntite() != null) {
			if (op.getEntite().getNom().equals("Fantome")) {
				Object obj = ManagerLocator.getEntiteManager()
					.findObjectByEntiteAndIdManager(op.getEntite(), 
						op.getObjetId());
	...
	public void setIdentifiantAndBanque(Operation op) {
		Object obj = null;
		if (op.getEntite() != null) { 
			if (!op.getEntite().getNom().contains("Code")) {
				obj = ManagerLocator.getEntiteManager()
						.findObjectByEntiteAndIdManager(op.getEntite(), 
								op.getObjetId());
			}
	...
	public static String getIdentifiantForOperation(Operation op) {
		Object obj = null;
		String value = "-";
		if (!op.getEntite().getNom().contains("Code")) {
			obj = ManagerLocator.getEntiteManager()
	...
	public static String getBanqueForOperation(Operation op) {
		Object obj = null;
		String value = "-";
		if (!op.getEntite().getNom().contains("Code")) {
			obj = ManagerLocator.getEntiteManager()
