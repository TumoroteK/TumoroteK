 ...
 * @version 2.0
 */
public class ModificationMultipleQuantification extends
		AbstractModificationMultipleComponent {
	...
	private List<Object> uniteValues = new ArrayList<Object>();
	private List<Object> allUniteValues = new ArrayList<Object>();
	private List<String> allStringUniteValues = new ArrayList<String>();
	...
	public void init(String pathToPage, String methodToCall, 
			...
			String label, String champToEdit, List<Object> allValuesThesaurus,
			String champNameThesaurus, String entiteNom, Constraint constrn, 
			Boolean isComined) {
		setPath(pathToPage);
		...
		if (modifPossible) {
			...
			if (getStringValues().size() <= 1) {
				multiUnitesBox.setSelectedIndex(
						allUniteValues.indexOf(selectedUnite));
			}
		} else {
			// si la modif est interdite, on affiche un message d'erreur
			String[] params = new String[] {Labels.getLabel(getChampLabel())};
			modifImpossibleLabel.setValue(
					ObjectTypesFormatters.getLabel(
								"modification.multiple.impossible", params));
	...
	public void validateModification() {
		String champActuel = getChamp().replace("Init", "");
		
		// pour chaque objet, on extrait les valeurs du champ
		// initial et actuel et on les compare
		for (int i = 0; i < getListObjets().size(); i++) {
			try {
				Float tmpFloatInit = (Float) PropertyUtils.getSimpleProperty(
						getListObjets().get(i), getChamp());
				
				Float tmpFloat = (Float) PropertyUtils.getSimpleProperty(
						getListObjets().get(i), champActuel);
	...
	public void extractValuesFromObjects() {
		...
		for (int i = 0; i < getListObjets().size(); i++) {
			try {
				...
				if (tmps != null) {
					Object formatted = formatValue(tmpFloat);
			...
			try {
				...
				if (allUniteValues.get(i) != null) {
					stringTmp = (String) PropertyUtils.getSimpleProperty(
							allUniteValues.get(i), getChampThesaurus());
				} else {
					stringTmp = "";
				}
	...
	public Object extractValueFromEraserBox() {
		if (eraseMultiUnitesBox.getSelectedIndex() > -1) {
			selectedUnite = 
				allUniteValues.get(eraseMultiUnitesBox.getSelectedIndex());
		} else {
			selectedUnite = allUniteValues.get(0);
		}
		if (eraseMultiNumeriqueBox.getValue() != null) {
			return ObjectTypesFormatters
					.floor(eraseMultiNumeriqueBox.getValue().floatValue(), 3);
		}
	...
	public Object extractValueFromMultiBox() {
		if (multiUnitesBox.getSelectedIndex() > -1) {
			selectedUnite = 
				allUniteValues.get(multiUnitesBox.getSelectedIndex());
		} else {
			selectedUnite = allUniteValues.get(0);
		}
		if (multiNumeriqueBox.getValue() != null) {
			return ObjectTypesFormatters
					.floor(multiNumeriqueBox.getValue().floatValue(), 3);
		}
	...
	public void onClick$validate() {
			...
			if (!multiValuesListBox.isDisabled()) {
				...
				if (getNewValue() == null || getNewValue().equals("")) {
					if (multiValuesListBox.getSelectedIndex() > -1) {
						setNewValue(getValues()
							.get(multiValuesListBox.getSelectedIndex()));
					}
				}
			}
		} else { // mode unique
		...
		if ((getNewValue() != null 
				&& (!getNewValue().equals(getOldUniqueValue()) 
					|| !selectedUnite.equals(oldUniqueUniteValue) 
					|| getHasNulls())) 
				|| (getNewValue() == null && getOldUniqueValue() != null)) {
			finalValue = new ArrayList<Object>();
	...
	public AnnotateDataBinder getBinder() {
		return ((AnnotateDataBinder) self
				.getParent()
				.getAttributeOrFellow("modificationQuantification", true));
	}
	...
	public void setUniteValues(List<Object> uniteVal) {
	...
	public void setAllUniteValues(List<Object> allUniteVal) {
	...
	public void setAllStringUniteValues(List<String> allStringUniteVal) {
	...
	public void setChampUnite(String champUnit) {
	...
	public void setModifPossible(boolean modif) {
	...
	public void setSelectedUnite(Object sU) {
	...
	public String formatLocalObject(Object obj) {
		if (obj != null && ((List<Object>) obj).get(0) != null) {
			...
			sb.append(String.valueOf(ObjectTypesFormatters
					.floor((Float) ((List<Object>) obj).get(0), 3)));
			sb.append(" ");
	...
	public void passValueToEraserBox() {	
		if (getValues()
			.get(multiValuesListBox.getSelectedIndex()) != null) {
			eraseMultiNumeriqueBox
				.setValue(new BigDecimal(((Float) getValues()
				.get(multiValuesListBox.getSelectedIndex())).floatValue()));
			eraseMultiUnitesBox.setSelectedIndex(allUniteValues
					.indexOf(uniteValues
							.get(multiValuesListBox.getSelectedIndex() - 1)));
		} else {
