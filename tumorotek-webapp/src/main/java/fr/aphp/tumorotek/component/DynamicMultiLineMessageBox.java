...
public class DynamicMultiLineMessageBox {
	...
	@Init
	public void init(@ExecutionArgParam("title") String _t, 
					@ExecutionArgParam("message") String _m,
					@ExecutionArgParam("exception") Exception _e,
					@ExecutionArgParam("exceptionDetails") String _s) {
		this.title = _t;
		this.message = _m;
		this.exception  = _e;
	...
	public void details() {
		...
		if (exceptionDetails == null) {
			if (exception instanceof DoublonFoundException) {
				...
				case "Prelevement":
					List<Prelevement> dbls = ManagerLocator.getPrelevementManager()
					.findByCodeInPlateformeManager(((DoublonFoundException) exception).getCode(), 
					SessionUtils.getCurrentPlateforme());
					for (Prelevement prelevement : dbls) {
						if (!banks.equals("")) {
							banks = banks + ", ";
						}
					...
					break;
				case "Echantillon":
					List<Echantillon> dblEs = ManagerLocator.getEchantillonManager()
						.findByCodeInPlateformeManager(((DoublonFoundException) exception).getCode(), 
								SessionUtils.getCurrentPlateforme());
					for (Echantillon echantillon : dblEs) {
						if (!banks.equals("")) {
							banks = banks + ", ";
						}
					...
					break;
				case "ProdDerive":
					List<ProdDerive> dblDs = ManagerLocator.getProdDeriveManager()
						.findByCodeInPlateformeManager(((DoublonFoundException) exception).getCode(), 
								SessionUtils.getCurrentPlateforme());
					for (ProdDerive derive : dblDs) {
						if (!banks.equals("")) {
							banks = banks + ", ";
						}
				...
				case "Cession":
					...
					List<Cession> dblCs = ManagerLocator.getCessionManager()
						.findByNumeroInPlateformeManager(((DoublonFoundException) exception).getCode(), 
								SessionUtils.getCurrentPlateforme());
					for (Cession cession : dblCs) {
						if (!banks.equals("")) {
							banks = banks + ", ";
						}
					...
					break;
				case "Patient":				
					List<Patient> pats = ManagerLocator.getPatientManager()
						.findByNipLikeManager(((DoublonFoundException) exception).getCode(), true);
					if (!pats.isEmpty()) {
				...
				exceptionDetails = !banks.equals("") ? 
						ObjectTypesFormatters.getLabel(label, 
								new String[]{((DoublonFoundException) exception).getCode(), banks}) 
					: label;
			}
		}
	...
	public void setDetailsLabelVisible(Boolean _d) {
	...
	public void setExceptionDetails(String _e) {
