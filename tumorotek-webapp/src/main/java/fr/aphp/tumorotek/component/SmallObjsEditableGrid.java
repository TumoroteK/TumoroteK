...
public class SmallObjsEditableGrid extends AbstractController {
	...
	private List<SmallObjDecorator> objs = new ArrayList<SmallObjDecorator>();
	private List<SmallObjDecorator> objToCreateOrEdit = 
											new ArrayList<SmallObjDecorator>();
	private List<SmallObjDecorator> objToDelete = 
											new ArrayList<SmallObjDecorator>();
	...
	public void setDeletionMessageKey(String dKey) {
	...
	public void setCurrentObjEdited(SmallObjDecorator c) {
		this.currentObjEdited = c;
	}

	public void setObjs(List<SmallObjDecorator> os) {
		this.objs = os;
	...
	public void setBeforeEditObjClone(Object bec) {
	...
	public void onClick$editObj(Event event) throws IOException {
		...
		this.currentObjEdited = (SmallObjDecorator) 
		...
		// colore Row
		((Row) rows.getChildren().get(getObjs().indexOf(currentObjEdited)))
	...
	public void onClick$deleteObj(Event event) {
		...
		if (Messagebox.show(ObjectTypesFormatters.getLabel(
				...
				new String[]{Labels.getLabel(deletionMessageKey)}),
	...
	public void onClick$validateObj(Event event) {
		...
		if (currentObjEdited != null) {
			...
			if (!getObjToCreateOrEdit().contains(getCurrentObjEdited()) 
				&& !getObjs().contains(getCurrentObjEdited())) {
	...
	protected void revertCurrentObjEdition() {
		if (this.currentObjEdited != null) {
			...
			if (this.currentObjEdited.getObjDbId() != null
					|| this.currentObjEdited.isValidated()) {
				// revert
	 ...
	 * @param event
	 */
	public void onClick$upObj(Event event) {
		revertCurrentObjEdition();
		upObject((SmallObjDecorator) 
				AbstractListeController2
					.getBindingData((ForwardEvent) event, false));
		reloadGrid();
	}
	 ...
	 * @param event
	 */
	public void onClick$downObj(Event event) {
		revertCurrentObjEdition();
		SmallObjDecorator deco = (SmallObjDecorator) 
			AbstractListeController2
					.getBindingData((ForwardEvent) event, false); 
		int tabIndex = getObjs().indexOf(deco);
	 ...
	 */
	private void upObject(SmallObjDecorator obj) {
		int tabIndex = getObjs().indexOf(obj);
		SmallObjDecorator supObjectInList = null;
		Integer ordreObj = obj.getOrdre();
		if (tabIndex - 1 > -1) {
