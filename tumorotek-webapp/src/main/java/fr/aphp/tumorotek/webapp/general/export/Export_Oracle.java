...
public class Export_Oracle extends Export {
	...
	public Export_Oracle(Desktop d, int ent, List<Integer> o, List<Banque> b,
			boolean anonyme, short type, Utilisateur u, List<Integer> rI,
			HtmlMacroComponent htmlMacroComponent, Map<String,?> params) {
	...
	protected void fetchResultSet() throws SQLException {
		...
		((CallableStatement) getPreparedStatement())
			.registerOutParameter(outParamIdx, OracleTypes.CURSOR);
		getPreparedStatement().setFetchSize(100);
		getPreparedStatement().executeQuery();
		setMainRset((ResultSet) 
			((CallableStatement) getPreparedStatement()).getObject(outParamIdx));
	}
	...
	protected void populateDeriveParentsList() throws SQLException {
		...
		((CallableStatement) getPreparedStatement())
			.registerOutParameter(1, OracleTypes.ARRAY, "ID_LIST");
		((CallableStatement) getPreparedStatement())
			.registerOutParameter(2, OracleTypes.ARRAY, "ID_LIST");
		echantillonIds.clear();
		getPreparedStatement().executeQuery();
		ARRAY a = (ARRAY) ((CallableStatement) getPreparedStatement()).getObject(1);
		...
		ARRAY b = (ARRAY) ((CallableStatement) getPreparedStatement()).getObject(2);
