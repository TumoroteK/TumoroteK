...
public class RestrictTablesModale { 
	...
	private List<Integer> restrictedId = new ArrayList<Integer>();
	...
	@Init
	public void init(@ExecutionArgParam("controller") AbstractListeController2 m, 
			@ExecutionArgParam("parent") Component p, 
			@ExecutionArgParam("entite") Entite e 
			) {
	...
	public void selectGroup(@BindingParam("data") Object data) {
	    if(data instanceof TableGroupModel.TableInfo) {
	    	TableGroupModel.TableInfo groupInfo = (TableGroupModel.TableInfo) data;
	        ...
	        for(int childIndex = 0; childIndex < childCount; childIndex++) {
	            TableAnnotation table = groupModel.getChild(groupIndex, childIndex);
	            if(added) {
	...
	private List<TableAnnotation> getTables() {
		if (tables.isEmpty()) {
			if (!entite.getNom().equals("Cession")) {
				for (TableAnnotation tab : 
					ManagerLocator.getTableAnnotationManager()
							.findByBanquesManager(SessionUtils
								.getSelectedBanques(_sess.getAttributes()), true)) {
					if (tab.getEntite().getEntiteId() <= getEntite().getEntiteId()
							&& tab.getEntite().getEntiteId() != 5) { // tables cession exclues
						tables.add(tab);
					}
				}
			} else {
				for (TableAnnotation tab : ManagerLocator.getTableAnnotationManager()
						.findByBanquesManager(SessionUtils
								.getSelectedBanques(_sess.getAttributes()), true)) {
	...
	public class TableComparator implements Comparator<TableAnnotation>, 
								GroupComparator<TableAnnotation>, Serializable {
	    private static final long serialVersionUID = 1L;
	 
		@Override
	    public int compare(TableAnnotation t1, TableAnnotation t2) {
	        return t1.getNom().compareTo(t2.getNom());
	    }
	 
	    public int compareGroup(TableAnnotation t1, TableAnnotation t2) {
	...
	public void setRestrictedId(List<Integer> restrictedId) {
	...
	public void setListeController(AbstractListeController2 l) {
	...
	public void cancel() {
		Events
		.postEvent("onClose", 
				fwinRestrictTablesModale, null);
	}
	
	@Command
	public void export() {
		for (Object tb : groupModel.getSelection()) {
			if (tb instanceof TableAnnotation) {
				getRestrictedId().add(((TableAnnotation) tb)
									.getTableAnnotationId());
			}
		}
		// envoie null au controller si la liste est vide
		getListeController().setRestrictedTableIds(
				!getRestrictedId().isEmpty() ? getRestrictedId() : null);
