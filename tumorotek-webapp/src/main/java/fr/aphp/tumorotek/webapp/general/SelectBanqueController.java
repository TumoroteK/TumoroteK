...
public class SelectBanqueController extends GenericForwardComposer<Component> {
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		selectionComponents = new Component[] {
				this.rowPlateformeTitle,
				this.rowPlateforme,
				this.rowBanqueTitle,
				this.rowBanque,
				this.validate
		};
		archiveComponents = new Component[] {
				this.rowMdpArchive1,
				this.rowMdpArchive2,
				this.rowMdpArchive3
		};
		...
		if (user != null) {
			...
			if (archive) {
				...
				ManagerLocator.getUtilisateurManager()
						.archiveUtilisateurManager(user, user);
			} else {
				...
				if (res.containsKey("SAUVER_CONNEXION_TK")
						&& res.getString("SAUVER_CONNEXION_TK") != null
						&& res.getString("SAUVER_CONNEXION_TK").equals("true")) {
					// last connection
					Operation lastCx = ManagerLocator
							.getOperationManager()
							.findLastByUtilisateurAndTypeManager(
									ManagerLocator
											.getOperationTypeManager()
											.findByNomLikeManager("Login", true)
											.get(0), user, 1);
					if (lastCx != null) {
						lastCxValue = Labels.getLabel("login.last.date")
								+ " "
								+ new SimpleDateFormat("d MMM yyyy HH:mm:ss",
										Locale.getDefault()).format(lastCx
										.getDate().getTime());
					}
				...
				if (nbJours != null && nbJours < 31 && nbJours > 0) {
					labelMdpWarning.setContent(ObjectTypesFormatters.getLabel(
							"login.compte.warning",
							new String[] { String.valueOf(nbJours) }));
				...
				if (sessionScope.containsKey("tkobj")) {
					setSelectedBanque(((TKAnnotableObject) sessionScope
							.get("tkobj")).getBanque());
					setSelectedPlateforme(((TKAnnotableObject) sessionScope
							.get("tkobj")).getBanque().getPlateforme());
					for (int i = 0; i < selectionComponents.length; i++) {
						selectionComponents[i].setVisible(false);
					}
			...
			((Html) ldapErrorRow.getFirstChild())
				.setContent(ObjectTypesFormatters.getLabel("login.ldap.nulluser", 
						new String[] {ConnexionUtils.getLoggedLogin()}));
		}
	}
	
	public void initPlateformesAndBanques() {
		// on récupère les plateformes accessibles
		plateformes = ManagerLocator.getUtilisateurManager()
				.getAvailablePlateformesManager(user);
	...
	public void onClick$validate() {
		...
		if (selectedBanque != null) {
			if (!selectedBanque.equals(toutesColl)) {		
				ConnexionUtils.initConnection(user, selectedPlateforme, 
						selectedBanque, banques, session);
...
//			ConnexionUtils.initInterfacages(selectedPlateforme, sessionScope);
			...
			if (user.isSuperAdmin()) {
				...
				ConnexionUtils.generateDroitsForSelectedBanque(
						null, user, sessionScope);
				Executions.sendRedirect("/zuls/main/main.zul");
			}
		}
	}
		
	public void onClientInfo$winSelectBanque(ClientInfoEvent event) {
	 ...
	 * @throws Exception
	 */
	public void onSelect$plateformesBox(Event event) throws Exception {
			...
			if (!pfs.contains(selectedPlateforme) && !user.isSuperAdmin()) {
				...
				for (int i = 0; i < banques.size(); i++) {
					List<ProfilUtilisateur> liste = ManagerLocator
						...
						.findByUtilisateurBanqueManager(user, banques.get(i));
	...
	public void saveConnection() {
		...
		List<OperationType> tmp = ManagerLocator.getOperationTypeManager()
			.findByNomLikeManager("Login", true);
		if (tmp.size() > 0) {
			ManagerLocator.getOperationManager()
			.createObjectManager(creationOp, user,
					tmp.get(0), 
					user);
