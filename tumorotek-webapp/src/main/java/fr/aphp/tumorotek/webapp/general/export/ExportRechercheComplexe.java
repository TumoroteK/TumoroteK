...
public class ExportRechercheComplexe extends Export {
	...
	public void init(Desktop d, List<? extends Object> o, 
			boolean e, HtmlMacroComponent htmlMacroComponent, Affichage a, 
			String n, Utilisateur u) {
		desktop = d;
	...
	public void init() throws DesktopUnavailableException, InterruptedException, IOException  {
		...
		setExportDetails(null, 1, 3,"progressbar.data.collect", null,
				null);
		for (int i = 0; i < objs.size(); i++) {
			if (!Thread.interrupted()) {
				matriceObjets
				.add(RechercheUtils
						.getListeObjetsCorrespondants(objs.get(i), affichage, null));
				if (((int) Math.ceil((i * 100) / objs.size())) % 10 == 0
						&& !Thread.interrupted()) {
					setExportDetails((int) Math.ceil((i * 100) / objs.size()), null, null, 
							"progressbar.data.collect", null, null);
				}
			} else {
		...
		setExportDetails(null, 2, 3, 
				"progressbar.recherche.export.affichage", null
				, null);
...
//			
		...
		setExportDetails(null, 3, 3, 
				"progressbar.recherche.export.excel", null, null);
		RechercheUtils
			.exportTableExcel(getWb(), nomRecherche, matriceAffichable, affichage);
