...
public class StockageRootNode extends TumoTreeNode {
	...
	// @since 2.1
	private List<Conteneur> conteneurs = new ArrayList<Conteneur>();
	
	
	public StockageRootNode(List<Banque> banks) {
		this.banques = banks;
	}
	...
	public void readChildren() {
		conteneurs.addAll(ManagerLocator.getConteneurManager()
			.findByBanquesWithOrderManager(banques));
		
		Collections
			.sort(conteneurs, new Conteneur.NomComparator());
		...
		for (int i = 0; i < conteneurs.size(); i++) {
			...
			ConteneurNode node = new ConteneurNode(conteneurs.get(i), banque);
