...
public class GatsbiController
{
    ...
    * @return liste ids
    */
   public static List<Integer> getHiddenChampEntiteIdsForContexte(final Contexte c){
    ...
    * @return liste ids
    */
   public static List<Integer> getRequiredChampEntiteIdsForContexte(final Contexte c){
      final List<Integer> requiredIds = new ArrayList<>();
      if(c != null){
   ...
   public static List<Div> wireBlockDivsFromMainComponent(final ContexteType type, final Component main){
   ...
   public static List<Div> wireItemDivsFromMainComponent(final ContexteType type, final Component main){
   ...
   public static List<Div> wireItemDivsFromMainComponentByIds(final Component main, final String... ids){
      if(ids.length > 0){
   ...
   public static void showOrhideItems(final List<Div> items, final List<Div> blocks, final Contexte c){
      log.debug("showing or hiding items");
      if(items != null && c != null){

         final List<Integer> hidden = getHiddenChampEntiteIdsForContexte(c);

         for(final Div div : items){
            div.setVisible(
   ...
   public static void hideGroupBoxIfEmpty(final Component comp){

      comp.setVisible(comp.getChildren().stream().filter(c -> (c instanceof Div)).filter(c -> !c.getChildren().isEmpty())
         .anyMatch(c -> c.isVisible()));
   }

   public static void switchItemsRequiredOrNot(final List<Div> items, final Contexte c, final List<Listbox> lboxes,
      final List<Combobox> cboxes, final List<Div> reqDivs){
      log.debug("switch items required or not");
      if(items != null && c != null){

         final List<Integer> required = getRequiredChampEntiteIdsForContexte(c);
    ...
    * @param contextes
    */
   public static void showOrhideItems(final List<Div> itemDivs, final List<Div> blockDivs, final List<Contexte> contextes){
   ...
   public static void appliThesaurusValues(final List<Div> items, final Contexte contexte, final AbstractController controller)
      ...
      if(items != null && contexte != null){
         ...
         final List<Integer> thesaurii = contexte.getThesaurusChampEntiteIds();
    ...
    */
   public static void appliThesaurusValues(final List<Div> items, final List<Contexte> contextes,
      final AbstractController controller) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException{
      for(final Contexte c : contextes){
         if(c != null){
   ...
   public static <T> List<T> filterExistingListModel(final Contexte contexte, final List<T> lModel, final Integer chpId)
      throws TKException{

      final List<ThesaurusValue> values = contexte.getThesaurusValuesForChampEntiteId(chpId);
      Collections.sort(values,
         Comparator.comparing(ThesaurusValue::getPosition, Comparator.nullsLast(Comparator.naturalOrder())));

      final List<T> thesObjs = new ArrayList<>();
      if(lModel.contains(null)){
         thesObjs.add(null);
      }
      for(final ThesaurusValue val : values){
    ...
    * @param conformeDivs
    */
   public static void checkRequiredNonInputComponents(final List<Listbox> reqListboxes, final List<Combobox> reqComboboxes,
      final List<Div> reqDivs){

      if(reqListboxes != null){
         for(final Listbox lb : reqListboxes){
      ...
      if(reqComboboxes != null){
         for(final Combobox lb : reqComboboxes){
            Clients.clearWrongValue(lb);
            if(lb.getSelectedItem() == null){
               Clients.scrollIntoView(lb);
               throw new WrongValueException(lb, Labels.getLabel("validation.syntax.empty"));
      ...
      if(reqDivs != null){
         for(final Div div : reqDivs){
            ...
            if(div.getId().startsWith("conforme")){ // non conformite
               ...
               final CodeAssigneEditableGrid controller = (CodeAssigneEditableGrid) div.getLastChild().getFirstChild()
               ...
               final Decimalbox tb = (Decimalbox) div.getLastChild().getChildren().stream().filter(c -> c instanceof Decimalbox)
               ...
               if(tb.getValue() == null){
                  ...
                  final Listbox unitesBox = (Listbox) div.getLastChild().getChildren().stream().filter(c -> c instanceof Listbox)
                     .findFirst().orElse(null);
   ...
   private static Component findInputOrListboxElement(final Div item){
      ...
      if(item.getLastChild() instanceof InputElement || item.getLastChild() instanceof Listbox
         ...
         for(final Component child : item.getLastChild().getChildren()){
   ...
   public static Constraint muteConstraintFromContexte(Constraint constraint, final boolean required){
      if(constraint != null){
   ...
   public static List<ChampEntite> findByEntiteImportAndIsNullableManager(final Entite entite, final Boolean canImport,
      ...
      final List<ChampEntite> chpE = new ArrayList<>();
      ...
      if(contexte == null){ // TK defaut
         ...
         final List<Integer> hiddenIds = getHiddenChampEntiteIdsForContexte(contexte);
         ...
         if(isNullable != null){
            ...
            final List<Integer> reqIds = getRequiredChampEntiteIdsForContexte(contexte);
   ...
   public static List<ImportColonneDecorator> decorateImportColonnes(final List<ImportColonne> cols, final boolean isSubderive){
      ...
      final List<ImportColonneDecorator> decos = ImportColonneDecorator.decorateListe(cols, isSubderive);

      // surcharge la propriété deletable suivant le contexte gastbi
      Contexte c;
      for(final ImportColonneDecorator deco : decos){
         c = SessionUtils
   ...
   public static ContexteDTO mockOneContexteTEST() throws JsonParseException, JsonMappingException, IOException{
      ...
      final ContexteDTO cont = mapper.readValue(GatsbiController.class.getResourceAsStream("contexte.json"), ContexteDTO.class);

      return cont;
   }

   // test only
   public static ParametrageDTO mockOneParametrageTEST(final Integer pId)
    ...
    * @return visibilité
    */
   public static boolean isChampEntiteVisible(final ChampEntite c){
      if(c != null){
         final Contexte contexte = SessionUtils.getCurrentGatsbiContexteForEntiteId(c.getEntite().getEntiteId());
         if(contexte != null){
    ...
    */
   public static void doGastbiContexte(final Banque... banks) throws GatsbiException{

      if(banks.length > 0){
         final Etude etude = banks[0].getEtude(); // même en toutes collections, une seule étude !!

         final UriComponentsBuilder etudeURIBld = UriComponentsBuilder
            .fromUriString(TkParam.GATSBI_URL_BASE.getValue().concat(TkParam.GATSBI_URL_ETUDE_PATH.getValue()));

         final UriComponentsBuilder contexteURIBld = UriComponentsBuilder
         ...
         try{
            ...
            for(final Banque bq : banks){ // applique l'étude enrichie des contextes à toutes les banques
               bq.setEtude(etude);
            }
         }catch(final ResourceAccessException e){ // gatsbi inaccessible
    ...
    */
   public static Banque enrichesBanqueWithEtudeContextes(final Banque bank, final Map<?, ?> sessionScp){
   ...
   public static boolean doesGatsbiRespond(){
      ...
      final UriComponentsBuilder headURIBld = UriComponentsBuilder.fromUriString(TkParam.GATSBI_URL_BASE.getValue());
      ...
      try{
         ...
         return true;
      }catch(final Exception e){ // gatsbi inaccessible
    ...
    * @throws GatsbiException
    */
   public static ParametrageDTO doGastbiParametrage(final Integer pId) throws GatsbiException{

      try{
         final UriComponentsBuilder parametrageURIBld = UriComponentsBuilder
            .fromUriString(TkParam.GATSBI_URL_BASE.getValue().concat(TkParam.GATSBI_URL_PARAMETRAGE_PATH.getValue()));
    ...
    * @return
    */
   public static ResultatInjection injectGatsbiObject(final Contexte contexte, final ParametrageDTO param, final Banque banque,
      final Consumer<Parametrage> validator){
      try{
         // repose sur InjectionManager comme interfaçages
         // crée dossier externe pour le transport des données
         // values
         final ResultatInjection injection = new ResultatInjection();
         ...
         if(param != null){
            ...
            final BlocExterne bloc = new BlocExterne();
            ValeurExterne val;
            for(final ParametrageValueDTO value : param.getParametrageValueDTOs()){
               if(!contexte.getHiddenChampEntiteIds().contains(value.getChampEntiteId())
                  ...
                  if(value.getThesaurusTableNom() != null){ // thesaurus value check!
                     for(final String defvalue : value.getDefaultValue().split(";")){
    ...
    * @return
    */
   public static Contexte initWireAndDisplay(final AbstractController controller, final Integer entiteId, final boolean edit,
      final List<Listbox> reqListboxes, final List<Combobox> reqComboboxes, final List<Div> reqDiv,
      final Groupbox... groupsToHideWhenEmpy){
      try{
         final Contexte c = SessionUtils.getCurrentGatsbiContexteForEntiteId(entiteId);

         final Div gatsbiContainer = (Div) controller.getSelfComponent().getFellow("gatsbiContainer");

         // wire
         final List<Div> itemDivs = wireItemDivsFromMainComponent(c.getContexteType(), gatsbiContainer);
         final List<Div> blockDivs = wireBlockDivsFromMainComponent(c.getContexteType(), gatsbiContainer);
         ...
         for(final Groupbox gp : groupsToHideWhenEmpy){
    ...
    * @return
    */
   public static void initWireAndDisplayForIds(final AbstractController controller, final Integer entiteId, final String... ids){
      try{
         final Contexte c = SessionUtils.getCurrentGatsbiContexteForEntiteId(entiteId);
         ...
         // wire
         final List<Div> itemDivs = wireItemDivsFromMainComponentByIds(gatsbiContainer, ids);
    ...
    * @throws IllegalAccessException
    */
   public static Column addColumn(final Grid grid, final String nameKey, final String width, final String align,
      final Component child, final String sort, final Boolean visible)
   ...
   // add new refactor
   public static void addNewObjectForContext(final Contexte contexte, final Component selfComponent,
      final Consumer<Event> elseMethod, final Event evt, final TKAnnotableObject parentObj){
   ...
   @SuppressWarnings("unchecked")
   public static void getSelectedParametrageFromSelectEvent(final Contexte contexte, final Banque banque,
      final AbstractObjectTabController objectTabController, final Consumer<Parametrage> validator, final Runnable elseMethod,
      final ForwardEvent evt) throws GatsbiException{

      ResultatInjection inject = null;
      if(((Map<String, Object>) evt.getOrigin().getData()).get("paramId") != null){
         final ParametrageDTO parametrageDTO =
