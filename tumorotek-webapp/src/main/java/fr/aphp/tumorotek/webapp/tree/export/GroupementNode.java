...
public class GroupementNode extends ExportNode {
	...
	public void setNode1(ExportNode n1) {
		this.node1 = n1;
	}
	public ExportNode getNode2() {
		return node2;
	}
	public void setNode2(ExportNode n2) {
		this.node2 = n2;
	}
	public Integer getOperateur() {
		return operateur;
	}
	public void setOperateur(Integer op) {
		this.operateur = op;
	}
	public Groupement getGroupement() {
		return groupement;
	}
	public void setGroupement(Groupement g) {
		this.groupement = g;
	}
	 ...
	 */
	public ExportNode getChild(int arg1) {
	 ...
	 */
	public boolean addCritere(Critere critere, CritereNode frere,
			Integer operateur) {
		...
		if (frere == null) {
			...
			GroupementNode nodeRef = getGroupementNode(frere);
			if (nodeRef.getNode1().equals(frere)) {
				if (nodeRef.getNode2() == null) {
					...
					GroupementNode newNode = new GroupementNode();
	...
	public void insertToNode1(ExportNode e) {
		setNode1(e);
		e.setParent(this);
	}
	
	public void insertToNode2(ExportNode e) {
		setNode2(e);
		e.setParent(this);
	}
	 ...
	 */
	public GroupementNode getGroupementNode(ExportNode eNode) {
			...
			if (getChild(1) != null && getChild(1).equals(eNode)) {
				...
				if (this.getChild(1) instanceof GroupementNode) {
					gNode = ((GroupementNode) this.getChild(1))
							.getGroupementNode(eNode);
				}
				if (gNode == null && this.getChild(2) instanceof GroupementNode) {
					gNode = ((GroupementNode) this.getChild(2))
							.getGroupementNode(eNode);
				}
			}
			return gNode;
	...
	public List<Critere> removeNode(ExportNode node) {
		List<Critere> crits = new ArrayList<Critere>();
		if (getNode1() != null && getNode1().equals(node)) {
			/** Si on supprime le noeud 1 */
			if (getNode1() instanceof GroupementNode) {
				crits.addAll(((GroupementNode) getNode1())
						.removeNode(((GroupementNode) getNode1()).getNode1()));
				crits.addAll(((GroupementNode) getNode1())
						.removeNode(((GroupementNode) getNode1()).getNode2()));
			...
			if (getNode2() instanceof GroupementNode) {			
				crits.addAll(((GroupementNode) getNode2())
						.removeNode(((GroupementNode) getNode2()).getNode1()));
				crits.addAll(((GroupementNode) getNode2())
						.removeNode(((GroupementNode) getNode2()).getNode2()));
	...
	public List<CritereNode> getLeaves() {
		List<CritereNode> crNodes = new ArrayList<CritereNode>();
		if (getNode1() != null) {
			if (getNode1().isLeaf()) {
	...
	public Groupement convertToGroupement() {
		...
		if (getNode1() != null) {
			if (getNode1() instanceof CritereNode) {
				if (((CritereNode) getNode1()).getCritere().getCritereId()
						!= null) {
					groupement.setCritere1(ManagerLocator.getCritereManager()
							.copyCritereManager(
									((CritereNode) getNode1()).getCritere()));
				} else {
					groupement.setCritere1(((CritereNode) getNode1())
							.getCritere());
				}
			}
		}
		if (getNode2() != null) {
			if (getNode2() instanceof CritereNode) {
				if (((CritereNode) getNode2()).getCritere().getCritereId()
						!= null) {
					groupement.setCritere2(ManagerLocator.getCritereManager()
							.copyCritereManager(
									((CritereNode) getNode2()).getCritere()));
				} else {
					groupement.setCritere2(((CritereNode) getNode2())
							.getCritere());
				}
			}
		}
		ManagerLocator.getGroupementManager().createObjectManager(
				groupement, groupement.getCritere1(), groupement.getCritere2(),
				groupement.getOperateur(), groupement.getParent());
		if (getNode1() != null) {
			if (getNode1() instanceof GroupementNode) {
				Groupement enfant = ((GroupementNode)getNode1()).convertToGroupement();
				enfant.setParent(groupement);
				ManagerLocator.getGroupementManager().updateObjectManager(
						enfant, enfant.getCritere1(), enfant.getCritere2(),
						enfant.getOperateur(), enfant.getParent());
			}
		}
		if (getNode2() != null) {
			if (getNode2() instanceof GroupementNode) {
				Groupement enfant = ((GroupementNode)getNode2()).convertToGroupement();
				enfant.setParent(groupement);
				ManagerLocator.getGroupementManager().updateObjectManager(
						enfant, enfant.getCritere1(), enfant.getCritere2(),
						enfant.getOperateur(), enfant.getParent());
	...
	public static GroupementNode convertFromGroupement(Groupement racine, GroupementNode parent) {
		GroupementNode n = new GroupementNode();
		Groupement groupement = ManagerLocator.getGroupementManager().findByIdManager(racine.getGroupementId());
		n.setGroupement(groupement);
		n.setParent(parent);
		//On récupère sa liste des enfants
		List<Groupement> enfants = ManagerLocator.getGroupementManager().findEnfantsManager(groupement);
