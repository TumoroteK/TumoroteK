...
public final class ExportUtils {
	 ...
	 */
	public static void downloadExportFileXls(Workbook wb, String fileName,
			ByteArrayOutputStream out, Desktop desktop, Integer allCollection)
			throws IOException, DesktopUnavailableException,
			InterruptedException {
		wb.write(out);
		AMedia media = new AMedia(fileName, "xlsx",
				(allCollection == 0) ? ConfigManager.OFFICE_EXCEL_MIME_TYPE
	...
	public static void downloadExportFileXls(Workbook wb, String fileName,
			ByteArrayOutputStream out, Desktop desktop) throws IOException,
			DesktopUnavailableException, InterruptedException {
		wb.write(out);
		AMedia media = new AMedia(fileName, "xlsx",
				ConfigManager.OFFICE_OPENXML_MIME_TYPE, out.toByteArray());
	...
	public static HSSFWorkbook exportPrelevements(List<Prelevement> prlvts,
			List<Banque> banques, boolean isExportAnonyme, Utilisateur user) {
		// création de la feuille excell
		HSSFWorkbook wb = ManagerLocator.getExportUtils().createExcellWorkBook(
				"Prelevements");
		HSSFSheet sheet = wb.getSheetAt(0);

		// ajout des entêtes du ficher
		int nbRow = addEnteteForExportFile(sheet, "export.prelevements",
				banques);
		...
		List<ChampAnnotation> casPrlvt = getAnnotationsByBanquesAndEntite(
				banques, entite);
		...
		entite = ManagerLocator.getEntiteManager().findByNomManager("Patient")
				.get(0);
		List<ChampAnnotation> casPatient = getAnnotationsByBanquesAndEntite(
				banques, entite);
		// ajout du nom des annotations aux entêtes de colonnes
		...
		for (int i = 0; i < prlvts.size(); i++) {
			...
			ManagerLocator.getExportUtils().addPrelevementData(row, wb, 0,
					prlvts.get(i), casPrlvt, isExportAnonyme, user);

			// si le prlvt a un patient
			if (prlvts.get(i).getMaladie() != null) {
				// ajout des données du patient
				int idxCell = row.getLastCellNum();
				ManagerLocator.getExportUtils().addMaladieData(row, wb,
						idxCell, prlvts.get(i).getMaladie(), casPatient,
	...
	public static int addEnteteForExportFile(HSSFSheet sheet, String key,
		...
		if (banques.size() == 1) {
			...
			tmp.clear();
			tmp.add(ObjectTypesFormatters.getLabel("export.collection",
	...
	public static List<String> addEnteteForPatients(List<String> entetes) {
		...
		entetes.add(ObjectTypesFormatters.getLabel("maladie.medecin.numero",
				new String[] { "1" }));
		entetes.add(ObjectTypesFormatters.getLabel("maladie.medecin.numero",
				new String[] { "2" }));
		entetes.add(ObjectTypesFormatters.getLabel("maladie.medecin.numero",
		...
		entetes.add(ObjectTypesFormatters.getLabel("patient.medecin.numero",
				new String[] { "1" }));
		entetes.add(ObjectTypesFormatters.getLabel("patient.medecin.numero",
				new String[] { "2" }));
		entetes.add(ObjectTypesFormatters.getLabel("patient.medecin.numero",
	 ...
	 * @return Liste complétée des entêtes.
	 */
	public static List<String> addEnteteForPrelevements(List<String> entetes) {
