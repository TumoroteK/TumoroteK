...
public final class ConnexionUtils {
	...
	public static void setSessionCatalogues(List<Banque> banques, 
												Map<String, Object> sess) {
		
		Map<String, Catalogue> catsMap = new HashMap<String, Catalogue>();
		Collection<Catalogue> cats = ManagerLocator.getCatalogueManager()
		...
		if (banques != null) {
			...
			for (Banque bank : banques) {
				cats = CollectionUtils.intersection(cats, 
						ManagerLocator.getCatalogueManager()
							.findByAssignedBanqueManager(bank));
			}
			Iterator<Catalogue> catsIt = cats.iterator();
			Catalogue cat;
	 ...
	 */
	public static void generateDroitsForSelectedBanque(Banque bk,
			Utilisateur user, Map<String, Object> sess) {
		...
		if (user.isSuperAdmin() || pfs.contains(bk.getPlateforme())) {
			...
			// sélectionnée
			List<ProfilUtilisateur> profs =  ManagerLocator
				.getProfilUtilisateurManager()
				.findByUtilisateurBanqueManager(
						user, bk);
			
			if (!profs.isEmpty()) {
				Profil profil = profs.get(0).getProfil();
	 ...
	 */
	public static Hashtable<String, List<OperationType>> 
									getOperationsForProfil(Profil profil) {
		Hashtable<String, List<OperationType>> operations = 
	...
	public static void initInterfacages(Plateforme pf,
			Map<String, Object> sess) {
    	...
    	if (interfacages != null && !interfacages.equals("")
    			&& !interfacages.equals("false")) {
    		// extraction des interfaçages
    		hashInterfacages = Utils
    			.extractAssosPlateformesEmetteursRecepteurs(interfacages);
    	}
    	// si des interfaçages sont définis pour la PF sélectionnée
    	if (hashInterfacages != null && 
    			hashInterfacages.containsKey(pf.getPlateformeId())) {
    		List<Integer> ids = hashInterfacages.get(
    				pf.getPlateformeId());
    		List<Emetteur> emetteurs = ManagerLocator.getEmetteurManager()
    			.findByIdinListManager(ids);
    	...
    	if (hashInterfacages != null 
    		...
    		List<Integer> ids = hashInterfacages.get(
    				pf.getPlateformeId());
    		List<Recepteur> recepteurs = ManagerLocator.getRecepteurManager()
    			.findByIdinListManager(ids);
    		// on place les emetteurs dans la session
	...
	public static Utilisateur getLoggedUtilisateur() {
		...
		// if (us != null) {
			list = ManagerLocator.getUtilisateurManager()
				.findByLoginManager(username);
		// }
	...
	public static void initConnection(Utilisateur user, 
			Plateforme pf, Banque bank, 
			List<Banque> banques, Session session) {
		Map<String, Object> sessionScp = session.getAttributes();
