...
public class EnceinteNode extends TumoTreeNode {
	...
	public EnceinteNode(Enceinte enc, Long nbLibres, 
			Long nbOccupes, Banque bank) {
		this.enceinte = enc;
		this.selectedBanque = bank;
		
		if (this.enceinte != null
				&& !this.enceinte.equals(new Enceinte())) {
			vide = false;
			if (nbLibres != null) {
				nbEmplacementsLibres = nbLibres;
			} else {
				nbEmplacementsLibres = ManagerLocator
						.getEnceinteManager()
						.getNbEmplacementsLibresByPS(enceinte);
			}
			
			if (nbOccupes != null) {
				nbEmplacementsOccupes = nbOccupes;
			} else {
				nbEmplacementsOccupes = ManagerLocator
						.getEnceinteManager()
						.getNbEmplacementsOccupesByPS(enceinte);
			}
			
			StringBuffer sb = new StringBuffer();
			sb.append(this.enceinte.getNom());
			if (this.enceinte.getAlias() != null
				&& !this.enceinte.getAlias().equals("")) {
				sb.append(" {");
	...
	public void readChildren() {
		...
		if (!vide && childrenAvailable()) {
			if (ManagerLocator.getEnceinteManager()
					.checkLastEnceinte(enceinte)) {
				List<Terminale> terminales = ManagerLocator
					.getTerminaleManager()
					.findByEnceinteWithOrderManager(enceinte);
				
				int cpt = 1;
				for (int i = 0; i < terminales.size(); i++) {
					Terminale term = terminales.get(i);
					...
					while (term.getPosition() > cpt) {
						if (!isHideComplete()) {
							TerminaleNode node = new TerminaleNode(
									new Terminale(), selectedBanque);
				...
				List<Enceinte> enceintes = ManagerLocator
					.getEnceinteManager()
					.findByEnceintePereWithOrderManager(enceinte);
				
				int cpt = 1;
				for (int i = 0; i < enceintes.size(); i++) {
					Enceinte enc = enceintes.get(i);
					// si des positions ne sont pas occupées par des enceintes
					// on les remplit par des emplacements vides
					while (enc.getPosition() > cpt) {
						if (!isHideComplete()) {
							EnceinteNode node = new EnceinteNode(
									new Enceinte(), null, null, selectedBanque);
					...
					// Enceinte non vide
					EnceinteNode node = new EnceinteNode(
							enc, null, null, selectedBanque);
					// n'ajoute pas le node si hideComplete et nb libres
					// égal à 0
					if (!isHideComplete() 
							|| node.getNbEmplacementsLibres() > 0) {
						node.setConteneur(conteneur);
						node.setNiveau(niveau + 1);
	...
	public boolean equals(Object obj) {
		...
		EnceinteNode node = (EnceinteNode) obj;
	...
	public void setNbEmplacementsLibres(Long nbLibres) {
	...
	public void setNiveau(Integer nv) {
		this.niveau = nv;
	}
	...
	public void setNbEmplacementsOccupes(Long nbOccupes) {
