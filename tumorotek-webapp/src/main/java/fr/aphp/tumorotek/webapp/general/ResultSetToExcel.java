...
public class ResultSetToExcel {
	...
	private static final int INDEX_PREL_ID = 26;
	// ecart entre colonnes permettant de retrouver l'id de l'echan/derive à partir de la colonne evts_stock
	private static final int INDEX_ECHAN_ID = 25;
	private static final int INDEX_DERIVE_ID = 28;
	...
	private ArrayList<Integer> anonymeColumn = new ArrayList<Integer>();
	private ArrayList<Integer> labo_interColumn = new ArrayList<Integer>();
	private ArrayList<Integer> multipleValuesColumn = new ArrayList<Integer>();
	...
	public ResultSetToExcel(SXSSFWorkbook wb, Export export, 
			ResultSet resultSet, ResultSet rs2,
			ResultSet erRs, ResultSet drRs,
			boolean isExportAnonyme, boolean hasP, String sheetName,
			boolean isCession) {
		workbook = wb;
		this.export = export;
		...
		this.isExportAnonyme = isExportAnonyme;
		hasPrelevement = hasP;
		resultLaboInter = rs2;
		retourEchanRs = erRs;
		retourDeriveRs = drRs;
	...
	public ResultSetToExcel(Export export, 
			ResultSet resultSet, 
			boolean isExportAnonyme) {
		setExport(export);
		setResultSet(resultSet);
		setExportAnonyme(isExportAnonyme);	
	}
	...
	public void generate() throws Exception {
		...
		for (int i = 0; i < numCols; i++) {
			...
			String title = (export.getExportType() == 1) ? resultSetMetaData.getColumnName(i + 1)
														: resultSetMetaData.getColumnLabel(i + 1);
			if (export.getMapCorrespondanceAnnotationName() != null
					&& export.getMapCorrespondanceAnnotationName()
							.containsKey(title)) {
				title = export.getMapCorrespondanceAnnotationName().get(
						title);
				isAnno = true;
			} else {
				isAnno = false;
			...
			// liste deroulante
			if (title.equalsIgnoreCase("echantillons")
					|| title.equalsIgnoreCase("produits_derives")) {
	...
	private void writeTKcolNames() {
		...
		"PATIENT.SEXE",
		"PRELEVEMENT.STATUT JURIDIQUE",
		"PATIENT.ETAT",
		"PRELEVEMENT.ETABLISSEMENT PRELEVEUR",
		"ETABLISSEMENT.FINESS", 
		"PRELEVEMENT.SERVICE PRELEVEUR",
		"PRELEVEMENT.CODE",
		"PRELEVEMENT.DATE PRELEVEMENT",
		"PRELEVEMENT.DATE PRELEVEMENT",
		"PRELEVEMENT.DATE ARRIVEE",
		"PRELEVEMENT.NATURE",
		"PRELEVEMENT.TYPE",
		"",
		"ECHANTILLON.CODES ORGANES",
		"ECHANTILLON.CODES LESIONNELS",
		"",
		"ECHANTILLON.CODES ORGANES",
		"ECHANTILLON.CODES LESIONNELS",
		"ANNOTATION_PREL.Taille tumeur : cT",
		"ANNOTATION_PREL.Envahissement ganglionnaire : CN",
		"ANNOTATION_PREL.Version pTNM",
		"ANNOTATION_PREL.Taille tumeur : pT",
		"ANNOTATION_PREL.Envahissement ganglionnaire : pN",
		"ANNOTATION_PREL.pM",
		"ANNOTATION_PREL.022 TYPE EVENT", 
		"ECHANTILLON.CODE",
		"ECHANTILLON.TUMORAL ou ECHANTILLON.TYPE",
		"CONTENEUR.TEMPERATURE",
		"ECHANTILLON.TYPE",
		"ECHANTILLON.MODE PREPARATION",
		"ECHANTILLON.DELAI CONGELATION",
		"ECHANTILLON.DATE_STOCKAGE",
		"ECHANTILLON.DATE_STOCKAGE",	
		"ANNOTATION_ECH.032 Controle tissu",
		"",
		"",
		"ANNOTATION_ECH.035 Pourcentage cellules tumorales",
		"PRODUIT_DERIVE.TYPE",
		"",
		"PRODUIT_DERIVE.TYPE",
		"",
		"PRODUIT_DERIVE.TYPE",
		"CONTENEUR.SERVICE.ETABLISSEMENT.NOM",
		"CONTENEUR.SERVICE.ETABLISSEMENT.FINESS",
		"", 
		"",
		"",
		"",
		"",
		"ANNOTATION_ECH.053 ADN constitutionnel",
		"", 
		"", 
		"ANNOTATION_PREL.CR standardise interroegable",
		"ANNOTATION_PREL.Donnees cliniques",
		"ANNOTATION_PREL.Inclusion protocole thérapeutique",
		"ANNOTATION_PREL.Inclusion projet recherche",
		"ECHANTILLON.STATUT",
		"",
		"",
		"",
		"",
		"PRELEVEMENT.NUMERO_LABO"};
		for (int i = 0;  i < tkcols.length; i++) {
			writeCell(row, i, tkcols[i], FormatType.TEXT);
		}
	}

	protected FormatType getFormatType(Class<?> _class, int precision) {
		if (_class == Boolean.class) {
			return FormatType.BOOL;
		} else if (_class == Date.class || _class == Timestamp.class) {
			return FormatType.DATE;
		} else if (_class == Integer.class || _class == Float.class || _class == Long.class
				|| _class == BigDecimal.class || Number.class.isAssignableFrom(_class)) {
	...
	protected void writeCell(Row row, int col, Object value,
			FormatType formatType, XSSFCellStyle st) {
		writeCell(row, col, value, formatType, null, st);
	}

	protected void writeCell(Row row, int col, Object value,
			FormatType formatType, Short bgColor, XSSFCellStyle st) {
		...
		if (value != null) {
			...
			if (isExportAnonyme && this.anonymeColumn.contains(col) 
					&& row.getRowNum() != 0) {
				...
				if (formatType.equals(FormatType.BOOL) && row.getRowNum() != 0) {
					...
					if (value instanceof BigDecimal) {
						...
						if (((BigDecimal) value).compareTo(BigDecimal.ZERO) > 0) {
							cell.setCellValue(ObjectTypesFormatters
			...
			if (col == retourD_interColumn && row.getRowNum() != 0) {
				if (!value.toString().contentEquals("0")) {
					cell.setCellType(Cell.CELL_TYPE_STRING);
					createHyperlink("id_rd_", cell, col, row, DETAILS_RETOUR, 
							INDEX_DERIVE_ID + (cessionExport ? 2 : 0));
				}
			}
	//		if (bgColor != null) {
	...
	private void createLaboInterSheet() 
			throws SQLException, ClassNotFoundException, InterruptedException {
		if (resultLaboInter.isBeforeFirst()) {
			// create a target sheet and cell
			Sheet sheet2 = workbook.createSheet(LABO_INTER_SHEET);
			int currentRow = 0;
			int numCols = 0;
			String title = "";
			ResultSetMetaData resultSetMetaData = resultLaboInter.getMetaData();
			...
			while (resultLaboInter.next()) {
				...
				for (int i = 0; i < numCols; i++) {
					Object value = resultLaboInter.getObject(i + 1);
					if (i == 0) {
						prelId = (Number) value;
					}
					writeCell(row, i, value, formatTypes[i]);
				...
				if (prev_prelId != null && !prelId.equals(prev_prelId)) {
					...
					if (cellRange[1] > cellRange[0]) {
						reference = LABO_INTER_SHEET + "!A" + cellRange[0]
								+ ":L" + cellRange[1];
					} else {
						reference = LABO_INTER_SHEET + "!A" + cellRange[0]
								+ ":L" + cellRange[0];
			...
			if (cellRange[1] > cellRange[0]) {
				reference = LABO_INTER_SHEET + "!A" + cellRange[0] + ":L"
						+ cellRange[1];
			} else {
				reference = LABO_INTER_SHEET + "!A" + cellRange[0] + ":L"
						+ cellRange[0];
	...
	private void createRetourSheet(int entiteId) 
			throws SQLException, ClassNotFoundException, InterruptedException {
		// create a target sheet and cell
		String sheetName = entiteId == 3 ? RETOUR_ECHAN_SHEET : RETOUR_DERIVE_SHEET;
		ResultSet rs = entiteId == 3 ? retourEchanRs : retourDeriveRs;
		String suff = entiteId == 3 ? "id_re_" : "id_rd_";
		...
		if (rs.isBeforeFirst()) {
			...
			while (rs.next()) {
				...
				if (prev_objId != null && !objId.equals(prev_objId)) {
					cellRange[1] = currentRow - 1;
					namedCell = workbook.createName();
					namedCell.setNameName(suff + prev_objId);
					if (cellRange[1] > cellRange[0]) {
						reference = sheetName + 
								"!A" + cellRange[0] + ":M" + cellRange[1];
					} else {
						reference = sheetName + "!A" 
								+ cellRange[0] + ":M" + cellRange[0];
					}
					namedCell.setRefersToFormula(reference);
	...
	private void createHyperlink(String suff, 
			Cell cell, int col, Row row, String text, int idEcartCol) {	
		cell.setCellValue(text);
		org.apache.poi.ss.usermodel.Hyperlink link2 = createHelper
				.createHyperlink(Hyperlink.LINK_DOCUMENT);
						
		link2.setAddress(suff
				+ String.valueOf((int) (row.getCell(col - idEcartCol)
						.getNumericCellValue())));
	...
	protected static String stringToBool(String s) {
		if (s.equals("1"))
			return "OUI";
		if (s.equals("0"))
			return "NON";
...
//	}
	...
	public void setUpdateThread(Export u) {
		this.updateThread = u;
	}
	...
	private String labelPrintTitle(String title, String entite) {
		...
		if (title.contains("_")) {
			...
			String endS = title.substring(title.indexOf("_") + 2);
			if (endS.contains("_")) {
		...
		if (entite.equals("Patient")) {
			...
			if (title.equals("CODE_ORGANE")) {
				...
				return Labels.getLabel("prelevement.nbEchantillons") 
				+ " " + Labels.getLabel("prelevement.echantillons.stockes");
	...
	public void setExport(Export e) {
		this.export = e;
	}
	...
	public void setExportAnonyme(boolean ie) {
		this.isExportAnonyme = ie;
	}
	...
	public void setAnonymeColumn(ArrayList<Integer> ac) {
	...
	public void setFormatTypes(FormatType[] ft) {
		this.formatTypes = ft;
	}
}
