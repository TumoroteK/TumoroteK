	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			if (req.getParameter("id") != null) {
				Integer prelID = new Integer(req.getParameter("id")) ;
				
				Prelevement prel = ManagerLocator.getPrelevementManager()
						.findByIdManager(prelID);
				
				if (prel != null) {
					// verifie que l'utilisateur a accès au prélèvement
					if (ManagerLocator.getUtilisateurManager()
						.getAvailableBanquesManager(ConnexionUtils
												.getLoggedUtilisateur())
												.contains(prel.getBanque())) {
						req.getSession().setAttribute("tkobj", prel);
						resp.sendRedirect(resp.encodeRedirectURL(req.getContextPath() 
															+ "/login/SelectBanque.zul"));
