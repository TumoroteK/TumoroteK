...
public class CodeNode extends TumoTreeNode
{
    ...
    * @param b isTopo
    */
   public CodeNode(final CodeCommon c, final TableCodage t, final Boolean b){
      this.code = c;
      this.codification = t;
      this.isTopo = b;
   }

   public CodeNode(final CodeDossier d, final TableCodage t){
      this.dossier = d;
      this.codification = t;
   }
    ...
    */
   public CodeNode(final AdicapGroupe groupe, final TableCodage t){
   ...
   public void setCodification(final TableCodage t){
      this.codification = t;
   }
   ...
   public void setCode(final CodeCommon c){
      this.code = c;
   }

   public void setDossier(final CodeDossier dos){
      this.dossier = dos;
   }

   public void setCodeSelect(final CodeSelect cSelect){
   ...
   public void setParentItem(final Treeitem it){
      this.parentItem = it;
   }
   ...
   public void setRoot(final boolean ir){
      this.isRoot = ir;
   }
   ...
   public void readChildren(){
      ...
      if(adicapGroupe != null){
         childrenGroupes = ManagerLocator.getAdicapManager().getAdicapGroupesManager(adicapGroupe);
         childrenCodes = ManagerLocator.getAdicapManager().findByAdicapGroupeManager(adicapGroupe);
      }else if(code != null){
         if("ADICAP".equals(codification.getNom())){
            childrenCodes = ManagerLocator.getAdicapManager().findByAdicapParentManager((Adicap) code, isTopo);
         }else if("CIM_MASTER".equals(codification.getNom())){
            childrenCodes = ManagerLocator.getCimMasterManager().findByCimParentManager((CimMaster) code);
         }else if("UTILISATEUR".equals(codification.getNom())){
            childrenCodes = ManagerLocator.getCodeUtilisateurManager().findByCodeParentManager((CodeUtilisateur) code);
         }
      }else if(dossier != null){
         childrenDossiers = ManagerLocator.getCodeDossierManager().findByCodeDossierParentManager(dossier);
         if("UTILISATEUR".equals(codification.getNom())){
            childrenCodes = ManagerLocator.getCodeUtilisateurManager().findByCodeDossierManager(dossier);
         }else if("FAVORIS".equals(codification.getNom())){
            childrenCodes = ManagerLocator.getCodeSelectManager().findCodesFromSelectByDossierManager(dossier);
         }
      }

      // renvoie les codes en premier
      for(int i = 0; i < childrenCodes.size(); i++){
         final CodeNode node = new CodeNode(childrenCodes.get(i), codification, isTopo);
         children.add(node);
      }
      for(int i = 0; i < childrenGroupes.size(); i++){
         final CodeNode node = new CodeNode(childrenGroupes.get(i), codification);
         children.add(node);
      }
      for(int i = 0; i < childrenDossiers.size(); i++){
         final CodeNode node = new CodeNode(childrenDossiers.get(i), codification);
         children.add(node);
      }
   ...
   public boolean equals(final Object obj){
      ...
      final CodeNode node = (CodeNode) obj;
