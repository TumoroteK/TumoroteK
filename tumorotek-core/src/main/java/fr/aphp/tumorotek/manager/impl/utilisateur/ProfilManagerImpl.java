...
public class ProfilManagerImpl implements ProfilManager {
	...
	private Log log = LogFactory.getLog(ProfilManager.class);
	...
	public void setDroitObjetManager(DroitObjetManager dManager) {
	...
	public void setProfilValidator(ProfilValidator pValidator) {
	...
	@Override
	public Profil findByIdManager(Integer profilId) {
	...
	@Override
	public Boolean findDoublonManager(Profil profil) {
		if (profil != null) {
			if (profil.getProfilId() == null) {
				return profilDao.findByNom(profil.getNom()).contains(profil);
			} else {
				return profilDao.findByExcludedId(
						profil.getProfilId()).contains(profil);
			}
		} else {
			return false;
		}
	...
	public Boolean isUsedObjectManager(Profil profil) {
		if (profil != null && profil.getProfilId() != null) {
			...
			return (profilUtilisateurManager
					.findByProfilManager(profil, null).size() > 0);
		} else {
			return false;
		}
	}
	...
	@Override
	public void createObjectManager(Profil profil, 
		...
		if (!findDoublonManager(profil)) {
			...
			BeanValidator.validateObject(
					profil, new Validator[]{profilValidator});
			
			// validation des droitsobjets
			if (droitObjets != null) {
				for (int i = 0; i < droitObjets.size(); i++) {
					DroitObjet obj = droitObjets.get(i);
			...
			if (droitObjets != null) {
				for (int i = 0; i < droitObjets.size(); i++) {
					DroitObjet obj = droitObjets.get(i);
					droitObjetManager.createObjectManager(
							obj, 
							profil, 
							obj.getEntite(), 
							obj.getOperationType());
			...
			log.warn("Doublon lors creation objet Profil "
					+ profil.toString());
	...
	@Override
	public void updateObjectManager(Profil profil, 
			List<DroitObjet> droitObjets,
			Utilisateur admin) {
		List<DroitObjet> droitsToCreate = new ArrayList<DroitObjet>();
		List<DroitObjet> droitsToRemove = new ArrayList<DroitObjet>();
		List<DroitObjet> oldDroits = new ArrayList<DroitObjet>();
		//Doublon
		if (!findDoublonManager(profil)) {
			...
			if (droitObjets != null) {
				for (int i = 0; i < droitObjets.size(); i++) {
					...
					if (droitObjetManager.findByIdManager(
							obj.getPk()) == null) {
						droitObjetManager.validateObjectManager(
								profil, 
								obj.getEntite(), 
			...
			if (droitObjets != null) {
				...
				for (int i = 0; i < droitsToRemove.size(); i++) {
					droitObjetManager.removeObjectManager(
							droitsToRemove.get(i));
				}
				
				// enregistrements des droitsobjets
				for (int i = 0; i < droitsToCreate.size(); i++) {
					DroitObjet obj = droitsToCreate.get(i);
					droitObjetManager.createObjectManager(
							obj, 
							profil, 
							obj.getEntite(), 
	...
	@Override
	public void removeObjectManager(Profil profil) {
		if (profil != null) {
			if (isUsedObjectManager(profil)) {
				log.warn("Objet utilisé lors de la suppression de l'objet " 
						+ "Profil : " + profil.toString());
				throw new ObjectUsedException("deletion.profil.isUsed", 
																	false);
			} else {
				// suppression des DroitsObjets
				List<DroitObjet> objets = droitObjetManager
				...
				List<ProfilUtilisateur> profils = profilUtilisateurManager
					.findByProfilManager(profil, null);
				for (int i = 0; i < profils.size(); i++) {
					profilUtilisateurManager
						.removeObjectManager(profils.get(i));
				}
				
				profilDao.removeObject(profil.getProfilId());
				log.info("Suppression de l'objet Profil : " 
						+ profil.toString());
				
				//Supprime operations associes
				List<Operation> ops = operationManager
									.findByObjectManager(profil);
	...
	@Override
	public List<Profil> findByPlateformeAndArchiveManager(Plateforme pf, Boolean archive) {
		if (archive != null) {
			return profilDao.findByPlateformeAndArchive(pf, archive);
		} else {
			List<Profil> prfs = profilDao.findByPlateformeAndArchive(pf, false);
