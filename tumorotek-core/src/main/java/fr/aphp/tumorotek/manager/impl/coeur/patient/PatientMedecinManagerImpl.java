...
import fr.aphp.tumorotek.manager.validation.coeur.patient
													.PatientMedecinValidator;
...
public class PatientMedecinManagerImpl implements PatientMedecinManager {
	...
	private Log log = LogFactory.getLog(PatientMedecinManager.class);
	...
	public void setPatientMedecinValidator(
										PatientMedecinValidator pmValidator) {
		this.patientMedecinValidator = pmValidator;
	}

	@Override
	public void createOrUpdateObjectManager(PatientMedecin medecin,
			Patient patient, Collaborateur collaborateur, String operation) {
		...
		checkRequiredObjectsAndValidate(medecin, 
											patient, collaborateur, operation);
		...
		if (!findDoublonManager(medecin)) {
			if (operation.equals("creation")) {
				//if (operation.equals("creation")) {
					...
					log.debug("Enregistrement objet PatientMedecin " 
													+ medecin.toString());
...
//				}
			...
			if (operation.equals("modification") 
					&& !medecin.getOrdre()
						.equals(patientMedecinDao.findById(medecin.getPk())
																.getOrdre())) {
				patientMedecinDao.updateObject(medecin);
				log.debug("Modification objet PatientMedecin "
												+ medecin.toString());
			} else {			
				log.warn("Doublon lors " + operation + " objet PatientMedecin "
						+ medecin.toString());
	...
	@Override
	public void removeObjectManager(PatientMedecin medecin) {
		if (medecin != null) {
			patientMedecinDao.removeObject(medecin.getPk());
				log.debug("Suppression objet PatientMedecin " 
												+ medecin.toString());
		} else {
	...
	@Override
	public boolean findDoublonManager(PatientMedecin medecin) {
	 ...
	 */
	private void checkRequiredObjectsAndValidate(PatientMedecin medecin, 
												Patient patient, 
		...
		if (patient != null) { 
			...
			log.warn("Objet obligatoire Patient manquant"
						+ " lors de la " + operation + " d'un PatientMedecin");
			throw new RequiredObjectIsNullException(
					"PatientMedecin", operation, "Patient");
		} 
		//Collaborateur required
		if (collaborateur != null) {
			medecin.setCollaborateur(collaborateurDao
												.mergeObject(collaborateur));
		} else if (medecin.getCollaborateur() == null) {			
			log.warn("Objet obligatoire Collaborateur  manquant"
						+ " lors de la " + operation + " d'un PatientMedecin");
			throw new RequiredObjectIsNullException(
				"PatientMedecin", operation, "Collaborateur");
		}
		
		//Validation
		BeanValidator.validateObject(medecin, 
								new Validator[]{patientMedecinValidator});
	}
}
