...
public class CimoMorphoManagerImpl implements CimoMorphoManager {
	...
	private Log log = LogFactory.getLog(CimoMorphoManager.class);
	
	private CimoMorphoDao cimoMorphoDao;
	
	public void setCimoMorphoDao(CimoMorphoDao cDao) {
	...
	public List< ? extends CodeCommon> findByLibelleLikeManager(String libelle,
		...
		log.debug("Recherche Cimo par libelle: " 
				+ libelle + " exactMatch " + String.valueOf(exactMatch));
		return cimoMorphoDao.findByLibelleLike(libelle);
	}
	
	@Override
	public Set<Adicap> getAdicapsManager(CimoMorpho cimo) {
		Set<Adicap> adicaps = new HashSet<Adicap>();
		CimoMorpho cimoM = cimoMorphoDao.mergeObject(cimo);
