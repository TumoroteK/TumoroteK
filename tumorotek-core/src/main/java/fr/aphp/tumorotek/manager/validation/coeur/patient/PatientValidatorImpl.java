...
//	}
	...
	public void validate(Object obj, Errors errs) {
		...
		if (patient.getNip() != null) {
			ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "nip", 
										"patient.nip.empty");
			if (!patient.getNip()
		...
		if (patient.getNomNaissance() != null) {
			ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "nomNaissance", 
										"patient.nomNaissance.empty");
			if (!patient.getNomNaissance()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errs.rejectValue("nomNaissance", 
						"patient.nomNaissance.illegal");
			}
			if (patient.getNomNaissance().length() > 50) {
				errs.rejectValue("nomNaissance", 
						"patient.nomNaissance.tooLong");
			}
		}
...
//										"patient.sexe.empty");
			if (!patient.getSexe()
					.matches(ValidationUtilities.SEXEREGEXP)) {
		...
		if (patient.getVilleNaissance() != null) {
			ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "villeNaissance", 
										"patient.villeNaissance.empty");
			if (!patient.getVilleNaissance()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errs.rejectValue("villeNaissance", 
								"patient.villeNaissance.illegal");
			}
			if (patient.getVilleNaissance().length() > 100) {
				errs.rejectValue("villeNaissance", 
						"patient.villeNaissance.tooLong");
			}
		}
		...
		if (patient.getPaysNaissance() != null) {
			ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "paysNaissance", 
										"patient.paysNaissance.empty");
			if (!patient.getPaysNaissance()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errs.rejectValue("paysNaissance", 
								"patient.paysNaissance.illegal");
			}
			if (patient.getPaysNaissance().length() > 100) {
				errs.rejectValue("paysNaissance", 
							"patient.paysNaissance.tooLong");
			}
		}
		
		//etat non null
		ValidationUtils
		.rejectIfEmptyOrWhitespace(errs, "patientEtat", 
									"patient.patientEtat.empty");
		
		//etat valide
		if (patient.getPatientEtat() != null) {
			if (!patient.getPatientEtat()
					.matches(ValidationUtilities.PATIENT_ETAT_REGEXP)) {
	...
	@Override
	public Errors checkDateNaissanceCoherence(Patient patient) {
		
		BindException errs = new BindException(patient, 
						"fr.aphp.tumorotek.model.coeur.patient.Patient");
			...
			if (patient.getDateDeces() != null) {
				if (!(patient.getDateNaissance().before(patient.getDateDeces())
						|| patient.getDateNaissance()
											.equals(patient.getDateDeces()))) {
					errs.rejectValue("dateNaissance", 
												"date.validation.supDateDeces");
				}
			}
			
			Object[] dateAndCode = coherenceDateManager
									.findPostRefDateForPatientManager(patient);
			if (ValidationUtilities
					.checkWithDate(patient.getDateNaissance(), null, 
							dateAndCode[0], null, null, null, null, false)) {
				errs.rejectValue("dateNaissance", (String) dateAndCode[1]);
			}
		} else { 
			errs.rejectValue("dateNaissance", 
				"patient.dateNaissance.empty");
		}
		return errs;
	}
	 ...
	 * @return Errors
	 */
	public static Errors checkDateEtatCoherence(Patient patient) {
		...
		if (patient.getDateEtat() != null) {
			if (patient.getDateNaissance() != null) {
				if (!(patient.getDateEtat().after(patient.getDateNaissance())
							|| patient.getDateEtat()
										.equals(patient.getDateNaissance()))) {
					errs.rejectValue("dateEtat", 
											"date.validation.infDateNaissance");
				}
			}
			
			if (!patient.getDateEtat()
	 ...
	 * @return Errors
	 */
	public static Errors checkDateDecesCoherence(Patient patient) {
		...
		if (patient.getDateDeces() != null) {
			if (patient.getDateNaissance() != null) {
				if (!(patient.getDateDeces().after(patient.getDateNaissance())
							|| patient.getDateDeces()
										.equals(patient.getDateNaissance()))) {
					errs.rejectValue("dateDeces", 
											"date.validation.infDateNaissance");
				}
			}
			
			if (!patient.getDateDeces()
				.before(Utils.getCurrentSystemDate())) {
				errs.rejectValue("dateDeces", 
			...
			if (!"D".equals(patient.getPatientEtat())) {
				errs.rejectValue("dateDeces", 
								"patient.dateDeces.incoherenceAvecPatientEtat");
			}
		}
		return errs;
	}
