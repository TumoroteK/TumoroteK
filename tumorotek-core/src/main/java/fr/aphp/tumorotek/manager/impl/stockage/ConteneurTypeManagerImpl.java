...
public class ConteneurTypeManagerImpl implements ConteneurTypeManager {
	...
	private Log log = LogFactory.getLog(ConteneurTypeManager.class);
	...
	public void setConteneurTypeValidator(
			ConteneurTypeValidator cValidator) {
		this.conteneurTypeValidator = cValidator;
	}

	@Override
	public ConteneurType findByIdManager(Integer conteneurTypeId) {
...
//	}
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			ConteneurType type = (ConteneurType) o;
			if (type.getConteneurTypeId() == null) {
				return conteneurTypeDao.findAll().contains(type);
			} else {
				return conteneurTypeDao.findByExcludedId(
						type.getConteneurTypeId()).contains(type);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object obj) {
		ConteneurType type = conteneurTypeDao
			.mergeObject((ConteneurType) obj);
	...
	public void createObjectManager(Object obj) {
		...
		BeanValidator.validateObject(obj, new Validator[]{
				conteneurTypeValidator});
		if (!findDoublonManager(obj)) {
			conteneurTypeDao.createObject((ConteneurType) obj);
			log.info("Enregistrement objet ConteneurType " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors creation objet ConteneurType " 
					+ obj.toString());
			throw new DoublonFoundException("ConteneurType", 
					"creation");
		}	
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (!findDoublonManager(obj)) {
			...
			log.warn("Doublon lors modification objet ConteneurType "
					+ obj.toString());
			throw new DoublonFoundException("ConteneurType", 
					"modification");
		}
...
//			if (!isUsedObjectManager(obj)) {
				conteneurTypeDao.removeObject(
						((ConteneurType) obj).getConteneurTypeId());
				log.info("Suppression objet ConteneurType " 
						+ obj.toString());
//			} else {
