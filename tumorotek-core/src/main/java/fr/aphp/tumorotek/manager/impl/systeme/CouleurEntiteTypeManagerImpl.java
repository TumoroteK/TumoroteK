...
public class CouleurEntiteTypeManagerImpl implements CouleurEntiteTypeManager {
	...
	private Log log = LogFactory.getLog(CouleurEntiteTypeManager.class);
	...
	public void setCouleurEntiteTypeDao(CouleurEntiteTypeDao cDao) {
	...
	@Override
	public CouleurEntiteType findByIdManager(Integer couleurEntiteTypeId) {
	...
	@Override
	public List<CouleurEntiteType> findAllObjectsByBanqueManager(
			Banque banque) {
		if (banque != null && banque.getBanqueId() != null) {
			return couleurEntiteTypeDao.findByBanque(banque);
		} else {
			return new ArrayList<CouleurEntiteType>();
		}
	}
	
	@Override
	public List<CouleurEntiteType> findAllCouleursForEchanTypeByBanqueManager(
			Banque banque) {
		if (banque != null && banque.getBanqueId() != null) {
	...
	@Override
	public List<CouleurEntiteType> findAllCouleursForProdTypeByBanqueManager(
			Banque banque) {
		if (banque != null && banque.getBanqueId() != null) {
	...
	@Override
	public Boolean findDoublonManager(CouleurEntiteType couleurEntiteType) {
		if (couleurEntiteType != null) {
			if (couleurEntiteType.getCouleurEntiteTypeId() == null) {
				return couleurEntiteTypeDao.findAll()
					.contains(couleurEntiteType);
			} else {
				return couleurEntiteTypeDao.findByExcludedId(
						couleurEntiteType.getCouleurEntiteTypeId())
						.contains(couleurEntiteType);
			}
		} else {
			return false;
	...
	@Override
	public void createObjectManager(CouleurEntiteType couleurEntiteType,
			Couleur couleur, Banque banque, EchantillonType echantillonType,
			ProdType prodType) {
		...
		if (banque != null) { 
			...
			throw new RequiredObjectIsNullException(
					"CouleurEntiteType", "creation", "Banque");
		} 
		...
		if (couleur != null) { 
			...
			log.warn("Objet obligatoire Couleur manquant"
							+ " lors de la création d'une" 
							+ " CouleurEntiteType");
			throw new RequiredObjectIsNullException(
					"CouleurEntiteType", "creation", "Couleur");
		}
		
		// il faut qu'un seul type soit défini pour cette relation
		if (echantillonType != null && prodType != null) {
			log.warn("Deux types sont définis"
					+ " lors de la création d'une CouleurEntiteType");
			throw new InvalidMultipleAssociationException(
					"CouleurEntiteType", "creation", false);
		} else if (echantillonType == null && prodType == null) {
			log.warn("Aucun type n'est défini"
					+ " lors de la création d'une CouleurEntiteType");
			throw new InvalidMultipleAssociationException(
					"CouleurEntiteType", "creation", true);
		}
		
		couleurEntiteType.setEchantillonType(echantillonTypeDao
					.mergeObject(echantillonType));
		...
		if (findDoublonManager(couleurEntiteType)) {
			...
			log.info("Enregistrement de l'objet CouleurEntiteType : " 
					+ couleurEntiteType.toString());
		}
	}

	@Override
	public void updateObjectManager(CouleurEntiteType couleurEntiteType,
			Couleur couleur, Banque banque, EchantillonType echantillonType,
			ProdType prodType) {
		//Banque required
		if (banque != null) { 
			...
			log.warn("Objet obligatoire Banque manquant"
							+ " lors de la modification d'une" 
							+ " CouleurEntiteType");
			throw new RequiredObjectIsNullException(
					"CouleurEntiteType", "modification", "Banque");
		} 
		...
		if (couleur != null) { 
			...
			log.warn("Objet obligatoire Couleur manquant"
							...
							+ " CouleurEntiteType");
			throw new RequiredObjectIsNullException(
					"CouleurEntiteType", "modification", "Couleur");
		}
		
		// il faut qu'un seul type soit défini pour cette relation
		if (echantillonType != null && prodType != null) {
			log.warn("Deux types sont définis"
					+ " lors de la modification d'une CouleurEntiteType");
			throw new InvalidMultipleAssociationException(
					"CouleurEntiteType", "modification", false);
		} else if (echantillonType == null && prodType == null) {
			log.warn("Aucun type n'est défini"
					+ " lors de la modification d'une CouleurEntiteType");
			throw new InvalidMultipleAssociationException(
					"CouleurEntiteType", "modification", true);
		}
		...
		if (findDoublonManager(couleurEntiteType)) {
			...
			throw new DoublonFoundException(
					"CouleurEntiteType", "modification");
		} else {
	...
	@Override
	public void removeObjectManager(CouleurEntiteType couleurEntiteType) {
		if (couleurEntiteType != null) {
			couleurEntiteTypeDao.removeObject(
					couleurEntiteType.getCouleurEntiteTypeId());
			log.info("Suppression de l'objet CouleurEntiteType : " 
					+ couleurEntiteType.toString());				
		} else {
