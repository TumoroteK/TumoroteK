		...
		if (critere.getChamp() == null) {			
			if (critere.getCombinaison() == null) {
				errs.rejectValue("champ",
						"critere.champ.isNotNullAsCombinaison");
			} else {
				// On recupere le type de la combinaison
				if (critere.getCombinaison().getChamp1().getChampAnnotation()
						!= null) {
					type = critere.getCombinaison().getChamp1()
							.getChampAnnotation().getDataType().getType();
				} else if (critere.getCombinaison().getChamp1()
						.getChampEntite() != null) {
					if (critere.getOperateur() != null) {
						type = critere.getCombinaison().getChamp1()
								.getChampEntite().getDataType().getType();
					}
				}
			}
			...
			if (critere.getChamp().getChampAnnotation() != null) {
				type = critere.getChamp().getChampAnnotation()
						.getDataType().getType();
			} else if (critere.getChamp().getChampEntite() != null) {
				if (critere.getOperateur() != null) {
					type = critere.getChamp().getChampEntite()
							.getDataType().getType();
				}
			}
		}
		if (type != null && critere.getOperateur() != null) {					
			if (type.equals("num") || type.equals("text")) {
				if (!critere.getOperateur().equals("<")
						&& !critere.getOperateur().equals("<=")
						&& !critere.getOperateur().equals(">")
						&& !critere.getOperateur().equals(">=")
						&& !critere.getOperateur().equals("=")
						&& !critere.getOperateur().equals("!=")
						&& !critere.getOperateur().equals("like")
						&& !critere.getOperateur().equals("not like")
						&& !critere.getOperateur()
								.equals("contains")
						&& !critere.getOperateur().equals("is null")) {
					errs.rejectValue("operateur",
							"critere.operateur.illegal");
				}
