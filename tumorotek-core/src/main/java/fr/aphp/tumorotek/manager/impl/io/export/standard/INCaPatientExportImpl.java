...
public class INCaPatientExportImpl implements IncaPatientExport {
	...
	@Override
	public String getFiness(Connection con, Prelevement prel, 
													Patient pat) {
		return ExportCatalogueManagerImpl.fetchItemAsString(con, 
			"SELECT ETABLISSEMENT.NOM "
				+ "FROM ETABLISSEMENT, SERVICE, PRELEVEMENT " 
				+ "WHERE PRELEVEMENT.SERVICE_PRELEVEUR_ID = SERVICE.SERVICE_ID "
			+ "AND ETABLISSEMENT.ETABLISSEMENT_ID = SERVICE.ETABLISSEMENT_ID "
				+ "AND PRELEVEMENT.PRELEVEMENT_ID =  " 
				+ prel.getPrelevementId().toString(), 
				"FINESS manquant pour le patient " + pat.getNip(), true, 
				null, null);
	}
	
	@Override
	public String getPatientId(Patient pat) {
		if (pat.getPatientId() != null) {
			return pat.getPatientId().toString();
		} else {
			throw new ItemException(2, 
					"Identifiant patient obligatoire manquant");
		}
	}
	
	@Override
	public String getDateNaissance(Patient pat, DateFormat df) {
	...
	@Override
	public String getSexe(Patient pat) {
		
		if (pat.getSexe() != null 
				&& pat.getSexe().matches("M|F")) {
			return pat.getSexe();
		} else {
			return "I";
		}
	...
	@Override
	public String getPatientEtat(Patient pat) {
	...
	@Override
	public String getDateEtat(Patient pat, DateFormat df) {
	...
	@Override
	public String getCauseDeces(Connection con, Patient pat, Banque bank) {
		return ExportCatalogueManagerImpl.fetchItemAsString(con, 
			"SELECT ITEM.LABEL FROM ITEM, ANNOTATION_VALEUR, "
				+ "CHAMP_ANNOTATION, TABLE_ANNOTATION" 
				+ " WHERE ANNOTATION_VALEUR.OBJET_ID=" 
				+ pat.getPatientId().toString()
				+ " AND CHAMP_ANNOTATION.NOM like '076 : Cause %'" 
