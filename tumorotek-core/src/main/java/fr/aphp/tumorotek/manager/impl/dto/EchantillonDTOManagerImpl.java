...
public class EchantillonDTOManagerImpl implements EchantillonDTOManager {
	...
	public void setProdDeriveDao(ProdDeriveDao _p) {
		this.prodDeriveDao = _p;
	}

	public void setCederObjetDao(CederObjetDao _c) {
		this.cederObjetDao = _c;
	}

	public void setEntiteDao(EntiteDao _e) {
		this.entiteDao = _e;
	}

	public void setCodeAssigneManager(CodeAssigneManager _c) {
	...
	public void setConteneurManager(ConteneurManager _c) {
		this.conteneurManager = _c;
	}

	@Override
	public EchantillonDTO initEchantillonDecoratorManager(Echantillon e) {
		if (e != null) {
		EchantillonDTO dto = new EchantillonDTO(e);
			
			if (e.getEchantillonId() != null) {
				
				dto.setNbDerives(prodDeriveDao
						.findCountByParent(e.getEchantillonId(), entiteDao.findById(3))
						.get(0).intValue());
				dto.setNbCessions(cederObjetDao
						.findCountObjCession(e.getEchantillonId(), entiteDao.findById(3))
						.get(0).intValue());
				dto.setDateCreation(operationManager
						.findDateCreationManager(e));
				dto.setEmplacementAdrl(echantillonManager.getEmplacementAdrlManager(e));
				dto.setTempStock(conteneurManager
						.findTempForEmplacementManager(e.getEmplacement()));
				
				dto.setCodesOrgsToCreateOrEdit(codeAssigneManager
								.findCodesOrganeByEchantillonManager(e));
						
				dto.setCodesLesToCreateOrEdit(codeAssigneManager
	...
	@Override
	public List<EchantillonDTO> decorateListeManager(List<Echantillon> echans) {
		List<EchantillonDTO> liste = new ArrayList<EchantillonDTO>();
	...
	@Override
	public List<Echantillon> extractListeManager(List<EchantillonDTO> decos) {
		List<Echantillon> liste = new ArrayList<Echantillon>();
		Iterator<EchantillonDTO> it = decos.iterator();	
		while (it.hasNext()) {
