...
public class ProtocoleTypeManagerImpl implements ProtocoleTypeManager {
	...
	private Log log = LogFactory.getLog(ProtocoleTypeManager.class);
	...
	public void setProtocoleTypeValidator(
			ProtocoleTypeValidator pValidator) {
	 ...
	 */
	@Override
	public ProtocoleType findByIdManager(Integer protocoleTypeId) {
...
//	}
	 ...
	 */
	@Override
	public List<ProtocoleType> findByTypeLikeManager(String type,
			boolean exactMatch) {
		
		log.debug("Recherche ProtocoleType par " 
		...
		if (type != null) {
			...
			return new ArrayList<ProtocoleType>();
		}
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			ProtocoleType type = (ProtocoleType) o;
			if (type.getProtocoleTypeId() == null) {
				return protocoleTypeDao.findAll().contains(type);
			} else {
				return protocoleTypeDao.findByExcludedId(
						type.getProtocoleTypeId()).contains(type);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object obj) {
		ProtocoleType type = protocoleTypeDao
			.mergeObject((ProtocoleType) obj);
	...
	public void createObjectManager(Object obj) {
		...
		if (((ProtocoleType) obj).getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"ProtocoleType", "creation", "Plateforme");
		} else {
			((ProtocoleType) obj).setPlateforme(plateformeDao
					.mergeObject(((ProtocoleType) obj).getPlateforme()));
		}
		BeanValidator.validateObject(obj, new Validator[]{
				protocoleTypeValidator});
		if (!findDoublonManager(obj)) {
			protocoleTypeDao.createObject((ProtocoleType) obj);
			log.info("Enregistrement objet ProtocoleType " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors creation objet ProtocoleType " 
					+ obj.toString());
			throw new DoublonFoundException("ProtocoleType", 
					"creation");
		}
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (!findDoublonManager(obj)) {
			...
			log.info("Modification objet ProtocoleType " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors modification objet ProtocoleType "
					+ obj.toString());
			throw new DoublonFoundException("ProtocoleType", 
					"modification");
		}
...
//			if (!isUsedObjectManager(obj)) {
				protocoleTypeDao.removeObject(
						((ProtocoleType) obj).getProtocoleTypeId());
				log.info("Suppression objet ProtocoleType " 
						+ obj.toString());
//			} else {
