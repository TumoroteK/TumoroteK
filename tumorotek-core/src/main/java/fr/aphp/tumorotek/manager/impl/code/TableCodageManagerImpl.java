...
public class TableCodageManagerImpl implements TableCodageManager {
	...
	public void setAdicapManager(AdicapManager aManager) {
		this.adicapManager = aManager;
	}

	public void setCimMasterManager(CimMasterManager cManager) {
		this.cimMasterManager = cManager;
	}

	public void setCimoMorphoManager(CimoMorphoManager cManager) {
		this.cimoMorphoManager = cManager;
	}

	public void setCodeUtilisateurManager(CodeUtilisateurManager cuManager) {
	...
	@Override
	public List<TableCodage> findByNomManager(String nom) {
...
//	}
	...
	@Override
	public List<CodeCommon> transcodeManager(CodeCommon code, 
													List<TableCodage> tables,
													List<Banque> banks) {
		List<CodeCommon> resTrans = new ArrayList<CodeCommon>();
		if (code != null && tables != null) {
			
			boolean containsAdicap = tables
						.contains(tableCodageDao.findByNom("ADICAP").get(0));
			boolean containsCim = tables
					.contains(tableCodageDao.findByNom("CIM_MASTER").get(0));
			boolean containsCimo = tables
					.contains(tableCodageDao.findByNom("CIMO_MORPHO").get(0));
			boolean containsUser = tables
					.contains(tableCodageDao.findByNom("UTILISATEUR").get(0));
			
			if (code instanceof Adicap) {
				if (containsCim) {
					resTrans.addAll(adicapManager
								.getCimMastersManager((Adicap) code));
				}
				if (containsCimo) {
					resTrans.addAll(adicapManager
								.getCimoMorphosManager((Adicap) code));
				}
				if (containsUser) {
					resTrans.addAll(codeUtilisateurManager
							.findByTranscodageManager(code, banks));
				}
			} else if (code instanceof CimMaster) { 
				if (containsAdicap) {	
					resTrans.addAll(cimMasterManager
									.getAdicapsManager((CimMaster) code));
				}
				if (containsUser) {
					resTrans.addAll(codeUtilisateurManager
							.findByTranscodageManager(code, banks));
				...
				if (containsAdicap) {
					resTrans.addAll(cimoMorphoManager
								.getAdicapsManager((CimoMorpho) code));
				}
				if (containsUser) {
					resTrans.addAll(codeUtilisateurManager
							.findByTranscodageManager(code, banks));
				...
				resTrans.addAll(codeUtilisateurManager
						.getTranscodesManager((CodeUtilisateur) code, tables));
			}
		}
		return resTrans;
	}
	...
	@SuppressWarnings("unchecked")
	@Override
	public List<CodeCommon> findCodesAndTranscodesFromStringManager(
			String codeorLib, List<TableCodage> tables, List<Banque> banks, 
			boolean exactMatch) {
		
		Set<CodeCommon> codes = new LinkedHashSet<CodeCommon>();
		
		List<CodeCommon> res = new ArrayList<CodeCommon>();
		List<CodeCommon> trans = new ArrayList<CodeCommon>();
		if (tables != null) {
			for (int i = 0; i < tables.size(); i++) {
				...
				if ("ADICAP".equals(tables.get(i).getNom())) {
					res.addAll((List<CodeCommon>) adicapManager
							.findByCodeLikeManager(codeorLib, exactMatch));
					res.addAll((List<CodeCommon>) adicapManager
						.findByLibelleLikeManager(codeorLib, exactMatch));
				} else if ("CIM_MASTER".equals(tables.get(i).getNom())) {
					res.addAll((List<CodeCommon>) cimMasterManager
								.findByCodeLikeManager(codeorLib, exactMatch));
					res.addAll((List<CodeCommon>) cimMasterManager
							.findByLibelleLikeManager(codeorLib, exactMatch));
				} else if ("CIMO_MORPHO".equals(tables.get(i).getNom())) {
					res.addAll((List<CodeCommon>) cimoMorphoManager
							.findByCodeLikeManager(codeorLib, exactMatch));
					res.addAll((List<CodeCommon>) cimoMorphoManager
							.findByLibelleLikeManager(codeorLib, exactMatch));
				} else if ("UTILISATEUR".equals(tables.get(i).getNom())) {
					res.addAll(codeUtilisateurManager
							.findByCodeLikeManager(codeorLib, exactMatch, banks));
					res.addAll(codeUtilisateurManager
							.findByLibelleLikeManager(codeorLib, exactMatch, banks));
				}
				
				// cherche transcodes dans autre tables
				Iterator<CodeCommon> resIt = res.iterator();
				CodeCommon next;
				while (resIt.hasNext()) {
					next = resIt.next();
		...
		return new ArrayList<CodeCommon>(codes);
	}
	 ...
	 */
	public List<String> getListCodesFromCodeCommon(List<CodeCommon> codes) {
		List<String> codesStr = new ArrayList<String>();
		if (codes != null) {
	 ...
	 */
	public List<String> getListLibellesFromCodeCommon(List<CodeCommon> 
																	codes) {
		List<String> libellesStr = new ArrayList<String>();
		if (codes != null) {
			for (int i = 0; i < codes.size(); i++) {
