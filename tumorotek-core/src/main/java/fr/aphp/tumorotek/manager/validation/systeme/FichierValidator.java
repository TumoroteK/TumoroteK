...
public class FichierValidator implements Validator {
	...
	public void validate(Object obj, Errors errs) {
		...
		//Nom non vide
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "nom", "fichier.nom.empty");
	
		//path valide
		if (fichier.getNom() != null) {
			if (!fichier.getNom()
				.matches(ValidationUtilities.FILENAMEREGEXP)) {
		...
		//Path non vide
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "path", "fichier.path.empty");
	
		//path valide
		if (fichier.getPath() != null) {
			if (!fichier.getPath()
				.matches(ValidationUtilities.PATHREGEXP)) {
