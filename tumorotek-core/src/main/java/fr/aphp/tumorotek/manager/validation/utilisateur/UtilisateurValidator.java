...
public class UtilisateurValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		// login non null
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errors, "login", 
									"utilisateur.login.empty");
		//login valide
		...
		if (utilisateur.getEncodedPassword() != null) {
			if (utilisateur.getEncodedPassword()
					.matches(ValidationUtilities.ONLYSPACESREGEXP)) {
				errors.rejectValue("encodedPassword", 
						"utilisateur.encodedPassword.empty");
			}
			if (!utilisateur.getEncodedPassword()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errors.rejectValue("encodedPassword", 
						"utilisateur.encodedPassword.illegal");
			}
			if (utilisateur.getEncodedPassword().length() > 100) {
				errors.rejectValue("encodedPassword", 
						"utilisateur.encodedPassword.tooLong");
			}
		}
	 ...
	 * @return Errors
	 */
	public static Errors checkDateDesactCoherence(Utilisateur user) {		
		
		BindException errs = new BindException(user, 
					"fr.aphp.tumorotek.model.utilisateur.Utilisateur");
		
		if (user.getTimeOut() != null) {		
			if (user.getTimeOut()
					.before(Utils.getCurrentSystemDate())) {
				errs.rejectValue("timeOut", 
							"date.validation.infDateActuelle");
			}
		}
		return errs;
	}
