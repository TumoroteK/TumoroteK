...
public class ExportTVGSOManagerImpl extends ExportCatalogueManagerImpl {
	...
	private Log log = LogFactory.getLog(ExportTVGSOManagerImpl.class);
	...
	public void setTvgsoPatientExport(TvgsoPatientExport tExport) {
		this.tvgsoPatientExport = tExport;
	}

	public void setTvgsoMaladieExport(TvgsoMaladieExport iExport) {
		this.tvgsoMaladieExport = iExport;
	}

	public void setTvgsoPrelevementExport(TvgsoPrelevementExport tExport) {
		this.tvgsoPrelevementExport = tExport;
	}
	
	public void setTvgsoEchantillonExport(TvgsoEchantillonExport iExport) {
		this.tvgsoEchantillonExport = iExport;
	}

	public void setTvgsoComplementExport(TvgsoComplementExport iExport) {
	...
	public Map<String, String> objetExport(Echantillon echan, 
		...
		try {
			...
			flag = "FINESS";
			map.put("FINESS", tvgsoPatientExport
			...
			map.put("Identifiant patient", 
							tvgsoPatientExport.getPatientCode(getPatient()));
				
			/*Item 3 Date de naissance du patient OBLIGATOIRE */
			flag = "Date naissance";
			map.put("Date naissance", 
					tvgsoPatientExport.getDateNaissance(getPatient(), 
			...
			map.put("Etat du patient", tvgsoPatientExport
										.getPatientEtat(con, getPatient(), 
												getBanque()));
			
			/*Item 6 Date de l'etat du patient */
			flag = "Date de l'Etat"; 
			map.put("Date de l'Etat", tvgsoPatientExport
					.getDateEtat(con, getPatient(), getDateFormatterINCA(), 
							getBanque()));
			...
			map.put("Diagnostic principal", tvgsoMaladieExport
					.getDiagnosticPrincipal(con, getPrelevement()));
			
			/*Item 8 Date du diagnostic */
			flag = "Date du diagnostic"; 
			map.put("Date du diagnostic", tvgsoMaladieExport
			...
			flag = "Version TNM"; 
			map.put("Version TNM", 
				tvgsoMaladieExport.getVersionCTNM(con, getPrelevement()));

			/*Item 10 Taille de la tumeur */
			flag = "T du cTNM"; 
			map.put("T du cTNM", 
				tvgsoMaladieExport.getTailleTumeur(con, getPrelevement()));
			
			/*Item 11 Envahissement ganglionnaire */
			flag = "N du cTNM"; 
			map.put("N du cTNM", tvgsoMaladieExport
						.getEnvahGangR(con, getPrelevement()));
			
			/*Item 12 Extension metastatique */
			flag = "M du cTNM"; 
			map.put("M du cTNM", 
				tvgsoMaladieExport.getExtMetastatique(con, getPrelevement()));
			...
			map.put("Centre stockage", tvgsoPrelevementExport
								.getCentreStockage(con, getEchantillon()));
			
			/*Item 14 Responsable tumorotheque OBLIGATOIRE*/
			flag = "Contact"; 
			map.put("Contact", 
				tvgsoComplementExport.getContactShort(getBanque()));
			
			/*colonne 15 Code prelevement OBLIGATOIRE item14 Inca*/
			flag = "Code prelevement"; 
			map.put("Code prelevement", tvgsoPrelevementExport
			...
			map.put("Version pTNM", 
				tvgsoPrelevementExport.getVersionPTNM(con, getPrelevement()));
			
			/*colonne 25 pT item 24 INCa*/
			flag = "T du pTNM"; 
			map.put("T du pTNM", tvgsoPrelevementExport
							.getTailleTumeurPT(con, getPrelevement()));
			
			/*colonne 26 Presence/absence metastases */
			/* ganglionnaires pN item25 INCa*/
			flag = "N du pTNM"; 
			map.put("N du pTNM", 
				tvgsoPrelevementExport.getEnvGangPN(con, getPrelevement()));
			...
			if (isTumoral) {
				...
				item30 = tvgsoEchantillonExport
					.getModePreparation(con, getEchantillon());
			} else {
				flag = "Mode preparation non tumoral"; 
				item42 = tvgsoEchantillonExport
					.getModePreparation(con, getEchantillon());
			}
			...
			flag = "Serum"; 
			map.put("Serum", tvgsoEchantillonExport
				.getRessourceBiolAssociee(getEchantillon(), "%SERUM%"));
			flag = "Plasma"; 
			map.put("Plasma", tvgsoEchantillonExport
				.getRessourceBiolAssociee(getEchantillon(), "%PLASMA%"));
			flag = "Liquides"; 
			map.put("Liquides", tvgsoEchantillonExport
				.getRessourceBiolAssociee(getEchantillon(), "%LIQUIDES%"));
			//ADN Constitutionnel
			flag = "ADN"; 
			map.put("ADN", tvgsoEchantillonExport
				.getADNconstitutionnel(con, getEchantillon()));
			...
			map.put("CR anapath interrogeable", tvgsoComplementExport
			...
			map.put("Donnees cliniques disponibles", 
				tvgsoComplementExport
					.getDonneesClinBase(con, getPrelevement()));
			
			/*Item56, item57 Inclusion dans un protocole therapeutique */ 
			/* et nom du protocole*/
			flag = "Inclusion protocole therapeutique"; 
			map.put("Inclusion protocole therapeutique", tvgsoComplementExport
									.getInclusionTherap(con, getPrelevement()));
			flag = "Nom protocole"; 
			map.put("Nom protocole", tvgsoComplementExport
					.getNomProtocoleTherap(con, getPrelevement()));
			
			/*Item58 item59 Caryotype et anomalies eventuelles*/
			flag = "Caryotype"; 
			map.put("Caryotype", tvgsoComplementExport
							.getCaryotype(con, getPatient(), getBanque()));
			flag = "Anomalie eventuelle"; 
			map.put("Anomalie eventuelle", tvgsoComplementExport
						.getAnomalieCaryo(con,  getPatient(), getBanque()));
			
			/*Item60 item61 Anomalie genomique et description*/
			flag = "Anomalie genomique"; 
			map.put("Anomalie genomique", tvgsoComplementExport
				.getAnomalieGenomique(con,  getPatient(), getBanque()));
			flag = "Description anomalie genomique"; 
			map.put("Description anomalie genomique", tvgsoComplementExport
				.getAnomalieGenomiqueDescr(con,  getPatient(), getBanque()));
			
			/*Item62 Controle qualite*/
			flag = "Controle qualite"; 
			map.put("Controle qualite", 
				tvgsoComplementExport.getControleQualite(getEchantillon()));
			
			/*Item63 item64 Inclusion dans un protocole de recherche 
			/* et nom du programme*/
			flag = "Inclusion programme recherche"; 
			map.put("Inclusion programme recherche", tvgsoComplementExport
						.getInclusionProtocoleRech(con, getEchantillon()));
			flag = "Nom programme"; 
			map.put("Nom programme", tvgsoComplementExport
							.getNomProtocoleRech(con, getEchantillon()));
			
			/* colonne 66 champs spécifique du cancer item 65 INCa*/
			flag = "Champ specifique"; 
			map.put("Champ specifique", tvgsoComplementExport
					.getChampSpecCancer(con, getPatient(), getBanque()));
			...
			/*44b consentement*/
			flag = "Consentement"; 
			map.put("Consentement", tvgsoComplementExport
					.getConsentement(getPrelevement(), ".*AUTORISATION.*", 
											"(.*ATTENTE.*)|(.*REFUS.*)"));
	...
	@Override
	public void printDataIntoRow(Object obj, int index, int nbRow,
			List<String> values) {
		try {
			getExportUtils()
				.addDataToRowCSV((BufferedWriter) obj, index, values, "|", "|\n");
		} catch (IOException e) {
			log.error(e);
		}
	}
