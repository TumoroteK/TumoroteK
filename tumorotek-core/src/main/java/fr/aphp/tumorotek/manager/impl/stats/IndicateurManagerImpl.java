...
public class IndicateurManagerImpl implements IndicateurManager {
	...
	private Log log = LogFactory.getLog(IndicateurManager.class);

	private List<Indicateur> NO_LIST = new ArrayList<Indicateur>();

	private IndicateurDao indicateurDao;
	// private SModeleDao sModeleDao;
	private IndicateurValidator indicateurValidator;

	public void setIndicateurDao(IndicateurDao sDao) {
...
//	}
	...
	public void setIndicateurValidator(IndicateurValidator sVal) {
		this.indicateurValidator = sVal;
	}

	@Override
	public Indicateur findByIdManager(Integer statementId) {
	...
	@Override
	public List<Indicateur> findBySubdivisionManager(Subdivision subdivision) {
		return indicateurDao.findBySubdivision(subdivision);
	}
	
	@Override
	public List<Indicateur> findBySModeleManager(SModele modele) {
		if (modele != null) {
	...
	@Override
	public Boolean findDoublonManager(Indicateur indic) {
		if (indic != null) {
			if (indic.getIndicateurId() == null) {
				return indicateurDao.findAll().contains(indic);
			} else {
				return indicateurDao.findByExcludedId(
						indic.getIndicateurId()).contains(indic);
			}
		} else {
			return false;
		}
	...
	@Override
	public void createObjectManager(Indicateur indic) {
		...
		if (findDoublonManager(indic)) {
			log.warn("Doublon lors de la creation de l'objet Indicateur : "
					+ indic.getNom());
			throw new DoublonFoundException("Indicateur", "creation");
		} else { // validation de l'objet
			// validation
			BeanValidator.validateObject(indic,
					new Validator[] { indicateurValidator });
		}
	...
	@Override
	public void updateObjectManager(Indicateur indic) {
	...
	@Override
	public void removeObjectManager(Indicateur indic) {
		if (indic != null) {
