...
public class ObjetNonConformeManagerImpl implements ObjetNonConformeManager {
	...
	private Log log = LogFactory.getLog(ObjetNonConformeManager.class);
	...
	@Override
	public ObjetNonConforme findByIdManager(Integer objetNonConformeId) {
	...
	@Override
	public List<ObjetNonConforme> findByObjetManager(Object obj) {
		log.debug("Recherche de tous les ObjetNonConformes d'un " 
				+ "objet");
		if (obj != null) {
			Entite entite = null;
			Integer id = null;
			...
			return new ArrayList<ObjetNonConforme>();
		}
	}

	@Override
	public List<ObjetNonConforme> findByObjetAndTypeManager(Object obj,
			Object type) {
		log.debug("Recherche de tous les ObjetNonConformes d'un " 
				+ "objet en fonction du type de conformite");
		if (obj != null && type != null) {
			...
			if (type instanceof String) {
				List<ConformiteType> list = conformiteTypeDao
					.findByEntiteAndType((String) type, entite);
				if (list.size() == 1) {
					ct = list.get(0);
				}
			...
			return objetNonConformeDao.findByObjetEntiteAndType(
					id, entite, ct);
		} else {
			return new ArrayList<ObjetNonConforme>();
		}
	...
	@Override
	public void createUpdateOrRemoveObjectManager(Object obj,
			NonConformite nonConformite, String type) {
		if (obj != null && type != null) {
			...
			if (nonConformite != null) {
				...
				// if (list.size() == 0) {
					ObjetNonConforme newObj = new ObjetNonConforme();
					...
					if (entite == null) {
						throw new RequiredObjectIsNullException(
								"ObjetNonConforme", "creation", "Entite");
					} else {
					...
					if (id == null) {
						throw new RequiredObjectIsNullException(
								"ObjetNonConforme", "creation", "ObjetId");
					} else {
					...
					newObj.setNonConformite(nonConformiteDao.mergeObject(
							nonConformite));
					
					// création
					objetNonConformeDao.createObject(newObj);
					log.info("Enregistrement de l'objet ObjetNonConforme : " 
							+ newObj.toString());
//				} 
//				else {
	...
	public void prepareListJDBCManager(EchantillonJdbcSuite jdbcSuite,
		...
		if (tkObj != null && nonConformites != null) {	
			if (!nonConformites.isEmpty()) {
				...
				// ResultSet rs2 = null;
				Integer maxNcId = jdbcSuite.getMaxObjetNonConformeId();
...
//							.prepareStatement(sql);
					...
					Integer objId = tkObj.listableObjectId();
					// si objetid null
					if (objId == null) {
						throw new RequiredObjectIsNullException(
								"ObjetNonConforme", "creation", "ObjetId");
					...
					Integer entiteId;
					// si entite null
					Entite entite = entiteDao
							.findByNom(tkObj.entiteNom()).get(0);
					...
					for (NonConformite nc : new LinkedHashSet<NonConformite>(nonConformites)) {
						// validation entite non conformite type
...
//					}
	...
	@Override
	public void createUpdateOrRemoveListObjectManager(Object obj,
			List<NonConformite> nonConformites, String type) {
		if (obj != null && type != null && nonConformites != null) {
			...
			if (!nonConformites.isEmpty()) {
				// clone sans doublons
				Set<NonConformite> ncfs = 
							new LinkedHashSet<NonConformite>(nonConformites);
	...
	@Override
	public void removeObjectManager(ObjetNonConforme objetNonConforme) {
		if (objetNonConforme != null) {
			objetNonConformeDao.removeObject(objetNonConforme
						.getObjetNonConformeId());
			log.info("Suppression de l'objet ObjetNonConforme : " 
					+ objetNonConforme.toString());
		}
	}
	
	@Override
	public List<Integer> 
			findObjetIdsByNonConformitesManager(List<NonConformite> nocfs) {
