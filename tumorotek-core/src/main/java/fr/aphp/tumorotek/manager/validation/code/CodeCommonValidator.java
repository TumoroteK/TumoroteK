...
public class CodeCommonValidator implements Validator
{
   ...
   public void validate(final Object obj, final Errors errs){
      ...
      final CodeCommon code = (CodeCommon) obj;

      //code non null
      ValidationUtils.rejectIfEmptyOrWhitespace(errs, "code", "code.code.empty");
      //code valide
      if(code.getCode() != null){
         if(!code.getCode().matches(ValidationUtilities.MOTREGEXP)){
      ...
      if(code.getLibelle() != null){
         ValidationUtils.rejectIfEmptyOrWhitespace(errs, "libelle", "code.libelle.empty");
         if(!code.getLibelle().matches(ValidationUtilities.MOTREGEXP)){
      ...
      if(code instanceof CodeAssigne){
         if(((CodeAssigne) code).getIsMorpho() != null && ((CodeAssigne) code).getIsOrgane() != null){
            if(((CodeAssigne) code).getIsMorpho() && ((CodeAssigne) code).getIsOrgane()){
               errs.rejectValue("isOrgane", "code.bothIsOrganeAndIsMorpho");
            }
         }
