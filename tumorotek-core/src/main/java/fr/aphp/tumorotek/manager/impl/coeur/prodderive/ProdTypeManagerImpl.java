...
public class ProdTypeManagerImpl implements ProdTypeManager {
	...
	private Log log = LogFactory.getLog(ProdTypeManager.class);
	...
	public void setProdTypeValidator(ProdTypeValidator validator) {
	 ...
	 * @return Un ProdType.
	 */
	public ProdType findByIdManager(Integer prodTypeId) {
...
//	}
	 ...
	 * @return Liste de ProdType.
	 */
	public List<ProdType> findByTypeLikeManager(
			String type, boolean exactMatch) {
		log.debug("Recherche ProdType par " 
	...
	public boolean findDoublonManager(Object obj) {
		ProdType type = (ProdType) obj;
		if (type != null) {
			if (type.getProdTypeId() == null) {
				return prodTypeDao.findAll().contains(type);
			} else {
				return prodTypeDao.findByExcludedId(
						type.getProdTypeId()).contains(type);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object obj) {
		...
		List<ProdDerive> list = prodDeriveDao.findByProdType(type);
	...
	public void createObjectManager(Object obj) {
		...
		if (findDoublonManager(type)) {
			log.warn("Doublon lors de la creation de l'objet ProdType : " 
					+ type.toString());
			throw new DoublonFoundException("ProdType", "creation");
		} else {
			BeanValidator.validateObject(
					type, new Validator[]{prodTypeValidator});
			prodTypeDao.createObject(type);
			log.info("Enregistrement de l'objet ProdType : " 
					+ type.toString());
		}
	}
	...
	public void updateObjectManager(Object obj) {
		ProdType type = (ProdType) obj;
		if (findDoublonManager(type)) {
			log.warn("Doublon lors de la modification de l'objet ProdType : " 
					+ type.toString());
			...
			log.info("Modification de l'objet ProdType : " 
					+ type.toString());
		}
	}
	...
	public void removeObjectManager(Object obj) {
		...
		if (isUsedObjectManager(type)) {
			...
			log.info("Suppression de l'objet ProdType : " 
					+ type.toString());
		}
	}
