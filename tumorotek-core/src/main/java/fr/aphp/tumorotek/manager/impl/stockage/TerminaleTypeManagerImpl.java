...
public class TerminaleTypeManagerImpl implements TerminaleTypeManager {
	...
	private Log log = LogFactory.getLog(TerminaleTypeManager.class);
	...
	public void setTerminaleTypeValidator(
			TerminaleTypeValidator tValidator) {
		this.terminaleTypeValidator = tValidator;
	}

	@Override
	public TerminaleType findByIdManager(Integer terminaleTypeId) {
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			TerminaleType type = (TerminaleType) o;
			if (type.getTerminaleTypeId() == null) {
				return terminaleTypeDao.findAll().contains(type);
			} else {
				return terminaleTypeDao.findByExcludedId(
						type.getTerminaleTypeId()).contains(type);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object obj) {
		TerminaleType type = terminaleTypeDao.mergeObject((TerminaleType) obj);
	...
	public void createObjectManager(Object obj) {
		BeanValidator.validateObject(obj, new Validator[]{
				terminaleTypeValidator});
		if (!findDoublonManager(obj)) {
			terminaleTypeDao.createObject((TerminaleType) obj);
			log.info("Enregistrement objet TerminaleType " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors creation objet TerminaleType " 
					+ obj.toString());
			throw new DoublonFoundException("TerminaleType", 
					"creation");
		}	
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (!findDoublonManager(obj)) {
			...
			log.info("Modification objet TerminaleType " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors modification objet TerminaleType "
					+ obj.toString());
			throw new DoublonFoundException("TerminaleType", 
					"modification");
		}
	...
	public void removeObjectManager(Object obj) {
		if (obj != null) {
			if (!isUsedObjectManager(obj)) {
				terminaleTypeDao.removeObject(((TerminaleType) obj)
						.getTerminaleTypeId());
				log.info("Suppression objet TerminaleType " 
						+ obj.toString());
			} else {
				log.warn("Suppression objet TerminaleType " 
						+ obj.toString()
						+ " impossible car est reference (par Terminale)");
				throw new 
					ObjectUsedException("terminaleType.deletion.isReferenced", 
																		false);
			}
		} else {
	...
	@Override
	public List<TerminaleType> findByTypeManager(String type) {
		if (type != null) {
			return terminaleTypeDao.findByType(type);
		} else {
			return new ArrayList<TerminaleType>();
		}
	}
}
