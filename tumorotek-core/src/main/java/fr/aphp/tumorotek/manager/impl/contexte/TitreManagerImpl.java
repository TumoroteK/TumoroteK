...
public class TitreManagerImpl implements TitreManager {
	...
	private Log log = LogFactory.getLog(TitreManager.class);
	 ...
	 */
	@Override
	public Titre findByIdManager(Integer titreId) {
	 ...
	 */
	@Override
	public List<Titre> findByTitreLikeManager(
			String titre, boolean exactMatch) {
		log.debug("Recherche Titre par " 
				+ titre + " exactMatch " + String.valueOf(exactMatch));
		if (titre != null) {
			if (!exactMatch) {
				titre = titre + "%";
			}
