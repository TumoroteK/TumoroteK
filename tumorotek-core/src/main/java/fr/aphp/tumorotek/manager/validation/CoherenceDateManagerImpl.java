...
public class CoherenceDateManagerImpl implements CoherenceDateManager {
	 ...
	 */
	@Override
	public Object[] findPostRefDateForPatientManager(Patient patient) {
		...
		if (patient.getPatientId() != null) {
			...
			List<Maladie> maladies = 
					new ArrayList<Maladie>(maladieDao.findByPatient(patient));
			...
			for (int i = 0; i < prels.size(); i++) {
				if (!prels.get(i).getArchive()) {
					if (prels.get(i).getDatePrelevement() != null) {
						...
						List<LaboInter> list =
							new ArrayList<LaboInter>(laboInterDao
									.findByPrelevementWithOrder(prels.get(i)));
						...
						if (ref == null) {
							...
							dateAndCodeForEchan = 
								findPostRefDateInEchantillonsManager(prels.get(i));
	 ...
	 */
	@Override
	public Object[] findPostRefDateInEchantillonsManager(Prelevement prelevement) {
		Object[] dateAndCode = new Object[]{null, null};
		Object ref = null;
		String code = null;
		Object previous = null;
		...
		for (int i = 0; i < echans.size(); i++) {
			if (!echans.get(i).getArchive()) {
				if (echans.get(i).getDateStock() != null) {
					...
					dateAndCodeForEchan =
						findPostRefDateInDerivesManager(echans.get(i));
					ref = dateAndCodeForEchan[0];
					if (ref != null) {
						code = (String) dateAndCodeForEchan[1];
					}
		...
		Object[] dateAndCodeForDerives = 
						findPostRefDateInDerivesManager(prelevement);
	 ...
	 */
	@Override
	public Object[] findAntRefDateInPrelevementManager(Prelevement prelevement, 
	...
	@Override
	public Object[] findPostRefDateInLabosManager(List<LaboInter> labos) {
		Object[] dateAndCode = new Object[]{null, null};
		Object ref = null;
		String code = null;
	 ...
	 */
	@Override
	public Object[] findAntRefDateForDeriveManager(ProdDerive derive) {
		Object[] dateAndCode = new Object[]{null, null};
		Object ref = null;
		String code = null;
		//trouve le parent
			...
			if (parent != null) {
				if (parent.getClass().getSimpleName().equals("ProdDerive")) {
					if (((ProdDerive) parent).getDateStock() != null) {
						...
						return findAntRefDateForDeriveManager(
								(ProdDerive) parent);
					}
					...
					if (((Echantillon) parent).getDateStock() != null) {
						...
						Prelevement prel = 
							echantillonDao.mergeObject((Echantillon) parent)
							.getPrelevement();
						if (prel != null) {
							return 
							findAntRefDateInPrelevementManager(prel, true);
						}
					}
				} else {
					return findAntRefDateInPrelevementManager((Prelevement) 
																parent, true);
				}
			}
		}
	...
	@Override
	public Object[] findPostRefDateInDerivesManager(Object parent) {
		Object[] dateAndCode = new Object[]{null, null};
		Calendar ref = null;
		String code = null;
