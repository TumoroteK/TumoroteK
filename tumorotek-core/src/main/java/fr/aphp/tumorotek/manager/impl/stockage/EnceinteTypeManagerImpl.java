...
public class EnceinteTypeManagerImpl implements EnceinteTypeManager {
	...
	private Log log = LogFactory.getLog(EnceinteTypeManager.class);
	...
	public void setEnceinteTypeValidator(EnceinteTypeValidator eValidator) {
		this.enceinteTypeValidator = eValidator;
	}

	@Override
	public EnceinteType findByIdManager(Integer enceinteTypeId) {
...
//	}
	...
	@Override
	public List<EnceinteType> findAllObjectsExceptBoiteManager(Plateforme pf) {
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			EnceinteType type = (EnceinteType) o;
			if (type.getEnceinteTypeId() == null) {
				return enceinteTypeDao.findAll().contains(type);
			} else {
				return enceinteTypeDao.findByExcludedId(
						type.getEnceinteTypeId()).contains(type);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object obj) {
		EnceinteType type = enceinteTypeDao.mergeObject(
				(EnceinteType) obj);
	...
	public void createObjectManager(Object obj) {
		...
		BeanValidator.validateObject(obj, new Validator[]{
				enceinteTypeValidator});
		if (!findDoublonManager(obj)) {
			enceinteTypeDao.createObject((EnceinteType) obj);
			log.info("Enregistrement objet EnceinteType " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors creation objet EnceinteType " 
					+ obj.toString());
			throw new DoublonFoundException("EnceinteType", 
					"creation");
		}
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (!findDoublonManager(obj)) {
			...
			log.info("Modification objet EnceinteType " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors modification objet EnceinteType "
					+ obj.toString());
			throw new DoublonFoundException("EnceinteType", 
					"modification");
		}
	...
	public void removeObjectManager(Object obj) {
		if (obj != null) {
			if (!isUsedObjectManager(obj)) {
				enceinteTypeDao.removeObject(
						((EnceinteType) obj).getEnceinteTypeId());
				log.info("Suppression objet EnceinteType " 
						+ obj.toString());
			} else {
				log.warn("Suppression objet EnceinteType " 
						+ obj.toString()
	...
	@Override
	public List<EnceinteType> findByTypeManager(String type,
			Plateforme pf) {
		if (type != null && pf != null) {
			return enceinteTypeDao.findByTypeAndPf(type, pf);
		} else {
			return new ArrayList<EnceinteType>();
		}
	}
}
