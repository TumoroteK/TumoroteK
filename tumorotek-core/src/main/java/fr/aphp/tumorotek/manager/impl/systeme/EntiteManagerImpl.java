...
public class EntiteManagerImpl implements EntiteManager {
	...
	private Log log = LogFactory.getLog(EntiteManager.class);
	 ...
	 * @return Une Entite.
	 */
	public Entite findByIdManager(Integer entiteId) {
	...
	@Override
	public List<Entite> findByNomManager(String nom) {
	...
	@Override
	public Object findObjectByEntiteAndIdManager(
			Entite entite, Integer objectId) {
		
		if (objectId != null && entite != null) {
			log.debug("Recherche l'objet correspondant au couple Entite : " 
					+ entite.toString() + " - ObjetId : " + objectId);
			String nomTable = entite.getNom();
			String first = nomTable.substring(0, 1);
			String end = nomTable.substring(1);
	...
	@Override
	public List<Integer> findIdsByEntiteAndIdAfterBanqueFiltreManager(
			Entite entite, List<Integer> objectIds, List<Banque> banks) {
		...
		if (objectIds != null && !objectIds.isEmpty()) {
			...
			if (banks != null && !banks.isEmpty()
					&& (entite.getNom().equals("Prelevement")
						|| entite.getNom().equals("Echantillon")
						|| entite.getNom().equals("ProdDerive")
						|| entite.getNom().equals("Cession"))) {
			...
			TypedQuery<Integer> query = 
						em.createQuery(sb.toString(), Integer.class);
