...
public class EnceinteTypeValidator implements Validator {
	...
	public void validate(Object obj, Errors errs) {
		//type non vide
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "type", "enceinteType.type.empty");
		
		EnceinteType type = (EnceinteType) obj;
		//type valide
		...
		//prefixe non vide
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "prefixe", 
					"enceinteType.prefixe.empty");
		
		//prefixe valide
		if (type.getPrefixe() != null) {
			if (!type.getPrefixe()
				.matches(ValidationUtilities.MOTREGEXP)) {
				errs.rejectValue("prefixe", 
						"enceinteType.prefixe.illegal");
			}
			if (type.getPrefixe().length() > 5) {
					errs.rejectValue("prefixe", 
							"enceinteType.prefixe.tooLong");
			}
		}
	}
}
