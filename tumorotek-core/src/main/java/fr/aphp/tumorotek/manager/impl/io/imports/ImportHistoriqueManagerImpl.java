...
public class ImportHistoriqueManagerImpl implements ImportHistoriqueManager {
	...
	private Log log = LogFactory.getLog(ImportHistoriqueManager.class);
	...
	public void setImportHistoriqueDao(ImportHistoriqueDao iDao) {
	...
	@Override
	public ImportHistorique findByIdManager(Integer importHistoriqueId) {
	...
	@Override
	public List<ImportHistorique> findByTemplateWithOrderManager(
			ImportTemplate importTemplate) {
		if (importTemplate != null) {
			return importHistoriqueDao.findByTemplateWithOrder(importTemplate);
		} else {
			return new ArrayList<ImportHistorique>();
		}
	}
	
	@Override
	public List<Importation> findImportationsByHistoriqueManager(
			ImportHistorique importHistorique) {
		if (importHistorique != null) {
			return importationDao.findByHistorique(importHistorique);
		} else {
			return new ArrayList<Importation>();
		}
	}
	
	@Override
	public List<Importation> findImportationsByHistoriqueAndEntiteManager(
			ImportHistorique importHistorique, Entite entite) {
		if (importHistorique != null && entite != null) {
			return importationDao.findByHistoriqueAndEntite(
					importHistorique, entite);
		} else {
			return new ArrayList<Importation>();
		}
	}
	...
	@Override
	public List<Importation> findImportationsByObjectManager(
			Object object) {
	...
	@Override
	public List<Importation> findImportationsByEntiteAndObjectIdManager(
	...
	@Override
	public void createObjectManager(ImportHistorique importHistorique,
			ImportTemplate importTemplate, Utilisateur utilisateur,
			List<Importation> importations) {
		// importTemplate required
		if (importTemplate != null) { 
			importHistorique.setImportTemplate(importTemplateDao
					.mergeObject(importTemplate));
		} else {
			log.warn("Objet obligatoire ImportTemplate manquant"
					+ " lors de la création d'un ImportHistorique");
			throw new RequiredObjectIsNullException(
					"ImportHistorique", "creation", "importTemplate");
		}
		
		// utilisateur required
		if (utilisateur != null) { 
			importHistorique.setUtilisateur(utilisateurDao
					.mergeObject(utilisateur));
		} else {
			log.warn("Objet obligatoire Utilisateur manquant"
					+ " lors de la création d'un ImportHistorique");
			throw new RequiredObjectIsNullException(
					"Utilisateur", "creation", "importTemplate");
		}
		
		if (importations != null) {
			importHistorique.setImportations(new HashSet<Importation>());
			for (int i = 0; i < importations.size(); i++) {
				Importation imp = importations.get(i);
		...
		log.info("Enregistrement objet ImportHistorique " 
				+ importHistorique.toString());
	}

	@Override
	public void removeObjectManager(ImportHistorique importHistorique) {
		if (importHistorique != null) {
			importHistoriqueDao.removeObject(
					importHistorique.getImportHistoriqueId());
			log.info("Suppression de l'objet ImportHistorique : " 
					+ importHistorique.toString());	
		} else {
	...
	@Override
	public void removeImportationManager(Importation importation) {
		if (importation != null) {
			importationDao.removeObject(
					importation.getImportationId());
			log.info("Suppression de l'objet Importation : " 
					+ importation.toString());	
		} else {
	...
	@Override
	public List<Prelevement> findPrelevementByImportHistoriqueManager(
			ImportHistorique ih) {
