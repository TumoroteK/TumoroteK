...
public class CritereManagerImpl implements CritereManager {
	...
	private Log log = LogFactory.getLog(CritereManager.class);
	 ...
	 */
	public void setCritereDao(CritereDao critDao) {
	 ...
	 */
	public void setCombinaisonManager(CombinaisonManager combManager) {
	 ...
	 */
	public void setCritereValidator(CritereValidator validator) {
	 ...
	 */
	@Override
	public Critere copyCritereManager(Critere critere) {
		Critere temp = null;
		//On vérifie que le critere n'est pas nul
		if (critere == null) {
			log.warn("Objet obligatoire Critere manquant lors " 
					+ "de la copie d'un objet Critere");
			throw new RequiredObjectIsNullException(
					"Critere", "copie", "Critere");
		} else {
			if (critere.getChamp() != null) {
				...
				temp = new Critere(champ, critere.getOperateur(),
						critere.getValeur());
			} else if (critere.getCombinaison() != null) {
				Combinaison combinaison = combinaisonManager
					.copyCombinaisonManager(critere.getCombinaison());
				temp = new Critere(combinaison, critere.getOperateur(),
						critere.getValeur());
			}
			BeanValidator.validateObject(critere,
					new Validator[] { critereValidator });
	 ...
	 */
	@Override
	public void createObjectManager(Critere critere,
			Champ champ, Combinaison combinaison) {
		//On vérifie que le critère n'est pas nul
		if (critere == null) {
			log.warn("Objet obligatoire Critere manquant lors " 
					+ "de la création d'un objet Critere");
			throw new RequiredObjectIsNullException(
					"Critere", "création", "Critere");
		}
		if (champ != null) {
	 ...
	 */
	@Override
	public void updateObjectManager(Critere critere, Champ champ,
			Combinaison combinaison) {
		//On vérifie que le critère n'est pas nul
		if (critere == null) {
			log.warn("Objet obligatoire Critere manquant lors " 
					+ "de la modification d'un objet Critere");
			throw new RequiredObjectIsNullException(
					"Critere", "modification", "Critere");
		}
		Champ oldChamp = critere.getChamp();
		if (champ != null) {
			if (champ.getChampId() != null) {
				champ = champDao.mergeObject(champ);
			} else {
		...
		if (oldChamp != null && oldChamp.getChampId() != null
				&& !oldChamp.equals(critere.getChamp())) {
	 ...
	 */
	@Override
	public void removeObjectManager(Critere critere) {
		//On vérifie que le critère n'est pas nul
		if (critere == null) {
			log.warn("Objet obligatoire Critere manquant lors " 
					+ "de la suppression d'un objet Critere");
			throw new RequiredObjectIsNullException(
					"Critere", "suppression", "Critere");
		}
		//On vérifie que le critère est en BDD
		if (findByIdManager(critere.getCritereId()) == null) {
			throw new SearchedObjectIdNotExistException("Critere",
					critere.getCritereId());
		}
		Champ oldChamp = critere.getChamp();
	 ...
	 */
	@Override
	public Critere findByIdManager(Integer idCritere) {
		//On vérifie que l'identifiant n'est pas nul
		if (idCritere == null) {
			log.warn("Objet obligatoire identifiant manquant lors de la " 
					+ "recherche par l'identifiant d'un objet Critere");
			throw new RequiredObjectIsNullException(
					"Critere", "recherche par identifiant", "identifiant");
		}
