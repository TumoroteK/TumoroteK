...
public class NonConformiteManagerImpl implements NonConformiteManager {
	...
	private Log log = LogFactory.getLog(NonConformiteManager.class);
	...
	public void setNonConformiteValidator(
			NonConformiteValidator nValidator) {
		this.nonConformiteValidator = nValidator;
	}

	public void setObjetNonConformeManager(
			ObjetNonConformeManager oManager) {
		this.objetNonConformeManager = oManager;
	}

	@Override
	public NonConformite findByIdManager(Integer nonConformiteId) {
	...
	@Override
	public List<NonConformite> findByPlateformeAndTypeObjManager(
			Plateforme plateforme, ConformiteType typeObj) {
		...
		if (plateforme != null && typeObj != null) {
			...
			return new ArrayList<NonConformite>();
		}
	}

	@Override
	public List<NonConformite> findByPlateformeEntiteAndTypeStringManager(
			Plateforme plateforme, String typeStr, Entite e) {
		log.debug("Recherche de tous les NonConformites en fct du type");
		if (plateforme != null && typeStr != null) {
			List<ConformiteType> types = conformiteTypeDao
					.findByEntiteAndType(typeStr, e);
			if (types.size() == 1) {
				return nonConformiteDao.findByTypeAndPf(
						types.get(0), plateforme);
			} else {
				return new ArrayList<NonConformite>();
			}
		} else {
	...
	@Override
	public boolean findDoublonManager(NonConformite nonConformite) {
		if (nonConformite != null) {
			if (nonConformite.getNonConformiteId() == null) {
				return nonConformiteDao.findAll().contains(nonConformite);
			} else {
				return nonConformiteDao.findByExcludedId(
						nonConformite.getNonConformiteId())
						.contains(nonConformite);
			}
		} else {
			return false;
		}
	...
	@Override
	public boolean isUsedObjectManager(NonConformite nonConformite) {
		List<ObjetNonConforme> objs = objetNonConformeDao
			.findByNonConformite(nonConformite);
		
		return (objs.size() > 0);
	}

	@Override
	public void createObjectManager(NonConformite nonConformite) {
		// On vérifie que la pf n'est pas null. Si c'est le cas on envoie
		// une exception
		if (nonConformite.getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"NonConformite", "creation", "Plateforme");
		} else {
			nonConformite.setPlateforme(plateformeDao
					.mergeObject(nonConformite.getPlateforme()));
		}
		...
		if (nonConformite.getConformiteType() == null) {
			throw new RequiredObjectIsNullException(
					"NonConformite", "creation", "ConformiteType");
		} else {
			nonConformite.setConformiteType(conformiteTypeDao
					.mergeObject(nonConformite.getConformiteType()));
		}
		
		if (findDoublonManager(nonConformite)) {
			log.warn("Doublon lors de la creation de l'objet " 
					+ "NonConformite : " + nonConformite.toString());
			throw new DoublonFoundException("NonConformite", "creation");
		} else {
			BeanValidator.validateObject(
					nonConformite, new Validator[]{nonConformiteValidator});
			nonConformiteDao.createObject(nonConformite);
			log.info("Enregistrement de l'objet NonConformite : " 
					+ nonConformite.toString());
		}
	}
	
	@Override
	public void updateObjectManager(NonConformite nonConformite) {
		// On vérifie que la pf n'est pas null. Si c'est le cas on envoie
		// une exception
		if (nonConformite.getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"NonConformite", "modification", "Plateforme");
		} else {
			nonConformite.setPlateforme(plateformeDao
					.mergeObject(nonConformite.getPlateforme()));
		}
		...
		if (nonConformite.getConformiteType() == null) {
			throw new RequiredObjectIsNullException(
					"NonConformite", "modification", "ConformiteType");
		} else {
			nonConformite.setConformiteType(conformiteTypeDao
					.mergeObject(nonConformite.getConformiteType()));
		}
		...
		if (findDoublonManager(nonConformite)) {
			...
			log.info("Modification de l'objet NonConformite : " 
					+ nonConformite.toString());
		}
	}

	@Override
	public void removeObjectManager(NonConformite nonConformite) {
		if (nonConformite != null) {
			...
			log.info("Suppression de l'objet NonConformite : " 
					+ nonConformite.toString());
		}
	}
	
	@Override
	public List<NonConformite> getFromObjetNonConformes(List<ObjetNonConforme> objs) {
		List<NonConformite> nonConfs = new ArrayList<NonConformite>();
		if (objs != null) {
			Iterator<ObjetNonConforme> it = objs.iterator();
			while (it.hasNext()) {
