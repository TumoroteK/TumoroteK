...
/**
 * pas de spécificité pour le produit dérivé pour le moment donc la classe reste vide
 * 
 ...
 *
 */
public class ImportChampAnnotationProdDeriveStrategy extends AbstractImportChampAnnotationEntiteStrategy
{

   //nomChampForControle vaut null dans le cas des produits dérivés car aucun champ de contrôle n'est prévu à date
   @Override
    ...
    * @param codes, codes des produits dérivés
    * @param banque, banque à prendre en compte
    * @return Liste des codes existants et de l'id associé.
    * @since 2.3.1
    */
   ///!\ cette requête est volontairement mis ici et non dans ProdDeriveManagerImpl car du fait de l'héritage, il ne faut pas gérer de transaction (cf commentaire TK-538 dans applicationContextManager.xml)
   private List<CodeIdPair> findIdByCodesAndBanque(List<String> codes, Banque banque) {
      String hql = "SELECT new "+CodeIdPair.class.getName() + "(p.code, p.prodDeriveId) FROM ProdDerive p WHERE p.code in (:listCode) AND p.banque = :banque ";
