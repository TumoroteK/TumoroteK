...
public class CouleurManagerImpl implements CouleurManager {
	...
	private Log log = LogFactory.getLog(CouleurManager.class);
	...
	@Override
	public List<Couleur> findByCouleurLikeManager(String couleur,
			boolean exactMatch) {
		log.debug("Recherche Couleur par " 
				+ couleur + " exactMatch " + String.valueOf(exactMatch));
		if (couleur != null) {
			if (!exactMatch) {
				couleur = couleur + "%";
			}
	...
	@Override
	public Couleur findByIdManager(Integer couleurId) {
