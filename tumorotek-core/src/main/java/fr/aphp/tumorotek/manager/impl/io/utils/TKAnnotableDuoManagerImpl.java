...
public class TKAnnotableDuoManagerImpl implements TKAnnotableDuoManager {
	...
	@Override
	public List<TKAnnotableObjectPropertyDuo> compareObjectsDuoManager(TKAnnotableObjectDuo duo, 
									List<ChampEntite> chpEntites, List<ChampAnnotation> chpAnnos) {
		
		List<TKAnnotableObjectPropertyDuo> propCompList = 
								new ArrayList<TKAnnotableObjectPropertyDuo>();
		
		if (duo != null && duo.getFirstObj() != null && duo.getSecondObj() != null) {
			TKAnnotableObjectPropertyDuo propDuo;
			try {
				// champs TK
				for (ChampEntite chpE : chpEntites) {
				...
				// annotations
				for (ChampAnnotation chpA : chpAnnos) {
					propDuo = new TKAnnotableObjectPropertyDuo();
	...
	private void populateTKPropDuo(ChampEntite chpE, TKAnnotableObjectDuo duo, TKAnnotableObjectPropertyDuo propDuo) 
		...
		if (duo.getEntite().getNom().equals("Prelevement") && chpE.getNom().equals("Risques")) {
		...
		} else if (duo.getEntite().getNom().equals("Prelevement") 
									&& chpE.getNom().equals("ConformeArrivee.Raison")) {
		...
		} else if (duo.getEntite().getNom().equals("Echantillon") 
				&& chpE.getNom().equals("ConformeTraitement.Raison")) {
			firstValue = duo.getFirstNoConfs();
			secondValue = duo.getSecondNoConfs();
			...
			if (!((List<NonConformite>) firstValue).containsAll((List<NonConformite>) secondValue)
				...
				propDuo.setChampEntite(chpE);
					...
					&& chpE.getNom().equals("ConformeCession.Raison")) {
//			firstValue = duo.getFirstNoConfs();
//			secondValue = duo.getSecondNoConfs();
//
//			if (!((List<NonConformite>) firstValue).containsAll((List<NonConformite>) secondValue)
...
//			}
			...
			String nomChamp = chpE.getNom().replaceFirst(".", 
					(chpE.getNom().charAt(0) + "").toLowerCase());
			if (nomChamp.endsWith("Id")) {
				nomChamp = nomChamp.substring(0, nomChamp
						.length() - 2);
			}
			if (PropertyUtils.isReadable(duo.getFirstObj(), nomChamp)
					&& PropertyUtils.isReadable(duo.getSecondObj(), nomChamp)) {
	 ...
	 * @throws IllegalAccessException 
	 */
	private void populateAnnoPropDuo(ChampAnnotation chpA, TKAnnotableObjectDuo duo, 
													TKAnnotableObjectPropertyDuo propDuo) {
		
		List<AnnotationValeur> firstAnnoValue = duo.getFirstAnnoValsMap().get(chpA);
		List<AnnotationValeur> secondAnnoValue = duo.getSecondAnnoValsMap().get(chpA);
		...
		if (!chpA.getDataType().getType().equals("thesaurusM")) {
			if (!firstAnnoValue.isEmpty()) {
				if (!secondAnnoValue.isEmpty()) { // second valeur existe 
					if (!firstAnnoValue.get(0).getValeur()
							.equals(secondAnnoValue.get(0).getValeur())) {
						isDiff = true;
					}
			...
			if (!firstAnnoValue.isEmpty()) {
				for (AnnotationValeur faV : firstAnnoValue) {
					isDiff = true;
					for (AnnotationValeur saV : secondAnnoValue) { // elle est dans l'autre liste
				...
				if (!isDiff) { // teste l'égalité dans l'autre sens
					for (AnnotationValeur saV : secondAnnoValue) {
						isDiff = true;
						for (AnnotationValeur faV : firstAnnoValue) { // elle est dans l'autre liste
	...
	@Override
	public Boolean mergeDuoObjectsManager(TKAnnotableObjectDuo duo, 
		...
		try {
			if (propDuos != null && duo != null) {
				...
				for (TKAnnotableObjectPropertyDuo pDuo : propDuos) {
	 ...
	 * @throws IllegalAccessException 
	 */
	private boolean mergeValues(TKAnnotableObject secondObj, TKAnnotableObjectPropertyDuo propDuo) 
						throws IllegalAccessException, InvocationTargetException, NoSuchMethodException {
		...
		} else if (secondObj instanceof Prelevement 
							&& propDuo.getChampEntite().getNom().equals("ConformeArrivee.Raison")) {

		} else if (secondObj instanceof Echantillon
				&& propDuo.getChampEntite().getNom().equals("ConformeTraitement.Raison")) {

		} else if (secondObj instanceof Echantillon
				&& propDuo.getChampEntite().getNom().equals("ConformeCession.Raison")) {

		} else {
			String nomChamp = Utils.getReadablePropertyFromChampEntite(propDuo.getChampEntite());
	 ...
	 */
	@SuppressWarnings("unchecked")
	private boolean mergeAnnoValues(TKAnnotableObjectDuo duo, TKAnnotableObjectPropertyDuo pDuo) {
