...
public class ResultatManagerImpl implements ResultatManager {
	...
	private Log log = LogFactory.getLog(ResultatManager.class);
	 ...
	 */
	public void setResultatDao(ResultatDao resDao) {
	 ...
	 */
	public void setResultatValidator(ResultatValidator validator) {
	 ...
	 */
	@Override
	public Resultat findByIdManager(Integer idResultat) {
		//On vérifie que l'identifiant n'est pas nul
		if (idResultat == null) {
			log.warn("Objet obligatoire identifiant manquant lors de la " 
					+ "recherche par l'identifiant d'un objet Resultat");
			throw new RequiredObjectIsNullException(
					"Resultat", "recherche par identifiant", "identifiant");
		}
	 ...
	 */
	@Override
	public Resultat copyResultatManager(Resultat resultat,
			Affichage affichage) {
		//On vérifie que le résultat n'est pas nul
		if (resultat == null) {
			log.warn("Objet obligatoire Resultat manquant lors " 
					+ "de la copie d'un objet Resultat");
			throw new RequiredObjectIsNullException(
					"Resultat", "copie", "Resultat");
		}
		...
		Resultat temp = new Resultat(resultat.getNomColonne(), champ,
				resultat.getTri(), resultat.getOrdreTri(),
				resultat.getPosition(), resultat.getFormat(), affichage);
		BeanValidator.validateObject(resultat,
				new Validator[] { resultatValidator });
	 ...
	 */
	@Override
	public void createObjectManager(Resultat resultat, Affichage affichage,
			Champ champ) {
		//On vérifie que le résultat n'est pas nul
		if (resultat == null) {
			log.warn("Objet obligatoire Resultat manquant lors " 
					+ "de la création d'un objet Resultat");
			throw new RequiredObjectIsNullException(
					"Resultat", "création", "Resultat");
		}
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors " 
	 ...
	 */
	@Override
	public void updateObjectManager(Resultat resultat, Affichage affichage,
			Champ champ) {
		//On vérifie que le résultat n'est pas nul
		if (resultat == null) {
			log.warn("Objet obligatoire Resultat manquant lors " 
					+ "de la modification d'un objet Resultat");
			throw new RequiredObjectIsNullException(
					"Resultat", "modification", "Resultat");
		}
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors " 
					+ "de la modification d'un objet Resultat");
			throw new RequiredObjectIsNullException(
					"Resultat", "modification", "Affichage");
		}
		//On vérifie que le champ n'est pas nul
		if (champ == null) {
			log.warn("Objet obligatoire Champ manquant lors " 
					+ "de la modification d'un objet Resultat");
			throw new RequiredObjectIsNullException(
					"Resultat", "modification", "Champ");
		}
		Champ oldChamp = resultat.getChamp();
		if (champ != null) {
			if (champ.getChampId() != null) {
				champ = champDao.mergeObject(champ);
			} else {
		...
		if (oldChamp != null && oldChamp.getChampId() != null
				&& !oldChamp.equals(resultat.getChamp())) {
	 ...
	 */
	@Override
	public void removeObjectManager(Resultat resultat) {
		//On vérifie que le resultat n'est pas nul
		if (resultat == null) {
			throw new RequiredObjectIsNullException("Resultat",
					"suppression", "Resultat");
		}
		//On vérifie que le resultat est en BDD
		if (findByIdManager(resultat.getResultatId()) == null) {
			throw new SearchedObjectIdNotExistException("Resultat",
	 ...
	 */
	@Override
	public List<Resultat> findByAffichageManager(Affichage affichage) {
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors de la " 
					+ "recherche par l'Affichage d'un objet Resultat");
			throw new RequiredObjectIsNullException(
					"Resultat", "recherche par Affichage", "Affichage");
		}
