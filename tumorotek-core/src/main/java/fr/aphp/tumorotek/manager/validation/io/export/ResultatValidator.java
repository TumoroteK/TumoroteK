...
public class ResultatValidator implements Validator
{
   ...
   public void validate(final Object obj, final Errors errs){
      //nomColonne non vide
      ValidationUtils.rejectIfEmptyOrWhitespace(errs, "nomColonne", "resultat.nomColonne.empty");

      final Resultat resultat = (Resultat) obj;
      //nomColonne valide
      if(resultat.getNomColonne() != null){
         if(!resultat.getNomColonne().matches(ValidationUtilities.MOTREGEXP)){
      ...
      if(resultat.getOrdreTri() != null){
         if(resultat.getOrdreTri() <= 0){
            errs.rejectValue("ordreTri", "resultat.ordreTri.negativeOrZero");
         }
      }else{
      ...
      if(resultat.getPosition() != null){
         if(resultat.getPosition() <= 0){
            errs.rejectValue("position", "resultat.position.negativeOrZero");
         }
      }else{
