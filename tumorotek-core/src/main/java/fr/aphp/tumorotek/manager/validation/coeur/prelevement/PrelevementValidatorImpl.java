...
//	}
	...
	public void validate(Object obj, Errors errs) {
		...
		Prelevement prel = (Prelevement) obj; 
		
		//code non null
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "code", "prelevement.code.empty");
		//code valide
		...
		if (prel.getTransportTemp() != null 
				&& !FloatValidator.getInstance()
				.isInRange(prel.getTransportTemp(), -1000.0, 1000.0)) {
			errs.rejectValue("transportTemp", 
					"prelevement.transportTemp.notInRange");
		}
		...
		if (prel.getPatientNda() != null) {
			//nda non vide
			ValidationUtils
				.rejectIfEmptyOrWhitespace(errs, "patientNda", 
											"prelevement.patientNda.empty");
			if (!prel.getPatientNda()
				.matches(ValidationUtilities.CODEREGEXP)) {
				errs.rejectValue("patientNda", 
								"prelevement.patientNda.illegal");
			}
			if (prel.getPatientNda().length() > 20) {
				errs.rejectValue("patientNda", 
								"prelevement.patientNda.tooLong");
			}
		}
		...
		if (prel.getNumeroLabo() != null) {
			//numeroLabo non vide
			ValidationUtils
				.rejectIfEmptyOrWhitespace(errs, "numeroLabo", 
											"prelevement.numeroLabo.empty");
			
			if (!prel.getNumeroLabo()
				.matches(ValidationUtilities.CODEREGEXP)) {
				errs.rejectValue("numeroLabo", 
								"prelevement.numeroLabo.illegal");
			}
			if (prel.getNumeroLabo().length() > 50) {
				errs.rejectValue("numeroLabo", 
								"prelevement.numeroLabo.tooLong");
			}
		}
		
		// congelation coherente
		if (prel.getCongDepart() != null && prel.getCongDepart()
				&& prel.getCongArrivee() && prel.getCongArrivee()) {
			errs.rejectValue("congDepart", 
					"prelevement.congelation.bothDepartArrivee");
		}
	...
	@Override
	public Errors checkDatePrelevementCoherence(Prelevement 
															prelevement) {
		
		BindException errs = new BindException(prelevement, 
					"fr.aphp.tumorotek.model.coeur.prelevement.Prelevement");
		...
		if (prelevement.getDatePrelevement() != null) {
			if (prelevement.getMaladie() != null 
				...
				ValidationUtilities.checkWithDate(prelevement
														.getDatePrelevement(), 
															"datePrelevement", 
					prelevement.getMaladie().getPatient().getDateNaissance(), 
						"date", "validation", "DateNaissance", errs, false);
			}
			
			//limites sup
			if (prelevement.getDateDepart() != null) {
				ValidationUtilities.checkWithDate(prelevement
								.getDatePrelevement(), "datePrelevement", 
												prelevement.getDateDepart(), 
				"date", "validation", "DateDepartPrelevement", errs, true);
			} else if (prelevement.getLaboInters() != null
					&& coherenceDateManager.findPostRefDateInLabosManager(new 
				ArrayList<LaboInter>(prelevement.getLaboInters()))[0] != null) {
				Object[] dateAndCode = 
					coherenceDateManager.findPostRefDateInLabosManager(new 
							ArrayList<LaboInter>(prelevement.getLaboInters()));
				if (dateAndCode[0] != null) {
					if (!ValidationUtilities.checkWithDate(prelevement
						.getDatePrelevement(), null, 
						dateAndCode[0], null, null, null, null, true)) {
						errs.rejectValue("datePrelevement", 
				...
				if (!ValidationUtilities
						.checkWithDate(prelevement.getDatePrelevement(), null, 
								dateAndCode[0], null, null, null, null, true)) {
					errs.rejectValue("datePrelevement", 
												(String) dateAndCode[1]);
				}
	...
	@Override
	public Errors checkDateConsentCoherence(Prelevement prelevement) {
		...
		if (prelevement.getConsentDate() != null) {
			...
			if (!prelevement.getConsentDate() 
					.before(Utils.getCurrentSystemDate())) {
						errs.rejectValue("consentDate", 
	...
	@Override
	public Errors checkDateDepartCoherence(Prelevement prelevement) {
		...
		if (prelevement.getDateDepart() != null) {
			// limites inf
			if (prelevement.getDatePrelevement() != null) {
				ValidationUtilities.checkWithDate(prelevement.getDateDepart(), 
																"dateDepart", 
			...
			if (prelevement.getLaboInters() != null
				...
				//			findPostRefDateInEchantillonsManager(prelevement);
				if (!ValidationUtilities
					.checkWithDate(prelevement.getDateDepart(), null, 
						Utils.getCurrentSystemDate(), null, null, 
						null, null, true)) {
	...
	@Override
	public Errors checkDateArriveeCoherence(Prelevement prelevement) {
		...
		if (prelevement.getDateArrivee() != null) {
			...
			// limites inf
			if (prelevement.getDateDepart() != null) {
				ValidationUtilities.checkWithDate(prelevement.getDateArrivee(), 
																"dateArrivee", 
												prelevement.getDateDepart(), 
				"date", "validation", "DateDepartPrelevement", errs, false);
			} else {
				if (prelevement.getDatePrelevement() != null) {
					ValidationUtilities.checkWithDate(prelevement
											.getDateArrivee(), "dateArrivee", 
											prelevement.getDatePrelevement(), 
						"date", "validation", "DatePrelevement", errs, false);
				} else {
					if (prelevement.getMaladie() != null 
							&& prelevement.getMaladie().getPatient()
												.getDateNaissance() != null) { 
						ValidationUtilities
							.checkWithDate(prelevement.getDateArrivee(), 
																"dateArrivee", 
							prelevement.getMaladie().getPatient()
														.getDateNaissance(), 
			...
			//				findPostRefDateInEchantillonsManager(prelevement);
			if (!ValidationUtilities
					.checkWithDate(prelevement.getDateArrivee(), null, 
							Utils.getCurrentSystemDate(), 
