...
echantillon.EchantillonTypeValidator;
...
public class EchantillonTypeManagerImpl implements EchantillonTypeManager {
	...
	private Log log = LogFactory.getLog(EchantillonTypeManager.class);
	...
	public void setEchantillonTypeValidator(
			EchantillonTypeValidator validator) {
	 ...
	 */
	public EchantillonType findByIdManager(Integer echantillonTypeId) {
...
//	}
	 ...
	 * @return Un EchantillonType.
	 */
	public EchantillonType findByEchantillonManager(Echantillon echantillon) {
		List<EchantillonType> types = echantillonTypeDao.
	...
	public Set<Echantillon> getEchantillonsManager(EchantillonType type) {
		...
		Set<Echantillon> echans = type.getEchantillons();
		echans.size();
		return echans;
	}
	 ...
	 * @return Liste de EchantillonType.
	 */
	public List<EchantillonType> findByTypeLikeManager(
			String type, boolean exactMatch) {
		log.debug("Recherche EchantillonType par " 
		...
		if (type != null) {
			...
			return new ArrayList<EchantillonType>();
		}
	}
	
	@Override
	public boolean findDoublonManager(Object obj) {
		EchantillonType type = (EchantillonType) obj;
		if (type != null) {
			if (type.getEchantillonTypeId() == null) {
				return echantillonTypeDao.findAll().contains(type);
			} else {
				return echantillonTypeDao.findByExcludedId(
						type.getEchantillonTypeId())
						.contains(type);
			}
		} else {
			return false;
	...
	public boolean isUsedObjectManager(Object obj) {
		List<Echantillon> echans = echantillonDao
			.findByEchantillonType((EchantillonType) obj);
	...
	public void createObjectManager(Object obj) {
		...
		if (type.getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"EchantillonType", "creation", "Plateforme");
		} else {
			type.setPlateforme(plateformeDao
					.mergeObject(type.getPlateforme()));
		}
		
		if (findDoublonManager(type)) {
			log.warn("Doublon lors de la creation de l'objet " 
					+ "EchantillonType : " + type.toString());
			throw new DoublonFoundException("EchantillonType", "creation");
		} else {
			BeanValidator.validateObject(
					type, new Validator[]{echantillonTypeValidator});
			echantillonTypeDao.createObject(type);
			log.info("Enregistrement de l'objet EchantillonType : " 
					+ type.toString());
		}
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (findDoublonManager(type)) {
			...
			log.info("Modification de l'objet EchantillonType : " 
					+ type.toString());
		}
	}
	...
	public void removeObjectManager(Object obj)  {
		...
		if (isUsedObjectManager(type)) {
			...
			log.info("Suppression de l'objet EchantillonType : " 
					+ type.toString());
		}
	}
