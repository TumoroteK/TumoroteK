...
public class ProdQualiteManagerImpl implements ProdQualiteManager {
	...
	private Log log = LogFactory.getLog(ProdQualiteManager.class);
	...
	public void setProdQualiteValidator(ProdQualiteValidator validator) {
	 ...
	 */
	public ProdQualite findByIdManager(Integer prodQualiteId) {
...
//	}
	 ...
	 * @return Liste de ProdQualite.
	 */
	public List<ProdQualite> findByQualiteLikeManager(
			String qualite, boolean exactMatch) {
		log.debug("Recherche ProdQualite par " 
		...
		if (qualite != null) {
			...
			return new ArrayList<ProdQualite>();
		}
	}
	
	@Override
	public boolean findDoublonManager(Object obj) {
		ProdQualite qualite = (ProdQualite) obj;
		if (qualite != null) {
			if (qualite.getProdQualiteId() == null) {
				return prodQualiteDao.findAll().contains(qualite);
			} else {
				return prodQualiteDao.findByExcludedId(
						qualite.getProdQualiteId()).contains(qualite);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object obj) {
		...
		List<ProdDerive> list = prodDeriveDao.findByProdQualite(qualite);
	...
	public void createObjectManager(Object obj) {
		...
		if (qualite.getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"ProdQualite", "creation", "Plateforme");
		} else {
		...
		if (findDoublonManager(qualite)) {
			log.warn("Doublon lors de la creation de l'objet ProdQualite : " 
					+ qualite.toString());
			throw new DoublonFoundException("ProdQualite", "creation");
		} else {
			BeanValidator.validateObject(
					qualite, new Validator[]{prodQualiteValidator});
			prodQualiteDao.createObject(qualite);
			log.info("Enregistrement de l'objet ProdQualite : " 
					+ qualite.toString());
		}
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (findDoublonManager(qualite)) {
			...
			log.info("Modification de l'objet ProdQualite : " 
					+ qualite.toString());
		}
	}
...
//		} else {
			...
			log.info("Suppression de l'objet ProdQualite : " 
					+ qualite.toString());
//		}
	}
