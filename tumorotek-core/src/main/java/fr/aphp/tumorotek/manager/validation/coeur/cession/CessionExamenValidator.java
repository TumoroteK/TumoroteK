...
public class CessionExamenValidator implements Validator {
	...
	public void validate(Object obj, Errors errs) {
		//Nature non vide
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "examen", 
					"cessionExamen.examen.empty");
		
		CessionExamen examen = (CessionExamen) obj;
		//nom valide
		if (examen.getExamen() != null) {
			if (!examen.getExamen()
				.matches(ValidationUtilities.MOTREGEXP)) {
				errs.rejectValue("examen", 
						"cessionExamen.examen.illegal");
			}
			if (examen.getExamen().length() > 200) {
					errs.rejectValue("examen", 
							"cessionExamen.examen.tooLong");
			}
		}
	}
