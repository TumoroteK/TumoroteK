...
public class DroitObjetManagerImpl implements DroitObjetManager {
	...
	private Log log = LogFactory.getLog(DroitObjetManager.class);
	...
	public void setDroitObjetDao(DroitObjetDao dDao) {
	...
	@Override
	public DroitObjet findByIdManager(DroitObjetPK droitObjetPK) {
	...
	@Override
	public List<DroitObjet> findByExcludedPKManager(DroitObjetPK pk) {
		if (pk != null) {
	...
	@Override
	public List<DroitObjet> findByProfilManager(Profil profil) {
		if (profil != null) {
			return droitObjetDao.findByProfil(profil);
		} else {
			return new ArrayList<DroitObjet>();
		}
	}

	@Override
	public List<DroitObjet> findByProfilEntiteManager(Profil profil,
			Entite entite) {
	...
	@Override
	public List<OperationType> getOperationsByProfilEntiteManager(
			Profil profil, String nomEntite) {
		...
		List<DroitObjet> droits = findByProfilEntiteManager(profil, entite);
	...
	@Override
	public List<DroitObjet> findByProfilOperationManager(Profil profil,
			OperationType type) {
	...
	@Override
	public Boolean hasProfilOperationOnEntitesManager(Profil profil,
			OperationType type, List<Entite> entites) {
				
		if (entites != null && !entites.isEmpty()) {
			for (Entite entite : entites) {
				if (!getOperationsByProfilEntiteManager(profil, 
								entite.getNom()).contains(type)) {
					return false;
				}
			}
		} else {
	...
	@Override
	public Boolean findDoublonManager(Profil profil, Entite entite,
			OperationType type) {
		
		DroitObjetPK pk = new DroitObjetPK(profil, entite, type);
		
		return (droitObjetDao.findById(pk) != null);
	}

	@Override
	public void validateObjectManager(Profil profil, Entite entite,
			OperationType type) {
		//profil required
		if (profil == null) {
			log.warn("Objet obligatoire Profil manquant"
							+ " lors de la validation d'un DroitObjet");
			throw new RequiredObjectIsNullException(
					"DroitObjet", "creation", "Profil");
		}
		...
		if (type == null) {
			log.warn("Objet obligatoire OperationType manquant"
							+ " lors de la validation d'un DroitObjet");
			throw new RequiredObjectIsNullException(
					"DroitObjet", "creation", "OperationType");
		}
	...
	@Override
	public void createObjectManager(DroitObjet droitObjet, Profil profil,
			Entite entite, OperationType type) {
		...
		log.info("Enregistrement objet DroitObjet " 
				+ droitObjet.toString());
		
	}

	@Override
	public void removeObjectManager(DroitObjet droitObjet) {
		if (droitObjet != null) {
			droitObjetDao.removeObject(droitObjet.getPk());
			log.info("Suppression de l'objet DroitObjet : " 
					+ droitObjet.toString());				
		} else {
