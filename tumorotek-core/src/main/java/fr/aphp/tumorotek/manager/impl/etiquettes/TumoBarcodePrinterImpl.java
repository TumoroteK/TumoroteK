...
public class TumoBarcodePrinterImpl implements TumoBarcodePrinter {
	...
	private ComponentPrinter componentPrinter = new ComponentPrinter();

	public TumoBarcodePrinterImpl() {

	}

	public void setTumoPrinterUtilsManager(TumoPrinterUtilsManager tManager) {
...
//	}
	...
	@Override
	public int printListCopiesData(List<List<LigneEtiquette>> liste, int nb, 
			Imprimante imprimante, Modele modele, String rawLang, BarcodeFieldDefault by) 
					throws PrinterException {
		int codeRetour = 0;
		...
		for (int i = 0; i < liste.size(); i++) {
			int j = 1;
			while (j <= nb) {
				components.add(ComponentPrinter
					.getComponent(liste.get(i), imprimante, modele, null));
				j++;
			}
		}
...
//		return codeRetour;
	...
	@Override
	public int printStockableObjects(List<? extends TKStockableObject> objects,
			int nb, Imprimante imp, Modele mod, String rawLang, BarcodeFieldDefault by) 
			throws PrinterException {
		boolean isDefault = true;
		List<LigneEtiquette> lignes = null;
		List<ComponentBarcodeLabel> components = new ArrayList<ComponentBarcodeLabel>();
		...
		for (i = 0; i < objects.size(); i++) {
			TKStockableObject obj = objects.get(i);
			if (isDefault) {
