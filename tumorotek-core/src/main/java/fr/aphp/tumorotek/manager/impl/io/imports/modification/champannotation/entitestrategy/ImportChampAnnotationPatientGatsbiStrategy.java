...
/**
 * strategy dédiée au patient pour les collections Gatsbi pour faire les contrôles en amont du traitement de modification des annotations par import.
 * 
 ...
 *
 */
public class ImportChampAnnotationPatientGatsbiStrategy extends ImportChampAnnotationPatientStrategy
{
   @Override
    ...
    * A noter que si le nom n'est pas visible pour la collection concernée, la valeur null sera ramenée pour chaque objet.
    * @param nips
    * @param banque collection concernée
   ...
   public List<CodeIdPair> findIdAndDataForControleByCodesAndBanque(List<String> listCodeForSelect, Banque banque, String nomChampForControle) {
      ...
      if(nomChampForControle != null) {
         ...
         hqlPartForControle = new StringBuilder( ", patId.pk.patient.").append(nomChampWithFirstCharInMinuscule).toString();
      }
      StringBuilder hql = new StringBuilder();
      hql.append("SELECT new ").append(CodeIdPair.class.getName()).append("(patId.identifiant, patId.pk.patient.patientId").append(hqlPartForControle).append(") ");
      hql.append("FROM PatientIdentifiant patId WHERE patId.identifiant in (:listIdentifiant) AND patId.pk.banque = :banque ");
      
      TypedQuery<CodeIdPair> query = getEntityManagerFactory().createEntityManager().createQuery(hql.toString(), CodeIdPair.class);
      query.setParameter("listIdentifiant", listCodeForSelect);
