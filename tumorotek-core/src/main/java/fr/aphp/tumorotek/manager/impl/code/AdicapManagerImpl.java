...
public class AdicapManagerImpl implements AdicapManager {
	...
	private Log log = LogFactory.getLog(AdicapManager.class);
	
	private AdicapDao adicapDao;
	private AdicapGroupeDao adicapGroupeDao;
		
	public void setAdicapDao(AdicapDao aDao) {
		this.adicapDao = aDao;
	}

	public void setAdicapGroupeDao(AdicapGroupeDao agDao) {
	...
	public List< ? extends CodeCommon> findByCodeLikeManager(String code, 
		...
		log.debug("Recherche Adicap par code: " 
	...
	public List<Adicap> findByAdicapGroupeManager(AdicapGroupe groupe) {
		...
		if (groupe != null && (groupe.getNom().equals("D8") 
				|| groupe.getNom().equals("D6"))) {
			groupe = adicapGroupeDao.findById(3);
		}
	...
	public List< ? extends CodeCommon> findByLibelleLikeManager(String libelle, 
		...
		log.info("Recherche Adicap par libelle: " 
				+ libelle + " exactMatch " + String.valueOf(exactMatch));
		return adicapDao.findByLibelleLike(libelle);
	}

	@Override
	public List<Adicap> findByMorphoManager(Boolean isMorpho) {
		return adicapDao.findByMorpho(isMorpho);
	}
	
	@Override
	public List<Adicap> findByAdicapParentManager(Adicap parent, 
														Boolean isTopo) {
		
		List<Adicap> ads = adicapDao
				.findByAdicapParentAndCodeOrLibelle(parent, "%");
				...
				for (int i = 0; i < ads.size(); i++) {
					if (ads.get(i).getAdicapGroupe().getNom().equals("D8")
						|| (ads.get(i).getAdicapGroupe().getNom().equals("D3")
							&& findByAdicapParentManager(ads.get(i), false)
																.isEmpty())) {
						ads.remove(ads.get(i));
						i--;
					}
	...
	@Override
	public Set<CimMaster> getCimMastersManager(Adicap adicap) {
		Set<CimMaster> cims = new HashSet<CimMaster>();
		Adicap aM = adicapDao.mergeObject(adicap);
	...
	@Override
	public Set<CimoMorpho> getCimoMorphosManager(Adicap adicap) {
		Set<CimoMorpho> cims = new HashSet<CimoMorpho>();
		Adicap aM = adicapDao.mergeObject(adicap);
	...
	@Override
	public List<AdicapGroupe> getAdicapGroupesManager(AdicapGroupe groupe) {
		List<AdicapGroupe> groupes = new ArrayList<AdicapGroupe>();
		AdicapGroupe aG = adicapGroupeDao.mergeObject(groupe);
	...
	@Override
	public List<Adicap> findChildrenCodesManager(Adicap code, 
										AdicapGroupe groupe, 
										String codeOrLibelle) {
		List<Adicap> codes = new ArrayList<Adicap>();
		if (code != null) {
			codes.add(code);
			findRecursiveChildren(code, codes, codeOrLibelle);
		} else if (groupe != null) {
			List<AdicapGroupe> grps = new ArrayList<AdicapGroupe>();
			grps.add(groupe);
			findRecursiveGroup(groupe, grps);
			List<Adicap> grpCodes;
			for (int i = 0; i < grps.size(); i++) {
				grpCodes = adicapDao
					.findByAdicapGroupeAndCodeOrLibelle(grps.get(i), 
							codeOrLibelle);
	...
	private void findRecursiveChildren(Adicap parent, List<Adicap> codes, 
													String codeOrLibelle) {
		List<Adicap> children  = adicapDao
				.findByAdicapParentAndCodeOrLibelle(parent, codeOrLibelle);
	...
	private void findRecursiveGroup(AdicapGroupe groupe, 
												List<AdicapGroupe> grps) {
		List<AdicapGroupe> groups = getAdicapGroupesManager(groupe);
		grps.addAll(groups);
	...
	@Override
	public List<Adicap> findByDicoAndCodeOrLibelleManager(AdicapGroupe grp,
