...
//			"[a-z_A-Z0-9'öÖôÔàâéèêôùûçîïÀÂÉÈÔÙÛÇ\\s\\.+\\-:();?]+";
	...
	public static final String MOTNOPUNCTREGEXP = 
			"[\\p{L}|\\p{N}|\\p{Z}|\\p{M}|\\+\\-@/<=>$%°~]+";
	...
	public static final String DATATYPE_REGEXP = "(alphanum)|(text)|(date)" 
		+ "|(bool)|(item)";
	
	public static final String MAILREGEXP = "[a-zA-Z0-9_.+\\-@]+";
	
	public static final String HTTPREGEXP = 
					"(https?:\\/\\/){0,1}[a-zA-Z0-9_\\s\\.\\-\\/\\?=\\+&%]+";
	
	public static final String PATHREGEXP = "[:a-zA-Z0-9_\\.\\-\\/\\s]+";
	
	public static final String FILENAMEREGEXP = "[a-zA-Z0-9_\\.\\-]+"; 
	
	public static final String PASSWORDREGEXP = 
		"^.*(?=.*[a-z])(?=.*[A-Z])(?=.*[\\d])(?=.*[\\W]).*$";
	 ...
	 */
	public static boolean checkWithDate(Object date, String champ, 
			Object dateRef, String prefixe, String mid, String suffixe, 
			Errors errs, boolean before) {
		...
		if (date instanceof Calendar && dateRef instanceof Calendar) {
			if (((Calendar) date).get(Calendar.HOUR) == 0
					&& ((Calendar) date).get(Calendar.HOUR_OF_DAY) == 0
					&& ((Calendar) date).get(Calendar.MINUTE) == 0
					&& ((Calendar) date).get(Calendar.SECOND) == 0) {
				if (((Calendar) dateRef).get(Calendar.HOUR) != 0
						|| ((Calendar) dateRef).get(Calendar.HOUR_OF_DAY) != 0
						|| ((Calendar) dateRef).get(Calendar.MINUTE) != 0
						|| ((Calendar) dateRef).get(Calendar.SECOND) != 0) {
			...
			} else if (((Calendar) dateRef).get(Calendar.HOUR) == 0
					&& ((Calendar) dateRef).get(Calendar.HOUR_OF_DAY) == 0
					&& ((Calendar) dateRef).get(Calendar.MINUTE) == 0
					&& ((Calendar) dateRef).get(Calendar.SECOND) == 0) {
		...
		if (!date1.equals(date2)) {
			if (before) {
				if (!date1.before(date2)) {
					if (errs != null) {
						errs.rejectValue(champ, 
							prefixe + "." + mid + ".sup" + suffixe);
					} else {
						return false;
					}
				}
