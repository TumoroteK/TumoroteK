...
public class InterfacageParsingUtilsImpl implements InterfacageParsingUtils
{
	...
	@Override
	public String extractInboxParamFromFile(final String fileXml, final String file, 
											final String boiteFtp, final String balise)
			throws IOException {
	...
	public List<Hashtable<String, List<String>>> parseFileToInjectInTk(final ConfigurationParsing conf, final String file)
		...
		final List<Hashtable<String, List<String>>> contenu = new ArrayList<Hashtable<String, List<String>>>();

		// communs à tous les ORCs
		List<String> mshRecord = new ArrayList<String>();
		List<String> pidRecord = new ArrayList<String>();
		List<String> pv1Record = new ArrayList<String>();
		Hashtable<String, List<String>> record = new Hashtable<String, List<String>>();
			...
			if(conf.getSeparateurChamps() != null){
				while((line = buff.readLine()) != null){
					...
					if (key.contains("MSH")){
						...
						if (!record.isEmpty()) {
							contenu.add(record);
							record = new Hashtable<String, List<String>>();
						}
		...
		if(config != null && contenu != null && racine != null){
			...
			if(confElt.getChild("NumeroDossier") != null){
				...
				if(confElt.getChild("IndexDossier") != null && confElt.getChild("IndexDossier").getContentSize() > 0){
					...
					String dossierIdx = getValueFromBlocAndEmplacement(contenu, config, blocIndex, empIndex);
					if (dossierIdx != null) {
	...
	@Override
	public String formateValueUsingFunction(final Element fonctionElt, String value, 
		...
		if (fonctionElt != null) { 
			...
			if(fonction != null && value != null){
				...
				if("stringToLowerCase".equals(fonction)){
					...
					value = Utils.concat(fonctionElt.getAttributeValue("sep"), 
							new String[] { value, getValueFromBlocAndEmplacement(contenu, conf, bloc, fonctionElt.getAttributeValue("key")) });
					...
					if(fonctionElt.getAttributeValue("niveau").equals("conteneur")) {
						...
						if (fonctionElt.getAttributeValue("alias") != null 
								&& fonctionElt.getAttributeValue("alias").equals("true")) {
							
							List<Terminale> terms = 
									terminaleManager.findByAliasManager(value);
	...
	public List<DossierExterne> parseInterfacageXmlFile(final String xmlFile, final String message, final Emetteur emetteur,
			final boolean delFile, final int max, String entiteId) throws Exception{ //TODO Refactorer
		...
		try{
			...
			for (Hashtable<String, List<String>> record : contenu) {
				blocExternes.clear();
				...
				final List<Element> dossierBlocs = new ArrayList<Element>();
				final List<Element> spmBlocs = new ArrayList<Element>();
				...
				for(int i = 0; i < dossierBlocs.size(); i++){
					...
					for(int j = 0; j < mappings.size(); j++){
						...
						if (sourceElt.getChild("Key") != null) {
							value = getValueFromBlocAndEmplacement(record, config, bloc.getAttributeValue("nom"), sourceElt.getChildText("Key"));
						...
						if(sourceElt.getChild("Fonctions") != null){
							...
							for(int k = 0; k < fonctions.size(); k++){
								...
								if(fonction.getAttribute("nom") != null){
									value = formateValueUsingFunction(fonction, value, record, config, bloc.getAttributeValue("nom"), null, mappingValues);
								}
							}
						}
				...
				List<String> spmKeys = new ArrayList<String>();
				...
				for (String spmK : spmKeys) {
					
					// creation d'un bloc externe
					BlocExterne blocSpm = new BlocExterne();
					...
					for (int i = 0; i < spmBlocs.size(); i++) { 
						...
						for(int j = 0; j < mappings.size(); j++){
							...
							String mapKey = bloc.getAttributeValue("nom").contains("_")  ? 
									spmK.concat(bloc.getAttributeValue("nom").substring(bloc.getAttributeValue("nom").indexOf("_"))) 
								:	spmK;
							String value =
									getValueFromBlocAndEmplacement(record, config, mapKey, sourceElt.getChildText("Key"));
							byte[] file = null;
							...
							if(sourceElt.getChild("Fonctions") != null){
								...
								for(int k = 0; k < fonctions.size(); k++){
									final Element fonction = (Element) fonctions.get(k);
									if(fonction.getAttribute("nom") != null){
										value = formateValueUsingFunction(fonction, value, record, config, 
											mapKey, emplacementAdrl, mappingValues);
									}
								}
							}
