...
/**
 * Validator pour le bean domaine Protocole.<br>
 * Classe creee le 13/02/12<br>
 * <br>
 ...
 * @version 2.0.6
 */
public class ProtocoleValidator implements Validator
{

   @Override
   public boolean supports(final Class<?> clazz){
      return Protocole.class.equals(clazz);
   }

   @Override
   public void validate(final Object obj, final Errors errs){

      final Protocole protocole = (Protocole) obj;
      //nom non vide
      ValidationUtils.rejectIfEmptyOrWhitespace(errs, "nom", "risque.nom.empty");
      //nom valide
      if(protocole.getNom() != null){
         if(!protocole.getNom().matches(ValidationUtilities.MOTREGEXP)){
            errs.rejectValue("nom", "risque.nom.illegal");
         }
         if(protocole.getNom().length() > 200){
