...
public class TerminaleManagerImpl implements TerminaleManager {
	...
	private Log log = LogFactory.getLog(TerminaleManager.class);
	...
	public void setTerminaleTypeDao(TerminaleTypeDao tDao) {
	...
	public void setTerminaleNumerotationDao(
			TerminaleNumerotationDao tDao) {
	...
	public void setTerminaleValidator(TerminaleValidator tValidator) {
	...
	@Override
	public Terminale findByIdManager(Integer terminaleId) {
	...
	@Override
	public List<Terminale> findByEnceinteWithOrderManager(Enceinte enceinte) {
		log.debug("Recherche de toutes les terminales d'une enceinte");
		if (enceinte != null) {
			return terminaleDao.findByEnceinteWithOrder(enceinte);
		} else {
			return new ArrayList<Terminale>();
		}
	}
	
	@Override
	public List<Terminale> findByEnceinteAndNomManager(Enceinte enceinte,
			String nom) {
	...
	public Set<Emplacement> getEmplacementsManager(Terminale terminale) {
		if (terminale != null && terminale.getTerminaleId() != null) {
			...
			Set<Emplacement> empls = terminale.getEmplacements();
			empls.size();
			
			return empls;
		} else {
			return new HashSet<Emplacement>();
		}
	}
	
	@Override
	public Boolean checkTerminaleInEnceinteLimitesManager(Terminale terminale) {
		Boolean valide = true;
		// on vérifie que la terminale n'est pas nulle
		if (terminale != null) {
			Enceinte enceinte = terminale.getEnceinte();
			
			if (enceinte != null && terminale.getPosition() != null) {
				// sa position doit être contenue dans les limites de
				// l'enceinte
				Integer nbPlaces = enceinte.getNbPlaces();
	 ...
	 */
	public Long getNumberEmplacementsLibresManager(Terminale terminale) {
		
		if (terminale != null && terminale.getTerminaleId() != null) {
			List<Long> nbPris = emplacementDao.findByCountTerminaleAndVide(
					terminale, false);
			
			Long libres = terminale.getTerminaleType().getNbPlaces() 
				- nbPris.get(0);
	...
	public Long getNumberEmplacementsOccupesManager(Terminale terminale) {
	...
	@Override
	public List<String> getCodesLignesManager(Terminale terminale) {
		...
		if (terminale != null && terminale.getTerminaleId() != null) {
			...
			if (terminale.getTerminaleType().getScheme() != null) {
				...
				nbLignes = terminale.getTerminaleType()
					.getScheme().split(";").length;
			} else {
			...
			if (nbLignes > 0) {
				...
				if (terminale.getTerminaleNumerotation()
						.getLigne().equals("CAR")) {
					results = Utils.createListChars(
							nbLignes, null, new ArrayList<String>());
				} else {
	...
	@Override
	public List<String> getCodesColonnesManager(Terminale terminale,
			Integer numLigne) {
		List<String> results = new ArrayList<String>();
		...
		if (terminale != null && terminale.getTerminaleId() != null
			...
			if (terminale.getTerminaleNumerotation()
				...
				results = Utils.createListChars(
						nbColonnes, null, new ArrayList<String>());
			} else {
	...
	@Override
	public List<Echantillon> getEchantillonsManager(Terminale terminale) {
		if (terminale != null && terminale.getTerminaleId() != null) {
	...
	@Override
	public List<ProdDerive> getProdDerivesManager(Terminale terminale) {
		if (terminale != null && terminale.getTerminaleId() != null) {
	...
	@Override
	public Boolean findDoublonManager(Terminale terminale) {
		if (terminale != null) {
			Enceinte enceinte = terminale.getEnceinte();
			if (terminale.getTerminaleId() == null) {
				...
				return terminaleDao.findByExcludedIdEnceinte(
						terminale.getTerminaleId(), enceinte)
						.contains(terminale);
			}
		} else {
			return false;
	...
	@Override
	public Boolean findDoublonWithoutTwoTerminalesManager(
			Terminale terminale, Terminale terminaleDestination) {
		
		if (terminale != null && terminaleDestination != null
				&& terminale.getTerminaleId() != null
				&& terminaleDestination.getTerminaleId() != null) {
			
			if (terminaleDestination.getEnceinte() != null) {
				return terminaleDao.findByTwoExcludedIdsWithEnceinte(
						terminale.getTerminaleId(), 
						terminaleDestination.getTerminaleId(), 
						terminale.getEnceinte())
						.contains(terminale);
			} else {
				return false;
			}
	...
	@Override
	public Boolean isUsedObjectManager(Terminale terminale) {
		if (terminale != null && terminale.getTerminaleId() != null) {
			List<Emplacement> empls = emplacementDao
				.findByTerminaleAndVide(terminale, false);
	...
	@Override
	public List<Object> getListOfParentsManager(Terminale terminale) {
	...
	@Override
	public void createObjectManager(Terminale terminale, Enceinte enceinte,
			TerminaleType terminaleType, Banque banque, Entite entite,
			TerminaleNumerotation terminaleNumerotation,
			Couleur couleur,
		...
		if (enceinte != null) { 
			...
			log.warn("Objet obligatoire Enceinte manquant"
							+ " lors de la création d'une Terminale");
			throw new RequiredObjectIsNullException(
					"Terminale", "creation", "Enceinte");
		}
		
		// TerminaleType required
		if (terminaleType != null) { 
			terminale.setTerminaleType(terminaleTypeDao
					.mergeObject(terminaleType));
		} else {
			log.warn("Objet obligatoire TerminaleType manquant"
							+ " lors de la création d'une Terminale");
			throw new RequiredObjectIsNullException(
					"Terminale", "creation", "TerminaleType");
		}
		
		// TerminaleNumerotation required
		if (terminaleNumerotation != null) { 
			terminale.setTerminaleNumerotation(
					terminaleNumerotationDao
					.mergeObject(terminaleNumerotation));
		} else {
			log.warn("Objet obligatoire TerminaleNumerotation manquant"
							+ " lors de la création d'une Terminale");
			throw new RequiredObjectIsNullException(
					"Terminale", "creation", "TerminaleNumerotation");
		}
		...
		if (!checkTerminaleInEnceinteLimitesManager(terminale)) {
			...
			throw new InvalidPositionException(
					"Terminale", "creation", terminale.getPosition());
		}
		
		// Test de la position
		if (!checkPositionManager.checkPositionLibreInEnceinteManager(
				terminale.getEnceinte(), terminale.getPosition(), null, null)) {
			log.warn("La position est déjà occupée par un autre objet dans " 
					+ "l'enceinte parent");
		...
		if (findDoublonManager(terminale)) {
			log.warn("Doublon lors de la creation de l'objet Terminale : " 
					+ terminale.toString());
			...
			BeanValidator.validateObject(
					terminale, new Validator[]{terminaleValidator});
						
			terminale.setArchive(false);
			terminaleDao.createObject(terminale);
			
			log.info("Enregistrement de l'objet Terminale : " 
					+ terminale.toString());
	...
	@Override
	public void updateObjectManager(Terminale terminale, Enceinte enceinte,
			TerminaleType terminaleType, Banque banque, Entite entite,
			TerminaleNumerotation terminaleNumerotation,
			Couleur couleur, List<Incident> incidents,
			Utilisateur utilisateur,
		...
		if (enceinte != null) { 
			...
			log.warn("Objet obligatoire Enceinte manquant"
							+ " lors de la modification d'une Terminale");
			throw new RequiredObjectIsNullException(
					"Terminale", "modification", "Enceinte");
		}
		...
		if (terminaleType != null) { 
			...
			log.warn("Objet obligatoire TerminaleType manquant"
							+ " lors de la modification d'une Terminale");
			throw new RequiredObjectIsNullException(
					"Terminale", "modification", "TerminaleType");
		}
		...
		if (terminaleNumerotation != null) { 
			...
			log.warn("Objet obligatoire TerminaleNumerotation manquant"
							+ " lors de la modification d'une Terminale");
			throw new RequiredObjectIsNullException(
					"Terminale", "modification", "TerminaleNumerotation");
		}
		...
		if (!checkTerminaleInEnceinteLimitesManager(terminale)) {
			log.warn("La position n'est pas dans la limite des places de " 
					+ "l'enceinte parent");
			throw new InvalidPositionException(
					"Terminale", "modification", terminale.getPosition());
		}
		
		// Test de la position
		if (!checkPositionManager.checkPositionLibreInEnceinteManager(
				terminale.getEnceinte(), terminale.getPosition(),
				terminale.getTerminaleId(), null)) {
			log.warn("La position est déjà occupée par un autre objet dans " 
					+ "l'enceinte parent");
			throw new UsedPositionException(
		...
		// Test s'il y a des doublons
		if (findDoublonManager(terminale)) {
			log.warn("Doublon lors de la modification de l'objet Terminale : " 
					+ terminale.toString());
			...
			log.info("Modification de l'objet Terminale : " 
					+ terminale.toString());
	...
	@Override
	public List<Terminale> createMultiObjetcsManager(Enceinte enceinte,
			Terminale terminale, Integer number, Integer firstPosition,
			Utilisateur utilisateur) {
		...
		if (terminale != null) {
			...
			if (enceinte != null) { 
				if (number != null) {
					if (number > enceinte.getNbPlaces()) {
						log.warn("La position n'est pas dans la limite " 
								...
								+ "l'enceinte");
						throw new InvalidPositionException(
								"Terminale", "creation", number);
					}
				
					for (int i = 0; i < number; i++) {
						Terminale term = new Terminale();
						StringBuffer nom = new StringBuffer(terminale.getNom());
						...
						createObjectManager(term, enceinte, 
								terminale.getTerminaleType(), 
								terminale.getBanque(), 
								terminale.getEntite(), 
				...
				log.warn("Objet obligatoire Enceinte manquant"
								+ " lors de la creation d'une Terminale");
				throw new RequiredObjectIsNullException(
						"Terminale", "creation", "Enceinte");
			}
		}
	...
	@Override
	public void echangerDeuxTerminalesManager(Terminale terminale1,
			Terminale terminale2, Utilisateur utilisateur) {
		if (terminale1 != null && terminale1.getTerminaleId() != null
				&& terminale2 != null && terminale2.getTerminaleId() != null) {
			
			// Test s'il y a des doublons
			if (findDoublonWithoutTwoTerminalesManager(
					terminale1, terminale2)) {
				log.warn("Doublon lors du déplacement de l'objet Terminale : " 
						+ terminale1.toString());
				throw new DoublonFoundException("Terminale", "deplacement");
			} else if (findDoublonWithoutTwoTerminalesManager(
					terminale2, terminale1)) {
				log.warn("Doublon lors du déplacement de l'objet Terminale : " 
						+ terminale2.toString());
				throw new DoublonFoundException("Terminale", "deplacement");
			} else {
				...
				BeanValidator.validateObject(
						terminale1, new Validator[]{terminaleValidator});
				BeanValidator.validateObject(
						terminale2, new Validator[]{terminaleValidator});
	...
	@Override
	public void updateNumerotationForMultiTerminales(
			List<Terminale> terminales, TerminaleNumerotation numrotation,
			Utilisateur utilisateur) {
		if (terminales != null && terminales.size() > 0
				&& numrotation != null
				&& utilisateur != null) {
			...
			for (int i = 0; i < terminales.size(); i++) {
				updateObjectManager(
						terminales.get(i), 
						terminales.get(i).getEnceinte(), 
						terminales.get(i).getTerminaleType(), 
						terminales.get(i).getBanque(), 
						terminales.get(i).getEntite(), 
						numrotation, 
						null, null,
						utilisateur, 
						null);
	...
	@Override
	public List<Terminale> createMultiVisotubesManager(Enceinte enceinte,
			Terminale terminale, Integer number, Integer firstPosition,
			boolean nameFromColor, Integer size,
			Utilisateur utilisateur) {
			...
			if (enceinte != null) { 
				if (number != null) {
					...
					for (int i = 0; i < number; i++) {
						...
						List<Couleur> couleurs = couleurDao.findByVisotube();
						...
						if (i == 0) {
							List<TerminaleType> tmp = terminaleTypeDao
								.findByType("VISOTUBE_" + size.toString() + "_ROND");
							if (tmp.size() > 0) {
								type = tmp.get(0);
							}
						} else {
							...
							List<TerminaleType> tmp = terminaleTypeDao
								.findByType("VISOTUBE_" + size.toString() + "_TRI");
							if (tmp.size() > 0) {
								type = tmp.get(0);
							}
						}
	...
	@Override
	public Map<TKStockableObject, Emplacement> 
					getTkObjectsAndEmplacementsManager(Terminale term) {
		Map<TKStockableObject, Emplacement> res = 
							new HashMap<TKStockableObject, Emplacement>();
		
		List<Emplacement> emps = emplacementDao.findByTerminaleAndVide(term, false);	
		List<Echantillon> echans = getEchantillonsManager(term);
		List<ProdDerive> derives = getProdDerivesManager(term);
		...
		for (Emplacement emp : emps) {
			if (emp.getEntite().getNom().equals("Echantillon")) {
				echanLoop : for (Echantillon e : echans) {
				...
				derivesLoop : for (ProdDerive p : derives) {
	...
	@Override
	public List<Integer> findTerminaleIdsFromNomManager(String nom, Enceinte enc, List<Conteneur> conts) {
