...
//	}
	...
	public void validate(Object obj, Errors errs) {
		//code non vide
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "code", "echantillon.code.empty");
		...
		if (echan.getQuantiteInit() != null) {
			if (echan.getQuantiteInit() < 0) {
				errs.rejectValue(
						"quantiteInit", "echantillon.quantiteInit.negative");
			}
		}
		// quantite >= 0
		...
		if (echan.getLateralite() != null) {
			if (!(echan.getLateralite().equals("D")
				|| echan.getLateralite().equals("G")
				|| echan.getLateralite().equals("I")
				|| echan.getLateralite().equals("B"))) {
				errs.rejectValue("lateralite", 
											"echantillon.lateralite.illegal");
			}
		}
		...
		if (echan.getEmplacement() != null) {
			if (echan.getObjetStatut().getStatut().equals("NON STOCKE") 
					|| (echan.getObjetStatut().getStatut().equals("EPUISE") 
					|| (echan.getObjetStatut().getStatut().equals("DETRUIT")))) {
				errs.rejectValue("objetStatut", "echantillon.statut.incoherent");
			}
		} else if (echan.getObjetStatut().getStatut().equals("STOCKE") 
					|| (echan.getObjetStatut().getStatut().equals("RESERVE"))) {
			errs.rejectValue("objetStatut", "echantillon.statut.incoherent"); 
		}
	...
	@Override
	public Errors checkDateStockageCoherence(Echantillon echantillon, 
													boolean skipToDatePrel) {
		
		BindException errs = new BindException(echantillon, 
					"fr.aphp.tumorotek.model.coeur.echantillon.Echantillon");
		if (echantillon.getDateStock() != null) {
			// limites inf
			if (echantillon.getPrelevement() != null) {
				Object[] dateAndCode = coherenceDateManager
					.findAntRefDateInPrelevementManager(echantillon
											.getPrelevement(), skipToDatePrel);
				if (dateAndCode[0] != null) {
					if (!ValidationUtilities
						.checkWithDate(echantillon.getDateStock(), null, 
			...
			//limites sup
			Object[] dateAndCode = coherenceDateManager
								.findPostRefDateInDerivesManager(echantillon);
			if (!ValidationUtilities
					.checkWithDate(echantillon.getDateStock(), null, 
	...
	@Override
	public Errors checkSteriliteAntecedence(Echantillon echantillon) {
		...
		if (echantillon.getSterile() != null && echantillon.getSterile()) {
			if (echantillon.getPrelevement() != null) {
				...
				if (echantillon.getPrelevement().getLaboInters() != null) {
					labos.addAll(echantillon.getPrelevement()
														.getLaboInters());
				}
				// dernier labo
				if (labos.size() > 0) {
					Iterator<LaboInter> it = labos.iterator();
					...
					if (echantillon.getPrelevement().getSterile() == null 
							|| !echantillon.getPrelevement().getSterile()) {
						errs.rejectValue("sterile", 
			...
			if (nonSterileRetour) {
				errs.rejectValue("sterile", 
									"sterile.validation.retourNotSterile");
			}
		}
