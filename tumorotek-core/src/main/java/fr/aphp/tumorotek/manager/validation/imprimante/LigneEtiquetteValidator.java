...
public class LigneEtiquetteValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		LigneEtiquette ligneEtiquette = (LigneEtiquette) target;
		
		//entete valide
		if (ligneEtiquette.getEntete() != null) {
			if (ligneEtiquette.getEntete()
					.matches(ValidationUtilities.ONLYSPACESREGEXP)) {
				errors.rejectValue("entete", 
						"ligneEtiquette.entete.empty");
			}
			if (!ligneEtiquette.getEntete()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errors.rejectValue("entete", 
						"ligneEtiquette.entete.illegal");
			}
			if (ligneEtiquette.getEntete().length() > 25) {
				errors.rejectValue("entete", 
						"ligneEtiquette.entete.tooLong");
			}
		}
		
		//contenu valide
		if (ligneEtiquette.getContenu() != null) {
			if (ligneEtiquette.getContenu()
					.matches(ValidationUtilities.ONLYSPACESREGEXP)) {
				errors.rejectValue("contenu", 
						"ligneEtiquette.contenu.empty");
			}
			if (!ligneEtiquette.getContenu()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errors.rejectValue("contenu", 
						"ligneEtiquette.contenu.illegal");
			}
			if (ligneEtiquette.getContenu().length() > 50) {
				errors.rejectValue("contenu", 
						"ligneEtiquette.contenu.tooLong");
			}
		}
		
		//font valide
		if (ligneEtiquette.getFont() != null) {
			if (ligneEtiquette.getFont()
					.matches(ValidationUtilities.ONLYSPACESREGEXP)) {
				errors.rejectValue("font", 
						"ligneEtiquette.font.empty");
			}
			if (!ligneEtiquette.getFont()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errors.rejectValue("font", 
						"ligneEtiquette.font.illegal");
			}
			if (ligneEtiquette.getFont().length() > 25) {
				errors.rejectValue("font", 
						"ligneEtiquette.font.tooLong");
			}
		}
		
		//style valide
		if (ligneEtiquette.getStyle() != null) {
			if (ligneEtiquette.getStyle()
					.matches(ValidationUtilities.ONLYSPACESREGEXP)) {
				errors.rejectValue("style", 
						"ligneEtiquette.style.empty");
			}
			if (!ligneEtiquette.getStyle()
					.matches(ValidationUtilities.MOTREGEXP)) {
				errors.rejectValue("style", 
						"ligneEtiquette.style.illegal");
			}
			if (ligneEtiquette.getStyle().length() > 25) {
				errors.rejectValue("style", 
						"ligneEtiquette.style.tooLong");
			}
		}
		
		//ordre valide
		if (ligneEtiquette.getOrdre() == null) {
			errors.rejectValue("ordre", 
					"ligneEtiquette.ordre.empty");
		} else {
			if (ligneEtiquette.getOrdre() < 1) {
				errors.rejectValue("ordre", 
					"ligneEtiquette.ordre.illegal");
			}
		}
		
		//size valide
		if (ligneEtiquette.getSize() != null
				&& ligneEtiquette.getSize() < 1) {
				errors.rejectValue("size", 
					"ligneEtiquette.size.illegal");
		}
	}
