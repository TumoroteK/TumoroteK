 ...
 * mais au format d'un nombre
 * 
 * @since 2.3.1.0 (TK-538)
 * @author chuet
 *
 */
public class ImportChampAnnotationTraitementForNumAsAlphanum extends ImportChampAnnotationTraitementForAlphanum
{
   private final Logger log = LoggerFactory.getLogger(ImportChampAnnotationTraitementForNumAsAlphanum.class);
   
   public ImportChampAnnotationTraitementForNumAsAlphanum(Integer champAnnotationId, int indexColonne, ImportColonne importColonne) {
      super(champAnnotationId, indexColonne, importColonne);
      //surcharge du reader du parent : 
      setImportChampAnnotationReader(ImportChampAnnotationReaderFactory.getNumericReader());
   }
   
   //les valeurs de type Long sont stockés dans un varchar
   @Override
   protected Object convertCellContent(Object cellContent) throws AbstractImportCellScopeException {
      return String.valueOf((Number)cellContent);
   }
}
