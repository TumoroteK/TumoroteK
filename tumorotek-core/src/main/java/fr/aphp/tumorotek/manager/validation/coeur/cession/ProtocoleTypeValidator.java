 ...
 * @version 2.0
 */
public class ProtocoleTypeValidator implements Validator
{

   @Override
   public boolean supports(final Class<?> clazz){
      return ProtocoleType.class.equals(clazz);
   }

   @Override
   public void validate(final Object obj, final Errors errs){
      //Nature non vide
      ValidationUtils.rejectIfEmptyOrWhitespace(errs, "type", "protocoleType.type.empty");

      final ProtocoleType type = (ProtocoleType) obj;
      //nom valide
      if(type.getType() != null){
         if(!type.getType().matches(ValidationUtilities.MOTREGEXP)){
            errs.rejectValue("type", "protocoleType.type.illegal");
         }
         if(type.getType().length() > 200){
            errs.rejectValue("type", "protocoleType.type.tooLong");
         }
      }
   }
