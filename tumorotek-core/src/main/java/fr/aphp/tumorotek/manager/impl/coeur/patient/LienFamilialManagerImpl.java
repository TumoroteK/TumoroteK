...
public class LienFamilialManagerImpl implements LienFamilialManager {
	...
	private Log log = LogFactory.getLog(LienFamilialManager.class);
	...
	public void setLienFamilialValidator(LienFamilialValidator lfValidator) {
	...
	public void createObjectManager(Object obj) {
		BeanValidator.validateObject(obj, 
									new Validator[]{lienFamilialValidator});
		if (!findDoublonManager(obj)) {
			LienFamilial reciproque = new LienFamilial();
			createReciprocity((LienFamilial) obj, reciproque, true);
			lienFamilialDao.createObject((LienFamilial) obj);
			log.info("Enregistrement objet LienFamilial " + obj.toString() 
					+ " et son reciproque " + reciproque.toString());
			
		} else {
			log.warn("Doublon lors creation objet LienFamilial "
					+ obj.toString());
	...
	public void updateObjectManager(Object obj) {
		BeanValidator.validateObject(obj, 
								new Validator[]{lienFamilialValidator});
		if (!findDoublonManager(obj)) { 
			LienFamilial reciproque = ((LienFamilial) obj).getReciproque();
			createReciprocity((LienFamilial) obj, reciproque, false);
			lienFamilialDao.updateObject((LienFamilial) obj);
			log.info("Modification objet LienFamilial " + obj.toString()
					+ " et son reciproque " + reciproque.toString());
		} else {
			log.warn("Doublon lors modification objet LienFamilial "
					+ obj.toString());
	...
	@Override
	public List<LienFamilial> 
				findByNomLikeManager(String nom, boolean exactMatch) {
		if (!exactMatch) {
			nom = nom + "%";
		}
		log.debug("Recherche LienFamilial par nom: " 
	...
	public void removeObjectManager(Object obj) {
		if (obj != null) {
			if (!isUsedObjectManager(obj)) {
				lienFamilialDao.removeObject(((LienFamilial) obj)
														.getLienFamilialId());
				log.debug("Suppression objet LienFamilial " + obj.toString()
								+ " et son reciproque en cascade");
			} else {
				log.warn("Suppression objet LienFamilial " + obj.toString()
						+ " impossible car est reference (par PatientLien)");
	...
	public boolean findDoublonManager(Object o) {
		if (((LienFamilial) o).getLienFamilialId() == null) {
			...
			return lienFamilialDao.findByExcludedId(
					((LienFamilial) o).getLienFamilialId()).contains(o);
		}
	}
	...
	public boolean isUsedObjectManager(Object o) {
		LienFamilial lienFamilial = 
								lienFamilialDao.mergeObject((LienFamilial) o);
		return ((lienFamilial.getPatientLiens().size() > 0)
			|| (lienFamilial.getReciproque().getPatientLiens().size() > 0));
	}
	 ...
	 */
	private void createReciprocity(LienFamilial obj, 
			LienFamilial reciproque, boolean doCreateLink) {
		reciproque
			.setNom(switchNomReciproque(obj.getNom()));
	 ...
	 */
	private String switchNomReciproque(String nom) {
