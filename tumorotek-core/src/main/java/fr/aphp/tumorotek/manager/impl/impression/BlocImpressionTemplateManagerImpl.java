...
public class BlocImpressionTemplateManagerImpl implements
		BlocImpressionTemplateManager {

	private Log log = LogFactory
		.getLog(BlocImpressionTemplateManager.class);
	...
	@Override
	public BlocImpressionTemplate findByIdManager(BlocImpressionTemplatePK pk) {
	...
	@Override
	public List<BlocImpressionTemplate> findByExcludedPKManager(
			BlocImpressionTemplatePK pk) {
		if (pk != null) {
	...
	@Override
	public List<BlocImpressionTemplate> 
		findByTemplateManager(Template template) {
		log.debug("Recherche de tous les BlocImpressionTemplates d'un " 
				+  "Template.");
		if (template != null) {
			return blocImpressionTemplateDao.findByTemplate(template);
		} else {
			return new ArrayList<BlocImpressionTemplate>();
		}
	}

	@Override
	public Boolean findDoublonManager(Template template,
			BlocImpression blocImpression) {
		
		BlocImpressionTemplatePK pk = new 
			BlocImpressionTemplatePK(template, blocImpression);
		
		return (blocImpressionTemplateDao.findById(pk) != null);
	}

	@Override
	public void validateObjectManager(Template template,
			BlocImpression blocImpression) {
		//template required
		if (template == null) {
			log.warn("Objet obligatoire Template manquant"
				+ " lors de la validation d'un BlocImpressionTemplate");
			throw new RequiredObjectIsNullException(
					"BlocImpressionTemplate", "creation", "Template");
		}
		...
		if (blocImpression == null) {
			log.warn("Objet obligatoire BlocImpression manquant"
				+ " lors de la validation d'un BlocImpressionTemplate");
			throw new RequiredObjectIsNullException(
					"BlocImpressionTemplate", "creation", "BlocImpression");
		}
		...
		if (template.getTemplateId() != null) {
			if (findDoublonManager(template, blocImpression)) {
				...
				throw new DoublonFoundException(
						"BlocImpressionTemplate", "creation");
			}
		}
	}
	...
	public void createObjectManager(
			BlocImpressionTemplate blocImpressionTemplate, Template template,
			BlocImpression blocImpression) {
		...
		blocImpressionTemplate.setBlocImpression(blocImpressionDao
				.mergeObject(blocImpression));
		
		// création
		blocImpressionTemplateDao.createObject(blocImpressionTemplate);
		
		log.info("Enregistrement objet BlocImpressionTemplate " 
				+ blocImpressionTemplate.toString());
	...
	public void updateObjectManager(
		...
		if (template == null) {
			log.warn("Objet obligatoire Template manquant"
				+ " lors de la validation d'un BlocImpressionTemplate");
			throw new RequiredObjectIsNullException(
					"BlocImpressionTemplate", "modification", 
					"Template");
		} else {
			blocImpressionTemplate.setTemplate(
					templateDao.mergeObject(template));
		}
	...
	@Override
	public void removeObjectManager(
			BlocImpressionTemplate blocImpressionTemplate) {
		if (blocImpressionTemplate != null) {
			blocImpressionTemplateDao.removeObject(
					blocImpressionTemplate.getPk());
			log.info("Suppression de l'objet BlocImpressionTemplate : " 
					+ blocImpressionTemplate.toString());				
		} else {
