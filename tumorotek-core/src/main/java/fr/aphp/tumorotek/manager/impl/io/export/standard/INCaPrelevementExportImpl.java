...
public class INCaPrelevementExportImpl implements IncaPrelevementExport
{
   ...
   @Override
   public String getCentreStockage(final Echantillon echantillon){
      String finess = null;
      if(echantillon.getBanque().getProprietaire() != null
         && echantillon.getBanque().getProprietaire().getEtablissement() != null){
         finess = echantillon.getBanque().getProprietaire().getEtablissement().getFiness();
      }
      if(finess != null){
         return finess;
      }
   ...
   @Override
   public String getCodePrelevement(final Prelevement prelevement){
   ...
   public String getModePrelevement(final Prelevement prelevement){
      ...
      if(prelevement.getPrelevementType() != null){
         final Pattern p = Pattern.compile("([BCPNOL9]) : .+");
         final Matcher m = p.matcher(prelevement.getPrelevementType().getType());
   ...
   public String getClassif(final Banque bank){
      ...
      final List<BanqueTableCodage> bcts = getBanqueManager().getBanqueTableCodageByBanqueManager(bank);

      for(int i = 0; i < bcts.size(); i++){
         if(bcts.get(i).getTableCodage().getNom().matches("CIM[^O].*")){
            classifs = classifs + "C";
         }else if(bcts.get(i).getTableCodage().getNom().matches("ADICAP")){
            classifs = classifs + "A";
         }
      }
   ...
   @Override
   public String getCodeOrgane(final Connection con, final Echantillon echan, final String codif){
   ...
   @Override
   public String getTypeLesionnel(final Connection con, final Echantillon echan, final String codif){
      String codifId = "1"; // ADICAP
      if(codif != null){
         if(codif.equals("C")){
            codifId = "3";
   ...
   @Override
   public String getTypeEvent(final Connection con, final Prelevement prelevement){
      return ExportCatalogueManagerImpl.fetchItemAsString(con,
         "SELECT ITEM.LABEL FROM ITEM, ANNOTATION_VALEUR, " + "CHAMP_ANNOTATION, TABLE_ANNOTATION "
            ...
            + " AND CHAMP_ANNOTATION.NOM like '022%' " + "AND ITEM.ITEM_ID=ANNOTATION_VALEUR.ITEM_ID "
   ...
   public String getVersionPTNM(final Connection con, final Prelevement prelevement){
      return ExportCatalogueManagerImpl.fetchItemAsString(con,
         "SELECT ITEM.LABEL FROM ITEM, ANNOTATION_VALEUR," + "CHAMP_ANNOTATION, TABLE_ANNOTATION "
            + "WHERE ANNOTATION_VALEUR.OBJET_ID=" + prelevement.getPrelevementId().toString()
            + " AND CHAMP_ANNOTATION.NOM like '023%' " + "AND ITEM.ITEM_ID=ANNOTATION_VALEUR.ITEM_ID "
            + "AND CHAMP_ANNOTATION.CHAMP_ANNOTATION_ID=" + "ANNOTATION_VALEUR.CHAMP_ANNOTATION_ID"
            + " AND CHAMP_ANNOTATION.TABLE_ANNOTATION_ID=" + "TABLE_ANNOTATION.TABLE_ANNOTATION_ID "
            + "AND TABLE_ANNOTATION.ENTITE_ID=2",
   ...
   public String getTailleTumeurPT(final Connection con, final Prelevement prelevement){
      final String res = ExportCatalogueManagerImpl.fetchItemAsString(con,
         "SELECT ITEM.LABEL FROM ITEM, ANNOTATION_VALEUR," + " CHAMP_ANNOTATION, TABLE_ANNOTATION"
            + " WHERE ANNOTATION_VALEUR.OBJET_ID=" + prelevement.getPrelevementId().toString()
            + " AND CHAMP_ANNOTATION.NOM like '024%' " + "AND ITEM.ITEM_ID=ANNOTATION_VALEUR.ITEM_ID "
            + "AND CHAMP_ANNOTATION.CHAMP_ANNOTATION_ID" + "=ANNOTATION_VALEUR.CHAMP_ANNOTATION_ID"
            + " AND CHAMP_ANNOTATION.TABLE_ANNOTATION_ID=" + "TABLE_ANNOTATION.TABLE_ANNOTATION_ID "
            + "AND TABLE_ANNOTATION.ENTITE_ID=2",
   ...
   public String getEnvGangPN(final Connection con, final Prelevement prelevement){
      final String res = ExportCatalogueManagerImpl.fetchItemAsString(con,
         "SELECT ITEM.LABEL FROM ITEM, ANNOTATION_VALEUR," + " CHAMP_ANNOTATION, TABLE_ANNOTATION"
            + " WHERE ANNOTATION_VALEUR.OBJET_ID=" + prelevement.getPrelevementId().toString()
            + " AND CHAMP_ANNOTATION.NOM like '025%' " + "AND ITEM.ITEM_ID=ANNOTATION_VALEUR.ITEM_ID "
            + "AND CHAMP_ANNOTATION.CHAMP_ANNOTATION_ID" + "=ANNOTATION_VALEUR.CHAMP_ANNOTATION_ID"
            + " AND CHAMP_ANNOTATION.TABLE_ANNOTATION_ID=" + "TABLE_ANNOTATION.TABLE_ANNOTATION_ID "
            + "AND TABLE_ANNOTATION.ENTITE_ID=2",
   ...
   public String getExtMetastaticPM(final Connection con, final Prelevement prelevement){
      final String res = ExportCatalogueManagerImpl.fetchItemAsString(con,
         "SELECT ITEM.LABEL FROM ITEM, ANNOTATION_VALEUR," + " CHAMP_ANNOTATION, TABLE_ANNOTATION"
            + " WHERE ANNOTATION_VALEUR.OBJET_ID=" + prelevement.getPrelevementId().toString() + " AND CHAMP_ANNOTATION.NOM like "
            + "'026 : Extension m%tastatique : pM%' " + "AND ITEM.ITEM_ID=ANNOTATION_VALEUR.ITEM_ID "
            + "AND CHAMP_ANNOTATION.CHAMP_ANNOTATION_ID" + "=ANNOTATION_VALEUR.CHAMP_ANNOTATION_ID"
            + " AND CHAMP_ANNOTATION.TABLE_ANNOTATION_ID=" + "TABLE_ANNOTATION.TABLE_ANNOTATION_ID "
            + "AND TABLE_ANNOTATION.ENTITE_ID=2",
