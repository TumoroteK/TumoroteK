...
import fr.aphp.tumorotek.manager.validation.coeur.prodderive.
...
public class TransformationManagerImpl implements TransformationManager {
	...
	private Log log = LogFactory.getLog(TransformationManager.class);
	...
	public void setTransformationValidator(
	 ...
	 */
	public Transformation findByIdManager(Integer transformationId) {
	 ...
	 */
	public List<Transformation> findByEntiteObjetManager(
			Entite entite, Integer objetId) {
		log.debug("Recherche Transformation par " 
				+ entite + " et " + objetId);
	 ...
	 * @return une liste de transformation.
	 */
	public List<Transformation> findByParentManager(Object parent) {
		Integer id = 0;
		List<Entite> list = new ArrayList<Entite>();
		Entite entite = new Entite();
		String nomClasse = parent.getClass().getSimpleName();
	 ...
	 */
	public Boolean findDoublonManager(Transformation transformation) {
		if (transformation.getTransformationId() == null) {
			return transformationDao.findAll().contains(transformation);
		} else {
			return transformationDao.findByExcludedId(
...
//	}
	 ...
	 */
	public void createObjectManager(Transformation transformation, 
			Entite entite, Unite quantiteUnite) {
		// On vérifie que l'entité n'est pas null. Si c'est le cas on envoie
		// une exception
		if (entite == null) {
			log.warn("Objet obligatoire Entite manquant lors de la creation " 
					+  "d'un objet Transformation");
			throw new RequiredObjectIsNullException(
					"Transformation", "creation", "Entite");
		} else {
		...
		if (quantiteUnite != null) {
			transformation.setQuantiteUnite(
					uniteDao.mergeObject(quantiteUnite));
		} else {
		...
		if (entiteManager.findObjectByEntiteAndIdManager(
				entite, transformation.getObjetId()) == null) {
			log.warn("Couple Entite : " + entite.toString() + " - ObjetId :"
					+ transformation.getObjetId() + " inexistant lors de la "
					+ "création d'un objet Transformation");
			throw new EntiteObjectIdNotExistException("Transformation", 
					entite.getNom(), transformation.getObjetId());
		}
		...
		/*if (findDoublonManager(transformation)) {
			...
			BeanValidator.validateObject(
					transformation, new Validator[]{transformationValidator});
			transformationDao.createObject(transformation);
			log.info("Enregistrement de l'objet Transformation : " 
					+ transformation.toString());
		//}
	 ...
	 * l'objet à créer se trouve déjà dans la base.
	 */
	public void updateObjectManager(Transformation transformation,
			Entite entite, Unite quantiteUnite) {
		
		// On vérifie que l'entité n'est pas null. Si c'est le cas on envoie
		// une exception
		if (entite == null) {
			log.warn("Objet obligatoire Entite manquant lors de la modif " 
					+  "d'un objet Transformation");
			throw new RequiredObjectIsNullException(
					"Transformation", "modification", "Entite");
		} else {
			transformation.setEntite(entiteDao.mergeObject(entite));
		}
		// On vérifie que l'unité de quantité n'est pas null
		...
		if (entiteManager.findObjectByEntiteAndIdManager(
			...
			log.warn("Couple Entite : " + entite.toString() + " - ObjetId :"
					+ transformation.getObjetId() + " inexistant lors de la "
					+ "modification d'un objet Transformation");
			throw new EntiteObjectIdNotExistException("Transformation", 
					entite.getNom(), transformation.getObjetId());
		}
		...
		/*if (findDoublonManager(transformation)) {
			...
			log.info("Modification de l'objet Transformation : " 
					+ transformation.toString());
		//}
	}

	@Override
	public void removeObjectManager(Transformation transformation, 
...
//		} else {
		...
		transformationDao.removeObject(
					transformation.getTransformationId());
	...
	@Override
	public List<ProdDerive> findAllDeriveFromParentManager(Object parent) {
		List<ProdDerive> derives = new ArrayList<ProdDerive>();
		List<Transformation> transfos = findByParentManager(parent);
		List<ProdDerive> derives1 = new ArrayList<ProdDerive>(); 
		for (int i = 0; i < transfos.size(); i++) {
			derives1 = prodDeriveDao
				.findByTransformation(transfos.get(i));
