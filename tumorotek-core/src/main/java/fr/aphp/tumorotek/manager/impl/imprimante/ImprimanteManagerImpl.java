...
public class ImprimanteManagerImpl implements ImprimanteManager {
	...
	private Log log = LogFactory.getLog(ImprimanteManager.class);
	...
	public void setImprimanteValidator(ImprimanteValidator iValidator) {
		this.imprimanteValidator = iValidator;
	}

	@Override
	public Imprimante findByIdManager(Integer imprimanteId) {
	...
	@Override
	public List<Imprimante> findByPlateformeManager(Plateforme plateforme) {
		log.debug("Recherche de toutes les Imprimantes d'une Plateforme");
		if (plateforme != null) {
			return imprimanteDao.findByPlateforme(plateforme);
		} else {
			return new ArrayList<Imprimante>();
		}
	}
	
	@Override
	public List<Imprimante> findByNomAndPlateformeManager(String nom,
	...
	@Override
	public List<String> findByPlateformeSelectNomManager(
			Plateforme plateforme) {
		log.debug("Recherche de toutes les Imprimantes d'une Plateforme");
		if (plateforme != null) {
	...
	@Override
	public Boolean findDoublonManager(Imprimante imprimante) {
		if (imprimante != null) {
			if (imprimante.getImprimanteId() == null) {
				return imprimanteDao.findAll().contains(imprimante);
			} else {
				return imprimanteDao.findByExcludedId(
						imprimante.getImprimanteId()).contains(imprimante);
			}
		} else {
			return false;
		}
	...
	@Override
	public void createObjectManager(Imprimante imprimante,
			Plateforme plateforme, ImprimanteApi imprimanteApi) {
		// plateforme required
		if (plateforme != null) { 
			imprimante.setPlateforme(plateformeDao.mergeObject(plateforme));
		} else {
			log.warn("Objet obligatoire Plateforme manquant"
							+ " lors de la création d'une Imprimante");
			throw new RequiredObjectIsNullException(
					"Imprimante", "creation", "Plateforme");
		}
		
		// imprimanteApi required
		if (imprimanteApi != null) { 
			imprimante.setImprimanteApi(
					imprimanteApiDao.mergeObject(imprimanteApi));
		} else {
			log.warn("Objet obligatoire ImprimanteApi manquant"
							+ " lors de la création d'une Imprimante");
			throw new RequiredObjectIsNullException(
					"Imprimante", "creation", "ImprimanteApi");
		}
		
		// Test s'il y a des doublons
		if (findDoublonManager(imprimante)) {
			log.warn("Doublon lors de la creation de l'objet Imprimante : " 
					+ imprimante.toString());
			...
			BeanValidator.validateObject(
					imprimante, new Validator[]{imprimanteValidator});
						
			imprimanteDao.createObject(imprimante);
			
			log.info("Enregistrement de l'objet Imprimante : " 
					+ imprimante.toString());
		}
	}

	@Override
	public void updateObjectManager(Imprimante imprimante,
			Plateforme plateforme, ImprimanteApi imprimanteApi) {
		// plateforme required
		if (plateforme != null) { 
			imprimante.setPlateforme(plateformeDao.mergeObject(plateforme));
			...
			log.warn("Objet obligatoire Plateforme manquant"
							+ " lors de la modification d'une Imprimante");
			throw new RequiredObjectIsNullException(
					"Imprimante", "modification", "Plateforme");
		}
		...
		if (imprimanteApi != null) { 
			...
			log.warn("Objet obligatoire ImprimanteApi manquant"
							+ " lors de la modification d'une Imprimante");
			throw new RequiredObjectIsNullException(
					"Imprimante", "modification", "ImprimanteApi");
		}
		
		// Test s'il y a des doublons
		if (findDoublonManager(imprimante)) {
			log.warn("Doublon lors de la modification de l'objet Imprimante : " 
					+ imprimante.toString());
	...
	@Override
	public void removeObjectManager(Imprimante imprimante) {
		if (imprimante != null) {			
			imprimanteDao.removeObject(imprimante.getImprimanteId());
			log.info("Suppression de l'objet Imprimante : " 
					+ imprimante.toString());
		} else {
