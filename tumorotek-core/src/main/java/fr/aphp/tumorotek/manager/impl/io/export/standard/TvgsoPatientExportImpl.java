...
public class TvgsoPatientExportImpl extends INCaPatientExportImpl 
									implements TvgsoPatientExport {
	
	private Log log = LogFactory.getLog(TvgsoPatientExport.class);
	
	@Override
	public String getPatientCode(Patient pat) {
	...
	@Override
	public String getPatientEtat(Connection con, Patient pat, 
												Banque bank) {
		
		String etat = super.getPatientEtat(pat);
		...
		if (etat.equals("I")) {
			...
			"SELECT ITEM.LABEL FROM ITEM, ANNOTATION_VALEUR, "
				...
				+ " AND CHAMP_ANNOTATION.NOM like '005%'" 
	...
	@Override
	public String getDateEtat(Connection con, Patient pat, DateFormat df, 
																Banque bank) {
		...
		try {
			...
			return ExportCatalogueManagerImpl.fetchItemDate(con, 
					...
					+ " WHERE ANNOTATION_VALEUR.OBJET_ID=" 
												+ pat.getPatientId().toString()
					+ " AND CHAMP_ANNOTATION.NOM like '006%'"
