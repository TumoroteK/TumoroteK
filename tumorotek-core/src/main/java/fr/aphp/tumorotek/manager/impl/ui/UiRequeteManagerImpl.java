...
public class UiRequeteManagerImpl implements UiRequeteManager {
	...
	public void setUiRequeteDao(UiRequeteDao uDao) {
		this.uiRequeteDao = uDao;
	}

	public void setUiRequeteValidator(UiRequeteValidator uv) {
		this.uiRequeteValidator = uv;
	}

	@Override
	public UiRequete findByIdManager(Integer id) {
		return uiRequeteDao.findById(id);
	}
	
	@Override
	public Boolean findDoublonManager(UiRequete req) {
		if (req != null) {
			List<UiRequete> reqs = uiRequeteDao
				.findByNomUtilisateurAndEntite(req.getUtilisateur(), 
						req.getEntite(), req.getNom());
			if (!reqs.isEmpty()) {
				for (UiRequete uiR : reqs) {
					if (!uiR.getUiRequeteId()
							.equals(req.getUiRequeteId())) {
						return true;
					}
				}
			}
	...
	@Override
	public List<UiRequete> findByUtilisateurAndEntiteManager(Utilisateur u,
			Entite e) {
		return uiRequeteDao.findByUtilisateurAndEntite(u, e);
	}
	
	@Override
	public void createObjectManager(String nom, Utilisateur ut, Entite et,
			Integer ordre, List<UiCompValue> vals) {
		UiRequete requete = new UiRequete(null, ut, et, nom, ordre);
		mergeObjectManager(requete, vals);
	}
	
	@Override
	public void mergeObjectManager(UiRequete requete, List<UiCompValue> vals) {
		if (requete != null) {
			if (requete.getUtilisateur() == null) {
				throw new RequiredObjectIsNullException(
						"Utilisateur", "creation", "UiRequete");
			} else if (requete.getEntite() == null) {
				throw new RequiredObjectIsNullException(
						"Entite", "creation", "UiRequete");
			} else if (requete.getOrdre() == null) {
				throw new RequiredObjectIsNullException(
						"Ordre", "creation", "UiRequete");
			} 
			
			if (vals != null) {
				requete.getUiCompValues().clear();
				for (UiCompValue uiVal : vals) {
			...
			if (findDoublonManager(requete)) {
				...
				BeanValidator.validateObject(
						requete, new Validator[]{uiRequeteValidator});
	...
	@Override
	public void removeObjectManager(UiRequete requete) {
		if (requete != null) { 
