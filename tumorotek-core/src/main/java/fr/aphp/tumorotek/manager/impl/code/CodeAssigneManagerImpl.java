...
public class CodeAssigneManagerImpl implements CodeAssigneManager {
	...
	private Log log = LogFactory.getLog(CodeAssigneManager.class);
	...
	public void setCodeAssigneDao(CodeAssigneDao cUDao) {
	...
	public void setBanqueTableCodageDao(BanqueTableCodageDao bDao) {
	...
	@Override
	public List<CodeAssigne> findByCodeLikeManager(String code, 
													boolean exactMatch) {
		if (!exactMatch) {
			code = "%" + code + "%";
		}
		log.debug("Recherche CodeAssigne par code: " 
	...
	@Override
	public List<CodeAssigne> findByLibelleLikeManager(String libelle,
			boolean exactMatch) {
		if (!exactMatch) {
			libelle = "%" + libelle + "%";
		}
		log.debug("Recherche CodeAssigne par libelle: " 
				+ libelle + " exactMatch " + String.valueOf(exactMatch));
		return codeAssigneDao.findByLibelleLike(libelle);
	}

	@Override
	public List<CodeAssigne> findCodesMorphoByEchantillonManager(
			Echantillon echan) {
		return codeAssigneDao.findCodesMorphoByEchantillon(echan);
	}

	@Override
	public List<CodeAssigne> findCodesOrganeByEchantillonManager(
			Echantillon echan) {
		return codeAssigneDao.findCodesOrganeByEchantillon(echan);
	}
	
	@Override
	public boolean findDoublonManager(CodeAssigne code) {
		if (code.getCodeAssigneId() == null) {
			return codeAssigneDao.findByCodeAndEchantillon(code.getCode(), 
									code.getEchantillon()).contains(code);
		} else {
			return codeAssigneDao.findByExcludedId(
				code.getCodeAssigneId(), code.getCode(), 
										code.getEchantillon()).contains(code);
		}	
	}

	@Override
	public void createOrUpdateManager(CodeAssigne code,
			Echantillon echantillon, TableCodage table, Utilisateur utilisateur,
															String operation) {
		...
		if (!findDoublonManager(code)) {
			if ((operation.equals("creation") 
				...
				if (operation.equals("creation")) {
					...
					log.info("Enregistrement objet CodeAssigne " 
														+ code.toString());
					...
					// a changé
					CodeAssigne inBase = codeAssigneDao
									.findById(code.getCodeAssigneId());
					
					boolean doRecordModif = (!inBase.getCode()
											.equals(code.getCode())) 
						|| ((inBase.getLibelle() != null 
								&& !inBase.getLibelle()
										.equals(code.getLibelle()))
								|| (inBase.getLibelle() == null 
										&& code.getLibelle() != null));
					
					codeAssigneDao.updateObject(code);
					log.info("Modification objet CodeAssigne "
													+ code.toString());
					if (doRecordModif) {
						CreateOrUpdateUtilities
						.createAssociateOperation(code, operationManager, 
	 ...
	 */
	private void checkRequiredObjectsAndValidate(CodeAssigne code, 
		...
		if (echantillon != null) { 
			...
			log.warn("Objet obligatoire Echantillon manquant"
					+ " lors de la " + operation + " du code assigne");
			throw new RequiredObjectIsNullException(
					"CodeAssigne", operation, "Echantillon");
		} 
	...
	@Override
	public void removeObjectManager(CodeAssigne code) {
		if (code != null) {
...
//			}
	...
	@Override
	public List<CodeAssigne> 
				findCodesLesExportedByPrelevementManager(Prelevement prel) {
		return codeAssigneDao.findCodesLesExportedByPrelevement(prel);
	}

	@Override
	public List<CodeAssigne> findCodesOrgExportedByPatientManager(Patient pat) {
		return codeAssigneDao.findCodesOrgExportedByPatient(pat);
	}

	@Override
	public List<CodeAssigne> findCodesOrgExportedByPrelevementManager(
			Prelevement prel) {
		return codeAssigneDao.findCodesOrgExportedByPrelevement(prel);
	}
	
	@Override
	public List<CodeAssigne> findFirstCodesLesByPrelevementManager(
			Prelevement prel) {
		List<CodeAssigne> results = new ArrayList<CodeAssigne>();
		if (prel != null) {
			List<Echantillon> echans = echantillonDao
				.findByPrelevement(prel);
			for (int i = 0; i < echans.size(); i++) {
				List<CodeAssigne> tmp = codeAssigneDao
					.findCodesMorphoByEchantillon(echans.get(i));
				if (tmp.size() > 0) {
	...
	@Override
	public List<CodeAssigne> findFirstCodesOrgByPrelevementManager(
			Prelevement prel) {
		List<CodeAssigne> results = new ArrayList<CodeAssigne>();
		if (prel != null) {
			List<Echantillon> echans = echantillonDao
			...
			for (int i = 0; i < echans.size(); i++) {
				List<CodeAssigne> tmp = codeAssigneDao
					.findCodesOrganeByEchantillon(echans.get(i));
				if (tmp.size() > 0) {
					results.add(tmp.get(0));
				}
			}
	...
	@Override
	public List<CodeAssigne> findFirstCodesOrgByPatientManager(Patient pat) {
		List<CodeAssigne> results = new ArrayList<CodeAssigne>();
		if (pat != null) {
			List<Prelevement> prlvts = prelevementDao.findByPatient(pat);
			for (int k = 0; k < prlvts.size(); k++) {
				List<Echantillon> echans = echantillonDao
					.findByPrelevement(prlvts.get(k));
				for (int i = 0; i < echans.size(); i++) {
					List<CodeAssigne> tmp = codeAssigneDao
						.findCodesOrganeByEchantillon(echans.get(i));
					if (tmp.size() > 0) {
	...
	@Override
	public List<String> formatCodesAsStringsManager(List<CodeAssigne> codes) {
		List<String> strs = new ArrayList<String>();
		List<BanqueTableCodage> btcs = new ArrayList<BanqueTableCodage>();
		String s;
		...
		if (codes != null) {
			for (int i = 0; i < codes.size(); i++) {
				...
				if (btcs.contains(bc) 
						&& btcs.get(btcs.indexOf(bc)).getLibelleExport()) {
	...
	@Override
	public void prepareListJDBCManager(EchantillonJdbcSuite jdbcSuite,
			Echantillon echantillon,
			List<CodeAssigne> codes, Utilisateur usr) 
			throws SQLException {
...
//						.prepareStatement(sql2);
				...
				if (echantillon == null 
					...
					throw new RequiredObjectIsNullException(
							"CodeAssigne", "creation", "EchantillonId");
				}
				Integer objId = echantillon.getEchantillonId();

				// création des nouveaux codes assignes
				// HashSet exclue doublons
				for (CodeAssigne code : new LinkedHashSet<CodeAssigne>(codes)) {
