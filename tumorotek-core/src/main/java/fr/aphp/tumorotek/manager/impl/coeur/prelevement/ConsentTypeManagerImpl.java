...
public class ConsentTypeManagerImpl implements ConsentTypeManager {
	...
	private Log log = LogFactory.getLog(ConsentTypeManager.class);
	...
	public void setConsentTypeValidator(ConsentTypeValidator ctValidator) {
	...
	public void createObjectManager(Object obj) {
		...
		if (((TKThesaurusObject) obj).getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"ConsentType", "creation", "Plateforme");
		} else {
	...
	public void updateObjectManager(Object obj) {
		...
		if (!findDoublonManager(obj)) {
			...
			log.warn("Doublon lors modification objet ConsentType "
					+ obj.toString());
...
//	}
	...
	@Override
	public List<ConsentType> findByTypeLikeManager(String type, 
											boolean exactMatch) {
		if (!exactMatch) {
			type = type + "%";
		}
		log.debug("Recherche ConsentType par type: " 
	...
	public void removeObjectManager(Object obj) {
		if (obj != null) {
			if (!isUsedObjectManager(obj)) {
				consentTypeDao
					.removeObject(((ConsentType) obj).getConsentTypeId());
				log.info("Suppression objet ConsentType " + obj.toString());
			} else {
				log.warn("Suppression objet ConsentType " + obj.toString()
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			ConsentType type = (ConsentType) o;
			if (type.getConsentTypeId() == null) {
				return consentTypeDao.findAll().contains(type);
			} else {
				return consentTypeDao.findByExcludedId(
						type.getConsentTypeId()).contains(type);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object o) {
		ConsentType consentType = consentTypeDao.mergeObject((ConsentType) o);
