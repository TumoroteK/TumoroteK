...
public class IncidentManagerImpl implements IncidentManager {
	...
	private Log log = LogFactory.getLog(IncidentManager.class);
	...
	public void setEnceinteDao(EnceinteDao ec) {
		this.enceinteDao = ec;;
	}

	public void setTerminaleDao(TerminaleDao terminaleDao) {
	...
	@Override
	public Incident findByIdManager(Integer incidentId) {
	...
	@Override
	public List<Incident> findAllObjectsByConteneurManager(
			Conteneur conteneur) {
	...
	@Override
	public List<Incident> findByEnceinteManager(Enceinte enceinte) {
		return incidentDao.findByEnceinte(enceinte);
	}
	
	@Override
	public List<Incident> findByTerminaleManager(Terminale terminale) {
		return incidentDao.findByTerminale(terminale);
	}
	
	@Override
	public Boolean findDoublonManager(Incident incident) {
		if (incident != null) {
			if (incident.getConteneur() != null) {
				if (incident.getIncidentId() == null) {
					return incidentDao.findByConteneurOrderByDate(incident.getConteneur())
						.contains(incident);
				} else {
					return incidentDao.findByExcludedIdAndConteneur(
							incident.getIncidentId(), incident.getConteneur())
							.contains(incident);
				}
			} else if (incident.getEnceinte() != null) {
				if (incident.getIncidentId() == null) {
					return incidentDao.findByEnceinte(incident.getEnceinte())
						.contains(incident);
				} else {
					return incidentDao.findByExcludedIdAndEnceinte(
							incident.getIncidentId(), incident.getEnceinte())
							.contains(incident);
				}
			} else if (incident.getTerminale() != null) {
				if (incident.getIncidentId() == null) {
					return incidentDao.findByTerminale(incident.getTerminale())
						.contains(incident);
				} else {
					return incidentDao.findByExcludedIdAndTerminale(
							incident.getIncidentId(), incident.getTerminale())
							.contains(incident);
				}
			} 
		}
	 ...
	 */
	public Boolean findDoublonInListManager(List<Incident> incidents) {
	...
	@Override
	public void createObjectManager(Incident incident, Conteneur conteneur, 
		...
		if (conteneur != null) { 
			...
			log.warn("Objet obligatoire Conteneur/Enceinte/Terminale manquant"
							+ " lors de la création d'un Incident");
			throw new RequiredObjectIsNullException(
					"Incident", "creation", "Conteneur/Enceinte/Terminale");
		} 
		
		// Test s'il y a des doublons
		if (findDoublonManager(incident)) {
			log.warn("Doublon lors de la creation de l'objet Incident : " 
					+ incident.toString());
			...
			log.info("Enregistrement de l'objet Incident : " 
					+ incident.toString());
		}
	}

	@Override
	public void updateObjectManager(Incident incident, Conteneur conteneur,
		Enceinte enceinte, Terminale terminale) {
		...
		if (conteneur != null) { 
			...
			log.warn("Objet obligatoire Conteneur/Enceinte/Terminale manquant"
						+ " lors de la modification d'un Incident");
			throw new RequiredObjectIsNullException(
					"Incident", "modification", "Conteneur/Enceinte/Terminale");
		} 
		
		// Test s'il y a des doublons
		if (findDoublonManager(incident)) {
			log.warn("Doublon lors de la modification de l'objet Incident : " 
					+ incident.toString());
			...
			log.info("Modification de l'objet Incident : " 
					+ incident.toString());
		}
	}

	@Override
	public void removeObjectManager(Incident incident) {
		if (incident != null) {
			incidentDao.removeObject(incident.getIncidentId());
			log.info("Suppression de l'objet Incident : " 
					+ incident.toString());				
		} else {
