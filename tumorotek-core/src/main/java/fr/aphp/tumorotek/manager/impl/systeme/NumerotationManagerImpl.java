...
public class NumerotationManagerImpl implements NumerotationManager {
	...
	private Log log = LogFactory.getLog(NumerotationManager.class);
	/** Bean Dao. */
	private NumerotationDao numerotationDao;
	private BanqueDao banqueDao;
	private EntiteDao entiteDao;

	public void setNumerotationDao(NumerotationDao nDao) {
	...
	@Override
	public Numerotation findByIdManager(Integer numerotationId) {
	...
	@Override
	public List<Numerotation> findByBanqueAndEntiteManager(Banque banque,
			Entite entite) {
		if (banque != null && entite != null) {
			return numerotationDao.findByBanqueAndEntite(banque, entite);
		} else {
			return new ArrayList<Numerotation>();
		}
	}

	@Override
	public List<Entite> findByBanqueSelectEntiteManager(Banque banque) {
		if (banque != null) {
	...
	@Override
	public List<Numerotation> findByBanquesManager(List<Banque> banques) {
	...
	@Override
	public Boolean findDoublonManager(Numerotation numerotation) {
		if (numerotation != null) {
			if (numerotation.getNumerotationId() == null) {
				return numerotationDao.findAll()
					.contains(numerotation);
			} else {
				return numerotationDao.findByExcludedId(
						numerotation.getNumerotationId())
						.contains(numerotation);
			}
		} else {
			return false;
	...
	@Override
	public String getGeneratedCodeManager(Numerotation numerotation) {
		if (numerotation != null 
				&& numerotation.getCodeFormula() != null) {
			if (numerotation.getCurrentIncrement() != null) {
				String num = String.valueOf(numerotation
						.getCurrentIncrement());
				if (numerotation.getZeroFill() != null 
						&& numerotation.getZeroFill()) {
					int indx = 0;
					if (numerotation.getNbChiffres() != null
							&& numerotation.getNbChiffres() > num.length()) {
	...
	@Override
	public void createObjectManager(Numerotation numerotation, Banque banque,
			Entite entite) {
		//Banque required
		if (banque != null) { 
			...
			log.warn("Objet obligatoire Banque manquant"
							+ " lors de la création d'une Numerotation");
			throw new RequiredObjectIsNullException(
					"Numerotation", "creation", "Banque");
		} 
		...
		if (entite != null) { 
			...
			log.warn("Objet obligatoire Entite manquant"
							+ " lors de la création d'une Numerotation");
			throw new RequiredObjectIsNullException(
					"Numerotation", "creation", "Entite");
		} 
		
		// Test s'il y a des doublons
		if (findDoublonManager(numerotation)) {
			log.warn("Doublon lors de la creation de l'objet Numerotation : " 
					+ numerotation.toString());
			throw new DoublonFoundException("Numerotation", "creation");
		} else {
			
			numerotationDao.createObject(numerotation);
			
			log.info("Enregistrement de l'objet Numerotation : " 
					+ numerotation.toString());
		}
	}

	@Override
	public void updateObjectManager(Numerotation numerotation, Banque banque,
			Entite entite) {
		//Banque required
		if (banque != null) { 
			numerotation.setBanque(banqueDao.mergeObject(banque));
			...
			log.warn("Objet obligatoire Banque manquant"
							+ " lors de la modification d'une Numerotation");
			throw new RequiredObjectIsNullException(
					"Numerotation", "modification", "Banque");
		} 
		...
		if (entite != null) { 
			...
			log.warn("Objet obligatoire Entite manquant"
							+ " lors de la modification d'une Numerotation");
			throw new RequiredObjectIsNullException(
					"Numerotation", "modification", "Entite");
		} 
		
		// Test s'il y a des doublons
		if (findDoublonManager(numerotation)) {
			log.warn("Doublon lors de la modification de " 
					+ "l'objet Numerotation : " 
					+ numerotation.toString());
	...
	@Override
	public void removeObjectManager(Numerotation numerotation) {
		numerotationDao.removeObject(numerotation.getNumerotationId());
		log.info("Suppression de l'objet Numerotation : " 
				+ numerotation.toString());
	}
}
