...
public class ContratManagerImpl implements ContratManager {
	...
	private Log log = LogFactory.getLog(ContratManager.class);
	 ...
	 */
	public void setProtocoleTypeDao(ProtocoleTypeDao pDao) {
	 ...
	 */
	public void setContratValidator(ContratValidator cValidator) {
	 ...
	 */
	@Override
	public Contrat findByIdManager(Integer mtaId) {
	 ...
	 */
	@Override
	public List<Contrat> findAllObjectsByPlateformeManager(
	 ...
	 */
	@Override
	public List<Cession> getCessionsManager(Contrat contrat) {
		if (contrat != null) {
	 ...
	 */
	@Override
	public List<Contrat> findByNumeroLikeManager(
			String numero, boolean exactMatch) {
		
		log.debug("Recherche Contrat par numero : " 
				+ numero + " exactMatch " + String.valueOf(exactMatch));
		if (numero != null) {
			if (!exactMatch) {
				numero = numero + "%";
			}
	 ...
	 */
	@Override
	public Boolean findDoublonManager(Contrat contrat) {
		if (contrat != null) {
			if (contrat.getContratId() == null) {
				return contratDao.findAll().contains(contrat);
			} else {
				return contratDao.findByExcludedId(
						contrat.getContratId()).contains(contrat);
			}
		} else {
			return false;
		}
	 ...
	 */
	@Override
	public Boolean isUsedObjectManager(Contrat contrat) {
		if (contrat != null) {
	 ...
	 */
	@Override
	public void createObjectManager(Contrat contrat, 
			Collaborateur collaborateur, 
			Service service, Etablissement etablissement,
			ProtocoleType protocoleType,
			Plateforme plateforme, Utilisateur utilisateur) {
		...
		if (findDoublonManager(contrat)) {
			log.warn("Doublon lors de la creation de l'objet Contrat : " 
					+ contrat.toString());
			...
			BeanValidator.validateObject(
					contrat, new Validator[]{contratValidator});
						
			contratDao.createObject(contrat);
			
			log.info("Enregistrement de l'objet Contrat : " 
					+ contrat.toString());
	 ...
	 */
	@Override
	public void updateObjectManager(Contrat contrat, 
			Collaborateur collaborateur, 
			Service service, Etablissement etablissement,
			ProtocoleType protocoleType,
			Plateforme plateforme, Utilisateur utilisateur) {
		...
		if (plateforme != null) { 
			...
			log.warn("Objet obligatoire Plateforme manquant"
							+ " lors de la modification d'un Contrat");
			throw new RequiredObjectIsNullException(
					"Contrat", "modification", "Plateforme");
		} 
		...
		// Test s'il y a des doublons
		if (findDoublonManager(contrat)) {
			log.warn("Doublon lors de la modification de l'objet Contrat : " 
					+ contrat.toString());
			...
			log.info("Modification de l'objet Contrat : " 
					+ contrat.toString());
	...
	@Override
	public void removeObjectManager(Contrat contrat, 
										String comments, Utilisateur u) {
		if (contrat != null) {
			if (isUsedObjectManager(contrat)) {
				log.warn("Objet utilisé lors de la suppression de l'objet " 
						+ "Contrat : " + contrat.toString());
				throw new 
					ObjectUsedException("contrat.deletion.isReferenced", 
																	false);
			} else {
				
				//Supprime operations associes
				CreateOrUpdateUtilities
					.removeAssociateOperations(contrat, operationManager, 
																comments, u);
				
				contratDao.removeObject(contrat.getContratId());
				log.info("Suppression de l'objet Contrat : " 
						+ contrat.toString());				
			}
		} else {
