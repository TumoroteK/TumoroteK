...
public class TumoPrinterUtilsManagerImpl implements TumoPrinterUtilsManager {
	...
	public void setExtractValueFromChampManager(
			ExtractValueFromChampManager eManager) {
		this.extractValueFromChampManager = eManager;
	}

	@Override
	public List<LigneEtiquette> extractDynData(TKStockableObject obj,
			Modele modele) {
		List<LigneEtiquette> db_lignes = null;
		List<LigneEtiquette> dyn_lignes = new ArrayList<LigneEtiquette>();
			...
			for (int i = 0; i < db_lignes.size(); i++) {
				...
				if (db_lignes.get(i).getContenu() == null) {
					...
					List<ChampLigneEtiquette> champs = champLigneEtiquetteManager
							.findByLigneEtiquetteAndEntiteManager(
									db_lignes.get(i),
									entiteManager.findByNomManager(
					...
					for (int j = 0; j < champs.size(); j++) {
						...
						if (obj.entiteNom().equals(DERIVE)) {
							sb = extractValueFromChampManager
									.extractValueForChampManager(
											(ProdDerive) obj, champs.get(j)
													.getChamp());
						} else if ((obj.entiteNom().equals(ECHANTILLON))) {
							sb = extractValueFromChampManager
									.extractValueForChampManager(
											(Echantillon) obj, champs.get(j)
													.getChamp());
						}
						...
						contenu.append(Utils.formateString(sb,
								champs.get(j).getExpReg()));
					}
	...
	@Override
	public List<LigneEtiquette> createListEtiquette(TKStockableObject obj,
			Modele modele) {
		List<LigneEtiquette> ligneEtiquette = new ArrayList<LigneEtiquette>();
		...
		String nom = "";
		if (obj.entiteNom().equals(DERIVE)) {
			Patient pat = prodDeriveManager
					.getPatientParentManager((ProdDerive) obj);
			if (pat != null) {
				nom = pat.getNom();
			}
			param4 = PrinterFormat.formaterNom(nom);
		} else if (obj.entiteNom().equals(ECHANTILLON)) {
			Prelevement prlvt = echantillonManager
					.getPrelevementManager((Echantillon) obj);
		...
		if (obj.getDateStock() != null) {
			date = new SimpleDateFormat("dd/MM/yyyy").format(obj.getDateStock()
					.getTime());
		}
		param5 = date;
		...
		if (obj.getQuantite() != null && obj.getQuantiteUnite() != null) {
			qte = String.valueOf(Math.round(obj.getQuantite() * 100.0) / 100.0)
					+ " "
					+ PrinterFormat.formaterUnite(obj.getQuantiteUnite()
							.getUnite());
	...
	@Override
	public Vector<String> extractStaticDataForMBio(TKStockableObject obj) {
