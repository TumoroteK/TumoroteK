...
public class ObjetStatutManagerImpl implements ObjetStatutManager {
	...
	private Log log = LogFactory.getLog(ObjetStatutManager.class);
	...
	public void setObjetStatutValidator(ObjetStatutValidator validator) {
		this.objetStatutValidator = validator;
	}

	public void setCederObjetManager(CederObjetManager c) {
	 ...
	 */
	public ObjetStatut findByIdManager(Integer objetStatutId) {
	 ...
	 */
	public List<ObjetStatut> findByStatutLikeManager(
			String statut, boolean exactMatch) {
		log.debug("Recherche ObjetStatut par " 
				+ statut + " exactMatch " + String.valueOf(exactMatch));
		if (statut != null) {
			if (!exactMatch) {
				statut = statut + "%";
			}
			...
			return new ArrayList<ObjetStatut>();
		}
	}
	 ...
	 */
	public Boolean findDoublonManager(ObjetStatut statut) {
	 ...
	 */
	public void createObjectManager(ObjetStatut statut) {
		if (findDoublonManager(statut)) {
			log.warn("Doublon lors de la creation de l'objet ObjetStatut : " 
					+ statut.toString());
			throw new DoublonFoundException("ObjetStatut", "creation");
		} else {
			BeanValidator.validateObject(
					statut, new Validator[]{objetStatutValidator});
			objetStatutDao.createObject(statut);
			log.debug("Enregistrement de l'objet ObjetStatut : " 
					+ statut.toString());
		}
	}
	 ...
	 * l'objet à créer se trouve déjà dans la base.
	 */
	public void updateObjectManager(ObjetStatut statut) {
		if (findDoublonManager(statut)) {
			log.warn("Doublon lors de la modif de l'objet ObjetStatut : " 
					+ statut.toString());
			...
			log.debug("Modification de l'objet ObjetStatut : " 
					+ statut.toString());
		}
	}
	 ...
	 */
	public void removeObjectManager(ObjetStatut statut) {
		if (isUsedObjectManager(statut)) {
			log.warn("Objet utilisé lors de la supperssion de l'objet " 
					+ "ObjetStatut : " + statut.toString());
			throw new ObjectUsedException("ObjetStatut", "suppression");
		} else {
			objetStatutDao.removeObject(statut.getObjetStatutId());
			log.debug("Suppression de l'objet ObjetStatut : " 
					+ statut.toString());
		}
	}
	...
	public ObjetStatut findStatutForTKStockableObject(TKStockableObject tkobj) {
		ObjetStatut nonStocke = findByStatutLikeManager("NON STOCKE", true).get(0);

		ObjetStatut stocke = findByStatutLikeManager("STOCKE", true).get(0);
		ObjetStatut epuise = null;

		List<Cession> cessionsEnAttente = cederObjetManager
			.getAllCessionsByStatutAndObjetManager("EN ATTENTE", tkobj);
