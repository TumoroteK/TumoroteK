...
public class ModeleManagerImpl implements ModeleManager {
	...
	private Log log = LogFactory.getLog(ModeleManager.class);
	...
	public void setModeleValidator(ModeleValidator mValidator) {
	...
	@Override
	public Modele findByIdManager(Integer modeleId) {
	...
	@Override
	public List<Modele> findByPlateformeManager(Plateforme plateforme) {
	...
	@Override
	public List<Modele> findByNomAndPlateformeManager(String nom,
	...
	@Override
	public Boolean findDoublonManager(Modele modele) {
		if (modele != null) {
			if (modele.getModeleId() == null) {
				return modeleDao.findAll().contains(modele);
			} else {
				return modeleDao.findByExcludedId(
						modele.getModeleId()).contains(modele);
			}
		} else {
			return false;
		}
	...
	@Override
	public void createObjectManager(Modele modele, Plateforme plateforme,
			ModeleType modeleType,
			List<LigneEtiquette> ligneEtiquettes,
			Hashtable<LigneEtiquette, List<ChampLigneEtiquette>> 
				champLigneEtiquettes) {
		// plateforme required
		if (plateforme != null) { 
			modele.setPlateforme(plateformeDao.mergeObject(plateforme));
		} else {
			log.warn("Objet obligatoire Plateforme manquant"
							+ " lors de la création d'un Modele");
			throw new RequiredObjectIsNullException(
					"Modele", "creation", "Plateforme");
		}
		...
		if (modeleType != null) { 
			...
			log.warn("Objet obligatoire ModeleType manquant"
							+ " lors de la création d'un Modele");
			throw new RequiredObjectIsNullException(
					"Modele", "creation", "ModeleType");
		}
		...
		if (findDoublonManager(modele)) {
			...
			BeanValidator.validateObject(
					modele, new Validator[]{modeleValidator});
			
			if (!modele.getIsDefault() 
					&& ligneEtiquettes != null) {
				for (int i = 0; i < ligneEtiquettes.size(); i++) {
					List<ChampLigneEtiquette> champs = 
						new ArrayList<ChampLigneEtiquette>();
					if (champLigneEtiquettes.containsKey(
							ligneEtiquettes.get(i))) {
						champs = champLigneEtiquettes
							.get(ligneEtiquettes.get(i));
					} else {
						champs = null;
					}
					ligneEtiquetteManager.validateObjectManager(
							ligneEtiquettes.get(i), 
							modele, champs, "creation");
				}
			}
						
			modeleDao.createObject(modele);
			
			updateAssociations(modele, ligneEtiquettes, 
					null, champLigneEtiquettes, null);
			
			log.info("Enregistrement de l'objet Modele : " 
					+ modele.toString());
		}
	}

	@Override
	public void updateObjectManager(Modele modele, 
			Plateforme plateforme,
			ModeleType modeleType,
			List<LigneEtiquette> ligneEtiquettesToCreate,
			List<LigneEtiquette> ligneEtiquettesToremove,
			Hashtable<LigneEtiquette, List<ChampLigneEtiquette>> 
				champLigneEtiquettesToCreate,
			Hashtable<LigneEtiquette, List<ChampLigneEtiquette>> 
		...
		if (plateforme != null) { 
			...
			log.warn("Objet obligatoire Plateforme manquant"
							+ " lors de la modification d'un Modele");
			throw new RequiredObjectIsNullException(
					"Modele", "modification", "Plateforme");
		}
		...
		if (modeleType != null) { 
			...
			log.warn("Objet obligatoire ModeleType manquant"
							+ " lors de la modification d'un Modele");
			throw new RequiredObjectIsNullException(
					"Modele", "modification", "ModeleType");
		}
		
		// Test s'il y a des doublons
		if (findDoublonManager(modele)) {
			log.warn("Doublon lors de la modification de l'objet Modele : " 
					+ modele.toString());
			...
			if (!modele.getIsDefault() 
					&& ligneEtiquettesToCreate != null) {
				for (int i = 0; i < ligneEtiquettesToCreate.size(); i++) {
					List<ChampLigneEtiquette> champs = 
						new ArrayList<ChampLigneEtiquette>();
					if (champLigneEtiquettesToCreate.containsKey(
							ligneEtiquettesToCreate.get(i))) {
						champs = champLigneEtiquettesToCreate
							.get(ligneEtiquettesToCreate.get(i));
					} else {
						champs = null;
					}
					ligneEtiquetteManager.validateObjectManager(
							ligneEtiquettesToCreate.get(i), 
							modele, champs, "modification");
				}
			}
						
			modeleDao.updateObject(modele);
			
			updateAssociations(modele, ligneEtiquettesToCreate, 
					ligneEtiquettesToremove, 
					champLigneEtiquettesToCreate, 
					champLigneEtiquettesToRemove);
	...
	@Override
	public void removeObjectManager(Modele modele) {
		if (modele != null) {			
			// suppression des lignes
			List<LigneEtiquette> lignes = ligneEtiquetteManager
				.findByModeleManager(modele);
			for (int i = 0; i < lignes.size(); i++) {
				ligneEtiquetteManager
					.removeObjectManager(lignes.get(i));
			}
			
			modeleDao.removeObject(modele.getModeleId());
			log.info("Suppression de l'objet Modele : " 
					+ modele.toString());
		} else {
	 ...
	 */
	public void updateAssociations(Modele modele, 
			List<LigneEtiquette> ligneEtiquettesToCreate,
			List<LigneEtiquette> ligneEtiquettesToremove,
			Hashtable<LigneEtiquette, List<ChampLigneEtiquette>> 
				champLigneEtiquettesToCreate,
		...
		if (ligneEtiquettesToremove != null) {
			...
			for (int i = 0; i < ligneEtiquettesToremove.size(); i++) {
				ligneEtiquetteManager.removeObjectManager(
						ligneEtiquettesToremove.get(i));
			}
		}
		...
		if (ligneEtiquettesToCreate != null) {
			...
			for (int i = 0; i < ligneEtiquettesToCreate.size(); i++) {
				...
				if (ligneEtiquettesToCreate.get(i)
						.getLigneEtiquetteId() == null) {
					ligneEtiquetteManager.createObjectManager(
							ligneEtiquettesToCreate.get(i), 
							modele, champs);
				} else { //update de la ligne
					// on récup les champs à supprimer
					List<ChampLigneEtiquette> champsToRmv = 
						new ArrayList<ChampLigneEtiquette>();
					if (champLigneEtiquettesToRemove.containsKey(
							ligneEtiquettesToCreate.get(i))) {
						champsToRmv = champLigneEtiquettesToRemove
							.get(ligneEtiquettesToCreate.get(i));
					} else {
						champsToRmv = null;
					}
