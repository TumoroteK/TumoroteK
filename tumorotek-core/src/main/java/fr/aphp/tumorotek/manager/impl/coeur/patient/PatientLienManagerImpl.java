...
public class PatientLienManagerImpl implements PatientLienManager {
	...
	private Log log = LogFactory.getLog(PatientLienManager.class);
	...
	@Override
	public void createOrUpdateObjectManager(PatientLien patientLien,
			Patient patient1, Patient patient2, LienFamilial lienFamilial,
			String operation) {
		...
		checkRequiredObjectsAndValidate(patientLien, patient1, patient2, 
												lienFamilial, operation);
		...
		if (!findDoublonManager(patientLien)) {
			if (operation.equals("creation")) {
				//if (operation.equals("creation")) {
					...
					log.debug("Enregistrement objet PatientLien " 
...
//				}
			...
			if (operation.equals("modification") 
					&& !patientLien.getLienFamilial()
						.equals(patientLienDao.findById(patientLien.getPk())
														.getLienFamilial())) {
				patientLienDao.updateObject(patientLien);
				log.debug("Modification objet PatientLien "
												+ patientLien.toString());
			} else {			
				log.warn("Doublon lors " + operation + " objet PatientLien "
						+ patientLien.toString());
	...
	@Override
	public void removeObjectManager(PatientLien patientLien) {
		if (patientLien != null) {
			patientLienDao.removeObject(patientLien.getPk());
				log.debug("Suppression objet PatientLien " 
												+ patientLien.toString());
		} else {
	...
	@Override
	public boolean findDoublonManager(PatientLien patientLien) {
	 ...
	 */
	private void checkRequiredObjectsAndValidate(PatientLien patientLien, 
		...
		if (patient1 != null) { 
			...
			log.warn("Objet obligatoire Patient1 manquant"
							+ " lors de la " + operation + " d'un PatientLien");
			throw new RequiredObjectIsNullException(
					"PatientLien", operation, "Patient1");
		} 
		//patient2 required
		if (patient2 != null) { 
			patientLien.setPatient2(patientDao.mergeObject(patient2));
		} else if (patientLien.getPatient2() == null) {
			log.warn("Objet obligatoire Patient2 manquant"
							+ " lors de la " + operation + " d'un PatientLien");
			throw new RequiredObjectIsNullException(
					"PatientLien", operation, "Patient2");
		} 
		//LienFamilial required
		if (lienFamilial != null) {
			patientLien.setLienFamilial(lienFamilialDao
													.mergeObject(lienFamilial));
		} else if (patientLien.getLienFamilial() == null) {			
			log.warn("Objet obligatoire LienFamilial  manquant"
						+ " lors de la " + operation + " d'un PatientLien");
			throw new RequiredObjectIsNullException(
				"PatientLien", operation, "LienFamilial");
		}
