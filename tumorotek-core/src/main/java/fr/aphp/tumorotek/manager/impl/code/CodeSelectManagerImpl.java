...
public class CodeSelectManagerImpl implements CodeSelectManager {
	...
	private Log log = LogFactory.getLog(CodeSelectManager.class);
	...
	public void setCodeSelectDao(CodeSelectDao cDao) {
	...
	public void setCommonUtilsManager(CommonUtilsManager cM) {
	...
	@Override
	public List<CodeSelect> findByCodeDossierManager(CodeDossier parent) {
		return codeSelectDao.findByCodeDossier(parent);
	}
	
	@Override
	public List<CodeCommon> findCodesFromSelectByDossierManager(
			CodeDossier parent) {		
		return extractCodeCommonFromCodeSelect(codeSelectDao
										.findByCodeDossier(parent), null);
	}

	@Override
	public List<CodeCommon> findByRootDossierManager(Utilisateur u, 
															Banque bank) {
		List<CodeSelect> codes = codeSelectDao.findByRootDossier(u, bank);
		return extractCodeCommonFromCodeSelect(codes, null);
	}

	@Override
	public List<CodeSelect> findByUtilisateurAndBanqueManager(
										Utilisateur u, Banque b) {
		return codeSelectDao.findByUtilisateurAndBanque(u, b);
	}
	
	@Override
	public List<CodeCommon> 
		findCodesFromSelectByUtilisateurAndBanqueManager(Utilisateur u, 
																Banque b) {
		return extractCodeCommonFromCodeSelect(codeSelectDao
										.findByUtilisateurAndBanque(u, b),
										null);
	}
	
	@Override
	public boolean findDoublonManager(CodeSelect code) {
		if (code.getCodeSelectId() == null) {
			return codeSelectDao.findAll().contains(code);
		} else {
			return codeSelectDao.findByExcludedId(
					code.getCodeSelectId()).contains(code);
		}	
	}

	@Override
	public void createOrUpdateManager(CodeSelect code, CodeDossier dos,
		...
		if (!findDoublonManager(code)) {
			if ((operation.equals("creation") 
				...
				if (operation.equals("creation")) {
					...
					log.info("Enregistrement objet CodeSelect " 
														+ code.toString());
					...
					log.info("Modification objet CodeSelect "
														+ code.toString());
					CreateOrUpdateUtilities
						.createAssociateOperation(code, operationManager, 
			...
			log.warn("Doublon lors " + operation + " objet CodeSelect "
					+ code.toString());
	 ...
	 */
	private void checkRequiredObjectsAndValidate(CodeSelect code, 
				Banque bank, Utilisateur utilisateur, String operation) {
		//Banque required
		if (bank != null) { 
			// merge banque object
			...
			log.warn("Objet obligatoire Banque manquant"
					+ " lors de la " + operation + " du code favori");
			throw new RequiredObjectIsNullException(
					"CodeSelect", operation, "Banque");
		} 
		...
		if (utilisateur != null) { 
			...
			log.warn("Objet obligatoire Utilisateur manquant"
					+ " lors de la " + operation + " du code favori");
			throw new RequiredObjectIsNullException(
					"CodeSelect", operation, "Utilisateur");
		} 	
	}
	
	@Override
	public void removeObjectManager(CodeSelect code) {
		if (code != null) {
	...
	@Override
	public List<CodeCommon> findByCodeOrLibelleLikeManager(String codeOrLibelle,
			boolean exactMatch, Utilisateur u, Banque b) {
				
		if (!exactMatch) {
			codeOrLibelle = ".*" + codeOrLibelle + ".*";
		}
		
		List<CodeSelect> codeUserBanque = new ArrayList<CodeSelect>();
		if (u == null) {
	  ...
	  */
	private List<CodeCommon> 
		extractCodeCommonFromCodeSelect(List<CodeSelect> codes,
													String codeOrLibelle) {
		List<CodeCommon> res  = new ArrayList<CodeCommon>();
		
		Iterator<CodeSelect> it = codes.iterator();
		CodeSelect next;
		CodeCommon ref;
		while (it.hasNext()) {
			next = it.next();
			ref = commonUtilsManager
				.findCodeByTableCodageAndIdManager(next.getCodeId(), 
													next.getTableCodage());
			
			if (ref != null) {
				ref.setCodeSelect(next);
				if (codeOrLibelle != null) {
					if (ref.getCode().matches(codeOrLibelle)
								|| ref.getLibelle().matches(codeOrLibelle)) {
						res.add(ref);
					}
				} else {
					res.add(ref);
	...
	@Override
	public List<CodeCommon> findByRootDossierAndBanqueManager(Banque bank) {
		return extractCodeCommonFromCodeSelect(codeSelectDao
								.findByRootDossierAndBanque(bank), null);
	}
}
