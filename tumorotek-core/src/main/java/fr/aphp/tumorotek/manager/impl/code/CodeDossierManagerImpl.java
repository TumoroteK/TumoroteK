...
public class CodeDossierManagerImpl implements CodeDossierManager {
	...
	private Log log = LogFactory.getLog(CodeDossierManager.class);
	...
	public void setCodeDossierValidator(CodeDossierValidator cDValidator) {
		this.codeDossierValidator = cDValidator;
	}

	public void setCodeDossierDao(CodeDossierDao cDao) {
	...
	public void setCodeSelectManager(CodeSelectManager cSManager) {
		this.codeSelectManager = cSManager;
	}

	@Override
	public void createOrUpdateManager(CodeDossier dos, CodeDossier parent,
		...
		if (!findDoublonManager(dos)) {
			if ((operation.equals("creation") 
				...
				if (operation.equals("creation")) {
					...
					log.debug("Enregistrement objet CodeDossier " 
														+ dos.toString());
					CreateOrUpdateUtilities
						.createAssociateOperation(dos, operationManager, 
								operationTypeDao.findByNom("Creation").get(0),
														dos.getUtilisateur());
				} else {
					codeDossierDao.updateObject(dos);
					log.debug("Modification objet CodeDossier "
														+ dos.toString());
					CreateOrUpdateUtilities
						.createAssociateOperation(dos, operationManager, 
			...
			log.warn("Doublon lors " + operation + " objet CodeDossier "
					+ dos.toString());
	...
	@Override
	public List<CodeDossier> 
					findByCodeDossierParentManager(CodeDossier parent) {
		return codeDossierDao.findByCodeDossierParent(parent);
	}

	@Override
	public List<CodeDossier> 
					findByNomLikeManager(String nom, boolean exactMatch, 
															Banque bank) {
		if (!exactMatch) {
			nom = "%" + nom + "%";
		}
		log.debug("Recherche CodeUtilisateur par code: " 
	...
	@Override
	public List<CodeDossier> 
				findByRootCodeDossierUtilisateurManager(Banque bank) {
		return codeDossierDao.findByRootCodeDossierUtilisateur(bank);
	}
	
	@Override
	public List<CodeDossier> 
					findByRootCodeDossierSelectManager(Utilisateur user, 
														Banque bank) {
		return codeDossierDao.findByRootCodeDossierSelect(user, bank);
	}

	@Override
	public List<CodeDossier> findBySelectUtilisateurAndBanqueManager(
			Utilisateur u, Banque b) {
		return codeDossierDao.findBySelectUtilisateurAndBanque(u, b);
	}

	@Override
	public List<CodeDossier> findByUtilisateurAndBanqueManager(Utilisateur u,
			Banque b) {
		return codeDossierDao.findByUtilisateurAndBanque(u, b);
	}

	@Override
	public boolean findDoublonManager(CodeDossier dos) {
		if (dos.getCodeDossierId() == null) {
			return codeDossierDao.findAll().contains(dos);
		} else {
			return codeDossierDao.findByExcludedId(
				dos.getCodeDossierId()).contains(dos);
		}	
	}

	@Override
	public void removeObjectManager(CodeDossier dos) {
		if (dos != null) {
			...
			if (!dos.getCodeSelect()) {
				Iterator<CodeUtilisateur> it 
						= codeUtilisateurManager
								.findByCodeDossierManager(dos).iterator();
				while (it.hasNext()) {
					codeUtilisateurManager.removeObjectManager(it.next());
				}
			} else {
				Iterator<CodeSelect> it 
					= codeSelectManager
								.findByCodeDossierManager(dos).iterator();
				while (it.hasNext()) {
			...
			CreateOrUpdateUtilities.
					removeAssociateOperations(dos, operationManager);
		} else {
	 ...
	 */
	private void checkRequiredObjectsAndValidate(CodeDossier dos, 
				Banque bank, Utilisateur utilisateur, String operation) {
		//Banque required
		if (bank != null) { 
			// merge banque object
			...
			log.warn("Objet obligatoire Banque manquant"
					+ " lors de la " + operation + " du code dossier");
			throw new RequiredObjectIsNullException(
					"CodeDossier", operation, "Banque");
		} 
		...
		if (utilisateur != null) { 
			...
			throw new RequiredObjectIsNullException(
					"CodeDossier", operation, "Utilisateur");
		} 	

		//Validation
		BeanValidator.validateObject(dos, 
								new Validator[]{codeDossierValidator});
	}

	@Override
	public List<CodeDossier> findByRootDossierBanqueManager(Banque bank,
														Boolean select) {
