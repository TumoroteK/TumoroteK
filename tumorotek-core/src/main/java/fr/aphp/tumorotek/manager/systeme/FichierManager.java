...
/**
 *
 * Interface pour le manager du bean de domaine Fichier. Interface créée le
 * 23/09/09.
 *
...
public interface FichierManager {
	...
	/**
	 * Recherche tous les Fichiers dont la valeur commence comme celle passée en
	 * paramètre.
	 * 
	...
	/**
	 * Test le path d'un Fichier est partagé par plusieurs fichiers et donc
	 * référencé par plusieurs échantillons ou annotations.
	 * 
	 * @param path Fichier que l'on souhaite tester.
	...
	/**
	 * Persist une instance de Fichier dans la base de données. Cree un fichier dans
	 * le systeme.
	 * 
	...
	/**
	 * Sauvegarde les modifications apportées à un objet persistant. Modifie ou non
	 * le contenu du fichier dans le systeme.
	 * 
	...
	/**
	 * Supprime un Fichier de la base de données. Supprime ou non le fichier dans le
	 * systeme
	 * 
	...
	/**
	 * Cree, modifie ou suppr la reference vers le fichier pour le
	 * TKFileSettableObject passé en paramètre. Si un stream est fourni, réalise les
	 * opérations de création/modification dans le file system en ajoutant dans la
	 * liste filesCreated les objets File créés et dans la liste filesToDelete les
	 * objets File qui seront à supprimés. La liste filesToDelete est parcouru en
	 * fin de méthode (quand aucun rollback n'est possible). La liste filesCreated
	 * est utilisé lors d'un rollback pour supprimer les fichiers du file system.
	 * 
	...
	/**
	 * [Correctif bug TK-155] Change le fichier de collection, lors du changement de
	 * collection d'un objet et de ses annotations. Met à jour le path et déplace
	 * physiquement le fichier dans le système de stockage.
	 * 
	 * @param file  Fichier
