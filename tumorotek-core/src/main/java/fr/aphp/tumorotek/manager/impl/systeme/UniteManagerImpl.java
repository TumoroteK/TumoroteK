...
public class UniteManagerImpl implements UniteManager {
	...
	private Log log = LogFactory.getLog(UniteManager.class);
	...
	public void setUniteValidator(UniteValidator validator) {
	 ...
	 * @return Une Unite.
	 */
	public Unite findByIdManager(Integer uniteId) {
	 ...
	 * @return Liste d'Unite
	 */
	public List<Unite> findByUniteLikeManager(
			String unite, boolean exactMatch) {
		log.debug("Recherche Unite par unite " 
				+ unite + " exactMatch " + String.valueOf(exactMatch));
		if (unite != null) {
			if (!exactMatch) {
				unite = unite + "%";
			}
	 ...
	 * @return Liste d'Unite
	 */
	public List<Unite> findByTypeLikeManager(
			String type, boolean exactMatch) {
		log.debug("Recherche Unite par type " 
	 ...
	 */
	public Boolean findDoublonManager(Unite unite) {
	 ...
	 */
	public void createObjectManager(Unite unite) {
		if (findDoublonManager(unite)) {
			log.warn("Doublon lors de la creation de l'objet Unite : " 
					+ unite.toString());
			throw new DoublonFoundException("Unite", "creation");
		} else {
			BeanValidator.validateObject(
					unite, new Validator[]{uniteValidator});
			uniteDao.createObject(unite);
			log.info("Enregistrement de l'objet Unite : " 
					+ unite.toString());
		}
	}
	 ...
	 * l'objet à créer se trouve déjà dans la base.
	 */
	public void updateObjectManager(Unite unite) {
		if (findDoublonManager(unite)) {
			log.warn("Doublon lors de la modif de l'objet Unite : " 
					+ unite.toString());
			...
			log.info("Modification de l'objet Unite : " 
					+ unite.toString());
		}
	}
	 ...
	 */
	public void removeObjectManager(Unite unite) {
		if (isUsedObjectManager(unite)) {
			log.warn("Objet utilisé lors de la suppression de l'objet Unite : " 
					+ unite.toString());
			throw new ObjectUsedException("Unite", "suppression");
		} else {
			uniteDao.removeObject(unite.getUniteId());
			log.info("Suppression de l'objet Unite : " 
					+ unite.toString());
		}
	}
