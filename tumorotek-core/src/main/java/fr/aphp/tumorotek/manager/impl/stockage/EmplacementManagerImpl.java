...
public class EmplacementManagerImpl implements EmplacementManager {
	...
	private Log log = LogFactory.getLog(EmplacementManager.class);
	...
	public void setEmplacementValidator(EmplacementValidator eValidator) {
	 ...
	 */
	public Emplacement findByIdManager(Integer emplacementId) {
	...
	@Override
	public List<Emplacement> findByTerminaleAndVideManager(Terminale terminale
			, boolean vide) {
		log.debug("Recherche de tous les emplacements vides " 
				+ "ou non d'une terminale");
		if (terminale != null) {
			return emplacementDao.findByTerminaleAndVide(terminale, vide);
		} else {
			return new ArrayList<Emplacement>();
		}
	}
	
	@Override
	public List<Emplacement> findByTerminaleAndPosition(Terminale terminale,
			Integer position) {
		if (terminale != null && position != null) {
			return emplacementDao.findByTerminaleAndPosition(
					terminale, position);
		} else {
			return new ArrayList<Emplacement>();
		}
	...
	@Override
	public List<Emplacement> findByTerminaleWithOrder(Terminale terminale) {
	...
	@Override
	public Boolean checkEmplacementInTerminale(Emplacement emplacement) {
		Boolean valide = true;
		
		// on vérifie que l'emplacement n'est pas vide
		if (emplacement != null) {
			Terminale term = emplacement.getTerminale();
			
			if (term != null && emplacement.getPosition() != null) {
				// sa position doit être contenue dans les limites de
				// l'enceinte terminale
				Integer nbPlaces = term.getTerminaleType().getNbPlaces();
	...
	@Override
	public Emplacement findByEmplacementAdrlManager(String adrl, 
												Banque banque) {
		Emplacement empl = null;
		if (adrl != null && !adrl.equals("")) {
			String[] elts = adrl.split("\\.");
			if (elts.length >= 3) {
				String position = elts[elts.length - 1];
				String terminale = elts[elts.length - 2];
				String conteneur = elts[0];
				List<String> enceintes = new ArrayList<String>();
				if (elts.length > 3) {
				...
				List<Conteneur> conteneurs = conteneurManager
					.findByBanqueAndCodeManager(banque, conteneur);
				if (conteneurs.size() == 1) {
					List<Enceinte> enceintesTmp = new ArrayList<Enceinte>();
					for (int i = 0; i < enceintes.size(); i++) {
						if (i == 0) {
							enceintesTmp = enceinteManager
								.findByConteneurAndNomManager(
								conteneurs.get(0), enceintes.get(i));
						} else if (enceintesTmp.size() == 1) {
							enceintesTmp = enceinteManager
								.findByEnceintePereAndNomManager(
								enceintesTmp.get(0), enceintes.get(i));
						}
					}
					
					List<Terminale> terminales = new ArrayList<Terminale>();
					if (enceintesTmp.size() == 1) {
						terminales = terminaleManager
							.findByEnceinteAndNomManager(
								enceintesTmp.get(0), terminale);
					}
...
//						}
						if (pos > 0) {
							List<Emplacement> empls = 
								findByTerminaleAndPosition(
										terminales.get(0), pos);
	...
	@Override
	public Emplacement findDoublonManager(Emplacement emplacement) {
		if (emplacement != null) {
			Terminale term = emplacement.getTerminale();
			if (emplacement.getEmplacementId() == null) {
				List<Emplacement> empls = emplacementDao
					.findByTerminaleAndPosition(term, 
...
//				}
				...
				List<Emplacement> empls = 
					emplacementDao.findByExcludedIdTerminale(
						emplacement.getEmplacementId(), term);
	...
	public Boolean isUsedObjectManager(Emplacement emplacement) {
		...
		if (emplacement != null) {
			...
			return !(emplacement.getEchantillons().size() == 0
					&& emplacement.getProdDerives().size() == 0);
	...
	@Override
	public Conteneur getConteneurManager(Emplacement emplacement) {
		Conteneur cont = null;
	...
	@Override
	public String getTerminaleAdrlManager(Terminale terminale) {
	...
	@Override
	public String getAdrlManager(Emplacement emplacement, boolean positions) {
		StringBuffer adrl = new StringBuffer();
		...
		if (emplacement != null) {
			...
			if (emplacement.getEmplacementId() != null) { 
				...
				try {
					...
					String func = !positions ? "get_adrl" : "get_adrl_positions";
					...
					if (res != null) {
						try { res.close(); } catch (Exception e) { res = null; }
					}
					if (pstmt != null) {
			...
			} else if (emplacement.getTerminale() != null
					&& emplacement.getPosition() != null) {
		
				Terminale terminale = emplacement.getTerminale();
	...
	@Override
	public List<String> getNomsForEmplacementsManager(
		...
		if (emplacements != null) {
			...
			try {
				...
				String sql = "select CODE FROM ECHANTILLON " 
					+ "WHERE ECHANTILLON_ID = ?";
				pstmt = con.prepareStatement(sql);
				
				// requete qui récupère le code d'un dérivé
				sql = "select CODE FROM PROD_DERIVE " 
					+ "WHERE PROD_DERIVE_ID = ?";
	...
	@Override
	public List<String> getTypesForEmplacementsManager(
			List<Emplacement> emplacements) {
		List<String> results = new ArrayList<String>();
		...
		if (emplacements != null) {
			...
			try {
				...
				String sql = "SELECT et.TYPE FROM ECHANTILLON_TYPE et "
					+ "JOIN ECHANTILLON e ON e.ECHANTILLON_TYPE_ID "
					+ "= et.ECHANTILLON_TYPE_ID "
					+ "WHERE e.ECHANTILLON_ID = ?";
				pstmt = con.prepareStatement(sql);
				
				// requete qui récupère le type d'un dérivé
				sql = "SELECT pt.TYPE FROM PROD_TYPE pt "
					+ "JOIN PROD_DERIVE p ON p.PROD_TYPE_ID "
					+ "= pt.PROD_TYPE_ID "
	...
	@Override
	public String getNumerotationByPositionAndTerminaleManager(Integer position,
			Terminale terminale) {
		...
		if (position != null 
			...
			if (terminale.getTerminaleType().getScheme() != null) {
				...
				// longueur
			} else if (terminale.getTerminaleType().getHauteur() != null
				...
				if (position <= terminale.getTerminaleType().getLongueur()) {
					numColonne = position;
					numLigne = 1;
				} else if (position < terminale.getTerminaleType()
						.getNbPlaces()) {
					// sinon on cherche le numéro de ligne et de colonne de
					// la position
					if (position % terminale.getTerminaleType()
							.getLongueur() != 0) {
						numLigne = (position / terminale.getTerminaleType()
								.getLongueur()) + 1;
						numColonne = (position % terminale.getTerminaleType()
								.getLongueur());
					} else {
						numLigne = position / terminale.getTerminaleType()
								.getLongueur();
						numColonne = terminale.getTerminaleType()
								.getLongueur();
					}
				} else if (position.equals(terminale.getTerminaleType()
						.getNbPlaces())) {
					// sinon on cherche le numéro de ligne et de colonne de
					// la position
		...
		if (numLigne != 0 && numColonne != 0) {
			if (terminale.getTerminaleNumerotation()
				...
				if (terminale.getTerminaleNumerotation()
					...
					sb.append(Utils.createListChars(
						numLigne, null, 
						new ArrayList<String>()).get(numLigne - 1));
				}
				sb.append("-");
				// si la numérotation est numérique, on met le numéro
				if (terminale.getTerminaleNumerotation()
						.getColonne().equals("NUM")) {
					sb.append(numColonne);
				} else {
					// sinon on crée un code en fct du numéro
					sb.append(Utils.createListChars(
						numColonne, null, 
						new ArrayList<String>()).get(numColonne - 1));
				}
			}
		}
	...
	@Override
	public Integer getPositionByAdrl(Terminale terminale, String adrlPos) {

		if (terminale != null && adrlPos != null) {
			// pos like
			if (adrlPos.matches("\\d+") || 
					terminale.getTerminaleNumerotation(
											).getLigne().equals("POS")) {
				if (Integer.valueOf(adrlPos) <= terminale
									.getTerminaleType().getNbPlaces()) {
				...
				Integer numLigne;
				Integer numColonne;
				String[] adrlPosLigneCol = adrlPos.split("-");
				...
				if (terminale.getTerminaleNumerotation()
					...
					if (terminale.getTerminaleType().getScheme() == null) {
						numLigne = Utils.createListChars(
							terminale.getTerminaleType().getHauteur(), null, 
							new ArrayList<String>()).indexOf(adrlPosLigneCol[0]) + 1;
					} else {
						numLigne = Utils.createListChars(
							terminale.getTerminaleType().getScheme().split(";").length, null, 
							new ArrayList<String>()).indexOf(adrlPosLigneCol[0]) + 1;
					}
				}
				// numCol
				if (terminale.getTerminaleNumerotation()
					...
					if (terminale.getTerminaleType().getScheme() == null) {
						numColonne = Utils.createListChars(
							terminale.getTerminaleType().getLongueur(), null, 
							new ArrayList<String>()).indexOf(adrlPosLigneCol[1]) + 1;
					} else {
						numColonne = Utils.createListChars(
							Integer.parseInt(terminale.getTerminaleType()
									.getScheme().split(";")[numLigne]), null, 
							new ArrayList<String>()).indexOf(adrlPosLigneCol[1]) + 1;
					}
				}
	...
	@Override
	public Integer getPositionByCoordonnees(Terminale terminale,
			Integer numLigne, Integer numColonne) {
		Integer position = 0;
		
		if (terminale != null && terminale.getTerminaleId() != null
				&& numLigne != null && numLigne > 0
				&& numColonne != null && numColonne > 0) {
			
			if (terminale.getTerminaleType().getScheme() != null) {
				String[] lignes = terminale.getTerminaleType()
					.getScheme().split(";");
				...
				if (numLigne <= terminale.getTerminaleType().getHauteur()
						&& numColonne <= terminale
						.getTerminaleType().getLongueur()) {
					 position = (numLigne - 1) * terminale
					 	.getTerminaleType().getHauteur();
					 position = position + numColonne;
				}
			}
		}
	...
	@Override
	public void createObjectManager(Emplacement emplacement,
			Terminale terminale, Entite entite) {
		
		//Terminale required
		if (terminale != null) { 
			emplacement.setTerminale(terminaleDao.mergeObject(terminale));
		} else {
			log.warn("Objet obligatoire Terminale manquant"
							+ " lors de la création d'un Emplacement");
			throw new RequiredObjectIsNullException(
					"Emplacement", "creation", "Terminale");
		}
		...
		if (!checkEmplacementInTerminale(emplacement)) {
			...
			throw new InvalidPositionException(
					"Emplacement", "creation", emplacement.getPosition());
		}
		
		if (emplacement.getEntite() != null 
				&& emplacement.getObjetId() != null) {
			// on vérifie que le couple Entité/ObjectId référence 
			// un objet existant
			if (entiteManager.findObjectByEntiteAndIdManager(
					emplacement.getEntite(), 
					emplacement.getObjetId()) == null) {
				log.warn("Couple Entite : " 
						+ emplacement.getEntite().toString() 
						+ " - ObjetId :"
						+ emplacement.getObjetId() + " inexistant lors de la "
						+ "création d'un objet Emplacement");
				throw new EntiteObjectIdNotExistException("Emplacement", 
						entite.getNom(), emplacement.getObjetId());
			}
		}
		
		// Test s"il y a des doublons
		if (findDoublonManager(emplacement) != null) {
			log.warn("Doublon lors de la creation de l'objet Emplacement : " 
					+ emplacement.toString());
			EmplacementDoublonFoundException dbe = 
					new EmplacementDoublonFoundException("Emplacement", "creation");
			...
			log.info("Enregistrement de l'objet Emplacement : " 
					+ emplacement.toString());
		}
	...
	@Override
	public void updateObjectManager(Emplacement emplacement,
			Terminale terminale, Entite entite) {
		...
		if (terminale != null) { 
			...
			log.warn("Objet obligatoire Terminale manquant"
							+ " lors de la modification d'un Emplacement");
			throw new RequiredObjectIsNullException(
					"Emplacement", "modification", "Terminale");
		}
		...
		if (!checkEmplacementInTerminale(emplacement)) {
			log.warn("La position n'est pas dans la limite des places de " 
					+ "la terminale");
			throw new InvalidPositionException(
					"Emplacement", "modification", emplacement.getPosition());
		}
		...
		if (emplacement.getEntite() != null 
			...
			if (entiteManager.findObjectByEntiteAndIdManager(
				...
				log.warn("Couple Entite : " 
						...
						+ " - ObjetId :"
						+ emplacement.getObjetId() + " inexistant lors de la "
						+ "modification d'un objet Emplacement");
				throw new EntiteObjectIdNotExistException("Emplacement", 
						entite.getNom(), emplacement.getObjetId());
			}
		}
		...
		if (findDoublonManager(emplacement) != null) {
			log.warn("Doublon lors de la modification de l'objet " 
					+ "Emplacement : " + emplacement.toString());
			EmplacementDoublonFoundException dbe = 
				new EmplacementDoublonFoundException("Emplacement", "modification");
			dbe.setMessage("error.emplacement.doublon");
			dbe.setTerminale(emplacement.getTerminale());
			dbe.setPosition(emplacement.getPosition());
			throw dbe;
			...
			log.info("Modification de l'objet Emplacement : " 
					+ emplacement.toString());
		}
	...
	@Override
	public void removeObjectManager(Emplacement emplacement) {
		if (emplacement != null) {
			if (isUsedObjectManager(emplacement)) {
				...
				log.info("Suppression de l'objet Emplacement : " 
						+ emplacement.toString());				
			}
		} else {
	...
	@Override
	public List<Emplacement> createMultiObjetcsManager(
			Terminale terminale, Integer number) {
		...
		if (terminale != null) { 
			if (number != null) {
				if (number > terminale.getTerminaleType().getNbPlaces()) {
					log.warn("La position n'est pas dans la limite " 
							...
							+ "la terminale");
					throw new InvalidPositionException(
							"Emplacement", "creation", number);
				}
			
				String adrl = getTerminaleAdrlManager(terminale);
				...
				for (int i = 0; i < number; i++) {
					...
					StringBuffer sb = new StringBuffer(adrl);
					sb.append(".");
			...
			log.warn("Objet obligatoire Terminale manquant"
							+ " lors de la creation d'un Emplacement");
			throw new RequiredObjectIsNullException(
					"Emplacement", "creation", "Terminale");
		}
	...
	@Override
	public void deplacerMultiEmplacementsManager(
			List<Emplacement> emplacements,
			Utilisateur utilisateur) {
		if (emplacements != null) {
			validateMultiEmplacementsManager(emplacements);

			OperationType opType = operationTypeDao
				.findByNom("Deplacement").get(0);
			for (int i = 0; i < emplacements.size(); i++) {
				Emplacement emplacement = emplacements.get(i);
				
				if (emplacement != null 
						&& emplacement.getEmplacementId() != null) {
//					// Test de la position
//					if (!checkEmplacementInTerminale(emplacement)) {
//						log.warn("La position n'est pas dans la limite " 
//								+ "des places de " 
...
//							emplacement, new Validator[]{emplacementValidator});
					...
					if (!emplacement.getVide() 
							&& emplacement.getObjetId() != null
							&& emplacement.getEntite() != null) {
						Object obj = entiteManager
						...
						if (obj != null) {
							...
							operationManager.createObjectManager(
									op, utilisateur,
									opType, obj);
						}
					}
	...
	@Override
	public void validateMultiEmplacementsManager(
			List<Emplacement> emplacements) {
		if (emplacements != null) {
			for (int i = 0; i < emplacements.size(); i++) {
				Emplacement empl = emplacements.get(i);
				...
				if (empl != null) {
					...
					if (!checkEmplacementInTerminale(empl)) {
						log.warn("La position n'est pas dans la " 
								+ "limite des places de " 
								+ "la terminale");
						throw new InvalidPositionException(
								"Emplacement", "creation", empl.getPosition());
					}
					...
					if (empl.getEntite() != null 
						...
						if (entiteManager.findObjectByEntiteAndIdManager(
								...
								empl.getObjetId()) == null) {
							log.warn("Couple Entite : " 
									+ empl.getEntite().toString() 
									+ " - ObjetId :"
									+ empl.getObjetId() 
									+ " inexistant lors de la "
									+ "création d'un objet Emplacement");
					...
					if (findDoublonManager(empl) != null) {
						Emplacement emp = findDoublonManager(empl);
						boolean skipErr = false;
						if (empl.getEmplacementId() != null) {
							// verifie si la liste contient le doublon
							// en cours de modification
							for (Emplacement empModif : emplacements) {
								if (emp.getEmplacementId()
										.equals(empModif.getEmplacementId())
						...
						if (!skipErr) {
							log.warn("Doublon lors de la creation de "
									+ "l'objet Emplacement : "
									+ empl.toString());
							EmplacementDoublonFoundException dbe = new EmplacementDoublonFoundException(
						...
						// validation du Contrat
						BeanValidator.validateObject(
								empl, new Validator[]{emplacementValidator});
					}
				}
			}
		}
	...
	@Override
	public void saveMultiEmplacementsManager(
			List<Emplacement> emplacements) {
		if (emplacements != null) {
			validateMultiEmplacementsManager(emplacements);
			...
			for (int i = 0; i < emplacements.size(); i++) {
				...
				if (empl != null) {
					...
					empl.setTerminale(terminaleDao.mergeObject(
							term));
					
					Entite ent = entiteDao.mergeObject(empl.getEntite());
					empl.setEntite(ent);
	...
	@Override
	public List<TKStockableObject> findObjByEmplacementManager(Emplacement empl,
			String entiteNom) {
		...
		if (entiteNom != null) {			
			if (entiteNom.equals("Echantillon")) {
				objs.addAll(echantillonDao.findByEmplacement(empl.getTerminale(), 
						empl.getPosition()));
			} else if (entiteNom.equals("ProdDerive")) {
				objs.addAll(prodDeriveDao.findByEmplacement(empl.getTerminale(), 
						empl.getPosition()));
			}
		}
