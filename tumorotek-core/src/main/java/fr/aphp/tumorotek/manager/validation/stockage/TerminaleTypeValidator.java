	...
	public void validate(Object obj, Errors errs) {
		...
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "type", 
					"terminaleType.type.empty");
		
		TerminaleType type = (TerminaleType) obj;
		//type valide
		...
		if (type.getNbPlaces() != null) {
			if (type.getNbPlaces() < 1) {
				errs.rejectValue("nbPlaces", 
						"terminaleType.nbPlaces.illegal");
			}
		} else {
			errs.rejectValue("nbPlaces", 
					"terminaleType.nbPlaces.illegal");
		...
		if (type.getHauteur() != null) {
			if (type.getHauteur() < 0) {
				errs.rejectValue("hauteur", 
						"terminaleType.hauteur.illegal");
			}
		} else {
			errs.rejectValue("hauteur", 
					"terminaleType.hauteur.illegal");
		...
		if (type.getLongueur() != null) {
			if (type.getLongueur() < 0) {
				errs.rejectValue("longueur", 
						"terminaleType.longueur.illegal");
			}
		} else {
			errs.rejectValue("longueur", 
					"terminaleType.longueur.illegal");
		...
		if (type.getDepartNumHaut() == null) {
			errs.rejectValue("departNumHaut", 
						"terminaleType.departNumHaut.illegal");
		}
	}
}
