...
public class OperationTypeManagerImpl implements OperationTypeManager {
	...
	private Log log = LogFactory.getLog(OperationTypeManager.class);
	...
	@Override
	public OperationType findByIdManager(Integer operationTypeId) {
	...
	@Override
	public List<OperationType> findByNomLikeManager(String nom,
			boolean exactMatch) {
		log.debug("Recherche OperationType par nom : " 
				+ nom + " exactMatch " + String.valueOf(exactMatch));
		if (nom != null) {
			if (!exactMatch) {
				nom = nom + "%";
			}
			return operationTypeDao.findByNom(nom);
		} else {
