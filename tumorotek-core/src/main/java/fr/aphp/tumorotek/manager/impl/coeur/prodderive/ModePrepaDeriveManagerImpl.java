...
public class ModePrepaDeriveManagerImpl implements ModePrepaDeriveManager {
	...
	private Log log = LogFactory.getLog(ModePrepaDeriveManager.class);
	...
	public void setModePrepaDeriveValidator(
			ModePrepaDeriveValidator mValidator) {
		this.modePrepaDeriveValidator = mValidator;
	}

	@Override
	public ModePrepaDerive findByIdManager(Integer modePrepaDeriveId) {
...
//	}
	...
	@Override
	public List<ModePrepaDerive> findByModePrepaDeriveLikeManager(String mode,
			boolean exactMatch) {
		log.debug("Recherche ModePrepaDerive par " 
		...
		if (mode != null) {
			...
			return new ArrayList<ModePrepaDerive>();
		}
	}

	@Override
	public boolean findDoublonManager(Object obj) {
		ModePrepaDerive mode = (ModePrepaDerive) obj;
		if (mode != null) {
			if (mode.getModePrepaDeriveId() == null) {
				return modePrepaDeriveDao.findAll().contains(mode);
			} else {
				return modePrepaDeriveDao.findByExcludedId(
						mode.getModePrepaDeriveId())
						.contains(mode);
			}
		} else {
			return false;
	...
	public boolean isUsedObjectManager(Object obj) {
		ModePrepaDerive mode = (ModePrepaDerive) obj;
		List<ProdDerive> derives = prodDeriveDao.findByModePrepaDerive(mode);
	...
	public void createObjectManager(Object obj) {
		...
		if (mode.getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"ModePrepaDerive", "creation", "Plateforme");
		} else {
		...
		if (findDoublonManager(mode)) {
			...
			BeanValidator.validateObject(
					mode, new Validator[]{modePrepaDeriveValidator});
			modePrepaDeriveDao.createObject(mode);
			log.info("Enregistrement de l'objet ModePrepaDerive : " 
					+ mode.toString());
		}
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (findDoublonManager(mode)) {
			...
			log.info("Modification de l'objet ModePrepaDerive : " 
					+ mode.toString());
		}
	}
...
//					+ "ModePrepaDerive : " + mode.toString());
//			throw new ObjectUsedException("ModePrepaDerive", "suppression");
//		} else {
			modePrepaDeriveDao.removeObject(mode.getModePrepaDeriveId());
			log.info("Suppression de l'objet ModePrepaDerive : " 
					+ mode.toString());
//		}
	}
