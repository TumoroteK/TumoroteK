...
/**
 * strategy dédiée à l'échantillon pour les contrôles en amont du traitement de modification des annotations par import.
 * 
 ...
 *
 */
public class ImportChampAnnotationEchantillonStrategy extends AbstractImportChampAnnotationEntiteStrategy
{

   //nomChampForControle vaut null dans le cas des échantillons car aucun champ de contrôle n'est prévu à date
   @Override
    ...
    * @param codes, codes des échantillons
    * @param banque, banque à prendre en compte
    * @return Liste des codes existants et de l'id associé.
    * @since 2.3.1
    */
   ///!\ cette requête est volontairement mis ici et non dans EchantillonManagerImpl car du fait de l'héritage, il ne faut pas gérer de transaction (cf commentaire TK-538 dans applicationContextManager.xml)
   private List<CodeIdPair> findIdByCodesAndBanque(List<String> codes, Banque banque) {
      String hql = "SELECT new "+CodeIdPair.class.getName() + "(e.code, e.echantillonId) FROM Echantillon e WHERE e.code in (:listCode) AND e.banque = :banque ";
