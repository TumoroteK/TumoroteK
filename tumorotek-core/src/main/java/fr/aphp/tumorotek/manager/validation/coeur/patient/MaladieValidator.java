...
public class MaladieValidator implements Validator {
	...
	public void validate(Object obj, Errors errs) {
		...
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "libelle", 
											"maladie.libelle.empty");
		//libelle valide
		if (maladie.getLibelle() != null) {
			if (!maladie.getLibelle()
	 ...
	 * @return errors
	 */
	public static Errors checkDateDebutCoherence(Maladie maladie) {
		
		BindException errs = new BindException(maladie, 
					"fr.aphp.tumorotek.model.coeur.patient.Maladie");
		
		if (maladie.getDateDebut() != null) {
			if (maladie.getPatient().getDateNaissance() != null 
					&& !(maladie.getDateDebut()
								.after(maladie.getPatient().getDateNaissance())
						|| maladie.getDateDebut()
							.equals(maladie.getPatient().getDateNaissance()))) {
					errs.rejectValue("dateDebut", 
							"date.validation.infDateNaissance");
			}
			
			if (maladie.getPatient().getDateDeces() != null 
					&& !(maladie.getDateDebut()
								.before(maladie.getPatient().getDateDeces())
						|| maladie.getDateDebut()
							.equals(maladie.getPatient().getDateDeces()))) {
					errs.rejectValue("dateDebut", 
							"date.validation.supDateDeces");
			} else {
				if (maladie.getDateDiagnostic() != null) {
					if (!(maladie.getDateDebut()
							.before(maladie.getDateDiagnostic())
							|| maladie.getDateDebut()
										.equals(maladie.getDateDiagnostic()))) {
							errs.rejectValue("dateDebut", 
									"maladie.dateDebut.supDateDiagnostic");
					}
				} else {
					if (!maladie.getDateDebut()
	 ...
	 * @return Errors
	 */
	public static Errors checkDateDiagCoherence(Maladie maladie) {		
		...
		if (maladie.getDateDiagnostic() != null) {
			if (maladie.getDateDebut() != null) {
				if (!(maladie.getDateDiagnostic()
						.after(maladie.getDateDebut())
						|| maladie.getDateDiagnostic()
									.equals(maladie.getDateDebut()))) {
						errs.rejectValue("dateDiagnostic", 
								"maladie.dateDiagnostic.infDateDebut");
					}
			} else {
				if (maladie.getPatient().getDateNaissance() != null 
						&& !(maladie.getDateDiagnostic()
							...
							.equals(maladie.getPatient().getDateNaissance()))) {
						errs.rejectValue("dateDiagnostic", 
								"datedate.validation.infDateNaissance");
					}
			}
