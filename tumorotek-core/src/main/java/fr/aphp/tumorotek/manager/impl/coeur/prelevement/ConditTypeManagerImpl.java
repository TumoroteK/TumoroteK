...
public class ConditTypeManagerImpl implements ConditTypeManager {
	...
	private Log log = LogFactory.getLog(ConditTypeManager.class);
	...
	/* Properties setters */
	public void setConditTypeDao(ConditTypeDao cDao) {
		this.conditTypeDao = cDao;
	}
	
	
	public void setConditTypeValidator(ConditTypeValidator ctValidator) {
	...
	public void createObjectManager(Object obj) {
		...
		if (((TKThesaurusObject) obj).getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"ConditType", "creation", "Plateforme");
		} else {
		...
		if (!findDoublonManager(obj)) {
			...
			log.warn("Doublon lors creation objet ConditType "
					+ obj.toString());
	...
	public void updateObjectManager(Object obj) {
		...
		if (!findDoublonManager(obj)) {
			...
			log.warn("Doublon lors modification objet ConditType "
					+ obj.toString());
...
//	}
	...
	@Override
	public List<ConditType> findByTypeLikeManager(String type, 
											boolean exactMatch) {
		if (!exactMatch) {
			type = type + "%";
		}
		log.debug("Recherche ConditType par type: " 
...
//			if (!isUsedObjectManager(obj)) {
				conditTypeDao
					.removeObject(((ConditType) obj).getConditTypeId());
...
//			}
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			ConditType type = (ConditType) o;
			if (type.getConditTypeId() == null) {
				return conditTypeDao.findAll().contains(type);
			} else {
				return conditTypeDao.findByExcludedId(
						type.getConditTypeId()).contains(type);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object o) {
		ConditType conditType = conditTypeDao.mergeObject((ConditType) o);
