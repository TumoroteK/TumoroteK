...
public class RequeteManagerImpl implements RequeteManager {
	...
	private Log log = LogFactory.getLog(RequeteManager.class);
	 ...
	 */
	public void setGroupementManager(GroupementManager grManager) {
	 ...
	 */
	public void setRequeteValidator(RequeteValidator validator) {
	 ...
	 */
	@Override
	public Requete findByIdManager(Integer id) {
		//On vérifie que l'identifiant n'est pas nul
		if (id == null) {
			log.warn("Objet obligatoire identifiant manquant lors de la " 
					+ "recherche par l'identifiant d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "recherche par identifiant", "identifiant");
		}
	 ...
	 */
	@Override
	public void renameRequeteManager(Requete requete, String intitule) {
		//On verifie que la requete n'est pas nulle
		if (requete == null) {
			log.warn("Objet obligatoire Requete manquant lors " 
					+ "du renommage d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "modification", "Requete");
		}
		if (findByIdManager(requete.getRequeteId()) == null) {
			throw new SearchedObjectIdNotExistException("Requete",
					requete.getRequeteId());
		} else {
			...
			if (findDoublonManager(requete)) {
				log.warn("Doublon lors de la modification de l'objet "
						+ "Requete : " + requete.toString());
				throw new DoublonFoundException("Requete", "modification");
			} else {
				BeanValidator.validateObject(requete,
						new Validator[] { requeteValidator });
	 ...
	 */
	@Override
	public Requete copyRequeteManager(Requete requete, Utilisateur copieur,
			Banque banque) {
		//On verifie que la requete n'est pas nulle
		if (requete == null) {
			log.warn("Objet obligatoire Requete manquant lors " 
					+ "de la copie d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "copie", "Requete");
		}
		...
		if (banque == null) {
			log.warn("Objet obligatoire Banque manquant lors " 
					+ "de la création d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "création", "Banque");
		}
		//On copie le groupement racine
		Groupement groupement = groupementManager.copyGroupementManager(requete
				.getGroupementRacine());
		//copie de la requete
		Requete r = new Requete(requete.getIntitule(), copieur, groupement);
		r.setBanque(banque);
		BeanValidator.validateObject(r,
				new Validator[] { requeteValidator });
	 ...
	 */
	@Override
	public void createObjectManager(Requete requete,
			Groupement groupement, Utilisateur createur,
			Banque banque) {
	 ...
	 */
	@Override
	public void updateObjectManager(Requete requete,
			Groupement groupement, Utilisateur createur) {
		//On vérifie que la requête n'est pas nulle
		if (requete == null) {
			log.warn("Objet obligatoire Requete manquant lors " 
					+ "de la modification d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "modification", "Requete");
		}
		//On vérifie que le groupement n'est pas nul
		if (groupement == null) {
			log.warn("Objet obligatoire Groupement manquant lors " 
					+ "de la modification d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "modification", "Groupement");
		}
		//On vérifie que le createur n'est pas nul
		if (createur == null) {
			log.warn("Objet obligatoire Utilisateur manquant lors " 
		...
		Groupement oldGroupement = requete.getGroupementRacine();
		if (groupement.getGroupementId() != null) {
			groupement = groupementDao.mergeObject(groupement);
		} else {			
			groupementManager.createObjectManager(groupement, groupement
		...
		groupementManager
				.removeObjectManager(oldGroupement);
		}
	}
	 ...
	 */
	@Override
	public void removeObjectManager(Requete requete) {
		//On vérifie que la requête n'est pas nulle
		if (requete == null) {
			throw new RequiredObjectIsNullException("Requete",
					"suppression", "Requete");
		}
		...
		if (findByIdManager(requete.getRequeteId()) == null) {
			...
			if (isUsedObjectManager(requete)) {
				...
				Iterator<Requete> it = requetes.iterator();
				while (it.hasNext()) {
					Requete temp = it.next();
				...
				groupementManager.removeObjectManager(requete
						.getGroupementRacine());
	 ...
	 */
	@Override
	public List<Requete> findByUtilisateurManager(Utilisateur util) {
		//On vérifie que l'utilisateur n'est pas nul
		if (util == null) {
			log.warn("Objet obligatoire Utilisateur manquant lors de la " 
					+ "recherche par l'Utilisateur d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "recherche par Utilisateur", "Utilisateur");
		}
	 ...
	 */
	@Override
	public List<Requete> findByIntituleManager(String intitule) {
		//On vérifie que l'utilisateur n'est pas nul
		if (intitule == null) {
			log.warn("Objet obligatoire intitule manquant lors de la " 
					+ "recherche par l'intitulé d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "recherche par Intitulé", "Intitulé");
		}
		return requeteDao.findByIntitule(intitule);
	}
	
	@Override
	public List<Requete> findByIntituleAndUtilisateurManager(String intitule,
			Utilisateur util) {
		if (intitule != null && util != null) {
			return requeteDao.findByIntituleUtilisateur(intitule, util);
		} else {
			return new ArrayList<Requete>();
		}
	}
	 ...
	 */
	@Override
	public Boolean findDoublonManager(Requete requete) {
		//On vérifie que l'affichage n'est pas nul
		if (requete == null) {
			log.warn("Objet obligatoire Requete manquant lors de la " 
					+ "recherche de doublon d'un objet Requete");
			throw new RequiredObjectIsNullException(
					"Requete", "recherche de doublon", "Requete");
		}
		if (requete.getRequeteId() == null) {
			return requeteDao.findAll().contains(requete);
		} else {
			return requeteDao.findByExcludedId(
					requete.getRequeteId()).contains(requete);
		}
	 ...
	 */
	public Boolean isCopyManager(Requete r, Requete copie) {
		if (copie == null) {
			return false;
			...
			if (copie.getIntitule() == null) {
				if (r.getGroupementRacine() == null) {
					...
					return groupementManager.isCopyManager(
							r.getGroupementRacine(),
							copie.getGroupementRacine());
				}
			} else {
				return false;
			}
	...
	@Override
	public Boolean isUsedObjectManager(Requete requete) {
	...
	@Override
	public List<Requete> findByBanqueManager(Banque banque) {
		if (banque != null) {
	...
	@Override
	public List<Requete> findByBanqueInLIstManager(List<Banque> banques) {
