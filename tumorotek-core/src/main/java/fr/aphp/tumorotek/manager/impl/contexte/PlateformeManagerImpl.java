...
public class PlateformeManagerImpl implements PlateformeManager {
	...
	private Log log = LogFactory.getLog(PlateformeManager.class);
	...
	/** Bean Dao OperationTypeDao. */
	private OperationTypeDao operationTypeDao;
	/** Bean Manager OperationManager. */
	...
	public void setConteneurPlateformeDao(ConteneurPlateformeDao cDao) {
		this.conteneurPlateformeDao = cDao;
	}

	public void setPlateformeValidator(PlateformeValidator pValidator) {
	 ...
	 */
	public Plateforme findByIdManager(Integer plateformeId) {
	...
	public Set<Banque> getBanquesManager(Plateforme plateforme) {
		...
		Set<Banque> banques = plateforme.getBanques();
		banques.size();
	...
	public Set<Utilisateur> getUtilisateursManager(Plateforme plateforme) {
		plateforme = plateformeDao.mergeObject(plateforme);
		Set<Utilisateur> utilisateurs = plateforme.getUtilisateurs();
		utilisateurs.size();
	...
	@Override
	public Boolean findDoublonManager(Plateforme plateforme) {
		if (plateforme != null) {
			if (plateforme.getPlateformeId() == null) {
				return plateformeDao.findAll().contains(plateforme);
			} else {
				return plateformeDao.findByExcludedId(
					plateforme.getPlateformeId()).contains(plateforme);
			}
		} else {
			return false;
		}
	...
	@Override
	public void updateObjectManager(Plateforme plateforme,
		...
		if (!findDoublonManager(plateforme)) {
			plateforme.setCollaborateur(collaborateurDao
						.mergeObject(collaborateur));
			
			// validation due la pf
			BeanValidator.validateObject(
					plateforme, new Validator[]{plateformeValidator});
			
			plateformeDao.updateObject(plateforme);
			log.info("Enregistrement objet Plateforme " 
					+ plateforme.toString());
			...
			log.warn("Doublon lors modification objet Plateforme "
					+ plateforme.toString());
	 ...
	 * l'utilisateur.
	 */
	public void updateAdministrateurs(Plateforme plateforme, 
			List<Utilisateur> utilisateurs) {
		Plateforme pf = plateformeDao.mergeObject(plateforme);

		if (utilisateurs != null) {
			Iterator<Utilisateur> it = pf.getUtilisateurs().iterator();
			List<Utilisateur> utilsToRemove = new ArrayList<Utilisateur>();
			// on parcourt les Utilisateurs qui sont actuellement associés
			// a la plateforme
			while (it.hasNext()) {
				Utilisateur tmp = it.next();
			...
			for (int i = 0; i < utilsToRemove.size(); i++) {
				Utilisateur u = utilisateurDao.mergeObject(
						utilsToRemove.get(i));
			...
			for (int i = 0; i < utilisateurs.size(); i++) {
				...
				if (!pf.getUtilisateurs().contains(utilisateurs.get(i))) {
					...
					pf.getUtilisateurs().add(utilisateurDao.mergeObject(
							utilisateurs.get(i)));
					utilisateurDao.mergeObject(
							utilisateurs.get(i)).getPlateformes().add(pf);
					
					log.debug("Ajout de l'association entre " 
							+ "l'utilisateur : " 
							+ utilisateurs.get(i).toString() 
							+ " et la plateforme : "
							+ pf.toString());
				}
			}
	 ...
	 */
	private void updateConteneurs(Plateforme plateforme, 
			List<Conteneur> conteneurs) {
		
		if (conteneurs != null) {
			Plateforme pf = plateformeDao.mergeObject(plateforme);
			
			Iterator<ConteneurPlateforme> it = pf.getConteneurPlateformes().iterator();
			List<ConteneurPlateforme> contsToRemove = 
			...
			for (int i = 0; i < conteneurs.size(); i++) {
				...
				ConteneurPlateformePK pk = 
							new ConteneurPlateformePK(conteneurs.get(i), pf);
				cp.setPk(pk);
				cp.setPartage(true);
				...
				if (!pf.getConteneurPlateformes().contains(cp)) {
					...
					pf.getConteneurPlateformes()
								.add(conteneurPlateformeDao.mergeObject(cp));
					
					log.debug("Ajout de l'association entre la plateforme : " 
							+ pf.toString() + " et le conteneur : "
