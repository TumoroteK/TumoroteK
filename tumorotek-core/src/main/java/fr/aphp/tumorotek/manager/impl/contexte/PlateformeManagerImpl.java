...
public class PlateformeManagerImpl implements PlateformeManager
{
   ...
   public Plateforme createObjectManager(Plateforme plateforme, final Collaborateur collaborateur,
      final List<Utilisateur> utilisateurs, final Utilisateur admin, final String baseDir){
      ...
      if(plateforme != null){
         ...
         if(!findDoublonManager(plateforme)){
            ...
            if(plateforme.getUtilisateurs().contains(admin)){
               currAdmin = plateforme.getUtilisateurs().stream().filter(u -> u.equals(admin)).findFirst().orElse(admin);
            }

            operationManager.createObjectManager(creationOp, currAdmin, operationTypeDao.findByNom("Creation").get(0),
               plateforme);
   ...
   @Override
   public void removeObjectManager(final Plateforme pf, final String comments, final Utilisateur user, final String basedir){
      if(pf != null){
