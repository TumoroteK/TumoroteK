...
public class ContexteManagerImpl implements ContexteManager {
	
	//private Log log = LogFactory.getLog(ContexteManager.class);
	...
	@Override
	public List<Contexte> findByNomManager(String nom) {
		return contexteDao.findByNom(nom);
	}

	@Override
	public List<Contexte> findByOrderManager() {
		return contexteDao.findByOrder();
	}

	@Override
	public Set<Catalogue> getCataloguesManager(Contexte c) {
		Set<Catalogue> catas = new HashSet<Catalogue>();
		if (c != null) {
			Contexte cont = contexteDao.mergeObject(c);
