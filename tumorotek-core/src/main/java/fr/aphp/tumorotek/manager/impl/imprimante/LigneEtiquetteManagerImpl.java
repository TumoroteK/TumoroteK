...
public class LigneEtiquetteManagerImpl implements LigneEtiquetteManager {
	...
	private Log log = LogFactory.getLog(LigneEtiquetteManager.class);
	...
	public void setLigneEtiquetteDao(LigneEtiquetteDao lDao) {
	...
	public void setChampLigneEtiquetteManager(
			ChampLigneEtiquetteManager cManager) {
		this.champLigneEtiquetteManager = cManager;
	}

	public void setLigneEtiquetteValidator(
			LigneEtiquetteValidator lValidator) {
		this.ligneEtiquetteValidator = lValidator;
	}

	@Override
	public LigneEtiquette findByIdManager(Integer ligneEtiquetteId) {
	...
	@Override
	public List<LigneEtiquette> findByModeleManager(Modele modele) {
		log.debug("Recherche de toutes les LigneEtiquettes d'un modèle.");
		if (modele != null) {
			return ligneEtiquetteDao.findByModele(modele);
		} else {
			return new ArrayList<LigneEtiquette>();
		}
	}
	
	@Override
	public void validateObjectManager(LigneEtiquette ligneEtiquette,
			Modele modele, List<ChampLigneEtiquette> champLigneEtiquettes,
			String operation) {
		// modele required
		if (modele == null) {
			log.warn("Objet obligatoire Modele manquant"
					+ " lors de la création d'une LigneEtiquette");
			throw new RequiredObjectIsNullException(
					"LigneEtiquette", operation, "Modele");
		}
		
		if (champLigneEtiquettes != null) {
			for (int i = 0; i < champLigneEtiquettes.size(); i++) {
				champLigneEtiquetteManager.validateObjectManager(
						champLigneEtiquettes.get(i), 
						ligneEtiquette, 
						champLigneEtiquettes.get(i).getEntite(), 
						champLigneEtiquettes.get(i).getChamp(), 
						operation);
			}
		}
		
		// validation due l'utilisateur
		BeanValidator.validateObject(ligneEtiquette, 
				new Validator[]{ligneEtiquetteValidator});
	}

	@Override
	public void createObjectManager(LigneEtiquette ligneEtiquette,
			Modele modele, List<ChampLigneEtiquette> champLigneEtiquettes) {
		// validation de l'objet
		validateObjectManager(ligneEtiquette, modele, 
				champLigneEtiquettes, "creation");
		...
		log.info("Enregistrement objet LigneEtiquette " 
				+ ligneEtiquette.toString());
	}

	@Override
	public void updateObjectManager(LigneEtiquette ligneEtiquette,
			Modele modele,
			List<ChampLigneEtiquette> champLigneEtiquettesToCreate,
			List<ChampLigneEtiquette> champLigneEtiquettesToremove) {
		// validation de l'objet
		validateObjectManager(ligneEtiquette, modele, 
				champLigneEtiquettesToCreate, "modification");
		
		ligneEtiquette.setModele(modeleDao.mergeObject(modele));
		
		ligneEtiquetteDao.updateObject(ligneEtiquette);
		
		updateAssociations(ligneEtiquette, 
	...
	@Override
	public void removeObjectManager(LigneEtiquette ligneEtiquette) {
		if (ligneEtiquette != null) {
			// suppression des champs
			List<ChampLigneEtiquette> champs = champLigneEtiquetteManager
				.findByLigneEtiquetteManager(ligneEtiquette);
			for (int i = 0; i < champs.size(); i++) {
				champLigneEtiquetteManager
					.removeObjectManager(champs.get(i));
			}
			
			ligneEtiquetteDao.removeObject(
					ligneEtiquette.getLigneEtiquetteId());
			log.info("Suppression de l'objet LigneEtiquette : " 
					+ ligneEtiquette.toString());	
		} else {
	 ...
	 */
	public void updateAssociations(LigneEtiquette ligneEtiquette, 
			List<ChampLigneEtiquette> champLigneEtiquettesToCreate,
			List<ChampLigneEtiquette> champLigneEtiquettesToremove) {		
		...
		if (champLigneEtiquettesToremove != null) {
			...
			for (int i = 0; i < champLigneEtiquettesToremove.size(); i++) {
				champLigneEtiquetteManager.removeObjectManager(
						champLigneEtiquettesToremove.get(i));
			}
		}
		...
		if (champLigneEtiquettesToCreate != null) {
			...
			for (int i = 0; i < champLigneEtiquettesToCreate.size(); i++) {
				if (champLigneEtiquettesToCreate.get(i)
						.getChampLigneEtiquetteId() == null) {
					champLigneEtiquetteManager.createObjectManager(
							champLigneEtiquettesToCreate.get(i), 
							ligneEtiquette, 
							champLigneEtiquettesToCreate.get(i).getEntite(), 
							champLigneEtiquettesToCreate.get(i).getChamp());
				} else {
					champLigneEtiquetteManager.updateObjectManager(
							champLigneEtiquettesToCreate.get(i), 
							ligneEtiquette, 
							champLigneEtiquettesToCreate.get(i).getEntite(), 
							champLigneEtiquettesToCreate.get(i).getChamp());
