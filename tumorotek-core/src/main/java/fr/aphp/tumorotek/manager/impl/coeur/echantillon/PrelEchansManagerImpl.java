...
public class PrelEchansManagerImpl implements PrelEchansManager {
	...
	@Override
	public Prelevement createQuickPrelAndEchansManager(Patient pat, Maladie mal, Prelevement prel, List<EchantillonDTO> echanDTOs, 
		...
		try {
			...
			if (prel != null && prel.getPrelevementId() == null) {
				...
				prelevementManager.createObjectManager(
						pClone, 
						banque, 
						pClone.getNature(), 
						mClone,
						pClone.getConsentType(), 
						pClone.getPreleveur(),
						pClone.getServicePreleveur(), 
						pClone.getPrelevementType(), 
						pClone.getConditType(), 
						pClone.getConditMilieu(), 
						pClone.getTransporteur(), 
						pClone.getOperateur(), 
						pClone.getQuantiteUnite(), 
						new ArrayList<LaboInter>(pClone.getLaboInters()), 
						null,
						filesCreated,
						utilisateur, true,
						baseDir, false);				
			...
			Map<Echantillon, Emplacement> echEmplsClone = new HashMap<Echantillon, Emplacement>();
			
			//enregistrement de l'echantillon
			for (int i = 0; i < echanDTOs.size(); i++) {
				if (echanDTOs.get(i).isNew()) {
					Echantillon newEchan = echanDTOs
										.get(i).getEchantillon().clone();
					...
					if (echansEmpl != null && echansEmpl.containsKey(newEchan)) {
						...
						echEmplsClone.put(newEchan, echansEmpl.get(echanDTOs
										.get(i).getEchantillon()));
					}
					
					List<CodeAssigne> codes = new ArrayList<CodeAssigne>();
					for (CodeAssigne codeA : echanDTOs
...
//							.get(i).getCodesLesToCreateOrEdit());
					...
					// non confs
					if (newEchan.getConformeTraitement() != null
							&& !newEchan.getConformeTraitement()) {
					...
					if (newEchan.getConformeCession() != null
							&& !newEchan.getConformeCession()) {
						// enregistrement de la non conformité 
						// à la cession
						objetNonConformeManager
							.createUpdateOrRemoveListObjectManager(
			...
			if (!echEmplsClone.isEmpty()) {
				...
				Set<Entry<Echantillon, Emplacement>> echansEntrySet = echEmplsClone.entrySet();
				Iterator<Entry<Echantillon, Emplacement>> itE = echansEntrySet.iterator();
				Entry<Echantillon, Emplacement> e;
				while (itE.hasNext()) {
					e = itE.next();
					Echantillon echan = e.getKey();
					Emplacement empl = e.getValue();
				...
				// emplacement
				Iterator<Echantillon> echansIt = (Iterator<Echantillon>) echEmplsClone.keySet().iterator();
				// it = echans.iterator();
				ObjetStatut statut = objetStatutDao.findByStatut("STOCKE").get(0);
				while (echansIt.hasNext()) {
					Echantillon echanToUpdate = echansIt.next();
					List<OperationType> ops = operationTypeDao.findByNom("Stockage");
					
					// update de l'objet
					echantillonManager
						.saveEchantillonEmplacementManager(
							echanToUpdate, statut, 
							echEmplsClone.get(echanToUpdate), 
							utilisateur, 
							ops);
