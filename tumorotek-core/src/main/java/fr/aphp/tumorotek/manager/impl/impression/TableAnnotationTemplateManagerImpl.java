...
public class TableAnnotationTemplateManagerImpl implements
		TableAnnotationTemplateManager {
	
	private Log log = LogFactory
		.getLog(TableAnnotationTemplateManager.class);
	...
	@Override
	public TableAnnotationTemplate findByIdManager(
			TableAnnotationTemplatePK pk) {
	...
	@Override
	public List<TableAnnotationTemplate> findByExcludedPKManager(
			TableAnnotationTemplatePK pk) {
		if (pk != null) {
	...
	@Override
	public List<TableAnnotationTemplate> findByTemplateManager(
			Template template) {
		log.debug("Recherche de tous les TableAnnotationTemplates d'un " 
				+  "Template.");
		if (template != null) {
			return tableAnnotationTemplateDao.findByTemplate(template);
		} else {
			return new ArrayList<TableAnnotationTemplate>();
		}
	}
	...
	public Boolean findDoublonManager(Template template,
		...
		TableAnnotationTemplatePK pk = new 
			TableAnnotationTemplatePK(template, tableAnnotation);
	...
	public void validateObjectManager(Template template,
		...
		if (template == null) {
			log.warn("Objet obligatoire Template manquant"
				+ " lors de la validation d'un TableAnnotationTemplate");
			throw new RequiredObjectIsNullException(
					"TableAnnotationTemplate", "creation", "Template");
		}
		...
		if (tableAnnotation == null) {
			log.warn("Objet obligatoire TableAnnotation manquant"
				+ " lors de la validation d'un TableAnnotationTemplate");
			throw new RequiredObjectIsNullException(
					"TableAnnotationTemplate", "creation", "TableAnnotation");
		}
		...
		if (template.getTemplateId() != null) {
			if (findDoublonManager(template, tableAnnotation)) {
				...
				log.warn("Doublon lors validation objet " 
						+ "TableAnnotationTemplate");
				throw new DoublonFoundException(
						"TableAnnotationTemplate", "creation");
			}
		}
	}
	...
	public void createObjectManager(
			TableAnnotationTemplate tableAnnotationTemplate, Template template,
			TableAnnotation tableAnnotation) {
		...
		tableAnnotationTemplate.setTableAnnotation(
				tableAnnotationDao.mergeObject(tableAnnotation));
		
		// création
		tableAnnotationTemplateDao.createObject(tableAnnotationTemplate);
		
		log.info("Enregistrement objet TableAnnotationTemplate " 
				+ tableAnnotationTemplate.toString());
	}
	...
	public void updateObjectManager(
		...
		if (template == null) {
			log.warn("Objet obligatoire Template manquant"
				+ " lors de la validation d'un TableAnnotationTemplate");
			throw new RequiredObjectIsNullException(
					"TableAnnotationTemplate", "modification", 
					"Template");
		} else {
			tableAnnotationTemplate.setTemplate(
	...
	public void removeObjectManager(
			TableAnnotationTemplate tableAnnotationTemplate) {
		if (tableAnnotationTemplate != null) {
			tableAnnotationTemplateDao.removeObject(
					tableAnnotationTemplate.getPk());
			log.info("Suppression de l'objet TableAnnotationTemplate : " 
					+ tableAnnotationTemplate.toString());				
		} else {
