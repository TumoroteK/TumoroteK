...
//	}
	...
	public void validate(Object obj, Errors errs) {
		//code non vide
		ValidationUtils
			.rejectIfEmptyOrWhitespace(errs, "code", "prodDerive.code.empty");
		...
		if (derive.getQuantiteInit() != null) {
			if (derive.getQuantiteInit() < 0) {
				errs.rejectValue(
						"quantiteInit", "prodDerive.quantiteInit.negative");
			}
		}
		// quantite >= 0
		...
		if (derive.getVolumeInit() != null) {
			if (derive.getVolumeInit() < 0) {
				errs.rejectValue(
						"volumeInit", "prodDerive.volumeInit.negative");
			}
		}
		// volume >= 0
		...
		if (derive.getQuantiteInit() != null
			...
			if (!derive.getQuantiteInit().equals(res)) {
				errs.rejectValue("quantiteInit", 
						"prodDerive.quantiteInit.illegal");
			}
		}
		// unites valides
		if (derive.getVolumeUnite() != null) {
			if (derive.getConcUnite() != null) {
				if (!derive.getConcUnite().getUnite()
						.contains(derive.getVolumeUnite()
						.getUnite())) {
					errs.rejectValue("concUnite", 
						"prodDerive.concUnite.illegal");
				}
				
				if (derive.getQuantiteUnite() != null) {
					if (!derive.getConcUnite().getUnite()
							.contains(derive.getQuantiteUnite()
							.getUnite())) {
						errs.rejectValue("concUnite", 
							"prodDerive.concUnite.illegal");
					}
		...
		if (derive.getEmplacement() != null) {
			if (derive.getObjetStatut().getStatut().equals("NON STOCKE") 
					|| (derive.getObjetStatut().getStatut().equals("EPUISE") 
					|| (derive.getObjetStatut().getStatut().equals("DETRUIT")))) {
				errs.rejectValue("objetStatut", "prodDerive.statut.incoherent");
			}
		} else if (derive.getObjetStatut().getStatut().equals("STOCKE") 
					|| (derive.getObjetStatut().getStatut().equals("RESERVE"))) {
			errs.rejectValue("objetStatut", "prodDerive.statut.incoherent"); 
		}
	...
	@Override
	public Errors checkDateStockCoherence(ProdDerive derive) {
		
		BindException errs = new BindException(derive, 
					"fr.aphp.tumorotek.model.coeur.prodderive.ProdDerive");
		// limites inf
		if (derive.getDateStock() != null) {
			if (derive.getDateTransformation() != null) {
				ValidationUtilities.checkWithDate(derive.getDateStock(), 
						"dateStock", derive.getDateTransformation(), 
						"date", "validation", 
						"DateTransfo", errs, false);
			}
			Object[] dateAndCode = coherenceDateManager
									.findAntRefDateForDeriveManager(derive);
			if (dateAndCode[0] != null) {
				if (!ValidationUtilities.checkWithDate(derive.getDateStock(), 
						null, dateAndCode[0], null, null, null, null, false)) {
				errs.rejectValue("dateStock", (String) dateAndCode[1]);
				} 
			}
			
			//limites sup
			dateAndCode = coherenceDateManager
				.findPostRefDateInDerivesManager(derive);
			if (!ValidationUtilities
					.checkWithDate(derive.getDateStock(), null, 
	...
	@Override
	public Errors checkDateTransfoCoherence(ProdDerive derive) {
		...
		if (derive.getDateTransformation() != null) { 
			...
			if (dateAndCode[0] != null) {
				if (!ValidationUtilities
					.checkWithDate(derive.getDateTransformation(), 
						null, dateAndCode[0], null, null, null, null, false)) {
			...
			//limites sup
			if (derive.getDateStock() != null) {
				ValidationUtilities.checkWithDate(derive
											.getDateTransformation(), 
						"dateTransformation", derive.getDateStock(), "date", 
			...
			if (!ValidationUtilities
					.checkWithDate(derive.getDateTransformation(), 
						null, dateAndCode[0], null, null, null, null, true)) {
				errs.rejectValue("dateTransformation", (String) dateAndCode[1]);
			}		
		}
		return errs;
