...
public class ExportINCaManagerImpl extends ExportCatalogueManagerImpl {
	...
	private Log log = LogFactory.getLog(ExportINCaManagerImpl.class);
	...
	public void setIncaPatientExport(IncaPatientExport iExport) {
		this.incaPatientExport = iExport;
	}

	public void setIncaMaladieExport(IncaMaladieExport iExport) {
		this.incaMaladieExport = iExport;
	}

	public void setIncaPrelevementExport(IncaPrelevementExport iExport) {
		this.incaPrelevementExport = iExport;
	}
	
	public void setIncaEchantillonExport(IncaEchantillonExport iExport) {
		this.incaEchantillonExport = iExport;
	}

	public void setIncaComplementExport(IncaComplementExport iExport) {
		this.incaComplementExport = iExport;
	}
	
	public void setIncaSpecifiqueExport(IncaSpecifiqueExport iExport) {
		this.incaSpecifiqueExport = iExport;
	}

	@Override
	public Map<String, String> objetExport(Echantillon echan, 
													Connection con) {
		...
		try {
			...
			flag = "FINESS";
			map.put("FINESS", incaPatientExport
					.getFiness(con, getPrelevement(), getPatient()));	

			/*Item 2 Code patient OBLIGATOIRE */
			flag = "Identifiant patient";
			map.put("Identifiant patient", 
							incaPatientExport.getPatientId(getPatient()));
				
			/*Item 3 Date de naissance du patient OBLIGATOIRE */
			flag = "Date naissance";
			map.put("Date naissance", 
					incaPatientExport.getDateNaissance(getPatient(), 
													getDateFormatterINCA()));
			...
			map.put("Etat du patient", incaPatientExport
										.getPatientEtat(getPatient()));
			
			/*Item 6 Date de l'etat du patient */
			flag = "Date de l'Etat"; 
			map.put("Date de l'Etat", incaPatientExport
					.getDateEtat(getPatient(), getDateFormatterINCA()));
			...
			map.put("Diagnostic principal", incaMaladieExport
					.getDiagnosticPrincipal(con, getPrelevement(), true));
			
			/*Item 8 Date du diagnostic */
			flag = "Date du diagnostic"; 
			map.put("Date du diagnostic", incaMaladieExport
				.getDateDiag(con, getPrelevement(), true, 
						getDateFormatterINCA()));
			
			/*Item 9 Version cTNM */
			flag = "Version TNM"; 
			map.put("Version TNM", 
				incaMaladieExport.getVersionCTNM(con, getPrelevement()));

			/*Item 10 Taille de la tumeur */
			flag = "T du cTNM"; 
			map.put("T du cTNM", 
				incaMaladieExport.getTailleTumeur(con, getPrelevement()));
			
			/*Item 11 Envahissement ganglionnaire */
			flag = "N du cTNM"; 
			map.put("N du cTNM", incaMaladieExport
						.getEnvahGangR(con, getPrelevement()));
			
			/*Item 12 Extension metastatique */
			flag = "M du cTNM"; 
			map.put("M du cTNM", 
				incaMaladieExport.getExtMetastatique(con, getPrelevement()));
			...
			map.put("Centre stockage", 
				incaPrelevementExport.getCentreStockage(getEchantillon()));
			
			/*Item 14 ID Prelevement OBLIGATOIRE*/
			flag = "ID prelevement"; 
			map.put("ID prelevement", incaPrelevementExport
									.getCodePrelevement(getPrelevement()));
			
			/*colonne 15 Date prelevement OBLIGATOIRE */
			flag = "Date prelevement"; 
			map.put("Date prelevement", incaPrelevementExport
					.getDatePrelevement(getPrelevement(), 
							getDateFormatterINCA()));
			
			/*colonne 16 Mode de prelevement OBLIGATOIRE */
			flag = "Mode prelevement"; 
			map.put("Mode prelevement", incaPrelevementExport
									.getModePrelevement(getPrelevement()));
			
			/*colonne 17 Classification utilisee OBLIGATOIRE*/
			flag = "Classification"; 
			map.put("Classification", 
						incaPrelevementExport.getClassif(getBanque()));

			/* colonne 18 Code organe CIM*/
			flag = "Code organe CIM"; 
			if (map.get("Classification").contains("C")) {
				map.put("Code organe CIM", incaPrelevementExport
								.getCodeOrgane(con, getEchantillon(), "C"));
			} else {
			...
			if (map.get("Classification").contains("C")) {
				map.put("Type lesionnel CIM", incaPrelevementExport
								.getTypeLesionnel(con, getEchantillon(), "C"));
			} else {
			...
			if (map.get("Classification").contains("A")) {
				map.put("Code organe ADICAP", incaPrelevementExport
								.getCodeOrgane(con, getEchantillon(), "A"));
			} else {
			...
			if (map.get("Classification").contains("A")) {
				map.put("Type lesionnel ADICAP", incaPrelevementExport
								.getCodeOrgane(con, getEchantillon(), "A"));
			} else {
			...
			flag = "TYPE_EVENT"; 
			map.put("TYPE_EVENT", 
				incaPrelevementExport.getTypeEvent(con, getPrelevement()));
			
			
			/*colonne 23 Version du pTNM */	
			flag = "Version pTNM";
			map.put("Version pTNM", 
				incaPrelevementExport.getVersionPTNM(con, getPrelevement()));
			
			/*colonne 24 pT */
			flag = "T du pTNM"; 
			map.put("T du pTNM",
				incaPrelevementExport.getTailleTumeurPT(con, getPrelevement()));
			
			/*colonne 25 Presence/absence metastases pN */
			flag = "N du pTNM"; 
			map.put("N du pTNM", 
				incaPrelevementExport.getEnvGangPN(con, getPrelevement()));

			/*colonne 26 Presence/absence metastases distance pM*/
			flag = "pM"; 
			map.put("pM", incaPrelevementExport
			...
			if (incaEchantillonExport
					.getIsTumoral(getEchantillon()).equals("O")) {
				item27 = "O";
				item39 = "N";
				isTumoral = true;
			} else {
			...
			if (isTumoral) {
				...
				item28 = incaEchantillonExport
							.getModeConservation(getEchantillon());
				item40 = "";
			} else {
				flag = "Mode conservation non tumoral"; 
				item40 = incaEchantillonExport
							.getModeConservation(getEchantillon());
				item28 = "";
			}
			...
			if (isTumoral) {
				...
				item29 = incaEchantillonExport
							.getEchantillonType(getEchantillon());
			} else {
				flag = "Type echantillon non tumoral"; 
				item41 = incaEchantillonExport
							.getEchantillonType(getEchantillon());
			}
			...
			if (isTumoral) {
				...
				item30 = incaEchantillonExport
					.getModePreparation(getEchantillon());
			} else {
				flag = "Mode preparation non tumoral"; 
				item42 = incaEchantillonExport
					.getModePreparation(getEchantillon());
			}
			...
			if (isTumoral) {
				...
				item31 = incaEchantillonExport
							.getDelaiCongelation(getEchantillon());
			} else {
				flag = "Delai congelation non tumoral"; 
				item43 = incaEchantillonExport
							.getDelaiCongelation(getEchantillon());
			}	
			...
			if (isTumoral) {
				...
				item32 = incaEchantillonExport
					.getControles(con, getEchantillon());
			} else {
				flag = "Controles sur tissu non tumoral"; 
				item44 = incaEchantillonExport
					.getControles(con, getEchantillon());
			}
			...
			if (getEchantillon().getQuantite() != null 
						&& getEchantillon().getQuantiteUnite() != null) {
				if (isTumoral) {
			...
			if (isTumoral) {
				...
				item35 = incaEchantillonExport
							.getPourcentageCellulesTumorales(con, echan);
			}
			
			/* colonnes 36, 37, 38: Item36, item37, item38*/
			/* (et item47, item48, item49) */
			/* respectivement ADN, ARN, proteine derives*/
			String item36 = "N", item37 = "N", item38 = "N", item47 = "N",
								item48 = "N",	item49 = "N";
			if (isTumoral) {
				flag = "ADN derive tumoral"; 
				item36 = incaEchantillonExport
					.getProdTypeAssocie(con, getEchantillon(), 
											"([Aa][Dd][nN])|([Dd][nN][Aa])");
				flag = "ARN derive tumoral"; 
				item37 = incaEchantillonExport
					.getProdTypeAssocie(con, getEchantillon(), 
											"([Aa][Rr][nN])|([Rr][nN][Aa])");
				flag = "Proteines derives tumoral"; 
				item38 = incaEchantillonExport
					.getProdTypeAssocie(con, getEchantillon(), 
											".*[Pp][Rr][Oo][Tt][Ee][Ii][Nn].*");
			} else {
				flag = "ADN derive non tumoral"; 
				item47 = incaEchantillonExport
					.getProdTypeAssocie(con, getEchantillon(), 
											"([Aa][Dd][nN])|([Dd][nN][Aa])");
				flag = "ARN derive non tumoral"; 
				item48 = incaEchantillonExport
					.getProdTypeAssocie(con, getEchantillon(), 
											"([Aa][Rr][nN])|([Rr][nN][Aa])");
				flag = "Proteines derives non tumoral"; 
				item49 = incaEchantillonExport
					.getProdTypeAssocie(con, getEchantillon(), 
											".*[Pp][Rr][Oo][Tt][Ee][Ii][Nn].*");
			}
			...
			flag = "Serum"; 
			map.put("Serum", incaEchantillonExport
				.getRessourceBiolAssociee(getEchantillon(), "%SERUM%"));
			flag = "Plasma"; 
			map.put("Plasma", incaEchantillonExport
				.getRessourceBiolAssociee(getEchantillon(), "%PLASMA%"));
			flag = "Liquides"; 
			map.put("Liquides", incaEchantillonExport
				.getRessourceBiolAssociee(getEchantillon(), "%LIQUIDES%"));
			//ADN Constitutionnel
			flag = "ADN"; 
			map.put("ADN", incaEchantillonExport
				.getADNconstitutionnel(con, getEchantillon(), "%GENETIQUE%"));
			...
			map.put("CR anapath interrogeable", incaComplementExport
							.getCRAnapathInterro(con, getPrelevement()));
			
			/*Item55 Donnees cliniques disponibles dans une base*/
			flag = "Donnees cliniques disponibles"; 
			map.put("Donnees cliniques disponibles", 
				incaComplementExport.getDonneesClinBase(con, getPatient(),
													getBanque()));
			
			/*Item56, item57 Inclusion dans un protocole therapeutique */ 
			/* et nom du protocole*/
			flag = "Inclusion protocole therapeutique"; 
			map.put("Inclusion protocole therapeutique", incaComplementExport
									.getInclusionTherap(con, getPatient(),
											getBanque()));
			flag = "Nom protocole"; 
			map.put("Nom protocole", incaComplementExport
					.getNomProtocoleTherap(con, getPatient(), getBanque()));
			
			/*Item58 item59 Caryotype et anomalies eventuelles*/
			flag = "Caryotype"; 
			map.put("Caryotype", incaComplementExport
					.getCaryotype(con, getPatient(), getBanque()));
			flag = "Anomalie eventuelle"; 
			map.put("Anomalie eventuelle", incaComplementExport
					.getAnomalieCaryo(con,  getPatient(), getBanque()));
			
			/*Item60 item61 Anomalie genomique et description*/
			flag = "Anomalie genomique"; 
			map.put("Anomalie genomique", incaComplementExport
				.getAnomalieGenomique(con,  getPatient(), getBanque()));
			flag = "Description anomalie genomique"; 
			map.put("Description anomalie genomique", incaComplementExport
				.getAnomalieGenomiqueDescr(con,  getPatient(), getBanque()));
			
			/*Item62 Controle qualite*/
			flag = "Controle qualite"; 
			map.put("Controle qualite", 
				incaComplementExport.getControleQualite(getEchantillon()));
			
			/*Item63 item64 Inclusion dans un protocole de recherche 
			/* et nom du programme*/
			flag = "Inclusion programme recherche"; 
			map.put("Inclusion programme recherche", incaComplementExport
						.getInclusionProtocoleRech(con, getPrelevement()));
			flag = "Nom programme"; 
			map.put("Nom programme", incaComplementExport
							.getNomProtocoleRech(con, getPrelevement()));
			
			/* Item 65 champs spécifique du cancer*/
			flag = "Champ specifique"; 
			map.put("Champ specifique", incaComplementExport
				.getChampSpecCancer(con, getPrelevement()));
			...
			flag = "Contact"; 
			List<String> contact = 
							incaComplementExport.getContact(getBanque());
			...
			map.put("Questionnaire antecedents tabac", 
				incaSpecifiqueExport.getQuestAntTabac(con, getPatient(), 
															getBanque()));
			
			/* Item 70 Questionnaire familial */
			flag = "questionnaire familial"; 
			map.put("Questionnaire familial", 
				incaSpecifiqueExport.getQuestFamilial(con, getPatient(), 
															getBanque()));
			
			/* Item 71 Questionnaire professionnel */
			flag = "questionnaire professionnel"; 
			map.put("Questionnaire professionnel", 
				incaSpecifiqueExport.getQuestPro(con, getPatient(), 
															getBanque()));
			
			/* Item 72 Echantillon radio-naif */
			flag = "Echantillon radio-naif"; 
			map.put("Echantillon radio-naif", 
				incaSpecifiqueExport.getRadioNaif(con, getEchantillon()));
			
			/* Item 73 Echantillon chimio-naif */
			flag = "Echantillon chimio-naif"; 
			map.put("Echantillon chimio-naif", 
				incaSpecifiqueExport.getChimioNaif(con, getEchantillon()));
			
			/* Item 74 Statut tabac */
			flag = "Statut tabac"; 
			map.put("Statut tabac", 
				incaSpecifiqueExport.getStatutTabac(con, getPatient(), 
															getBanque()));
			
			/* Item 75 NPA */
			flag = "NPA"; 
			map.put("NPA", 
				incaSpecifiqueExport.getNPA(con, getPatient(), 
														getBanque()));
			
			/*Item 76 Cause de deces du patient */
			flag = "Cause deces"; 
			map.put("Cause deces", incaPatientExport
					.getCauseDeces(con, getPatient(), getBanque()));
			
		} catch (ItemException ie) {
			map.put("Erreur", "Erreur echantillon: " 
					+ getEchantillon().getCode() 
					+ ". Item :" + flag + ". Message: "
					+ ie.getMessage());
			log.info("Erreur echantillon: " 
					+ getEchantillon().getCode() 
					+ ". Item :" + flag + ". Message: "
					+ ie.getMessage());
			
		} catch (Exception e) {
			map.put("Erreur", "Erreur non attendue echantillon: " 
					+ getEchantillon().getCode() 
					+ ". Item :" + flag + ". Message: "
					+ e.getMessage());
			log.error("Erreur echantillon: " 
					+ getEchantillon().getCode() 
					+ ". Item :" + flag + ". Message: "
					+ e.getMessage());
		}
