...
public class BlocExterneManagerImpl implements BlocExterneManager {
	...
	private Log log = LogFactory.getLog(BlocExterneManager.class);
	...
	@Override
	public BlocExterne findByIdManager(Integer blocExterneId) {
	...
	@Override
	public List<BlocExterne> findByDossierExterneManager(
		...
		if (dossierExterne != null) {
			...
			return new ArrayList<BlocExterne>();
		}
	}
	
	@Override
	public List<BlocExterne> findByDossierExterneAndEntiteManager(
			DossierExterne dossierExterne, Entite entite) {
		log.debug("Recherche de tous les BlocExternes pour un dossier " 
				+ "et une entité");
		if (dossierExterne != null && entite != null) {
			return blocExterneDao.findByDossierExterneAndEntite(
					dossierExterne, entite.getEntiteId());
		} else {
			return new ArrayList<BlocExterne>();
		}
	}
	...
	@Override
	public Entite getEntiteManager(BlocExterne blocExterne) {
	...
	@Override
	public boolean findDoublonManager(BlocExterne blocExterne) {
		if (blocExterne != null) {
			return blocExterneDao.findByDossierExterne(
					blocExterne.getDossierExterne())
					.contains(blocExterne);
		} else {
			return false;
		}
	}
	...
	@Override
	public void validateBlocExterneManager(BlocExterne blocExterne,
			DossierExterne dossierExterne) {
		// dossierExterne required
		if (dossierExterne == null) { 
			log.warn("Objet obligatoire DossierExterne manquant"
				+ " lors de la création d'un BlocExterne");
			throw new RequiredObjectIsNullException(
					"BlocExterne", "creation", "DossierExterne");
		}
		...
		if (blocExterne.getEntiteId() == null) { 
			log.warn("Objet obligatoire Entite manquant"
				+ " lors de la création d'un BlocExterne");
			throw new RequiredObjectIsNullException(
					"BlocExterne", "creation", "Entite");
		}
		
		// il faut que le entité existe
		if (blocExterne.getEntiteId() != null) {
			if (entiteDao.findById(
					blocExterne.getEntiteId()) == null) {
				throw new EntiteObjectIdNotExistException(
						"BlocExterne", "Entite", 
						blocExterne.getEntiteId());
			}
		}
		...
		if (blocExterne.getOrdre() == null) { 
			log.warn("Objet obligatoire Ordre manquant"
				+ " lors de la création d'un BlocExterne");
			throw new RequiredObjectIsNullException(
					"BlocExterne", "creation", "Ordre");
		}
	}
	
	@Override
	public void createObjectManager(BlocExterne blocExterne,
			DossierExterne dossierExterne,
			List<ValeurExterne> valeurExternes) {
		// Validation du bloc
		validateBlocExterneManager(blocExterne, dossierExterne);
		
		blocExterne.setDossierExterne(
				dossierExterneDao.mergeObject(dossierExterne));
		
		if (findDoublonManager(blocExterne)) {
			removeObjectManager(blocExterneDao
					.findByDossierExterne(dossierExterne).get(
						blocExterneDao.findByDossierExterne(dossierExterne)
						.indexOf(blocExterne)));
		}
		blocExterneDao.createObject(blocExterne);
		log.info("Enregistrement de l'objet BlocExterne : " 
				+ blocExterne.toString());
		
		// création des valeurs
		if (valeurExternes != null) {
			for (int i = 0; i < valeurExternes.size(); i++) {
				valeurExterneManager.createObjectManager(
						valeurExternes.get(i), blocExterne);
			}
		}
	}

	@Override
	public void removeObjectManager(BlocExterne blocExterne) {
		if (blocExterne != null 
				&& blocExterne.getBlocExterneId() != null) {
			
			// suppression des valeurs
			List<ValeurExterne> valeurs = valeurExterneManager
				.findByBlocExterneManager(blocExterne);
			for (int i = 0; i < valeurs.size(); i++) {
				valeurExterneManager.removeObjectManager(valeurs.get(i));
			}
			
			blocExterneDao.removeObject(blocExterne.getBlocExterneId());
			log.info("Suppression de l'objet BlocExterne : " 
					+ blocExterne.toString());
		} else {
