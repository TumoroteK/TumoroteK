...
public class ChampImprimeManagerImpl implements ChampImprimeManager {
	...
	private Log log = LogFactory
		.getLog(ChampImprimeManager.class);
	...
	public void setChampImprimeDao(ChampImprimeDao cDao) {
	...
	@Override
	public ChampImprime findByIdManager(ChampImprimePK pk) {
	...
	@Override
	public List<ChampImprime> findByExcludedPKManager(ChampImprimePK pk) {
		if (pk != null) {
	...
	@Override
	public List<ChampImprime> findByTemplateManager(Template template) {
		log.debug("Recherche de tous les ChampImprimes d'un " 
				+  "Template.");
		if (template != null) {
			return champImprimeDao.findByTemplate(template);
		} else {
			return new ArrayList<ChampImprime>();
		}
	}
	
	@Override
	public List<ChampImprime> findByTemplateAndBlocManager(
			Template template, BlocImpression bloc) {
		log.debug("Recherche de tous les ChampImprimes d'un " 
				+  "Template.");
	...
	@Override
	public Boolean findDoublonManager(
			Template template, ChampEntite champEntite, 
			BlocImpression bloc) {
		ChampImprimePK pk = new 
			ChampImprimePK(template, champEntite, bloc);
	
		return (champImprimeDao.findById(pk) != null);
	}
	
	@Override
	public void validateObjectManager(
			Template template, ChampEntite champEntite,
			BlocImpression bloc) {
		//template required
		if (template == null) {
			log.warn("Objet obligatoire Template manquant"
				+ " lors de la validation d'un ChampImprime");
			throw new RequiredObjectIsNullException(
					"ChampImprime", "creation", "Template");
		}
		
		//champEntite required
		if (champEntite == null) {
			log.warn("Objet obligatoire ChampEntite manquant"
				+ " lors de la validation d'un ChampImprime");
			throw new RequiredObjectIsNullException(
					"ChampImprime", "creation", "ChampEntite");
		}
		
		//bloc required
		if (bloc == null) {
			log.warn("Objet obligatoire BlocImpression manquant"
				+ " lors de la validation d'un ChampImprime");
			throw new RequiredObjectIsNullException(
					"ChampImprime", "creation", "BlocImpression");
		}
		
		//Doublon
		if (template.getTemplateId() != null) {
			if (findDoublonManager(
					template, champEntite, bloc)) {
				
				log.warn("Doublon lors validation objet " 
						+ "ChampImprime");
				throw new DoublonFoundException(
						"ChampImprime", "creation");
			}
		}
	}
	
	@Override
	public void createObjectManager(ChampImprime champImprime,
			Template template, ChampEntite champEntite, 
			BlocImpression bloc) {
		...
		log.info("Enregistrement objet ChampImprime " 
				+ champImprime.toString());
	}
	
	@Override
	public void updateObjectManager(ChampImprime champImprime,
			Template template, ChampEntite champEntite, BlocImpression bloc) {
		...
		if (template == null) {
			log.warn("Objet obligatoire Template manquant"
				+ " lors de la validation d'un ChampImprime");
			throw new RequiredObjectIsNullException(
					"ChampImprime", "modification", "Template");
		} else {
			champImprime.setTemplate(templateDao.mergeObject(template));
		}
		...
		if (champEntite == null) {
			log.warn("Objet obligatoire ChampEntite manquant"
				+ " lors de la validation d'un ChampImprime");
			throw new RequiredObjectIsNullException(
					"ChampImprime", "modification", "ChampEntite");
		} else {
			champImprime.setChampEntite(
					champEntiteDao.mergeObject(champEntite));
		}
		...
		if (bloc == null) {
			log.warn("Objet obligatoire BlocImpression manquant"
				+ " lors de la validation d'un ChampImprime");
			throw new RequiredObjectIsNullException(
					"ChampImprime", "modification", "BlocImpression");
		} else {
			champImprime.setBlocImpression(blocImpressionDao.mergeObject(bloc));
		}
	...
	@Override
	public void removeObjectManager(ChampImprime champImprime) {
		if (champImprime != null) {
			champImprimeDao.removeObject(
					champImprime.getPk());
			log.info("Suppression de l'objet ChampImprime : " 
					+ champImprime.toString());				
		} else {
