...
public class INCaEchantillonExportImpl implements IncaEchantillonExport
{
   ...
   private final Log log = LogFactory.getLog(IncaEchantillonExport.class);
   ...
   public String getModeConservation(final Echantillon echantillon){
      ...
      if(echantillon.getEmplacement() != null){
         final Conteneur cont =
            getConteneurManager().findFromEmplacementManager(echantillonManager.getEmplacementManager(echantillon));
   ...
   @Override
   public String getEchantillonType(final Echantillon echantillon){
      if(echantillon.getEchantillonType().getType().matches(".*[Tt][Ii][Ss][Ss][Uu].*")){
         return "T";
      }else if(echantillon.getEchantillonType().getType().matches(".*[Cc][Ee][Ll][Ll][Uu][Ll][Ee][Ss].*")){
         return "C";
      }
      return "9";
   }
   ...
   @Override
   public String getModePreparation(final Echantillon echantillon){
      if(echantillon.getModePrepa() != null){
         if(echantillon.getModePrepa().getNom().matches(".*[Dd][Mm][Ss][Oo].*")){
            return "1";
         }else if(echantillon.getModePrepa().getNom().matches(".*[Cc][Uu][Ll][Oo][Tt].*")){
            return "2";
         }else if(echantillon.getModePrepa().getNom().matches(".*[Tt][Ii][Ss][Ss][Uu].*")){
            return "3";
         }else{
            return "9";
         }
   ...
   @Override
   public String getDelaiCongelation(final Echantillon echantillon){
   ...
   public String getControles(final Connection con, final Echantillon echantillon){
      final String controle = ExportCatalogueManagerImpl.fetchItemAsString(con,
      ...
      if(controle.matches(".*[Cc][Oo][Uu][Pp][Ee].*")){
         ...
         return "2";
      }else if(controle.matches(".*[Ee][Mm][Pp][Rr][Ee][Ii][Nn][Tt][Ee].*")){
         return "3";
   ...
   public String getPourcentageCellulesTumorales(final Connection con, final Echantillon echantillon){
      ...
      final String pourcentage = ExportCatalogueManagerImpl.fetchItemAsString(con,
         "SELECT ANNOTATION_VALEUR.ALPHANUM FROM " + "ANNOTATION_VALEUR, CHAMP_ANNOTATION, TABLE_ANNOTATION "
            + "WHERE ANNOTATION_VALEUR.OBJET_ID=" + echantillon.getEchantillonId().toString()
            + " AND CHAMP_ANNOTATION.NOM like '035%' " + "AND CHAMP_ANNOTATION.CHAMP_ANNOTATION_ID="
   ...
   @Override
   public String getProdTypeAssocie(final Connection con, final Echantillon echantillon, final String regexp){
      Statement s = null;
      ResultSet rs = null;
      ...
      try{
         ...
         rs = s.executeQuery("SELECT DISTINCT(PROD_TYPE.TYPE) " + "FROM PROD_DERIVE, PROD_TYPE, TRANSFORMATION "
            + "WHERE TRANSFORMATION.TRANSFORMATION_ID=" + "PROD_DERIVE.TRANSFORMATION_ID " + "AND TRANSFORMATION.OBJET_ID="
            + echantillon.getEchantillonId().toString() + " AND TRANSFORMATION.ENTITE_ID=3"
   ...
   public String getRessourceBiolAssociee(final Echantillon echantillon, final String resType){
      ...
      final boolean res = getEchantillonManager().itemINCa50To53Manager(echantillon, resType);
   ...
   @Override
   public String getADNconstitutionnel(final Connection con, final Echantillon echantillon, final String consentSQLRegexp){
      final String res = ExportCatalogueManagerImpl.fetchItemAsString(con,
         "SELECT ANNOTATION_VALEUR.BOOL FROM ANNOTATION_VALEUR, " + "CHAMP_ANNOTATION, TABLE_ANNOTATION, "
            + "ECHANTILLON, PRELEVEMENT, CONSENT_TYPE " + "WHERE ANNOTATION_VALEUR.OBJET_ID="
            + echantillon.getEchantillonId().toString() + " AND CHAMP_ANNOTATION.NOM like '053%' "
            + "AND CHAMP_ANNOTATION.CHAMP_ANNOTATION_ID=" + "ANNOTATION_VALEUR.CHAMP_ANNOTATION_ID"
            + " AND CHAMP_ANNOTATION.TABLE_ANNOTATION_ID=" + "TABLE_ANNOTATION.TABLE_ANNOTATION_ID "
            + "AND TABLE_ANNOTATION.ENTITE_ID=3 " + "AND ECHANTILLON.ECHANTILLON_ID=" + echantillon.getEchantillonId().toString()
            + " AND ECHANTILLON.PRELEVEMENT_ID=PRELEVEMENT.PRELEVEMENT_ID " + "AND PRELEVEMENT.CONSENT_TYPE_ID="
            + "CONSENT_TYPE.CONSENT_TYPE_ID " + "AND CONSENT_TYPE.TYPE like '" + consentSQLRegexp + "'",
         null, true, null, "");
