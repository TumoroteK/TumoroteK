...
public class DestructionMotifManagerImpl implements DestructionMotifManager {
	...
	private Log log = LogFactory.getLog(DestructionMotifManager.class);
	...
	public void setDestructionMotifValidator(
			DestructionMotifValidator dValidator) {
	 ...
	 */
	@Override
	public DestructionMotif findByIdManager(Integer destructionMotifId) {
...
//	}
	 ...
	 */
	@Override
	public List<DestructionMotif> findByMotifLikeManager(String motif,
			boolean exactMatch) {
		
		log.debug("Recherche DestructionMotif par " 
				+ motif + " exactMatch " + String.valueOf(exactMatch));
		if (motif != null) {
			if (!exactMatch) {
				motif = motif + "%";
			}
			...
			return new ArrayList<DestructionMotif>();
		}
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			DestructionMotif motif = (DestructionMotif) o;
			if (motif.getDestructionMotifId() == null) {
				return destructionMotifDao.findAll().contains(motif);
			} else {
				return destructionMotifDao.findByExcludedId(
						motif.getDestructionMotifId())
						.contains(motif);
			}
		} else {
			return false;
	...
	public boolean isUsedObjectManager(Object obj) {
		DestructionMotif motif = destructionMotifDao
			.mergeObject((DestructionMotif) obj);
	...
	public void createObjectManager(Object obj) {
		...
		if (((DestructionMotif) obj).getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"DestructionMotif", "creation", "Plateforme");
		} else {
			((DestructionMotif) obj).setPlateforme(plateformeDao
					.mergeObject(((DestructionMotif) obj).getPlateforme()));
		}
		BeanValidator.validateObject(obj, new Validator[]{
				destructionMotifValidator});
		if (!findDoublonManager(obj)) {
			destructionMotifDao.createObject(
					(DestructionMotif) obj);
			log.info("Enregistrement objet DestructionMotif " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors creation objet DestructionMotif " 
					+ obj.toString());
			throw new DoublonFoundException("DestructionMotif", 
					"creation");
		}
	}
	...
	public void updateObjectManager(Object obj) {
		...
		if (!findDoublonManager(obj)) {
			...
			log.info("Modification objet DestructionMotif " 
					+ obj.toString());
		} else {
			log.warn("Doublon lors modification objet DestructionMotif "
					+ obj.toString());
			throw new DoublonFoundException("DestructionMotif", 
					"modification");
		}
...
//			if (!isUsedObjectManager(obj)) {
				destructionMotifDao.removeObject(((DestructionMotif) obj)
						.getDestructionMotifId());
				log.info("Suppression objet DestructionMotif " 
						+ obj.toString());
//			} else {
