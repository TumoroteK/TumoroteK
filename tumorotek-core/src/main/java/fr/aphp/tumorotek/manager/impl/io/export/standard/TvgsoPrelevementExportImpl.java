...
public class TvgsoPrelevementExportImpl extends INCaPrelevementExportImpl 
											implements TvgsoPrelevementExport {
	
	private Log log = LogFactory.getLog(IncaPrelevementExport.class);
	

	@Override
	public String getCentreStockage(Connection con, Echantillon echantillon) {
		String siteStockage = 
			ExportCatalogueManagerImpl.fetchItemAsString(con, 
				"SELECT ETABLISSEMENT.NOM FROM ETABLISSEMENT, "
				+ "SERVICE, ECHANTILLON, SERVICE_COLLABORATEUR"
			+ " WHERE ECHANTILLON.ECHANTILLON_ID=" 
				+ echantillon.getEchantillonId().toString()
			+ " AND ECHANTILLON.COLLABORATEUR_ID="
				+ "SERVICE_COLLABORATEUR.COLLABORATEUR_ID"
			+ " AND SERVICE_COLLABORATEUR.SERVICE_ID=SERVICE.SERVICE_ID"
			+ " AND SERVICE.ETABLISSEMENT_ID="
		...
		if (siteStockage != null) {
			...
			if (siteStockage.matches(".*MONTPELLIER.*")) {
				...
				throw new ItemException(2, 
					"Creation du code pour le centre de stockage GSO :" 
						+ siteStockage
						+ " impossible pour echantillon " 
						+ echantillon.getCode());
			}
		}
		return null;
	...
	@Override
	public String getNumSejour(Prelevement prelevement) {
		return null;
	}
	...
	public String getClassif(Banque bank) {
		...
		Matcher m = p.matcher(bank.getNom());
		boolean b = m.matches();
		if (b) {
			return m.group(1);
		} else {
	...
	public String getCodeOrgane(Connection con, 
		...
		Pattern p = Pattern.compile("([A-X]{2}) : .+");
