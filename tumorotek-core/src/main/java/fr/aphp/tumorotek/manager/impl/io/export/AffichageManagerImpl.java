...
public class AffichageManagerImpl implements AffichageManager {
	...
	private Log log = LogFactory.getLog(AffichageManager.class);
	...
	public void setAffichageValidator(AffichageValidator validator) {
	 ...
	 */
	public void setResultatDao(ResultatDao rDao) {
	 ...
	 */
	public void setResultatManager(ResultatManager resManager) {
		this.resultatManager = resManager;
	}

	
	public void setRechercheDao(RechercheDao rDao) {
	 ...
	 */
	@Override
	public Affichage findByIdManager(Integer id) {
		//On vérifie que l'identifiant n'est pas nul
		if (id == null) {
			log.warn("Objet obligatoire identifiant manquant lors de la " 
					+ "recherche par l'identifiant d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "recherche par identifiant", "identifiant");
		}
	 ...
	 */
	@Override
	public void renameAffichageManager(Affichage affichage, String intitule) {
		//On verifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors " 
					+ "du renommage d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "modification", "Affichage");
		}
		if (findByIdManager(affichage.getAffichageId()) == null) {
			throw new SearchedObjectIdNotExistException("Affichage",
					affichage.getAffichageId());
		} else {
			...
			if (findDoublonManager(affichage)) {
				...
				BeanValidator.validateObject(affichage,
						new Validator[] { affichageValidator });
				affichageDao.updateObject(affichage);
				log.info("Modification de l'objet Affichage : "
						+ affichage.toString());
			}
		}		
	}
	 ...
	 */
	@Override
	public Affichage copyAffichageManager(Affichage affichage,
			Utilisateur copieur, Banque banque) {
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors " 
					+ "de la copie d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "copie", "Affichage");
		}
		...
		if (copieur == null) {
			log.warn("Objet obligatoire Utilisateur manquant lors " 
					+ "de la copie d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "copie", "Utilisateur");
		}
		//On vérifie que la banque n'est pas nul
		if (banque == null) {
			log.warn("Objet obligatoire Banque manquant lors " 
					+ "de la création d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "création", "Banque");
		}
		...
		if (findDoublonManager(a)) {
			...
			Iterator<Resultat> itR = resultatDao.findByAffichage(affichage)
					.iterator();
			ArrayList<Resultat> resultats = new ArrayList<Resultat>();
			while (itR.hasNext()) {
				resultats.add(resultatManager
						.copyResultatManager(itR.next(), a));
			}
			a.setResultats(resultats);
			
			createObjectManager(a, a.getResultats(), a.getCreateur(),
					a.getBanque());
			// ajout de la requete dans la liste
			affichages.add(a);
			log.info("Enregistrement de l'objet Affichage : "
					+ a.toString());
		}
	 ...
	 */
	@Override
	public void createObjectManager(Affichage affichage, 
			List<Resultat> resultats, Utilisateur createur,
			Banque banque) {
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors " 
		...
		if (createur == null) {
			log.warn("Objet obligatoire Utilisateur manquant lors " 
					+ "de la création d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "création", "Utilisateur");
		}
		...
		if (findDoublonManager(affichage)) {
			log.warn("Doublon lors de la creation de l'objet Affichage : "
					+ affichage.toString());
	 ...
	 */
	@Override
	public void updateObjectManager(Affichage affichage, 
			List<Resultat> resultats,
			List<Resultat> resultatsToRemove) {
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors " 
					+ "de la modification d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "modification", "Affichage");
		}
		if (findByIdManager(affichage.getAffichageId()) == null) {
			...
			if (findDoublonManager(affichage)) {
				...
				if (resultats != null) {
					updateResultatsManager(affichage, resultats,
							resultatsToRemove);
				}
	...
	@Override
	public Boolean isUsedObjectManager(Affichage affichage) {
	 ...
	 */
	@Override
	public void removeObjectManager(Affichage affichage) {
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			throw new RequiredObjectIsNullException("Affichage",
		...
		if (findByIdManager(affichage.getAffichageId()) == null) {
			...
			if (isUsedObjectManager(affichage)) {
				...
				Iterator<Affichage> it = affichages.iterator();
				while (it.hasNext()) {
					Affichage temp = it.next();
					if (temp.getAffichageId().equals(
							affichage.getAffichageId())) {
				...
				List<Resultat> res = resultatManager
					.findByAffichageManager(affichage);
	 ...
	 */
	@Override
	public void addResultatManager(Affichage affichage, Resultat resultat) {
		...
		if (affichage == null) {
			...
			resultatManager.createObjectManager(resultat,
					affichage, resultat.getChamp());
		}
	}
	 ...
	 */
	@Override
	public void removeResultatManager(Affichage affichage, Resultat resultat) {
		//On verifie que ni l'affichage ni le résultat ne sont nuls
		if (affichage == null) {
			throw new RequiredObjectIsNullException("Affichage",
					"modification", "Affichage");
			...
			while (it.hasNext()) {
				...
				if (modified) {
					resultatManager.updateObjectManager(temp,
							temp.getAffichage(), temp.getChamp());
				}
			}
	 ...
	 */
	@Override
	public List<Affichage> findByUtilisateurManager(Utilisateur util) {
		//On vérifie que l'utilisateur n'est pas nul
		if (util == null) {
			log.warn("Objet obligatoire Utilisateur manquant lors de la " 
					+ "recherche par l'Utilisateur d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "recherche par Utilisateur", "Utilisateur");
		}
	 ...
	 */
	@Override
	public List<Affichage> findByIntituleManager(String intitule) {
		//On vérifie que l'utilisateur n'est pas nul
		if (intitule == null) {
			log.warn("Objet obligatoire intitule manquant lors de la " 
					+ "recherche par l'intitulé d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "recherche par Intitulé", "Intitulé");
		}
		return affichageDao.findByIntitule(intitule);
	}
	
	@Override
	public List<Affichage> findByIntituleAndUtilisateurManager(String intitule,
			Utilisateur util) {
		if (intitule != null && util != null) {
			return affichageDao.findByIntituleUtilisateur(intitule, util);
		} else {
			return new ArrayList<Affichage>();
		}
	}
	 ...
	 */
	@Override
	public void moveResultatManager(Affichage affichage, Resultat resultat,
			int nouvellePosition) {
		affichage.setResultats(
				resultatManager.findByAffichageManager(affichage));
		if (affichage != null) {
	 ...
	 */
	@Override
	public Boolean findDoublonManager(Affichage affichage) {
		//On vérifie que l'affichage n'est pas nul
		if (affichage == null) {
			log.warn("Objet obligatoire Affichage manquant lors de la " 
					+ "recherche de doublon d'un objet Affichage");
			throw new RequiredObjectIsNullException(
					"Affichage", "recherche de doublon", "Affichage");
		}
		if (affichage.getAffichageId() == null) {
			return affichageDao.findAll().contains(affichage);
		} else {
			return affichageDao.findByExcludedId(
					affichage.getAffichageId()).contains(affichage);
		}
	 ...
	 */	
	public Boolean isCopyManager(Affichage a, Affichage copie) {
		if (copie == null) {
		 return false;
		} else if (!a.getCreateur().equals(copie.getCreateur())) {
			if (a.getIntitule().equals(copie .getIntitule())
					&& a.getNbLignes().equals(copie.getNbLignes())) {
				a = findByIdManager(a.getAffichageId());
				a.getResultats().size();
				Iterator<Resultat> itResultats = a.getResultats().iterator();
				while (itResultats.hasNext()) {
					boolean found = false;
					Resultat temp = itResultats.next();
					Iterator<Resultat> itTemp = copie.getResultats().iterator();
					while (itTemp.hasNext()) {
						Resultat temp2 = itTemp.next();
	 ...
	 */
	public void updateResultatsManager2(
			Affichage affichage, List<Resultat> resultats) {
		...
		Iterator<Resultat> it = affichage.getResultats().iterator();
		List<Resultat> resToRemove = new ArrayList<Resultat>();
		
		// on parcourt les resultats qui sont actuellement associés
		// à l'affichage
		while (it.hasNext()) {
			Resultat tmp = it.next();
		...
		for (int i = 0; i < resToRemove.size(); i++) {
			Resultat resultat = 
				resultatDao.mergeObject(resToRemove.get(i));
			// on retire le collab de chaque coté de l'association
			affichage.getResultats().remove(resultat);
			log.debug("Suppression de l'association entre le resultat : " 
					+ resultat.toString() + " et l'affichage : "
					+ affichage.toString());
		...
		for (int i = 0; i < resultats.size(); i++) {
			...
			if (!found) {
				...
				if (resultats.get(i).getResultatId() == null) {
					...
					affichage.getResultats().add(
							resultatDao.mergeObject(resultats.get(i)));
				} else {
					resultats.get(i).setAffichage(affichage);
					resultatDao.updateObject(resultats.get(i));
				}
				log.debug("Ajout de l'association entre l'affichage : " 
						+ affichage.toString() + " et le resultat : "
						+ resultats.get(i).toString());
			} else {
				//Si sa position ou son ordre de tri a changé, on l'update
				Resultat r = resultats.get(i);
				Resultat rdb = resultatManager.findByIdManager(r.getResultatId());
				if (r.getPosition() != rdb.getPosition()
						|| r.getOrdreTri() != rdb.getOrdreTri()) {
					resultatManager.updateObjectManager(r, r.getAffichage(), r
							.getChamp());
				}
			}
		}
	...
	public void updateResultatsManager(
			Affichage affichage, List<Resultat> resultats,
			List<Resultat> resultatsToRemove) {
		...
		if (resultatsToRemove != null) {
			for (int i = 0; i < resultatsToRemove.size(); i++) {
				resultatManager.removeObjectManager(
						resultatsToRemove.get(i));
			}
		}
		...
		if (resultats != null) {
			for (int i = 0; i < resultats.size(); i++) {
				resultats.get(i).setAffichage(affichage);
				if (resultats.get(i).getResultatId() == null) {
					resultatManager.createObjectManager(resultats.get(i), 
							affichage, resultats.get(i).getChamp());
				} else {
					resultatManager.updateObjectManager(resultats.get(i), 
							affichage, resultats.get(i).getChamp());
				}
			}
		}
	...
	@Override
	public List<Affichage> findByBanqueManager(Banque banque) {
		if (banque != null) {
	...
	@Override
	public List<Affichage> findByBanqueInLIstManager(List<Banque> banques) {
