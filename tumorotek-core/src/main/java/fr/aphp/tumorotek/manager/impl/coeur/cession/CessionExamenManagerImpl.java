...
import fr.aphp.tumorotek.manager.validation.coeur.cession
.CessionExamenValidator;
...
public class CessionExamenManagerImpl implements CessionExamenManager {
	...
	private Log log = LogFactory.getLog(CessionExamenManager.class);
	...
	public void setCessionExamenValidator(
			CessionExamenValidator cValidator) {
	 ...
	 */
	@Override
	public CessionExamen findByIdManager(Integer cessionExamenId) {
		return cessionExamenDao.findById(cessionExamenId);
	}
	
//	/**
//	 * Recherche tous les CessionExamens présents dans la base.
//	 * @return Liste de CessionExamens.
//	 */
//	@Override
//	public List<CessionExamen> findAllObjectsManager() {
...
//	}
	 ...
	 */
	@Override
	public List<CessionExamen> findByExamenLikeManager(String examen,
			boolean exactMatch) {
		
		log.debug("Recherche CessionExamen par " 
				+ examen + " exactMatch " + String.valueOf(exactMatch));
		if (examen != null) {
			if (!exactMatch) {
				examen = examen + "%";
			}
			...
			return new ArrayList<CessionExamen>();
		}
	}
	...
	public boolean findDoublonManager(Object o) {
		if (o != null) {
			CessionExamen examen = (CessionExamen) o;
			if (examen.getCessionExamenId() == null) {
				return cessionExamenDao.findAll().contains(examen);
			} else {
				return cessionExamenDao.findByExcludedId(
						examen.getCessionExamenId()).contains(examen);
			}
		} else {
			return false;
		}
	...
	public boolean isUsedObjectManager(Object obj) {
		CessionExamen examen = cessionExamenDao.mergeObject(
				(CessionExamen) obj);
	...
	public void createObjectManager(Object obj) {
		...
		// une exception
		if (((CessionExamen) obj).getPlateforme() == null) {
			throw new RequiredObjectIsNullException(
					"CessionExamen", "creation", "Plateforme");
		} else {
			((CessionExamen) obj).setPlateforme(plateformeDao
					.mergeObject(((CessionExamen) obj).getPlateforme()));
		}
		BeanValidator.validateObject(obj, new Validator[]{
				cessionExamenValidator});
		if (!findDoublonManager(obj)) {
			cessionExamenDao.createObject((CessionExamen) obj);
			log.debug("Enregistrement objet CessionExamen " + obj.toString());
		} else {
			log.warn("Doublon lors creation objet CessionExamen " 
					+ obj.toString());
	...
	public void updateObjectManager(Object obj) {
		BeanValidator.validateObject(obj, new Validator[]{
				cessionExamenValidator});
		if (!findDoublonManager(obj)) {
			cessionExamenDao.updateObject((CessionExamen) obj);
			log.debug("Modification objet CessionExamen " + obj.toString());
		} else {
			log.warn("Doublon lors modification objet CessionExamen "
					+ obj.toString());
			throw new DoublonFoundException("CessionExamen", 
					"modification");
		}
	}
...
//			if (!isUsedObjectManager(obj)) {
				cessionExamenDao.removeObject(((CessionExamen) obj)
						.getCessionExamenId());
				log.debug("Suppression objet CessionExamen " 
						+ obj.toString());
//			} else {
//				log.warn("Suppression objet CessionExamen " 
//						+ obj.toString()
...
//			}
	...
	@Override
	public List<? extends TKThesaurusObject> findByOrderManager(Plateforme pf) {
