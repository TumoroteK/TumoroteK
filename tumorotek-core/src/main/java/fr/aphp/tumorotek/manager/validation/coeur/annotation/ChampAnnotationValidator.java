...
public class ChampAnnotationValidator implements Validator
{
   ...
   public void validate(final Object obj, final Errors errs){
      ...
      final ChampAnnotation champ = (ChampAnnotation) obj;

      //nom non null
      ValidationUtils.rejectIfEmptyOrWhitespace(errs, "nom", "champAnnotation.nom.empty");
      //nom valide
      if(champ.getNom() != null){
         if(!champ.getNom().matches(ValidationUtilities.MOTREGEXP)){
      ...
      if(champ.getDataType() != null){
         if((champ.getDataType().getType().equals("boolean") || champ.getDataType().getType().equals("fichier"))
            && (champ.getCombine() != null && champ.getCombine())){
            errs.rejectValue("combine", "champAnnotation.combine.notWithbooleanOrFichier");
         }
      }
   }
}
