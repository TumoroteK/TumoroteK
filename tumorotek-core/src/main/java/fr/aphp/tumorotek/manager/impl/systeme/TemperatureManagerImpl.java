...
public class TemperatureManagerImpl implements TemperatureManager {
	...
	private Log log = LogFactory.getLog(TemperatureManager.class);
	...
	public void setTemperatureValidator(TemperatureValidator tValidator) {
		this.temperatureValidator = tValidator;
	}

	@Override
	public Temperature findByIdManager(Integer temperatureId) {
	...
	@Override
	public Boolean findDoublonManager(Temperature temperature) {
		if (temperature != null) {
			if (temperature.getTemperatureId() == null) {
				return temperatureDao.findAll().contains(temperature);
			} else {
				return temperatureDao.findByExcludedId(
	...
	@Override
	public void createObjectManager(Temperature temperature) {
		// Test s'il y a des doublons
		if (findDoublonManager(temperature)) {
			log.warn("Doublon lors de la creation de l'objet Temperature : " 
			...
			BeanValidator.validateObject(
					temperature, new Validator[]{temperatureValidator});
						
			temperatureDao.createObject(temperature);
			
			log.info("Enregistrement de l'objet Temperature : " 
					+ temperature.toString());
		}
	}

	@Override
	public void updateObjectManager(Temperature temperature) {
		// Test s'il y a des doublons
		if (findDoublonManager(temperature)) {
	...
	@Override
	public void removeObjectManager(Temperature temperature) {
		if (temperature != null) {
			temperatureDao.removeObject(temperature.getTemperatureId());
			log.info("Suppression de l'objet Temperature : " 
					+ temperature.toString());				
		} else {
