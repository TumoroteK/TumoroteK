...
public class ConteneurValidator implements Validator
{
   ...
   public void validate(final Object target, final Errors errors){
      ...
      final Conteneur conteneur = (Conteneur) target;

      // code non null
      ValidationUtils.rejectIfEmptyOrWhitespace(errors, "code", "conteneur.code.empty");
      //code valide
      if(conteneur.getCode() != null){
         if(!conteneur.getCode().matches(ValidationUtilities.MOTREGEXP)){
      ...
      // code non null
      ValidationUtils.rejectIfEmptyOrWhitespace(errors, "nom", "conteneur.nom.empty");

      //nom valide
      if(conteneur.getNom() != null){
         if(!conteneur.getNom().matches(ValidationUtilities.MOTREGEXP)){
      ...
      if(conteneur.getPiece() != null){
         if(conteneur.getPiece().matches(ValidationUtilities.ONLYSPACESREGEXP)){
            errors.rejectValue("piece", "conteneur.piece.empty");
         }
         if(!conteneur.getPiece().matches(ValidationUtilities.MOTREGEXP)){
      ...
      if(conteneur.getDescription() != null){
         if(conteneur.getDescription().matches(ValidationUtilities.ONLYSPACESREGEXP)){
            errors.rejectValue("description", "conteneur.description.empty");
         }
         if(conteneur.getDescription().length() > 250){
            errors.rejectValue("description", "conteneur.description.tooLong");
         }
      }
