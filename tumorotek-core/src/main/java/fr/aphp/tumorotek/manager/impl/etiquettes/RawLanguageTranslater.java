...
public class RawLanguageTranslater {
	...
	public static String printZPL(List<LigneEtiquette> l, 
									int x0, int y0, int dpiResolution, 
		...
		// HOME LOCATION
		buf.append("^LH" + String.valueOf(x0 * (dpiResolution/72)) 
						+ "," + String.valueOf(y0 * (dpiResolution/72)));
...
//		}
	 ...
	 * @return
	 */
	public static String printJSCRIPT(List<LigneEtiquette> l, 
			int h0, int w0, int x0, int y0,
			int dpiResolution, 
			BarcodeFieldDefault by) {
		...
		for (int i = 0; i < l.size(); i++) {
			...
			if (le.getIsBarcode()) {
				if (contenu != null && !contenu.trim().equals("")) {
					...
					double b = (by.getModuleWidth() / (double) dpiResolution) * 25.4;
