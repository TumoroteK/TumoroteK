...
/**
 *
 * Interface pour le manager du bean de domaine Echantillon. Interface créée le
 * 25/09/09.
 *
...
public interface EchantillonManager {
	...
	/**
	 * Recherche une liste d'échantillons dont la date est plus récente que celle
	 * passée en paramètre.
	 * 
	...
	/**
	 * Recherche une liste d'échantillons restants (quantite ou volume superieurs à
	 * 0) pour le prelevement passé en paramètre.
	 * 
	 * @param prelevement Prelevement pour lequel on recherche des échantillons
	 *                    restants.
	 * @return Liste d'Echantillons.
	 */
	...
	/**
	 * Recherche une liste d'échantillons pour le prelevement et le statut passés en
	 * paramètre.
	 * 
	 * @param prelevement Prelevement pour lequel on recherche des échantillons.
	...
	/**
	 * Recherche une liste d'échantillons dont le code commence comme celui passé en
	 * paramètre.
	 * 
	...
	/**
	 * Recherche une liste d'Ids d'échantillons dont le code contient celui passé en
	 * paramètre.
	 * 
	 * @param code       Code pour lequel on recherche des échantillons.
	 * @param banques    Banque auxquelles appartiennent les échantillons.
	 ...
	 * @return Liste d'Echantillons.
	 */
	List<Integer> findByCodeLikeBothSideWithBanqueReturnIdsManager(String code, List<Banque> banques,
			boolean exactMatch);
	...
	/**
	 * Recherche tous les échantillons Ids dont le code est présent dans la liste
	...
	/**
	 * Recherche tous les échantillons ids issus des patients passés en paramètres.
	 * 
	 ...
	 * 
	 * @param echantillon Echantillon pour lequel on recherche un prélèvement.
	 * @return Un Prelevement.
	 */
	Prelevement getPrelevementManager(Echantillon echantillon);

	/**
	 * Recherche une liste de produits dérivés dont l'échantillon est passé en
	 * paramètre.
	 * 
	 * @param echantillon Echantillon pour lequel on recherche des produits dérivés.
	 * @return List de ProdDerives.
	 */
	List<ProdDerive> getProdDerivesManager(Echantillon echantillon);

	/**
	 * Recherche un emplacement dont l'échantillon est passé en paramètre.
	 * 
	 * @param echantillon Echantillon pour lequel on recherche un emplacement.
	 * @return Emplacement de l'échantillon.
	 */
	Emplacement getEmplacementManager(Echantillon echantillon);

	/**
	 * Recherche d'adresse de l'emplacement dont l'échantillon est passé en
	 * paramètre.
	 * 
	 * @param echantillon Echantillon pour lequel on recherche l'adresse de son
	 *                    emplacement.
	...
	/**
	 * Recherche la liste des codes utilisés par les échantillons liés à la banque
	 * passée en paramètre.
	 * 
	...
	/**
	 * Recherche la liste des codes utilisés par les échantillons liés à la banque
	 * passée en paramètre et dont la quantité n'est pas égale à 0.
	 * 
	 * @param banque Banque pour laquelle on recherche les codes.
	 * @return Liste de codes.
	 */
	...
	/**
	 * Recherche la liste des codes utilisés par les échantillons pour associer à un
	 * produit dérivé : Echantillons liés à la banque passée en paramètre et dont la
	 * quantité n'est pas égale à 0, ou dans une cession de type traotement
	 * 
	 * @param banque Banque pour laquelle on recherche les codes.
	 * @return Liste de codes.
	 */
	...
	/**
	 * Recherche la liste des codes utilisés par les échantillons liés à la banque
	 * passée en paramètre et qui sont stockés.
	 * 
	 * @param banque Banque pour laquelle on recherche les codes.
	 * @return Liste de codes.
	 */
	...
	/**
	 * Teste les items 50 à 53 pour un échantillon. Modifiée par Mathieu le
	 * 22/09/2011 pour intégrer le fait que les ressources associées doivent être
	 * issues de prelevements à la même date de prelevement.
	 * 
	 * @param echantillon Echantillon à tester.
	 * @param liste       de banques dans lesquelles rechercher les données
	 ...
	 * @param banque                   Banque associée à l'échantillon
	 *                                 (obligatoire).
	 ...
	 * @param filesCreated             liste des fichiers créés physiquement lors de
	 *                                 la transaction (utilisés pour rollback)
	 ...
	 */
	void createObjectManager(Echantillon echantillon, Banque banque, Prelevement prelevement,
			Collaborateur collaborateur, ObjetStatut statut, Emplacement emplacement, EchantillonType type,
			List<CodeAssigne> codesAssigne, Unite quantite, EchanQualite qualite, ModePrepa preparation,
			List<AnnotationValeur> listAnnoToCreateOrUpdate, List<File> filesCreated, Utilisateur utilisateur,
			boolean doValidation, String baseDir, boolean isImport);

	/**
	 * Surcharge de la creation d'un echantillon pour intégrer à la transaction la
	 * création d'un fichier associé.
	 * 
	 * @param echantillon   Nouvelle instance de l'objet à créer.
	 ...
	 * @param filesCreated  liste des fichiers créés physiquement lors de la
	 *                      transaction (utilisés pour rollback)
	 * @param utilisateur   Utilisateur ayant créé l'échantillon.
	 * @param doValidation  True : la validation sera faite.
	 * @param base          directory pour enregistrer un fichier associé dans le
	 *                      file system
	 * @since 2.0.11
	 */
	void createObjectWithCrAnapathManager(Echantillon echantillon, Banque banque, Prelevement prelevement,
			...
			String baseDir, boolean isImport);

	/**
	 * Sauvegarde les modifications apportées à un objet persistant. Enregistre les
	 * modifications sur le compte-rendu anapath: Si le fichier est réferencé par
	 * plusieurs echantillons, la methode entraine la creation d'un nouveau fichier
	 * et d'une nouvelle reference pour l'echantillon qui demande la modification.
	 * 
	 ...
	 *                      file system
	 */
	void updateObjectManager(Echantillon echantillon, Banque banque, Prelevement prelevement,
			Collaborateur collaborateur, ObjetStatut statut, Emplacement emplacement, EchantillonType type,
			List<CodeAssigne> codesAssigne, List<CodeAssigne> codesToDelete, Unite quantite, EchanQualite qualite,
			ModePrepa preparation, List<AnnotationValeur> listAnnoToCreateOrUpdate,
	...
	/**
	 * Surcharge de la modification d'un echantillon pour intégrer à la transaction
	 * la création/modification d'un fichier associé. Si le fichier est réferencé
	 * par plusieurs echantillons, la methode entraine la creation d'un nouveau
	 * fichier et d'une nouvelle reference pour l'echantillon qui demande la
	 * modification.
	 * 
	 * @param echantillon   Objet à mettre à jour dans la base.
	 * @param banque        Banque associée à l'échantillon (obligatoire).
	...
	public void updateObjectWithCrAnapathManager(Echantillon echantillon, Banque banque, Prelevement prelevement,
			Collaborateur collaborateur, ObjetStatut statut, Emplacement emplacement, EchantillonType type,
			List<CodeAssigne> codes, List<CodeAssigne> codesToDelete, Unite quantite, EchanQualite qualite,
			ModePrepa preparation, Fichier anapath, InputStream anapathStream, List<File> filesCreated,
			List<File> filesToDelete, List<AnnotationValeur> listAnnoToCreateOrUpdate,
			List<AnnotationValeur> listAnnoToDelete, Utilisateur utilisateur, boolean doValidation,
			List<OperationType> operations, String baseDir);

	/**
	 * Supprime un objet de la base de données. La suppression engendre la creation
	 * d'un fantome de cet objet.
	 * 
	 ...
	 * @throws ObjectUsedException si echantillon reference des derives ou est
	 *                             référencé par des cessions.
	 */
	void removeObjectManager(Echantillon echantillon, String comments, Utilisateur user, List<File> filesToDelete);

	/**
	 * Cette méthode met à jour les associations entre un échantillon et une liste
	 * de CodeAssignes (organes ou morpho), et assigne le code qui sera exporté si
	 * ce code est en creation.
	 * 
	 ...
	 * @param code        en cours creation qui sera exporté (cad assignation de
	 *                    l'export lors de la creation de ce code)
	...
	/**
	 * Supprime un objet de la base de données et en cascade tous les objets dont il
	 * est le parent, deletion cascadant à leur tour sur les objets en descendant la
	 * hierarchie.
	 * 
	 * @param echantillon Echantillon à supprimer de la base de données.
	 * @param comments    commentaires liés à la suppression
	 ...
	 * @param liste       des fichiers à supprimer après transaction
	 */
	void removeObjectCascadeManager(Echantillon echantillon, String comments, Utilisateur user,
	...
	/**
	 * Change l'echantillon de collection vers celle passée en paramètre. Une erreur
	 * est lancée si l'échantillon est déplacé vers une banque appartenant à une
	 * autre plateforme.
	 * 
	 ...
	 * @param liste        des fichiers à supprimer après transaction. La
	 *                     suppression sera réalisée dans la méthode parente.
	 * @param liste        de déplacements (uniques) de fichiers à programmer
	 *                     [Correctif bug TK-155]
	 * @version 2.2.0
	 */
	void switchBanqueCascadeManager(Echantillon echan, Banque bank, boolean doValidation, Utilisateur u,
			List<File> filesToDelete, Set<MvFichier> filesToMove);

	/**
	 * Réalise la modification multiple d'une liste d'échantillons.
	 * 
	 * @param echantillons Liste des echantillons à mettre à jour, cad clone des
	 *                     échantillons ayant au moins une valeur modifiée
	 * @param Liste        échantillons à la base de la modification multiple, cette
	 *                     liste est utilisée pour la création des annotations
	 *                     Fichier en batch
	 ...
	 * @param list         Annotations associées à mettre à jour ou créer (doivent
	 *                     donc avoir objet, champ, et banque referencees)
	...
	void updateMultipleObjectsManager(List<Echantillon> echantillonsToUpdate, List<Echantillon> baseEchantillons,
			List<CodeAssigne> codeOrganes, List<CodeAssigne> codesToDelete, Fichier crAnapath,
			InputStream anapathStream, Boolean deleteAnapath, List<AnnotationValeur> listAnnoToCreateOrUpdate,
			List<AnnotationValeur> listAnnoToDelete, List<NonConformite> ncfsTrait, List<NonConformite> ncfsCess,
			Utilisateur utilisateur, String baseDir);

	/**
	 * Mets à jour le préfixe du code de plusieurs échantillons. Méthode utilisée
	 ...
	 */
	List<Echantillon> updateCodeEchantillonsManager(List<Echantillon> echantillons, String oldPrefixe,
			String newPrefixe, Utilisateur utilisateur);
	...
	/**
	 * Compte le nombre d'échantillons d'un prelevement qui ont la qté > 0.
	 * 
	 * @param prlvt
	 * @return
	 */
	...
	/**
	 * Compte le nombre d'échantillons d'un prelevement qui sont stockés ou
	 * réservés.
	 * 
	 * @param prlvt
	 * @return
	...
	/**
	 * Compte le nombre d'échantillons dont l'opérateur est le collaborateur passé
	 * en param
	 * 
	 * @param collaborateur
	 * @return compte Long
	...
	/**
	 * Surcharge du manager updateObject pour lui ajouter les non conformites.
	 * 
	 * @param echantillon
	 * @param banque
	 * @param prelevement
	...
	/**
	 * Surcharge de la methode de creation d'un échantillon intégrant la relation
	 * avec les non-conformites.
	 * 
	 * @param echantillon
	 * @param banque
	 * @param prelevement
	...
	void createObjectWithNonConformitesManager(Echantillon echantillon, Banque banque, Prelevement prelevement,
			Collaborateur collaborateur, ObjetStatut statut, Emplacement emplacement, EchantillonType type,
			List<CodeAssigne> codes, Unite quantite, EchanQualite qualite, ModePrepa preparation, Fichier anapath,
			InputStream anapathStream, List<AnnotationValeur> listAnnoToCreateOrUpdate, Utilisateur utilisateur,
			boolean doValidation, String baseDir, boolean isImport, List<NonConformite> noconfsTrait,
			List<NonConformite> noconfsCess);

	/**
	 * Préparations batch statements pour full JDBC d'un échantillon, des valeurs
	 * annotations, codes organes/diagnostics et non conformités associées. Les
	 * validations sont identiques à celles appliquées par la création en JPA.
	 ...
	 * 
	 * @param jdbcSuite                contenant les ids et statements permettant la
	 *                                 creation des objets en full JDBC
	 * @param echantillon
	 * @param banque
	 * @param prelevement
	 * @param collaborateur
	...
	/**
	 * Supprime les échantillons et en cascade les dérivés dont ils sont parents à
	 * partir des ids passés en paramètres.
	 * 
	 * @param ids
	...
	/**
	 * Renvoie tous les échantillons pour un code spécifié dans toutes les
	 * collections d'une plateforme passée en paramètre.
	 * 
	 * @param code
	 * @param pf   Plateforme
	...
	/**
	 * Calcule le délai de stockage. Actuellement défini par la différence entre la
	 * date de prélèvement et la date de stockage de l' echantillon. (anciennement
	 * délai de congélation, prenait en compte la date de congélation dans un site
	 * intermédiaire. Le délai est persisté et affiché en minutes donc ces dates
	 * doivent contenir obligatoirement des heures/minutes pour etre prise en compte
	 * dans le calcul.
	 * 
	 * @param echan
	 * @param prel
