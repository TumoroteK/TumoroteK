 ...
 *
 */
public class AffectationImprimanteManagerImpl implements
		AffectationImprimanteManager {
	
	private Log log = LogFactory.getLog(AffectationImprimanteManager.class);
	...
	public void setAffectationImprimanteDao(
			AffectationImprimanteDao aDao) {
	...
	@Override
	public AffectationImprimante findByIdManager(AffectationImprimantePK pk) {
	...
	@Override
	public List<AffectationImprimante> findByBanqueUtilisateurManager(
			Banque banque, Utilisateur utilisateur) {
		if (utilisateur != null && banque != null) {
			return affectationImprimanteDao.findByBanqueUtilisateur(
					banque, utilisateur);
		} else {
			return new ArrayList<AffectationImprimante>();
		}
	}

	@Override
	public List<AffectationImprimante> findByExcludedPKManager(
	...
	public Boolean findDoublonManager(Utilisateur utilisateur, Banque banque,
		...
		AffectationImprimantePK pk = new AffectationImprimantePK(
				utilisateur, banque, imprimante);
	...
	public void validateObjectManager(Utilisateur utilisateur, Banque banque,
		...
		if (utilisateur == null) {
			log.warn("Objet obligatoire Utilisateur manquant"
				+ " lors de la validation d'une AffectationImprimante");
			throw new RequiredObjectIsNullException(
					"AffectationImprimante", "creation", "Utilisateur");
		}
		...
		if (banque == null) {
			log.warn("Objet obligatoire Banque manquant"
				+ " lors de la validation d'une AffectationImprimante");
			throw new RequiredObjectIsNullException(
					"AffectationImprimante", "creation", "Banque");
		}
		
		//imprimante required
		if (imprimante == null) {
			log.warn("Objet obligatoire Imprimante manquant"
				+ " lors de la validation d'une AffectationImprimante");
			throw new RequiredObjectIsNullException(
					"AffectationImprimante", "creation", "Imprimante");
		}
	}
	
	@Override
	public void createObjectManager(
			AffectationImprimante affectationImprimante,
			Utilisateur utilisateur, Banque banque, Imprimante imprimante,
			Modele modele) {
		// validation de l'objet à créer
		validateObjectManager(utilisateur, banque, imprimante);
		
		affectationImprimante.setUtilisateur(
				utilisateurDao.mergeObject(utilisateur));
		affectationImprimante.setBanque(banqueDao.mergeObject(banque));
		affectationImprimante.setImprimante(
				imprimanteDao.mergeObject(imprimante));
		if (modele != null) {
		...
		if (!findDoublonManager(utilisateur, banque, imprimante)) {
			...
			log.info("Enregistrement objet AffectationImprimante " 
					+ affectationImprimante.toString());
		} else {
			// update
	...
	public void removeObjectManager(
		...
		if (affectationImprimante != null) {
			affectationImprimanteDao.removeObject(
					affectationImprimante.getPk());
			log.info("Suppression de l'objet AffectationImprimante : " 
					+ affectationImprimante.toString());				
		} else {
