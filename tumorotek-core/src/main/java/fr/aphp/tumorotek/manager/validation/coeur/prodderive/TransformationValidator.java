...
/**
 * Validator pour le bean domaine Transformation (de prodderive).
 * Classe creee le 09/10/09
 *
 * Regles de validation:
 * 	- le champ quantite doit etre supérieur à 0
 *  - le champ volume doit etre supérieur à 0
 *
...
public class TransformationValidator implements Validator
{
   ...
   public void validate(final Object obj, final Errors errs){
      ...
      if(t.getQuantite() != null){
         if(t.getQuantite() < 0){
            errs.rejectValue("quantite", "transformation.quantite.negative");
         }
      }
   }
}
