...
public class GroupementManagerImpl implements GroupementManager {
	 ...
	 *            est le bean Dao.
	 */
	public void setGroupementDao(GroupementDao grDao) {
	 ...
	 *            est le bean Dao.
	 */
	public void setCritereDao(CritereDao cDao) {
	 ...
	 *            est le bean Manager.
	 */
	public void setCritereManager(CritereManager critManager) {
	 ...
	 */
	public void setGroupementValidator(GroupementValidator validator) {
	 ...
	 */
	@Override
	public Groupement copyGroupementManager(Groupement groupement) {
	 ...
	 */
	@Override
	public void createObjectManager(Groupement groupement, Critere critere1,
			Critere critere2, String operateur, Groupement parent) {
		// On vérifie que le groupement n'est pas nul
		if (groupement == null) {
			log.warn("Objet obligatoire Groupement manquant lors "
					+ "de la création d'un objet Groupement");
			throw new RequiredObjectIsNullException("Groupement", "création",
					"Groupement");
		}
		...
		if (parent != null) {
			if (parent.getGroupementId() != null) {
				...
				createObjectManager(parent, parent.getCritere1(), parent
						.getCritere2(), parent.getOperateur(), parent
						.getParent());
			}
		}
		...
		if (critere1 != null) {
			if (critere1.getCritereId() != null) {
				...
				critereManager.createObjectManager(critere1, critere1
						.getChamp(), critere1.getCombinaison());
			}
		}
		...
		if (critere2 != null) {
			if (critere2.getCritereId() != null) {
				...
				critereManager.createObjectManager(critere2, critere2
						.getChamp(), critere2.getCombinaison());
			}
		}
		groupement.setCritere2(critere2);
		groupement.setOperateur(operateur);
		BeanValidator.validateObject(groupement,
				new Validator[] { groupementValidator });
	 ...
	 */
	@Override
	public void updateObjectManager(Groupement groupement, Critere critere1,
			Critere critere2, String operateur, Groupement parent) {
		//On vérifie que le groupement n'est pas nul
		if (groupement == null) {
			log.warn("Objet obligatoire Groupement manquant lors " 
					+ "de la modification d'un objet Groupement");
			throw new RequiredObjectIsNullException(
					"Groupement", "modification", "Groupement");
		}
		...
		if (parent != null) {
			if (parent.getGroupementId() != null) {
				...
				updateObjectManager(parent, parent.getCritere1(), parent
						.getCritere2(), parent.getOperateur(), parent
						.getParent());
			}
		}
		...
		if (critere1 != null) {
			if (groupement.getCritere1() != null
					&& groupement.getCritere1().getCritereId() != null
					&& !groupement.getCritere1().getCritereId().equals(
							critere1.getCritereId())) {
				critereDao
						.removeObject(groupement.getCritere1().getCritereId());
			}
			if (critere1.getCritereId() != null) {
				critere1 = critereDao.mergeObject(critere1);
			} else {
		...
		if (critere2 != null) {
			if (groupement.getCritere2() != null
					&& groupement.getCritere2().getCritereId() != null
					&& !groupement.getCritere2().getCritereId().equals(
							critere2.getCritereId())) {
				critereDao
						.removeObject(groupement.getCritere2().getCritereId());
			}
			if (critere2.getCritereId() != null) {
				critere2 = critereDao.mergeObject(critere2);
			} else {
	 ...
	 */
	@Override
	public void removeObjectManager(Groupement groupement) {
		// On vérifie que le groupement n'est pas nul
		if (groupement == null) {
			log.warn("Objet obligatoire Groupement manquant lors "
					+ "de la suppression d'un objet Groupement");
			throw new RequiredObjectIsNullException("Groupement",
					"suppression", "Groupement");
		}
		// On vérifie que le groupement est en BDD
		if (findByIdManager(groupement.getGroupementId()) == null) {
			throw new SearchedObjectIdNotExistException("Groupement",
					groupement.getGroupementId());
		} else {
			// On supprime d'abord ses enfants
			List<Groupement> enfants = groupementDao.findEnfants(groupement);
			Iterator<Groupement> it = enfants.iterator();
			while (it.hasNext()) {
				Groupement temp = it.next();
				removeObjectManager(temp);
			}

			// On supprime les criteres
			Critere oldCritere1 = groupement.getCritere1();
			Critere oldCritere2 = groupement.getCritere2();
	 ...
	 */
	@Override
	public ArrayList<Groupement> findEnfantsManager(Groupement groupement) {
		// On vérifie que le groupement n'est pas nul
		if (groupement == null) {
			log.warn("Objet obligatoire Groupement manquant lors "
					+ "de la recherche d'enfants d'un objet Groupement");
			throw new RequiredObjectIsNullException("Groupement",
					"recherche d'enfants", "Groupement");
		}
	 ...
	 */
	@Override
	public List<Critere> findCriteresManager(Groupement groupementRacine) {
		// On vérifie que le groupementRacine n'est pas nul
		if (groupementRacine == null) {
			log.warn("Objet obligatoire Groupement manquant lors de la "
					+ "recherche par de Criteres d'un objet Groupement");
			throw new RequiredObjectIsNullException("Groupement",
					"recherche de Criteres", "Groupement");
		}
		...
		Iterator<Groupement> it = findEnfantsManager(groupementRacine)
				.iterator();
		while (it.hasNext()) {
			Groupement temp = it.next();
	 ...
	 */
	public Boolean isCopyManager(Groupement g, Groupement copie) {
		if (copie == null) {
			return false;
		} else {
			boolean ok = false;
			...
			Iterator<Groupement> it = findEnfantsManager(g).iterator();
			while (it.hasNext()) {
				boolean found = false;
				Groupement temp = it.next();
				Iterator<Groupement> it2 = findEnfantsManager(g).iterator();
				while (it2.hasNext()) {
					Groupement copyTemp = it2.next();
	 ...
	 * @return le Groupement copié.
	 */
	private Groupement copyGroupementWithParentManager(Groupement groupement,
			Groupement parent) {
		Groupement temp = null;
		// On vérifie que le groupement n'est pas nul
		if (groupement == null) {
			log.warn("Objet obligatoire Groupement manquant lors "
					+ "de la copie d'un objet Groupement");
			throw new RequiredObjectIsNullException("Groupement", "copie",
					"Groupement");
		} else {
			BeanValidator.validateObject(groupement,
					new Validator[] { groupementValidator });
			...
			if (groupement.getCritere1() != null) {
				critere1 = critereManager.copyCritereManager(groupement
						.getCritere1());
			}
			Critere critere2 = null;
			if (groupement.getCritere2() != null) {
				critere2 = critereManager.copyCritereManager(groupement
						.getCritere2());
			}
			temp = new Groupement(critere1, critere2,
					groupement.getOperateur(), parent);
			BeanValidator.validateObject(temp,
					new Validator[] { groupementValidator });
			groupementDao.createObject(temp);

			// On copie ses enfants
			Iterator<Groupement> it = findEnfantsManager(temp).iterator();
			while (it.hasNext()) {
				Groupement enfTemp = it.next();
