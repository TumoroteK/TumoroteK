...
public class ExtractValueFromChampManagerImpl 
	implements ExtractValueFromChampManager {
		...
		if (echantillon != null && champ != null) {
			...
			if (champ.getChampEntite() != null) {
				if (champ.getChampEntite().getEntite()
					...
					value = (String) 
						champManager.getValueForObjectManager(
							champ, patient, true);
					...
					if (champ.getChampEntite()
						...
						Iterator<Risque> risksIt = prelevementManager
							.getRisquesManager(prlvt).iterator();
		...
		if (prodDerive != null && champ != null) {
			...
			if (champ.getChampEntite() != null) {
				if (champ.getChampEntite().getEntite()
						.getNom().equals("Patient")) {
					// on récupère le patient
					Patient patient = prodDeriveManager
						.getPatientParentManager(prodDerive);
					value = (String) 
						champManager.getValueForObjectManager(
							champ, patient, true);
				} else if (champ.getChampEntite().getEntite()
						.getNom().equals("Maladie")) {
					// on récupère la maladie
					Prelevement prlvt = prodDeriveManager
					...
					Object parent = prodDeriveManager.findParent(prodDerive);
					if (parent.getClass()
							.getSimpleName().equals("Echantillon")) {
						value = extractValueForChampManager(
					...
					if (champ.getChampEntite()
							.getNom().equals("EmplacementId")) {
						value = prodDeriveManager.getEmplacementAdrlManager(
								prodDerive);
					} else {
						value = (String) 
							champManager.getValueForObjectManager(
							champ, prodDerive, true);
					}
				}
			}
			return value;
