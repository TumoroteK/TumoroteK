...
public class TraitementRequeteManagerImpl implements TraitementRequeteManager {
	 ...
	 */
	public void setCritereManager(CritereManager crManager) {
	}
	 ...
	 */
	public void setCorrespondanceManager(CorrespondanceManager cManager) {
	 ...
	 */
	public void setTraitementQueryManager(TraitementQueryManager tqManager) {
		this.traitementQueryManager = tqManager;
	}
	
	public void executeManager(Requete requete, Affichage affichage) {
	 ...
	 * de la requête)
	 */
	public List<Object> traitementRequeteManager(Requete requete,
			List<Banque> banques, Hashtable<Integer, Object> criteresValues,
			String jdbcDialect) {
		
		/** On récupère le Groupement racine. */
		Groupement racine = requete.getGroupementRacine();
		/** On parcourt le Groupement racine en profondeur d'abord pour filtrer
		 * les objets récupérés.
		 */
		List<Object> listeObjets = traitementArbreGroupementManager(
				racine, banques, criteresValues, jdbcDialect);
		return listeObjets;
	}
	 ...
	 */
	public List<Object> traitementArbreGroupementManager(Groupement noeud,
			List<Banque> banques, Hashtable<Integer, Object> criteresValues,
			String jdbcDialect) {
		List<Object> listeResultante = new ArrayList<Object>();
		/** On vérifie que l'opérateur est correct. */

		List<Object> liste1 = new ArrayList<Object>();
		List<Object> liste2 = new ArrayList<Object>();
		/** On traite les Criteres. */
		// On traite les enfants d'abord.
		if (noeud.getCritere1() == null || noeud.getCritere2() == null) {
			List<Groupement> enfants = groupementManager
					.findEnfantsManager(noeud);
			if (enfants == null || enfants.size() == 0) {
				...
				if (enfants.get(0) != null) {
					List<Object> listeObjets
							= traitementArbreGroupementManager(
									enfants.get(0), banques, criteresValues,
									jdbcDialect);
					...
					throw new RequiredObjectIsNullException("Groupement",
							"traitement", "enfant recquis");
				}
				if (enfants.size() > 1 && enfants.get(1) != null) {
					List<Object> listeObjets2
							= traitementArbreGroupementManager(
									enfants.get(1), banques, criteresValues,
									jdbcDialect);
		...
		if (noeud.getCritere1() != null) {
			Object value = null;
			if (criteresValues.containsKey(noeud.getCritere1()
					.getCritereId())) {
				value = criteresValues.get(noeud.getCritere1()
						.getCritereId());
			}
			if (liste1.size() == 0) {
				liste1 = traitementCritereManager(
						noeud.getCritere1(), banques, value,
						jdbcDialect);
			} else if (liste2.size() == 0) {
				liste2 = traitementCritereManager(
						noeud.getCritere1(), banques, value,
						jdbcDialect);
			}
		}
		...
		if (noeud.getCritere2() != null) {
			Object value = null;
			if (criteresValues.containsKey(noeud.getCritere2()
					.getCritereId())) {
				value = criteresValues.get(noeud.getCritere2()
						.getCritereId());
			}
			if (liste1.size() == 0) {
				liste1 = traitementCritereManager(
						noeud.getCritere2(), banques, value,
						jdbcDialect);
			} else if (liste2.size() == 0) {
				liste2 = traitementCritereManager(
						noeud.getCritere2(), banques, value,
	 ...
	 */
	public List<Object> traitementCritereManager(Critere critere,
			List<Banque> banques, Object value,
			String jdbcDialect) {
		/**
		...
		listeResultante = traitementQueryManager
				.findObjetByCritereManager(critere, banques, value,
						jdbcDialect);
		return listeResultante;
	}
	 ...
	 */
	public List<Object> additionnerListesManager(List<Object> liste1,
			List<Object> liste2) {
		/**
		...
		if (liste1 != null) {
			if (liste1.size() > 0) {
				...
				if (liste1.get(0).getClass().getSimpleName()
						.equals("Patient")) {
					if (liste2 != null && liste2.size() > 0) {
						if (liste2.get(0).getClass().getSimpleName().equals(
								"Patient")) {
							entiteBasse = "Patient";
							listeResultante = new ArrayList<Object>();
							/*
							 ...
							 */
							Iterator<Object> it = liste2.iterator();
							while (it.hasNext()) {
						...
						} else if (liste2.get(0).getClass().getSimpleName()
								.equals("Maladie")
								|| liste2.get(0).getClass().getSimpleName()
										.equals("Prelevement")
								|| liste2.get(0).getClass().getSimpleName()
										.equals("Echantillon")
								|| liste2.get(0).getClass().getSimpleName()
								...
								|| liste2.get(0).getClass().getSimpleName()
										.equals("Cession")) {
							entiteBasse = liste2.get(0).getClass()
									.getSimpleName();
							listeResultante = new ArrayList<Object>();
							/* On récupère les patients des objets. */
							listeResultante.addAll(correspondanceManager
									.recupereEntitesViaDAutres(liste2,
											"Patient"));
							/*
							 * On ajoute tous les patients de la liste 1 dans la
							 * liste résultante s'ils n'y sont pas déjà.
							 */
							Iterator<Object> it = liste1.iterator();
							while (it.hasNext()) {
								Patient patient = (Patient) it.next();
								if (!listeResultante.contains(patient)) {
									listeResultante.add(patient);
					...
					} else if (liste1.get(0).getClass().getSimpleName().equals(
							"Maladie")) {
						if (liste2 != null && liste2.size() > 0) {
							if (liste2.get(0).getClass().getSimpleName()
									.equals("Patient")) {
								/* On récupère les patients des maladies. */
								entiteBasse = "Maladie";
								listeResultante = new ArrayList<Object>();
								listeResultante.addAll(correspondanceManager
										.recupereEntitesViaDAutres(liste1,
												"Patient"));
								/*
						...
						if (liste2 != null && liste2.size() > 0) {
							if (liste2.get(0).getClass().getSimpleName()
								...
								Iterator<Object> it = liste1.iterator();
								while (it.hasNext()) {
									ProdDerive prodDerive = (ProdDerive) it
											.next();
									if (!listeResultante.contains(
											prodDerive)) {
					...
					if (liste2 != null && liste2.size() > 0) {
						if (liste2.get(0).getClass().getSimpleName().equals(
							...
							Iterator<Object> it = liste2.iterator();
							while (it.hasNext()) {
								Cession cession = (Cession) it.next();
				...
				if (listeResultante != null && listeResultante.size() > 0
						&& !listeResultante.get(0).getClass()
								.getSimpleName().equals(entiteBasse)) {
					listeResultante = correspondanceManager
							.recupereEntitesViaDAutres(listeResultante,
									entiteBasse);
				}
			}
		} else {
	...
	public List<Object> fusionnerListesManager(List<Object> liste1,
			List<Object> liste2) {
		/**
		 * On doit retourner la liste dont le type d'objet (Entite) est la plus
		 * haute: Pour cela on compare les deux types d'objet, on filtre au plus
				...
				if (liste1.get(0).getClass().getSimpleName()
					...
					if (liste2 != null && liste2.size() > 0) {
						if (liste2.get(0).getClass().getSimpleName().equals(
							...
							listeResultante = ListUtils.intersection(
									liste1, liste2);
						} else if (liste2.get(0).getClass().getSimpleName()
								.equals("Maladie")
								|| liste2.get(0).getClass().getSimpleName()
							...
							/* On récupère les patients des objets. */
							liste3.addAll(correspondanceManager
									.recupereEntitesViaDAutres(liste2,
											"Patient"));
							/*
...
//							}
				...
				/* On récupère les entités au plus bas. */
				if (listeResultante != null
						&& listeResultante.size() > 0
						&& !listeResultante.get(0).getClass()
								.getSimpleName().equals(entiteBasse)
					...
					if (liste1 != null && liste1.size() > 0
							&& liste1.get(0).getClass().getSimpleName()
							.equals(entiteBasse)) {
						listeResultante = fusionnerListesManager(liste1, listeResultante);
					} else if (liste2 != null && liste2.size() > 0
							&& liste2.get(0).getClass().getSimpleName()
								.equals(entiteBasse)) {
