...
/**
 * Validator pour le bean domaine ModePrepa (de echantillon).
 ...
 * @version 2.0
 */
public class ModePrepaValidator implements Validator
{

   @Override
   public boolean supports(final Class<?> clazz){
      return ModePrepa.class.equals(clazz);
   }

   @Override
   public void validate(final Object obj, final Errors errs){
      //Path non vide
      ValidationUtils.rejectIfEmptyOrWhitespace(errs, "nom", "modePrepa.nom.empty");

      final ModePrepa modePrepa = (ModePrepa) obj;
      //nom valide
      if(modePrepa.getNom() != null){
         if(!modePrepa.getNom().matches(ValidationUtilities.MOTREGEXP)){
            errs.rejectValue("nom", "modePrepa.nom.illegal");
         }
         if(modePrepa.getNom().length() > 200){
            errs.rejectValue("nom", "modePrepa.nom.tooLong");
         }
      }
      ...
      if(modePrepa.getNomEn() != null){
         if(modePrepa.getNomEn().matches(ValidationUtilities.ONLYSPACESREGEXP)){
            errs.rejectValue("nomEn", "modePrepa.nomEn.empty");
         }
         if(!modePrepa.getNomEn().matches(ValidationUtilities.MOTREGEXP)){
            errs.rejectValue("nomEn", "modePrepa.nomEn.illegal");
         }
         if(modePrepa.getNomEn().length() > 25){
            errs.rejectValue("nomEn", "modePrepa.nomEn.tooLong");
         }
      }
   }
}
