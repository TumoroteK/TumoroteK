...
public class TransporteurManagerImpl implements TransporteurManager {
	...
	private Log log = LogFactory.getLog(TransporteurManager.class);
	...
	public void setTransporteurValidator(TransporteurValidator tValidator) {
	 ...
	 */
	public Transporteur findByIdManager(Integer transporteurId) {
	...
	@Override
	public Boolean findDoublonManager(Transporteur transporteur) {
		if (transporteur != null) {
			if (transporteur.getTransporteurId() == null) {
				return transporteurDao.findAll().contains(transporteur);
			} else {
				return transporteurDao.findByExcludedId(
						transporteur.getTransporteurId())
	...
	@Override
	public void createObjectManager(Transporteur transporteur,
			Coordonnee coordonnee, Utilisateur utilisateur) {
		if (findDoublonManager(transporteur)) {
			log.warn("Doublon lors de la creation de l'objet Transporteur : " 
					+ transporteur.toString());
			throw new DoublonFoundException("Transporteur", "creation");
		} else {
			
			BeanValidator.validateObject(
					transporteur, new Validator[]{transporteurValidator});
			...
			log.info("Enregistrement de l'objet Transporteur : " 
					+ transporteur.toString());	
	...
	@Override
	public void updateObjectManager(Transporteur transporteur,
			Coordonnee coordonnee, Utilisateur utilisateur) {
		if (findDoublonManager(transporteur)) {
			log.warn("Doublon lors de la modif de l'objet Transporteur : " 
					+ transporteur.toString());
	...
	@Override
	public void removeObjectManager(Transporteur transporteur, 
										String comments, Utilisateur user) {
		if (transporteur != null) {
			if (!isReferencedObjectManager(transporteur)) {	
				//Supprime operations associes
				CreateOrUpdateUtilities
					.removeAssociateOperations(transporteur, operationManager, 
																comments, user);
				
				transporteurDao.removeObject(transporteur.getTransporteurId());
				log.info("Suppression de l'objet Transporteur : " 
													+ transporteur.toString());	
			} else {
				log.warn("Objet référencé lors de la suppression " 
						+ "de l'objet Transporteur : " 
						+ transporteur.toString());
				throw new ObjectReferencedException("transporteur"
								+ ".deletion.isReferenced", false);
			}
		}
	}

	@Override
	public boolean isReferencedObjectManager(Transporteur tr) {
		Transporteur transp = transporteurDao.mergeObject(tr);
		return !transp.getPrelevements().isEmpty()
			|| !transp.getLaboInters().isEmpty()
			|| !transp.getCessions().isEmpty();
	}
}
