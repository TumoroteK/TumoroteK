...
public class ComponentPrinter {
	 ...
	 * @return
	 */
	public static int printComponents(
			List componentsToBePrinted, int nb, 
		...
		if (componentsToBePrinted != null && componentsToBePrinted.size() > 0) {
			for (int i = 0; i < componentsToBePrinted.size(); i++) {
				Component aComponent = (Component) componentsToBePrinted.get(i);
				ComponentPrintable aPrintableComponent = 
					new ComponentPrintable(aComponent);
			...
			for (int i = 0; i < printServices.length; i++) {
				if (printServices[i].getName().equals(
						imprimante.getNom())) {
					try {
	 ...
	 * @return
	 */
	public static int print(List liste, int nb,
			Imprimante imprimanteBean, String texte) {
		List components = getComponents(liste, imprimanteBean, texte);
		int codeRetour = printComponents(components, nb, imprimanteBean);
		return codeRetour;
	}
	 ...
	 * @return
	 */
	public static List getComponents(List liste, Imprimante imprimanteBean, 
			String texte) {
		List<ComponentBarcodeLabel> components = 
			new ArrayList<ComponentBarcodeLabel>();

		for (int i = 0; i < liste.size(); i++) {
			Vector aData = (Vector) liste.get(i);
			ComponentBarcodeLabel aBarcodeLabel = new 
			ComponentBarcodeLabel(aData, 
					texte,
					imprimanteBean.getAbscisse(),
					imprimanteBean.getOrdonnee());
