...
public class ExcelUtility
{
   ...
   //le try catch semble inutile à ce niveau
   //on impose que normalText soit en première position .... si on veut l'inverse, il faut une autre méthode
   //dans l'absolu
   /**
   ...
   public static void writeToCellWithHalfItalic(Cell cell, String normalText, String italicText, short fontHeight) {
      ...
      //CHT try catch inutile car aucune méthode ne lance une Execption qui n'est pas une RuntimeException
...
*/   }
   ...
   public static void applyCustomBordersAndCenterContent(Cell cell, List<BorderProperties> borders, boolean isToCenter) {
      ...
      //CHT : cette création d'un nouveau style et son application à cell écrase le style précédemment défini
...
//      return new XSSFColor();
   ...
   //Par contre, il faut gérer le cas d'un doublon notamment si tronqué.
   ...
   public static Sheet createSheet(Workbook workbook, String sheetName){
      ...
      try {
         if(sheetName == null) {
            sheetName = new StringBuilder("Feuille n°").append(workbook.getNumberOfSheets()).toString();
         }
         return workbook.createSheet(WorkbookUtil.createSafeSheetName(sheetName));
      }
      catch(IllegalArgumentException ie) {
         return workbook.createSheet(new StringBuilder("(").append(workbook.getNumberOfSheets()).append(")").toString());
      }
   }
   ...
   //ou alors passer le CellStyle en param et le compléter à chaque fois... 
