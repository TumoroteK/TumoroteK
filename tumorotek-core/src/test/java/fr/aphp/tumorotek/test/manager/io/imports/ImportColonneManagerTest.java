...
public class ImportColonneManagerTest extends AbstractManagerTest4
{
	...
	private void saveManager(){
		...
		// on test avec l'importtemplate null
		try{
			importColonneManager.saveManager(ic1, null, c2);
		}catch(final Exception e){
			if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
				catched = true;
			}
		...
		importColonneManager.saveManager(ic2, it, c2);
	...
	private void saveManager(){
		...
		importColonneManager.saveManager(ic1, it, c1);
		assertTrue(importColonneManager.findAllObjectsManager().size() == tots + 1);
		assertTrue(champManager.findAllObjectsManager().size() == cTots + 1);
		...
		// on test avec l'importtemplate null
		try{
			importColonneManager.saveManager(icUp, null, c1);
		}catch(final Exception e){
			if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
				catched = true;
			}
		...
		importColonneManager.saveManager(icUp, it, c2);
		assertTrue(importColonneManager.findAllObjectsManager().size() == tots + 1);
