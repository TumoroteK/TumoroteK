...
public class TransporteurManagerTest extends AbstractManagerTest4
{
   ...
   private void saveManagerTest(){
      ...
      // est lancée
      try{
         transporteurManager.saveManager(t1, coordExcp, u);
      ...
      catched = false;
      try{
         transporteurManager.saveManager(t1, newCoord1, u);
      }catch(final Exception e){
      ...
      transporteurManager.saveManager(t1, null, u);
      ...
      transporteurManager.saveManager(t2, newCoord1, u);
   ...
   private void saveManagerTest(){
      ...
      t.setContactNom("NOM");
      final Utilisateur u = utilisateurDao.findById(1);
      transporteurManager.saveManager(t, null, u);
      assertTrue(coordonneeManager.findAllObjectsManager().size() == 5);
      assertTrue(transporteurManager.findAllObjectsManager().size() == 4);
      ...
      // est lancée
      try{
         transporteurManager.saveManager(tUp1, null, u);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("DoublonFoundException")){
            catched = true;
         }
      ...
      catched = false;
      try{
         transporteurManager.saveManager(tUp1, newCoord1, u);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("ValidationException")){
            catched = true;
         }
      ...
      newCoord1.setAdresse("TMP");
      transporteurManager.saveManager(tUp2, newCoord1, u);
      final int id = newCoord1.getCoordonneeId();
      assertTrue(coordonneeManager.findAllObjectsManager().size() == 6);
      assertTrue(transporteurManager.findAllObjectsManager().size() == 4);
      ...
      transporteurManager.saveManager(testInsert, testInsert.getCoordonnee(), u);
      assertTrue(coordonneeManager.findAllObjectsManager().size() == 6);
      assertTrue(transporteurManager.findAllObjectsManager().size() == 4);
            ...
            for(int k = 0; k < contactPrenomValues.length; k++){
               ...
               try{
                  ...
                  if(i != 5 || j != 5 || k != 4){ //car creation valide
                     transporteurManager.saveManager(transporteur, null, u);
                  }
               }catch(final Exception e){
                  if(e.getClass().getSimpleName().equals("ValidationException")){
                     catchedInsert = true;
