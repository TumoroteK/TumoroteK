...
public class MaladieManagerTest extends AbstractManagerTest4
{
	...
	public void saveManagerTest() throws ParseException{
		...
		//insertion
		maladieManager.createOrsaveManager(m, p, medecins, u, "creation");
		...
		try{
			maladieManager.createOrsaveManager(m2, p, null, u, "creation");
				...
				for(int k = 0; k < codes.length; k++){
					...
					try{
						if(!(i == 1 && j == 5 && k > 3)){ //car creation valide
							maladieManager.createOrsaveManager(m2, patients[i], null, u, "creation");
						}
					}catch(final Exception e){
						if(i == 0){
		...
		for(int x = 0; x < dates.length; x++){
			for(int y = 0; y < dates.length; y++){
				...
				try{
					maladieManager.createOrsaveManager(m2, null, null, null, "creation");
				}catch(final Exception e){
					assertTrue(e.getClass().getSimpleName().equals("ValidationException"));
				}
			}
		...
		try{
			maladieManager.createOrsaveManager(m2, patients[1], null, u, null);
		}catch(final NullPointerException ne){
			assertTrue(true);
		}
		try{
			maladieManager.createOrsaveManager(m2, patients[1], null, u, "dummy");
	...
	public void saveManagerTest() throws ParseException{
		...
		maladieManager.createOrsaveManager(m, null, medecins, u, "modification");
		...
		maladieManager.createOrsaveManager(m, null, null, u, "modification");
		assertTrue(maladieManager.getCollaborateursManager(m).size() == 2);
		maladieManager.createOrsaveManager(m, null, new ArrayList<Collaborateur>(), u, "modification");
		...
		try{
			...
			maladieManager.createOrsaveManager(m, p, null, u, "modification");
	...
	public void testCreateMaladieAndPatient() throws ParseException{
		...
		maladieManager.createOrsaveManager(m, p, null, u, "creation");
