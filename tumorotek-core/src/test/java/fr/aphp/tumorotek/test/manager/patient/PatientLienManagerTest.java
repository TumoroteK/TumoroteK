...
public class PatientLienManagerTest extends AbstractManagerTest4
{
	...
	private void saveManagerTest() throws ParseException{
		...
		//insertion
		patientLienManager.createOrsaveManager(pl, p1, p2, l, "creation");
		...
		try{
			patientLienManager.createOrsaveManager(pl2, p2, p1, l, "creation");
		...
		try{
			patientLienManager.createOrsaveManager(pl2, null, null, null, null);
		}catch(final NullPointerException ne){
			assertTrue(true);
		}
		try{
			patientLienManager.createOrsaveManager(pl2, null, null, l, "dummy");
	...
	private void validationTest(final PatientLien pl, final Patient p1, final Patient p2, final LienFamilial lien,
		...
		try{
			if(!isValide){ //car creation valide
				patientLienManager.createOrsaveManager(pl, p1, p2, lien, "creation");
			}
		}catch(final Exception e){
	...
	private void saveManagerTest() throws ParseException{
		...
		patientLienManager.createOrsaveManager(pl, null, null, null, "modification");
		...
		//		patientLienManager.createOrsaveManager(pl, null, p3, 
		//														null, "modification");
		//		pk.setPatient2(p3);
		...
		Boolean catched = false;
		try{
			patientLienManager.createOrsaveManager(pl, patientDao.findById(2), patientDao.findById(5),
	...
	private void deleteByIdManagerTest(){
		...
		patientLienManager.deleteByIdManager(pl1);
		assertNull(patientLienDao.findById(pk));

		patientLienManager.deleteByIdManager(null);
