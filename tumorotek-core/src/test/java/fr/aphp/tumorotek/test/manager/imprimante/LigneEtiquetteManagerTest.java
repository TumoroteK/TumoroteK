...
public class LigneEtiquetteManagerTest extends AbstractManagerTest4
{
   ...
   public void testFindById(){
      final LigneEtiquette le = ligneEtiquetteManager.findByIdManager(1);
      assertNotNull(le);

      final LigneEtiquette leNull = ligneEtiquetteManager.findByIdManager(100);
      assertNull(leNull);
   }
   ...
   public void testFindAll(){
      final List<LigneEtiquette> list = ligneEtiquetteManager.findAllObjectsManager();
   ...
   public void testFindByModeleManager(){
      ...
      List<LigneEtiquette> list = ligneEtiquetteManager.findByModeleManager(m2);
      assertTrue(list.size() == 7);
   ...
   public void testValidateObjectManager(){
      final Modele m1 = modeleDao.findById(1);
      final ChampLigneEtiquette cle1 = champLigneEtiquetteManager.findByIdManager(1);
      final ChampLigneEtiquette cle2 = champLigneEtiquetteManager.findByIdManager(2);
      final List<ChampLigneEtiquette> champs = new ArrayList<>();
      champs.add(cle1);
      champs.add(cle2);
      ...
      try{
         ligneEtiquetteManager.validateObjectManager(le1, null, champs, "creation");
      ...
      try{
         ligneEtiquetteManager.validateObjectManager(le1, m1, champs, "creation");
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
   ...
   private void saveManager() throws ParseException{

      final int cTots = champManager.findAllObjectsManager().size();

      final Modele m3 = modeleDao.findById(3);
      ...
      try{
         ligneEtiquetteManager.saveManager(le1, null, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      champs.add(new ChampLigneEtiquette());
      try{
         ligneEtiquetteManager.saveManager(le1, m3, champs);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      ligneEtiquetteManager.saveManager(le1, m3, null);
      assertTrue(ligneEtiquetteManager.findAllObjectsManager().size() == 8);
      assertTrue(champLigneEtiquetteManager.findAllObjectsManager().size() == 10);
      final Integer idL1 = le1.getLigneEtiquetteId();

      // Vérification
      final LigneEtiquette leTest = ligneEtiquetteManager.findByIdManager(idL1);
      ...
      ligneEtiquetteManager.saveManager(le2, m3, champs);
      ...
      final Integer idL2 = le2.getLigneEtiquetteId();

      // Vérification
      final LigneEtiquette leTest2 = ligneEtiquetteManager.findByIdManager(idL2);
      assertNotNull(leTest2);
      assertNotNull(leTest2.getModele());
      final List<ChampLigneEtiquette> chps = champLigneEtiquetteManager.findByLigneEtiquetteManager(leTest2);
   ...
   private void saveManager() throws ParseException{
      ...
      ligneEtiquetteManager.saveManager(le, m3, champs);
      assertTrue(ligneEtiquetteManager.findAllObjectsManager().size() == 8);
      assertTrue(champLigneEtiquetteManager.findAllObjectsManager().size() == 12);
      assertTrue(champManager.findAllObjectsManager().size() == cTots + 2);
      final Integer idL = le.getLigneEtiquetteId();

      final LigneEtiquette leUp = ligneEtiquetteManager.findByIdManager(idL);
      Boolean catched = false;
      // on test l'update avec le modele null
      try{
         ligneEtiquetteManager.saveManager(leUp, null, champs, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      champs.add(new ChampLigneEtiquette());
      try{
         ligneEtiquetteManager.saveManager(leUp, m3, champs, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      leUp.setSize(15);
      leUp.setEntete("UP");
      ligneEtiquetteManager.saveManager(leUp, m3, null, null);
      assertTrue(ligneEtiquetteManager.findAllObjectsManager().size() == 8);
      assertTrue(champLigneEtiquetteManager.findAllObjectsManager().size() == 12);
      assertTrue(champManager.findAllObjectsManager().size() == cTots + 2);

      // Vérification
      final LigneEtiquette leTest = ligneEtiquetteManager.findByIdManager(idL);
      assertNotNull(leTest);
      assertTrue(leTest.getEntete().equals("UP"));
      assertTrue(leTest.getSize() == 15);
      assertTrue(champLigneEtiquetteManager.findByLigneEtiquetteManager(leTest).size() == 2);
      ...
      cle3.setOrdre(2);
      champs = new ArrayList<>();
      champs.add(champLigneEtiquetteManager.findByLigneEtiquetteManager(leTest).get(0));
      champs.add(cle3);
      final List<ChampLigneEtiquette> champsToRemove = new ArrayList<>();
      champsToRemove.add(champLigneEtiquetteManager.findByLigneEtiquetteManager(leTest).get(1));

      ligneEtiquetteManager.saveManager(leUp, m3, champs, champsToRemove);
      assertTrue(ligneEtiquetteManager.findAllObjectsManager().size() == 8);
      assertTrue(champLigneEtiquetteManager.findAllObjectsManager().size() == 12);
      assertTrue(champManager.findAllObjectsManager().size() == cTots + 2);

      // Vérification
      final LigneEtiquette leTest3 = ligneEtiquetteManager.findByIdManager(idL);
      assertNotNull(leTest3);

      final List<ChampLigneEtiquette> chps = champLigneEtiquetteManager.findByLigneEtiquetteManager(leTest3);
      assertTrue(chps.size() == 2);
      assertTrue(chps.get(0).getChamp().equals(ch1));
   ...
   private void validationChamps(final LigneEtiquette le, final Modele m){
      // Test de la validation lors de la création
      // On teste une insertion avec un attribut entete non valide
      String[] emptyValues = new String[] {"", "  ", "¢¢kjs", createOverLength(25)};
      for(int i = 0; i < emptyValues.length; i++){
         boolean catched = false;
         try{
            le.setEntete(emptyValues[i]);
            ligneEtiquetteManager.validateObjectManager(le, m, null, "creation");
         }catch(final Exception e){
      ...
      // contenu
      emptyValues = new String[] {"", "  ", "¢¢kjs", createOverLength(50)};
      for(int i = 0; i < emptyValues.length; i++){
         boolean catched = false;
         try{
