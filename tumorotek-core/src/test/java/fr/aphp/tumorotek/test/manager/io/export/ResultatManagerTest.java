...
public class ResultatManagerTest extends AbstractManagerTest4
{
   ...
   public void testCrud(){
      ...
      for(int i = 0; i < Math.pow(2, 3) - 1; i++){
         ...
         try{
            manager.saveManager(resultat, affichage, champ);
         }catch(final RequiredObjectIsNullException e){
            assertEquals("RequiredObjectIsNullException", e.getClass().getSimpleName());
         }
         try{
      ...
      /** Save. */
      final Affichage affichage = affichageDao.findById(3);
      final Champ champ = new Champ(champEntiteDao.findById(56));
      ...
      final Champ champ2 = new Champ(champEntiteDao.findById(58));

      manager.saveManager(resultat, resultat.getAffichage(), champ2);
