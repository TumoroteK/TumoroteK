...
public class TransformationManagerTest extends AbstractManagerTest4
{
   ...
   public void testCrud(){
      ...
      try{
         transformationManager.saveManager(transfo1, null, null);
      ...
      // est lancée
      /*try {
      	transformationManager.saveManager(
      ...
      catched = false;
      try{
         transformationManager.saveManager(transfo1, entite, quantite);
      }catch(final Exception e){
      ...
      transfo2.setEntite(entite);
      try{
         transformationManager.saveManager(transfo2, null, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catchedUpdate = true;
         }
      ...
      try{
         transformationManager.saveManager(transfo2, entite, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("EntiteObjectIdNotExistException")){
            catchedUpdate = true;
         }
      ...
      try{
         transformationManager.saveManager(transfo2, entite, quantite);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("ValidationException")){
            catchedUpdate = true;
         }
