...
public class InterfacageParsingUtilsTest extends AbstractManagerTest4 {
	...
	public void testInitConfigurationParsing() {
		String folder = 
			"src/test/java/fr/aphp/tumorotek/manager/test/interfacage/";
		...
		ConfigurationParsing conf = interfacageParsingUtils
			.initConfigurationParsing(racine);
		assertNotNull(conf);
		...
		conf = interfacageParsingUtils.initConfigurationParsing(
				racine);
		assertNotNull(conf);
		assertTrue(conf.getSeparateurChamps().equals("|"));
	...
	public void testParseFileToInjectInTk() throws IOException {
		...
		try {
			...
			Hashtable<String, List<String>> contenu = interfacageParsingUtils
				.parseFileToInjectInTk(conf, str);
	...
	public void testParseFileToExtractBlocsLibres() throws IOException {
		...
		try {
			...
			List<String> blocsLibres = interfacageParsingUtils
				.parseFileToExtractBlocsLibres(conf, str);
			assertNotNull(blocsLibres);
			assertTrue(blocsLibres.size() == 34);
			assertTrue(blocsLibres.get(0).equals(
					"1|TX|ANTERIORITE| |GE00516022 GE00592767 "));
			assertTrue(blocsLibres.get(3).equals(
				"1|ST|~ADICAP| |GHLOJ7F4| | | | | |F"));
			assertTrue(blocsLibres.get(33).equals(
				"29|NM|Ttt_preop| |non| | | | | |F"));
		} catch (Exception e) {
			e.printStackTrace();
			assertTrue(false);
		} finally {
	...
	public void testExtractMappingValuesForThesaurus() throws Exception {
		...
		Element currBloc = blocs.get(1);
		List<Element> mappings = currBloc.getChildren("Mapping");
		Element currMapping = mappings.get(3);
		Element modifier = currMapping.getChild("Source")
			.getChild("Modifier");
		Hashtable<String, String> res = interfacageParsingUtils
			.extractMappingValuesForThesaurs(modifier);
		assertTrue(res.size() == 3);
		
		assertTrue(res.containsKey("Prélèvement Sanguin"));
		assertTrue(res.get("TISSU TUMORAL").equals("TISSU"));
		
		currMapping = mappings.get(1);
		modifier = currMapping.getChild("Source").getChild("Modifier");
		res = interfacageParsingUtils
			.extractMappingValuesForThesaurs(modifier);
		assertTrue(res.size() == 0);
		
		res = interfacageParsingUtils
			.extractMappingValuesForThesaurs(currMapping);
		assertTrue(res.size() == 0);
			
		res = interfacageParsingUtils
			.extractMappingValuesForThesaurs(null);
		assertTrue(res.size() == 0);
	}
	...
	public void testInitNewDossierExterne() throws IOException {
		...
		try {
			...
			DossierExterne dossier = interfacageParsingUtils.initNewDossierExterne(
					conf, contenu, racine);
			assertNotNull(dossier);
			assertTrue(dossier.getIdentificationDossier().equals("ANP59248"));
			assertTrue(dossier.getOperation().equals("ORU"));
			assertTrue(format.format(dossier.getDateOperation().getTime())
					.equals("2007-05-03"));
		} catch (Exception e) {
			e.printStackTrace();
			assertTrue(false);	
	...
	public void testExtractEmetteurFromFileToInjectInTk() throws IOException {
		...
		try {
			...
			Emetteur eTest = interfacageParsingUtils
				.extractEmetteurFromFileToInjectInTk(
						folder + "configuration_interfacage.xml", 
						str, 
						"Boite Apix");
			assertNotNull(eTest);
			assertTrue(eTest.getEmetteurId().equals(2));
			
			eTest = interfacageParsingUtils
				.extractEmetteurFromFileToInjectInTk(
						folder + "configuration_interfacage.xml", 
						str, 
						"Boite");
	...
	public void testExtractXMLFIleFromFileToInjectInTk() throws IOException {
		...
		try {
			...
			String xml = interfacageParsingUtils
				.extractXMLFIleFromFileToInjectInTk(
						folder + "configuration_interfacage.xml", 
						str, 
						"Boite Apix");
						//e2);
	...
	public void testGetValueFromBlocAndEmplacement() throws IOException {
		...
		try {
			...
			// test niveau 1
			String value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "H", "2");
			assertTrue(value.equals("ANP59248"));
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "1");
			...
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "OBR", "10.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "OBR", "10.2");
			assertTrue(value.equals("MT"));
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "OBR", "10.4");
			...
			// test niveau 3
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "H", "14.2.1");
			assertTrue(value.equals("TEST1"));
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "H", "14.2.2");
			assertTrue(value.equals("TEST2"));
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "10.1.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "10.1.3");
			assertTrue(value.equals("SANG"));
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "10.1.5");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "5.2.1");
			assertTrue(value.equals("Testp3"));
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "5.2.2");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "9.2.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocAndEmplacement(
					contenu, conf, "P", "3.1.1");
			assertNull(value);
	...
	public void testGetValueFromEmplacement() throws IOException {
		...
		try {
			...
			// test niveau 1
			String value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), conf, "2");
			assertTrue(value.equals("ANP59248"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "1");
			assertTrue(value.equals("1"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), conf, "13");
			assertTrue(value.equals("20070503160500"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), conf, "5");
			assertNull(value);
			
			// test niveau 2
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "2.1");
			assertTrue(value.equals("1493053993"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "5.1");
			assertTrue(value.equals("TESTN3"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "5.2");
			assertTrue(value.equals("Testp3"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("OBR"), conf, "10.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("OBR"), conf, "10.2");
			assertTrue(value.equals("MT"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("OBR"), conf, "10.4");
			assertTrue(value.equals("Docteur"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "9.2");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "7.1");
			assertTrue(value.equals("19410904"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "7.2");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "3.1");
			assertNull(value);
			
			// test niveau 3
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "2.5.2");
			assertTrue(value.equals("93285"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), conf, "14.2.1");
			assertTrue(value.equals("TEST1"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), conf, "14.2.2");
			assertTrue(value.equals("TEST2"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "10.1.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "10.1.3");
			assertTrue(value.equals("SANG"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "10.1.5");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "5.2.1");
			assertTrue(value.equals("Testp3"));
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "5.2.2");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "9.2.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("P"), conf, "3.1.1");
			assertNull(value);
			...
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), null, "2");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), conf, null);
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("H"), conf, "15");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					contenu.get("OBR"), conf, "3.5");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromEmplacement(
					new ArrayList<String>(), conf, "2");
			assertNull(value);
		} catch (Exception e) {
			e.printStackTrace();
			assertTrue(false);	
	...
	public void testGetValueFromBlocLibre() throws IOException {
		...
		try {
			...
			ConfigurationParsing conf = interfacageParsingUtils
				.initConfigurationParsing(racine);
			List<String> blocs = interfacageParsingUtils
				.parseFileToExtractBlocsLibres(conf, str);
			
			// OK
			String value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, conf, "ANTERIORITE", "3", "5");
			assertTrue(value.equals("GE00516022 GE00592767 "));
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, conf, "ADICAP", "3.2", "5");
			assertTrue(value.equals("GHLOJ7F4"));
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, conf, "Tumor Budding (bourgeonnement)", "3", "5.1");
			assertTrue(value.equals("non"));
			
			// NON
			value = interfacageParsingUtils.getValueFromBlocLibre(
					null, conf, "ANTERIORITE", "3", "5");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, null, "ADICAP", "3.2", "5");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, conf, null, "3", "5.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, conf, "ANTERIORITE", null, "5");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, conf, "ADICAP", "3.2", null);
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					new ArrayList<String>(), 
					conf, "Tumor Budding (bourgeonnement)", "3", "5.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, 
					conf, "Tumor Budding (bourgeonnement)", "1", "5.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, 
					conf, "Tumor Budding (bourgeonnement)", "20", "5.1");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, 
					conf, "Tumor Budding (bourgeonnement)", "3", "25");
			assertNull(value);
			value = interfacageParsingUtils.getValueFromBlocLibre(
					blocs, 
					conf, "Tumor Buddi", "3", "5.1");
			assertNull(value);
		} catch (Exception e) {
			e.printStackTrace();
			assertTrue(false);
	...
	public void testFormateValueUsingFunction() {
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction("stringToLowerCase", 
						"TEST25").equals("test25"));
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction("stringToLowerCase", 
						"").equals(""));
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction("stringToUpperCase", 
						"test25").equals("TEST25"));
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction("stringToUpperCase", 
						"").equals(""));
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction("replaceCommaByDot", 
						"15,25").equals("15.25"));
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction("replaceCommaByDot", 
						"").equals(""));
		...
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction("autre", 
						"test25").equals("test25"));
		assertTrue(interfacageParsingUtils
				.formateValueUsingFunction(null, 
						"test25").equals("test25"));
		assertNull(interfacageParsingUtils
				.formateValueUsingFunction("stringToUpperCase", 
						null));
	}
		...
		try {
			...
			DossierExterne dossier = interfacageParsingUtils
				.parseInterfacageXmlFile(
					folder + "interfacage.xml", str, e1, false, 10);
	//		DossierExterne dossier = interfacageParsingUtils
	//			.parseInterfacageXmlFile(
	//			folder + "apix-mapping.xml", str, e1);
			...
			for (ValeurExterne val : 
						valeurExterneManager.findAllObjectsManager()) {
