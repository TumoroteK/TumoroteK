...
public class CouleurEntiteTypeManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		CouleurEntiteType c1 = couleurEntiteTypeManager.findByIdManager(1);
		assertNotNull(c1);
		assertTrue(c1.getCouleur().getCouleur().equals("ROUGE"));
		
		CouleurEntiteType cNull = couleurEntiteTypeManager.findByIdManager(10);
		assertNull(cNull);
	}
	...
	public void testFindAll() {
		List<CouleurEntiteType> list = couleurEntiteTypeManager
	...
	public void testFindAllObjectsByBanqueManager() {
		...
		List<CouleurEntiteType> list = couleurEntiteTypeManager
			.findAllObjectsByBanqueManager(b1);
		...
		list = couleurEntiteTypeManager.findAllObjectsByBanqueManager(
				new Banque());
		assertTrue(list.size() == 0);
	...
	public void testFindAllCouleursForEchanTypeByBanqueManager() {
		Banque b1 = banqueManager.findByIdManager(1);
		List<CouleurEntiteType> list = couleurEntiteTypeManager
			.findAllCouleursForEchanTypeByBanqueManager(b1);
		assertTrue(list.size() == 1);
		assertNotNull(list.get(0).getEchantillonType());
		assertNull(list.get(0).getProdType());
		
		Banque b3 = banqueManager.findByIdManager(3);
		list = couleurEntiteTypeManager
			.findAllCouleursForEchanTypeByBanqueManager(b3);
		assertTrue(list.size() == 0);
		
		list = couleurEntiteTypeManager
			.findAllCouleursForEchanTypeByBanqueManager(
				new Banque());
		assertTrue(list.size() == 0);
		
		list = couleurEntiteTypeManager
			.findAllCouleursForEchanTypeByBanqueManager(null);
		assertTrue(list.size() == 0);
	}
	...
	public void testFindAllCouleursForProdTypeByBanqueManager() {
		Banque b1 = banqueManager.findByIdManager(1);
		List<CouleurEntiteType> list = couleurEntiteTypeManager
			.findAllCouleursForProdTypeByBanqueManager(b1);
		assertTrue(list.size() == 1);
		...
		Banque b3 = banqueManager.findByIdManager(3);
		list = couleurEntiteTypeManager
			.findAllCouleursForProdTypeByBanqueManager(b3);
		assertTrue(list.size() == 0);
		
		list = couleurEntiteTypeManager
			.findAllCouleursForProdTypeByBanqueManager(
				new Banque());
		assertTrue(list.size() == 0);
		
		list = couleurEntiteTypeManager
			.findAllCouleursForProdTypeByBanqueManager(null);
		assertTrue(list.size() == 0);
	}
	...
	public void testFindDoublonManager() {
		...
		EchantillonType eType2 = echantillonTypeManager.findByIdManager(2);
		
		CouleurEntiteType c1 = new CouleurEntiteType();
		...
		CouleurEntiteType c2 = couleurEntiteTypeManager.findByIdManager(2);
		...
		assertFalse(couleurEntiteTypeManager.findDoublonManager(
				new CouleurEntiteType()));
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Banque b1 = banqueManager.findByIdManager(1);
		Couleur couleur = couleurManager.findByIdManager(1);
		EchantillonType eTypeDoublon = echantillonTypeManager
			.findByIdManager(1);
		EchantillonType eType = echantillonTypeManager
			.findByIdManager(2);
		ProdType pType = prodTypeManager.findByIdManager(1);
		
		CouleurEntiteType cet1 = new CouleurEntiteType();
		
		Boolean catched = false;
		// on test l'insertion avec la banque nulle
		try {
			couleurEntiteTypeManager.createObjectManager(
					cet1, couleur, null, eType, null);
		} catch (Exception e) {
		...
		assertTrue(catched);
		catched = false;
		assertTrue(couleurEntiteTypeManager.findAllObjectsManager()
				.size() == 3);
		
		// on test l'insertion avec la couleur nulle
		try {
			couleurEntiteTypeManager.createObjectManager(
					cet1, null, b1, eType, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		try {
			couleurEntiteTypeManager.createObjectManager(
					cet1, couleur, b1, eTypeDoublon, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
		...
		couleurEntiteTypeManager.createObjectManager(
				cet1, couleur, b1, eType, null);
		assertTrue(couleurEntiteTypeManager.findAllObjectsManager()
				.size() == 4);
		int id = cet1.getCouleurEntiteTypeId();
		
		// Vérification
		CouleurEntiteType cetTest1 = couleurEntiteTypeManager
			.findByIdManager(id);
		...
		CouleurEntiteType cet2 = new CouleurEntiteType();
		couleurEntiteTypeManager.createObjectManager(
				cet2, couleur, b1, null, pType);
		assertTrue(couleurEntiteTypeManager.findAllObjectsManager()
				.size() == 5);
		int id2 = cet2.getCouleurEntiteTypeId();
		
		// Vérification
		CouleurEntiteType cetTest2 = couleurEntiteTypeManager
			.findByIdManager(id2);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		EchantillonType eType = echantillonTypeManager
			.findByIdManager(2);
		EchantillonType eType2 = echantillonTypeManager
			.findByIdManager(3);
		ProdType pType = prodTypeManager.findByIdManager(1);
		
		CouleurEntiteType cet = new CouleurEntiteType();
		couleurEntiteTypeManager.createObjectManager(
				cet, couleur, b1, eType, null);
		assertTrue(couleurEntiteTypeManager.findAllObjectsManager()
				.size() == 4);
		int id = cet.getCouleurEntiteTypeId();
		
		CouleurEntiteType cetUp1 = couleurEntiteTypeManager.findByIdManager(id);
		Boolean catched = false;
		// on test l'update avec la banque nulle
		try {
			couleurEntiteTypeManager.updateObjectManager(
					cetUp1, couleur, null, eType, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		try {
			couleurEntiteTypeManager.updateObjectManager(
					cetUp1, couleur, b1, eTypeDoublon, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catched = true;
		...
		CouleurEntiteType cetUp2 = couleurEntiteTypeManager.findByIdManager(id);
		couleurEntiteTypeManager.updateObjectManager(
				cetUp2, couleur, b1, null, pType);
		assertTrue(couleurEntiteTypeManager.findAllObjectsManager()
				.size() == 4);
