...
public class TemplateManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Template temp = templateManager.findByIdManager(1);
		assertNotNull(temp);
		
		Template tempNull = templateManager.findByIdManager(10);
		assertNull(tempNull);
	}
	...
	public void testFindAll() {
		List<Template> list = templateManager.findAllObjectsManager();
	...
	public void testFindDoublon() {
		...
		Template t2 = templateManager.findByIdManager(2);
	...
	public void testCreateObjectManager() throws ParseException {
		...
		ChampEntite ce1 = champEntiteDao.findById(1);
		...
		TableAnnotation ta1 = tableAnnotationManager
			.findByNomLikeManager("TABLE_PAT1", true).get(0);
		TableAnnotation ta2 = tableAnnotationManager
			.findByNomLikeManager("TABLE_PREL1", true).get(0);
		...
		catched = false;
		try {
			templateManager.createObjectManager(
					t1, b1, e2, blocs, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		catched = false;
		assertTrue(templateManager.findAllObjectsManager().size() == 3);
		assertTrue(blocImpressionTemplateManager
				.findAllObjectsManager().size() == 4);
		
		// on teste la validation des ChampImprimes
		ChampImprime ci1 = new ChampImprime();
		ci1.setTemplate(t1);
		...
		catched = false;
		assertTrue(templateManager.findAllObjectsManager().size() == 3);
		assertTrue(tableAnnotationTemplateManager
				.findAllObjectsManager().size() == 1);
		...
		assertTrue(champImprimeManager
				.findAllObjectsManager().size() == 4);
		Integer idT1 = t1.getTemplateId();
		
		// Vérification
		Template tTest = templateManager.findByIdManager(idT1);
		assertNotNull(tTest);
		...
		bit2.setOrdre(2);
		blocs = new ArrayList<BlocImpressionTemplate>();
		blocs.add(bit1);
		blocs.add(bit2);
		...
		ci2.setOrdre(2);
		ci2.setBlocImpression(bi1);
		champs = new ArrayList<ChampImprime>();
		champs.add(ci1);
		champs.add(ci2);
		tables = new ArrayList<TableAnnotationTemplate>();
		...
		assertTrue(blocImpressionTemplateManager
				.findAllObjectsManager().size() == 6);
		assertTrue(champImprimeManager
				.findAllObjectsManager().size() == 6);
		assertTrue(tableAnnotationTemplateManager
				.findAllObjectsManager().size() == 3);
		Integer idT2 = t2.getTemplateId();
		
		// Vérification
		Template tTest2 = templateManager.findByIdManager(idT2);
		...
		assertNotNull(tTest.getBanque());
		assertTrue(blocImpressionTemplateManager
				.findByTemplateManager(tTest2).size() == 2);
		assertTrue(champImprimeManager
				.findByTemplateManager(tTest2).size() == 2);
		assertTrue(tableAnnotationTemplateManager
				.findByTemplateManager(tTest2).size() == 2);
	 ...
	 */
	@Test
	public void testUpdateObjectManager() throws ParseException {
		...
		BlocImpression bi1 = blocImpressionDao.findById(1);
		BlocImpression bi2 = blocImpressionDao.findById(2);
		BlocImpression bi3 = blocImpressionDao.findById(3);
		ChampEntite ce1 = champEntiteDao.findById(1);
		ChampEntite ce2 = champEntiteDao.findById(2);
		ChampEntite ce3 = champEntiteDao.findById(3);
		List<BlocImpressionTemplate> blocs = 
			new ArrayList<BlocImpressionTemplate>();
		...
		TableAnnotation ta2 = tableAnnotationManager
			.findByNomLikeManager("TABLE_PREL1", true).get(0);
		TableAnnotation ta3 = tableAnnotationManager
		...
		assertTrue(tableAnnotationTemplateManager
				.findAllObjectsManager().size() == 3);
		Integer idT = t.getTemplateId();
		
		Template tUp = templateManager.findByIdManager(idT);
		Boolean catched = false;
		// on test l'insertion avec la banque null
		try {
			templateManager.updateObjectManager(tUp, null, e2, 
		...
		// on test l'insertion avec l'entité null
		try {
			templateManager.updateObjectManager(tUp, b1, null, 
					null, null, null, null, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
		...
		try {
			templateManager.updateObjectManager(tUp, b1, e2, 
					null, null, null, null, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
		...
		try {
			templateManager.updateObjectManager(tUp, b1, e2, 
					blocs, blocsToCreate, null, null, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		// on teste la validation des ChampImprimes
		ChampImprime ci3 = new ChampImprime();
		ci3.setTemplate(tUp);
		...
		try {
			templateManager.updateObjectManager(tUp, b1, e2, 
					null, null, champs, champsToCreate, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		try {
			templateManager.updateObjectManager(tUp, b1, e2, 
					null, null, null, null, tables, tablesToCreate);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		Template tUp2 = templateManager.findByIdManager(idT);
		...
		bit3.setOrdre(3);
		blocs = blocImpressionTemplateManager
			.findByTemplateManager(tUp2);
		blocsToCreate = new ArrayList<BlocImpressionTemplate>();
		blocsToCreate.add(bit3);
		...
		champsToCreate = new ArrayList<ChampImprime>();
		champsToCreate.add(ci3);
		tat3.setTemplate(tUp2);
		tat3.setTableAnnotation(ta3);
		tat3.setOrdre(4);
		tables = tableAnnotationTemplateManager
			.findByTemplateManager(tUp2);
		tablesToCreate = new ArrayList<TableAnnotationTemplate>();
		tablesToCreate.add(tat3);
		templateManager.updateObjectManager(tUp2, b1, e2, 
				blocs, blocsToCreate, champs, champsToCreate, 
	 ...
	 * @throws ParseException 
	 */
	private void validationInsert(Template template) 
		throws ParseException {
	 ...
	 * @throws ParseException 
	 */
	private void validationUpdate(Template template) 
		throws ParseException {
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				template.setNom(emptyValues[i]);
				templateManager.updateObjectManager(
						template, b1, e2, null, null, null,
						null, null, null);
			} catch (Exception e) {
				if (e.getClass().getSimpleName().equals(
