...
public class IncaPatientExportTest extends AbstractManagerTest {
	...
	public void testGetFiness() 
		...
		assertTrue(incaPatientExport.getFiness(con, prel, p)
								.equals("310783063 : HOPITAL LA GRAVE"));
		
		prel.setPrelevementId(8);
		assertTrue(incaPatientExport.getFiness(con, prel, p)
									.equals("310783048 : CHU PURPAN"));
		boolean catched = false;
		try {
			prel.setPrelevementId(4);
			assertTrue(incaPatientExport.getFiness(con, prel, p)
									.equals("D310783048 : CHU PURPAN"));
		} catch (ItemException ie) {
			catched = true;
			assertTrue(ie.getSeverity() == 2);
			assertTrue(ie.getMessage()
					.equals("FINESS manquant pour le patient TGVSO NIP"));
		}
		assertTrue(catched);
	}
	...
	public void testGetPatientId() 
		...
		assertTrue(incaPatientExport.getPatientId(p)
												.equals("125"));
		...
		try {
			...
			assertTrue(ie.getSeverity() == 2);
			assertTrue(ie.getMessage()
					.equals("Identifiant patient obligatoire manquant"));
		}
		assertTrue(catched);
	}
	...
	public void testGetDateNaissance() throws ParseException {
		...
		assertTrue(incaPatientExport.getDateNaissance(p,
								dateFormatterINCA).equals("24/03/1978"));
		
		boolean catched = false;
		try {
			p.setDateNaissance(null);
			assertTrue(incaPatientExport.getDateNaissance(p, null)
												.equals("TVGSO NIP"));
		} catch (ItemException ie) {
			catched = true;
			assertTrue(ie.getSeverity() == 2);
			assertTrue(ie.getMessage()
				.equals("Date de naissance manquante"));
		}
		assertTrue(catched);
		...
		try {
			...
			assertTrue(incaPatientExport.getDateNaissance(p,
									dateFormatterINCA).equals("TVGSO NIP"));
		} catch (ItemException ie) {
			catched = true;
			assertTrue(ie.getSeverity() == 2);
			assertTrue(ie.getMessage()
	...
	public void testGetPatientEtat() 
				throws SQLException, ClassNotFoundException {
	...
	public void testGetDateEtat() throws SQLException, 
								ClassNotFoundException, ParseException {
		DateFormat dateFormatterINCA = new SimpleDateFormat("yyyyMMdd");
		
		Patient p = new Patient();
		
		boolean catched = false;
		try {
			assertTrue(incaPatientExport.getDateEtat(p, 
					dateFormatterINCA).equals("00000000"));
		} catch (ItemException ie) {
			catched = true;
			assertTrue(ie.getSeverity() == 2);
			assertTrue(ie.getMessage()
				.equals("Date etat patient manquante"));
		}
		assertTrue(catched);
		
		p.setDateEtat(dateFormatterINCA.parse("18760222"));
		assertTrue(incaPatientExport.getDateEtat(p, 
				dateFormatterINCA).equals("18760222"));
		
		p.setDateDeces(dateFormatterINCA.parse("19990909"));
		assertTrue(incaPatientExport.getDateEtat(p, 
				dateFormatterINCA).equals("19990909"));
	}
	
	public void testGetCauseDeces() 
					throws SQLException, ClassNotFoundException {
