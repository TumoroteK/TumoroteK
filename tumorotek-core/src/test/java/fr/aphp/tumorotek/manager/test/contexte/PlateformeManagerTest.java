...
public class PlateformeManagerTest extends AbstractManagerTest4
{
	...
	public void testUpdateObjectManager(){
		...
		plateformeManager.updateObjectManager(pf2, c1, users, conteneurs, admin);
		assertTrue(plateformeManager.findAllObjectsManager().size() == 2);
		...
		conteneurs.add(ct2);
		plateformeManager.updateObjectManager(pf3, c1, users, conteneurs, admin);
		assertTrue(plateformeManager.findAllObjectsManager().size() == 2);
		...
		plateformeManager.updateObjectManager(pf4, c4, users, conteneurs, admin);
		assertTrue(plateformeManager.findAllObjectsManager().size() == 2);
		...
		// clean up 
		conteneurPlateformeManager.removeObjectManager(conteneurPlateformeDao.findById(new ConteneurPlateformePK(ct2, pf5))).orElse(null);
		conteneurPlateformeManager.removeObjectManager(conteneurPlateformeDao.findById(new ConteneurPlateformePK(ct3, pf5))).orElse(null);
	 ...
	 */
	@Test
	public void testCreateObjectManager() throws IOException {
		...
		pf.setNom(null);
		try{
			plateformeManager.createObjectManager(pf, null, null, u1, baseDirFolder.getAbsolutePath());
		}catch(final Exception e){
			if(e.getClass().getSimpleName().equals("ValidationException")){
				catched = true;
			}
		...
		// empty nom
		try{
			plateformeManager.createObjectManager(pf, null, null, null, baseDirFolder.getAbsolutePath());
		}catch(final Exception e){
			if(e.getClass().getSimpleName().equals("ValidationException")){
				catched = true;
			}
		...
		// Empty path
		try{
			plateformeManager.createObjectManager(pf, null, null, u1, null);
		}catch(final Exception e){
		...
		admins.add(u2);
		Collaborateur c = collaborateurDao.findById(2).orElse(null);

		pf = plateformeManager.createObjectManager(pf, c, admins, u1, baseDirFolder.getAbsolutePath());
		...
		// nulls
		assertNull(plateformeManager.createObjectManager(null, null, null, u1, baseDirFolder.getAbsolutePath()));
	...
	public void testIsReferencedAndRemoveObjectManager() throws IOException {
		...
		pf = plateformeManager.createObjectManager(pf, null, admins, u1, baseDirFolder.getAbsolutePath());
		...
		plateformeManager.updateObjectManager(pf, null, admins, conteneurs, u1);
		assertTrue(plateformeManager.isReferencedObjectManager(pf));
		
		conteneurs.clear();
		conteneurPlateformeManager.removeObjectManager(conteneurPlateformeDao.findById(new ConteneurPlateformePK(ct1, pf))).orElse(null);
		conteneurPlateformeManager.removeObjectManager(conteneurPlateformeDao.findById(new ConteneurPlateformePK(ct2, pf))).orElse(null);	
		assertFalse(plateformeManager.isReferencedObjectManager(pf));
		
		// ajout banques
		Banque b = new Banque();
		b.setNom("bb1");
	    Contexte ct = contexteDao.findById(1).orElse(null);
		banqueManager.createOrUpdateObjectManager(b, pf, ct, null, null, null, 
	 ...
	 */
	@Test
	public void testCreateObjectWithAdminIsLoggedUserManager() throws IOException {
