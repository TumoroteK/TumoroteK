...
public class LaboInterManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<LaboInter> laboInters = laboInterManager.findAllObjectsManager();
	...
	public void testFindCollaborateurManager() {
		...
		List<LaboInter> laboInters = 
			laboInterManager.findByCollaborateurManager(c1);
		assertTrue(laboInters.size() == 3);
		//teste une recherche infructueuse
		...
		//null recherche
		laboInters = 
			laboInterManager.findByCollaborateurManager(null);
		assertTrue(laboInters.size() == 0);
	}
	...
	public void testFindTransporteurManager() {
		...
		//teste une recherche fructueuse
		List<LaboInter> laboInters = 
			laboInterManager.findByTransporteurManager(t1);
		...
		//null recherche
		laboInters = 
			laboInterManager.findByTransporteurManager(null);
		assertTrue(laboInters.size() == 0);
	}
	...
	public void testFindDoublon() {
		Service s1 = serviceDao.findById(2);
		//Cree le doublon
		LaboInter l1 = (laboInterManager
				.findByServiceManager(s1)).get(0);
	...
	private void createObjectManagerTest() {
		...
		Transporteur t1 = transporteurDao.findById(2);
		...
		assertTrue((laboInterManager
				.findByTransporteurManager(t1)).size() == 1);
		...
		l2.setSterile(false);
		try {
			laboInterManager.createObjectManager(l2, p1, null, null , 
															null, true);
		} catch (Exception e) {
		...
		//Validation tests
		Prelevement[] prelValues = new Prelevement[]{null, p1};
		Integer[] ordreValues = new Integer[]{0, -10, null, 2};
		Float[] conservTempValues = new Float[]{new Float(-1001.44), 
				new Float(1003.87), null, new Float(0.08) };
		Float[] transportTempValues = new Float[]{new Float(-1001.44), 
				new Float(1003.87), null, new Float(0.08) };
		l2 = new LaboInter();
		l2.setSterile(false);
				...
				for (int k = 0; k < conservTempValues.length; k++) {
					for (int g = 0; g < transportTempValues.length; g++) {
						try {
							...
							if (!(i == 1 && j == 3 
								&& k > 1 && g > 1)) { //car creation valide
								laboInterManager
									.createObjectManager(l2, prelValues[i], 
													null, null, null, true);
							}
						} catch (Exception e) {
							if (i == 0) {
	...
	private void updateObjectManagerTest() {
		...
		Service s1 = serviceDao.findById(3);
		Transporteur t1 = transporteurDao.findById(2);
		LaboInter l1 = (laboInterManager
			.findByTransporteurManager(t1)).get(0);
		Prelevement p1 = prelevementDao.findById(3);
		l1.setOrdre(5);
		laboInterManager.updateObjectManager(l1, p1, s1, null, null, true);
		assertTrue((laboInterManager
				.findByServiceManager(s1)).size() == 1);
		//Modification en un doublon engendrant une exception
		Boolean catched = false;
		try {
			l1.setOrdre(1);
			Prelevement p2 = prelevementDao.findById(1);
				...
				for (int k = 0; k < conservTempValues.length; k++) {
					for (int g = 0; g < transportTempValues.length; g++) {
						try {
							...
							if (i != 1 && j != 3 
								&& k != 3 && g != 3) { //car creation valide
								laboInterManager
									.updateObjectManager(l1, prelValues[i], 
													null, null, null, true);
							}
						} catch (Exception e) {
							if (i == 0) {
							...
							assertTrue((laboInterManager
								.findByTransporteurManager(t1))
									.get(0).getOrdre() == 5);
							assertTrue((laboInterManager
									.findByTransporteurManager(t1))
											.get(0).getTransportTemp()
												.equals(new Float(-12.08)));
							assertTrue((laboInterManager
									.findByTransporteurManager(t1))
										.get(0).getConservTemp()
												.equals(new Float(-10.10)));
						}
					}	
				}
			}
	...
	private void removeObjectManagerTest() {
		...
		assertTrue((laboInterManager
				.findByTransporteurManager(t1)).size() == 0);
		//null remove
	...
	public void testDateArriveeCoherence() throws ParseException {
		...
		LaboInter l4 = new LaboInter(); l4.setOrdre(4); set.add(l4);
		LaboInter l5 = new LaboInter(); l5.setOrdre(5); set.add(l5);
		...
		ref.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
											.parse("01/01/1974 10:00:12"));
		
		Calendar refNoHours = Calendar.getInstance();
		refNoHours.setTime(new SimpleDateFormat("dd/MM/yyyy")
											.parse("01/01/1974"));
		...
		arrDate.setTime(new SimpleDateFormat("dd/MM/yyyy")
														.parse("01/01/1971"));
		...
		errs = laboInterValidator.checkDateArriveeCoherence(l3);
		assertEquals("laboInter.dateArrivee.infPreviousDateArrivee", 
												errs.getFieldError().getCode());
		
		l1.setDateDepart(ref);
		errs = laboInterValidator.checkDateArriveeCoherence(l3);
		assertEquals("laboInter.dateArrivee.infPreviousDateDepart", 
												errs.getFieldError().getCode());
		...
		errs = laboInterValidator.checkDateArriveeCoherence(l3);
		assertEquals("date.validation.supDateArriveePrelevement", 
											errs.getFieldError().getCode());
		...
		errs = laboInterValidator.checkDateArriveeCoherence(l3);
		assertEquals("laboInter.dateArrivee.supNextDateDepart", 
											errs.getFieldError().getCode());
		
		l5.setDateArrivee(ref);
		errs = laboInterValidator.checkDateArriveeCoherence(l3);
		assertEquals("laboInter.dateArrivee.supNextDateArrivee", 
											errs.getFieldError().getCode());
		...
		errs = laboInterValidator.checkDateArriveeCoherence(l3);
		assertEquals("laboInter.dateArrivee.supDateDepart", 
											errs.getFieldError().getCode());
		
		// teste laboInter date sup dateStockage d'un echantillon
		p = prelevementDao.findById(1);
		p.setDateArrivee(null);
		List<LaboInter> labs = laboInterManager.findByPrelevementWithOrder(p);
		LaboInter l = labs.get(2);
		l.setPrelevement(p);
		p.setLaboInters(new HashSet<LaboInter>(labs));
	...
	public void testDateDepartCoherence() throws ParseException {
		...
		depDate.setTime(new SimpleDateFormat("dd/MM/yyyy")
													.parse("01/01/1971"));
		...
		l1.setDateArrivee(ref);
		errs = laboInterValidator.checkDateDepartCoherence(l3);
		assertEquals("laboInter.dateDepart.infPreviousDateArrivee", 
												errs.getFieldError().getCode());
		
		l1.setDateDepart(ref);
		errs = laboInterValidator.checkDateDepartCoherence(l3);
		assertEquals("laboInter.dateDepart.infPreviousDateDepart", 
												errs.getFieldError().getCode());
		...
		errs = laboInterValidator.checkDateDepartCoherence(l3);
		assertEquals("laboInter.dateDepart.infDateArrivee", 
												errs.getFieldError().getCode());
		...
		l5.setDateDepart(ref);
		errs = laboInterValidator.checkDateDepartCoherence(l3);
		assertEquals("laboInter.dateDepart.supNextDateDepart", 
											errs.getFieldError().getCode());
		
		l5.setDateArrivee(ref);
		errs = laboInterValidator.checkDateDepartCoherence(l3);
		assertEquals("laboInter.dateDepart.supNextDateArrivee", 
											errs.getFieldError().getCode());
	...
	public void testSteriliteCoherence() {
		...
		Prelevement p  = prelevementDao.findById(3);
		Set<LaboInter> set = new HashSet<LaboInter>();
		LaboInter l1 = new LaboInter(); l1.setOrdre(1); set.add(l1);
		l1.setPrelevement(p);
		l1.setSterile(null);
		...
		l1.setSterile(false);
		LaboInter l2 = new LaboInter(); l2.setOrdre(3); set.add(l2);
		l2.setPrelevement(p);
		p.setLaboInters(set);
		l2.setSterile(true);
