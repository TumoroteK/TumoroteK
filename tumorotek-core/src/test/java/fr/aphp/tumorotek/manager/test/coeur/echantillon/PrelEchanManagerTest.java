...
public class PrelEchanManagerTest extends AbstractManagerTest4
{
   ...
   public void testCreateQuickPrelAndEchansExistingPatAndMaladie(){
      ...
      prel.setNature(natureDao.findById(1)).orElse(null);
      prel.setConsentType(consentTypeDao.findById(1)).orElse(null);
   ...
   public void testCreateQuickPrelAndEchansExistingPatNewMaladie(){
      ...
      dtos.get(0).getNonConformiteTraitements().add(nonConformiteDao.findById(4)).orElse(null);
      dtos.get(0).getEchantillon().setConformeCession(false);
      dtos.get(0).getNonConformiteCessions().add(nonConformiteDao.findById(7)).orElse(null);
      dtos.get(0).getCodesLesToCreateOrEdit().addAll(cLes1);
      ...
      dtos.get(3).getNonConformiteTraitements().add(nonConformiteDao.findById(4)).orElse(null);
      dtos.get(3).getEchantillon().setConformeCession(true);
      dtos.get(3).getNonConformiteCessions().add(nonConformiteDao.findById(7)).orElse(null);
      ...
      assertTrue(
         objetNonConformeManager.findByObjetAndTypeManager(echans.get(0), nonConformiteDao.findById(4).getConformiteType()).get(0).orElse(null)
            .getNonConformite().getId() == 4);
      assertTrue(
         objetNonConformeManager.findByObjetAndTypeManager(echans.get(0), nonConformiteDao.findById(7).getConformiteType()).get(0).orElse(null)
   ...
   public void testCreateQuickPrelAndEchansNewPatNewMaladie() throws ParseException{
      ...
      newE.setTerminale(terminaleDao.findById(6)).orElse(null);
      newE.setPosition(3);
      empls.put(e3, newE);
   ...
   public void setup(){
      ...
      final EchantillonType type1 = echantillonTypeDao.findById(1).orElse(null);
      final EchantillonType type2 = echantillonTypeDao.findById(2).orElse(null);
   ...
   private void assertNothingHappened(){
      ...
      nbEmplOccupeBeforeTests = emplacementManager.findByTerminaleAndVideManager(terminaleDao.findById(1), false).size().orElse(null)
         + emplacementManager.findByTerminaleAndVideManager(terminaleDao.findById(2), false).size().orElse(null);
   ...
   public void tearDown(){
      ...
      if(prel.getPrelevementId() != null){
         prelevementManager.removeObjectCascadeManager(prel, null, utilisateurDao.findById(1), null).orElse(null);
         fs.add(prel);
         fs.addAll(echans);
      }

      if(mal != null && mal.getMaladieId() != null){
         maladieManager.removeObjectManager(mal, null, utilisateurDao.findById(1)).orElse(null);
         fs.add(mal);
      }

      if(pat != null && pat.getPatientId() != null){
         patientManager.removeObjectManager(pat, null, utilisateurDao.findById(1), null).orElse(null);
         fs.add(pat);
      }
