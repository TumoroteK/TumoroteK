...
public class PrelEchanManagerTest extends AbstractManagerTest4 {
	...
	private List<EchantillonDTO> dtos = new ArrayList<EchantillonDTO>();
	private List<CodeAssigne> cOrgs1 = new ArrayList<CodeAssigne>();
	private List<CodeAssigne> cOrgs2 = new ArrayList<CodeAssigne>();
	private List<CodeAssigne> cLes1 = new ArrayList<CodeAssigne>();
	private List<CodeAssigne> cLes2 = new ArrayList<CodeAssigne>();
	...
	private String bDir = "/tmp/pt_1/coll_1";

	public PrelEchanManagerTest() {	
	}
	
	private void initCodesAssigne() {
		CodeAssigne o1 = new CodeAssigne();
		o1.setCode("C1"); o1.setLibelle("L1"); 
		o1.setOrdre(1); o1.setExport(true);
		o1.setIsOrgane(true); o1.setIsMorpho(false); 
		cOrgs1.add(o1);
		CodeAssigne o2 = new CodeAssigne();
		o2.setIsOrgane(true); o2.setIsMorpho(false); 
		o2.setOrdre(2); o2.setExport(false);
		o2.setCode("C2"); o2.setLibelle("L2");
		cOrgs1.add(o2);
		CodeAssigne o3 = new CodeAssigne();
		o3.setIsOrgane(true); o3.setIsMorpho(false); 
		o3.setOrdre(1); o3.setExport(true);
		o3.setCode("C3"); o3.setLibelle("L3");
		cOrgs2.add(o3);
		
		CodeAssigne l1 = new CodeAssigne();
		l1.setIsOrgane(false); l1.setIsMorpho(true);  
		l1.setOrdre(1); l1.setExport(true);
		l1.setCode("1111"); l1.setLibelle("LES1");
		cLes1.add(l1);
		CodeAssigne l2 = new CodeAssigne();
		l2.setIsOrgane(false); l2.setIsMorpho(true); 
		l2.setOrdre(1); l2.setExport(true);
		l2.setCode("2222"); l2.setLibelle("LES2");
		cLes2.add(l2);
		CodeAssigne l3 = new CodeAssigne();
		l3.setIsOrgane(false); l3.setIsMorpho(true); 
		l3.setOrdre(2); l3.setExport(false);
		l3.setCode("3333"); l3.setLibelle("LES3");
		cLes2.add(l3);
	}
	
	private void assertsPrelEchansOK(Banque b) {
		// prel
	...
	public void testCreateQuickPrelAndEchansExistingPatAndMaladie() {
		...
		prel = prelEchansManager
			.createQuickPrelAndEchansManager(patientManager.findByIdManager(5), 
					maladieManager.findByCodeLikeManager("C34.5", true).get(0), prel, dtos, null, 
					b1, utilisateur, null, null);
		...
		List<Echantillon> echans = new ArrayList<Echantillon>(prelevementManager.getEchantillonsManager(prel));
	...
	public void testCreateQuickPrelAndEchansExistingPatNewMaladie() {
		...
		prel = prelEchansManager
			.createQuickPrelAndEchansManager(patientManager.findByIdManager(5), mal , prel, dtos, null, 
					b1, utilisateur, null, null);
		...
		assertTrue(objetNonConformeManager.findByObjetAndTypeManager(echans.get(0), 
				nonConformiteDao.findById(4).getConformiteType()).get(0).getNonConformite().getNonConformiteId() == 4);
		assertTrue(objetNonConformeManager.findByObjetAndTypeManager(echans.get(0), 
				nonConformiteDao.findById(7).getConformiteType()).get(0).getNonConformite().getNonConformiteId() == 7);
	...
	public void testCreateQuickPrelAndEchansNewPatNewMaladie() throws ParseException {
		...
		// stockage
		Map<Echantillon, Emplacement> empls = new HashMap<Echantillon, Emplacement>();
		empls.put(e2, emplacementManager.findByIdManager(5)); // existing empl
		Emplacement newE = new Emplacement(); // new Empl T6 pos 3
		...
		prel = prelEchansManager
			.createQuickPrelAndEchansManager(pat, mal , prel, dtos, empls, 
					b1, utilisateur, null, null);
		...
		Emplacement newEmp = echantillonManager.getEmplacementManager(echans.get(2));
		...
		emp5.setVide(true);
		emp5.setObjetId(null); emp5.setEntite(null);
		emplacementManager.updateObjectManager(emp5, emp5.getTerminale(), null);
		echantillonManager.updateObjectManager(echans.get(1), b1, prel, null, echans.get(4).getObjetStatut(), 
				null, echans.get(4).getEchantillonType(), null, null, null, null, null, null, null, null, 
				null, null, utilisateur, false, null, null);
		Emplacement emp7 = emplacementManager.findByIdManager(7);
		emp7.setVide(true);
		emp7.setObjetId(null); emp7.setEntite(null);
		emplacementManager.updateObjectManager(emp7, emp7.getTerminale(), null);
		echantillonManager.updateObjectManager(echans.get(4), b1, prel, null, echans.get(4).getObjetStatut(), 
				null, echans.get(4).getEchantillonType(), null, null, null, null, null, null, null, null, 
				null, null, utilisateur, false, null, null);	
	}
	...
	public void testCreateQuickPrelAndEchansErrorOnEchan() {
		...
		try {
			prelEchansManager
				.createQuickPrelAndEchansManager(null, maladieManager.findByCodeLikeManager("C34.5", true).get(0), 
						prel, dtos, null, 
					b1, utilisateur, null, null);
	...
	public void testCreateQuickPrelAndEchansNewPatNewMaladieErrorOnEchan() throws ParseException {
		...
		Fichier cr2 = new Fichier();
		cr2.setNom("anapath2");	
		cr2.setMimeType("text/plain");
		String tmp2 = "Echantillon diagnostic no virus";
		byte[] byteArray2 = tmp2.getBytes(); 
		ByteArrayInputStream bais2 = new ByteArrayInputStream(byteArray2);
		...
		assertTrue(echans.get(3).getCrAnapath().getPath()
							.equals(echans.get(4).getCrAnapath().getPath()));
	...
	public void setup() {
		...
		EchantillonType type1 = echantillonTypeDao.findById(1);
		EchantillonType type2 = echantillonTypeDao.findById(2);
		
		e1 = new Echantillon(); e1.setCode("QuickE1"); 
		e1.setEchantillonType(type1);
		EchantillonDTO dto1 = 
				echantillonDTOManager.initEchantillonDecoratorManager(e1);
		
		e2 = new Echantillon(); e2.setCode("QuickE2");
		e2.setEchantillonType(type1);
		EchantillonDTO dto2 = 
				echantillonDTOManager.initEchantillonDecoratorManager(e2);
			
		e3 = new Echantillon(); e3.setCode("QuickE3");
		e3.setEchantillonType(type2);
		EchantillonDTO dto3 = 
				echantillonDTOManager.initEchantillonDecoratorManager(e3);
		
		e4 = new Echantillon(); e4.setCode("QuickE4");
		e4.setEchantillonType(type2);
		EchantillonDTO dto4 = 
				echantillonDTOManager.initEchantillonDecoratorManager(e4);
		
		e5 = new Echantillon(); e5.setCode("QuickE5");
		e5.setEchantillonType(type2);
		EchantillonDTO dto5 = 
				echantillonDTOManager.initEchantillonDecoratorManager(e5);
		
		dtos.add(dto1); dtos.add(dto2); dtos.add(dto3); dtos.add(dto4); dtos.add(dto5); 
	...
	private void assertNothingHappened() {
		...
		nbEmplOccupeBeforeTests = emplacementManager
				.findByTerminaleAndVideManager(terminaleDao.findById(1), false).size()
			+ emplacementManager
				.findByTerminaleAndVideManager(terminaleDao.findById(2), false).size();
	...
	public void tearDown() {
		...
		List<Echantillon> echans = echantillonDTOManager.extractListeManager(dtos);
		...
		assertTrue(nbEmplOccupeBeforeTests == emplacementManager
				.findByTerminaleAndVideManager(terminaleDao.findById(1), false).size()
			+ emplacementManager
				.findByTerminaleAndVideManager(terminaleDao.findById(2), false).size());
