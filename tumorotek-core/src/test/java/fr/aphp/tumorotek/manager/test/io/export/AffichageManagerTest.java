...
public class AffichageManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Affichage affichage = manager.findByIdManager(1);
		assertNotNull(affichage);
		assertTrue(affichage.getNbLignes().equals(20));
		
		Affichage affichageNull = manager.findByIdManager(5);
	...
	public void testFindAllObjects() {
		List<Affichage> list = manager.findAllObjectsManager();
		//On vérifie chaque élément par un findById
		Iterator<Affichage> it = list.iterator();
		while (it.hasNext()) {
			Affichage temp = it.next();
			if (temp != null) {
				assertTrue(temp.equals(
						manager.findByIdManager(temp.getAffichageId())));
			}
		}
	}
	...
	public void testCopyAffichage() {
		...
		Affichage copie = manager.copyAffichageManager(affichage, util, b1);
		...
		Affichage copie2 = manager.findByIdManager(copie.getAffichageId());
	...
	public void testRenameAffichage() {
		...
		String oldIntitule = affichage.getIntitule();
		String intitule = affichage.getIntitule() + " MODIFICATION";
		manager.renameAffichageManager(affichage, intitule);
		
		//On récupère l'objet modifié
		Affichage affichage2 = manager.findByIdManager(
	...
	public void testAddResultat() {
		...
		//On récupère un affichage
		Affichage affichage = manager.findByIdManager(3);
		//On récupère un champ
		Champ champ = new Champ(champEntiteDao.findById(20));
		//On crée un résultat
		Resultat resultat = new Resultat("code derive", champ,
				ConstanteIO.TRI_ASC, 3, 1,	"Chaine", affichage);
		...
		Iterator<Resultat> it = affichage2.getResultats().iterator();
		while (it.hasNext()) {
	...
	public void testFindByUtilisateur() {
		...
		List<Affichage> affichages = manager.findByUtilisateurManager(util);
	...
	public void testFindByIntituleAndUtilisateurManager() {
		...
		List<Affichage> liste = manager
			.findByIntituleAndUtilisateurManager("Essen%", u1);
		assertTrue(liste.size() == 1);
	...
	public void testMoveResultat() {
		Affichage affichage = manager.findByIdManager(2);
		
		//MAJ des resultats
		affichage.setResultats(resultatDao.findByAffichage(affichage));
			
		Resultat resultat = affichage.getResultats().get(0);
		int idResultat = resultat.getResultatId();
		int positionInitiale = resultat.getPosition();
		
		manager.moveResultatManager(affichage, resultat, 1);
			
		//On vérifie que la position en BDD a été modifiée
		Resultat resultat2 = resultatDao.findById(idResultat);
		assertFalse(resultat2.getPosition().equals(positionInitiale));
		
		
		//On vérifie que la position dans la liste a été modifiée
		assertFalse(affichage.getResultats().get(0).getPosition()
				.equals(positionInitiale));
	...
	public void testCrud() {
		...
		Champ champ1 = new Champ(champEntiteDao.findById(3));
		Resultat resultat1 = new Resultat("col1", champ1, true,
				1, 2, "format", null);
		Champ champ2 = new Champ(champEntiteDao.findById(4));
		...
		Affichage affichage = new Affichage("test enregistrement affichage", 
				util, 20);
		manager.createObjectManager(affichage, res, util, b1);
		...
		/** Update. */
		String oldIntitule = affichage.getIntitule();
		String intitule = "test modification affichage";		
		affichage.setIntitule(intitule);
		
		Champ champ3 = new Champ(champEntiteDao.findById(1));
		Resultat resultat3 = new Resultat("col3", champ3, true,
				3, 3, null, null);
				
		//MAJ des résultats
		List<Resultat> resultats = resultatDao.findByAffichage(affichage);
		resultats.add(resultat3);
		
		manager.updateObjectManager(affichage, resultats, null);
		...
		boolean found = false;
		Iterator<Resultat> it = resultatDao.findByAffichage(affichage)
				.iterator();
		while (it.hasNext()) {
			Resultat temp = it.next();
		...
		Resultat resultat = resultatManager
				.findByIdManager(resultat3.getResultatId());
		...
		/** Delete. */
		//On récupère un affichage
		int id = affichage.getAffichageId();
		//On le supprime
		...
		Affichage a1 = manager.findByIdManager(1);
	...
	public void testIsUsedObjectManager() {
		...
		Affichage a2 = manager.findByIdManager(2);
		assertTrue(manager.isUsedObjectManager(a2));
		
		Affichage a4 = manager.findByIdManager(4);
