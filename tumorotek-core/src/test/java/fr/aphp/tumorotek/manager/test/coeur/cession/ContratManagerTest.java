...
public class ContratManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Contrat mtaNull = contratManager.findByIdManager(10);
		assertNull(mtaNull);
	}
	...
	public void testFindAll() {
		List<Contrat> list = contratManager.findAllObjectsManager();
	...
	public void testFindAllObjectsByPlateformeManager() {
		...
		List<Contrat> list = contratManager
			.findAllObjectsByPlateformeManager(pf1);
	...
	public void testFindByNumeroLikeExactManager() {
		List<Contrat> list = 
			contratManager.findByNumeroLikeManager("CONTRAT 78551269", true);
		assertTrue(list.size() == 1);
	...
	public void testFindByNumeroLikeManager() {
		List<Contrat> list = 
			contratManager.findByNumeroLikeManager(
					"CONTRAT 78551269", false);
		assertTrue(list.size() == 1);
	...
	public void testFindDoublon() {
		...
		String numero1 = "CONTRAT 78551269";
		String numero2 = "MTA";
		Plateforme pf1 = plateformeDao.findById(1);
		Plateforme pf2 = plateformeDao.findById(2);
		
		Contrat mta1 = new Contrat();
		...
		Contrat mta2 = contratManager.findByIdManager(2);
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Service serv = serviceManager.findByIdManager(1);
		Etablissement etab = etablissementDao.findById(1);
		ProtocoleType type = protocoleTypeManager.findByIdManager(1);
		...
		try {
			contratManager.createObjectManager(
					contrat1, null, null, null, null, null, u);
		} catch (Exception e) {
		...
		try {
			contratManager.createObjectManager(
					contrat1, null, null, null, null, pf1, u);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
		...
		assertTrue(getOperationManager().findByObjectManager(
				contrat1).size() == 1);
		int id = contrat1.getContratId();
		
		// On test une insertion valide
		Contrat contrat2 = new Contrat();
		Date demande = new SimpleDateFormat("dd/MM/yyyy").parse("01/09/2009");
		Date validation = new SimpleDateFormat(
				"dd/MM/yyyy").parse("20/09/2009");
		Date demandeRedac = new SimpleDateFormat(
				"dd/MM/yyyy").parse("05/10/2009");
		Date envoi = new SimpleDateFormat("dd/MM/yyyy").parse("10/10/2009");
		Date signature = new SimpleDateFormat("dd/MM/yyyy").parse("15/10/2009");
		Float montant = new Float(142);
		...
		contratManager.createObjectManager(
				contrat2, collab, serv, etab, type, pf1, u);
		assertTrue(contratManager.findAllObjectsManager().size() == 6);
		assertTrue(getOperationManager().findByObjectManager(
				contrat2).size() == 1);
		int id2 = contrat2.getContratId();
		
		// Vérification
		Contrat contratTest = contratManager.findByIdManager(id2);
		...
		// Suppression
		Contrat cToRemove = contratManager.findByIdManager(id);
		...
		assertTrue(getOperationManager()
						.findByObjectManager(cToRemove).size() == 0);
		assertTrue(getOperationManager()
						.findByObjectManager(contratTest).size() == 0);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		contratManager.createObjectManager(
				contrat, null, null, null, null, pf1, u);
		assertTrue(contratManager.findAllObjectsManager().size() == 5);
		assertTrue(getOperationManager().findByObjectManager(
				contrat).size() == 1);
		int id = contrat.getContratId();
		
		Contrat contrat2 = contratManager.findByIdManager(id);
		Boolean catched = false;
		// on test l'update avec la pf nulle
		try {
			contratManager.updateObjectManager(
					contrat2, null, null, null, null, null, u);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		try {
			contratManager.updateObjectManager(
					contrat2, null, null, null, null, pf1, u);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catched = true;
		...
		assertTrue(getOperationManager().findByObjectManager(
				contrat2).size() == 2);
		Contrat contrat3 = contratManager.findByIdManager(id);
		...
		contratManager.updateObjectManager(
				contrat3, collab, serv, etab, type, pf1, u);
		assertTrue(contratManager.findAllObjectsManager().size() == 5);
		assertTrue(getOperationManager().findByObjectManager(
				contrat3).size() == 3);
		
		// Vérification
		Contrat contratTest = contratManager.findByIdManager(id);
		assertNotNull(contratTest);
		assertNotNull(contratTest.getCollaborateur());
		assertNotNull(contratTest.getService());
		assertNotNull(contratTest.getProtocoleType());
	...
	private void removeObjectManagerTest() {
		...
		try {
			...
			if (e.getClass().getSimpleName().equals(
					"ObjectUsedException")) {
				catched = true;
				assertTrue(((ObjectUsedException) e).getKey()
						.equals("contrat.deletion.isReferenced"));
			}
		}
		assertTrue(catched);
		assertTrue(contratManager.findAllObjectsManager().size() == 4);
	 ...
	 * @throws ParseException 
	 */
	private void validationInsert(Contrat contrat) 
		throws ParseException {
		Utilisateur u = utilisateurDao.findById(1);
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				...
				contratManager.createObjectManager(
						contrat, null, null, null, null, pf, u);
			} catch (Exception e) {
				if (e.getClass().getSimpleName().equals(
						"ValidationException")) {
					catchedInsert = true;
	 ...
	 * @throws ParseException 
	 */
	private void validationUpdate(Contrat contrat) 
		throws ParseException {
		Utilisateur u = utilisateurDao.findById(1);
		Plateforme pf = plateformeDao.findById(1);
		boolean catched = false;
	...
	public void testDemandeDateCoherence() throws ParseException {
		...
		// limites inf
		c.setDateDemandeCession(new SimpleDateFormat("dd/MM/yyyy")
												.parse("14/07/2010"));
		errs = ContratValidator.checkDemandeDateCoherence(c);
		assertTrue(errs.getAllErrors().size() == 0);
		
		// limites sup
		c.setDateValidation(new SimpleDateFormat("dd/MM/yyyy")
												.parse("13/07/2010"));
		errs = ContratValidator.checkDemandeDateCoherence(c);
		assertEquals("date.validation.supDateValidation",
		...
		c.setDateDemandeRedaction(new SimpleDateFormat("dd/MM/yyyy")
			.parse("13/07/2010"));
		errs = ContratValidator.checkDemandeDateCoherence(c);
		assertEquals("date.validation.supDateDemandeRedaction",
		errs.getFieldError().getCode());
		...
		c.setDateEnvoiContrat(new SimpleDateFormat("dd/MM/yyyy")
			.parse("13/07/2010"));
		errs = ContratValidator.checkDemandeDateCoherence(c);
		assertEquals("date.validation.supDateEnvoiContrat",
		errs.getFieldError().getCode());
		...
		// date de signature
		c.setDateSignature(new SimpleDateFormat("dd/MM/yyyy")
			.parse("13/07/2010"));
		errs = ContratValidator.checkDemandeDateCoherence(c);
		assertEquals("date.validation.supDateSignature",
		errs.getFieldError().getCode());
	...
	public void testValidationDateCoherence() throws ParseException {
		...
		c.setDateDemandeCession(new SimpleDateFormat("dd/MM/yyyy")
												.parse("15/07/2010"));
		errs = ContratValidator.checkDateValidation(c);
		assertEquals("date.validation.infDemandeCession",
									errs.getFieldError().getCode());
	...
	public void testDateEnvoiContratCoherence() throws ParseException {
		...
		c.setDateDemandeRedaction(new SimpleDateFormat("dd/MM/yyyy")
												.parse("15/07/2010"));
		errs = ContratValidator.checkDateEnvoiContrat(c);
		assertEquals("date.validation.infDateDemandeRedaction",
									errs.getFieldError().getCode());
	...
	public void testDateSignatureCoherence() throws ParseException {
		...
		c.setDateEnvoiContrat(new SimpleDateFormat("dd/MM/yyyy")
												.parse("15/07/2010"));
		errs = ContratValidator.checkDateSignature(c);
		assertEquals("date.validation.infDateEnvoiContrat",
									errs.getFieldError().getCode());
