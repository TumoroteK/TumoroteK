...
public class ConditTypeManagerTest extends AbstractManagerTest4 {
	...
	public void testFindByTypeLikeManager() {
		...
		List<ConditType> conditTypes = 
			conditTypeManager.findByTypeLikeManager("TUBE", true);
		assertTrue(conditTypes.size() == 1);
		//teste une recherche non exactMatch
		conditTypes = 
			conditTypeManager.findByTypeLikeManager("P", false);
		assertTrue(conditTypes.size() == 1);
		assertTrue(conditTypes.get(0).getType().equals("POUDRIER"));
		//teste une recherche infructueuse
		conditTypes = 
			conditTypeManager.findByTypeLikeManager("V", false);
		assertTrue(conditTypes.size() == 0);
		//null recherche
		conditTypes = 
			conditTypeManager.findByTypeLikeManager(null, false);
		assertTrue(conditTypes.size() == 0);
	}
	...
	public void testIsUsedObject() {
		...
		ConditType ct2 = (conditTypeManager
				.findByTypeLikeManager("POUDRIER", true)).get(0);
	...
	private void createObjectManagerTest() {
		...
		//validation test
		String[] emptyValues = new String[]{"", "  ", "¢€€lou{", 
	...
	private void updateObjectManagerTest() {
		...
		ConditType ct1 = (conditTypeManager
			.findByTypeLikeManager("PILLULIER 1.-", true)).get(0);
		ct1.setType("EPPEN-DORF");
		conditTypeManager.updateObjectManager(ct1);
		assertTrue((conditTypeManager
				.findByTypeLikeManager("EPPEN-DORF", true)).size() == 1);
		...
		assertTrue(catched);
		assertTrue((conditTypeManager
				.findByTypeLikeManager("POUDRIER", true)).size() == 1);
		
		//Validation test
		String[] emptyValues = new String[]{"", "  ", "¢¢$$anti^", 
											createOverLength(200), null};
		for (int i = 0; i < emptyValues.length; i++) {
			try {
	...
	private void removeObjectManagerTest() {
		...
		ConditType ct1 = (conditTypeManager
				.findByTypeLikeManager("EPPEN-DORF", true)).get(0);
		conditTypeManager.removeObjectManager(ct1);
		assertTrue((conditTypeManager
				.findByTypeLikeManager("EPPEN-DORF", true)).size() == 0);
...
//		assertTrue(catched);
		assertTrue((conditTypeManager
				.findByTypeLikeManager("TUBE", true)).size() > 0);
		//null remove
		conditTypeManager.removeObjectManager(null);
		assertTrue(conditTypeManager
				.findByOrderManager(ct1.getPlateforme()).size() == 1);
	}
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					conditTypeManager.findByOrderManager(pf);
