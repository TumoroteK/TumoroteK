...
public class ImprimanteManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Imprimante m = imprimanteManager.findByIdManager(1);
		assertNotNull(m);
		assertTrue(m.getNom().equals("PDF"));
		
		Imprimante mNull = imprimanteManager.findByIdManager(10);
		assertNull(mNull);
	}
	...
	public void testFindAll() {
		List<Imprimante> list = imprimanteManager.findAllObjectsManager();
	...
	public void testFindByNomAndPlateformeManager() {
		...
		List<Imprimante> list = imprimanteManager
			.findByNomAndPlateformeManager("PDF", pf1);
	...
	public void testFindByPlateformeSelectNomManager() {
		...
		List<String> list = imprimanteManager
			.findByPlateformeSelectNomManager(pf1);
		assertTrue(list.size() == 2);
	...
	public void testFindDoublon() {
		...
		Imprimante m2 = imprimanteManager.findByIdManager(2);
	...
	public void testIsUsedObjectManager() {
		Imprimante m1 = imprimanteManager.findByIdManager(1);
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Plateforme pf1 = plateformeManager.findByIdManager(1);
		ImprimanteApi ia1 = imprimanteApiManager.findByIdManager(1);
		...
		try {
			imprimanteManager.createObjectManager(i1, null, ia1);
		} catch (Exception e) {
		...
		try {
			imprimanteManager.createObjectManager(i1, pf1, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		i1.setNom("PDF");
		try {
			imprimanteManager.createObjectManager(i1, pf1, ia1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
		...
		Integer idI = i1.getImprimanteId();
		
		Imprimante mTest = imprimanteManager.findByIdManager(idI);
		assertNotNull(mTest);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		im.setPort(150);
		imprimanteManager.createObjectManager(im, pf1, ia1);
		assertTrue(imprimanteManager.findAllObjectsManager().size() == 3);
		Integer idI = im.getImprimanteId();
		
		Imprimante iUp = imprimanteManager.findByIdManager(idI);
		boolean catched = false;
		// on test l'insertion avec une pf nulle
		try {
			imprimanteManager.updateObjectManager(iUp, null, ia1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		// on test l'insertion avec un api null
		try {
			imprimanteManager.updateObjectManager(iUp, pf1, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		iUp.setNom("PDF");
		try {
			imprimanteManager.updateObjectManager(iUp, pf1, ia1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catched = true;
	...
	private void removeObjectManagerTest() throws ParseException {
		...
		affectationImprimanteManager.createObjectManager(ai, u1, b1, im, m1);
	...
	private void validationInsert(Imprimante imp) {
		Plateforme pf1 = plateformeManager.findByIdManager(1);
		ImprimanteApi ia1 = imprimanteApiManager.findByIdManager(1);
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				...
				imprimanteManager.createObjectManager(imp, pf1, ia1);
			} catch (Exception e) {
				if (e.getClass().getSimpleName().equals(
						"ValidationException")) {
					catched = true;
	...
	private void validationUpdate(Imprimante imp) {
		Plateforme pf1 = plateformeManager.findByIdManager(1);
		ImprimanteApi ia1 = imprimanteApiManager.findByIdManager(1);
		// on teste la validation du nom lors de la création
		String[] emptyValues = new String[]{null, "", "  ", "%¬ ↓»üß*d", 
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				imp.setNom(emptyValues[i]);
				imprimanteManager.updateObjectManager(imp, pf1, ia1);
			} catch (Exception e) {
				if (e.getClass().getSimpleName().equals(
						"ValidationException")) {
					catched = true;
