...
public class DossierExterneManagerTest extends AbstractManagerTest4
{
	...
	public void testFindByEmetteurInListAndIdentificationManager(){
		...
		final Emetteur e3 = emetteurDao.findById(3).orElse(null);
		emetteurs.add(e3);
	...
	public void testFirstInFirstOut() throws ParseException, NoSuchFieldException{
		...
		final Hashtable<BlocExterne, List<ValeurExterne>> valeurs = new Hashtable<>();
		valeurs.put(b3, vals1);
		dossierExterneManager.createObjectManager(de3, emetteurDao.findById(1), blocs, valeurs, 7).orElse(null);
		...
		de1.setDateOperation(date);
		dossierExterneManager.createObjectManager(de1, emetteurDao.findById(1), null, null, 7).orElse(null);
	...
	public void testFindChildrenByEmetteurValeurManager(){
		Emetteur emet = emetteurDao.findById(2).orElse(null);
		...
		dossierExterneManager.createObjectManager(der4, emetteurDao.findById(2), blocsDer4, valeursDer4Map, 2000).orElse(null);
	...
	public void testFindByEmetteurAndEntiteNullManager(){
		...
		dos = dossierExterneManager.findByEmetteurAndEntiteNullManager(emetteurDao.findById(1)).orElse(null);
		assertTrue(dos.isEmpty());
