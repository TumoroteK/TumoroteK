...
public class ObjetStatutManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		ObjetStatut objet = objetStatutManager.findByIdManager(1);
		assertNotNull(objet);
		assertTrue(objet.getStatut().equals("STOCKE"));
		
		ObjetStatut objetNull = objetStatutManager.findByIdManager(7);
	...
	public void testFindAll() {
		List<ObjetStatut> list = objetStatutManager.findAllObjectsManager();
	...
	public void testFindByStatutLikeExactManager() {
		List<ObjetStatut> list = 
			objetStatutManager.findByStatutLikeManager("EPUISE", true);
	...
	public void testFindByStatutLikeManager() {
		List<ObjetStatut> list = 
			objetStatutManager.findByStatutLikeManager("EPUISE", false);
		assertTrue(list.size() == 1);
	...
	public void testIsUsed() {
		ObjetStatut objet1 = objetStatutManager.findByIdManager(1);
		assertNotNull(objet1);
		assertTrue(objetStatutManager.isUsedObjectManager(objet1));
		
		ObjetStatut objet2 = objetStatutManager.findByIdManager(5);
	...
	public void testCrud() {
		...
		// est lancé
		try {
			objetStatutManager.createObjectManager(objet1);
		} catch (Exception e) {
		...
		String[] emptyValues = new String[]{"", "  ", null, "$$*te¤¤15|", 
				createOverLength(20)};
		...
		ObjetStatut objet2 = objetStatutManager.findByIdManager(7);
		String statutUpdated1 = "NEW";
		String statutUpdated2 = "DETRUIT";
		// On teste l'update d'un doublon pour vérifier que l'exception
		// est lancée
		objet2.setStatut(statutUpdated2);
		Boolean catchedUpdate = false;
		try {
			objetStatutManager.updateObjectManager(objet2);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catchedUpdate = true;
		...
		objetStatutManager.updateObjectManager(objet2);
		ObjetStatut objet3 = objetStatutManager.findByIdManager(7);
		assertTrue(objet3.getStatut().equals(statutUpdated1));
		
		// Test de la suppression
		ObjetStatut objet4 = objetStatutManager.findByIdManager(7);
		...
		ObjetStatut objet5 = objetStatutManager.findByIdManager(1);
