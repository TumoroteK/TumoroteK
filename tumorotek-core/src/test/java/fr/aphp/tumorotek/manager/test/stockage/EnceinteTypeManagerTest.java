...
public class EnceinteTypeManagerTest extends AbstractManagerTest4
{
   ...
   public void testFindByTypeManager(){
      List<EnceinteType> list = enceinteTypeManager.findByTypeManager("CASIER", plateformeDao.findById(1)).orElse(null);
      assertTrue(list.size() == 1);

      list = enceinteTypeManager.findByTypeManager("CASIER2", plateformeDao.findById(1)).orElse(null);
      assertTrue(list.size() == 0);

      list = enceinteTypeManager.findByTypeManager("CASIER", plateformeDao.findById(2)).orElse(null);
      assertTrue(list.size() == 0);

      list = enceinteTypeManager.findByTypeManager("CASIER", null);
      assertTrue(list.size() == 0);

      list = enceinteTypeManager.findByTypeManager(null, plateformeDao.findById(1)).orElse(null);
      assertTrue(list.size() == 0);
   ...
   public void testFindDoublon(){
      ...
      e2.setPlateforme(plateformeDao.findById(1)).orElse(null);
   ...
   private void createObjectManagerTest(){
      ...
      e1.setNom("NEW");
      e1.setPrefixe("NEW");
      e1.setPlateforme(plateformeDao.findById(1)).orElse(null);
