...
public class EnceinteTypeManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		EnceinteType et1 = enceinteTypeManager.findByIdManager(1);
		assertNotNull(et1);
		assertTrue(et1.getType().equals("CASIER"));
		
		EnceinteType csNull = enceinteTypeManager.findByIdManager(100);
		assertNull(csNull);
	}
...
//	}
	...
	public void testFindAllObjectsExceptBoiteManager() {
		...
		List<EnceinteType> list = enceinteTypeManager
			.findAllObjectsExceptBoiteManager(pf1);
	...
	public void testFindByTypeManager() {
		List<EnceinteType> list = enceinteTypeManager.findByTypeManager(
				"CASIER", plateformeDao.findById(1));
		assertTrue(list.size() == 1);
		
		list = enceinteTypeManager.findByTypeManager(
				"CASIER2", plateformeDao.findById(1));
		assertTrue(list.size() == 0);
		
		list = enceinteTypeManager.findByTypeManager(
				"CASIER", plateformeDao.findById(2));
		assertTrue(list.size() == 0);
	...
	public void testIsUsedObjectManager() {
		...
		EnceinteType e2 = enceinteTypeManager.findByIdManager(2);
	...
	private void createObjectManagerTest() {
		...
		EnceinteType e1 = new EnceinteType();
		e1.setType("NEW");
		e1.setPrefixe("NEW");
		e1.setPlateforme(plateformeDao.findById(1));
		enceinteTypeManager.createObjectManager(e1);
		assertTrue(enceinteTypeManager
				.findByOrderManager(e1.getPlateforme()).size() == 9);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		EnceinteType e1Bis = new EnceinteType();
		e1Bis.setType("NEW");
		...
		String[] prefValues = new String[]{"", "  ", null, 
				"HOO$$%", createOverLength(5), "goo"};
		EnceinteType e2 = new EnceinteType();
	...
	private void updateObjectManagerTest() {
		...
		EnceinteType e1 = enceinteTypeManager.findByIdManager(10);
		e1.setType("NEW BIS");
		enceinteTypeManager.updateObjectManager(e1);
		EnceinteType e1Bis = enceinteTypeManager.findByIdManager(10);
	...
	private void removeObjectManagerTest() {
		...
		assertTrue(enceinteTypeManager
				.findByOrderManager(e1.getPlateforme()).size() == 8);
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
			enceinteTypeManager.findByOrderManager(pf);
		assertTrue(list.size() == 8);
