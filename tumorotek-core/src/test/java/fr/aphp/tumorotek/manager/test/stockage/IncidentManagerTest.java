...
public class IncidentManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Incident cs1 = incidentManager.findByIdManager(1);
		assertNotNull(cs1);
		assertTrue(cs1.getNom().equals("COUPURE ELECTRIQUE"));
		
		Incident csNull = incidentManager.findByIdManager(10);
		assertNull(csNull);
	}
	...
	public void testFindAll() {
		List<Incident> list = incidentManager.findAllObjectsManager();
	...
	public void testFindAllObjectsByConteneurManager() {
		...
		List<Incident> list = incidentManager
			.findAllObjectsByConteneurManager(c1);
	...
	public void testFindDoublonManager() throws ParseException {
		String nom1 = "CHUTE MATERIEL";
		String nom2 = "INCIDENT";
		Date d1 = new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
			.parse("10/05/2009 10:15:00");
		Date d2 = new SimpleDateFormat("dd/MM/yyyy").parse("01/09/2009");
		Conteneur c1 = conteneurDao.findById(1);
		...
		Incident i2 = incidentManager.findByIdManager(1);
	...
	private void createObjectManagerTest() throws ParseException {
		...
		int id = i1.getIncidentId();
		
		// Vérification
		Incident iTest = incidentManager.findByIdManager(id);
		assertNotNull(iTest);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		assertTrue(incidentManager.findAllObjectsManager().size() == 6);
		int id = i.getIncidentId();
		
		Incident iUp = incidentManager.findByIdManager(id);
		...
		Date dateUp = new SimpleDateFormat("dd/MM/yyyy").parse("10/09/2009");
		...
		// Vérification
		Incident iUpTest = incidentManager.findByIdManager(id);
	 ...
	 * @throws ParseException 
	 */
	private void validationInsert(Incident incident) 
		throws ParseException {
		
		Conteneur c = conteneurDao.findById(1);
		boolean catchedInsert = false;
		// On teste une insertion avec un attribut nom non valide
		String[] emptyValues = new String[]{"", "  ", null, "%$$*g¤¤d", 
				createOverLength(50)};
		...
		Date d = new SimpleDateFormat("dd/MM/yyyy")
			.parse("10/05/2040");
		Date[] dateValues = new Date[]{null, d};
	 ...
	 * @throws ParseException 
	 */
	private void validationUpdate(Incident incident) 
		throws ParseException {
