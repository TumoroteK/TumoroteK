 ...
 */
@Transactional
public class CritereValidationImplManagerTest extends AbstractInMemoryManagerTest
{
   
   @Autowired
   private CritereValidationManager critereValidationManager;

   @Autowired
   private ChampManager champManager;

   /**
    * Test method for {@link fr.aphp.tumorotek.manager.impl.validation.workflow.CritereValidationManagerImpl#create(fr.aphp.tumorotek.model.validation.CritereValidation)}.
    */
   @Test
   public void testCreate(){

      final CritereValidation critere = new CritereValidation();
      critere.setChamp(champManager.findByIdManager(1));
      critere.setOperateur(OperateursComparaison.EGAL);

      critereValidationManager.create(critere);
   ...
   /**
    * Test method for {@link fr.aphp.tumorotek.manager.impl.validation.workflow.CritereValidationManagerImpl#findByIdManager(java.lang.Integer)}.
    */
   @Test
   public void testFindByIdManager(){

      final CritereValidation critere = critereValidationManager.findByIdManager(1);

      assertThat(critere, is(notNullValue()));
      assertThat(critere.getId(), is(equalTo(1)));
