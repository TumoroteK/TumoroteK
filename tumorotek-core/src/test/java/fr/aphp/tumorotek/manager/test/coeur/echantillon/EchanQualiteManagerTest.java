...
public class EchanQualiteManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		EchanQualite qualiteNull = echanQualiteManager.findByIdManager(4);
	...
	public void testFindByQualiteLikeExactManager() {
		List<EchanQualite> list = 
			echanQualiteManager.findByQualiteLikeManager("MELANGE MO", true);
	...
	public void testFindByQualiteLikeManager() {
		List<EchanQualite> list = 
			echanQualiteManager.findByQualiteLikeManager(
					"MELANGE MO+SG", false);
		assertTrue(list.size() == 1);
	...
	public void testIsUsed() {
		EchanQualite qualite1 = echanQualiteManager.findByIdManager(1);
		assertNotNull(qualite1);
		assertTrue(echanQualiteManager.isUsedObjectManager(qualite1));
		
		EchanQualite qualite2 = echanQualiteManager.findByIdManager(3);
	...
	public void testCrud() {
		// Test de l'insertion
		EchanQualite qualite1 = new EchanQualite();
		qualite1.setEchanQualite("MELANGE MO");
		Boolean catchedInsert = false;
		try {
			echanQualiteManager.createObjectManager(qualite1);
		} catch (Exception e) {
		...
		assertTrue(echanQualiteManager
				.findByOrderManager(qualite1.getPlateforme()).size() == 2);
		...
		EchanQualite qualite2 = echanQualiteManager.findByIdManager(4);
		String qualiteUpdated1 = "NEW";
		String qualiteUpdated2 = "MELANGE MO";
		...
		try {
			echanQualiteManager.updateObjectManager(qualite2);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catchedUpdate = true;
		...
		echanQualiteManager.updateObjectManager(qualite2);
		EchanQualite qualite3 = echanQualiteManager.findByIdManager(4);
		assertTrue(qualite3.getEchanQualite().equals(qualiteUpdated1));
		
		// Test de la suppression
		EchanQualite qualite4 = echanQualiteManager.findByIdManager(4);
...
//		assertTrue(catchedDelete);
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					echanQualiteManager.findByOrderManager(pf);
