...
public class TVGSOProcedureTest extends AbstractManagerTest {
	...
	public void setExportTVGSOManager(ExportTVGSOManagerImpl etm) {
	...
	public void testTvgsoExport() throws SQLException {
		...
		Catalogue tvgso = catalogueDao.findById(3);
		exportTVGSOManager
			.exportEchansCatalogueManager(echans, u, tvgso);
		
		assertTrue(getOperationManager()
			.findByObjetIdEntiteAndOpeTypeManager(e, 
				operationTypeDao
					.findByNom("Export TVGSO").get(0)).size() == 1);
		
		getOperationManager().removeObjectManager(getOperationManager()
			.findByObjetIdEntiteAndOpeTypeManager(e, 
				operationTypeDao
					.findByNom("%TVGSO").get(0)).get(0));
