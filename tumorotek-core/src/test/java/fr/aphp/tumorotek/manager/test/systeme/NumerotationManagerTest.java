...
public class NumerotationManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Numerotation num1 = numerotationManager.findByIdManager(1);
		assertNotNull(num1);
		assertTrue(num1.getCodeFormula().equals("PRLVT[]"));
		
		Numerotation numNull = numerotationManager.findByIdManager(10);
		assertNull(numNull);
	}
	...
	public void testFindAll() {
		List<Numerotation> list = numerotationManager.findAllObjectsManager();
	...
	public void testFindByBanquesManager() {
		...
		List<Numerotation> liste = numerotationManager
			.findByBanquesManager(banques);
	...
	public void testFindByBanqueAndEntiteManager() {
		...
		List<Numerotation> liste = numerotationManager.
		findByBanqueAndEntiteManager(b1, e2);
	...
	public void testFindByBanqueSelectEntiteManager() {
		...
		List<Entite> liste = numerotationManager.
			findByBanqueSelectEntiteManager(b1);
	...
	public void testFindDoublonManager() {
		...
		Numerotation num1 = new Numerotation();
		num1.setBanque(b1);
		num1.setEntite(e1);
		...
		Numerotation num2 = numerotationManager.findByIdManager(1);
	...
	public void testGetGeneratedCodeManager() {
		...
		assertTrue(numerotationManager
				.getGeneratedCodeManager(num).equals("PRLVT00003"));
		
		num = numerotationManager.findByIdManager(2);
		assertTrue(numerotationManager
				.getGeneratedCodeManager(num).equals("CONT00152TK"));
		
		num = numerotationManager.findByIdManager(3);
		assertTrue(numerotationManager
				.getGeneratedCodeManager(num).equals("15BK2"));
		
		assertNull(numerotationManager.getGeneratedCodeManager(null));
		
		Numerotation numTest = new Numerotation();
		assertNull(numerotationManager.getGeneratedCodeManager(numTest));
		
		numTest.setCodeFormula("TMP[]");
		assertTrue(numerotationManager
				.getGeneratedCodeManager(numTest).equals("TMP[]"));
		
		numTest.setCurrentIncrement(5);
		assertTrue(numerotationManager
				.getGeneratedCodeManager(numTest).equals("TMP5"));
		...
		assertTrue(numerotationManager
				.getGeneratedCodeManager(numTest).equals("TMP005"));
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Numerotation n1 = new Numerotation();
		...
		int id = n1.getNumerotationId();
		
		// Vérification
		Numerotation nTest = numerotationManager.findByIdManager(id);
		assertNotNull(nTest);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		assertTrue(numerotationManager.findAllObjectsManager().size() == 4);
		int id = n.getNumerotationId();
		
		Numerotation nUp = numerotationManager.findByIdManager(id);
		Boolean catched = false;
