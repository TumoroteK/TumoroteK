...
public class ModePrepaDeriveManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		ModePrepaDerive mode = modePrepaDeriveManager.findByIdManager(1);
		assertNotNull(mode);
		assertTrue(mode.getNom().equals("PREPA1_DERIVE"));
		
		ModePrepaDerive modeNull = modePrepaDeriveManager.findByIdManager(5);
		assertNull(modeNull);
	}
	
	@Test
	public void testFindByModePrepaLikeExactManager() {
		List<ModePrepaDerive> list = 
			modePrepaDeriveManager.findByModePrepaDeriveLikeManager(
					"PREPA1_DERIVE", true);
		assertTrue(list.size() == 1);
		
		list = modePrepaDeriveManager.findByModePrepaDeriveLikeManager(
				"PREPA", true);
	...
	public void testIsUsed() {
		ModePrepaDerive mode1 = modePrepaDeriveManager.findByIdManager(1);
		assertNotNull(mode1);
		assertTrue(modePrepaDeriveManager.isUsedObjectManager(mode1));
		
		ModePrepaDerive mode2 = modePrepaDeriveManager.findByIdManager(4);
		assertNotNull(mode2);
	...
	public void testCrud() {
		// Insertion
		ModePrepaDerive mode1 = new ModePrepaDerive();
		mode1.setNom("PREPA1_DERIVE");
		mode1.setNomEn("PREPA_EN1_DERIVE");
		Boolean catched = false;
		try {
			modePrepaDeriveManager.createObjectManager(mode1);
		} catch (Exception e) {
		...
		String[] nomEnValues = new String[]{"", " ", "€€¢¢¢®®®€", 
				createOverLength(25), "GOOD"};
		...
		assertTrue(modePrepaDeriveManager
				.findByOrderManager(mode1.getPlateforme()).size() == 3);
		...
		ModePrepaDerive mode2 = modePrepaDeriveManager.findByIdManager(5);
		String modeUpdated1 = "NEW";
		String modeUpdated2 = "PREPA1_DERIVE";
		...
		try {
			modePrepaDeriveManager.updateObjectManager(mode2);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catchedUpdate = true;
		...
		modePrepaDeriveManager.updateObjectManager(mode2);
		ModePrepaDerive mode3 = modePrepaDeriveManager.findByIdManager(5);
		assertTrue(mode3.getNom().equals(modeUpdated1));
		
		// Test de la suppression
		ModePrepaDerive mode4 = modePrepaDeriveManager.findByIdManager(5);
...
//		assertTrue(catchedDelete);
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					modePrepaDeriveManager.findByOrderManager(pf);
