...
public class TraitementRequeteManagerTest extends AbstractManagerTest4 {
	...
	private boolean testLyon = false;
...
//	}
	...
	public void testAdditionnerListesManagerTest() {
		...
		List<Object> prelevements1 = new ArrayList<Object>();
		List<Object> prelevements2 = new ArrayList<Object>();
		int i = 0;
		...
		List<Object> resultats = traitementRequeteManager
				.additionnerListesManager(prelevements1, prelevements2);
		
		Iterator<Prelevement> itPrelevements = getPrelevements().iterator();
		...
		resultats = traitementRequeteManager
				.additionnerListesManager(maladies, prelevements);
		List<Object> resultats2 = traitementRequeteManager
				.additionnerListesManager(prelevements, maladies);
	...
	public void testFusionnerListesManagerTest() {
		...
		List<Object> resultats = traitementRequeteManager
				.fusionnerListesManager(prelevements1, prelevements2);
		...
		resultats = traitementRequeteManager
				.fusionnerListesManager(maladies, prelevements);
		resultats2 = traitementRequeteManager
				.fusionnerListesManager(prelevements, maladies);
		assertTrue(resultats.containsAll(resultats2));
		assertTrue(resultats2.containsAll(resultats));
	}
	...
	public void testTraitementCritereManager() {
		...
		Critere critere = new Critere(new Champ(codeDerive), "like",
		codeP);
		...
		Critere critere2 = new Critere(new Champ(codeDerive), "not like",
		codeP);
		/* On traite le critère. */
		List<Object> resultats2 
				= traitementRequeteManager.traitementCritereManager(critere2,
						banques, codeP + "%", "mysql");
		/* On vérifie que la liste résultante correspond au critère. */
...
//		}
	...
	public void testCritereWithBanques() {
		...
		List<Object> resultats 
				...
				.findObjetByCritereWithBanquesManager(
						critere, banques, codeP, false);
		/* On vérifie que la liste résultante correspond au critère. */
	...
	public void testMultiParents() {
		...
		ChampEntite codePrlvt = champEntiteDao.findById(23);
		...
		ChampEntite champEntiteEchantillons = champEntiteDao.findById(224);
		Champ parentToQueryEchantillon = new Champ(champEntiteEchantillons);
		parentToQueryEchantillon.setChampParent(parentToQueryPrlvt);
		
		ChampEntite codeEchantillon = champEntiteDao.findById(54);
	...
	public void testFindIdsFromEntite() {
		...
		ChampEntite idP = champEntiteDao.findById(1);
		...
		List<Integer> idsPat = new ArrayList<Integer>();
		idsPat.add(1); idsPat.add(2); idsPat.add(3); idsPat.add(4);
		
		/* On traite le critère. */
		List<Object> resultats 
				= traitementQueryManager
				.findObjetByCritereWithBanquesManager(
						critere, banques, idsPat, true);
		/* On vérifie que la liste résultante correspond au critère. */
		assertEquals(2, resultats.size());
		...
		ChampEntite idPrlvt = champEntiteDao.findById(21);
		champ = new Champ(idPrlvt);
		champ.setChampParent(parentToQueryPrlvt);
		critere = new Critere(champ, "in", "");
		
		List<Integer> idsPrel = new ArrayList<Integer>();
		idsPrel.add(1); idsPrel.add(2); idsPrel.add(3); idsPrel.add(4);
		
		
		/* On traite le critère. */
		resultats = traitementQueryManager
				.findObjetByCritereWithBanquesManager(
						critere, banques, idsPrel, true);
		/* On vérifie que la liste résultante correspond au critère. */
		assertTrue(resultats.size() == 2);
		assertTrue(resultats.contains(1));
		...
		ChampEntite idE = champEntiteDao.findById(50);
		champ = new Champ(idE);
		champ.setChampParent(parentToQueryEchantillon);
		critere = new Critere(champ, "in", "");
		
		List<Integer> idsEchan = new ArrayList<Integer>();
		idsEchan.add(1); idsEchan.add(2); idsEchan.add(3); idsEchan.add(4);
		
		/* On traite le critère. */
		resultats = traitementQueryManager
				.findObjetByCritereWithBanquesManager(
						critere, banques, idsEchan, true);
		/* On vérifie que la liste résultante correspond au critère. */
		assertTrue(resultats.size() == 2);
		assertTrue(resultats.contains(3));
		assertTrue(resultats.contains(1));
	...
	public void testFindIdsFromEntiteDeriveVersion() {
		...
		// création du parent pour accéder aux échantillons
		ChampEntite champEntiteEchans = champEntiteDao.findById(226);
		Champ parentToQueryEchans = new Champ(champEntiteEchans);
		...
		List<Object> resultats 
				...
				.findObjetByCritereWithBanquesDeriveVersionManager(
						critere, banques, idsPat, true, true);
		/* On vérifie que la liste résultante correspond au critère. */
		assertTrue(resultats.size() == 1);
		assertTrue(resultats.contains(1));
		// assertTrue(resultats.contains(2));
		...
		resultats = traitementQueryManager
				.findObjetByCritereWithBanquesDeriveVersionManager(
						critere, banques, null, true, true);
		assertTrue(resultats.isEmpty());
		...
		parentToQueryPrlvt.setChampParent(parentToQueryMaladie);
		ChampEntite champEntiteEchantillons = champEntiteDao.findById(224);
		Champ parentToQueryEchan = new Champ(champEntiteEchantillons);
		parentToQueryEchan.setChampParent(parentToQueryPrlvt);
		ChampEntite champEntiteDerives = champEntiteDao.findById(226);
		Champ parentToQueryDerive = new Champ(champEntiteDerives);
		parentToQueryDerive.setChampParent(parentToQueryEchan);
	
		
		ChampEntite idDer = champEntiteDao.findById(76);
		champ = new Champ(idDer);
		champ.setChampParent(parentToQueryDerive);
		critere = new Critere(champ, "in", "");
		
		List<Integer> idsDer = new ArrayList<Integer>();
		idsDer.add(1); 
		idsDer.add(2); idsDer.add(4);
		idsDer.add(3); 
		
		resultats = traitementQueryManager
				.findObjetByCritereWithBanquesDeriveVersionManager(
						critere, banques, idsDer, false, true);
		assertTrue(resultats.size() == 2);
		assertTrue(resultats.contains(3));
		assertTrue(resultats.contains(1));
		
		// teste voie par prélèvement);
		ChampEntite champEntiteDerives2 = champEntiteDao.findById(225);
		Champ parentToQueryDerive2 = new Champ(champEntiteDerives2);
	...
	public void testMultiCriteres() {
		...
		Champ champ = new Champ(nomPatient);
		champ.setChampParent(parentToQueryPatient);
		Critere critere1 = new Critere(champ, "like", "DEL" + "%");
		// critere code prelevement
		ChampEntite codePrlvt = champEntiteDao.findById(23);
		champ = new Champ(codePrlvt);
		champ.setChampParent(null);
		Critere critere2 = new Critere(champ, "like", "PRLV" + "%");
		...
		champ = new Champ(codeEchantillon);
		champ.setChampParent(parentToQueryEchantillon);
		Critere critere3 = new Critere(champ, "like", "PT" + "%");
		
		// critere sur une maladie
		ChampEntite codeMaladie = champEntiteDao.findById(18);
		champ = new Champ(codeMaladie);
		champ.setChampParent(parentToQueryOneMaladie);
		Critere critere4 = new Critere(champ, "like", "C45" + "%");
		
		// critere sur un champannotation
		ChampAnnotation ca = champAnnotationDao.findById(9);
		champ = new Champ(ca);
		champ.setChampParent(parentToQueryEchantillon);
		Critere critereA = new Critere(champ, "=", "textVal1");
		...
		champ = new Champ(nomPatient);
		champ.setChampParent(parentToQueryPatient);
		Critere critere5 = new Critere(champ, "like", "DEL" + "%");
		criteres.clear();
		...
		champ = new Champ(codePrlvt);
		champ.setChampParent(parentToQueryPrlvt);
		Critere critere6 = new Critere(champ, "like", "PRLV" + "%");
		criteres.clear();
		...
		champ = new Champ(nomPatient);
		banques.add(b1);
		Critere critere7 = new Critere(champ, "like", "DEL" + "%");
		criteres.clear();
		...
		parentToQueryEchantillon.setChampParent(parentToQueryPrlvt);
		champ.setChampParent(parentToQueryEchantillon);
		Critere critere8 = new Critere(champ, "like", "PT" + "%");
		criteres.clear();
	...
	public void testNumCritereAnnot() {
		...
		// critere sur un champannotation
		ChampAnnotation ca = champAnnotationDao.findById(7);
		Champ champ = new Champ(ca);
		champ.setChampParent(parentToQueryEchantillon);
		Critere critere1 = new Critere(champ, ">", "textVal1");
	...
	public void testProdDeriveQueryWithCriteresList() {
		...
		champ = new Champ(codeDerive);
		champ.setChampParent(parentToQueryDerives);
		Critere critere3 = new Critere(champ, "like", "PTR" + "%");
		ChampEntite laboDerive = champEntiteDao.findById(80);
		champ = new Champ(laboDerive);
		champ.setChampParent(parentToQueryDerives);
		Critere critere4 = new Critere(champ, "like", "PTR" + "%");
		criteres.clear();
		values.clear();
		criteres.add(critere3);
		criteres.add(critere4);
...
//		groupementManager.createObjectManager(groupement, groupement
...
//		
//		/* On enregistre le groupement racine pour pouvoir l'explorer. */
//		groupementManager.createObjectManager(groupement3, groupement3
...
//		
//		/* On enregistre le groupement racine pour pouvoir l'explorer. */
//		groupementManager.createObjectManager(groupement4, groupement4
...
//
//		/* On enregistre le groupement racine pour pouvoir l'explorer. */
//		groupementManager.createObjectManager(groupement5, groupement5
...
//			
//			/* On enregistre le groupement racine pour pouvoir l'explorer. */
//			groupementManager.createObjectManager(groupement7, groupement7
...
//	}
	...
	public void testFindPrelevementsByNbEchantillonsWithBanquesManager() {
		...
		List<Integer> liste = traitementQueryManager
			.findPrelevementsByNbEchantillonsWithBanquesManager(
					"=", 2, banques);
		assertTrue(liste.size() == 0);
		
		liste = traitementQueryManager
			.findPrelevementsByNbEchantillonsWithBanquesManager(
					"=", 1, banques);
	...
	public void testCodesInList() {
		...
		ChampEntite champEntiteCodeOrganes = champEntiteDao.findById(241);
		Champ parentToQueryOrganes = new Champ(champEntiteCodeOrganes);
		parentToQueryOrganes.setChampParent(parentToQueryEchantillon);
		
		ChampEntite codeOrgane = champEntiteDao.findById(125);
		Champ champ = new Champ(codeOrgane);
		champ.setChampParent(parentToQueryOrganes);
		
		Critere critere = new Critere(champ, "like", "PT" + "%");
		/* On traite le critère. */
		List<Integer> resultats = traitementQueryManager
				.findObjetByCritereOnCodesWithBanquesManager(
						critere, banques, values, values, "BL",
						false);
		/* On vérifie que la liste résultante correspond au critère. */
		assertTrue(resultats.size() == 1);
		...
		ChampEntite champEntiteCodeMorphos = champEntiteDao.findById(241);
		Champ parentToQueryMorphos = new Champ(champEntiteCodeMorphos);
		parentToQueryMorphos.setChampParent(parentToQueryEchantillon);
		
		ChampEntite codeMorpho = champEntiteDao.findById(231);
		...
		resultats = traitementQueryManager
				.findObjetByCritereOnCodesWithBanquesManager(
						critere, banques, values, values, "K14%",
						true);
		/* On vérifie que la liste résultante correspond au critère. */
		assertTrue(resultats.size() == 1);
	}
	...
	public void testFindEchantillonsByRequeteBiocapManager() throws ParseException {
		...
		List<Banque> banques = new ArrayList<Banque>();
		banques.add(b2);
		Service s1 = serviceDao.findById(1);
		List<Service> services = new ArrayList<Service>();
		services.add(s1);
		
		Calendar dateInf = Calendar.getInstance();
		dateInf.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
							.parse("17/09/1983 00:00:00"));
		Calendar dateSup = Calendar.getInstance();
		dateSup.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
			.parse("20/09/1983 00:00:00"));
		
		// Requête ok
		List<Integer> liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager(
					"mysql", banques, services, dateInf, dateSup, 12, null);
		assertTrue(liste.size() == 1);
		...
		liste = traitementQueryManager
				.findEchantillonsByRequeteBiocapManager(
						"mysql", banques, services, dateInf, dateSup, 12, 
		...
		// Erreur sur l'age
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, services, dateInf, dateSup, 11, null);
		assertTrue(liste.size() == 0);
		
		// Erreur sur la date inf
		dateInf.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
			.parse("18/09/1983 00:00:00"));
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, services, dateInf, dateSup, 18, null);
		assertTrue(liste.size() == 0);
		
		// Erreur sur la date sup
		dateInf.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
			.parse("10/09/1983 00:00:00"));
		dateSup.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
			.parse("15/09/1983 00:00:00"));
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, services, dateInf, dateSup, 18, null);
		assertTrue(liste.size() == 0);
		...
		banques.add(banqueDao.findById(2));
		 liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, services, dateInf, dateSup, 12, objetStatutDao.findById(3));
		assertTrue(liste.size() == 1);
		
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, services, dateInf, dateSup, null, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, services, dateInf, dateSup, 0, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
	
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					null, services, dateInf, dateSup, 18, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					new ArrayList<Banque>(), services, 
					dateInf, dateSup, 18, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
		
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, null, dateInf, dateSup, 18, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
		banques = new ArrayList<Banque>();
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, new ArrayList<Service>(), 
					dateInf, dateSup, 18, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
		
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
					banques, services, null, dateSup, 18, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
		
		liste = traitementQueryManager
			.findEchantillonsByRequeteBiocapManager("mysql",
				banques, services, dateInf, null, 18, objetStatutDao.findById(3));
		assertTrue(liste.size() == 0);
	}
	...
	public void testFindPrelevementsByMedecinsManager() {
		...
		List<Integer> liste = traitementQueryManager
			.findPrelevementsByMedecinsManager(c, banques);
		...
		// mauvais paramétrage
		liste = traitementQueryManager
			.findPrelevementsByMedecinsManager(null, banques);
		assertTrue(liste.size() == 0);
		liste = traitementQueryManager
			.findPrelevementsByMedecinsManager(c, null);
		assertTrue(liste.size() == 0);
		banques = new ArrayList<Banque>();
		liste = traitementQueryManager
			.findPrelevementsByMedecinsManager(c, banques);
	...
	public void testFindObjetIdsFromNonConformiteNomManager() {
		...
		ConformiteType c2 = conformiteTypeDao.findById(2);
		List<Banque> banks = new ArrayList<Banque>();
		
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Prob", c2, pf1, banks);
		assertEquals(1, ids.size());
		assertTrue(ids.contains(3));
		
		banks.add(banqueDao.findById(2));
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Problème", c2, pf1, banks);
		assertTrue(ids.isEmpty());
		
		banks.add(banqueDao.findById(1));
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Problème", c2, pf1, null);
		assertTrue(ids.size() == 1);
		assertTrue(ids.contains(3));
		
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Stérile", c2, pf1, banks);
		assertTrue(ids.isEmpty());
		
		ConformiteType c3 = conformiteTypeDao.findById(3);
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Non", c3, pf1, banks);
		assertTrue(ids.size() == 2);
		assertTrue(ids.contains(3));
		assertTrue(ids.contains(2));
		
		ConformiteType c5 = conformiteTypeDao.findById(5);
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Non", c5, pf1, banks);
		assertTrue(ids.size() == 1);
		assertTrue(ids.contains(3));
		
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Non", c2, pf1, banks);
		assertTrue(ids.isEmpty());
		
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("", c2, pf1, banks);
		assertTrue(ids.isEmpty());
		
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager(null, c2, pf1, banks);
		assertTrue(ids.isEmpty());
		
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Problème", null, pf1, banks);
		assertTrue(ids.isEmpty());
		
		ids = traitementQueryManager
				.findObjetIdsFromNonConformiteNomManager("Problème", c2, null, banks);
		assertTrue(ids.isEmpty());
	}
	...
	public void testNoConfCritereWithBanques() {
		...
		ChampEntite raisonNc = champEntiteDao.findById(257);
		Banque b1 = banqueDao.findById(1);
		List<Banque> banques = new ArrayList<Banque>();
		banques.add(b1);
		
		Critere critere = new Critere(new Champ(raisonNc), "like", ncf);
		/* On traite le critère. */
		List<Object> resultats 
				= traitementQueryManager
				.findObjetByCritereManager(
						critere, banques, ncf, "mysql");
		/* On vérifie que la liste résultante correspond au critère. */
		assertTrue(resultats.size() == 1);
		...
		resultats = traitementQueryManager
				.findObjetByCritereManager(
					critere, banques, "erreuzzzr%", "mysql");
			assertTrue(resultats.size() == 0);
	}
	...
	public void testTempStockCritereWithBanques() {
		...
		ChampEntite tempStockE = champEntiteDao.findById(265);
		Banque b1 = banqueDao.findById(1);
		List<Banque> banques = new ArrayList<Banque>();
		banques.add(b1);
		...
		ChampEntite tempStockD = champEntiteDao.findById(266);
