...
public class PrelevementTypeManagerTest extends AbstractManagerTest4 {
	...
	public void testFindByTypeLikeManager() {
		...
		List<PrelevementType> prelevementTypes = 
			prelevementTypeManager.findByTypeLikeManager("BIOPSIE", true);
		assertTrue(prelevementTypes.size() == 1);
		//teste une recherche non exactMatch
		prelevementTypes = 
			prelevementTypeManager.findByTypeLikeManager("PONC", false);
		assertTrue(prelevementTypes.size() == 1);
		//teste une recherche infructueuse
		prelevementTypes = 
			prelevementTypeManager.findByTypeLikeManager("PRISE", false);
		assertTrue(prelevementTypes.size() == 0);
		//null recherche
		prelevementTypes = 
			prelevementTypeManager.findByTypeLikeManager(null, false);
		assertTrue(prelevementTypes.size() == 0);
	}
	...
	public void testFindByIncaCatManager() {
		...
		List<PrelevementType> prelevementTypes = 
			prelevementTypeManager.findByIncaCatManager("P");
		assertTrue(prelevementTypes.size() == 2);
		//teste une recherche infructueuse
		prelevementTypes = 
			prelevementTypeManager.findByIncaCatManager("C");
		assertTrue(prelevementTypes.size() == 0);
		//null recherche
		prelevementTypes = 
			prelevementTypeManager.findByIncaCatManager(null);
		assertTrue(prelevementTypes.size() == 0);
	}
	...
	public void testIsUsedObject() {
		...
		PrelevementType ct2 = (prelevementTypeManager
				.findByTypeLikeManager("NECROPSIE", true)).get(0);
	...
	private void createObjectManagerTest() {
		...
		assertTrue(catched);
		assertTrue((prelevementTypeManager
				.findByTypeLikeManager("PRISE%", false)).size() == 1);
		assertTrue((prelevementTypeManager
				.findByIncaCatManager("S")).size() == 1);
		
		//validation test Type
		String[] typeValues = new String[]{"", "  ", null, "*$$¢**", 
										createOverLength(200), "goodType"};
		String[] incaCatValues = new String[]{"", " ", null, "EEE", "goodCat"};
		PrelevementType pt2 = new PrelevementType();
		pt2.setPlateforme(pt1.getPlateforme());
		for (int i = 0; i < typeValues.length; i++) {
			for (int j = 0; j < incaCatValues.length; j++) {
				try {
					...
					assertTrue(prelevementTypeManager
						.findByOrderManager(pt1.getPlateforme()).size() == 4);
				}
			}
		}
	}
	...
	private void updateObjectManagerTest() {
		...
		PrelevementType pt1 = (prelevementTypeManager
			.findByTypeLikeManager("PRISE DE SANG.", true)).get(0);
		pt1.setType("Èré+");
		pt1.setIncaCat("F");
		prelevementTypeManager.updateObjectManager(pt1);
		assertTrue((prelevementTypeManager
				.findByTypeLikeManager("Èré+", true))
				.get(0).getIncaCat().equals("F"));
		...
		assertTrue(catched);
		assertTrue((prelevementTypeManager
				.findByTypeLikeManager("BIOPSIE", true)).size() == 1);
	...
	private void removeObjectManagerTest() {
		...
		PrelevementType pt1 = (prelevementTypeManager
				.findByTypeLikeManager("Èré+", true)).get(0);
		prelevementTypeManager.removeObjectManager(pt1);
		assertTrue((prelevementTypeManager
				.findByTypeLikeManager("Èré+", true)).size() == 0);
...
//		assertTrue(catched);
		assertTrue((prelevementTypeManager
				.findByTypeLikeManager("BIOPSIE", true)).size() > 0);
		//null remove
		prelevementTypeManager.removeObjectManager(null);
		assertTrue(prelevementTypeManager
				.findByOrderManager(pt1.getPlateforme()).size() == 3);
	}
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					prelevementTypeManager.findByOrderManager(pf);
