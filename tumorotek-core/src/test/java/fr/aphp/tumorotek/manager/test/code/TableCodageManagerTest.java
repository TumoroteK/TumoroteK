...
public class TableCodageManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<TableCodage> tables =  tableCodageManager.findAllObjectsManager();
	...
	public void testFindByNomManager() {
		List<TableCodage> tables = 
								tableCodageManager.findByNomManager("ADICAP");
	...
	public void testfindCodeByTableCodageAndIdManager() {
		...
		CodeCommon a1 = 
			commonUtilsManager.findCodeByTableCodageAndIdManager(4, t1);
		assertNotNull(a1);
		...
		CodeCommon c1 = commonUtilsManager
							.findCodeByTableCodageAndIdManager(7, t1);
		assertNotNull(c1);
		...
		CodeCommon c2 = commonUtilsManager
							.findCodeByTableCodageAndIdManager(29, t1);
		assertNotNull(c2);
		...
		CodeCommon c3 = commonUtilsManager
							.findCodeByTableCodageAndIdManager(14, t1); 
		assertNull(c3);
	...
	public void testGetTableCodageFromCodeCommon() {
		TableCodage t1 = tableCodageManager.findByNomManager("ADICAP").get(0);
		CodeCommon c = commonUtilsManager
								.findCodeByTableCodageAndIdManager(4, t1);
		
		t1 = tableCodageManager.findByNomManager("CIM_MASTER").get(0);
		c = commonUtilsManager.findCodeByTableCodageAndIdManager(7, t1);
		assertTrue(commonUtilsManager
							.getTableCodageFromCodeCommonManager(c).equals(t1));
	...
	public void testTranscodeManager() {
		...
		TableCodage adicap = tableCodageManager
										.findByNomManager("ADICAP").get(0);
		TableCodage cim = tableCodageManager
									.findByNomManager("CIM_MASTER").get(0);
		TableCodage cimo = tableCodageManager
									.findByNomManager("CIMO_MORPHO").get(0);
		TableCodage user = tableCodageManager
		...
		Adicap a = (Adicap) adicapManager
							.findByCodeLikeManager("OR!TE", true).get(0);
		...
		a = (Adicap) adicapManager
							.findByCodeLikeManager("E0G8", true).get(0);
		transcodes = tableCodageManager.transcodeManager(a, tables, banks);
		assertTrue(transcodes.size() == 1);
		...
		tables.add(adicap);
		CimMaster cm = (CimMaster) cimMasterManager
						.findByCodeLikeManager("C02.9", true).get(0);
		transcodes = tableCodageManager.transcodeManager(cm, tables, banks);
		assertTrue(transcodes.size() == 4);
		
		cm = (CimMaster) cimMasterManager
								.findByCodeLikeManager("A01.4", true).get(0);
		transcodes = tableCodageManager.transcodeManager(cm, tables, banks);
		assertTrue(transcodes.size() == 1);
		assertTrue(transcodes.get(0).getCode().equals("code2"));
		...
		CimoMorpho co = (CimoMorpho) cimoMorphoManager
						.findByCodeLikeManager("M-8031/3", true).get(0);
		transcodes = tableCodageManager.transcodeManager(co, tables, null);
		assertTrue(transcodes.size() == 2);
		
		co = (CimoMorpho) cimoMorphoManager
						.findByCodeLikeManager("D0-20150", true).get(0);
		...
		CodeUtilisateur cu = codeUtilisateurManager
						.findByCodeLikeManager("code1", true, banks).get(0);
	...
	public void testFindCodesAndTranscodesFromStringManager() {
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("C02.9", 
					tables, banks, false);
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("langue, sans précision", 
													tables, banks, false);
...
//		assertTrue(codes.size() == 0);
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("OR!TE", tables, banks, 
																	false);
		assertTrue(codes.size() == 2);
		assertTrue(codes.get(1).getCode().equals("C30.1"));
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("TROMPE D'EUSTACHE", 
													tables, banks, false);
		assertTrue(codes.size() == 10);
		//assertTrue(codes.get(1).getCode().equals("C30.1"));
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("TROMPE", 
													tables, banks, false);
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("H0V2", 
												tables, banks, false);
		assertTrue(codes.size() == 2);
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("M-8031/3", tables, 
															banks, false);
		assertTrue(codes.size() == 3);
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("Carcinome à "
					+ "cellules géantes", tables, banks, false);
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("D0-20150", 
											tables, banks, false);
		assertTrue(codes.size() == 2);
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("code1", tables, 
															banks, false);
		assertTrue(codes.size() == 0);
		...
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("libelle1", tables, banks, 
																		false);
		assertTrue(codes.size() == 7);
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("libelle", tables, banks, 
																		false);
		assertTrue(codes.size() == 8);
		
		codes = tableCodageManager
			.findCodesAndTranscodesFromStringManager("libelle1", 
												null, banks, true);
		assertTrue(codes.size() == 0);
	}
	...
	public void testGetListCodesFromCodeCommon() {
		CodeCommon c1 = new Adicap();
		c1.setCode("c1");
		c1.setLibelle("lib1");
		CodeCommon c2 = new CimMaster();
		c2.setCode("c2");
		c2.setLibelle(null);
		CodeCommon c3 = new CimoMorpho();
		c3.setCode("c3");
		c3.setLibelle("lib3");
		CodeCommon c4 = new CodeUtilisateur();
		c4.setCode(null);
		c4.setLibelle("lib4");
		List<CodeCommon> codes = new ArrayList<CodeCommon>();
		codes.add(c1); codes.add(c2); codes.add(c3); codes.add(c4);
		
		List<String> codesStr = tableCodageManager
									.getListCodesFromCodeCommon(codes);
	...
	public void testGetListLibellesFromCodeCommon() {
		...
		List<String> libStr = tableCodageManager
									.getListLibellesFromCodeCommon(codes);
