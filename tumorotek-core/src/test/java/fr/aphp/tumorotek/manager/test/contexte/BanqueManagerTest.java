...
public class BanqueManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Banque banqueNull = banqueManager.findByIdManager(5);
	...
	public void testFindAll() {
		List<Banque> list = banqueManager.findAllObjectsManager();
	...
	public void testGetPrelevementsManager() {
		...
		Set<Prelevement> list = 
			banqueManager.getPrelevementsManager(banque1);
		...
		assertTrue(banqueManager.getPrelevementsManager(null)
				.size() == 0);
	}
	...
	public void testGetEchantillonsManager() {
		...
		assertTrue(banqueManager.getEchantillonsManager(null)
				.size() == 0);
	}
	...
	public void testGetProdDerivesManager() {
		...
		assertTrue(banqueManager.getProdDerivesManager(null)
				.size() == 0);
	}
	...
	public void testGetServicesStockageManager() {
		Banque banque1 = banqueManager.findByIdManager(1);
		assertNotNull(banque1);
		Set<Service> list = banqueManager
			.getServicesStockageManager(banque1);
		assertTrue(list.size() == 1);
		...
		assertTrue(banqueManager.getServicesStockageManager(null)
				.size() == 0);
	}
	...
	public void testFindByEntiteConsultByUtilisateurManager() {
		...
		Plateforme pf = plateformeDao.findById(1);
		Entite prel = entiteDao.findById(2);
		List<Banque> banks = banqueManager
			.findByEntiteConsultByUtilisateurManager(utilisateur, prel, pf);
		...
		banks = banqueManager
			.findByEntiteModifByUtilisateurManager(utilisateurDao
				.findById(2), prel, pf);
		assertTrue(banks.size() == 2);
		...
		banks = banqueManager
			.findByEntiteModifByUtilisateurManager(utilisateur, prel, pf);
		...
		Entite der = entiteDao.findById(8);
		banks = banqueManager
		.findByEntiteModifByUtilisateurManager(utilisateur, der, pf);
		assertTrue(banks.size() == 2);
		...
		banks = banqueManager
			.findByEntiteConsultByUtilisateurManager(utilisateurDao
													.findById(4), prel, pf);
		assertTrue(banks.size() == 0);
	}
	...
	public void testFindByAutoriseCrossPatientManager() {
		List<Banque> banks = banqueManager
			.findByAutoriseCrossPatientManager(true);
		assertTrue(banks.size() == 2);
		assertTrue(banks.contains(banqueManager.findByIdManager(1)));
		assertTrue(banks.contains(banqueManager.findByIdManager(4)));
		
		banks = banqueManager
			.findByAutoriseCrossPatientManager(false);
		assertTrue(banks.size() == 2);
	}
	...
	public void testFindByUtilisateurIsAdminManager() {
		...
		Plateforme pf = plateformeDao.findById(1);
		List<Banque> banks = banqueManager
				.findByUtilisateurIsAdminManager(utilisateur, pf);
		assertTrue(banks.size() == 2);
		assertTrue(banks.contains(banqueManager.findByIdManager(1)));
		assertTrue(banks.contains(banqueManager.findByIdManager(2)));
		banks = banqueManager
			.findByUtilisateurIsAdminManager(utilisateurDao.findById(3), pf);
		assertTrue(banks.size() == 1);
	...
	public void testFindByUtilisateurAndPFManager() {
		...
		List<Banque> banks = banqueManager
							.findByUtilisateurAndPFManager(utilisateur, pf1);
		assertTrue(banks.size() == 2);
		assertTrue(banks.contains(banqueManager.findByIdManager(1)));
		assertTrue(banks.contains(banqueManager.findByIdManager(2)));
	...
	public void testValidation() {
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.nom.illegal"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.nom.tooLong"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
		// identification
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.identification.empty"));
		}	
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.identification.illegal"));
		}	
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.identification.tooLong"));
		}
		// description
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.description.empty"));
		}	
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.description.tooLong"));
		}	
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.defMaladies.empty"));
		}	
		// libelle defaut
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.defautMaladie.empty"));
		}	
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.defautMaladie.illegal"));
		}	
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("banque.defautMaladie.tooLong"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
						.getCode().equals("banque.defautMaladie.noDefMaladie"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
	...
	public void testCRUD() throws ParseException {
		Banque b = createObjectManagerTest();
		b = updateObjectManagerTest(b);
		removeObjectManagerTest(b);
	}
	
	private Banque createObjectManagerTest() {
		Banque b = new Banque();
		Utilisateur u = utilisateurDao.findById(1);
		
		Plateforme pf = plateformeDao.findById(1);
		Contexte ct = contexteDao.findById(1);
		Set<Catalogue> cats = new HashSet<Catalogue>();
		cats.add(catalogueDao.findById(1));
		cats.add(catalogueDao.findById(2));
		
		//required plateforme
		try {
			banqueManager.createOrUpdateObjectManager(b, null, null, null, 
					null, null, null, null, null, null, null, null, null, 
					null, null, null, u, "creation", null);
		} catch (RequiredObjectIsNullException re) {
			assertTrue(re.getMessage().equals("Banque: Plateforme est "
					+ "null lors de l'opération de creation"));
		}
		
		//required contexte
		try {
			banqueManager.createOrUpdateObjectManager(b, pf, null, null, 
					null, null, null, null, null, null, null, null, null, 
					null, null, null, u, "creation", null);
		} catch (RequiredObjectIsNullException re) {
			assertTrue(re.getMessage().equals("Banque: Contexte est "
					+ "null lors de l'opération de creation"));
		}
		
		//validation
		try {
			banqueManager.createOrUpdateObjectManager(b, pf, ct, null, 
					null, null, null, null, null, null, null, null, null, 
					null, null, null, u, "creation", null);
		} catch (ValidationException ve) {
		...
		Collaborateur co = collaborateurDao.findById(3);
		Couleur c1 = couleurDao.findById(3);
		Couleur c2 = couleurDao.findById(4);
		...
		try {
			banqueManager.createOrUpdateObjectManager(b, pf, ct, null, 
					null, null, null, null, null, null, null, null, null, 
					null, null, null, u, "jiggs", null);
		} catch (IllegalArgumentException ie) {
			assertTrue(ie.getMessage().equals("Operation must match "
										+ "'creation/modification' values"));
		}
		assertTrue(banqueManager
			.findByPlateformeAndArchiveManager(plateformeDao.findById(1), null).size() == 3);

		//insertion
		banqueManager.createOrUpdateObjectManager(b, pf, ct, s, c, co, 
				null, null, null,
				null, null, null, null, null, c1, c2, u, "creation", "/tmp/");
		assertTrue(banqueManager.findAllObjectsManager().size() == 5);
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
				.getPlateforme().equals(pf));
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
				.getContexte().equals(ct));
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
				.getNom().equals("Credit Agricole"));
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
				.getIdentification().equals("CA"));	
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
				.getDescription().equals("La banque des paysans"));	
		assertNull(banqueManager.findByIdManager(b.getBanqueId())
				.getAutoriseCrossPatient());
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
				.getDefMaladies());	
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
											.getProprietaire().equals(s));	
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
										.getCollaborateur().equals(c));	
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
				.getContact().equals(co));	
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
									.getEchantillonCouleur().equals(c1));
		assertTrue(banqueManager.findByIdManager(b.getBanqueId())
									.getProdDeriveCouleur().equals(c2));	
		assertTrue(getOperationManager().findByObjectManager(b).size() == 1);
		assertTrue(getOperationManager().findByObjectManager(b).get(0)
	...
	private Banque updateObjectManagerTest(Banque bank) {
		// recupere la derniere banque inseree
		Banque b2 = banqueManager
			.findByIdManager(bank.getBanqueId());
		...
		try {
			b2.setNom("BANQUE2");
			banqueManager.createOrUpdateObjectManager(b2, null, null, 
					null, null, null, null, null, null, null, null, null, 
		...
		try {
			banqueManager.createOrUpdateObjectManager(b2, null, null,
					null, null, null, null, 
					null, null, null, null, null, null, null, null, null, u, 
														"modification", null);
		...
		banqueManager.createOrUpdateObjectManager(b2, null, null, null, 
				null, null, null, 
				null, null, null, null, null, null, null, null, null, u, 
													"modification", null);
		
		assertTrue(banqueManager.findAllObjectsManager().size() == 5);
		assertTrue(banqueManager.findByIdManager(b2.getBanqueId())
				.getNom().equals("BNP Parisbas"));
		assertNotNull(banqueManager.findByIdManager(b2.getBanqueId())
				.getContexte());
		assertTrue(banqueManager.findByIdManager(b2.getBanqueId())
				.getIdentification().equals("CA"));	
		assertTrue(banqueManager.findByIdManager(b2.getBanqueId())
		...
		assertTrue(banqueManager.findByIdManager(b2.getBanqueId())
				.getDefMaladies());	
		assertTrue(banqueManager.findByIdManager(b2.getBanqueId())
				.getDefautMaladie().equals("Voleurs"));	
		assertTrue(banqueManager.findByIdManager(b2.getBanqueId())
				.getDefautMaladieCode().equals("05045"));	
		assertNull(banqueManager.findByIdManager(b2.getBanqueId())
									.getProprietaire());	
		assertNull(banqueManager.findByIdManager(b2.getBanqueId())
										.getCollaborateur());	
		assertNull(banqueManager.findByIdManager(b2.getBanqueId())
									.getEchantillonCouleur());
		assertNull(banqueManager.findByIdManager(b2.getBanqueId())
									.getProdDeriveCouleur());	
		assertTrue(getOperationManager().findByObjectManager(b2).size() == 2);
		assertTrue(getOperationManager().findByObjectManager(b2).get(1)
										.getOperationType().getNom()
												.equals("Modification"));
	...
	private void removeObjectManagerTest(Banque bank) {
		Utilisateur u = utilisateurDao.findById(1);
		Banque b = banqueManager
			.findByIdManager(bank.getBanqueId());
	...
	public void testCreateUpdateConteneurs() {
		...
		List<Conteneur> conts = new ArrayList<Conteneur>();
		Conteneur c1 = conteneurDao.findById(1);
		Conteneur c2 = conteneurDao.findById(2);
		Conteneur c3 = conteneurDao.findById(3);
		conts.add(c1);
		conts.add(c2);
		conts.add(c3);
		
		List<BanqueTableCodage> tabs = new ArrayList<BanqueTableCodage>();
		List<TableAnnotation> tabsPat = new ArrayList<TableAnnotation>();
		List<TableAnnotation> tabsPrel = new ArrayList<TableAnnotation>();
		List<TableAnnotation> tabsEchan = new ArrayList<TableAnnotation>();
		List<TableAnnotation> tabsCess = new ArrayList<TableAnnotation>();
		List<CouleurEntiteType> couls = new ArrayList<CouleurEntiteType>();
		
		banqueManager.createOrUpdateObjectManager(b, pf, ct, null, null,
				null,
				conts, tabs, tabsPat, tabsPrel, tabsEchan, null, tabsCess, 
				couls, null, null, u, "creation", "/tmp/");
		...
		Enceinte e6 = enceinteManager.findByIdManager(6);
		Set<Banque> banske6 = enceinteManager.getBanquesManager(e6);
		banske6.add(b);
		enceinteManager.updateObjectManager(e6, e6.getEnceinteType(), 
				e6.getConteneur(), e6.getEnceintePere(), e6.getEntite(), 
				new ArrayList<Banque>(banske6), null, null, u, null);
		Enceinte ec3 = new Enceinte();
		ec3.setNom("R3");
		ec3.setPosition(1);
		ec3.setAlias("ALIAS");
		ec3.setNbPlaces(6);
		List<Banque> banskec3 = new ArrayList<Banque>();
		banskec3.add(b);
		enceinteManager.createObjectManager(ec3, 
				enceinteTypeManager.findByIdManager(1), 
							c3, null, null, banskec3, null, u);
		...
		conts.remove(c1);
		conts.remove(c2);
		Conteneur c4 = conteneurDao.findById(4);
		conts.add(c4);
		
		banqueManager.createOrUpdateObjectManager(b, null, null, null, null, 
				null,
				conts, tabs, tabsPat, tabsPrel, tabsEchan, null, null,
				couls, null, null, u, "modification", null);
		assertTrue(banqueManager.findAllObjectsManager().size() == 5);
		// recupere la derniere banque
		Banque b2 = banqueManager.findByIdManager(b.getBanqueId());
		...
		testFindAll();
		Enceinte e = enceinteManager.findByIdManager(ec3.getEnceinteId());
		assertTrue(enceinteManager.getBanquesManager(e).size() == 0);
		List<Operation> ops = getOperationManager().findByObjectManager(e6);
	...
	public void testCreateUpdateTablesCodage() {
		...
		BanqueTableCodage btc1 = new BanqueTableCodage();
		btc1.setTableCodage(t1);
		btc1.setLibelleExport(true);
		TableCodage t2 = tableCodageDao.findById(2);
		BanqueTableCodage btc2 = new BanqueTableCodage();
		btc2.setTableCodage(t2);
		btc2.setLibelleExport(true);
		TableCodage t3 = tableCodageDao.findById(4);
		BanqueTableCodage btc3 = new BanqueTableCodage();
		...
		b = banqueManager.findByIdManager(b.getBanqueId());
		assertTrue(banqueManager
					.getBanqueTableCodageByBanqueManager(b).size() == 3);
		assertTrue(banqueManager
				.getBanqueTableCodageByBanqueManager(b).contains(btc1));
		assertTrue(banqueManager
				.getBanqueTableCodageByBanqueManager(b).contains(btc2));
		assertTrue(banqueManager
				.getBanqueTableCodageByBanqueManager(b).contains(btc3));
		//assertTrue(tableCodageManager.getBanquesManager(t3).size()  == 2);
			
		// ajout d'une table et remove des deux t1 et t2
		tabs.remove(btc1);
		tabs.remove(btc2);
		TableCodage t4 = tableCodageDao.findById(5);
		BanqueTableCodage btc4 = new BanqueTableCodage();
		btc4.setTableCodage(t4);
		btc4.setLibelleExport(false);
		tabs.add(btc4);
		
		banqueManager.createOrUpdateObjectManager(b, null, null, null, null, 
				null, null, tabs, null, null, null, null, 
				null, null, null, null, u, "modification", null);
		assertTrue(banqueManager.findAllObjectsManager().size() == 5);
		// recupere la derniere banque
		Banque b2 = banqueManager.findByIdManager(b.getBanqueId());
		...
		assertTrue(banqueManager
				.getBanqueTableCodageByBanqueManager(b2).size() == 2);
		assertTrue(banqueManager
				.getBanqueTableCodageByBanqueManager(b2).contains(btc3));
		assertTrue(banqueManager
				.getBanqueTableCodageByBanqueManager(b2).contains(btc4));
		//assertTrue(tableCodageManager.getBanquesManager(t3).size()  == 2);
	...
	public void testCreateUpdateTablesAnnotation() {
		...
		tabsPrel.add(t2);
		List<TableAnnotation> tabsEchan = new ArrayList<TableAnnotation>();
		TableAnnotation t3 = tableAnnotationDao.findById(3);
		TableAnnotation t4 = tableAnnotationDao.findById(4);
		TableAnnotation t5 = tableAnnotationDao.findById(5);	
		tabsEchan.add(t5);
		tabsEchan.add(t3);
		List<TableAnnotation> tabsDerive = new ArrayList<TableAnnotation>();
		TableAnnotation t6 = tableAnnotationDao.findById(6);
		tabsDerive.add(t6);
		
		banqueManager.createOrUpdateObjectManager(b, pf, ct, null, null, 
				null,
				null, null, tabsPat, tabsPrel, tabsEchan, tabsDerive, null,
									null, null, null, u, "creation", "/tmp/");
		
		assertTrue(banqueManager.findAllObjectsManager().size() == 5);		
		b = banqueManager.findByIdManager(b.getBanqueId());
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Patient").get(0), b).size() == 1);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Patient").get(0), b).contains(t1));
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Prelevement").get(0), b).size() == 1);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Prelevement").get(0), b).contains(t2));
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Echantillon").get(0), b).size() == 2);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
						.findByNom("Echantillon").get(0), b).get(0).equals(t5));
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
						.findByNom("Echantillon").get(0), b).get(1).equals(t3));
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("ProdDerive").get(0), b).size() == 1);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("ProdDerive").get(0), b).contains(t6));
		...
		banqueManager.createOrUpdateObjectManager(b, pf, null, null, null, 
				null,
				null, null, tabsPat, tabsPrel, tabsEchan, tabsDerive, null,
		...
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Patient").get(0), b2).size() == 0);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Prelevement").get(0), b2).size() == 1);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Prelevement").get(0), b2).contains(t2));
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Echantillon").get(0), b).size() == 2);
		assertTrue(tableAnnotationManager
			.findByEntiteAndBanqueManager(entiteDao
					.findByNom("Echantillon").get(0), b2).get(0).equals(t3));
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
						.findByNom("Echantillon").get(0), b).get(1).equals(t4));
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("ProdDerive").get(0), b).size() == 1);
		assertTrue(tableAnnotationManager
		...
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Prelevement").get(0), b2).size() == 0);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("Echantillon").get(0), b).size() == 0);
		assertTrue(tableAnnotationManager
				.findByEntiteAndBanqueManager(entiteDao
							.findByNom("ProdDerive").get(0), b).size() == 0);
	...
	public void testCreateUpdateCouleurEntiteTypes() {
		...
		EchantillonType e1 = echantillonTypeDao.findById(1);
		EchantillonType e2 = echantillonTypeDao.findById(2);
		EchantillonType e3 = echantillonTypeDao.findById(3);
		ProdType p1 = prodTypeDao.findById(1);
		Couleur coul5 = couleurDao.findById(5);
		CouleurEntiteType cE1 = new CouleurEntiteType();
		cE1.setCouleur(couleurDao.findById(1));
		cE1.setEchantillonType(e1);
		CouleurEntiteType cE2 = new CouleurEntiteType();
		cE2.setCouleur(couleurDao.findById(2));
		cE2.setEchantillonType(e2);
		CouleurEntiteType cE3 = new CouleurEntiteType();
		cE3.setCouleur(couleurDao.findById(3));
		cE3.setEchantillonType(e3);
		CouleurEntiteType cD1 = new CouleurEntiteType();
		...
		banqueManager.createOrUpdateObjectManager(b, pf, ct, null, null, 
				...
				null, null, null, null, null, null, null,
								coulTypes, null, null, u, "creation", "/tmp/");
		
		assertTrue(banqueManager.findAllObjectsManager().size() == 5);		
		b = banqueManager.findByIdManager(b.getBanqueId());
		assertTrue(couleurEntiteTypeManager
				.findAllObjectsByBanqueManager(b).size() == 4);
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForEchanTypeByBanqueManager(b).size() == 3);
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForEchanTypeByBanqueManager(b).contains(cE1));
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForEchanTypeByBanqueManager(b).contains(cE2));
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForEchanTypeByBanqueManager(b).contains(cE3));
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForProdTypeByBanqueManager(b).size() == 1);
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForProdTypeByBanqueManager(b).contains(cD1));
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForProdTypeByBanqueManager(b).get(0)
												.getCouleur().equals(coul5));
		...
		EchantillonType e4 = echantillonTypeDao.findById(4);
		CouleurEntiteType cE4 = new CouleurEntiteType();
		Couleur coul4 = couleurDao.findById(4);
		...
		Banque b2 = banqueManager.findByIdManager(b.getBanqueId());
		assertTrue(couleurEntiteTypeManager
				.findAllObjectsByBanqueManager(b2).size() == 3);
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForEchanTypeByBanqueManager(b2).size() == 2);
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForEchanTypeByBanqueManager(b2).contains(cE3));
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForEchanTypeByBanqueManager(b2).contains(cE4));
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForProdTypeByBanqueManager(b2).size() == 1);
		assertTrue(couleurEntiteTypeManager
				.findAllCouleursForProdTypeByBanqueManager(b2).contains(cD1));
		assertTrue(couleurEntiteTypeManager
			.findAllCouleursForProdTypeByBanqueManager(b2).get(0).getCouleur()
															.equals(coul4));
		...
		testFindAll();
		assertTrue(couleurEntiteTypeManager
				.findAllObjectsByBanqueManager(b2).size() == 0);
	...
	public void testGetBanqueTableCodageByBanqueManager() {
		Banque b1 = banqueManager.findByIdManager(1);
		List<BanqueTableCodage> btcs = banqueManager
			.getBanqueTableCodageByBanqueManager(b1);
	...
	public void testCascadeDeletion() {
		...
		banqueManager.createOrUpdateObjectManager(b, pf, ct, 
				b1.getProprietaire(), 
				b1.getCollaborateur(), 
				b1.getContact(), 
				new ArrayList<Conteneur>(banqueManager
									.getConteneursManager(b1)), 
				new ArrayList<BanqueTableCodage>(banqueManager
					.getBanqueTableCodageByBanqueManager(b1)), 
				null, null,
				null, null, null, null, 
				null, couleurDao.findById(2), u, "creation", "/tmp/");
		
		assertTrue(banqueManager.findAllObjectsManager().size() == 5);		
		b = banqueManager.findByIdManager(b.getBanqueId());
		
		// cree code dosser select
		CodeDossier codeDos = new CodeDossier();
		codeDos.setNom("dos");
		codeDos.setCodeSelect(true);
		codeDossierManager
			.createOrUpdateManager(codeDos, null, b, u, "creation");
		// cree code Select
		CodeSelect codeSel = new CodeSelect();
		codeSel.setCodeId(1);
		codeSel.setTableCodage(tableCodageDao.findById(1));	
		codeSelectManager
			.createOrUpdateManager(codeSel, codeDos, b, u, "creation");
		// cree code dosser utilisateur
		CodeDossier codeDos2 = new CodeDossier();
		codeDos2.setNom("dos2");
		codeDos2.setCodeSelect(false);
		codeDossierManager
			.createOrUpdateManager(codeDos2, null, b, u, "creation");
		...
		codeUtilisateurManager
			.createOrUpdateManager(codeU, codeDos2, b, u, null, null, 
					"creation");
		CodeUtilisateur codeU2 = new CodeUtilisateur();
		codeU2.setCode("codeCas2");
		codeU2.setLibelle("libCasc2");
		codeUtilisateurManager
			.createOrUpdateManager(codeU2, null, b, u, codeU, null, "creation");
		// cree code dosser utilisateur
		CodeDossier codeDos3 = new CodeDossier();
		codeDos3.setNom("dos3");
		codeDos3.setCodeSelect(false);
		codeDossierManager
			.createOrUpdateManager(codeDos3, codeDos2, b, u, "creation");
		...
		a.setIntitule("Aff");
		a.setNbLignes(2);
		affichageManager.createObjectManager(a, null, u, b);
		
		CouleurEntiteType cType = new CouleurEntiteType();
		couleurEntiteTypeManager.createObjectManager(cType, 
				couleurDao.findById(2), 
				b, echantillonTypeDao.findById(3), null);
		...
		prelevementManager.createObjectManager(p, b, p1.getNature(), 
				null, p1.getConsentType(), null, null, null, null, null, null, 
				null, null, null, null, null, u, false, "/tmp/",
				false);
		...
		echantillonManager.createObjectManager(e, b, null, null, 
				e1.getObjetStatut(), null, e1.getEchantillonType(), 
				null, null, null, null, null,
				null, null, u, false, "/tmp/",
				false);
		...
		// reference rendant cascade impossible
		ProdDerive d = new ProdDerive();
		d.setCode("derive");
		ProdDerive d1 = prodDeriveManager.findByIdManager(1);
		prodDeriveManager.createObjectManager(d, b, d1.getProdType(), 
				objetStatutDao.findById(4), null, null, null, null, null, null, 
				null, null, null, null, null, u, true, "/tmp/",
				false);
		...
		cessionManager.createObjectManager(c, b, c1.getCessionType(), null, 
				null, null, null, null, c1.getCessionStatut(), null, null, 
				null, null, null, u, null, "/tmp/");
	...
	public void testBanqueFileSystem() throws IOException {
		...
		assertTrue(new File(Utils.writeAnnoFilePath("/tmp/", b, null, null))
																.exists());
		assertTrue(new File(Utils.writeAnnoFilePath("/tmp/", b, null, null) 
														+ "/anno").exists());
		assertTrue(new File(Utils.writeAnnoFilePath("/tmp/", b, null, null) 
		...
		new File(Utils.writeAnnoFilePath("/tmp/", b, null, null) 
				+ "/anno/chp1").mkdir();
		new File(Utils.writeAnnoFilePath("/tmp/", b, null, null) 
				+ "/anno/chp1/test1").createNewFile();
		
		new File(Utils.writeAnnoFilePath("/tmp/", b, null, null) 
				+ "/cr_anapath/test2").createNewFile();
	
		banqueManager.removeObjectManager(b, null, u, "/tmp/", false);
		assertFalse(new File(Utils.writeAnnoFilePath("/tmp/", b, null, null))
																	.exists());
		assertFalse(new File(Utils.writeAnnoFilePath("/tmp/", b, null, null) 
														+ "/anno").exists());
		assertFalse(new File(Utils.writeAnnoFilePath("/tmp/", b, null, null) 
	...
	public void testExistingAnnotationValuesException() {
		...
		TableAnnotation t4 = tableAnnotationDao.findById(4);
		TableAnnotation t3 = tableAnnotationDao.findById(3);
		List<TableAnnotation> tabEchans = new ArrayList<TableAnnotation>();
		tabEchans.add(t3); tabEchans.add(t4);
		
		assertTrue(tableAnnotationManager.findByEntiteAndBanqueManager(entiteDao.findById(3), 
				b2).size() == 1);
		assertTrue(tableAnnotationManager.findByEntiteAndBanqueManager(entiteDao.findById(3), 
				b2).contains(t3));
		
		banqueManager.createOrUpdateObjectManager(b2, b2.getPlateforme(),
				b2.getContexte(), b2.getProprietaire(), b2.getCollaborateur(), b2.getContact(), 
				null, null, null, null, tabEchans, null, null, null, 
				b2.getEchantillonCouleur(), b2.getProdDeriveCouleur(), 
				utilisateurDao.findById(1), "modification", "/tmp");
		...
		try {	
			banqueManager.createOrUpdateObjectManager(b2, b2.getPlateforme(),
				...
				b2.getEchantillonCouleur(), b2.getProdDeriveCouleur(), 
				utilisateurDao.findById(1), "modification", "/tmp");
		} catch (ExistingAnnotationValuesException ex) {
		...
		// clean up operations
		List<Operation> ops = getOperationManager().findByObjectManager(b2);
