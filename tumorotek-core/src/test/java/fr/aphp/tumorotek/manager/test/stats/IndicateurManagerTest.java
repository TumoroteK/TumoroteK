...
public class IndicateurManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjects() {
		List<Indicateur> ss = indicateurManager.findAllObjectsManager();
		assertTrue(ss.size() == totCounts);
	}
	
	@Test
	public void testFindBySModeleManager() {
		SModele m1 = sModeleDao.findById(1);
		SModele m2 = sModeleDao.findById(2);
		
		List<Indicateur> ss = indicateurManager.findBySModeleManager(m1);
		assertTrue(ss.size() == 2);
		assertTrue(ss.contains(indicateurDao.findById(1)));
		assertTrue(ss.contains(indicateurDao.findById(2)));
	...
	public void testValidateObject() {
		...
		try {
			...
			assertEquals("stats.indicateur.nom.empty", 
		...
		try {
			...
			assertTrue(ve.getErrors().size() == 1);
			assertEquals("stats.indicateur.nom.illegal", 
					ve.getErrors().get(0).getFieldError().getCode());
			catched = true;
		}
		assertTrue(catched);
		...
		try {
			...
			assertTrue(ve.getErrors().size() == 1);
			assertEquals("stats.indicateur.nom.tooLong", 
					ve.getErrors().get(0).getFieldError().getCode());
			catched = true;
		}
		assertTrue(catched);
	...
	public void testCrud() throws ParseException {
		Integer idIndic = createObjectManager();
	...
	private Integer createObjectManager() {
		...
		Integer idIndic = indic.getIndicateurId();
		Indicateur itest = indicateurManager.findByIdManager(idIndic);
		assertNotNull(itest);
	...
	private void updateObjectManager(Integer idIndic) throws ParseException {
		
		Indicateur itest = indicateurDao.findById(idIndic);
		itest.setNom("¤¤¤¤OO");
		...
		assertTrue(indicateurDao.findById(idIndic)
								.getNom().equals("testCreate"));
		assertTrue(indicateurDao.findById(idIndic)
							.getCallingProcedure().equals("call"));
		assertTrue(indicateurDao.findById(idIndic)
				.getDescription().equals("Une courte description"));
