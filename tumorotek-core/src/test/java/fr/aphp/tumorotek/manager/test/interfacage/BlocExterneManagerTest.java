...
//    }
	...
	public void testFindById() {
		BlocExterne m = blocExterneManager.findByIdManager(1);
		assertNotNull(m);
		
		BlocExterne mNull = blocExterneManager.findByIdManager(100);
		assertNull(mNull);
	}

	@Test
	public void testFindAll() {
		List<BlocExterne> list = blocExterneManager
	...
	public void testFindByDossierExterneManager() {
		...
		List<BlocExterne> list = blocExterneManager
			.findByDossierExterneManager(d1);
	...
	public void testFindByDossierExterneAndEntiteManager() {
		...
		Entite e5 = entiteDao.findById(5);
		
		List<BlocExterne> list = blocExterneManager
			.findByDossierExterneAndEntiteManager(d1, e1);
		assertTrue(list.size() == 1);
		
		list = blocExterneManager.findByDossierExterneAndEntiteManager(
				d1, e5);
		assertTrue(list.size() == 0);
	...
	public void testGetEntiteManager() {
		BlocExterne b1 = blocExterneManager.findByIdManager(1);
		Entite e = blocExterneManager.getEntiteManager(b1);
		assertNotNull(e);
		assertTrue(e.getNom().equals("Patient"));
		
		BlocExterne b3 = blocExterneManager.findByIdManager(3);
	...
	public void testFindDoublon() {
		DossierExterne de1 = dossierExterneDao.findById(1);
		DossierExterne de2 = dossierExterneDao.findById(2);
		
		BlocExterne be = new BlocExterne();
	...
	public void testCrud() {
		DossierExterne de = dossierExterneDao.findById(2);
		BlocExterne be1 = new BlocExterne();
		be1.setEntiteId(1);
		be1.setOrdre(1);
		
		Integer nb = blocExterneManager.findAllObjectsManager().size();
		Integer nbV = valeurExterneManager.findAllObjectsManager().size();
		boolean catched = false;
		// on test l'insertion avec un dossier null
		try {
			blocExterneManager.createObjectManager(be1, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		blocExterneManager.createObjectManager(be1, de, null);
		assertTrue(blocExterneManager.findAllObjectsManager().size() 
				== nb + 1);
		Integer id1 = be1.getBlocExterneId();
		
		BlocExterne bTest = blocExterneManager.findByIdManager(id1);
		assertNotNull(bTest);
		...
		BlocExterne be2 = new BlocExterne();
		be2.setEntiteId(1);
		be2.setOrdre(1);
		blocExterneManager.createObjectManager(be2, de, null);
		assertTrue(blocExterneManager.findAllObjectsManager().size() 
				== nb + 1);
		Integer id2 = be2.getBlocExterneId();
		assertTrue(!id1.equals(id2));
		
		BlocExterne bTest2 = blocExterneManager.findByIdManager(id2);
		...
		BlocExterne be3 = new BlocExterne();
		be3.setEntiteId(2);
		be3.setOrdre(2);
		...
		valeurs.add(ve1);
		valeurs.add(ve2);
		blocExterneManager.createObjectManager(be3, de, valeurs);
		assertTrue(blocExterneManager.findAllObjectsManager().size() 
				== nb + 2);
		assertTrue(valeurExterneManager.findAllObjectsManager().size() 
				== nbV + 2);
		Integer id3 = be3.getBlocExterneId();
		
		BlocExterne bTest3 = blocExterneManager.findByIdManager(id3);
