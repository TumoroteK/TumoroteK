...
public class CessionExamenManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		CessionExamen ce1 = cessionExamenManager.findByIdManager(1);
		assertNotNull(ce1);
		assertTrue(ce1.getExamen().equals("EXAMEN 1"));
		
		CessionExamen ceNull = cessionExamenManager.findByIdManager(10);
		assertNull(ceNull);
	}
	...
	public void testFindByExamenLikeExactManager() {
		List<CessionExamen> list = 
			cessionExamenManager.findByExamenLikeManager("EXAMEN 1", true);
	...
	public void testFindByExamenLikeManager() {
		List<CessionExamen> list = 
			cessionExamenManager.findByExamenLikeManager("EXAMEN 1", false);
		assertTrue(list.size() == 1);
	...
	public void testFindDoublon() {
		//Cree le doublon
		CessionExamen c1 = cessionExamenManager.findByIdManager(1);
		...
		CessionExamen c2 = new CessionExamen();
	...
	public void testIsUsedObjectManager() {
		...
		CessionExamen c2 = cessionExamenManager.findByIdManager(2);
	...
	public void createObjectManagerTest() {
		...
		CessionExamen c1 = new CessionExamen();
		c1.setExamen("RAAAAA");
		c1.setPlateforme(plateformeDao.findById(1));
		cessionExamenManager.createObjectManager(c1);
		assertTrue((cessionExamenManager
				.findByExamenLikeManager("RAAAAA", true)).size() == 1);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		CessionExamen c1Bis = new CessionExamen();
	...
	public void updateObjectManagerTest() {
		...
		CessionExamen c1 = (cessionExamenManager
				.findByExamenLikeManager("RAAAAA", true)).get(0);
		c1.setExamen("TEST");
		cessionExamenManager.updateObjectManager(c1);
		assertTrue((cessionExamenManager
				.findByExamenLikeManager("TEST", true)).size() == 1);
		...
		assertTrue(catched);
		assertTrue((cessionExamenManager
				.findByExamenLikeManager("EXAMEN 1", true)).size() == 1);
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			//verifie que l'enregistrement n'a pas ete modifie
				assertTrue((cessionExamenManager
					.findByExamenLikeManager("EXAMEN 1", true))
					.size() == 1);
			}
		}
	}
	...
	public void removeObjectManagerTest() {
		...
		CessionExamen c1 = (cessionExamenManager
				.findByExamenLikeManager("TEST", true)).get(0);
		cessionExamenManager.removeObjectManager(c1);
		assertTrue((cessionExamenManager
				.findByExamenLikeManager("TEST", true)).size() == 0);
...
//		assertTrue(catched);
		assertTrue((cessionExamenManager
				.findByExamenLikeManager("EXAMEN 1", true)).size() > 0);
		//null remove
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
			cessionExamenManager.findByOrderManager(pf);
