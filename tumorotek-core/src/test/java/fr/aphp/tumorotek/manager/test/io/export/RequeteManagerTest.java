...
public class RequeteManagerTest extends AbstractManagerTest4 {
	...
	public void testRenameRequete() {
		...
		try {
			...
			assertEquals("DoublonFoundException",
					e.getClass().getSimpleName());
		}
		
		//On récupère une requete
		requete = manager.findByIdManager(2);
		int idRequete = requete.getRequeteId();
		String oldIntitule = requete.getIntitule();
		String intitule = "MODIFICATION";
		manager.renameRequeteManager(requete, intitule);
		
		//on récupère l'objet modifié
		Requete requete2 = manager.findByIdManager(idRequete);
	...
	public void testCopyRequete() {
		...
		Utilisateur copieur = utilisateurDao.findById(2);
		try {
		...
		Requete copie = manager.copyRequeteManager(requete, util, b);
		...
		Requete copie2 = manager.findByIdManager(copie.getRequeteId());
	...
	public void testCrud() {
		...
		Groupement gCopie = groupementManager.copyGroupementManager(groupement);
		...
		/** Update. */
		util = utilisateurDao.findById(3);
		groupement = groupementManager.findByIdManager(2);
		Groupement gCopie2 
			= groupementManager.copyGroupementManager(groupement);
		...
		assertFalse(requete.getGroupementRacine()
				.equals(requete2.getGroupementRacine()));
		

		/** Remove. */
		//On récupère une requete
		int id = requete.getRequeteId();
		//On la supprime
		...
		Requete r3 = manager.findByIdManager(3);
	...
	public void testFindById() {
		...
		Requete requeteNull = manager.findByIdManager(50);
	...
	public void testFindAllObjects() {
		List<Requete> list = manager.findAllObjectsManager();
		//On vérifie chaque élément par un findById
		Iterator<Requete> it = list.iterator();
		while (it.hasNext()) {
			Requete temp = it.next();
			if (temp != null) {
				assertTrue(temp.equals(
						manager.findByIdManager(temp.getRequeteId())));
			}
		}
	}
	...
	public void testFindByUtilisateur() {
		Iterator<Utilisateur> itUtil = utilisateurDao.findAll().iterator();
		while (itUtil.hasNext()) {
			Utilisateur utilisateur = itUtil.next();
			Iterator<Requete> it = manager
					.findByUtilisateurManager(utilisateur).iterator();
			while (it.hasNext()) {
				Requete temp = it.next();
				if (temp != null) {
	...
	public void testFindByIntituleAndUtilisateurManager() {
		...
		List<Requete> liste = manager
			.findByIntituleAndUtilisateurManager("Echantillon%", u1);
		assertTrue(liste.size() == 1);
		...
		liste = manager
		.findByIntituleAndUtilisateurManager("Echantillon%", u2);
		assertTrue(liste.size() == 0);
		...
		liste = manager
		.findByIntituleAndUtilisateurManager("Echantillon%", null);
		assertTrue(liste.size() == 0);
	}
	...
	public void testIsUsedObjectManager() {
		...
		Requete r2 = manager.findByIdManager(2);
		assertTrue(manager.isUsedObjectManager(r2));
		
		Requete r4 = manager.findByIdManager(4);
