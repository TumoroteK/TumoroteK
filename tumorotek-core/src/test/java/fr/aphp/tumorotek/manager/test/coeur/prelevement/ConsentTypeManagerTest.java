...
public class ConsentTypeManagerTest extends AbstractManagerTest4 {
	...
	public void testFindByTypeLikeManager() {
		...
		List<ConsentType> consentTypes = 
			consentTypeManager.findByTypeLikeManager("EN ATTENTE", true);
		assertTrue(consentTypes.size() == 1);
		//teste une recherche non exactMatch
		consentTypes = 
			consentTypeManager.findByTypeLikeManager("RECH", false);
		assertTrue(consentTypes.size() == 1);
		assertTrue(consentTypes.get(0).getType().equals("RECHERCHE"));
		//teste une recherche infructueuse
		consentTypes = 
			consentTypeManager.findByTypeLikeManager("ACCORD", false);
		assertTrue(consentTypes.size() == 0);
		//null recherche
		consentTypes = 
			consentTypeManager.findByTypeLikeManager(null, false);
		assertTrue(consentTypes.size() == 0);
	}
	...
	private void createObjectManagerTest() {
		...
		ConsentType ct1 = new ConsentType();
		ct1.setType("GENETIQUE 1.0");
		ct1.setPlateforme(plateformeDao.findById(1));
		consentTypeManager.createObjectManager(ct1);
		assertTrue((consentTypeManager
				.findByTypeLikeManager("GENETIQUE 1.0", true)).size() == 1);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		ConsentType ct1Bis = new ConsentType();
		...
		//validation test
		String[] emptyValues = new String[]{"", "  ", "$$¢\\uy", 
		...
		for (int i = 0; i < emptyValues.length; i++) {
			try {
				...
				assertTrue(consentTypeManager
						.findByOrderManager(ct1.getPlateforme()).size() == 3);
			}
		}
	}
	...
	private void updateObjectManagerTest() {
		...
		ConsentType ct1 = (consentTypeManager
			.findByTypeLikeManager("GENETIQUE 1.0", true)).get(0);
		ct1.setType("ADN -");
		consentTypeManager.updateObjectManager(ct1);
		assertTrue((consentTypeManager
				.findByTypeLikeManager("ADN -", true)).size() == 1);
		...
		assertTrue(catched);
		assertTrue((consentTypeManager
				.findByTypeLikeManager("EN ATTENTE", true)).size() == 1);
	
		//Validation test
		String[] emptyValues = new String[]{"", "  ", "¤¤$$[plk]", 
										createOverLength(200), null};
		for (int i = 0; i < emptyValues.length; i++) {
			try {
	...
	private void removeObjectManagerTest() {
		...
		ConsentType ct1 = (consentTypeManager
				.findByTypeLikeManager("ADN -", true)).get(0);
		consentTypeManager.removeObjectManager(ct1);
		assertTrue((consentTypeManager
				.findByTypeLikeManager("ADN -", true)).size() == 0);
		...
		assertTrue(catched);
		assertTrue((consentTypeManager
				.findByTypeLikeManager("EN ATTENTE", true)).size() > 0);
		//null remove
		consentTypeManager.removeObjectManager(null);
		assertTrue(consentTypeManager
				.findByOrderManager(ct1.getPlateforme()).size() == 2);
	}
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					consentTypeManager.findByOrderManager(pf);
