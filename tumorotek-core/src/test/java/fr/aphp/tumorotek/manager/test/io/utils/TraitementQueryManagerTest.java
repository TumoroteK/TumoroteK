...
public class TraitementQueryManagerTest extends AbstractManagerTest4 {
	...
	public void testfindFileUploadedManager() {
		...
		Champ crAnapath = new Champ(champEntiteManager.findByIdManager(255));		
	...
	public void findTKStockableObjectsByTempStockWithBanquesManager() {
		...
		// nulls
		List<? extends Object> resultats = traitementQueryManager
				.findTKStockableObjectsByTempStockWithBanquesManager(null, temp, "<", banques, false);
		assertTrue(resultats.isEmpty());
		
		resultats = traitementQueryManager
				.findTKStockableObjectsByTempStockWithBanquesManager(entiteManager.findByIdManager(3), 
		...
		// null temp
		resultats = traitementQueryManager
				.findTKStockableObjectsByTempStockWithBanquesManager(entiteManager.findByIdManager(8), 
						null, "=", banques, false);
		assertTrue(resultats.isEmpty());
	...
	public void testFindObjetByCritereInListWithBanquesManager() {
		...
		// sexe patient
		Entite ePatient = entiteManager.findByIdManager(1);
		ChampEntite champEntite = champEntiteManager
				.findByEntiteAndNomManager(ePatient, "sexe").get(0);

		// création du champ avec ajout du parent si nécessaire
		Champ champ = new Champ(champEntite);
	//	if (parent1 != null) {
	...
	//	}
		...
		List<Integer> res = traitementQueryManager
		...
		// cherche risques 
		Entite ePrel = entiteManager.findByIdManager(2);

		ChampEntite champRisques = champEntiteManager
				.findByEntiteAndNomManager(ePrel,
						"Risques").get(0);
		Champ parent1 = new Champ(champRisques);
		parent1.setChampParent(null);
		
		// risque nom
		Entite risqueEntite = entiteManager
				.findByNomManager("Risque").get(0);
		ChampEntite nomRisque = champEntiteManager
				.findByEntiteAndNomManager(risqueEntite, "nom").get(0);

		// création du champ avec ajout du parent si nécessaire
		Champ champ2 = new Champ(nomRisque);
		...
		ChampAnnotation thes2 = champAnnotationManager
												.findByIdManager(12);

		// création du champ avec ajout du parent si nécessaire
		Champ champA12 = new Champ(thes2);
		critere = new Critere(champA12, "=", null); 
				
		List<Object> itemVals = new ArrayList<Object>();
		itemVals.add(itemDao.findById(5));// item2-2);

		res = traitementQueryManager
				.findObjetByCritereInListWithBanquesManager(critere,
						banques, itemVals, false);
		assertTrue(res.size() == 1);
		assertTrue(res.get(0) == 1);
		...
		Champ champA1 = new Champ(champAnnotationManager.findByIdManager(1));
