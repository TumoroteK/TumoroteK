...
public class ModeleManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Modele m = modeleManager.findByIdManager(1);
		assertNotNull(m);
		assertTrue(m.getNom().equals("NBT"));
		
		Modele mNull = modeleManager.findByIdManager(10);
		assertNull(mNull);
	}
	...
	public void testFindAll() {
		List<Modele> list = modeleManager.findAllObjectsManager();
	...
	public void testFindByNomAndPlateformeManager() {
		...
		List<Modele> list = modeleManager
			.findByNomAndPlateformeManager("NBT", pf1);
	...
	public void testFindDoublon() {
		...
		Modele m2 = modeleManager.findByIdManager(2);
	...
	public void testIsUsedObjectManager() {
		Modele m1 = modeleManager.findByIdManager(1);
		assertTrue(modeleManager.isUsedObjectManager(m1));
		
		Modele m3 = modeleManager.findByIdManager(3);
...
//		cle1.setChamp(ch1);
//		cle1.setEntite(entiteDao.findById(1));
//		cle1.setOrdre(1);
//		ChampLigneEtiquette cle2 = new ChampLigneEtiquette();
//		cle2.setChamp(ch2);
//		cle2.setEntite(entiteDao.findById(2));
//		cle2.setOrdre(2);
...
//	}
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		Modele mUp1 = modeleManager.findByIdManager(idM);
		boolean catched = false;
		// on test l'insertion avec une pf nulle
		try {
			modeleManager.updateObjectManager(mUp1, null, mt1,
					null, null, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
		...
		// on test l'insertion avec un type null
		try {
			modeleManager.updateObjectManager(mUp1, pf1, null,
					null, null, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
		...
		mUp1.setNom("NBT");
		try {
			modeleManager.updateObjectManager(mUp1, pf1, mt1,
					null, null, null, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
		...
		cle3.setChamp(ch3);
		cle3.setEntite(entiteDao.findById(1));
		cle3.setOrdre(1);
		lignes = new ArrayList<LigneEtiquette>();
		lignes.add(le1);
		lignes.add(le2);
		...
		champs.add(cle3);
		champs.add(cle2);
		List<ChampLigneEtiquette> chpsToRmv = 
			new ArrayList<ChampLigneEtiquette>();
		chpsToRmv.add(cle1);
		hash.put(le1, champs);
		Hashtable<LigneEtiquette, List<ChampLigneEtiquette>> hashToRmv = 
			new Hashtable<LigneEtiquette, List<ChampLigneEtiquette>>();
		hashToRmv.put(le1, chpsToRmv);
		
		modeleManager.updateObjectManager(mTest, pf1, mt1, 
				lignes, null, hash, hashToRmv);
		assertTrue(modeleManager.findAllObjectsManager().size() == 4);
		assertTrue(ligneEtiquetteManager.findAllObjectsManager().size() == 9);
		assertTrue(champLigneEtiquetteManager
				.findAllObjectsManager().size() == 12);
		...
		Modele mTest2 = modeleManager.findByIdManager(idM);
		assertTrue(ligneEtiquetteManager
				.findByModeleManager(mTest2).size() == 2);
		List<ChampLigneEtiquette> tmp = champLigneEtiquetteManager
			.findByLigneEtiquetteManager(ligneEtiquetteManager
				.findByModeleManager(mTest2).get(0));
		...
		// avec un champ
		lignes = new ArrayList<LigneEtiquette>();
		LigneEtiquette le3 = new LigneEtiquette();
		le3.setOrdre(1);
		...
		lignes.add(le3);
		Champ ch4 = new Champ();
		ch4.setChampEntite(champEntiteDao.findById(4));
		ChampLigneEtiquette cle4 = new ChampLigneEtiquette();
		cle4.setChamp(ch4);
		cle4.setEntite(entiteDao.findById(1));
		cle4.setOrdre(1);
		champs = new ArrayList<ChampLigneEtiquette>();
		champs.add(cle4);
		hash.put(le3, champs);
		List<LigneEtiquette> lignesToRmv = new ArrayList<LigneEtiquette>();
		lignesToRmv = ligneEtiquetteManager
			.findByModeleManager(mTest2);
		
		modeleManager.updateObjectManager(mTest2, pf1, mt1, 
				lignes, lignesToRmv, hash, null);
		assertTrue(modeleManager.findAllObjectsManager().size() == 4);
		assertTrue(ligneEtiquetteManager.findAllObjectsManager().size() == 8);
		assertTrue(champLigneEtiquetteManager
				.findAllObjectsManager().size() == 11);
		assertTrue(champManager.findAllObjectsManager().size() == cTots + 1);
		
		Modele mTest3 = modeleManager.findByIdManager(idM);
		List<LigneEtiquette> lis = ligneEtiquetteManager
			.findByModeleManager(mTest3);
		assertTrue(lis.size() == 1);
		assertTrue(lis.get(0).getEntete().equals("ENTETE3"));
		tmp = champLigneEtiquetteManager
			.findByLigneEtiquetteManager(lis.get(0));
...
//		Imprimante i2 = imprimanteDao.findById(2);
