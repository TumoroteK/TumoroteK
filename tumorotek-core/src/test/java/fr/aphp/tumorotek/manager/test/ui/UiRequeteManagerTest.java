...
public class UiRequeteManagerTest extends AbstractManagerTest {
	...
	public void setUiRequeteManager(UiRequeteManager uManager) {
	...
	public void testFindByNomUtilisateurAndEntiteManager() {
		List<UiRequete> reqs = uiRequeteManager
			.findByUtilisateurAndEntiteManager(utilisateurDao.findById(1), 
				entiteDao.findById(2));
		assertTrue(reqs.size() == 3);
		
		reqs = uiRequeteManager
				.findByUtilisateurAndEntiteManager(utilisateurDao.findById(1), 
		entiteDao.findById(3));
		assertTrue(reqs.isEmpty());
		
		reqs = uiRequeteManager
				.findByUtilisateurAndEntiteManager(utilisateurDao.findById(5), 
		entiteDao.findById(3));
		...
		reqs = uiRequeteManager
				.findByUtilisateurAndEntiteManager(null, entiteDao.findById(2));
		assertTrue(reqs.isEmpty());	
	}
	...
	public void testFindDoublonManager() {
		...
		req = new UiRequete(null, utilisateurDao.findById(1), entiteDao.findById(2), 
				"REQ3", 4);
	...
	public void testCrud() throws ParseException {
		List<UiCompValue> vals = new ArrayList<UiCompValue>();
		UiCompValue u1 = new UiCompValue(null, null, "nipBox", "textbox", 
				null, "121212", null, null);
		vals.add(u1);
		Calendar dateNaissance = Calendar.getInstance();
		dateNaissance.setTime(new SimpleDateFormat("dd/mm/yyyy").parse("02/12/1965"));
		UiCompValue u2 = new UiCompValue(null, null, "dateNaisBox", "datebox", 
				null, null, null, dateNaissance);
		vals.add(u2);
		
		// Test de l'insertion
		uiRequeteManager.createObjectManager("REQPAT", 
				utilisateurDao.findById(2), entiteDao.findById(1), 1, vals);

		UiRequete r1 = uiRequeteManager
			.findByUtilisateurAndEntiteManager(utilisateurDao.findById(2), 
									entiteDao.findById(1)).get(0);
		...
		Iterator<UiCompValue> it = r1.getUiCompValues().iterator();
		...
		UiCompValue u3 = new UiCompValue(null, null, 
				"sexeMFBox", "checkbox", null, null, false, null);
		vals.clear();
		vals.add(u3);
		...
		assertTrue(uiRequeteManager
				.findByUtilisateurAndEntiteManager(utilisateurDao.findById(2), 
						entiteDao.findById(1)).isEmpty());
		// null remove
	...
	public void testUiRequeteValidation() {
		...
		try {
			uiRequeteManager.createObjectManager("TESTVAL", 
							null, entiteDao.findById(5), 1, null);
		...
		try {
			uiRequeteManager.createObjectManager("TESTVAL", 
							utilisateurDao.findById(5), null, 1, null);
		} catch (RequiredObjectIsNullException re) {
			catched = true;
		} 
		assertTrue(catched);
		...
		try {
			uiRequeteManager.createObjectManager("TESTVAL", 
					utilisateurDao.findById(5), entiteDao.findById(5), null, null);
		} catch (RequiredObjectIsNullException re) {
			catched = true;
		} 
		assertTrue(catched);
		assertTrue(uiRequeteManager
			.findByUtilisateurAndEntiteManager(utilisateurDao.findById(5), 
					entiteDao.findById(5)).isEmpty());
		
		// test doublon		
		catched = false;
		try {
			uiRequeteManager.createObjectManager("REQ2", 
					utilisateurDao.findById(1), entiteDao.findById(2), 6, null);
		...
		assertTrue(uiRequeteManager
				.findByUtilisateurAndEntiteManager(utilisateurDao.findById(1), 
						entiteDao.findById(2)).size() == 3);
		
		// test validator
		List<Errors> errs = new ArrayList<Errors>();
		try {
			uiRequeteManager.createObjectManager(null, 
					utilisateurDao.findById(1), entiteDao.findById(2), 6, null);
		} catch (ValidationException ve) {
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("uiRequete.nom.empty"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
		try {
			uiRequeteManager.createObjectManager("", 
					utilisateurDao.findById(1), entiteDao.findById(2), 6, null);
		} catch (ValidationException ve) {
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("uiRequete.nom.illegal"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
		try {
			uiRequeteManager.createObjectManager(createOverLength(250), 
					utilisateurDao.findById(1), entiteDao.findById(2), 6, null);
		} catch (ValidationException ve) {
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("uiRequete.nom.tooLong"));
		}
		assertFalse(errs.isEmpty());
