...
public class TerminaleManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Terminale tNull = terminaleManager.findByIdManager(10);
		assertNull(tNull);
	}
	...
	public void testFindAll() {
		List<Terminale> list = terminaleManager.findAllObjectsManager();
	...
	public void testFindByEnceinteWithOrder() {
		Enceinte e3 = enceinteDao.findById(3);
		
		List<Terminale> list = terminaleManager
			.findByEnceinteWithOrderManager(e3);
		assertTrue(list.size() == 3);
		assertTrue(list.get(0).getNom().equals("BT1"));
		
		Enceinte e5 = enceinteDao.findById(5);
		list = terminaleManager
			.findByEnceinteWithOrderManager(e5);
	...
	public void testFindByEnceinteAndNomManager() {
		...
		List<Terminale> list = terminaleManager
			.findByEnceinteAndNomManager(e3, "BT1");
		assertTrue(list.size() == 1);
		assertTrue(list.get(0).getNom().equals("BT1"));
		
		list = terminaleManager
			.findByEnceinteAndNomManager(e3, "jivoqsji");
		assertTrue(list.size() == 0);
	...
	public void testCheckTerminaleInEnceinteLimites() {
		...
		assertTrue(terminaleManager
				.checkTerminaleInEnceinteLimitesManager(t1));
		
		t1.setPosition(1000);
		assertFalse(terminaleManager
				.checkTerminaleInEnceinteLimitesManager(t1));
		
		Terminale newT = new Terminale();
		assertFalse(terminaleManager
				.checkTerminaleInEnceinteLimitesManager(newT));
		...
		assertFalse(terminaleManager
				.checkTerminaleInEnceinteLimitesManager(null));
	...
	public void testGetEmplacementsManager() {
		...
		Iterator<Emplacement> it = set.iterator();
		assertTrue(it.next().getPosition() == 1);
		
		Terminale t6 = terminaleManager.findByIdManager(6);
		set = terminaleManager.getEmplacementsManager(t6);
		assertTrue(set.size() == 2);
		
		Terminale t3 = terminaleManager.findByIdManager(3);
	...
	public void testGetCodesLignesManager() {
		...
		Terminale t2 = terminaleManager.findByIdManager(2);
		...
		Terminale t4 = terminaleManager.findByIdManager(4);
		...
		Terminale t5 = terminaleManager.findByIdManager(5);
	...
	public void testGetEchantillonsManager() {
		Terminale t1 = terminaleManager.findByIdManager(1);
		List<Echantillon> liste = terminaleManager
			.getEchantillonsManager(t1);
	...
	public void testGetProdDerivesManager() {
		Terminale t1 = terminaleManager.findByIdManager(1);
		List<ProdDerive> liste = terminaleManager
			.getProdDerivesManager(t1);
	...
	public void testFindDoublon() {
		...
		String nom1 = "BT1";
		String nom2 = "BT10";
		Enceinte e1 = enceinteDao.findById(3);
		Enceinte e2 = enceinteDao.findById(2);
	...
	public void testFindDoublonWithoutTwoTerminalesManager() {
		...
		Terminale t1 = terminaleManager.findByIdManager(1);
		Terminale t2 = terminaleManager.findByIdManager(2);
		assertFalse(terminaleManager.findDoublonWithoutTwoTerminalesManager(
				t1, t2));
		...
		assertTrue(terminaleManager.findDoublonWithoutTwoTerminalesManager(
				t4, t2));
		assertFalse(terminaleManager.findDoublonWithoutTwoTerminalesManager(
				t1, t4));
		assertFalse(terminaleManager.findDoublonWithoutTwoTerminalesManager(
		...
		Enceinte e = enceinteDao.findById(5);
		t4.setEnceinte(e);
		assertFalse(terminaleManager.findDoublonWithoutTwoTerminalesManager(
				t4, t2));
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Enceinte enc = enceinteDao.findById(3);
		TerminaleType type = terminaleTypeDao.findById(1);
		...
		try {
			terminaleManager.createObjectManager(t1, null, type, 
					null, null, num, null, u);
		} catch (Exception e) {
		...
		try {
			terminaleManager.createObjectManager(t1, enc, null, 
					null, null, num, null, u);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
		...
		try {
			terminaleManager.createObjectManager(t1, enc, type, 
		...
		int id = t1.getTerminaleId();
		// Vérification
		Terminale tTest = terminaleManager.findByIdManager(id);
		assertNotNull(tTest);
		...
		t2.setPosition(4);
		t2.setAlias("ALIAS4");
		Couleur coul = couleurDao.findById(6);
		terminaleManager.createObjectManager(t2, enc, type, 
				banque, ent, num, coul, u);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 8);
		assertTrue(getOperationManager().findByObjectManager(t2).size() == 1);
		int id2 = t2.getTerminaleId();
		// Vérification
		Terminale tTest2 = terminaleManager.findByIdManager(id2);
		...
		assertTrue(getOperationManager()
						.findByObjectManager(tTest).size() == 0);
		assertTrue(getOperationManager()
						.findByObjectManager(tTest2).size() == 0);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 6);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		t.setNom("BT3");
		t.setPosition(3);
		terminaleManager.createObjectManager(
				t, enc, type, null, ent, num, null, u);
		Integer id = t.getTerminaleId();
		assertTrue(terminaleManager.findAllObjectsManager().size() == 7);
		assertTrue(getOperationManager().findByObjectManager(t).size() == 1);
		
		Terminale tUp1 = terminaleManager.findByIdManager(id);
		Boolean catched = false;
		// on test l'update avec l'enceinte nulle
		try {
			terminaleManager.updateObjectManager(tUp1, null, type, 
					null, null, num, null, null, u, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		try {
			terminaleManager.updateObjectManager(tUp1, enc, null, 
					null, null, num, null, null, u, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
		...
		try {
			terminaleManager.updateObjectManager(tUp1, enc, type, 
		...
		try {
			terminaleManager.updateObjectManager(tUp1, enc, type, 
					null, null, num, null, incs, u, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
				catched = true;
		...
		Terminale tUp2 = terminaleManager.findByIdManager(id);
		tUp2.setNom("BTUP4");
		tUp2.setPosition(4);
		...
		terminaleManager.updateObjectManager(tUp2, enc, type, 
				banque, ent, num, 
				couleurDao.findById(8), incs,
				u, opTypes);
		assertTrue(getOperationManager().findByObjectManager(tUp2).size() == 5);
		// Vérification
		Terminale tTest2 = terminaleManager.findByIdManager(id);
		assertNotNull(tTest2);
		assertNotNull(tTest2.getEnceinte());
		assertNotNull(tTest2.getTerminaleNumerotation());
		assertNotNull(tTest2.getTerminaleType());
	...
	private void removeObjectManagerTest() {
		...
		newT.setPosition(3);
		terminaleManager.createObjectManager(
				newT, enc, type, null, null, num, null, u);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 7);
		Integer id = newT.getTerminaleId();
		
		Terminale tToRemove = terminaleManager.findByIdManager(id);
	...
	public void testCreateMultiObjetcsManager() {
		Utilisateur u = utilisateurDao.findById(1);
		Enceinte e3 = enceinteDao.findById(3);
		Enceinte e4 = enceinteDao.findById(4);
		TerminaleType type = terminaleTypeDao.findById(1);
		TerminaleNumerotation num = terminaleNumerotationDao.findById(1);
		Banque banque = banqueDao.findById(1);
		Entite ent = entiteDao.findById(3);
		...
		// à 50
		List<Terminale> list = terminaleManager
			.createMultiObjetcsManager(e4, newTs, 5, 50, u);
		...
		// on teste une création multiple valide avec une 1ère position
		// à null
		List<Terminale> list2 = terminaleManager
						.createMultiObjetcsManager(e4, newTs, 5, null, u);
	 ...
	 * @throws ParseException 
	 */
	private void validationInsert(Terminale terminale) 
		throws ParseException {
		Utilisateur u = utilisateurDao.findById(1);
		Enceinte enc = enceinteDao.findById(3);
		TerminaleType type = terminaleTypeDao.findById(1);
		...
		try {
			...
			terminaleManager.createObjectManager(terminale, enc, type, 
					null, null, num, null, u);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
	 ...
	 * @throws ParseException 
	 */
	private void validationUpdate(Terminale terminale) 
		throws ParseException {
		Utilisateur u = utilisateurDao.findById(1);
		Enceinte enc = enceinteDao.findById(3);
		TerminaleType type = terminaleTypeDao.findById(1);
		...
		try {
			terminale.setPosition(-1);
			terminaleManager.updateObjectManager(terminale, enc, type, 
					null, null, num, null, null, u, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
		...
		List<Incident> incs = incidentManager.findByTerminaleManager(terminale);
		Incident inc1 = new Incident();
	...
	public void testEchangerDeuxTerminalesManager() {
		...
		Enceinte ep1 = t1.getEnceinte();
		Integer pos1 = t1.getPosition();
		
		Enceinte ep2 = t2.getEnceinte();
		Integer pos2 = t2.getPosition();
		
		Enceinte ep4 = t4.getEnceinte();
		Integer pos4 = t4.getPosition();
		...
		assertTrue(getOperationManager()
					.findByObjectManager(tTest4).size() == 1);
		assertTrue(getOperationManager()
					.findByObjectManager(tTest1).size() == 1);
		...
		assertTrue(getOperationManager()
					.findByObjectManager(tTest4).size() == 2);
		assertTrue(getOperationManager()
					.findByObjectManager(tTest1).size() == 2);
	...
	public void testUpdateNumerotationForMultiTerminales() {
		...
		try {
			conteneurManager.createAllArborescenceManager(
				...
				banques, plateformes, null, false, u, bank1.getPlateforme());
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		List<Terminale> termsToUp = conteneurManager
			.getAllTerminalesInArborescenceManager(cTest);
		TerminaleNumerotation num2 = terminaleNumerotationDao.findById(2);
		// modif des numérotations
		terminaleManager.updateNumerotationForMultiTerminales(
				termsToUp, num2, u);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 96);
		
		// Vérification
		List<Terminale> termsToTest = conteneurManager
			.getAllTerminalesInArborescenceManager(cTest);
		for (int i = 0; i < termsToTest.size(); i++) {
			assertTrue(termsToTest.get(i)
					.getTerminaleNumerotation().equals(num2));
			assertTrue(termsToTest.get(i)
					.getEnceinte().equals(termsToUp.get(i)
							.getEnceinte()));
			assertTrue(termsToTest.get(i)
					.getTerminaleType().equals(termsToUp.get(i)
							.getTerminaleType()));
			assertTrue(termsToTest.get(i)
					.getNom().equals(termsToUp.get(i)
							.getNom()));
			assertTrue(termsToTest.get(i)
					.getPosition().equals(termsToUp.get(i)
							.getPosition()));
		}
	...
	public void testCreateMultiVisotubesManager() {
		...
		terminaleManager.createMultiVisotubesManager(
				e4, null, 5, null, false, 16, u);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 6);
	
		// teste visotube sans taille
		terminaleManager.createMultiVisotubesManager(
			e4, null, 5, null, false, null, u);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 6);
		...
		// à 50
		List<Terminale> list = terminaleManager
			.createMultiVisotubesManager(e4, newTs, 5, 50, false, 16, u);
		assertTrue(list.size() == 5);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 11);
		assertNotNull(list.get(0));
		assertNotNull(list.get(0).getEnceinte());
		...
		assertTrue(list.get(0).getTerminaleType().getType()
				.equals("VISOTUBE_16_ROND"));
		assertTrue(list.get(0).getCouleur().getCouleur()
				.equals("TRANSPARENT"));
		
		// on vérifie que les autres sont bien des visotubes traingulaires
		assertTrue(list.get(1).getTerminaleType().getType()
				.equals("VISOTUBE_16_TRI"));
		assertTrue(list.get(1).getCouleur().getCouleur()
				.equals("NOIR"));
		assertTrue(list.get(2).getTerminaleType().getType()
				.equals("VISOTUBE_16_TRI"));
		assertTrue(list.get(2).getCouleur().getCouleur()
				.equals("MARRON"));
		assertTrue(list.get(3).getTerminaleType().getType()
				.equals("VISOTUBE_16_TRI"));
		assertTrue(list.get(3).getCouleur().getCouleur()
				.equals("ROUGE"));
		...
		List<Terminale> list2 = terminaleManager
						.createMultiVisotubesManager(
								e4, newTs, 5, null, true, 16, u);
		assertTrue(list2.size() == 5);
		assertTrue(terminaleManager.findAllObjectsManager().size() == 11);
		assertNotNull(list2.get(0));
	...
	public void testGetTkObjectsAndEmplacementsManager() {
		Map<TKStockableObject, Emplacement> emps = terminaleManager
					.getTkObjectsAndEmplacementsManager(terminaleManager.findByIdManager(1));
		...
		emps = terminaleManager
				.getTkObjectsAndEmplacementsManager(terminaleManager.findByIdManager(2));
	...
	public void testFindTerminaleIdsFromNomManager() {
		List<Integer> ids = terminaleManager.findTerminaleIdsFromNomManager(null, null, null);
		...
		conts.addAll(conteneurDao.findAll());
		// nom find
		ids.addAll(terminaleManager.findTerminaleIdsFromNomManager("BT1", null, conts));
