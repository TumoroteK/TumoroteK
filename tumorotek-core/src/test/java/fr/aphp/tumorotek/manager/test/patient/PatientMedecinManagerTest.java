...
@Autowired
   ...
   private void createObjectManagerTest() throws ParseException{
      ...
      /*Champs obligatoires*/
      pm.setOrdre(1);
      final Patient p1 = patientDao.findById(2).orElse(null);
      ...
      assertNotNull(patientmedecinDao.findById(new PatientMedecinPK(c, p1))).orElse(null);
      ...
      assertTrue(catched);
      assertEquals(4, IterableUtils.toList(patientmedecinDao.findAll()).size());

      //validation test Type
      final Patient[] patients = new Patient[] {null, patientDao.findById(3).orElse(null)};
   ...
   private void updateObjectManagerTest() throws ParseException{
      ...
      pk.setPatient(patientDao.findById(2)).orElse(null);
      pk.setCollaborateur(collaborateurDao.findById(1)).orElse(null);
      //Modification d'un enregistrement
      final PatientMedecin pm = patientmedecinDao.findById(pk).orElse(null);
      pm.setOrdre(3);
      patientMedecinManager.createOrUpdateObjectManager(pm, null, null, "modification");
      assertTrue(patientmedecinDao.findById(pk).getOrdre().equals(3)).orElse(null);
      ...
      pm.setOrdre(2);
      try{
         patientMedecinManager.createOrUpdateObjectManager(pm, patientDao.findById(1), collaborateurDao.findById(2).orElse(null),
            "modification");
      }catch(final DoublonFoundException e){
         catched = true;
      }
      assertTrue(catched);
      assertNotNull(patientmedecinDao.findById(pk)).orElse(null);

      //test validation inutile car teste dans le create
   }

   private void removeObjectManagerTest(){
      final PatientMedecinPK pk = new PatientMedecinPK(collaborateurDao.findById(1), patientDao.findById(2)).orElse(null);
      //Suppression de l'enregistrement precedemment insere
      final PatientMedecin pm1 = patientmedecinDao.findById(pk).orElse(null);
      patientMedecinManager.removeObjectManager(pm1);
      assertNull(patientmedecinDao.findById(pk)).orElse(null);
