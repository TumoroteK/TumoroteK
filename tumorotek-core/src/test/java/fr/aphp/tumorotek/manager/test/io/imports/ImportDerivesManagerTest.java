...
public class ImportDerivesManagerTest extends AbstractManagerTest4 {
	...
	@Test
	public void testImportDerivesBatchesFile() {
		...
		File file = new File("src/test/java/fr/aphp/tumorotek/manager/"
				+ "test/io/imports/importDerivesBatches.xls");
		...
		Patient pat = patientManager.findByNomLikeManager("MEUNIER", true).get(0);
		...
		Maladie mal = maladieManager.findByLibelleLikeManager("TUMEUR", true).get(0);
		...
		Prelevement prel = prelevementManager.findByCodeLikeManager("PRLVT456", true).get(0);
		...
		Echantillon ech = echantillonManager.findByCodeLikeManager("PRLVT456.1", true).get(0);
		...
		Transformation t1 = transformationManager.findByEntiteObjetManager(entiteDao.findById(3), 
		...
		Transformation t2 = transformationManager.findByEntiteObjetManager(entiteDao.findById(3), 
		...
		assertTrue(importHistoriqueManager
				.findImportationsByHistoriqueManager(ih).size() == 12);
		...
		List<ProdDerive> derives = prodDeriveManager.findByCodeLikeManager("BATCH", false);
		fs.addAll(derives);
		for (ProdDerive prodDerive : derives) {
			prodDeriveManager.removeObjectManager(prodDerive, null, u, null);
		}
		
		transformationManager.removeObjectManager(t2, null, u);
		
		Emplacement e10 = echantillonManager.getEmplacementManager(ech);
		assertTrue(e10 != null);
	...
	public void testImportDerivesBatchesFileErrors() {
		...
		assertTrue(((ObjectStatutException) errors.get(1).getException())
												.getEntite().equals("Echantillon"));
		assertTrue(errors.get(2).getNbRow() == 6);
		assertTrue(errors.get(2).getException() instanceof ValidationException);
		assertTrue(((ValidationException) errors.get(2).getException()).getErrors()
				.get(0).getFieldError().getCode().equals("date.validation.supDateActuelle"));
		assertTrue(errors.get(3).getNbRow() == 11);
		assertTrue(errors.get(3).getException() instanceof ValidationException);
		assertTrue(((ValidationException) errors.get(3).getException()).getErrors()
				.get(0).getFieldError().getCode().equals("prodDerive.code.illegal"));
	...
	public void testImportSubDeriveForPrelevement() {
		Utilisateur u = utilisateurDao.findById(1);
		Entite prelEntite = entiteDao.findById(2);
		Entite deriveEntite = entiteDao.findById(8);
		ImportTemplate itTest2 = setDeriveImportTemplateUp(prelEntite, banqueDao.findById(2));
		...
		prelevementManager.createObjectManager(newPrel, itTest2.getBanque(), 
				p3.getNature(), p3.getMaladie(), p3.getConsentType(), null, null, null, 
				null, null, null, null, null, null, null, null, 
				u, false, null, false);
		assertTrue(prelevementManager.findByCodeLikeManager("newprel", true).get(0)
				.getQuantite().equals(new Float(100.0)));
				
		ImportHistorique ih = null;
		Integer totDerives = prodDeriveManager.findAllObjectsManager().size();
		Integer totOncf = objetNonConformeDao.findAll().size();
		Integer totAnno = annotationValeurManager.findAllObjectsManager().size();
				
		File file = new File("src/test/java/fr/aphp/tumorotek/manager/"
						+ "test/io/imports/importSubDerives.xls");
				
		List<ImportError> errors = new ArrayList<ImportError>();
		try {
			Workbook wb = WorkbookFactory.create(new FileInputStream(file));
			ih = importManager.importSubDeriveFileManager(itTest2, u, 
					wb.getSheetAt(0), null);
		} catch (Exception re) {
			errors = ((ErrorsInImportException) re).getErrors();
		...
		// transformations
		List<Transformation> trPrel3 = transformationManager
								.findByEntiteObjetManager(prelEntite, 3);
		assertTrue(trPrel3.size() == 3);
		// t1
		Transformation t1 = trPrel3.get(0);
		assertTrue(t1.getQuantite().equals(new Float(12.0)));
		assertTrue(t1.getQuantiteUnite().equals(p3.getQuantiteUnite()));
		List<ProdDerive> t1Derives = prodDeriveManager.findByTransformationManager(t1);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t1Derives.get(0).getProdDeriveId(), deriveEntite)
				.isEmpty());
		List<AnnotationValeur> vals = annotationValeurManager
									.findByObjectManager(t1Derives.get(0));
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t1Derives.get(1).getProdDeriveId(), deriveEntite)
				.size() == 4);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t1Derives.get(2).getProdDeriveId(), deriveEntite)
				.isEmpty());
		...
		// t2
		Transformation t2 = trPrel3.get(1);
		assertTrue(t2.getQuantite().equals(new Float(58.0)));
		assertTrue(t2.getQuantiteUnite().equals(p3.getQuantiteUnite()));
		List<ProdDerive> t2Derives = prodDeriveManager.findByTransformationManager(t2);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t2Derives.get(1).getProdDeriveId(), deriveEntite)
				.size() == 2);
		...
		// t3
		Transformation t3 = trPrel3.get(2);
		assertTrue(t3.getQuantite() == null);
		assertTrue(t3.getQuantiteUnite().equals(p3.getQuantiteUnite()));
		List<ProdDerive> t3Derives = prodDeriveManager.findByTransformationManager(t3);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t3Derives.get(1).getProdDeriveId(), deriveEntite)
				.size() == 1);
		...
		// transformations
		List<Transformation> trNewPrel = transformationManager
			.findByEntiteObjetManager(prelEntite, newPrel.getPrelevementId());
		assertTrue(trNewPrel.size() == 2);
		// t4
		Transformation t4 = trNewPrel.get(0);
		assertTrue(t4.getQuantite().equals(new Float(25.0)));
		assertTrue(t4.getQuantiteUnite() == null);
		List<ProdDerive> t4Derives = prodDeriveManager.findByTransformationManager(t4);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t4Derives.get(1).getProdDeriveId(), deriveEntite)
				.isEmpty());
		...
		// t5
		Transformation t5 = trNewPrel.get(1);
		assertTrue(t5.getQuantite().equals(new Float(75.0)));
		assertTrue(t5.getQuantiteUnite() == null);
		List<ProdDerive> t5Derives = prodDeriveManager.findByTransformationManager(t5);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t5Derives.get(0).getProdDeriveId(), deriveEntite)
				.isEmpty());
		...
		prelevementManager.updateObjectManager(p3, p3.getBanque(), p3.getNature(), 
				p3.getMaladie(), p3.getConsentType(), p3.getPreleveur(), 
				p3.getServicePreleveur(), p3.getPrelevementType(),
				p3.getConditType(), p3.getConditMilieu(), 
				p3.getTransporteur(), p3.getOperateur(), p3.getQuantiteUnite(), 
				null, null, null, null, null, u, null, false, null, false);
		
		List<Operation> ops = getOperationManager()
				.findByObjetIdEntiteAndOpeTypeManager(p3, 
								operationTypeDao.findById(5));
	...
	public void testImportSubDeriveForPrelevementErrors() {
		...
		prelevementManager.createObjectManager(newPrel, itTest2.getBanque(), 
				p3.getNature(), p3.getMaladie(), p3.getConsentType(), null, null, null, 
				null, null, null, null, p3.getQuantiteUnite(), null, null, null, 
				u, false, null, false);
		assertTrue(prelevementManager.findByCodeLikeManager("newprel", true).get(0)
				.getQuantite().equals(new Float(100.0)));
		...
		assertTrue(((ValidationException) e.getException()).getErrors()
				.get(0).getFieldError().getCode().equals("prodDerive.code.illegal"));
		i++;
		e = errors.get(i);
		assertTrue(e.getNbRow() == 8);
		assertTrue(e.getException() instanceof TransformationQuantiteOverDemandException);
		assertTrue(((TransformationQuantiteOverDemandException) e.getException())
				.getQteDemandee().equals(new Float(500)));
		assertTrue(((TransformationQuantiteOverDemandException) e.getException())
				.getQteRestante().equals(new Float(100)));
		i++;
		e = errors.get(i);
		assertTrue(e.getNbRow() == 10);
		assertTrue(e.getException() instanceof ValidationException);
		assertTrue(((ValidationException) e.getException()).getErrors()
										.get(0).getFieldError().getCode().equals("anno.alphanum.illegal"));
	...
	public void testImportSubDeriveForEchantillon() throws ParseException {
		Utilisateur u = utilisateurDao.findById(1);
		Entite echEntite = entiteDao.findById(3);
		Entite deriveEntite = entiteDao.findById(8);
		ImportTemplate itTest2 = setDeriveImportTemplateUp(echEntite, banqueDao.findById(1));
		
		Integer totDerives = prodDeriveManager.findAllObjectsManager().size();
		Integer totOncf = objetNonConformeDao.findAll().size();
		Integer totAnno = annotationValeurManager.findAllObjectsManager().size();
		Integer totRetour = retourManager.findAllObjectsManager().size();
		...
		echantillonManager.createObjectManager(newEch, itTest2.getBanque(), 
				e2.getPrelevement(), 
				null, objetStatutDao.findById(1), e4, 
				e2.getEchantillonType(), null, null, null,
				null, null, null, null, u, false, null, false);
		assertTrue(echantillonManager.findByCodeLikeManager("NEWECH", true).get(0)
				.getQuantite().equals(new Float(100.0)));
		e4.setVide(false);
		...
		Retour incomp1 = new Retour();
		incomp1.setTempMoyenne(new Float(20));
		incomp1.setDateSortie(dateS);
		retourManager.createOrUpdateObjectManager(incomp1, newEch, 
				emplacementManager.findByIdManager(4), 
					null, null, null, null, u, "creation");
		assertTrue(echantillonManager.findByIdManager(newEch.getEchantillonId())
				.getObjetStatut().getStatut().equals("ENCOURS"));
		Retour incomp2 = new Retour();
		incomp2.setTempMoyenne(new Float(22));
		incomp2.setDateSortie(dateS);
		retourManager.createOrUpdateObjectManager(incomp2, e2, 
				emplacementManager.findByIdManager(3), 
					null, null, null, null, u, "creation");
		assertTrue(echantillonManager.findByIdManager(e2.getEchantillonId())
				.getObjetStatut().getStatut().equals("ENCOURS"));
		...
		Retour complete1 = retourManager.getRetoursForObjectManager(e2).get(0);
		assertTrue(complete1.getDateSortie().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:10")));
		assertTrue(complete1.getDateRetour().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:30")));
		assertTrue(complete1.getTempMoyenne().equals(new Float(22)));

		List<TKFantomableObject> fs = new ArrayList<TKFantomableObject>();
		
		// transformations
		List<Transformation> trEch2 = transformationManager
								.findByEntiteObjetManager(echEntite, 2);
		assertTrue(trEch2.size() == 3);
		// t1 = complete Retour
		Transformation t1 = trEch2.get(0);
		assertTrue(t1.getQuantite().equals(new Float(12.0)));
		...
		// t3
		Transformation t3 = trEch2.get(1);
		assertTrue(t3.getQuantite() == null);
		...
		// t4
		Transformation t4 = trEch2.get(2);
		...
		Retour complete2 = retourManager.getRetoursForObjectManager(newEch).get(0);
		assertTrue(complete2.getDateSortie().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:10")));
		assertTrue(complete2.getDateRetour().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:33")));
		assertTrue(complete2.getTempMoyenne().equals(new Float(20)));
		Retour epuisement = retourManager.getRetoursForObjectManager(newEch).get(1);
		assertTrue(epuisement.getDateSortie().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:40")));
		...
		// transformations
		List<Transformation> trNewEch = transformationManager
			.findByEntiteObjetManager(echEntite, newEch.getEchantillonId());
		assertTrue(trNewEch.size() == 3);
		
		// t2
		Transformation t2 = trNewEch.get(0);
		...
		// t5
		Transformation t5 = trNewEch.get(1);
		...
		// t6
		Transformation t6 = trNewEch.get(2);
		assertTrue(t6.getQuantite().equals(new Float(65.0)));
		assertTrue(t6.getQuantiteUnite() == null);
		List<ProdDerive> t6Derives = prodDeriveManager.findByTransformationManager(t6);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t6Derives.get(0).getProdDeriveId(), deriveEntite)
				.isEmpty());
		...
		echantillonManager.updateObjectManager(e2, e2.getBanque(), e2.getPrelevement(), 
				e2.getCollaborateur(), e2.getObjetStatut(), e2.getEmplacement(), 
				e2.getEchantillonType(), null, null, e2.getQuantiteUnite(), e2.getEchanQualite(), 
				e2.getModePrepa(), null, null, null, null, null, u, false, 
				null, null);
		List<Operation> ops = getOperationManager()
				.findByObjetIdEntiteAndOpeTypeManager(e2, 
								operationTypeDao.findById(5));
		assertTrue(ops.size() == 1);
		getOperationManager().removeObjectManager(ops.get(0));
	...
	public void testImportSubDeriveForEchantillonErrors() throws ParseException {
		...
		echantillonManager.createObjectManager(newEch, itTest2.getBanque(), 
				e2.getPrelevement(), 
				null, objetStatutDao.findById(1), e4, 
				e2.getEchantillonType(), null, e2.getQuantiteUnite(), null, 
				null, null, null, null, u, false, null, false);
		assertTrue(echantillonManager.findByCodeLikeManager("NEWECH", true).get(0)
				.getQuantite().equals(new Float(100.0)));
		e4.setVide(false);
		...
		assertTrue(e.getException() instanceof ValidationException);
		assertTrue(((ValidationException) e.getException()).getErrors()
										.get(0).getFieldError().getCode().equals("date.validation.supDateRetour"));
		i++;
		e = errors.get(i);
		assertTrue(e.getNbRow() == 6);
		...
		assertTrue(e.getException() instanceof TransformationQuantiteOverDemandException);
		assertTrue(((TransformationQuantiteOverDemandException) e.getException())
				.getQteDemandee().equals(new Float(40)));
		// les deux premiers transf ont ampute la qte avant exception
		assertTrue(((TransformationQuantiteOverDemandException) e.getException())
				.getQteRestante().equals(new Float(1)));
		...
		echantillonManager.updateObjectManager(newEch, newEch.getBanque(), 
				newEch.getPrelevement(), null, newEch.getObjetStatut(), null, 
				newEch.getEchantillonType(), null, null, null, null, 
	...
	public void testImportSubDeriveForDerive() throws ParseException {
		Utilisateur u = utilisateurDao.findById(1);
		Entite deriveEntite = entiteDao.findById(8);
		ImportTemplate itTest2 = setDeriveImportTemplateUp(deriveEntite, banqueDao.findById(1));
		...
		Emplacement e4 = emplacementManager.findByIdManager(4);
		prodDeriveManager.createObjectManager(newDerive, itTest2.getBanque(), p2.getProdType(), 
				objetStatutDao.findById(1), null, e4, null, null, null, null, null, null, 
				null, null, null, u, false, null, false);
		assertTrue(prodDeriveManager.findByCodeLikeManager("NEWDER", true).get(0)
				.getQuantite().equals(new Float(100.0)));
		e4.setVide(false);
		...
		incomp1.setTempMoyenne(new Float(20));
		incomp1.setDateSortie(dateS);
		retourManager.createOrUpdateObjectManager(incomp1, newDerive, 
				emplacementManager.findByIdManager(4), 
					null, null, null, null, u, "creation");
		assertTrue(prodDeriveManager.findByIdManager(newDerive.getProdDeriveId())
				.getObjetStatut().getStatut().equals("ENCOURS"));
		Retour incomp2 = new Retour();
		incomp2.setTempMoyenne(new Float(22));
		incomp2.setDateSortie(dateS);
		retourManager.createOrUpdateObjectManager(incomp2, p2, 
				emplacementManager.findByIdManager(3), 
					null, null, null, null, u, "creation");
		assertTrue(prodDeriveManager.findByIdManager(p2.getProdDeriveId())
				.getObjetStatut().getStatut().equals("ENCOURS"));
		...
		try {
			Workbook wb = WorkbookFactory.create(new FileInputStream(file));
			ih = importManager.importSubDeriveFileManager(itTest2, u, 
					wb.getSheetAt(4), "OBS SUB DERIVES TEST");
		} catch (Exception re) {
			errors = ((ErrorsInImportException) re).getErrors();
		 }
		...
		Retour complete1 = retourManager.getRetoursForObjectManager(p2).get(0);
		assertTrue(complete1.getDateSortie().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:10")));
		...
		// transformations
		List<Transformation> trEch2 = transformationManager
								.findByEntiteObjetManager(deriveEntite, 2);
		...
		assertTrue(objetNonConformeDao
				.findByObjetAndEntite(t1Derives.get(4).getProdDeriveId(), deriveEntite)
				.size() == 1);
		...
		Retour complete2 = retourManager.getRetoursForObjectManager(newDerive).get(0);
		assertTrue(complete2.getDateSortie().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:10")));
		// date retour = current date au moment de l'enregistrement
		assertTrue(complete2.getDateRetour().getTime()
				.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("24/03/2015 10:37")));
		assertTrue(complete2.getTempMoyenne().equals(new Float(20)));
		Retour epuisement = retourManager.getRetoursForObjectManager(newDerive).get(1);
		...
		// transformations
		List<Transformation> trNewEch = transformationManager
			.findByEntiteObjetManager(deriveEntite, newDerive.getProdDeriveId());
		assertTrue(trNewEch.size() == 3);
	...
	public void testImportSubDeriveForDeriveErrors() throws ParseException {
		...
		try {
			Workbook wb = WorkbookFactory.create(new FileInputStream(file));
			ih = importManager.importSubDeriveFileManager(itTest2, u, 
					wb.getSheetAt(5), "OBS DERIVE TEST");
		} catch (Exception re) {
			errors = ((ErrorsInImportException) re).getErrors();
		 }
		...
		assertTrue(e.getException() instanceof TransformationQuantiteOverDemandException);
		assertTrue(((TransformationQuantiteOverDemandException) e.getException())
				.getQteDemandee().equals(new Float(67)));
		// les deux premiers transf ont ampute la qte avant exception
		assertTrue(((TransformationQuantiteOverDemandException) e.getException())
				.getQteRestante().equals(new Float(65)));
		...
		prodDeriveManager.updateObjectManager(newDerive, newDerive.getBanque(), newDerive.getProdType(), 
				newDerive.getObjetStatut(), null, null, null, null, null, null, null, 
				null, null, null, null, null, null, u, false, null, null);
	...
	private ImportTemplate setDeriveImportTemplateUp(Entite parentEntite, Banque b) {
		// creation du filtre
		ImportTemplate itDerive = new ImportTemplate();
		...
		Champ ch5 = new Champ();
		ImportColonne ic5 = new ImportColonne();
		...
		Champ ch6 = new Champ();
		ImportColonne ic6 = new ImportColonne();
		...
		Champ ch7 = new Champ();
		ImportColonne ic7 = new ImportColonne();
		...
		Champ ch8 = new Champ();
		ImportColonne ic8 = new ImportColonne();
		...
		if (importTemplateManager.findDoublonManager(itDerive)) {
			...
			for (ImportTemplate tpl : importTemplateManager.findByBanqueManager(b)) {
		...
		Integer idT2 = itDerive.getImportTemplateId();
		...
		assertTrue(importColonneManager
						.findByImportTemplateManager(itTest2).size() == 8);
