...
public class EchantillonTypeManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		EchantillonType typeNull = echantillonTypeManager.findByIdManager(5);
		assertNull(typeNull);
	}
	...
	public void testFindByEchantillon() {
		...
		EchantillonType type = 
			echantillonTypeManager.findByEchantillonManager(e);
		assertNotNull(type);
	...
	public void testGetEchantillons() {
		...
		Set<Echantillon> echans = 
			echantillonTypeManager.getEchantillonsManager(type);
		assertNotNull(echans);
		assertTrue(echans.size() == 3);
		
		EchantillonType type2 = echantillonTypeManager.findByIdManager(2);
		Set<Echantillon> echans2 = 
			echantillonTypeManager.getEchantillonsManager(type2);
	...
	public void testFindByTypeLikeExactManager() {
		List<EchantillonType> list = 
			echantillonTypeManager.findByTypeLikeManager("ADN", true);
	...
	public void testFindByTypeLikeManager() {
		List<EchantillonType> list = 
			echantillonTypeManager.findByTypeLikeManager("ADN", false);
		assertTrue(list.size() == 1);
	...
	public void testCrud() {
		// Insertion
		EchantillonType type1 = new EchantillonType();
		type1.setType("ADN");
		type1.setIncaCat("CAT2");
		Boolean catched = false;
		try {
			echantillonTypeManager.createObjectManager(type1);
		} catch (Exception e) {
		...
		String[] emptyValues = new String[]{"", "  ", null, "€€¢¢¢®®®€", 
				createOverLength(200), "GOOD"};
		String[] incaCatValues = new String[]{"", " ", "€€¢¢¢®®®€", 
		...
		assertTrue(echantillonTypeManager
		...
		// Update
		EchantillonType type2 = echantillonTypeManager.findByIdManager(5);
		String typeUpdated = "ADN";
		String catUpdated1 = "CAT2";
		String catUpdated2 = "CAT";
		// On teste une mise à jour valide
		type2.setType(typeUpdated);
		type2.setIncaCat(catUpdated2);
		echantillonTypeManager.updateObjectManager(type2);
		// On teste l'update d'un doublon pour vérifier que l'exception
		// est lancée
		EchantillonType type3 = echantillonTypeManager.findByIdManager(5);
		type3.setIncaCat(catUpdated1);
		Boolean catchedUpdate = false;
		try {
			echantillonTypeManager.updateObjectManager(type3);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catchedUpdate = true;
		...
		EchantillonType type4 = echantillonTypeManager.findByIdManager(5);
		assertTrue(type4.getType().equals(typeUpdated));

		// Suppression
		EchantillonType type5 = echantillonTypeManager.findByIdManager(5);
		echantillonTypeManager.removeObjectManager(type5);
		assertNull(echantillonTypeManager.findByIdManager(5));
		EchantillonType type6 = echantillonTypeManager.findByIdManager(1);
		try {
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					echantillonTypeManager.findByOrderManager(pf);
