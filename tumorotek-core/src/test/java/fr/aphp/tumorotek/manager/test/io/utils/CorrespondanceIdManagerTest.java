...
public class CorrespondanceIdManagerTest extends AbstractManagerTest4
{
   ...
   public void testPatientPrelevements(){
      final Entite patient = entiteDao.findById(1).orElse(null);
      final Entite prelevement = entiteDao.findById(2).orElse(null);

      // from Patient
      final List<Banque> banks = IterableUtils.toList(banqueDao.findAll());
   ...
   public void testPrelevementsEchantillons(){
      final Entite echantillon = entiteDao.findById(3).orElse(null);
      final Entite prelevement = entiteDao.findById(2).orElse(null);

      // from Patient
      List<Banque> banks = IterableUtils.toList(banqueDao.findAll());
      final List<Integer> pIds = new ArrayList<>();
      List<Integer> res;
      ...
      banks.remove(banqueDao.findById(1)).orElse(null);
      res = correspondanceIdManager.findTargetIdsFromIdsManager(pIds, prelevement, echantillon, banks, null);
      ...
      banks = IterableUtils.toList(banqueDao.findAll());
      res = correspondanceIdManager.findTargetIdsFromIdsManager(pIds, echantillon, prelevement, banks, null);
      assertTrue(res.size() == 3);
   }
   ...
   public void testEchantillonsDerives(){
      final Entite echantillon = entiteDao.findById(3).orElse(null);
      final Entite derive = entiteDao.findById(8).orElse(null);
   ...
   public void testCessionEchantillons(){
      final Entite cession = entiteDao.findById(5).orElse(null);
      final Entite echantillon = entiteDao.findById(3).orElse(null);
