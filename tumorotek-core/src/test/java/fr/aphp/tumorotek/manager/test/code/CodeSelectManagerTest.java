...
public class CodeSelectManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<CodeSelect> codes = (List<CodeSelect>) 
						codeSelectManager.findAllObjectsManager();
	...
	public void testFindByCodeDossierManager() {
		...
		List<CodeSelect> codes = 
			codeSelectManager.findByCodeDossierManager(dos);
	...
	public void testFindCodesFromSelectByDossierManager() {
		...
		List<CodeCommon> codes = 
			codeSelectManager.findCodesFromSelectByDossierManager(dos);
		assertTrue(codes.size() == 2);
		dos = codeDossierDao.findById(2);
	...
	public void testFindByRootDossierManager() {
		...
		List<CodeCommon> codes = 
			codeSelectManager.findByRootDossierManager(u, b);
	...
	public void testFindByUtilisateurAndBanqueManager() {
		...
		Banque b = banqueDao.findById(1);
		List<CodeSelect> codes = 
			codeSelectManager.findByUtilisateurAndBanqueManager(u, b);
	...
	public void testFindCodesFromSelectByUtilisateurAndBanqueManager() {
		...
		List<CodeCommon> codes = 
			codeSelectManager
					.findCodesFromSelectByUtilisateurAndBanqueManager(u, b);
		assertTrue(codes.size() == 3);
		u = utilisateurDao.findById(2);
		codes = codeSelectManager
					.findCodesFromSelectByUtilisateurAndBanqueManager(u, b);
		...
		//null recherche
		codes = codeSelectManager
			.findCodesFromSelectByUtilisateurAndBanqueManager(null, null);
		assertTrue(codes.size() == 0);
	}
	...
	public void testFindDoublonManager() {
		...
		Utilisateur u = utilisateurDao.findById(1);
		//Cree le doublon
		CodeSelect c1 = codeSelectManager
					.findByUtilisateurAndBanqueManager(u, b).get(0);
		
		CodeSelect c2 = new CodeSelect();
	...
	private void createObjectManagerTest() {
		...
		CodeSelect code = new CodeSelect();		
		/*Champs obligatoires*/
		...
		//required Banque
		try {
			codeSelectManager.createOrUpdateManager(code, dos, null, 
														u, "creation");
		...
		try {
			codeSelectManager.createOrUpdateManager(code, dos, b, 
													null, "creation");
		} catch (RequiredObjectIsNullException re) {
			assertTrue(true);
		}
		//operation invalide
		try {
			codeSelectManager.createOrUpdateManager(code, null, b, 
															u, null);
		...
		assertTrue(codeSelectManager
					.findByUtilisateurAndBanqueManager(u, b).size() == 1);
		
		assertTrue(codeSelectManager
			.findByCodeDossierManager(codeDossierDao.findById(3)).size() == 3);
		...
		try {
			codeSelectManager.createOrUpdateManager(c2, null, null, 
															null, "creation");
		} catch (Exception ex) {
	...
	private void updateObjectManagerTest() {
		...
		Utilisateur u = utilisateurDao.findById(3);
		CodeDossier dos = codeDossierDao.findById(3);
		CodeSelect c = (CodeSelect) codeSelectManager
										.findByCodeDossierManager(dos).get(2);
		c.setCodeDossier(null);
		codeSelectManager
					.createOrUpdateManager(c, null, null, null, "modification");
		
		assertTrue(codeSelectManager
								.findByRootDossierManager(u, b).size() == 1);
	...
	private void removeObjectManagerTest() {
		...
		assertTrue(codeSelectManager
						.findByUtilisateurAndBanqueManager(u, b).size() == 0);
	...
	public void testFindByCodeOrLibelleLikeManager() {
		...
		List<CodeCommon> codes = codeSelectManager
						.findByCodeOrLibelleLikeManager("A", true, u, b);
		assertTrue(codes.size() == 1);
		//teste une recherche non exactMatch
		codes = codeSelectManager
						.findByCodeOrLibelleLikeManager("A", false, u, b);
		assertTrue(codes.size() == 2);
		//teste une recherche non exactMatch
		codes = codeSelectManager
						.findByCodeOrLibelleLikeManager("ales", false, u, b);
		assertTrue(codes.size() == 1);
		//teste une recherche infructueuse
		codes = codeSelectManager
						.findByCodeOrLibelleLikeManager("LUX", true, u, b);
		assertTrue(codes.size() == 0);
		//null recherche
		codes = codeSelectManager
						.findByCodeOrLibelleLikeManager(null, false, u, b);
		assertTrue(codes.size() == 0);
	}
	...
	public void testFindByRootDossierAndBanqueManager() {
		Banque b = banqueDao.findById(1);
		List<CodeCommon> codes 
			= codeSelectManager.findByRootDossierAndBanqueManager(b);
		assertTrue(codes.size() == 1);
		b = banqueDao.findById(2);
