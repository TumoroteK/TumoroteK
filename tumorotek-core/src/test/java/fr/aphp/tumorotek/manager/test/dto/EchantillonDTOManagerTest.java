...
public class EchantillonDTOManagerTest extends AbstractManagerTest4 {
	...
	public void testInitEchantillonDecoratorManager() {
		...
		// echantillon test2
		dto = echantillonDTOManager.initEchantillonDecoratorManager(echantillonDao.findById(2));
	...
	public void testDecorateAndUnDecorateManager() {
		...
		List<EchantillonDTO> dtos = echantillonDTOManager.decorateListeManager(echans);
		assertTrue(dtos.size() == echans.size());
		
		List<Echantillon> extracted = echantillonDTOManager.extractListeManager(dtos);
