...
public class ResultatManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Resultat resultat = manager.findByIdManager(3);
		assertNotNull(resultat);
		assertTrue(resultat.getResultatId().equals(3));
		
		Resultat resultatNull = manager.findByIdManager(50);
	...
	public void testFindAllObjects() {
		List<Resultat> list = manager.findAllObjectsManager();
		//On vérifie chaque élément par un findById
		Iterator<Resultat> it = list.iterator();
		while (it.hasNext()) {
			Resultat temp = it.next();
			if (temp != null) {
				assertTrue(temp.equals(
						manager.findByIdManager(temp.getResultatId())));
			}
		}
	}
	...
	public void testCopyResultat() {
		...
		Affichage affichage = affichageDao.findById(3);
		Resultat resultat = manager.findByIdManager(4);
		Resultat copie = manager.copyResultatManager(resultat, affichage);
		...
		assertTrue(copie.equals(
				manager.findByIdManager(copie.getResultatId())));
	...
	public void testCrud() {
		...
		for (int i = 0; i < Math.pow(2, 3) - 1; i++) {
			...
			Champ champ = null;
			if (toTest > 0) {				
				champ = new Champ(champEntiteDao.findById(29));
			}
			try {
		...
		/** Save. */
		Affichage affichage = affichageDao.findById(3);
		Champ champ = new Champ(champEntiteDao.findById(56));
		Resultat resultat = new Resultat("columnName", champ,
				ConstanteIO.TRI_ASC, 2, 1, "", affichage);
		manager.createObjectManager(resultat, affichage, champ);
		...
		Champ champ2 = new Champ(champEntiteDao.findById(58));
		
		manager.updateObjectManager(resultat, resultat.getAffichage(), 
				champ2);
		
		assertFalse(resultat.equals(resultat2));
		assertFalse(resultat.getNomColonne().equals(resultat2.getNomColonne()));
		assertTrue(manager.findByIdManager(resultat.getResultatId()).getChamp()
				.getChampEntite().getChampEntiteId().equals(58));
		/** Remove. */
		int id = resultat.getResultatId();
	...
	public void testFindByAffichage() {
		...
		List<Affichage> affichages = affichageDao.findAll();
		//On vérifie chaque élément par un findById
		Iterator<Affichage> it = affichages.iterator();
		while (it.hasNext()) {
			Affichage temp = it.next();
			if (temp != null) {
				List<Resultat> list = manager.findByAffichageManager(temp);
				//On vérifie chaque élément par un findById
				Iterator<Resultat> it2 = list.iterator();
				while (it2.hasNext()) {
					Resultat res = it2.next();
					if (res != null) {
