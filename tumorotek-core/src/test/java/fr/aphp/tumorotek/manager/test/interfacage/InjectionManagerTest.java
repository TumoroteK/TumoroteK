...
//    }
	...
	public void testSetPropertyValueForObject() {
		...
		injectionManager.setPropertyValueForObject(
				"20051110123517", ce, prlvt);
	...
	public void testSetPropertyValueForAnnotationValeur() {
		...
		av = injectionManager.setPropertyValueForAnnotationValeur(
				"CODE", ca, banque);
	...
	public void testExtractValueForOneThesaurus() {
		...
		// Thésaurus Nature
		ChampEntite ce111 = champEntiteDao.findById(111);
		Object obj = injectionManager.extractValueForOneThesaurus(
				ce111, banque, "TISSU");
		assertNotNull(obj);
		assertTrue(((Nature) obj).getNature().equals("TISSU"));
		obj = injectionManager.extractValueForOneThesaurus(
				ce111, banque, "XXX");
		assertNull(obj);
		
		// Thésaurus Collaborateur
		ChampEntite ce199 = champEntiteDao.findById(199);
		obj = injectionManager.extractValueForOneThesaurus(
				ce199, banque, 2);
		assertNotNull(obj);
	...
	public void testExtractValueForOneAnnotationThesaurus() {
		...
		// Thésaurus
		ChampAnnotation ce11 = champAnnotationManager
			.findByNomManager("Thes1").get(0);
		Object obj = injectionManager.extractValueForOneAnnotationThesaurus(
				ce11, banque, "item1-1");
		assertNotNull(obj);
		assertTrue(((Item) obj).getLabel().equals("item1-1"));
		obj = injectionManager.extractValueForOneAnnotationThesaurus(
				ce11, banque, "XXX");
		assertNull(obj);
		
		// Thésaurus
		ChampAnnotation ce28 = champAnnotationManager
			.findByNomManager("010 : Taille de la tumeur : cT").get(0);
		obj = injectionManager.extractValueForOneAnnotationThesaurus(
				ce28, banque, "X");
		assertNotNull(obj);
	...
	public void testInjectValeurExterneInAnnotationObject() {
		...
		assertTrue(
			annos.get(0).getChampAnnotation().getChampAnnotationId() == 1);
		assertTrue(annos.get(0).getBanque().equals(banque));
		
		// Injection d'une date valide
		annos = new ArrayList<AnnotationValeur>();
		ve = new ValeurExterne();
		ve.setValeur("20051110123517");
		ve.setChampAnnotationId(5);
		injectionManager.injectValeurExterneInObject(echan, banque, ve, annos);
		assertTrue(annos.size() == 1);
		assertTrue(format.format(annos.get(0).getDate().getTime())
				.equals("2005-11-10"));
		assertTrue(
			annos.get(0).getChampAnnotation().getChampAnnotationId() == 5);
		assertTrue(annos.get(0).getBanque().equals(banque));
		...
		assertTrue(
			annos.get(0).getChampAnnotation().getChampAnnotationId() == 3);
		assertTrue(annos.get(0).getBanque().equals(banque));
		...
		assertTrue(
			annos.get(0).getChampAnnotation().getChampAnnotationId() == 7);
		assertTrue(annos.get(0).getBanque().equals(banque));
		...
		assertTrue(
			annos.get(0).getChampAnnotation().getChampAnnotationId() == 11);
		assertTrue(annos.get(0).getBanque().equals(banque));
	...
	*/
	...
	public void testInjectBlocExterneInObject() {
		Banque banque = banqueDao.findById(1);
		List<AnnotationValeur> annoValeurs = new ArrayList<AnnotationValeur>();
		
		BlocExterne b1 = blocExterneDao.findById(1);
		Patient patient = new Patient();
		injectionManager.injectBlocExterneInObject(
				patient, banque, b1, annoValeurs);
		...
		annoValeurs = new ArrayList<AnnotationValeur>();
		BlocExterne b2 = blocExterneDao.findById(2);
		Prelevement prlvt = new Prelevement();
		injectionManager.injectBlocExterneInObject(
				prlvt, banque, b2, annoValeurs);
	...
	public void testInjectDossierManager() {
		DossierExterne dossier = dossierExterneDao.findById(1);
		Banque b1 = banqueDao.findById(1);
		ResultatInjection resultat = injectionManager
			.injectDossierManager(dossier, b1);
