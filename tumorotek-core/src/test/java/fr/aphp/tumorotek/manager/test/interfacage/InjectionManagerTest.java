...
public class InjectionManagerTest extends AbstractManagerTest4
{
	...
	public void testSetPropertyValueForAnnotationValeur(){
		...
		// test avec un thesaurus
		final Item it = itemDao.findById(12).orElse(null);
		av = new AnnotationValeur();
	...
	public void testExtractValueForOneThesaurus(){
		...
		// Thésaurus Nature
		final ChampEntite ce111 = champEntiteDao.findById(111).orElse(null);
		...
		final ChampEntite ce199 = champEntiteDao.findById(199).orElse(null);
	...
	public void testInjectBlocExterneInObject(){
		...
		final BlocExterne b1 = blocExterneDao.findById(1).orElse(null);
		...
		final BlocExterne b2 = blocExterneDao.findById(2).orElse(null);
		final Prelevement prlvt = new Prelevement();
	...
	public void testDaveDossierAndChildrenManager() throws ParseException {
		...
		final Emetteur e1 = emetteurDao.findById(1).orElse(null);

		final Integer nbD = dossierExterneManager.findAllObjectsManager().size();
		final Integer nbB = IterableUtils.toList(blocExterneDao.findAll()).size();
		final Integer nbV = IterableUtils.toList(valeurExterneDao.findAll()).size();
		...
		der2.setEmetteur(emetteurDao.findById(1)).orElse(null);
		der2.setOperation("D");
		der2.setEntiteId(8);
		...
		assertTrue(IterableUtils.toList(blocExterneDao.findAll()).size() == nbB + 6);
		assertTrue(IterableUtils.toList(valeurExterneDao.findAll()).size() == nbV + 15);
		...
		assertTrue(IterableUtils.toList(blocExterneDao.findAll()).size() == nbB);
	...
	public void testFindExistingPrelevementByEmetteurManager() {
		Emetteur emet = emetteurDao.findById(3).orElse(null);
		Plateforme pf1 = plateformeDao.findById(1).orElse(null);
	...
	public void testSynchronizeDeriveChildrenManager() {
		...
		final Utilisateur u1 = utilisateurDao.findById(1).orElse(null);
		final Emetteur e2 = emetteurDao.findById(2).orElse(null);
		...
		assertTrue(getOperationManager()
			.findByObjetIdEntiteAndOpeTypeManager(prelevementManager.findByIdManager(1), 
				operationTypeDao.findById(19)).size() == 1).orElse(null); // sync
		assertTrue(getOperationManager()
		...
		getOperationManager().removeObjectManager(getOperationManager()
			.findByObjetIdEntiteAndOpeTypeManager(prelevementManager.findByIdManager(1), 
					operationTypeDao.findById(19)).get(0)).orElse(null); // sync
		getOperationManager().removeObjectManager(getOperationManager()
			.findByObjetIdEntiteAndOpeTypeManager(prelevementManager.findByIdManager(2), 
					operationTypeDao.findById(19)).get(0)).orElse(null); // sync
