 ...
 *
 */
public class MaladieManagerTest extends AbstractManagerTest {
	...
	public void setPatientManager(PatientManager ptManager) {
		this.patientManager = ptManager;
	}
	
	public void setPatientDao(PatientDao ptDao) {
		this.patientDao = ptDao;
	}
	...
	public void testFindAllObjectsManager() {
		List<Maladie> mals = maladieManager.findAllObjectsManager();
		assertEquals(6, mals.size());
	}
	
	/**
	 * Test la methode findByLibelleLikeManager.
	 */
	public void testFindByLibelleLikeManager() {
		//teste une recherche exactMatch
		List<Maladie> mals = 
			maladieManager.findByLibelleLikeManager("Fracture", true);
	...
	public void testFindByCodeLikeManager() {
		//teste une recherche exactMatch
		List<Maladie> mals = 
			maladieManager.findByCodeLikeManager("12.56", true);
		assertTrue(mals.size() == 1);
		//teste une recherche non exactMatch
	...
	public void testFindDoublon() throws ParseException {
		//Cree le doublon
		Maladie m1 = maladieManager
								.findByCodeLikeManager("12.56", true).get(0);
		Maladie m2 = new Maladie();
		m2.setLibelle(m1.getLibelle());
		m2.setPatient(m1.getPatient());
		m2.setDateDiagnostic(new SimpleDateFormat("dd/MM/yyyy")
														.parse("03/06/2009"));
		assertTrue(m2.equals(m1));
		assertTrue(maladieManager.findDoublonManager(m2));
	}
	...
	public void testIsUsedObject() {
		...
		Maladie m1 = maladieManager
					.findByLibelleLikeManager("Addiction medocs", true).get(0);
	...
	public void testGetPrelevementsManager() {
		Maladie m = maladieManager
					.findByLibelleLikeManager("Addiction coco", true).get(0);
		assertTrue((maladieManager.getPrelevementsManager(m)).size() == 1);
		assertTrue((maladieManager
			.getPrelevementsManager(m)).iterator().next()
										.equals(prelevementDao.findById(2)));
		m = maladieManager
			.findByLibelleLikeManager("Fracture", true).get(0);
	...
	public void createObjectManagerTest() throws ParseException {
		...
		/*Champs obligatoires*/
		Patient p = patientDao.findById(2);
		m.setLibelle("La maladie du siecle --2009");
		m.setCode("12.12-1");
		Date dateDiag = new SimpleDateFormat("dd/MM/yyyy")
												.parse("17/09/2006");
		m.setDateDiagnostic(dateDiag);
		Date dateDebut = new SimpleDateFormat("dd/MM/yyyy").parse("06/09/2000");
		...
		//insertion
		maladieManager.createOrUpdateObjectManager(m, p, 
													medecins, u, "creation");
		assertTrue((maladieManager
				.findByCodeLikeManager("12.12-1", true)).size() == 1);
		assertTrue((maladieManager.getCollaborateursManager(m)).size() == 2);
		assertTrue(getOperationManager().findByObjectManager(m).size() == 1);
		assertTrue(getOperationManager().findByObjectManager(m).get(0)
		...
		try {
			maladieManager.createOrUpdateObjectManager(m2, p, null, 
															u, "creation");
		} catch (Exception e) {
		...
		assertTrue(catched);
		assertTrue(maladieManager
				.findByCodeLikeManager("12.12-1", false).size() == 1);
		
		//validation test Type
		Patient[] patients = new Patient[]{null, p};
		String[] libelles = new String[]{"", "  ", null, "12¤¤.4", 
				createOverLength(300), "12 La poyloçite"};
		String[] codes = new String[]{"", "  ", "12$$¤.4", 
									createOverLength(50), null, "14GH.0"};
		m2 = new Maladie();
		...
		for (int k = 0; k < codes.length; k++) {
			...
			try {
				if (!(i == 1 && j == 5 && k > 3)) { //car creation valide
					maladieManager.createOrUpdateObjectManager(m2, 
											patients[i], null, u, "creation");
				}
			} catch (Exception e) {
				if (i == 0) {
		...
		Date avant = new SimpleDateFormat("dd/MM/yyyy").parse("01/11/1973");
		Date apres = new SimpleDateFormat("dd/MM/yyyy").parse("02/11/2000");
		Date naissance = p.getDateNaissance();
		Date deces = p.getDateDeces();
		Date futur = new SimpleDateFormat("dd/MM/yyyy").parse("31/11/2119");
		Date[] dates = new Date[]{futur, avant, null, apres, naissance, deces};
		...
		for (int x = 0; x < dates.length; x++) {
			for (int y = 0; y < dates.length; y++) {
				...
				try {
					maladieManager.createOrUpdateObjectManager(m2, null,
											null, null, "creation");
				} catch (Exception e) {
					assertTrue(e.getClass().getSimpleName().equals(
													"ValidationException"));
		...
		try {
			maladieManager.createOrUpdateObjectManager(m2, 
									patients[1], null, u, null);
		} catch (NullPointerException ne) {
			assertTrue(true);
		}
		try {
			maladieManager.createOrUpdateObjectManager(m2, 
									patients[1], null, u, "dummy");
	...
	public void updateObjectManagerTest() throws ParseException {
		...
		Maladie m = maladieManager
						.findByCodeLikeManager("12.12-1", false).get(0);
		/*Champs obligatoires*/
		...
		maladieManager.createOrUpdateObjectManager(m, null, medecins, 
														u, "modification");
		assertNull((maladieManager
				.findByLibelleLikeManager("Maladie grave", true))
														.get(0).getCode());
		assertTrue(maladieManager.getCollaborateursManager(m).size() == 2);
		assertTrue(maladieManager.getCollaborateursManager(m)
								.contains(collaborateurDao.findById(3)));
		assertTrue(getOperationManager().findByObjectManager(m).size() == 2);
		assertTrue(getOperationManager().findByObjectManager(m).get(1)
										.getOperationType().getNom()
												.equals("Modification"));
		// supprime toutes les associations vers collaborateur
		maladieManager.createOrUpdateObjectManager(m, null, null, 
													u, "modification");
		assertTrue(maladieManager.getCollaborateursManager(m).size() == 2);
		maladieManager.createOrUpdateObjectManager(m, null, 
				new ArrayList<Collaborateur>(), u, "modification");
		...
		try {
			...
			m.setDateDiagnostic(new SimpleDateFormat("dd/MM/yyyy")
														.parse("03/12/2006"));
			maladieManager.createOrUpdateObjectManager(m, p, null, 
														u, "modification");
		...
		assertTrue(catched);
		assertTrue(maladieManager
				.findByLibelleLikeManager("Fracture", true).size() == 1);
	...
	public void removeObjectManagerTest() {
		...
		assertTrue(catched);
		assertTrue(maladieManager
				.findByLibelleLikeManager("Addiction medocs", true).size() > 0);
		//null remove
	...
	/**
	 * teste la creation conjointe d'une maladie et d'un patient.
	 * @throws ParseException 
	 */
	public void testCreateMaladieAndPatient() throws ParseException {
		...
		assertTrue((maladieManager
				.findByLibelleLikeManager("Deshydratation", true)).size() == 1);
		assertTrue((patientDao.findByNom("Snoop")).size() == 1);
		// clean up
		maladieManager.removeObjectManager(m, null, u);
		patientManager.removeObjectManager(patientDao.findByNom("Snoop")
														.get(0), null, u, null);
	...
	public void testFindByPatientManager() {
		Patient p = patientDao.findById(1);
		List<Maladie> mals = maladieManager.findByPatientManager(p);
		assertTrue(mals.size() == 2);
		assertTrue(mals.get(0).equals(maladieManager
					.findByLibelleLikeManager("Non precise", true).get(0)));
		assertTrue(mals.get(1).equals(maladieManager
						.findByLibelleLikeManager("Fracture", true).get(0)));
	}
	
	public void testFindByPatientNoSystemManager() {
		Patient p = patientDao.findById(1);
		List<Maladie> mals = maladieManager.findByPatientNoSystemManager(p);
		assertTrue(mals.size() == 2);
		assertTrue(mals.contains(maladieManager
					.findByLibelleLikeManager("Non precise", true).get(0)));
		assertTrue(mals.contains(maladieManager
						.findByLibelleLikeManager("Fracture", true).get(0)));
	}
