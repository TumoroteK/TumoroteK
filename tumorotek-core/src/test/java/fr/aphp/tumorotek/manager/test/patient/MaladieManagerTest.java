...
public class MaladieManagerTest extends AbstractManagerTest4
{
	...
	public void testFindByLibelleAndPatientManager() {
		Patient pat3 = patientDao.findById(3).orElse(null);
		...
		maladies = maladieManager.findByLibelleAndPatientManager("Addiction%", patientDao.findById(2)).orElse(null);
		assertTrue(maladies.size() == 0);
		maladies = maladieManager.findByLibelleAndPatientManager(null, patientDao.findById(3)).orElse(null);
		assertTrue(maladies.size() == 0);
	...
	public void testGetPrelevementsManager(){
		...
		assertTrue((maladieManager.getPrelevementsManager(m)).iterator().next().equals(prelevementDao.findById(2))).orElse(null);
	...
	public void createObjectManagerTest() throws ParseException{
		...
		/*Champs obligatoires*/
		final Patient p = patientDao.findById(2).orElse(null);
		...
		medecins.add(collaborateurDao.findById(1)).orElse(null);
		medecins.add(collaborateurDao.findById(2)).orElse(null);
	...
	public void updateObjectManagerTest() throws ParseException{
		...
		final List<Collaborateur> medecins = new ArrayList<>();
		medecins.add(collaborateurDao.findById(1)).orElse(null);
		medecins.add(collaborateurDao.findById(3)).orElse(null);
		...
		assertTrue(maladieManager.getCollaborateursManager(m).size() == 2);
		assertTrue(maladieManager.getCollaborateursManager(m).contains(collaborateurDao.findById(3))).orElse(null);
		...
		final Patient p = patientDao.findById(1).orElse(null);
		Boolean catched = false;
		try{
	...
	public void testFindByPatientManager(){
		final Patient p = patientDao.findById(1).orElse(null);
		final List<Maladie> mals = maladieManager.findByPatientManager(p);
		assertTrue(mals.size() == 2);
	...
	public void testMaladieDelegate(){
		...
		sero.setDiagnostic(diagnosticDao.findById(1)).orElse(null);
		// sero.setContexte(contexteDao.findById(1)).orElse(null);
		sero.setDelegator(m);
		m.setDelegate(sero);
		...
		m = maladieManager.findByCodeLikeManager("xde", true).get(0);
		assertTrue(m.getDelegate() != null);
		assertTrue(((MaladieSero) m.getDelegate()).getDiagnostic().equals(diagnosticDao.findById(1))).orElse(null);
