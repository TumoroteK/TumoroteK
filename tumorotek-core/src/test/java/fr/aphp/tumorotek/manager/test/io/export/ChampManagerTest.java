...
public class ChampManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Champ champ = manager.findByIdManager(1);
		assertNotNull(champ);
		assertTrue(champ.getChampId().equals(1));
		
		Champ champNull = manager.findByIdManager(500);
	...
	public void testFindAllObjects() {
		List<Champ> list = manager.findAllObjectsManager();
		//On vérifie chaque élément par un findById
		Iterator<Champ> it = list.iterator();
		while (it.hasNext()) {
			Champ temp = it.next();
			if (temp != null) {
				assertTrue(temp.equals(
						manager.findByIdManager(temp.getChampId())));
			}
		}
	}
	...
	public void testFindEnfants() {
		...
		while (it.hasNext()) {
			Champ temp = it.next();
			if (temp != null) {
				List<Champ> enfants = manager.findEnfantsManager(temp);
				Iterator<Champ> it2 = enfants.iterator();
				while (it2.hasNext()) {
					Champ tempEnfant = it2.next();
					if (tempEnfant != null) {
	...
	public void testCopyChamp() {
		Champ champ = manager.findByIdManager(4);
		Champ copie = manager.copyChampManager(champ);
		assertTrue(copie.getChampId() > 0);
		//On verifie que le champ et la copie sont identiques
		assertNotNull(copie);
		//On verifie que la copie est bien en BDD
		assertTrue(copie.equals(
				manager.findByIdManager(copie.getChampId())));
	...
	public void testGetValueForObjectManager() throws ParseException {
		...
		Champ c1 = new Champ(champEntiteDao.findById(3));
		assertTrue(manager
				.getValueForObjectManager(c1, p1, true).equals("MAYER"));
		
		Prelevement prlvt1 = prelevementDao.findById(1);
		Champ c24 = new Champ(champEntiteDao.findById(24));
		assertTrue(manager.getValueForObjectManager(c24, prlvt1, true)
				.equals("SANG"));
		assertTrue(manager.getValueForObjectManager(c24, prlvt1, false)
				.equals("SANG"));
		Champ c28 = new Champ(champEntiteDao.findById(28));
		assertTrue(manager.getValueForObjectManager(c28, prlvt1, true)
				.equals("VIAL"));
		Champ c27 = new Champ(champEntiteDao.findById(27));
		assertTrue(manager.getValueForObjectManager(c27, prlvt1, true)
				.equals("06/09/1983"));
		assertTrue(manager.getValueForObjectManager(c27, prlvt1, false)
				.equals(new SimpleDateFormat("dd/MM/yyyy")
				.parse("06/09/1983")));
		Champ c30 = new Champ(champEntiteDao.findById(30));
		assertTrue(manager.getValueForObjectManager(c30, prlvt1, true)
				.equals("06/09/1983 10:00"));
		Champ c34 = new Champ(champEntiteDao.findById(34));
		assertTrue(manager.getValueForObjectManager(c34, prlvt1, true)
				.equals("10"));
		Champ c37 = new Champ(champEntiteDao.findById(37));
		assertTrue(manager.getValueForObjectManager(c37, prlvt1, true)
				.equals("-5.0"));
		Champ c47 = new Champ(champEntiteDao.findById(47));
		assertTrue(manager.getValueForObjectManager(c47, prlvt1, true)
				.equals("Oui"));
		
		// Mauvais paramétrage
		Champ cWrong = new Champ(champEntiteDao.findById(24));
