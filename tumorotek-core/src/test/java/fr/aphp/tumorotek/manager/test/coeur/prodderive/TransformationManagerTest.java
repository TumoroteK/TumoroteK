...
public class TransformationManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Transformation transfoNull = transformationManager.findByIdManager(10);
		assertNull(transfoNull);
	}

	@Test
	public void testFindAll() {
		List<Transformation> transfos = 
			transformationManager.findAllObjectsManager();
		assertTrue(transfos.size() == 5);
	}
	
	@Test
	public void testFindByEntiteObjetManager() {
		Entite entite1 = entiteManager.findByIdManager(3);
		List<Transformation> transfos = 
			transformationManager.findByEntiteObjetManager(entite1, 1);
		assertTrue(transfos.size() == 2);
		
		Entite entite2 = entiteManager.findByIdManager(5);
		transfos = 
			transformationManager.findByEntiteObjetManager(entite2, 5);
		assertTrue(transfos.size() == 0);
	}
	
	@Test
	public void testFindByParentManager() {
		Echantillon echantillon = echantillonManager.findByIdManager(1);
		List<Transformation> transfos = 
	...
	public void testFindDoublon() {
		Entite entite = entiteManager.findByIdManager(3);
		...
		Transformation transfo2 = transformationManager.findByIdManager(2);
	...
	public void testCrud() {
		...
		int id = transfo1.getTransformationId();
		
		// Update
		Transformation transfo2 = transformationManager.findByIdManager(id);
		...
		assertTrue(catchedUpdate);*/
		Transformation transfo4 = transformationManager.findByIdManager(id);
		assertTrue(transfo4.getObjetId() == 4);

		// Suppression
		Utilisateur u = utilisateurDao.findById(1);
		Transformation transfo5 = transformationManager.findByIdManager(id);
		transformationManager.removeObjectManager(transfo5, null, u);
		assertNull(transformationManager.findByIdManager(id));
		Transformation transfo6 = transformationManager.findByIdManager(1);
		try {
	...
	public void testfindAllDeriveFromParentManager() {
		...
		List<ProdDerive> derives = 
				transformationManager.findAllDeriveFromParentManager(e); 
