...
public class AdicapManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		...
		List<Adicap> ads = (List<Adicap>) adicapManager.findAllObjectsManager();
	...
	public void testFindByCodeLikeManager() {
		...
		List<Adicap> ads = 
			(List<Adicap>) adicapManager.findByCodeLikeManager("N", true);
	...
	public void testFindByLibelleLikeManager() {
		//teste une recherche exactMatch
		List<Adicap> ads = 
			(List<Adicap>) adicapManager
							.findByLibelleLikeManager("NEZ", true);
		assertTrue(ads.size() == 1);
		//teste une recherche non exactMatch
		ads = (List<Adicap>) adicapManager
								.findByLibelleLikeManager("NEZ", false);
		assertTrue(ads.size() == 11);
		//teste une recherche infructueuse
		ads = (List<Adicap>) adicapManager
								.findByLibelleLikeManager("INCUBUS", false);
		assertTrue(ads.size() == 0);
		//null recherche
		ads = (List<Adicap>) adicapManager
								.findByLibelleLikeManager(null, true);
		assertTrue(ads.size() == 0);
	}
	
	@Test
	public void testfindDictionnairesManager() {
		List<AdicapGroupe> groupes = adicapManager.findDictionnairesManager();
		assertTrue(groupes.size() == 8);
	}
	
	@Test
	public void testFindByAdicapParentManager() {
		Adicap parent = (Adicap) 
			adicapManager.findByCodeLikeManager("AC", true).get(0);
		List<Adicap> ads = 
			adicapManager.findByAdicapParentManager(parent, true);
		assertTrue(ads.size() == 10);
		ads = adicapManager.findByAdicapParentManager(parent, false);
		assertTrue(ads.size() == 3);
		
		parent = (Adicap) 
			adicapManager.findByCodeLikeManager("AP", true).get(0);
		ads = adicapManager.findByAdicapParentManager(parent, false);
		assertTrue(ads.size() == 0);
		
		parent = (Adicap) 
		adicapManager.findByCodeLikeManager("0N72", true).get(0);
	...
	public void testFindByMorphoManager() {
		List<Adicap> ads = 
			adicapManager.findByMorphoManager(true);
	...
	public void testFindByAdicapGroupManager() {
		AdicapGroupe g = adicapManager.findDictionnairesManager().get(2);
		List<Adicap> ads = 
			adicapManager.findByAdicapGroupeManager(g);
	...
	public void testGetCimMastersManager() {
		Adicap a = (Adicap) adicapManager
						.findByCodeLikeManager("BV", true).get(0);
	...
	public void testGetAdicapGroupesManager() {
		AdicapGroupe g = adicapManager.findDictionnairesManager().get(6);
		assertTrue(adicapManager.getAdicapGroupesManager(g).size() == 19);
		assertTrue(adicapManager.getAdicapGroupesManager(g)
									.get(0).getAdicapGroupeId() == 40);
		assertTrue(adicapManager.getAdicapGroupesManager(
					adicapManager.getAdicapGroupesManager(g)
									.get(0)).size() == 0);
	}
	...
	public void testFindChildrenCodesManager() {
		Adicap a = (Adicap) adicapManager
							.findByCodeLikeManager("EZ", true).get(0);
	...
	public void testFindByDicoAndCodeOrLibelleManager() {
		...
		List<Adicap> adicaps = adicapManager
					.findByDicoAndCodeOrLibelleManager(g, "BD0", false);
		assertTrue(adicaps.size() == 37);
		g = adicapGroupeDao.findById(3);
		adicaps = adicapManager
					.findByDicoAndCodeOrLibelleManager(g, "BD0", true);
		assertTrue(adicaps.size() == 0);
		g = adicapGroupeDao.findById(3);
		adicaps = adicapManager
					.findByDicoAndCodeOrLibelleManager(g, "PHARYNX", true);
		assertTrue(adicaps.size() == 1);
		adicaps = adicapManager
					.findByDicoAndCodeOrLibelleManager(g, "PHARYNX", false);
		assertTrue(adicaps.size() == 6);
		adicaps = adicapManager
					.findByDicoAndCodeOrLibelleManager(null, "PHARYNX", false);
		assertTrue(adicaps.size() == 0);
		adicaps = adicapManager
					.findByDicoAndCodeOrLibelleManager(g, null, false);
		assertTrue(adicaps.size() == 0);
	}
}
