...
import fr.aphp.tumorotek.manager.test.ConfigCodes;
import fr.aphp.tumorotek.model.code.Adicap;
import fr.aphp.tumorotek.model.code.AdicapGroupe;

/**
 *
 * Classe de test pour le manager AdicapManager. Classe créée le 19/05/10.
 *
...
public class AdicapManagerTest {
	...
	public void testFindChildrenCodesManager() {
		...
		AdicapGroupe g = adicapGroupeDao.findById(19).orElse(null);
		codes = adicapManager.findChildrenCodesManager(null, g, "%");
		assertTrue(codes.size() == 344);

		g = adicapGroupeDao.findById(5).orElse(null);
		codes = adicapManager.findChildrenCodesManager(null, g, "%");
