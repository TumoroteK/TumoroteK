...
public class ProtocoleTypeManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		ProtocoleType pt1 = protocoleTypeManager.findByIdManager(1);
		assertNotNull(pt1);
		assertTrue(pt1.getType().equals("RECHERCHE"));
		
		ProtocoleType ptNull = protocoleTypeManager.findByIdManager(10);
		assertNull(ptNull);
	}
	...
	public void testFindByTypeLikeExactManager() {
		List<ProtocoleType> list = 
			protocoleTypeManager.findByTypeLikeManager("RECHERCHE", true);
	...
	public void testFindByTypeLikeManager() {
		List<ProtocoleType> list = 
			protocoleTypeManager.findByTypeLikeManager("RECHERCHE", false);
		assertTrue(list.size() == 1);
	...
	public void testFindDoublon() {
		//Cree le doublon
		ProtocoleType p1 = (protocoleTypeManager
		...
		ProtocoleType p2 = new ProtocoleType();
	...
	public void createObjectManagerTest() {
		...
		ProtocoleType p1 = new ProtocoleType();
		p1.setType("TEST");
		p1.setPlateforme(plateformeDao.findById(1));
		protocoleTypeManager.createObjectManager(p1);
		assertTrue((protocoleTypeManager
				.findByTypeLikeManager("TEST", true)).size() == 1);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		ProtocoleType p1Bis = new ProtocoleType();
		p1Bis.setType("TEST");
		try {
		...
		for (int i = 0; i < emptyValues.length; i++) {
			try {
				...
				assertTrue(protocoleTypeManager
					.findByOrderManager(p1.getPlateforme()).size() == 3);
			}
		}		
	}
	...
	public void updateObjectManagerTest() {
		...
		ProtocoleType p1 = (protocoleTypeManager
			.findByTypeLikeManager("TEST", true)).get(0);
		p1.setType("TEST2");
		protocoleTypeManager.updateObjectManager(p1);
		assertTrue((protocoleTypeManager
				.findByTypeLikeManager("TEST2", true)).size() == 1);
		...
		assertTrue(catched);
		assertTrue((protocoleTypeManager
				.findByTypeLikeManager("RECHERCHE", true)).size() == 1);
	...
	public void removeObjectManagerTest() {
		...
		ProtocoleType p1 = (protocoleTypeManager
				.findByTypeLikeManager("TEST2", true)).get(0);
		protocoleTypeManager.removeObjectManager(p1);
		assertTrue((protocoleTypeManager
				.findByTypeLikeManager("TEST2", true)).size() == 0);
...
//		assertTrue(catched);
		assertTrue((protocoleTypeManager
				.findByTypeLikeManager("RECHERCHE", true)).size() > 0);
		//null remove
		protocoleTypeManager.removeObjectManager(null);
		assertTrue(protocoleTypeManager
				.findByOrderManager(p1.getPlateforme()).size() == 2);
	}
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
						protocoleTypeManager.findByOrderManager(pf);
