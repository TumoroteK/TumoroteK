...
public class GroupementManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Groupement groupement = manager.findByIdManager(1);
		assertNotNull(groupement);
		assertTrue(groupement.getGroupementId().equals(1));
		
		Groupement groupementNull = manager.findByIdManager(50);
	...
	public void testFindAllObjects() {
		List<Groupement> list = manager.findAllObjectsManager();
		//On vérifie chaque élément par un findById
		Iterator<Groupement> it = list.iterator();
		while (it.hasNext()) {
			Groupement temp = it.next();
			if (temp != null) {
				assertTrue(temp.equals(
						manager.findByIdManager(temp.getGroupementId())));
			}
		}
	}
	...
	public void testFindEnfants() {
		...
		while (it.hasNext()) {
			Groupement temp = it.next();
			if (temp != null) {
				List<Groupement> enfants = manager.findEnfantsManager(temp);
				Iterator<Groupement> it2 = enfants.iterator();
				while (it2.hasNext()) {
					Groupement tempEnfant = it2.next();
					if (tempEnfant != null) {
	...
	public void testFindCriteres() {
		...
		while (it.hasNext()) {
			Groupement temp = it.next();
			if (temp != null) {
				List<Critere> criteres = findCriteres(temp);
				Iterator<Critere> itCrit
					= manager.findCriteresManager(temp).iterator();
				while (itCrit.hasNext()) {
					Critere crit = itCrit.next();
					if (crit != null) {
	...
	public void testCopyGroupement() {
		...
		Groupement copie = manager.copyGroupementManager(groupement);
		...
		Groupement copie2 = manager.findByIdManager(copie.getGroupementId());
	...
	public void testCrud() {
		 ...
		 * valides. */
		try {
			manager.createObjectManager(null, new Critere(), new Critere(),
					"=", new Groupement());
		} catch (RequiredObjectIsNullException e) {
			assertEquals("RequiredObjectIsNullException",
					e.getClass().getSimpleName());
		...
		try {
			manager.updateObjectManager(null, new Critere(), new Critere(),
					"!=", new Groupement());
		} catch (RequiredObjectIsNullException e) {
			assertEquals("RequiredObjectIsNullException",
					e.getClass().getSimpleName());
		...
		Groupement groupement = new Groupement();
		manager.createObjectManager(groupement, critere1, critere2, operateur,
				parent);
		//On vérifie que la sauvegarde a été correctement effectuée
		Groupement groupement2
			= manager.findByIdManager(groupement.getGroupementId());
		...
		String oldOperateur = groupement.getOperateur();
		operateur = "or";
		groupement.setOperateur(operateur);
		Groupement oldParent = groupement.getParent();
		parent = manager.findByIdManager(2);
		groupement.setParent(parent);
		
		manager.updateObjectManager(groupement, critere1, critere2, operateur,
				parent);
		...
		/** Remove. */
		int id = groupement.getGroupementId();
		//On le supprime
	...
	private List<Critere> findCriteres(Groupement groupement) {
		...
		if (groupement != null) {			
			...
			Iterator<Groupement> it = manager
				.findEnfantsManager(groupement).iterator();
			while (it.hasNext()) {
				Groupement temp = it.next();
