 ...
 *
 */
public class OperationManagerTest extends AbstractManagerTest4
{
	...
	public void testFindByUtilisateurManager(){
		...
		u = utilisateurDao.findById(3).orElse(null);
	...
	public void testFindByObjectManager(){
		//teste une recherche fructueuse
		final Prelevement p = prelevementDao.findById(2).orElse(null);
		List<Operation> ops = operationManager.findByObjectManager(p);
		assertTrue(ops.size() == 2);
		//teste une recherche infructueuse
		final Echantillon e = echantillonDao.findById(3).orElse(null);
	...
	public void testFindDateCreationManager() throws ParseException{
		...
		cal.setTime(date);
		final Prelevement p = prelevementDao.findById(1).orElse(null);
		...
		//teste une recherche infructueuse
		e = echantillonDao.findById(3).orElse(null);
	...
	private void createObjectManagerTest() throws ParseException{
		...
		final Utilisateur u = utilisateurDao.findById(3).orElse(null);
		final Echantillon e = echantillonDao.findById(3).orElse(null);
	...
	public void testFindByMultiCriteresManager() throws ParseException{
		...
		final OperationType t3 = operationTypeDao.findById(3).orElse(null);
		final OperationType t5 = operationTypeDao.findById(5).orElse(null);
	...
	public void testFindLastByUtilisateurAndType() throws ParseException{
		Operation last;
		Utilisateur u = utilisateurDao.findById(2).orElse(null);
		OperationType oT = operationTypeDao.findById(3).orElse(null);
		...
		oT = operationTypeDao.findById(5).orElse(null);
		last = operationManager.findLastByUtilisateurAndTypeManager(oT, u, 1);
		assertNotNull(last);
		...
		u = utilisateurDao.findById(1).orElse(null);
		oT = operationTypeDao.findById(17).orElse(null);
		last = operationManager.findLastByUtilisateurAndTypeManager(oT, u, 1);
		assertNull(last);

		u = utilisateurDao.findById(1).orElse(null);
		oT = operationTypeDao.findById(null).orElse(null);
		last = operationManager.findLastByUtilisateurAndTypeManager(oT, u, 1);
		assertNull(last);

		u = utilisateurDao.findById(null).orElse(null);
		oT = operationTypeDao.findById(3).orElse(null);
		last = operationManager.findLastByUtilisateurAndTypeManager(oT, u, 1);
		assertNull(last);
	...
	public void testBatchSaveManager() throws ParseException{    
		...
		final Entite e = entiteDao.findById(3).orElse(null);
		final OperationType oT = operationTypeDao.findById(2).orElse(null);
      ...
      cal.setTime(date);
		...
		//		ope.setDate(cal);
		//		ope.setEntite(entiteDao.findById(1)).orElse(null);
		//		ope.setObjetId(2);
		//		operationManager.createObjectManager(ope, u, oT, prelevementDao.findById(1)).orElse(null);
