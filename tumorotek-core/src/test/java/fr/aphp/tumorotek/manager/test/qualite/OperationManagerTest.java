...
public class OperationManagerTest extends AbstractManagerTest {
	...
	public void testFindAllObjectsManager() {
		List<Operation> ops = operationManager.findAllObjectsManager();
	...
	public void testFindByObjectManager() {
		//teste une recherche fructueuse
		Prelevement p = prelevementDao.findById(2);
		List<Operation> ops = operationManager.findByObjectManager(p);
		assertTrue(ops.size() == 2);
		//teste une recherche infructueuse
		Echantillon e = echantillonDao.findById(3);
	...
	public void testFindByObjectForHistoriqueManager() {
		...
		List<Operation> ops = operationManager
			.findByObjectForHistoriqueManager(p);
		assertTrue(ops.size() == 2);
		//teste une recherche infructueuse
		Echantillon e = echantillonDao.findById(3);
	...
	public void testFindDateCreationManager() throws ParseException {
		...
		Calendar datePrlvt = operationManager.findDateCreationManager(p);
	...
	public void testFindDoublon() {
		...
		Operation o1 = operationManager.findByUtilisateurManager(u).get(0);
	...
	public void createObjectManagerTest() throws ParseException {
		...
		Utilisateur u = utilisateurDao.findById(3);
		Echantillon e = echantillonDao.findById(3);
		OperationType oType = operationTypeDao.findByNom("Creation").get(0);
		Operation o1 = new Operation();
		Date date = new SimpleDateFormat("dd/MM/yyyy").parse("14/10/2009");
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		o1.setDate(cal);
		...
		Calendar[] dateValues = new Calendar[]{null, cal};
		Utilisateur[] utilisateurValues = new Utilisateur[]{null, u};
		OperationType[] oTypeValues = new OperationType[]{null, oType};
		Prelevement pNullId = new Prelevement();
		Object[] objectValues = new Object[]{null, pNullId, e};
		Operation o2 = new Operation();
					...
					for (int l = 0; l < objectValues.length; l++) {
						try {
							...
							if (i != 1 
								...
								operationManager.createObjectManager(o2, 
														utilisateurValues[j], 
														oTypeValues[k], 
														objectValues[l]);
							}
							...
							if (j == 0 || k == 0 || l == 0) {
								assertTrue(ex.getClass().getSimpleName().equals(
	...
	public void testFindAfterDateOperationManager() throws ParseException {
		...
		cal.setTime(date);
		List<Operation> ops = operationManager
			.findAfterDateOperationManager(cal);
	...
	public void testFindBeforeDateOperationManager() throws ParseException {
		...
		cal.setTime(date);
		List<Operation> ops = operationManager
			.findBeforeDateOperationManager(cal);
	...
	public void testFindBetweenDatesOperationManager() throws ParseException {
		...
		cal2.setTime(date);
		List<Operation> ops = operationManager
			.findBetweenDatesOperationManager(cal1, cal2);
	...
	public void testFindByMultiCriteresManager() throws ParseException {
		...
		OperationType t3 = operationTypeDao.findById(3);
		OperationType t5 = operationTypeDao.findById(5);
		
		Date date = new SimpleDateFormat("dd/MM/yyyy HH:mm")
			.parse("01/03/2001 10:10");
		Calendar cal = Calendar.getInstance();
		cal.setTime(date);
		List<Operation> ops = operationManager.findByMultiCriteresManager(
				"=", cal, null, null, null, null, false);
		assertTrue(ops.size() == 2);
		
		ops = operationManager.findByMultiCriteresManager(
				"=", cal, null, null, t3, null, true);
		assertTrue(ops.size() == 1);
		
		us.add(u1);
		ops = operationManager.findByMultiCriteresManager(
				"=", cal, null, null, null, us, false);
		assertTrue(ops.size() == 2);
		
		ops = operationManager.findByMultiCriteresManager(
				"=", cal, null, null, t3, us, true);
		assertTrue(ops.size() == 1);
		...
		cal2.setTime(date);
		ops = operationManager.findByMultiCriteresManager(
				">=", cal1, "<=", cal2, null, null, false);
		assertTrue(ops.size() == 5);
		
		ops = operationManager.findByMultiCriteresManager(
				">=", cal1, "<=", cal2, t3, null, false);
		assertTrue(ops.size() == 4);
		
		ops = operationManager.findByMultiCriteresManager(
				">=", cal1, "<=", cal2, null, us, false);
		assertTrue(ops.size() == 5);
		
		us.remove(u1);
		ops = operationManager.findByMultiCriteresManager(
				">=", cal1, "<=", cal2, t3, us, false);
		assertTrue(ops.size() == 1);
		
		ops = operationManager.findByMultiCriteresManager(
				null, null, null, null, t5, null, false);
		assertTrue(ops.size() == 5);
		
		ops = operationManager.findByMultiCriteresManager(
				null, null, null, null, null, us, true);
		assertTrue(ops.size() == 2);
	...
	public void testFindLastByUtilisateurAndType() throws ParseException {
		...
		assertTrue(last.getDate().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy").parse("10/12/2009")));
		
		last = operationManager.findLastByUtilisateurAndTypeManager(oT, u, 2);
		assertNotNull(last);
		assertTrue(last.getOperationId() == 9);
		assertTrue(last.getDate().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy").parse("05/11/2009")));
		
		last = operationManager.findLastByUtilisateurAndTypeManager(oT, u, 3);
		assertNotNull(last);
		assertTrue(last.getOperationId() == 13);
		assertTrue(last.getDate().getTime()
			.equals(new SimpleDateFormat("dd/MM/yyyy").parse("02/11/2009")));
	...
	public void testBatchSaveManager() {
		...
		OperationType oT = operationTypeDao.findById(2);
		Calendar cal = Calendar.getInstance();
		
		int count = operationManager.findAllObjectsManager().size();
