...
public class CategorieManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Categorie cat1 = categorieManager.findByIdManager(1);
		assertNotNull(cat1);
		assertTrue(cat1.getNom().equals("CAT1"));
		
		Categorie catNull = categorieManager.findByIdManager(10);
		assertNull(catNull);
	}
	...
	public void testFindAll() {
		List<Categorie> list = categorieManager.findAllObjectsManager();
	...
	public void testFindByNomLikeExactManager() {
		List<Categorie> list = 
			categorieManager.findByNomLikeManager("CAT1", true);
	...
	public void testFindByNomLikeManager() {
		List<Categorie> list = 
			categorieManager.findByNomLikeManager("CAT1", false);
		assertTrue(list.size() == 1);
	...
	public void testFindDoublon() {
		//Cree le doublon
		Categorie c1 = categorieManager.findByIdManager(1);
	...
	private void createObjectManagerTest() {
		...
		c1.setNom("NEW");
		categorieManager.createObjectManager(c1);
		assertTrue(categorieManager.findAllObjectsManager().size() == 3);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		Categorie c1Bis = new Categorie();
		c1Bis.setNom("NEW");
		try {
			categorieManager.createObjectManager(c1Bis);
		} catch (Exception e) {
		...
		//Validation test
		String[] emptyValues = new String[]{"", "  ", "=12$$€", 
											createOverLength(50), null};
		Categorie c2 = new Categorie();
		for (int i = 0; i < emptyValues.length; i++) {
			try {
				c2.setNom(emptyValues[i]);
				categorieManager.createObjectManager(c2);
			} catch (ValidationException e) {
				//verifie qu'aucune ligne n'a ete ajoutee
				assertTrue(categorieManager
	...
	private void updateObjectManagerTest() {
		...
		Categorie c1 = categorieManager.findByIdManager(3);
		c1.setNom("NEW BIS");
		categorieManager.updateObjectManager(c1);
		Categorie c1Bis = categorieManager.findByIdManager(3);
		assertTrue(c1Bis.getNom().equals("NEW BIS"));
		//Modification en un doublon engendrant une exception
		Boolean catched = false;
		try {
			c1Bis.setNom("CAT1");
			categorieManager.updateObjectManager(c1Bis);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catched = true;
