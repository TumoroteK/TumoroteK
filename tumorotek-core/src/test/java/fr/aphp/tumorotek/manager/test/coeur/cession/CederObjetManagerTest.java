...
public class CederObjetManagerTest extends AbstractManagerTest4
{
   ...
   public void testFindByObjetManager(){
      ...
      final Echantillon echan1 = echantillonDao.findById(1).orElse(null);
      List<CederObjet> list = cederObjetManager.findByObjetManager(echan1);
      assertTrue(list.size() == 2);

      final Echantillon echan2 = echantillonDao.findById(4).orElse(null);
      list = cederObjetManager.findByObjetManager(echan2);
      assertTrue(list.size() == 0);

      final ProdDerive derive1 = prodDeriveDao.findById(1).orElse(null);
      list = cederObjetManager.findByObjetManager(derive1);
      assertTrue(list.size() == 1);

      final ProdDerive derive2 = prodDeriveDao.findById(2).orElse(null);
   ...
   public void testGetAllCessionsByStatutAndObjetManager(){
      ...
      final Echantillon echan4 = echantillonDao.findById(4).orElse(null);
   ...
   public void testFindByCessionEntiteManager(){
      Entite e1 = entiteDao.findByNom("Echantillon").get(0);
      final Cession c2 = cessionDao.findById(2).orElse(null);
   ...
   public void testGetEchantillonsCedesByCessionManager(){
      ...
      final Cession c4 = cessionDao.findById(4).orElse(null);
   ...
   public void createObjectManagerTest(){
      ...
      // on test l'insertion avec la cession nulle
      try{
         cederObjetManager.createObjectManager(ced1, null, entite, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      // on test l'insertion avec l'entité nulle
      try{
         cederObjetManager.createObjectManager(ced1, cession, null, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      cederObjetManager.createObjectManager(ced2, cession, entite, qteUnite);
   ...
   public void updateObjectManagerTest(){
      ...
      ced.setObjetId(2);
      cederObjetManager.createObjectManager(ced, cession, entite, null);
      assertTrue(cederObjetManager.findAllObjectsManager().size() == 7);
      ...
      try{
         cederObjetManager.updateObjectManager(cedUp1, null, entite, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      try{
         cederObjetManager.updateObjectManager(cedUp1, cession, null, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("RequiredObjectIsNullException")){
            catched = true;
         }
      ...
      try{
         cederObjetManager.updateObjectManager(cedUp1, cession, entite, null);
      }catch(final Exception e){
         if(e.getClass().getSimpleName().equals("InvalidPKException")){
            catched = true;
         }
      ...
      cederObjetManager.updateObjectManager(cedUp1, cession, entite, qteUnite);
      // On vérifie que toutes associations ont étées enregistrées
   ...
   private void validationInsert(final CederObjet ce){
      ...
      for(int i = 0; i < ids.length; i++){
         ...
         try{
            ce.setObjetId(ids[i]);
            cederObjetManager.createObjectManager(ce, cession, entite, null);
         }catch(final Exception e){
            if(e.getClass().getSimpleName().equals("ValidationException")){
               catchedInsert = true;
            }
   ...
   private void validationUpdate(final CederObjet ce){
      ...
      for(int i = 0; i < ids.length; i++){
         ...
         try{
            ce.setObjetId(ids[i]);
            cederObjetManager.updateObjectManager(ce, cession, entite, null);
         }catch(final Exception e){
            if(e.getClass().getSimpleName().equals("ValidationException")){
               catched = true;
            }
   ...
   public void testCountObjectCessedManager(){
      ...
      final Entite e8 = entiteDao.findById(8).orElse(null);
      final Cession c1 = cessionDao.findById(1).orElse(null);
