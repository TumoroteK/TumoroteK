...
public class TransporteurManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Transporteur transporteur = transporteurManager.findByIdManager(1);
		assertNotNull(transporteur);
		assertTrue(transporteur.getNom().equals("HOPITAL ST LOUIS - ANAPATH"));
		
		Transporteur transporteurNull = transporteurManager.findByIdManager(5);
	...
	public void testFindAll() {
		List<Transporteur> list = transporteurManager.findAllObjectsManager();
	...
	public void testFindAllActiveManager() {
		List<Transporteur> transporteurs = 
					transporteurManager.findAllActiveManager();
	...
	private void createObjectManagerTest() {
		...
		catched = false;
		try {
			transporteurManager.createObjectManager(
							t1, newCoord1, u);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
		...
		Transporteur testInsert = transporteurManager.findByIdManager(idT2);
		...
		// Suppression
		Transporteur t3 = transporteurManager.findByIdManager(idT1);
		transporteurManager.removeObjectManager(t3, null, u);
		Transporteur t4 = transporteurManager.findByIdManager(idT2);
	...
	private void updateObjectManagerTest() {
		...
		Integer idT = t.getTransporteurId();
		
		// Insertion
		Transporteur tUp1 = transporteurManager.findByIdManager(idT);
		...
		catched = false;
		try {
			transporteurManager.updateObjectManager(
					tUp1, newCoord1, u);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
		...
		// on teste une insertion valide avec une nouvelle coordonnée
		Transporteur tUp2 = transporteurManager.findByIdManager(idT);
		...
		// On vérifie que toutes associations ont étées enregistrées
		Transporteur testInsert = transporteurManager.findByIdManager(idT);
		assertNotNull(testInsert.getCoordonnee());
		assertTrue(testInsert.getCoordonnee().getCoordonneeId() == id);
		// On test les attributs
		...
		transporteurManager.updateObjectManager(testInsert, 
				testInsert.getCoordonnee(), u);
		assertTrue(coordonneeManager.findAllObjectsManager().size() == 6);
		assertTrue(transporteurManager.findAllObjectsManager().size() == 4);
		Coordonnee cTest = coordonneeManager.findByIdManager(id);
		assertTrue(cTest.getAdresse().equals("LAS-BAS"));
		assertTrue(getOperationManager().findByObjectManager(
				testInsert).size() == 4);
				
		// Suppression
		Transporteur t3 = transporteurManager.findByIdManager(idT);
		transporteurManager.removeObjectManager(t3, null, u);
		assertNull(coordonneeManager.findByIdManager(id));
		assertTrue(coordonneeManager.findAllObjectsManager().size() == 5);
		assertTrue(transporteurManager.findAllObjectsManager().size() == 3);
	 ...
	 */
	private void validationInsert(Transporteur transporteur) {
		Utilisateur u = utilisateurDao.findById(1);
		boolean catchedInsert = false;
		String[] nomValues = new String[]{null, "", "  ", "l#$$¤jd%", 
				createOverLength(50), "GOOD"};
		String[] contactNomValues = new String[]{null, "", " ", "%*¤¤$$€kjs", 
				createOverLength(50), "50"};
		String[] contactPrenomValues = new String[]{"", " ", "%€$*k¤", 
				createOverLength(50), "50"};
		String[] contactTelValues = new String[]{"", " ", "%*k$$j¤s", 
				createOverLength(15), "GOOD"};
		String[] contactFaxValues = new String[]{"", " ", "%*€$k¤", 
				createOverLength(15), "GOOD"};
		String[] contactMailValues = new String[]{"", " ", "%*€$k¤", 
	 ...
	 * @param coordonnee Coordonnee à tester.
	 */
	private void validationUpdate(Transporteur transporteur) {
		Utilisateur u = utilisateurDao.findById(1);
		...
		String[] contactPrenomValues = new String[]{"", " ", "%€$*k", 
				createOverLength(50), "50"};
		String[] contactTelValues = new String[]{"", " ", "%*k$$¤js", 
				createOverLength(15), "GOOD"};
		String[] contactFaxValues = new String[]{"", " ", "%*€$k", 
				createOverLength(15), "GOOD"};
		String[] contactMailValues = new String[]{"", " ", "%*€$k", 
	...
	public void testIsObjectReferencedManager() {
		...
		try {
			...
			assertTrue(ore.getKey()
					.equals("transporteur.deletion.isReferenced"));
		}
		assertTrue(catched);
