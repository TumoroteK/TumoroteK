...
public class NatureManagerTest extends AbstractManagerTest4 {
	...
	public void testFindByNatureLikeManager() {
		...
		List<Nature> natures = 
			natureManager.findByNatureLikeManager("SANG", true);
		assertTrue(natures.size() == 1);
		//teste une recherche non exactMatch
		natures = 
			natureManager.findByNatureLikeManager("S", false);
		assertTrue(natures.size() == 1);
		assertTrue(natures.get(0).getNature().equals("SANG"));
		//teste une recherche infructueuse
		natures = 
			natureManager.findByNatureLikeManager("URINES", false);
		assertTrue(natures.size() == 0);
		//null recherche
		natures = 
			natureManager.findByNatureLikeManager(null, false);
		assertTrue(natures.size() == 0);
	}
	...
	public void testFindDoublon() {
		//Cree le doublon
		Nature n1 = (natureManager
				.findByNatureLikeManager("TISSU", true)).get(0);
	...
	public void testIsUsedObjectManager() {
		...
		Nature n1 = (natureManager
				.findByNatureLikeManager("SANG", true)).get(0);
		assertTrue(natureManager.isUsedObjectManager(n1));
		//Enregistrement n'est pas reference
		Nature n2 = (natureManager
				.findByNatureLikeManager("LCR", true)).get(0);
	...
	private void createObjectManagerTest() {
		...
		assertTrue((natureManager
				.findByNatureLikeManager("CERVEAU -", true)).size() == 1);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		Nature nBis = new Nature();
		...
		for (int i = 0; i < emptyValues.length; i++) {
			try {
				...
				assertTrue(natureManager
						.findByOrderManager(n1.getPlateforme()).size() == 4);
			}
		}		
	}
	...
	private void updateObjectManagerTest() {
		...
		Nature n1 = (natureManager
			.findByNatureLikeManager("CERVEAU -", true)).get(0);
		n1.setNature("UR. INES");
		natureManager.updateObjectManager(n1);
		assertTrue((natureManager
				.findByNatureLikeManager("UR. INES", true)).size() == 1);
		...
		assertTrue(catched);
		assertTrue((natureManager
				.findByNatureLikeManager("SANG", true)).size() == 1);
	...
	private void removeObjectManagerTest() {
		...
		Nature n1 = (natureManager
				.findByNatureLikeManager("UR. INES", true)).get(0);
		natureManager.removeObjectManager(n1);
		assertTrue((natureManager
				.findByNatureLikeManager("UR. INES", true)).size() == 0);
		...
		assertTrue(catched);
		assertTrue((natureManager
				.findByNatureLikeManager("SANG", true)).size() > 0);
		//null remove
		natureManager.removeObjectManager(null);
		assertTrue(natureManager
				.findByOrderManager(n1.getPlateforme()).size() == 3);
	}
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					natureManager.findByOrderManager(pf);
