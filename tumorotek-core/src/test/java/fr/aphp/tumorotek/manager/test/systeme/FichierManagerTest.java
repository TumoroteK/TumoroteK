...
public class FichierManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Fichier path = fichierManager.findByIdManager(1);
		assertNotNull(path);
		assertTrue(path.getPath().equals("PATH1"));
		Fichier pathNull = fichierManager.findByIdManager(5);
		assertNull(pathNull);
	}
	...
	public void testFindAll() {
		List<Fichier> list = fichierManager.findAllObjectsManager();
	...
	public void testFindByCrAnapathLikeExactManager() {
		List<Fichier> list = 
			fichierManager.findByPathLikeManager("PATH1", true);
	...
	public void testIsUsed() {
		Fichier path1 = fichierManager.findByIdManager(1);
		assertNotNull(path1);
		assertTrue(fichierManager.isUsedObjectManager(path1));
		
		Fichier path2 = fichierManager.findByIdManager(3);
	...
	public void testIsPathShared() {
		...
		Fichier path2 = fichierManager.findByIdManager(2);
	...
	public void testCrud() {
		...
		// Test de l'insertion
		Fichier path1 = new Fichier();
		path1.setPath("PATH1"); path1.setNom("xavi"); 
...
//		assertTrue(catchedInsert);
		...
		String[] emptyValues = new String[]{"", "  ", null, "*/uh", 
...
//		assertTrue(catchedUpdate);
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				...
				fichierManager.updateObjectManager(path2, null, filesCreated, 
						filesToDelete);
			} catch (Exception e) {
				if (e.getClass().getSimpleName().equals(
						"ValidationException")) {
		...
		Fichier path4 = fichierManager.findByIdManager(path2.getFichierId());
		...
		tmp = "domenech for ever"; byteArray = tmp.getBytes();
		bais = new ByteArrayInputStream(byteArray);
