...
public class ExportUtilsTest extends AbstractManagerTest4 {
	...
	public void testCreateExcellWorkBook() {
		HSSFWorkbook wb = exportUtils.createExcellWorkBook("test");
		assertNotNull(wb);
	...
	public void testAddMaladieData() {
		...
		TableAnnotation ta = tableAnnotationManager
			.findByNomLikeManager("TABLE_PAT1", true).get(0);
		Iterator<ChampAnnotation> it = tableAnnotationManager
			.getChampAnnotationsManager(ta).iterator();
		List<ChampAnnotation> listeCas = new ArrayList<ChampAnnotation>();
		while (it.hasNext()) {
			listeCas.add(it.next());
		}
		
		HSSFRow row = sheet.createRow(1);
		Utilisateur u = utilisateurManager.findByIdManager(1);
		...
		List<Operation> ops = operationManager.findByObjectManager(
				m1.getPatient());
	...
	public void testAddPrelevementData() {
		...
		assertTrue(operationManager.findAllObjectsManager().size() == 20);
		List<Operation> ops = operationManager.findByObjectManager(p1);
		operationManager.removeObjectManager(ops.get(ops.size() - 1));
		assertTrue(operationManager.findAllObjectsManager().size() == 19);
	}
	...
	public void testAddEchantillonData() {
		...
		assertTrue(operationManager.findAllObjectsManager().size() == 20);
		List<Operation> ops = operationManager.findByObjectManager(e1);
		operationManager.removeObjectManager(ops.get(ops.size() - 1));
		assertTrue(operationManager.findAllObjectsManager().size() == 19);
	}
	...
	public void testAddProdDeriveData() {
		...
		TableAnnotation ta = tableAnnotationManager
			.findByNomLikeManager("TABLE_DERIVE1", true).get(0);
		Iterator<ChampAnnotation> it = tableAnnotationManager
			.getChampAnnotationsManager(ta).iterator();
		List<ChampAnnotation> listeCas = new ArrayList<ChampAnnotation>();
		while (it.hasNext()) {
		...
		assertTrue(operationManager.findAllObjectsManager().size() == 20);
		List<Operation> ops = operationManager.findByObjectManager(d1);
		operationManager.removeObjectManager(ops.get(ops.size() - 1));
		assertTrue(operationManager.findAllObjectsManager().size() == 19);
	}
	...
	public void testAddCessionData() {
		...
		HSSFWorkbook wb = exportUtils.createExcellWorkBook("test");
		HSSFSheet sheet = wb.getSheetAt(0);
		Cession c1 = cessionManager.findByIdManager(2);
		...
		assertTrue(operationManager.findAllObjectsManager().size() == 20);
		List<Operation> ops = operationManager.findByObjectManager(c1);
		operationManager.removeObjectManager(ops.get(ops.size() - 1));
		assertTrue(operationManager.findAllObjectsManager().size() == 19);
	}
	...
	public void testDateRenderer() throws ParseException {
		...
		assertTrue(exportUtils.dateRenderer(date)
				.equals("17/09/2006 14:57"));
		
		date.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
			.parse("17/09/2006 00:00:00"));
		assertTrue(exportUtils.dateRenderer(date)
				.equals("17/09/2006"));
		
		assertTrue(exportUtils.dateRenderer(date.getTime())
				.equals("17/09/2006"));
	...
	public void testAddDataToRowCsv() {
		...
		try {
			...
			try { bw.close(); } catch (Exception e) { bw = null; }
			try { sw.close(); } catch (Exception e) { sw = null; }
		}
	}
}
