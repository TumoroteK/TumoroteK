...
public class CodeDossierManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<CodeDossier> dos = codeDossierManager.findAllCodeDossiersManager();
	...
	public void testFindByNomLikeManager() {
		...
		List<CodeDossier> dos = codeDossierManager
									.findByNomLikeManager("Dossier2", true, b);
	...
	public void testFindByCodeDossierParentManager() {
		CodeDossier d1 = codeDossierManager.findAllCodeDossiersManager().get(0);
		List<CodeDossier> dos = 
			codeDossierManager.findByCodeDossierParentManager(d1);
		assertTrue(dos.size() == 1);
		assertTrue(dos.get(0).getNom().equals("Dossier2"));
		CodeDossier d2 = codeDossierManager.findAllCodeDossiersManager().get(1);
	...
	public void testFindByRootDossierUtilisateurManager() {
		Banque b = banqueDao.findById(1);
		List<CodeDossier> dos = 
			codeDossierManager.findByRootCodeDossierUtilisateurManager(b);
		assertTrue(dos.size() == 2);
	...
	public void testFindByRootSelectUtilisateurManager() {
		...
		List<CodeDossier> dos = 
			codeDossierManager.findByRootCodeDossierSelectManager(u, b);
		assertTrue(dos.size() == 1);
	...
	public void testFindByUtilisateurAndBanqueManager() {
		...
		Banque b = banqueDao.findById(1);
		List<CodeDossier> dos = 
			codeDossierManager.findByUtilisateurAndBanqueManager(u, b);
		assertTrue(dos.size() == 2);
		u = utilisateurDao.findById(2);
	...
	public void testFindBySelectUtilisateurAndBanqueManager() {
		...
		Banque b = banqueDao.findById(1);
		List<CodeDossier> dos = 
			codeDossierManager.findBySelectUtilisateurAndBanqueManager(u, b);
		assertTrue(dos.size() == 1);
		...
		assertTrue(dos.size() == 0);
		dos = codeDossierManager
						.findBySelectUtilisateurAndBanqueManager(null, null);
		assertTrue(dos.size() == 0);
	}
	...
	public void testFindDoublonManager() {
		Banque b = banqueDao.findById(1);
		//Cree le doublon
		CodeDossier d1 = codeDossierManager
							.findByNomLikeManager("Dossier2", true, b).get(0);
		
		CodeDossier d2 = new CodeDossier();
		d2.setNom(d1.getNom());
	...
	public void createObjectManagerTest() {
		...
		CodeDossier dos = new CodeDossier();
		/*Champs obligatoires*/
		dos.setNom("newDossier");
		Banque b = banqueDao.findById(1);
		Utilisateur u = utilisateurDao.findById(2);
		dos.setCodeSelect(false);
		CodeDossier parent = codeDossierManager
										.findAllCodeDossiersManager().get(3);
		
		//required Entite
		try {
			codeDossierManager.createOrUpdateManager(dos, 
		...
		codeDossierManager
					.createOrUpdateManager(dos, parent, b, u, "creation");
		assertTrue((codeDossierManager
					.findByNomLikeManager("newDossier", true, b)).size() == 1);
		assertTrue(codeDossierManager
						.findByCodeDossierParentManager(parent).size() == 1);

		assertTrue(operationManager.findByObjectManager(dos).size() == 1);
		assertTrue(operationManager
				.findByObjectManager(dos).get(0)
		...
		try {
			codeDossierManager
				.createOrUpdateManager(d2, null, null, null, "creation");
		} catch (Exception ex) {
		...
		codeUtilisateurManager
			.createOrUpdateManager(herited, dos, b, u, null, null, "creation");
		assertTrue(codeUtilisateurManager
				.findByCodeDossierManager(dos).size() == 1);
		
		// creation sous-dossier
		CodeDossier sousdos = new CodeDossier();
		sousdos.setNom("sousdos");
		sousdos.setCodeSelect(false);
		codeDossierManager
			.createOrUpdateManager(sousdos, dos, b, u, "creation");
		assertTrue(codeDossierManager
				.findByCodeDossierParentManager(dos).size() == 1);
	}
	

	private void updateObjectManagerTest() {
		Banque b = banqueDao.findById(1);
		CodeDossier d = codeDossierManager
							.findByNomLikeManager("newDossier", true, b).get(0);
		...
		try {
			codeDossierManager
				.createOrUpdateManager(d, null, null, null, "modification");
		...
		codeDossierManager
				.createOrUpdateManager(d, null, null, null, "modification");
		assertTrue((codeDossierManager
				.findByNomLikeManager("AnotherDossier", true, b)).size() == 1);

		assertTrue(operationManager.findByObjectManager(d).size() == 2);
		assertTrue(operationManager.findByObjetIdEntiteAndOpeTypeManager(d, 
	...
	private void removeObjectManagerTest() {
		Banque b = banqueDao.findById(1);
		CodeDossier d = codeDossierManager
						.findByNomLikeManager("AnotherDossier", true, b).get(0);
		codeDossierManager.removeObjectManager(d);
		assertTrue((codeDossierManager
				.findByNomLikeManager("AnotherDossier", true, b)).size() == 0);
	...
	public void testNomValidation() {
		CodeDossier d = new CodeDossier();
		
		// nom
		List<Errors> errs = new ArrayList<Errors>();
		try {
			BeanValidator.validateObject((Object) d, 
									new Validator[] {codeDossierValidator});
		} catch (ValidationException ve) {
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("dossier.nom.empty"));
		}
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("dossier.nom.illegal"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
							.getCode().equals("dossier.nom.tooLong"));
		}
		assertFalse(errs.isEmpty());
		errs.clear();
	}
	...
	public void testcreateAndRemoveSelectDossierManagerTest() {
		...
		codeDossierManager
					.createOrUpdateManager(dos, null, b, u, "creation");
		assertTrue((codeDossierManager
			.findByNomLikeManager("newDossierSelect", true, b)).size() == 1);
		
		
		
		// creation codes appartenant au dossier
		CodeSelect cs1 = new CodeSelect();
		cs1.setCodeId(28);
		cs1.setTableCodage(tableCodageDao.findById(3));
		codeSelectManager.createOrUpdateManager(cs1, dos, b, u, "creation");
		assertTrue(codeSelectManager
							.findByCodeDossierManager(dos).size() == 1);
		
		CodeSelect cs2 = new CodeSelect();
		cs2.setCodeId(32);
		cs2.setTableCodage(tableCodageDao.findById(3));
		...
		assertTrue((codeDossierManager
			.findByNomLikeManager("newDossierSelect", true, b)).size() == 0);
	...
	public void testFindByRootDossierBanqueManager() {
		Banque b = banqueDao.findById(1);
		List<CodeDossier> doss 
			= codeDossierManager.findByRootDossierBanqueManager(b, true);
