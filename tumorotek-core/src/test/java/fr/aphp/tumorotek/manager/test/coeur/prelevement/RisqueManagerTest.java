...
public class RisqueManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<Risque> risques = risqueManager.findAllObjectsManager();
	...
	public void testfindByNomLikeManager() {
		...
		List<Risque> risques = risqueManager
								.findByNomLikeManager("HIV", true);
	...
	public void testfindByInfectieuxManager() {
		...
		List<Risque> risques = 
			risqueManager.findByInfectieuxManager(true);
	...
	public void testIsUsedObject() {
		...
		Risque r2 = (risqueManager
				.findByNomLikeManager("GRIPPE", false)).get(0);
	...
	private void createObjectManagerTest() {
		...
		Risque r1 = new Risque();
		r1.setNom("HEPATITE B-");
		r1.setInfectieux(false);
		r1.setPlateforme(plateformeDao.findById(1));
		risqueManager.createObjectManager(r1);
		assertTrue((risqueManager
				.findByNomLikeManager("HEPATITE B-", true)).size() == 1);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		Risque r1Bis = new Risque();
		...
		assertTrue(catched);
		assertTrue((risqueManager
				.findByNomLikeManager("HEPAT%", false)).size() == 1);
		assertTrue((risqueManager
				.findByInfectieuxManager(false)).size() == 2);
		
		//validation test Type
		String[] nomValues = new String[]{"", "  ", null, 
						"HOO%", createOverLength(200), "goo d98-Type"};
		Boolean[] infValues = new Boolean[]{null, false};
		Risque r2 = new Risque();
		r2.setPlateforme(plateformeDao.findById(2));
	...
	private void updateObjectManagerTest() {
		...
		Risque r1 = (risqueManager
			.findByNomLikeManager("HEPATITE B-", true)).get(0);
		r1.setNom("L'uôpuç+");
		r1.setInfectieux(true);
		risqueManager.updateObjectManager(r1);
		assertTrue((risqueManager
				.findByNomLikeManager("L'uôpuç+", true))
				.get(0).getInfectieux().equals(true));
		...
		assertTrue(catched);
		assertTrue((risqueManager
				.findByNomLikeManager("HIV", false)).size() == 1);
		
		//Validation test
		String[] nomValues = new String[]{"", "  ", null, "|mpl1", 
								createOverLength(200), "goo d98-Type"};
		Boolean[] infValues = new Boolean[]{null, false};
		for (int i = 0; i < nomValues.length; i++) {
			for (int j = 0; j < infValues.length; j++) {
				try {
	...
	private void removeObjectManagerTest() {
		...
		Risque r1 = (risqueManager
				.findByNomLikeManager("L'uôpuç+", true)).get(0);
		risqueManager.removeObjectManager(r1);
		assertTrue((risqueManager
				.findByNomLikeManager("L'uôpuç+", true)).size() == 0);
...
//		assertTrue(catched);
		assertTrue((risqueManager
				.findByNomLikeManager("HIV", true)).size() > 0);
		//null remove
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
						risqueManager.findByOrderManager(pf);
	...
	public void testFindByPatientAndPlateformeManager() {
		Patient pat = patientDao.findById(3);
		Plateforme pf = plateformeDao.findById(1);
		List<Risque> risks = risqueManager
				.findByPatientAndPlateformeManager(pat, pf); 
		assertTrue(risks.size() == 2);
		assertTrue(risks.contains(risqueManager
								.findByNomLikeManager("HIV", true).get(0)));
		assertTrue(risks.contains(risqueManager
							.findByNomLikeManager("LEUCEMIE", true).get(0)));
