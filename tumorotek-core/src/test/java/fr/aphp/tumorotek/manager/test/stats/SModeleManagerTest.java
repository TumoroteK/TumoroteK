...
public class SModeleManagerTest extends AbstractManagerTest4 {
	...
	public void testFindByIdManager() {
		...
		Integer id2 = 13;
		SModele im2 = sModeleManager.findByIdManager(id2);
		assertNull(im2);
	}

	@Test
	public void testFindAllObjectsManager() {
		List<SModele> ims = modeleDao.findAll();
	...
	public void testFindDoublon() {
		...
		String nom1 = "MOD1";
		String nom2 = "MyModele";
		Plateforme pf1 = plateformeManager.findByIdManager(1);

		SModele m1 = new SModele();
		...
		SModele m2 = sModeleManager.findByIdManager(2);
	...
	public void testValidateObject() {
		...
		try {
			...
			assertEquals("stats.modele.nom.empty", 
					ve.getErrors().get(0).getFieldError().getCode());
			catched = true;
		}
		assertTrue(catched);
		catched = false;
		...
		try {
			...
			assertTrue(ve.getErrors().size() == 1);
			assertEquals("stats.modele.nom.illegal", 
					ve.getErrors().get(0).getFieldError().getCode());
			catched = true;
		}
		assertTrue(catched);
		...
		try {
			...
			assertTrue(ve.getErrors().size() == 1);
			assertEquals("stats.modele.nom.tooLong", 
					ve.getErrors().get(0).getFieldError().getCode());
			catched = true;
		}
		assertTrue(catched);
	...
	public void testGetIndicateursManager() {
		SModele m1 = sModeleManager.findByIdManager(1);
		assertTrue(sModeleManager.getIndicateursManager(m1).size() == 2);
		assertTrue(sModeleManager.getIndicateursManager(m1)
						.get(0).equals(indicateurManager.findByIdManager(1)));
		assertTrue(sModeleManager.getIndicateursManager(m1)
				.get(1).equals(indicateurManager.findByIdManager(2)));
		SModele m3 = sModeleManager.findByIdManager(3);
	...
	public void testCrud() {
		...
		int sModeleTotCount = 3;
		int indicTotCount = 5;
		int sMIndicTotCount = 3;
		...
		try {
			...
			if (e.getClass().getSimpleName()
					.equals("RequiredObjectIsNullException")) {
				catched = true;
			}
		}
		assertTrue(catched);
		...
		assertTrue(sModeleManager.getBanquesManager(sM2)
				.containsAll(banqueManager.findByPlateformeAndArchiveManager(pf1, false)));
		...
		assertTrue(sM2.getSModeleIndicateurs()
		...
		try {
			...
			assertTrue(re.getMessage()
				.equals("stats.modele.indicateurs.toomany.subdivisions"));
			catched = true;
		}
		assertTrue(catched);
	...
	public void testGetSubdivisionManager() {
		SModele sM1 = sModeleManager.findByIdManager(1);
		Subdivision s = sModeleManager.getSubdivisionManager(sM1);
