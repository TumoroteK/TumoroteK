...
public class DestructionMotifManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		DestructionMotif dm1 = destructionMotifManager.findByIdManager(1);
		assertNotNull(dm1);
		assertTrue(dm1.getMotif().equals("TUBE ILLISIBLE"));
		
		DestructionMotif dmNull = destructionMotifManager.findByIdManager(10);
		assertNull(dmNull);
	}
	...
	public void testFindByMotifLikeExactManager() {
		List<DestructionMotif> list = 
			destructionMotifManager
			.findByMotifLikeManager("INUTILISABLE", true);
	...
	public void testFindByMotifLikeManager() {
		List<DestructionMotif> list = 
			destructionMotifManager
			.findByMotifLikeManager("INUTILISABLE", false);
		assertTrue(list.size() == 1);
	...
	public void testFindDoublon() {
		//Cree le doublon
		DestructionMotif d1 = (destructionMotifManager
		...
		DestructionMotif d2 = new DestructionMotif();
	...
	public void createObjectManagerTest() {
		...
		DestructionMotif d1 = new DestructionMotif();
		d1.setMotif("PAS BON");
		d1.setPlateforme(plateformeDao.findById(1));
		destructionMotifManager.createObjectManager(d1);
		assertTrue((destructionMotifManager
				.findByMotifLikeManager("PAS BON", true)).size() == 1);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		DestructionMotif d1Bis = new DestructionMotif();
		d1Bis.setMotif("PAS BON");
		try {
			destructionMotifManager.createObjectManager(d1Bis);
		} catch (Exception e) {
		...
		for (int i = 0; i < emptyValues.length; i++) {
			try {
				...
				destructionMotifManager.createObjectManager(d2);
			} catch (ValidationException e) {
				//verifie qu'aucune ligne n'a ete ajoutee
				assertTrue(destructionMotifManager
					.findByOrderManager(d1.getPlateforme()).size() == 3);
			}
		}		
	}
	...
	public void updateObjectManagerTest() {
		...
		DestructionMotif d1 = (destructionMotifManager
				.findByMotifLikeManager("PAS BON", true)).get(0);
		d1.setMotif("NON");
		destructionMotifManager.updateObjectManager(d1);
		assertTrue((destructionMotifManager
				.findByMotifLikeManager("NON", true)).size() == 1);
		...
		assertTrue(catched);
		assertTrue((destructionMotifManager
				.findByMotifLikeManager("INUTILISABLE", true)).size() == 1);
	...
	public void removeObjectManagerTest() {
		...
		DestructionMotif d1 = (destructionMotifManager
				.findByMotifLikeManager("NON", true)).get(0);
		destructionMotifManager.removeObjectManager(d1);
		assertTrue((destructionMotifManager
				.findByMotifLikeManager("NON", true)).size() == 0);
...
//		assertTrue(catched);
		assertTrue((destructionMotifManager
				.findByMotifLikeManager("TUBE ILLISIBLE", true)).size() > 0);
		//null remove
		destructionMotifManager.removeObjectManager(null);
		assertTrue(destructionMotifManager
						.findByOrderManager(d1.getPlateforme()).size() == 2);
	}
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
			destructionMotifManager.findByOrderManager(pf);
