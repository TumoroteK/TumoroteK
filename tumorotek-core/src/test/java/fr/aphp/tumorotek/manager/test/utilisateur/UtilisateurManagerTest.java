...
public class UtilisateurManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Utilisateur utilisateurNull = utilisateurManager.findByIdManager(10);
	...
	public void testFindByLoginAndArchvieManager() {
		List<Plateforme> pfs = plateformeManager.findAllObjectsManager();
		List<Utilisateur> list = utilisateurManager
			.findByLoginAndArchiveManager("USER1", false, pfs);
		assertTrue(list.size() == 1);
		
		list = utilisateurManager
					.findByLoginAndArchiveManager("USER1", true, pfs);
		assertTrue(list.size() == 0);
		
		list = utilisateurManager
				.findByLoginAndArchiveManager("USER", true, pfs);
		assertTrue(list.size() == 0);
		
		list = utilisateurManager
				.findByLoginAndArchiveManager("", false, pfs);
		assertTrue(list.size() == 0);
		
		list = utilisateurManager
				.findByLoginAndArchiveManager(null, false, pfs);
		assertTrue(list.size() == 0);
	}
	...
	public void testFindAll() {
		List<Utilisateur> list = utilisateurManager.findAllObjectsManager();
	...
	public void testGetPlateformes() {
		...
		Utilisateur u5 = utilisateurManager.findByIdManager(5);
	...
	public void testGetAvailableBanquesManager() {
		...
		Utilisateur u4 = utilisateurManager.findByIdManager(4);
	...
	public void testGetAvailableBanquesAsAdminManager() {
		Utilisateur u1 = utilisateurManager.findByIdManager(1);
		List<Banque> bks = utilisateurManager
			.getAvailableBanquesAsAdminManager(u1);
		assertTrue(bks.size() == 2);
	...
	public void testIsUsedObjectManager() {
		...
		assertFalse(utilisateurManager.isUsedObjectManager(
				new Utilisateur()));
	...
	public void testFindByLoginPasswordAndArchiveManager() {
		List<Utilisateur> liste = 
			utilisateurManager.findByLoginPasswordAndArchiveManager(
					"USER5", "b383bb08bd750d8ef04d034ad648a208", false);
		assertTrue(liste.size() == 1);
		
		liste =  utilisateurManager.findByLoginPasswordAndArchiveManager(
					"USER6", "b383bb08bd750d8ef04d034ad648a208", false);
		assertTrue(liste.size() == 0);
		
		liste =  utilisateurManager.findByLoginPasswordAndArchiveManager(
				"USER5", "b383bb08bd750d8ef04d034ad64dvs8a208", false);
		assertTrue(liste.size() == 0);
		
		liste =  utilisateurManager.findByLoginPasswordAndArchiveManager(
				"USER5", "b383bb08bd750d8ef04d034ad648a208", true);
		assertTrue(liste.size() == 0);
		
		liste =  utilisateurManager.findByLoginPasswordAndArchiveManager(
				null, "b383bb08bd750d8ef04d034ad648a208", false);
		assertTrue(liste.size() == 0);
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Utilisateur admin = utilisateurManager.findByIdManager(1);
		Utilisateur u1 = new Utilisateur();
		...
		u1.setPassword("PASS");
		try {
			utilisateurManager.createObjectManager(
					u1, null, null, null, admin, null);
		} catch (Exception e) {
		...
		pu1.setBanque(b1);
		pu1.setProfil(p1);
		ProfilUtilisateur pu2 = new ProfilUtilisateur();
		pu2.setBanque(b1);
		pu2.setProfil(null);
		profils.add(pu1);
		profils.add(pu2);
		...
		try {
			utilisateurManager.createObjectManager(
					u1, null, profils, null, admin, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		Plateforme pf2 = plateformeManager.findByIdManager(2);
		List<Plateforme> plateformes = new ArrayList<Plateforme>();
		OperationType oType = operationTypeDao.findById(5);
		...
		plateformes.add(pf1);
		utilisateurManager.createObjectManager(
				u, null, profils, plateformes, admin, 
				plateformeManager.findByIdManager(1));
		assertTrue(utilisateurManager.findAllObjectsManager().size() == 5);
		assertTrue(profilUtilisateurManager
				.findAllObjectsManager().size() == 9);
		assertTrue(plateformeManager.findAllObjectsManager().size() == 2);
		Integer idUp1 = u.getUtilisateurId();
		
		Utilisateur uUp1 = utilisateurManager.findByIdManager(idUp1);
		boolean catched = false;
		// on test l'update d'un doublon
		uUp1.setLogin("USER1");
		try {
			utilisateurManager.updateObjectManager(
					uUp1, null, null, null, admin, oType);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catched = true;
		...
		try {
			utilisateurManager.updateObjectManager(
					uUp1, null, profils, null, admin, oType);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		plateformes.add(pf2);
		utilisateurManager.updateObjectManager(
				uUp1, null, profils, plateformes, admin, oType);
		assertTrue(utilisateurManager.findAllObjectsManager().size() == 5);
		assertTrue(profilUtilisateurManager
				.findAllObjectsManager().size() == 9);
		assertTrue(plateformeManager.findAllObjectsManager().size() == 2);
		...
		Utilisateur uTest = utilisateurManager.findByIdManager(idUp1);
		assertNotNull(uTest);
		assertTrue(uTest.getLogin().equals("HEYO"));
		assertNull(uTest.getCollaborateur());
		assertTrue(profilUtilisateurManager
				.findByUtilisateurManager(uTest, null).size() == 1);
		assertTrue(utilisateurManager.getPlateformesManager(uTest).size() == 1);
		assertTrue(profilUtilisateurManager
				.findByUtilisateurManager(uTest, null).get(0).equals(pu2));
		Iterator<Plateforme> it = utilisateurManager
			.getPlateformesManager(uTest).iterator();
		...
		utilisateurManager.updateObjectManager(
				uTest, c, profils, plateformes, admin, oType);
		assertTrue(utilisateurManager.findAllObjectsManager().size() == 5);
		assertTrue(profilUtilisateurManager
				.findAllObjectsManager().size() == 8);
		assertTrue(plateformeManager.findAllObjectsManager().size() == 2);
		...
		Utilisateur uTest2 = utilisateurManager.findByIdManager(idUp1);
		assertNotNull(uTest2);
		assertNotNull(uTest2.getCollaborateur());
		assertTrue(uTest2.getLogin().equals("MATHIEU"));
		assertTrue(profilUtilisateurManager
				.findByUtilisateurManager(uTest2, null).size() == 0);
		assertTrue(utilisateurManager.getPlateformesManager(uTest2)
				.size() == 0);
	...
	public void testUpdatePasswordManager() {
		...
		utilisateurManager.createObjectManager(
				u1, c, profils, plateformes, admin, 
				plateformeManager.findByIdManager(2));
		assertTrue(utilisateurManager.findAllObjectsManager().size() == 5);
		assertTrue(profilUtilisateurManager
				.findAllObjectsManager().size() == 10);
		assertTrue(plateformeManager.findAllObjectsManager().size() == 2);
		...
		assertTrue(profilUtilisateurManager
				.findByUtilisateurManager(uTest, null).size() == 2);
		assertTrue(utilisateurManager.getPlateformesManager(uTest)
				.size() == 2);
		
		// modif mot de passe avec même pwd
		utilisateurManager.updatePasswordManager(uTest, "PASS MAT", 
				7, admin);
		assertTrue(operationManager.findByObjectManager(uTest).size() == 1);
		
		// modif mot de passe avec pwd null
		Utilisateur uTest3 = utilisateurManager.updatePasswordManager(
				uTest, null, 7, admin);
		assertTrue(operationManager.findByObjectManager(uTest).size() == 1);
		
		// modif valide avec Timeout
		utilisateurManager.updatePasswordManager(uTest, "NEW PASS",
				7, admin);
		Utilisateur uTest2 = utilisateurManager.findByIdManager(idU1);
		assertNotNull(uTest2);
		assertNotNull(uTest2.getCollaborateur());
		assertTrue(uTest2.getLogin().equals("MATHIEU"));
		assertTrue(profilUtilisateurManager
		...
		assertTrue(calTest.get(Calendar.YEAR) 
				== calTest2.get(Calendar.YEAR));
		assertTrue(calTest.get(Calendar.MONTH) 
				== calTest2.get(Calendar.MONTH));
		assertTrue(calTest.get(Calendar.DAY_OF_MONTH) 
				== calTest2.get(Calendar.DAY_OF_MONTH));
		
		// modif valide sans mois
		calTest.setTime(uTest2.getTimeOut());
		utilisateurManager.updatePasswordManager(uTest2, "NEW PASS2",
				null, admin);
		Utilisateur uTest4 = utilisateurManager.findByIdManager(idU1);
		...
		utilisateurManager.updatePasswordManager(uTest4, "NEW PASS3",
				0, admin);
		Utilisateur uTest5 = utilisateurManager.findByIdManager(idU1);
		...
		utilisateurManager.updatePasswordManager(uTest5, "NEW PASS5",
				7, admin);
		Utilisateur uTest6 = utilisateurManager.findByIdManager(idU1);
	...
	public void testArchivageUtilisateurManager() {
		...
		Utilisateur uTest3 = utilisateurManager.findByIdManager(idU1);
		...
		assertTrue(profilUtilisateurManager
				.findByUtilisateurManager(uTest4, null).size() == 2);
		assertTrue(utilisateurManager.getPlateformesManager(uTest4)
				.size() == 2);
		...
		assertTrue(operationManager.findByObjectManager(uTest4).get(1)
				.getOperationType().getNom().equals("Archivage"));
	 ...
	 * @throws ParseException 
	 */
	private void validationInsert(Utilisateur utilisateur) 
		throws ParseException {
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				...
				utilisateurManager.createObjectManager(utilisateur, 
		...
		String[] emptyValues2 = new String[]{null, "", "  ", 
				createOverLength(100)};
	 ...
	 * @throws ParseException 
	 */
	private void validationUpdate(Utilisateur utilisateur) 
		throws ParseException {
		OperationType oType = operationTypeDao.findById(5);
		boolean catched = false;
		Utilisateur admin = utilisateurManager.findByIdManager(1);
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				utilisateur.setLogin(emptyValues[i]);
				utilisateurManager.updateObjectManager(utilisateur, 
						null, null, null, admin, oType);
			} catch (Exception e) {
				if (e.getClass().getSimpleName().equals(
						"ValidationException")) {
					catched = true;
	...
	private void removeObjectManagerTest() {
		...
		u.setLogin("TEST");
		u.setPassword("PASS");
		utilisateurManager.createObjectManager(u, null, null, null, admin, 
				plateformeManager.findByIdManager(1));
		assertTrue(utilisateurManager.findAllObjectsManager().size() == 5);
		Integer id = u.getUtilisateurId();
...
//		assertTrue(codeUtilisateurManager.findAllObjectsManager().size() == 7);
		...
		Utilisateur uTest = utilisateurManager.findByIdManager(id);		
		utilisateurManager.removeObjectManager(uTest);
		assertTrue(utilisateurManager.findAllObjectsManager().size() == 4);
		assertTrue(operationManager.findByObjectManager(uTest).size() == 0);
	...
	public void testGetAvailablePlateformesManager() {
		Utilisateur u1 = utilisateurManager.findByIdManager(1);
		List<Plateforme> pfs = utilisateurManager
			.getAvailablePlateformesManager(u1);
		// @since 2.1 PF2 ne sort pas car la seule banque 2 est archivée
		assertTrue(pfs.size() == 1);
		
		Utilisateur u2 = utilisateurManager.findByIdManager(2);
		pfs = utilisateurManager
			.getAvailablePlateformesManager(u2);
		assertTrue(pfs.size() == 1);
		
		Utilisateur u3 = utilisateurManager.findByIdManager(3);
		pfs = utilisateurManager
			.getAvailablePlateformesManager(u3);
		assertTrue(pfs.size() == 1);
		
		Utilisateur u4 = utilisateurManager.findByIdManager(4);
		pfs = utilisateurManager
			.getAvailablePlateformesManager(u4);
		assertTrue(pfs.size() == 0);
		
		Utilisateur u5 = utilisateurManager.findByIdManager(5);
		pfs = utilisateurManager
			.getAvailablePlateformesManager(u5);
		assertTrue(pfs.size() == 2);
		
		pfs = utilisateurManager
			.getAvailablePlateformesManager(null);
		assertTrue(pfs.size() == 0);
	}
	...
	public void testGetAvailableBanquesByPlateformeManager() {
		...
		Utilisateur u1 = utilisateurManager.findByIdManager(1);
		List<Banque> bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u1, pf1);
		assertTrue(bks.size() == 2);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u1, pf2);
		// assertTrue(bks.size() == 1);
		assertTrue(bks.size() == 0); // since 2.1 archive = true
		
		Utilisateur u3 = utilisateurManager.findByIdManager(3);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u3, pf1);
		assertTrue(bks.size() == 2);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u3, pf2);
		assertTrue(bks.size() == 0);
		
		Utilisateur u4 = utilisateurManager.findByIdManager(4);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u4, pf1);
		assertTrue(bks.size() == 0);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u4, pf2);
		assertTrue(bks.size() == 0);
		
		Utilisateur u5 = utilisateurManager.findByIdManager(5);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u5, pf1);
		assertTrue(bks.size() == 3);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u5, pf2);
		// assertTrue(bks.size() == 1);
		assertTrue(bks.size() == 0); // since 2.1 archive = true
		
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(null, pf2);
		assertTrue(bks.size() == 0);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(u5, null);
		assertTrue(bks.size() == 0);
		bks = utilisateurManager
			.getAvailableBanquesByPlateformeManager(null, null);
		assertTrue(bks.size() == 0);
	}
	...
	public void testArchiveScheduledUtilisateursManager() {
		...
		Utilisateur uNew1 = new Utilisateur();
		...
		utilisateurManager.createObjectManager(
				uNew1, c, null, null, admin, 
				plateformeManager.findByIdManager(2));
		
		Utilisateur uNew2 = new Utilisateur();
		uNew2.setLogin("EXPIR2");
		uNew2.setPassword("EXPIR2");
		uNew2.setTimeOut(cal.getTime());
		uNew2.setArchive(false);
		utilisateurManager.createObjectManager(
				uNew2, c, null, null, admin, 
				plateformeManager.findByIdManager(2));
		
		List<Utilisateur> toArchiveNow = utilisateurDao.findByTimeOutBefore(Calendar.getInstance().getTime());
		...
		u2.setArchive(false);
		utilisateurManager.updateObjectManager(u2, u2.getCollaborateur(), 
				null, null, admin, operationTypeDao.findById(5));
		// suppr 2 dernieres ops
		operationManager.removeObjectManager(operationManager.findByObjectManager(u2)
							.get(operationManager.findByObjectManager(u2).size() - 1));
		operationManager.removeObjectManager(operationManager.findByObjectManager(u2)
				.get(operationManager.findByObjectManager(u2).size() - 1));
		Utilisateur u4 = utilisateurDao.findById(4);
		u4.setArchive(false);
		utilisateurManager.updateObjectManager(u4, u4.getCollaborateur(), 
				null, null, admin, operationTypeDao.findById(5));
		// suppr 2 derniere ops
		operationManager.removeObjectManager(operationManager.findByObjectManager(u4)
							.get(operationManager.findByObjectManager(u4).size() - 1));
		operationManager.removeObjectManager(operationManager.findByObjectManager(u4)
				.get(operationManager.findByObjectManager(u4).size() - 1));
