...
public class EchantillonManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Echantillon echan = echantillonManager.findByIdManager(1);
		assertNotNull(echan);
		assertTrue(echan.getCode().equals("PTRA.1"));

		Echantillon echanNull = echantillonManager.findByIdManager(5);
		assertNull(echanNull);
	}

	@Test
	public void testFindAll() {
		List<Echantillon> echans = echantillonManager.findAllObjectsManager();
		assertEquals(4, echans.size());
	}

	@Test
	public void testFindByBanquesManager() {
		List<Banque> banks = new java.util.ArrayList<Banque>();
		banks.add(banqueDao.findById(1));
		banks.add(banqueDao.findById(2));
		List<Echantillon> res = echantillonManager
				.findByBanquesManager(banks);
	...
	public void testFindAllObjectsIdsByBanquesManager() {
		...
		banks.add(banqueDao.findById(1));
		banks.add(banqueDao.findById(2));
		List<Integer> res = echantillonManager
	...
	public void testFindByEchantillonTypeManager() {
		...
		List<Echantillon> echans = 
				echantillonManager.findByEchantillonTypeManager(type);
		assertTrue(echans.size() == 3);

		EchantillonType type2 = echantillonTypeManager.findByIdManager(2);
	...
	public void testFindByDate() throws Exception {
		...
		List<Echantillon> echans = 
				echantillonManager.findByDateStockAfterDateManager(search);
	...
	@Test
	public void testFindByDateStockAfterDateWithBanqueManager() 
			throws Exception {
		...
		List<Echantillon> echans = 
				echantillonManager
				.findByDateStockAfterDateWithBanqueManager(search, banks);
		assertTrue(echans.size() == 1);

		banks.add(b2);
		echans = echantillonManager
				.findByDateStockAfterDateWithBanqueManager(search, banks);
		assertTrue(echans.size() == 1);

		echans = echantillonManager
				.findByDateStockAfterDateWithBanqueManager(search, null);

		assertTrue(echans.size() == 0);

		echans = echantillonManager
				.findByDateStockAfterDateWithBanqueManager(null, banks);
		assertTrue(echans.size() == 0);
		...
		echans = echantillonManager
				.findByDateStockAfterDateWithBanqueManager(null, null);
		assertTrue(echans.size() == 0);
	}
	...
	public void testFindLastCreationManager() {
		...
		List<Echantillon> list = echantillonManager
	...
	public void testFindRestantsByPrelevementManager() {
		...
		List<Echantillon> echans = 
				echantillonManager.findRestantsByPrelevementManager(p);
		assertTrue(echans.size() == 1);
	...
	public void testFindByPrelevementAndStatutManager() {
		Prelevement p = prelevementDao.findById(1);
		ObjetStatut s = objetStatutManager.findByIdManager(1);
		List<Echantillon> echans = 
				echantillonManager.findByPrelevementAndStatutManager(p, s);
		assertTrue(echans.size() == 2);
		...
		echans = echantillonManager.findByPrelevementAndStatutManager(
				null, null);
		assertTrue(echans.size() == 0);
	}
	...
	public void testFindByPatientNomReturnIdsManager() {
		...
		banks.add(b1);
		List<Integer> echans = 
				echantillonManager.findByPatientNomReturnIdsManager(
						"DELPHINO", banks, true);
		assertTrue(echans.size() == 3);

		echans = echantillonManager.findByPatientNomReturnIdsManager(
				"ELPH", banks, false);
		assertTrue(echans.size() == 3);
	...
	public void testFindByCodeLikeExactManager() {
		List<Echantillon> echans = 
				echantillonManager.findByCodeLikeManager("PTRA.1", true);
		assertTrue(echans.size() == 1);
	...
	public void testFindByCodeLikeManager() {
		List<Echantillon> echans = 
				echantillonManager.findByCodeLikeManager("PTRA", false);
		assertTrue(echans.size() == 2);
	...
	public void testFindByCodeLikeWithBanqueManager() {
		...
		List<Echantillon> echans = 
				echantillonManager.findByCodeLikeWithBanqueManager(
						"PTRA", b1, false);
		assertTrue(echans.size() == 2);

		echans = echantillonManager.findByCodeLikeWithBanqueManager(
				"PTRA", b2, false);
		assertTrue(echans.size() == 0);
	...
	public void testFindAllCodesForBanqueManager() {
		...
		List<String> codes = 
				echantillonManager.findAllCodesForBanqueManager(b);
	...
	public void testFindAllCodesForBanqueAndStockesManager() {
		Banque b = banqueManager.findByIdManager(1);
		List<String> codes = 
				echantillonManager.findAllCodesForBanqueAndStockesManager(b);
	...
	public void testFindAllCodesForMultiBanquesAndStockesManager() {
		...
		List<String> codes = 
				echantillonManager
				.findAllCodesForMultiBanquesAndStockesManager(banques);
		assertTrue(codes.size() == 2);
		assertTrue(codes.get(0).equals("PTRA.1"));

		banques.remove(b1);
		codes = echantillonManager
				.findAllCodesForMultiBanquesAndStockesManager(banques);
		assertTrue(codes.size() == 0);

		codes = echantillonManager
				.findAllCodesForMultiBanquesAndStockesManager(null);
		assertTrue(codes.size() == 0);
	}
	...
	public void testFindAllCodesForBanqueAndQuantiteManager() {
		Banque b = banqueManager.findByIdManager(1);
		List<String> codes = 
				echantillonManager.findAllCodesForBanqueAndQuantiteManager(b);
		assertTrue(codes.size() == 2);
		assertTrue(codes.get(0).equals("PTRA.1"));
		assertTrue(codes.get(1).equals("PTRA.2"));
		...
		codes = echantillonManager
				.findAllCodesForBanqueAndQuantiteManager(null);
		assertTrue(codes.size() == 0);
	}
	...
	public void testGetPrelevementManager() {
		Echantillon echan1 = echantillonManager.findByIdManager(1);
		Prelevement prlvt1 = 
				echantillonManager.getPrelevementManager(echan1);
		assertTrue(prlvt1.getCode().equals("PRLVT1"));
		assertTrue(prlvt1.getClass().getSimpleName().equals("Prelevement"));

		Echantillon echan2 = echantillonManager.findByIdManager(3);
		Prelevement prlvt2 = 
				echantillonManager.getPrelevementManager(echan2);
		assertTrue(prlvt2.getCode().equals("PRLVT2"));

		Prelevement prlvtNull = 
				echantillonManager.getPrelevementManager(null);
	...
	public void testGetProdDerivesManager() {
		...
		List<ProdDerive> derives = 
				echantillonManager.getProdDerivesManager(echan1);
		assertTrue(derives.size() == 1);

		Echantillon echan2 = echantillonManager.findByIdManager(2);
		derives = 
				echantillonManager.getProdDerivesManager(echan2);
		assertTrue(derives.size() == 0);

		derives = 
				echantillonManager.getProdDerivesManager(null);
		assertTrue(derives.size() == 0);
	}
	...
	public void testGetEmplacementManager() {
		Echantillon echan1 = echantillonManager.findByIdManager(1);
		Emplacement empl1 = 
				echantillonManager.getEmplacementManager(echan1);
		assertNull(empl1);

		Echantillon echan2 = echantillonManager.findByIdManager(2);
		Emplacement empl2 = 
				echantillonManager.getEmplacementManager(echan2);
		assertNotNull(empl2);
		assertTrue(empl2.getPosition().equals(3));
		assertFalse(empl2.getVide());

		Emplacement emplNull = 
				echantillonManager.getEmplacementManager(null);
		assertNull(emplNull);
	}
	...
	public void testGetEmplacementAdrlManager() {
		Echantillon echan1 = echantillonManager.findByIdManager(1);
		String adrl1 = 
				echantillonManager.getEmplacementAdrlManager(echan1);
		assertNotNull(adrl1);
		assertTrue(adrl1.equals(""));

		Echantillon echan2 = echantillonManager.findByIdManager(2);
		String adrl2 = 
				echantillonManager.getEmplacementAdrlManager(echan2);
		assertNotNull(adrl2);
		assertTrue(adrl2.equals("CC1.R1.T1.BT1.A-C"));

		String adrlNull = 
				echantillonManager.getEmplacementAdrlManager(null);
	...
	public void testFindDoublon() {
		Banque banque1 = banqueManager.findByIdManager(1);
		Banque banque2 = banqueManager.findByIdManager(2);
		Echantillon echan = new Echantillon();
		...
		Echantillon echan2 = echantillonManager.findByIdManager(1);
	...
	public void testFindAfterDateCreationReturnIdsManager() 
		...
		cal.setTime(search);
		liste = echantillonManager.findAfterDateCreationReturnIdsManager(
				cal, banks);
	...
	public void testFindByCodeInListManager() {
		...
		notfounds.clear();
		criteres.add("PTRA.1");
		liste = echantillonManager.findByCodeInListManager(
				criteres, bks, notfounds);
		assertTrue(liste.size() == 1);
		...
		assertTrue(liste.size() == 0);
		liste = echantillonManager.findByCodeInListManager(
				criteres, null, notfounds);
		assertTrue(liste.size() == 0);
		liste = echantillonManager.findByCodeInListManager(
				new ArrayList<String>(), bks, notfounds);
		assertTrue(liste.size() == 0);
		liste = echantillonManager.findByCodeInListManager(
				criteres, new ArrayList<Banque>(), notfounds);
		assertTrue(liste.size() == 0);
		liste = echantillonManager.findByCodeInListManager(
				criteres, bks, null);
		assertTrue(liste.size() == 0);
	...
	public void testFindByPatientNomOrNipInListManager() {
		...
		List<Integer> liste = echantillonManager
				.findByPatientNomOrNipInListManager(
						criteres, bks);
		...
		criteres.add("DELPHINO");
		criteres.add("12");
		liste = echantillonManager.findByPatientNomOrNipInListManager(
				criteres, bks);
		assertTrue(liste.size() == 4);
	...
	public void testCrud() {
		...
		Prelevement prelevement = prelevementDao.findById(1);
		...
		EchantillonType type = echantillonTypeManager.findByIdManager(1);
		Unite quantite = uniteManager.findByIdManager(1);
		EchanQualite qualite = echanQualiteManager.findByIdManager(1);
		ModePrepa prepa = modePrepaManager.findByIdManager(1);
		...
		try {
			echantillonManager.createObjectManager(echan1, null, 
		...
		try {
			echantillonManager.createObjectManager(echan1, banque, 
					null, null, statut, null, type, null, null, null, 
					null, null, null, null, null, true, null,
					false);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
		...
		Emplacement empl = emplacementManager.findByIdManager(2);
		catched = false;
		try {
			echantillonManager.createObjectManager(echan1, banque, 
					null, null, statut, empl, type, null, null, null, 
					null, null, null, null, null, true, null,
					false);
		} catch (TKException ex) {
			catched = true; 
		...
		echantillonManager.createObjectManager(echan1, banque, 
				null, null, statut, null, type, null, null, null, 
				null, null, null, null, utilisateur,
				false, null,
				false);
		assertTrue(echantillonManager
				.findByCodeLikeManager("PTRA.3", true).size() == 1);
		assertNull(echan1.getSterile());
		assertTrue(getOperationManager()
				.findByObjectManager(echan1).size() == 1);
		Echantillon testInsert = echantillonManager
				.findByCodeLikeManager("PTRA.3", true).get(0);
		...
		Echantillon echan2 = new Echantillon();
		float num1 = (float) 15.1111;
		float num2 = (float) 15.2555;
		...
		echantillonManager.createObjectManager(echan2, banque, 
				prelevement, collab, statut, emp, type, 
				null, quantite, qualite, prepa, reserv,
				null, null, utilisateur, true, null,
				false);
		assertTrue(echantillonManager
				.findByCodeLikeManager("PTRA.4", true).size() == 1);
		assertTrue(getOperationManager()
				.findByObjectManager(echan2).size() == 1);

		// On vérifie que toutes associations ont étées enregistrées
		testInsert = echantillonManager
				.findByCodeLikeManager("PTRA.4", true).get(0);
		...
		// Suppression
		Echantillon echan3 = echantillonManager
				.findByCodeLikeManager("PTRA.3", true).get(0);
		echantillonManager.removeObjectManager(echan3, null, utilisateur, null);
		Echantillon echan4 = echantillonManager
				.findByCodeLikeManager("PTRA.4", true).get(0);
		echantillonManager.removeObjectManager(echan4, null, utilisateur, null);
		assertTrue(getOperationManager()
				.findByObjectManager(echan1).size() == 0);
		assertTrue(getOperationManager()
				.findByObjectManager(echan2).size() == 0);

		// Update
		String codeUpdated1 = "PTRA.5";
		String codeUpdated2 = "PTRA.2";
		String codeUpdated3 = "PTRA.6";
		Echantillon echan5 = new Echantillon();
		echan5.setCode("PTRA.3");
		echan5.setSterile(null);
		statut = objetStatutManager.findByIdManager(4);
		echantillonManager.createObjectManager(echan5, banque, 
				null, null, statut, null, type, null, null,
		...
		Echantillon echan6 = echantillonManager
				.findByCodeLikeManager("PTRA.3", true).get(0);
		echan6.setCode(codeUpdated1);
		try {
			echantillonManager.updateObjectManager(echan6, null, 
					null, null, null, null, type, null, null, null,
					null, null, null, null, null, null, null, null,
					true, 
					null, null);
		...
		Echantillon echan8 = echantillonManager
				.findByCodeLikeManager("PTRA.3", true).get(0);
		echan8.setCode(codeUpdated1);
		try {
			echantillonManager.updateObjectManager(echan8, banque, 
					null, null, null, null, null, null, null, null,
		...
		// est lancée
		Echantillon echan9 = echantillonManager
				.findByCodeLikeManager("PTRA.3", true).get(0);
		echan9.setCode(codeUpdated2);
		try {
			echantillonManager.updateObjectManager(echan9, banque, 
		...
		Echantillon echan10 = echantillonManager
				.findByCodeLikeManager("PTRA.3", true).get(0);
		// MAJ emplacemet Emplacement occupé par un dérivé
		catched = false;
		try {
			echantillonManager.updateObjectManager(echan10, banque, 
					null, null, null, empl, type, null, null, null, null,
					null, null, null, null, null, null,
					utilisateur, false, null, null);
		} catch (TKException ex) {
			catched = true; 
			assertTrue(ex.getMessage().equals("PTRA.3 : error.emplacement.notEmpty"));
			assertTrue(ex.getIdentificationObjetException().equals("PTRA.3"));
		...
		assertTrue(getOperationManager()
				.findByObjectManager(echan10).size() == 2);
		Echantillon testInsert2 = echantillonManager
				.findByCodeLikeManager("test", true).get(0);
		...
		Echantillon echan12 = echantillonManager
				.findByCodeLikeManager("test", true).get(0);
		...
		List<OperationType> opTypes = new ArrayList<OperationType>();
		opTypes.add(operationTypeManager.findByIdManager(1));
		opTypes.add(operationTypeManager.findByIdManager(2));
		echantillonManager.updateObjectManager(echan12, banque, 
				prelevement, collab, objetStatutManager.findByIdManager(1), 
				emp, type, 
				null, null, quantite, qualite, prepa, 
				reserv, null, null, null, null, utilisateur, true,
				opTypes, null);
		Echantillon echan13 = echantillonManager
				.findByCodeLikeManager("PTRA.6", true).get(0);
		assertTrue(echan13.getCode().equals(codeUpdated3));
		assertTrue(getOperationManager()
				.findByObjectManager(echan12).size() == 5);

		// On vérifie que toutes associations ont étées enregistrées
		testInsert2 = echantillonManager
				.findByCodeLikeManager("PTRA.6", true).get(0);
		...
		Echantillon echan14 = echantillonManager
				.findByCodeLikeManager("PTRA.6", true).get(0);
		...
		assertTrue(getOperationManager()
				.findByObjectManager(echan14).size() == 0);
	 ...
	 */
	private void validationInsert(Echantillon echantillon) {
		Banque banque = banqueManager.findByIdManager(1);
		EchantillonType type = echantillonTypeManager.findByIdManager(1);
		Utilisateur utilisateur = utilisateurDao.findById(1);
		ObjetStatut st = objetStatutManager.findByIdManager(4);
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				...
				echantillonManager.createObjectManager(echantillon, banque, 
						null, null, st, null, type, null, null,
						null, null, null, null, null,
						utilisateur, true, null,
						false);
			} catch (Exception e) {
	 ...
	 */
	private void validationUpdate(Echantillon echantillon) {
		Banque banque = banqueManager.findByIdManager(1);
		EchantillonType type = echantillonTypeManager.findByIdManager(1);
		Utilisateur utilisateur = utilisateurDao.findById(1);
		...
		for (int i = 0; i < emptyValues.length; i++) {
			...
			try {
				echantillon.setCode(emptyValues[i]);
				echantillonManager.updateObjectManager(echantillon, banque, 
						null, null, null, null, type, null, null, 
						null, null, null, null, null, null, null, null,
						utilisateur, true, 
						null, null);
	...
	public void testDateStockageCoherence() throws ParseException {
		...
		Set<LaboInter> set = new HashSet<LaboInter>();
		LaboInter l1 = new LaboInter(); l1.setOrdre(1); set.add(l1);
		LaboInter l3 = new LaboInter(); l3.setOrdre(3); set.add(l3);
		LaboInter l2 = new LaboInter(); l2.setOrdre(2); set.add(l2);
		p.setLaboInters(set);
		...
		Calendar dateStock = Calendar.getInstance();
		dateStock.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
		.parse("01/01/1970 12:00:13"));
		...
		assertEquals("date.validation.infDateDepartPrelevement", 
				errs.getFieldError().getCode());
		errs = echantillonValidator.checkDateStockageCoherence(e, true);
		...
		errs = echantillonValidator.checkDateStockageCoherence(e, false);
		assertEquals("date.validation.infLastDateLaboInter", 
				errs.getFieldError().getCode());
		errs = echantillonValidator.checkDateStockageCoherence(e, true);
		assertTrue(errs.getAllErrors().size() == 0);
		...
		errs = echantillonValidator.checkDateStockageCoherence(e, false);
		assertEquals("date.validation.infDateArriveePrelevement", 
				errs.getFieldError().getCode());
		errs = echantillonValidator.checkDateStockageCoherence(e, true);
		assertTrue(errs.getAllErrors().size() == 0);
		...
		dateStock.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
		.parse("01/01/2209 12:57:01"));
		e.setDateStock(dateStock);
		errs = echantillonValidator.checkDateStockageCoherence(e, true);
		assertEquals("date.validation.supDateActuelle", errs.getFieldError()
				.getCode());
		...
		e.setPrelevement(null);
		dateStock.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
		.parse("17/03/2009 12:57:01"));
		e.setDateStock(dateStock);

		errs = echantillonValidator.checkDateStockageCoherence(e, true);
		assertEquals("date.validation.supDateTransfoEnfant",
				errs.getFieldError().getCode());
	...
	public void testSteriliteCoherence() {	
		...
		p.setSterile(null);
		errs = echantillonValidator.checkSteriliteAntecedence(e);
		assertEquals("sterile.validation.prelevementNotSterile", 
				errs.getFieldError().getCode());
		p.setSterile(false);
		errs = echantillonValidator.checkSteriliteAntecedence(e);
		assertEquals("sterile.validation.prelevementNotSterile", 
		...
		p.setLaboInters(set);		
		errs = echantillonValidator.checkSteriliteAntecedence(e);
		assertEquals("sterile.validation.previousNotSterile", 
				errs.getFieldError().getCode());
		...
		e.setSterile(true);
		errs = echantillonValidator.checkSteriliteAntecedence(e);
		assertEquals("sterile.validation.retourNotSterile", 
				errs.getFieldError().getCode());
	...
	public void testCreateAndUpdateCodesAssignes() {
		...
		codes.add(cOrg1); codes.add(cOrg2);
		...
		try {
			echantillonManager.createObjectManager(echan1, banque, 
					null, null, null, null, type, codes, null, 
					null, null, null, null, null,
					utilisateur, true, null,
					false);
		} catch (RuntimeException e) {
			// e.printStackTrace();
			assertTrue(e.getMessage().equals("echantillon"
		...
		cOrg2.setCode("codePerso");
		echantillonManager.createObjectManager(echan1, banque, 
				null, null, null, null, type, codes, null, null,
				null, null, null, null, utilisateur, true, null, 
				false);
		assertTrue(getOperationManager()
				.findByObjectManager(echan1).size() == 1);
		...
		codeAssigneManager
		.findCodesOrganeByEchantillonManager(echan1).get(0).getCode()
		.equals("BL");
		codeAssigneManager
		.findCodesOrganeByEchantillonManager(echan1).get(1).getCode()
		.equals("codePerso");
		Echantillon testInsert = echantillonManager
				.findByCodeLikeManager("CodeWithAssigne", true).get(0);

		codes.clear();
		CodeAssigne cLes1 = new CodeAssigne();
		cLes1.setCode("K14.5");
		...
		List<CodeAssigne> codesToD = new ArrayList<CodeAssigne>();
		codesToD.add(codeAssigneManager
				.findCodesOrganeByEchantillonManager(echan1).get(0));

		echantillonManager.updateObjectManager(testInsert, banque, 
				null, null, null, null, type, codes, codesToD, null, null,
				null, null, null, null, null, null,
				utilisateur, true, 
				null, null);

		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(echan1).size() == 1);
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(echan1).size() == 1);
		...
		echantillonManager.createObjectManager(echan2, banque, 
				null, null, objetStatutManager.findByIdManager(4), 
				null, type, null, null, null,
				null, null, null, null, utilisateur, 
				true, null,
				false);

		List<CodeAssigne> codes2 = new ArrayList<CodeAssigne>();
		CodeAssigne cOrg3 = new CodeAssigne();
		cOrg3.setCode("BL2");
		...
		cOrg3.setOrdre(1);
		codes2.add(cOrg3); 
		CodeAssigne cLes3 = new CodeAssigne();
		cLes3.setCode("K14.5");
		...
		codes2.add(cLes3);
		try {
			echantillonManager.updateObjectManager(echan2, banque, 
					null, null, null, null, type, codes2, null, 
					null, null, null, null, null, null, null, null,
					utilisateur, true, 
					null, null);
		} catch (RuntimeException e) {
		...
		echantillonManager.updateObjectManager(echan2, banque, 
				...
				null, null);
		assertTrue(getOperationManager()
				.findByObjectManager(echan2).size() == 2);
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(echan2).size() == 1);
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(echan2).size() == 1);
		codeAssigneManager
		.findCodesOrganeByEchantillonManager(echan2).get(0).getCode()
		.equals("BL2");
		//		Echantillon testInsert2 = echantillonManager
		...
		codes = new ArrayList<CodeAssigne>();
		cOrg1.setExport(false);
		cOrg2 = codeAssigneManager
				.findByCodeLikeManager("codePerso", true).get(0);
		cOrg2.setCode("newCode");
		cOrg2.setOrdre(3);
		CodeAssigne cOrg4 = new CodeAssigne();
		cOrg4.setCode("encoreUnCode");
		cOrg4.setLibelle("languedechat");
		cOrg4.setIsOrgane(true);
		cOrg4.setOrdre(2);
		cOrg4.setExport(true);
		codes.add(cOrg1); codes.add(cOrg2); codes.add(cOrg4);
		codes.addAll(codeAssigneManager
				.findCodesMorphoByEchantillonManager(testInsert));
		CodeAssigne cLes2 = new CodeAssigne();
		cLes2.setCode("12.12");
		...
		assertTrue(getOperationManager()
				.findByObjectManager(testInsert).size() == 3);
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(testInsert).size() == 2);
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(testInsert)
				.get(0).getCode().equals("K14.5"));
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(testInsert)
				.get(1).getCode().equals("12.12"));
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(testInsert).size() == 3);
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(testInsert)
				.get(0).getCode().equals("BL"));
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(testInsert)
				.get(1).getCode().equals("encoreUnCode"));
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(testInsert)
				.get(2).getCode().equals("newCode"));

		Echantillon testUpdate = echantillonManager
				.findByCodeLikeManager("CodeWithAssigne", true).get(0);
		...
		List<CodeAssigne> les  = new ArrayList<CodeAssigne>();
		les.addAll(codeAssigneManager
				.findCodesMorphoByEchantillonManager(testUpdate));
		for (int j = 0; j < les.size(); j++) {		
			codeAssigneManager.removeObjectManager(les.get(j));
		}
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(testUpdate).isEmpty());
	...
	public void testCreateUpdateEchantillonWithAnnots() {
		...
		ChampAnnotation c = champAnnotationDao.findById(7);

		AnnotationValeur num = new AnnotationValeur();
		...
		boolean catched = false;
		try {
			echantillonManager.createObjectManager(echan, b1, 
					null, null, null, null, type, null, null, 
					null, null,  null, listAnnots, null,
		...
		echantillonManager.createObjectManager(echan, b1, 
				null, null, null, null, type, null, null, 
				null, null, null, listAnnots, null,
				utilisateur, false, "/tmp/",
				false);

		assertTrue(echantillonManager
				.findByCodeLikeManager("EchanAnnotated", true).size() == 1);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, echan).get(0)
				.getAlphanum().equals("123456"));
		assertTrue(getOperationManager()
				.findByObjectManager(echan).size() == 2);
		assertTrue(getOperationManager()
				.findByObjectManager(echan).get(1).getOperationType().getNom()
				.equals("Annotation"));
		...
		listDelete.add(annotationValeurManager
				.findByChampAndObjetManager(c, echan).get(0));
		listAnnots.clear();
		AnnotationValeur num2 = new AnnotationValeur();
		num2.setChampAnnotation(c);
		num2.setAlphanum("789");
		num2.setBanque(b1);
		listAnnots.add(num2);
		AnnotationValeur bool1 = new AnnotationValeur();
		ChampAnnotation c2 = champAnnotationDao.findById(4);
		...
		echantillonManager.updateObjectManager(echan, b1, 
				null, null, null, null, type, null, null, 
				null, null, null, null, listAnnots, listDelete,
				null, null, utilisateur, false, null, "/tmp/");

		assertTrue(echantillonManager
				.findByCodeLikeManager("EchanAnnotated", true).size() == 1);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, echan).size() == 1);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, echan).get(0)
				.getAlphanum().equals("789"));
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c2, echan).get(0).getBool());
		assertTrue(getOperationManager()
				.findByObjectManager(echan).size() == 4);
		assertTrue(getOperationManager()
				.findByObjectManager(echan).get(3).getOperationType().getNom()
				.equals("Annotation"));
		...
		echantillonManager.updateObjectManager(echan, b1, 
				null, null, null, null, type, null, null, 
				null, null, null, null, listAnnots, listDelete,
				null, null, utilisateur, false, 
				null, "/tmp/");

		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, echan).size() == 0);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c2, echan).size() == 1);
		assertTrue(getOperationManager()
				.findByObjectManager(echan).size() == 6);
		assertTrue(getOperationManager()
				.findByObjectManager(echan).get(5).getOperationType().getNom()
				.equals("Annotation"));
		...
		assertTrue(getOperationManager()
				.findByObjectManager(echan).size() == 0);
		assertTrue(annotationValeurManager
	...
	public void testCreateUpdateEchantillonCrAnapath() {
		...
		Integer totFichiers = fichierManager.findAllObjectsManager().size();

		// creation architecture fichiers
		new File("/tmp/" + "pt_" 
				+ b1.getPlateforme().getPlateformeId()
				+ "/" + "coll_" + b1.getBanqueId() 
				+ "/cr_anapath/").mkdirs();

		Fichier cr = new Fichier();
		cr.setNom("anapath1");	
		...
		echan.setCode("*ù$&àé");
		try {
			echantillonManager.createObjectWithCrAnapathManager(echan, b1, 
					null, null, null, null, type, null, null, 
					null, null, cr, bais, filesCreated, null, null,
					utilisateur, true, "/tmp/",
					false);
		} catch (Exception e) {
			testFindAll();
		...
		echantillonManager.createObjectWithCrAnapathManager(echan, b1, 
				null, null, null, null, type, null, null, 
				null, null, cr, bais, null, null, null, 
				utilisateur, false, "/tmp/",
				false);

		assertTrue(echantillonManager
				.findByCodeLikeManager("EchanAnapath", true).size() == 1);
		...
		Fichier cr2 = cr.clone();
		cr2.setNom("cr2");
		echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
				null, null, null, null, type, null, 
				null, null, null, null, cr2, null, filesCreated, filesToDelete, 
				null, null, null,  
				utilisateur, false, 
				null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers + 1);
		assertTrue(fichierManager
				.findByIdManager(cr2.getFichierId()).getNom().equals("cr2"));
		assertTrue(new File(cr2.getPath()).length() == 37);

		// update exception
		Fichier cr2ex = cr2.clone();
		cr2ex.setNom("*ù$ù##");
		...
		try {
			echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
					null, null, null, null, type, null, 
					null, null, null, null, 
					cr2ex, null, filesCreated, filesToDelete,
					null, null, null, utilisateur, false, 
					null, "/tmp/");
		} catch (Exception e) {
			assertNull(echantillonManager
					.findByCodeLikeManager("EchanAnapath", true).get(0)
					.getLateralite());
			assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers + 1);
			assertTrue(echantillonManager
					.findByCodeLikeManager("EchanAnapath", true)
					.get(0).getCrAnapath().getNom().equals("cr2"));
			assertTrue(new File(cr2.getPath()).length() == 37);
		}
		echan.setLateralite(null);

		// modification du contenu
		Fichier cr4 = cr2.clone();
		...
		echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
				null, null, null, null, type, null, null, null, 
				null, null, cr4, bais, filesCreated, filesToDelete, 
				null, null, null, 
				utilisateur, false, null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers + 1);
		...
		echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
				null, null, null, null, type, null, 
				null, null, null, null, null, null, filesCreated, filesToDelete,
				null, null,  null,
				utilisateur, false, null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers);
		assertTrue(new File("/tmp/pt_1/coll_1/cr_anapath").listFiles().length == 0);
		...
		try {
			echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
					null, null, null, null, type, null, 
					null, null, null, null, 
					cr3, bais, filesCreated, filesToDelete, 
					null, null, null, 
					utilisateur, true, null, "/tmp/");
		} catch (Exception e) {
			assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers);
			assertTrue(new File("/tmp/pt_1/coll_1/cr_anapath").listFiles().length == 0);
		}	
		...
		echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
				null, null, null, null, type, null, null, null, 
				null, null, cr3, bais, 
				filesCreated, filesToDelete, null, null, null,
				utilisateur, true, null, "/tmp/");
		assertTrue(echantillonManager
				.findByCodeLikeManager("ok", true).size() == 1);
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers + 1);
		...
		echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
				null, null, null, null, type, null, null, null, 
				null, null, cr3, null, filesCreated, filesToDelete, 
				null, null, null,
				utilisateur, false, null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers + 1);
		assertTrue(new File(cr3.getPath()).length() == 29);	
		...
		try {
			echantillonManager.updateObjectWithCrAnapathManager(echan, b1, 
					null, null, null, null, type, null, null, null,
					null, null, cr3, bais, filesCreated, filesToDelete,
					null, null, null, 
					utilisateur, true, null, "/tmp/");
		} catch (Exception e) {
			assertTrue(fichierManager.findAllObjectsManager().size() == totFichiers + 1);
			assertTrue(new File("/tmp/pt_1/coll_1/cr_anapath").listFiles().length == 1);
			assertTrue(cr3.getPath().equals("/tmp/pt_1/coll_1/cr_anapath/fin_" 
															+ cr3.getFichierId()));
			assertTrue(new File(cr3.getPath()).length() == 29);	
		}
		
		Integer c3Id = cr3.getFichierId();
		...
		assertTrue(echan.getCrAnapath().getPath()
				.equals("/tmp/pt_1/coll_1/cr_anapath/fin_" 
				+ echan.getCrAnapath().getFichierId()));
	...
	public void testCreateUpdateCrAnapathForEchantillons() {
		...
		Integer totFichier = fichierManager.findAllObjectsManager().size();

		Echantillon e1 = new Echantillon(); e1.setCode("E1");
		Echantillon e2 = new Echantillon(); e2.setCode("E2");
		Echantillon e3 = new Echantillon(); e3.setCode("E3");
		Echantillon e4 = new Echantillon(); e4.setCode("E4");
		Echantillon e5 = new Echantillon(); e5.setCode("E5");
		...
		new File("/tmp/" + "pt_" 
					...
					+ "/cr_anapath/").mkdirs();
		// test
		String path = echantillonManager
				.writeCrAnapathFilePath("/tmp/", b1, null);
		assertNotNull(path);
		...
		echantillonManager.createObjectWithCrAnapathManager(e1, b1, 
				null, null, statut, null, type, null, null, 
				null, null, cr, bais, filesCreated, null, null, utilisateur, 
				false, "/tmp/",
				false);
		
		Fichier cr2 = cr.cloneNoId();
		echantillonManager.createObjectWithCrAnapathManager(e2, b1, 
				null, null, statut, null, type, null, null, 
				null, null, cr2, null, filesCreated, null, null, utilisateur, 
				false, "/tmp/",
				false);
		Fichier cr3 = cr.cloneNoId();
		echantillonManager.createObjectWithCrAnapathManager(e3, b1, 
				null, null, statut, null, type, null, null, 
				null, null, cr3, null, filesCreated, null, null, utilisateur, 
				false, "/tmp/",
				false);
		Fichier cr4 = cr.cloneNoId();
		echantillonManager.createObjectWithCrAnapathManager(e4, b1, 
				null, null, statut, null, type, null, null, 
				null, null, cr4, null, filesCreated, null, null, utilisateur, 
				false, "/tmp/",
				false);
		Fichier cr5 = cr.cloneNoId();
		echantillonManager.createObjectWithCrAnapathManager(e5, b1, 
				null, null, statut, null, type, null, null, 
				null, null, cr5, null, filesCreated, null, null, utilisateur, 
				false, "/tmp/",
				false);
		...
		cr.setNom("cr2");
		try {
			echantillonManager.updateObjectWithCrAnapathManager(e1, b1, 
					null, null, null, null, type, null, null, null, 
					null, null, cr, null, filesCreated, filesToDelete,
					null, null, null, 
					utilisateur, true, null, "/tmp/");
		} catch (Exception e) {
			assertEquals(totFichier + 5, fichierManager.findAllObjectsManager().size());
			assertTrue(echantillonManager
					.findByCodeLikeManager("E1", true).get(0)
					.getCrAnapath().getFichierId().equals(cr.getFichierId()));
			assertFalse(echantillonManager
					.findByCodeLikeManager("E1", true).get(0)
					.getCrAnapath().getNom().equals("cr2"));
		...
		echantillonManager.updateObjectWithCrAnapathManager(e1, b1, 
				...
				utilisateur, false, null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 5);
		Fichier f = echantillonManager
				.findByCodeLikeManager("ok", true).get(0).getCrAnapath();
		...
		e3.setCode("&*ù*ù$");
		try {
			echantillonManager.updateObjectWithCrAnapathManager(e3, b1, 
					null, null, null, null, type, null, null, null, 
					null, null, cr3, bais, filesCreated, filesToDelete, 
					null, null, null, 
					utilisateur, true, null, "/tmp/");
			...
			assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 5);
			assertTrue(echantillonManager
					.findByCodeLikeManager("E3", true).get(0)
					.getCrAnapath().getFichierId().equals(cr3.getFichierId()));
			assertTrue(echantillonManager
					.findByCodeLikeManager("E3", true).get(0)
					.getCrAnapath().getNom().equals(cr2.getNom()));
			assertTrue(new File(echantillonManager
					.findByCodeLikeManager("E3", true).get(0)
					.getCrAnapath().getPath()).length() == 26);
			assertTrue(new File("/tmp/pt_1/coll_1/cr_anapath").listFiles().length == 1);
		}
		assertTrue(filesCreated.size() == 0);
		assertTrue(filesToDelete.size() == 0);
		...
		echantillonManager.updateObjectWithCrAnapathManager(e3, b1, 
				...
				utilisateur, false, null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 5);
		f = echantillonManager
				.findByCodeLikeManager("ok2", true).get(0).getCrAnapath();
		...
		echantillonManager.updateObjectWithCrAnapathManager(e3, b1, 
				null, null, null, null, type, null, null, null, 
				null, null, e3.getCrAnapath(), null, 
				filesCreated, filesToDelete, null,
				null, null, utilisateur, false, null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 5);
		f = echantillonManager
				.findByCodeLikeManager("ok2", true).get(0).getCrAnapath();
		...
		// nullifying pour e4
		echantillonManager.updateObjectWithCrAnapathManager(e4, b1, 
				null, null, null, null, type, null, null, null, 
				null, null, null, null,  filesCreated, filesToDelete,
				null, null, null, utilisateur, false, null, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 4);
		assertNull(echantillonManager
				.findByCodeLikeManager("e4", true).get(0).getCrAnapath());
		...
		testFindAll();
		assertTrue(new File("/tmp/pt_1/coll_1/cr_anapath")
		.listFiles().length == 0);
	...
	public void testValidationCoherenceDate() throws ParseException {
		...
		Calendar val2 = Calendar.getInstance();
		val2.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
		.parse("01/01/1973 10:12:12"));
	...
	public void testRemoveUsedAndCessedObject() {
		...
		Echantillon e3 = echantillonManager.findByIdManager(3);
		try {
			echantillonManager.removeObjectManager(e3, null, u, null);
		} catch (ObjectUsedException oe) {
			assertTrue(oe.getKey()
					.equals("echantillon.deletion.isUsedNonCascade"));
	...
	public void testCascadeEchantillonChildren() {
		...
		EchantillonType eType = echantillonTypeManager.findByIdManager(1);
		Utilisateur utilisateur = utilisateurDao.findById(1);
		e.setObjetStatut(objetStatutManager.findByIdManager(4));

		echantillonManager.createObjectManager(e, b, null, null, 
				null, null, eType, null, null, null, null, null, null,
				null, utilisateur, false, null,
				false);
		...
		//insertion valide
		retourManager.createOrUpdateObjectManager(r, e, null, null, null, 
				null, null, u, "creation");

		assertTrue(echantillonManager
				.findByCodeLikeManager("EchanWithDerives", true).size() == 1);
		assertTrue(retourManager.getRetoursForObjectManager(e).size() == 1);
		assertTrue(retourManager.findAllObjectsManager().size() == 9);

		Transformation transfo1 = new Transformation();
		...
		ProdDerive derive1 = new ProdDerive();
		derive1.setCode("D1"); derive1.setProdType(type);
		derives.add(derive1);
		ProdDerive derive2 = new ProdDerive();
		derive2.setCode("D2"); derive2.setProdType(type);
		derives.add(derive2);
		ProdDerive derive3 = new ProdDerive();
		derive3.setCode("D3"); derive3.setProdType(type);
		derives.add(derive3);

		prodDeriveManager.createDeriveListWithAnnotsManager(derives, 
				b, transfo1, utilisateur, null, null, null, null);

		assertTrue(transformationManager
				.findAllDeriveFromParentManager(e).size() == 3);
		...
		cederObjetManager.createObjectManager(ced2, c, 
				entiteDao.findByNom("ProdDerive").get(0), qteUnite);

		boolean catched = false;
		try {
			echantillonManager
			.removeObjectCascadeManager(e, null, u, null);
		} catch (ObjectUsedException oe) {
	...
	public void testFindByPrelevementManager() {
		Prelevement p = prelevementDao.findById(1);
		List<Echantillon> echans = echantillonManager
				.findByPrelevementManager(p);
		assertTrue(echans.size() == 2);
	...
	public void testSwitchBanque() {
		...
		e.setCode("JEG.2");
		echantillonManager.createObjectManager(e, b3, 
				null, null, null, null, echantillonTypeManager.findByIdManager(1), 
				null, null, null, null, null, null,
				null, u, false, null,
				false);
		...
		Echantillon ebis = new Echantillon(); ebis.setCode("JEG.2");	
		ebis.setObjetStatut(objetStatutManager.findByIdManager(4));
		echantillonManager.createObjectManager(ebis, b4, 
				null, null, null, null, echantillonTypeManager.findByIdManager(4), 
				null, null, null, null, null, null,
				null, u, false, null,
				false);
		...
		} catch (DoublonFoundException dbe) {
			catched = true;
		}
		assertTrue(catched);
		assertTrue(banqueManager.getEchantillonsManager(b3).size() == 1);
		...
		echantillonManager.updateObjectManager(e, e.getBanque(), e.getPrelevement(), 
				null, e.getObjetStatut(), null, e.getEchantillonType(), null, null, null, 
				null, null, null, null, null, null, null, u, false, null, null);
		...
		Transformation transfo1 = new Transformation();
		transfo1.setObjetId(e.getEchantillonId());
		transformationManager.createObjectManager(transfo1, 
				entiteDao.findById(3), null);	
		ProdDerive derive = new ProdDerive();
		derive.setCode("deriveSwitch");
		prodDeriveManager.createObjectManager(derive, b2, 
				prodTypeDao.findById(1), objetStatutManager.findByIdManager(4), 
				null, null, null, null, null, 
				null, null, transfo1, null, null, null, u, false, null,
				false);
		...
		try {
			...
			assertTrue(re.getMessage()
					.equals("echantillon.switchBanque.isCessed"));
		}
		assertTrue(catched);
		assertTrue(banqueManager.getEchantillonsManager(b3).size() == 1);
		assertTrue(banqueManager.getEchantillonsManager(b2).size() == 1);
		...
		cederObjetManager.createObjectManager(
				ced2, cession, entiteDao.findById(8), null);
		catched = false;
		try {
			echantillonManager.switchBanqueCascadeManager(e, b4, true, u, null);
		} catch (RuntimeException re) {
			catched = true;
			assertTrue(re.getMessage()
					.equals("derive.switchBanque.isCessed"));
		}
		assertTrue(banqueManager.getEchantillonsManager(b3).size() == 1);
		assertTrue(banqueManager.getEchantillonsManager(b2).size() == 1);
		assertTrue(getOperationManager().findByObjectManager(e).size() == 4);
		...
		// emplacement erreur
		Conteneur cbase = conteneurManager.findByIdManager(1);
		Terminale t = terminaleDao.findById(1);
		List<Banque> banques = new ArrayList<Banque>();
		banques.add(b3); banques.add(b1);		
		...
		Enceinte e1 = new Enceinte(); e1.setEnceinteType(eType1);
		e1.setNom("R"); e1.setNbPlaces(1); enceintes.add(e1);
		Enceinte e2 = new Enceinte(); e2.setEnceinteType(eType2);
		e2.setNom("T"); e2.setNbPlaces(1); enceintes.add(e2);
		Enceinte e3 = new Enceinte(); e3.setEnceinteType(eType3);
		e3.setNom("C"); e3.setNbPlaces(1); enceintes.add(e3);
		...
		goodPositions.add(10); goodPositions.add(null);
		goodPositions.add(25); goodPositions.add(null);
		conteneurManager.createAllArborescenceManager(
				c, enceintes, term, goodPositions,
				banques, null, null, false, u, b1.getPlateforme());
		Emplacement emp = new Emplacement();
		emp.setPosition(55);
		emp.setVide(false);
		emp.setObjetId(derive.getProdDeriveId());
		Terminale t1 = enceinteManager.getTerminalesManager(enceinteManager
				.getEnceintesManager(enceinteManager
						.getEnceintesManager(conteneurManager
								.getEnceintesManager(c).iterator().next())
								.iterator().next()).iterator().next())
								.iterator().next();
		emplacementManager.createObjectManager(emp, t1, entiteDao.findById(8));
		prodDeriveManager.updateObjectManager(derive, derive.getBanque(), 
				derive.getProdType(), null, null, emp, null, null, null, null, 
				null, derive.getTransformation(), null, null, null, null, null, u, false, null, 
				null);
		...
		try {
			...
			catched = true;
			assertTrue(re.getMessage()
					.equals("derive.switchBanque.badBanqueStockage"));
		}
		assertTrue(catched);
		assertTrue(banqueManager.getEchantillonsManager(b3).size() == 1);
		assertTrue(banqueManager.getEchantillonsManager(b2).size() == 1);
		...
		prodDeriveManager.updateObjectManager(derive, derive.getBanque(), 
				derive.getProdType(), null, null, null, null, null, null, null, 
				null, transfo1, null, null, null, null, null, u, false, null, null);
		echantillonManager.updateObjectManager(e, e.getBanque(), null, null, 
				null, emp, e.getEchantillonType(), null, null, null,  
				null, null, null, null, null, null, null, u, false, null, null);
		// e = echantillonManager.findByIdManager(e.getEchantillonId());
		...
		try {
			...
			catched = true;
			assertTrue(re.getMessage()
					.equals("echantillon.switchBanque.badBanqueStockage"));
		}
		assertTrue(catched);
		assertTrue(banqueManager.getEchantillonsManager(b3).size() == 1);
		assertTrue(banqueManager.getEchantillonsManager(b2).size() == 1);
		...
		num.setAlphanum("123456"); num.setBanque(b1);
		listAnnots.add(num);
		AnnotationValeur link = new AnnotationValeur();
		link.setChampAnnotation(champAnnotationDao.findById(16));
		link.setAlphanum("http://yahoo.fr"); link.setBanque(b1);
		listAnnots.add(link);
		AnnotationValeur thes = new AnnotationValeur();
		thes.setChampAnnotation(champAnnotationDao.findById(40));
		thes.setItem(itemDao.findById(83)); thes.setBanque(b1);
		listAnnots.add(thes);	
		echantillonManager.updateObjectManager(e, e.getBanque(), 
				null, null, null, emp, e.getEchantillonType(), 
				null, null, null, null, null, null, 
				listAnnots, null, null, null, u, false, null, "/tmp/");
		assertTrue(annotationValeurManager
				.findByObjectManager(e).size() == 3);
		...
		echantillonManager.switchBanqueCascadeManager(e, b3, true, u, null);
		assertTrue(annotationValeurManager
				.findByObjectManager(e).size() == 1);
		assertTrue(annotationValeurManager.findByObjectManager(e)
				.get(0).getChampAnnotation().getChampAnnotationId() == 7);
		...
		conteneurManager.removeObjectManager(conteneurManager
				.findByIdManager(c.getConteneurId()), null, u);		
		List<TKFantomableObject> fs = new ArrayList<TKFantomableObject>();
		fs.add(e);
		fs.add(ebis);
		fs.add(derive);
	...
	public void testUpdateMultipleObjectsManager() throws ParseException {
		...
		Echantillon echan1 = new Echantillon();
		Echantillon echan2 = new Echantillon();
		Echantillon echan3 = new Echantillon();
		echan1.setCode("e1"); echan2.setCode("e2"); echan3.setCode("e3");
		echan1.setSterile(false); echan2.setSterile(false);
		echan3.setSterile(false);
		EchantillonType eT = echantillonTypeManager.findByIdManager(1);
		Collaborateur c = collaborateurManager.findByIdManager(1);
		Calendar stockDate = Calendar.getInstance();
		stockDate.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
		.parse("17/09/2010 14:57:01"));
		echan1.setDateStock(stockDate);
		echan2.setDateStock(stockDate);
		ObjetStatut statut = objetStatutManager.findByIdManager(2);
		EchanQualite eQual = echanQualiteManager.findByIdManager(1);
		ModePrepa prepa = modePrepaManager.findByIdManager(1);
		Utilisateur u = utilisateurDao.findById(1);
		
		int totFichier = fichierManager.findAllObjectsManager().size();
		int totEchans = echantillonManager.findAllObjectsManager().size();
		...
		Fichier cr1 = new Fichier(); cr1.setNom("cr1"); String tmp = "CR1";
		ByteArrayInputStream bais1 = new ByteArrayInputStream(tmp.getBytes());
		Fichier cr2 = new Fichier(); cr2.setNom("cr2"); String tmp2 = "CR24";  
		ByteArrayInputStream bais2 = new ByteArrayInputStream(tmp2.getBytes());
		...
		echans.add(echan1); echans.add(echan2); echans.add(echan3);
		List<AnnotationValeur> listAnnots = new ArrayList<AnnotationValeur>();
		List<AnnotationValeur> listDelete = new ArrayList<AnnotationValeur>();

		AnnotationValeur num1 = new AnnotationValeur();
		...
		echantillonManager.createObjectWithCrAnapathManager(echan1, b1, p1, c, statut, 
				null, eT, codes, null , eQual, prepa, cr1, bais1, filesCreated,
				null,  null, utilisateur, false, "/tmp/",
				false);
		echan2.setDelaiCgl(new Float(15));
		cOrg1.setExport(false);
		cOrg2.setExport(true);
		echantillonManager.createObjectWithCrAnapathManager(echan2, b1, p1, c, statut, 
				null, eT, codes, null , eQual, prepa, cr2, bais2, filesCreated,
				null, null, utilisateur, false, "/tmp/",
				false);
		echantillonManager.createObjectWithCrAnapathManager(echan3, b1, p1, c, statut, 
				null, eT, null, null , eQual, prepa, null, null, filesCreated,
				null, null, utilisateur, false, "/tmp/",
				false);
		...
		for (int i = 0; i < listAnnots.size(); i++) {		
			annotationValeurManager
				.createOrUpdateObjectManager(listAnnots.get(i), 
						listAnnots.get(i).getChampAnnotation(),
					echans.get(i), b1, null, utilisateur, "creation", "/tmp/", 
					filesCreated, filesToDelete);
		}

		int id1 = echan1.getEchantillonId();
		int id2 = echan2.getEchantillonId();
		int id3 = echan3.getEchantillonId();
		...
		Echantillon eUp1 = echantillonManager
				.findByCodeLikeManager("e1", true).get(0);
		Echantillon eUp2 = echantillonManager
				.findByCodeLikeManager("e2", true).get(0);
		Echantillon eUp3 = echantillonManager
				.findByCodeLikeManager("e3", true).get(0);
		...
		assertTrue(objetNonConformeDao.findByObjetAndEntite(id1, 
				entiteDao.findById(3)).isEmpty());
		assertTrue(objetNonConformeDao.findByObjetAndEntite(id2, 
				entiteDao.findById(3)).isEmpty());
		assertTrue(objetNonConformeDao.findByObjetAndEntite(id3, 
				entiteDao.findById(3)).isEmpty());
		...
		List<Echantillon> list = new ArrayList<Echantillon>();
		list.add(eUp1);
		list.add(eUp2);
		list.add(eUp3);
		listAnnots.clear();
		listAnnots.addAll(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), 
						echan1));
		listAnnots.addAll(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), 
						echan2));
		AnnotationValeur num3 = new AnnotationValeur();
		...
		List<CodeAssigne> toDeletes = new ArrayList<CodeAssigne>();
		toDeletes.addAll(codeAssigneManager
				.findCodesOrganeByEchantillonManager(eUp1));
		toDeletes.addAll(codeAssigneManager
				.findCodesMorphoByEchantillonManager(eUp1));
		toDeletes.addAll(codeAssigneManager
				.findCodesOrganeByEchantillonManager(eUp2));
		toDeletes.addAll(codeAssigneManager
				.findCodesMorphoByEchantillonManager(eUp2));

		List<NonConformite> ncfsTrait = new ArrayList<NonConformite>();
		ncfsTrait.add(nonConformiteDao.findById(4));

		List<NonConformite> ncfsCess = new ArrayList<NonConformite>();
		...
		echantillonManager.updateMultipleObjectsManager(list, null, codes, 
				toDeletes, eUp1.getCrAnapath(), null, null,
				listAnnots, listDelete, 
				ncfsTrait, ncfsCess, utilisateur, "/tmp/");
		assertTrue(codeAssigneManager.findAllObjectsManager().size() == 11);
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 3);
		assertTrue(annotationValeurManager
				.findAllObjectsManager().size() == 15);

		Echantillon eTest1 = echantillonManager
				.findByCodeLikeManager("e1", true).get(0);
		Echantillon eTest2 = echantillonManager
				.findByCodeLikeManager("e2", true).get(0);
		Echantillon eTest3 = echantillonManager
				.findByCodeLikeManager("e3", true).get(0);
		assertTrue(eTest1.getLateralite().equals(lat));
		assertTrue(eTest2.getLateralite().equals(lat));
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), eTest1)
				.get(0).getAlphanum().equals("1111"));
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), eTest2)
				.get(0).getAlphanum().equals("2222"));
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), eTest3)
				.get(0).getAlphanum().equals("91011"));

		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(eTest1).size() == 1);
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(eTest1).size() == 1);
		//assertTrue(eTest1.getCodeOrganeExport().getCode().equals("GR"));
		//assertTrue(eTest1.getCodeLesExport().getCode().equals("B12"));
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(eTest2).size() == 1);
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(eTest2).size() == 1);
		//assertTrue(eTest2.getCodeOrganeExport().getCode().equals("GR"));
		//assertTrue(eTest2.getCodeLesExport().getCode().equals("B12"));
		assertTrue(codeAssigneManager
				.findCodesOrganeByEchantillonManager(eTest3).size() == 1);
		assertTrue(codeAssigneManager
				.findCodesMorphoByEchantillonManager(eTest3).size() == 1);
		...
		assertTrue(getOperationManager()
				.findByObjectManager(eTest1).size() == 2);
		assertTrue(getOperationManager().findByObjectManager(eTest1).get(1)
				.getOperationType().getNom().equals("ModifMultiple"));

		assertTrue(objetNonConformeDao.findByObjetAndEntite(id1, 
				entiteDao.findById(3)).size() == 2);
		assertTrue(objetNonConformeDao.findByObjetAndEntite(id2, 
				entiteDao.findById(3)).size() == 2);
		assertTrue(objetNonConformeDao.findByObjetAndEntite(id3, 
				entiteDao.findById(3)).size() == 2);
		...
		list = new ArrayList<Echantillon>();
		list.add(eTest1);
		list.add(eTest2);
		list.add(eTest3);
		...
		boolean catched = false;
		try {
			echantillonManager.updateMultipleObjectsManager(list, list, null, 
					null, null, null, null,
					listAnnots, listDelete, ncfsTrait, ncfsTrait, utilisateur, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
				catched = true;
		...
		toDeletes.clear();
		toDeletes.add(codeAssigneManager
				.findCodesOrganeByEchantillonManager(eTest3).get(0));
		//eTest3.setCodeOrganeExport(null);
		codes.clear();

		catched = false;
		try {
			echantillonManager.updateMultipleObjectsManager(list, list, codes, 
					toDeletes, null, null, null, listAnnots, 
					listDelete, ncfsTrait, ncfsCess, utilisateur, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
				catched = true;
		...
		listAnnots.clear();
		eTest1.setCode("NEW");
		AnnotationValeur bool4 = new AnnotationValeur();
		bool4.setBool(true);
		...
		assertFalse(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(4), 
						eTest1).size() > 0);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), 
						eTest2).get(0).getAlphanum().equals("2222"));

		// modifications valides
		//alpha1.setAnnotationValeurId(null);
		alpha5.setAlphanum("8888");
		echantillonManager.updateMultipleObjectsManager(list, null, null,
				null, null, null, false, listAnnots, 
				listDelete, null, null, utilisateur, "/tmp/");
		...
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(4), 
						eTest1).get(0).getBool());
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), 
						eTest2).get(0).getAlphanum().equals("8888"));
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(champAnnotationDao.findById(7), 
						eTest3).get(0).getAlphanum().equals("91011"));
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 3);
		assertTrue(eTest3.getCrAnapath().getNom().equals("cr1"));
		assertTrue(new File(eTest3.getCrAnapath().getPath()).length() == 3);

		// update cr anapath partagé
		Fichier cr4 = new Fichier(); cr4.setNom("cr4"); 
		String tmp4 = "CR3456789";
		ByteArrayInputStream bais4 = new ByteArrayInputStream(tmp4.getBytes());
		echantillonManager.updateMultipleObjectsManager(list, null, null, 
				null, cr4, bais4, false, null, null, null, null, 
				utilisateur, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 3);
		eTest1 = echantillonManager.findByCodeLikeManager("NEW", true).get(0);
		eTest2 = echantillonManager.findByCodeLikeManager("e2", true).get(0);
		...
		String oldPath = eTest1.getCrAnapath().getPath();
		...
		cr4.setNom("cr5");
		echantillonManager.updateMultipleObjectsManager(list, list, null, 
				null, cr4, null, null, null, null, null, null, utilisateur, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 3);
		eTest1 = echantillonManager.findByCodeLikeManager("NEW", true).get(0);
		eTest2 = echantillonManager.findByCodeLikeManager("e2", true).get(0);
		eTest3 = echantillonManager.findByCodeLikeManager("e3", true).get(0);
		...
		list.remove(eTest3);
		echantillonManager.updateMultipleObjectsManager(list, list, null, null, 
				null, null, true, null, null, null, null, utilisateur, "/tmp/");
		...
		Fichier cr6 = new Fichier(); cr6.setNom("cr6"); String tmp6 = "666";
		ByteArrayInputStream bais6 = new ByteArrayInputStream(tmp6.getBytes());
		eTest1 = echantillonManager.findByCodeLikeManager("NEW", true).get(0);
		eTest2 = echantillonManager.findByCodeLikeManager("e2", true).get(0);
		eTest3 = echantillonManager.findByCodeLikeManager("e3", true).get(0);
		list.clear(); list.add(eTest1); list.add(eTest2); list.add(eTest3);
		echantillonManager.updateMultipleObjectsManager(list, null, null, null,
				cr6, bais6, false, null, null, null, null, 
				utilisateur, "/tmp/");
		assertTrue(fichierManager.findAllObjectsManager().size() == totFichier + 3);
		eTest1 = echantillonManager.findByCodeLikeManager("NEW", true).get(0);
		eTest2 = echantillonManager.findByCodeLikeManager("e2", true).get(0);
		...
		for (int i = 0; i < list.size(); i++) {
			...
			echantillonManager
				.removeObjectCascadeManager(list.get(i), null, u, null);
		}
		// verification de l'etat de la base
		testFindAll();
		assertTrue(annotationValeurManager
	...
	public void testExtractValueForChampManager() {
		Echantillon e1 = echantillonManager.findByIdManager(1);
		Champ c54 = new Champ(champEntiteDao.findById(54));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c54).equals("PTRA.1"));

		Champ c3 = new Champ(champEntiteDao.findById(3));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c3).equals("DELPHINO"));

		Champ c23 = new Champ(champEntiteDao.findById(23));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c23).equals("PRLVT1"));

		Champ c18 = new Champ(champEntiteDao.findById(18));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c18).equals("C45.3"));

		// test sur le risque
		Champ c249 = new Champ(champEntiteDao.findById(249));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c249).equals("LEUCEMIE, HIV"));

		// test sur l'emplacement
		Echantillon e2 = echantillonManager.findByIdManager(2);
		Champ c57 = new Champ(champEntiteDao.findById(57));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e2, c57).equals("CC1.R1.T1.BT1.A-C"));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c57).equals(""));

		// test sur les codes organes
		Champ c229 = new Champ(champEntiteDao.findById(229));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c229).equals("BL, face dorsale de la langue"));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e2, c229).equals(""));

		// test sur les codes morphos
		Champ c230 = new Champ(champEntiteDao.findById(230));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e1, c230).equals("langue plicaturée, BL0211-2, D5-22050"));
		assertTrue(extractValueFromChampManager.extractValueForChampManager(
				e2, c230).equals(""));
		Echantillon nully = null;
		assertNull(extractValueFromChampManager.extractValueForChampManager(
				nully, c18));
		assertNull(extractValueFromChampManager.extractValueForChampManager(
				e1, null));
		assertNull(extractValueFromChampManager.extractValueForChampManager(
	...
	public void testUpdateCodeEchantillonsManager() {
		...
		echantillonManager.createObjectManager(echan3, banque, 
				null, null, null, null, type, null, null, null, 
				null, null, null, null, utilisateur, 
				false, null,
				false);
		assertTrue(echantillonManager.findAllObjectsManager().size() == 7);
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XTRA", false).size() == 3);
		...
		echantillonManager.updateCodeEchantillonsManager(
				liste, null, "XXX", utilisateur);
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XTRA", false).size() == 3);
		echantillonManager.updateCodeEchantillonsManager(
				null, "XTRA", "XXX", utilisateur);
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XTRA", false).size() == 3);
		echantillonManager.updateCodeEchantillonsManager(
				liste, "XTRA", null, utilisateur);
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XTRA", false).size() == 3);
		echantillonManager.updateCodeEchantillonsManager(
				liste, "XTRA", "XXX", null);
		...
		List<Echantillon> res = echantillonManager
				.updateCodeEchantillonsManager(
						liste, "XTRA", "XTRA", utilisateur);
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XTRA", false).size() == 3);
		assertTrue(res.size() == 3);
		...
		res = echantillonManager
				.updateCodeEchantillonsManager(
						liste, "XXX", "XPOR", utilisateur);
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XTRA", false).size() == 3);
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XPOR", false).size() == 0);
		...
		List<TKFantomableObject> fs = new ArrayList<TKFantomableObject>();
		for (int i = 0; i < res.size(); i++) {
			echantillonManager.removeObjectManager(
									res.get(i), null, utilisateur, null);
			fs.add(res.get(i));
		}
		cleanUpFantomes(fs);
		assertTrue(getOperationManager()
				.findAllObjectsManager().size() == 19);
	}
	...
	public void testSaveEchantillonEmplacementManager() {
		...
		EchantillonType type = echantillonTypeManager.findByIdManager(1);
		Utilisateur utilisateur = utilisateurDao.findById(1);
		ObjetStatut statut = objetStatutManager.findByStatutLikeManager(
				"STOCKE", true).get(0);
		Emplacement empl = emplacementManager.findByIdManager(4);
		OperationType op = operationTypeManager.findByNomLikeManager(
				"Stockage", true).get(0);
		...
		assertTrue(echantillonManager.findByCodeLikeManager(
				"XTRA", false).size() == 1);
		Integer id = echan1.getEchantillonId();

		// maj non valide : 1 élt null
		echantillonManager.saveEchantillonEmplacementManager(
				null, statut, empl, utilisateur, ops);
		Echantillon eTest1 = echantillonManager.findByIdManager(id);
		assertNull(echantillonManager.getEmplacementManager(eTest1));
		echantillonManager.saveEchantillonEmplacementManager(
				echan1, statut, empl, null, ops);
		assertNull(echantillonManager.getEmplacementManager(eTest1));
		...
		try {
			echantillonManager.saveEchantillonEmplacementManager(
					echan1, statut, empl, utilisateur, ops);
		} catch (TKException ex) {
			catched = true; 
		...
		try {
			echantillonManager.saveEchantillonEmplacementManager(
					eTest1, statut, empl, utilisateur, ops);
		} catch (TKException ex) {
			catched = true; 
			assertTrue(ex.getMessage().equals("XTRA.1 : error.emplacement.notEmpty"));
			assertTrue(ex.getIdentificationObjetException().equals("XTRA.1"));
		...
		echantillonManager.saveEchantillonEmplacementManager(
				echan1, statut, empl, utilisateur, ops);
		Echantillon eTest2 = echantillonManager.findByIdManager(id);
		Emplacement empl2 = echantillonManager.getEmplacementManager(eTest2);
		assertNotNull(empl2);
		...
		try {
			...
			echantillonManager.saveEchantillonEmplacementManager(
					eTest2, null, null, utilisateur, ops);
		} catch (RequiredObjectIsNullException re) {
			catched = true;
		}
		assertTrue(catched);

		Echantillon eTest3 = echantillonManager.findByIdManager(id);
		echantillonManager.saveEchantillonEmplacementManager(
				eTest3, objetStatutManager.findByIdManager(4), 
				null, utilisateur, ops);
	...
	public void testFindCountByPrelevement() {
		...
		Prelevement p2 = prelevementDao.findById(2);
		res = echantillonManager.findCountByPrelevementManager(p2);
		assertTrue(res == 1);

		Prelevement p3 = prelevementDao.findById(3);
		res = echantillonManager.findCountByPrelevementManager(p3);
		assertTrue(res == 1);

		Prelevement p4 = prelevementDao.findById(4);
	...
	public void testFindCountByPrelevementAndStockeReserve() {
		Prelevement p1 = prelevementDao.findById(1);
		Long res = echantillonManager
				.findCountByPrelevementAndStockeReserveManager(p1);
		assertTrue(res == 2);

		Prelevement p2 = prelevementDao.findById(2);
		res = echantillonManager
				.findCountByPrelevementAndStockeReserveManager(p2);
		assertTrue(res == 0);

		Prelevement p3 = prelevementDao.findById(3);
		res = echantillonManager
				.findCountByPrelevementAndStockeReserveManager(p3);
		assertTrue(res == 1);

		Prelevement p4 = prelevementDao.findById(4);
		res = echantillonManager
				.findCountByPrelevementAndStockeReserveManager(p4);
		assertTrue(res == 0);
	}
	...
	public void testStatutEmplacementValidation() {
		...
		try {
			BeanValidator.validateObject(
					e, new Validator[]{echantillonValidator});
		} catch (ValidationException ve) {
			assertEquals("echantillon.statut.incoherent", ve.getErrors()
					.get(0).getFieldError().getCode());
			catched = true;
		}
		assertTrue(catched);
	...
	public void testCreateUpdateWithNonConformites() {
		...
		echantillonManager.createObjectWithNonConformitesManager(e0, 
				b1, null, null, 
				objetStatutManager.findByIdManager(4), null, 
				echantillonTypeManager.findByIdManager(1), 
				null, null, null, null, null, null, null, null, u, false, "/tmp/", false, 
				ncfsTrait, ncfsCess);

		e0 = echantillonManager.findByCodeLikeManager("e0", true).get(0);

		assertTrue(objetNonConformeDao.findByObjetAndEntite(e0.getEchantillonId(),
				entiteDao.findById(3)).size() == 0);
		...
		echantillonManager.createObjectWithNonConformitesManager(e1, 
				banqueDao.findById(1), null, null, 
				objetStatutManager.findByIdManager(4), null, 
				echantillonTypeManager.findByIdManager(1), 
				null, null, null, null, null, null, null, null, u, false, "/tmp/", false, 
				ncfsTrait, ncfsCess);
		...
		assertTrue(objetNonConformeDao.findByObjetAndEntite(e1.getEchantillonId(),
				entiteDao.findById(3)).size() == 3);
		...
		echantillonManager.createObjectWithNonConformitesManager(e2, 
				banqueDao.findById(1), null, null, 
				objetStatutManager.findByIdManager(4), null, 
				echantillonTypeManager.findByIdManager(1), 
				null, null, null, null, null, null, null, null, u, false, "/tmp/", false, 
		...
		assertTrue(objetNonConformeDao.findByObjetAndEntite(e2.getEchantillonId(),
				entiteDao.findById(3)).size() == 0);
		assertNull(e2.getConformeCession());
		assertNull(e2.getConformeTraitement());

		echantillonManager.updateObjectWithNonConformitesManager(e1, banqueDao.findById(1), 
				null, null, objetStatutManager.findByIdManager(4), null, 
				echantillonTypeManager.findByIdManager(1), null, null, null, null, null, 
				null, null, null, null, null, u, 
				false, null, "/tmp/", ncfsTrait, ncfsCess);
		...
		echantillonManager.updateObjectWithNonConformitesManager(e1, banqueDao.findById(1), 
				null, null, objetStatutManager.findByIdManager(4), null, 
				e1.getEchantillonType(), null, null, null, null, null, 
				null, null, null, null, null, u, 
				false, null, "/tmp/", ncfsTrait, ncfsCess);
		...
		assertTrue(objetNonConformeDao.findByObjetAndEntite(e1.getEchantillonId(),
				entiteDao.findById(3)).size() == 2);
		assertTrue(objetNonConformeDao.findByObjetAndEntite(e1.getEchantillonId(),
				entiteDao.findById(3)).get(0)
				.getNonConformite().getNom().equals("Echantillon non stérile")
				|| objetNonConformeDao.findByObjetAndEntite(e1.getEchantillonId(),
						entiteDao.findById(3)).get(0)
						.getNonConformite().getNom().equals("Non cédable"));
		assertTrue(objetNonConformeDao.findByObjetEntiteAndType(e1.getEchantillonId(),
				entiteDao.findById(3), conformiteTypeDao.findById(3)).size() == 1);
		assertTrue(objetNonConformeDao.findByObjetEntiteAndType(e1.getEchantillonId(),
				entiteDao.findById(3), conformiteTypeDao.findById(2)).size() == 1);
		assertFalse(e1.getConformeTraitement());
		assertFalse(e1.getConformeCession());
	...
	public void testPrepareJDBC() throws ParseException, SQLException {
		...
		Statement stmt = null;	
		ResultSet rs = null;
		EchantillonJdbcSuite jdbcSuite = new EchantillonJdbcSuite();
		...
		try {
			...
			Prelevement prelevement = prelevementDao.findById(1);
			Collaborateur collab = collaborateurManager.findByIdManager(1);
			ObjetStatut nonstocke = objetStatutManager.findByIdManager(4);
			//Emplacement emplacement = emplacementManager.findByIdManager(1);
			EchantillonType type = echantillonTypeManager.findByIdManager(1);
			Unite quantite = uniteManager.findByIdManager(1);
			EchanQualite qualite = echanQualiteManager.findByIdManager(1);
			...
			// on test l'insertion avec la banque nulle
			try {
				echantillonManager.prepareObjectJDBCManager(jdbcSuite,
						echan1, null, null, 
						null, null, null, type, 
						null, null, null, null, 
						null, null, null, 
			...
			// Emplacement occupé par un dérivé
			echan1.setCode("PTRA.3");
			ObjetStatut stocke = objetStatutManager.findByIdManager(1);
			Emplacement empl = emplacementManager.findByIdManager(2);
			catched = false;
			try {
				echantillonManager.prepareObjectJDBCManager(jdbcSuite,
						echan1, banque, null, 
						null, stocke, empl, type, 
						null, null, null, null, 
						null, null, null, 
						utilisateur, true, false);
			} catch (TKException ex) {
			...
			CodeAssigne c3 = new CodeAssigne();
			c3.setCode("c3");
			c3.setIsOrgane(true);
			c3.setIsMorpho(true);
			c3.setOrdre(1);
			...
			try {
				echantillonManager.prepareObjectJDBCManager(jdbcSuite,
						echan1, banque, null, 
						null, stocke, null, type, 
						null, null, null, codes, 
						null, null, null, 
						utilisateur, true, false);
			...
			echantillonManager.prepareObjectJDBCManager(jdbcSuite,
					...
					null, nonstocke, null, type, 
					null, null, null, null, 
					new ArrayList<AnnotationValeur>(), null, null, 
					utilisateur, true, false);
			...
			AnnotationValeur av1 = new AnnotationValeur();
			av1.setAlphanum("1.36");
			av1.setChampAnnotation(champAnnotationDao.findById(7));
			avs.add(av1);
			AnnotationValeur av2 = new AnnotationValeur();
			av2.setBool(false);
			...
			CodeAssigne c1 = new CodeAssigne();
			c1.setCode("c1");
			c1.setLibelle("lib1");
			c1.setIsOrgane(true);
			...
			echantillonManager.prepareObjectJDBCManager(jdbcSuite,
					echan2, banque, prelevement, 
					collab, stocke, emp, type, 
					quantite, qualite, prepa, codes, 
					avs, ncfsTrait, ncfsCess, 
					utilisateur, true, false);
			...
			assertTrue(objetNonConformeDao.findByObjetAndEntite(testInsert.getEchantillonId(), 
					entiteDao.findById(3)).isEmpty());
			assertTrue(codeAssigneManager
					.findCodesOrganeByEchantillonManager(testInsert).isEmpty());
			assertTrue(codeAssigneManager
					.findCodesMorphoByEchantillonManager(testInsert).isEmpty());

			Operation op = getOperationManager().findByObjectManager(testInsert).get(0);
			...
			assertTrue(annotationValeurManager
					.findByChampAndObjetManager(champAnnotationDao.findById(7), testInsert)
					.get(0).getAlphanum().equals("1.36"));
			assertTrue(annotationValeurManager
					.findByChampAndObjetManager(champAnnotationDao.findById(4), testInsert)
					.get(0).getBool().equals(false));

			assertTrue(getOperationManager()
					.findByObjectManager(testInsert).size() == 2);
			assertTrue(getOperationManager()
					.findByObjectManager(testInsert).get(0)
					.getOperationType().getNom().equals("Creation"));
			assertTrue(getOperationManager()
					.findByObjectManager(testInsert).get(1)
					.getOperationType().getNom().equals("Annotation"));

			assertTrue(objetNonConformeDao
					.findByObjetAndEntite(testInsert.getEchantillonId(),
							entiteDao.findById(3)).size() == 3);

			assertTrue(objetNonConformeDao
					.findByObjetEntiteAndType(testInsert.getEchantillonId(),
							entiteDao.findById(3), conformiteTypeDao.findById(3)).size() == 2);
			assertTrue(objetNonConformeDao
					.findByObjetEntiteAndType(testInsert.getEchantillonId(),
							entiteDao.findById(3), conformiteTypeDao.findById(3)).get(0).getNonConformite()
							.equals(nonConformiteDao.findById(6))
							|| objetNonConformeDao
							.findByObjetEntiteAndType(testInsert.getEchantillonId(),
									entiteDao.findById(3), conformiteTypeDao.findById(3)).get(1).getNonConformite()
									.equals(nonConformiteDao.findById(6)));
			assertTrue(objetNonConformeDao
					.findByObjetEntiteAndType(testInsert.getEchantillonId(),
							entiteDao.findById(3), conformiteTypeDao.findById(3)).get(0).getNonConformite()
							.equals(nonConformiteDao.findById(7))
							|| objetNonConformeDao
							.findByObjetEntiteAndType(testInsert.getEchantillonId(),
									entiteDao.findById(3), conformiteTypeDao.findById(3)).get(1).getNonConformite()
									.equals(nonConformiteDao.findById(7)));
			...
			assertTrue(objetNonConformeDao
					.findByObjetEntiteAndType(testInsert.getEchantillonId(),
							entiteDao.findById(3), conformiteTypeDao.findById(2))
							.get(0).getNonConformite().equals(nonConformiteDao.findById(4)));
			...
			// codes
			assertTrue(codeAssigneManager
					.findCodesOrganeByEchantillonManager(testInsert).size() == 2);
			assertTrue(codeAssigneManager
					.findCodesMorphoByEchantillonManager(testInsert).size() == 1);
	...
	public void testRemoveListFromIds() throws ParseException {
		...
		EchantillonType et1 = echantillonTypeManager.findByIdManager(1);
		List<Integer> ids = new ArrayList<Integer>();
		...
		new File("/tmp/" + "pt_" 
				+ b2.getPlateforme().getPlateformeId()
				+ "/" + "coll_" + b2.getBanqueId() 
				+ "/cr_anapath/").mkdirs();
		...
		Echantillon ech1 = new Echantillon();
		ech1.setCode("ECH1");
		echantillonManager.createObjectWithCrAnapathManager(ech1, b2, null, null, 
				objetStatutManager.findByIdManager(1), 
				null, et1, null, null, null,  null, cr, bais, filesCreated, null, null, u2, 
				false, "/tmp/", false);
		...
		echantillonManager.createObjectManager(ech2, b2, null, null, 
				objetStatutManager.findByIdManager(1), 
				null, et1, null, null, null, null, null, null, filesCreated, u2, 
				false, null, false);
		Echantillon ech3 = new Echantillon();
		ech3.setCode("ECH3");
		echantillonManager.createObjectManager(ech3, b2, null, null, 
				objetStatutManager.findByIdManager(1), 
				null, et1, null, null, null, null, null, null, filesCreated, u2, 
				false, null, false);
		
		assertTrue(echantillonManager
				.findByCodeLikeWithBanqueManager("ECH_", b2, false).size() == 3);
		
		ids.add(ech1.getEchantillonId());
		ids.add(ech2.getEchantillonId());
		ids.add(ech3.getEchantillonId());
		Integer cessedEchanId = new Integer(1);
		ids.add(cessedEchanId); // cet echantillon est cede
		// rollback used object Exception
		boolean catched = false;
		try {
			echantillonManager
				.removeListFromIdsManager(ids, "test suppr ids", u2);
		} catch (ObjectUsedException oe) {
			catched = true;
			assertTrue(oe.getMessage()
				.equals("echantillon.cascade.isCessed"));
		}
		assertTrue(catched);
		assertTrue(echantillonManager
				.findByCodeLikeWithBanqueManager("ECH_", b2, false).size() == 3);
		...
		assertTrue(echantillonManager
				.findByCodeLikeWithBanqueManager("ECH_", b2, false).isEmpty());
		
		assertFalse(new File(ech1.getCrAnapath().getPath()).exists());
		
		assertTrue(getFantomeDao().findByNom("ECH3").get(0)
				.getCommentaires().equals("test suppr ids"));
	...
	public void testFindByCodeInPlateformeBanqueManager() {
		...
		List<Echantillon> echans = echantillonManager
				.findByCodeInPlateformeManager("PTRA.1", p1);
		assertTrue(echans.size() == 1);
		echans = echantillonManager
				.findByCodeInPlateformeManager("%", p1);
		assertTrue(echans.size() == 4);
		echans = echantillonManager
				.findByCodeInPlateformeManager(null, p1);
		assertTrue(echans.size() == 0);
		echans = echantillonManager
				.findByCodeInPlateformeManager("%", null);
		assertTrue(echans.size() == 0);
	}
}
