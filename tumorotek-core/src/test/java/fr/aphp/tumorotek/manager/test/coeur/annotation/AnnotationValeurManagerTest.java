...
public class AnnotationValeurManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<AnnotationValeur> valeurs = annotationValeurManager
	...
	public void testFindByChampAndObjetManager() {
		...
		List<AnnotationValeur> valeurs = 
			annotationValeurManager.findByChampAndObjetManager(c, p1);
		...
		assertTrue(valeurs.size() == 0);
		valeurs = annotationValeurManager.findByChampAndObjetManager(null, 
																		null);
		assertTrue(valeurs.size() == 0);
	}
	...
	public void testFindByTableAndBanqueManager() {
		...
		List<AnnotationValeur> valeurs = 
			annotationValeurManager.findByTableAndBanqueManager(t, b);
		...
		assertTrue(valeurs.size() == 0);
		valeurs = annotationValeurManager.findByTableAndBanqueManager(null, 
																		null);
		assertTrue(valeurs.size() == 0);
	}
	...
	public void testFindByObjectManager() {
		List<AnnotationValeur> vals = annotationValeurManager
							.findByObjectManager(echantillonDao.findById(1));
		assertTrue(vals.size() == 6);
		
		vals = annotationValeurManager
								.findByObjectManager(patientDao.findById(1));
		assertTrue(vals.size() == 1);
		
		vals = annotationValeurManager
							.findByObjectManager(echantillonDao.findById(3));
	...
	public void testFindDoublonManager() {
		//Cree le doublon
		AnnotationValeur av1 = annotationValeurDao.findById(8);
	...
	public void createObjectManagerTest() {
		...
		Prelevement prel = prelevementDao.findById(1);
		...
		try {
			annotationValeurManager
				.createOrUpdateObjectManager(valeur, null, null, null,  
								null, u, null, null, filesCreated, filesToDelete);
		...
		try {
			annotationValeurManager
				.createOrUpdateObjectManager(valeur, null, null, null, 
								null, u, "op", null, filesCreated, filesToDelete);
		} catch (RequiredObjectIsNullException re) {
			assertTrue(re.getRequiredObject().equals("ChampAnnotation"));
		}
		try {
			annotationValeurManager
				.createOrUpdateObjectManager(valeur, c, null, null, null, 
									u, "op", null, filesCreated, filesToDelete);
		} catch (RequiredObjectIsNullException re) {
			assertTrue(re.getRequiredObject().equals("TKAnnotableObject"));
		}
		try {
			annotationValeurManager
				.createOrUpdateObjectManager(valeur, c, pat, null, null,  
									u, "op", null, filesCreated, filesToDelete);
		} catch (RequiredObjectIsNullException re) {
		...
		try {
			annotationValeurManager
				.createOrUpdateObjectManager(valeur, c, pat, b, null,
									u, "op", null, filesCreated, filesToDelete);
		...
		try {
			annotationValeurManager
				.createOrUpdateObjectManager(valeur, c, prel, b, null,
							u, "creation", null, filesCreated, filesToDelete);
		...
		annotationValeurManager
				.createOrUpdateObjectManager(valeur, c, prel, b, null, 
							u, "creation", "/tmp/", filesCreated, filesToDelete);
		assertTrue(annotationValeurManager
							.findByChampAndObjetManager(c, prel).size() == 1);
		assertTrue(getOperationManager().findByObjectManager(valeur).size() == 1);
		assertTrue(getOperationManager().findByObjectManager(valeur).get(0)
										.getOperationType().getNom()
														.equals("Creation"));
		
		//Insertion d'un doublon engendrant une exception
		AnnotationValeur v2 = new AnnotationValeur();
		v2.setAlphanum("doublon");
		boolean catched = false;
		try {
			annotationValeurManager
				.createOrUpdateObjectManager(v2, c, prel, b, null, 
							u, "creation", "/tmp/", filesCreated, filesToDelete);
		} catch (Exception e) {
		...
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, prel).size() == 1);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, prel).get(0).getAlphanum()
														.equals("winner"));
		assertTrue(getOperationManager().findByObjectManager(valeur).size() == 1);
	}
	...
	public void updateObjectManagerTest() {
		Prelevement prel = prelevementDao.findById(1);
		ChampAnnotation c = champAnnotationDao.findById(2);
		AnnotationValeur valeur = annotationValeurManager
									.findByChampAndObjetManager(c, prel).get(0);
		...
		annotationValeurManager
			.createOrUpdateObjectManager(valeur, null, null, null, null,
							u, "modification", "/tmp/", filesCreated, filesToDelete);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, prel).size() == 1);
		assertTrue(annotationValeurManager
				.findByChampAndObjetManager(c, prel).get(0).getAlphanum()
		...
		assertTrue(getOperationManager()
				.findByObjetIdEntiteAndOpeTypeManager(valeur, 
	...
	public void removeObjectManagerTest() {
		...
		assertTrue(annotationValeurManager
							.findByChampAndObjetManager(c, prel).size() == 0);
	...
	public void testAnnotationCommonValidation() throws ParseException {
		AnnotationValeur v = new AnnotationValeur();
		
		// champAnnotation null
		v.setAlphanum("");		
		try {
			BeanValidator.validateObject((Object) v, 
								new Validator[] {annotationCommonValidator});
		} catch (ValidationException ve) {
			assertTrue(ve.getErrors().get(0).getFieldError()
							.getCode().equals("anno.alphanum.empty"));
		}
		...
		try {
			...
			assertTrue(ve.getErrors().get(0).getFieldError()
							.getCode().equals("anno.alphanum.tooLong"));
		}
		...
		try {
			...
			assertTrue(ve.getErrors().get(0).getFieldError()
							.getCode().equals("anno.texte.empty"));
		}
		...
		try {
			...
			assertTrue(ve.getErrors().get(0).getFieldError()
							.getCode().equals("anno.num.illegal"));
		}
		...
		try {
			...
			assertTrue(ve.getErrors().get(0).getFieldError()
							.getCode().equals("anno.hyperlien.illegal"));
		}
	...
	public void testRemoveValeurWithBanqueRelation() {
		...
		ChampAnnotation alpha = champAnnotationDao.findById(1);
		ChampAnnotation file1 = champAnnotationDao.findById(13);
		Patient p1 = patientDao.findById(1);
		...
		List<Banque> banks = 
			new ArrayList<Banque>(tableAnnotationManager.getBanquesManager(t1));
		Banque b2 = banqueDao.findById(2);
		banks.add(b2);
		Utilisateur u = utilisateurDao.findById(2);
		tableAnnotationManager.createOrUpdateObjectManager(t1, null, null,
						null, banks, null, u, "modification", "/tmp/", null);
		assertTrue(tableAnnotationManager.getBanquesManager(t1).size() == 2);
		
		// creation de 4 valeurs annotations
		AnnotationValeur p1alpha = new AnnotationValeur();
		p1alpha.setAlphanum("p1lapha");
		annotationValeurManager
			.createOrUpdateObjectManager(p1alpha, alpha, p1, b2, null, u, 
							"creation", "/tmp/", filesCreated, filesToDelete);
		
		AnnotationValeur p2alpha = new AnnotationValeur();
		p2alpha.setAlphanum("p2lapha");
		annotationValeurManager
			.createOrUpdateObjectManager(p2alpha, alpha, p2, b2, null, u, 
								"creation", "/tmp/", filesCreated, filesToDelete);
		
		AnnotationValeur p1file1 = new AnnotationValeur();
		p1file1.setAlphanum("p1file1");
		annotationValeurManager
			.createOrUpdateObjectManager(p1file1, file1, p1, b2, null, u, 
								"creation", "/tmp/", filesCreated, filesToDelete);
		assertTrue(filesCreated.isEmpty());
		
		AnnotationValeur p2file1 = new AnnotationValeur();
		p2file1.setAlphanum("p2file1");
		annotationValeurManager
			.createOrUpdateObjectManager(p2file1, file1, p2, b2, null, u, 
								"creation", "/tmp/", filesCreated, filesToDelete);
		assertTrue(filesCreated.isEmpty());
		
		assertTrue(annotationValeurManager
										.findAllObjectsManager().size() == 16);
		...
		assertTrue(annotationValeurManager
							.findByTableAndBanqueManager(t1, b2).size() == 0);
	...
	public void testStoreFileAnno() {
		AnnotationValeur valeur = new AnnotationValeur();
		Patient pat = patientDao.findById(2);
		ChampAnnotation c = champAnnotationDao.findById(13);
		Banque b = banqueDao.findById(1);
		Utilisateur u = utilisateurDao.findById(2);
		
		Integer totFichier = fichierDao.findAll().size();
		
		List<File> filesCreated = new ArrayList<File>();
		List<File> filesToDelete = new ArrayList<File>();
		
		// creation architecture fichiers
		File baseDir = new File(Utils.writeAnnoFilePath("/tmp/", b, c, null));
		baseDir.mkdirs();
		...
		try {
			annotationValeurManager
			.createOrUpdateObjectManager(valeur, c, pat, b, f, 
							 u, "creation", "/tmp/", filesCreated, filesToDelete);
		...
		try {
			annotationValeurManager
			.createOrUpdateObjectManager(valeur, c, pat, b, f, 
							u, "creation", "rm *",filesCreated, filesToDelete);
		...
		assertTrue(f.getPath()
			.equals(Utils.writeAnnoFilePath("/tmp/", b, c, f) + "_" + f.getFichierId()));
		...
		valeur.setStream(bais);
		annotationValeurManager.createOrUpdateObjectManager(valeur, c, pat, b, 
				f, u, "modification", "/tmp/", filesCreated, filesToDelete);
		assertTrue(fichierDao.findAll().size() == totFichier + 1);
		f = valeur.getFichier();
	...
	public void testCreateAnnotationValeurListManager() {
		...
		Echantillon obj = echantillonDao.findById(4);
		Banque b = banqueDao.findById(1);
		...
		num1.setBanque(b);
		valeurs.add(num1);
		AnnotationValeur thes1 = new AnnotationValeur();
		...
		try {
			annotationValeurManager.createAnnotationValeurListManager(valeurs,
							obj, u, "creation", "/tmp/", filesCreated, filesToDelete);
		} catch (ValidationException ve) {
			testFindAllObjectsManager();
		}
		// creation réussie
		num1.setAlphanum("12");
		valeurs = annotationValeurManager.
				createAnnotationValeurListManager(valeurs,
						obj, u, "creation", "/tmp/", filesCreated, filesToDelete);
...
//		valeurs.add(boolUp); valeurs.add(numUp);
		...
		annotationValeurManager.createAnnotationValeurListManager(valeurs,
				obj, u, "modification", "/tmp/", filesCreated, filesToDelete);
		assertTrue(annotationValeurManager
...
//				.get(0));
		...
		List<Operation> ops = getOperationManager().findByObjectManager(obj);
	...
	public void testFindByIdManager() {
		AnnotationValeur val1 = annotationValeurManager.findByIdManager(1);
		assertNotNull(val1);
		assertTrue(val1.getAlphanum().equals("AlphanumValue1"));
		
		AnnotationValeur valNull = annotationValeurManager.findByIdManager(20);
		assertNull(valNull);
	}
	...
	public void testGetValueForAnnotationValeur() {
		...
		AnnotationValeur val1 = annotationValeurManager.findByIdManager(1);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val1)
				.equals(val1.getAlphanum()));
		
		// boolean
		AnnotationValeur val4 = annotationValeurManager.findByIdManager(4);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val4)
				.equals(val4.getBool()));
		
		// date
		AnnotationValeur val5 = annotationValeurManager.findByIdManager(5);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val5)
				.equals(val5.getDate()));
		
		// texte
		AnnotationValeur val7 = annotationValeurManager.findByIdManager(7);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val7)
				.equals(val7.getTexte()));
		
		// thes
		AnnotationValeur val8 = annotationValeurManager.findByIdManager(8);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val8)
				.equals(val8.getItem()));
		
		// thes2
		AnnotationValeur val9 = annotationValeurManager.findByIdManager(9);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val9)
				.equals(val9.getItem()));
		
		// file
		AnnotationValeur val10 = annotationValeurManager.findByIdManager(10);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val10)
				.equals(val10.getFichier()));
		
		// link
		AnnotationValeur val11 = annotationValeurManager.findByIdManager(11);
		assertTrue(annotationValeurManager
				.getValueForAnnotationValeur(val11)
				.equals(val11.getAlphanum()));
		
		assertNull(annotationValeurManager
				.getValueForAnnotationValeur(new AnnotationValeur()));
		
		assertNull(annotationValeurManager
				.getValueForAnnotationValeur(null));
	}
	...
	public void testPrepareAnnotationValeurListJDBCManager() throws SQLException {
		...
		Statement stmt = null;
		ResultSet rs = null;
		EchantillonJdbcSuite suite = new EchantillonJdbcSuite();
		
		try {
			conn = DataSourceUtils.getConnection(dataSource);
			stmt = conn.createStatement();
			rs = stmt.executeQuery("select max(annotation_valeur_id)"
							+ " from ANNOTATION_VALEUR");
			rs.first();
			Integer maxAnnoId = rs.getInt(1);
			...
			try {
				annotationValeurManager
					.prepareAnnotationValeurListJDBCManager(suite, valeurs, obj, u);
			} catch (ValidationException ve) {
				catched = true;
			}
			assertTrue(catched);
			...
			// asserts
			assertTrue(annotationValeurManager
									.findByObjectManager(obj).size() == 7);
			
			AnnotationValeur av1 = annotationValeurManager
					.findByChampAndObjetManager(champAnnotationDao.findById(4), obj)
					.get(0);
			assertTrue(av1.getBool());
			assertTrue(getOperationManager().findByObjectManager(av1).get(0)
					.getOperationType().getNom().equals("Creation"));
			
			AnnotationValeur av2 = annotationValeurManager
					.findByChampAndObjetManager(champAnnotationDao.findById(7), obj)
					.get(0);	
			assertTrue(av2.getAlphanum().equals("12"));
			assertTrue(getOperationManager().findByObjectManager(av2).get(0)
					.getOperationType().getNom().equals("Creation"));
			
			AnnotationValeur av3 = annotationValeurManager
					.findByChampAndObjetManager(champAnnotationDao.findById(12), obj)
					.get(0);	
			assertTrue(av3.getItem().equals(itemDao.findById(4)));
			assertTrue(getOperationManager().findByObjectManager(av3).get(0)
					.getOperationType().getNom().equals("Creation"));
			
			AnnotationValeur av4 = annotationValeurManager
					.findByChampAndObjetManager(champAnnotationDao.findById(9), obj)
					.get(0);	
			assertTrue(av4.getTexte().equals("bob l'éponge le film"));
			assertTrue(getOperationManager().findByObjectManager(av4)
					.get(0).getOperationType().getNom().equals("Creation"));
			
			AnnotationValeur av5 = annotationValeurManager
					.findByChampAndObjetManager(champAnnotationDao.findById(6), obj)
					.get(0);
			assertTrue(av5.getDate().equals(cal));
			assertTrue(getOperationManager().findByObjectManager(av5)
					.get(0).getOperationType().getNom().equals("Creation"));
	...
	public void testCreateFileBatchForTKObjectsManager() {
		...
		Integer totFichier = fichierDao.findAll().size();
		Integer totValeurs = annotationValeurDao.findAll().size();
		...
		try {
			annotationValeurManager
				.createFileBatchForTKObjectsManager(objs, f, bais, c, b, u, "/tmp/", filesCreated);
		} catch (ValidationException e) {
			catched = true;
		}
		assertTrue(catched);
		...
		try {
			annotationValeurManager
			.createFileBatchForTKObjectsManager(objs, f, bais, c, b, u, "/rm *", filesCreated);
		} catch (RuntimeException re) {
			catched = true;
		}
		assertTrue(catched);
		...
		// null tests
		annotationValeurManager
			.createFileBatchForTKObjectsManager(null, f, bais, c, b, u, "/tmp/", filesCreated);
		annotationValeurManager
			.createFileBatchForTKObjectsManager(objs, null, bais, c, b, u, "/tmp/", filesCreated);
		annotationValeurManager
			.createFileBatchForTKObjectsManager(objs, f, null, c, b, u, "/tmp/", filesCreated);
		...
		Fichier f1 = annotationValeurManager
			.findByChampAndObjetManager(c, patientDao.findById(1)).get(0).getFichier();
		Fichier f2 = annotationValeurManager
			.findByChampAndObjetManager(c, patientDao.findById(2)).get(0).getFichier();
		Fichier f3 = annotationValeurManager
			.findByChampAndObjetManager(c, patientDao.findById(3)).get(0).getFichier();
		
		assertTrue(f1.getNom().equals("fichierTest"));
		assertTrue(f2.getNom().equals("fichierTest"));
		assertTrue(f3.getNom().equals("fichierTest"));
		assertTrue(f1.getPath().equals(f2.getPath()) 
									&& f2.getPath().equals(f3.getPath()));
		assertTrue(f1.getMimeType().equals(f2.getMimeType()) 
							&& f2.getMimeType().equals(f3.getMimeType()));

		assertTrue(f1.getPath()
			.equals(Utils.writeAnnoFilePath("/tmp/", b, c, f) + "_" + f2.getFichierId()));
		assertTrue(f2.getPath()
			.equals(Utils.writeAnnoFilePath("/tmp/", b, c, f) + "_" + f2.getFichierId()));
		assertTrue(f3.getPath()
			.equals(Utils.writeAnnoFilePath("/tmp/", b, c, f) + "_" + f2.getFichierId()));
		
		// nettoyage
		annotationValeurManager.removeObjectManager(annotationValeurManager
				.findByChampAndObjetManager(c, patientDao.findById(1)).get(0), null);
		...
		annotationValeurManager.removeObjectManager(annotationValeurManager
				.findByChampAndObjetManager(c, patientDao.findById(2)).get(0), null);
		assertTrue(fichierDao.findAll().size() == totFichier + 1);
		...
		annotationValeurManager.removeObjectManager(annotationValeurManager
				.findByChampAndObjetManager(c, patientDao.findById(3)).get(0), null);
		assertTrue(fichierDao.findAll().size() == totFichier);
		assertTrue(annotationValeurDao.findAll().size() == totValeurs);
		assertTrue(baseDir.listFiles().length == 0);
	...
	public void testFindCountByTableAnnotationBanqueManager() {
		...
		Long count = annotationValeurManager
				.findCountByTableAnnotationBanqueManager(t3, b1);
		assertTrue(count == 3);
		Banque b2 = banqueDao.findById(2);
		count = annotationValeurManager
				.findCountByTableAnnotationBanqueManager(t3, b2);
		assertTrue(count == 2);
		TableAnnotation t5 = tableAnnotationDao.findById(5);
		count = annotationValeurManager
				.findCountByTableAnnotationBanqueManager(t5, b2);
		assertTrue(count == 0);
		count = annotationValeurManager
				.findCountByTableAnnotationBanqueManager(t5, b1);
		assertTrue(count == 3);
		count = annotationValeurManager
				.findCountByTableAnnotationBanqueManager(t5, null);
		assertNull(count);
		count = annotationValeurManager
				.findCountByTableAnnotationBanqueManager(null, b1);
		assertNull(count);
