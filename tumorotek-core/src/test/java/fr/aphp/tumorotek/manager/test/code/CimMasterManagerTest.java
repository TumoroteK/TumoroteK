...
public class CimMasterManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<CimMaster> cims = (List<CimMaster>) 
						cimMasterManager.findAllObjectsManager();
	...
	public void testFindByCodeLikeManager() {
		...
		List<CimMaster> cims = 
			(List<CimMaster>) cimMasterManager
										.findByCodeLikeManager("B00", true);
		assertTrue(cims.size() == 1);
		//teste une recherche non exactMatch
		cims = (List<CimMaster>) cimMasterManager
										.findByCodeLikeManager("B00", false);
		assertTrue(cims.size() == 11);
		//teste une recherche infructueuse
		cims = (List<CimMaster>) cimMasterManager
										.findByCodeLikeManager("LUX", true);
		assertTrue(cims.size() == 0);
		//null recherche
		cims = (List<CimMaster>) cimMasterManager
										.findByCodeLikeManager(null, false);
		assertTrue(cims.size() == 0);
	}
	...
	public void testFindByLibelleLikeManager() {
		...
		List<CimMaster> cims = 
			(List<CimMaster>) cimMasterManager
							.findByLibelleLikeManager("choléra", true);
		assertTrue(cims.size() == 1);
		assertTrue(cims.get(0).getCode().equals("A00"));
		//teste une recherche non exactMatch
		cims = (List<CimMaster>) cimMasterManager
								.findByLibelleLikeManager("choléra", false);
		assertTrue(cims.size() == 6);
		//teste une recherche infructueuse
		cims = (List<CimMaster>) cimMasterManager
								.findByLibelleLikeManager("LUXTEST", false);
		assertTrue(cims.size() == 0);
		//null recherche
		cims = (List<CimMaster>) cimMasterManager
								.findByLibelleLikeManager(null, false);
		assertTrue(cims.size() == 0);
	}
	...
	public void testFindByCimParentManager() {
		CimMaster parent = (CimMaster) 
			cimMasterManager.findByCodeLikeManager("A00", true).get(0);
		List<CimMaster> cims = 
			cimMasterManager.findByCimParentManager(parent);
		assertTrue(cims.size() == 3);
		List<CimMaster> cims2 = (List<CimMaster>) 
				cimMasterManager.findByCodeLikeManager("A00.", false); 
		assertTrue(cims.equals(cims2));
		parent = (CimMaster) cimMasterManager
			.findByCodeLikeManager("(A00-A09)", true).get(0);
	...
	public void testGetAdicapsManager() {
		CimMaster a = (CimMaster) cimMasterManager
						.findByCodeLikeManager("C00.9", true).get(0);
		assertTrue(cimMasterManager.getAdicapsManager(a).size() == 4);
		a = (CimMaster) cimMasterManager
							.findByCodeLikeManager("A00.1", false).get(0);
	...
	public void testFindChildrenCodesManager() {
		...
		a = (CimMaster) cimMasterManager
							.findByCodeLikeManager("M00.0", true).get(0);
		codes = cimMasterManager.findChildrenCodesManager(a);
		assertTrue(codes.size() == 11);
		
		a = (CimMaster) cimMasterManager
							.findByCodeLikeManager("M00.07", true).get(0);
		codes = cimMasterManager.findChildrenCodesManager(a);
	...
	public void testFindByIdManager() {
		assertTrue(cimMasterManager.findByIdManager(34)
											.getCode().equals("A04.7"));
