...
public class ConditMilieuManagerTest extends AbstractManagerTest4 {
	...
	public void testFindByMilieuLikeManager() {
		...
		List<ConditMilieu> conditMilieux = 
			conditMilieuManager.findByMilieuLikeManager("SEC", true);
		assertTrue(conditMilieux.size() == 1);
		//teste une recherche non exactMatch
		conditMilieux = 
			conditMilieuManager.findByMilieuLikeManager("HEP", false);
		assertTrue(conditMilieux.size() == 1);
		assertTrue(conditMilieux.get(0).getMilieu().equals("HEPARINE"));
		//teste une recherche infructueuse
		conditMilieux = 
			conditMilieuManager.findByMilieuLikeManager("TRIZOL", false);
		assertTrue(conditMilieux.size() == 0);
		//null recherche
		conditMilieux = 
			conditMilieuManager.findByMilieuLikeManager(null, false);
		assertTrue(conditMilieux.size() == 0);
	}
	...
	public void testIsUsedObject() {
		...
		ConditMilieu cm2 = (conditMilieuManager
				.findByMilieuLikeManager("HEPARINE", true)).get(0);
	...
	private void createObjectManagerTest() {
		...
		ConditMilieu cm1 = new ConditMilieu();
		cm1.setMilieu("EDTA-");
		cm1.setPlateforme(plateformeDao.findById(1));
		conditMilieuManager.createObjectManager(cm1);
		assertTrue((conditMilieuManager
				.findByMilieuLikeManager("EDTA-", true)).size() == 1);
		//Insertion d'un doublon engendrant une exception
		Boolean catched = false;
		ConditMilieu cm1Bis = new ConditMilieu();
		...
		//validation test
		String[] emptyValues = new String[]{"", "  ", "lalo¤¤$", 
		...
		for (int i = 0; i < emptyValues.length; i++) {
			try {
				...
				assertTrue(conditMilieuManager
						.findByOrderManager(cm1.getPlateforme()).size() == 2);
			}
		}
	}
	...
	private void updateObjectManagerTest() {
		...
		ConditMilieu cm1 = (conditMilieuManager
			.findByMilieuLikeManager("EDTA-", true)).get(0);
		cm1.setMilieu("Triz.zol +");
		conditMilieuManager.updateObjectManager(cm1);
		assertTrue((conditMilieuManager
				.findByMilieuLikeManager("Triz.zol +", true)).size() == 1);
		...
		assertTrue(catched);
		assertTrue((conditMilieuManager
				.findByMilieuLikeManager("HEPARINE", true)).size() == 1);
		
		//Validation test
		String[] emptyValues = new String[]{"", "  ", "$$lk#¤¤", 
											createOverLength(200), null};
		for (int i = 0; i < emptyValues.length; i++) {
			try {
	...
	private void removeObjectManagerTest() {
		...
		ConditMilieu cm1 = (conditMilieuManager
				.findByMilieuLikeManager("Triz.zol +", true)).get(0);
		conditMilieuManager.removeObjectManager(cm1);
		assertTrue((conditMilieuManager
				.findByMilieuLikeManager("Triz.zol +", true)).size() == 0);
...
//		assertTrue(catched);
		assertTrue((conditMilieuManager
				.findByMilieuLikeManager("SEC", true)).size() > 0);
		//null remove
		conditMilieuManager.removeObjectManager(null);
		assertTrue(conditMilieuManager
				.findByOrderManager(cm1.getPlateforme()).size() == 1);
	}
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
					conditMilieuManager.findByOrderManager(pf);
