...
public class AffectationImprimanteManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<AffectationImprimante> liste = 
			affectationImprimanteManager.findAllObjectsManager();
	...
	public void testFindByIdManager() {
		...
		AffectationImprimantePK pk = 
			new AffectationImprimantePK(u1, b1, i1);
		
		AffectationImprimante ai = 
			affectationImprimanteManager.findByIdManager(pk);
		assertNotNull(ai);
	...
	public void testFindByExcludedPKManager() {
		...
		List<AffectationImprimante> liste = 
			affectationImprimanteManager.findByExcludedPKManager(pk);
	...
	public void testFindByUtilisateurBanqueManager() {
		...
		List<AffectationImprimante> liste = 
			affectationImprimanteManager.findByBanqueUtilisateurManager(b1, u1);
		assertTrue(liste.size() == 1);
		
		liste = affectationImprimanteManager
			.findByBanqueUtilisateurManager(b1, u4);
		assertTrue(liste.size() == 0);
		
		liste = affectationImprimanteManager
			.findByBanqueUtilisateurManager(b3, u1);
		assertTrue(liste.size() == 0);
		
		liste = affectationImprimanteManager
			.findByBanqueUtilisateurManager(b1, null);
		assertTrue(liste.size() == 0);
		
		liste = affectationImprimanteManager
			.findByBanqueUtilisateurManager(null, u1);
		assertTrue(liste.size() == 0);
		
		liste = affectationImprimanteManager
			.findByBanqueUtilisateurManager(null, null);
		assertTrue(liste.size() == 0);
	}
	...
	public void testFindDoublonManager() {
		...
		assertTrue(affectationImprimanteManager.findDoublonManager(
				u1, b1, i1));
		assertFalse(affectationImprimanteManager.findDoublonManager(
				u4, b1, i1));
	...
	public void testValidateObjectManager() {
		...
		try {
			affectationImprimanteManager.validateObjectManager(
					null, b1, i1);
		} catch (Exception e) {
	...
	public void testCrud() {
		...
		// on test l'insertion avec l'utilisateur null
		try {
			affectationImprimanteManager.createObjectManager(
					ai, null, b1, i1, m1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
		...
		AffectationImprimante aiTest = affectationImprimanteManager
			.findByIdManager(pk);
		...
		AffectationImprimante aiTest2 = affectationImprimanteManager
			.findByIdManager(pk);
		...
		// on test un update
		AffectationImprimante aiUp1 = new AffectationImprimante();
		affectationImprimanteManager.createObjectManager(aiUp1, u3, b1, i2, m1);
		pk = aiUp1.getPk();
		assertTrue(affectationImprimanteManager
				.findAllObjectsManager().size() == 6);
		AffectationImprimante aiTest1 = affectationImprimanteManager
			.findByIdManager(pk);
