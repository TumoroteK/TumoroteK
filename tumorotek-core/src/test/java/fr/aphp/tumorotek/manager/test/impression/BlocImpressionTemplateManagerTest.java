...
public class BlocImpressionTemplateManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<BlocImpressionTemplate> liste = 
			blocImpressionTemplateManager.findAllObjectsManager();
	...
	public void testFindByIdManager() {
		...
		BlocImpressionTemplate bt = blocImpressionTemplateManager
			.findByIdManager(pk);
		assertNotNull(bt);
	...
	public void testFindByExcludedPKManager() {
		...
		List<BlocImpressionTemplate> liste = 
			blocImpressionTemplateManager.findByExcludedPKManager(pk);
	...
	public void testFindByTemplateManager() {
		...
		List<BlocImpressionTemplate> liste = 
			blocImpressionTemplateManager.findByTemplateManager(t1);
		assertTrue(liste.size() == 4);
	...
	public void testFindDoublonManager() {
		...
		assertFalse(blocImpressionTemplateManager
				.findDoublonManager(null, null));
	...
	public void testValidateObjectManager() {
		...
		try {
			blocImpressionTemplateManager.validateObjectManager(
					null, b1);
		} catch (Exception e) {
	...
	public void testCrud() {
		Template t1 = templateDao.findById(1);
		BlocImpression b1 = blocImpressionDao.findById(1);
		BlocImpression b4 = blocImpressionDao.findById(4);
		...
		try {
			blocImpressionTemplateManager.createObjectManager(bit1, t1, null);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		// on test l'insertion avec le template null
		try {
			blocImpressionTemplateManager.createObjectManager(bit1, null, b1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		// est lancée
		try {
			blocImpressionTemplateManager.createObjectManager(bit1, t1, b1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catched = true;
		...
		blocImpressionTemplateManager.createObjectManager(bit1, t1, b4);
		BlocImpressionTemplatePK pk = bit1.getPk();
		assertTrue(blocImpressionTemplateManager
				.findAllObjectsManager().size() == 5);
		
		BlocImpressionTemplate bitTest = blocImpressionTemplateManager
			.findByIdManager(pk);
		...
		bitTest.setOrdre(10);
		blocImpressionTemplateManager.updateObjectManager(
				bitTest, bitTest.getTemplate(), 
				bitTest.getBlocImpression());
		BlocImpressionTemplate bitTest2 = blocImpressionTemplateManager
			.findByIdManager(pk);
