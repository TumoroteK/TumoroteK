...
public class CimoMorphoManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<CimoMorpho> cims = (List<CimoMorpho>) 
						cimoMorphoManager.findAllObjectsManager();
	...
	public void testFindByCodeLikeManager() {
		...
		List<CimoMorpho> cimos = 
			(List<CimoMorpho>) cimoMorphoManager
									.findByCodeLikeManager("D0-20150", true);
		assertTrue(cimos.size() == 1);
		//teste une recherche non exactMatch
		cimos = (List<CimoMorpho>) cimoMorphoManager
									.findByCodeLikeManager("D0", false);
		assertTrue(cimos.size() == 6);
		//teste une recherche CIMREF
		cimos = (List<CimoMorpho>) cimoMorphoManager
									.findByCodeLikeManager("C5", false);
		assertTrue(cimos.size() == 87);
		// teste une recherche CIMREF -> verifie que le resultat ne 
		// contient pas de doublons
		cimos = (List<CimoMorpho>) cimoMorphoManager
									.findByCodeLikeManager("50", false);
	...
	public void testFindByLibelleLikeManager() {
		...
		List<CimoMorpho> cimos = 
			(List<CimoMorpho>) cimoMorphoManager
							.findByLibelleLikeManager("FIBROMATOSE", true);
		assertTrue(cimos.size() == 1);
		//teste une recherche non exactMatch
		cimos = (List<CimoMorpho>) cimoMorphoManager
								.findByLibelleLikeManager("FIBROMATOSE", false);
		assertTrue(cimos.size() == 7);
		//teste une recherche infructueuse
		cimos = (List<CimoMorpho>) cimoMorphoManager
								.findByLibelleLikeManager("CULLUM", false);
		assertTrue(cimos.size() == 0);
		//null recherche
		cimos = (List<CimoMorpho>) cimoMorphoManager
	...
	public void testGetAdicapsManager() {
		CimoMorpho a = (CimoMorpho) cimoMorphoManager
						.findByCodeLikeManager("M-8005/0", true).get(0);
		assertTrue(cimoMorphoManager.getAdicapsManager(a).size() == 2);
		a = (CimoMorpho) cimoMorphoManager
							.findByLibelleLikeManager("Keasbey", false).get(0);
