...
public class ProfilManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Profil p = profilManager.findByIdManager(1);
		assertNotNull(p);
		assertTrue(p.getNom().equals("CONSULTATION"));
		assertFalse(p.getAccesAdministration());
		
		Profil pNull = profilManager.findByIdManager(10);
		assertNull(pNull);
	}
	...
	public void testFindAll() {
		List<Profil> list = profilManager.findAllObjectsManager();
	...
	public void testFindDoublon() {
		...
		Profil p2 = profilManager.findByIdManager(2);
	...
	public void testIsUsedObjectManager() {
		Profil p1 = profilManager.findByIdManager(1);
		assertTrue(profilManager.isUsedObjectManager(p1));
		
		Profil p3 = profilManager.findByIdManager(3);
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Utilisateur admin = utilisateurDao.findById(1);
		OperationType type1 = operationTypeDao.findById(1);
		Entite ent1 = entiteManager.findByIdManager(1);
		Entite ent2 = entiteManager.findByIdManager(2);
		List<DroitObjet> droits = new ArrayList<DroitObjet>();
		...
		// null plateforme
		try {
			profilManager.createObjectManager(p1, null, admin, null);
		} catch (Exception e) {
		...
		try {
			profilManager.createObjectManager(p1, null, admin, pf1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
		...
		catched = false;
		try {
			profilManager.createObjectManager(p1, droits, admin, pf1);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"RequiredObjectIsNullException")) {
				catched = true;
		...
		Integer idP1 = p1.getProfilId();
		
		Profil pTest = profilManager.findByIdManager(idP1);
		assertNotNull(pTest);
		...
		droits = new ArrayList<DroitObjet>();
		droits.add(do1);
		droits.add(do2);
		...
		p2.setArchive(true);
		profilManager.createObjectManager(p2, droits, admin, pf1);
		...
		Integer idP2 = p2.getProfilId();
		assertTrue(operationManager.findByObjectManager(p2).size() == 1);
		
		Profil pTest2 = profilManager.findByIdManager(idP2);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		do1.setOperationType(type1);
		droits.add(do1);
		profilManager.createObjectManager(p, droits, admin, pf2);
		assertTrue(profilManager.findAllObjectsManager().size() == 6);
		...
		Integer idP = p.getProfilId();
		
		Profil pUp = profilManager.findByIdManager(idP);
		boolean catched = false;
		// on test l'insertion d'un doublon
		pUp.setNom("UTILISATEUR");
		try {
			profilManager.updateObjectManager(pUp, droits, admin);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"DoublonFoundException")) {
				catched = true;
		...
		try {
			pUp.setAdmin(null);
			profilManager.updateObjectManager(pUp, null, admin);
		} catch (Exception e) {
			if (e.getClass().getSimpleName().equals(
					"ValidationException")) {
				catched = true;
		...
		Profil pTest = profilManager.findByIdManager(idP);
		assertNotNull(pTest);
		assertTrue(pTest.getNom().equals("NEW"));
		...
		Profil pTest2 = profilManager.findByIdManager(idP);
		assertNotNull(pTest2);
		assertTrue(droitObjetManager.findByProfilManager(pTest2).size() == 1);
		assertTrue(droitObjetManager.findByProfilManager(pTest2)
				.get(0).equals(do1));
		
		// insertion valide avec les droits nulls
		pUp.setArchive(true);
		profilManager.updateObjectManager(
				pUp, new ArrayList<DroitObjet>(), admin);
		assertTrue(profilManager.findAllObjectsManager().size() == 6);
		assertTrue(droitObjetManager.findAllObjectsManager().size() == 9);
		...
		Profil pTest3 = profilManager.findByIdManager(idP);
	...
	private void removeObjectManagerTest() {
		...
		p.setAdmin(false);
		p.setAccesAdministration(true);
		profilManager.createObjectManager(p, null, admin, plateformeDao.findById(1));
		assertTrue(profilManager.findAllObjectsManager().size() == 6);
		Integer id = p.getProfilId();
		
		Profil pTest = profilManager.findByIdManager(id);		
		profilManager.removeObjectManager(pTest);
		assertTrue(profilManager.findAllObjectsManager().size() == 5);
		assertTrue(operationManager.findByObjectManager(pTest).size() == 0);
		
		// test de la non suppression d'un profil utilisé
		Profil p4 = profilManager.findByIdManager(4);
		boolean catched = false;
		try {
	...
	public void testFindByPlateformeAndArchiveManager() {
		...
		List<Profil> prfs = 
			profilManager.findByPlateformeAndArchiveManager(pf1, false);
