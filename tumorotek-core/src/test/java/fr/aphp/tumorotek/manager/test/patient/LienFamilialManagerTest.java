...
public class LienFamilialManagerTest extends AbstractManagerTest {
	...
	public void setLienFamilialManager(LienFamilialManager lfManager) {
	...
	public void testFindAllObjectsManager() {
		List<LienFamilial> lienFamilials = 
							lienFamilialManager.findAllObjectsManager();
	...
	public void testfindByNomLikeManager() {
		...
		List<LienFamilial> lienFamilials = lienFamilialManager
								.findByNomLikeManager("Pere-Fille", true);
	...
	public void testFindDoublon() {
		//Cree le doublon
		LienFamilial lf1 = (lienFamilialManager
				.findByNomLikeManager("Tante-Neveu", true)).get(0);
	...
	public void testIsUsedObject() {
		...
		LienFamilial lf1 = (lienFamilialManager
				.findByNomLikeManager("Pere-Fille", true)).get(0);
		assertTrue(lienFamilialManager.isUsedObjectManager(lf1));
		//Enregistrement n'est pas reference
		LienFamilial lf2 = (lienFamilialManager
				.findByNomLikeManager("Frere-Soeur", false)).get(0);
	...
	public void createObjectManagerTest() {
		...
		LienFamilial lf1 = new LienFamilial();
		lf1.setNom("GrandMere-Petitfils");
		lf1.setAscendant(false);
		lienFamilialManager.createObjectManager(lf1);
		assertTrue((lienFamilialManager
			.findByNomLikeManager("GrandMere-Petitfils", true)).size() == 1);
		assertTrue((lienFamilialManager
			.findByNomLikeManager("Petitfils-GrandMere", true)).size() == 1);
		assertTrue((lienFamilialManager
				.findByNomLikeManager("Petitfils-GrandMere", true))
										.get(0).getReciproque().equals(lf1));
		assertTrue((lienFamilialManager
				.findByNomLikeManager("Petitfils-GrandMere", true))
										.get(0).getAscendant());
		...
		assertTrue(catched);
		assertTrue((lienFamilialManager
				.findByNomLikeManager("%GrandMere", false)).size() == 2);
		
		//validation test Type
		String[] nomValues = new String[]{"", "  ", null, "HOO", "RR--TTT", 
						createOverLength(25) + "-" + createOverLength(25), 
														"good-TypeDS"};
	...
	public void updateObjectManagerTest() {
		...
		LienFamilial lf1 = (lienFamilialManager
			.findByNomLikeManager("GrandMere-Petitfils", true)).get(0);
		lf1.setNom("Kilo-Jaffar");
		lf1.setAscendant(true);
		lienFamilialManager.updateObjectManager(lf1);
		assertTrue((lienFamilialManager
				.findByNomLikeManager("Kilo-Jaffar", true))
				.get(0).getAscendant());
		assertTrue((lienFamilialManager
				.findByNomLikeManager("Kilo-Jaffar", true))
				.get(0).getReciproque().getNom().equals("Jaffar-Kilo"));
		assertFalse((lienFamilialManager
				.findByNomLikeManager("Kilo-Jaffar", true))
				.get(0).getReciproque().getAscendant());
		lf1.setAscendant(null);
		lienFamilialManager.updateObjectManager(lf1);
		assertNull((lienFamilialManager
				.findByNomLikeManager("Kilo-Jaffar", true))
				.get(0).getAscendant());
		assertNull((lienFamilialManager
				.findByNomLikeManager("Kilo-Jaffar", true))
		...
		assertTrue(catched);
		assertTrue((lienFamilialManager
				.findByNomLikeManager("Fille-Pere", false)).size() == 1);
	}
	...
	public void removeObjectManagerTest() {
		...
		LienFamilial lf1 = (lienFamilialManager
				.findByNomLikeManager("Kilo-Jaffar", true)).get(0);
		lienFamilialManager.removeObjectManager(lf1);
		assertTrue((lienFamilialManager
				.findByNomLikeManager("%Jaffar", false)).size() == 0);
		//Suppression engrendrant une exception
		Boolean catched = false;
		try {
			LienFamilial lf2 = (lienFamilialManager
					.findByNomLikeManager("Fille-Pere", true)).get(0);
		...
		assertTrue(catched);
		assertTrue((lienFamilialManager
				.findByNomLikeManager("Fille-Pere", true)).size() > 0);
		//null remove
