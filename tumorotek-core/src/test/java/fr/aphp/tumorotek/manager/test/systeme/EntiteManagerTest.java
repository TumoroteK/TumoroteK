...
public class EntiteManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		Entite entite = entiteManager.findByIdManager(1);
		assertNotNull(entite);
		assertTrue(entite.getNom().equals("Patient"));
		
		Entite entiteNull = entiteManager.findByIdManager(100);
	...
	public void testReadAll() {
		List<Entite> entites = entiteManager.findAllObjectsManager();
	...
	public void testFindAnnotablesManager() {
		List<Entite> entites = entiteManager.findAnnotablesManager();
	...
	public void testFindObjectByEntiteAndIdManager() {
		Entite echan = entiteManager.findByIdManager(3);
		Object objEchan = 
			entiteManager.findObjectByEntiteAndIdManager(echan, 2);
		assertNotNull(objEchan);
		assertTrue(objEchan.getClass().getSimpleName().equals("Echantillon"));
		Echantillon newEchan = (Echantillon) objEchan;
		assertNotNull(newEchan);
		assertTrue(newEchan.getEchantillonId() == 2);
		
		Entite prod = entiteManager.findByIdManager(8);
		Object objProd = entiteManager.findObjectByEntiteAndIdManager(prod, 1);
		assertNotNull(objProd);
		assertTrue(objProd.getClass().getSimpleName().equals("ProdDerive"));
		ProdDerive newProd = (ProdDerive) objProd;
		assertNotNull(newProd);
		assertTrue(newProd.getProdDeriveId() == 1);
				
		Entite retour = entiteManager.findByIdManager(19);
		Object objRetour = 
			entiteManager.findObjectByEntiteAndIdManager(retour, 10);
	...
	public void testFindIdsByEntiteAndIdAfterBanqueFiltreManager() {
		...
		assertTrue(ids.contains(1)); assertTrue(ids.contains(2));
		assertTrue(ids.contains(3)); assertTrue(ids.contains(4));
		...
		Entite cession = entiteManager.findByIdManager(5);
		banks.remove(banqueDao.findById(2));
		assertTrue(entiteManager
				.findIdsByEntiteAndIdAfterBanqueFiltreManager(cession, ids, banks)
			.size() == 3);
		
		Entite prelevement = entiteManager.findByIdManager(2);
		assertTrue(entiteManager
				.findIdsByEntiteAndIdAfterBanqueFiltreManager(prelevement, ids, banks)
			.size() == 3);
		
		Entite retour = entiteManager.findByIdManager(19);
		assertFalse(entiteManager
			.findIdsByEntiteAndIdAfterBanqueFiltreManager(retour, ids, banks).isEmpty());
