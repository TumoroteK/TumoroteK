...
//	
	...
	public void testFindAllObjectsManager() {
		List<PatientSip> pats = patientSipManager.findAllObjectsManager();
	...
	public void testFindByNomLikeManager() {
		...
		List<PatientSip> pats = 
			patientSipManager.findByNomLikeManager("Sun", true);
	...
	public void testFindByNipLikeManager() {
		//teste une recherche exactMatch
		List<PatientSip> pats = 
			patientSipManager.findByNipLikeManager("713705", true);
		assertTrue(pats.size() == 1);
		//teste une recherche non exactMatch
	...
	public void testFindByNumeroSejourManager() {
		List<PatientSip> pats = 
			patientSipManager.findByNumeroSejourManager("770000391", true);
		assertTrue(pats.size() == 1);
	...
	public void testCreateObjectManagerTest() throws ParseException {
		...
		assertTrue((patientSipManager
				.findByNomLikeManager("KIEDIS", true)).size() == 1);
		assertNotNull(p.getDateCreation());
		Calendar today = Utils.getCurrentSystemCalendar();
		assertTrue(today.get(Calendar.DAY_OF_MONTH)
			== p.getDateCreation().get(Calendar.DAY_OF_MONTH));
		assertTrue(today.get(Calendar.MONTH)
				== p.getDateCreation().get(Calendar.MONTH));
		assertTrue(today.get(Calendar.YEAR)
				== p.getDateCreation().get(Calendar.YEAR));
		assertNull(p.getDateModification());

		//update
		PatientSip p2 = p.clone();
		p2.setPatientSipId(null);
		p2.setPrenom("YOuch");
		// update
		patientSipManager.createOrUpdatePatientInTempTableManager(p2, 10, false);
		assertTrue(patientSipManager
				.findByNipLikeManager("68", false).size() == 1);
		p = patientSipManager
				.findByNipLikeManager("68", false).get(0);
		assertNotNull(p.getDateCreation());
	...
	public void testUpdatePatientInTempTableManager() throws ParseException {
		...
		//Update
		PatientSip p2 = patientSipManager
					.findByNomLikeManager("KIEDIS", true).get(0);
		...
		p2 = patientSipManager
					.findByNomLikeManager("Cantrell", true).get(0);
		
		assertNotNull(p2.getDateCreation());
		assertNotNull(p2.getDateModification());
		Calendar today = Utils.getCurrentSystemCalendar();
		Calendar modif = p2.getDateModification();
		assertTrue(today.get(Calendar.DAY_OF_MONTH)
			== modif.get(Calendar.DAY_OF_MONTH));
		assertTrue(today.get(Calendar.MONTH)
			== modif.get(Calendar.MONTH));
		assertTrue(today.get(Calendar.YEAR)
			== modif.get(Calendar.YEAR));
		...
		assertTrue(patientSipManager
				.findByNipLikeManager("68", false).get(0)
							.getDateModification().equals(modif));
	...
	public void testIsSynchronizedPatientManager() 
		...
		PatientSip patSip = p.clone();
		
		//insertion
		patientSipManager.createOrUpdatePatientInTempTableManager(p, 10, false);
		assertTrue((patientSipManager
				.findByNomLikeManager("Furtado", true)).size() == 1);
		p = patientSipManager.findByNomLikeManager("Furtado", true).get(0);
		
		// identiques et nulls des deux cotés
		assertTrue(patientSipManager
					.isSynchronizedPatientManager(patSip).isEmpty());
		...
		List<Field> fields = patientSipManager
								.isSynchronizedPatientManager(patSip2);
		assertTrue(fields.size() == 5);
		assertTrue(fields.contains(PatientSip.class.getDeclaredField("nom")));
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("prenom")));
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("dateNaissance")));
		assertTrue(fields.contains(PatientSip.class.getDeclaredField("sexe")));
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("patientEtat")));
		...
		assertTrue(fields.size() == 5);
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("nomNaissance")));
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("villeNaissance")));
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("paysNaissance")));
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("dateDeces")));
		assertTrue(fields
				.contains(PatientSip.class.getDeclaredField("dateEtat")));
		
		// valeurs identiques pour les champs nulls 
		p.setNomNaissance("MALIK");
		p.setVilleNaissance("Islamabad");
		p.setPaysNaissance("PAKISTAN");
		p.setDateEtat(new SimpleDateFormat("dd/MM/yyyy")
													.parse("01/05/2011"));
		...
		assertTrue(patientSipManager
							.isSynchronizedPatientManager(patSip2).isEmpty());
		...
		patSip3.setDateEtat(new SimpleDateFormat("dd/MM/yyyy")
													.parse("01/06/2011"));
		patSip3.setDateDeces(new SimpleDateFormat("dd/MM/yyyy")
													.parse("01/06/2011"));
		fields = patientSipManager.isSynchronizedPatientManager(patSip3);
		assertTrue(fields.size() == 5);
		assertTrue(fields
		...
		assertTrue(patientSipManager
				.isSynchronizedPatientManager(null).isEmpty());
	...
	public void testDoSynchronizePatientManager() {
		...
		// synchronisation
		p1.setNip("876");
		assertTrue(patientSipManager.doSynchronizePatientManager(p1)
			.equals(patientManager.findByNomLikeManager("DELPHINO", true)
			.get(0)));
		// null
	...
	public void testUpdatePatientSystem() throws ParseException {
		...
		patientManager.createOrUpdateObjectManager(p, null, null, null, 
						null, null, null, null, u, "creation", null,
											false);
		assertFalse(patientManager.findByNipLikeManager("69", true).isEmpty());
		assertFalse(patientManager
						.findByNomLikeManager("Gainsbourg", true).isEmpty());
		...
		assertFalse(patientManager
						.findByNomLikeManager("Gainsbarre", true).isEmpty());
		...
		assertTrue(getOperationManager().findByObjectManager(p).get(1)
					.getOperationType().getNom().equals("Synchronisation"));
		assertNull(getOperationManager().findByObjectManager(p).get(1)
														.getUtilisateur());
	...
	@Test
	public void testFirstInFirstOut() throws ParseException, 
		...
		assertTrue((patientSipManager
				.findByNomLikeManager("Last", true)).size() == 1);
		assertTrue((patientSipManager
				.findByNomLikeManager(p1.getNom(), true)).isEmpty());
		assertTrue(patientSipManager.findAllObjectsManager().size() == 4);
		
		// clean up
		patientSipManager.removeObjectManager(patientSipManager
								.findByNomLikeManager("Last", true).get(0));
	...
	public void testparseHl7MessagePID() throws IOException, ParseException {
		String pid = "PID|1|"
			+ "|1330000303^^^IF_MCKN||TESTEXPLOBALUN^ARNAUD^^^^^D~NOMNAISS^ARNAUD^^^^^L|"
			+ "MAMAN_NAME|19550115|M||"
		...
		assertTrue(pSip.getDateNaissance()
			.equals(new SimpleDateFormat("dd/MM/yyyy").parse("15/01/1955")));
		...
		PatientSipSejour s = pSip.getSejours().iterator().next();
		assertTrue(s.getNumero().equals("770000391"));
		assertTrue(s.getDateSejour()
				.equals(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
				.parse("15/09/2011 08:00:00")));
		
		pid = "PID|1|"
			+ "|1330000304^^^IF_MCKN||DEADUNK^^^^^^Z|"
			+ "|19120115|X||"
		...
		assertTrue(pSip.getDateNaissance()
			.equals(new SimpleDateFormat("dd/MM/yyyy").parse("15/01/1912")));
		assertTrue(pSip.getSexe().equals("Ind"));
		assertNull(pSip.getPaysNaissance());
		assertNull(pSip.getVilleNaissance());
		assertNull(pSip.getDateEtat());
		assertTrue(pSip.getDateDeces()
			.equals(new SimpleDateFormat("dd/MM/yyyy").parse("19/09/2010")));
