...
public class ProtocoleManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<Protocole> protocoles = protocoleManager.findAllObjectsManager();
	...
	public void testFindDoublon() {
		//Cree le doublon
		Protocole p1 = protocoleManager.findAllObjectsManager().get(0);
		assertFalse(protocoleManager.findDoublonManager(p1));
		Protocole p2 = new Protocole();
		p2.setNom(p1.getNom());
	...
	public void testIsUsedObject() {
		...
		Protocole tysa = (Protocole) protocoleManager
		...
		Protocole edmus = (Protocole) protocoleManager
								.findByOrderManager(pf2).get(0);
	...
	private void createObjectManagerTest() {
		...
		String[] nomValues = new String[]{"", "  ", null, 
						"HOOÉÀ≠ÉÇ™È–", createOverLength(200), "goo d98-Type"};
		Protocole p2 = new Protocole();
	...
	private void updateObjectManagerTest() {
		...
		Plateforme pf1 = plateformeDao.findById(1);
		Protocole p1 = (Protocole) protocoleManager
						.findByOrderManager(pf1).get(0);
		p1.setNom("Melbase+234");
		protocoleManager.updateObjectManager(p1);
		assertTrue(((Protocole) protocoleManager
				.findByOrderManager(pf1).get(0)).getNom()
				.equals("Melbase+234"));
		...
		//Validation test
		String[] nomValues = new String[]{"", "  ", null, "|mpl1‑ÀÇÈ≠ÀÀÈÇ™Ç", 
...
//			.findByOrderManager(pf1)).size() == 2);
	...
	public void testFindByOrderManager() {
		...
		List< ? extends TKThesaurusObject> list = 
						protocoleManager.findByOrderManager(pf);
