...
public class RetourManagerTest extends AbstractManagerTest4 {
	...
	public void testFindAllObjectsManager() {
		List<Retour> retours = retourManager.findAllObjectsManager();
	...
	public void testFindDoublon() throws ParseException {
		//Cree le doublon
		Retour r1 = retourManager.findAllObjectsManager().get(0);
	...
	private void createObjectManagerTest() throws ParseException {
		...
		Retour r = new Retour();
		/*Champs obligatoires*/
		Echantillon e1 = echantillonDao.findById(1);
		Echantillon e2 = echantillonDao.findById(2);
		Utilisateur u = utilisateurDao.findById(1);
		
		r.setTempMoyenne(new Float(22.0));
		
		Collaborateur c = collaborateurDao.findById(2);		
		Incident i1 =  incidentDao.findById(1);
		
		Retour r1 = retourManager.getRetoursForObjectManager(e1).get(0);
		r.setDateRetour(r1.getDateRetour());
		r.setDateSortie(r1.getDateSortie());
		//insertion doublon
		Boolean catched = false;
		try {
			retourManager.createOrUpdateObjectManager(r, e1, null, c, 
					null, null, i1, u, "creation");
		} catch (Exception e) {
		...
		retourManager.createOrUpdateObjectManager(r, e2, null, c, null, null, 
				 i1, u, "creation");
		...
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
				.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-C"));
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
				.getConteneur().equals(conteneurDao.findById(1)));
		assertTrue(getOperationManager().findByObjectManager(r).size() == 1);
		assertNull(retourManager
				.getRetoursForObjectManager(e2).get(0).getDateRetour());
		assertTrue(echantillonDao.findById(2).getObjetStatut()
				.getStatut().equals("ENCOURS"));
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
		...
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
										.parse("02/01/2010 16:47:10"));
		r.setDateRetour(dateRetour);
		catched = false;
		try {
			retourManager.createOrUpdateObjectManager(r, e2, null, c, null, 
					null, i1, u, null);
	...
	private void updateObjectManagerTest() throws ParseException {
		...
		Echantillon e2 = echantillonDao.findById(2);
		Echantillon e1 = echantillonDao.findById(1);
		Retour r = retourManager.getRetoursForObjectManager(e2).get(0);
		...
		//modification valide
		retourManager.createOrUpdateObjectManager(r, e2, 
				emplacementDao.findById(2),
				r.getCollaborateur(), 
				r.getCession(), r.getTransformation(), 
				r.getIncident(), u, "modification");
		assertTrue(retourManager.findAllObjectsManager().size() == 9);
		assertTrue(retourManager.getRetoursForObjectManager(e2).size() == 1);
		assertTrue(getOperationManager().findByObjectManager(r).size() == 2);
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
				.getTempMoyenne().equals(new Float(28.0)));
		// emplacement a changé
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
				.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-B"));
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
				.getConteneur().equals(conteneurDao.findById(1)));
		
		// statut ré-affecté
		assertNotNull(retourManager
				.getRetoursForObjectManager(e2).get(0).getDateRetour());
		assertTrue(echantillonDao.findById(2).getObjetStatut()
				.getStatut().equals("STOCKE"));
		assertNull(retourManager.getRetoursForObjectManager(e2).get(0)
				.getObjetStatut());
	}
	...
	public void testValidation() throws ParseException {
		...
		try {
			BeanValidator.validateObject((Object) r, 
									new Validator[] {retourValidator});
		} catch (ValidationException ve) {
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
					.getCode().equals("validation.syntax.empty"));
		}
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
					.getCode().equals("date.validation.supDateRetour"));
		}
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
					.getCode().equals("retour.observations.empty"));
		}
		...
		try {
			...
			errs = ve.getErrors();
			assertTrue(errs.get(0).getFieldError()
					.getCode().equals("retour.observations.tooLong"));
		}
	}
	
	@Test
	public void testCreateRetourListManager() throws ParseException {
		Retour base = new Retour();
		
		/*Champs obligatoires*/
		Calendar dateSortie = Calendar.getInstance();
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
										.parse("01/01/2013 12:10:25"));
		base.setDateSortie(dateSortie);
		Calendar dateRetour = Calendar.getInstance();
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
										.parse("01/01/2013 14:22:45"));
		...
		// old emplacements
		Hashtable<TKStockableObject, Emplacement> oldEmps = 
		...
		retourManager.createRetourListManager(objs, oldEmps, base, c, c1, 
				null, null, u);
		
		assertTrue(retourManager.findAllObjectsManager().size() == 12);
		assertTrue(retourManager
			.getRetoursForObjectManager(echantillonDao.findById(1))
			.size() == 6);
		assertTrue(retourManager
			.getRetoursForObjectManager(prodDeriveDao.findById(2)).size() == 1);
		assertTrue(retourManager
			.getRetoursForObjectManager(prodDeriveDao.findById(1)).size() == 1);
		assertTrue(retourManager
			.getRetoursForObjectManager(echantillonDao
												.findById(2)).size() == 1);
		assertFalse(echantillonDao.findById(2).getObjetStatut().getStatut()
					.equals("ENCOURS"));
		
		Retour r = retourManager
				.getRetoursForObjectManager(echantillonDao.findById(2)).get(0);
		...
		assertTrue(r.getDateRetour().getTime().equals(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
					.parse("01/01/2013 14:22:45")));
		assertTrue(r.getDateSortie().getTime().equals(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
					.parse("01/01/2013 12:10:25")));
		assertTrue(r.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-B"));
		assertTrue(r.getConteneur().equals(conteneurDao.findById(1)));
		
		r = retourManager
			.getRetoursForObjectManager(prodDeriveDao.findById(2)).get(0);
		assertTrue(r.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-B"));
		assertTrue(r.getConteneur().equals(conteneurDao.findById(1)));
		
		r = retourManager
				.getRetoursForObjectManager(prodDeriveDao.findById(1)).get(0);
		...
		// aux objets
		retourManager.createRetourListManager(objs, null, base, c, c1, 
				null, null, u);
		...
		retourManager.createRetourListManager(null, null, base, c, c1, 
				null, null, u);
	...
	public void testCreateRetourHugeListManager() throws ParseException {
		...
		Calendar dateSortie = Calendar.getInstance();
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
										.parse("01/01/2011 12:10:00"));
		base.setDateSortie(dateSortie);
		Calendar dateRetour = Calendar.getInstance();
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
										.parse("01/01/2011 16:15:00"));
		base.setDateRetour(dateRetour);
		...
		Collaborateur c = collaborateurDao.findById(2);		
		Cession c1 =  cessionDao.findById(1);
		Transformation transformation = transformationDao.findById(1);
		Incident incident = incidentDao.findById(1);
		...
		retourManager.createRetourHugeListManager(
				objs, oldEmps, base, c, c1, 
				transformation, incident, u);
		...
		assertTrue(retourManager
			.getRetoursForObjectManager(echantillonDao
												.findById(2)).size() == 1);
		Retour test = retourManager
			.getRetoursForObjectManager(echantillonDao
				.findById(2)).get(0);
		assertNotNull(test);
		assertTrue(test.getObjetId() == 2);
		assertTrue(test.getEntite().getEntiteId() == 3);
		assertTrue(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
			.format(test.getDateSortie().getTime())
					.equals("01/01/2011 12:10:00"));
		assertTrue(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
			.format(test.getDateRetour().getTime())
				.equals("01/01/2011 16:15:00"));
		...
		assertTrue(retourManager
				.getRetoursForObjectManager(prodDeriveDao
						.findById(1)).get(0)
			.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-A"));
		assertTrue(retourManager
				.getRetoursForObjectManager(prodDeriveDao
						.findById(1)).get(0)
			.getConteneur().equals(conteneurDao.findById(1)));
		...
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
										.parse("01/01/2011 22:10:00"));
		r2.setDateSortie(dateSortie);
		
		retourManager.createOrUpdateObjectManager(r2, echantillonDao.findById(1), 
				null, null, null, null, null, u, "creation");
		assertTrue(retourManager.findAllObjectsManager().size() == 13);
		assertTrue(retourManager
			.getRetoursForObjectManager(echantillonDao.findById(1))
			.size() == 7);
		assertTrue(echantillonDao.findById(1).getObjetStatut()
												.getStatut().equals("ENCOURS"));
		...
		assertTrue(catched);
		assertTrue(retourManager.findAllObjectsManager().size() == 13);
		assertTrue(echantillonDao
				.findByObjetStatut(objetStatutDao.findByStatut("ENCOURS").get(0)).size() == 1);
		
		Retour last = retourManager
				.getRetoursForObjectManager(echantillonDao.findById(1)).get(6);
		assertTrue(last.getDateRetour() == null);
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
												.parse("01/01/2011 23:10:00"));
		last.setDateRetour(dateRetour);
		retourManager.createOrUpdateObjectManager(last, echantillonDao.findById(1), 
													null, null, null, null, null, u, "modification");
		...
		try {
			...
			assertTrue(re.getMessage()
					.contains("date.validation.retourExistant.incoherent"));
			catched = true;
		}
		assertTrue(catched);
		assertTrue(retourManager.findAllObjectsManager().size() == 13);
		...
		// teste update objs
		assertTrue(echantillonDao.findById(objs.get(0).listableObjectId())
	...
	public void testDeleteIncompleteRetour() {
		...
		Echantillon e1 = echantillonDao.findById(1);
		Echantillon e2 = echantillonDao.findById(2);
		Echantillon e3 = echantillonDao.findById(3);
		ProdDerive p1 = prodDeriveDao.findById(1);
		Utilisateur u = utilisateurDao.findById(1);
		...
		Boolean catched = false;
		try { // EPUISE
			retourManager.createOrUpdateObjectManager(r, e3, null, c, null, null, 
				 i1, u, "creation");
		} catch (ObjectStatutException oe) {
			catched = true;
		}
		assertTrue(catched);
		retourManager.createOrUpdateObjectManager(r2, e2, null, c, null, null, 
				 i1, u, "creation");
		retourManager.createOrUpdateObjectManager(rp, p1, null, c, null, null, 
				 i1, u, "creation");
		...
		// emplacement n'a pas changé
		assertTrue(retourManager.getRetoursForObjectManager(p1).get(0)
				.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-A"));
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
				.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-C"));	
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
		...
		assertNull(retourManager
				.getRetoursForObjectManager(e2).get(0).getDateRetour());
		assertNull(retourManager
				.getRetoursForObjectManager(p1).get(0).getDateRetour());
		assertTrue(echantillonDao.findById(3).getObjetStatut()
				.getStatut().equals("EPUISE"));
		assertTrue(echantillonDao.findById(2).getObjetStatut()
				.getStatut().equals("ENCOURS"));
		assertTrue(prodDeriveDao.findById(1).getObjetStatut()
				.getStatut().equals("ENCOURS"));
		assertTrue(retourManager.getRetoursForObjectManager(e2).get(0)
				.getObjetStatut().getStatut().equals("STOCKE"));
		assertTrue(retourManager.getRetoursForObjectManager(p1).get(0)
		...
		r.setDateSortie(r1.getDateSortie());
		try {
			retourManager.createOrUpdateObjectManager(r, p1, null, c, 
					null, null, i1, u, "creation");
		} catch (ObjectStatutException e) {
			catched = true;
		}
		assertTrue(catched);
		...
		assertTrue(prodDeriveDao.findById(1)
				.getObjetStatut().getStatut().equals("STOCKE"));
		assertTrue(echantillonDao.findById(2).getObjetStatut()
				.getStatut().equals("STOCKE"));
	...
	public void testDateSortieCoherence() throws ParseException {
		...
		assertTrue(errs.getFieldError()
				.getCode().equals("retour.dateSortie.empty"));
		
		
		Calendar dateSortie = Calendar.getInstance();
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
										.parse("01/01/2007 12:10:25"));
		r.setDateSortie(dateSortie);
		
		errs = retourValidator.checkDateSortieCoherence(r);
		assertTrue(errs.getFieldError()
				.getCode().equals("date.validation.infDateStockage"));
		
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
										.parse("16/03/2008 00:00:00"));
		r.setDateSortie(dateSortie);
		
		Calendar dateRetour = Calendar.getInstance();
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
										.parse("15/03/2008 23:59:59"));
		r.setDateRetour(dateRetour);
		errs = retourValidator.checkDateSortieCoherence(r);
		assertTrue(errs.getFieldError()
				.getCode().equals("date.validation.supDateRetour"));
		...
		r.setObjetId(1);
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
											.parse("31/12/2009 00:00:00"));
		r.setDateSortie(dateSortie);
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
											.parse("02/01/2010 23:59:59"));
		r.setDateRetour(dateRetour);
		errs = retourValidator.checkDateSortieCoherence(r);
		assertTrue(errs.getFieldError()
				.getCode().equals("date.validation.inclueRetourExistant"));
		
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
			.parse("08/01/2010 10:00:00"));
		r.setDateSortie(dateSortie);
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
			.parse("10/01/2010 23:59:59"));
		r.setDateRetour(dateRetour);
		errs = retourValidator.checkDateSortieCoherence(r);
		assertTrue(errs.getFieldError()
				.getCode().equals("date.validation.incluDansRetourExistant"));
		
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
			.parse("10/01/2010 10:00:00"));
		r.setDateSortie(dateSortie);
		errs = retourValidator.checkDateSortieCoherence(r);
	...
	public void testDateRetourCoherence() throws ParseException {
		...
		dateSortie.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
			.parse("31/12/2009 10:00:00"));
		r.setDateSortie(dateSortie);
		dateRetour.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm:ss")
			.parse("01/01/2010 16:59:59"));
		r.setDateRetour(dateRetour);
		errs = retourValidator.checkDateRetourCoherence(r);
		assertTrue(errs.getFieldError()
				.getCode().equals("date.validation.incluDansRetourExistant"));
	...
	public void testFindByObjectDateRetourEmptyManager() {
		List<Integer> objsIds = new ArrayList<Integer>();
		
		List<Retour> incomps = retourManager
				.findByObjectDateRetourEmptyManager(objsIds, entiteDao.findById(3));
		assertTrue(incomps.isEmpty());
		
		incomps = retourManager
				.findByObjectDateRetourEmptyManager(null, entiteDao.findById(3));
		assertTrue(incomps.isEmpty());
		...
		retourManager.createOrUpdateObjectManager(r1, e2, null, c, null, null, 
				 i1, u, "creation");
		...
		assertTrue(echantillonDao.findById(2)
				.getObjetStatut().getStatut().equals("ENCOURS"));
		...
		retourManager.createOrUpdateObjectManager(r2, e1, null, c, null, null, 
				 null, u, "creation");	
	...
	public void testUpdateMultipleObjectManager() {
		...
		List<TKStockableObject> objs = new ArrayList<TKStockableObject>();
		objs.add(e1); objs.add(e2); objs.add(p1);
		...
		Incident i1 =  incidentDao.findById(1);
		Cession c1 = cessionDao.findById(1);
		Transformation t1 = transformationDao.findById(1);
		...
		echanIds.add(1); echanIds.add(2);
		List<Integer> pdIds = new ArrayList<Integer>();
		pdIds.add(1);
		...
		assertTrue(retourManager
				.getRetoursForObjectManager(echantillonDao.findById(2)).size() == 1);
		...
		Retour r1 = retourManager
			.findByObjectDateRetourEmptyManager(echanIds, entiteDao.findById(3)).get(0);
		r1.setDateRetour(dateR);
		Retour r2 = retourManager
				.getRetoursForObjectManager(echantillonDao.findById(2)).get(0);
		r2.setDateRetour(dateR);
		r2.setTempMoyenne(new Float(0.0));
		Retour r3 = retourManager
		.getRetoursForObjectManager(prodDeriveDao.findById(1)).get(0);
		r3.setDateRetour(dateR);
		
		retours.clear();
		retours.add(r1); retours.add(r2); retours.add(r3);
