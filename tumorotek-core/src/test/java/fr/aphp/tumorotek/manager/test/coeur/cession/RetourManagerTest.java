...
public class RetourManagerTest extends AbstractManagerTest4
{
   ...
   private void createObjectManagerTest() throws ParseException{
      ...
      /*Champs obligatoires*/
      final Echantillon e1 = echantillonDao.findById(1).orElse(null);
      final Echantillon e2 = echantillonDao.findById(2).orElse(null);
      final Utilisateur u = utilisateurDao.findById(1).orElse(null);

      r.setTempMoyenne(new Float(22.0));

      final Collaborateur c = collaborateurDao.findById(2).orElse(null);
      final Incident i1 = incidentDao.findById(1).orElse(null);
      ...
      assertTrue(retourManager.getRetoursForObjectManager(e2).get(0).getConteneur().equals(conteneurDao.findById(1))).orElse(null);
      ...
      assertTrue(echantillonDao.findById(2).getObjetStatut().getStatut().equals("ENCOURS")).orElse(null);
   ...
   private void updateObjectManagerTest() throws ParseException{
      ...
      //modification valide
      retourManager.createOrUpdateObjectManager(r, e2, emplacementDao.findById(2), r.getCollaborateur(), r.getCession().orElse(null),
      ...
      assertTrue(echantillonDao.findById(2).getObjetStatut().getStatut().equals("STOCKE")).orElse(null);
   ...
   public void testCreateRetourListManager() throws ParseException{
      ...
      final Collaborateur c = collaborateurDao.findById(2).orElse(null);
      final Cession c1 = cessionDao.findById(1).orElse(null);

      final Utilisateur u = utilisateurDao.findById(1).orElse(null);

      // objects
      final List<TKStockableObject> objs = new ArrayList<>();
      objs.add(echantillonDao.findById(1)).orElse(null);
      objs.add(echantillonDao.findById(2)).orElse(null);
      objs.add(prodDeriveDao.findById(2)).orElse(null);
      objs.add(prodDeriveDao.findById(1)).orElse(null);

      // old emplacements
      final List<OldEmplTrace> oldEmps = new ArrayList<OldEmplTrace>();
      oldEmps.add(new OldEmplTrace(echantillonDao.findById(2).orElse(null), "OLD.C1.R1.T1.23", 
    		  conteneurDao.findById(1), emplacementDao.findById(2))).orElse(null);
      oldEmps.add(new OldEmplTrace(prodDeriveDao.findById(1), null, null, emplacementDao.findById(4))).orElse(null);
      ...
      assertTrue(retourManager.getRetoursForObjectManager(echantillonDao.findById(1)).size() == 6).orElse(null);
      assertTrue(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(2)).size() == 1).orElse(null);
      assertTrue(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(1)).size() == 1).orElse(null);
      assertTrue(retourManager.getRetoursForObjectManager(echantillonDao.findById(2)).size() == 1).orElse(null);
      assertFalse(echantillonDao.findById(2).getObjetStatut().getStatut().equals("ENCOURS")).orElse(null);

      Retour r = retourManager.getRetoursForObjectManager(echantillonDao.findById(2)).get(0).orElse(null);
      ...
      assertTrue(r.getConteneur().equals(conteneurDao.findById(1))).orElse(null);

      r = retourManager.getRetoursForObjectManager(prodDeriveDao.findById(2)).get(0).orElse(null);
      assertTrue(r.getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-B"));
      assertTrue(r.getConteneur().equals(conteneurDao.findById(1))).orElse(null);

      r = retourManager.getRetoursForObjectManager(prodDeriveDao.findById(1)).get(0).orElse(null);
      ...
      r = retourManager.getRetoursForObjectManager(echantillonDao.findById(2)).get(0).orElse(null);
      ...
      // r = retourManager
      // .getRetoursForObjectManager(echantillonDao.findById(1)).get(0).orElse(null);
   ...
   public void testCreateRetourHugeListManager() throws ParseException{
      ...
      final Collaborateur c = collaborateurDao.findById(2).orElse(null);
      final Cession c1 = cessionDao.findById(1).orElse(null);
      final Transformation transformation = transformationDao.findById(1).orElse(null);
      final Incident incident = incidentDao.findById(1).orElse(null);
      ...
      // old emplacements
      final List<OldEmplTrace> oldEmps = new ArrayList<OldEmplTrace>();
      oldEmps.add(new OldEmplTrace(echantillonDao.findById(2).orElse(null), "CC1.R2.T6.BT1.A-1", 
    		  conteneurDao.findById(1), emplacementDao.findById(3))).orElse(null);
      oldEmps.add(new OldEmplTrace(prodDeriveDao.findById(1).orElse(null), "CC1.R1.T1.BT1.A-A", 
    		  conteneurDao.findById(1), emplacementDao.findById(1))).orElse(null);
      // oldEmps.put(echantillonDao.findById(2), emplacementDao.findById(6)).orElse(null);
      // oldEmps.put(prodDeriveDao.findById(1), emplacementDao.findById(1)).orElse(null);
      ...
      assertTrue(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(1)).size() == 1).orElse(null);
      assertTrue(retourManager.getRetoursForObjectManager(echantillonDao.findById(2)).size() == 1).orElse(null);
      Retour test = retourManager.getRetoursForObjectManager(echantillonDao.findById(2)).get(0).orElse(null);
      assertNotNull(test);
      ...
      assertTrue(test.getConteneur().equals(conteneurDao.findById(1))).orElse(null);
      assertTrue(test.getIncident().equals(incident));

      assertTrue(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(1)).get(0).getOldEmplacementAdrl().orElse(null)
         .equals("CC1.R1.T1.BT1.A-A"));
      assertTrue(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(1)).get(0).getConteneur().orElse(null)
         .equals(conteneurDao.findById(1))).orElse(null);
      ...
      retourManager.createOrUpdateObjectManager(r2, echantillonDao.findById(1), null, null, null, null, null, u, "creation").orElse(null);
      assertTrue(retourManager.findAllObjectsManager().size() == 13);
      assertTrue(retourManager.getRetoursForObjectManager(echantillonDao.findById(1)).size() == 7).orElse(null);
      assertTrue(echantillonDao.findById(1).getObjetStatut().getStatut().equals("ENCOURS")).orElse(null);
      assertTrue(echantillonDao.findById(1).getSterile()).orElse(null);
      ...
      final Retour last = retourManager.getRetoursForObjectManager(echantillonDao.findById(1)).get(6).orElse(null);
      ...
      retourManager.createOrUpdateObjectManager(last, echantillonDao.findById(1).orElse(null), null, null, null, null, null, u,
         "modification");
      ...
      assertTrue(retourManager.findAllObjectsManager().size() == 12);
      test = retourManager.getRetoursForObjectManager(echantillonDao.findById(2)).get(0).orElse(null);
      assertNotNull(test);
      assertTrue(test.getObjetId() == 2);
      assertTrue(test.getEntite().getEntiteId() == 3);
      assertTrue(
      ...
      // teste update objs
      assertTrue(echantillonDao.findById(objs.get(0).listableObjectId()).getObjetStatut().getStatut().equals("ENCOURS")).orElse(null);
      ...
      // null emplacement
      assertNull(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(3)).get(0).getOldEmplacementAdrl()).orElse(null);
      assertNull(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(3)).get(0).getConteneur()).orElse(null);
   ...
   public void testDeleteIncompleteRetour(){
      ...
      final Echantillon e1 = echantillonDao.findById(1).orElse(null);
      final Echantillon e2 = echantillonDao.findById(2).orElse(null);
      final Echantillon e3 = echantillonDao.findById(3).orElse(null);
      final ProdDerive p1 = prodDeriveDao.findById(1).orElse(null);
      final Utilisateur u = utilisateurDao.findById(1).orElse(null);
      ...
      assertTrue(retourManager.getRetoursForObjectManager(e2).get(0).getOldEmplacementAdrl().equals("CC1.R1.T1.BT1.A-C"));
      assertTrue(retourManager.getRetoursForObjectManager(e2).get(0).getConteneur().equals(conteneurDao.findById(1))).orElse(null);
      assertTrue(retourManager.getRetoursForObjectManager(p1).get(0).getConteneur().equals(conteneurDao.findById(1))).orElse(null);
      assertNull(retourManager.getRetoursForObjectManager(e2).get(0).getDateRetour());
      assertNull(retourManager.getRetoursForObjectManager(p1).get(0).getDateRetour());
      assertTrue(echantillonDao.findById(3).getObjetStatut().getStatut().equals("EPUISE")).orElse(null);
      assertTrue(echantillonDao.findById(2).getObjetStatut().getStatut().equals("ENCOURS")).orElse(null);
      assertTrue(prodDeriveDao.findById(1).getObjetStatut().getStatut().equals("ENCOURS")).orElse(null);
      assertTrue(retourManager.getRetoursForObjectManager(e2).get(0).getObjetStatut().getStatut().equals("STOCKE"));
      ...
      assertTrue(prodDeriveDao.findById(1).getObjetStatut().getStatut().equals("STOCKE")).orElse(null);
      assertTrue(echantillonDao.findById(2).getObjetStatut().getStatut().equals("STOCKE")).orElse(null);
   ...
   public void testFindByObjDatesManager() throws ParseException{
      ...
      List<Retour> rs = retourValidator.findByObjDatesManager(cal, echantillonDao.findById(1), 0).orElse(null);
      assertTrue(rs.size() == 1);
      assertTrue(rs.get(0).getRetourId().equals(4));

      rs = retourValidator.findByObjDatesManager(cal, echantillonDao.findById(4), null).orElse(null);
      assertTrue(rs.isEmpty());

      cal.setTime(new SimpleDateFormat("dd/MM/yyyy HH:mm").parse("01/01/2010 12:12"));
      rs = retourValidator.findByObjDatesManager(cal, echantillonDao.findById(4), -1).orElse(null);
      assertTrue(rs.isEmpty());

      cal.setTime(new SimpleDateFormat("dd/MM/yyyy").parse("01/01/2010"));
      rs = retourValidator.findByObjDatesManager(cal, echantillonDao.findById(4), 0).orElse(null);
      assertTrue(rs.size() == 1);
      assertTrue(rs.get(0).getRetourId().equals(6));

      rs = retourValidator.findByObjDatesManager(cal, prodDeriveDao.findById(3), null).orElse(null);
      ...
      rs = retourValidator.findByObjDatesManager(null, prodDeriveDao.findById(3), 7).orElse(null);
      assertTrue(rs.isEmpty());
   }
   ...
   public void testFindByObjInsideDates() throws ParseException{
      ...
      List<Retour> rs = retourValidator.findByObjInsideDatesManager(cal1, cal2, echantillonDao.findById(1), 0).orElse(null);
      assertTrue(rs.size() == 1);
      assertTrue(rs.get(0).getRetourId().equals(5));

      cal1.setTime(new SimpleDateFormat("dd/MM/yyyy hh:mm").parse("31/12/2009 12:12"));
      rs = retourValidator.findByObjInsideDatesManager(cal1, cal2, echantillonDao.findById(1), null).orElse(null);
      assertTrue(rs.size() == 5);

      rs = retourValidator.findByObjInsideDatesManager(cal1, cal2, echantillonDao.findById(4), -1).orElse(null);
      assertTrue(rs.size() == 1);
      assertTrue(rs.get(0).getRetourId().equals(6));

      rs = retourValidator.findByObjInsideDatesManager(cal1, cal2, echantillonDao.findById(2), null).orElse(null);
      assertTrue(rs.isEmpty());

      cal1.setTime(new SimpleDateFormat("dd/MM/yyyy").parse("01/01/2010"));

      rs = retourValidator.findByObjInsideDatesManager(cal1, cal2, echantillonDao.findById(4), 0).orElse(null);
      assertTrue(rs.size() == 1);
      assertTrue(rs.get(0).getRetourId().equals(6));

      rs = retourValidator.findByObjInsideDatesManager(cal1, cal2, prodDeriveDao.findById(3), -1).orElse(null);
      assertTrue(rs.size() == 2);
      assertTrue(rs.get(0).getRetourId() > 6);
      ...
      rs = retourValidator.findByObjInsideDatesManager(cal1, null, prodDeriveDao.findById(3), 7).orElse(null);
      assertTrue(rs.isEmpty());

      rs = retourValidator.findByObjInsideDatesManager(null, cal2, prodDeriveDao.findById(3), 3).orElse(null);
      assertTrue(rs.isEmpty());
   }
   ...
   public void testDateSortieCoherence() throws ParseException{
      Errors errs;
      final Retour r = new Retour();
      r.setEntite(entiteDao.findById(3)).orElse(null);
      r.setObjetId(2);
   ...
   public void testFindByObjectDateRetourEmptyManager(){
      ...
      List<Retour> incomps = retourManager.findByObjectDateRetourEmptyManager(objsIds, entiteDao.findById(3)).orElse(null);
      assertTrue(incomps.isEmpty());

      incomps = retourManager.findByObjectDateRetourEmptyManager(null, entiteDao.findById(3)).orElse(null);
      assertTrue(incomps.isEmpty());

      objsIds.add(2);
      incomps = retourManager.findByObjectDateRetourEmptyManager(objsIds, entiteDao.findById(3)).orElse(null);
      assertTrue(incomps.isEmpty());
      ...
      assertTrue(echantillonDao.findById(2).getObjetStatut().getStatut().equals("STOCKE")).orElse(null);
      assertTrue(echantillonDao.findById(1).getObjetStatut().getStatut().equals("STOCKE")).orElse(null);
   ...
   public void testUpdateMultipleObjectManager(){
      ...
      final Incident i1 = incidentDao.findById(1).orElse(null);
      final Cession c1 = cessionDao.findById(1).orElse(null);
      final Transformation t1 = transformationDao.findById(1).orElse(null);
      ...
      List<Retour> retours = retourManager.findByObjectDateRetourEmptyManager(echanIds, entiteDao.findById(3)).orElse(null);
      assertTrue(retours.size() == 2);
      ...
      retours = retourManager.findByObjectDateRetourEmptyManager(pdIds, entiteDao.findById(8)).orElse(null);
      assertTrue(retourManager.getRetoursForObjectManager(prodDeriveDao.findById(1)).size() == 1).orElse(null);
      ...
      final Retour r1 = retourManager.findByObjectDateRetourEmptyManager(echanIds, entiteDao.findById(3)).get(0).orElse(null);
      r1.setDateRetour(dateR);
      Retour r2 = retourManager.getRetoursForObjectManager(echantillonDao.findById(2)).get(0).orElse(null);
      r2.setDateRetour(dateR);
      r2.setTempMoyenne(new Float(0.0));
      final Retour r3 = retourManager.getRetoursForObjectManager(prodDeriveDao.findById(1)).get(0).orElse(null);
   ...
   public void testFindByObjectAndImpactManager(){
      List<Retour> impacts = retourManager.findByObjectAndImpactManager(echantillonDao.findById(1), true).orElse(null);
      assertTrue(impacts.size() == 3);
      impacts = retourManager.findByObjectAndImpactManager(echantillonDao.findById(1), false).orElse(null);
      assertTrue(impacts.size() == 1);
      impacts = retourManager.findByObjectAndImpactManager(echantillonDao.findById(1), null).orElse(null);
      assertTrue(impacts.size() == 0);
      impacts = retourManager.findByObjectAndImpactManager(prodDeriveDao.findById(3), false).orElse(null);
