...
public class CombinaisonManagerTest extends AbstractManagerTest4 {
	...
	public void testFindById() {
		...
		Combinaison combinaison = manager.findByIdManager(1);
		assertNotNull(combinaison);
		assertTrue(combinaison.getCombinaisonId().equals(1));
		
		Combinaison combinaisonNull = manager.findByIdManager(5);
	...
	public void testFindAllObjects() {
		List<Combinaison> list = manager.findAllObjectsManager();
		//On vérifie chaque élément par un findById
		Iterator<Combinaison> it = list.iterator();
		while (it.hasNext()) {
			Combinaison temp = it.next();
			if (temp != null) {
				assertTrue(temp.equals(
						manager.findByIdManager(temp.getCombinaisonId())));
			}
		}
	}
	...
	public void testCopyCombinaison() {
		...
		Combinaison combinaison = manager.findByIdManager(1);
		Combinaison copie = manager.copyCombinaisonManager(combinaison);
		...
		assertTrue(copie.equals(manager.findByIdManager(
				copie.getCombinaisonId())));
	...
	public void testCrud() {
		...
		Combinaison combinaison2 = manager.findByIdManager(combinaison
				.getCombinaisonId());
		//On vérifie l'égalité des deux
		assertTrue(combinaison.equals(combinaison2));
		
		/** Update. */
		String oldOperateur = combinaison.getOperateur();
		String operateur = "+";		
		combinaison.setOperateur(operateur);
		...
		int id = combinaison.getCombinaisonId();
		//On le supprime
